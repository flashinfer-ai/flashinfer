...
// general boundary-merge pipeline on the V2-G global grid).
//
...
// port of the pinned SageAttention fork kernels with zero-amax V protection.
// Element order,
...
// Payload byte-layout version; independent of the statistics/padding policy.
...
#pragma unroll
  for (uint32_t i = 0; i < pack_size; ++i) {
    // Keep the pinned kernel's multiply-by-reciprocal behavior for nonzero amax.
    // A zero-amax channel encodes +0 instead of evaluating 0 * (2.25 / 0).
    ...
    x_val_float[i] =
        amax_val == 0.0f ? 0.0f : detail::convert_to_float(x_val[i]) * __fdividef(2.25f, amax_val);
  }
...
#pragma unroll
  for (uint32_t i = 0; i < pack_size; ++i) {
    const float amax_val =
        detail::convert_to_float(detail::convert_from_float<T>(scale_val[i] * 2.25f));
    // Match the canonical quantizer's zero-amax guard, including signed-zero inputs.
    x_val_float[i] =
        amax_val == 0.0f ? 0.0f : detail::convert_to_float(x_val[i]) * __fdividef(2.25f, amax_val);
  }
...
__host__ __device__ constexpr uint32_t grouped_block_threads() {
  ...
  static_assert(grouped_tokens_per_iter<head_dim, GROUP>() * (head_dim / 8) <= kMaxThreadsPerBlock,
...
#pragma unroll
    for (uint32_t j = 0; j < 2; ++j) {
      quantized[j] =
          make_char4(detail::float_to_int8_rn(x_val_float[j * 4 + 0] * reciprocal_scale),
...
__global__ void UnpackForSageUnalignedKernel(
  ...
  static_assert((head_dim == 64 || head_dim == 128) && CTA_SIZE == 64);
