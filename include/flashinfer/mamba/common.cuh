...
__device__ __forceinline__ uint16_t cvt_rs_f16_f32(float x, uint32_t rand13) {
  ...
  return static_cast<uint16_t>(packed & 0xFFFFu);  // low half = our x
#else
  return cvt_rs_f16_sw(x, rand13);
#endif
}
...
__device__ __forceinline__ uint32_t cvt_rs_f16x2_f32(float a, float b, uint32_t rbits) {
  ...
  uint16_t a_fp16 = cvt_rs_f16_f32(a, rand_a);
  uint16_t b_fp16 = cvt_rs_f16_f32(b, rand_b);
