/*
 * SM120 SVDQuant fused GEMM: LoRA/epilogue-overlap epilogue collective.
 *
 * This is a copy of the pinned CUTLASS Sm90TmaWarpSpecialized CollectiveEpilogue
 * (3rdparty/cutlass/include/cutlass/epilogue/collective/sm90_epilogue_tma_warpspecialized.hpp)
 * with ONE addition: store() takes a LoRA subtile prologue hook that is invoked with the
 * accumulator MMA-block range of each epilogue subtile BEFORE that subtile's fragments are
 * visited, so the rank-32 bf16 LoRA product for subtile i+1 executes while subtile i's TMA
 * store is in flight. The copy exists because the upstream class keeps its type machinery
 * private (deriving cannot reuse it) and the 3rdparty tree is frozen; everything except the
 * hook is verbatim, including the builder-chosen DelayTmaStore schedule (a measured
 * iteration that forced the non-delayed schedule regressed every shape - the delayed
 * schedule's interleave is worth more than any fold-placement subtlety, and the fold
 * before visit() still overlaps stores issued in earlier iterations). It is only
 * instantiated by the SVDQ_SM120_LORA_EPI_OVERLAP build.
 */
...
// The class body below is a verbatim copy written against the upstream namespace, so it
// is defined inside cutlass::epilogue::collective (as a NEW class name - no collision with
// the upstream template) and aliased into the flashinfer namespace at the end of the file.
namespace cutlass {
namespace epilogue {
...
// Inactive default: store() without a hook runs the verbatim upstream schedule.
// The accumulator is passed INTO the hook by store(): store() receives the accumulator
// fragment BY VALUE (an owning rmem tensor), so a hook bound to the caller's fragment
// would mutate dead registers - it must fold into store()'s own copy.
struct Sm120LoRaNoopHook {
  static constexpr bool kActive = false;
  template <class FrgTensorC>
  CUTLASS_DEVICE void operator()(FrgTensorC&, int, int, int, int) const {}
};

// Active hook: forwards each epilogue subtile's accumulator MMA-block range (and
// store()'s accumulator fragment) to the mainloop collective's per-subtile LoRA fold
// out of the register-resident D/L1 fragments staged by mma_lora_begin.
template <class CollectiveMainloop, class LoRaFrags>
struct Sm120LoRaSubtileHook {
  static constexpr bool kActive = true;
  CollectiveMainloop& mainloop;
  LoRaFrags const& frags;

  template <class FrgTensorC>
  CUTLASS_DEVICE void operator()(FrgTensorC& accum, int mma_m_begin, int mma_m_count,
                                 int mma_n_begin, int mma_n_count) const {
    mainloop.mma_lora_subtile(frags, accum, mma_m_begin, mma_m_count, mma_n_begin, mma_n_count);
  }
};

template <class CollectiveMainloop, class LoRaFrags>
CUTLASS_DEVICE Sm120LoRaSubtileHook<CollectiveMainloop, LoRaFrags> make_sm120_lora_subtile_hook(
    CollectiveMainloop& mainloop, LoRaFrags const& frags) {
  return {mainloop, frags};
}

// Detection trait: whether a CollectiveEpilogue type is the hook-enabled copy (the
// kernel only compiles the hooked store path for those configs).
template <class Epilogue>
inline constexpr bool is_sm120_lora_hooked_epilogue_v = false;
...
class Sm120LoRaOverlapEpilogue {
 public:
  //
  // Type Aliases
  //
  ...
  using ThreadEpilogueOp =
      typename epilogue::fusion::FusionCallbacksTraits<FusionCallbacks>::Operation;
 ...
 private:
  ...
  using TmaElementD = cute::conditional_t<cute::is_same_v<NonVoidElementD, cutlass::complex<float>>,
                                          uint64_t, NonVoidElementD>;
  using TmaElementC = cute::conditional_t<cute::is_same_v<NonVoidElementC, cutlass::complex<float>>,
                                          uint64_t, NonVoidElementC>;
 ...
 public:
  ...
  static constexpr Params to_underlying_arguments(ProblemShape const& problem_shape,
    ...
    if constexpr (is_source_supported) {
      Tensor tensor_c = make_tensor(make_gmem_ptr<TmaElementC const>(args.ptr_C),
                                    make_layout(make_shape(M, N, L), args.dC));
      tma_load_c =
          make_tma_copy_C_sm90(CopyOpG2S{}, tensor_c, take<0, 2>(SmemLayoutC{}), EpilogueTile{});
    }
    ...
    if constexpr (is_destination_supported) {
      ...
      tma_store_d =
          make_tma_copy_C_sm90(CopyOpS2G{}, tensor_d, take<0, 2>(SmemLayoutD{}), EpilogueTile{});
    }

    return {FusionCallbacks::to_underlying_arguments(problem_shape, args.thread, workspace),
            tma_load_c, tma_store_d, transaction_bytes};
  }
  ...
  static bool can_implement(ProblemShape const& problem_shape,
    ...
    if constexpr (is_destination_supported) {
      ...
      if constexpr (cute::is_same_v<CopyOpS2G,
                                    SM90_TMA_STORE_IM2COL>) {  // ignore L stride for implicit gemm
        implementable = cutlass::detail::check_alignment<min_tma_aligned_elements_D>(
            take<0, 2>(shape), take<0, 2>(StrideD{}));
      } else {
        implementable =
    ...
    if constexpr (not cute::is_void_v<ElementC>) {
      ...
      if constexpr (cute::is_same_v<CopyOpG2S,
                                    SM90_TMA_LOAD_IM2COL>) {  // ignore L stride for implicit gemm
        implementable =
            implementable && cutlass::detail::check_alignment<min_tma_aligned_elements_C>(
                                 take<0, 2>(shape), take<0, 2>(StrideC{}));
      } else {
        implementable =
            implementable &&
  ...
  CUTLASS_HOST_DEVICE
  Sm120LoRaOverlapEpilogue(Params const& params_, TensorStorage& shared_tensors)
  ...
  CUTLASS_DEVICE auto load(LoadPipeline load_pipeline, LoadPipelineState load_pipe_producer_state,
    ...
    auto coord_shape = conditional_return<is_im2col_C>(make_coord(m_coord, n_coord),
  ...
  template <class ProblemShapeMNKL, class TileShapeMNK, class TileCoordMNKL, class AccEngine,
            class AccLayout, class TiledMma, class LoraHook = Sm120LoRaNoopHook>
  CUTLASS_DEVICE auto store(LoadPipeline load_pipeline, LoadPipelineState load_pipe_consumer_state,
    ...
    using ElementCompute_ =
        typename epilogue::fusion::FusionCallbacksTraits<FusionCallbacks>::ElementCompute;
    ...
    // should be local tiled with only (m_coord, n_coord).
    auto coord_shape = conditional_return<is_im2col_D>(make_coord(m_coord, n_coord),
                                                       make_coord(m_coord, n_coord, l_coord));
    ...
    Tensor tRS_cD_mn = [&]() CUTLASS_LAMBDA_FUNC_INLINE {
      if constexpr (IsUseR2R) {
        ...
        TiledCopy tiled_cst =
            make_tiled_copy_S(Copy_Atom<CopyOpR2S, SmemElementC>{}, tiled_copy_C_atom);
    ...
    for (int epi_n = 0; epi_n < size<3>(gD_epi); ++epi_n) {
      ...
      for (int epi_m = 0; epi_m < size<2>(gD_epi); ++epi_m) {
        ...
        if constexpr (epi_tile_m * epi_tile_n > mma_tile_m * mma_tile_n) {
          ...
          // LoRA subtile prologue: fold the rank-32 D@L1^T product into exactly this
          // subtile's accumulator MMA blocks before their fragments are visited. Each
          // accumulator block belongs to exactly one epilogue subtile, so the product
          // is applied exactly once per output element. The fold targets THIS function's
          // accumulator copy (tRS_rAcc views the same registers).
          lora_hook(accumulators, epi_m * MmaMPerEpiM, MmaMPerEpiM, epi_n * MmaNPerEpiN,
                    MmaNPerEpiN);
          ...
          for (int mma_n_in_epi = 0; mma_n_in_epi < MmaNPerEpiN; ++mma_n_in_epi) {
            ...
            for (int mma_m_in_epi = 0; mma_m_in_epi < MmaMPerEpiM; ++mma_m_in_epi) {
              ...
              tRS_rCompute_frg(idx_in_epi_subtile) =
                  cst_callbacks.visit(tRS_rAcc_frg_mn(0), idx_in_epi_subtile, epi_m, epi_n);
            }
          }
        } else {
          static_assert(!LoraHook::kActive,
                        "the LoRA subtile prologue requires EpilogueTile >= MMA tile in both"
                        " modes (one accumulator MMA block per subtile); this configuration"
                        " splits one MMA block across subtiles");
          int mma_m = epi_m;
        ...
        cst_callbacks.reduce(sD_epi(_, _, store_pipe_producer_state.index()), synchronize, epi_m,
                             epi_n, is_last_iteration, tRS_rCompute_frg);
...
template <int StagesC_, int StagesD_, int FragmentSize_, bool ReuseSmemC_, bool DelayTmaStore_,
          class CtaTileMNK_, class EpilogueTile_, class ElementC_, class StrideC_, class ElementD_,
          class StrideD_, class FusionCallbacks_, class CopyOpG2S_, class SmemLayoutAtomC_,
          class CopyOpS2R_, class CopyOpS2G_, class SmemLayoutAtomD_, class CopyOpR2S_,
          class CopyAtomC_, class CopyOpR2R_>
inline constexpr bool is_sm120_lora_hooked_epilogue_v<Sm120LoRaOverlapEpilogue<
    StagesC_, StagesD_, FragmentSize_, ReuseSmemC_, DelayTmaStore_, CtaTileMNK_, EpilogueTile_,
    ElementC_, StrideC_, ElementD_, StrideD_, FusionCallbacks_, CopyOpG2S_, SmemLayoutAtomC_,
    CopyOpS2R_, CopyOpS2G_, SmemLayoutAtomD_, CopyOpR2S_, CopyAtomC_, CopyOpR2R_>> = true;

// Rebind the CollectiveBuilder's epilogue to this hook-enabled copy. The builder's
// DelayTmaStore choice is PRESERVED: forcing it off was measured to regress every
// shape (the delayed-store interleave is worth more than the fold overlap).
template <class BuilderEpilogue>
struct MakeSm120LoRaOverlapEpilogue;
...
template <int StagesC_, int StagesD_, int FragmentSize_, bool ReuseSmemC_, bool DelayTmaStore_,
          ...
          class CopyOpS2R_, class CopyOpS2G_, class SmemLayoutAtomD_, class CopyOpR2S_,
          class CopyAtomC_, class CopyOpR2R_>
struct MakeSm120LoRaOverlapEpilogue<cutlass::epilogue::collective::CollectiveEpilogue<
    cutlass::epilogue::Sm90TmaWarpSpecialized<StagesC_, StagesD_, FragmentSize_, ReuseSmemC_,
                                              DelayTmaStore_>,
    CtaTileMNK_, EpilogueTile_, ElementC_, StrideC_, ElementD_, StrideD_, FusionCallbacks_,
    CopyOpG2S_, SmemLayoutAtomC_, CopyOpS2R_, CopyOpS2G_, SmemLayoutAtomD_, CopyOpR2S_, CopyAtomC_,
    CopyOpR2R_>> {
  // Keep the builder's DelayTmaStore choice: the first measured iteration forced the
  // non-delayed schedule and regressed EVERY shape by ~3ns/subtile-iteration (up to
  // -35us on 14-wave sentinels) - the delayed schedule's interleave is worth more than
  // any fold placement subtlety. The fold before visit() still overlaps stores issued
  // in earlier iterations.
  using Type =
      Sm120LoRaOverlapEpilogue<StagesC_, StagesD_, FragmentSize_, ReuseSmemC_, DelayTmaStore_,
                               CtaTileMNK_, EpilogueTile_, ElementC_, StrideC_, ElementD_, StrideD_,
                               FusionCallbacks_, CopyOpG2S_, SmemLayoutAtomC_, CopyOpS2R_,
                               CopyOpS2G_, SmemLayoutAtomD_, CopyOpR2S_, CopyAtomC_, CopyOpR2R_>;
};
...
using cutlass::epilogue::collective::is_sm120_lora_hooked_epilogue_v;
using cutlass::epilogue::collective::make_sm120_lora_subtile_hook;
using cutlass::epilogue::collective::MakeSm120LoRaOverlapEpilogue;
using cutlass::epilogue::collective::Sm120LoRaNoopHook;
using cutlass::epilogue::collective::Sm120LoRaOverlapEpilogue;
using cutlass::epilogue::collective::Sm120LoRaSubtileHook;
