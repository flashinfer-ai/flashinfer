...
CUTE_HOST_DEVICE constexpr auto SelectSmemCopyAtomB() {
  if      constexpr (kPerWarpN >= 16) return Copy_Atom<SM75_U32x4_LDSM_N, Element>{};
  else if constexpr (kPerWarpN >=  8) return Copy_Atom<SM75_U32x2_LDSM_N, Element>{};
  else                                 return Copy_Atom<SM75_U32x1_LDSM_N, Element>{};
}

template <int kTileN>
CUTE_HOST_DEVICE constexpr auto SelectSmemCopyAtomR2S() {
  if      constexpr (kTileN >= 32) return SM90_U16x8_STSM_T{};
  else if constexpr (kTileN >= 16) return SM90_U16x4_STSM_T{};
  else                              return SM90_U16x2_STSM_T{};
}
...
CUTE_HOST_DEVICE constexpr auto SelectSmemLayoutAtomK() {
  if      constexpr (kTileK == 128) return GMMA::Layout_K_SW128_Atom<Element>{};
  else if constexpr (kTileK == 64)  return GMMA::Layout_K_SW64_Atom<Element>{};
  else                              return GMMA::Layout_K_SW32_Atom<Element>{};
}

} // namespace utils

template <int TileM_, int TileN_, int TileK_, int Stages_, int GranK_,
          typename ElementSFLoad_, typename ElementSFCompute_,
          GemmType kGemmType_ = GemmType::Normal,
          bool kSwapAB_ = false>
struct Sm120BlockScaledSFConfig {
  ...
  static constexpr int SF_Stages = AB_Stages > kNumTileKPerPackSF ? AB_Stages / kNumTileKPerPackSF : 1;
  ...
  CUTE_HOST_DEVICE
  static auto
  get_tma_aligned_size(const int& x) {
  ...
  thrfrg_SFA(SFATensor&& sfatensor, TiledMMA<Atom, TiledThr, TiledPerm>& mma)
  {
    ...
    auto atom_tile = make_tile(make_layout(size<0>(AtomShape_MNK{})),
                            make_layout(_1{}));
  ...
  thrfrg_SFB(SFBTensor&& sfbtensor, TiledMMA<Atom, TiledThr, TiledPerm>& mma)
  {
    ...
    using AtomShape_MNK  = typename Atom::Shape_MNK;
    auto atom_tile = make_tile(make_layout(size<1>(AtomShape_MNK{})),
                            make_layout(_1{}));
  ...
  transform_fragment_for_qmma(Tensor&& tensor)
  {
    ...
    auto new_layout = make_layout(
        make_shape (Int<SFVecSize>{}, num_mn,
                    make_shape(atoms_per_sf_per_tilek, num_sf_per_tilek),
                    make_shape(num_tilek_per_sf,       num_tilek_groups)),
        make_stride(_0{},             Int<PACK_NSF>{},
                    make_stride(_0{},                  _1{}),
  ...
  static constexpr int kSfaStageElems = (int(cute::cosize_v<SmemLayoutAtomSFA>) < kSfStageStrideElems)
                                         ? kSfStageStrideElems : int(cute::cosize_v<SmemLayoutAtomSFA>);
  static constexpr int kSfbStageElems = (int(cute::cosize_v<SmemLayoutAtomSFB>) < kSfStageStrideElems)
                                         ? kSfStageStrideElems : int(cute::cosize_v<SmemLayoutAtomSFB>);

  using SmemLayoutSFA = Layout<
      Shape<decltype(shape<0>(ScaleTileShape{})), decltype(shape<2>(ScaleTileShape{})), Int<SF_Stages>>,
      Stride<_1, _0, Int<kSfaStageElems>>>;

  using SmemLayoutSFB = Layout<
      Shape<decltype(shape<1>(ScaleTileShape{})), decltype(shape<2>(ScaleTileShape{})), Int<SF_Stages>>,
...
struct Sm120BlockScaledTmaStoreConfig {
  ...
  using SmemLayoutD = decltype(tile_to_shape(
    SmemLayoutAtomD{},
    make_shape(Int<kEpiTileM>{}, Int<kEpiTileN>{}, Int<StagesD>{}),
    Step<_1,_2,_3>{}));
...
struct Sm120BlockScaledR2GStoreConfig {
  ...
  using SmemAtomLayoutO = decltype(
    composition(Swizzle<3,3,3>{},
                Layout<Shape <_8,Shape <_8, _8>>,
                Stride<_8,Stride<_1,_64>>>{}));
  using SmemLayoutO = decltype(tile_to_shape(SmemAtomLayoutO{},
                                              Shape<Int<kTileM>, Int<kSmemON>>{}));
...
template <int TileM_=32, int TileN_=128, int TileK_=128, int Stages_=4, int GranK_=128,
          GemmType GemmType_=GemmType::MGroupedContiguousWithPsumLayout,
          bool SwapAB_=false>
struct SM120BlockScaledBuilder {
  ...
  static constexpr bool kPerBatchAB = (GemmType_ == GemmType::Batched ||
                                        GemmType_ == GemmType::MGroupedMasked);
  static constexpr bool kUseTmaStore =
      utils::EnableTmaStore<kFlat, kSwapAB, TileN_, kPerBatchAB>();
  ...
  using ABLoadConfig   = Sm120BlockScaledABLoadConfig<kTileM, kTileN, kTileK, AB_Stages,
                                                      ElementA, ElementB>;
  using TmaStoreConfig = std::conditional_t<kSwapAB,
      Sm120BlockScaledSwapABTmaStoreConfig<kTileM, kTileN, ElementD, kUseTmaStore>,
      Sm120BlockScaledTmaStoreConfig<kTileM, kTileN, ElementD, kUseTmaStore>>;
  using R2GStoreConfig = std::conditional_t<kSwapAB,
      Sm120BlockScaledSwapABR2GStoreConfig<kTileM, kTileN, ElementD>,
