...
struct SM120BlockScaledGemmKernel {
  ...
  using Scheduler = std::conditional_t<KT::kSwapAB,
  ...
  to_underlying_arguments(ProblemShape const& problem_shape, Arguments const& args)
  {
    ...
    if constexpr (KT::kUseTmaStore) {
      ...
      tma_store_d = make_tma_copy_C_sm90(
          typename KT::TmaStoreConfig::CopyOpS2G{},
          tensor_d,
          take<0,2>(typename KT::TmaStoreConfig::SmemLayoutD{}),
          typename KT::TmaStoreConfig::EpilogueTile_MN{});
    }

    return {tma_load_a, tma_load_b, tma_load_sfa, tma_load_sfb,
            tma_store_d,
            args.ptr_D,
            M, N, K, num_experts, args.grouped_layout};
  }

  static dim3 get_grid_shape(int num_sms) {
    return dim3(num_sms, 1, 1);
  }
  ...
  using TensorStorage = std::conditional_t<
      KT::kUseTmaStore,
      typename KT::TensorStorageSplit,
  ...
  static auto get_mbarriers(SharedStorage& shared_storage){
    ...
    return cute::make_tuple(ab_full_mbar, ab_empty_mbar, sf_full_mbar, sf_empty_mbar, store_full_mbar, store_empty_mbar);
  }
  ...
  {
    auto [tAgSFA, tBgSFB] = utils::tma_sf_partition<KT>(
        params.tma_load_sfa, params.tma_load_sfb,
    ...
    auto sSFA_ = make_tensor(make_smem_ptr(shared_storage.tensors.load.smem_SFA.begin()), typename KT::SFConfig::SmemLayoutSFA{});
    auto sSFB_ = make_tensor(make_smem_ptr(shared_storage.tensors.load.smem_SFB.begin()), typename KT::SFConfig::SmemLayoutSFB{});
    ...
    auto [ab_full_mbar, ab_empty_mbar, sf_full_mbar, sf_empty_mbar, store_full_mbar, store_empty_mbar] = get_mbarriers(shared_storage);
    ...
    for (int32_t sf_cycle = 0; sf_cycle < num_sf_cycles; ++sf_cycle)
    {
      cute::for_each(cute::make_int_sequence<KT::SFConfig::SF_Stages>{}, [&](auto sf_stage) {
        ...
        cute::copy(tma_copy_sfa, tAgSFA(_,_,_, sf_cycle * KT::SFConfig::SF_Stages + sf_stage), tAsSFA(_,_,_, sf_stage));
        auto tma_copy_sfb = params.tma_load_sfb.with(*recast_ptr<ProducerBarrierType>(&sf_full_barrier));
        cute::copy(tma_copy_sfb, tBgSFB(_,_,_, sf_cycle * KT::SFConfig::SF_Stages + sf_stage), tBsSFB(_,_,_, sf_stage));
  ...
  {
    auto [tAgA, tBgB] = utils::tma_ab_partition<KT>(
    ...
    auto sA_ = make_tensor(make_smem_ptr(shared_storage.tensors.load.smem_A.begin()), typename KT::ABLoadConfig::SmemLayoutA{});
    auto sB_ = make_tensor(make_smem_ptr(shared_storage.tensors.load.smem_B.begin()), typename KT::ABLoadConfig::SmemLayoutB{});
    ...
    int32_t k_tile_count = num_sf_cycles * KT::SFConfig::SF_Stages * KT::SFConfig::kNumTileKPerPackSF;
  ...
  static void mma (
    Params const& params, SharedStorage& shared_storage,
    int32_t num_sf_cycles, int32_t m_offset, int32_t m_boundary,
    int32_t m_block_idx, int32_t n_block_idx, int32_t expert_idx,
    uint32_t &sf_phase, uint32_t &ab_phase,
    int &epi_stage, uint32_t *se_phase)
  {
    ...
    for (int32_t sf_cycle = 0; sf_cycle < num_sf_cycles - 1; ++sf_cycle)
    {
      cute::for_each(cute::make_int_sequence<KT::SFConfig::SF_Stages>{}, [&](auto sf_stage) {
        ...
        cute::for_each(cute::make_int_sequence<KT::SFConfig::kNumTileKPerPackSF>{}, [&](auto k_in_sf) {
          ...
          cute::gemm(mma,
                     make_zip_tensor(tCrA, tCrSFA_frg(_, _, _, k_in_sf)),
                     make_zip_tensor(tCrB, tCrSFB_frg(_, _, _, k_in_sf)),
                     accum);
        });
      });
      sf_phase ^= 1;
    ...
    if constexpr (KT::kUseTmaStore) {
      utils::epi_r2s<KT>(params, shared_storage, accum, thread_idx,
                          epi_stage, se_phase,
                          store_full_mbar, store_empty_mbar);
    } else if constexpr (!KT::kFlat && KT::kSwapAB) {
      utils::epi_pred_stg<KT>(params, accum, thread_idx,
                                m_offset, m_boundary, m_block_idx, n_block_idx,
                                store_empty_mbar);
    } else {
      utils::epi_pred_r2g<KT>(params, shared_storage, accum, thread_idx,
                                m_offset, m_boundary, m_block_idx, n_block_idx,
                                expert_idx, store_empty_mbar);
    }
  }
    ...
    uint32_t *sf_phase, int &epi_stage)
  {
    if constexpr (KT::kUseTmaStore) {
      ...
      utils::tma_store<KT>(params, shared_storage,
  ...
  void operator()(Params const& params, char* smem_buf) {
    ...
    int32_t num_sf_cycles = math::ceil_div(params.K, int(KT::SFConfig::PACK_NK * KT::SFConfig::SF_Stages));
    ...
    if (warp_idx >= KT::MMAConfig::kNumMathWarps)
    {
        ...
        if constexpr (KT::kUseTmaStore) {
          ...
          if (lane_predicate)
          {
            ...
            while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
              auto blk_coord = utils::make_blk_coord<KT::kSwapAB>(m_block_idx, n_block_idx, scheduler.get_expert_idx(m_block_idx));
      ...
      if (warp_idx == ab_warp_idx)
      {
        ...
        if (lane_predicate)
        {
          ...
          while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            auto blk_coord = utils::make_blk_coord<KT::kSwapAB>(m_block_idx, n_block_idx, scheduler.get_expert_idx(m_block_idx));
            load_ab(params, shared_storage,
                    blk_coord, scheduler.get_m_offset(),
                    num_sf_cycles, ab_phase, store_phase);
          }
        }
        __syncwarp();
      }
      if (warp_idx == sf_warp_idx)
      {
        ...
        if (lane_predicate)
        {
          ...
          while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            auto blk_coord = utils::make_blk_coord<KT::kSwapAB>(m_block_idx, n_block_idx, scheduler.get_expert_idx(m_block_idx));
            load_sf(params, shared_storage,
                    blk_coord, scheduler.get_m_offset(),
                    num_sf_cycles, sf_phase, store_phase);
          }
        }
        __syncwarp();
      }
    ...
    else
    {
      ...
      while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
        auto blk_coord = utils::make_blk_coord<KT::kSwapAB>(m_block_idx, n_block_idx, scheduler.get_expert_idx(m_block_idx));
        mma(params, shared_storage, num_sf_cycles, scheduler.get_m_offset(), scheduler.get_m_boundary(),
            cute::get<0>(blk_coord), cute::get<1>(blk_coord), cute::get<2>(blk_coord),
