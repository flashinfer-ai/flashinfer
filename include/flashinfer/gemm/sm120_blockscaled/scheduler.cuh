...
constexpr bool is_flat_gemm(GemmType t) {
  return t == GemmType::Normal || t == GemmType::Batched ||
         t == GemmType::MGroupedMasked;
}
...
struct Scheduler {
    static_assert(kGemmType == GemmType::Normal ||
    ...
    __device__ __forceinline__ explicit
    Scheduler(int shape_m_, int shape_n_, int num_groups_, int32_t const* grouped_layout_ = nullptr)
        : shape_m(shape_m_), shape_n(shape_n_),
          num_groups(num_groups_), num_n_blocks((shape_n_ + BlockN - 1) / BlockN),
    ...
    get_swizzled_block_idx_local(int32_t block_idx, int32_t num_m_blocks_local,
                                  int32_t& m_block_idx, int32_t& n_block_idx) const
    {
    ...
    get_next_block(int32_t& m_block_idx, int32_t& n_block_idx)
    {
    ...
    __device__ __forceinline__ int32_t get_m_offset() const {
        if constexpr (kGemmType == GemmType::Normal || kGemmType == GemmType::Batched ||
                      kGemmType == GemmType::MGroupedContiguous) return 0;
        else return prev_psum_m;
    }
    __device__ __forceinline__ int32_t get_m_boundary() const {
        if constexpr (kGemmType == GemmType::Normal || kGemmType == GemmType::Batched ||
                      kGemmType == GemmType::MGroupedContiguous) return shape_m;
        else return current_psum_m;
    }
