...
cudaError_t launch_decode(const AttentionParams& p, const ExecutionPlan& plan, cudaStream_t stream) {
  ...
  if constexpr (MT == ModelType::DSV4_1) {
    if (plan.implementation == Implementation::MixedCache) {
      kernel = sparse_mla_decode_dsv4_kernel<MT, NUM_HEADS, PAGE_BLOCK_SIZE, Dsv41MixedCacheDecodeSchedule>;
  ...
  kernel<<<dim3(p.num_tokens, h_blocks, p.allocated_splits), dim3(block_threads), smem_bytes, stream>>>(
      p.q, p.kv, p.indices, p.mid_out, p.mid_lse, p.topk_length, p.extra_kv, p.extra_indices,
      p.extra_topk_length, p.extra_topk, p.extra_page_size, p.extra_page_stride_bytes, p.num_tokens,
      q_heads, p.topk, p.allocated_splits, cpb, p.sm_scale, p.page_stride_bytes, p.indices_stride_elems,
  ...
  merge<<<dim3(p.num_tokens, q_heads), dim3(MERGE_THREADS), size_t(p.allocated_splits) * sizeof(float),
          stream>>>(p.mid_out, p.mid_lse, p.output, p.out_lse, p.attn_sink, p.num_tokens,
                    p.allocated_splits, q_heads, h_blocks * HPB, p.out_lse_stride_elems);
