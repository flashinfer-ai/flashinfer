...
__device__ __forceinline__ void run_bf16_tiles(
    Dsv41Bf16Smem& sm, const uint8_t* __restrict__ KV_cache, const int32_t* __restrict__ indices,
    const uint8_t* __restrict__ extra_KV_cache, const int32_t* __restrict__ extra_indices, int t,
    int lo, int hi, int len, int elen, int main_chunks, int main_page_block_size, int pbs_extra,
    size_t page_stride_bytes, size_t extra_page_stride_bytes, size_t indices_stride_elems,
    size_t extra_indices_stride_elems, float sm_scale,
    float (&acc)[Dsv41Bf16Resources::PV_TILES_PER_WARP][4], float (&m)[2], float (&sum)[2]) {
  ...
  for (int chunk = lo; chunk < hi; ++chunk) {
    ...
    auto gather = [&](bool power_of_two) __attribute__((always_inline)) {
      static_assert((R::CANDIDATES * R::QK_VECTORS) % (2 * R::BLOCK_THREADS) == 0);
      for (int first = tidx; first < R::CANDIDATES * R::QK_VECTORS; first += 2 * R::BLOCK_THREADS) {
        uint64_t raw[2];
        ...
        for (int item = 0; item < 2; ++item) {
          ...
          if (valid[item]) {
            ...
            constexpr int fp4_scale_shift = []() constexpr {
              int shift = 0;
              for (int group = Dsv41Fp4Layout::SCALE_GROUP; group > 1; group >>= 1) ++shift;
              return shift;
            }();
            constexpr int fp8_scale_shift = []() constexpr {
              int shift = 0;
              for (int group = Dsv41Fp8Layout::QUANT_TILE; group > 1; group >>= 1) ++shift;
              return shift;
            }();
            static_assert((1u << fp4_scale_shift) == Dsv41Fp4Layout::SCALE_GROUP);
            static_assert((1u << fp8_scale_shift) == Dsv41Fp8Layout::QUANT_TILE);
            scale_addr[item] =
                base + scale_offset + (unsigned(d) >> (fp4 ? fp4_scale_shift : fp8_scale_shift));
          }
        }
        asm volatile(
            "{\n"
            ...
            "}\n"
            : "=l"(raw[0]), "=l"(raw[1]), "=r"(scale_code[0]), "=r"(scale_code[1])
            : "l"(data_addr[0]), "l"(scale_addr[0]), "l"(data_addr[1]), "l"(scale_addr[1]),
              "r"(int(valid[0])), "r"(int(valid[1])), "r"(int(fp4))
            : "memory");
#pragma unroll
        for (int item = 0; item < 2; ++item) {
          const int task = first + item * R::BLOCK_THREADS;
          const int row = task / R::QK_VECTORS, d = task % R::QK_VECTORS * R::VECTOR_ELEMS;
          const __nv_bfloat162 scale = valid[item]
...
#pragma unroll
          for (int j = 0; j < 4; ++j)
            pairs[j] = __hmul2(
                decode_e2m1_or_e4m3_pair_bf16(uint32_t(raw[item] >> (j * (fp4 ? 8 : 16))), fp4),
                scale);
...
__global__ void __launch_bounds__(Dsv41Bf16Resources::BLOCK_THREADS, 1)
  ...
  const int t = blockIdx.x / groups, hs = blockIdx.x % groups * R::HEADS_PER_CTA;
  if (t >= params.num_tokens) return;
  const int vh = min(R::HEADS_PER_CTA, heads - hs);
  const int warp = threadIdx.x / 32, lane = threadIdx.x % 32, gid = lane / 4, tid = lane % 4;
  const int len =
      max(0, min(params.topk, params.topk_length ? params.topk_length[t] : params.topk));
  const int elen =
      params.extra_kv
          ? max(0, min(params.extra_topk,
                       params.extra_topk_length ? params.extra_topk_length[t] : params.extra_topk))
          : 0;
  ...
  for (int i = threadIdx.x; i < R::HEADS_PER_CTA * R::QK_VECTORS; i += R::BLOCK_THREADS) {
    const int h = i / R::QK_VECTORS, d = i % R::QK_VECTORS * R::VECTOR_ELEMS;
    uint4 data = h < vh ? *reinterpret_cast<const uint4*>(
                              params.q + ((size_t)t * heads + hs + h) * R::Geometry::D_QK + d)
                        : make_uint4(0, 0, 0, 0);
    *reinterpret_cast<uint4*>(&sm.q[h][d]) = data;
  }
  float acc[R::PV_TILES_PER_WARP][4] = {};
  float m[2] = {-1e30f, -1e30f}, sum[2] = {0.f, 0.f};
  run_bf16_tiles<EXTRA_FP4>(sm, params.kv, params.indices, params.extra_kv, params.extra_indices, t,
