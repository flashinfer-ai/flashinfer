...
inline auto PrefillSplitQOKVIndptr(IdType* qo_indptr_h, IdType* kv_indptr_h,
  ...
  // A single query row per request is a graph invariant supplied by tensor-core decode. With
  // split-KV disabled, every q tile produces exactly one scheduler entry, so new_batch_size is
  // graph-stable and fully initialized. Keep multi-token prefill scheduling unchanged.
  const bool exact_no_split_graph = enable_cuda_graph && disable_split_kv && uniform_q_len == 1;
  size_t padded_batch_size;
