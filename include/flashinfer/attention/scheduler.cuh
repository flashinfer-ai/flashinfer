...
inline auto PrefillSplitQOKVIndptr(IdType* qo_start_ptr_h, uint32_t* qo_len_ptr_h,
                                   IdType* kv_indptr_h, uint32_t* kv_len_ptr_h,
...
inline cudaError_t PrefillPlan(void* float_buffer, size_t float_workspace_size_in_bytes,
  ...
  std::vector<uint32_t> qo_len_ptr, kv_len_ptr;
  for (uint32_t i = 0; i < batch_size; ++i) {
    qo_len_ptr.push_back(qo_indptr_h[i + 1] - qo_indptr_h[i]);
    kv_len_ptr.push_back(kv_indptr_h[i + 1] - kv_indptr_h[i]);
  }
  auto [split_kv, new_batch_size, padded_batch_size, cta_tile_q, kv_chunk_size, request_indices_vec,
      ...
      PrefillSplitQOKVIndptr(qo_indptr_h, qo_len_ptr.data(), kv_indptr_h, kv_len_ptr.data(),
                             total_num_rows, batch_size, num_qo_heads, num_kv_heads, head_dim_vo,
                             page_size, max_batch_size_if_split, enable_cuda_graph);
