...
__global__ void MergeStateKernel(DTypeIn* __restrict__ v_a, float* __restrict__ s_a,
  ...
  // A finite shift avoids (-inf) - (-inf) when both states are empty.
  float s_shift = (s_max == -math::inf) ? 0.f : s_max;
  s_a_val = math::ptx_exp2(s_a_val - s_shift);
  s_b_val = math::ptx_exp2(s_b_val - s_shift);
...
__global__ void MergeStateInPlaceKernel(DType* __restrict__ v, float* __restrict__ s,
  ...
  // A finite shift avoids (-inf) - (-inf) when both states are empty.
  float s_shift = (s_max == -math::inf) ? 0.f : s_max;
  s_val = math::ptx_exp2(s_val - s_shift);
  s_other_val = math::ptx_exp2(s_other_val - s_shift);
