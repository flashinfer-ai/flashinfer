...
__global__
__launch_bounds__(NUM_WARPS_Q * NUM_WARPS_KV * WARP_SIZE) void PODWithKVCacheNoTensorKernel(
...
template <PosEncodingMode POS_ENCODING_MODE, MaskMode MASK_MODE_P, MaskMode MASK_MODE_D, 
          // Prefill template
          uint32_t NUM_MMA_Q_P, uint32_t NUM_MMA_D_QK_P, uint32_t NUM_MMA_D_VO_P, 
          uint32_t NUM_MMA_KV_P, uint32_t NUM_WARPS_Q_P, uint32_t NUM_WARPS_KV_P, 
          typename DTypeQKAccum_P, typename PrefillAttentionVariant, bool PartitionKV_P,
          // Decode template
          uint32_t NUM_MMA_Q_D, uint32_t NUM_MMA_D_QK_D, uint32_t NUM_MMA_D_VO_D, 
          uint32_t NUM_MMA_KV_D, uint32_t NUM_WARPS_Q_D, uint32_t NUM_WARPS_KV_D, 
          typename DTypeQKAccum_D, typename DecodeAttentionVariant,
          typename PrefillParams, typename DecodeParams>
__global__
__launch_bounds__(std::max(NUM_WARPS_Q_P * NUM_WARPS_KV_P, NUM_WARPS_Q_D * NUM_WARPS_KV_D) * WARP_SIZE)
void PODWithKVCacheTensorKernel(const uint_fastdiv group_size, 
  ...
  const uint32_t num_kv_heads_p = prefill_params.num_kv_heads;
  const uint32_t num_chunks = prefill_params.partition_kv;
  constexpr uint32_t num_rows_per_cta = NUM_MMA_Q_P * NUM_WARPS_Q_P * 16;
  const uint32_t qo_len = prefill_params.qo_len;
  const uint32_t xsize = ceil_div(qo_len * group_size, num_rows_per_cta);
  ...
  if(op == PREFILL) {
    const uint32_t linear_tid = threadIdx.x;
    // Return if threadId exceeds number of threads for this op
    if(linear_tid >= 32 * NUM_WARPS_Q_P * NUM_WARPS_KV_P)
      return;

    const dim3 tid = dim3(linear_tid % 32, (linear_tid / 32) % NUM_WARPS_Q_P, (linear_tid / 32) / NUM_WARPS_Q_P);
    //dim3 nblks(ceil_div(qo_len * group_size, num_rows_per_cta), 1, num_kv_heads);
    //dim3 nblks(ceil_div(qo_len * group_size, num_rows_per_cta), num_chunks, num_kv_heads);
    // BlockID exceeds limit
    if(linear_bid >= prefill_blocks) return;
    ...
    if constexpr(!PartitionKV_P) {
      ...
      SinglePrefillWithKVCacheDevice<MASK_MODE_P, POS_ENCODING_MODE, NUM_MMA_Q_P, NUM_MMA_D_QK_P,
        NUM_MMA_D_VO_P, NUM_MMA_KV_P, NUM_WARPS_Q_P, NUM_WARPS_KV_P, DTypeQKAccum_P, PrefillAttentionVariant>
        (group_size, prefill_params, smem, tid, bx, chunk_idx, kv_head_idx, 1, num_kv_heads_p);
    } else {
      const uint32_t bx = linear_bid % xsize;
      const uint32_t chunk_idx = (linear_bid / xsize) % num_chunks;
    ...
    const uint32_t linear_tid = threadIdx.x;
    // Return if threadId exceeds number of threads for this op
    if(linear_tid >= 32 * NUM_WARPS_Q_D * NUM_WARPS_KV_D)
      return;

    const dim3 tid = dim3(linear_tid % 32, (linear_tid / 32) % NUM_WARPS_Q_D, (linear_tid / 32) / NUM_WARPS_Q_D);

    BatchPrefillWithPagedKVCacheDevice<MASK_MODE_D, POS_ENCODING_MODE, 
      NUM_MMA_Q_D, NUM_MMA_D_QK_D, NUM_MMA_D_VO_D, NUM_MMA_KV_D, NUM_WARPS_Q_D, 
      NUM_WARPS_KV_D, DTypeQKAccum_D, DecodeAttentionVariant>
      (group_size, decode_params, smem, tid, bx, kv_head_idx, num_kv_heads_d); 
  }
}
...
cudaError_t PODWithKVCacheTensorDispatched(PrefillParams prefill_params, 
  ...
  //DISPATCH_CTA_TILE_Q(cta_tile_q_p, CTA_TILE_Q_P, {
    ...
    //DISPATCH_NUM_MMA_KV(min(max_num_mma_kv_smem_p, max_num_mma_kv_reg_p), NUM_MMA_KV_P, {
      ...
      if constexpr (is_invalid_configuration<POS_ENCODING_MODE, DTypeKV_P, DTypeQKAccum_P>(
                        NUM_MMA_Q_P, NUM_MMA_D_QK, NUM_MMA_D_VO, NUM_MMA_KV_P, NUM_WARPS_Q_P, 
                        NUM_WARPS_KV_P)) {
        // Invalid configuration, skip
        std::ostringstream err_msg;
        ...
        //DISPATCH_NUM_MMA_KV(min(max_num_mma_kv_smem_d, max_num_mma_kv_reg_d), NUM_MMA_KV_D, {
          ...
          if constexpr (is_invalid_configuration<POS_ENCODING_MODE, DTypeKV_D, DTypeQKAccum_D>(
                            NUM_MMA_Q_D, NUM_MMA_D_QK, NUM_MMA_D_VO, NUM_MMA_KV_D, NUM_WARPS_Q_D,
                            NUM_WARPS_KV_D)) {
            // Invalid configuration, skip
            std::ostringstream err_msg;
            ...
            // TODO(Zihao): fix the following computation
            size_t smem_size_p =
                max(SmemSizeThreadBlockAttnSync<NUM_WARPS_Q_P, NUM_WARPS_KV_P, NUM_MMA_Q_P, HEAD_DIM_VO,
                  DTypeO_P>(),
                    NUM_MMA_Q_P * NUM_WARPS_Q_P * 16 * HEAD_DIM_QK * sizeof(DTypeQ_P) +
                        NUM_MMA_KV_P * NUM_WARPS_KV_P * 16 * (HEAD_DIM_QK + HEAD_DIM_VO) * sizeof(DTypeKV_P));
            
            size_t smem_size_d = max(
              SmemSizeThreadBlockAttnSync<NUM_WARPS_Q_D, NUM_WARPS_KV_D, NUM_MMA_Q_D, HEAD_DIM_VO, DTypeO_D>(),
              NUM_MMA_Q_D * NUM_WARPS_Q_D * 16 * HEAD_DIM_QK * sizeof(DTypeQ_D) +
                  NUM_MMA_KV_D * NUM_WARPS_KV_D * 16 * (HEAD_DIM_QK + HEAD_DIM_VO) * sizeof(DTypeKV_D));

            auto kernel =
                PODWithKVCacheTensorKernel<POS_ENCODING_MODE, MASK_MODE_P, MASK_MODE_D,
                          // Prefill template
                          NUM_MMA_Q_P, NUM_MMA_D_QK, NUM_MMA_D_VO, 
                          NUM_MMA_KV_P, NUM_WARPS_Q_P, NUM_WARPS_KV_P, 
                          DTypeQKAccum_P, PrefillAttentionVariant, true,
                          // Decode template
                          NUM_MMA_Q_D, NUM_MMA_D_QK, NUM_MMA_D_VO, 
                          NUM_MMA_KV_D, NUM_WARPS_Q_D, NUM_WARPS_KV_D, 
                          DTypeQKAccum_D, DecodeAttentionVariant,
                          PrefillParams, DecodeParams>;
            // Prefill: decide num_splits for split-kv
            int num_blocks_per_sm = 0;
            int num_sm = 0;
            ...
            if (num_chunks <= 1 || tmp_p == nullptr) {
              ...
              kernel =
                  PODWithKVCacheTensorKernel<POS_ENCODING_MODE, MASK_MODE_P, MASK_MODE_D,
                        ...
                        NUM_MMA_Q_P, NUM_MMA_D_QK, NUM_MMA_D_VO, 
                        NUM_MMA_KV_P, NUM_WARPS_Q_P, NUM_WARPS_KV_P, 
                        DTypeQKAccum_P, PrefillAttentionVariant, false,
                        // Decode template
                        NUM_MMA_Q_D, NUM_MMA_D_QK, NUM_MMA_D_VO, 
                        NUM_MMA_KV_D, NUM_WARPS_Q_D, NUM_WARPS_KV_D, 
                        DTypeQKAccum_D, DecodeAttentionVariant,
