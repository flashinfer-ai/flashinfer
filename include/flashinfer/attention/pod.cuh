...
template <MaskMode MASK_MODE, PosEncodingMode POS_ENCODING_MODE, uint32_t NUM_MMA_Q,
          ...
          typename DTypeQKAccum, typename AttentionVariant, typename PrefillParams>
__global__
__launch_bounds__(NUM_WARPS_Q* NUM_WARPS_KV* WARP_SIZE) void PODWithKVCacheKernel(
    ...
    int linear_blk_id = blockIdx.x * blockIdx.y * blockIdx.z;
    ...
    if(op == PREFILL) {
      ...
      if (!prefill_params.partition_kv) {
        ...
        SinglePrefillWithKVCacheDevice<MASK_MODE, POS_ENCODING_MODE, NUM_MMA_Q, NUM_MMA_D, 
          NUM_MMA_KV, NUM_WARPS_Q, NUM_WARPS_KV, DTypeQKAccum, AttentionVariant, PrefillParams>
...
template <uint32_t HEAD_DIM, PosEncodingMode POS_ENCODING_MODE, bool USE_FP16_QK_REDUCTION,
          MaskMode MASK_MODE, typename AttentionVariant, typename PrefillParams>
  ...
  DISPATCH_CTA_TILE_Q(cta_tile_q, CTA_TILE_Q, {
    ...
    DISPATCH_NUM_MMA_KV(min(max_num_mma_kv_smem, max_num_mma_kv_reg), NUM_MMA_KV, {
      if constexpr (is_invalid_configuration<POS_ENCODING_MODE, DTypeKV, DTypeQKAccum>(
        ...
        constexpr uint32_t num_rows_per_cta = NUM_MMA_Q * NUM_WARPS_Q * 16;
        auto kernel =
            PODWithKVCacheKernel<MASK_MODE, POS_ENCODING_MODE, NUM_MMA_Q, NUM_MMA_D,
                                           NUM_MMA_KV, NUM_WARPS_Q, NUM_WARPS_KV, DTypeQKAccum,
                                           AttentionVariant, PrefillParams>;
        ...
        if (num_chunks <= 1 || prefill_tmp == nullptr) {
          ...
          //dim3 nblks(ceil_div(qo_len * group_size, num_rows_per_cta), 1, num_kv_heads);
          dim3 nblks(ceil_div(qo_len * group_size, num_rows_per_cta) * num_kv_heads, 1, 1);
          dim3 nthrs(32, NUM_WARPS_Q, NUM_WARPS_KV);
          FLASHINFER_CUDA_CALL(
          ...
          float* tmp_lse = (float*)(prefill_tmp + num_chunks * qo_len * num_qo_heads * HEAD_DIM);
          ...
          if constexpr (AttentionVariant::use_softmax) {
            ...
            FLASHINFER_CUDA_CALL(
                AttentionSum(prefill_tmp, o, num_chunks, qo_len, num_qo_heads, HEAD_DIM, stream));
          }
        }
      }
    })
