...
template <uint32_t num_frags_x, uint32_t num_frags_y, typename DTypeIn, typename IdType>
...
__global__ void SinglePrefillWithKVCacheKernel(DTypeIn* __restrict__ q, DTypeIn* __restrict__ k,
  ...
  const uint32_t qo_packed_idx_base = bx * num_warps_x * num_frags_x * 16;
...
#pragma unroll
      for (uint32_t j = 0; j < 2; ++j) {
        const uint32_t qo_head_idx =
            ...
            (qo_packed_idx_base + get_warp_idx_x<num_warps_x, num_warps_z>() * num_frags_x * 16 +
...
__global__ void BatchPrefillWithRaggedKVCacheKernel(
  ...
  const uint32_t qo_packed_idx_base = tile_idx * num_warps_x * num_frags_x * 16;
  const uint32_t kv_n_stride = qkv_info.get_kv_n_stride(), qo_n_stride = qkv_info.get_qo_n_stride(),
                 qo_h_stride = qkv_info.get_qo_h_stride();
  smem_t qo_smem(smem);
  ...
  if constexpr (pos_encoding_mode == PosEncodingMode::kRoPELlama) {
    if (get_warp_idx_z<num_warps_x, num_warps_z>() == 0) {
      if (!q_offset) {
        ...
        q_smem_inplace_apply_rotary_with_pos_multiply_sm_scale<num_frags_x, num_frags_y, DTypeIn>(
...
#pragma unroll 1
  for (uint32_t iter = 0; iter < num_iterations; ++iter) {
    ...
    if constexpr (mask_mode == MaskMode::kCustom) {
      mask_s<partition_kv, mask_mode, num_frags_x, num_frags_y, num_frags_z>(
          ...
          iter * 16 * num_warps_z * num_frags_z +
              get_warp_idx_z<num_warps_x, num_warps_z>() * num_frags_z * 16,
          qo_len, kv_len, kv_len, group_size, custom_mask + qk_indptr[request_idx], s_frag);
    } else {
      if (iter >= mask_iteration) {
        mask_s<partition_kv, mask_mode, num_frags_x, num_frags_y, num_frags_z>(
            qo_packed_idx_base + get_warp_idx_x<num_warps_x, num_warps_z>() * num_frags_x * 16,
  ...
  // write back
  write_o_reg_gmem<num_warps_x, num_warps_z, num_frags_x, num_frags_y>(
      o_frag, &qo_smem, o_ptr_base, qo_packed_idx_base, qo_len, qo_n_stride, qo_h_stride,
      group_size);
...
#pragma unroll
      for (uint32_t j = 0; j < 2; ++j) {
        const uint32_t qo_head_idx =
            kv_head_idx * group_size +
            (qo_packed_idx_base + get_warp_idx_x<num_warps_x, num_warps_z>() * num_warps_x * 16 +
             lane_idx / 4 + j * 8 + fx * 16) %
                group_size;
        alibi_slopes[fx][j] = get_alibi_slope(qo_head_idx, num_qo_heads) * math::log2e;
      }
...
#pragma unroll
  for (uint32_t iter = 0; iter < num_iterations; ++iter) {
    ...
    page_produce_kv<false, page_size, num_warps_x, num_warps_z, num_frags_y, num_frags_z>(
        ...
        page_iter_base, kv_len, last_indptr);
    cp_async::commit_group();
    cp_async::wait_group<1>();
    block.sync();
