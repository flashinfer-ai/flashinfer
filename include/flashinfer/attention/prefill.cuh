...
template <SharedMemFillMode fill_mode, uint32_t NUM_WARPS_Q, uint32_t NUM_WARPS_KV,
          uint32_t NUM_MMA_D_VO, uint32_t NUM_MMA_KV, SwizzleMode swizzle_mode, typename T>
...
#pragma unroll
      for (uint32_t j = 0; j < NUM_MMA_D_VO / (8 / sizeof(T)); ++j) {
      ...
      *smem_offset =
          ...
          sizeof(T) * NUM_MMA_D_VO;
      *gptr += num_warps * 4 * kv_stride_n - sizeof(T) * NUM_MMA_D_VO * num_elems_per_128b<T>();
    }
...
__device__ __forceinline__ void q_smem_inplace_apply_rotary(
  ...
  if (get_warp_idx_kv<NUM_WARPS_Q, NUM_WARPS_KV>() == 0) {
    ...
    static_assert(NUM_MMA_D_VO % 4 == 0, "NUM_MMA_D must be a multiple of 4");
#pragma unroll
    for (uint32_t mma_q = 0; mma_q < NUM_MMA_Q; ++mma_q) {
      uint32_t q_smem_offset_r_first_half = *q_smem_offset_r;
#pragma unroll
      for (uint32_t mma_di = 0; mma_di < NUM_MMA_D_VO / 2; ++mma_di) {
        q_smem->ldmatrix_m8n8x4(q_smem_offset_r_first_half, q_frag_local[0]);
        uint32_t q_smem_offset_r_last_half =
            q_smem->template advance_offset_by_column<NUM_MMA_D_VO>(q_smem_offset_r_first_half, 0);
...
template <uint32_t NUM_WARPS_Q, uint32_t NUM_WARPS_KV, uint32_t NUM_MMA_D_VO, uint32_t NUM_MMA_KV,
...
__device__ __forceinline__ void k_smem_inplace_apply_rotary(const uint32_t kv_idx_base,
  ...
  const uint32_t lane_idx = threadIdx.x;
  if constexpr (NUM_MMA_D_VO == 4 && NUM_WARPS_Q == 4) {
    ...
    static_assert(NUM_MMA_KV % 2 == 0,
                  "when NUM_MMA_D_VO == 4, NUM_MMA_KV must be a multiple of 2");
...
#pragma unroll
    ...
    *k_smem_offset_r = (*k_smem_offset_r ^ (0x2 * (warp_idx % 2))) -
    ...
    static_assert(NUM_MMA_D_VO % (2 * NUM_WARPS_Q) == 0);
    // horizontal axis: y
    // vertical axis: z
...
#pragma unroll
    for (uint32_t i = 0; i < NUM_MMA_KV; ++i) {
      uint32_t k_smem_offset_r_first_half = *k_smem_offset_r;
#pragma unroll
      for (uint32_t j = 0; j < NUM_MMA_D_VO / (2 * NUM_WARPS_Q); ++j) {
        uint32_t mma_di = warp_idx_x + j * NUM_WARPS_Q;
        k_smem->ldmatrix_m8n8x4(k_smem_offset_r_first_half, k_frag_local[0]);
        uint32_t k_smem_offset_r_last_half =
            k_smem->template advance_offset_by_column<NUM_MMA_D_VO>(k_smem_offset_r_first_half, 0);
        k_smem->ldmatrix_m8n8x4(k_smem_offset_r_last_half, k_frag_local[1]);
        k_frag_apply_llama_rope<DTypeKV>((DTypeKV*)k_frag_local[0], (DTypeKV*)k_frag_local[1],
                                         rope_freq[mma_di], kv_idx);
        k_smem->stmatrix_m8n8x4(k_smem_offset_r_last_half, k_frag_local[1]);
...
template <uint32_t NUM_MMA_Q, uint32_t NUM_MMA_D_VO, uint32_t NUM_MMA_KV, typename AttentionVariant,
...
template <MaskMode MASK_MODE, uint32_t NUM_MMA_Q, uint32_t NUM_MMA_D_VO, uint32_t NUM_MMA_KV,
...
__launch_bounds__(NUM_WARPS_Q* NUM_WARPS_KV* WARP_SIZE) void SinglePrefillWithKVCacheKernel(
  ...
  if constexpr (std::is_same_v<DTypeQ, nv_bfloat16>) {
    ...
    const tensor_info_t qkv_info(qo_len, kv_len, num_qo_heads, num_kv_heads, q_stride_n, q_stride_h,
                                 kv_stride_n, kv_stride_h, /*head_dim=*/NUM_MMA_D_VO * 16);
    ...
    load_q_global_smem<NUM_WARPS_Q, NUM_WARPS_KV, NUM_MMA_Q, NUM_MMA_D_VO>(
    ...
    produce_kv<SharedMemFillMode::kNoFill, NUM_WARPS_Q, NUM_WARPS_KV, NUM_MMA_D_VO, NUM_MMA_KV>(
...
#pragma unroll 1
    for (uint32_t iter = 0; iter < num_iterations; ++iter) {
      ...
      compute_qk<NUM_MMA_Q, NUM_MMA_D_VO, NUM_MMA_KV, swizzle_mode_q, swizzle_mode_kv, DTypeQ,
                 DTypeKV>(&qo_smem, &q_smem_offset_r, &k_smem, &k_smem_offset_r, s_frag);

      logits_transform<NUM_MMA_Q, NUM_MMA_D_VO, NUM_MMA_KV>(
      ...
      if (MASK_MODE == MaskMode::kCustom || (iter >= mask_iteration || iter < window_iteration)) {
        logits_mask<MASK_MODE, NUM_MMA_Q, NUM_MMA_D_VO, NUM_MMA_KV>(
            params, variant, /*batch_idx=*/0, qo_packed_idx_base,
            chunk_start + (iter * NUM_WARPS_KV + get_warp_idx_kv<NUM_WARPS_Q, NUM_WARPS_KV>()) *
                              NUM_MMA_KV * 16,
...
#pragma unroll
    ...
    page_produce_kv<false, NUM_WARPS_Q, NUM_WARPS_KV, NUM_MMA_D_VO, NUM_MMA_KV>(
