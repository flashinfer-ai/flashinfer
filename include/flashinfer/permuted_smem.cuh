...
template <typename T>
constexpr __host__ __device__ __forceinline__ uint32_t bank_capacity() {
...
__host__ __device__ __forceinline__ bank_t *get_smem_ptr(T *smem_base, uint32_t i, uint32_t j) {
...
template <uint32_t stride, typename T>
__device__ __forceinline__ void load_bank(T *smem_base, uint32_t i, uint32_t j, const T *gptr) {
  *get_smem_ptr<stride>(smem_base, i, j) = *reinterpret_cast<const bank_t *>(gptr);
}

template <uint32_t stride, typename T>
__device__ __forceinline__ void store_bank(T *smem_base, uint32_t i, uint32_t j, T *gptr) {
  *reinterpret_cast<bank_t *>(gptr) = *get_smem_ptr<stride>(smem_base, i, j);
}

template <uint32_t stride, typename T>
__device__ __forceinline__ void load_bank_async(T *smem_base, uint32_t i, uint32_t j, const T *gptr,
                                                bool predicate) {
  bank_t *smem_ptr = get_smem_ptr<stride>(smem_base, i, j);
...
template <uint32_t stride, typename T>
__device__ __forceinline__ void load_bank_async(T *smem_base, uint32_t i, uint32_t j,
                                                const T *gptr) {
  bank_t *smem_ptr = get_smem_ptr<stride>(smem_base, i, j);
...
struct permuted_smem_t {
  ...
  __device__ __forceinline__ bank_t *get_ptr(uint32_t i, uint32_t j) {
  ...
  __device__ __forceinline__ void load_bank(uint32_t i, uint32_t j, const T *gptr) {
    permuted_smem_impl::load_bank<stride>(base, i, j, gptr);
  }
  __device__ __forceinline__ void load_bank_async(uint32_t i, uint32_t j, const T *gptr,
                                                  bool predicate) {
  ...
  __device__ __forceinline__ void store_bank(uint32_t i, uint32_t j, T *gptr) {
