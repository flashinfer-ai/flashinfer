...
using HisT = std::conditional_t<
    IsDeterministic,
    std::conditional_t<std::is_same_v<T, float>, uint64_t, uint32_t>,
    float>;
...
template <typename T>
__host__ __device__ void setBufPointers(T const* in, T* buf1, T* buf2,
    int pass, T const*& inBuf, T*& outBuf) {
    if (pass == 0) { inBuf = in; outBuf = nullptr; }
    else if (pass == 1) { inBuf = in; outBuf = buf1; }
    else if (pass % 2 == 0) { inBuf = buf1; outBuf = buf2; }
    else { inBuf = buf2; outBuf = buf1; }
}
...
__device__ void vectorizedProcess(
    size_t threadRank, size_t numThreads, T const* in, IdxT len, Func f) {
...
__device__ __forceinline__ void filterAndHistogram(
    T const* inBuf, T* outBuf, int previousLen, Counter<T>* counter,
    HisT_* histogram, IdxT* countHistogram,
    HisT_* histogramSmem, IdxT* countHistogramSmem, int pass) {
    for (IdxT i = threadIdx.x; i < NUM_BUCKETS; i += blockDim.x) {
        histogramSmem[i] = 0; countHistogramSmem[i] = 0;
    }
    __syncthreads();
    ...
    if (pass == 0) {
        ...
        auto f = [outBuf, startBit, mask, previousStartBit, kthValueBits,
                  pFilterCnt, histogramSmem, countHistogramSmem](T value, IdxT) {
            auto const previousBits = (twiddleIn(value, false) >> previousStartBit) << previousStartBit;
            if (previousBits == kthValueBits) {
                if (outBuf) { IdxT pos = atomicAdd(pFilterCnt, static_cast<IdxT>(1)); outBuf[pos] = value; }
                int bucket = calcBucket<T>(value, startBit, mask);
                histAtomicAdd<IsDeterministic, T>(histogramSmem + bucket, value);
                atomicAdd(countHistogramSmem + bucket, static_cast<IdxT>(1));
            }
...
template <bool IsDeterministic, typename T>
__global__ void AirTopPRenormRadixKernel(
    Counter<T>* counters, HisT<IsDeterministic, T>* histograms,
    ...
    AccT currentSum; IdxT previousLen, currentLen;
    if (pass == 0) { currentSum = 0; previousLen = counter->len; currentLen = counter->len; }
    else { currentSum = counter->sum; currentLen = counter->len; previousLen = counter->previousLen; }
    if (currentLen == 0) return;
    IdxT const bufLen = calcBufLen<T>(counter->oriLen);
    T const* inBuf = nullptr; T* outBuf = nullptr;
    setBufPointers(counter->in, buf1 + bufLen * batchId, buf2 + bufLen * batchId, pass, inBuf, outBuf);
    if (pass == 0 || pass == 1 || previousLen > bufLen) { inBuf = counter->in; previousLen = counter->oriLen; }
    if (pass == 0 || currentLen > bufLen) { outBuf = nullptr; }
    ...
    filterAndHistogram<IsDeterministic, T, HisT_>(
        inBuf, outBuf, previousLen, counter, histogram, countHistogram, histogramSmem, countHistogramSmem, pass);
    __syncthreads(); __threadfence();
    bool isLastBlock = false;
    if (threadIdx.x == 0) { uint32_t finished = atomicInc(&counter->finishedBlockCnt, gridDim.x - 1); isLastBlock = (finished == (gridDim.x - 1)); }
    if (__syncthreads_or(isLastBlock)) {
        AccT* histValueSmem = reinterpret_cast<AccT*>(histogramSmem);
        if constexpr (IsDeterministic) {
            for (int i = threadIdx.x; i < NUM_BUCKETS; i += blockDim.x) {
                uint64_t value = static_cast<uint64_t>(histogram[i]); IdxT count = countHistogram[i];
                if (count != 0) {
                    uint32_t sb = calcStartBit<T>(pass); uint32_t bv = counter->kthValueBits;
                    if (pass == 0) bv = i << sb;
                    histValueSmem[i] = calcFloatValue(static_cast<uint32_t>(count), calcExponent(twiddleOut<T>(bv, false)), value);
                } else { histValueSmem[i] = 0.0f; }
            }
            __syncthreads();
        }
        constexpr int WARP_SIZE = 32; constexpr int WARP_COUNT = NUM_BUCKETS / WARP_SIZE;
        ...
        if constexpr (BITS_PER_PASS != 11) { for (int i = threadIdx.x; i < NUM_BUCKETS; i += BLOCK_SIZE) warpSum[i] = 0; __syncthreads(); }
        ...
        if (threadIdx.x < WARP_SIZE) {
            ...
            if constexpr (BITS_PER_PASS == 11) reduce_update_async(warp, blockSum, warpSum[threadIdx.x + WARP_SIZE], cg::plus<float>{});
        }
        __syncthreads();
        if (pass == 0) currentSum = blockSum * counter->p;
        if (threadIdx.x == 0) {
            AccT prev = 0; int targetStep = 0;
            for (int i = 0; i < WARP_COUNT; i++) { if (warpSum[i]) { targetStep = i; if ((prev + warpSum[i]) >= currentSum) break; prev += warpSum[i]; } }
            int targetIdx = 0;
            for (int i = targetStep * WARP_SIZE; i < NUM_BUCKETS; i++) { if (countHistogram[i]) { targetIdx = i; if ((prev + histPtr[i]) >= currentSum) break; prev += histPtr[i]; } }
            counter->sum = currentSum - prev; counter->len = countHistogram[targetIdx];
            typename cub::Traits<T>::UnsignedBits bucket = targetIdx;
            counter->kthValueBits |= bucket << calcStartBit<T>(pass);
        }
        __syncthreads();
        if (pass != NUM_PASSES<T> - 1) { for (int i = threadIdx.x; i < NUM_BUCKETS; i += blockDim.x) { histogram[i] = 0; countHistogram[i] = 0; } }
        if (threadIdx.x == 0) { counter->previousLen = currentLen; counter->filterCnt = 0; }
    }
}
...
__global__ void AirTopPRenormInitKernel(
    Counter<T>* counters, int len, T const* in, float topP,
    HisT_* histograms, IdxT* countHistograms) {
    auto const batchIdx = blockIdx.x;
    Counter<T>* counter = counters + batchIdx;
    if (threadIdx.x == 0) {
        counter->in = in + batchIdx * len; counter->len = len; counter->oriLen = len;
        counter->previousLen = len; counter->p = topP; counter->sum = 0;
        counter->kthValueBits = 0; counter->finishedBlockCnt = 0; counter->filterCnt = 0;
    }
    HisT_* hist = histograms + batchIdx * NUM_BUCKETS;
    IdxT* cntHist = countHistograms + batchIdx * NUM_BUCKETS;
    for (int i = threadIdx.x; i < NUM_BUCKETS; i += blockDim.x) { hist[i] = 0; cntHist[i] = 0; }
}

// ======================== Renorm apply kernel ========================

template <typename T>
__global__ void AirTopPRenormApplyKernel(
    T const* probs, T* renormedProbs, Counter<T>* counters, int vocabSize) {
    int const batchId = blockIdx.x; int const tid = threadIdx.x;
...
uint32_t CalcAirTopPBlockNum(int batchSize, int len, int smCnt) {
    ...
    activeBlocks *= smCnt;
    IdxT bestNumBlocks = 0; float bestTailWavePenalty = 1.0f;
    ...
    for (int numWaves = 1;; ++numWaves) {
        ...
        if (tailWavePenalty < 0.15f) { bestNumBlocks = numBlocks; break; }
        else if (tailWavePenalty < bestTailWavePenalty) { bestNumBlocks = numBlocks; bestTailWavePenalty = tailWavePenalty; }
...
cudaError_t AirTopPRenormProb(DType* probs, DType* renormed_prob, float* top_p_arr,
    ...
    AirTopPRenormInitKernel<DType, HisT_><<<batch_size, 256, 0, stream>>>(
        counters, d, probs, top_p_val, histograms, countHistograms);

    dim3 grid(blockNum, batch_size);
    constexpr int numPasses = NUM_PASSES<DType>;
    for (int pass = 0; pass < numPasses; ++pass) {
        AirTopPRenormRadixKernel<IsDeterministic, DType><<<grid, BLOCK_SIZE, 0, stream>>>(
            counters, histograms, countHistograms, pass, buf1, buf2);
    }

    AirTopPRenormApplyKernel<DType><<<batch_size, 1024, 0, stream>>>(
