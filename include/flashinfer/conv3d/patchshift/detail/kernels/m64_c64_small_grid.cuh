...
// Native M64/P16 small-grid path with C64 B2/A6 macro stages.
// Included after output_tail.cuh and small_grid.cuh.

struct M64P16C64B2A6SharedStorage {
  K64C64B2A3K32ABStage b_stage[kK64C64B2A3K32ABRing];
  // Two A slots per filter row let the TMA warp publish half-task t+1 while
  // the MMA warps are still consuming t.  With a single slot the producer
  // must wait after every row, leaving the TMA pipeline empty.
  M64P16ARowStage a_stage[2][3];
  uint32_t tmem_base;
...
static_assert(sizeof(M64P16C64B2A6SharedStorage) == 227328);
static_assert(sizeof(M64P16C64B2A6SharedStorage) <= 232448,
              "M64/P16 C64 macro pipeline must fit one SM100 CTA");

__device__ __forceinline__ void issue_m64_p16_c64_workset_row(
...
template <bool ExactFull = false>
__global__ void general_m64n256_k64_p16_b2a6_c64_kernel(
  ...
  __shared__ M64P16C64B2A6SharedStorage shared;
...
#pragma unroll
      for (int row = 0; row < 3; ++row) {
        patchshift::mbarrier_init(&shared.a_stage[slot][row].ready, 1);
      }
    }
  }
  __syncthreads();
...
#pragma unroll
      for (int row = 0; row < 3; ++row) {
        ...
        patchshift::mbarrier_arrive_expect_tx(
            &shared.a_stage[a_slot][row].ready, a_row_bytes);
        int weight_task =
            (m_tile * full_k32_supergroups + full_sg) * 3 + row;
        patchshift::tma_load_5d(
            weight_k32_map, &shared.a_stage[a_slot][row].ready,
            shared.a_stage[a_slot][row].a[0][0],
            0, 0, 0, 0, weight_task);
        __threadfence_block();
...
#pragma unroll
        for (int row = 0; row < 3; ++row) {
          ...
          while (!patchshift::mbarrier_try_wait(
              &shared.a_stage[a_slot][row].ready, a_seq & 1)) {
          }
          patchshift::fence_view_async_shared();
          issue_m64_p16_c64_workset_row(
              shared.b_stage[slot], shared.a_stage[a_slot][row], half, row,
        ...
        if (full_tile) {
          size_t pixel =
              ((size_t(flat_batch) * size_t(h_size) + size_t(out_p)) *
                   size_t(w_size) + size_t(q_base));
          Element* out =
              output + pixel * size_t(output_pitch_k) + k_base;
#pragma unroll
          for (int q = 0; q < kOutQ; ++q) {
            uint16_t bits = patchshift::element_bits_from_float(
                __uint_as_float(values[q]));
