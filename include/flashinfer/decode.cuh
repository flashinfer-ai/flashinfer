  ...
  if (qkv_layout == QKVLayout::kNHD) {
    ...
    SWITCH_HEAD_DIM(
        head_dim, HEAD_DIM, {SWITCH_ROTARY_MODE(rotary_mode, ROTARY_MODE, {
          ...
          if (seq_len <= 128) {
            // no need to use cooperative kernel
            auto kernel = HND::SingleDecodeWithKVCacheKernel<false, ROTARY_MODE, HEAD_DIM, vec_size,
            ...
            // use cooperative kernel
            auto kernel = HND::SingleDecodeWithKVCacheKernel<true, ROTARY_MODE, HEAD_DIM, vec_size,
                                                             bdx, bdy, DTypeIn, DTypeOut>;
            int num_blocks_per_sm = 0;
            int num_sm = 0;
            CUDA_CALL(cudaDeviceGetAttribute(&num_sm, cudaDevAttrMultiProcessorCount, dev_id));
