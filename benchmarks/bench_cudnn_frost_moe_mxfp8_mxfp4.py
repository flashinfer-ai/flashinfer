...
"""Benchmark frozen Frost MXFP8 × MXFP4 stages and complete routed MoELayer pools.
...
timed alongside those pools. For example::
    ...
    python benchmarks/bench_cudnn_frost_moe_mxfp8_mxfp4.py benchmark \
...
Example (select an idle GPU via CUDA_VISIBLE_DEVICES):
    python benchmarks/bench_cudnn_frost_moe_mxfp8_mxfp4.py \
        --activation all --stage both --tokens 128,1024 \
        --experts 8 --hidden 1024 --intermediate 512 --top-k 2 \
        --output /tmp/frost_mxfp8_mxfp4.jsonl
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--artifacts", type=Path, default=runtime.artifact_root("mxfp8_mxfp4")
    )
    parser.add_argument(
...
def random_operand(shape, *, packed=False):
    """Independent E4M3 activations or packed E2M1 weights with E8M0 scales."""
    if packed:
        data = torch.randint(
            256, (*shape[:-1], shape[-1] // 2), dtype=torch.uint8, device="cuda"
        )
    else:
...
def dequantize(data, scales):
    """Independently decode E4M3/E2M1 and linear E8M0 scales in FP64."""
    if data.dtype == torch.uint8:
        magnitude = torch.tensor(
            [0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0], device=data.device
        )
        nibbles = torch.stack((data & 15, data >> 4), dim=-1).flatten(-2)
        values = magnitude[(nibbles & 7).long()] * torch.where(nibbles < 8, 1, -1)
    else:
        values = data
    return values.double() * scales.contiguous().view(
...
def benchmark_stage(
    ...
    if not candidates:
        raise ValueError(
            f"no matching MXFP8 × MXFP4 artifacts for {stage}/{activation}: S={rows}, N={n}, K={k}, E={args.experts}"
        )
    x, x_sf = random_operand((rows, k))
    operands = [
        random_operand((args.experts, n, k), packed=True)
    ...
    packed_x_sf = mxfp8_mxfp4.pack_token_scales(x_sf, offsets)
    packed_w_sf = tuple(mxfp8_mxfp4.pack_weight_scales(sf) for sf in weight_sf)
    ...
    for kernel in candidates:
        ...
        plan = mxfp8_mxfp4.PreparedMxfp8Mxfp4GroupedGemm(
            kernel,
            x,
            weights,
            offsets,
...
def stage_main(argv=None):
    ...
    check_idle_gpu(gpu_uuid)
    kernels = mxfp8_mxfp4.discover(args.artifacts)
    if args.kernel_id:
...
_FROST_BACKEND = "cudnn_frost_mxfp8_mxfp4"
...
def reference(act, view, activation, rows):
    ...
    for expert in torch.unique(ids).tolist():
        ...
        for stage in (1, 2):
            ...
            weights.append(dequantize(q, sf).float())
        middle = (
...
def discover_original(arch, quant, activation):
    from flashinfer.fused_moe.api import SiTU
    ...
    for cfg_cls, runner_cls in _BACKEND_RUNNERS.items():
        ...
        # The registry spans several architectures. Rubin's CuTe runner
        # rejects W4A8, and its pinned TRTLLM artifact rejects SiTU.
        checks["kernel_activation"] = not (
            arch == 107
            and (
                runner_cls.backend_key == "cute_dsl"
                or (
...
def prepare_problem(args, activation_name, geometry):
    ...
    if not eligible:
        raise RuntimeError("No original MXFP8 × MXFP4 backend applies")
    config = MoEConfig(
    ...
    views = {
        "cutlass_mxfp8_mxfp4": CutlassMxfp8Mxfp4Config.prepare_weights(w1, w2, **kwargs)
    }
...
def benchmark_case(args, config, weights, activation, geometry, routing, tokens, file):
    ...
    xq, xsf = CutlassMxfp8Mxfp4Config.prepare_activations(x, quant=config.quant)
    ...
    for label, layer in layers.items():
        ...
        for runner in layer.runners:
            ...
            if runner.backend_key == "cutlass_mxfp8_mxfp4":
    ...
    if args.reference_tokens:
        ref = reference(
            act,
            weights.get_view("cutlass_mxfp8_mxfp4"),
            activation,
...
def parse_e2e_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Benchmark full routed MXFP8 × MXFP4 MoELayer pools"
    )
    parser.add_argument("--activation", choices=(*ACTIVATIONS, "all"), default="swiglu")
    parser.add_argument("--experts", type=int, default=8)
