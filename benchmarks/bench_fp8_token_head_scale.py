...
comparison of KV-cache configurations (Q/O dtype is fp16 or bf16):
  ...
  * ``fp8_ths``    - FP8 KV cache, per-(token, head) scale in a separate
                     float32 tensor (``kv_cache_sf``; the feature under test).
  * ``kv_eq_qo``   - KV cache dtype == Q/O dtype (FP16 KV if Q/O is FP16, BF16 KV
                     if Q/O is BF16).

Two ratios are reported per row:

  * ``ths/tensor`` = fp8_ths / fp8_tensor -> overhead of the token-head-scale
                     feature vs per-tensor FP8 (~1.0 means no regression).
  * ``ths/qo``     = fp8_ths / kv_eq_qo   -> how the FP8 token-head-scale path
                     stacks up against the matching 16-bit KV cache
                     (<1 means FP8 is faster).

Modes (the 4 public entry points, mirroring the NVFP4 FA2 paths — there is
no standalone single-decode path for either feature):
...
Decode has no CUDA-core token-head-scale kernel: the token-head config routes
to the tensor-core (fa2 prefill) path.
...
Notes on sm75 (e.g. T600):
  * No FP8 tensor cores -> FP8 KV is software-dequantized before the MMA; the
    dequant, not the token-head scale, dominates the FP8 cost.

Timing: uses ``bench_gpu_time`` with CUPTI (pure GPU kernel time, most accurate).
Pass ``--no-cupti`` to force CUDA-event timing.

Usage:
    python benchmarks/bench_fp8_token_head_scale.py
    python benchmarks/bench_fp8_token_head_scale.py --head-dims 128 256 --kv-lens 512 2048
    python benchmarks/bench_fp8_token_head_scale.py --modes single_prefill_ragged batch_decode_paged
"""
...
def quantize(x_ref, dtype, head_dim):
    # Per-(token, head) FP8 quantization. Returns (x_fp8, scale) where scale
    # is float32 with shape x_ref.shape[:-1] (the KV layout minus head_dim).
    fp8_max = 448.0 if dtype == torch.float8_e4m3fn else 57344.0
    # float32 scale (the kernel requirement); division in float32.
...
def main():
    ...
    is_ths = config == "fp8_ths"
    is_fp8 = config in ("fp8_tensor", "fp8_ths")
    num_qo = num_kv_heads if not gqa else 2 * num_kv_heads

    def ragged_kv(n):
        # Apply the KV config to reference tensors. All configs store the same
        # effective KV values so the ratios isolate the token-head-scale
        ...
        if is_ths:
            kk, ks = quantize(k, fp8, head_dim)
            vv, vs = quantize(v, fp8, head_dim)
            return kk, vv, (ks, vs)
        if is_fp8:
            return k.to(fp8), v.to(fp8), None
        return k, v, None
    ...
    def paged_kv():
        ...
        if is_ths:
            kk, ks = quantize(k, fp8, head_dim)
            vv, vs = quantize(v, fp8, head_dim)
            # [pages,2,nk,ps,kv] HND cache + 3-D float32 scales (pages,nk,ps).
            return torch.cat([kk.unsqueeze(1), vv.unsqueeze(1)], dim=1), (ks, vs)
        kv = torch.cat([k.unsqueeze(1), v.unsqueeze(1)], dim=1)
        if is_fp8:
            return kv.to(fp8), None
        return kv, None
    ...
    if mode == "single_prefill_ragged":
        ...
        if is_ths:
            plan_kw["use_token_head_sf"] = True
        w.plan(qo_indptr, kv_indptr, num_qo, num_kv_heads, head_dim, **plan_kw)
        fn = lambda: w.run(q, k, v, kv_cache_sf=sf)
        ...
        fn = lambda: w.run(q, pkv, kv_cache_sf=sf)
        ...
        if is_ths:
            plan_kw["use_token_head_sf"] = True  # plan() forces tensor-core
...
_CONFIGS = ["fp8_tensor", "fp8_ths", "kv_eq_qo"]
...
def main():
    ...
    print(
        ...
        "fp8_ths = per-(token, head) scale (separate float32 tensor); "
    ...
    for head_dim in args.head_dims:
        for kv_len in args.kv_lens:
            ...
            print(
                ...
                f"{'fp8_tensor(µs)':>14} {'fp8_ths(µs)':>12} {'kv_eq_qo(µs)':>12} | "
                f"{'ths/tensor':>11} {'ths/qo':>9}"
            )
            print("-" * 102)
                ...
                for gqa in [False, True]:
                    for q_dtype in q_dtypes:
                        ...
                        print(
                            ...
                            f"{_f_us(times['fp8_ths']):>12} "
                            f"{_f_us(times['kv_eq_qo']):>12} | "
                            f"{_f_ratio(times['fp8_ths'], times['fp8_tensor']):>11} "
                            f"{_f_ratio(times['fp8_ths'], times['kv_eq_qo']):>9}"
                        )
            print()
