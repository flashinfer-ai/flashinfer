...
"""Benchmark frozen Frost NVFP4 stages and complete routed MoELayer pools.
...
preserved. Each original backend is also independently autotuned, checked, and
timed alongside those pools. For example::

    python benchmarks/bench_cudnn_frost_moe_nvfp4.py benchmark \
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--artifacts", type=Path, default=runtime.artifact_root("nvfp4")
    )
    parser.add_argument(
...
def random_operand(shape):
    """Independent packed E2M1 data with finite E4M3 block scales."""
    data = torch.randint(
        0, 256, (*shape[:-1], shape[-1] // 2), dtype=torch.uint8, device="cuda"
    )
    choices = torch.tensor([0.0625, 0.125, 0.25, 0.5], device="cuda")
    indices = torch.randint(4, (*shape[:-1], shape[-1] // 16), device="cuda")
    return data, choices[indices].to(torch.float8_e4m3fn).view(torch.uint8)


def dequantize(data, scales):
    """Decode E2M1 nibbles and E4M3 scales independently in FP64."""
    ...
    return values[codes.long()] * scales.contiguous().view(
        torch.float8_e4m3fn
...
def benchmark_stage(
    ...
    if not candidates:
        raise ValueError(
            f"no matching NVFP4 artifacts for {stage}/{activation}: S={rows}, N={n}, K={k}, E={args.experts}"
        )
    ...
    packed_x_sf = nvfp4.pack_token_scales(x_sf, offsets)
    packed_w_sf = tuple(nvfp4.pack_weight_scales(sf) for sf in weight_sf)
...
def stage_main(argv=None):
    ...
    check_idle_gpu(gpu_uuid)
    kernels = nvfp4.discover(args.artifacts)
    if args.kernel_id:
...
_FROST_BACKEND = "cudnn_frost_nvfp4"
...
def reference(act, view, activation, rows):
    """Independent dequantized math with BF16 stage rounding.

    Intermediate E2M1 requantization is omitted so this also measures
    quantization error independently of the native MoE implementations.
    """
    rows = min(rows, act.hidden_states_q.shape[0])
    x = dequantize(act.hidden_states_q[:rows], act.hidden_states_scale[:rows]).float()
    ...
    for expert in torch.unique(ids).tolist():
        ...
        for stage in (1, 2):
            ...
            n, packed_k = q.shape
            k = 2 * packed_k
            sf = view[f"fc{stage}_weight_block_scale"][expert].view(torch.uint8)
            sf = sf.reshape((n + 127) // 128, (k // 16 + 3) // 4, 32, 4, 4)
            sf = sf.permute(0, 3, 2, 1, 4).reshape(-1, ((k // 16 + 3) // 4) * 4)
            weights.append(dequantize(q, sf[:n, : k // 16]).float())
        middle = (
            activation_reference(
                (x[token] @ weights[0].T) * view["fc1_dequant_scale"][expert],
                activation,
            )
            .bfloat16()
            .float()
        ...
        result[token, slot] = (
            ((middle * view["fc2_act_global_scale"]) @ weights[1].T)
            * view["fc2_dequant_scale"][expert]
        ).bfloat16().float() * scores[token, slot, None]
...
def discover_original(arch, quant, activation):
    ...
    from flashinfer.fused_moe.api import SwiGLU, SiTU
    ...
    for cfg_cls, runner_cls in _BACKEND_RUNNERS.items():
        ...
        # The generic registry also covers Blackwell. Rubin's CuTe kernels
        # implement default SwiGLU only; its pinned TRTLLM BMM rejects SiTU.
        checks["kernel_activation"] = not (
            arch == 107
            and (
                (runner_cls.backend_key == "cute_dsl" and activation != SwiGLU())
                or (
                    runner_cls.backend_key == "trtllm_fp4_routed"
                    and isinstance(activation, SiTU)
                )
            )
        )
        audit.append(
...
def prepare_problem(args, activation_name, geometry):
    ...
    unsupported = set(eligible) - {CutlassNvfp4Config, TrtllmFp4Config, CuteDslConfig}
    if unsupported:
        raise RuntimeError(
    ...
    if not eligible:
        raise RuntimeError("No original NVFP4 backend applies")
    config = MoEConfig(
    ...
    views = {"cutlass_nvfp4": CutlassNvfp4Config.prepare_weights(w1, w2, **kwargs)}
    if TrtllmFp4Config in eligible:
        views["trtllm_fp4_routed"] = TrtllmFp4Config.prepare_weights(
            w1, w2, quant=quant, **kwargs
        )
    if CuteDslConfig in eligible:
        views["cute_dsl"] = CuteDslConfig.prepare_weights(w1, w2, quant=quant, **kwargs)
    del w1, w2
    return (
        config,
        MoEWeightPack(views),
...
def benchmark_case(args, config, weights, activation, geometry, routing, tokens, file):
    ...
    from flashinfer.fused_moe import CutlassNvfp4Config, MoEActivationPack, MoELayer
    ...
    # TRTLLM packs routing weights as BF16; give every backend those same values.
    scores = torch.rand(tokens, top_k, device="cuda").softmax(-1).bfloat16().float()
    ...
    for label, layer in layers.items():
        ...
        for runner in layer.runners:
            ...
            if runner.backend_key == "cutlass_nvfp4":
    ...
    pool_labels = tuple(outputs)
    backend_labels, backend_tactics = {}, {}
    backend_graph_outputs, backend_graph_errors = {}, {}
    layer = layers["original"]
    for runner in layer.runners:
        backend = runner.backend_key
        label = f"backend:{backend}"
        backend_labels[backend] = label
        packed = runner.pack_inputs(act, weights)
        packed_keepalive.append(packed)
        with autotune():
            _, tactic = layer.tuner.choose_one(
                custom_op=f"moe_{backend}",
                runners=[runner],
                inputs=packed,
                tuning_config=runner.tuning_config_for(packed),
                **runner.launch_kwargs_for(packed),
            )
        backend_tactics[backend] = tactic
        captured_output = [None]

        def run_backend(runner=runner, tactic=tactic, holder=captured_output):
            # Include the same per-call GPU preparation as MoELayer. In
            # particular, TRTLLM packs prerouted IDs and weights here.
            inputs = runner.pack_inputs(act, weights)
            holder[0] = runner.forward(
                inputs, tactic=tactic, **runner.launch_kwargs_for(inputs)
            )
            return holder[0]

        outputs[label] = run_backend().clone()
        graphs[label] = capture(run_backend, args.graph_batch, warmup=5)
        out = captured_output[0]
        out.fill_(float("nan"))
        graphs[label].replay()
        backend_graph_errors[backend] = relative_l2(out, outputs["original"])
        if backend_graph_errors[backend] >= args.cross_l2:
            raise RuntimeError(
                f"Original backend graph failed validation: {backend}: "
                f"relative L2={backend_graph_errors[backend]}"
            )
        backend_graph_outputs[backend] = out.clone()
    errors = {
    ...
    if args.reference_tokens:
        ref = reference(
            act, weights.get_view("cutlass_nvfp4"), activation, args.reference_tokens
        )
        reference_errors = {
            label: relative_l2(out[: len(ref)], ref) for label, out in outputs.items()
        }
        for backend, out in backend_graph_outputs.items():
            label = backend_labels[backend]
            reference_errors[label] = max(
                reference_errors[label], relative_l2(out[: len(ref)], ref)
            )
    ...
    record = dict(
        ...
        milliseconds={label: medians[label] for label in pool_labels},
        relative_l2={label: errors[label] for label in pool_labels},
        reference_relative_l2={
            label: reference_errors[label]
            for label in pool_labels
            if label in reference_errors
        },
        reference_tokens=min(tokens, args.reference_tokens),
        samples_ms={label: samples[label] for label in pool_labels},
        backend_results={
            backend: dict(
                milliseconds=medians[label],
                samples_ms=samples[label],
                tactic=backend_tactics[backend],
                relative_l2=errors[label],
                reference_relative_l2=reference_errors.get(label),
                graph_relative_l2=backend_graph_errors[backend],
            )
...
def parse_e2e_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Benchmark full routed NVFP4 MoELayer pools"
    )
    parser.add_argument("--activation", choices=(*ACTIVATIONS, "all"), default="swiglu")
    parser.add_argument("--experts", type=int, default=8)
    ...
    # With unit global scales, smaller inputs can underflow NVFP4's E4M3
    # intermediate block scales after a gated activation.
