...
# Actual launch mechanism of the compute-only series. This is deliberately
# appended after every pre-existing field; cold/JIT and e2e remain direct.
COMPUTE_LAUNCH_CSV_FIELDS = "compute_launch_mode,graph_capture_warmup"
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    # Shared launch-geometry spelling.  The current complete-MNK path is
    # consumed by MXFP4 fused; ordinary FP8 keeps its established
    # --mma-tiler M,N / --fp8-knobs-json interfaces.
    p.add_argument(
        "--mma-tiler-mnk",
        dest="mma_tiler_mnk",
        type=str,
        default=None,
        metavar="M,N,K",
        help="full fused MMA tile. MXFP4 supports K=128 or K=256; "
        "the historical --mma-tiler M,N remains a K128-compatible alias.",
    )
    p.add_argument(
        "--cluster-shape",
        dest="cluster_shape_mnk",
        type=str,
        default=None,
        metavar="M,N,K",
        help="fused cluster shape (default: 1,1,1 for explicit MXFP4).",
    )
    p.add_argument(
        "--group-hint",
        type=int,
    ...
    # Historical MXFP4 spellings remain parse-compatible for existing
    # manifests/launchers, but are intentionally omitted from --help.  They
    # write the same argparse destinations as the dtype-neutral spelling.
    p.add_argument(
        "--mxfp4-mma-tiler",
        dest="mma_tiler_mnk",
        type=str,
    ...
    p.add_argument(
        ...
        help="atomic_counter matches the drop's perf-run setting; MXFP4 "
        "split resolves an omitted flag to its required static scheduler",
    )
    p.add_argument(
        "--split-k1-mma-tiler",
    ...
    p.add_argument(
        "--compute-launch-mode",
        choices=["direct", "cuda_graph"],
        default="direct",
        help="launch mode for the compute-only timed series. cuda_graph is "
        "an explicit MXFP4 fused winner-replay mode only; candidate discovery, "
        "cold/JIT, and e2e remain direct.",
    )
    p.add_argument(
        "--graph-capture-warmup",
        type=int,
        default=3,
        help="eager calls on the capture stream before MXFP4 fused compute "
        "CUDA Graph capture (outside the timed region).",
    )
    p.add_argument(
        "--routing-mode",
    ...
    p.add_argument(
        "--input-audit",
        choices=["none", "point", "full"],
        default="none",
        help="emit one rank-aggregated INPUT_AUDIT JSON record per point. "
        "point hashes the actual logical and staged live inputs after timing; "
        "full additionally hashes canonical and transformed weights. The "
        "default preserves the historical benchmark with no audit overhead.",
    )
    p.add_argument(
        "--no-sparse-data",
...
_LEGACY_MXFP4_FUSED_GEOMETRY_OPTIONS = frozenset(
    {
        "--mxfp4-mma-tiler",
        "--mxfp4-cluster",
        "--mxfp4-group-hint",
...
def _reject_cli_alias_conflicts(args: argparse.Namespace) -> None:
    """Fail closed when neutral and deprecated spellings target one field."""
    ...
    for spellings in groups:
        used = sorted(args._specified_options.intersection(spellings))
        if len(used) > 1:
            raise ValueError(
                "conflicting aliases for one fused tactic field: " + ", ".join(used)
            )
...
def _resolve_sweep(
    ...
    _reject_cli_alias_conflicts(args)
    if args.graph_capture_warmup <= 0:
        raise ValueError("--graph-capture-warmup must be positive")
    if args.compute_launch_mode == "cuda_graph":
        if args.backend != MXFP4_BACKEND:
            raise ValueError(
                "--compute-launch-mode cuda_graph currently requires "
                f"--backend {MXFP4_BACKEND}"
            )
        if args.execution_mode != "fused":
            raise ValueError(
                "--compute-launch-mode cuda_graph is an external fused-only "
                "capture; split already owns its internal Green CUDA Graph"
            )
        if args.mxfp4_tactic_source != "explicit":
            raise ValueError(
                "--compute-launch-mode cuda_graph requires an explicit fused "
                "winner tactic; cache/heuristic discovery remains direct"
            )
    ...
    if args.backend == MXFP4_BACKEND:
        ...
        if args.execution_mode == "fused":
            split_conflicts = sorted(
                args._specified_options.intersection(_SPLIT_TACTIC_OPTIONS)
            )
            if split_conflicts:
                raise ValueError(
        ...
        if args.execution_mode == "split" and fused_overrides:
            raise ValueError(
                "fused tactic flags cannot be used with --execution-mode split: "
                + ", ".join(fused_overrides)
            )
        split_execution_knobs = sorted(
            args._specified_options.intersection(_FUSED_EXECUTION_KNOB_OPTIONS)
        )
        if args.execution_mode == "split" and split_execution_knobs:
            raise ValueError(
                "fused execution knobs are fixed by split mode: "
                + ", ".join(split_execution_knobs)
            )
        ...
        if args.mma_tiler is not None and args.mma_tiler_mnk is not None:
            raise ValueError(
                "pass either historical --mma-tiler M,N or "
                "--mma-tiler-mnk M,N,K, not both"
            )
        tile = MXFP4_DEFAULT_TILE
        if args.mma_tiler is not None:
            tile = _parse_mma_tile(args.mma_tiler)
        elif args.mma_tiler_mnk is not None:
            tile = _parse_positive_triplet(args.mma_tiler_mnk, "--mma-tiler-mnk")[:2]
        if args.execution_mode == "split":
    ...
    mxfp4_only_options = sorted(
        args._specified_options.intersection(
            ...
            | _LEGACY_MXFP4_FUSED_GEOMETRY_OPTIONS
        )
    )
...
def _resolved_load_balance_mode(args: argparse.Namespace) -> str:
    if (
        ...
        and "--load-balance-mode" not in args._specified_options
    ):
...
def _mxfp4_fused_tactic(
    ...
    --mma-tiler-mnk M,N,K so K256 is preserved in config and output records.
    """

    if args.execution_mode != "fused":
        raise ValueError("MXFP4 fused tactic resolution requires fused mode")
    mma = (
        _parse_positive_triplet(args.mma_tiler_mnk, "--mma-tiler-mnk")
        if args.mma_tiler_mnk is not None
        else (legacy_tile[0], legacy_tile[1], MXFP4_TILE_K)
    )
    cluster = (
        _parse_positive_triplet(args.cluster_shape_mnk, "--cluster-shape")
...
def _tensor_audit_from_chunks(
    *,
    ...
    """Build a canonical tensor record from logical row-major byte chunks."""

    content = hashlib.sha256()
    observed_nbytes = 0
    for chunk in chunks:
        data = memoryview(chunk).cast("B")
        content.update(data)
        observed_nbytes += data.nbytes
    expected_nbytes = int(numel) * int(element_size)
    if observed_nbytes != expected_nbytes:
        raise RuntimeError(
            f"tensor audit byte count mismatch: {observed_nbytes} != {expected_nbytes}"
        )
    layout = {
        "dtype": str(dtype),
    ...
    return {
        **layout,
        ...
        "tensor_sha256": _canonical_json_sha256(
            {
                "content_sha256": content_sha256,
                "layout_sha256": layout_sha256,
            }
...
def _tensor_logical_byte_chunks(tensor, *, max_chunk_bytes: int):
    """Yield bounded logical row-major byte chunks without a whole-tensor copy."""
    ...
    def visit(view, start_dim: int):
        if int(view.numel()) * element_size <= max_chunk_bytes:
            packed = view.contiguous().view(torch.uint8).reshape(-1).cpu()
            yield packed.numpy().tobytes()
            return
        dim = start_dim
        while dim < view.ndim and int(view.shape[dim]) <= 1:
            dim += 1
        if dim == view.ndim:
            raise RuntimeError("cannot split tensor audit chunk to the requested bound")
        bytes_per_index = int(view.numel()) // int(view.shape[dim]) * element_size
        indices_per_chunk = max(1, max_chunk_bytes // bytes_per_index)
        for begin in range(0, int(view.shape[dim]), indices_per_chunk):
            count = min(indices_per_chunk, int(view.shape[dim]) - begin)
            yield from visit(view.narrow(dim, begin, count), dim)

    if tensor.numel():
        yield from visit(tensor, 0)


def _tensor_audit(
    tensor, *, max_chunk_bytes: int = INPUT_AUDIT_CHUNK_BYTES
) -> dict[str, object]:
    """Hash one tensor's values and descriptor without retaining host payloads."""

    return _tensor_audit_from_chunks(
        ...
        element_size=int(tensor.element_size()),
        chunks=_tensor_logical_byte_chunks(
            tensor,
...
def _audit_named_tensors(tensors: dict[str, object]) -> dict[str, object]:
    return {
        name: None if tensor is None else _tensor_audit(tensor)
        for name, tensor in tensors.items()
    }


def _audit_transformed_weights(backend: str, transformed) -> dict[str, object]:
    if not isinstance(transformed, tuple) or len(transformed) != 2:
        raise RuntimeError("transformed weights must contain FC1 and FC2 legs")
    slots = (
        (
            "weight",
            "folded_offset",
        ...
        if backend == MXFP4_BACKEND
            ...
            "weight",
            "weight_sf",
            "activation_dequant_scale",
            "weight_dequant_scale",
        )
    )
    components: dict[str, object] = {}
    for leg_name, leg in zip(("fc1", "fc2"), transformed, strict=True):
        if not isinstance(leg, tuple) or len(leg) != len(slots):
            raise RuntimeError(
                f"transformed {leg_name} weights do not match the four-slot ABI"
            )
        components.update(
            {
                f"{leg_name}.{slot}": value
                for slot, value in zip(slots, leg, strict=True)
            }
        )
    return _audit_named_tensors(components)


def _input_audit_recipes(
    args,
    *,
    scale_mode: str,
    tokens: int,
    ...
    logical_payload = {
        ...
        "activation_and_topk_weight_seed": (
            f"{ACTIVATION_AND_TOPK_WEIGHT_SEED_BASE}+rank"
        ),
        "draw_order": [
            "hidden_states.randn_bfloat16",
            "topk_weights.randn_float32_softmax",
        ],
        "routing": {
    ...
    if args.backend == MXFP4_BACKEND:
        weight_recipe = {
            "format": "packed_e2m1_k32_e8m0_humming_range11_v1",
            "seed": f"0x{MXFP4_WEIGHT_SEED_BASE:08X}+rank",
            "draw_order": ["w13", "w2", "w13_scale", "w2_scale"],
        }
        activation_staging = "e4m3_per_token_full_hidden_fp32_wire4_v1"
    else:
        weight_recipe = {
            "format": f"bf16_master_to_{args.kind}_{scale_mode}_v1",
            "seed": f"{FP8_WEIGHT_SEED_BASE}+rank",
            "draw_order": ["w13", "w2"],
        }
        activation_staging = {
            "blockwise": "e4m3_per_token_k128_fp32_scale_v1",
            "per_tensor": "e4m3_static_per_tensor_e8m0_unit_wire_v1",
        }[scale_mode]
    effective_data_mode = (
        "drop_sparse_override_v1"
        if args.use_sparse_data and args.backend == FP8_BACKEND
        else "dense_no_sparse_v1"
    )
    physical_payload = {
        "implementation": _expected_runtime_implementation(args, scale_mode),
        ...
        "weight_recipe_sha256": _canonical_json_sha256(weight_recipe),
    }
    return (
        {
            "payload": logical_payload,
            "sha256": _canonical_json_sha256(logical_payload),
        },
        {
            "payload": physical_payload,
            "sha256": _canonical_json_sha256(physical_payload),
        },
    )


def _rank_input_audit(
    args,
    *,
    scale_mode: str,
    tokens: int,
    ...
    workspace,
    transformed,
    canonical_weight_audit: dict[str, object] | None,
) -> dict[str, object] | None:
    if args.input_audit == "none":
        return None
    import torch

    profile = sm90_routing_profile_from_benchmark_mode(args.routing_mode)
    canonical_routes = generate_sm90_routing_numpy(
    ...
    route_payload = sm90_routing_audit_payload(
        canonical_routes,
        routing_profile=profile,
        seed=ROUTING_SEED,
        total_experts=args.num_experts,
    ...
    canonical_local = torch.from_numpy(
        canonical_routes[rank].astype("int64", copy=False)
    )
    logical = _audit_named_tensors(
        {
    ...
    staged = _audit_named_tensors(
        {
            "x": workspace.x[:tokens],
            "x_sf": workspace.x_sf[:tokens],
            "topk_idx": workspace.topk_idx[:tokens],
            "topk_weights": workspace.topk_weights[:tokens],
        }
    )
    canonical_route = _tensor_audit(canonical_local)
    input_route = logical["topk_ids"]
    staged_route = staged["topk_idx"]
    assert isinstance(input_route, dict) and isinstance(staged_route, dict)
    input_matches = input_route["content_sha256"] == canonical_route["content_sha256"]
    staged_matches = staged_route["content_sha256"] == input_route["content_sha256"]
    if not input_matches or not staged_matches:
        raise RuntimeError(
            "input-audit route mismatch among canonical, input, and staged IDs"
        )
    logical_topk_weights = logical["topk_weights"]
    staged_topk_weights = staged["topk_weights"]
    assert isinstance(logical_topk_weights, dict)
    assert isinstance(staged_topk_weights, dict)
    if logical_topk_weights["content_sha256"] != staged_topk_weights["content_sha256"]:
        raise RuntimeError("input-audit staged top-k weights differ from input")

    weights = None
    if args.input_audit == "full":
        if canonical_weight_audit is None:
            raise RuntimeError("full input audit lacks canonical weight records")
        weights = {
            "canonical": canonical_weight_audit,
            "transformed": _audit_transformed_weights(args.backend, transformed),
        }
    return {
        "rank": rank,
        "route_ids": {
            "canonical_global_i64le_sha256": route_payload["route_ids_sha256"],
            "canonical_local_sha256": canonical_route["content_sha256"],
            "input_local_sha256": input_route["content_sha256"],
            "staged_local_sha256": staged_route["content_sha256"],
            "input_matches_canonical": input_matches,
            "staged_matches_input": staged_matches,
        },
        "logical": logical,
        "staged": staged,
        "weights": weights,
...
def _ranked_tensor_sha256(
    rank_audits: Sequence[dict[str, object]], section: str, name: str
) -> str:
    values = []
    for record in rank_audits:
        tensors = record[section]
        if not isinstance(tensors, dict):
            raise RuntimeError(f"input audit rank lacks {section} records")
        tensor = tensors.get(name)
        if not isinstance(tensor, dict) or "tensor_sha256" not in tensor:
            raise RuntimeError(f"input audit rank lacks {section}.{name}")
        values.append(
            {"rank": record["rank"], "tensor_sha256": tensor["tensor_sha256"]}
        )
    return _canonical_json_sha256(values)


def _ranked_weight_sha256(
    rank_audits: Sequence[dict[str, object]], section: str
) -> str:
    values = []
    for record in rank_audits:
        weights = record["weights"]
        if not isinstance(weights, dict):
            raise RuntimeError("full input audit rank lacks weight records")
        components = weights.get(section)
        if not isinstance(components, dict):
            raise RuntimeError(f"full input audit rank lacks {section} weights")
        component_hashes = {}
        for name, value in sorted(components.items()):
            if value is None:
                component_hashes[name] = None
            elif isinstance(value, dict) and "tensor_sha256" in value:
                component_hashes[name] = value["tensor_sha256"]
            else:
                raise RuntimeError(f"full input audit has malformed {section}.{name}")
        values.append({"rank": record["rank"], "components": component_hashes})
    return _canonical_json_sha256(values)


def _aggregate_input_audit(
    args,
    *,
    scale_mode: str,
    tokens: int,
    world_size: int,
    rank_audits: Sequence[dict[str, object] | None],
) -> dict[str, object] | None:
    if args.input_audit == "none":
        if any(record is not None for record in rank_audits):
            raise RuntimeError("disabled input audit unexpectedly produced records")
        return None
    if any(not isinstance(record, dict) for record in rank_audits):
        raise RuntimeError("passing point lacks an input audit from every rank")
    records = sorted(
        (record for record in rank_audits if isinstance(record, dict)),
        key=lambda record: int(record["rank"]),
    )
    if [int(record["rank"]) for record in records] != list(range(world_size)):
        raise RuntimeError("input audit rank coverage is incomplete")
    route_hashes = set()
    for record in records:
        route = record.get("route_ids")
        if not isinstance(route, dict):
            raise RuntimeError("input audit rank lacks route records")
        route_hashes.add(str(route["canonical_global_i64le_sha256"]))
        if not route.get("input_matches_canonical") or not route.get(
            "staged_matches_input"
        ):
            raise RuntimeError("input audit contains an unverified route record")
    if len(route_hashes) != 1:
        raise RuntimeError("input audit ranks disagree on canonical route identity")

    logical_recipe, physical_recipe = _input_audit_recipes(
        args,
        scale_mode=scale_mode,
        tokens=tokens,
        world_size=world_size,
    ...
    audit: dict[str, object] = {
        ...
        "level": args.input_audit,
        "implementation": _expected_runtime_implementation(args, scale_mode),
        "logical_recipe": logical_recipe,
        "physical_recipe": physical_recipe,
        "route_ids": {
            "canonical_global_i64le_sha256": next(iter(route_hashes)),
            "input_matches_canonical": True,
            "staged_matches_input": True,
        },
        "logical": {
            f"{name}_sha256": _ranked_tensor_sha256(records, "logical", name)
            for name in ("hidden_states", "topk_ids", "topk_weights")
        },
        "staged": {
            f"{name}_sha256": _ranked_tensor_sha256(records, "staged", name)
            for name in ("x", "x_sf", "topk_idx", "topk_weights")
        },
        "weights": (
            {
                "canonical_sha256": _ranked_weight_sha256(records, "canonical"),
                "transformed_sha256": _ranked_weight_sha256(records, "transformed"),
            }
            if args.input_audit == "full"
            else None
        ),
...
def _make_transformed_weights(
    ...
    device,
    *,
    capture_canonical_audit: bool = False,
):
    """Canonical input pack -> production kernel-ready weight tuples.

    The optional audit is captured before preprocessing can release or mutate
    the source tensors. It is disabled by default, so the historical benchmark
    path pays no hashing or host-copy cost.
    """
    import torch
    ...
    if backend == MXFP4_BACKEND:
        ...
        canonical_audit = (
            _audit_named_tensors(
                {
                    "w13": raw.w13,
                    "w2": raw.w2,
                    "w13_scale": raw.w13_scale,
                    "w2_scale": raw.w2_scale,
                }
            )
            if capture_canonical_audit
            else None
        )
        ...
        del raw
        return transformed, canonical_audit
    ...
    canonical_audit = (
        _audit_named_tensors({"w13": w13, "w2": w2})
        if capture_canonical_audit
        else None
    )
...
def _megakernel_config(args, scale_mode: str, operand_order: str, tile, tokens=None):
    if args.backend == MXFP4_BACKEND:
        ...
        return Sm90_Fp8_Mxfp4_Bf16_PullCutedsl_MegaMoeConfig(
            ...
            # Split normalizes omission to its fixed static protocol.
...
@dataclass
class _CapturedComputeCall:
    """Keep graph-owned pointers alive while exposing a zero-argument replay."""
...
def _capture_compute_cuda_graph(call, *, warmup: int) -> _CapturedComputeCall:
    """Capture one fused compute call collectively, outside timed latency."""
    ...
    with torch.cuda.stream(stream):
        # Besides JIT, this populates the frontend stream-specific launch
        # cache. Capture must not take a cache-miss setup path.
    ...
    stream.synchronize()
    dist.barrier()
    return _CapturedComputeCall(graph, stream, captured_output)
...
def _run_point(
    ...
    try:
        ...
        transformed, canonical_weight_audit = _make_transformed_weights(
            args,
            args.backend,
            scale_mode,
            local_experts,
            rank,
            device,
            capture_canonical_audit=args.input_audit == "full",
        )
        if args.use_sparse_data and args.backend == FP8_BACKEND:
        ...
        direct_compute_call = lambda: bench_backend.compute(
            bench_workspace, transformed, output=None
        )
        if args.compute_launch_mode == "cuda_graph":
            captured_compute = _capture_compute_cuda_graph(
                direct_compute_call,
                warmup=args.graph_capture_warmup,
            )
            compute_call = captured_compute
        else:
            compute_call = direct_compute_call
        _cooldown(args.cooldown_s)
        compute = _time_calls(
            compute_call,
            warmup=args.warmup,
        ...
        rank_input_audit = _rank_input_audit(
            args,
            scale_mode=scale_mode,
            tokens=tokens,
            rank=rank,
            ...
            canonical_weight_audit=canonical_weight_audit,
        )
    ...
    if all(s == "pass" for s in statuses):
        result = PointResult(
            ...
            input_audit=_aggregate_input_audit(
                args,
                scale_mode=scale_mode,
                tokens=tokens,
                world_size=world_size,
                rank_audits=[s[8] for s in all_stats],
            ),
        )
    else:
...
def _compute_launch_csv_cols(args: argparse.Namespace) -> list[str]:
    """Append-only identity for the compute-only launch mechanism."""

    if args.backend == MXFP4_BACKEND and args.execution_mode == "split":
        cols = ["split_internal_cuda_graph", ""]
    elif args.compute_launch_mode == "cuda_graph":
        cols = ["fused_external_cuda_graph", str(args.graph_capture_warmup)]
    else:
        cols = ["direct", ""]
    expected = len(COMPUTE_LAUNCH_CSV_FIELDS.split(","))
    if len(cols) != expected:
        raise RuntimeError(
            f"compute launch CSV schema mismatch: {len(cols)} != {expected}"
        )
    return cols
...
def _fused_execution_runtime_cols(
    ...
    if (
        result.status != "pass"
        or args.execution_mode != "fused"
        or args.backend not in (FP8_BACKEND, MXFP4_BACKEND)
    ):
        return [""] * count
    metadata = result.runtime_metadata
    if metadata is None or len(metadata) != world_size:
...
def _validated_input_audit_json(
    args: argparse.Namespace, result: PointResult
) -> str | None:
    """Validate the self-hash and serialize one successful point's audit."""

    if result.status != "pass":
        if result.input_audit is not None:
            raise RuntimeError("a failed benchmark point must not carry input audit")
        return None
    if args.input_audit == "none":
        if result.input_audit is not None:
            raise RuntimeError("disabled input audit unexpectedly produced a record")
        return None
    if not isinstance(result.input_audit, dict):
        raise RuntimeError("passing benchmark point lacks its requested input audit")
    payload = dict(result.input_audit)
    observed = payload.pop("audit_sha256", None)
    expected = _canonical_json_sha256(payload)
    if observed != expected:
        raise RuntimeError(
            f"input audit self-hash mismatch: observed={observed!r}, "
            f"expected={expected!r}"
        )
    return _canonical_json(result.input_audit)
...
def _emit_row(
    ...
    *,
    ...
    input_audit_json = _validated_input_audit_json(args, result)
    ...
    compute_launch_row = ",".join(_compute_launch_csv_cols(args))
    ...
    if input_audit_json is not None:
        print(f"INPUT_AUDIT,{input_audit_json}", flush=True)
