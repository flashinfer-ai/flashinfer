...
def classify_benchmark_runtime_error(exc: RuntimeError) -> str | None:
    message = str(exc).lower()
    if "out of memory" in message:
        return "OOM"

    forced_algo = os.environ.get("FLASHINFER_TOPK_ALGO")
    if forced_algo in {"filtered", "multi_cta"} and "invalid argument" in message:
        return "UNSUPPORTED"
...
def generate_scores(
    ...
    def quantize_scores(scores: torch.Tensor, levels: float = 32.0) -> torch.Tensor:
        return (torch.round(scores * levels) / levels).to(dtype)

    if input_pattern == "random":
        return torch.randn(batch_size, seq_len, device="cuda", dtype=dtype)

    if input_pattern == "quantized_random":
        base = torch.randn(batch_size, seq_len, device="cuda", dtype=torch.float32)
        return quantize_scores(base)
    ...
    if input_pattern == "relu_quantized":
        # Sparse-attention/indexer-like scores often go through ReLU or clipping
        # before quantization, which creates realistic moderate tie pressure.
        base = torch.randn(batch_size, seq_len, device="cuda", dtype=torch.float32)
        return quantize_scores(torch.relu(base))
...
def build_top_k_cases(
        ...
        for seq_len in seq_lens:
            for k in k_values:
                if k <= seq_len:
                    cases.append(
                        TopKCase(
                            name=f"grid_b{batch_size}_l{seq_len}_k{k}",
                            batch_size=batch_size,
                            seq_len=seq_len,
                            k=k,
                        )
    ...
    # These deterministic large-batch/long-vocab cases are not covered by the
    # original grid but surfaced real correctness/performance differences.
    cases.extend(
        [
            TopKCase("stress_b2048_l131072_k1024", 2048, 131072, 1024),
            TopKCase("stress_b4096_l200000_k1024", 4096, 200000, 1024),
        ]
    )
...
def main():
    ...
    parser.add_argument(
        ...
        default="random",
        help=(
            "Input score pattern: random | quantized_random | relu_quantized | "
            "tie_heavy | pivot_tie"
        ),
    )
    parser.add_argument(
        "--dsa-input-pattern",
        choices=["random", "quantized_random", "dsa_relu"],
        default="dsa_relu",
        help="DSA top-k input pattern: random | quantized_random | dsa_relu",
    )
    parser.add_argument(
        "--dsa-case",
    ...
    if args.op in ["all", "top_k"]:
        ...
        header = (
            ...
            f"{fi_label:>14} {torch_label:>12} {'Speedup':>10}"
        )
    ...
    if args.op in ["all", "dsa_topk"]:
        ...
        header = (
            ...
            f"{'FlashInfer':>12} {torch_label:>12} {'Speedup':>10}"
        )
        if args.compare_torch_deterministic and not args.deterministic:
            header += f" {'torch.det':>12} {'Speedup':>10}"
        print(header)
        print(
            ...
            * (
                86
                if not (args.compare_torch_deterministic and not args.deterministic)
                else 110
            )
        )
