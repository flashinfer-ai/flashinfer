...
(RTX 5090 / RTX PRO 6000 Blackwell) the BF16 tensor pipe is the bound, so
``flashinfer.diffusion_ops.minimax_h3_sm120_varlen_attention_fp8`` quantizes Q / K / V / P to
E4M3 (FP32 softmax) and runs the whole operator in four launches.  This script times that
operator against the FlashInfer BF16 ragged routes (``fa2``, ``cudnn`` and ``auto``, which
resolves to the SM120 ``fmha_v2`` kernels where available) with ``bench_gpu_time`` (CUPTI,
cold L2), reports the error of every backend against an FP32 exact-softmax oracle, and the peak
extra device memory each backend needs.

Usage::

    python benchmarks/bench_minimax_h3_sm120_quant_varlen_attention.py --suite short
    python benchmarks/bench_minimax_h3_sm120_quant_varlen_attention.py --shapes center_33472 seg4_33472
    python benchmarks/bench_minimax_h3_sm120_quant_varlen_attention.py --suite all --json results.json
"""
...
def bench_shape(
    ...
    for backend in backends:
        ...
        try:
            ...
            if backend == "cake_fp8":
                ...
                out = fn()
                torch.cuda.synchronize()
                entry["error"] = error_stats(out, expected, ATOL, RTOL)
    ...
    fi_ok = {
        ...
        if b != "cake_fp8" and e.get("status") == "ok"
    }
    cake = row["backends"].get("cake_fp8", {})
...
def main() -> None:
    ...
    parser.add_argument("--backends", nargs="*", default=["cake_fp8", *FI_ROUTES])
