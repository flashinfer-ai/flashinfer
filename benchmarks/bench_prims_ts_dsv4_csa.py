...
def _load_shared_workload(
    ...
    *,
    ...
    if int(routes.max().item()) >= pool_rows:
        # Gather4/TMA reads through the route unchecked; an out-of-range row
        # would surface as an illegal access or silent garbage, not an error.
        raise ValueError(
            f"shared route exceeds pool_rows={pool_rows}; the workload directory "
            "was generated with a different pool geometry"
        )
    return (
        q_cpu.to(device),
...
def _shared_fp32_sample_reference(
    *,
    ...
    for query_idx in positions:
        ...
        swa_routes = routes[query_idx, :swa_valid]
        if bool((swa_routes < 0).any()):
            # -1 marks an inactive slot; a negative index inside the active
            # prefix would silently wrap to the last pool row here.
            raise ValueError(
                f"query {query_idx}: -1 SWA route inside the active prefix"
            )
        kv_tiles = [swa_pool[swa_routes.long()].float()]
