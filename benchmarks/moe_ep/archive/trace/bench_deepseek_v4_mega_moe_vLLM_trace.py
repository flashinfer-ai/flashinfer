#!/usr/bin/env python3
"""NVTX-instrumented vLLM DeepseekV4MegaMoEExperts forward benchmark for nsys.

Same problem/CLI as ``bench_deepseek_v4_mega_moe_vLLM.py`` with NVTX ranges
for ``bench_deepseek_v4_mega_moe_nsys.sh`` (--capture-range=nvtx).

Launch (example, 4 GPUs on one node):
    torchrun --nproc_per_node=4 \\
        benchmarks/moe_ep/trace/bench_deepseek_v4_mega_moe_vLLM_trace.py

Profile steady forwards only:
    bash benchmarks/moe_ep/trace/bench_deepseek_v4_mega_moe_nsys.sh nvtx vllm
"""
...
for path in (_BENCH_DIR, _TRACE_DIR):
...
def main() -> int:
    args = parse_benchmark_args(
        description=(
            "NVTX trace benchmark for DeepseekV4MegaMoEExperts forward "
            "(multi-rank EP)."
        )
    )
    ...
    try:
        ...
        print_benchmark_header(
            title="DeepseekV4MegaMoEExperts forward trace benchmark (NVTX)",
            rank=rank,
            world_size=world_size,
            num_local_experts=num_local_experts,
            args=args,
            ...
            extra_lines=("  nvtx_ranges=setup,cold_start,warmup,steady_capture,forward",),
