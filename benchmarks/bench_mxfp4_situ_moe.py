...
def main():
    ...
    parser.add_argument(
        "--paired-decode-weights",
        action="store_true",
        help="also prepare the paired W1/scale pair (prepare_cute_dsl_mxfp4_decode_weights) "
        "and pass it to every plan; it is selected only for eligible B300 T=2/4/16 rows",
    )
    ...
    configuration = {
        ...
        "paired_decode_weights": args.paired_decode_weights,
        "cold_l2_cache": True,
    ...
    decode_pair = None
    if args.paired_decode_weights:
        decode_pair = reference.prepare_cute_decode_weights(base)
        state["paired_decode_weight_bytes"] = _storage_bytes(decode_pair)
            ...
            for distribution in distributions:
                ...
                plan = wrapper.plan(
                    ...
                    *candidate_weights,
                    ...
                    paired_decode_weights=decode_pair,
                    workspace=workspace,
                    output=output,
                )
                paired_selected = (
                    decode_pair is not None
                    and plan._kwargs.get("w1_weight") is decode_pair[0]
                )
                ...
                for mode in modes:
                    ...
                    state["rows"][active_key] = {
                        ...
                        "mode": mode,
                        "routing": args.routing,
                        "paired_decode_weights": decode_pair is not None,
                        "paired_decode_weights_selected": paired_selected,
