...
"""Compare prepared Hopper VSA and FA3 GPU hot paths on the #5110 shapes.

Run from an editable FlashInfer checkout on H100:
    python benchmarks/bench_vsa_sm90.py --output /tmp/vsa-sm90.json

Preparation, host-to-device descriptor copies, JIT compilation, input generation,
and output allocation all happen before timing. Alternating AB/BA measurements
reduce ordering bias. CUPTI measures kernel activity and is required; this is
not an end-to-end host latency benchmark. Capture a profiler trace separately to audit
the timing boundary before publishing performance claims.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare prepared SM90 VSA and FA3 GPU kernels; requires H100 and CUPTI."
    )
    ...
    report = {
        ...
        "seed": args.seed,
        "timing": "prepared GPU kernel activity, CUPTI, warm L2, AB/BA",
        "cases": [],
    }
    for case in cases:
        mask, q, k, v, scale = make_inputs(case["axes"], args.seed)
        wrappers = {
            name: prepare(mask, name, scale) for name in ("fa3", "vsa_sm90_blk64")
        }
        outputs = {
            name: torch.empty(
        ...
        expected, actual = outputs["fa3"], outputs["vsa_sm90_blk64"]
        ...
        row = {
            **case,
            ...
            "median_ms": medians,
            "speedup": medians["fa3"] / medians["vsa_sm90_blk64"],
            "rounds": rounds,
        }
        ...
        print(
            f"{case['uuid']}: FA3={medians['fa3'] * 1e3:.3f} us; "
            f"CuTe={medians['vsa_sm90_blk64'] * 1e3:.3f} us; {row['speedup']:.3f}x",
            flush=True,
        )
    report["latency_geomean_us"] = {
        ...
        * math.exp(
        ...
        for backend in ("fa3", "vsa_sm90_blk64")
    }
