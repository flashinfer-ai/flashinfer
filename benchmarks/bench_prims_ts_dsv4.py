...
"""Benchmark the PrimTS DeepSeek V4 sparse MLA (CSA / HCA) attention kernels.

Default shape is the CSA context: B=1, Q=KV=8192, Hq=128, D=512, E4M3 in,
BF16 out, compression ratio 4, Kmax=1152, skip-correction threshold 8.  Only
the attention kernel is timed.  ``--refcheck`` compares sampled query rows
against an FP32 model of the kernel's online-softmax protocol; ``--scheduler``
pins the static or CLC-persistent 2CTA variant (``auto`` follows the API policy).
"""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="DSV4 compression ratio R (CSA=4, HCA=128)",
    )
...
def _build_geometry_metadata(
    *,
    ...
    # Inactive SWA slots are -1.  The Gather4 path reaches them before QK
    # masking, so replacing them with row 0 would change the measured address
    # stream.  The unselected compressed tail stays 0.
    ...
    # Deterministic per-query permutation of the visible compressed tokens.
    # Stride 37 is coprime to power-of-two capacities; other geometries may
    # repeat rows, which changes neither the work nor address validity.
...
def _fp32_sample_reference(
    *,
    ...
    """Check sampled query rows (SWA/first-tile transitions plus evenly spaced positions) against an FP32 model of the kernel's protocol."""
...
def main() -> None:
    ...
    # Page size 1 with uniform request lengths needs B * KV sliding-window
    # rows and B * floor(KV / R) compressed rows.
    ...
    if args.rope_quant:
        out_shape, out_scale_shape, out_scale_dtype = rope_quant_output_shapes(
            total_q, ue8m0=args.scale_format == "ue8m0"
        )
        out = torch.empty(out_shape, device=device, dtype=torch.float8_e4m3fn)
        out_scale = torch.empty(out_scale_shape, device=device, dtype=out_scale_dtype)
