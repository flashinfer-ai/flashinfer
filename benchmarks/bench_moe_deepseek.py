...
def bench_trtllm(
    ...
    def prepare_hidden(hidden_states, hidden_states_scale):
        per_token_scale = hidden_per_token_scale
        if include_activation_quant:
    ...
    if routing_input_mode == "routed":
        ...
        def run(topk_ids, topk_weights, hidden_states, hidden_states_scale):
            hidden_states, hidden_states_scale, per_token_scale = prepare_hidden(
        ...
        input_kwargs = {
            ...
            "hidden_states": (
                inputs["hidden_bf16"] if include_activation_quant else hfp
            ),
...
def run_benchmark(
    ...
    for distribution in benchmark_distributions:
        for n in token_counts:
            ...
            # Each row rebuilds full-model weights; release cached allocations
            # so measurements do not depend on scan order.
            gc.collect()
...
def _benchmark_single(
    ...
    run_cute_dsl_w4a4 = "cutedsl" in backends and profile_backend in (
        None,
        "cute-dsl",
    )
    run_cute_dsl_w4a16 = "cutedsl" in backends and profile_backend in (
        None,
        "cute-dsl-w4a16",
    )
    run_cutlass = "cutlass" in backends and profile_backend in (None, "cutlass")
    run_trtllm = "trtllm" in backends and profile_backend in (None, "trtllm")
    lat = {}
...
def _print_row(results, histogram_record, use_per_token_activation=False):
    ...
    w4a4 = r.get("CuteDSL W4A4") or r.get("CuteDSL")
    w4a16 = r.get("CuteDSL W4A16")
    ...
    def speedup_columns(result):
        return " ".join(
            f"{'n/a':>8}"
            if reference is None or result is None
            else f"{result.latency_ms / reference.latency_ms:>7.2f}x"
            for reference in (w4a4, w4a16)
        )
    ...
    if use_per_token_activation:
        print(
            ...
            f"{backend_columns(w4a4)} | "
            f"{backend_columns(w4a16)} | "
            f"{backend_columns(trtllm)} | "
            f"{speedup_columns(trtllm):>18} | "
            f"{winner:^8} | "
            f"{active_experts:>7} | "
            f"{stats:>14}"
        )
