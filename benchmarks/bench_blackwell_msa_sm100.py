...
driven by the same frozen 13-row manifest below. The first six rows preserve
the original production cases; the remaining rows freeze coverage selected
from the pinned MiniMax benchmark/correctness suites and FlashInfer boundary
tests.
...
SHAPE_MANIFEST: tuple[MSAShape, ...] = (
    ...
    MSAShape(
        ...
        provenance=(
            "csrc/blackwell_msa/blackwell_msa_decode_fp8_flat.cu and "
...
def _validate_shape_manifest(shapes: tuple[MSAShape, ...]) -> None:
    ...
    if stable_ids != FROZEN_SHAPE_IDS:
        raise ValueError("the frozen 13-shape manifest must not change")
