...
def test_create_fleet_unknown_backend_raises_key_error():
    from flashinfer.moe_ep import BootstrapConfig, FleetParams, create_fleet, dummy_moe_weights

    with pytest.raises(KeyError, match="unknown backend"):
        create_fleet(
            BootstrapConfig(world_size=1, rank=0),
            FleetParams(num_experts=2, max_tokens_per_rank=4, token_hidden_size=8, weights=dummy_moe_weights(num_local_experts=2, hidden=8)),
...
def test_create_fleet_accepts_config_object_backend_name(stubbed_fleet_registry):
    from flashinfer.moe_ep import BootstrapConfig, NvepConfig, FleetParams, create_fleet, dummy_moe_weights
