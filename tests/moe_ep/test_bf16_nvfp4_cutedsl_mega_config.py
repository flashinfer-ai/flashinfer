"""Host-only validation for mixed NVFP4-weight/BF16-activation MegaMoE."""

from __future__ import annotations

from types import SimpleNamespace

import pytest

from flashinfer.moe_ep.core.validation.common import is_bf16_nvfp4_cutedsl_supported

cuda_13_required = pytest.mark.skipif(
    not is_bf16_nvfp4_cutedsl_supported(),
    reason="bf16_nvfp4 requires CUDA 13+",
)

from flashinfer.moe_ep import Sm100_Bf16_Nvfp4_Bf16_Cutedsl_MegaMoeConfig
from flashinfer.moe_ep.backends.mega.kernel.sm100.common.bf16_config import (
    Sm100_Bf16_Cutedsl_MegaMoeConfigBase,
)
from flashinfer.moe_ep.core.kernel.registry import create_mega_kernel
from flashinfer.moe_ep.kernel_src.cutedsl_megamoe import MegaMoEBf16Nvfp4Config
from flashinfer.moe_ep.kernel_src.cutedsl_megamoe.shim.autotune import (
    bf16_nvfp4_candidates,
)
from flashinfer.moe_ep.kernel_src.cutedsl_megamoe.shim.bf16_nvfp4 import (
    MegaMoEBf16Nvfp4Frontend,
)
from flashinfer.moe_ep.kernel_src.cutedsl_megamoe.shim.tuner import (
    default_knobs,
    is_valid_bf16_nvfp4,
)

_LEGAL_IMPLS = {
    ((256, 128, 128), "tmem", False, 128),
    ((256, 256, 128), "smem", False, 128),
    ((256, 256, 128), "tmem", True, 64),
}


def _config(**kwargs):
    return MegaMoEBf16Nvfp4Config(
        rank=0,
        world_size=4,
        num_tokens_per_rank=256,
        num_topk=4,
        num_total_experts=32,
        hidden=1024,
        intermediate=1024,
        **kwargs,
    )


def _impl_key(knobs):
    return (
        knobs["mma_tiler_mnk"],
        knobs["transform_buffer"],
        knobs["accumulator_overlap"],
        knobs["transform_k_tile"],
    )


@cuda_13_required
def test_mixed_knobs_and_candidates():
    knobs = default_knobs(256, dtype="bf16_nvfp4")
    assert is_valid_bf16_nvfp4(knobs)
    assert is_valid_bf16_nvfp4(
        {
            **knobs,
            "mma_tiler_mnk": (256, 256, 128),
            "transform_buffer": "smem",
            "accumulator_overlap": False,
            "transform_k_tile": 128,
        }
    )
    assert not is_valid_bf16_nvfp4({**knobs, "mma_tiler_mnk": (256, 256, 128)})
    assert not is_valid_bf16_nvfp4({**knobs, "token_back_mode": "standalone_warps"})
    assert not is_valid_bf16_nvfp4({**knobs, "clc_bundle_size": 1})
    for token_back in ("epi_warps", "reuse_dispatch_warps"):
        assert is_valid_bf16_nvfp4(
            {
                **knobs,
                "in_kernel_fc2_reduce": True,
                "token_back_mode": token_back,
            }
        )

    candidates = bf16_nvfp4_candidates()
    assert len(candidates) == 18
    assert all(is_valid_bf16_nvfp4(c) for c in candidates)
    assert {_impl_key(c) for c in candidates} == _LEGAL_IMPLS
    assert {c["token_back_mode"] for c in candidates} == {
        "epi_warps",
        "reuse_dispatch_warps",
    }

    assert all(c["in_kernel_fc2_reduce"] is False for c in candidates)
    # Permitting ikr adds it as a sweep axis: the deterministic half stays,
    # and stays first so --max-candidates truncates to it.
    permitted = bf16_nvfp4_candidates(enable_in_kernel_fc2_reduce=True)
    assert len(permitted) == 36
    assert permitted[:18] == candidates
    assert all(c["in_kernel_fc2_reduce"] is True for c in permitted[18:])

    frontend = MegaMoEBf16Nvfp4Frontend(_config())
    frontend.apply_knobs(knobs)
    assert frontend.config == _config()


@cuda_13_required
@pytest.mark.parametrize(
    ("mma_tiler_mnk", "transform_buffer", "accumulator_overlap", "transform_k_tile"),
    sorted(_LEGAL_IMPLS),
)
def test_mixed_config_accepts_supported_implementation(
    mma_tiler_mnk, transform_buffer, accumulator_overlap, transform_k_tile
):
    config = _config(
        mma_tiler_mnk=mma_tiler_mnk,
        transform_buffer=transform_buffer,
        accumulator_overlap=accumulator_overlap,
        transform_k_tile=transform_k_tile,
    )
    assert config.num_experts_per_rank == 8


@cuda_13_required
def test_mixed_config_rejects_unsupported_implementation_and_token_back():
    with pytest.raises(ValueError, match="implementation tuple"):
        _config(mma_tiler_mnk=(256, 256, 128))
    with pytest.raises(ValueError, match="standalone"):
        _config(token_back_mode="standalone_warps")  # type: ignore[arg-type]


@cuda_13_required
def test_mixed_frontend_rejects_unsupported_knobs():
    frontend = MegaMoEBf16Nvfp4Frontend(_config())
    with pytest.raises(ValueError, match="unsupported mixed MegaMoE knobs"):
        frontend.apply_knobs({"mma_tiler_mnk": (256, 256, 128)})


@cuda_13_required
def test_mixed_factory_accepts_session_compatible_pinned_knobs(monkeypatch):
    import torch

    from flashinfer.moe_ep.kernel_src.cutedsl_megamoe.shim import bf16_nvfp4

    def fake_zeros(shape, dtype):
        tensor = torch.zeros(shape, dtype=dtype)
        tensor._mega_plain_alloc = True
        return tensor

    monkeypatch.setattr(bf16_nvfp4, "sym_zeros", fake_zeros)
    knobs = default_knobs(8, dtype="bf16_nvfp4")
    knobs["token_back_mode"] = "reuse_dispatch_warps"
    buf = bf16_nvfp4.get_symm_buffer_for_bf16_nvfp4_mega_moe(
        4,
        8,
        2,
        128,
        128,
        0,
        1,
        knobs=knobs,
    )
    try:
        assert buf._frontend.config.token_back_mode == "reuse_dispatch_warps"
        assert buf._frontend.config.in_kernel_fc2_reduce is False
    finally:
        buf.destroy()


@cuda_13_required
def test_mixed_factory_accepts_knobs_on_an_ikr_session(monkeypatch):
    """An ikr + dispatch-warp session must still accept pinned perf knobs."""
    import torch

    from flashinfer.moe_ep.kernel_src.cutedsl_megamoe.shim import bf16_nvfp4

    def fake_zeros(shape, dtype):
        tensor = torch.zeros(shape, dtype=dtype)
        tensor._mega_plain_alloc = True
        return tensor

    monkeypatch.setattr(bf16_nvfp4, "sym_zeros", fake_zeros)
    buf = bf16_nvfp4.get_symm_buffer_for_bf16_nvfp4_mega_moe(
        4,
        8,
        2,
        128,
        128,
        0,
        1,
        enable_in_kernel_fc2_reduce=True,
        knobs={
            "flag_batch": 4,
            "token_back_mode": "reuse_dispatch_warps",
            "in_kernel_fc2_reduce": True,
        },
    )
    try:
        config = buf._frontend.config
        assert config.in_kernel_fc2_reduce is True
        assert config.token_back_mode == "reuse_dispatch_warps"
        assert config.flag_batch == 4
        # Both destinations exist either way, so the knob can flip later.
        assert buf.combine_output.shape == (8, 2, 128)
        assert buf.reduced_output.shape == (8, 1, 128)
        assert buf.kernel_combine_output is buf.reduced_output
    finally:
        buf.destroy()


@cuda_13_required
def test_mixed_factory_rejects_unpermitted_pinned_ikr(monkeypatch):
    import torch

    from flashinfer.moe_ep.kernel_src.cutedsl_megamoe.shim import bf16_nvfp4

    monkeypatch.setattr(
        bf16_nvfp4, "sym_zeros", lambda shape, dtype: torch.zeros(shape, dtype=dtype)
    )
    with pytest.raises(ValueError, match="unsupported mixed MegaMoE knobs"):
        bf16_nvfp4.get_symm_buffer_for_bf16_nvfp4_mega_moe(
            4,
            8,
            2,
            128,
            128,
            0,
            1,
            knobs={
                **default_knobs(8, dtype="bf16_nvfp4"),
                "in_kernel_fc2_reduce": True,
            },
        )


@cuda_13_required
def test_mixed_frontend_rejects_unpermitted_ikr_knobs():
    with pytest.raises(ValueError, match="unsupported mixed MegaMoE knobs"):
        MegaMoEBf16Nvfp4Frontend(_config()).apply_knobs({"in_kernel_fc2_reduce": True})

    permitted = MegaMoEBf16Nvfp4Frontend(_config(enable_in_kernel_fc2_reduce=True))
    permitted.apply_knobs({"in_kernel_fc2_reduce": True})
    assert permitted.config.in_kernel_fc2_reduce is True


@cuda_13_required
def test_mixed_autotune_filters_unpermitted_ikr_candidates(monkeypatch):
    from flashinfer.moe_ep.kernel_src.cutedsl_megamoe.shim import autotune

    frontend = MegaMoEBf16Nvfp4Frontend(_config())
    valid = default_knobs(8, dtype="bf16_nvfp4")
    monkeypatch.setattr(
        autotune,
        "autotune_knobs",
        lambda _frontend, _launch, candidates, **_kwargs: candidates,
    )
    buffer = SimpleNamespace(_frontend=frontend)
    with pytest.warns(
        RuntimeWarning, match="in_kernel_fc2_reduce=True is not permitted"
    ):
        assert autotune.autotune_bf16_nvfp4_mega_moe(
            None,
            None,
            None,
            buffer,
            candidates=[{**valid, "in_kernel_fc2_reduce": True}, valid],
        ) == [valid]
    with pytest.raises(ValueError, match="no valid mixed") as excinfo:
        autotune.autotune_bf16_nvfp4_mega_moe(
            None,
            None,
            None,
            buffer,
            candidates=[{**valid, "in_kernel_fc2_reduce": True}],
        )
    assert "in_kernel_fc2_reduce=True is not permitted" in str(excinfo.value)


@cuda_13_required
def test_mixed_backend_is_registered():
    backend = create_mega_kernel(
        Sm100_Bf16_Nvfp4_Bf16_Cutedsl_MegaMoeConfig(
            intermediate_size=128,
            top_k=2,
        )
    )
    assert backend.kernel_name() == "sm100_bf16_nvfp4_bf16_cutedsl"


@cuda_13_required
def test_mixed_config_inherits_bf16_options():
    config = Sm100_Bf16_Nvfp4_Bf16_Cutedsl_MegaMoeConfig(
        intermediate_size=128,
        top_k=2,
        gate_up_clamp=1.5,
        enable_in_kernel_fc2_reduce=True,
    )
    assert isinstance(config, Sm100_Bf16_Cutedsl_MegaMoeConfigBase)
    assert config.gate_up_clamp == 1.5
    assert config.enable_in_kernel_fc2_reduce


@cuda_13_required
def test_mixed_alpha_staging_validates_and_restores_config_default():
    import torch

    from flashinfer.moe_ep.backends.mega.kernel.sm100.bf16_nvfp4_bf16_cutedsl.backend import (
        _stage_alpha,
    )

    destination = torch.empty(4, dtype=torch.float32, device="cuda")
    override = torch.arange(4, dtype=torch.float32, device="cuda")
    _stage_alpha("fc1_alpha", destination, override, 2.5)
    assert torch.equal(destination, override)

    _stage_alpha("fc1_alpha", destination, None, 2.5)
    assert torch.equal(destination, torch.full_like(destination, 2.5))

    with pytest.raises(ValueError, match="shape"):
        _stage_alpha("fc1_alpha", destination, torch.ones((), device="cuda"), None)
