"""Multi-rank fused-launch tests for MoEEpMegaLayer (sm100_bf16_nvfp4_bf16_cutedsl).

Launched via torchrun:
    torchrun --nproc_per_node=4 -m pytest tests/moe_ep/test_moe_ep_bf16_nvfp4_cutedsl_mega_multirank.py -v -m "gpu_4 and arch_blackwell"
"""
...
def _megakernel_config(
    ...
    *,
    ...
    return Sm100_Bf16_Nvfp4_Bf16_Cutedsl_MegaMoeConfig(
...
def _run_mega_torch_oracle(rank, world_size, *, in_kernel_fc2_reduce: bool = False):
    ...
    try:
        ...
        try:
            ...
            print(
                f"[bf16_nvfp4 multirank oracle rank {rank} "
...
def test_moe_ep_bf16_nvfp4_cutedsl_mega_layer_matches_reference():
    _require_cuda()
    rank, world_size = _launcher_ranks()
    if world_size < 4:
        pytest.skip("needs >=4 ranks")
    rank = _run_mega_layer(rank, world_size)
    print(f"rank {rank}: sm100_bf16_nvfp4_bf16_cutedsl mega layer matches reference")


@cuda_13_required
@pytest.mark.gpu_4
@pytest.mark.arch_blackwell
def test_moe_ep_bf16_nvfp4_cutedsl_mega_layer_in_kernel_fc2_reduce():
    _require_cuda()
    rank, world_size = _launcher_ranks()
    if world_size < 4:
        pytest.skip("needs >=4 ranks")
    ...
    print(
        f"rank {rank}: sm100_bf16_nvfp4_bf16_cutedsl mega layer "
...
def test_moe_ep_bf16_nvfp4_cutedsl_mega_multirank_torch_oracle(in_kernel_fc2_reduce):
    _require_cuda()
    rank, world_size = _launcher_ranks()
    if world_size < 4:
        pytest.skip("needs >=4 ranks")
    ...
    print(
        f"rank {rank}: sm100_bf16_nvfp4_bf16_cutedsl mega kernel "
