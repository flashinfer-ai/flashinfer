...
def test_factory_builds_two_roles_with_independent_tactics_and_sm_hints() -> None:
    ...
    # The split builder exposes no fake layout knobs, especially no K2
    # dispatch/fold axis. split_role makes the vendor constructor select the
    # fixed old protocol for each role.
    fused_only_layout_fields = {
        "dedup_dispatch",
        "grouped_token_back",
        "combine_format",
    ...
    assert fused_only_layout_fields.isdisjoint(k1.kwargs)
    assert fused_only_layout_fields.isdisjoint(k2.kwargs)
