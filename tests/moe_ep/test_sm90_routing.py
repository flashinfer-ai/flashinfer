...
def test_published_exact_standard_hashes_do_not_change(
    ...
    assert np.array_equal(routes, repeated)
    assert not np.array_equal(routes[0], routes[1])
    _assert_exact_balanced(
        routes, world_size=4, tokens=tokens, topk=6, total_experts=384, seed=1234
    )
