...
@pytest.mark.parametrize("legacy_fields", (17, 19))
def test_sm90_mxfp4_old_tactic_resets_tail_pairs_and_compile_identity(legacy_fields):
    frontend, complete = _frontend_with_complete_tactic(cluster_shape_mnk=(1, 2, 1))
    original_key = frontend._mega_compile_key()
    legacy = {
        key: value for key, value in complete.items() if key != "tail_split_pairs"
    }
    if legacy_fields == 17:
        legacy.pop("fc2_tail_n8")
        legacy.pop("fc1_ready_mode")
    assert len(legacy) == legacy_fields
    enabled = dict(complete, tail_split_pairs=True)
    module = "flashinfer.moe_ep.kernel_src.sm90.pull_style_cutedsl_megakernel.shim.hopper_mxfp4"
    with (
        mock.patch(module + ".ensure_not_capturing"),
        mock.patch.object(frontend, "_release_workspace") as release,
    ...
    assert frontend.config.tail_split_pairs is False
    assert frontend._mega_compile_key() == original_key
...
@pytest.mark.parametrize(
    "overrides,match",
    [
        ({"cluster_shape_mnk": (2, 1, 1)}, "cluster_shape_mnk"),
        ({"fc1_ready_mode": "k256"}, "fc1_ready_mode='tile'"),
    ],
)
def test_sm90_mxfp4_tail_pairs_reject_invalid_tactic_before_release(overrides, match):
    frontend, tactic = _frontend_with_complete_tactic(cluster_shape_mnk=(1, 2, 1))
    with mock.patch.object(frontend, "_release_workspace") as release:
        with pytest.raises(ValueError, match=match):
            frontend.apply_knobs(dict(tactic, tail_split_pairs=True, **overrides))
...
def test_sm90_mxfp4_pingpong_uses_m128_and_keeps_scalar_stores():
    ...
    assert config.optimizations.peer32 is False
    with pytest.raises(ValueError, match="ping-pong requires mma_tiler M=128"):
        dataclasses.replace(config, mma_tiler_mnk=(256, 64, 256))
...
def test_sm90_mxfp4_configs_reject_non_bool_inherited_flags(field, value):
    public_field = (
        "enable_in_kernel_fc2_reduce" if field == "in_kernel_fc2_reduce" else field
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("enable_in_kernel_fc2_reduce", True, "standalone top-k"),
    ],
)
...
class TestMxfp4OptimizationPolicy(unittest.TestCase):
    ...
    def test_other_formats_keep_legacy_paths(self):
        for overrides in (
    ...
    def test_cross_h_keeps_other_formats_and_writeback_guards(self):
        for hidden in (4096, 6144, 7168, 8192):
            for overrides in (
                {"fp8_scale_mode": "blockwise"},
                {"fp8_scale_mode": "per_tensor"},
                {"pingpong": True, "mma_tiler_mnk": (128, 64, 256)},
                {"token_back_by_dispatch": True},
                {"fc2_in_kernel_topk_reduce": True},
                {"dedup_dispatch": True},
...
def test_mxfp4_keeps_qualified_layout_when_fp8_defaults_expand(field):
    frontend, _ = _frontend_with_complete_tactic()
    assert getattr(frontend.config, field) is False
    with pytest.raises(
        ValueError, match="compact_pull_buffer=False and generate_c=False"
    ):
        dataclasses.replace(frontend.config, **{field: True})
