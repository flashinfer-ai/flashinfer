...
class TestShimAndBackendWiring:
    ...
    @pytest.mark.parametrize(
        ("overrides", "compact_pull_buffer", "generate_c"),
        [
            ({}, True, False),
            ({"compact_pull_buffer": False, "generate_c": True}, False, True),
        ],
    )
    @pytest.mark.parametrize("tail_split_pairs", [None, False, True])
    def test_public_symm_buffer_preserves_allocation_options(
        self, scale_mode, overrides, compact_pull_buffer, generate_c, tail_split_pairs
    ):
        pkg = _pkg()
        from flashinfer.moe_ep.kernel_src.sm90.pull_style_cutedsl_megakernel.shim import (  # noqa: E501
            hopper_fp8,
        ...
        with mock.patch.object(
            ...
            result = pkg.get_symm_buffer_for_hopper_fp8_mega_moe(
                8,
                64,
                4,
                1024,
        ...
        resolved = allocator.call_args.args[0]
        assert isinstance(resolved, pkg.MegaMoEHopperFp8Config)
        assert resolved.fp8_scale_mode == scale_mode
        assert resolved.compact_pull_buffer is compact_pull_buffer
        assert resolved.generate_c is generate_c
        assert resolved.tail_split_pairs is bool(tail_split_pairs)
    ...
    @pytest.mark.parametrize("scale_mode", ["per_tensor", "blockwise"])
    def test_auto_workspace_uses_public_allocator(self, scale_mode):
        ...
        assert result is workspace
        allocator.assert_called_once()
        assert allocator.call_args.args[0].fp8_scale_mode == scale_mode
        assert backend._autotune_pending

    @pytest.mark.parametrize("tail_split_pairs", [False, True])
    def test_prepare_workspace_binds_key_and_allocator_to_one_resolution(
        self, tail_split_pairs
    ):
        pkg = _pkg()
        from flashinfer.moe_ep import (
            BootstrapConfig,
        ...
        assert resolver.call_args.kwargs["tail_split_pairs"] is tail_split_pairs
