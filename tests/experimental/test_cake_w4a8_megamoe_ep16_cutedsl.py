...
def environment():
    ...
    torch.cuda.set_device(
        0 if torch.cuda.device_count() == 1 else int(os.environ.get("LOCAL_RANK", "0"))
    )
    ...
    candidate_weights = preprocess_cake_w4a8_megamoe_ep16_cutedsl_weights(weights)
    ...
    identities = []
    yield candidate_weights, native_weights, workspace, identities
...
def test_forward_and_replay(environment, tokens, hot):
    ...
    weights, native, workspace, identities = environment
    ...
    if tokens == 17:
        ...
        x[1] = (
            torch.tensor(
                [0.0, 2.0**-133, 2.0**-127, 2.0**-126, 2.0**-120, 2.0**-20, 1.0, 448.0],
    ...
    identities.append(session._module.identity["cubin_sha256"])
    prepared_forward = session.prepare(x, rw, out=out)
    ...
    for replay in range(3):
        result = session.forward(x, rw, out=out) if replay == 0 else prepared_forward()
    ...
    for value in outputs:
        ...
        torch.testing.assert_close(
            value.double(), expected.double(), atol=0.01, rtol=0.01
        )
        assert torch.equal(value, outputs[0])
    relative_l2 = (
        outputs[0].double() - expected.double()
    ).norm() / expected.double().norm().clamp_min(1e-6)
    ...
    if tokens == 16 and not hot:
        # The prepared launch must observe changed contents at the same addresses.
        x.mul_(0.5)
        rw.copy_(rw.flip(-1))
        compute_reference()
        torch.cuda.synchronize()
        ...
        torch.cuda.synchronize()
        dist.barrier()
        assert not torch.equal(out, outputs[0])
        torch.testing.assert_close(
            out.double(), expected.double(), atol=0.01, rtol=0.01
        )
        relative_l2 = (
            out.double() - expected.double()
        ).norm() / expected.double().norm().clamp_min(1e-6)
        assert relative_l2.item() < 0.02
        assert (guarded[[0, -1]] == -123).all()
    prepared_forward.close()
