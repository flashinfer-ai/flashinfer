...
_HELPER = Path(__file__).resolve().parents[2] / "examples/experimental/mega_moe_inputs.py"
...
def supported_gpu():
    ...
    if (properties.major, properties.minor, properties.multi_processor_count) != (10, 3, 152):
...
def check_v3_metadata(plan, inputs):
    ...
    tokens = torch.arange(inputs["num_tokens"], device=routes.device)[:, None].expand_as(routes)
    slots = torch.arange(inputs["top_k"], device=routes.device)[None, :].expand_as(routes)
    assert torch.all(permuted >= offsets[routes]).item()
    assert torch.all(permuted < (offsets + count)[routes]).item()
    torch.testing.assert_close(bindings["meta_token"][permuted].long(), tokens, atol=0, rtol=0)
    torch.testing.assert_close(bindings["meta_slot"][permuted].long(), slots, atol=0, rtol=0)
    torch.testing.assert_close(bindings["routing_weight_pool"][permuted], inputs["topk_weights"], atol=0, rtol=0)
    expected_x = inputs["x_fp8_packed"].view(torch.int32)[:, None, :].expand(-1, inputs["top_k"], -1)
    torch.testing.assert_close(bindings["pool_fp8"][permuted], expected_x, atol=0, rtol=0)
    expected_sf = inputs["x_sf_packed"][:, None, :].expand(-1, inputs["top_k"], -1)
    torch.testing.assert_close(bindings["pool_sf"].view(torch.int32)[permuted], expected_sf, atol=0, rtol=0)
...
def test_pipeline_reference_and_replay(family, precision, seed):
    ...
    atol = 1.0 if precision == "fp4" else .1
    ...
    for _ in range(2):
        ...
        torch.testing.assert_close(output, expected, atol=atol, rtol=.1)
        check_metadata(plan, inputs)
        if first is None:
            first = output.clone()
        else:
            torch.testing.assert_close(output, first, atol=0 if family == "source" else atol, rtol=0 if family == "source" else .1)
...
def test_source_update_inputs_reuses_workspace():
    ...
    plan.update_inputs(changed["x_fp8_packed"], changed["x_sf_packed"], changed["topk_idx"], changed["topk_weights"])
    plan.output.fill_(float("nan"))
    plan.run()
    torch.cuda.synchronize()
    assert plan.workspace.data_ptr() == pointer
    assert not torch.equal(plan.output, original)
    torch.testing.assert_close(plan.output, fixtures.reference(changed, xs, shared), atol=1., rtol=.1)
    # A fresh independent workspace must agree exactly with the reused one.
    fresh = prepare_mega_moe(changed["x_fp8_packed"], changed["x_sf_packed"], changed["topk_idx"], changed["topk_weights"],
                            weights=fixtures.source_weights(inputs, shared), num_experts=8, intermediate=256,
                            routed_weight_dtype="fp4", num_shared_experts=1, activation_clamp=10., fast_math=True, num_sms=2)
    fresh.run()
    torch.cuda.synchronize()
...
def test_grouped_l2_repack_and_graph_replay():
    ...
    for changed in (None, "activation", "weight"):
        ...
        difference = 1 - 2 * (actual * target).sum() / (actual.square().sum() + target.square().sum())
        ...
        permuted_a = words_a.reshape(4096 // 128, 4, 32, -1).transpose(1, 2).contiguous().reshape(4096, -1)
        torch.testing.assert_close(repack["dst_a"].view(torch.int32), permuted_a.T.contiguous(), atol=0, rtol=0)
        permuted_b = words_b.reshape(4, 7168 // 128, 4, 32, -1).transpose(2, 3).contiguous().reshape(4, 7168, -1)
        expected_b = permuted_b.permute(0, 2, 1).contiguous().reshape(-1, 7168)
        torch.testing.assert_close(repack["dst_b"].view(torch.int32), expected_b, atol=0, rtol=0)
        graph = torch.cuda.CUDAGraph()
        with torch.cuda.graph(graph):
            plan.run()
        plan.outputs.fill_(float("nan"))
