...
def test_mm_fp4_sm103_cutlass_epilogue() -> None:
    ...
    # First verify the public API and autotuner integration. The cold-L2,
    # single-call ranking can legitimately select either the generic or native
    # family when their timings are close, so do not make its winner a
    # correctness requirement.
    public_output = torch.empty((m, n), device="cuda", dtype=torch.bfloat16)
    tuner = AutoTuner.get()
    tuner.clear_cache()
    with autotune(True, tuning_buckets=(m,)):
        public_result = mm_fp4(
            a_fp4,
            b_fp4.T,
            a_sf,
            b_sf.T,
            alpha=base_alpha,
            out_dtype=public_output.dtype,
            out=public_output,
            block_size=16,
    ...
    assert tactic in _ALL_SM103_TACTICS
    assert public_result.data_ptr() == public_output.data_ptr()
    public_cosine = F.cosine_similarity(
        reference.reshape(-1), public_output.float().reshape(-1), dim=0
    )
    assert public_cosine > 0.98

    # Exercise the native K768 epilogue deterministically rather than relying
    # on a timing-based autotuner choice to provide path coverage.
    workspace = torch.empty(DEFAULT_WORKSPACE_SIZE, device="cuda", dtype=torch.uint8)
    native_runner = get_gemm_sm103_module_cutlass_fp4().cutlass_fp4_gemm_runner()
