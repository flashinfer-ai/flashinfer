...
def test_fa_backends_share_fp8_plan_validation(monkeypatch, backend_name):
    ...
    monkeypatch.setattr(
        fa2_backend, "_get_compute_capability", lambda device: (9, 0), raising=False
    )
    wrapper = _minimal_uninitialized_wrapper(mla.BatchMLAPagedAttentionWrapper)
...
def test_unplanned_cutlass_compatibility_replans_for_dynamic_shape(monkeypatch):
    import flashinfer.mla as mla
    ...
    assert captured["plan"].metadata.block_tables is page_table
    assert isinstance(captured["run"]["query"], torch.Tensor)
    assert isinstance(captured["run"]["kv_cache"], torch.Tensor)
...
def test_cuda_graph_replan_failure_rolls_back_reserved_metadata(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *args: _FailsAfterPlanningStarts(),
    )
