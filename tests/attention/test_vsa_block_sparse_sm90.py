...
pytestmark = pytest.mark.skipif(
    ...
    reason="vsa_sm90_blk64 backend requires SM90 GPU",
)

# sm90_blk64 kernel constants
R = C = 64
HEAD_DIM = 128
...
def _make_wrapper(workspace):
    return BlockSparseAttentionWrapper(workspace, backend="vsa_sm90_blk64")
...
def test_vsa_sm90_accuracy(dtype, density, num_blocks, num_heads, workspace):
    """sm90_blk64 output must match PyTorch dense block-sparse reference."""
...
def test_vsa_sm90_sm_scale(sm_scale, workspace):
...
def test_vsa_sm90_per_head_mask_correctness(workspace):
...
def test_vsa_sm90_gqa(num_qo_heads, num_kv_heads, dtype, workspace):
    """sm90_blk64 GQA must match per-head PyTorch reference."""
    device = torch.device("cuda")
    torch.manual_seed(42)
    num_blocks = 8
    M = N = num_blocks * R
...
def test_vsa_sm90_asymmetric_seqlen(MB, NB, num_heads, density, workspace):
    """sm90_blk64 with seqlen_q != seqlen_k must match PyTorch reference."""
    device = torch.device("cuda")
...
def test_vsa_sm90_return_lse(dtype, num_blocks, num_heads, workspace):
...
def test_vsa_sm90_empty_row(workspace):
...
def test_vsa_sm90_variable_blocks_per_q(workspace):
...
def test_vsa_sm90_perf_sweep(workspace):
    """sm90_blk64 throughput across seqlen × density configurations."""
    device = torch.device("cuda")
    torch.manual_seed(0)
    num_heads = 8
    dtype = torch.bfloat16
