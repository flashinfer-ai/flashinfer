...
def _assert_uniform_no_split_plan(
    ...
    expected_padded_grid = expected_grid
    if q_len_per_req > 1:
        # Multi-token decode retains the existing prefill padding policy.
        num_sm = torch.cuda.get_device_properties(wrapper.device).multi_processor_count
        expected_padded_grid = max(2 * num_sm // num_kv_heads, expected_grid)
    assert plan[0] == expected_padded_grid
    ...
    mask = host_workspace[plan[12] : plan[12] + expected_padded_grid]
    expected_mask = (torch.arange(expected_padded_grid) < expected_grid).to(mask.dtype)
...
def test_cuda_graph_uniform_no_split_replan_replay(
    use_fast_plan: bool, q_len_per_req: int, dtype: torch.dtype, page_size: int
):
    ...
    num_kv_heads = 2
    head_dim = 128
    short_kv_lens = [max(2, q_len_per_req), max(4, q_len_per_req)]
    long_kv_lens = [129, 65]
    ...
    def build_kv_layout(kv_lens):
        ...
        indptr = torch.tensor(
            [0] + pages_per_request, dtype=torch.int32, device="cuda:0"
        ).cumsum(0, dtype=torch.int32)
        indices = torch.arange(
            sum(pages_per_request), dtype=torch.int32, device="cuda:0"
        )
    ...
    short_layout = build_kv_layout(short_kv_lens)
    long_layout = build_kv_layout(long_kv_lens)
    max_num_pages = len(long_layout[1])
    kv_data = torch.randn(
        max_num_pages,
        2,
        page_size,
...
@pytest.mark.parametrize("kv_layout", ["HND", "NHD"])
@pytest.mark.parametrize("pos_encoding_mode", ["NONE", "ROPE_LLAMA"])
def test_batch_decode_tensor_cores_with_fast_plan(
    batch_size: int,
    invariant_bs: int,
    kv_len: int,
