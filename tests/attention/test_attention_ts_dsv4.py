...
from flashinfer.attention.prims_ts.kernels.mla_decode.throughput_2cta.token_sparse_launch import (
    MAX_ROPE_QUANT_TOTAL_Q,
...
_REQUIRES_PRIMTS_GPU = pytest.mark.skipif(
    ...
    reason="PrimTS DSV4 sparse MLA requires SM100 or SM103",
)
...
def _reference_tiled_e4m3_softmax(
    query: torch.Tensor,
...
def test_prims_ts_dsv4_sparse_mla_runtime_launch_shape_reuses_compilation() -> None:
    ...
    def run_case(
        *,
        ...
        page_idx_kv, sparse_lens = _make_physical_routing(
        ...
        expected, expected_lse = _reference_tiled_e4m3_softmax(
            query,
            swa_pool,
            compressed_pool,
            page_idx_kv,
    ...
    cache_after_first = compile_token_sparse_kernel.cache_info()
    second = run_case(
        q_lens=(2,),
        max_seq_len_q=2,
        sparse_capacity=384,
    ...
    cache_after_second = compile_token_sparse_kernel.cache_info()
    third = run_case(
        q_lens=(2, 3),
    ...
    cache_after_third = compile_token_sparse_kernel.cache_info()
    fourth = run_case(
        q_lens=(1,),
        max_seq_len_q=1,
        sparse_capacity=192,
    ...
    cache_after_fourth = compile_token_sparse_kernel.cache_info()
...
def test_prims_ts_dsv4_hca_r128_padded_workids_preserve_output(
    ...
    cache_after_one_padding = compile_token_sparse_kernel.cache_info()
    three_padding_output, three_padding_lse = run(4)
    cache_after_three_padding = compile_token_sparse_kernel.cache_info()
...
def _empty_rope_quant_scale(
    ...
    _, scale_shape, scale_dtype = rope_quant_output_shapes(total_q, ue8m0=ue8m0)
    return torch.empty(scale_shape, device=device, dtype=scale_dtype)
...
def test_prims_ts_dsv4_rope_quant_padded_workids_preserve_output(
    ...
    def run(max_seq_len_q: int, output_fill: int):
        out_shape, _, _ = rope_quant_output_shapes(total_q, ue8m0=ue8m0)
        output = torch.empty(out_shape, device="cuda", dtype=_FP8)
    ...
    expected, _ = _reference_tiled_e4m3_softmax(
        query,
        swa_pool,
        compressed_pool,
        page_idx_kv,
...
def reference_protocol(fx: Fixture, skip_corr_threshold: float):
    ...
    return _reference_tiled_e4m3_softmax(
        fx.query,
        fx.swa_pool,
        fx.compressed_pool,
        fx.page_idx_kv,
...
def reference_protocol_batched(fx: Fixture, skip_corr_threshold: float):
    """Vectorized form of ``_reference_tiled_e4m3_softmax`` for large fixtures.
...
def _validated_call(fx: Fixture, **overrides):
    kwargs = dict(
        ...
        sparse_indices=fx.page_idx_kv,
        sparse_topk_lens=fx.sparse_lens,
        seq_lens=fx.seq_lens_kv,
        cum_seq_lens_q=fx.cu_seqlens_q,
        max_seq_len_q=fx.max_seq_len_q,
    )
...
def test_prims_ts_dsv4_sparse_mla_validate_inputs(
    ...
    with pytest.raises(ValueError, match="start at zero and end at T"):
        _validated_call(fx, cum_seq_lens_q=torch.tensor([1, 2, 5], **i32))
    with pytest.raises(ValueError, match="start at zero and end at T"):
        _validated_call(fx, cum_seq_lens_q=torch.tensor([0, 2, 4], **i32))
    with pytest.raises(ValueError, match="at least one Q row"):
        _validated_call(
            fx,
            cum_seq_lens_q=torch.tensor([0, 2, 2, 5], **i32),
            seq_lens=torch.tensor([300, 300, 129], **i32),
        )
    with pytest.raises(ValueError, match="raw lengths"):
        _validated_call(fx, seq_lens=torch.tensor([300, 2], **i32))
    ...
    with pytest.raises(ValueError, match=r"\[128, Kmax\]"):
        _validated_call(fx, sparse_topk_lens=too_short)
    too_long = fx.sparse_lens.clone()
    too_long[-1] = case.kmax + 4
    with pytest.raises(ValueError, match=r"\[128, Kmax\]"):
        _validated_call(fx, sparse_topk_lens=too_long)
    ...
    with pytest.raises(ValueError, match="SWA slots"):
        _validated_call(fx, sparse_indices=swa_out_of_range)
    swa_negative = fx.page_idx_kv.clone()
    swa_negative[0, 0] = -2  # only -1 marks an inactive SWA slot
    with pytest.raises(ValueError, match="SWA slots"):
        _validated_call(fx, sparse_indices=swa_negative)
    ...
    with pytest.raises(ValueError, match="compressed slots"):
        _validated_call(fx, sparse_indices=compressed_out_of_range)
    compressed_negative = fx.page_idx_kv.clone()
    compressed_negative[1, _SWA_WIDTH] = -1
    with pytest.raises(ValueError, match="compressed slots"):
        _validated_call(fx, sparse_indices=compressed_negative)
    ...
    assert torch.equal(
        _validated_call(fx, sparse_indices=unreachable_garbage), baseline
    )
...
_COMMON_INVALID_ARGUMENTS = (
    ...
    (
        ...
        ValueError,
        "max_seq_len_q must be positive",
    ),
    (
        "max-seq-len-q-float",
        lambda fx: dict(max_seq_len_q=float(fx.max_seq_len_q)),
        TypeError,
        "max_seq_len_q must be a positive integer",
    ),
    (
    ...
    (
        "page_idx_kv-dtype",
        lambda fx: dict(sparse_indices=fx.page_idx_kv.to(torch.int64)),
        TypeError,
        "sparse_indices must have dtype",
    ),
    (
        "page_idx_kv-rows",
        lambda fx: dict(sparse_indices=fx.page_idx_kv[:-1]),
        ValueError,
        r"sparse_indices must have shape \[T, Kmax\]",
    ),
    (
        "page_idx_kv-kmax-below-swa",
        lambda fx: dict(
            sparse_indices=fx.page_idx_kv[:, : _SWA_WIDTH - 4].contiguous()
        ),
        ValueError,
    ...
    (
        ...
        lambda fx: dict(
            sparse_indices=fx.page_idx_kv[:, : _SWA_WIDTH + 2].contiguous()
        ),
        ValueError,
        "Kmax must be at least 128 and divisible by 4",
    ),
    (
        "sparse-lens-dtype",
        lambda fx: dict(sparse_topk_lens=fx.sparse_lens.to(torch.int64)),
        TypeError,
        "sparse_topk_lens must have dtype",
    ),
    (
        "sparse-lens-rank",
        lambda fx: dict(sparse_topk_lens=fx.sparse_lens[None, :]),
        ValueError,
        "sparse_topk_lens must have rank 1",
    ),
    (
        "sparse-lens-length",
        lambda fx: dict(sparse_topk_lens=fx.sparse_lens[:-1]),
        ValueError,
        r"sparse_topk_lens must have shape \[T\]",
    ),
    (
        "seq-lens-kv-dtype",
        lambda fx: dict(seq_lens=fx.seq_lens_kv.to(torch.int64)),
        TypeError,
        "seq_lens must have dtype",
    ),
    (
        "seq-lens-kv-cpu",
        lambda fx: dict(seq_lens=fx.seq_lens_kv.cpu()),
        ValueError,
        "seq_lens must be a CUDA tensor",
    ),
    (
        "seq-lens-kv-empty",
        lambda fx: dict(seq_lens=fx.seq_lens_kv[:0]),
        ValueError,
    ...
    (
        "cu-seqlens-q-length",
        lambda fx: dict(
            cum_seq_lens_q=torch.cat((fx.cu_seqlens_q, fx.cu_seqlens_q[-1:]))
        ),
        ValueError,
        r"sequence metadata must have shapes \[B\] and \[B \+ 1\]",
    ),
    (
        "cu-seqlens-q-rank",
        lambda fx: dict(cum_seq_lens_q=fx.cu_seqlens_q[None, :]),
        ValueError,
        "cum_seq_lens_q must have rank 1",
    ),
    (
        "bmm1-scale-inf",
...
_ROPE_SHARED_INVALID_ARGUMENTS = (
    ...
    (
        ...
        lambda fx: dict(
            out=torch.empty(
                rope_quant_output_shapes(fx.case.total_q, ue8m0=fx.case.ue8m0)[0],
                dtype=torch.bfloat16,
                device=fx.query.device,
            )
        ),
    ...
    (
        ...
        lambda fx: dict(
            out_scale=torch.empty(
                rope_quant_output_shapes(fx.case.total_q, ue8m0=False)[1],
                dtype=torch.bfloat16,
                device=fx.query.device,
            )
        ),
...
_UE8M0_SCALE_INVALID_ARGUMENTS = (
    (
        "out-scale-dtype",
        # The FP32 scale dtype is not accepted by the UE8M0 entry point.
        lambda fx: dict(
            out_scale=torch.empty(
                rope_quant_output_shapes(fx.case.total_q, ue8m0=True)[1],
                dtype=torch.float32,
                device=fx.query.device,
            )
        ),
