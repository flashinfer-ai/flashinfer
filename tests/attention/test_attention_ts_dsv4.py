...
"""Correctness coverage for PrimTS DSV4 sparse MLA (CSA / HCA) kernels.

Config/scheduler contracts, Contract-F kernel regressions, comparisons with
independent PyTorch references, CUDA graph replay and input validation.

Fixtures use the physical routing contract: 128 SWA slots then compressed
slots, ``-1`` inactive sentinels, per-token scan widths, raw KV lengths and
packed Q offsets. Flat ``[N, 512]`` row pools represent a page size of one.

The references model exact FP32 softmax and the tiled E4M3 online-softmax
protocol, including disabled and enabled skip correction. Expected results
are computed with PyTorch operations without invoking another backend.
"""
...
def test_dsv4_async_config_matches_generated_reference() -> None:
    ...
    # Skip correction is a generic throughput-2CTA specialization, not a DSV4
    # one: dense BF16 and dense E4M3 configs accept it.  Only the threshold
    # bound depends on the P dtype (E4M3: 1.75 * 2**8 == 448; BF16: cap 64).
...
def _reference_contract_f(
    ...
    """Model the tiled E4M3 online-softmax protocol with PyTorch operations.
...
def test_prims_ts_dsv4_sparse_mla_runtime_launch_shape_reuses_compilation() -> None:
    """Keep packed B/T/maxQ/Kmax as runtime launch parameters.
...
def test_prims_ts_dsv4_hca_r128_padded_workids_preserve_output(
    ...
    The static grid gives every padded ``B x maxQ`` row its own cluster with an
    empty K domain; the CLC variant advances the WorkId/throttle protocol.

    Compression ratio is an upstream selector property rather than a kernel
    argument.  This fixture nevertheless uses the canonical HCA R128 page-index layout
...
def _expected_rope_quant(blocks: torch.Tensor, *, ue8m0: bool):
    """Return ``(scale, dequant)`` for FP32 ``[T, H, 4, 128]`` blocks.

    FP32 scales are ``max(amax, 1e-12) / 448``; UE8M0 scales are the power of
    two ``exp2(ceil(log2(max(amax, 1e-10) / 448)))`` (``RopeQuantUe8m0Sf`` /
    vLLM ``fused_inv_rope_fp8_quant``).
    """
...
@pytest.mark.arch_blackwell
@_REQUIRES_PRIMTS_GPU
def test_prims_ts_dsv4_sparse_mla_source_sparse_length_pair_boundaries(
...
def test_prims_ts_dsv4_sparse_mla_bf16_output_satfinite() -> None:
    """Saturate the BF16 epilogue when runtime scaling overflows.
...
def test_prims_ts_dsv4_sparse_mla_output_scale_ftz() -> None:
    ...
    # representable non-zero BF16 result.  The kernel flushes it before that
    # vector multiply.
...
# PrimTS vs the tiled FP32 protocol reference (matches the existing
# PrimTS regression bound).
...
# RopeQuant dequantized values against the quantized PyTorch reference.
_TOL_ROPE_DEQUANT_VS_REF = dict(atol=1e-2, rtol=1e-1)
...
# Coverage matched to the dense PrimTS MLA decode suite: 160/320-request
# batches (direct vs persistent grid) and long packed contexts.
LARGE_CASES = (
    Case(
...
def test_prims_ts_dsv4_sparse_mla_static_and_persistent_schedules_match(
    ...
    """Static2Cta and Persistent2Cta must agree bitwise on O, LSE, and scales.

    Uneven request lengths leave padded work tiles in the ``B x maxQ`` grid and
    truncated scan widths vary the K domain per token; the static grid has to
    skip and bound those tiles exactly as the CLC WorkId protocol does.
    """
