...
def _fp8_request_reference(
    ...
    *,
    ...
    """Model E4M3 probabilities across split-local KV instruction streams.

    With ``skip_corr_threshold == 0`` this is the exact P448 protocol.  A
    positive threshold models the kernel's skip-correction contract: the P
    scale drops to 1.75 and a stream's row max is frozen while a tile raises
    it by at most ``threshold / (bmm1_scale * log2(e))`` raw score units.
    """
...
def test_attention_ts_mla_decode_skip_correction(
    ...
    """Threshold skip correction matches the protocol oracle on both kernels.

    BF16 P is unscaled, so the frozen-max protocol is algebraically exact and
    the exact softmax oracle applies at the usual tolerance up to the dtype
    bound of 64.  E4M3 P switches to the 1.75 scale, which the FP8 oracle
    models explicitly per KV instruction stream.  The knob is independent of
    the kernel family: the automatic policy still picks 1CTA or 2CTA by shape,
    and both entry points must accept it and agree.
    """
