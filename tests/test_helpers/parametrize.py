...
def parametrize_product(
    ...
    *,
    ...
    """Parametrize a dictionary's product, or its full product under ``--full``.

    A key may name several arguments (for example, ``"dtype,output_dtype"``);
    each entry in that axis must then provide one value for every named argument.
    """
    parameter_items = tuple(parameters.items())
    argname_groups = tuple(
        tuple(name.strip() for name in group.split(",")) for group, _ in parameter_items
    )
    names = tuple(name for group in argname_groups for name in group)
    axis_values = tuple(tuple(axis) for _, axis in parameter_items)
