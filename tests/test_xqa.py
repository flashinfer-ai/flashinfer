...
class CacheSeq:
    ...
    def __init__(self, pool: torch.Tensor, page_indices: torch.Tensor,
                 nb_heads: int, idx_head: int, tokens_per_page: int = 32):
    ...
    def __getitem__(self, i: int) -> torch.Tensor:
        ...
        idx_head = (self.tokens_per_page * self.nb_heads * page_idx +
...
def ref_attention(q, k_cache_seq, v_cache_seq, seq_len, q_scale, kv_scale, x_scale, attention_sinks, sliding_win_size, valid_elems_per_head):
    ...
    k_cache_f32 = torch.zeros(seq_len, valid_elems_per_head, dtype=torch.float32, device="cuda")
    v_cache_f32 = torch.zeros(seq_len, valid_elems_per_head, dtype=torch.float32, device="cuda")
...
def test_xqa(batch_size, nb_k_heads, seq_len, tokens_per_page, use_fp16, valid_elems_per_head, head_grp_size, use_attention_sinks, use_sliding_window):
    ...
    output = torch.zeros(batch_size, beam_width, nb_q_heads, valid_elems_per_head, dtype=torch.bfloat16 if not use_fp16 else torch.float16, device="cuda")
    ...
    cache_heads = torch.zeros(total_nb_cache_heads, valid_elems_per_head, dtype=torch.bfloat16 if not use_fp16 else torch.float16, device="cuda")
    ...
    page_list_arg = torch.zeros(batch_size, beam_width, 2, nb_pages_per_seq, dtype=torch.uint32, device="cuda")
    page_list_arg.view(-1)[:total_nb_pages] = torch.arange(total_nb_pages, dtype=torch.int32, device="cuda").to(torch.uint32)
    ...
    def cache_head_at(batch, is_k, idx_kv_head, pos, cache_heads, page_list,
                  beam_width, nb_k_heads, tokens_per_page):
        beam = 0
        kv = 0 if is_k else 1

        page_idx = page_list_arg[batch][beam][kv][pos // tokens_per_page].to(torch.int32)

        idx_head = tokens_per_page * (nb_k_heads * page_idx + idx_kv_head) + pos % tokens_per_page
            ...
            for idx_kv_head in range(nb_k_heads):
                for pos in range(seq_len, max_seq_len):
                    cache_head = cache_head_at(
                        batch, kv == 0, idx_kv_head, pos,
                        cache_heads, page_list_arg, beam_width,
                        nb_k_heads, tokens_per_page
                    )
                    cache_head.fill_(0.0)

    seq_len_list = torch.zeros(batch_size, beam_width, dtype=torch.uint32, device="cuda")
    ...
    for req in range(batch_size):
        ...
        ref_output = ref_attention(
            q=q_heads[req][b][idx_k_head*head_grp_size:(idx_k_head+1)*head_grp_size],
