    ...
    try:
        ...
        if world_size == 2:
            with pytest.raises(RuntimeError, match="workspace layout overflows"):
                get_pcie_ipc_ag_rs_module().all_gather_workspace_size(
                    2, 1 << 62, 4, 64, False
                )
...
def _mismatched_collective_worker(world_size: int, rank: int, port: int) -> None:
    _init_process_group(world_size, rank, port)
    try:
        workspace_type = (
            comm.PcieIpcAllGatherWorkspace
            if rank == 0
            else comm.PcieIpcReduceScatterWorkspace
        )
        with pytest.raises(
            ValueError, match=r"identical collective arguments.*collective"
        ):
            workspace_type(
                dist.group.WORLD,
                max_numel=1024,
...
def test_pcie_ipc_mismatched_ag_rs_construction_fails() -> None:
    if torch.cuda.device_count() < 2:
        pytest.skip("not enough GPUs")
    multi_process_parallel(2, _mismatched_collective_worker, timeout_s=60)
