"""Distributed correctness coverage for SM100/SM103 MoE all-reduce."""
...
# The public pointer table stores its local control pointer after three
# world-sized pointer regions; the Lamport phase is raw int32 control[2].
PUBLIC_WORKSPACE_CONTROL_INDEX_FACTOR = 3
PUBLIC_WORKSPACE_PHASE_I32_OFFSET = 2
PUBLIC_POINTER_PHASE_TRACE = (0, 1, 2, 0)
...
def _read_public_pointer_phase(
    workspace_ptrs: torch.Tensor,
    world_size: int,
    cuda_runtime: comm.CudaRTLibrary,
) -> int:
    control_ptr = int(
        workspace_ptrs[PUBLIC_WORKSPACE_CONTROL_INDEX_FACTOR * world_size].item()
    )
    phase = ctypes.c_int32()
    cuda_runtime.cudaMemcpy(
        ctypes.cast(ctypes.pointer(phase), ctypes.c_void_p),
        ctypes.c_void_p(
            control_ptr
            + PUBLIC_WORKSPACE_PHASE_I32_OFFSET * ctypes.sizeof(ctypes.c_int32)
        ),
        ctypes.sizeof(phase),
    )
...
def _run_eager_public_pointer_phase_probe(
    call: Callable[[], None],
    *,
    label: str,
    workspace_ptrs: torch.Tensor,
    ...
    rank: int,
    group: dist.ProcessGroup,
    output_checks: tuple[tuple[str, torch.Tensor, torch.Tensor], ...],
) -> tuple[int, ...]:
    assert world_size == 8, "the public pointer phase probe is a TP8 gate"
    device = workspace_ptrs.device
    cuda_runtime = comm.CudaRTLibrary()
    local_phases: list[int] = []

    for replay_index, expected_phase in enumerate(PUBLIC_POINTER_PHASE_TRACE):
        torch.cuda.synchronize(device)
        dist.barrier(group=group)
        torch.cuda.synchronize(device)
        observed_phase = _read_public_pointer_phase(
            workspace_ptrs,
            world_size,
            cuda_runtime,
        )
        local_phases.append(observed_phase)

        phase_tensor = torch.tensor([observed_phase], dtype=torch.int32, device=device)
        phases_by_rank = [torch.empty_like(phase_tensor) for _ in range(world_size)]
        dist.all_gather(phases_by_rank, phase_tensor, group=group)
        torch.cuda.synchronize(device)
        observed_by_rank = tuple(int(phase.item()) for phase in phases_by_rank)
        expected_by_rank = (expected_phase,) * world_size
        if observed_by_rank != expected_by_rank:
            raise AssertionError(
                f"{label}/replay{replay_index} public pointer phase mismatch: "
                f"expected={expected_by_rank}, observed={observed_by_rank}"
            )

        for _, actual, _ in output_checks:
            actual.fill_(float("nan"))
        call()
        torch.cuda.synchronize(device)
        for output_label, actual, expected in output_checks:
            _assert_distributed_close(
                actual,
                expected,
                label=f"{label}/replay{replay_index}/{output_label}",
                group=group,
            )

    observed_trace = tuple(local_phases)
    if observed_trace != PUBLIC_POINTER_PHASE_TRACE:
        raise AssertionError(
            f"{label} public pointer phase trace mismatch: "
            f"expected={PUBLIC_POINTER_PHASE_TRACE}, observed={observed_trace}"
        )
    print(
        f"CAKE_PUBLIC_POINTER_PHASE label={label} world_size={world_size} "
        f"rank={rank} phases={list(observed_trace)} all_ranks_consistent=True",
        flush=True,
    )
    return observed_trace


def _run_reduction_eager_public_pointer_phase_probe(
    call: Callable[[], None],
    *,
    workspace_ptrs: torch.Tensor,
    world_size: int,
    rank: int,
    group: dist.ProcessGroup,
    moe_allreduce_out: torch.Tensor,
    allreduce_ref: torch.Tensor,
    residual_out: torch.Tensor,
    residual_ref: torch.Tensor,
    norm_out: torch.Tensor,
    norm_ref: torch.Tensor,
) -> tuple[int, ...]:
    return _run_eager_public_pointer_phase_probe(
        call,
        label="reduction/tp8/public_pointer_phase",
        workspace_ptrs=workspace_ptrs,
        world_size=world_size,
        rank=rank,
        group=group,
        output_checks=(
            ("moe_allreduce_out", moe_allreduce_out, allreduce_ref),
            ("residual_out", residual_out, residual_ref),
            ("norm_out", norm_out, norm_ref),
        ),
    )
                ...
                for launch_with_pdl in (False, True):
                    for mode in ("eager", "graph"):
                        ...
                        if (
                            ...
                            assert moe_allreduce_out is not None
                            _run_reduction_eager_public_pointer_phase_probe(
                                call,
                                workspace_ptrs=workspace_tensor,
                                world_size=world_size,
                                rank=rank,
                                group=group,
                                moe_allreduce_out=moe_allreduce_out,
                                allreduce_ref=allreduce_ref_f32,
                                residual_out=residual_out,
                                residual_ref=residual_ref,
                                norm_out=norm_out,
                                norm_ref=norm_ref,
                            )
                            phase_probe_complete = True
...
def _run_distributed(
    ...
    processes = [
        context.Process(
            ...
            name=f"cake-moe-allreduce-rank-{rank}",
        )
    ...
    assert not failures, f"distributed Cake MoE all-reduce workers failed: {failures}"
...
@pytest.mark.parametrize(
    "world_size,dtype",
    _DISTRIBUTED_CASES,
    ids=("tp2-fp16", "tp2-bf16", "tp4-fp16", "tp4-bf16", "tp8-fp16", "tp8-bf16"),
)
