...
def test_mla_dispatch_requires_compatible_decode_plan():
    ...
    dispatcher = _trace_dispatcher_for(original)
    ...
    decode = dispatcher(**namespace)
    assert decode.name_prefix == "mla_paged_decode"
    namespace = decode.normalize_kwargs(namespace)
    assert namespace["q_nope"] is q_nope
    assert namespace["q_pe"] is q_pe

    wrapper._causal = True
    assert dispatcher(**{"self": wrapper, "q_nope": torch.empty(4, 2, 8)}) is None
    ...
    wrapper._all_query_lengths_one = True
    assert dispatcher(**{"self": wrapper, "q_nope": torch.empty(7, 2, 8)}) is None

    wrapper._all_query_lengths_one = None
    assert dispatcher(**{"self": wrapper, "q_nope": torch.empty(4, 2, 8)}) is None

    direct = dispatcher(**{"q_nope": torch.empty(4, 2, 8)})
...
@pytest.mark.parametrize("structural_inputs", [False, True])
def test_mla_decode_solution_falls_back_for_prefill(monkeypatch, structural_inputs):
    ...
    def cpu_original(
        self, q_nope=None, q_pe=None, ckv_cache=None, kpe_cache=None, **kwargs
    ):
        if q_nope is None:
            q_nope = kwargs["query"][0]
    ...
    def call_run(total_q):
        q_nope = torch.empty(total_q, 2, 4)
        q_pe = torch.empty(total_q, 2, 2)
        if structural_inputs:
            return BatchMLAPagedAttentionWrapper.run(
                wrapper, query=(q_nope, q_pe), kv_cache=(ckv, kpe)
            )
        return BatchMLAPagedAttentionWrapper.run(wrapper, q_nope, q_pe, ckv, kpe)
