...
class DeepseekV2AttentionVanilla(nn.Module):
    ...
    def __init__(self):
        ...
        self.q_b_proj = nn.Linear(
            self.q_lora_rank, 
            self.num_heads * self.q_head_dim, bias=False
        )
    ...
    def run_decode(
        ...
        if kpe_bsz != bsz or kpe_dim != self.qk_rope_head_dim or kv_len != kpe_len:
            raise ValueError(
                f"Unexpected shape: k_pe_cache[{k_pe_cache.size()}]"
            )
...
class DeepseekV2AttentionMatAbsorbDecode(nn.Module):
    ...
    def __init__(self, mla_vanilla: DeepseekV2AttentionVanilla):
        ...
        self.q_head_dim = mla_vanilla.q_head_dim # qk_nope_head_dim + qk_rope_head_dim # 128+64=192 
        ...
        # W_UQ ~ [1536, 128, 128]
        W_UQ, W_QR = torch.split(mla_vanilla.q_b_proj.weight.t().view(self.q_lora_rank, self.num_heads, self.q_head_dim), 
        ...
        W_UK, W_UV = torch.split(mla_vanilla.kv_b_proj.weight.t().view(self.kv_lora_rank, self.num_heads, self.qk_nope_head_dim+self.v_head_dim), 
    ...
    def run_proof_of_concept(
        ...
        if not use_flashinfer_kernel:
            ...
            attn_weights_pe = torch.matmul(
                    ...
                    k_pe_cache.transpose(1, 2) # [bsz, kv_len, 64] view(bsz, kv_len, self.qk_rope_head_dim)
            )
...
if __name__ == "__main__":
    ...
    compressed_kv_normed_cache = torch.randn([bsz, kv_len, mla_vanilla.kv_lora_rank]).to(dev_id)
