...
@pytest.mark.parametrize("device", CUDA_DEVICES)
@torch.inference_mode()
def test_fp4_quantize_backend_parity_non_swizzled(
    dtype: torch.dtype,
    backend: str,
    device: str,
) -> None:
    """Test CUDA and CuTe-DSL produce matching results for linear layout padding cases."""
    if not _is_fp4_supported(torch.device(device)):
        pytest.skip("Nvfp4 Requires compute capability >= 10 and CUDA >= 12.8")
    if backend == "cute-dsl" and not _is_cute_dsl_available():
        pytest.skip("CuTe-DSL not available")
    if backend == "cuda":
        pytest.skip("Parity test only runs for cute-dsl")
    torch.set_default_device(device)
    torch.manual_seed(42)

    # Shapes that stress padding: unaligned M and small K with column padding
    shapes = [
        (17, 512),
        (33, 1024),
        (1, 512),
    ...
    for m, n in shapes:
        ...
        out_cuda, sf_cuda = fp4_quantize(
            x, global_scale, sf_vec_size, False, False, backend="cuda"
        )
        out_cute, sf_cute = fp4_quantize(
            x, global_scale, sf_vec_size, False, False, backend="cute-dsl"
        )

        assert out_cuda.shape == out_cute.shape, (
            f"Shape mismatch at ({m},{n}): {out_cuda.shape} vs {out_cute.shape}"
        )
        assert sf_cuda.shape == sf_cute.shape, (
            f"SF shape mismatch at ({m},{n}): {sf_cuda.shape} vs {sf_cute.shape}"
        )

        quant_match = (out_cuda == out_cute).float().mean().item() * 100
        scale_match = (sf_cuda == sf_cute).float().mean().item() * 100
        assert quant_match > 95.0, f"({m},{n}) quant match {quant_match:.1f}% < 95%"
        assert scale_match > 95.0, f"({m},{n}) scale match {scale_match:.1f}% < 95%"
