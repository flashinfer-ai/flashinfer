...
from flashinfer.utils import is_sm100a_supported, is_sm110a_supported, is_sm12x_supported
...
def test_scale_swizzling(
    ...
    if not (is_sm100a_supported(torch.device("cuda")) or is_sm110a_supported(torch.device("cuda")) or is_sm12x_supported(torch.device("cuda"))):
        pytest.skip("Nvfp4 Requires compute capability >= 10 and CUDA >= 12.8")
    torch.set_default_device(device)
    torch.manual_seed(seed)
    m, n = shape
