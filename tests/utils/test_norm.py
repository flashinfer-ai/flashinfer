...
@pytest.mark.parametrize("enable_pdl", [True, False])
@pytest.mark.parametrize("contiguous", [True, False])
def test_weightless_norm(
    batch_size, hidden_size, dtype, specify_out, enable_pdl, contiguous
):
    """Test RMSNorm without normalization weights."""
    if contiguous:
    ...
    if specify_out:
        y = torch.empty_like(x)
        flashinfer.norm.rmsnorm(x, None, out=y, enable_pdl=enable_pdl)
    else:
...
@pytest.mark.parametrize("batch_size", [1, 19])
@pytest.mark.parametrize("num_heads", [4, 7])
@pytest.mark.parametrize("head_dim", [64, 128])
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
@pytest.mark.parametrize("enable_pdl", [True, False])
def test_weightless_qknorm(batch_size, num_heads, head_dim, dtype, enable_pdl):
    """Test QK RMSNorm without normalization weights."""
    x = torch.randn(batch_size, num_heads, head_dim, dtype=dtype, device="cuda")
...
def test_weightless_fused_add_rmsnorm(batch_size, hidden_size, dtype, enable_pdl):
    """Test fused add RMSNorm without normalization weights."""
    eps = 1e-6
    x = torch.randn(batch_size, hidden_size, dtype=dtype, device="cuda")
