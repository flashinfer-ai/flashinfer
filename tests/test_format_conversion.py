...
import pytest
import numpy as np
from flashinfer.format_conversion import pad_ragged_tensor_to_multiple_of


def pad_ragged_tensor_to_multiple_of_pytorch(ragged_tensor, indptr, multiple_of):
    ...
    for i in range(n_rows):
        ...
        padded_ragged_tensor[padded_row_start:padded_row_start+row_length] = ragged_tensor[row_start:row_end]
...
def test_pad_ragged_tensor_to_multiple_of(dtype, n_rows, dim, multiple_of):
    ...
    padded_ragged_tensor_ref, padded_indptr_ref = pad_ragged_tensor_to_multiple_of_pytorch(
        ragged_tensor, indptr, multiple_of
    )
    ...
    # Check tensor values
    assert torch.allclose(padded_ragged_tensor, padded_ragged_tensor_ref, rtol=1e-3, atol=1e-3)
