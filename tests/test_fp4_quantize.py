...
@pytest.mark.parametrize("batch_shape", [(2, 128, 64), (1, 256, 128), (3, 111, 64)])
...
def test_silu_and_mul_fp4_batched_quantize(
    ...
    global_scale = FLOAT8_E4M3_MAX * FLOAT4_E2M1_MAX / tensor_amax
    # ref_output, ref_output_scales = nvfp4_batched_quantize(
    #     ref_y,
    #     global_scale,
    # )
    # Test the silu and mul + batched quantization
    

    out, out_scale = silu_and_mul_fp4_batched_quantize(x, mask, global_scale)
    # print(mask)
    # print(f"out: {out}, {out.size()}")
    # print(f"ref_output: {ref_output}, {ref_output.size()}")
    ...
    for i in range(b):
        ...
        torch.testing.assert_close(out[i], single_out, rtol=1e-5, atol=1e-5)
        # print(f"out_scale: {out_scale[i].view(128, 4)}, {out_scale[i].size()}")
        # print(f"ref_output_scales: {ref_output_scales[i]}, {ref_output_scales[i].size()}")
        # print(f"single_scale: {single_scale.flatten().view(128, 4)}, {single_scale.flatten().size()}")
        torch.testing.assert_close(
            out_scale[i], single_scale.flatten(), rtol=1e-5, atol=1e-5
        )
