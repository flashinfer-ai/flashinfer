...
def test_top_k_top_p_sampling_reference_correctness(shape_kwargs):
    ...
    inputs = top_k_top_p_sampling_trace.init(**shape_kwargs)
    probs = inputs["probs"]
    B, V = probs.shape
    ...
    _check(top_k_top_p_sampling_trace, ref, api.to(torch.int64), atol=0.0, rtol=0.0)
