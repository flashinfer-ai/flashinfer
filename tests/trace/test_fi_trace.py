...
def test_pcie_ipc_collective_bound_trace(collective, world_size, dtype, provide_out):
    ...
    workspace_type = (
        ...
        else PcieIpcReduceScatterWorkspace
    )
    # Exercise the actual decorated method without CUDA IPC initialization.
    workspace = object.__new__(workspace_type)
    workspace._world_size = world_size
    local_rows, hidden = 8, 64
    input_rows, output_rows = (
        (local_rows, local_rows * world_size)
        if collective == "all_gather"
        else (local_rows * world_size, local_rows)
    )
    inp = torch.empty(input_rows, hidden, dtype=dtype)
    kwargs = {"inp": inp}
    if provide_out:
        kwargs["out"] = torch.empty(output_rows, hidden, dtype=dtype)
    defn = fi_trace(getattr(workspace, collective), **kwargs)

    _check_defn(defn, "comm", f"{workspace_type.__name__}.{collective}")
    assert defn["name"] == f"pcie_ipc_{collective}_tp{world_size}_h{hidden}"
    assert defn["axes"]["world_size"]["value"] == world_size
    assert defn["axes"]["hidden_dim"]["value"] == hidden
    assert defn["constraints"] == ["total_rows == world_size * local_rows"]
    sizes = {
        "local_rows": local_rows,
        "total_rows": world_size * local_rows,
        "hidden_dim": hidden,
    }
    assert tuple(sizes[dim] for dim in defn["inputs"]["inp"]["shape"]) == inp.shape
    output = defn["outputs"]["output"]
    assert tuple(sizes[dim] for dim in output["shape"]) == (output_rows, hidden)
    assert output["param"] == "out"
    assert output["dtype"] == defn["inputs"]["inp"]["dtype"] == str(dtype)[6:]
    assert "reference" not in defn and "init" not in defn


@pytest.mark.parametrize("collective", ["all_gather", "reduce_scatter"])
def test_pcie_ipc_collective_trace_requires_bound_workspace(collective):
    from flashinfer.comm import (
        PcieIpcAllGatherWorkspace,
        PcieIpcReduceScatterWorkspace,
    )
    ...
    with pytest.raises(ValueError, match="requires a bound workspace"):
        fi_trace(getattr(workspace_type, collective), inp=torch.empty(8, 64))
