...
def test_native_finalize_live_capture(tokens, hidden, top_k, rounded):
    _require_blackwell()
    rows = tokens * top_k + 7
...
@pytest.mark.parametrize(
    "bad", ["pdl", "expanded", "wide", "weights", "indices", "alignment", "vector_tail"]
)
def test_native_finalize_rejects_invalid_metadata(bad):
    _require_blackwell()
    tokens, hidden, top_k = (148, 2057, 8) if bad == "vector_tail" else (1, 2048, 8)
