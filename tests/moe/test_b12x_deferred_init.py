...
class TestDeferredInit:
    ...
    def test_forced_full_clear_and_deferred_off_agree_bitwise(self, monkeypatch):
        ...
        monkeypatch.setenv(md._STATIC_DEFERRED_INIT_ENV, "0")
        assert md._static_deferred_init() is False
        moe_off = _wrapper()
...
def test_environment_override_parsing(monkeypatch):
    from flashinfer.fused_moe.cute_dsl.blackwell_sm12x import moe_dispatch as md

    monkeypatch.delenv(md._STATIC_DEFERRED_INIT_ENV, raising=False)
    assert md._static_deferred_init() is True
    floor = md._STATIC_DEFERRED_INIT_MIN_PAIRS
    assert md._static_deferred_init(floor)
    assert not md._static_deferred_init(floor - 1)
    for value, expected in (
        ...
        monkeypatch.setenv(md._STATIC_DEFERRED_INIT_ENV, value)
        assert md._static_deferred_init() is expected, value
    assert os.environ.get(md._STATIC_DEFERRED_INIT_ENV) == ""
