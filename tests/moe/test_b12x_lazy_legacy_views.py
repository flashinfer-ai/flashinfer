...
@pytest.mark.parametrize("n", [256, 320, 384])
def test_operand_view_helpers_preserve_layout(branches, sf_vec_size, n):
    """Shared preparation preserves byte values and aliases at every extent."""
    ...
    def scale(rows, cols):
        rows_pad = md._align_up(rows, 128)
        cols_pad = md._align_up(cols // sf_vec_size, 4)
        storage = torch.randint(
            0, 128, (experts * rows_pad, cols_pad), dtype=torch.uint8
        )
        return convert_sf_to_mma_layout(
            storage, m=rows, k=cols, num_groups=experts, sf_vec_size=sf_vec_size
        )

    scales = (scale(branches * n, k), scale(k, n))
    actual = md._convert_weight_scales(
        *scales, n=n, k=k, branches=branches, experts=experts, sf_vec_size=sf_vec_size
    )
    for source, converted, (rows, cols) in zip(
        scales, actual, [(branches * n, k), (k, n)], strict=True
    ):
        expected = convert_sf_from_mma_layout(
            source, m=rows, k=cols, num_groups=experts, sf_vec_size=sf_vec_size
        ).contiguous()
        assert converted.shape == expected.shape
        assert converted.stride() == expected.stride()
        assert torch.equal(converted, expected)
        assert converted.data_ptr() == source.data_ptr()

    copied_scales = tuple(source.contiguous() for source in scales)
    converted_copies = md._convert_weight_scales(
        *copied_scales,
        n=n,
        k=k,
        branches=branches,
    ...
    for source, converted, (rows, cols) in zip(
        copied_scales, converted_copies, [(branches * n, k), (k, n)], strict=True
    ):
        expected = convert_sf_from_mma_layout(
            source, m=rows, k=cols, num_groups=experts, sf_vec_size=sf_vec_size
        ).contiguous()
    ...
    weights = (
        torch.zeros((experts, branches * n, k // 2), dtype=torch.uint8),
        torch.zeros((experts, k, n // 2), dtype=torch.uint8),
    )
    for source, actual_view in zip(
        weights, md._legacy_fp4_views(*weights), strict=True
    ):
        expected = source.permute(1, 2, 0).view(torch.float4_e2m1fn_x2)
        assert actual_view.shape == expected.shape
        assert actual_view.stride() == expected.stride()
        assert actual_view.dtype == expected.dtype
        assert actual_view.data_ptr() == source.data_ptr()
        source[0, 0, 0] = 42
        assert torch.equal(actual_view.view(torch.uint8), source.permute(1, 2, 0))
...
class TestLazyLegacyViews:
    ...
    def test_static_then_dynamic_on_one_wrapper(self):
        ...
        source_ptrs = tuple(
            t.data_ptr()
            for t in (
                views.static_w13_fp4,
                views.static_down_fp4,
                views._w13_sf_storage,
                views._down_sf_storage,
            )
        )
        ...
        padded_ptrs = (views.w13_fp4.data_ptr(), views.down_fp4.data_ptr())
        returned = moe.run(**small)
        torch.cuda.synchronize()
        assert moe._weight_views is views
        assert source_ptrs == tuple(
            t.data_ptr()
            for t in (
                views.static_w13_fp4,
                views.static_down_fp4,
        ...
        assert padded_ptrs == (views.w13_fp4.data_ptr(), views.down_fp4.data_ptr())
        assert check_accuracy(returned, ref[:64])[0]
