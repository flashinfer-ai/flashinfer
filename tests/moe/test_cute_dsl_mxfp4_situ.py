...
def _compiled_counts():
    return tuple(len(getattr(importlib.import_module("flashinfer.fused_moe.cute_dsl." + module), cache)) for module, cache in (
        ("blockscaled_contiguous_gather_grouped_gemm_act_fusion", "_gather_kernel_cache"),
        ("blockscaled_contiguous_grouped_gemm_finalize_fusion", "_finalize_kernel_cache"),
    ))
...
def prepare_candidate(case, packed=False, prepared_weights=None):
    ...
    wrapper = CuteDslMxfp4MoEWrapper(
        case.num_experts, case.topk_ids.shape[1], case.hidden_size, case.intermediate_size,
        num_local_experts=case.local_num_experts, local_expert_offset=case.local_expert_offset,
        enable_pdl=False,
    )
    weights = prepare_cute_weights(case) if prepared_weights is None else prepared_weights
    output = torch.empty(case.x.shape, device=case.x.device, dtype=torch.bfloat16)
    workspace = torch.empty(wrapper.get_workspace_size(case.x.shape[0]), device=case.x.device, dtype=torch.uint8)
    ...
    plan = wrapper.plan(
        case.x, case.x_scale, ids, route_weights, *weights,
        beta=case.beta, linear_beta=case.linear_beta, workspace=workspace, output=output,
    )
...
def test_error_quantiles_above_torch_limit():
    ...
    expected = torch.tensor([.5, .95, .99], device="cuda", dtype=torch.float64) * (count - 1)
    torch.testing.assert_close(absolute_error_quantiles(values), expected, atol=0, rtol=0)
...
def test_e2m1_and_ue8m0_encoding():
    ...
    expected = torch.tensor([0., .5, 1., 1.5, 2., 3., 4., 6., -0., -.5, -1., -1.5, -2., -3., -4., -6.], dtype=torch.float64).repeat(2).reshape(1, 32)
    torch.testing.assert_close(actual, expected, atol=0, rtol=0)
    scale = decode_ue8m0(torch.tensor([0, 126, 127, 128, 254, 255], dtype=torch.uint8))
    torch.testing.assert_close(scale[:5], torch.tensor([2.**-127, .5, 1., 2., 2.**127], dtype=torch.float64), atol=0, rtol=0)
...
def test_fp64_oracle_sparse_exact_case(linear_beta):
    ...
    w2[0, 0, 0] = 2
    case = MXFP4SiTUCase(
        x.to(torch.float8_e4m3fn), torch.full((1, 1), 127, dtype=torch.uint8),
        w1, torch.full((1, 64, 1), 127, dtype=torch.uint8),
        w2, torch.full((1, 32, 1), 127, dtype=torch.uint8),
        torch.tensor([[5]], dtype=torch.int32), torch.tensor([[.5]], dtype=torch.bfloat16),
        torch.tensor([2.5]), None if linear_beta is None else torch.tensor([linear_beta]),
        8, 5,
    )
    out = reference_moe(case, modes=("ideal_fp64",))["ideal_fp64"]
    expected = torch.zeros_like(out)
    expected[0, 0] = situ_reference(torch.tensor(1., dtype=torch.float64), torch.tensor(2., dtype=torch.float64), 2.5, linear_beta) * .5
...
def test_situ_paired_fp64_report(tokens, distribution, record_property):
    ...
    report = {name: paired_accuracy(output, baseline_output, ref) for name, ref in references.items()}
...
def test_native_weight_preparation_is_lossless():
    ...
    canonical_w1 = w1.reshape(e, rows // 128, 2, 64, half_k).transpose(1, 2).reshape_as(case.w1)
    ...
    canonical_sf1 = linear1.reshape(e, rows // 128, 2, 64, half_k // 16).transpose(1, 2).reshape_as(case.w1_scale)
    linear2 = sf2.permute(5, 2, 1, 0, 4, 3).contiguous().reshape_as(case.w2_scale)
    torch.testing.assert_close(canonical_sf1.cpu().view(torch.uint8), case.w1_scale, atol=0, rtol=0)
    torch.testing.assert_close(linear2.cpu().view(torch.uint8), case.w2_scale, atol=0, rtol=0)
...
def test_scalar_and_per_expert_runtime_betas():
    _require_blackwell()
    case = make_case(tokens=16, distribution="hot", beta=2.5, linear_beta=7.)
    scalar_case = replace(case, beta=case.beta[:1].clone(), linear_beta=case.linear_beta[:1].clone())
    per_expert, output, _ = prepare_candidate(case)
    scalar, scalar_output, _ = prepare_candidate(scalar_case)
    compiled_before = _compiled_counts()
    for beta, linear_beta in ((2.5, 7.), (4., 25.), (7., 3.)):
        case.beta.fill_(beta)
...
@pytest.mark.parametrize("input_distribution", ["zero", "small", "outliers", "saturated"])
@pytest.mark.parametrize("linear_beta", [None, 7.0, 25.0])
def test_situ_runtime_parameters(input_distribution, linear_beta, record_property):
    _require_blackwell()
    case = make_case(tokens=16, distribution="hot", input_distribution=input_distribution,
                     beta=2.5, linear_beta=linear_beta)
    plan, output, _ = prepare_candidate(case)
    plan.run()
    first = output.clone()
...
def _check_concurrent_caller_streams(cases, *, repeats, noise_size, prepared_weights=None):
    prepared = [prepare_candidate(case, prepared_weights=prepared_weights) for case in cases]
    ...
    for iteration in range(repeats):
        for index, (stream, (plan, output, _)) in enumerate(zip(streams, prepared)):
    ...
    for (_, output, _), eager in zip(prepared, expected):
        torch.testing.assert_close(output, eager, atol=1e-2, rtol=1e-2)
...
def full_kimi_case():
    ...
    return make_case(tokens=2048, hidden=7168, intermediate=3072, num_experts=896,
                     local_num_experts=112, local_expert_offset=336, top_k=16)
...
def test_full_kimi_paired_fp64(full_kimi_case, tokens, distribution, record_property):
    ...
    case = replace(base, x=base.x[:tokens], x_scale=base.x_scale[:tokens], topk_ids=ids, topk_weights=weights)
    plan, output, _ = prepare_candidate(case)
    baseline, baseline_output = make_trt_baseline(case)
    plan.run()
    baseline()
    refs = reference_moe(case)
    report = {name: paired_accuracy(output, baseline_output, ref) for name, ref in refs.items()}
    report["routing"] = routing_histogram(case)
    record_property("numerical_report", json.dumps(report))
    assert report["ideal_fp64"]["candidate"]["finite"]
    assert report["ideal_fp64"]["baseline"]["finite"]
    ref = refs["ideal_fp64"]
    floor = torch.linalg.vector_norm(ref.to(torch.bfloat16).double() - ref)
    assert torch.linalg.vector_norm(output.double() - ref) <= torch.linalg.vector_norm(baseline_output.double() - ref) + floor, report
...
def test_full_kimi_concurrent_caller_streams(full_kimi_case):
    base = full_kimi_case
    cases = []
    for tokens, distribution, beta in ((16, "hot", 2.5), (128, "empty", 4.), (2048, "balanced", 7.)):
        ids, weights = make_routing(tokens, 896, 16, 112, 336, distribution)
        cases.append(replace(
            base, x=base.x[:tokens], x_scale=base.x_scale[:tokens],
            topk_ids=ids, topk_weights=weights,
            beta=torch.full_like(base.beta, beta),
        ))
    _check_concurrent_caller_streams(
        cases, repeats=100, noise_size=2048,
