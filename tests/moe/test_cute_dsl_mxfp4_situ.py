...
def prepare_candidate(
    ...
    plan = wrapper.plan(
        ...
        *weights,
        ...
        paired_decode_weights=paired_decode_weights,
        workspace=workspace,
        output=output,
    )
...
def _selected_w1(plan):
    """The W1 tensor object the plan bound (canonical or paired representation)."""
    return plan._kwargs["w1_weight"]


# Paired decode weights apply to the full B300 decode shape at T=2/4/16 only.
PAIRED_DECODE_TOKENS = (2, 4, 16)
...
@pytest.fixture(scope="module")
def full_kimi_decode_pair(full_kimi_case):
    """Paired W1/scale representation prepared once from the full-shape bank."""
...
@pytest.mark.parametrize(
    ("tokens", "paired"),
    [(tokens, False) for tokens in (1, 16, 128, 512, 1024, 2048)]
    + [(tokens, True) for tokens in PAIRED_DECODE_TOKENS],
)
@pytest.mark.parametrize("distribution", ["balanced", "empty", "hot"])
def test_full_kimi_paired_fp64(
    full_kimi_case, tokens, paired, distribution, record_property, request
):
    case = _full_kimi_case(full_kimi_case, tokens, distribution)
    decode_pair = request.getfixturevalue("full_kimi_decode_pair") if paired else None
    plan, output, _ = prepare_candidate(case, paired_decode_weights=decode_pair)
    baseline, baseline_output = make_trt_baseline(case)
    plan.run()
    baseline()
    if paired:
        # The pair must actually be bound, and it must agree with the canonical
        # representation of the same bank within BF16 output tolerance.
        assert _selected_w1(plan) is decode_pair[0]
        canonical_plan, canonical_output, _ = prepare_candidate(case)
        canonical_plan.run()
        torch.testing.assert_close(output, canonical_output, atol=1e-2, rtol=1e-2)
    ...
    report["routing"] = routing_histogram(case)
    report["paired_decode_weights"] = paired
    record_property("numerical_report", json.dumps(report))
    assert report["ideal_fp64"]["candidate"]["finite"]
    assert report["ideal_fp64"]["baseline"]["finite"]
    ref = refs["ideal_fp64"]
...
@pytest.mark.parametrize(
    ("tokens", "paired"),
    [(tokens, False) for tokens in range(1, 17)]
    + [(tokens, True) for tokens in PAIRED_DECODE_TOKENS],
)
def test_full_kimi_graph_replay(full_kimi_case, tokens, paired, request):
...
def test_full_kimi_paired_decode_weights_require_exact_pair(
    full_kimi_case, full_kimi_decode_pair
):
    """Only the exact prepared pair is accepted where the paired path is eligible."""
    weights = prepare_cute_weights(full_kimi_case)
    canonical_w1 = weights[0]
    w1, w1_sf = full_kimi_decode_pair
    case = _full_kimi_case(full_kimi_case, 4, "hot")
    for bad in (
        (w1.clone(), w1_sf),  # clone drops the prepared-pair identity
        (w1, w1_sf.clone()),
        (w1[:], w1_sf),  # a view is a different tensor object
        (canonical_w1, weights[1]),  # canonical representation is not a pair
        (w1,),
        (w1, w1_sf, w1),
        [w1, w1_sf],
    ):
        with pytest.raises(ValueError, match="exact pair"):
            prepare_candidate(case, prepared_weights=weights, paired_decode_weights=bad)
    # Outside the eligible token counts the pair is accepted and ignored: the
