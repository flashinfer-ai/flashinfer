...
class TestCuteDslMoeW4A16:
    def test_wrapper_releases_eager_workspace_with_live_wrappers(
        ...
        workspace_refs: list[weakref.ReferenceType[torch.Tensor]] = []
        get_workspace = moe_w4a16._get_workspace

        def track_workspace(*args, **kwargs):
            workspace = get_workspace(*args, **kwargs)
            workspace_refs.extend(
                weakref.ref(tensor)
                for tensor in (
                    workspace.hidden_workspace,
                    workspace.intermediate,
                    *workspace.moe_sort_buffers.values(),
                )
            )
            return workspace

        monkeypatch.setattr(moe_w4a16, "_get_workspace", track_workspace)
        wrappers = [
            CuteDslMoEWrapper(
                num_experts=num_experts,
                top_k=top_k,
        ...
        outputs = [
            wrapper.run(**kwargs, tactic=DEFAULT_W4A16_MOE_TACTIC)
        ...
        assert all(ref() is None for ref in workspace_refs)

    def test_wrapper_cuda_graph_owns_invocation_workspace(
        self, monkeypatch: pytest.MonkeyPatch
    ):
        from flashinfer import CuteDslMoEWrapper
        from flashinfer.fused_moe.cute_dsl.blackwell import moe_w4a16
        ...
        # Compile before capture. The old runner cache is also populated here,
        # which makes the behavioral lifetime assertion below catch regressions.
        run_twice()
        torch.cuda.synchronize()
        ...
        with torch.cuda.graph(graph):
            graph_output_1, graph_output_2 = run_twice()
        torch.cuda.synchronize()
        gc.collect()
        ...
        graph.replay()
        torch.cuda.synchronize()
        first_replay_outputs = (graph_output_1.clone(), graph_output_2.clone())

        # Replay must read the current contents at the captured addresses, and
        # an eager call between replays must not alias the graph-private scratch.
        kwargs["x"].neg_()
        kwargs["token_selected_experts"].copy_(
            (kwargs["token_selected_experts"] + 1) % num_experts
        )
        kwargs["token_final_scales"].copy_(kwargs["token_final_scales"].flip(-1))
        eager_output_1, eager_output_2 = run_twice()
        torch.cuda.synchronize()
        gc.collect()
        assert all(ref() is None for ref in workspace_refs)
        ...
        assert not torch.equal(graph_output_1, first_replay_outputs[0])
        assert not torch.equal(graph_output_2, first_replay_outputs[1])
        torch.testing.assert_close(graph_output_1, eager_output_1, rtol=0, atol=0)
        torch.testing.assert_close(graph_output_2, eager_output_2, rtol=0, atol=0)
