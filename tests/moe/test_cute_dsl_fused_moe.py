...
class TestCuteDslMoeBf16Activation:
    ...
    def test_autotune_cuda_graph(self):
        ...
        tensors["x_bf16"].normal_().div_(10)
        ref_output = compute_reference_moe_fp4(
            hidden_states=tensors["x_bf16"],
            gemm1_weights=tensors["w1_weight_bf16"],
            gemm2_weights=tensors["w2_weight_bf16"],
...
# =============================================================================
# Test Class: MXFP8 activations with online NVFP4 weight conversion
# =============================================================================


@cute_dsl_available
@sm100_required
class TestCuteDslMoeW4A8:
    @staticmethod
    def _quantize_input(x: torch.Tensor):
        ...
        x_quantized, x_sf = mxfp8_quantize(
            x,
            backend="cute-dsl",
            sf_swizzle_layout=SfLayout.layout_linear,
        )
        x_ref = mxfp8_dequantize_host(
            x_quantized.cpu().view(torch.uint8),
            x_sf.cpu(),
            sf_swizzle_layout=SfLayout.layout_linear,
        ).to(x.device)
        return x_quantized, x_sf, x_ref
    ...
    def test_numerical_accuracy(
        ...
        x, x_sf, x_ref = self._quantize_input(tensors["x_bf16"])
        kwargs = {
            "x": x,
            "x_sf": x_sf,
            "token_selected_experts": tensors["token_selected_experts"],
    ...
    def test_all_tactics_numerical_accuracy(self):
        from flashinfer.fused_moe.cute_dsl.fused_moe import _moe_core_impl
        from flashinfer.fused_moe.cute_dsl.tuner import W4A8_MOE_TACTICS
        ...
        for tactic in W4A8_MOE_TACTICS:
            result = _moe_core_impl(
                x=x,
                x_sf=x_sf,
                token_selected_experts=tensors["token_selected_experts"],
            ...
            passed, percent_within, atol = check_accuracy(result, ref_output)
            assert passed, (
                f"Tactic {tactic} only has {percent_within * 100:.2f}% "
                f"within tolerance (atol={atol:.4f})"
            )
