"""cute_dsl_sm12x_fc1_act_fp8: correctness against a pure-torch reference."""
...
def test_cute_dsl_sm12x_fc1_act_fp8_matches_reference():
    ...
    a, b, a_scale, b_scale, m_indptr, ref = make_gated_inputs(
        [64, 64, 64, 64], 512, 512
    )
    out = cute_dsl_sm12x_fc1_act_fp8(a, a_scale, b, b_scale, m_indptr)
    diff = calc_diff(out.float(), ref.float())
    assert diff < 2e-3, f"calc_diff={diff:.6e}"
