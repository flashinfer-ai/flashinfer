...
class TestCutoverRegistry:
    ...
    def test_wrappers_of_different_capacity_resolve_independently_in_one_process(self):
        """Alternating wrappers resolve from their own shape and capacity only."""
        cases = (
            (Q35, 1792),
            ({**Q35, "capacity_tokens": 4096}, 2048),
            (Q38, 8704),
            ({**Q38, "capacity_tokens": 4096}, 8704),
            ({**Q35, "capacity_tokens": None}, 2048),
        )
        for args, expected in cases * 2:
            assert md._get_static_compact_cutover_pairs("fp4", **args) == expected
    ...
    def test_environment_override_wins_over_the_registry(self, monkeypatch):
        env_name = "FLASHINFER_B12X_STATIC_COMPACT_CUTOVER_PAIRS"
        cases = (
            (Q35, 1792),
            (Q38, 8704),
            ({**Q35, "capacity_tokens": None}, 2048),
        )
        # Resolve first, then change the override repeatedly without resetting state.
        for args, expected in cases:
            assert (
                md._get_static_compact_cutover_pairs("fp4", **args, sm_count=110)
                == expected
            )
        for override in (4096, 256, 0, -1):
            monkeypatch.setenv(env_name, str(override))
            for args, _ in cases:
                assert md._get_static_compact_cutover_pairs(
                    "fp4", **args, sm_count=110
                ) == max(0, override)
