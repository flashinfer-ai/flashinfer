...
def _tensors(num_tokens: int, intermediate: int, seed: int):
    return _tensors_shape(num_tokens, EXPERTS, HIDDEN, intermediate, TOPK, seed)
...
def _reference(t, num_tokens: int, intermediate: int):
    return _reference_shape(t, num_tokens, EXPERTS, HIDDEN, intermediate, TOPK)
...
class TestStaticExtentRules:
    ...
    def test_hot_expert_chunk_boundaries(self, num_tokens, merged, monkeypatch):
        """Cross virtual-expert boundaries with both retained-slice schedules."""
        from flashinfer import B12xMoEWrapper
        from flashinfer.fused_moe.cute_dsl.blackwell_sm12x import moe_dispatch as md

        monkeypatch.setattr(md, "_FORCED_BACKEND", "static")
        monkeypatch.setenv(md._STATIC_MERGED_GROUPS_ENV, "1" if merged else "0")
        t = _tensors_shape(num_tokens, 4, 256, 320, 2, seed=91)
        t["token_selected_experts"][:] = torch.tensor([0, 1], device="cuda")
        t["token_final_scales"].fill_(0.5)
        ...
        expected = _reference_shape(t, num_tokens, 4, 256, 320, 2)
        eager = wrapper.run(**_kwargs(t)).clone()
        torch.cuda.synchronize()
        assert check_accuracy(eager, expected)[0]
        graph = torch.cuda.CUDAGraph()
        with torch.cuda.graph(graph):
            replayed = wrapper.run(**_kwargs(t))
        for _ in range(3):
            graph.replay()
        torch.cuda.synchronize()
        assert check_accuracy(replayed, expected)[0]
        torch.testing.assert_close(replayed, eager)

    @pytest.mark.parametrize("top_k", [1, 2])
    def test_three_slice_merge_selection_and_accuracy(self, top_k, monkeypatch):
        from flashinfer import B12xMoEWrapper
        from flashinfer.fused_moe.cute_dsl.blackwell_sm12x import moe_dispatch as md

        monkeypatch.delenv(md._STATIC_MERGED_GROUPS_ENV, raising=False)
    ...
    def test_single_slice_dynamic_keeps_branch_major_true_extent(self, intermediate):
        ...
        # (tile_m, branch_major_extent); the extent is the second-to-last key
        # field, followed by source_down_scales (see _dynamic_kernel_cache_key).
    ...
    def test_single_slice_micro_band_uses_256_aligned_operands(self, shape, num_tokens):
        ...
        assert (
            views.static_family_w13_fp4 is not None
            and views.static_family_w13_fp4.shape[0] == 2 * 256
        )
        expected = (
