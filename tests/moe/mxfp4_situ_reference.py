...
def quantize_mxfp8_reference(values: torch.Tensor):
    ...
    scaled = torch.where((scale == 0).unsqueeze(-1), 0., scaled)
    quantized = scaled.clamp(-448., 448.).float().to(torch.float8_e4m3fn)
...
def make_routing(tokens, num_experts, top_k, local_num_experts,
                 local_expert_offset=0, distribution="balanced", *, device="cuda", seed=17):
    ...
    if local_expert_offset + local_num_experts > num_experts or not 1 <= top_k <= num_experts:
    ...
    if distribution == "balanced":
        ...
        ids = (t[:, None] + k * (num_experts // top_k) + local_expert_offset) % num_experts
    else:
        local = list(range(local_expert_offset, local_expert_offset + local_num_experts))
        ...
        if distribution == "empty":
            ...
            pool = torch.tensor([e for e in range(num_experts) if e != local[0]], dtype=torch.int64)
            ids = torch.empty((tokens, top_k), dtype=torch.int64)
            ids[:, 0] = local[0]
            if top_k > 1:
                ids[:, 1:] = pool[(t[:, None] * (top_k - 1) + torch.arange(top_k - 1)) % len(pool)]
    ...
    weights = torch.rand((tokens, top_k), generator=generator) + .125
...
def routing_histogram(case):
    counts = torch.bincount(case.topk_ids.cpu().long().flatten(), minlength=case.num_experts)
    local = counts[case.local_expert_offset:case.local_expert_offset + case.local_num_experts]
    return {"global": counts.tolist(), "local": local.tolist(),
            "local_assignments": int(local.sum()), "empty_local_experts": int((local == 0).sum())}
...
def reference_moe(case: MXFP4SiTUCase, modes=("ideal_fp64", "mxfp8_fp32")):
    ...
    outputs = {mode: torch.zeros(x.shape, dtype=torch.float64, device=device) for mode in modes}
    ...
    for local in range(case.local_num_experts):
        ...
        beta = case.beta[0 if case.beta.numel() == 1 else local].to(device=device, dtype=torch.float64)
        linear_beta = None if case.linear_beta is None else case.linear_beta[0 if case.linear_beta.numel() == 1 else local].to(device=device, dtype=torch.float64)
        activation = situ_reference(up, gate, beta, linear_beta)
        w2 = decode_mxfp4(case.w2[local], case.w2_scale[local], device=device)
        for mode in modes:
            middle = activation
            if mode != "ideal_fp64":
                source = activation.to(torch.bfloat16) if mode == "mxfp8_bf16" else activation.float()
...
def error_metrics(actual, reference):
    ...
    return {"finite": True, "relative_l2": float(relative_l2), "cosine": float(cosine),
            "mean_abs": float(absolute.mean()), "p50_abs": float(quantiles[0]),
            "p95_abs": float(quantiles[1]), "p99_abs": float(quantiles[2]),
            "max_abs": float(absolute.max()), "worst_token_relative_l2": float(token_relative.max()),
...
def paired_accuracy(candidate, baseline, reference):
    ...
    result = {"candidate": candidate_metrics, "baseline": baseline_metrics,
              "candidate_vs_baseline": error_metrics(candidate, baseline)}
    if candidate_metrics["finite"] and baseline_metrics["finite"]:
        result["relative_l2_delta"] = candidate_metrics["relative_l2"] - baseline_metrics["relative_l2"]
        result["cosine_delta"] = candidate_metrics["cosine"] - baseline_metrics["cosine"]
    return result


@torch.no_grad()
def make_case(tokens=16, hidden=256, intermediate=128, num_experts=8, local_num_experts=4,
              local_expert_offset=2, top_k=2, distribution="balanced", seed=123,
              beta=4.0, linear_beta=25.0, input_distribution="normal", device="cuda"):
    ...
    for rows, columns in ((2 * intermediate, hidden), (hidden, intermediate)):
        ...
        scales = torch.empty((local_num_experts, rows, columns // 32), dtype=torch.uint8)
        for expert in range(local_num_experts):
            dense = (torch.randn((rows, columns), device=device, generator=generator) / columns**.5).to(torch.bfloat16)
            q, sf = fp4_quantize(dense, global_scale=torch.ones(1, device=device), sf_vec_size=32,
                                 sf_use_ue8m0=True, is_sf_swizzled_layout=False)
            packed[expert].copy_(q.cpu())
            scales[expert].copy_(sf.view(torch.uint8).reshape(rows, columns // 32).cpu())
        banks.extend((packed, scales))
    ids, weights = make_routing(tokens, num_experts, top_k, local_num_experts,
                                local_expert_offset, distribution, device=device, seed=seed + 1)
    ...
    return MXFP4SiTUCase(x, x_sf, *banks, ids, weights, betas, linear_betas,
...
def prepare_cute_weights(case):
    ...
    return prepare_cute_dsl_mxfp4_weights(
        case.w1.to(case.x.device), case.w1_scale.to(case.x.device),
        case.w2.to(case.x.device), case.w2_scale.to(case.x.device),
    )
...
def prepare_trt_weights(case):
    ...
    device = case.x.device
    cache, result = {}, []
    for packed, scales, first in ((case.w1, case.w1_scale, True), (case.w2, case.w2_scale, False)):
        q = torch.empty(packed.shape, dtype=torch.uint8, device=device)
        sf = torch.empty(scales.shape, dtype=torch.uint8, device=device)
        permute = _maybe_get_cached_w3_w1_permute_indices if first else get_w2_permute_indices_with_cache
        for expert in range(packed.shape[0]):
            ...
            p_sf = permute(cache, expert_sf, 128, num_elts_per_sf=16, is_gated_act_gemm=True).to(device)
            q[expert].copy_(expert_q[p])
            sf[expert].copy_(block_scale_interleave(expert_sf[p_sf].contiguous()).reshape_as(expert_sf))
...
def make_trt_baseline(case, prepared_weights=None, *, packed=True):
    ...
    w1, s1, w2, s2 = prepare_trt_weights(case) if prepared_weights is None else prepared_weights
    ones = torch.ones(case.local_num_experts, device=case.x.device, dtype=torch.float32)
    routes = pack_topk(case.topk_ids, case.topk_weights) if packed else (case.topk_ids, case.topk_weights)
    output = torch.empty_like(case.x, dtype=torch.bfloat16)
    kwargs = dict(
        topk_ids=routes, routing_bias=None, hidden_states=case.x,
        ...
        gemm1_alpha=case.beta, gemm1_beta=case.linear_beta, gemm1_clamp_limit=None,
        ...
        per_token_scale=None, num_experts=case.num_experts, top_k=case.topk_ids.shape[1],
        n_group=None, topk_group=None, intermediate_size=case.intermediate_size,
        local_expert_offset=case.local_expert_offset, local_num_experts=case.local_num_experts,
        routed_scaling_factor=1.0, routing_method_type=0, do_finalize=True, enable_pdl=False,
        activation_type=ActivationType.Situ, output=output,
