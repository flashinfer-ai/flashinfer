...
@torch.no_grad()
def prepare_cute_decode_weights(case):
    """Prepare the optional paired W1/scale pair for B300 T=2/4/16 SiTU decode.

    Consumes the same canonical packed bank as ``prepare_cute_weights``; the
    returned pair is passed to ``plan(paired_decode_weights=...)`` alongside the
    four canonical prepared weights and is selected only for eligible plans.
    """
    from flashinfer.fused_moe.prepare import prepare_cute_dsl_mxfp4_decode_weights

    return prepare_cute_dsl_mxfp4_decode_weights(
        case.w1.to(case.x.device),
        case.w1_scale.to(case.x.device),
    )
