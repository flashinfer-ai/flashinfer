...
SM = md.get_num_sm(torch.device("cuda"))
...
def test_registry_is_versioned_and_keyed_by_the_full_exact_key():
    assert md._STATIC_CUTOVER_REGISTRY_VERSION == 3
    for key, pairs in md._STATIC_CUTOVER_REGISTRY.items():
        ...
        assert capacity == md._STATIC_CUTOVER_MEASURED_CAPACITY_TOKENS == 8192
...
def test_partial_keys_do_not_alias_activation_or_slice_count(monkeypatch):
    args = dict(
        quant_mode="nvfp4",
        num_experts=512,
        intermediate_size=320,
    ...
    monkeypatch.setenv(md._STATIC_MERGED_GROUPS_ENV, "0")
    assert md._get_static_compact_cutover_pairs(**args) == 8192
    monkeypatch.delenv(md._STATIC_MERGED_GROUPS_ENV)
    assert md._get_static_compact_cutover_pairs(**args) == 8704
