...
_GQA_GROUP = _NUM_QO_HEADS // _NUM_KV_HEADS

# MiniMax-M3's per-rank head geometry under tensor parallelism, as vLLM shards
# it. Every row keeps GQA group 16, which is what makes them instantiations of
# one kernel rather than three kernels; tp 8 gives group 8 and is refused by
# name, because `kQueriesPerTile * kTopK <= kHashSize` holds here with exact
# equality at 8 * 16 == 128.
_TP_GEOMETRIES = ((1, 64, 4), (2, 32, 2), (4, 16, 1))


def _page_map(num_kv_heads: int) -> dict[str, int]:
...
# ---------------------------------------------------------------------------
def _page_views(
    pool: torch.Tensor, num_pages: int, num_kv_heads: int = _NUM_KV_HEADS
    ...
    page = _page_map(num_kv_heads)
    data_shape = (num_pages, num_kv_heads, _PAGE_SIZE, _DATA_DIM)
    scale_shape = (num_pages, num_kv_heads, _PAGE_SIZE, _SCALE_DIM)
    data_stride = (page["page_bytes"], _DATA_HEAD_STRIDE, _DATA_DIM, 1)
    scale_stride = (page["page_bytes"], _SCALE_HEAD_STRIDE, _SCALE_DIM, 1)
    return (
        torch.as_strided(pool, data_shape, data_stride, 0),
        torch.as_strided(pool, data_shape, data_stride, page["v_data"]),
        torch.as_strided(pool, scale_shape, scale_stride, page["k_scale"]),
        torch.as_strided(pool, scale_shape, scale_stride, page["v_scale"]),
    )
...
def _random_packed_pool(
    ...
    pool = torch.empty(
        num_pages * _page_map(num_kv_heads)["page_bytes"],
        dtype=torch.uint8,
        device=device,
    )
    k, v, k_scale, v_scale = _page_views(pool, num_pages, num_kv_heads)
    ...
    for start in range(0, num_pages, _POOL_CHUNK_PAGES):
        ...
        code_shape = (stop - start, num_kv_heads, _PAGE_SIZE, _HEAD_DIM)
        scale_shape = (stop - start, num_kv_heads, _PAGE_SIZE, _SCALE_DIM)
...
def _make_q2k_indices(
    *,
    ...
    total_q = sum(q_lens)
    output = torch.full((num_kv_heads, total_q, _TOPK), -1, dtype=torch.int32)
...
def _build_problem(
    *,
    ...
    pool, k, v, k_scale, v_scale = _random_packed_pool(
        num_pages, device, seed, num_kv_heads
    )
    ...
    q = (
        torch.randn(
            (total_q, num_kv_heads * _GQA_GROUP, _HEAD_DIM),
            generator=generator,
            dtype=torch.float32,
            device=device,
        )
...
def _cpu_surface(
    *,
    ...
    pool = torch.zeros(
        num_pages * _page_map(num_kv_heads)["page_bytes"], dtype=torch.uint8
    )
    k, v, k_scale, v_scale = _page_views(pool, num_pages, num_kv_heads)
    if scale_dtype is not torch.uint8:
        k_scale = k_scale.view(scale_dtype)
        v_scale = v_scale.view(scale_dtype)
    q2k_indices = torch.full((num_kv_heads, total_q, _TOPK), -1, dtype=torch.int32)
    q2k_indices[:, :, 0] = 0
    counts = [total_q // batch_size] * batch_size
    counts[-1] += total_q - sum(counts)
    return {
        "q": torch.zeros(
            (total_q, num_kv_heads * _GQA_GROUP, _HEAD_DIM), dtype=torch.bfloat16
        ),
        "k": k,
        "v": v,
        "k_scale": k_scale,
...
@pytest.mark.parametrize(
    ("num_qo_heads", "head_dim", "needle"),
    [
        pytest.param(32, _HEAD_DIM, "64 query heads", id="32-query-heads"),
        pytest.param(_NUM_QO_HEADS, 64, "head_dim 128", id="head-dim-64"),
    ],
)
def test_the_model_geometry_is_fixed(
    num_qo_heads: int, head_dim: int, needle: str
) -> None:
    """A neighbouring geometry must never reach a kernel built from this one.

    head_dim 128 is a compile-time constant of the shared-memory map and of the
    MMA K extent, and it is refused on its own line. The QUERY-HEAD count is
    refused for a different reason and the message says which: 4 KV heads at
    GQA group 16 require 64 query heads, so 32 over a 4-head page is a
    mismatched pair rather than an unsupported count -- 32 query heads over a
    TWO-head page is a tensor-parallel rank and is served.
    """
...
def test_the_allowlist_is_one_geometry_and_matches_the_module() -> None:
    ...
    # One row per compiled KV-head count, every one of them at the same GQA
    # group -- so the rows are ranks of one deployment, not unrelated models.
    expected_rows = [
        [kv * _GQA_GROUP, kv, _HEAD_DIM, _PAGE_SIZE, _TOPK]
        for _tp, _qo, kv in _TP_GEOMETRIES
    ]
    assert payload["workloads"] == expected_rows
    assert module._load_allowlist() == frozenset(tuple(row) for row in expected_rows)
    assert tuple(module._COMPILED_KV_HEAD_COUNTS) == tuple(
        kv for _tp, _qo, kv in _TP_GEOMETRIES
    )
    assert module._GQA_GROUP == _GQA_GROUP
...
def test_the_page_map_agrees_with_the_storage_contract() -> None:
    ...
    # ...and the map is a FUNCTION of the head count, so it has to agree at
    # every count that has a body, not only at the single-rank one. This file's
    # fixtures and the guard state it separately; a rank where they disagreed
    # would build pages the guard's pointer-difference proof then rejects, or
    # worse, accepts at the wrong spacing.
    for _tp, _qo, kv in _TP_GEOMETRIES:
        mine = _page_map(kv)
        theirs = module.page_layout(kv)
        assert mine["k_scale"] == theirs["k_scale_byte_offset"], kv
        assert mine["v_data"] == theirs["v_data_byte_offset"], kv
        assert mine["v_scale"] == theirs["v_scale_byte_offset"], kv
        assert mine["page_bytes"] == theirs["page_bytes"], kv
    assert [_page_map(kv)["page_bytes"] for _tp, _qo, kv in _TP_GEOMETRIES] == [
        73728,
        36864,
        18432,
    ]
...
# ---------------------------------------------------------------------------
# tensor parallelism: one compiled body per per-rank KV-head count
# ---------------------------------------------------------------------------
@pytest.mark.parametrize("tp, num_qo_heads, num_kv_heads", _TP_GEOMETRIES)
def test_every_tensor_parallel_rank_matches_the_reference(
    tp: int, num_qo_heads: int, num_kv_heads: int
) -> None:
    """A relaxed assertion nobody executed at the relaxed value is not support.

    Each rank presents fewer query heads over fewer KV heads and a page that has
    shrunk with it, so the addressing constants folded into its body -- the page
    stride, the three region offsets and the query row stride -- are all
    different. The shapes here are the ragged ones: a partial final block, a
    request shorter than the 8-query row tile, and one long enough to span
    several tiles, because that is where the tile scan and the causal mask
    interact and neither is head-count-free.
    """

    device = _require_supported_gpu()
    problem = _build_problem(
        q_lens=[13, 1, 40],
        kv_lens=[1000, 130, 4096],
        device=device,
        seed=4000 + tp,
        num_kv_heads=num_kv_heads,
    )
    assert problem["q"].shape[1] == num_qo_heads
    assert problem["k"].shape[1] == num_kv_heads
    assert int(problem["k"].stride(0)) == _page_map(num_kv_heads)["page_bytes"]
    cosine, relative_rms = _agreement(_serve(problem), _reference(problem))
    assert cosine >= 0.999, f"cosine similarity {cosine}"
    assert relative_rms <= 0.02, f"relative RMS {relative_rms}"


@pytest.mark.parametrize("tp, num_qo_heads, num_kv_heads", _TP_GEOMETRIES)
def test_every_tensor_parallel_rank_is_bit_stable_below_the_table_width(
    tp: int, num_qo_heads: int, num_kv_heads: int
) -> None:
    """Determinism is a property of the union insert, not of the head count.

    Below the table width the slot is a permutation and the insert takes the
    collision-free ``atomicOr`` path, so repeated calls must agree bit for bit
    at every rank. If a rank's addressing were wrong in a way that happened to
    stay finite, this would still pass -- which is why it runs alongside the
    reference comparison rather than instead of it.
    """
    ...
    for _ in range(3):
        assert torch.equal(_serve(problem), first)


def test_the_tensor_parallel_8_geometry_is_refused_by_name() -> None:
    """Group 8 is a TILE SHAPE, and saying so is the whole point of the refusal.

    vLLM replicates the KV heads once tp exceeds ``num_key_value_heads``, so
    tp 8 presents 8 query heads over 1 KV head. This kernel cannot serve it --
    ``kQueriesPerTile = kTileM / kGroupSize`` would double to 16 and
    ``kQueriesPerTile * kTopK <= kHashSize`` holds today with EXACT equality --
    and it has to say which axis declined, because there is no other
    implementation of MSA over an NVFP4 cache to fall through to. A silent
    "not supported" would send an operator looking at the cache dtype.
    """

    module = _guard_module()
    surface = _cpu_surface(num_kv_heads=1)
    surface["q"] = torch.zeros((8, 8, _HEAD_DIM), dtype=torch.bfloat16)
    reason = module.check_surface(**surface)
    assert reason is not None and "GQA group 16" in reason
    assert "16 query heads" in reason and "got 8" in reason

    # ...and an unsupported KV-head count is a DIFFERENT refusal, naming the
    # counts that do have a body rather than the group.
    surface = _cpu_surface(num_kv_heads=1)
    surface["k"] = surface["k"][:, :1].expand(-1, 3, -1, -1)
    reason = module.check_surface(**surface)
    assert reason is not None and "compiled bodies for kv-head counts" in reason
