...
from benchmarks import bench_cake_msa_sm100 as benchmark
...
def test_manifest_preserves_production_prefix_and_freezes_coverage_rows():
    stable_ids = tuple(shape.stable_id for shape in benchmark.SHAPE_MANIFEST)

    assert stable_ids[: len(benchmark.PRODUCTION_SHAPE_IDS)] == (
        benchmark.PRODUCTION_SHAPE_IDS
    )
    assert stable_ids[len(benchmark.PRODUCTION_SHAPE_IDS) :] == EXPECTED_COVERAGE_IDS
...
def test_shape_filter_defaults_to_all_and_preserves_manifest_order():
    ...
    requested = [EXPECTED_COVERAGE_IDS[-1], benchmark.PRODUCTION_SHAPE_IDS[0]]
    selected = benchmark._selected_shapes(Namespace(shapes=requested))
    assert tuple(shape.stable_id for shape in selected) == (
        benchmark.PRODUCTION_SHAPE_IDS[0],
        EXPECTED_COVERAGE_IDS[-1],
    )
