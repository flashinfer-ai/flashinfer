...
# ---------------------------------------------------------------------------
@sm100_only
@pytest.mark.parametrize("tp, num_qo_heads, num_kv_heads", TP_GEOMETRIES)
@pytest.mark.parametrize("batch, seq_lengths", [(8, [8192] * 8), (32, [8192] * 32)])
def test_matches_the_reference_at_every_tensor_parallel_geometry(
    tp, num_qo_heads, num_kv_heads, batch, seq_lengths
):
...
def test_a_tensor_parallel_rank_survives_the_ragged_shapes_too(
    ...
    inputs = _build_inputs(
        11,
        [9000, 130, 8500, 0, 9000, 1, 8192, 127, 4096, 255, 3],
        torch.device("cuda"),
        seed=2000 + tp,
        num_kv_heads=num_kv_heads,
        num_qo_heads=num_qo_heads,
...
def test_a_tensor_parallel_rank_takes_the_parametric_family(
    ...
    """Which body served it, asserted rather than assumed.

    Only the tp=1 geometry reaches the pinned CUDA family and the CuTe-DSL
    scored instantiations; every other rank is served by the parametric family.
    That is a LATENCY statement, and it is only true if the dispatch actually
    behaves that way, so the counters are read on both sides of the call.
    """
    inputs = _build_inputs(
        8,
        [8192] * 8,
    ...
    if tp == 1:
        # The claim is that tp=1 reaches a FAST family, not which one: the route
        # picks between the CuTe-DSL instantiations and the pinned CUDA family
        # on grounds -- warmth, the route override -- that are properties of the
        # process, not of the geometry. What the geometry decides is that the
        # PARAMETRIC family is not what serves it, and that is what is asserted.
        assert after["general_dispatch_count"] == before["general_dispatch_count"]
        assert (
            after["specialised_route"]["dispatch_count"]
            + after["pinned_dispatch_count"]
        ) > (
            before["specialised_route"]["dispatch_count"]
            + before["pinned_dispatch_count"]
        )
    else:
        assert after["general_dispatch_count"] > before["general_dispatch_count"]
...
@sm100_only
@pytest.mark.parametrize("batch, lengths", [(8, [8192] * 8), (32, [8192] * 32)])
def test_out_is_written_WHOLE_and_carries_nothing_from_its_prior_contents(
    batch, lengths
):
    """Every element of `out` must be written, and the test must not assume it.

    `torch.empty_like` is UNINITIALISED, so every other bit-identity assertion
    on this route compares two buffers whose unwritten elements -- if any exist
    -- hold whatever the caching allocator last left there. Those tests then
    pass or fail on allocator history rather than on the kernel, which is a
    test that reports something other than what it claims. Two calls into
    buffers pre-filled with DIFFERENT sentinels answer the question directly:
    if the results agree, every element was written; if they differ, the
    positions that differ are exactly the ones the kernel left alone.
    """
    inputs = _build_inputs(batch, lengths, torch.device("cuda"), seed=6000 + batch)

    sentinel_a, sentinel_b = 1.5, -3.25
    first = torch.full_like(inputs["q"], sentinel_a)
    second = torch.full_like(inputs["q"], sentinel_b)
    assert not torch.equal(first, second)
    _call(inputs, out=first)
    _call(inputs, out=second)
    torch.cuda.synchronize()

    if not torch.equal(first, second):
        # WHICH of the two failures is it? A position that still holds its
        # sentinel was never written; a position that holds neither sentinel was
        # written BOTH times, with different values, and that is
        # nondeterminism rather than an addressing hole. Reporting the counts
        # separately is the whole point: the two have different severities and
        # different fixes, and "not equal" cannot tell them apart.
        mask = first != second
        untouched_a = int((mask & (first == sentinel_a)).sum())
        untouched_b = int((mask & (second == sentinel_b)).sum())
        both_written = int(
            (mask & (first != sentinel_a) & (second != sentinel_b)).sum()
        )
        where = mask.nonzero()
        rows = sorted({int(x[0]) for x in where})
        heads = sorted({int(x[1]) for x in where})
        cols = sorted({int(x[2]) for x in where})
        raise AssertionError(
            f"batch {batch}: {int(mask.sum())} of {first.numel()} output "
            f"elements disagree between two calls into differently pre-filled "
            f"buffers. UNWRITTEN in call 1: {untouched_a}; unwritten in call 2: "
            f"{untouched_b}; WRITTEN BOTH TIMES with different values: "
            f"{both_written}. token rows {rows[:12]}{'...' if len(rows) > 12 else ''} "
            f"({len(rows)} distinct), heads {heads[:12]}"
            f"{'...' if len(heads) > 12 else ''} ({len(heads)} distinct), "
            f"head-dim columns {cols[:12]}{'...' if len(cols) > 12 else ''} "
            f"({len(cols)} distinct)"
        )
    # ...and the value it wrote is the right one, so this cannot pass by the
    # kernel writing the same wrong thing twice.
    _assert_peer(first, _reference(inputs))


@sm100_only
@pytest.mark.parametrize("batch, lengths", [(32, [8192] * 32)])
def test_the_same_call_repeated_gives_the_same_bits(batch, lengths):
    """Run-to-run reproducibility, separated from the destination question.

    ``test_out_is_written_WHOLE_...`` cannot distinguish an unwritten element
    from a nondeterministic one on its own if BOTH are present. This one asks
    only about determinism: every call writes into a buffer pre-filled with the
    SAME sentinel, so an unwritten position holds that sentinel every time and
    contributes nothing, and a difference can only be a value the kernel
    computed differently.
    """
    inputs = _build_inputs(batch, lengths, torch.device("cuda"), seed=8000 + batch)
    reference_run = torch.full_like(inputs["q"], 1.5)
    _call(inputs, out=reference_run)
    torch.cuda.synchronize()
    for attempt in range(4):
        again = torch.full_like(inputs["q"], 1.5)
        _call(inputs, out=again)
        torch.cuda.synchronize()
        if not torch.equal(again, reference_run):
            mask = again != reference_run
            where = mask.nonzero()
            raise AssertionError(
                f"batch {batch}, attempt {attempt}: {int(mask.sum())} of "
                f"{again.numel()} elements differ from the first run with the "
                f"SAME destination fill -- {len(sorted({int(x[0]) for x in where}))} "
                f"distinct token rows, "
                f"{len(sorted({int(x[1]) for x in where}))} distinct heads. "
                f"This is nondeterminism, not an unwritten destination."
            )
