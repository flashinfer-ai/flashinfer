...
_VARIANT_CASES = (
    ...
    ("compact_async_positive_f32", 12, 25, torch.float32, "page", "positive"),
    ("compact_async_f32", 12, 25, torch.float32, "page", "null"),
    ...
    ("high_work_positive_f32", 12, 99, torch.float32, "padded", "positive"),
...
def _require_cake_fused_kda_decode():
    ...
    assert cake_fused_kda_decode_is_available(), (
        "the checked-in Cake fused KDA source registry must be complete"
    )
...
def _guarded_strided_clone(tensor):
    # Preserve the Cake launcher's 8-byte BF16 and 32-byte FP32 base alignment
    # while retaining independent prefix and suffix write guards.
    prefix_elements = 24
    suffix_elements = 31
    span = 1 + sum(
        (size - 1) * stride
...
def _run_and_check_cake(
    ...
    *,
    ...
    monkeypatch.setattr(_impl, "_run_cake_variant", record_cake_route)
    kwargs = {
        **inputs,
...
def test_cake_fused_kda_decode_official_and_boundary_shapes(
    monkeypatch, num_heads, num_rows
):
    inputs = _make_inputs(num_heads, num_rows)
    _run_and_check_cake(monkeypatch, inputs)
...
def test_cake_fused_kda_decode_runtime_configuration_domain(
    monkeypatch,
    state_dtype,
    lower_bound,
    norm_eps,
...
def test_cake_fused_kda_decode_cross_surface_boundaries(
