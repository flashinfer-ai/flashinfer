...
GDN-2 (Gated Delta Net 2) prefill tests.  Mirrors test_prefill_delta_rule.py
with the GDN-2 interface: channel-wise forget gate ``g`` and erase gate
...
def _skip_if_not_sm100():
    ...
    if not is_sm100a_supported(device):
        pytest.skip("GDN-2 prefill requires SM100 (Blackwell)")
    cuda_major = int(torch.version.cuda.split(".")[0]) if torch.version.cuda else 0
    if cuda_major < 13:
        pytest.skip(f"SM100 GDN-2 prefill requires CUDA 13+, got {torch.version.cuda}")
...
def _make_gates(
    ...
    beta = (
        (
            torch.rand(total_seqlen, num_sab_heads, head_size, device=device).sigmoid()
            * 2.0
        ).to(io_dtype)
        if use_beta
        else None
    ...
    w = (
        torch.rand(total_seqlen, num_sab_heads, head_size, device=device)
        .sigmoid()
        .to(io_dtype)
        if use_w
        else None
...
def _test_prefill_kernel(
    ...
    with device:
        ...
        alpha, beta, w = _make_gates(
            total_seqlen, num_sab_heads, head_size, dtype, alpha, beta, w, device
        )
    ...
    ref_o, ref_state = recurrent_gdn2(
        q.float(),
        k.float(),
        v.float(),
        seq_lens,
...
@pytest.mark.parametrize("w", [False, True])
...
def test_prefill_kernel_zero_length_sequence(
    ...
    with device:
        ...
        k = torch.nn.functional.normalize(k, p=2.0, dim=-1)
        alpha, beta, w = _make_gates(
            seq_len, num_sab_heads, head_size, dtype, True, True, True, device
        )
...
def _test_chunked_prefill(
    ...
    with device:
        ...
        alpha1, beta1, w1 = _make_gates(
            total_seqlen1, num_sab_heads, head_size, dtype, alpha, beta, w, device
        )
        alpha2, beta2, w2 = _make_gates(
            total_seqlen2, num_sab_heads, head_size, dtype, alpha, beta, w, device
        )
    ...
    w = concat_varlen(w1, cu_seq_lens1, w2, cu_seq_lens2) if w else None
...
def _test_checkpoint(
    ...
    with device:
        ...
        cu_seq_lens = torch.tensor(exclusive_cumsum(seq_lens), dtype=torch.int64)
        alpha, beta, w = _make_gates(
            total_seqlen, num_sab_heads, head_size, dtype, True, True, True, device
        )
    ...
    for seq_idx in range(num_seqs):
        ...
        for ckpt_idx in range(n_ckpts):
            ...
            prefix_w = w[seq_start : seq_start + prefix_len].contiguous()
