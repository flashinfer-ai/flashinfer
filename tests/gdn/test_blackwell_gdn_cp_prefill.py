...
from flashinfer.gdn_kernels.blackwell import source_gdn_cp_prefill as source_backend
from flashinfer.jit import source_gdn_cp_prefill as source_jit


def _gdn_cp_sm100_toolchain_available() -> bool:
    if not torch.cuda.is_available():
        return False
    minimum = {(10, 0): (12, 8), (10, 3): (12, 9)}.get(
        torch.cuda.get_device_capability()
    )
    if minimum is None or gdn_prefill._gdn_cp_nvcc_version is None:
        return False
    try:
        return gdn_prefill._gdn_cp_nvcc_version() >= minimum
    except RuntimeError:
        return False


_GDN_CP_SM100_AVAILABLE = _gdn_cp_sm100_toolchain_available()


def _source_root() -> Path:
    return Path(__file__).resolve().parents[2] / "csrc" / "gdn" / "blackwell_cp_prefill"
...
def _run_fresh_gdn_oracle(
    *,
    ...
    worker = Path(__file__).with_name("_gdn_prefill_checkpoint_oracle.py")
...
def _delegate_frozen_graph_case_to_fresh_pytest(
    *, tmp_path: Path, total: int, hq: int, hv: int
    ...
    case = f"{total}-{hq}-{hv}"
    marker = "FLASHINFER_GDN_CP_FRESH_FROZEN_CASE"
    ...
    environment.update(
        {
            marker: case,
            "FLASHINFER_GDN_CP_FROZEN_RECEIPT": str(receipt_path),
...
def test_jit_loader_accepts_checked_in_manifest() -> None:
    source_jit._manifest.cache_clear()
    assert source_jit._manifest()["schema"] == (
...
def test_long_row_dispatch_is_exact(
    ...
    monkeypatch.setattr(source_backend, "_arch_for", lambda _device: arch)
    monkeypatch.setattr(
        source_backend.torch.cuda,
    ...
    plan = source_backend._build_plan(q, k, v, (65536,))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("sm_100a", (2048,), torch.float16, (2, 2, 8), "cp_prefill"),
...
def test_generic_plan_selects_head_dtype_and_tail_routes(
    ...
    plan = source_backend._build_plan(q, k, v, seq_lens)
...
def test_zero_length_plan_uses_simt_state_fixup(
    ...
    monkeypatch.setattr(source_backend, "_arch_for", lambda _device: "sm_103a")
    monkeypatch.setattr(
        source_backend.torch.cuda,
        "get_device_properties",
        lambda _device: SimpleNamespace(multi_processor_count=148),
    ...
    plan = source_backend._build_plan(q, k, v, (0, 64, 65, 0))

    assert source_backend._choose_fixup_kind(16, 148) == "state_fixup_utcmma64"
...
def test_bf16_default_splits_only_two_block_stage2_factors(
    ...
    monkeypatch.setattr(source_backend, "_arch_for", lambda _device: "sm_100a")
    monkeypatch.setattr(
        source_backend.torch.cuda,
        "get_device_properties",
        lambda _device: SimpleNamespace(multi_processor_count=148),
    ...
    crossed = source_backend._build_plan(**common, seq_lens=(64, 129))
    exact = source_backend._build_plan(
        q=SimpleNamespace(
            shape=(128, 4, 128),
            device=device,
            dtype=torch.bfloat16,
    ...
    longer = source_backend._build_plan(
        q=SimpleNamespace(
            shape=(4096, 4, 128),
            device=device,
            dtype=torch.bfloat16,
...
def test_checkpoint_interval_becomes_cp_chunk_and_maps_boundaries(
    ...
    plan = source_backend._build_plan(
        q,
        k,
        v,
        (256, 384),
    ...
    assert source_backend._checkpoint_fixed_state_indices(plan, device).tolist() == [
        0,
        1,
        2,
        3,
...
def test_indexed_state_rows_must_not_overlap() -> None:
    ...
    with pytest.raises(ValueError, match="rows overlap"):
        source_backend._validate_state(
            overlapping,
            name="initial_state",
            plan=plan,
            device=torch.device("cpu"),
...
def test_indexed_state_preserves_positive_inner_strides() -> None:
    ...
    assert source_backend._state_carrier(state).numel() == span
...
def test_public_gdn_cp_cache_reuses_equal_metadata_and_rebinds_tensor_addresses(
    ...
    monkeypatch.setattr(source_backend, "prepare_gdn_cp_prefill", fake_prepare)
    monkeypatch.setattr(
        source_backend.torch.cuda,
        "current_stream",
    ...
    for _ in range(2):
        ...
        output = torch.empty_like(q)
        output_state = torch.empty_like(initial_state)
        source_backend.chunk_gated_delta_rule_source_sm100(
            output,
            output_state,
            q,
            q,
    ...
    assert source_backend._public_key is None
    assert source_backend._public_metadata_binding is None
    assert source_backend._public_prepared is None
...
def test_public_dispatch_preserves_auto_and_explicit_cp_routes(
    ...
    monkeypatch.setattr(gdn_prefill, "_gdn_cp_nvcc_version", lambda: (13, 0))
    monkeypatch.setattr(
        gdn_prefill,
...
def test_public_dispatch_gates_gdn_cp_with_its_jit_nvcc(
    monkeypatch: pytest.MonkeyPatch,
    ...
    monkeypatch.setattr(gdn_prefill, "_gdn_cp_nvcc_version", lambda: nvcc_version)
    monkeypatch.setattr(
        gdn_prefill,
        "_chunk_gated_delta_rule_source_sm100",
        lambda *_args, **_kwargs: calls.append("gdn_cp"),
...
def test_public_dispatch_preserves_cute_cp_cuda13_gate(
    ...
    monkeypatch.setattr(gdn_prefill, "_gdn_cp_nvcc_version", lambda: (12, 9))
    monkeypatch.setattr(
...
def test_public_dispatch_fails_closed_when_gdn_cp_is_unavailable(
    ...
    monkeypatch.setattr(gdn_prefill.torch.version, "cuda", "13.0")
    monkeypatch.setattr(gdn_prefill, "_gdn_cp_nvcc_version", lambda: (13, 0))
    monkeypatch.setattr(gdn_prefill, "_chunk_gated_delta_rule_source_sm100", None)

    q = torch.zeros((2, 1, 128), dtype=torch.float16)
    with pytest.raises(ValueError, match="Blackwell-only CP delta rule SM100"):
        gdn_prefill.chunk_gated_delta_rule(
            q,
            q,
            q,
...
@pytest.mark.skipif(
    not _GDN_CP_SM100_AVAILABLE,
    reason="requires an SM100a or SM103a GPU with a supported Blackwell nvcc toolchain",
)
def test_odd_cp_chunk_uses_generic_kernel_and_matches_semantic_oracle(
    ...
    prepared = source_backend.prepare_gdn_cp_prefill(
        q,
        k,
        v,
        alpha,
...
def test_frozen_graph_matches_pr4078_and_preserves_inputs(
    ...
    receipt_path = Path(os.environ["FLASHINFER_GDN_CP_FROZEN_RECEIPT"])
...
def test_public_checkpoint_matches_cute_on_caller_stream_and_cuda_graph(
    ...
    def forbidden_external(*_args, **_kwargs):
        raise AssertionError("checkpoint route left the Blackwell backend")
    ...
    assert source_backend._public_prepared is not None
    assert source_backend._public_prepared.plan.checkpoint_count == 3
    assert source_backend._public_prepared._checkpoint is not None
...
def test_public_dispatcher_uses_only_gdn_cp_for_indexed_inplace_gqa(
    ...
    """Exercise the Blackwell public use_cp=True route with no external arm."""
    ...
    real_gdn_cp = gdn_prefill._chunk_gated_delta_rule_source_sm100
    ...
    def forbidden_external(*_args, **_kwargs):
        raise AssertionError("public SM100/SM103 use_cp=True route left Blackwell")
    ...
    assert source_backend._public_prepared is not None
    assert source_backend._public_prepared.plan.source_cp_chunk_len == 128
    assert source_backend._public_prepared.plan.cp_chunk_len == 64
...
def test_public_gdn_cp_inference_empty_int64_inner_strided_cache_rebind(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    real_prepare = source_backend.prepare_gdn_cp_prefill
    prepare_calls = 0
    ...
    monkeypatch.setattr(source_backend, "prepare_gdn_cp_prefill", counted_prepare)
    source_backend._reset_gdn_cp_prefill_cache()
    try:
