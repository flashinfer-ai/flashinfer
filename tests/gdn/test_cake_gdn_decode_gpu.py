...
def _make_fp32_mtp_inputs(*, batch_size: int, seq_len: int) -> dict[str, torch.Tensor]:
    ...
    return {
        ...
        "a": a,
        "dt_bias": torch.randn(num_v_heads, device=device, dtype=torch.float32).mul_(
            0.1
        ),
        "b": b,
        "out": torch.full(
...
def _fp32_mtp_reference(
    ...
    for token in range(tensors["q"].shape[1]):
        q = _exact_qk_l2norm(tensors["q"][:, token]).repeat_interleave(2, dim=1)
        k = _exact_qk_l2norm(tensors["k"][:, token]).repeat_interleave(2, dim=1)
        alpha = torch.exp(
            -torch.exp(tensors["A_log"])
            * torch.nn.functional.softplus(
        ...
        v_delta = (
            tensors["v"][:, token].float() - torch.einsum("bhvk,bhk->bhv", state, k)
        ) * beta[:, :, None]
...
def _assert_fp32_mtp_result(
    ...
    *,
    ...
    if padding.numel():
        ...
        assert torch.all(tensors["intermediate_state"].index_select(0, padding) == 3.25)
    if disable_state_update:
        ...
        assert torch.equal(tensors["state"][~touched], backing_before[:, ::2][~touched])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (8, 4, 16, 32, True, True, 4, False),
        (4, 1, 4, 8, True, False, 0, False),
        (1, 4, 4, 8, True, True, 4, False),
        (2, 4, 4, 8, True, True, 4, False),
        (3, 4, 4, 8, True, True, 4, False),
        (4, 4, 4, 8, True, True, 4, False),
        (5, 4, 4, 8, True, True, 4, False),
        (6, 4, 4, 8, True, True, 4, False),
        (7, 4, 4, 8, True, True, 4, False),
        (8, 4, 4, 8, True, True, 4, False),
        (5, 4, 4, 8, False, True, 4, False),
    ],
)
def test_public_cake_gdn_bf16_rows_match_independent_recurrence_on_caller_stream(
    batch_size,
    seq_len,
    num_q_heads,
    num_v_heads,
...
def test_public_cake_gdn_bf16_verify_is_cuda_graph_safe(
    batch_size, num_q_heads, num_v_heads, pack_gates
):
    tensors = _make_bf16_serving_inputs(
        batch_size=batch_size,
...
@pytest.mark.parametrize("batch_size", (1, 32))
def test_public_cake_gdn_fp32_t1_pretranspose_matches_independent_recurrence(
    batch_size,
):
...
@pytest.mark.parametrize("batch_size,seq_len", ((1, 2), (4, 4), (16, 4), (64, 4)))
def test_public_cake_gdn_fp32_mtp_rows_match_independent_recurrence_on_caller_stream(
    batch_size, seq_len
):
    tensors = _make_fp32_mtp_inputs(batch_size=batch_size, seq_len=seq_len)
    expected = _fp32_mtp_reference(tensors)
...
def test_public_cake_gdn_fp32_mtp_verify_is_cuda_graph_safe():
    tensors = _make_fp32_mtp_inputs(batch_size=1, seq_len=2)
    expected = _fp32_mtp_reference(tensors)
    backing_before = tensors["state_backing"].clone()
...
@pytest.mark.parametrize("batch_size", (1, 32))
def test_public_cake_gdn_fp32_nontranspose_t1_matches_independent_recurrence_on_stream(
    batch_size,
):
    tensors = _make_inputs(batch_size)
    initial_state = tensors["state"].clone()
...
def test_public_cake_gdn_fp32_nontranspose_t1_is_cuda_graph_safe():
...
def test_public_cake_gdn_decode_rejects_cpu_state_indices_before_launch(index_name):
...
def test_public_cake_gdn_decode_rejects_mismatched_cuda_device_before_launch(
    index_name,
):
    if torch.cuda.device_count() < 2:
...
def _run_public_decode_invalid_slot_child(case_name):
    ...
    invalid_value = (
        -2 if case_name.endswith("lower") else int(tensors["state"].shape[0])
    )
...
def test_public_cake_gdn_decode_invalid_cuda_slots_fail_in_isolated_process(case_name):
    completed = subprocess.run(
        [
            sys.executable,
            str(Path(__file__).resolve()),
            "--cake-gdn-invalid-slot",
            case_name,
        ],
        capture_output=True,
        text=True,
    ...
    exact_async_assertion = (
        "_assert_async_cuda_kernel" in combined and expected_slot_assertion in combined
    )
...
if (
    __name__ == "__main__"
    and len(sys.argv) == 3
    and sys.argv[1] == "--cake-gdn-invalid-slot"
):
