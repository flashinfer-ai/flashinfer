...
@pytest.mark.parametrize("batch_size", (1, 32))
def test_public_gdn_noncp_fp32_t1_pretranspose_matches_independent_recurrence(
    batch_size,
):
...
def test_public_gdn_noncp_decode_rejects_mismatched_cuda_device_before_launch(
    index_name,
):
    if torch.cuda.device_count() < 2:
...
if (
    __name__ == "__main__"
    and len(sys.argv) == 3
    and sys.argv[1] == "--gdn-noncp-invalid-slot"
):
