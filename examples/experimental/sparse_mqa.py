...
sparse = (
    torch.arange(2048, device="cuda", dtype=torch.int32).clamp_max(63).repeat(16, 1)
)
starts = torch.zeros(16, device="cuda", dtype=torch.int32)
ends = torch.full_like(starts, 512)
metadata = prepare_sparse_mqa_metadata(
    sparse, fmt="mxfp4", starts=starts, ends=ends, num_kv_tokens=512
)
