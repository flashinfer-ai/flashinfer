...
def scale_words(scales):
    return ((scales.contiguous().view(torch.int32) >> 23) & 255).to(torch.uint8).contiguous().view(torch.int32)


def unpack(values, scales, precision):
    if precision == "fp4":
        table = torch.tensor([0, .5, 1, 1.5, 2, 3, 4, 6, -0., -.5, -1, -1.5, -2, -3, -4, -6], device=values.device)
        byte = values.view(torch.uint8)
        decoded = torch.stack((table[(byte & 15).long()], table[(byte >> 4).long()]), dim=-1).flatten(-2)
    else:
...
def operand(shape, precision, generator, exponent_low=-5):
    exponents = torch.randint(exponent_low, exponent_low + 3, (*shape[:-1], shape[-1] // 32), device="cuda", generator=generator)
    scales = torch.pow(2., exponents.float())
    if precision == "fp4":
        codes = torch.randint(0, 16, shape, device="cuda", dtype=torch.uint8, generator=generator)
        values = codes[..., 0::2] | (codes[..., 1::2] << 4)
    else:
        values = (torch.randint(-16, 17, shape, device="cuda", generator=generator).float() / 2).to(torch.float8_e4m3fn)
...
def smoke_inputs(family, precision, seed=0):
    ...
    routes = torch.stack(((token + seed) % experts, (token + seed + 3) % experts), dim=1).long()
    weights = torch.stack((torch.ones_like(token).float(), torch.where(token % 2 == 0, -.5, 1.)), dim=1)
    inputs = dict(num_tokens=tokens, num_experts=experts, top_k=top_k, hidden=hidden, intermediate=intermediate,
                  routed_weight_dtype=precision, activation_clamp=10., x_fp8_packed=x, x_sf_packed=scale_words(xs),
                  topk_idx=routes, topk_weights=weights, w1_sf=s1, w2_sf=s2)
...
def source_weights(inputs, shared):
    def encode(values, sf, first_layer):
        ...
        words = words.reshape(experts, rows // 128, 4, 32, -1).transpose(2, 3).contiguous().reshape(experts, rows, -1)
...
def reference(inputs, x_scales, shared=None):
    ...
    def expert(x_rows, first, second, route_weight):
        ...
        gate = projected[:, :width].clamp(max=10.)
        up = projected[:, width:].clamp(min=-10., max=10.)
        intermediate = gate * torch.sigmoid(gate) * up * route_weight[:, None]
        groups = intermediate.reshape(-1, width // 32, 32)
        scales = torch.pow(2., torch.ceil(torch.log2(groups.abs().amax(-1).clamp_min(1e-4) / 448.)))
    ...
    for slot in range(inputs["top_k"]):
        for index in range(inputs["num_experts"]):
            ...
            output[selected] += expert(x[selected], w1[index], w2[index], inputs["topk_weights"][selected, slot])
    if shared is not None:
        output += expert(x, unpack(shared[0], shared[1], "fp8")[0], unpack(shared[2], shared[3], "fp8")[0], torch.ones(x.shape[0], device=x.device))
...
def make_smoke(family, precision, seed=0):
    ...
    if family == "source":
        ...
        plan = prepare_mega_moe(inputs["x_fp8_packed"], inputs["x_sf_packed"], inputs["topk_idx"], inputs["topk_weights"],
                                weights=source_weights(inputs, shared), num_experts=8, intermediate=256,
                                routed_weight_dtype=precision, num_shared_experts=1, activation_clamp=10., fast_math=True, num_sms=2)
    else:
...
def grouped_reference(a, b, sa, sb):
    ...
    for expert in range(4):
        ...
        output[rows] = (unpack(a[rows], sa[rows], "fp8") @ unpack(b[expert], sb[expert], "fp4").T).bfloat16()
    return output
