    ...
    for cur_backend in backends:
        ...
        if is_cuda_graph_compatible:
            backend_times[cur_backend] = bench_gpu_time_with_cudagraph(
                ...
                nvtx_range_name=f"decode.{cur_backend}",
                l2_flush=True,
    ...
    for backend in backends:
        if len(backend_times[backend]) > 0:
            ...
            if args.output_path is not None:
                ...
                cur_res["routine"] = "testBatchDecodeWithPagedKVCacheWrapper"
...
def testBatchPrefillWithPagedKVCacheWrapper(args):
    ...
    avg_seq_len_q = actual_seq_lens_q.sum() // batch_size
    if args.verbose >= 1:
    ...
    for cur_backend in backends:
        ...
        if is_cuda_graph_compatible:
            backend_times[cur_backend] = bench_gpu_time_with_cudagraph(
                ...
                num_iters_within_graph=20,
                num_iters=args.num_iters,
                nvtx_range_name=f"prefill.paged.{cur_backend}",
                l2_flush=True,
                l2_flush_size_mb=256,
                l2_flush_device=device,
                sleep_after_run=False,
    ...
    for backend in backends:
        if len(backend_times[backend]) > 0:
            ...
            if args.output_path is not None:
                cur_res = defaultdict(str)
                cur_res["routine"] = "testBatchPrefillWithPagedKVCacheWrapper"
                cur_res["median_time"] = median_time
                cur_res["std_time"] = std_time
                cur_res["tflops"] = tflops
                cur_res["tb_per_sec"] = tb_per_sec
...
def testBatchPrefillWithRaggedKVCacheWrapper(args):
    ...
    for cur_backend in backends:
        ...
        if is_cuda_graph_compatible:
            backend_times[cur_backend] = bench_gpu_time_with_cudagraph(
                ...
                num_iters_within_graph=20,
                num_iters=args.num_iters,
                nvtx_range_name=f"prefill.ragged.{cur_backend}",
                l2_flush=True,
                l2_flush_size_mb=256,
                l2_flush_device=device,
    ...
    for backend in backends:
        if len(backend_times[backend]) > 0:
            ...
            if args.output_path is not None:
                cur_res = defaultdict(str)
                cur_res["routine"] = "testBatchPrefillWithRaggedKVCacheWrapper"
                cur_res["median_time"] = median_time
                cur_res["std_time"] = std_time
                cur_res["tflops"] = tflops
                cur_res["tb_per_sec"] = tb_per_sec
