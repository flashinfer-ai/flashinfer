...
| `FLASHINFER_FMHA_CONTEXT_BALANCE_CAUSAL_WORKLOAD` | unset | `flashinfer/attention/prims_ts/kernels/fmha_context/fmha_kernel.py` | Override the FMHA-TS context causal workload-balancing decision (heavy-first reversed Q-tile raster): `1` forces it on for every causal topology, `0` forces it off. Unset keeps the default policy (explicit request, or auto-enable on the causal + CLC-dynamic + paired path). Tuning knob. |
