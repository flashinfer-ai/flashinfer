...
| `FLASHINFER_CUBIN_MAX_RETRIES` | unset | `flashinfer/artifacts.py` | Optional per-attempt retry budget forwarded to `download_file()`; when unset, `download_file()` keeps its built-in default. |
| `FLASHINFER_CUBIN_RETRY_WINDOW_SECONDS` | `0` | `flashinfer/artifacts.py` | Total retry window for `download_artifacts()`. Failed downloads keep retrying until this many seconds elapse (e.g. `86400` for a 24-hour nightly window). |
