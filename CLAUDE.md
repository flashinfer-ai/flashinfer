...
| `FLASHINFER_DCP_DIRECT_BACKEND` | `auto` | `flashinfer/comm/dcp_direct_reduce.py` | Selects the dest-owned DCP reduce backend: `triton`, `cuda`, or `auto`. `auto` uses CUDA when the module loaded on every rank and the stream is not CUDA-graph capturing; otherwise Triton. |
