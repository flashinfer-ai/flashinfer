...
| `FLASHINFER_MOE_EP_KNOB_CACHE` | `~/.cache/flashinfer/moe_ep_knob_cache.json` | `flashinfer/moe_ep/kernel_src/cutedsl_megamoe/shim/knob_cache.py` | Path of the persistent cutedsl mega-MoE knob cache (offline-tuned winners; `knobs=None` sessions resolve it with a pure lookup before the built-in heuristic). Set to `0`/`off` to disable. Populate with `python -m flashinfer.moe_ep.tune` or a one-off `knobs="auto"` run outside the engine. |
