...
| `FLASHINFER_DISABLE_CUTE_DSL_FUSED_MOE_NVFP4_SWIGLU_FAST_MATH` | unset | `flashinfer/fused_moe/cute_dsl/moe_utils.py` | `1` makes the SM100/SM103 `cute_dsl_fused_moe_nvfp4` SwiGLU epilogue use precise exponential and round-to-nearest FP32 division/multiplication. Unset or `0` preserves the default fast `exp2`/approximate-reciprocal path. Capture the setting when constructing `CuteDslMoEWrapper`; changing it requires a new wrapper and CUDA-graph capture. |
