...
| `FLASHINFER_SSU_PRECOMPUTE_NUM_WARPS` | unset (auto) | `include/flashinfer/mamba/launch_checkpointing_ssu.cuh` | Overrides the experimental ReplaySSM precompute-kernel warp count for performance sweeps. Positive values dispatch to the 4, 8, or 16 warp specialization; leave unset for the heads-per-CTA-derived default. The ReplaySSM autotune cache key includes this value. |
