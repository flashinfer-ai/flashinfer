...
| `FLASHINFER_AUTOTUNE_CACHE_DIR` | `autotune` subdirectory of `FLASHINFER_CACHE_DIR` | `flashinfer/autotune_cache.py` | Root **directory** of the managed v2 autotune store used by `autotune_v2()` (placement only; distinct from the MLA-specific `FLASHINFER_AUTOTUNE_DIR` above). |
