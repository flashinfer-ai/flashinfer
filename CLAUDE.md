...
| `FLASHINFER_CUDNN_FROST_PTXAS` | unset | `flashinfer/experimental/cudnn_frost_selected_kernels_moe_grouped_gemm/compiler.py` | Select an external PTXAS executable path for all Frost dtypes; unset or empty uses the bundled assembler. The executable path, version, and SHA-256 participate in compiler and tactic cache identities. |
...
| `FLASHINFER_CUDNN_FROST_ENGINES` | unset | `flashinfer/cudnn_frost.py` (applied in `flashinfer/__init__.py` before any `import cudnn`) | `1` switches on cudnn-frontend's opt-in FROST (CuTe-DSL) SDPA engines for the process by setting the frontend's `CUDNN_FRONTEND_ENABLE_FROST_ENGINES` before FlashInfer's first `import cudnn`; `0` switches them off; unset leaves the frontend's own variable alone. The FROST rows are what make `backend="cudnn"` decode fast on Blackwell (d128 / d256 decode tiles, multi-token rows, sinks at `q_len_per_req == 1`); they need cudnn-frontend 1.30.0+ (a warning is emitted once when an older frontend is installed). Has no effect if cudnn was imported before flashinfer (warns). |
| `FLASHINFER_DECODE_AUTO_CUDNN` | unset | `flashinfer/decode.py` (`_auto_decode_prefers_cudnn`) | `BatchDecodeWithPagedKVCacheWrapper(backend="auto")`: unset applies the measured rule (SM100/SM103, FROST engines on, fp16/bf16 head_dim 128, GQA group dividing 128, batch >= 8 with `batch * num_kv_heads` within one wave of SMs, no RoPE / soft-cap / window, `q_len_per_req <= 4` with `q_len_per_req * group <= 128` resolve to `cudnn`; the d256 decode tile stays on fa2 until it leads across the batch range); `0` never resolves to cudnn; `1` applies the rule even without the FROST engines (benchmarking; the backend engine rejects sinks at `q_len_per_req == 1`). `wrapper.resolved_backend` reports the choice. |
