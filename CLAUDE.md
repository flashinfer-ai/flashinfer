...
| `FLASHINFER_SPECIALIZED_KERNEL_DISABLE` | unset | `flashinfer/fused_moe/experimental/moe_routing.py` | Exactly `1` makes the experimental fused MoE routing op decline: `moe_routing_supported()` answers `False` and all three entry points (`moe_routing_prologue` / `_align` / `_finalize`) serve their portable torch composition. Read at *call* time, so it takes effect without a reimport, and it is the operator escape hatch for an op that displaces computation the engine already had. Scope is that op only — it is not a repo-wide switch, and other ops (including the experimental fused GDN decode step) deliberately do not read it. |
