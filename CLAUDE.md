...
| `FLASHINFER_MEGA_FUSED_STAGE` | `1` | `flashinfer/moe_ep/backends/mega/kernel/{nvfp4,mxfp8}_cutedsl/staging.py` | `0` switches the cutedsl mega-MoE activation staging from the fused single-launch quant+repack kernel back to the original multi-kernel torch path. Bisection aid; the two paths are bit-identical (see `tests/moe_ep/test_fused_quant_stage.py`). |
| `FLASHINFER_MOE_EP_KNOB_CACHE` | `~/.cache/flashinfer/moe_ep_knob_cache.json` | `flashinfer/moe_ep/kernel_src/sm100/cutedsl_megamoe/shim/knob_cache.py` | Path of the persistent cutedsl mega-MoE knob cache (offline-tuned winners; `knobs=None` sessions resolve it with a pure lookup before the built-in heuristic). Set to `0`/`off` to disable. Populate with `python -m flashinfer.moe_ep.tune` or a one-off `knobs="auto"` run outside the engine. |
