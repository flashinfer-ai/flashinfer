...
template <typename T>
void _TestBatchPrefillKernelOneHotCorrectness(size_t num_kv_heads,
  ...
  for (uint32_t layer_idx = 0; layer_idx < num_layers; ++layer_idx) {
    ...
    for (uint32_t request_idx = 0; request_idx < batch_size; ++request_idx) {
      ...
      std::vector<T> k(kv_len * num_kv_heads * head_dim),
  ...
  kv_data.resize(page_counter * num_layers * 2 * num_kv_heads * page_size *
                 head_dim);
  flashinfer::paged_kv_t<T, int32_t> paged_kv_cpu(
      num_layers, 0, num_kv_heads, page_size, head_dim, batch_size,
      kv_data.data(), kv_indptr.data(), kv_indices.data(),
      kv_last_page_offset.data());
  for (uint32_t layer_idx = 0; layer_idx < num_layers; ++layer_idx) {
    paged_kv_cpu.layer_idx = layer_idx;
    cpu_reference::append_paged_kv_cache<T, int32_t>(
        paged_kv_cpu, keys[layer_idx], values[layer_idx], append_indptr);
  }
  ...
  for (uint32_t layer_idx = 0; layer_idx < num_layers; ++layer_idx) {
    ...
    for (uint32_t request_idx = 0; request_idx < batch_size; ++request_idx) {
      ...
      std::vector<T> o_ref = cpu_reference::single_mha<T, T>(
          q, keys[layer_idx][request_idx], values[layer_idx][request_idx],
      ...
      float result_accuracy =
          1. - float(num_result_errors_atol_1e_3_rtol_1e_3) /
                   float(q_len * num_qo_heads * head_dim);
      ...
      for (size_t causal : {false, true}) {
        for (size_t rotary_mode : {0, 1}) {
          _TestBatchPrefillKernelOneHotCorrectness<T>(
              num_kv_heads, head_dim, causal, RotaryMode(rotary_mode));
        }
      }
    }
  }
