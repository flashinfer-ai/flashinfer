// adapted from
// https://github.com/NVIDIA/TensorRT-LLM/blob/main/cpp/tests/unit_tests/kernels/allReduce/allReduceFusionTest.cu
...
template <typename DType>
__global__ void quantize_to_fp8_kernel(DType* data, __nv_fp8_e4m3* data_fp8, int size,
                                       float* scale_factor) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx >= size) return;
...
template <typename DType>
void quantize_to_fp8(DType* data, __nv_fp8_e4m3* data_fp8, int size, float* scale_factor,
                     cudaStream_t stream) {
  quantize_to_fp8_kernel<<<(size + 127) / 128, 128, 0, stream>>>(data, data_fp8, size,
                                                                 scale_factor);
}

template <typename T>
bool compare(int rank, void* p_real, void* p_ref, int size, std::string const& cmp_info = "",
             float atol = 1e-3) {
  ...
  for (int n = 0; n < size; ++n) {
    ...
    if (ar_debug && is_error && rank == 0 && print_cnt < 20) {
      ++print_cnt;
      if (rank == 0) printf("idx %d, v_real %f, v_ref %f\n", n, v_real, v_ref);
    }
  }
  bool pass = error_cnt == 0;
  if (!pass && rank == 0) {
    printf(
        "[%s] rank %d, atol %8.4f, max absolute diff %8.4f(%8.4f vs %8.4f), avg absolute diff "
        "%8.4f, absolute "
        "error count %d/%d\n",
...
struct CudaBuffer {
  ...
  void clear() { TLLM_CUDA_CHECK(cudaMemset(m_d_data, 0, m_size)); }
...
class TestRunner {
  ...
  static_assert(kFP4QuantOutSupport ||
                    Pattern != ar_fusion::AllReduceFusionPattern::kARResidualRMSNormFP4Quant,
                "kARResidualRMSNormFP4Quant is not supported for float dtype");

 public:
  TestRunner(int max_token_num, int hidden_dim) : m_mpi_comm(mpi::MpiComm::world()) {
  ...
  void reset_io() {
    ...
    // Because scale_out internally performs layout interleaving, not all elements will be covered,
    // so it should be reset before calling the kernel to ensure correct comparison results
  ...
  void verify(int token_num, int hidden_dim) {
    ...
    // We directly compare the results of AR+AddResidual here, as the accumulation order in NCCL's
    // AR might be inconsistent across different kernels. Therefore, we set atol to 1 (setting it to
    // 0 locally also passes the test).
    ...
    if constexpr (ar_fusion::HasAllReduceOut<Pattern>) {
      TLLM_CHECK(compare<DType>(m_rank, m_allreduce_out.host_data(), ref_output.host_data(),
                                message_size, "allreduce out", 1));
    }
    if constexpr (ar_fusion::HasResidual<Pattern>) {
      residual_add(ref_output.device_data<DType>(), m_residual_in.device_data<DType>(),
                   message_size, 0);
      if constexpr (ar_fusion::HasResidualOut<Pattern>) {
        TLLM_CHECK(compare<DType>(m_rank, m_residual_out.host_data(), ref_output.host_data(),
                                  message_size, "residual out", 1));
      }
    }
    if constexpr (ar_fusion::HasRMSNorm<Pattern>) {
      // This excludes the accumulation order errors introduced by AR and only compares the accuracy
      // of the RMSNorm. The atol is set to 1e-2 to exclude errors caused by accumulation order
      // changes due to differences in cluster/block size.
      invokeGeneralRmsNorm<DType, int8_t>(
          ref_output.device_data<DType>(), m_residual_out.device_data<DType>(),
      ...
      if constexpr (ar_fusion::HasNormOut<Pattern>) {
        TLLM_CHECK(compare<DType>(m_rank, m_norm_out.host_data(), ref_output.host_data(),
                                  message_size, "norm out", 1e-2));
      }
    }
    if constexpr (ar_fusion::GetQuantType<Pattern> == ar_fusion::QuantType::kFP4) {
      ...
      TLLM_CHECK(compare<int8_t>(m_rank, m_quant_out.host_data(), ref_output.host_data(),
                                 message_size / 2, "fp4 quant out", 0));
      TLLM_CHECK(compare<int8_t>(m_rank, m_scale_out.host_data(), ref_scale.host_data(),
                                 scale_out_size, "fp4 scale out", 0));
      ...
      quantize_to_fp8(m_norm_out.device_data<DType>(), ref_fp8_output.device_data<__nv_fp8_e4m3>(),
                      message_size, m_scale_factor.device_data<float>(), m_stream->get());
      TLLM_CHECK(compare<__nv_fp8_e4m3>(m_rank, m_quant_out.host_data(), ref_fp8_output.host_data(),
                                        message_size, "fp8 quant out", 0));
    }
  }
  ...
  void run_residual_add(int token_num, int hidden_dim) {
    residual_add(m_residual_out.device_data<DType>(), m_residual_in.device_data<DType>(),
                 token_num * hidden_dim, m_stream->get());
  }

  void run_rms_norm(int token_num, int hidden_dim) {
    invokeGeneralRmsNorm<DType, int8_t>(
        m_norm_out.device_data<DType>(), m_residual_out.device_data<DType>(),
        m_rms_gamma.device_data<DType>(), nullptr, m_params.rms_eps, token_num, hidden_dim,
  ...
  void run_fp4_quant(int token_num, int hidden_dim) {
    invokeFP4Quantization(token_num, hidden_dim, m_norm_out.device_data<DType>(),
                          m_scale_factor.device_data<float>(), m_quant_out.device_data<int64_t>(),
                          m_scale_out.device_data<int32_t>(), false,
                          tensorrt_llm::FP4QuantizationSFLayout::SWIZZLED, 128, m_stream->get());
  }

  void run_kernel(int token_num, int hidden_dim) { ar_fusion::allreduce_fusion_op(m_params); }

  ~TestRunner() { TLLM_NCCL_CHECK(ncclCommDestroy(m_nccl_comm)); }
...
#define TEST_AR_FUSION(DType, FusionPattern)                                               \
  {                                                                                        \
    ...
    for (int token_num = min_token_num; token_num <= max_token_num; token_num *= 2) {      \
      if (rank == 0) {                                                                     \
        printf("[Verify] pattern %-20s, dtype %-10s, token_num %-4d, hidden_dim %-4d ...", \
               #FusionPattern, #DType, token_num, hidden_dim);                             \
      }                                                                                    \
      runner.reset_io();                                                                   \
      runner.run_once(&Runner::run_kernel, token_num, hidden_dim);                         \
      runner.verify(token_num, hidden_dim);                                                \
...
TEST(Kernel_AllReduceFusion, Perf) {
  ...
  for (auto token_num : candidate_token_num) {
    ...
    if (rank == 0) {
      TLLM_LOG_INFO("token_num %-4d, hidden_dim %-4d, fusion kernel latency %4.4fus", token_num,
                    hidden_dim, latency);
    }
    auto nccl_latency =
        runner.benchmark(&Runner::run_nccl_allreduce, warmup, iter, token_num, hidden_dim);
    if (rank == 0) {
      TLLM_LOG_INFO("nccl allreduce latency %4.4fus", nccl_latency);
    }
    auto residual_latency =
        runner.benchmark(&Runner::run_residual_add, warmup, iter, token_num, hidden_dim);
    if (rank == 0) {
    ...
    auto quant_latency =
        runner.benchmark(&Runner::run_fp4_quant, warmup, iter, token_num, hidden_dim);
    if (rank == 0) {
      ...
      TLLM_LOG_INFO(
          "fusion kernel latency %4.4fus, nccl + ops latency %4.4fus, total speedup %2.4fx",
