...
void bench_flashinfer_batch_decode_mla(nvbench::state& state) {
  ...
  if (dev_to_bench.count(dev_id) == 0)
    return;
  ...
  flashinfer::paged_kv_mla_t<T, int32_t> paged_kv_mla(
      ...
      head_dim_kpe, 
      batch_size, 
      thrust::raw_pointer_cast(ckv_data.data()), 
      thrust::raw_pointer_cast(kpe_data.data()), 
  ...
  flashinfer::BatchDecodeHandlerPlanMLA<T, T, T, int32_t>(
      ...
      kv_indptr_host.data(), kv_last_page_len_host.data(), 
      batch_size, num_qo_heads, head_dim_ckv, page_size);
  
  state.exec([&](nvbench::launch&) {
    cudaError_t status =
        flashinfer::BatchDecodeWithPagedKVCacheWrapperMLA<T, T, T, int32_t>(
            &handler, thrust::raw_pointer_cast(q_nope.data()), thrust::raw_pointer_cast(q_pe.data()), 
            /*q_offset=*/nullptr, paged_kv_mla,
            thrust::raw_pointer_cast(o.data()), /*lse=*/nullptr, 
