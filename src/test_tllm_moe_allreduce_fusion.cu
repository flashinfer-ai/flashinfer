// adapted from
// https://github.com/NVIDIA/TensorRT-LLM/blob/main/cpp/tests/unit_tests/kernels/allReduce/moeAllReduceFusionTest.cu
...
float compare(int rank, void* _pa, void* _pb, int size, float scale,
  ...
  if (rank == 0) {
    TLLM_LOG_INFO("[%s] rank %d, max diff %f (diff threshold %f), avg diff %f, diff cnt %d/%d",
                  cmp_info.c_str(), rank, max_diff, diff_thres, tot_diff / std::max(diff_cnt, 1),
                  diff_cnt, size);
  }
...
struct CudaBuffer {
  ...
  CudaBuffer(int size_in_bytes = 0) : m_size(size_in_bytes), m_d_data(nullptr), m_h_data(nullptr) {
  ...
  void h2d() { TLLM_CUDA_CHECK(cudaMemcpy(m_d_data, m_h_data, m_size, cudaMemcpyHostToDevice)); }

  void d2h() { TLLM_CUDA_CHECK(cudaMemcpy(m_h_data, m_d_data, m_size, cudaMemcpyDeviceToHost)); }
...
__global__ void moe_reduction_kernel(IOType const* ggemm2_actexp_m_hidden_in,
                                     IOType const* fc2_m_hidden_in,
                                     ScaleType const* scale_actexp_m_in,
                                     int const* actexpi_to_global_expid, IOType* reduce_m_hidden_ou,
                                     int num_act_exp, int num_token, int hidden_size) {
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 900))

  static_assert(sizeof(ScaleType) >= sizeof(IOType),
                "This kernel assume scale type is more precious than io type");
...
#pragma unroll
    ...
    for (int actexp_i = 0; actexp_i < num_act_exp; ++actexp_i) {
      ...
      int thread_offset_across_actexp_token =
          actexp_i * (hidden_size * num_token) + thread_offset_across_token;
...
#pragma unroll
    ...
    // * FC2 + reduced(gGEMM2)
    ACC_TYPE fc2_data;
    fc2_data.packed = reinterpret_cast<int4 const*>(
        fc2_m_hidden_in)[thread_offset_across_token / ACC_TYPE::ELEM_PER_ACCESS];
#pragma unroll
    for (int i = 0; i < ACC_TYPE::ELEM_PER_ACCESS; ++i) {
    ...
    reinterpret_cast<int4*>(
        reduce_m_hidden_ou)[thread_offset_across_token / ACC_TYPE::ELEM_PER_ACCESS] =
        accumulator.packed;
  }
...
template <typename IOType, typename ScaleType>
void moe_reduction_kernel_launcher(IOType const* ggemm2_actexp_m_hidden_in,
                                   IOType const* fc2_m_hidden_in,
                                   ScaleType const* scale_actexp_m_in,
                                   int const* actexpi_to_global_expid, IOType* reduce_m_hidden_ou,
                                   int num_act_exp, int num_token, int hidden_size) {
  ...
  if (hidden_size % ACC_TYPE::ELEM_PER_ACCESS != 0) {
    printf(
        "FAILED. Unable to launch as hidden_size must be multiplier of "
        "ACC_TYPE::ELEM_PER_ACCESS\n");
    return;
  }
  ...
  printf(
      "* num_act_exp %d, num_token %d, hidden_size %d, num_warp_per_token %d, heuristic pick grid "
      "%d cluster %d "
      "block %d\n",
  ...
  TLLM_CUDA_CHECK(cudaLaunchKernelEx(&config, moe_reduction_kernel<IOType, ScaleType>,
                                     ggemm2_actexp_m_hidden_in, fc2_m_hidden_in, scale_actexp_m_in,
                                     actexpi_to_global_expid, reduce_m_hidden_ou, num_act_exp,
...
class MoEARFuseTestRunner {
  ...
  static constexpr ncclDataType_t kNCCLDataType =
 ...
 public:
  MoEARFuseTestRunner(int max_token_num, int hidden_dim, int max_expert_num)
    ...
    m_workspace = std::make_shared<ar_fusion::Workspace>(m_rank, m_world_size, max_token_num,
                                                         hidden_dim, m_stream);
    ...
    m_moe_reduction_active_experts_token_input.allocate(m_max_expert_num * m_message_size *
                                                        sizeof(DType));
    // [m, 7168]
    ...
    m_params.moe_reduction_scale_input =
        reinterpret_cast<float*>(m_moe_reduction_scale_input.device_data());
    m_params.moe_reduction_active_experts_token_input =
        m_moe_reduction_active_experts_token_input.device_data();
  ...
  float benchmark(Func func, int warmup, int iter, int token_num, int hidden_dim,
                  int num_active_expert = 0) {
    m_params.size = token_num * hidden_dim;
    m_params.hidden_dim = hidden_dim;
    cudaMemcpy(m_params.moe_reduction_device_num_experts, &num_active_expert, sizeof(int),
  ...
  void verify(int token_num, int hidden_dim, int num_active_expert) {
    ...
    // * MoE Reduction
    moe_reduction_kernel_launcher<DType, float>(
        m_moe_reduction_active_experts_token_input.device_data<DType>(),
        m_moe_reduction_token_input.device_data<DType>(),
        m_moe_reduction_scale_input.device_data<float>(), ref_output.device_data<DType>(),
        num_active_expert, token_num, hidden_dim);

    compare<DType>(m_rank, m_allreduce_in.host_data(), ref_output.host_data(), message_size, 1e-3,
                   "moe reduction out");

    // * AR
    TLLM_NCCL_CHECK(ncclAllReduce(m_allreduce_in.device_data(), ref_output.device_data(),
                                  message_size, kNCCLDataType, ncclSum, m_nccl_comm, 0));

    // * Add
    residual_add(ref_output.device_data<DType>(), m_residual_in.device_data<DType>(), message_size,
                 0);

    // * Norm
    invokeGeneralRmsNorm<DType, int8_t>(
        ref_output.device_data<DType>(), ref_output.device_data<DType>(),
        m_rms_gamma.device_data<DType>(), nullptr, m_params.rms_eps, token_num, hidden_dim,
        tensorrt_llm::common::QuantMode(), 0);

    compare<DType>(m_rank, m_norm_out.host_data(), ref_output.host_data(), message_size, 1e-3,
                   "norm out");
    ...
    invokeFP4Quantization(token_num, hidden_dim, m_norm_out.device_data<DType>(),
                          ...
                          ref_scale.device_data<int32_t>(), false,
                          tensorrt_llm::FP4QuantizationSFLayout::SWIZZLED, 128, 0);
    compare<int8_t>(m_rank, m_quant_out.host_data(), ref_output.host_data(), message_size / 2, 1e-3,
                    "quant out");
    compare<int8_t>(m_rank, m_scale_out.host_data(), ref_scale.host_data(), message_size / 16, 1e-3,
                    "scale out");
  }
  ...
  void run_nccl_allreduce(int token_num, int hidden_dim, int) {
    TLLM_NCCL_CHECK(ncclAllReduce(m_allreduce_in.device_data(), m_residual_out.device_data(),
                                  token_num * hidden_dim, kNCCLDataType, ncclSum, m_nccl_comm,
                                  m_stream->get()));
  }
  ...
  void run_moe_reduction(int token_num, int hidden_dim, int num_active_expert) {
    moe_reduction_kernel_launcher<DType, float>(
        m_moe_reduction_active_experts_token_input.device_data<DType>(),
        m_moe_reduction_token_input.device_data<DType>(),
        m_moe_reduction_scale_input.device_data<float>(), m_allreduce_in.device_data<DType>(),
        num_active_expert, token_num, hidden_dim);
  }
  ...
  void run_rms_norm(int token_num, int hidden_dim, int) {
    invokeGeneralRmsNorm<DType, int8_t>(
        m_residual_out.device_data<DType>(), m_norm_out.device_data<DType>(),
        m_rms_gamma.device_data<DType>(), nullptr, m_params.rms_eps, token_num, hidden_dim,
  ...
  void run_fp4_quant(int token_num, int hidden_dim, int) {
    invokeFP4Quantization(token_num,                            // m
                          ...
                          false, tensorrt_llm::FP4QuantizationSFLayout::SWIZZLED, 128,
                          m_stream->get());
  }
  ...
  ~MoEARFuseTestRunner() { TLLM_NCCL_CHECK(ncclCommDestroy(m_nccl_comm)); }
...
TEST(Kernel, MoEReduceAddARFuse) {
  ...
  for (auto token_num : candidate_token_num) {
    for (auto act_exp_num : candidate_active_expert_num) {
      auto latency = runner.benchmark(&MoEARFuseTestRunner<half>::run_kernel, warmup, iter,
                                      token_num, hidden_dim, act_exp_num);
      runner.verify(token_num, hidden_dim, act_exp_num);
      if (rank == 0) {
        TLLM_LOG_INFO("token_num %d, hidden_dim %d, act_exp_num %d, latency %fus", token_num,
                      hidden_dim, act_exp_num, latency);
      }
      auto moe_reduce_latency = runner.benchmark(&MoEARFuseTestRunner<half>::run_moe_reduction,
                                                 warmup, iter, token_num, hidden_dim, act_exp_num);
      if (rank == 0) {
        TLLM_LOG_INFO("moe reduce latency %fus", moe_reduce_latency);
      }
      auto nccl_latency = runner.benchmark(&MoEARFuseTestRunner<half>::run_nccl_allreduce, warmup,
                                           iter, token_num, hidden_dim);
      if (rank == 0) {
        TLLM_LOG_INFO("nccl allreduce latency %fus", nccl_latency);
      }
      auto residual_latency = runner.benchmark(&MoEARFuseTestRunner<half>::run_residual_add, warmup,
                                               iter, token_num, hidden_dim);
      if (rank == 0) {
        TLLM_LOG_INFO("residual add latency %fus", residual_latency);
      }
      auto rms_latency = runner.benchmark(&MoEARFuseTestRunner<half>::run_rms_norm, warmup, iter,
                                          token_num, hidden_dim);
      if (rank == 0) {
        TLLM_LOG_INFO("rms norm latency %fus", rms_latency);
      }
      auto quant_latency = runner.benchmark(&MoEARFuseTestRunner<half>::run_fp4_quant, warmup, iter,
                                            token_num, hidden_dim);
      if (rank == 0) {
        TLLM_LOG_INFO("fp4 quant latency %fus", quant_latency);
        auto tot_latency =
            moe_reduce_latency + nccl_latency + residual_latency + rms_latency + quant_latency;
        TLLM_LOG_INFO(
            "fusion kernel latency %fus, moe reduce + nccl + ops latency %fus, total speedup %fx",
