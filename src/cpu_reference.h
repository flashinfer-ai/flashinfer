...
template <typename T>
inline std::vector<float> apply_llama_rope(const T* input, size_t D,
                                           size_t offset, float rope_scale,
                                           float rope_theta) {
  std::vector<float> rst(D);
  std::vector<float> permuted_input(D);
  for (size_t k = 0; k < D; ++k) {
    permuted_input[k] =
        (k < D / 2) ? -float(input[k + D / 2]) : float(input[k - D / 2]);
  }
...
std::vector<dtype_out> single_mha(
    const std::vector<dtype_in>& q, const std::vector<dtype_in>& k,
  ...
  SWITCH_GQA_GROUP_SIZE(
      num_q_heads / num_kv_heads, GROUP_SIZE, {SWITCH_LAYOUT(layout, LAYOUT, {
        tensor_info_t<LAYOUT, GROUP_SIZE> info(qo_len, kv_len, num_kv_heads,
                                               head_dim);
        for (size_t qo_head_idx = 0; qo_head_idx < info.get_num_qo_heads();
             ++qo_head_idx) {
          ...
          for (size_t q_idx = 0; q_idx < qo_len; ++q_idx) {
            ...
            if (rotary_mode == RotaryMode::kLlama) {
              q_rotary_local = std::move(cpu_reference::apply_llama_rope(
                  q.data() + info.get_qo_elem_offset(q_idx, qo_head_idx, 0),
                  head_dim, q_idx + kv_len - qo_len, rope_scale, rope_theta));
            }
              ...
              switch (rotary_mode) {
                case RotaryMode::kNone: {
                  for (size_t feat_idx = 0; feat_idx < head_dim; ++feat_idx) {
                    att[kv_idx] += float(q[info.get_qo_elem_offset(
                                       q_idx, qo_head_idx, feat_idx)]) *
                                   float(k[info.get_kv_elem_offset(
                ...
                case RotaryMode::kLlama: {
                  k_rotary_local = std::move(cpu_reference::apply_llama_rope(
                      k.data() +
                          info.get_kv_elem_offset(kv_idx, kv_head_idx, 0),
            ...
            for (size_t feat_idx = 0; feat_idx < head_dim; ++feat_idx) {
              float o_float = 0.;
              for (size_t kv_idx = 0; kv_idx < kv_len; ++kv_idx) {
                o_float += att[kv_idx] * float(v[info.get_kv_elem_offset(
                                             kv_idx, kv_head_idx, feat_idx)]);
              }
              o[info.get_qo_elem_offset(q_idx, qo_head_idx, feat_idx)] =
                  dtype_out(o_float);
            }
          }
        }
...
void append_paged_kv_cache(paged_kv_t<T, IdxType> page_cpu,
  ...
  for (size_t i = 0; i < batch_size; ++i) {
    ...
    size_t seq_len =
        (num_pages_i - 1) * page_size + page_cpu.last_page_offset[i];
    ...
    for (size_t j = 0; j < append_seq_len; ++j) {
      ...
      for (size_t h = 0; h < num_heads; ++h) {
        std::copy(ki.begin() + (j * num_heads + h) * head_dim,
                  ...
                  page_cpu.data +
                      page_cpu.get_k_elem_offset(page_idx, h, entry_idx, 0));
        std::copy(vi.begin() + (j * num_heads + h) * head_dim,
                  vi.begin() + (j * num_heads + h + 1) * head_dim,
                  page_cpu.data +
                      page_cpu.get_v_elem_offset(page_idx, h, entry_idx, 0));
      }
    }
  }
}
