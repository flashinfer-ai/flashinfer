...
int main(int argc, char* argv[]) {
  ...
  std::vector<float> S(nranks * batch_size * num_heads);
  for (uint32_t r = 0; r < nranks; ++r) {
    for (uint32_t i = 0; i < batch_size; ++i) {
      for (uint32_t h = 0; h < num_heads; ++h) {
        for (uint32_t j = 0; j < head_dim; ++j) {
          V[r * batch_size * num_heads * head_dim + i * num_heads * head_dim + h * head_dim + j] =
              T((r + i + h + j) % 10);
        }
        S[r * batch_size * num_heads + i * num_heads + h] = (r + i + h) % 5 - 2;
      }
    }
  }
  
  thrust::device_vector<T> V_d(V);
  thrust::device_vector<float> S_d(S);
  ...
  auto status = MergeStates(
    ...
    thrust::raw_pointer_cast(V_reduce_ref.data()),
    thrust::raw_pointer_cast(S_reduce_ref.data()),
    nranks,
  ...
  memcpy(host_buf_ptr, V.data() + rank * batch_size * num_heads * head_dim, batch_size * num_heads * head_dim * sizeof(T));
  host_buf_ptr += batch_size * num_heads * head_dim * sizeof(T);
  memcpy(host_buf_ptr, S.data() + rank * batch_size * num_heads, batch_size * num_heads * sizeof(float));
  ...
  for (uint32_t i = 0; i < batch_size * num_heads * head_dim; ++i) {
    ...
    if (!utils::isclose(float(V_allreduce_h[i]), float(V_reduce_ref_h[i]), 1e-3, 1e-3)) {
      spdlog::error("rank: {}, i: {}, V_allreduce_h[i]: {}, V_reduce_ref_h[i]: {}", rank, i, float(V_allreduce_h[i]), float(V_reduce_ref_h[i]));
      V_num_results_error_atol_1e_3_rtol_1e_3++;
    }
  }

  float V_accuracy = 1. - float(V_num_results_error_atol_1e_3_rtol_1e_3) / float(batch_size * num_heads * head_dim);
  ...
  for (uint32_t i = 0; i < batch_size * num_heads; ++i) {
    ...
    if (!utils::isclose(float(S_allreduce_h[i]), float(S_reduce_ref_h[i]), 1e-3, 1e-3)) {
      spdlog::error("rank: {}, i: {}, S_allreduce_h[i]: {}, S_reduce_ref_h[i]: {}", rank, i, S_allreduce_h[i], S_reduce_ref_h[i]);
      S_num_results_error_atol_1e_3_rtol_1e_3++;
    }
  }

  float S_accuracy = 1. - float(S_num_results_error_atol_1e_3_rtol_1e_3) / float(batch_size * num_heads);
  ...
  if (V_accuracy < 0.9 || S_accuracy < 0.9 || V_nan_detected || S_nan_detected) {
