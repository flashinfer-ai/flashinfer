...
void _TestSinglePrefillKernelCorrectness(size_t q_len, size_t kv_len, size_t num_heads,
                                         size_t head_dim, QKVLayout layout,
                                         RotaryMode rotary_mode) {
  std::vector<DTypeIn> q(q_len * num_heads * head_dim);
  std::vector<DTypeIn> k(kv_len * num_heads * head_dim);
  std::vector<DTypeIn> v(kv_len * num_heads * head_dim);
  std::vector<DTypeOut> o(q_len * num_heads * head_dim);
  ...
  cudaError_t status = flashinfer::SinglePrefillWithKVCache<DTypeIn, DTypeOut>(
      ...
      thrust::raw_pointer_cast(tmp_d.data()), num_heads, q_len, kv_len, head_dim, layout,
      rotary_mode);

  thrust::host_vector<DTypeOut> o_h(o_d);
  std::vector<DTypeOut> o_ref = cpu_reference::single_mha<DTypeIn, DTypeOut>(
      q, k, v, q_len, kv_len, num_heads, head_dim, layout, rotary_mode);
  ...
  std::cout << "num_heads=" << num_heads << ", q_len=" << q_len << ", kv_len=" << kv_len
          ...
          for (size_t rotary_mode : {0, 1}) {
            for (size_t layout : {0, 1}) {
              _TestSinglePrefillKernelCorrectness<DTypeIn, DTypeOut>(
                  q_len, kv_len, num_heads, head_dim, QKVLayout(layout), RotaryMode(rotary_mode));
            }
          }
        }
      }
