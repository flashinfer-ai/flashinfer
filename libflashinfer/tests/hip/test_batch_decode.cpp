...
std::pair<float, bool> nan_detection_and_accuracy(const std::vector<DType>& o_host,
                                                  const std::vector<DType>& o_ref,
                                                  uint64_t batch_size, uint64_t num_qo_heads,
                                                  uint64_t head_dim) {
...
void _TestBatchDecodingKernelCorrectness(size_t page_size, size_t batch_size, size_t num_qo_heads,
  ...
  hipMalloc(&kv_last_page_len_device, kv_last_page_len.size() * sizeof(int32_t));
  hipMalloc(&q_device, q.size() * sizeof(DTypeQO));
  hipMalloc(&o_device, o_ref.size() * sizeof(DTypeQO));

  hipMemcpy(k_data_device, k_data.data(), k_data.size() * sizeof(DTypeKV), hipMemcpyHostToDevice);
  hipMemcpy(v_data_device, v_data.data(), v_data.size() * sizeof(DTypeKV), hipMemcpyHostToDevice);
  hipMemcpy(kv_indptr_device, kv_indptr.data(), kv_indptr.size() * sizeof(int32_t),
            hipMemcpyHostToDevice);
  hipMemcpy(kv_indices_device, kv_indices.data(), kv_indices.size() * sizeof(int32_t),
            hipMemcpyHostToDevice);
  hipMemcpy(kv_last_page_len_device, kv_last_page_len.data(),
            kv_last_page_len.size() * sizeof(int32_t), hipMemcpyHostToDevice);
  hipMemcpy(q_device, q.data(), q.size() * sizeof(DTypeQO), hipMemcpyHostToDevice);

  // create paged_kv object
  paged_kv_t<DTypeKV, int32_t> paged_kv(num_kv_heads, page_size, head_dim, batch_size, kv_layout,
                                        k_data_device, v_data_device, kv_indices_device,
                                        kv_indptr_device, kv_last_page_len_device);
  ...
  BatchDecodeHandlerPlan<DTypeQO, DTypeKV, DTypeQO, int32_t>(
      &handler, (void*)float_buffer, float_workspace_size_in_bytes, (void*)int_buffer,
      int_workspace_size_in_bytes, kv_indptr.data(), kv_last_page_len.data(), batch_size,
      num_qo_heads, num_kv_heads, head_dim, page_size, pos_encoding_mode);

  hipError_t status = BatchDecodeWithPagedKVCacheWrapper<DTypeQO, DTypeKV, DTypeQO, int32_t>(
  ...
  hipMemcpy(o_host.data(), o_device, o_ref.size() * sizeof(DTypeQO), hipMemcpyDeviceToHost);

  bool is_empty = o_host.empty();
  EXPECT_EQ(is_empty, false) << "Output is empty.";

  auto [result_accuracy, nan_detected] =
      nan_detection_and_accuracy<DTypeQO>(o_host, o_ref, batch_size, num_qo_heads, head_dim);
