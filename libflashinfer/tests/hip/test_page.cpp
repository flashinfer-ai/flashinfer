...
bool hasNaN(const torch::Tensor& tensor) { return torch::isnan(tensor).any().item<bool>(); }
...
torch::Tensor vectorToTensor(const std::vector<T>& vec) {
  torch::Tensor tensor =
      torch::from_blob(
          ...
          torch::TensorOptions().dtype(std::is_same<T, float>::value     ? torch::kFloat32
...
bool tensorIsClose(const torch::Tensor& a, const torch::Tensor& b, float atol = 1e-3,
                   float rtol = 1e-3) {
...
void _TestAppendPagedKVKernelCorrectness(size_t page_size, size_t batch_size, size_t num_heads,
  ...
  torch::TensorOptions tensor_options =
      torch::TensorOptions()
          .dtype(std::is_same<T, float>::value  ? torch::kFloat32
                 : std::is_same<T, half>::value ? torch::kFloat16
                                                : torch::kFloat32)  // Default to float32 if type is
                                                                    // not recognized
          .device(torch::kCUDA);
  ...
  torch::Tensor k_data_gpu = torch::zeros(
      {static_cast<int64_t>(max_num_pages * page_size * num_heads * head_dim)}, tensor_options);
  ...
  for (size_t round = 0; round < 2 * num_conv_rounds; ++round) {
    ...
    torch::Tensor indptr_gpu =
        torch::from_blob(indptr_cpu.data(), {static_cast<int64_t>(indptr_cpu.size())},
                         torch::kInt32)
            .clone()
            .to(torch::kCUDA);

    torch::Tensor indices_gpu =
        torch::from_blob(indices_cpu.data(), {static_cast<int64_t>(indices_cpu.size())},
                         torch::kInt32)
            .clone()
            .to(torch::kCUDA);

    torch::Tensor last_page_len_gpu =
        torch::from_blob(last_page_len.data(), {static_cast<int64_t>(batch_size)}, torch::kInt32)
            .clone()
            .to(torch::kCUDA);
    ...
    torch::Tensor batch_indices_gpu =
        torch::from_blob(batch_indices.data(), {static_cast<int64_t>(batch_indices.size())},
                         torch::kInt32)
            .clone()
            .to(torch::kCUDA);

    torch::Tensor positions_gpu =
        torch::from_blob(positions.data(), {static_cast<int64_t>(positions.size())}, torch::kInt32)
            .clone()
            .to(torch::kCUDA);

    // Create keys and values tensors
    torch::Tensor keys_gpu = torch::zeros(
        {static_cast<int64_t>(append_indptr.back() * num_heads * head_dim)}, tensor_options);
    ...
    for (size_t i = 0; i < batch_size; ++i) {
      torch::Tensor ki = torch::from_blob(keys[i].data(), {static_cast<int64_t>(keys[i].size())},
                                          tensor_options.device(torch::kCPU))
                             .clone()
                             .to(torch::kCUDA);

      torch::Tensor vi =
          torch::from_blob(values[i].data(), {static_cast<int64_t>(values[i].size())},
                           tensor_options.device(torch::kCPU))
              .clone()
              .to(torch::kCUDA);
    ...
    if (round % 2 == 0) {
      // Call prefill kernel
      hipError_t status = AppendPagedKVCache(paged_kv_gpu, static_cast<T*>(keys_gpu.data_ptr()),
      ...
      EXPECT_EQ(status, hipSuccess) << "AppendPagedKVCache kernel launch failed, error message: "
                                    << hipGetErrorString(status);
    } else {
      // Call decode kernel
      hipError_t status =
          AppendPagedKVCacheDecode(paged_kv_gpu, static_cast<T*>(keys_gpu.data_ptr()),
                                   static_cast<T*>(values_gpu.data_ptr()));
  ...
  torch::Tensor k_data_cpu =
      torch::from_blob(k_data_cpu_vec.data(), {static_cast<int64_t>(k_data_cpu_vec.size())},
                       tensor_options.device(torch::kCPU))
          .clone();

  torch::Tensor v_data_cpu =
      torch::from_blob(v_data_cpu_vec.data(), {static_cast<int64_t>(v_data_cpu_vec.size())},
                       tensor_options.device(torch::kCPU))
          .clone();
...
class PagedKVParameterizedTest
    : public PagedKVTest,
      public ::testing::WithParamInterface<std::tuple<size_t, size_t, size_t, size_t, QKVLayout>> {
};
...
INSTANTIATE_TEST_SUITE_P(PagedKVTests, PagedKVParameterizedTest,
                         ::testing::Combine(
                             // page_size
