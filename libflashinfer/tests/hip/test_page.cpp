...
void append_paged_kv_cache(paged_kv_t<T, IdxType> page_cpu, const std::vector<std::vector<T>>& keys,
 ...
 template <typename T>
 torch::Tensor vectorToTensor(const std::vector<T>& vec) {
   torch::Tensor tensor = torch::from_blob(
       const_cast<T*>(vec.data()), 
       {static_cast<int64_t>(vec.size())}, 
       torch::TensorOptions().dtype(
           std::is_same<T, float>::value ? torch::kFloat32 : 
           std::is_same<T, half>::value ? torch::kFloat16 : 
           std::is_same<T, int32_t>::value ? torch::kInt32 : torch::kFloat32
       )
   ).clone();
   return tensor;
 }
 ...
 bool tensorIsClose(const torch::Tensor& a, const torch::Tensor& b, float atol = 1e-3, float rtol = 1e-3) {
 ...
 void _TestAppendPagedKVKernelCorrectness(size_t page_size, size_t batch_size, size_t num_heads,
   ...
   torch::TensorOptions tensor_options = torch::TensorOptions()
       .dtype(std::is_same<T, float>::value ? torch::kFloat32 : 
              std::is_same<T, half>::value ? torch::kFloat16 :
              torch::kFloat32) // Default to float32 if type is not recognized
       .device(torch::kCUDA);
   
   torch::TensorOptions int_options = torch::TensorOptions()
       .dtype(torch::kInt32)
       .device(torch::kCUDA);
 
   // Create CPU tensors for reference
   std::vector<T> k_data_cpu_vec(max_num_pages * page_size * num_heads * head_dim, 0);
   std::vector<T> v_data_cpu_vec(max_num_pages * page_size * num_heads * head_dim, 0);
 
   // Create GPU tensors
   torch::Tensor k_data_gpu = torch::zeros(
       {static_cast<int64_t>(max_num_pages * page_size * num_heads * head_dim)}, 
       tensor_options);
   
   torch::Tensor v_data_gpu = torch::zeros(
       {static_cast<int64_t>(max_num_pages * page_size * num_heads * head_dim)}, 
       tensor_options);
   ...
   for (size_t round = 0; round < 2 * num_conv_rounds; ++round) {
     ...
     torch::Tensor indptr_gpu = torch::from_blob(
         indptr_cpu.data(), {static_cast<int64_t>(indptr_cpu.size())}, 
         torch::kInt32).clone().to(torch::kCUDA);
         
     torch::Tensor indices_gpu = torch::from_blob(
         indices_cpu.data(), {static_cast<int64_t>(indices_cpu.size())}, 
         torch::kInt32).clone().to(torch::kCUDA);
         
     torch::Tensor last_page_len_gpu = torch::from_blob(
         last_page_len.data(), {static_cast<int64_t>(batch_size)}, 
         torch::kInt32).clone().to(torch::kCUDA);
     ...
     torch::Tensor batch_indices_gpu = torch::from_blob(
         batch_indices.data(), {static_cast<int64_t>(batch_indices.size())}, 
         torch::kInt32).clone().to(torch::kCUDA);
         
     torch::Tensor positions_gpu = torch::from_blob(
     ...
     for (size_t i = 0; i < batch_size; ++i) {
       torch::Tensor ki = torch::from_blob(
           keys[i].data(), {static_cast<int64_t>(keys[i].size())}, 
           tensor_options.device(torch::kCPU)).clone().to(torch::kCUDA);
           
       torch::Tensor vi = torch::from_blob(
           values[i].data(), {static_cast<int64_t>(values[i].size())}, 
           tensor_options.device(torch::kCPU)).clone().to(torch::kCUDA);
           
       keys_gpu.slice(0, 
                     append_indptr[i] * num_heads * head_dim, 
                     append_indptr[i+1] * num_heads * head_dim).copy_(ki);
                     
       values_gpu.slice(0, 
                       append_indptr[i] * num_heads * head_dim, 
                       append_indptr[i+1] * num_heads * head_dim).copy_(vi);
     }
     ...
     if (round % 2 == 0) {
       ...
       hipError_t status = AppendPagedKVCache(
           paged_kv_gpu, 
           static_cast<T*>(keys_gpu.data_ptr()),
   ...
   torch::Tensor k_data_cpu = torch::from_blob(
       k_data_cpu_vec.data(), 
       {static_cast<int64_t>(k_data_cpu_vec.size())}, 
       tensor_options.device(torch::kCPU)).clone();
       
   torch::Tensor v_data_cpu = torch::from_blob(
       v_data_cpu_vec.data(), 
   ...
   // Check accuracy
   torch::Tensor k_close = torch::isclose(k_data_cpu_f32, k_data_gpu_cpu_f32, 1e-3, 1e-3);
   torch::Tensor v_close = torch::isclose(v_data_cpu_f32, v_data_gpu_cpu_f32, 1e-3, 1e-3);
 ...
 class PagedKVParameterizedTest : public PagedKVTest,
                                  public ::testing::WithParamInterface<
                                    std::tuple<size_t, size_t, size_t, size_t, QKVLayout>> {
 };
 
// This is disabled because std::vector cant handle __half dtypes. We will need a torch tensor wrapper
// to handle this.
...
//    _TestAppendPagedKVKernelCorrectness<half>(page_size, batch_size, num_heads, head_dim, kv_layout);
//  }
 ...
 TEST_P(PagedKVParameterizedTest, AppendPagedKVKernelCorrectnessTestFP32) {
   ...
   _TestAppendPagedKVKernelCorrectness<float>(page_size, batch_size, num_heads, head_dim, kv_layout);
 }
 ...
 INSTANTIATE_TEST_SUITE_P(
     ...
     ::testing::Combine(
         ...
         // kv_layout
         ::testing::Values(QKVLayout::kNHD, QKVLayout::kHND)
     )
 );
...
//  }
 ...
 TEST_F(PagedKVTest, AppendPagedKVKernelCorrectnessTestBF16) {
   _TestAppendPagedKVKernelCorrectness<__hip_bfloat16>(4, 2, 32, 64, QKVLayout::kHND);
 }
 #endif
 
 #ifdef FLASHINFER_ENABLE_FP8_E4M3
 TEST_F(PagedKVTest, AppendPagedKVKernelCorrectnessTestE4M3) {
   _TestAppendPagedKVKernelCorrectness<__hip_fp8_e4m3_fnuz>(4, 2, 32, 64, QKVLayout::kHND);
 }
 #endif
 
 #ifdef FLASHINFER_ENABLE_FP8_E5M2
 TEST_F(PagedKVTest, AppendPagedKVKernelCorrectnessTestE5M2) {
   _TestAppendPagedKVKernelCorrectness<__hip_fp8_e5m2_fnuz>(4, 2, 32, 64, QKVLayout::kHND);
 }
 #endif
