...
__device__ __forceinline__ void debug_print_registers(const char* stage, uint32_t lane_id,
                                                      uint32_t lane_in_group, uint32_t* regs,
                                                      int num_regs, uint32_t debug_group = 0) {
  // Only debug a specific group to avoid excessive output
  if (lane_id / 4 != debug_group) return;

  // Print identification info
  printf("STAGE: %s | Thread %d (lane_in_group=%d): ", stage, lane_id, lane_in_group);

  // Print raw 32-bit values
  printf("RAW=[");
  for (int i = 0; i < num_regs; i++) {
    printf("0x%08x", regs[i]);
    if (i < num_regs - 1) printf(", ");
  }
  printf("] | ");
...
__device__ __forceinline__ void transpose_4x4_half_registers_naive(uint32_t* R) {
  ...
  if (lane_id == 3) {
    debug_print_registers("After Round 1 shuffles", lane_id, lane_in_group, R, 2, 0);
  }
  ...
  if (lane_id == 0) {
    debug_print_registers("After Round 2 shuffles", lane_id, lane_in_group, R, 2, 0);
  }
...
int main() {
  ...
  const int values_per_thread = 8;  // Each thread stores 4 original + 4 transposed values
  ...
  FI_GPU_CALL(
      hipMemcpy(h_output.data(), d_output, total_values * sizeof(uint16_t), hipMemcpyDeviceToHost));

  // Verify the results
  bool success = true;
  std::cout << "Testing matrix transposition with shuffle operations..." << std::endl;
  ...
  if (success) {
    std::cout << "\nTranspose test PASSED! All values correctly transposed." << std::endl;
  } else {
