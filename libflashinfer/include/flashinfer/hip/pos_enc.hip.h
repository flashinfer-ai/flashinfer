...
#include "/root/flashinfer/include/flashinfer/hip/layout.hip.h"
#include "/root/flashinfer/include/flashinfer/hip/utils.hip.h"
#include "/root/flashinfer/include/flashinfer/hip/vec_dtypes.hip.h"
...
hipError_t BatchQKApplyRotaryPosIdsCosSinCache(
    ...
    size_t k_rope_stride_h, bool interleave, hipStream_t stream = nullptr) {
  int dev_id = 0;
  int num_sms = 0;
  ...
  DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {
    DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
      ...
      if ((nnz + bdy - 1) / bdy >= num_ctas_0) {
        dim3 nblks(nblks_x);
        dim3 nthrs(bdx, bdy);
        kernel_0<<<nblks, nthrs, 0, stream>>>(q, k, q_rope, k_rope, cos_sin_cache, pos_ids,
                                                                  nnz, num_qo_heads, num_kv_heads,
                                                                  rotary_dim, q_stride_n, q_stride_h,
                                                                  k_stride_n, k_stride_h, q_rope_stride_n,
                                                                  q_rope_stride_h, k_rope_stride_n,
                                                                  k_rope_stride_h);
      } else {
        ...
        auto kernel_1 =
            BatchQKApplyRotaryPosIdsCosSinCacheHeadParallelismKernel<INTERLEAVE, HEAD_DIM, vec_size,
                                                                     bdx, DType, IdType>;
        kernel_1<<<nblks, nthrs, 0, stream>>>(q, k, q_rope, k_rope, cos_sin_cache, pos_ids,
                                                                  nnz, num_qo_heads, num_kv_heads,
                                                                  rotary_dim, q_stride_n, q_stride_h,
                                                                  k_stride_n, k_stride_h, q_rope_stride_n,
                                                                  q_rope_stride_h, k_rope_stride_n,
  ...
  DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {
    DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
      ...
      if (nblks_x >= num_ctas_0) {
        ...
        kernel_0<<<nblks, nthrs, 0, stream>>>(q, k, q_rope, k_rope, pos_ids, nnz, num_qo_heads,
        ...
        // FLASHINFER_CUDA_CALL(hipLaunchKernelGGL((void*)kernel_1, nblks, nthrs, args, 0, stream));
        kernel_1<<<nblks, nthrs, 0, stream>>>(q, k, q_rope, k_rope, pos_ids, nnz, num_qo_heads,
                                             num_kv_heads, rotary_dim, q_stride_n, q_stride_h,
                                             k_stride_n, k_stride_h, q_rope_stride_n, q_rope_stride_h,
                                             k_rope_stride_n, k_rope_stride_h);
      }
  ...
  DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {
    DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
      ...
      void* args[] = {(void*)&q,
                      ...
                      (void*)&rope_rcp_scale,
                      (void*)&rope_rcp_theta};
      // FLASHINFER_CUDA_CALL(hipLaunchKernelGGL((void*)kernel, nblks, nthrs, args, 0, stream));
      kernel<<<nblks, nthrs, 0, stream>>>(q, k, q_rope, k_rope, indptr, offsets, batch_size,
                                         num_qo_heads, num_kv_heads, rotary_dim, q_stride_n,
                                         q_stride_h, k_stride_n, k_stride_h, q_rope_stride_n,
                                         q_rope_stride_h, k_rope_stride_n, k_rope_stride_h, smooth_a,
                                         smooth_b, rope_rcp_scale, rope_rcp_theta);
    });
  });
  ...
  DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {
    DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
      ...
      void* args[] = {(void*)&q,
                      ...
                      (void*)&rope_rcp_theta};
      // FLASHINFER_CUDA_CALL(hipLaunchKernelGGL((void*)kernel, nblks, nthrs, args, 0, stream));
      kernel<<<nblks, nthrs, 0, stream>>>(q, k, q_rope, k_rope, pos_ids, nnz, num_qo_heads,
