...
hipError_t BatchQKApplyRotaryPosIdsCosSinCache(
  ...
  FLASHINFER_CUDA_CALL(
      hipDeviceGetAttribute(&num_sms, hipDeviceAttributeMultiprocessorCount, dev_id));
  ...
  DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {
    DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
      ...
      if ((nnz + bdy - 1) / bdy >= num_ctas_0) {
        dim3 nblks(nblks_x);
        dim3 nthrs(bdx, bdy);
        BatchQKApplyRotaryPosIdsCosSinCacheKernel<INTERLEAVE, HEAD_DIM, vec_size, bdx, DType,
                                                  IdType><<<nblks, nthrs, 0, stream>>>(
            q, k, q_rope, k_rope, cos_sin_cache, pos_ids, nnz, num_qo_heads, num_kv_heads,
            rotary_dim, q_stride_n, q_stride_h, k_stride_n, k_stride_h, q_rope_stride_n,
        ...
        BatchQKApplyRotaryPosIdsCosSinCacheHeadParallelismKernel<INTERLEAVE, HEAD_DIM, vec_size,
                                                                 bdx, DType, IdType>
            <<<nblks, nthrs, 0, stream>>>(q, k, q_rope, k_rope, cos_sin_cache, pos_ids, nnz,
  ...
  DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {
    DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
      ...
      if (nblks_x >= num_ctas_0) {
        ...
        BatchQKApplyRotaryPosIdsKernel<INTERLEAVE, HEAD_DIM, vec_size, bdx, DType, IdType>
            <<<nblks, nthrs, 0, stream>>>(q, k, q_rope, k_rope, pos_ids, nnz, num_qo_heads,
        ...
        BatchQKApplyRotaryPosIdsHeadParallelismKernel<INTERLEAVE, HEAD_DIM, vec_size, bdx, DType,
                                                      IdType><<<nblks, nthrs, 0, stream>>>(
            q, k, q_rope, k_rope, pos_ids, nnz, num_qo_heads, num_kv_heads, rotary_dim, q_stride_n,
            q_stride_h, k_stride_n, k_stride_h, q_rope_stride_n, q_rope_stride_h, k_rope_stride_n,
            k_rope_stride_h, smooth_a, smooth_b, rope_rcp_scale, rope_rcp_theta);
      }
    });
  });
  ...
  DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {
    DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
      ...
      kernel<<<nblks, nthrs, 0, stream>>>(
          q, k, q_rope, k_rope, indptr, offsets, batch_size, num_qo_heads, num_kv_heads, rotary_dim,
          q_stride_n, q_stride_h, k_stride_n, k_stride_h, q_rope_stride_n, q_rope_stride_h,
          k_rope_stride_n, k_rope_stride_h, smooth_a, smooth_b, rope_rcp_scale, rope_rcp_theta);
    });
  });
