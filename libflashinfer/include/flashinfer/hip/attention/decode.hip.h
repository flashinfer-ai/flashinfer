    ...
    DISPATCH_COMPUTE_CAP_DECODE_NUM_STAGES_SMEM(compute_capacity, NUM_STAGES_SMEM, {
      ...
      if (seq_len <= 256 || tmp == nullptr) {
        ...
        if constexpr (AttentionVariant::use_softmax) {
          ...
          CHECK_HIP_ERROR(AttentionSum(tmp, o, num_chunks, 1, num_qo_heads, HEAD_DIM, stream));
        }
      }
    });
  });
    ...
    DISPATCH_COMPUTE_CAP_DECODE_NUM_STAGES_SMEM(compute_capacity, NUM_STAGES_SMEM, {
      ...
      if (tmp_v == nullptr) {
        ...
        BatchDecodeWithPagedKVCacheKernel<POS_ENCODING_MODE, NUM_STAGES_SMEM, tile_size_per_bdx,
            ...
            <<<nblks, nthrs, smem_size, stream>>>(params);
        if constexpr (AttentionVariant::use_softmax) {
          CHECK_HIP_ERROR(VariableLengthMergeStates(tmp_v, tmp_s, params.o_indptr, o, lse,
                                                    params.paged_kv.batch_size, nullptr,
                                                    num_qo_heads, HEAD_DIM, stream));
        } else {
          CHECK_HIP_ERROR(VariableLengthAttentionSum(tmp_v, params.o_indptr, o,
                                                     params.paged_kv.batch_size, nullptr,
                                                     num_qo_heads, HEAD_DIM, stream));
        }
      }
