  ...
  DISPATCH_GQA_GROUP_SIZE(num_qo_heads / num_kv_heads, GROUP_SIZE, {
    ...
    DISPATCH_COMPUTE_CAP_DECODE_NUM_STAGES_SMEM(compute_capacity, NUM_STAGES_SMEM, {
      ...
      if (seq_len <= 256 || tmp == nullptr) {
        ...
        SingleDecodeWithKVCacheKernel<POS_ENCODING_MODE, NUM_STAGES_SMEM, tile_size_per_bdx,
            vec_size, bdx, bdy, bdz, AttentionVariant, Params><<<nblks, nthrs, smem_size, stream>>>(params);
      } else {
  ...
  DISPATCH_GQA_GROUP_SIZE(num_qo_heads / num_kv_heads, GROUP_SIZE, {
    ...
    DISPATCH_COMPUTE_CAP_DECODE_NUM_STAGES_SMEM(compute_capacity, NUM_STAGES_SMEM, {
      ...
      if (tmp_v == nullptr) {
        ...
        vec_size, bdx, bdy, bdz, AttentionVariant, Params><<<nblks, nthrs, smem_size, stream>>>(params);
        if constexpr (AttentionVariant::use_softmax) {
          hipError_t status = VariableLengthMergeStates(tmp_v, tmp_s, params.o_indptr, o, lse,
                                                         params.paged_kv.batch_size, nullptr,
                                                         num_qo_heads, HEAD_DIM, stream);
        } else {
          hipError_t status = VariableLengthAttentionSum(tmp_v, params.o_indptr, o,
                                                          params.paged_kv.batch_size, nullptr,
                                                          num_qo_heads, HEAD_DIM, stream);
        }
      }
...
hipError_t BatchDecodeWithPagedKVCacheDispatchedMLA(Params params, typename Params::DTypeO* tmp_v,
  ...
  DISPATCH_COMPUTE_CAP_DECODE_NUM_STAGES_SMEM(compute_capacity, NUM_STAGES_SMEM, {
    ...
    if (tmp_v == nullptr) {
      ...
      params.partition_kv = false;
      BatchDecodeWithPagedKVCacheKernelMLA<NUM_STAGES_SMEM, vec_size_ckv, vec_size_kpe, bdx, bdy,
                                             bdz, tile_size_qo_heads, AttentionVariant, Params><<<nblks, nthrs, smem_size, stream>>>(params);
