...
/*!
 * \brief CUDA kernel to append new keys/values to the paged key-value cache in
 * the decode phase
 * \tparam head_dim The dimension of each head
...
hipError_t BlockSparseIndicesToVectorSparseOffset(
  ...
  // FLASHINFER_CUDA_CALL(cudaLaunchKernel((void *)kernel, num_sms,
  // num_threads,
  //                                       args, 0, stream));

  hipLaunchKernel((void*)kernel, dim3(num_sms), dim3(num_threads), args, 0, stream);
  // BlockSparseIndicesToVectorSparseOffsetsKernel<IdType><<<num_sms,
...
hipError_t AppendPagedKVMlaCache(paged_kv_mla_t<DType, IdType> paged_kv, DType* append_ckv,
  ...
  AppendPagedKVMlaCacheKernel<HEAD_CKV_DIM, HEAD_KPE_DIM, vec_size, DType, IdType>
      <<<nblks, nthrs, smem_size, stream>>>(paged_kv, append_ckv, append_kpe, batch_indices,
                                            positions, nnz, append_ckv_stride_n,
                                            append_kpe_stride_n);
