...
hipError_t BlockSparseIndicesToVectorSparseOffset(
  ...
  BlockSparseIndicesToVectorSparseOffsetsKernel<IdType><<<num_sms, num_threads, 0, stream>>>(
      block_sparse_indices, block_sparse_indptr, vector_sparse_offsets, vector_sparse_indptr, kv_lens,
      stride_block, stride_n, batch_size, block_size_fastdiv);
...
hipError_t AppendPagedKVCacheDecode(paged_kv_t<DType, IdType> paged_kv, DType* key, DType* value,
  ...
  DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
    ...
    dim3 nthrs(bdx, bdy);
    AppendPagedKVCacheDecodeKernel<HEAD_DIM, vec_size, DType, IdType> <<<nblks, nthrs, 0, stream>>>(paged_kv, key, value);
...
hipError_t AppendPagedKVCache(paged_kv_t<DType, IdType> paged_kv, DType* append_key,
  ...
  DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
    ...
    FLASHINFER_CUDA_CALL(hipOccupancyMaxActiveBlocksPerMultiprocessor(&num_blocks_per_sm, (void*)kernel,
                                                                       num_threads, smem_size));
    num_blocks_per_sm = min(num_blocks_per_sm, ceil_div(int(nnz), num_sms));
    dim3 nblks(num_blocks_per_sm * num_sms);
    dim3 nthrs(bdx, bdy);

    AppendPagedKVCacheKernel<HEAD_DIM, vec_size, DType, IdType><<<nblks, nthrs, smem_size, stream>>>(
        paged_kv, append_key, append_value, batch_indices, positions, nnz, append_k_stride_n,
