...
#if defined(PLATFORM_CUDA_DEVICE)
        FI_GPU_CALL(gpuLaunchKernelEx(&config, kernel, input, weight, output, d,
                                      stride_input, stride_output, weight_bias,
                                      eps));
#else
...
template <typename T>
gpuError_t FusedAddRMSNorm(T *input,
                           T *residual,
                           T *weight,
                           uint32_t batch_size,
                           uint32_t d,
                           ...
                           gpuStream_t stream = 0)
{
    ...
    DISPATCH_ALIGNED_VEC_SIZE(vec_size, VEC_SIZE, {
        ...
        FI_GPU_CALL(gpuFuncSetAttribute(
            ...
            smem_size));
#if defined(PLATFORM_CUDA_DEVICE)
        FI_GPU_CALL(gpuLaunchKernelEx(&config, kernel, input, residual, weight,
                                      d, stride_input, stride_residual,
                                      weight_bias, eps));
#else
            FI_GPU_CALL(gpuLaunchKernel((void*)kernel, nblks, nthrs, args, smem_size, stream));
#endif
...
template <typename T>
gpuError_t GemmaRMSNorm(T *input,
                        T *weight,
                        T *output,
                        uint32_t batch_size,
                        uint32_t d,
...
template <typename T>
gpuError_t GemmaFusedAddRMSNorm(T *input,
                                T *residual,
                                T *weight,
                                uint32_t batch_size,
                                uint32_t d,
