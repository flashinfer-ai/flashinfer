...
__device__ void fast_dequant_f8f16x4(uint32_t *input, uint2 *output)
{
                  ...
                  std::is_same_v<fp16_dtype, half>)
    {
        ...
        if constexpr (std::is_same_v<fp16_dtype, half>) {
            ...
            *(nv_bfloat162 *)&(output->x) = __hmul2(
                *reinterpret_cast<const nv_bfloat162 *>(&Out1), bias_reg);
            *(nv_bfloat162 *)&(output->y) = __hmul2(
                *reinterpret_cast<const nv_bfloat162 *>(&Out2), bias_reg);
        }
    }
}

template <> struct vec_cast<nv_bfloat16, __nv_fp8_e4m3>
{
    template <size_t vec_size>
...
#pragma unroll
            for (uint32_t i = 0; i < vec_size / 4; ++i) {
                fast_dequant_f8f16x4<__nv_fp8_e4m3, nv_bfloat16>(
...
template <> struct vec_cast<nv_bfloat16, __nv_fp8_e5m2>
{
    template <size_t vec_size>
    FLASHINFER_INLINE static void cast(nv_bfloat16 *dst,
...
#pragma unroll
            for (uint32_t i = 0; i < vec_size / 4; ++i) {
                fast_dequant_f8f16x4<__nv_fp8_e5m2, nv_bfloat16>(
                    (uint32_t *)&src[i * 4], (uint2 *)&dst[i * 4]);
            }
        }
    }
...
template <> struct vec_cast<__nv_fp8_e4m3, half>
{
    template <size_t vec_size>
...
template <> struct vec_cast<__nv_fp8_e5m2, half>
{
    template <size_t vec_size>
...
template <> struct vec_cast<half, __nv_fp8_e4m3>
{
    template <size_t vec_size>
...
#pragma unroll
            for (uint32_t i = 0; i < vec_size / 4; ++i) {
                fast_dequant_f8f16x4<__nv_fp8_e4m3, half>(
                    (uint32_t *)&src[i * 4], (uint2 *)&dst[i * 4]);
            }
        }
#endif // FLASHINFER_HARDWARE_FP8_CONVERSION_ENABLED
...
template <> struct vec_cast<half, __nv_fp8_e5m2>
{
    template <size_t vec_size>
...
#pragma unroll
            for (uint32_t i = 0; i < vec_size / 4; ++i) {
                fast_dequant_f8f16x4<__nv_fp8_e5m2, half>(
                    (uint32_t *)&src[i * 4], (uint2 *)&dst[i * 4]);
            }
        }
#endif // FLASHINFER_HARDWARE_FP8_CONVERSION_ENABLED
...
template <> struct vec_cast<float, nv_bfloat16>
{
    template <size_t vec_size>
...
#pragma unroll
            for (size_t i = 0; i < vec_size / 2; ++i) {
                ((float2 *)dst)[i] =
                    __bfloat1622float2(((nv_bfloat162 *)src)[i]);
            }
        }
    }
};

template <> struct vec_cast<nv_bfloat16, float>
{
    template <size_t vec_size>
...
// __nv_fp8_e4m3 x 1
template <> struct vec_t<__nv_fp8_e4m3, 1>
{
    __nv_fp8_e4m3 data;
...
// __nv_fp8_e4m3 x 2
template <> struct vec_t<__nv_fp8_e4m3, 2>
{
...
template <> struct vec_t<__nv_fp8_e4m3, 4>
{
...
template <> struct vec_t<__nv_fp8_e4m3, 8>
{
    uint2 data;
...
template <size_t vec_size> struct vec_t<__nv_fp8_e4m3, vec_size>
{
...
// __nv_fp8_e5m2 x 1
template <> struct vec_t<__nv_fp8_e5m2, 1>
{
    __nv_fp8_e5m2 data;
...
// __nv_fp8_e5m2 x 2
template <> struct vec_t<__nv_fp8_e5m2, 2>
{
...
template <> struct vec_t<__nv_fp8_e5m2, 4>
{
...
template <> struct vec_t<__nv_fp8_e5m2, 8>
{
    uint2 data;
...
template <size_t vec_size> struct vec_t<__nv_fp8_e5m2, vec_size>
{
    uint4 data[vec_size / 16];
...
// nv_bfloat16 x 1
template <> struct vec_t<nv_bfloat16, 1>
{
    nv_bfloat16 data;
...
// nv_bfloat16 x 2
template <> struct vec_t<nv_bfloat16, 2>
{
    nv_bfloat162 data;
...
template <> struct vec_t<nv_bfloat16, 4>
{
    uint2 data;
...
template <size_t vec_size> struct vec_t<nv_bfloat16, vec_size>
{
    uint4 data[vec_size / 8];
