...
static constexpr uint16_t fp16_ieee_from_fp32_value(float f)
{
    ...
    const float saturated_f =
        boost::math::ccmath::fabs<float>(f) * scale_to_inf;
...
static constexpr float fp16_ieee_to_fp32_value(uint16_t h)
{
    ...
    const float normalized_value =
        std::bit_cast<float>((two_w >> 4) + exp_offset) * exp_scale;
    ...
    const float denormalized_value =
        std::bit_cast<float>((two_w >> 17) | magic_mask) - magic_bias;
