...
__device__ __forceinline__ void load_matrix_m8n8x4(uint32_t *R, T *smem_ptr)
{
    ...
    FLASHINFER_RUNTIME_ASSERT(
        "Unsupported CUDA architecture for ldmatrix instruction");
#endif
}
 ...
 */
template <typename T>
__device__ __forceinline__ void load_matrix_m8n8x4_left_half(uint32_t *R,
                                                             T *smem_ptr)
{
#ifdef FLASHINFER_LDMATRIX_M8N8X4_ENABLED
    uint32_t smem_int_ptr =
 ...
 */
template <typename T>
__device__ __forceinline__ void load_matrix_m8n8x4_right_half(uint32_t *R,
                                                              T *smem_ptr)
{
#ifdef FLASHINFER_LDMATRIX_M8N8X4_ENABLED
    uint32_t smem_int_ptr =
 ...
 */
template <typename T>
__device__ __forceinline__ void load_matrix_m8n8x4_trans(uint32_t *R,
                                                         T *smem_ptr)
{
#ifdef FLASHINFER_LDMATRIX_M8N8X4_ENABLED
    uint32_t smem_int_ptr =
        static_cast<uint32_t>(__cvta_generic_to_shared(smem_ptr));
    asm volatile("ldmatrix.sync.aligned.trans.m8n8.x4.shared.b16 {%0, %1, %2, "
                 "%3}, [%4];\n"
                 : "=r"(R[0]), "=r"(R[1]), "=r"(R[2]), "=r"(R[3])
                 : "r"(smem_int_ptr));
#else
 ...
 */
template <typename T>
__device__ __forceinline__ void load_matrix_m8n8x4_trans_left_half(uint32_t *R,
                                                                   T *smem_ptr)
{
#ifdef FLASHINFER_LDMATRIX_M8N8X4_ENABLED
    uint32_t smem_int_ptr =
 ...
 */
template <typename T>
__device__ __forceinline__ void load_matrix_m8n8x4_trans_right_half(uint32_t *R,
                                                                    T *smem_ptr)
{
#ifdef FLASHINFER_LDMATRIX_M8N8X4_ENABLED
    uint32_t smem_int_ptr =
...
#else
    FLASHINFER_RUNTIME_ASSERT("fp8 mma instruction is only available for sm89, "
...
mma_sync_m16n16k16_row_col_f16f16f32(float *C, uint32_t *A, uint32_t *B)
{
    ...
    if constexpr (std::is_same_v<T, half>) {
        ...
        FLASHINFER_RUNTIME_ASSERT(
            "Unsupported CUDA architecture for mma instruction");
    }
#else
    FLASHINFER_RUNTIME_ASSERT(
        "Unsupported CUDA architecture for mma instruction");
...
__device__ __forceinline__ void m16k32_rowsum_f8f8f32(float *d, DType *s)
{
    ...
    if constexpr (std::is_same_v<DType, __nv_fp8_e4m3>) {
        asm volatile("{\n"
                     ...
                     "}\n"
                     : "=f"(d[0]), "=f"(d[1])
                     : "r"(s_u32[0]), "r"(s_u32[1]), "r"(s_u32[2]),
                       "r"(s_u32[3]), "r"(943208504), "r"(943208504), "f"(d[0]),
                       "f"(d[1]));
    }
    else { // e5m2
        asm volatile("{\n"
                     ...
                     : "=f"(d[0]), "=f"(d[1])
                     : "r"(s_u32[0]), "r"(s_u32[1]), "r"(s_u32[2]),
                       "r"(s_u32[3]), "r"(1010580540), "r"(1010580540),
                       "f"(d[0]), "f"(d[1]));
    }
#else
    FLASHINFER_RUNTIME_ASSERT("fp8 mma instruction is only available for sm89, "
...
__device__ __forceinline__ void m16k16_rowsum_f16f16f32(float *d, DType *s)
{
    ...
    if constexpr (std::is_same_v<DType, half>) {
        asm volatile("{\n"
                     ...
                     : "=f"(d[0]), "=f"(d[1])
                     : "r"(s_u32[0]), "r"(s_u32[1]), "r"(s_u32[2]),
                       "r"(s_u32[3]), "r"(1006648320), "r"(1006648320),
                       "f"(d[0]), "f"(d[1]));
    }
    else {
        asm volatile("{\n"
                     ...
                     : "=f"(d[0]), "=f"(d[1])
                     : "r"(s_u32[0]), "r"(s_u32[1]), "r"(s_u32[2]),
                       "r"(s_u32[3]), "r"(1065369472), "r"(1065369472),
                       "f"(d[0]), "f"(d[1]));
    }
#elif defined(FLASHINFER_MMA_F16F16F32_M16N8K8_ENABLED)
    if constexpr (std::is_same_v<DType, half>) {
        asm volatile("{\n"
                     ...
                     "}\n"
                     : "=f"(d[0]), "=f"(d[1])
                     : "r"(s_u32[0]), "r"(s_u32[1]), "r"(1006648320), "f"(d[0]),
                       "f"(d[1]));
        asm volatile("{\n"
                     "mma.sync.aligned.m16n8k8.row.col.f32.f16.f16.f32 "
                     "{%0,  _,  %1,  _},"
                     ...
                     "}\n"
                     : "=f"(d[0]), "=f"(d[1])
                     : "r"(s_u32[2]), "r"(s_u32[3]), "r"(1006648320), "f"(d[0]),
                       "f"(d[1]));
    }
    else {
        FLASHINFER_RUNTIME_ASSERT(
