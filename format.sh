...
################################################################################
# Optionally, check clang-tidy if it's available (no pip install attempt here)
################################################################################
echo 'flashinfer clang-tidy: Check Start'
...
# Decide how to run clang-tidy
if [[ "$CLANG_TIDY_AVAILABLE" != false ]]; then
    if [[ "$1" == '--files' ]]; then
        # Analyze only the provided files
        clang-tidy "${@:2}" -- -std=c++17
    elif [[ "$1" == '--all' ]]; then
        # Analyze all eligible C/C++ files
        clang_tidy_all
    else
        # Analyze only changed C/C++ files
        clang_tidy_changed
    fi
else
...
echo 'flashinfer clang-tidy: Done'

################################################################################
# Final check if anything changed after clang-tidy suggestions or other steps
################################################################################
if ! git diff --quiet &>/dev/null; then
    echo 'Reformatted files. Please review and stage the changes.'
    echo 'Changes not staged for commit:'
