  ...
  DISPATCH_context(
      ...
      USE_SLIDING_WINDOW, USE_LOGITS_SOFT_CAP, AttentionVariant, Params, [&] {
        DISPATCH_GQA_GROUP_SIZE(num_qo_heads / num_kv_heads, GROUP_SIZE, {
          auto work_estimation_func = BatchDecodeWithPagedKVCacheWorkEstimationDispatched<
              GROUP_SIZE, HEAD_DIM_QK, POS_ENCODING_MODE, AttentionVariant, Params>;
          cudaError_t status = DecodePlanWorkspaceSizeUpperBound<HEAD_DIM_QK, POS_ENCODING_MODE,
                                                                 AttentionVariant, Params>(
              float_workspace_size_in_bytes, int_workspace_size_in_bytes, max_batch_size,
              max_num_pages_per_request, num_qo_heads, page_size, enable_cuda_graph,
              /*stream=*/stream, work_estimation_func);

          TVM_FFI_ICHECK(status == cudaSuccess)
              << "BatchDecodeWithPagedKVCache workspace bound failed with error "
              << cudaGetErrorString(status);
          return true;
        });
      });
