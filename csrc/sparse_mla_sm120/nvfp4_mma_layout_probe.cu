...
__global__ void NVFP4M16N32K64Kernel(const uint8_t* a, const uint8_t* b, const uint8_t* sfa,
  ...
  ldmatrix_load_a_packed_16x32_bytes(a0, a1, a2, a3, &sm_a[0][0], kPackedKBytes, lane);

#pragma unroll
  for (int nt = 0; nt < kN / 8; ++nt) {
    uint32_t b0, b1;
    ldmatrix_load_b_packed_8x32_bytes(b0, b1, &sm_b[nt * 8][0], kPackedKBytes, lane);
...
__global__ void NVFP4M16N8K64CandidateMajorKernel(const uint8_t* a,
  ...
  ldmatrix_load_a_packed_16x32_bytes(a0, a1, a2, a3, &sm_a[0][0], kPackedKBytes, lane);
  load_candidate_major_e2m1_b_n8<kPvPackedDims>(b0, b1, &sm_b[0][0], 0, 0, lane);
