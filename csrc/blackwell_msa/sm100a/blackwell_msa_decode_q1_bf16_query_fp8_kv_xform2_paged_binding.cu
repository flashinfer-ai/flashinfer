...
void Run(TensorView arg_Q, TensorView arg_K, TensorView arg_V, TensorView arg_O, TensorView arg_msa_lse, TensorView arg_kv_indices, TensorView arg_kv_indptr, TensorView arg_task_kind, TensorView arg_task_request, TensorView arg_task_kv_head, int64_t arg_num_requests, int64_t arg_num_q_heads, int64_t arg_num_kv_heads, double arg_softmax_scale_log2, int64_t arg_msa_max_pages, int64_t grid_x, int64_t grid_y, int64_t grid_z, int64_t cuda_stream) {
  ...
  TVM_FFI_CHECK(arg_K.size(3) == host_extent_1, ValueError)
      << "K dimension 3 must be equal " << host_extent_1      << ", got " << arg_K.size(3);
  int64_t host_extent_2 = 128;
  TVM_FFI_CHECK(arg_K.size(2) >= host_extent_2, ValueError)
      << "K dimension 2 must be at least " << host_extent_2      << ", got " << arg_K.size(2);
  TVM_FFI_CHECK(arg_V.ndim() == 4, ValueError)
      << "V must have rank 4, got " << arg_V.ndim();
  int64_t host_extent_3 = 128;
  TVM_FFI_CHECK(arg_V.size(3) == host_extent_3, ValueError)
      << "V dimension 3 must be equal " << host_extent_3      << ", got " << arg_V.size(3);
  int64_t host_extent_4 = 128;
  TVM_FFI_CHECK(arg_V.size(2) >= host_extent_4, ValueError)
      << "V dimension 2 must be at least " << host_extent_4      << ", got " << arg_V.size(2);
