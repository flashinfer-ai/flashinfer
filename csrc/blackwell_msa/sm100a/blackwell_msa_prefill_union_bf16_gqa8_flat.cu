...
#define SMEM_Q2K_SMEM_STAGE_BYTES 4096
#define SMEM_Q2K_SMEM_STRIDE 4096
#define SMEM_KV_FP8_SMEM_OFF 123904
...
kernel_minimax_sparse_prefill_union_sm100(const __grid_constant__ CUtensorMap q, const __grid_constant__ CUtensorMap k, const __grid_constant__ CUtensorMap v, __nv_bfloat16* __restrict__ out, float* __restrict__ lse, float* __restrict__ temperature_lse, int* __restrict__ q2k_indices, int* __restrict__ cu_seqlens_q, int* __restrict__ cu_seqlens_k, int* __restrict__ q_offsets, int* __restrict__ kv_lens, int* __restrict__ page_table, int total_q, int num_q_heads, int num_kv_heads, int topk, int batch_size, int uniform_q_len, int max_pages, int causal, int derive_q_offset, float softmax_scale_log2, float lse_temperature_scale, int return_softmax_lse, int return_temperature_lse)
{
    ...
    uint8_t* kv_fp8_smem = reinterpret_cast<uint8_t*>(smem_raw + 123904);
    const int kv_fp8_smem_addr = smem + 123904;

    // Mbarrier init (15 groups, 22 barriers)
    ...
    if (warp <= 7) {
        ...
        { // softmax_main
            ...
            int stage_query_offset = stage * 16;
            int query_in_stage = my_row / 8;
            const int stage_query_capacity = 16;
            const int selection_rows = 32;
            ...
            {
                {
                    if (lane % 8 == 0 && row_valid != 0 && num_n_blocks <= 64) {
                        #pragma unroll 1
                ...
                unsigned int _shfl_0 = __shfl_sync(0xFFFFFFFF, selection_mask, lane / 8 * 8);
                selection_mask = _shfl_0;
                unsigned int _shfl_1 = __shfl_sync(0xFFFFFFFF, selection_mask_high, lane / 8 * 8);
    ...
    if (warp >= 8 && warp <= 11) {
        ...
        { // correction_main
            ...
            for (int stage_1 = 0; stage_1 < 2; stage_1++) {
                ...
                int stage_query_offset_1 = stage_1 * 16;
                int query_in_stage_1 = my_row_1 / 8;
                const int stage_query_capacity_1 = 16;
                int output_head = q_head_1 + my_row_1 % 8;
    ...
    if (warp == 12) {
        { // mma_warp_main
            ...
            kv_stage += 1;
            if (kv_stage == 1) { kv_stage = 0; kv_phase ^= 1; }
            int first_pv = 1;
    ...
    if (warp == 14) {
        { // load_warp_main
            ...
            if (elect_sync()) {
                ...
                {
                    tma_4d_gmem2smem(q1_smem_addr, (&q), 0, q_head_3, query_base_3 + 16, 0, q_full_addr + 8);
                }
            }
            int selection_base = (kv_head_3 * total_q + query_base_3) * topk;
            int selection_elems = q_valid_3 * topk;
            const int selection_rows_1 = 32;
            #pragma unroll 1
            ...
            for (int n_iter_1 = 0; n_iter_1 < num_n_blocks_3; n_iter_1++) {
                ...
                load_stage += 1;
                if (load_stage == 1) { load_stage = 0; _phase_kv_empty ^= 1; }
                {
                }
            }
        }
        ...
        { // transform_main
            unsigned int _phase_v_empty = 1;
            {
                ...
                for (int n_iter_2 = 0; n_iter_2 < num_n_blocks_4; n_iter_2++) {
                    ...
                    v_stage_1 += 1;
                    if (v_stage_1 == 1) { v_stage_1 = 0; _phase_v_empty ^= 1; }
                }
            }
        }
    }
