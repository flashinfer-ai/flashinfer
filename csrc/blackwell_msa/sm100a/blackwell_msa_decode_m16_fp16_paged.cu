...
kernel_minimax_sparse_decode_m16_paged_fp16_sm100(const __grid_constant__ CUtensorMap Q, const __grid_constant__ CUtensorMap Q_prefill, __half* __restrict__ Q_prefill_raw, const __grid_constant__ CUtensorMap K, const __grid_constant__ CUtensorMap K_prefill_pair, const __grid_constant__ CUtensorMap V, const __grid_constant__ CUtensorMap V_prefill_pair, const __grid_constant__ CUtensorMap KV, __half* __restrict__ O, float* __restrict__ partial_O, float* __restrict__ partial_M, float* __restrict__ partial_D, int* __restrict__ split_completion, float* __restrict__ msa_lse, int* __restrict__ kv_indices, int* __restrict__ qo_indptr, int* __restrict__ kv_indptr, int* __restrict__ kv_len_arr, int* __restrict__ task_kind, int* __restrict__ task_request, int* __restrict__ task_kv_head, int* __restrict__ task_q_tile, int* __restrict__ task_split, int* __restrict__ task_kv_tile_begin, int* __restrict__ task_kv_tile_end, int* __restrict__ task_qo_begin, int* __restrict__ task_qo_end, int* __restrict__ task_page_begin, int* __restrict__ task_page_end, int* __restrict__ status, int num_requests, int num_q_heads, int num_kv_heads, int max_kv_tiles, int max_splits, int max_task_claims, float softmax_scale_log2, int attention_mode, int is_causal, int derive_q_offset, int record_tasks, int msa_max_pages, int msa_split_policy)
{
        ...
        { // softmax_main
            ...
            for (int task_iter = 0; task_iter < max_task_claims + 1; task_iter++) {
                ...
                for (int pair = 0; pair < max_decode_pairs; pair++) {
                    ...
                    for (int h = 0; h < 16; h++) {
                        {
                            __half _hval_0 = __float2half_rn(exp_vals[h]);
                            uint16_t _bits_0 = *(uint16_t*)&_hval_0;
                ...
                for (int h_4 = 0; h_4 < 16; h_4++) {
                    if (group_size_1 > h_4) {
                        ...
                        if (splits == 1) {
                            ...
                            *(reinterpret_cast<__half*>(O + out_idx) + (0)) = __float2half_rn(merged * inv_sum[h_4]);
                        } else {
