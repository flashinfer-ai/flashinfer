...
#define SMEM_SMEM_PAGE_INDICES_STAGE_BYTES 4096
#define SMEM_SMEM_PAGE_INDICES_STRIDE 4096
...
kernel_blackwell_batch_attention_msa_decode_q1_fp8_paged_xform2_v1(const __grid_constant__ CUtensorMap Q, const __grid_constant__ CUtensorMap K, const __grid_constant__ CUtensorMap V, __nv_bfloat16* __restrict__ O, float* __restrict__ msa_lse, int* __restrict__ kv_indices, int* __restrict__ kv_indptr, int* __restrict__ task_kind, int* __restrict__ task_request, int* __restrict__ task_kv_head, int num_requests, int num_q_heads, int num_kv_heads, float softmax_scale_log2, int msa_max_pages)
{
    ...
    // Mbarrier init (11 groups, 29 barriers)
    ...
    #define kv_src_full_addr (mbar_base + 80)
    ...
    #define decode_done_addr (mbar_base + 224)
    ...
    if (warp <= 7) {
        ...
        { // softmax_main
            unsigned int total_work_items_s = num_requests * num_kv_heads;
            unsigned int first_work_item_s = blockIdx.x * num_kv_heads + blockIdx.y;
            unsigned int work_stride_s = gridDim.x * gridDim.y;
            unsigned int _phase_s_full_1 = 0;
            unsigned int _phase_s_full_0 = 0;
            #pragma unroll 1
            for (unsigned int work_idx_s = first_work_item_s; work_idx_s < total_work_items_s; work_idx_s += work_stride_s) {
                ...
                direct_request = work_idx_s / (unsigned int)num_kv_heads;
                int kv_tile_begin = 0;
                int kv_tile_end = 16;
                ...
                for (int pair = 0; pair < max_decode_pairs; pair++) {
                    ...
                    for (int c_2 = 0; c_2 < 16; c_2++) {
                        float _warp_reduce_0 = _tmem_load_0[c_2];
                        #pragma unroll
                        for (int offset = 16; offset > 0; offset >>= 1)
                            _warp_reduce_0 = max_noftz(_warp_reduce_0, __shfl_xor_sync(0xFFFFFFFF, _warp_reduce_0, offset));
                        _tmem_load_0[c_2] = _warp_reduce_0;
                    }
                    if (lane < 16) {
                        my_exch_ptr[warp_in_wg * 16 + lane] = _tmem_load_0[lane];
                    }
                    if (is_wg1 != 0) {
                    ...
                    for (int c_3 = 0; c_3 < 16; c_3++) {
                        float _shfl_0 = __shfl_sync(0xFFFFFFFF, tile_max[c_3], c_3);
                        tile_max[c_3] = _shfl_0;
                    }
                    float acc_scale[16];
                    #pragma unroll
                    for (int c_4 = 0; c_4 < 16; c_4++) {
                        float old_max = decode_row_max[row_state_base + c_4];
                        float _max_3 = max_noftz(old_max, tile_max[c_4]);
                        float new_max = _max_3;
                        decode_row_max[row_state_base + c_4] = new_max;
                        float delta = softmax_scale_log2 * (old_max - new_max);
                        float _exp2_0 = approx_exp2(delta);
                        acc_scale[c_4] = ((old_max > -BLACKWELL_MSA_INF) ? _exp2_0 : 1.0f);
                    }
                    ...
                    tmem_ld_x16(&_tmem_load_1[0], my_tmem_s);
                    if (token_in_block >= valid_cols) {
                        #pragma unroll
                        for (int c_5 = 0; c_5 < 16; c_5++) {
                            _tmem_load_1[c_5] = -BLACKWELL_MSA_INF;
                        }
                    }
                    float exp_vals[16];
                    #pragma unroll
                    for (int c_6 = 0; c_6 < 16; c_6++) {
                        ...
                        float _exp2_1 = approx_exp2(_tmem_load_1[c_6] * softmax_scale_log2 - max_scaled);
                    ...
                    for (int c_7 = 0; c_7 < 16; c_7++) {
                        float _warp_reduce_1 = exp_vals[c_7];
                        #pragma unroll
                        for (int offset = 16; offset > 0; offset >>= 1)
                            _warp_reduce_1 += __shfl_xor_sync(0xFFFFFFFF, _warp_reduce_1, offset);
                        exp_vals[c_7] = _warp_reduce_1;
                    }
                    #pragma unroll
                    for (int c_8 = 0; c_8 < 16; c_8++) {
                        float old_sum = decode_row_sum[row_state_base + c_8];
                        decode_row_sum[row_state_base + c_8] = old_sum * acc_scale[c_8] + exp_vals[c_8];
                    }
    ...
    if (warp >= 8 && warp <= 11) {
        ...
        { // correction_main
            unsigned int total_work_items_c = num_requests * num_kv_heads;
            unsigned int first_work_item_c = blockIdx.x * num_kv_heads + blockIdx.y;
            unsigned int work_stride_c = gridDim.x * gridDim.y;
            unsigned int _phase_corr_sig_0 = 0;
            unsigned int _phase_corr_sig_1 = 0;
            unsigned int _phase_o_full_0 = 0;
            #pragma unroll 1
            for (unsigned int work_idx_c = first_work_item_c; work_idx_c < total_work_items_c; work_idx_c += work_stride_c) {
                const int tmem_row_base_v_1 = warp % 4 * 32;
                const int corr_row = tmem_row_base_v_1 << 16;
                int d_idx = warp % 4 * 32 + lane;
                int request = 0;
                int kv_head = 0;
                request = work_idx_c / (unsigned int)num_kv_heads;
                kv_head = work_idx_c % (unsigned int)num_kv_heads;
                ...
                for (int pair_1 = 0; pair_1 < max_decode_pairs_1; pair_1++) {
                    ...
                    tmem_ld_x16(&_tmem_load_2[0], taddr + 64 + (unsigned int)corr_row);
                    float _tmem_load_3[16];
                    tmem_ld_x16(&_tmem_load_3[0], taddr + 32 + (unsigned int)corr_row);
                    #pragma unroll
                    for (int h_1 = 0; h_1 < 16; h_1++) {
                        _tmem_load_3[h_1] = _tmem_load_3[h_1] * _tmem_load_2[h_1];
                    }
                    tmem_st_x16_f32(taddr + 32 + (unsigned int)corr_row, _tmem_load_3);
                    asm volatile("tcgen05.wait::st.sync.aligned;" ::: "memory");
                    mbarrier_arrive(p_full_addr);
                    ...
                    tmem_ld_x16(&_tmem_load_4[0], taddr + 80 + (unsigned int)corr_row);
                    float _tmem_load_5[16];
                    tmem_ld_x16(&_tmem_load_5[0], taddr + 48 + (unsigned int)corr_row);
                    #pragma unroll
                    for (int h_2 = 0; h_2 < 16; h_2++) {
                        _tmem_load_5[h_2] = _tmem_load_5[h_2] * _tmem_load_4[h_2];
                    }
                    tmem_st_x16_f32(taddr + 48 + (unsigned int)corr_row, _tmem_load_5);
                    asm volatile("tcgen05.wait::st.sync.aligned;" ::: "memory");
                    mbarrier_arrive(p_full_addr + 8);
                }
                mbarrier_wait(corr_sig_addr, _phase_corr_sig_0);
                ...
                for (int c_9 = 0; c_9 < 16; c_9++) {
                    float _shfl_1 = __shfl_sync(0xFFFFFFFF, smem_exch0[c_9], c_9);
                    float _shfl_2 = __shfl_sync(0xFFFFFFFF, smem_exch1[c_9], c_9);
                    float _shfl_3 = __shfl_sync(0xFFFFFFFF, smem_corr0[c_9], c_9);
                    float _shfl_4 = __shfl_sync(0xFFFFFFFF, smem_corr1[c_9], c_9);
                    ...
                    scale1[c_9] = _exp2_3;
                    final_sum[c_9] = _shfl_3 * scale0[c_9] + _shfl_4 * scale1[c_9];
                }
                ...
                for (int h_4 = 0; h_4 < 16; h_4++) {
                    if (group_size_1 > h_4) {
                        float merged = _tmem_load_6[h_4] * scale0[h_4] + _tmem_load_7[h_4] * scale1[h_4];
    ...
    if (warp == 12) {
        { // mma_main
            unsigned int total_work_items_m = num_requests * num_kv_heads;
            unsigned int first_work_item_m = blockIdx.x * num_kv_heads + blockIdx.y;
            unsigned int work_stride_m = gridDim.x * gridDim.y;
            unsigned int _phase_q_full_0 = 0;
            unsigned int _phase_p_full_0 = 0;
            unsigned int _phase_p_full_1 = 0;
            unsigned int _phase_decode_done_0 = 0;
            #pragma unroll 1
            for (unsigned int work_idx_m = first_work_item_m; work_idx_m < total_work_items_m; work_idx_m += work_stride_m) {
                int direct_request_1 = 0;
                direct_request_1 = work_idx_m / (unsigned int)num_kv_heads;
    ...
    if (warp == 13) {
        { // producer_main
            unsigned int total_work_items_l = num_requests * num_kv_heads;
            unsigned int first_work_item_l = blockIdx.x * num_kv_heads + blockIdx.y;
            unsigned int work_stride_l = gridDim.x * gridDim.y;
            unsigned int _phase_q_empty_0 = 1;
            #pragma unroll 1
            for (unsigned int work_idx_l = first_work_item_l; work_idx_l < total_work_items_l; work_idx_l += work_stride_l) {
                int direct_request_2 = 0;
                int kv_head_1 = 0;
                direct_request_2 = work_idx_l / (unsigned int)num_kv_heads;
                kv_head_1 = work_idx_l % (unsigned int)num_kv_heads;
                int q_tile = 0;
                ...
                if (elect_sync()) {
                    ...
                    for (int native_ni = 0; native_ni < 4; native_ni++) {
                        ...
                        native_kv_stage += 1;
                        if (native_kv_stage == 8) { native_kv_stage = 0; native_kv_phase ^= 1; }
                    }
                    #pragma unroll 1
                    for (int native_ni_1 = 0; native_ni_1 < 16; native_ni_1++) {
                        ...
                        if (native_next_ni < native_num_n_blocks) {
                            ...
                            int page_head_6 = 0;
                            {
                                ...
                                if (selected_block_2 >= 0) {
                                    physical_page_2 = kv_indices[batch_0 * msa_max_pages + selected_block_2];
                                    if (physical_page_2 < 0) {
                                        valid_cols_4 = 0;
                                        physical_page_2 = 0;
                                    }
                                }
                                page_head_6 = physical_page_2 * num_kv_heads + kv_head_1;
                            }
