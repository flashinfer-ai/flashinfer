...
#include "blackwell_msa_prefill_m64_bf16_gqa16_flat.cu"
#undef CUtensorMap
#undef uint8_t
#undef uint16_t
#undef uint32_t
...
inline CUtensorMap EncodeTma_q(const TensorView& t) {
  ...
  TVM_FFI_CHECK(64u <= global_dim[0] && 16u <= global_dim[1] && 2u <= global_dim[3], ValueError)
      << "TMA box (64, 16, 4, 2) exceeds resolved global dims for 'q'";
  uint64_t global_strides[3] = {
      (uint64_t)((d1 * 16) / 8),
      (uint64_t)(((d2 * d1) * 16) / 8),
      (uint64_t)((64 * 16) / 8),
  };
  uint32_t box_dim[4] = {64u, 16u, 4u, 2u};
...
void Run(TensorView arg_q, TensorView arg_k, TensorView arg_v, TensorView arg_out, TensorView arg_lse, TensorView arg_temperature_lse, TensorView arg_q2k_indices, TensorView arg_cu_seqlens_q, TensorView arg_cu_seqlens_k, TensorView arg_q_offsets, TensorView arg_kv_lens, int64_t arg_total_q, int64_t arg_num_q_heads, int64_t arg_num_kv_heads, int64_t arg_topk, int64_t arg_batch_size, int64_t arg_uniform_q_len, int64_t arg_causal, int64_t arg_derive_q_offset, double arg_softmax_scale_log2, double arg_lse_temperature_scale, int64_t arg_return_softmax_lse, int64_t arg_return_temperature_lse, int64_t grid_x, int64_t grid_y, int64_t grid_z, int64_t cuda_stream) {
  ...
  void* kargs[] = {&p_q, &p_k, &p_v, &p_out, &p_lse, &p_temperature_lse, &p_q2k_indices, &p_cu_seqlens_q, &p_cu_seqlens_k, &p_q_offsets, &p_kv_lens, &v_total_q, &v_num_q_heads, &v_num_kv_heads, &v_topk, &v_batch_size, &v_uniform_q_len, &v_causal, &v_derive_q_offset, &v_softmax_scale_log2, &v_lse_temperature_scale, &v_return_softmax_lse, &v_return_temperature_lse};

  dim3 grid((uint32_t)grid_x, (uint32_t)grid_y, (uint32_t)grid_z);
  dim3 block(512u, 1u, 1u);

  cudaError_t status = cudaFuncSetAttribute(
      kernel_minimax_sparse_prefill_exact_union_m64_sm100, cudaFuncAttributeMaxDynamicSharedMemorySize, 134784);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_minimax_sparse_prefill_exact_union_m64_sm100) failed: " << cudaGetErrorString(status);
  status = cudaLaunchKernel(reinterpret_cast<const void*>(kernel_minimax_sparse_prefill_exact_union_m64_sm100), grid, block, kargs,
                            134784u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_minimax_sparse_prefill_exact_union_m64_sm100 launch failed: " << cudaGetErrorString(status);
}
