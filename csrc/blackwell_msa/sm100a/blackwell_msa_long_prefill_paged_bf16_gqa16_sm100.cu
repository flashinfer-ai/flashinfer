...
kernel_minimax_sparse_reverse_prefill_paged_bf16_gqa4_qload4_nobar_sm100(const __grid_constant__ CUtensorMap q, const __grid_constant__ CUtensorMap k, const __grid_constant__ CUtensorMap v, int* __restrict__ scheduler_metadata, int* __restrict__ k2q_row_ptr, int* __restrict__ k2q_qsplit_indices, uint8_t* __restrict__ partial_o, __nv_bfloat16* __restrict__ partial_scale, float* __restrict__ partial_lse, float* __restrict__ partial_temperature_lse, __nv_bfloat16* __restrict__ out, int* __restrict__ cu_seqlens_q, int* __restrict__ cu_seqlens_k, int* __restrict__ q_offsets, int* __restrict__ kv_lens, int* __restrict__ page_table, int q_group_segment_end_128, int q_group_segment_end_64, int q_group_segment_end_32, int q_group_segment_end_16, int q_group_segment_end_8, int q_group_segment_end_4, int q_group_segment_end_2, int total_q, int num_q_heads, int num_kv_heads, int total_rows, int nnz_per_head, int work_capacity, int num_work_items, int topk, int max_pages, int causal, int derive_q_offset, float softmax_scale_log2, float lse_temperature_scale, int return_temperature_lse)
{
                        ...
                        {
                            {
                                ...
                                for (int output_segment = 0; output_segment < 4; output_segment++) {
                                    ...
                                    tmem_ld_x32(&_tmem_load_0[0], output_row_addr + output_segment * 32);
                                    if (single_split != 0) {
                                        const float2 _scale2_12 = {inv_sum, inv_sum};
                                        #pragma unroll
                                        for (int _ls = 0; _ls < 16; _ls++)
                                            mul_f32x2_inplace(&reinterpret_cast<float2*>(_tmem_load_0)[_ls], _scale2_12);
                                        {
                                            ...
                                            *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(out + (final_row * 128 + (long long)output_segment * 32)))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                                            *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(out + (final_row * 128 + (long long)output_segment * 32)))[8]) = *reinterpret_cast<uint4*>(&_pk[4]);
                                        }
                                        {
                                            __nv_bfloat162 _pk[8];
                                            ...
                                            *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(out + (final_row * 128 + (long long)output_segment * 32 + 16)))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                                            *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(out + (final_row * 128 + (long long)output_segment * 32 + 16)))[8]) = *reinterpret_cast<uint4*>(&_pk[4]);
                                        }
                                    } else {
                                        float _tmem_load_0_min = _tmem_load_0[0];
                                        #pragma unroll
                                        for (int _lr = 1; _lr < 32; _lr++) {
                                            _tmem_load_0_min = fminf(_tmem_load_0_min, _tmem_load_0[_lr]);
                                        }
                                        float segment_min = _tmem_load_0_min;
                                        float2 _reg_reduce_max2_13 = {-BLACKWELL_MSA_INF, -BLACKWELL_MSA_INF};
                                        row_max_x32_accum(&_tmem_load_0[0], _reg_reduce_max2_13);
                                        float _tmem_load_0_max = row_max_reduce(_reg_reduce_max2_13);
                                        float segment_max = _tmem_load_0_max;
                                        float segment_center = 0.0f;
                                        float _max_0 = max_noftz(segment_max, -segment_min);
                                        float residual_abs_max = _max_0;
                                        ...
                                        if (output_valid != 0) {
                                            {
                                                ...
                                                for (int _ps = 0; _ps < 8; _ps++)
                                                    mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_0[0])[_ps], _prescale2_14);
                                                #else
                                                #pragma unroll
                                                for (int _ps = 0; _ps < 16; _ps++)
                                                    _tmem_load_0[0 + _ps] *= quant_scale;
                                                #endif
                                                unsigned int _fp8_pk[4];
                                                { unsigned short _lo, _hi;
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_0[0 + 1]), "f"(_tmem_load_0[0 + 0]));
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_0[0 + 3]), "f"(_tmem_load_0[0 + 2]));
                                                    _fp8_pk[0] = (unsigned)_lo | ((unsigned)_hi << 16);
                                                }
                                                { unsigned short _lo, _hi;
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_0[0 + 5]), "f"(_tmem_load_0[0 + 4]));
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_0[0 + 7]), "f"(_tmem_load_0[0 + 6]));
                                                    _fp8_pk[1] = (unsigned)_lo | ((unsigned)_hi << 16);
                                                }
                                                { unsigned short _lo, _hi;
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_0[0 + 9]), "f"(_tmem_load_0[0 + 8]));
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_0[0 + 11]), "f"(_tmem_load_0[0 + 10]));
                                                    _fp8_pk[2] = (unsigned)_lo | ((unsigned)_hi << 16);
                                                }
                                                { unsigned short _lo, _hi;
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_0[0 + 13]), "f"(_tmem_load_0[0 + 12]));
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_0[0 + 15]), "f"(_tmem_load_0[0 + 14]));
                                                    _fp8_pk[3] = (unsigned)_lo | ((unsigned)_hi << 16);
                                                }
                                                *reinterpret_cast<uint4*>(reinterpret_cast<unsigned char*>(partial_o + (partial_base + (long long)output_segment * 32)) + (0)) = *reinterpret_cast<uint4*>(_fp8_pk);
                                            }
                                            {
                                                ...
                                                #pragma unroll
                                                for (int _ps = 0; _ps < 8; _ps++)
                                                    mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_0[16])[_ps], _prescale2_15);
                                                #else
                                                #pragma unroll
                                                for (int _ps = 0; _ps < 16; _ps++)
                                                    _tmem_load_0[16 + _ps] *= quant_scale;
                                                #endif
                                                unsigned int _fp8_pk[4];
                                                { unsigned short _lo, _hi;
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_0[16 + 1]), "f"(_tmem_load_0[16 + 0]));
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_0[16 + 3]), "f"(_tmem_load_0[16 + 2]));
                                                    _fp8_pk[0] = (unsigned)_lo | ((unsigned)_hi << 16);
                                                }
                                                { unsigned short _lo, _hi;
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_0[16 + 5]), "f"(_tmem_load_0[16 + 4]));
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_0[16 + 7]), "f"(_tmem_load_0[16 + 6]));
                                                    _fp8_pk[1] = (unsigned)_lo | ((unsigned)_hi << 16);
                                                }
                                                { unsigned short _lo, _hi;
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_0[16 + 9]), "f"(_tmem_load_0[16 + 8]));
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_0[16 + 11]), "f"(_tmem_load_0[16 + 10]));
                                                    _fp8_pk[2] = (unsigned)_lo | ((unsigned)_hi << 16);
                                                }
                                                { unsigned short _lo, _hi;
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_0[16 + 13]), "f"(_tmem_load_0[16 + 12]));
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_0[16 + 15]), "f"(_tmem_load_0[16 + 14]));
                                                    _fp8_pk[3] = (unsigned)_lo | ((unsigned)_hi << 16);
                                                }
                                                *reinterpret_cast<uint4*>(reinterpret_cast<unsigned char*>(partial_o + (partial_base + (long long)output_segment * 32 + 16)) + (0)) = *reinterpret_cast<uint4*>(_fp8_pk);
                                            }
                                        }
                                    }
                                }
            ...
            for (int stage_iteration_1 = 0; stage_iteration_1 < group_count_0_1 / 2; stage_iteration_1++) {
                ...
                if (whole_group_valid_1 != 0) {
                    ...
                    long long partial_base_1 = partial_row_1 * 128;
                    {
                        ...
                        {
                            {
                                float pending_scale_1 = 0.0f;
                                #pragma unroll 1
                                for (int output_segment_1 = 0; output_segment_1 < 4; output_segment_1++) {
                                    float _tmem_load_6[32];
                                    tmem_ld_x32(&_tmem_load_6[0], output_row_addr_1 + output_segment_1 * 32);
                                    if (single_split_1 != 0) {
                                        const float2 _scale2_12 = {inv_sum_1, inv_sum_1};
                                        #pragma unroll
                                        for (int _ls = 0; _ls < 16; _ls++)
                                            mul_f32x2_inplace(&reinterpret_cast<float2*>(_tmem_load_6)[_ls], _scale2_12);
                                        {
                                            __nv_bfloat162 _pk[8];
                                            ...
                                            *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(out + (final_row_1 * 128 + (long long)output_segment_1 * 32)))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                                            *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(out + (final_row_1 * 128 + (long long)output_segment_1 * 32)))[8]) = *reinterpret_cast<uint4*>(&_pk[4]);
                                        }
                                        {
                                            __nv_bfloat162 _pk[8];
                                            ...
                                            *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(out + (final_row_1 * 128 + (long long)output_segment_1 * 32 + 16)))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                                            *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(out + (final_row_1 * 128 + (long long)output_segment_1 * 32 + 16)))[8]) = *reinterpret_cast<uint4*>(&_pk[4]);
                                        }
                                    } else {
                                        ...
                                        float segment_min_1 = _tmem_load_6_min;
                                        float2 _reg_reduce_max2_13 = {-BLACKWELL_MSA_INF, -BLACKWELL_MSA_INF};
                                        row_max_x32_accum(&_tmem_load_6[0], _reg_reduce_max2_13);
                                        float _tmem_load_6_max = row_max_reduce(_reg_reduce_max2_13);
                                        float segment_max_1 = _tmem_load_6_max;
                                        float segment_center_1 = 0.0f;
                                        float _max_3 = max_noftz(segment_max_1, -segment_min_1);
                                        float residual_abs_max_1 = _max_3;
                                        ...
                                        if (output_valid_1 != 0) {
                                            {
                                                ...
                                                #pragma unroll
                                                for (int _ps = 0; _ps < 8; _ps++)
                                                    mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_6[0])[_ps], _prescale2_14);
                                                #else
                                                #pragma unroll
                                                for (int _ps = 0; _ps < 16; _ps++)
                                                    _tmem_load_6[0 + _ps] *= quant_scale_1;
                                                #endif
                                                unsigned int _fp8_pk[4];
                                                { unsigned short _lo, _hi;
                                                ...
                                                *reinterpret_cast<uint4*>(reinterpret_cast<unsigned char*>(partial_o + (partial_base_1 + (long long)output_segment_1 * 32)) + (0)) = *reinterpret_cast<uint4*>(_fp8_pk);
                                            }
                                            {
                                                ...
                                                #pragma unroll
                                                for (int _ps = 0; _ps < 8; _ps++)
                                                    mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_6[16])[_ps], _prescale2_15);
                                                #else
                                                #pragma unroll
                                                for (int _ps = 0; _ps < 16; _ps++)
                                                    _tmem_load_6[16 + _ps] *= quant_scale_1;
                                                #endif
                                                unsigned int _fp8_pk[4];
                                                { unsigned short _lo, _hi;
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_6[16 + 1]), "f"(_tmem_load_6[16 + 0]));
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_6[16 + 3]), "f"(_tmem_load_6[16 + 2]));
                                                    _fp8_pk[0] = (unsigned)_lo | ((unsigned)_hi << 16);
                                                }
                                                { unsigned short _lo, _hi;
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_6[16 + 5]), "f"(_tmem_load_6[16 + 4]));
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_6[16 + 7]), "f"(_tmem_load_6[16 + 6]));
                                                    _fp8_pk[1] = (unsigned)_lo | ((unsigned)_hi << 16);
                                                }
                                                { unsigned short _lo, _hi;
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_6[16 + 9]), "f"(_tmem_load_6[16 + 8]));
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_6[16 + 11]), "f"(_tmem_load_6[16 + 10]));
                                                    _fp8_pk[2] = (unsigned)_lo | ((unsigned)_hi << 16);
                                                }
                                                { unsigned short _lo, _hi;
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_6[16 + 13]), "f"(_tmem_load_6[16 + 12]));
                                                    asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_6[16 + 15]), "f"(_tmem_load_6[16 + 14]));
                                                    _fp8_pk[3] = (unsigned)_lo | ((unsigned)_hi << 16);
                                                }
                                                *reinterpret_cast<uint4*>(reinterpret_cast<unsigned char*>(partial_o + (partial_base_1 + (long long)output_segment_1 * 32 + 16)) + (0)) = *reinterpret_cast<uint4*>(_fp8_pk);
                                            }
                                        }
                                    }
                                }
