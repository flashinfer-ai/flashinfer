...
#include "blackwell_msa_reverse_prefill_bf16_query_fp8_kv_flat_topk8_qagg_pdl_reduce.cu"
#undef CUtensorMap
#undef uint8_t
#undef uint16_t
#undef uint32_t
...
void Run(TensorView arg_partial_o, TensorView arg_partial_lse, TensorView arg_partial_temperature_lse, TensorView arg_split_counts, TensorView arg_q_order, TensorView arg_contributor_work_ids, TensorView arg_completion_counts, TensorView arg_out, TensorView arg_lse, TensorView arg_temperature_lse, int64_t arg_total_q, int64_t arg_num_q_heads, int64_t arg_num_kv_heads, int64_t arg_qhead_per_kv, int64_t arg_topk, int64_t arg_generation, int64_t arg_return_softmax_lse, int64_t arg_return_temperature_lse, int64_t grid_x, int64_t grid_y, int64_t grid_z, int64_t cuda_stream) {
  ...
  CheckCudaTensor(arg_q_order, "q_order");
  CheckDtype(arg_q_order, "q_order", 0, 32, 1);
  CheckContiguous(arg_q_order, "q_order");
  CheckCudaTensor(arg_contributor_work_ids, "contributor_work_ids");
  CheckDtype(arg_contributor_work_ids, "contributor_work_ids", 0, 32, 1);
  CheckContiguous(arg_contributor_work_ids, "contributor_work_ids");
  ...
  TVM_FFI_CHECK(arg_topk >= -2147483648LL && arg_topk <= 2147483647LL, ValueError)
      ...
      << " is outside i32 range [-2147483648, 2147483647]";
  TVM_FFI_CHECK(arg_generation >= 0LL && arg_generation <= 4294967295LL, ValueError)
  ...
  CheckSameCudaDevice(arg_q_order, arg_partial_o, "q_order", "partial_o");
  CheckSameCudaDevice(arg_contributor_work_ids, arg_partial_o, "contributor_work_ids", "partial_o");
  CheckSameCudaDevice(arg_completion_counts, arg_partial_o, "completion_counts", "partial_o");
  ...
  host_extent_7 = HostCheckedExtentMul(host_extent_7, HostCheckedExtentValue(static_cast<int64_t>(arg_qhead_per_kv), "qhead_per_kv"), "qhead_per_kv * num_kv_heads");
  host_extent_7 = HostCheckedExtentMul(host_extent_7, HostCheckedExtentValue(static_cast<int64_t>(arg_num_kv_heads), "num_kv_heads"), "qhead_per_kv * num_kv_heads");
  TVM_FFI_CHECK(arg_num_q_heads == host_extent_7, ValueError)
      << "num_q_heads must equal " << host_extent_7      << ", got " << arg_num_q_heads;
  ...
  void* p_q_order = arg_q_order.data_ptr();
  void* p_contributor_work_ids = arg_contributor_work_ids.data_ptr();
  ...
  uint32_t v_generation = (uint32_t)arg_generation;
  ...
  void* kargs[] = {&p_partial_o, &p_partial_lse, &p_partial_temperature_lse, &p_split_counts, &p_q_order, &p_contributor_work_ids, &p_completion_counts, &p_out, &p_lse, &p_temperature_lse, &v_total_q, &v_num_q_heads, &v_num_kv_heads, &v_qhead_per_kv, &v_topk, &v_generation, &v_return_softmax_lse, &v_return_temperature_lse};

  dim3 grid((uint32_t)grid_x, (uint32_t)grid_y, (uint32_t)grid_z);
  dim3 block(256u, 1u, 1u);

  cudaError_t status = cudaFuncSetAttribute(
      kernel_minimax_sparse_reverse_prefill_combine_unscaled_fp8_qagg_pdl_dualcohort_sm100, cudaFuncAttributeMaxDynamicSharedMemorySize, 0);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_minimax_sparse_reverse_prefill_combine_unscaled_fp8_qagg_pdl_dualcohort_sm100) failed: " << cudaGetErrorString(status);
  ...
  config.attrs = attrs;
  config.numAttrs = 1;
  status = cudaLaunchKernelExC(&config, reinterpret_cast<const void*>(kernel_minimax_sparse_reverse_prefill_combine_unscaled_fp8_qagg_pdl_dualcohort_sm100), kargs);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_minimax_sparse_reverse_prefill_combine_unscaled_fp8_qagg_pdl_dualcohort_sm100 launch failed: " << cudaGetErrorString(status);
}
