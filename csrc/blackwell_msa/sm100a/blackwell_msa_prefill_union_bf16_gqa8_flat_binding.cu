...
#include "blackwell_msa_prefill_union_bf16_gqa8_flat.cu"
#undef CUtensorMap
#undef uint8_t
#undef uint16_t
#undef uint32_t
...
inline CUtensorMap EncodeTma_q(const TensorView& t) {
  ...
  TVM_FFI_CHECK(64u <= global_dim[0] && 8u <= global_dim[1] && 2u <= global_dim[3], ValueError)
      << "TMA box (64, 8, 16, 2) exceeds resolved global dims for 'q'";
  uint64_t global_strides[3] = {
      (uint64_t)((d1 * 16) / 8),
      (uint64_t)(((d2 * d1) * 16) / 8),
      (uint64_t)((64 * 16) / 8),
  };
  uint32_t box_dim[4] = {64u, 8u, 16u, 2u};
...
void Run(TensorView arg_q, TensorView arg_k, TensorView arg_v, TensorView arg_out, TensorView arg_lse, TensorView arg_temperature_lse, TensorView arg_q2k_indices, TensorView arg_cu_seqlens_q, TensorView arg_cu_seqlens_k, TensorView arg_q_offsets, TensorView arg_kv_lens, TensorView arg_page_table, int64_t arg_total_q, int64_t arg_num_q_heads, int64_t arg_num_kv_heads, int64_t arg_topk, int64_t arg_batch_size, int64_t arg_uniform_q_len, int64_t arg_max_pages, int64_t arg_causal, int64_t arg_derive_q_offset, double arg_softmax_scale_log2, double arg_lse_temperature_scale, int64_t arg_return_softmax_lse, int64_t arg_return_temperature_lse, int64_t grid_x, int64_t grid_y, int64_t grid_z, int64_t cuda_stream) {
  ...
  host_extent_3 = HostCheckedExtentMul(host_extent_3, HostCheckedExtentValue(static_cast<int64_t>(arg_total_q), "total_q"), "128 * total_q * num_q_heads");
  host_extent_3 = HostCheckedExtentMul(host_extent_3, HostCheckedExtentValue(static_cast<int64_t>(arg_num_q_heads), "num_q_heads"), "128 * total_q * num_q_heads");
  TVM_FFI_CHECK(arg_out.numel() >= host_extent_3, ValueError)
      << "out requires at least " << (host_extent_3)      << " TensorView storage elements, got " << arg_out.numel();
  int64_t host_extent_4 = 1;
  host_extent_4 = HostCheckedExtentMul(host_extent_4, HostCheckedExtentValue(static_cast<int64_t>(arg_total_q), "total_q"), "total_q * num_q_heads");
  host_extent_4 = HostCheckedExtentMul(host_extent_4, HostCheckedExtentValue(static_cast<int64_t>(arg_num_q_heads), "num_q_heads"), "total_q * num_q_heads");
  TVM_FFI_CHECK(arg_lse.numel() >= host_extent_4, ValueError)
      << "lse requires at least " << (host_extent_4)      << " TensorView storage elements, got " << arg_lse.numel();
  ...
  int64_t host_extent_6 = 16;
  TVM_FFI_CHECK(arg_topk == host_extent_6, ValueError)
      << "topk must equal " << host_extent_6      << ", got " << arg_topk;
  ...
  cudaError_t status = cudaFuncSetAttribute(
      kernel_minimax_sparse_prefill_union_sm100, cudaFuncAttributeMaxDynamicSharedMemorySize, 140288);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
