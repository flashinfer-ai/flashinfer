            ...
            for (int task_iter = 0; task_iter < max_task_claims + 1; task_iter++) {
                ...
                if (kind == 1) {
                    ...
                    for (int pair = 0; pair < max_decode_pairs; pair++) {
                        ...
                        for (int h = 0; h < 16; h++) {
                            {
                                __nv_bfloat16 _bval_1766654592 = __float2bfloat16_rn(exp_vals[h]);
                                uint16_t _bits_1766654592 = *(uint16_t*)&_bval_1766654592;
                                const void* _ptr_1766654592 = reinterpret_cast<const void*>((reinterpret_cast<uint8_t*>(base) + (wg_tid % 64 / 64 * 2048 + (wg_tid / 64 * 16 + h) * 128 + wg_tid % 64 % 64 * 2 ^ (wg_tid % 64 / 64 * 2048 + (wg_tid / 64 * 16 + h) * 128 + wg_tid % 64 % 64 * 2 >> 7 & 7) << 4)));
                                uint64_t _addr64_1766654592;
                                asm volatile("cvta.to.shared.u64 %0, %1;" : "=l"(_addr64_1766654592) : "l"(_ptr_1766654592));
                                uint32_t _addr_1766654592;
                                asm volatile("cvt.u32.u64 %0, %1;" : "=r"(_addr_1766654592) : "l"(_addr64_1766654592));
                                asm volatile("st.shared.b16 [%0], %1;" :: "r"(_addr_1766654592), "h"(_bits_1766654592) : "memory");
                            }
                        }
                        ...
                        for (int native_ni = 0; native_ni < 4; native_ni++) {
                            ...
                            int msa_valid_cols = 128;
                            {
                                ...
                                int page_head = 0;
                                {
                                    ...
                                    page_head = physical_page * num_kv_heads + kv_head_2;
                                }
                        ...
                        for (int native_ni_1 = 0; native_ni_1 < max_kv_tiles; native_ni_1++) {
                            ...
                            int msa_valid_cols_1 = 128;
                            {
                                ...
                                int page_head_1 = 0;
                                {
                                    ...
                                    page_head_1 = physical_page_1 * num_kv_heads + kv_head_2;
                                }
                            ...
                            if (native_next_ni < native_num_n_blocks) {
                                ...
                                int msa_next_valid_cols = 128;
                                {
                                    ...
                                    int page_head_2 = 0;
                                    {
                                        int physical_page_2 = 0;
                                        if (selected_block_2 >= 0) {
                                            physical_page_2 = kv_indices[batch_2 * msa_max_pages + selected_block_2];
                                            if (physical_page_2 < 0) {
                                                valid_cols_3 = 0;
                                                physical_page_2 = 0;
                                            }
                                        }
                                        page_head_2 = physical_page_2 * num_kv_heads + kv_head_2;
                                    }
        ...
        { // producer_aux_main
            ...
            unsigned int _phase_work_full_1_0_4 = 0;
            {
                ...
                for (int task_iter_4 = 0; task_iter_4 < max_task_claims + 1; task_iter_4++) {
                    ...
                    if (direct_decode_5 != 0) {
                        if (task_iter_4 == 0) {
                            ticket_5 = blockIdx.x * num_kv_heads + blockIdx.y;
                        }
                    } else {
                        ticket_5 = work_desc_5[0];
                    }
                    if (ticket_5 < 0) {
                        if (direct_decode_5 == 0) {
                            if (elect_sync()) {
                    ...
                    int num_n_blocks_4 = max_kv_tiles;
                    if (direct_decode_5 == 0) {
                        num_n_blocks_4 = work_desc_5[7] - work_desc_5[6];
                    }
                    int prefill_1 = ((num_n_blocks_4 < 4) ? num_n_blocks_4 : 4);
                    #pragma unroll
                    for (int ni_3 = 0; ni_3 < 4; ni_3++) {
                        if (prefill_1 <= ni_3) {
                            break;
                        }
                        mbarrier_wait(kv_src_full_addr + (ni_3) * 8, 0);
                        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                        {
                            const char* _src_ptr = smem_raw + (smem_kv_fp8_addr + (unsigned int)(ni_3 * 16384) - smem);
                            char* _dst_ptr = smem_raw + (smem_kv_addr + (unsigned int)(ni_3 * 32768) - smem);
                        ...
                        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                        if (elect_sync()) {
                            mbarrier_arrive(kv_full_addr + (ni_3) * 8);
                        }
                    }
                    #pragma unroll 1
                    for (int ni_4 = 0; ni_4 < max_kv_tiles; ni_4++) {
                        if (num_n_blocks_4 <= ni_4) {
                            break;
                        }
                        int stage_3 = ni_4 % 4;
                        mbarrier_wait(kv_src_full_addr + (stage_3) * 8, 1);
                        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                        {
                            const char* _src_ptr = smem_raw + (smem_kv_fp8_addr + (unsigned int)(stage_3 * 16384) - smem);
                            char* _dst_ptr = smem_raw + (smem_kv_addr + (unsigned int)(stage_3 * 32768) - smem);
                            const int _tid = (int)threadIdx.x - (15) * 32;
                            #pragma unroll 4
                            for (int _off = _tid; _off < 2048; _off += 32) {
                                uint64_t _src64 = reinterpret_cast<const uint64_t*>(_src_ptr)[_off];
                        ...
                        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                        if (elect_sync()) {
                            mbarrier_arrive(kv_full_addr + (stage_3) * 8);
                        }
                        int next_ni_1 = ni_4 + 4;
                        if (next_ni_1 < num_n_blocks_4) {
                            mbarrier_wait(kv_src_full_addr + (stage_3) * 8, 0);
                            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                            {
                                const char* _src_ptr = smem_raw + (smem_kv_fp8_addr + (unsigned int)(stage_3 * 16384) - smem);
                                char* _dst_ptr = smem_raw + (smem_kv_addr + (unsigned int)(stage_3 * 32768) - smem);
