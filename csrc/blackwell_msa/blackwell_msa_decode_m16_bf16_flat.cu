            ...
            for (int task_iter = 0; task_iter < max_task_claims + 1; task_iter++) {
                ...
                int num_n_blocks = kv_tile_end - kv_tile_begin;
                {
                    ...
                    for (int pair = 0; pair < max_decode_pairs; pair++) {
                        ...
                        for (int h = 0; h < 16; h++) {
                            {
                                __nv_bfloat16 _bval_1944970640 = __float2bfloat16_rn(exp_vals[h]);
                                uint16_t _bits_1944970640 = *(uint16_t*)&_bval_1944970640;
                                const void* _ptr_1944970640 = reinterpret_cast<const void*>((reinterpret_cast<uint8_t*>(base) + (wg_tid % 64 / 64 * 2048 + (wg_tid / 64 * 16 + h) * 128 + wg_tid % 64 % 64 * 2 ^ (wg_tid % 64 / 64 * 2048 + (wg_tid / 64 * 16 + h) * 128 + wg_tid % 64 % 64 * 2 >> 7 & 7) << 4)));
                                uint64_t _addr64_1944970640;
                                asm volatile("cvta.to.shared.u64 %0, %1;" : "=l"(_addr64_1944970640) : "l"(_ptr_1944970640));
                                uint32_t _addr_1944970640;
                                asm volatile("cvt.u32.u64 %0, %1;" : "=r"(_addr_1944970640) : "l"(_addr64_1944970640));
                                asm volatile("st.shared.b16 [%0], %1;" :: "r"(_addr_1944970640), "h"(_bits_1944970640) : "memory");
                            }
                        }
                    ...
                    for (int h_4 = 0; h_4 < 16; h_4++) {
                        if (group_size_1 > h_4) {
                            ...
                            if (splits == 1) {
                                ...
                                *((__nv_bfloat16*)(O + out_idx)) = __float2bfloat16_rn(merged * inv_sum[h_4]);
                            } else {
                                int logical_output = request * num_kv_heads + kv_head;
                        ...
                        if (split_reduce_flag[0] != 0) {
                            ...
                            for (int h_5 = 0; h_5 < 16; h_5++) {
                                if (group_size_1 > h_5) {
                                    ...
                                    int q_row_1 = qo_begin * num_q_heads + kv_head * group_size_1 + h_5;
                                    ...
                                    *((__nv_bfloat16*)(O + out_idx_1)) = __float2bfloat16_rn(((reduce_d > 0.0f) ? reduce_o * _rcp_1 : 0.0f));
                                }
                            }
                        }
                    }
            ...
            if (elect_sync()) {
                ...
                for (int claim_iter = 0; claim_iter < max_task_claims + 1; claim_iter++) {
                    ...
                    if (ticket_3 < device_total) {
                        ...
                        int kv_tile_end_3 = 0;
                        if (attention_mode == 1) {
                            request_1 = ticket_3 / num_kv_heads;
                            kv_head_1 = ticket_3 % num_kv_heads;
                            int fast_kv_len = kv_len_arr[request_1];
                            ...
                            request_1 = lo;
                            int local = ticket_3 - task_offsets[request_1];
                            int q_len = qo_indptr[request_1 + 1] - qo_indptr[request_1];
                            {
                                q_len = 1;
                            }
                            int packed_q = q_len * group_size_2;
                            int kv_len = kv_len_arr[request_1];
                            ...
                            kind_3 = 0;
                            int prefill_token_tiles = (q_len + 256 - 1) / 256;
    ...
    // ---- Role: producer ----
        ...
        { // producer_main
            ...
            for (int task_iter_3 = 0; task_iter_3 < max_task_claims + 1; task_iter_3++) {
                ...
                {
                    if (elect_sync()) {
                        int q_row_2 = qo_begin_1 * num_q_heads + kv_head_2 * group_size_3 + q_tile_1 * 16;
                    ...
                    for (int ni = 0; ni < num_n_blocks_3; ni++) {
                        ...
                        int n_block = kv_tile_end_4 - 1 - ni;
                        #pragma unroll
        ...
        { // producer_aux_main
            ...
            if (direct_decode_5 == 0) {
                ...
                for (int task_iter_4 = 0; task_iter_4 < max_task_claims + 1; task_iter_4++) {
                    ...
                    if (kind_5 == 0) {
                        int splits_2 = work_desc_5[12];
                        if (splits_2 > 1) {
                            int request_2 = work_desc_5[2];
                            int kv_head_3 = work_desc_5[3];
                            int q_tile_2 = work_desc_5[4];
                            int qo_begin_2 = work_desc_5[8];
                            int qo_end = work_desc_5[9];
                            int q_len_1 = qo_end - qo_begin_2;
                            int group_size_4 = num_q_heads / num_kv_heads;
                            int token_tiles = (q_len_1 + 256 - 1) / 256;
                            int q_token_base = q_tile_2 % token_tiles * 256;
                            int q_head_local = q_tile_2 / token_tiles;
                            int q_head = kv_head_3 * group_size_4 + q_head_local;
                            int packed_gqa = 0;
                            if (group_size_4 > 1) {
                                if (q_len_1 * group_size_4 <= 128) {
                                    packed_gqa = 1;
                                }
                            }
                            ...
                            int reduce_worker = warp_in_pair * 32 + lane_0_1;
                            int valid_rows = q_len_1;
                            if (packed_gqa != 0) {
                                valid_rows = q_len_1 * group_size_4;
                            }
                            int logical_output_2 = request_2 * num_kv_heads + kv_head_3;
                            int partial_slot_2 = logical_output_2 * max_splits + work_desc_5[5];
                            #pragma unroll 1
                            for (int publish_pass = 0; publish_pass < 2; publish_pass++) {
                                int publish_group = publish_pass * 2 + warp_in_pair;
                                int publish_row = publish_group * 32 + lane_0_1;
                                if (publish_row < valid_rows) {
                                    ...
                                    *((float*)(partial_D + partial_stat_idx)) = prefill_scale[publish_row + 256];
                                    ...
                                    for (int col = 0; col < 8; col++) {
                                        int smem_addr = prefill_partial_tile_addr + (unsigned int)((publish_row * 128 + col * 16) * 4);
                                        int partial_o_idx_1 = partial_stat_idx * 128 + col * 16;
                                        float values_lo[8];
                                        float values_hi[8];
                            ...
                            if (reduce_worker == 0) {
                                int _atomic_old_2 = atomicAdd(&split_completion[logical_output_2], 1);
                                int old_count_1 = _atomic_old_2;
                            ...
                            if (split_reduce_flag[0] != 0) {
                                ...
                                for (int row_group = 0; row_group < 2; row_group++) {
                                    int reduce_row = row_group * 64 + reduce_worker;
                                    if (reduce_row < valid_rows) {
                                        ...
                                        for (int reduce_split_1 = 0; reduce_split_1 < max_splits; reduce_split_1++) {
                                            ...
                                            float _max_10 = max_noftz(reduce_m_1, split_m_1);
                                            reduce_m_1 = _max_10;
                                        }
                                        ...
                                        for (int reduce_split_2 = 0; reduce_split_2 < max_splits; reduce_split_2++) {
                                            ...
                                            float _exp2_10 = approx_exp2(softmax_scale_log2 * (split_m_2 - reduce_m_1));
                                            float split_scale_1 = ((split_m_2 == -BLACKWELL_MSA_INF) ? 0.0f : _exp2_10);
                                            prefill_split_weights[reduce_split_2 * 128 + reduce_row] = split_scale_1;
                                            reduce_d_1 += split_d_1 * split_scale_1;
                                        }
                                        float _rcp_5 = approx_rcp(reduce_d_1);
                                        prefill_scale[reduce_row] = _rcp_5;
                                    }
                                }
                                asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                                asm volatile("barrier.sync 9, 64;" ::: "memory");
                                    ...
                                    if (reduce_row_1 < valid_rows) {
                                        ...
                                        for (int reduce_split_3 = 0; reduce_split_3 < max_splits; reduce_split_3++) {
                                            ...
                                            for (int elem_1 = 0; elem_1 < 8; elem_1++) {
                                                float _fma_2 = __fmaf_rn(split_o_1[elem_1], split_scale_2, reduce_o_1[elem_1]);
                                                reduce_o_1[elem_1] = _fma_2;
                                            }
                                        }
                                        int reduce_local_q_row = q_token_base + reduce_row_1;
                                        int reduce_q_head = q_head;
                                        if (packed_gqa != 0) {
                                            reduce_local_q_row = reduce_row_1 / group_size_4;
                                            reduce_q_head = kv_head_3 * group_size_4 + reduce_row_1 % group_size_4;
                                        }
                                        int reduce_output_row = (qo_begin_2 + reduce_local_q_row) * num_q_heads + reduce_q_head;
                                        {
