...
#include "blackwell_msa_prefill_m64_bf16_flat.cu"
#undef uint8_t
#undef uint16_t
#undef uint32_t
#undef uint64_t
...
void Run(TensorView arg_q, TensorView arg_k, TensorView arg_v, TensorView arg_out,
         ...
         TensorView arg_kv_lens, int64_t arg_total_q, int64_t arg_num_q_heads,
         int64_t arg_num_kv_heads, int64_t arg_topk, int64_t arg_batch_size, int64_t arg_causal,
  ...
  cudaError_t status = cudaFuncSetAttribute(kernel_blackwell_msa_prefill_m64_bf16_flat,
                                            cudaFuncAttributeMaxDynamicSharedMemorySize, 134784);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_blackwell_msa_prefill_m64_bf16_flat) failed: "
      << cudaGetErrorString(status);
  status = cudaLaunchKernel(reinterpret_cast<const void*>(kernel_blackwell_msa_prefill_m64_bf16_flat),
                            grid, block, kargs, 134784u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_blackwell_msa_prefill_m64_bf16_flat launch failed: " << cudaGetErrorString(status);
}
