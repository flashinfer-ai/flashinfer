...
#include "blackwell_msa_decode_fp16_flat.cu"
#undef uint8_t
#undef uint16_t
#undef uint32_t
#undef uint64_t
...
void Run(TensorView arg_Q, TensorView arg_Q_prefill, TensorView arg_Q_prefill_raw, TensorView arg_K,
  ...
  cudaError_t status = cudaFuncSetAttribute(kernel_blackwell_msa_decode_fp16_flat,
                                            cudaFuncAttributeMaxDynamicSharedMemorySize, 151168);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_blackwell_msa_decode_fp16_flat) failed: "
      << cudaGetErrorString(status);
  status = cudaLaunchKernel(reinterpret_cast<const void*>(kernel_blackwell_msa_decode_fp16_flat), grid,
                            block, kargs, 151168u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_blackwell_msa_decode_fp16_flat launch failed: " << cudaGetErrorString(status);
}
