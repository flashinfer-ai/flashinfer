...
#include "blackwell_msa_prefill_m128_bf16_gqa16_paged.cu"
#undef uint8_t
#undef uint16_t
#undef uint32_t
#undef uint64_t
...
void Run(TensorView arg_q, TensorView arg_k, TensorView arg_v, TensorView arg_out,
  ...
  cudaError_t status = cudaFuncSetAttribute(kernel_blackwell_msa_prefill_m128_bf16_gqa16_paged,
                                            cudaFuncAttributeMaxDynamicSharedMemorySize, 171008);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_blackwell_msa_prefill_m128_bf16_gqa16_paged) failed: "
      << cudaGetErrorString(status);
  status =
      cudaLaunchKernel(reinterpret_cast<const void*>(kernel_blackwell_msa_prefill_m128_bf16_gqa16_paged),
                       grid, block, kargs, 171008u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_blackwell_msa_prefill_m128_bf16_gqa16_paged launch failed: "
      << cudaGetErrorString(status);
}
