...
#include "blackwell_msa_decode_fp8_paged.cu"
#undef uint8_t
#undef uint16_t
#undef uint32_t
#undef uint64_t
...
void Run(TensorView arg_Q, TensorView arg_Q_prefill, TensorView arg_Q_prefill_raw, TensorView arg_K,
  ...
  cudaError_t status = cudaFuncSetAttribute(kernel_blackwell_msa_decode_fp8_paged,
                                            cudaFuncAttributeMaxDynamicSharedMemorySize, 216704);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_blackwell_msa_decode_fp8_paged) failed: "
      << cudaGetErrorString(status);
  status = cudaLaunchKernel(reinterpret_cast<const void*>(kernel_blackwell_msa_decode_fp8_paged), grid,
                            block, kargs, 216704u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_blackwell_msa_decode_fp8_paged launch failed: " << cudaGetErrorString(status);
}
