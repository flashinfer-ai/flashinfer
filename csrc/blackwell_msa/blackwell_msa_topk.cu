...
kernel_blackwell_msa_topk(float* __restrict__ max_score, int* __restrict__ output, int num_heads, int max_k_tiles, int total_q, int num_valid_pages, int force_begin_blocks, int force_end_blocks)
{
    ...
    for (int slot = 0; slot < 16; slot++) {
        if (forced <= slot) {
            float local_score = -BLACKWELL_MSA_INF;
            ...
            __syncthreads();
            if (warp_2 == 0) {
                float block_score = -BLACKWELL_MSA_INF;
