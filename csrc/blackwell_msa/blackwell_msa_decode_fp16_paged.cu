            ...
            for (int task_iter = 0; task_iter < max_task_claims + 1; task_iter++) {
                ...
                if (kind == 1) {
                    ...
                    for (int pair = 0; pair < max_decode_pairs; pair++) {
                        ...
                        for (int h = 0; h < 16; h++) {
                            {
                                __half _hval_1945067216 = __float2half_rn(exp_vals[h]);
                                uint16_t _bits_1945067216 = *(uint16_t*)&_hval_1945067216;
                                const void* _ptr_1945067216 = reinterpret_cast<const void*>((reinterpret_cast<uint8_t*>(base) + (wg_tid % 64 / 64 * 2048 + (wg_tid / 64 * 16 + h) * 128 + wg_tid % 64 % 64 * 2 ^ (wg_tid % 64 / 64 * 2048 + (wg_tid / 64 * 16 + h) * 128 + wg_tid % 64 % 64 * 2 >> 7 & 7) << 4)));
                                uint64_t _addr64_1945067216;
                                asm volatile("cvta.to.shared.u64 %0, %1;" : "=l"(_addr64_1945067216) : "l"(_ptr_1945067216));
                                uint32_t _addr_1945067216;
                                asm volatile("cvt.u32.u64 %0, %1;" : "=r"(_addr_1945067216) : "l"(_addr64_1945067216));
                                asm volatile("st.shared.b16 [%0], %1;" :: "r"(_addr_1945067216), "h"(_bits_1945067216) : "memory");
                            }
                        }
