            ...
            for (int task_iter = 0; task_iter < max_task_claims + 1; task_iter++) {
                ...
                int num_n_blocks = kv_tile_end - kv_tile_begin;
                {
                    ...
                    for (int pair = 0; pair < max_decode_pairs; pair++) {
                        ...
                        for (int h = 0; h < 16; h++) {
                            {
                                __nv_bfloat16 _bval_1945068080 = __float2bfloat16_rn(exp_vals[h]);
                                uint16_t _bits_1945068080 = *(uint16_t*)&_bval_1945068080;
                                const void* _ptr_1945068080 = reinterpret_cast<const void*>((reinterpret_cast<uint8_t*>(base) + (wg_tid % 64 / 64 * 2048 + (wg_tid / 64 * 16 + h) * 128 + wg_tid % 64 % 64 * 2 ^ (wg_tid % 64 / 64 * 2048 + (wg_tid / 64 * 16 + h) * 128 + wg_tid % 64 % 64 * 2 >> 7 & 7) << 4)));
                                uint64_t _addr64_1945068080;
                                asm volatile("cvta.to.shared.u64 %0, %1;" : "=l"(_addr64_1945068080) : "l"(_ptr_1945068080));
                                uint32_t _addr_1945068080;
                                asm volatile("cvt.u32.u64 %0, %1;" : "=r"(_addr_1945068080) : "l"(_addr64_1945068080));
                                asm volatile("st.shared.b16 [%0], %1;" :: "r"(_addr_1945068080), "h"(_bits_1945068080) : "memory");
                            }
                        }
