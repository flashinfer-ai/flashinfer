...
#define SMEM_KV_FP8_SMEM_OFF 154624
...
kernel_blackwell_msa_prefill_m128_bf16_gqa16_paged(const __grid_constant__ BlackwellMsaTensorMap q_value, const __grid_constant__ BlackwellMsaTensorMap k_value, const __grid_constant__ BlackwellMsaTensorMap v_value, __nv_bfloat16* __restrict__ out, float* __restrict__ lse, float* __restrict__ temperature_lse, int* __restrict__ q2k_indices, int* __restrict__ cu_seqlens_q, int* __restrict__ cu_seqlens_k, int* __restrict__ q_offsets, int* __restrict__ kv_lens, int* __restrict__ page_table, int total_q, int num_q_heads, int num_kv_heads, int topk, int batch_size, int max_pages, int causal, int derive_q_offset, float softmax_scale_log2, float lse_temperature_scale, int return_softmax_lse, int return_temperature_lse)
{
    ...
    uint8_t* kv_fp8_smem = reinterpret_cast<uint8_t*>(smem_raw + 154624);
    const int kv_fp8_smem_addr = smem + 154624;
    ...
    if (warp <= 7) {
        ...
        { // softmax_main
            ...
            for (int candidate_batch = 0; candidate_batch < batch_size; candidate_batch++) {
                ...
                int candidate_tiles = (candidate_q_len + 16 - 1) / 16;
            ...
            int stage_query_offset = stage * 8;
            int query_in_stage = my_row / 16;
            const int stage_query_capacity = 8;
            const int selection_rows = 16;
            ...
            _phase_q2k_full_0 ^= 1;
            {
                if (lane % 16 == 0 && row_valid != 0 && num_n_blocks <= 32) {
                    #pragma unroll 1
                ...
                unsigned int _shfl_0 = __shfl_sync(0xFFFFFFFF, selection_mask, lane / 16 * 16);
    ...
    // ---- Role: correction ----
        ...
        { // correction_main
            ...
            for (int candidate_batch_1 = 0; candidate_batch_1 < batch_size; candidate_batch_1++) {
                ...
                int candidate_tiles_1 = (candidate_q_len_1 + 16 - 1) / 16;
            ...
            for (int stage_1 = 0; stage_1 < 2; stage_1++) {
                ...
                int stage_query_offset_1 = stage_1 * 8;
                int query_in_stage_1 = my_row_1 / 16;
                const int stage_query_capacity_1 = 8;
    ...
    // ---- Role: mma_warp ----
        ...
        { // mma_warp_main
            ...
            for (int candidate_batch_2 = 0; candidate_batch_2 < batch_size; candidate_batch_2++) {
                ...
                int candidate_tiles_2 = (candidate_q_len_2 + 16 - 1) / 16;
    ...
    // ---- Role: load_warp ----
        ...
        { // load_warp_main
            ...
            for (int candidate_batch_3 = 0; candidate_batch_3 < batch_size; candidate_batch_3++) {
                ...
                int candidate_tiles_3 = (candidate_q_len_3 + 16 - 1) / 16;
            ...
            if (elect_sync()) {
                ...
                {
                    tma_4d_gmem2smem(q1_smem_addr, q, 0, q_head_3, query_base_3 + 8, 0, q_full_addr + 8);
                }
            }
