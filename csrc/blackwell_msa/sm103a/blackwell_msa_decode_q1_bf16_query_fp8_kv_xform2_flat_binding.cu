...
#include "blackwell_msa_decode_q1_bf16_query_fp8_kv_xform2_flat.cu"
#undef CUtensorMap
#undef uint8_t
#undef uint16_t
#undef uint32_t
...
void Run(TensorView arg_Q, TensorView arg_K, TensorView arg_V, TensorView arg_O, TensorView arg_msa_lse, TensorView arg_kv_indices, TensorView arg_kv_indptr, TensorView arg_task_kind, TensorView arg_task_request, TensorView arg_task_kv_head, int64_t arg_num_requests, double arg_softmax_scale_log2, int64_t arg_msa_max_pages, int64_t grid_x, int64_t grid_y, int64_t grid_z, int64_t cuda_stream) {
  ...
  cudaError_t status = cudaFuncSetAttribute(
      kernel_blackwell_batch_attention_msa_decode_q1_fp8_flat_xform2_v1, cudaFuncAttributeMaxDynamicSharedMemorySize, 216704);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_blackwell_batch_attention_msa_decode_q1_fp8_flat_xform2_v1) failed: " << cudaGetErrorString(status);
  status = cudaLaunchKernel(reinterpret_cast<const void*>(kernel_blackwell_batch_attention_msa_decode_q1_fp8_flat_xform2_v1), grid, block, kargs,
                            216704u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_blackwell_batch_attention_msa_decode_q1_fp8_flat_xform2_v1 launch failed: " << cudaGetErrorString(status);
}
