...
#define SMEM_KV_FP8_SMEM_OFF 121856
...
kernel_minimax_sparse_prefill_union_sm100(const __grid_constant__ CUtensorMap q, const __grid_constant__ CUtensorMap k, const __grid_constant__ CUtensorMap v, __nv_bfloat16* __restrict__ out, float* __restrict__ lse, float* __restrict__ temperature_lse, int* __restrict__ q2k_indices, int* __restrict__ cu_seqlens_q, int* __restrict__ cu_seqlens_k, int* __restrict__ q_offsets, int* __restrict__ kv_lens, int* __restrict__ page_table, int total_q, int num_q_heads, int num_kv_heads, int topk, int batch_size, int uniform_q_len, int max_pages, int causal, int derive_q_offset, float softmax_scale_log2, float lse_temperature_scale, int return_softmax_lse, int return_temperature_lse)
{
    ...
    uint8_t* kv_fp8_smem = reinterpret_cast<uint8_t*>(smem_raw + 121856);
    const int kv_fp8_smem_addr = smem + 121856;
    ...
    if (warp <= 7) {
        ...
        { // softmax_main
            ...
            {
                {
                    if (lane % 16 == 0 && row_valid != 0 && num_n_blocks <= 64) {
                        #pragma unroll 1
                ...
                unsigned int _shfl_1 = __shfl_sync(0xFFFFFFFF, selection_mask_high, lane / 16 * 16);
