...
kernel_minimax_sparse_prefill_exact_union_m64_sm100(const __grid_constant__ CUtensorMap q, const __grid_constant__ CUtensorMap k, const __grid_constant__ CUtensorMap v, __nv_bfloat16* __restrict__ out, float* __restrict__ lse, float* __restrict__ temperature_lse, int* __restrict__ q2k_indices, int* __restrict__ cu_seqlens_q, int* __restrict__ cu_seqlens_k, int* __restrict__ q_offsets, int* __restrict__ kv_lens, int total_q, int num_q_heads, int num_kv_heads, int topk, int batch_size, int uniform_q_len, int causal, int derive_q_offset, float softmax_scale_log2, float lse_temperature_scale, int return_softmax_lse, int return_temperature_lse)
{
    ...
    if (warp <= 7) {
        ...
        { // softmax_main
            ...
            int tile_active = 0;
            if (uniform_q_len > 0) {
                int uniform_tiles = (uniform_q_len + 8 - 1) / 8;
            ...
            #pragma unroll 1
            for (int union_index = 0; union_index < 64; union_index++) {
                if (union_count > union_index) {
                    ...
                    if (return_temperature_lse != 0) {
                        ...
                        if (valid_cols > 0 && half_valid < 64) {
                            ...
                            for (int _i_9 = 0; _i_9 < 32; _i_9++) {
                                if (!(_slice_lo_mask_2 & (1u << _i_9))) _tmem_load_1[0 + _i_9] = -BLACKWELL_MSA_INF;
                            }
                            ...
                            for (int _i_11 = 0; _i_11 < 32; _i_11++) {
                                if (!(_slice_lo_mask_3 & (1u << _i_11))) _tmem_load_1[32 + _i_11] = -BLACKWELL_MSA_INF;
                            }
                        }
    ...
    if (warp >= 8 && warp <= 11) {
        ...
        { // correction_main
            ...
            int tile_active_1 = 0;
            if (uniform_q_len > 0) {
                int uniform_tiles_1 = (uniform_q_len + 8 - 1) / 8;
            ...
            #pragma unroll 1
            for (int union_index_1 = 1; union_index_1 < 64; union_index_1++) {
    ...
    if (warp == 12) {
        { // mma_warp_main
            ...
            #pragma unroll 1
            for (int union_index_2 = 0; union_index_2 < 64; union_index_2++) {
    ...
    if (warp == 15) {
        { // load_warp_main
            ...
            int tile_active_2 = 0;
            if (uniform_q_len > 0) {
                int uniform_tiles_2 = (uniform_q_len + 8 - 1) / 8;
            ...
            if (elect_sync()) {
                ...
                tma_4d_gmem2smem(q1_smem_addr, (&q), 0, q_head_2, query_base_2 + 4, 0, q_full_addr);
            }
            int token = lane;
            ...
            if (lane < 2) {
                ...
                #pragma unroll 1
                for (int high_index = 0; high_index < 32; high_index++) {
                    if (high_count > high_index) {
                ...
                #pragma unroll 1
                for (int low_index = 0; low_index < 32; low_index++) {
                    if (low_count > low_index) {
            ...
            #pragma unroll 1
            for (int union_index_3 = 0; union_index_3 < 64; union_index_3++) {
                #pragma unroll
