                ...
                if (whole_group_valid != 0) {
                    ...
                    long long partial_base = partial_row * 128;
                    {
                        ...
                        long long partial_metadata_rows = (long long)topk * (long long)total_q * (long long)num_q_heads;
                        {
                            ...
                            tmem_ld_x32(&_tmem_load_2[0], output_row_addr + 64);
                            tmem_ld_x32(&_tmem_load_2[32], output_row_addr + 64 + 32);
                            ...
                            row_max_x32_accum(&_tmem_load_2[0], _reg_reduce_max2_12);
                            row_max_x32_accum(&_tmem_load_2[32], _reg_reduce_max2_12);
                            float _tmem_load_2_max = row_max_reduce(_reg_reduce_max2_12);
                            ...
                            for (int output_segment_1 = 0; output_segment_1 < 4; output_segment_1++) {
                                ...
                                for (int _ls = 0; _ls < 8; _ls++)
                                    sub_f32x2_inplace(&reinterpret_cast<float2*>(_tmem_load_3)[_ls], _sub2_13);
                                if (output_valid != 0) {
                                    {
                                        ...
                                        for (int _ps = 0; _ps < 8; _ps++)
                                            mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_3[0])[_ps], _prescale2_14);
                                        #else
                                        #pragma unroll
                            ...
                            for (int _ls = 0; _ls < 32; _ls++)
                                sub_f32x2_inplace(&reinterpret_cast<float2*>(_tmem_load_2)[_ls], _sub2_15);
                            if (output_valid != 0) {
                                {
                                    ...
                                    #pragma unroll
                                    for (int _ps = 0; _ps < 8; _ps++)
                                        mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_2[0])[_ps], _prescale2_16);
                                    #else
                                    #pragma unroll
                                    for (int _ps = 0; _ps < 16; _ps++)
                                    ...
                                    unsigned int _fp8_pk[4];
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_2[0 + 1]), "f"(_tmem_load_2[0 + 0]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_2[0 + 3]), "f"(_tmem_load_2[0 + 2]));
                                        _fp8_pk[0] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_2[0 + 5]), "f"(_tmem_load_2[0 + 4]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_2[0 + 7]), "f"(_tmem_load_2[0 + 6]));
                                        _fp8_pk[1] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_2[0 + 9]), "f"(_tmem_load_2[0 + 8]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_2[0 + 11]), "f"(_tmem_load_2[0 + 10]));
                                        _fp8_pk[2] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_2[0 + 13]), "f"(_tmem_load_2[0 + 12]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_2[0 + 15]), "f"(_tmem_load_2[0 + 14]));
                                        _fp8_pk[3] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                            ...
                            if (output_valid != 0) {
                                {
                                    ...
                                    #pragma unroll
                                    for (int _ps = 0; _ps < 8; _ps++)
                                        mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_2[16])[_ps], _prescale2_17);
                                    #else
                                    #pragma unroll
                                    for (int _ps = 0; _ps < 16; _ps++)
                                    ...
                                    unsigned int _fp8_pk[4];
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_2[16 + 1]), "f"(_tmem_load_2[16 + 0]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_2[16 + 3]), "f"(_tmem_load_2[16 + 2]));
                                        _fp8_pk[0] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_2[16 + 5]), "f"(_tmem_load_2[16 + 4]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_2[16 + 7]), "f"(_tmem_load_2[16 + 6]));
                                        _fp8_pk[1] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_2[16 + 9]), "f"(_tmem_load_2[16 + 8]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_2[16 + 11]), "f"(_tmem_load_2[16 + 10]));
                                        _fp8_pk[2] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_2[16 + 13]), "f"(_tmem_load_2[16 + 12]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_2[16 + 15]), "f"(_tmem_load_2[16 + 14]));
                                        _fp8_pk[3] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                            ...
                            if (output_valid != 0) {
                                {
                                    ...
                                    #pragma unroll
                                    for (int _ps = 0; _ps < 8; _ps++)
                                        mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_2[32])[_ps], _prescale2_18);
                                    #else
                                    #pragma unroll
                                    for (int _ps = 0; _ps < 16; _ps++)
                                    ...
                                    unsigned int _fp8_pk[4];
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_2[32 + 1]), "f"(_tmem_load_2[32 + 0]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_2[32 + 3]), "f"(_tmem_load_2[32 + 2]));
                                        _fp8_pk[0] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_2[32 + 5]), "f"(_tmem_load_2[32 + 4]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_2[32 + 7]), "f"(_tmem_load_2[32 + 6]));
                                        _fp8_pk[1] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_2[32 + 9]), "f"(_tmem_load_2[32 + 8]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_2[32 + 11]), "f"(_tmem_load_2[32 + 10]));
                                        _fp8_pk[2] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_2[32 + 13]), "f"(_tmem_load_2[32 + 12]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_2[32 + 15]), "f"(_tmem_load_2[32 + 14]));
                                        _fp8_pk[3] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                            ...
                            if (output_valid != 0) {
                                {
                                    ...
                                    #pragma unroll
                                    for (int _ps = 0; _ps < 8; _ps++)
                                        mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_2[48])[_ps], _prescale2_19);
                                    #else
                                    #pragma unroll
                                    for (int _ps = 0; _ps < 16; _ps++)
                                    ...
                                    unsigned int _fp8_pk[4];
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_2[48 + 1]), "f"(_tmem_load_2[48 + 0]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_2[48 + 3]), "f"(_tmem_load_2[48 + 2]));
                                        _fp8_pk[0] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_2[48 + 5]), "f"(_tmem_load_2[48 + 4]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_2[48 + 7]), "f"(_tmem_load_2[48 + 6]));
                                        _fp8_pk[1] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_2[48 + 9]), "f"(_tmem_load_2[48 + 8]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_2[48 + 11]), "f"(_tmem_load_2[48 + 10]));
                                        _fp8_pk[2] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_2[48 + 13]), "f"(_tmem_load_2[48 + 12]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_2[48 + 15]), "f"(_tmem_load_2[48 + 14]));
                                        _fp8_pk[3] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                ...
                if (whole_group_valid_1 != 0) {
                    ...
                    long long partial_base_1 = partial_row_1 * 128;
                    {
                        ...
                        long long partial_metadata_rows_1 = (long long)topk * (long long)total_q * (long long)num_q_heads;
                        {
                            ...
                            tmem_ld_x32(&_tmem_load_7[0], output_row_addr_1 + 64);
                            tmem_ld_x32(&_tmem_load_7[32], output_row_addr_1 + 64 + 32);
                            ...
                            row_min_1 = _min_3;
                            float2 _reg_reduce_max2_12 = {-BLACKWELL_MSA_INF, -BLACKWELL_MSA_INF};
                            row_max_x32_accum(&_tmem_load_7[0], _reg_reduce_max2_12);
                            row_max_x32_accum(&_tmem_load_7[32], _reg_reduce_max2_12);
                            float _tmem_load_7_max = row_max_reduce(_reg_reduce_max2_12);
                            ...
                            for (int output_segment_3 = 0; output_segment_3 < 4; output_segment_3++) {
                                ...
                                #pragma unroll
                                for (int _ls = 0; _ls < 8; _ls++)
                                    sub_f32x2_inplace(&reinterpret_cast<float2*>(_tmem_load_8)[_ls], _sub2_13);
                                if (output_valid_1 != 0) {
                                    {
                                        ...
                                        #pragma unroll
                                        for (int _ps = 0; _ps < 8; _ps++)
                                            mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_8[0])[_ps], _prescale2_14);
                                        #else
                                        #pragma unroll
                                        for (int _ps = 0; _ps < 16; _ps++)
                                        ...
                                        unsigned int _fp8_pk[4];
                                        { unsigned short _lo, _hi;
                                            asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_8[0 + 1]), "f"(_tmem_load_8[0 + 0]));
                                            asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_8[0 + 3]), "f"(_tmem_load_8[0 + 2]));
                                            _fp8_pk[0] = (unsigned)_lo | ((unsigned)_hi << 16);
                                        }
                                        { unsigned short _lo, _hi;
                                            asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_8[0 + 5]), "f"(_tmem_load_8[0 + 4]));
                                            asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_8[0 + 7]), "f"(_tmem_load_8[0 + 6]));
                                            _fp8_pk[1] = (unsigned)_lo | ((unsigned)_hi << 16);
                                        }
                                        { unsigned short _lo, _hi;
                                            asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_8[0 + 9]), "f"(_tmem_load_8[0 + 8]));
                                            asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_8[0 + 11]), "f"(_tmem_load_8[0 + 10]));
                                            _fp8_pk[2] = (unsigned)_lo | ((unsigned)_hi << 16);
                                        }
                                        { unsigned short _lo, _hi;
                                            asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_8[0 + 13]), "f"(_tmem_load_8[0 + 12]));
                                            asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_8[0 + 15]), "f"(_tmem_load_8[0 + 14]));
                                            _fp8_pk[3] = (unsigned)_lo | ((unsigned)_hi << 16);
                                        }
                            ...
                            #pragma unroll
                            for (int _ls = 0; _ls < 32; _ls++)
                                sub_f32x2_inplace(&reinterpret_cast<float2*>(_tmem_load_7)[_ls], _sub2_15);
                            if (output_valid_1 != 0) {
                                {
                                    ...
                                    #pragma unroll
                                    for (int _ps = 0; _ps < 8; _ps++)
                                        mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_7[0])[_ps], _prescale2_16);
                                    #else
                                    #pragma unroll
                                    for (int _ps = 0; _ps < 16; _ps++)
                                    ...
                                    unsigned int _fp8_pk[4];
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_7[0 + 1]), "f"(_tmem_load_7[0 + 0]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_7[0 + 3]), "f"(_tmem_load_7[0 + 2]));
                                        _fp8_pk[0] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_7[0 + 5]), "f"(_tmem_load_7[0 + 4]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_7[0 + 7]), "f"(_tmem_load_7[0 + 6]));
                                        _fp8_pk[1] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_7[0 + 9]), "f"(_tmem_load_7[0 + 8]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_7[0 + 11]), "f"(_tmem_load_7[0 + 10]));
                                        _fp8_pk[2] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_7[0 + 13]), "f"(_tmem_load_7[0 + 12]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_7[0 + 15]), "f"(_tmem_load_7[0 + 14]));
                                        _fp8_pk[3] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                            ...
                            if (output_valid_1 != 0) {
                                {
                                    ...
                                    #pragma unroll
                                    for (int _ps = 0; _ps < 8; _ps++)
                                        mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_7[16])[_ps], _prescale2_17);
                                    #else
                                    #pragma unroll
                                    for (int _ps = 0; _ps < 16; _ps++)
                                    ...
                                    unsigned int _fp8_pk[4];
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_7[16 + 1]), "f"(_tmem_load_7[16 + 0]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_7[16 + 3]), "f"(_tmem_load_7[16 + 2]));
                                        _fp8_pk[0] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_7[16 + 5]), "f"(_tmem_load_7[16 + 4]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_7[16 + 7]), "f"(_tmem_load_7[16 + 6]));
                                        _fp8_pk[1] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_7[16 + 9]), "f"(_tmem_load_7[16 + 8]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_7[16 + 11]), "f"(_tmem_load_7[16 + 10]));
                                        _fp8_pk[2] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_7[16 + 13]), "f"(_tmem_load_7[16 + 12]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_7[16 + 15]), "f"(_tmem_load_7[16 + 14]));
                                        _fp8_pk[3] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                            ...
                            if (output_valid_1 != 0) {
                                {
                                    ...
                                    #pragma unroll
                                    for (int _ps = 0; _ps < 8; _ps++)
                                        mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_7[32])[_ps], _prescale2_18);
                                    #else
                                    #pragma unroll
                                    for (int _ps = 0; _ps < 16; _ps++)
                                    ...
                                    unsigned int _fp8_pk[4];
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_7[32 + 1]), "f"(_tmem_load_7[32 + 0]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_7[32 + 3]), "f"(_tmem_load_7[32 + 2]));
                                        _fp8_pk[0] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_7[32 + 5]), "f"(_tmem_load_7[32 + 4]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_7[32 + 7]), "f"(_tmem_load_7[32 + 6]));
                                        _fp8_pk[1] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_7[32 + 9]), "f"(_tmem_load_7[32 + 8]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_7[32 + 11]), "f"(_tmem_load_7[32 + 10]));
                                        _fp8_pk[2] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_7[32 + 13]), "f"(_tmem_load_7[32 + 12]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_7[32 + 15]), "f"(_tmem_load_7[32 + 14]));
                                        _fp8_pk[3] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                            ...
                            if (output_valid_1 != 0) {
                                {
                                    ...
                                    #pragma unroll
                                    for (int _ps = 0; _ps < 8; _ps++)
                                        mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_7[48])[_ps], _prescale2_19);
                                    #else
                                    #pragma unroll
                                    for (int _ps = 0; _ps < 16; _ps++)
                                    ...
                                    unsigned int _fp8_pk[4];
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_7[48 + 1]), "f"(_tmem_load_7[48 + 0]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_7[48 + 3]), "f"(_tmem_load_7[48 + 2]));
                                        _fp8_pk[0] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_7[48 + 5]), "f"(_tmem_load_7[48 + 4]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_7[48 + 7]), "f"(_tmem_load_7[48 + 6]));
                                        _fp8_pk[1] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_7[48 + 9]), "f"(_tmem_load_7[48 + 8]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_7[48 + 11]), "f"(_tmem_load_7[48 + 10]));
                                        _fp8_pk[2] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
                                    { unsigned short _lo, _hi;
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(_tmem_load_7[48 + 13]), "f"(_tmem_load_7[48 + 12]));
                                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(_tmem_load_7[48 + 15]), "f"(_tmem_load_7[48 + 14]));
                                        _fp8_pk[3] = (unsigned)_lo | ((unsigned)_hi << 16);
                                    }
