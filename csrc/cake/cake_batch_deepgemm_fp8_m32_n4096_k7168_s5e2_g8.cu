...
kernel_flashinfer_blackwell_batch_deepgemm_fp8_seed_swap_m32(CakeTensorMap const* A, CakeTensorMap const* B, CakeTensorMap const* C_tma, CakeTensorMap const* SFA_packed, CakeTensorMap const* SFB_packed, int* __restrict__ masked_m, unsigned int num_groups, unsigned int shape_m, unsigned int N, unsigned int K)
{
    ...
    uint8_t* smem_a = reinterpret_cast<uint8_t*>(smem_raw + 9216);
    const int smem_a_addr = smem + 9216;
    uint8_t* smem_b = reinterpret_cast<uint8_t*>(smem_raw + 13312);
    const int smem_b_addr = smem + 13312;
    unsigned int* smem_sfa = reinterpret_cast<unsigned int*>(smem_raw + 46080);
    const int smem_sfa_addr = smem + 46080;
    unsigned int* smem_sfb = reinterpret_cast<unsigned int*>(smem_raw + 46592);
    const int smem_sfb_addr = smem + 46592;
        ...
        { // mma_main
            ...
            if (cta_rank == 0) {
                ...
                for (int current_iter_1 = 0; current_iter_1 < max_scheduler_iters_1; current_iter_1++) {
                    ...
                    unsigned int blocks_per_l2_group_1 = m_blocks_g_1 * 8;
        ...
        { // epilogue_main
            ...
            for (int current_iter_3 = 0; current_iter_3 < max_scheduler_iters_3; current_iter_3++) {
                ...
                for (int strip = 0; strip < 2; strip++) {
                    ...
                    __syncwarp();
                    tma_stage_1 += 1;
                    if (tma_stage_1 == 2) { tma_stage_1 = 0; }
                }
                epi_stage_1 += 1;
