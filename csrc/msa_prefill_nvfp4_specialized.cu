...
// ---------------------------------------------------------------------------
// PER-RANK GEOMETRY, as a compile-time parameter.
//
// Under tensor parallelism a rank holds `max(1, num_key_value_heads / tp)` KV
// heads and `num_attention_heads / tp` query heads, and vLLM sizes its paged
// cache from the per-rank KV count (`num_head_slots = 2 * spec.num_kv_heads`).
// So the page SHRINKS with the rank -- 36,864 B at tp 2, 18,432 B at tp 4 --
// while EVERY tiling relation below stays exactly where it is, because the GQA
// GROUP does not move: 64/4, 32/2 and 16/1 are all group 16.
//
// That is why this is a template parameter and not a kernel argument. The
// numbers below are addressing constants -- a page stride, three region offsets
// and the query row stride -- which the compiler folds into immediate operands;
// passing them at runtime would put register arithmetic on the load path of the
// ONE geometry that ships today, for the benefit of two that do not. Compiled
// per value, the tp=1 body is bit-identical to the one that was measured.
//
// It stops at tp 4 on purpose. tp 8 gives group 8, and the group is NOT a free
// parameter here: `kQueriesPerTile = kTileM / kGroupSize` doubles to 16, and
// `kQueriesPerTile * kTopK <= kHashSize` holds today with EXACT equality at
// 8 * 16 == 128. Group 8 needs a 256-entry union table, which breaks
// `kHashSize == 4 * 32` (the ballot-ranked compaction is four warps, one word
// each). That is a kernel change, not another instantiation, and it is refused
// by name rather than compiled wrong.
// ---------------------------------------------------------------------------
template <int KV>
struct RankGeom {
  static constexpr int kNumKVHeads = KV;
  static constexpr int kNumQHeads = KV * kGroupSize;
  static constexpr int64_t kKScaleByteOffset = KV * kDataHeadStride;
  static constexpr int64_t kVDataByteOffset = kKScaleByteOffset + KV * kScaleHeadStride;
  static constexpr int64_t kVScaleByteOffset = kVDataByteOffset + kKScaleByteOffset;
  static constexpr int64_t kPageBytes = kVScaleByteOffset + KV * kScaleHeadStride;
  static_assert(KV >= 1, "a rank holds at least one KV head");
};

// The tp=1 instantiation must reproduce the constants above, or the page map
// the cache writer produces and the one the kernel reads have parted company.
static_assert(RankGeom<kNumKVHeads>::kNumQHeads == kNumQHeads, "tp=1 q head count");
static_assert(RankGeom<kNumKVHeads>::kPageBytes == kPageBytes, "tp=1 page bytes");
static_assert(RankGeom<kNumKVHeads>::kKScaleByteOffset == kKScaleByteOffset, "tp=1 K scale");
static_assert(RankGeom<kNumKVHeads>::kVDataByteOffset == kVDataByteOffset, "tp=1 V data");
static_assert(RankGeom<kNumKVHeads>::kVScaleByteOffset == kVScaleByteOffset, "tp=1 V scale");

// The per-rank page map, host-side, for the binding to assert against.
struct PageLayout {
...
template <bool ValueSide, typename G>
__device__ __forceinline__ void load_page_side(RawSide& r, const uint8_t* page, int kv_head,
                                               int base_chunk) {
  const uint8_t* data = page + (ValueSide ? static_cast<int>(G::kVDataByteOffset) : 0) +
                        kv_head * static_cast<int>(kDataHeadStride);
  const uint8_t* scales = page +
                          (ValueSide ? static_cast<int>(G::kVScaleByteOffset)
                                     : static_cast<int>(G::kKScaleByteOffset)) +
...
template <bool Repair, typename G>
__device__ __forceinline__ bool sparse_prefill_tile(
  ...
  if constexpr (Repair) {
    const __nv_bfloat16 marker =
        output[(static_cast<size_t>(query_base) * G::kNumQHeads + kv_head * kGroupSize) * kHeadDim];
  ...
  for (int idx = tid; idx < kTileM * kVecChunks; idx += kThreads) {
    ...
    if (row_query < q_valid) {
      const __nv_bfloat16* src =
          q + (static_cast<size_t>(query_base + row_query) * G::kNumQHeads + row_head) * kHeadDim;
...
#pragma unroll
  ...
  if (u < union_count) {
    ...
    load_page_side<false, G>(raw_k, kv_cache + static_cast<size_t>(current_page) * G::kPageBytes,
                             kv_head, base_chunk);
    store_page_side<false>(k_smem, raw_k, base_chunk);
    load_page_side<true, G>(raw_v, kv_cache + static_cast<size_t>(current_page) * G::kPageBytes,
                            kv_head, base_chunk);
    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
    __syncthreads();
    asm volatile("tcgen05.fence::after_thread_sync;" ::: "memory");
...
#pragma unroll
    ...
    if (next_u < union_count) {
      load_page_side<false, G>(raw_k, kv_cache + static_cast<size_t>(next_page) * G::kPageBytes,
                               kv_head, base_chunk);
    }
...
#pragma unroll
    ...
    if (next_u < union_count) {
      load_page_side<true, G>(raw_v, kv_cache + static_cast<size_t>(next_page) * G::kPageBytes,
                              kv_head, base_chunk);
      store_page_side<false>(k_smem, raw_k, base_chunk);
...
#pragma unroll 4
  for (int idx = tid; idx < kTileM * kVecChunks; idx += kThreads) {
    ...
    if (row_query < q_valid) {
      ...
      __nv_bfloat16* out =
          output +
          (static_cast<size_t>(query_base + row_query) * G::kNumQHeads + row_head) * kHeadDim;
...
__device__ __noinline__ void replay_sparse_prefill_tile(
    ...
    int q2k_head_stride, int q2k_token_stride, int virtual_block_x, int virtual_kv_head,
    char* smem) {
  (void)sparse_prefill_tile<true, G>(q, kv_cache, q2k, cu_q, page_table, seqused_k,
                                     logits_scale_log2, v_global_scale, output, total_q, batch_size,
                                     max_pages, q2k_head_stride, q2k_token_stride, virtual_block_x,
...
__global__ __launch_bounds__(kThreads, 2) void sparse_prefill_kernel(
  ...
  const bool active = sparse_prefill_tile<false, G>(
  ...
  if (active && tile_state[kRepairFlagWord] != 0) {
    replay_sparse_prefill_tile<G>(q, kv_cache, q2k, cu_q, page_table, seqused_k, logits_scale_log2,
...
// The attribute is a property of the (DEVICE, KERNEL IMAGE) pair, and there is
// now more than one kernel image: one per compiled KV-head count.  The cache is
// therefore keyed on both.  Keying it on the device alone would set the
// attribute on whichever instantiation ran first and leave the others
// unconfigured -- the same class of bug as the process-wide flag this already
// replaced, one level down.
template <typename G>
cudaError_t configure_dynamic_smem(int device_id) {
  constexpr int kMaxCachedDevices = 64;
  static std::atomic<bool> configured[kMaxCachedDevices];  // zero-initialized, per G
  ...
  const cudaError_t status =
      cudaFuncSetAttribute(reinterpret_cast<const void*>(sparse_prefill_kernel<G>),
...
template <typename G>
cudaError_t launch_for(const void* q, const void* kv_cache, const int* q2k_indices,
  ...
  const cudaError_t configured = configure_dynamic_smem<G>(device_id);
  if (configured != cudaSuccess) return configured;

  // KV head is the fastest-changing launch coordinate so that all of a rank's
  // heads share one global heavy-to-light traversal.  The tile count is an
  // upper bound: ceil(total_q / kQueriesPerTile) can undercount a ragged batch
  // by at most one tile per request after the first, and every CTA finds its
  // own request by walking cu_seqlens_q, so the surplus CTAs simply exit.
  const dim3 grid(G::kNumKVHeads, (total_q + kQueriesPerTile - 1) / kQueriesPerTile + batch - 1, 1);
  constexpr float kLog2e = 1.4426950408889634f;
  sparse_prefill_kernel<G><<<grid, kThreads, kDynamicSmemBytes, stream>>>(
...
// The KV-head counts that have a compiled body.  {4, 2, 1} are MiniMax-M3's
// tensor-parallel sizes 1, 2 and 4; every one of them keeps GQA group 16, which
// is what makes them the SAME kernel at three addressing constants rather than
// three kernels.  Anything else returns cudaErrorInvalidValue and the binding
// turns that into a named refusal.
cudaError_t launch(const void* q, const void* kv_cache, const int* q2k_indices,
                   const int* cu_seqlens_q, const int* page_table, const int* seqused_k,
                   void* output, int total_q, int batch, int max_pages, int q2k_head_stride,
                   int q2k_token_stride, float softmax_scale, float k_global_scale,
                   float v_global_scale, int num_kv_heads, int device_id, cudaStream_t stream) {
#define FLASHINFER_MSA_PREFILL_LAUNCH(KV)                                                          \
  case KV:                                                                                         \
    return launch_for<RankGeom<KV>>(q, kv_cache, q2k_indices, cu_seqlens_q, page_table, seqused_k, \
                                    output, total_q, batch, max_pages, q2k_head_stride,            \
                                    q2k_token_stride, softmax_scale, k_global_scale,               \
                                    v_global_scale, device_id, stream)
  switch (num_kv_heads) {
    FLASHINFER_MSA_PREFILL_LAUNCH(4);
    FLASHINFER_MSA_PREFILL_LAUNCH(2);
    FLASHINFER_MSA_PREFILL_LAUNCH(1);
    default:
      return cudaErrorInvalidValue;
  }
#undef FLASHINFER_MSA_PREFILL_LAUNCH
}

// The same set, for the binding to refuse against before it launches.
inline bool is_compiled_kv_head_count(int num_kv_heads) {
  return num_kv_heads == 4 || num_kv_heads == 2 || num_kv_heads == 1;
}
...
// `num_kv_heads` and `page_bytes` are ARGUMENTS, not `geom::` constants: a
// tensor-parallel rank's page holds fewer KV heads and is correspondingly
// smaller (36,864 B at tp 2, 18,432 B at tp 4).  The instantiation the launch
// picks is chosen from the same number, so a mismatch here and a mismatch there
// cannot disagree.
...
void msa_prefill_nvfp4_specialized(TensorView q, TensorView k_data, TensorView v_data,
  ...
  // The KV-head count comes from the CACHE and selects the instantiation.  The
  // GQA GROUP is what is fixed at 16: `kQueriesPerTile = kTileM / kGroupSize`
  // and `kQueriesPerTile * kTopK <= kHashSize` holds with exact equality, so a
  // different group is a different tile shape and a different union table, not
  // another value of a constant.  Refused by name.
  CHECK_DIM(4, k_data);
  const int64_t num_kv_heads = k_data.size(1);
  TVM_FFI_ICHECK(geom::is_compiled_kv_head_count(static_cast<int>(num_kv_heads)))
      << "this route has compiled bodies for 4, 2 and 1 kv heads (MiniMax-M3 at tensor-parallel "
         "size 1, 2 and 4), got "
      << num_kv_heads;
  TVM_FFI_ICHECK_EQ(q.size(1), num_kv_heads * geom::kGroupSize)
      << "this route serves GQA group " << geom::kGroupSize << ", so " << num_kv_heads
      << " kv heads require " << (num_kv_heads * geom::kGroupSize) << " query heads, got "
      << q.size(1);
  const geom::PageLayout page = geom::page_layout(static_cast<int>(num_kv_heads));
  ...
  TVM_FFI_ICHECK_EQ(q2k_indices.size(0), num_kv_heads)
  ...
  TVM_FFI_ICHECK_LE(
      (num_kv_heads - 1) * q2k_head_stride + (total_q - 1) * q2k_token_stride + geom::kTopK,
      0x7fffffffLL)
  ...
  check_page_region(k_data, "k_data", num_pages, num_kv_heads, page.page_bytes, geom::kDataDim,
                    geom::kDataHeadStride);
  check_page_region(v_data, "v_data", num_pages, num_kv_heads, page.page_bytes, geom::kDataDim,
                    geom::kDataHeadStride);
  check_page_region(k_scale, "k_scale", num_pages, num_kv_heads, page.page_bytes, geom::kScaleDim,
                    geom::kScaleHeadStride);
  check_page_region(v_scale, "v_scale", num_pages, num_kv_heads, page.page_bytes, geom::kScaleDim,
                    geom::kScaleHeadStride);
  ...
  const cudaError_t status = flashinfer::msa_prefill_nvfp4::launch(
      ...
      static_cast<float>(k_global_scale), static_cast<float>(v_global_scale),
      static_cast<int>(num_kv_heads), device_id, stream);
