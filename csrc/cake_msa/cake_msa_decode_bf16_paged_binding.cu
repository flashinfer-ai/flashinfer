...
#include "cake_msa_decode_bf16_paged.cu"
#undef uint8_t
#undef uint16_t
#undef uint32_t
#undef uint64_t
...
void Run(TensorView arg_Q, TensorView arg_Q_prefill, TensorView arg_Q_prefill_raw, TensorView arg_K,
  ...
  cudaError_t status = cudaFuncSetAttribute(kernel_cake_msa_decode_bf16_paged,
                                            cudaFuncAttributeMaxDynamicSharedMemorySize, 151168);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_cake_msa_decode_bf16_paged) failed: "
      << cudaGetErrorString(status);
  status = cudaLaunchKernel(reinterpret_cast<const void*>(kernel_cake_msa_decode_bf16_paged), grid,
                            block, kargs, 151168u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_cake_msa_decode_bf16_paged launch failed: " << cudaGetErrorString(status);
}
