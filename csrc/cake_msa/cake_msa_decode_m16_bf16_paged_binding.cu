...
#define uint8_t cake_msa_generated_uint8_t
#define uint16_t cake_msa_generated_uint16_t
#define uint32_t cake_msa_generated_uint32_t
#define uint64_t cake_msa_generated_uint64_t
#define int32_t cake_msa_generated_int32_t
#define int16_t cake_msa_generated_int16_t
#include "cake_msa_decode_m16_bf16_paged.cu"
#undef uint8_t
#undef uint16_t
#undef uint32_t
#undef uint64_t
...
#if defined(FLASHINFER_CAKE_MSA_TARGET_MINOR) == defined(FLASHINFER_CAKE_MSA_TARGET_FAMILY)
#error "exactly one CAKE MSA target must be defined"
#endif

inline void CheckCakeMsaTarget(int32_t device_id) {
  int major = 0;
  int minor = 0;
...
#if defined(FLASHINFER_CAKE_MSA_TARGET_MINOR)
  static_assert(FLASHINFER_CAKE_MSA_TARGET_MINOR == 0);
  TVM_FFI_CHECK(major == 10 && minor == 0, RuntimeError)
      << "this CAKE MSA module requires compute capability 10.0, got " << major << "." << minor;
#else
  static_assert(FLASHINFER_CAKE_MSA_TARGET_FAMILY == 100);
...
inline CUtensorMap EncodeTma_Q_prefill(const TensorView& t) {
  ...
  TVM_FFI_CHECK(d1 % 64 == 0, ValueError)
      << "TMA source 'Q_prefill' extent " << d1 << " must divide exactly by " << 64;
...
inline CUtensorMap EncodeTma_K(const TensorView& t) {
  ...
  int64_t d1 = t.size(t.ndim() - 1);
  int64_t d2 = t.size(t.ndim() - 2);
  TVM_FFI_CHECK(d1 > 0 && d2 > 0, ValueError) << "TMA source 'K' trailing dims must be positive";
  int64_t outer2 = t.numel() / (d1 * d2);
  TVM_FFI_CHECK(d1 % 64 == 0, ValueError)
      << "TMA source 'K' extent " << d1 << " must divide exactly by " << 64;
  uint64_t global_dim[4] = {(uint64_t)(64), (uint64_t)(d2), (uint64_t)((d1 / 64)),
                            (uint64_t)(outer2)};
  TVM_FFI_CHECK(global_dim[0] > 0 && global_dim[1] > 0 && global_dim[2] > 0 && global_dim[3] > 0,
                ValueError)
      << "TMA descriptor for 'K' resolved a non-positive global dim";
  TVM_FFI_CHECK(
      64u <= global_dim[0] && 64u <= global_dim[1] && 1u <= global_dim[2] && 1u <= global_dim[3],
      ValueError)
...
inline CUtensorMap EncodeTma_K_prefill_pair(const TensorView& t) {
  ...
  TVM_FFI_CHECK(d1 % 64 == 0, ValueError)
      << "TMA source 'K_prefill_pair' extent " << d1 << " must divide exactly by " << 64;
  uint64_t global_dim[5] = {(uint64_t)(64), (uint64_t)(d2), (uint64_t)(outer3),
                            (uint64_t)((d1 / 64)), (uint64_t)(d3)};
  TVM_FFI_CHECK(global_dim[0] > 0 && global_dim[1] > 0 && global_dim[2] > 0 && global_dim[3] > 0 &&
                    global_dim[4] > 0,
                ValueError)
      << "TMA descriptor for 'K_prefill_pair' resolved a non-positive global dim";
  TVM_FFI_CHECK(64u <= global_dim[0] && 64u <= global_dim[1] && 2u <= global_dim[2] &&
                    2u <= global_dim[3] && 1u <= global_dim[4],
                ValueError)
...
inline CUtensorMap EncodeTma_V(const TensorView& t) {
  ...
  int64_t d1 = t.size(t.ndim() - 1);
  int64_t d2 = t.size(t.ndim() - 2);
  TVM_FFI_CHECK(d1 > 0 && d2 > 0, ValueError) << "TMA source 'V' trailing dims must be positive";
  int64_t outer2 = t.numel() / (d1 * d2);
  TVM_FFI_CHECK(d1 % 64 == 0, ValueError)
...
inline CUtensorMap EncodeTma_V_prefill_pair(const TensorView& t) {
  ...
  int64_t outer3 = t.numel() / (d1 * d2 * d3);
  TVM_FFI_CHECK(d1 % 64 == 0, ValueError)
      << "TMA source 'V_prefill_pair' extent " << d1 << " must divide exactly by " << 64;
  uint64_t global_dim[5] = {(uint64_t)(64), (uint64_t)(d2), (uint64_t)(outer3),
                            (uint64_t)((d1 / 64)), (uint64_t)(d3)};
  TVM_FFI_CHECK(global_dim[0] > 0 && global_dim[1] > 0 && global_dim[2] > 0 && global_dim[3] > 0 &&
                    global_dim[4] > 0,
...
inline CUtensorMap EncodeTma_KV(const TensorView& t) {
  ...
  int64_t d1 = t.size(t.ndim() - 1);
  TVM_FFI_CHECK(d1 > 0, ValueError) << "TMA source 'KV' trailing dims must be positive";
  int64_t outer1 = t.numel() / (d1);
  CheckDenseLeadingFold(t, 1, "KV");
  int64_t s2 = t.stride(t.ndim() - 2) * 1;
  TVM_FFI_CHECK(s2 > 0, ValueError) << "TMA source 'KV' physical strides must be positive";
  ...
  CUtensorMap tm;
  CUresult r = cuTensorMapEncodeTiled(
      &tm, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, 2, t.data_ptr(), global_dim, global_strides, box_dim,
      elem_strides, CU_TENSOR_MAP_INTERLEAVE_NONE, CU_TENSOR_MAP_SWIZZLE_128B,
      CU_TENSOR_MAP_L2_PROMOTION_NONE, CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
  TVM_FFI_CHECK(r == CUDA_SUCCESS, RuntimeError)
...
void Run(TensorView arg_Q, TensorView arg_Q_prefill, TensorView arg_Q_prefill_raw, TensorView arg_K,
         TensorView arg_K_prefill_pair, TensorView arg_V, TensorView arg_V_prefill_pair,
         TensorView arg_KV, TensorView arg_O, TensorView arg_partial_O, TensorView arg_partial_M,
         TensorView arg_partial_D, TensorView arg_split_completion, TensorView arg_msa_lse,
         TensorView arg_kv_indices, TensorView arg_qo_indptr, TensorView arg_kv_indptr,
         TensorView arg_kv_len_arr, TensorView arg_task_kind, TensorView arg_task_request,
         TensorView arg_task_kv_head, TensorView arg_task_q_tile, TensorView arg_task_split,
         TensorView arg_task_kv_tile_begin, TensorView arg_task_kv_tile_end,
         TensorView arg_task_qo_begin, TensorView arg_task_qo_end, TensorView arg_task_page_begin,
         TensorView arg_task_page_end, TensorView arg_status, int64_t arg_num_requests,
         int64_t arg_num_q_heads, int64_t arg_num_kv_heads, int64_t arg_max_kv_tiles,
         int64_t arg_max_splits, int64_t arg_max_task_claims, double arg_softmax_scale_log2,
         int64_t arg_attention_mode, int64_t arg_is_causal, int64_t arg_derive_q_offset,
         int64_t arg_record_tasks, int64_t arg_msa_max_pages, int64_t arg_msa_split_policy,
  ...
  CheckDtype(arg_Q_prefill, "Q_prefill", 4, 16, 1);
  CheckContiguous(arg_Q_prefill, "Q_prefill");
  CheckCudaTensor(arg_Q_prefill_raw, "Q_prefill_raw");
  CheckDtype(arg_Q_prefill_raw, "Q_prefill_raw", 4, 16, 1);
  ...
  CheckDtype(arg_K_prefill_pair, "K_prefill_pair", 4, 16, 1);
  ...
  CheckDtype(arg_V_prefill_pair, "V_prefill_pair", 4, 16, 1);
  ...
  TVM_FFI_CHECK(arg_max_splits >= -2147483648LL && arg_max_splits <= 2147483647LL, ValueError)
      ...
      << " is outside i32 range [-2147483648, 2147483647]";
  TVM_FFI_CHECK(arg_max_task_claims >= -2147483648LL && arg_max_task_claims <= 2147483647LL,
                ValueError)
      << "scalar 'max_task_claims' value " << arg_max_task_claims
      << " is outside i32 range [-2147483648, 2147483647]";
  TVM_FFI_CHECK(arg_attention_mode >= -2147483648LL && arg_attention_mode <= 2147483647LL,
                ValueError)
  ...
  TVM_FFI_CHECK(arg_is_causal >= -2147483648LL && arg_is_causal <= 2147483647LL, ValueError)
      ...
      << " is outside i32 range [-2147483648, 2147483647]";
  TVM_FFI_CHECK(arg_derive_q_offset >= -2147483648LL && arg_derive_q_offset <= 2147483647LL,
                ValueError)
  ...
  TVM_FFI_CHECK(arg_msa_max_pages >= -2147483648LL && arg_msa_max_pages <= 2147483647LL, ValueError)
      ...
      << " is outside i32 range [-2147483648, 2147483647]";
  TVM_FFI_CHECK(arg_msa_split_policy >= -2147483648LL && arg_msa_split_policy <= 2147483647LL,
                ValueError)
  ...
  cudaError_t status = cudaFuncSetAttribute(kernel_cake_msa_decode_m16_bf16_paged,
                                            cudaFuncAttributeMaxDynamicSharedMemorySize, 151168);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_cake_msa_decode_m16_bf16_paged) failed: "
      << cudaGetErrorString(status);
  status = cudaLaunchKernel(reinterpret_cast<const void*>(kernel_cake_msa_decode_m16_bf16_paged),
                            grid, block, kargs, 151168u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_cake_msa_decode_m16_bf16_paged launch failed: " << cudaGetErrorString(status);
}

}  // namespace flashinfer::cake_msa

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, flashinfer::cake_msa::Run);
