...
#include "cake_msa_prefill_m128_bf16_flat.cu"
#undef uint8_t
#undef uint16_t
#undef uint32_t
#undef uint64_t
...
void Run(TensorView arg_q, TensorView arg_k, TensorView arg_v, TensorView arg_out,
  ...
  cudaError_t status = cudaFuncSetAttribute(kernel_cake_msa_prefill_m128_bf16_flat,
                                            cudaFuncAttributeMaxDynamicSharedMemorySize, 201728);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_cake_msa_prefill_m128_bf16_flat) failed: "
      << cudaGetErrorString(status);
  status = cudaLaunchKernel(reinterpret_cast<const void*>(kernel_cake_msa_prefill_m128_bf16_flat),
                            grid, block, kargs, 201728u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_cake_msa_prefill_m128_bf16_flat launch failed: " << cudaGetErrorString(status);
}
