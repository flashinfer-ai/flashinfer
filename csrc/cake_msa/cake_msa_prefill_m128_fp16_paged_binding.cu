...
#include "cake_msa_prefill_m128_fp16_paged.cu"
#undef uint8_t
#undef uint16_t
#undef uint32_t
#undef uint64_t
...
void Run(TensorView arg_q, TensorView arg_k, TensorView arg_v, TensorView arg_out,
         TensorView arg_lse, TensorView arg_temperature_lse, TensorView arg_q2k_indices,
         TensorView arg_cu_seqlens_q, TensorView arg_cu_seqlens_k, TensorView arg_q_offsets,
         TensorView arg_kv_lens, TensorView arg_page_table, int64_t arg_total_q,
         int64_t arg_num_q_heads, int64_t arg_num_kv_heads, int64_t arg_topk,
         int64_t arg_batch_size, int64_t arg_max_pages, int64_t arg_causal,
         int64_t arg_derive_q_offset, double arg_softmax_scale_log2,
         double arg_lse_temperature_scale, int64_t arg_return_softmax_lse,
         int64_t arg_return_temperature_lse, int64_t grid_x, int64_t grid_y, int64_t grid_z,
         int64_t cuda_stream) {
  ...
  TVM_FFI_CHECK(arg_topk >= -2147483648LL && arg_topk <= 2147483647LL, ValueError)
      << "scalar 'topk' value " << arg_topk << " is outside i32 range [-2147483648, 2147483647]";
  ...
  TVM_FFI_CHECK(arg_derive_q_offset >= -2147483648LL && arg_derive_q_offset <= 2147483647LL,
      ...
      << " is outside i32 range [-2147483648, 2147483647]";
  TVM_FFI_CHECK(arg_return_softmax_lse >= -2147483648LL && arg_return_softmax_lse <= 2147483647LL,
                ValueError)
      << "scalar 'return_softmax_lse' value " << arg_return_softmax_lse
      << " is outside i32 range [-2147483648, 2147483647]";
  TVM_FFI_CHECK(
      arg_return_temperature_lse >= -2147483648LL && arg_return_temperature_lse <= 2147483647LL,
      ValueError)
  ...
  cudaError_t status = cudaFuncSetAttribute(kernel_cake_msa_prefill_m128_fp16_paged,
                                            cudaFuncAttributeMaxDynamicSharedMemorySize, 201728);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_cake_msa_prefill_m128_fp16_paged) failed: "
      << cudaGetErrorString(status);
  status = cudaLaunchKernel(reinterpret_cast<const void*>(kernel_cake_msa_prefill_m128_fp16_paged),
                            grid, block, kargs, 201728u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_cake_msa_prefill_m128_fp16_paged launch failed: " << cudaGetErrorString(status);
}
