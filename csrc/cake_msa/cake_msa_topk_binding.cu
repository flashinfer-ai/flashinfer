...
void Run(TensorView arg_max_score, TensorView arg_output, int64_t arg_num_heads,
  ...
  CheckCakeMsaTarget(arg_max_score.device().device_id);
  ...
  cudaError_t status =
      cudaFuncSetAttribute(kernel_cake_msa_topk, cudaFuncAttributeMaxDynamicSharedMemorySize, 128);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_cake_msa_topk) failed: " << cudaGetErrorString(status);
  status = cudaLaunchKernel(reinterpret_cast<const void*>(kernel_cake_msa_topk), grid, block, kargs,
                            128u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_cake_msa_topk launch failed: " << cudaGetErrorString(status);
}
