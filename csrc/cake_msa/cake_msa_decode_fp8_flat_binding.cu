...
#include "cake_msa_decode_fp8_flat.cu"
#undef uint8_t
#undef uint16_t
#undef uint32_t
#undef uint64_t
...
void Run(TensorView arg_Q, TensorView arg_Q_prefill, TensorView arg_Q_prefill_raw, TensorView arg_K,
  ...
  cudaError_t status = cudaFuncSetAttribute(kernel_cake_msa_decode_fp8_flat,
                                            cudaFuncAttributeMaxDynamicSharedMemorySize, 216704);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_cake_msa_decode_fp8_flat) failed: "
      << cudaGetErrorString(status);
  status = cudaLaunchKernel(reinterpret_cast<const void*>(kernel_cake_msa_decode_fp8_flat), grid,
                            block, kargs, 216704u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_cake_msa_decode_fp8_flat launch failed: " << cudaGetErrorString(status);
}
