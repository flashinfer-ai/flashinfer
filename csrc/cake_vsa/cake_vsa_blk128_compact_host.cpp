...
// tvm-ffi host launcher for Cake kernel 'kernel_flashinfer_blackwell_vsa_blk128_seed_sm100'.
#include <cuda.h>
...
TVM_FFI_EMBED_CUBIN(flashinfer_blackwell_vsa_blk128_seed_sm100_d8d08ef84d);
...
inline CUtensorMap EncodeTma_q(const TensorView& t) {
  ...
  TVM_FFI_CHECK(
      64u <= global_dim[0] && 64u <= global_dim[1] && 1u <= global_dim[2] && 2u <= global_dim[3],
      ValueError)
...
void Run(TensorView arg_q, TensorView arg_k, TensorView arg_v, TensorView arg_out,
         TensorView arg_lse, TensorView arg_temperature_lse, TensorView arg_block_mask,
         int64_t arg_mb, int64_t arg_nb, int64_t arg_num_q_heads, int64_t arg_num_kv_heads,
         double arg_softmax_scale_log2, double arg_lse_temperature_scale,
         int64_t arg_return_softmax_lse, int64_t arg_return_temperature_lse, int64_t grid_x,
  ...
  TVM_FFI_CHECK(arg_mb >= -2147483648LL && arg_mb <= 2147483647LL, ValueError)
      << "scalar 'mb' value " << arg_mb << " is outside i32 range [-2147483648, 2147483647]";
  TVM_FFI_CHECK(arg_nb >= -2147483648LL && arg_nb <= 2147483647LL, ValueError)
      << "scalar 'nb' value " << arg_nb << " is outside i32 range [-2147483648, 2147483647]";
  ...
  static auto kernel =
      EmbedCubinModule_flashinfer_blackwell_vsa_blk128_seed_sm100_d8d08ef84d::Global()
          ->mod.GetKernelWithMaxDynamicSharedMemory(
              "kernel_flashinfer_blackwell_vsa_blk128_seed_sm100", 134784);
  tvm::ffi::dim3 grid((uint32_t)grid_x, (uint32_t)grid_y, (uint32_t)grid_z);
  tvm::ffi::dim3 block(512u, 1u, 1u);

  TVM_FFI_CHECK_CUBIN_LAUNCHER_CUDA_ERROR(kernel.Launch(kargs, grid, block, stream, 134784u));
}
