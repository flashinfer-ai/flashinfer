...
// tvm-ffi host launcher for Cake kernel 'kernel_flashinfer_blackwell_vsa_head96_native_sm100'.
#include <cuda.h>
...
TVM_FFI_EMBED_CUBIN(flashinfer_blackwell_vsa_head96_native_sm100_b7e0f1508d);
...
inline int64_t CheckedElementCount(
    const char* name,
    std::initializer_list<int64_t> factors) {
  int64_t count = 1;
  for (int64_t factor : factors) {
    TVM_FFI_CHECK(factor >= 0, ValueError)
        << name << " element-count factors must be non-negative, got " << factor;
    if (factor != 0) {
      TVM_FFI_CHECK(
          count <= std::numeric_limits<int64_t>::max() / factor,
          ValueError)
          << name << " required element count overflows int64";
    }
    count *= factor;
  }
  return count;
...
inline void CheckMinElements(
    const TensorView& t,
    const char* name,
    int64_t required) {
  TVM_FFI_CHECK(t.numel() >= required, ValueError)
      << name << " must hold at least " << required
...
inline CUtensorMap EncodeTma_q(const TensorView& t) {
  ...
  TVM_FFI_CHECK(d1 == 96, ValueError)
      << "TMA source 'q' head dimension must be 96, got " << d1;
  ...
  TVM_FFI_CHECK(64u <= global_dim[1] && 1u <= global_dim[2], ValueError)
...
inline CUtensorMap EncodeTma_k(const TensorView& t) {
  ...
  TVM_FFI_CHECK(d1 == 96, ValueError)
      << "TMA source 'k' head dimension must be 96, got " << d1;
  uint64_t global_dim[3] = {(uint64_t)(d1), (uint64_t)(d3), (uint64_t)(d2)};
  TVM_FFI_CHECK(global_dim[0] > 0 && global_dim[1] > 0 && global_dim[2] > 0, ValueError)
...
inline CUtensorMap EncodeTma_v(const TensorView& t) {
  ...
  TVM_FFI_CHECK(d1 == 96, ValueError)
      << "TMA source 'v' head dimension must be 96, got " << d1;
  uint64_t global_dim[3] = {(uint64_t)(d1), (uint64_t)(d3), (uint64_t)(d2)};
  TVM_FFI_CHECK(global_dim[0] > 0 && global_dim[1] > 0 && global_dim[2] > 0, ValueError)
...
void Run(TensorView arg_q, TensorView arg_k, TensorView arg_v, TensorView arg_out, TensorView arg_lse, TensorView arg_temperature_lse, TensorView arg_block_mask, int64_t arg_mb, int64_t arg_nb, int64_t arg_num_q_heads, int64_t arg_num_kv_heads, double arg_softmax_scale_log2, double arg_lse_temperature_scale, int64_t arg_return_softmax_lse, int64_t arg_return_temperature_lse, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  const int64_t mask_elements = CheckedElementCount(
      "block_mask", {arg_num_q_heads, arg_mb, arg_nb});
  const int64_t output_rows = CheckedElementCount(
      "out", {arg_num_q_heads, arg_mb, 128});
  const int64_t output_elements = CheckedElementCount(
      "out", {output_rows, 96});
  CheckMinElements(arg_block_mask, "block_mask", mask_elements);
  CheckMinElements(arg_out, "out", output_elements);
  if (arg_return_softmax_lse != 0) {
    CheckMinElements(arg_lse, "lse", output_rows);
  }
  if (arg_return_temperature_lse != 0) {
    CheckMinElements(arg_temperature_lse, "temperature_lse", output_rows);
  }
  ...
  static auto kernel = EmbedCubinModule_flashinfer_blackwell_vsa_head96_native_sm100_b7e0f1508d::Global()->mod.GetKernelWithMaxDynamicSharedMemory("kernel_flashinfer_blackwell_vsa_head96_native_sm100", 182656);
  tvm::ffi::dim3 grid((uint32_t)grid_x, (uint32_t)grid_y, (uint32_t)grid_z);
  tvm::ffi::dim3 block(384u, 1u, 1u);

  TVM_FFI_CHECK_CUBIN_LAUNCHER_CUDA_ERROR(kernel.Launch(kargs, grid, block, stream, 182656u));
}
