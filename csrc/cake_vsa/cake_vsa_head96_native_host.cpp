...
TVM_FFI_EMBED_CUBIN(flashinfer_blackwell_vsa_head96_native_sm100_83db436a27);
...
void Run(TensorView arg_q, TensorView arg_k, TensorView arg_v, TensorView arg_out, TensorView arg_lse, TensorView arg_temperature_lse, TensorView arg_block_mask, int64_t arg_mb, int64_t arg_nb, int64_t arg_num_q_heads, int64_t arg_num_kv_heads, double arg_softmax_scale_log2, double arg_lse_temperature_scale, int64_t arg_return_softmax_lse, int64_t arg_return_temperature_lse, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  TVM_FFI_CHECK(grid_x > 0 && grid_y > 0 && grid_z > 0, ValueError)
      ...
      << ", " << grid_z << ")";
  TVM_FFI_CHECK(arg_out.numel() >= CheckedHostExtent({12288, arg_num_q_heads, arg_mb}), ValueError)
      << "out requires at least " << (CheckedHostExtent({12288, arg_num_q_heads, arg_mb}))      << " elements, got " << arg_out.numel();
  ...
  TVM_FFI_CHECK(arg_q.ndim() >= 1, ValueError)
      << "q dimension -1 requires at least 1 dimensions, got " << arg_q.ndim();
  TVM_FFI_CHECK(arg_q.size(-1) == 96, ValueError)
      << "q dimension -1 must equal " << (96)      << ", got " << arg_q.size(-1);
  TVM_FFI_CHECK(arg_k.ndim() >= 1, ValueError)
      << "k dimension -1 requires at least 1 dimensions, got " << arg_k.ndim();
  TVM_FFI_CHECK(arg_k.size(-1) == 96, ValueError)
      << "k dimension -1 must equal " << (96)      << ", got " << arg_k.size(-1);
  TVM_FFI_CHECK(arg_v.ndim() >= 1, ValueError)
      << "v dimension -1 requires at least 1 dimensions, got " << arg_v.ndim();
  TVM_FFI_CHECK(arg_v.size(-1) == 96, ValueError)
      << "v dimension -1 must equal " << (96)      << ", got " << arg_v.size(-1);
  TVM_FFI_CHECK(arg_block_mask.numel() >= CheckedHostExtent({arg_num_q_heads, arg_mb, arg_nb}), ValueError)
      << "block_mask requires at least " << (CheckedHostExtent({arg_num_q_heads, arg_mb, arg_nb}))      << " elements, got " << arg_block_mask.numel();
  ...
  static auto kernel = EmbedCubinModule_flashinfer_blackwell_vsa_head96_native_sm100_83db436a27::Global()->mod.GetKernel("kernel_flashinfer_blackwell_vsa_head96_native_sm100");
  static signed char cake_smem_mode_cache[64] = {0};
  const bool use_oversized_smem = CakeConfigureDynamicSmem(
      kernel, (int)arg_q.device().device_id, 182656,
