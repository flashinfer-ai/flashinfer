...
__global__
__launch_bounds__(512, 1) void kernel_flashinfer_blackwell_vsa_longseq_warp_specialized_sm100(
    ...
    uint8_t* __restrict__ block_mask, int mb, int nb, int selected_blocks, int num_q_heads,
    int num_kv_heads, float softmax_scale_log2, float lse_temperature_scale, int return_softmax_lse,
...
#pragma unroll 1
      for (int union_index_2 = 0; union_index_2 < max_union_count_1; union_index_2++) {
        ...
        if (next_union_index < union_count0_1) {
          ...
          int _mma_b_lo_3 =
              make_warp_uniform((((k_smem_addr) >> 4) & 0x3FFF) + (next_k_stage0) * 2048);
          asm volatile(
              "{\n\t"
              ".reg .pred leader, p0, p1;\n\t"
              ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_3),
              "r"(_mma_b_lo_3), "r"(tmem_scores0), "r"(0));
          elect_commit(s_full_addr);
        ...
        if (union_count1_1 > union_index_2) {
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%0], [%2 + 56], db, id, p1;\n\t"
              "}\n" ::"r"(tmem_output1),
              "r"(_mma_b_lo_4), "r"(tmem_scores1 + 64), "r"(((first_pv1) ? 0 : 1)));
          first_pv1 = 0;
        ...
        if (next_union_index < union_count1_1) {
          ...
          int _mma_b_lo_5 =
              make_warp_uniform((((k_smem_addr) >> 4) & 0x3FFF) + (next_k_stage1) * 2048);
          asm volatile(
              "{\n\t"
              ".reg .pred leader, p0, p1;\n\t"
              ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_5),
              "r"(_mma_b_lo_5), "r"(tmem_scores1), "r"(0));
          elect_commit(s_full_addr + 8);
...
#pragma unroll
        for (int instance_2 = 0; instance_2 < 2; instance_2++) {
          ...
          if (elect_sync()) {
            ...
            tma_4d_gmem2smem(k_smem_addr + k_stage_1 * 32768, k, 0, token0, 0, kv_head_2,
                             k_full_addr + (k_stage_1) * 8);
            tma_4d_gmem2smem(k_smem_addr + k_stage_1 * 32768 + 8192, k, 0, token1, 0, kv_head_2,
                             k_full_addr + (k_stage_1) * 8);
            tma_4d_gmem2smem(k_smem_addr + k_stage_1 * 32768 + 16384, k, 0, token0, 1, kv_head_2,
                             k_full_addr + (k_stage_1) * 8);
            tma_4d_gmem2smem(k_smem_addr + k_stage_1 * 32768 + 24576, k, 0, token1, 1, kv_head_2,
                             k_full_addr + (k_stage_1) * 8);
          }
          k_stage_1 += 1;
          if (k_stage_1 == 3) {
...
#pragma unroll
        for (int instance_3 = 0; instance_3 < 2; instance_3++) {
          ...
          if (elect_sync()) {
            ...
            tma_4d_gmem2smem(v_smem_addr + v_stage_1 * 32768, v, 0, token0_1, 0, kv_head_3,
                             v_full_addr + (v_stage_1) * 8);
            tma_4d_gmem2smem(v_smem_addr + v_stage_1 * 32768 + 8192, v, 0, token1_1, 0, kv_head_3,
                             v_full_addr + (v_stage_1) * 8);
            tma_4d_gmem2smem(v_smem_addr + v_stage_1 * 32768 + 16384, v, 0, token0_1, 1, kv_head_3,
                             v_full_addr + (v_stage_1) * 8);
            tma_4d_gmem2smem(v_smem_addr + v_stage_1 * 32768 + 24576, v, 0, token1_1, 1, kv_head_3,
                             v_full_addr + (v_stage_1) * 8);
          }
          v_stage_1 += 1;
          if (v_stage_1 == 2) {
