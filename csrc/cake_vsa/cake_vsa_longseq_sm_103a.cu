...
#define SMEM_V_SMEM_OFF 132096
#define SMEM_V_SMEM_STAGE_BYTES 32768
#define SMEM_V_SMEM_STRIDE 32768
#define SMEM_SCALE_SMEM_OFF 197632
#define SMEM_SCALE_SMEM_STAGE_BYTES 1024
#define SMEM_SCALE_SMEM_STRIDE 1024
#define SMEM_UNION_COUNT_SMEM_OFF 199680
#define SMEM_UNION_COUNT_SMEM_STAGE_BYTES 8
#define SMEM_UNION_COUNT_SMEM_STRIDE 8
#define SMEM_UNION_BLOCKS_OFF 199688
#define SMEM_UNION_BLOCKS_STAGE_BYTES 1536
#define SMEM_UNION_BLOCKS_STRIDE 1536
#define SMEM_TOTAL 201344
#define THREADS 512
...
kernel_flashinfer_blackwell_vsa_longseq_warp_specialized_sm100(const __grid_constant__ CUtensorMap q, const __grid_constant__ CUtensorMap k, const __grid_constant__ CUtensorMap v, __nv_bfloat16* __restrict__ out, float* __restrict__ lse, float* __restrict__ temperature_lse, uint8_t* __restrict__ block_mask, int mb, int nb, int selected_blocks, int num_q_heads, int num_kv_heads, float softmax_scale_log2, float lse_temperature_scale, int return_softmax_lse, int return_temperature_lse)
{
    ...
    __nv_bfloat16* v_smem = reinterpret_cast<__nv_bfloat16*>(smem_raw + 132096);
    const int v_smem_addr = smem + 132096;
    float* scale_smem = reinterpret_cast<float*>(smem_raw + 197632);
    const int scale_smem_addr = smem + 197632;
    int* union_count_smem = reinterpret_cast<int*>(smem_raw + 199680);
    const int union_count_smem_addr = smem + 199680;
    int* union_blocks = reinterpret_cast<int*>(smem_raw + 199688);
    const int union_blocks_addr = smem + 199688;

    // Mbarrier init (12 groups, 23 barriers)
    ...
    if (warp <= 7) {
        ...
        { // softmax_main
            ...
            int union_count = selected_blocks;
            ...
            for (int union_index = 0; union_index < union_count; union_index++) {
                int n_block = union_blocks[instance * 192 + union_index];
                if (instance == 0) {
    ...
    if (warp >= 8 && warp <= 11) {
        ...
        { // correction_main
            ...
            int max_union_count = selected_blocks;
            ...
            for (int union_index_1 = 1; union_index_1 < max_union_count; union_index_1++) {
                int union_count0 = selected_blocks;
                ...
                int union_count1 = selected_blocks;
                if (union_count1 > union_index_1) {
                    mbarrier_wait(corr_sig_addr + 8, _phase_corr_sig_1);
                    _phase_corr_sig_1 ^= 1;
    ...
    if (warp == 12) {
        { // mma_warp_main
            ...
            int max_union_count_1 = selected_blocks;
            ...
            int first_pv0 = 1;
            int first_pv1 = 1;
            int union_count0_1 = selected_blocks;
            if (union_count0_1 > 0) {
                unsigned int k_stage0 = k_stage;
                unsigned int k_phase0 = k_phase;
                k_stage += 1;
                if (k_stage == 3) { k_stage = 0; k_phase ^= 1; }
                mbarrier_wait(k_full_addr + (k_stage0) * 8, k_phase0);
                int _mma_a_lo_0 = make_warp_uniform(((q0_smem_addr) >> 4) & 0x3FFF);
                int _mma_b_lo_0 = make_warp_uniform((((k_smem_addr) >> 4) & 0x3FFF) + (k_stage0) * 2048);
                asm volatile(
                    "{\n\t"
                ...
                elect_commit(k_empty_addr + (k_stage0) * 8);
            }
            int union_count1_1 = selected_blocks;
            if (union_count1_1 > 0) {
                unsigned int k_stage1 = k_stage;
                unsigned int k_phase1 = k_phase;
                k_stage += 1;
                if (k_stage == 3) { k_stage = 0; k_phase ^= 1; }
                mbarrier_wait(k_full_addr + (k_stage1) * 8, k_phase1);
                int _mma_a_lo_1 = make_warp_uniform(((q1_smem_addr) >> 4) & 0x3FFF);
                int _mma_b_lo_1 = make_warp_uniform((((k_smem_addr) >> 4) & 0x3FFF) + (k_stage1) * 2048);
                asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
                ...
                elect_commit(k_empty_addr + (k_stage1) * 8);
            }
            ...
            for (int union_index_2 = 0; union_index_2 < max_union_count_1; union_index_2++) {
                if (union_count0_1 > union_index_2) {
                    unsigned int v_stage0 = v_stage;
                    unsigned int v_phase0 = v_phase;
                    v_stage += 1;
                    if (v_stage == 2) { v_stage = 0; v_phase ^= 1; }
                    mbarrier_wait(v_full_addr + (v_stage0) * 8, v_phase0);
                    ...
                    elect_commit(v_empty_addr + (v_stage0) * 8);
                }
                int next_union_index = union_index_2 + 1;
                if (next_union_index < union_count0_1) {
                    unsigned int next_k_stage0 = k_stage;
                    unsigned int next_k_phase0 = k_phase;
                    k_stage += 1;
                    if (k_stage == 3) { k_stage = 0; k_phase ^= 1; }
                    mbarrier_wait(k_full_addr + (next_k_stage0) * 8, next_k_phase0);
                    int _mma_a_lo_3 = make_warp_uniform(((q0_smem_addr) >> 4) & 0x3FFF);
                    int _mma_b_lo_3 = make_warp_uniform((((k_smem_addr) >> 4) & 0x3FFF) + (next_k_stage0) * 2048);
                    asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
                    "}\n"
                    :: "r"(_mma_a_lo_3), "r"(_mma_b_lo_3), "r"(tmem_scores0), "r"(0));
                    elect_commit(s_full_addr);
                    elect_commit(k_empty_addr + (next_k_stage0) * 8);
                }
                if (union_count1_1 > union_index_2) {
                    unsigned int v_stage1 = v_stage;
                    unsigned int v_phase1 = v_phase;
                    v_stage += 1;
                    if (v_stage == 2) { v_stage = 0; v_phase ^= 1; }
                    mbarrier_wait(v_full_addr + (v_stage1) * 8, v_phase1);
                    mbarrier_wait(p_full_addr + 8, _phase_p_full_1);
                    _phase_p_full_1 ^= 1;
                    int _mma_b_lo_4 = make_warp_uniform(((((v_smem_addr) >> 4) & 0x3FFF) | 0x4000000) + (v_stage1) * 2048);
                    asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 dhi, blo, ta, id;\n\t"
                    ...
                    first_pv1 = 0;
                    if (union_index_2 + 1 == union_count1_1) {
                        elect_commit(o_full_addr + 8);
                    }
                    elect_commit(v_empty_addr + (v_stage1) * 8);
                }
                if (next_union_index < union_count1_1) {
                    unsigned int next_k_stage1 = k_stage;
                    unsigned int next_k_phase1 = k_phase;
                    k_stage += 1;
                    if (k_stage == 3) { k_stage = 0; k_phase ^= 1; }
                    mbarrier_wait(k_full_addr + (next_k_stage1) * 8, next_k_phase1);
                    int _mma_a_lo_5 = make_warp_uniform(((q1_smem_addr) >> 4) & 0x3FFF);
                    int _mma_b_lo_5 = make_warp_uniform((((k_smem_addr) >> 4) & 0x3FFF) + (next_k_stage1) * 2048);
                    asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
                    ...
                    elect_commit(s_full_addr + 8);
                    elect_commit(k_empty_addr + (next_k_stage1) * 8);
                }
            }
        ...
        { // qk_load_warp_main
            ...
            for (int block_base = 0; block_base < num_n_blocks_2; block_base += 32) {
                ...
                if (selected_1 != 0) {
                    ...
                    union_blocks[192 + selected_slot] = n_block_1;
                }
            ...
            #pragma unroll 1
            for (int union_index_3 = 0; union_index_3 < selected_blocks; union_index_3++) {
                #pragma unroll
                for (int instance_2 = 0; instance_2 < 2; instance_2++) {
                    int n_block_2 = union_blocks[instance_2 * 192 + union_index_3];
                    int token_base = k_start_2 + n_block_2 * 128;
                    mbarrier_wait(k_empty_addr + (k_stage_1) * 8, _phase_k_empty);
                    if (elect_sync()) {
                        mbarrier_arrive_expect_tx(k_full_addr + (k_stage_1) * 8, 32768);
                        int token0 = token_base;
                        int token1 = token_base + 64;
                        tma_4d_gmem2smem(k_smem_addr + k_stage_1 * 32768, (&k), 0, token0, 0, kv_head_2, k_full_addr + (k_stage_1) * 8);
                        tma_4d_gmem2smem(k_smem_addr + k_stage_1 * 32768 + 8192, (&k), 0, token1, 0, kv_head_2, k_full_addr + (k_stage_1) * 8);
                        tma_4d_gmem2smem(k_smem_addr + k_stage_1 * 32768 + 16384, (&k), 0, token0, 1, kv_head_2, k_full_addr + (k_stage_1) * 8);
                        tma_4d_gmem2smem(k_smem_addr + k_stage_1 * 32768 + 24576, (&k), 0, token1, 1, kv_head_2, k_full_addr + (k_stage_1) * 8);
                    }
                    k_stage_1 += 1;
    ...
    if (warp == 14) {
        { // v_load_warp_main
            unsigned int _phase_union_ready_0_3 = 0;
            mbarrier_wait(union_ready_addr, _phase_union_ready_0_3);
            _phase_union_ready_0_3 ^= 1;
            int batch_3 = 0;
            int q_tile_3 = blockIdx.x;
            int q_head_3 = blockIdx.y;
            int kv_head_3 = q_head_3;
            int q_local_base_3 = q_tile_3 * 128;
            int q_valid_3 = 128;
            if (q_tile_3 >= mb) {
                q_valid_3 = 0;
            }
            ...
            #pragma unroll 1
            for (int union_index_4 = 0; union_index_4 < selected_blocks; union_index_4++) {
                #pragma unroll
                for (int instance_3 = 0; instance_3 < 2; instance_3++) {
                    int n_block_3 = union_blocks[instance_3 * 192 + union_index_4];
                    ...
                    if (elect_sync()) {
                        ...
                        tma_4d_gmem2smem(v_smem_addr + v_stage_1 * 32768, (&v), 0, token0_1, 0, kv_head_3, v_full_addr + (v_stage_1) * 8);
                        tma_4d_gmem2smem(v_smem_addr + v_stage_1 * 32768 + 8192, (&v), 0, token1_1, 0, kv_head_3, v_full_addr + (v_stage_1) * 8);
                        tma_4d_gmem2smem(v_smem_addr + v_stage_1 * 32768 + 16384, (&v), 0, token0_1, 1, kv_head_3, v_full_addr + (v_stage_1) * 8);
                        tma_4d_gmem2smem(v_smem_addr + v_stage_1 * 32768 + 24576, (&v), 0, token1_1, 1, kv_head_3, v_full_addr + (v_stage_1) * 8);
                    }
                    v_stage_1 += 1;
