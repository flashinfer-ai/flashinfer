...
__global__ __launch_bounds__(384, 1) void kernel_flashinfer_blackwell_vsa_head96_native_sm100(
...
#pragma unroll
                for (int _ps = 0; _ps < 4; _ps++)
                  mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_3[offset])[_ps],
                                    _prescale2_1);
#else
#pragma unroll
                for (int _ps = 0; _ps < 8; _ps++) _tmem_load_3[offset + _ps] *= inv_sum;
#endif
...
#pragma unroll 1
      for (int union_index_2 = 0; union_index_2 < max_union_count_1; union_index_2++) {
        ...
        asm volatile(
            ...
            "}\n" ::"r"(tmem_output0),
            "r"(_mma_b_lo_1), "r"(tmem_scores0 + 64), "r"(((first_pv0) ? 0 : 1)));
        first_pv0 = 0;
...
#pragma unroll 1
      for (int union_index_3 = 0; union_index_3 < max_union_count_2; union_index_3++) {
        ...
        if (elect_sync()) {
          ...
          tma_3d_gmem2smem(kv_smem_addr + load_stage * 32768, k, 0, token0, kv_head_2,
                           kv_full_addr + (load_stage) * 8);
          tma_3d_gmem2smem(kv_smem_addr + load_stage * 32768 + 8192, k, 0, token1, kv_head_2,
                           kv_full_addr + (load_stage) * 8);
          tma_3d_gmem2smem(kv_smem_addr + load_stage * 32768 + 16384, k, 64, token0, kv_head_2,
                           kv_full_addr + (load_stage) * 8);
          tma_3d_gmem2smem(kv_smem_addr + load_stage * 32768 + 24576, k, 64, token1, kv_head_2,
                           kv_full_addr + (load_stage) * 8);
        }
        load_stage += 1;
        if (load_stage == 5) {
        ...
        if (elect_sync()) {
          ...
          tma_3d_gmem2smem(v_smem_addr + load_stage * 32768, v, 0, token0_1, kv_head_2,
                           kv_full_addr + (load_stage) * 8);
          tma_3d_gmem2smem(v_smem_addr + load_stage * 32768 + 8192, v, 0, token1_1, kv_head_2,
                           kv_full_addr + (load_stage) * 8);
          tma_3d_gmem2smem(v_smem_addr + load_stage * 32768 + 16384, v, 64, token0_1, kv_head_2,
                           kv_full_addr + (load_stage) * 8);
          tma_3d_gmem2smem(v_smem_addr + load_stage * 32768 + 24576, v, 64, token1_1, kv_head_2,
                           kv_full_addr + (load_stage) * 8);
        }
        load_stage += 1;
        if (load_stage == 5) {
