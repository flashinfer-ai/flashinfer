        ...
        { // mma_warp_main
            ...
            for (int tile_iter_2 = 0; tile_iter_2 < tiles_per_cta; tile_iter_2++) {
                ...
                if (tile_idx_2 < total_tiles) {
                    ...
                    "}\n"
                    :: "r"(tmem_output), "r"(_mma_b_lo_1), "r"(tmem_scores + 64), "r"(((first_pv) ? 0 : 1)));
                        first_pv = 0;
                        if (pair_index_1 + 1 == pair_count_2) {
            ...
            for (int tile_iter_3 = 0; tile_iter_3 < tiles_per_cta; tile_iter_3++) {
                ...
                if (tile_idx_3 < total_tiles) {
                    ...
                    for (int pair_index_2 = 0; pair_index_2 < pair_count_3; pair_index_2++) {
                        ...
                        if (elect_sync()) {
                            mbarrier_arrive_expect_tx(kv_full_addr + (load_stage) * 8, 32768);
                            tma_4d_gmem2smem(v_smem_addr + load_stage * 32768, v, 0, token0, 0, head_3, kv_full_addr + (load_stage) * 8);
                            tma_4d_gmem2smem(v_smem_addr + load_stage * 32768 + 8192, v, 0, token1, 0, head_3, kv_full_addr + (load_stage) * 8);
                            tma_4d_gmem2smem(v_smem_addr + load_stage * 32768 + 16384, v, 0, token0, 1, head_3, kv_full_addr + (load_stage) * 8);
                            tma_4d_gmem2smem(v_smem_addr + load_stage * 32768 + 24576, v, 0, token1, 1, head_3, kv_full_addr + (load_stage) * 8);
                        }
                        load_stage += 1;
                        if (load_stage == 2) { load_stage = 0; _phase_kv_empty ^= 1; }
                    }
