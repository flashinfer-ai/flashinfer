...
#define SMEM_SCALE_SMEM_OFF 82944
#define SMEM_SCALE_SMEM_STAGE_BYTES 768
#define SMEM_SCALE_SMEM_STRIDE 768
...
__global__ __launch_bounds__(384, 1) void kernel_flashinfer_vsa_blk64_persistent_m64_sm100(
    CakeTensorMap const* q, CakeTensorMap const* k, CakeTensorMap const* v,
    __nv_bfloat16* __restrict__ out, float* __restrict__ lse, int* __restrict__ indptr,
    int* __restrict__ indices, int sequence_q, int query_blocks, int total_tiles, int tiles_per_cta,
    int num_heads, float softmax_scale_log2, int return_lse) {
  ...
  float* scale_smem = reinterpret_cast<float*>(smem_raw + 82944);
  const int scale_smem_addr = smem + 82944;

  // Mbarrier init (10 groups, 12 barriers)
...
#pragma unroll 1
      for (int tile_iter = 0; tile_iter < tiles_per_cta; tile_iter++) {
        ...
        if (tile_idx < total_tiles) {
          ...
          int row_begin = indptr[q_block];
          int selected_count = indptr[q_block + 1] - row_begin;
...
#pragma unroll 1
          for (int pair_index = 0; pair_index < pair_count; pair_index++) {
            ...
            _phase_s_full_0 ^= 1;
            int valid_cols = 128;
            if (selected_count <= pair_index * 2 + 1 || row_valid == 0) {
              valid_cols = ((row_valid != 0) ? 64 : 0);
            }
...
#pragma unroll
            for (int _lf = 0; _lf < 32; _lf++)
              fma_f32x2_inplace(&reinterpret_cast<float2*>(_tmem_load_0)[_lf], _fma_b2_5,
                                _fma_c2_6);
#pragma unroll
...
#pragma unroll
            ...
            int p_addr = taddr + 64 + (unsigned int)(tmem_row_origin << 16);
            asm volatile(
          ...
          if (col_half == 0) {
            scale_smem[64 + my_row] = row_sum;
            scale_smem[128 + my_row] = row_max;
          }
          asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
          mbarrier_arrive(corr_sig_addr);
        }
...
#pragma unroll 1
      for (int tile_iter_1 = 0; tile_iter_1 < tiles_per_cta; tile_iter_1++) {
        ...
        if (tile_idx_1 < total_tiles) {
          ...
          int row_begin_1 = indptr[q_block_1];
          int selected_count_1 = indptr[q_block_1 + 1] - row_begin_1;
          int pair_count_1 = (selected_count_1 + 1) / 2;
          const int warp_in_role_1 = warp - 4;
          const int tmem_row_origin_1 = warp_in_role_1 * 32;
          const int logical_row_origin_1 = warp_in_role_1 * 16;
...
#pragma unroll 1
          for (int _pair_index = 1; _pair_index < pair_count_1; _pair_index++) {
            mbarrier_wait(corr_sig_addr, _phase_corr_sig_0);
            _phase_corr_sig_0 ^= 1;
            float acc_scale_1 = scale_smem[my_row_1];
...
#pragma unroll
              ...
              asm volatile(
                  ...
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[0])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[1])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[2])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[3])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[4])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[5])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[6])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[7])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[8])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[9])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[10])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[11])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[12])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[13])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[14])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[15])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[16])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[17])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[18])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[19])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[20])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[21])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[22])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[23])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[24])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[25])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[26])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[27])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[28])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[29])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[30])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[31])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[32])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[33])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[34])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[35])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[36])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[37])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[38])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[39])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[40])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[41])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[42])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[43])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[44])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[45])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[46])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[47])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[48])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[49])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[50])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[51])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[52])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[53])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[54])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[55])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[56])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[57])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[58])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[59])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[60])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[61])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[62])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_1[63]))
                  : "memory");
              asm volatile("tcgen05.wait::st.sync.aligned;" ::: "memory");
            }
            mbarrier_arrive(p_full_addr);
          ...
          _phase_corr_sig_0 ^= 1;
          asm volatile("tcgen05.fence::after_thread_sync;");
          float final_sum = scale_smem[64 + my_row_1];
          float final_max = scale_smem[128 + my_row_1];
...
#pragma unroll
                for (int _ps = 0; _ps < 4; _ps++)
                  mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_2[offset])[_ps],
                                    _prescale2_1);
#else
#pragma unroll
                for (int _ps = 0; _ps < 8; _ps++) _tmem_load_2[offset + _ps] *= inv_sum;
#endif
...
#pragma unroll 1
      for (int tile_iter_2 = 0; tile_iter_2 < tiles_per_cta; tile_iter_2++) {
        ...
        if (tile_idx_2 < total_tiles) {
          ...
          int row_begin_2 = indptr[q_block_2];
          int selected_count_2 = indptr[q_block_2 + 1] - row_begin_2;
          int pair_count_2 = (selected_count_2 + 1) / 2;
          mbarrier_wait(q_full_addr, _phase_q_full_0);
          _phase_q_full_0 ^= 1;
          int first_pv = 1;
#pragma unroll 1
          for (int pair_index_1 = 0; pair_index_1 < pair_count_2; pair_index_1++) {
            ...
            int _mma_b_lo_0 =
                make_warp_uniform((((kv_smem_addr) >> 4) & 0x3FFF) + (k_stage) * 2048);
            asm volatile(
                "{\n\t"
                ...
                "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
                "}\n" ::"r"(_mma_a_lo_0),
                "r"(_mma_b_lo_0), "r"(tmem_scores), "r"(0));
            if (pair_index_1 + 1 == pair_count_2) {
            ...
            _phase_p_full_0 ^= 1;
            int _mma_b_lo_1 =
                make_warp_uniform(((((v_smem_addr) >> 4) & 0x3FFF) | 0x4000000) + (v_stage) * 2048);
            asm volatile(
                "{\n\t"
                ".reg .pred leader, p0, p1;\n\t"
                ".reg .b32 dhi, blo, id;\n\t"
                ...
                "}\n" ::"r"(tmem_output),
                "r"(_mma_b_lo_1), "r"(tmem_scores + 64), "r"(((first_pv) ? 0 : 1)));
            first_pv = 0;
            if (pair_index_1 + 1 == pair_count_2) {
...
#pragma unroll 1
      for (int tile_iter_3 = 0; tile_iter_3 < tiles_per_cta; tile_iter_3++) {
        ...
        if (tile_idx_3 < total_tiles) {
          ...
          int row_begin_3 = indptr[q_block_3];
          int selected_count_3 = indptr[q_block_3 + 1] - row_begin_3;
...
#pragma unroll 1
          for (int pair_index_2 = 0; pair_index_2 < pair_count_3; pair_index_2++) {
            int entry0 = pair_index_2 * 2;
            int entry1 = entry0 + 1;
            int block0 = indices[row_begin_3 + entry0];
            int block1 = block0;
            if (entry1 < selected_count_3) {
              block1 = indices[row_begin_3 + entry1];
            }
            int token0 = block0 * 64;
            int token1 = block1 * 64;
            mbarrier_wait(kv_empty_addr + (load_stage) * 8, _phase_kv_empty);
            if (elect_sync()) {
              mbarrier_arrive_expect_tx(kv_full_addr + (load_stage) * 8, 32768);
              tma_4d_gmem2smem(kv_smem_addr + load_stage * 32768, k, 0, token0, 0, head_3,
                               kv_full_addr + (load_stage) * 8);
              tma_4d_gmem2smem(kv_smem_addr + load_stage * 32768 + 8192, k, 0, token1, 0, head_3,
                               kv_full_addr + (load_stage) * 8);
              tma_4d_gmem2smem(kv_smem_addr + load_stage * 32768 + 16384, k, 0, token0, 1, head_3,
                               kv_full_addr + (load_stage) * 8);
              tma_4d_gmem2smem(kv_smem_addr + load_stage * 32768 + 24576, k, 0, token1, 1, head_3,
                               kv_full_addr + (load_stage) * 8);
            }
            load_stage += 1;
            if (load_stage == 2) {
            ...
            if (elect_sync()) {
              mbarrier_arrive_expect_tx(kv_full_addr + (load_stage) * 8, 32768);
              tma_4d_gmem2smem(v_smem_addr + load_stage * 32768, v, 0, token0, 0, head_3,
                               kv_full_addr + (load_stage) * 8);
              tma_4d_gmem2smem(v_smem_addr + load_stage * 32768 + 8192, v, 0, token1, 0, head_3,
                               kv_full_addr + (load_stage) * 8);
              tma_4d_gmem2smem(v_smem_addr + load_stage * 32768 + 16384, v, 0, token0, 1, head_3,
                               kv_full_addr + (load_stage) * 8);
              tma_4d_gmem2smem(v_smem_addr + load_stage * 32768 + 24576, v, 0, token1, 1, head_3,
                               kv_full_addr + (load_stage) * 8);
            }
            load_stage += 1;
            if (load_stage == 2) {
