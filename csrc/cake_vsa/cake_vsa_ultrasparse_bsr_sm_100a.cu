...
__global__ __launch_bounds__(512, 1) void kernel_flashinfer_blackwell_vsa_ultrasparse_bsr_sm100(
    ...
    int* __restrict__ bsr_indices, int mb, int nb, int selected_blocks, int total_tiles,
    int num_q_heads, int num_kv_heads, float softmax_scale_log2, float lse_temperature_scale,
...
#pragma unroll
            for (int cr_col_1 = 0; cr_col_1 < 8; cr_col_1++) {
              int cr_addr1 =
                  taddr + 256 + 128 + (unsigned int)row_addr_1 + (unsigned int)(cr_col_1 * 16);
...
#else
#pragma unroll
              for (int _ps = 0; _ps < 8; _ps++) _tmem_load_3[0 + _ps] *= 1.0f;
#endif
              ...
              *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(out + (output_row + out_col * 8)))[0]) =
...
#pragma unroll
          for (int n_stage_2 = 0; n_stage_2 < 2; n_stage_2++) {
            ...
            if (n_stage_2 == 0) {
              ...
              _phase_p_full_1 ^= 1;
              int _mma_b_lo_3 = make_warp_uniform(((((v_smem_addr) >> 4) & 0x3FFF) | 0x4000000) +
                                                  (v_stage) * 2048);
              asm volatile(
                  "{\n\t"
                  ".reg .pred leader, p0, p1;\n\t"
            ...
            if (pair_index_1 + 1 < 3) {
              ...
              if (n_stage_2 == 0) {
                ...
                int _mma_b_lo_4 =
                    make_warp_uniform((((kv_smem_addr) >> 4) & 0x3FFF) + (next_k_stage) * 2048);
                asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
...
#pragma unroll
        for (int n_stage_3 = 0; n_stage_3 < 2; n_stage_3++) {
          ...
          if (elect_sync()) {
            ...
            tma_4d_gmem2smem(kv_smem_addr + kv_stage_1 * 32768, k, 0, token0, 0, kv_head_2,
                             kv_full_addr + (kv_stage_1) * 8);
            tma_4d_gmem2smem(kv_smem_addr + kv_stage_1 * 32768 + 8192, k, 0, token1, 0, kv_head_2,
                             kv_full_addr + (kv_stage_1) * 8);
            tma_4d_gmem2smem(kv_smem_addr + kv_stage_1 * 32768 + 16384, k, 0, token0, 1, kv_head_2,
                             kv_full_addr + (kv_stage_1) * 8);
            tma_4d_gmem2smem(kv_smem_addr + kv_stage_1 * 32768 + 24576, k, 0, token1, 1, kv_head_2,
                             kv_full_addr + (kv_stage_1) * 8);
          }
          kv_stage_1 += 1;
          if (kv_stage_1 == 3) {
...
#pragma unroll
          for (int n_stage_4 = 0; n_stage_4 < 2; n_stage_4++) {
            ...
            if (elect_sync()) {
              ...
              tma_4d_gmem2smem(v_smem_addr + kv_stage_1 * 32768, v, 0, token0_1, 0, kv_head_2,
                               kv_full_addr + (kv_stage_1) * 8);
              tma_4d_gmem2smem(v_smem_addr + kv_stage_1 * 32768 + 8192, v, 0, token1_1, 0,
                               kv_head_2, kv_full_addr + (kv_stage_1) * 8);
              tma_4d_gmem2smem(v_smem_addr + kv_stage_1 * 32768 + 16384, v, 0, token0_1, 1,
                               kv_head_2, kv_full_addr + (kv_stage_1) * 8);
              tma_4d_gmem2smem(v_smem_addr + kv_stage_1 * 32768 + 24576, v, 0, token1_1, 1,
                               kv_head_2, kv_full_addr + (kv_stage_1) * 8);
            }
            kv_stage_1 += 1;
            if (kv_stage_1 == 3) {
            ...
            if (next_logical_index >= 0) {
              ...
              if (elect_sync()) {
                ...
                tma_4d_gmem2smem(kv_smem_addr + kv_stage_1 * 32768, k, 0, token0_2, 0, kv_head_2,
                                 kv_full_addr + (kv_stage_1) * 8);
                tma_4d_gmem2smem(kv_smem_addr + kv_stage_1 * 32768 + 8192, k, 0, token1_2, 0,
                                 kv_head_2, kv_full_addr + (kv_stage_1) * 8);
                tma_4d_gmem2smem(kv_smem_addr + kv_stage_1 * 32768 + 16384, k, 0, token0_2, 1,
                                 kv_head_2, kv_full_addr + (kv_stage_1) * 8);
                tma_4d_gmem2smem(kv_smem_addr + kv_stage_1 * 32768 + 24576, k, 0, token1_2, 1,
                                 kv_head_2, kv_full_addr + (kv_stage_1) * 8);
              }
              kv_stage_1 += 1;
              if (kv_stage_1 == 3) {
