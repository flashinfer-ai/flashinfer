...
__global__ __launch_bounds__(512, 1) void kernel_flashinfer_blackwell_vsa_blk128_fp16_compact_sm100(
...
#pragma unroll
              ...
              *reinterpret_cast<uint4*>(&((__half*)(out + (output_row + col_base + offset)))[0]) =
        ...
        for (int instance_2 = 0; instance_2 < 2; instance_2++) {
          ...
          if (instance_union_count > union_index_3) {
            ...
            if (elect_sync()) {
              ...
              tma_4d_gmem2smem(kv_smem_addr + load_stage * 32768, k, 0, token0, 0, kv_head_2,
                               kv_full_addr + (load_stage) * 8);
              tma_4d_gmem2smem(kv_smem_addr + load_stage * 32768 + 8192, k, 0, token1, 0, kv_head_2,
                               kv_full_addr + (load_stage) * 8);
              tma_4d_gmem2smem(kv_smem_addr + load_stage * 32768 + 16384, k, 0, token0, 1,
                               kv_head_2, kv_full_addr + (load_stage) * 8);
        ...
        for (int instance_3 = 0; instance_3 < 2; instance_3++) {
          ...
          if (instance_union_count_1 > union_index_3) {
            ...
            if (elect_sync()) {
              ...
              tma_4d_gmem2smem(v_smem_addr + load_stage * 32768, v, 0, token0_1, 0, kv_head_2,
                               kv_full_addr + (load_stage) * 8);
