...
__global__ __launch_bounds__(384, 1) void kernel_flashinfer_blackwell_vsa_head64_native_sm100(
...
#pragma unroll 1
      for (int union_index_2 = 0; union_index_2 < max_union_count_1; union_index_2++) {
        ...
        _phase_p_full_0 ^= 1;
        int _mma_b_lo_1 =
            make_warp_uniform(((((v_smem_addr) >> 4) & 0x3FFF) | 0x4000000) + (v_stage) * 1024);
        asm volatile(
            "{\n\t"
            ".reg .pred leader, p0, p1;\n\t"
            ".reg .b32 dhi, blo, id;\n\t"
...
#pragma unroll 1
      for (int union_index_3 = 0; union_index_3 < max_union_count_2; union_index_3++) {
        ...
        if (elect_sync()) {
          ...
          tma_4d_gmem2smem(kv_smem_addr + load_stage * 16384, k, 0, token0, 0, kv_head_2,
                           kv_full_addr + (load_stage) * 8);
          tma_4d_gmem2smem(kv_smem_addr + load_stage * 16384 + 8192, k, 0, token1, 0, kv_head_2,
                           kv_full_addr + (load_stage) * 8);
        }
        load_stage += 1;
        if (load_stage == 6) {
        ...
        if (elect_sync()) {
          ...
          tma_4d_gmem2smem(v_smem_addr + load_stage * 16384, v, 0, token0_1, 0, kv_head_2,
                           kv_full_addr + (load_stage) * 8);
          tma_4d_gmem2smem(v_smem_addr + load_stage * 16384 + 8192, v, 0, token1_1, 0, kv_head_2,
                           kv_full_addr + (load_stage) * 8);
        }
        load_stage += 1;
        if (load_stage == 6) {
