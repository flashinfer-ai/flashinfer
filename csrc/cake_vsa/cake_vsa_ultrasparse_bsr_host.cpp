...
inline CUtensorMap EncodeTma_q(const TensorView& t) {
  ...
  TVM_FFI_CHECK(
      64u <= global_dim[0] && 128u <= global_dim[1] && 1u <= global_dim[2] && 2u <= global_dim[3],
      ValueError)
...
void Run(TensorView arg_q, TensorView arg_k, TensorView arg_v, TensorView arg_out,
         TensorView arg_lse, TensorView arg_temperature_lse, TensorView arg_bsr_indices,
         int64_t arg_mb, int64_t arg_nb, int64_t arg_selected_blocks, int64_t arg_total_tiles,
         int64_t arg_num_q_heads, int64_t arg_num_kv_heads, double arg_softmax_scale_log2,
  ...
  TVM_FFI_CHECK(arg_selected_blocks >= -2147483648LL && arg_selected_blocks <= 2147483647LL,
                ValueError)
  ...
  static auto kernel =
      EmbedCubinModule_flashinfer_blackwell_vsa_ultrasparse_bsr_sm100_d29e134a87::Global()
          ->mod.GetKernelWithMaxDynamicSharedMemory(
              "kernel_flashinfer_blackwell_vsa_ultrasparse_bsr_sm100", 135424);
