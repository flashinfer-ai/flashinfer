...
at::Tensor NVFP4BlockScaleInterleave(at::Tensor const& blockScale)
{
    ...
    at::Tensor interleavedBlockScale = at::empty(
        {expert_out_size * num_experts}, at::dtype(SF_DTYPE).device(blockScale.device()).requires_grad(false));
...
at::Tensor NVFP4BlockScaleInterleaveReverse(at::Tensor const& blockScale)
{
    ...
    at::Tensor reversedBlockScale
        = at::empty(blockScaleShape, at::dtype(SF_DTYPE).device(blockScale.device()).requires_grad(false));
