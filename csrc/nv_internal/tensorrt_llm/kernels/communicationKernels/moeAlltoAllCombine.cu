...
__device__ __forceinline__ void combine_top_k(
    ...
    unsigned long long workspace_stride_bytes, unsigned long long topk_target_ranks_offset,
  ...
  if (token < local_num_tokens) {
    unsigned long long target_base = (local_workspace_base + topk_target_ranks_offset) / 4 +
                                     (unsigned long long)token * (unsigned long long)TOP_K;
    unsigned long long send_base = (local_workspace_base + topk_send_indices_offset) / 4 +
                                   (unsigned long long)token * (unsigned long long)TOP_K;
#pragma unroll 1
...
#pragma unroll
      for (int route = 0; route < TOP_K; ++route) {
        ...
        if (target_rank >= 0 && send_index >= 0) {
          ...
          unsigned long long byte_index = (unsigned long long)target_rank * workspace_stride_bytes +
      ...
      unsigned long long output_item =
      ...
      if (output_dtype_code == 2) {
        uint8_t* output_fp8 = reinterpret_cast<uint8_t*>(output);
        {
          ...
          *(reinterpret_cast<unsigned char*>(output_fp8 + output_item) + (0)) =
              (unsigned char)(_fp8_pair & 0xFF);
        }
      }
...
#define DEFINE_COMBINE_TOP_K(TOP_K)                                                               \
  extern "C" __global__ __launch_bounds__(256) void                                               \
      kernel_flashinfer_mnnvl_moe_alltoall_combine_top_k_##TOP_K(                                 \
          ...
          int payload_element_bytes, int payload_dtype_code, int output_dtype_code, int ep_rank,  \
          bool use_low_precision, bool enable_pdl) {                                              \
    combine_top_k<TOP_K>(workspace, output, workspace_stride_bytes, topk_target_ranks_offset,     \
                         topk_send_indices_offset, combine_payload_offset, max_tokens_per_rank,   \
                         local_num_tokens, elements_per_token, payload_element_bytes,             \
                         payload_dtype_code, output_dtype_code, ep_rank, use_low_precision,       \
                         enable_pdl);                                                             \
  }
