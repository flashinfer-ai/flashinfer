...
__global__ __launch_bounds__(256) void kernel_flashinfer_mnnvl_moe_alltoall_stage_combine(
    ...
    unsigned long long destination_offset, unsigned long long payload_bytes, int ep_rank,
    bool enable_pdl) {
  ...
  if (bid == 0) {
    if (tid == 0) {
      ...
      unsigned long long flag_index =
          ((unsigned long long)ep_rank * workspace_stride_bytes + flag_val_offset) / 4;
...
#pragma unroll 1
  for (unsigned long long byte_index = linear_thread; byte_index < payload_bytes;
       byte_index += thread_stride) {
