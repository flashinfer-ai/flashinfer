...
template <int VEC_SIZE_BYTES, int TOP_K, typename ThreadingPolicy, typename T,
...
__device__ void vectorized_combine_impl(void* output_buffer, void* sf_output, int row_idx,
  ...
  int const stride = ThreadingPolicy::stride() * VEC_SIZE_BYTES;
  int const local_token_idx = ThreadingPolicy::token_idx();

  for (int offset = ThreadingPolicy::offset() * VEC_SIZE_BYTES; offset < size_per_token;
       offset += stride) {
...
__device__ void vectorized_combine(void* output_buffer, void* sf_output, int row_idx, int row_size,
                                   ...
                                   CombineKernelPointers const& ptrs) {
  if constexpr (QuantMode != MoeA2ACombineQuantMode::NONE) {
    vectorized_combine_impl<16, TOP_K, ThreadingPolicy, T, QuantMode, SwizzleMode>(
...
#pragma unroll 1  // No unroll
  ...
  vectorized_combine<TOP_K, ThreadingPolicy, T, QuantMode, SwizzleMode>(
      ...
      SWITCH_TOP_K(params.top_k, TOP_K, {
        SWITCH_QUANT_MODE(TKernelType, params.quant_mode, QUANT_MODE, {
          SWITCH_SWIZZLE_MODE(params.swizzle_mode, SWIZZLE_MODE, {
            uint32_t grid_size = params.one_block_per_token ? grid_size_block : grid_size_warp;
            uint32_t block_size = kBlockSize;
            moeA2ACombineKernel<TKernelType, Policy, TOP_K, QUANT_MODE, SWIZZLE_MODE>
