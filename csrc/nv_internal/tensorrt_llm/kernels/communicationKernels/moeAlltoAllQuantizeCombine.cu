...
__global__ __launch_bounds__(32) void kernel_flashinfer_mnnvl_moe_alltoall_quantize_combine(
    float* __restrict__ accumulated, uint8_t* __restrict__ quantized_fp8,
    uint8_t* __restrict__ quantized_packed, uint8_t* __restrict__ scales_u8,
    uint8_t* __restrict__ scales_fp8, int elements_per_token, int payload_dtype_code,
    int quant_mode, int scale_layout, float output_scalar_scale, int blocks_per_row,
    int padded_scale_cols, bool enable_pdl) {
  ...
  if (quant_mode == 3) {
    ...
    actual_scale = block_max != 0.0f
                       ? reciprocal_approximate_ftz(fp8_scale *
                                                    reciprocal_approximate_ftz(output_scalar_scale))
                       : 0.0f;
  } else {
