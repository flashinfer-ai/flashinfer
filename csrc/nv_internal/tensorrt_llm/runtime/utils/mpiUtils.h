...
class MpiComm {
 public:
  ...
  static MpiComm const& session() { return mutableSession(); }

  //! \brief Returns the MPI local communicator.
  static MpiComm const& localSession() { return mutableLocalSession(); }
  ...
  std::unique_ptr<MpiRequest> sendAsync(void const* buffer, std::size_t size, MpiType dtype,
                                        int dest, MpiTag tag) const;
  ...
  void allgatherv(void const* sendbuf, int sendcount, MpiType sendtype, void* recvbuf,
                  std::vector<int> const& recvcounts, std::vector<int> const& displs,
                  MpiType recvtype) const;
...
void initialize(MpiThreadSupport threadMode = MpiThreadSupport::THREAD_MULTIPLE,
                bool forwardAbortToParent = false);

class MpiWaitThread {
 public:
  explicit MpiWaitThread(std::string name, std::function<void()> funcWait,
                         std::function<void()> funcSetup = nullptr);
  ~MpiWaitThread();
