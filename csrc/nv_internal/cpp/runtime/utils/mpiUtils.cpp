...
MpiComm initLocalSession() {
  ...
  MPI_Comm_split_type(COMM_SESSION, OMPI_COMM_TYPE_HOST, COMM_SESSION.getRank(), MPI_INFO_NULL,
                      &localComm);
...
std::vector<int> getWorldRanks(MpiComm const& comm) {
  ...
  MPICHECK(
      MPI_Group_translate_ranks(group, groupSize, ranks.data(), worldGroup, worldRanks.data()));
...
void initialize(MpiThreadSupport threadMode, bool forwardAbortToParent) {
  ...
  if (!initialized) {
    ...
    /*
     * We only catch SIGABRT and SIGSEGV because most, of not all errors in the worker will cause
     * one of these 2 signals. Signals like SIGINT and SIGTERM should be issued to the parent and
     * should terminate MPI workers correctly.
     */
...
std::unique_ptr<MpiRequest> MpiComm::bcastAsync(void* buffer, size_t size, MpiType dtype,
                                                int root) const {
...
std::unique_ptr<MpiRequest> MpiComm::sendAsync(void const* buffer, size_t size, MpiType dtype,
                                               int dest, MpiTag tag) const {
  TLLM_LOG_DEBUG("start MPI_Isend with dest %d, tag %d, size %d", dest, static_cast<int>(tag),
                 size);
  std::unique_ptr<MpiRequest> r = std::make_unique<MpiRequest>();
#if ENABLE_MULTI_DEVICE
  invokeChunked(MPI_Isend, buffer, size, getMpiDtype(dtype), dest, static_cast<int>(tag), mComm,
                &r->mRequest);
#else
  TLLM_THROW("Multi device support is disabled.");
#endif
...
// std::unique_ptr<MpiRequest> MpiComm::sendAsync(runtime::IBuffer const& buf, int dest, MpiTag tag)
// const
// {
...
MPI_Status MpiComm::recvRawTag(void* buffer, size_t size, MpiType dtype, int source,
                               int tag) const {
...
#if ENABLE_MULTI_DEVICE
  MPICHECK(MPI_Allgatherv(sendbuf, sendcount, getMpiDtype(sendtype), recvbuf, recvcounts.data(),
                          displs.data(), getMpiDtype(recvtype), mComm));
...
void MpiComm::refreshLocalSession() {
  ...
  MPICHECK(
      MPI_Group_incl(worldGroup, intersectionRanks.size(), intersectionRanks.data(), &localGroup));
  MPI_Comm localComm = nullptr;
  MPICHECK(
      MPI_Comm_create_group(MPI_COMM_WORLD, localGroup, intersectionRanks.front(), &localComm));
...
MpiWaitThread::MpiWaitThread(std::string name, std::function<void()> funcWait,
                             std::function<void()> funcSetup)
    : mName{name.c_str()}, mFuncWait{funcWait}, mFuncSetup{funcSetup} {
