...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_vsa_sm90_16f7f6889302cb63b4c8'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_cake_vsa_sm90_16f7f6889302cb63b4c8(const __grid_constant__ CUtensorMap Q, const __grid_constant__ CUtensorMap K, const __grid_constant__ CUtensorMap Vt, __nv_bfloat16* __restrict__ O, const CakeParamArray<int16_t, 1750> plan, int seqlen_q, int seqlen_k, float scale_log2);


namespace cake_host_shim_40f3a5136bd9a081 {
...
void Run(TensorView arg_Q, TensorView arg_K, TensorView arg_Vt, TensorView arg_O, TensorView arg_plan, int64_t arg_seqlen_q, int64_t arg_seqlen_k, double arg_scale_log2, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_vsa_sm90_16f7f6889302cb63b4c8),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_vsa_sm90_16f7f6889302cb63b4c8 failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_vsa_sm90_16f7f6889302cb63b4c8), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_vsa_sm90_16f7f6889302cb63b4c8 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_40f3a5136bd9a081

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_40f3a5136bd9a081::Run);
