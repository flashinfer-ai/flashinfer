...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_vsa_sm90_0144b5f123596d98e5f5'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
void Run(TensorView arg_Q, TensorView arg_K, TensorView arg_Vt, TensorView arg_O, TensorView arg_meta, int64_t arg_tile_stride, int64_t arg_seqlen_q, int64_t arg_seqlen_k, double arg_scale_log2, TensorView arg_dbg, TensorView arg_tl, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  CheckCudaTensor(arg_meta, "meta");
  CheckDtype(arg_meta, "meta", 0, 32, 1);
  CheckContiguous(arg_meta, "meta");
  TVM_FFI_CHECK(arg_tile_stride >= -2147483648LL && arg_tile_stride <= 2147483647LL, ValueError)
      << "scalar 'tile_stride' value " << arg_tile_stride
  ...
  CheckSameCudaDevice(arg_meta, arg_Q, "meta", "Q");
  CheckSameCudaDevice(arg_dbg, arg_Q, "dbg", "Q");
  CheckSameCudaDevice(arg_tl, arg_Q, "tl", "Q");
  ...
  int* p_meta = static_cast<int*>(arg_meta.data_ptr());
  int32_t v_tile_stride = (int32_t)arg_tile_stride;
  int32_t v_seqlen_q = (int32_t)arg_seqlen_q;
  int32_t v_seqlen_k = (int32_t)arg_seqlen_k;
  float v_scale_log2 = (float)arg_scale_log2;
  int* p_dbg = static_cast<int*>(arg_dbg.data_ptr());
  unsigned long long* p_tl = static_cast<unsigned long long*>(arg_tl.data_ptr());
  void* kargs[] = {&p_Q, &p_K, &p_Vt, &p_O, &p_meta, &v_tile_stride, &v_seqlen_q, &v_seqlen_k, &v_scale_log2, &p_dbg, &p_tl};
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_vsa_sm90_0144b5f123596d98e5f5),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_vsa_sm90_0144b5f123596d98e5f5 failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_vsa_sm90_0144b5f123596d98e5f5), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_vsa_sm90_0144b5f123596d98e5f5 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_c0acdf59ff57bec1

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_c0acdf59ff57bec1::Run);
