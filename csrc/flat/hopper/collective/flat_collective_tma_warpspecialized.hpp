...
struct FlatMainloopTmaWarpSpecialized {
  ...
  static constexpr int NeedsScale = find_option_t<Tag::kNeedsScale, cute::true_type, Options>::value;
  static constexpr int NeedsDecay = find_option_t<Tag::kNeedsDecay, cute::true_type, Options>::value;
  ...
  //   O_intra,i = mma(select<1,0>(Vi), mma(Qi, Ki))             GMEM  V: Blk x Dv   RMEM QK: Blk x Blk
  //   O_inter,i = mma(KVi, Qi)                                  RMEM KV:  Dv x Dqk  GMEM  Q: Blk x Dqk
  //   KVi+1     = KVi + mma(select<1,0>(Vi), select<1,0>(Ki))   GMEM  V: Blk x Dv   GMEM  K: Dqk x Blk -> RMEM KV:  Dv x Dqk
  ...
  //   O_intra,i = mma(Vi, mma(Qi, Ki))                          SMEM  V:  Dv x Blk  RMEM QK: Blk x Blk
  //   O_inter,i = mma(KVi, Qi)                                  RMEM KV:  Dv x Dqk  SMEM  Q: Blk x Dqk
  //   KVi+1     = KVi + mma(Vi, select<1,0>(Ki))                SMEM  V:  Dv x Blk  SMEM  K: Dqk x Blk -> RMEM KV:  Dv x Dqk
  ...
  // further notice, we need to Swap AB so that QK is operand A. All MMA planning shall be much more natural than current ones.
  ...
  using SmemLayoutQK = decltype(tile_to_shape(GMMA::Layout_K_INTER_Atom<Element>{}, select<0, 1>(TileShapeQK{})));
  ...
  precompute_decay(float decay_factor, float* decay) {
    ...
    for (int base = 0; base < len; base += cutlass::NumThreadsPerWarp) {
      ...
      if (vidx == base) { prod = 1.0f; };                  // correct prod as d^0, d^1, ..., d^tid, ... for first iter
      if (base != 0) { prod *= scale * decay[base - 1]; }  // correct prod as d^(tid+1) for remaining iters
      if (vidx < len) {
        decay[vidx] = prod;
      }
    }
  ...
  template <
      typename CollectiveLoad, typename TMA, typename ProblemShape, typename LoadTileShape, typename WorkDesc,
      class MainloopPipeline, class PipelineState, class SharedStorage>
  CUTE_DEVICE void
  load(
      TMA const&           tma_load,
  ...
  compute(
    ...
    float decay_factor      = is_per_head_decay ? params.per_head_decay[work_desc.q_head_idx()] : params.decay;
    ...
    auto qk_thread_idx = IsQKCooperative ? thread_idx : (thread_idx % cutlass::NumThreadsPerWarpGroup);
    ...
    if constexpr (!NeedsDecay) { fill(tOrDecay, 1.0f); }
    ...
    auto kv_epi = [&](auto& tKVrKV, auto& tKVrKV_inc, auto is_first_block_, auto is_final_block_, auto B /*valid seqlen*/) {
      ...
      if constexpr (is_final_block || is_first_block) {
        if constexpr (NeedsDecay) { block_decay = storage.decay[B]; }
      }
      cute::transform(tKVrKV, tKVrKV_inc, tKVrKV, [&](auto carried_kv, auto inc_kv) { return block_decay * carried_kv + inc_kv; });
    };
    ...
    auto kv_store = [&]() {  // tKVrKV is carried over whole mainloop
      bool is_lead_kv_head = (q_head_idx % (problem_size.num_q_heads / problem_size.num_k_heads)) == 0;
    ...
    auto compute_loop_body = [&](int blk, auto is_first_block_, auto is_final_block_) {
      ...
      if (wg_compute_qk) {
        ...
        cutlass::arch::NamedBarrier::arrive(NumMmaThreads, FlatNamedBarriers::QKLaunchedAndDecayComputed);
        warpgroup_wait<0>();  // q@k batch finished
      } else {
        cutlass::arch::NamedBarrier::arrive_and_wait(NumMmaThreads, FlatNamedBarriers::QKLaunchedAndDecayComputed);
      }
      if (wg_compute_qk) { qk_mask(tQKrQK, is_final_block_, B); }
      if (wg_compute_qk) { qk_store(tQKrQK); }

      cutlass::arch::fence_view_async_shared();
      cutlass::arch::NamedBarrier::sync(NumMmaThreads, FlatNamedBarriers::AllMmaThreadsSync);  // qk -> smem
      ...
      copy(KV_S2R{}, tKVsV(_, _, _, v_smem_pipe_read.index()), tKVrV_cv);  // NOTE: tKVsV and tOrV have the same layout
      ...
      // copy(KV_S2R{}, tKVsV(_, _, _, v_smem_pipe_read.index()), tKVrV_cv);  // already loaded for O2
