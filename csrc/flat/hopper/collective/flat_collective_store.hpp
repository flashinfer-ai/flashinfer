...
template <
    typename TileShape_MNK_, typename ClusterShape,
    typename ElementO,
    typename ElementAccumulator,
    typename SmemElementO, typename StrideO,
    int Stages>
struct CollectiveStoreTma {
  static_assert(size_v<ClusterShape> == 1);
  using TileShape_MNK = TileShape_MNK_;
  using TileShape_MN  = decltype(select<0, 1>(TileShape_MNK{}));  // Collective work on TileShape_MN, it is also the OutputTile
  ...
  using CopyOpR2S   = decltype(cutlass::epilogue::collective::detail::sm90_get_smem_store_op_for_accumulator<
                               StrideO, ElementO>());
  using CopyAtomR2S = Copy_Atom<CopyOpR2S, SmemElementO>;

  using CopyOpS2G = SM90_TMA_STORE;

  using SharedStorage = cute::array_aligned<SmemElementO, cute::cosize_v<SmemLayoutO>, alignment_for_swizzle(SmemLayoutO{})>;
  ...
  struct Params {
    using TMA_O = decltype(make_tma_copy(
        CopyOpS2G{},
        make_tensor(
            make_gmem_ptr<ElementO>(nullptr),
  ...
  to_underlying_arguments(
    ...
    TMA    tma_store_o = make_tma_copy_C_sm90(
        CopyOpS2G{},
        tensor_o,
        take<0, 2>(SmemLayoutO{}),
        TileShape_MN{}
  ...
  CUTE_DEVICE void
  step(
      ProblemSize const& problem_size, WorkDesc const& work_desc,
      SrcDst const& src_dst, PipelineState& src_pipe, int dst_iter, int num_iters, uint32_t lane_predicate
  ) {
    ...
    if constexpr (kAcquireBarrier) { pipeline_.consumer_wait(src_pipe); }
    ...
    if (can_process(problem_size, work_desc, dst_iter, num_iters)) {
      ...
      if (lane_predicate == 1) { copy(tma_store_.with(tensormap), src(_, _, _, src_pipe.index()), dst(_, _, _, dst_iter)); }
    }

    if constexpr (kAcquireBarrier) { pipeline_.consumer_release(src_pipe); }
    ++src_pipe;
  }
