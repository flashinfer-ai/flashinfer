...
struct FlatMainloopTmaWarpSpecializedDeltaRule {
  ...
  using CollectiveMmaQK = typename cutlass::gemm::collective::CollectiveBuilder<
      ...
      TileShapeQK, ClusterShape, DummyStages,
      std::conditional_t<IsQKCooperative, cutlass::gemm::KernelTmaWarpSpecializedCooperative, cutlass::gemm::KernelTmaWarpSpecialized>>::CollectiveOp;

  using CollectiveMmaKV_G2S = typename cutlass::gemm::collective::CollectiveBuilder<
      cutlass::arch::Sm90, cutlass::arch::OpClassTensorOp,
      Element, decltype(select<1, 0, 2>(LayoutV{})), Alignment,  // direct TMA copy for GMEM -> SMEM
      Element, decltype(select<1, 0, 2>(LayoutK{})), Alignment,
  ...
  using SmemLayoutK_DS = decltype(unstage_smem_layout(typename CollectiveMmaKV_G2S::SmemLayoutB{}, Int<StagesK::value>{}));
  using SmemLayoutV_DS = decltype(unstage_smem_layout(typename CollectiveMmaKV_G2S::SmemLayoutA{}, Int<StagesV::value>{}));
  ...
  using CollectiveMmaKV = typename cutlass::gemm::collective::CollectiveBuilder<
      cutlass::arch::Sm90, cutlass::arch::OpClassTensorOp,
      Element, RefLayoutV, Alignment,  // needs a S2R transposition for MMA
      Element, decltype(select<1, 0, 2>(LayoutK{})), Alignment,
      ElementAccumulatorKV,
      TileShapeKV, ClusterShape, DummyStages,
      cutlass::gemm::KernelTmaWarpSpecializedCooperative>::CollectiveOp;
  ...
  using CollectiveMmaO1 = typename cutlass::gemm::collective::CollectiveBuilder<
      cutlass::arch::Sm90, cutlass::arch::OpClassTensorOp,
      Element, RefLayoutKV, Alignment,
      Element, LayoutQ, Alignment,
      ElementAccumulatorO,
      TileShapeO1, ClusterShape, DummyStages,
      cutlass::gemm::KernelTmaWarpSpecializedCooperative>::CollectiveOp;
  ...
  using CollectiveMmaO2 = typename cutlass::gemm::collective::CollectiveBuilder<
      cutlass::arch::Sm90, cutlass::arch::OpClassTensorOp,
      Element, RefLayoutV, Alignment,
      Element, DesiredLayoutQK, Alignment,
      ElementAccumulatorO,
      TileShapeO2, ClusterShape, DummyStages,
      cutlass::gemm::KernelTmaWarpSpecializedCooperative>::CollectiveOp;
  ...
  using CollectiveStoreO = CollectiveStoreTma<
      TileShapeO1, ClusterShape,
      ElementO, ElementAccumulatorO,
      /*Seme*/ ElementO, decltype(select<1, 0, 2>(LayoutO{})),
      StagesO::value>;
  ...
  using CollectiveMmaSK      = typename cutlass::gemm::collective::CollectiveBuilder<  // basically the same as O1
      ...
      TileShapeSK, ClusterShape, DummyStages,
      cutlass::gemm::KernelTmaWarpSpecializedCooperative>::CollectiveOp;
  ...
  using CollectiveMmaNewV = typename cutlass::gemm::collective::CollectiveBuilder<
      cutlass::arch::Sm90, cutlass::arch::OpClassTensorOp,
      Element, RefLayoutSK, Alignment,
      Element, DesiredLayoutKK, Alignment,
      ElementAccumulatorKV,
      TileShapeNewV, ClusterShape, DummyStages,
      cutlass::gemm::KernelTmaWarpSpecializedCooperative>::CollectiveOp;
  ...
  using SmemLayoutAlpha = decltype(make_layout(make_shape(BlkSeqQ, Int<3>{}, Int<StagesAlphaBeta::value>{})));
  ...
  using MainloopAlphaPipeline = std::conditional_t<NeedsAlpha, cutlass::PipelineAsync<StagesAlphaBeta::value>, Unused>;
  using MainloopBetaPipeline  = std::conditional_t<NeedsBeta, cutlass::PipelineAsync<StagesAlphaBeta::value>, Unused>;
  ...
  using LoadAlpha = CollectiveLoadVector<LoadKindVector::kAlpha, MainloopAlphaPipeline, float, GmemLayoutAlphaBeta, float, SmemLayoutAlpha, AlphaProcessor>;
  using LoadBeta  = CollectiveLoadVector<LoadKindVector::kBeta, MainloopBetaPipeline, float, GmemLayoutAlphaBeta, float, SmemLayoutBeta, BetaProcessor>;
  ...
  struct Params {
    ...
    float const* alpha_ptr; GmemLayoutAlphaBeta alpha_layout;
    float const* beta_ptr;  GmemLayoutAlphaBeta beta_layout;
  };
  ...
  to_underlying_arguments(ProblemShape const& problem_size, Arguments const& args, void* workspace) {
    ...
    auto params_o = CollectiveStoreO::to_underlying_arguments(
        ...
        typename CollectiveStoreO::Arguments{args.ptr_O, select<1, 0, 2>(args.dO)},
        workspace
    );
  ...
  CUTE_DEVICE void
  load_qkv(
      Params const& params, ProblemShape const&  problem_size, LoadTileShape const& load_tile_shape, WorkDesc const& work_desc,
  ...
  CUTE_DEVICE void
  load_beta(
      Params const& params, ProblemShape const& problem_size, TileShape const& tile_shape, WorkDesc const& work_desc,
    ...
    // auto collective_load = LoadBeta{params.beta_ptr, params.beta_layout, /*oob_value=*/1.0f, pipeline, storage.smem_beta};
    auto collective_load = LoadBeta{params.beta_ptr, params.beta_layout, /*oob_value=*/0.0f, pipeline, storage.smem_beta};
  ...
  load_alpha(
    ...
    auto collective_load = LoadAlpha{params.alpha_ptr, params.alpha_layout, /*oob_value=*/1.0f, pipeline, storage.smem_alpha};
    auto src_dst         = collective_load.partition_SD(problem_size, tile_shape, work_desc);
    ...
    collective_load.step</*IsTail=*/true>(src_dst, num_blocks - 1, smem_pipe_write, num_blocks, processor);
  }


  template <
      typename ProblemSize, typename StoreTileShape, typename WorkDesc, typename PipelineState>
  CUTE_DEVICE void
  store(
  ...
  compute(
    ...
    auto tSKrAlpha = sk_thr_mma.partition_C(Alpha.compose(layout_SKAlpha))(_, _, _, AlphaCumProdIdx{}, _);  // (frag, iter_D, iter_Blk_Q, pipe)
    ...
    auto tOrAlphaScale = o1_thr_mma.partition_C(Alpha.compose(layout_OAlpha))(_, _, _, AlphaCumProdScaleIdx{}, _);  // (frag, iter_D, iter_Blk_Q, pipe)
    ...
    auto sk_epi = [&](auto& tSKrSK, auto const& alpha_smem_pipe_read) INLINE_LAMBDA {
      if constexpr (NeedsAlpha) {
        transform(tSKrSK, tSKrAlpha(_, _, _, alpha_smem_pipe_read.index()), tSKrSK, [&](auto sk, auto coeff) {
          return sk * coeff;
        });
      }
    };
    ...
    auto kv_decay_v = [&](auto& tKVrV, auto const& alpha_smem_pipe_read, auto is_final_block_, auto B) INLINE_LAMBDA {
    ...
    auto compute_loop_body = [&](int blk, auto is_first_block_, auto is_final_block_) INLINE_LAMBDA {
      ...
      ++v_smem_pipe_read;  // NOTE: if we delay this increment after consumer_release, race condition happens, why?
    ...
    if (num_blocks != 1) {
      compute_loop_body(num_blocks - 1, /*is_first_block_=*/cute::false_type{}, /*is_final_block_=*/cute::true_type{});
    }
    kv_store();
  }
  ...
  compute_aux(
      ...
      MainloopQPipeline& q_pipeline, QPipelineState& q_smem_pipe_read,
      MainloopKPipeline& k_pipeline, KPipelineState& k_smem_pipe_read,
      MainloopQKPipeline& qk_pipeline, QKPipelineState& qk_smem_pipe_write,
      MainloopKKPipeline& kk_pipeline, KKPipelineState& kk_smem_pipe_write,
      MainloopAlphaPipeline& alpha_pipeline, AlphaPipelineState& alpha_smem_pipe_read,
      MainloopBetaPipeline& beta_pipeline, BetaPipelineState& beta_smem_pipe_read,
      SharedStorage& storage
  ) {
    ...
    auto qk_and_kk_epi = [&](auto& tQKrQK, auto& tKKrKK, auto const& alpha_smem_pipe_read, auto const& beta_smem_pipe_read, auto is_final_block_, auto B /*valid seqlen*/) {
      if constexpr (NeedsAlpha) {
        Tensor Alpha_cumsum_log = Alpha(_, AlphaCumSumLogIdx{}, alpha_smem_pipe_read.index());
      ...
      for_each(make_int_sequence<size(tKKcMkk)>{}, [&](auto i){
          ...
          tKKrKK(i) = pred ? tKKrKK(i) : 0.0f;  // diagonal is garbage filled, will process during inversion
          if constexpr (is_final_block) {
            bool pred = s < B || t < B ;
            tQKrQK(i) = pred ? tQKrQK(i) : 0.0f;
    ...
    auto kk_store_and_inv = [&](auto tKKrKK, auto const& kk_smem_pipe_write) INLINE_LAMBDA {
      ...
      cutlass::arch::NamedBarrier::arrive_and_wait(cutlass::NumThreadsPerWarpGroup, DeltaRuleNamedBarriers::AuxMath);
    ...
    auto compute_aux_loop_body = [&](int blk, auto is_final_block_) INLINE_LAMBDA {
      ...
      warpgroup_arrive();
      gemm_zero_acc(kk_tiled_mma, tKKrA(_, _, _, k_smem_pipe_read.index()), tKKrB(_, _, _, k_smem_pipe_read.index()), tKKrKK);
      ...
      warpgroup_arrive();
      gemm_zero_acc(qk_tiled_mma, tQKrQ(_, _, _, q_smem_pipe_read.index()), tQKrK(_, _, _, k_smem_pipe_read.index()), tQKrQK);
      ...
      if constexpr (NeedsAlpha) { alpha_pipeline.consumer_wait(alpha_smem_pipe_read); }
      if constexpr (NeedsBeta) { beta_pipeline.consumer_wait(beta_smem_pipe_read); }
      cutlass::arch::fence_view_async_shared();
