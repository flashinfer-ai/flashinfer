...
struct FlatKernelTmaWarpSpecialized {
  ...
  static constexpr uint32_t LoadRegisterRequirement = get<0>(
      get_register_requirements(MaxThreadsPerBlock, MinBlocksPerMultiprocessor, NumMmaWarpGroups));
  static constexpr uint32_t MmaRegisterRequirement = get<1>(
      get_register_requirements(MaxThreadsPerBlock, MinBlocksPerMultiprocessor, NumMmaWarpGroups));
    ...
    if (warp_group_role == WarpGroupRole::LdSt) {
      ...
      if (ldst_warp_role == LdStWarpRole::LoadQ) {
        ...
        for (; work_desc.is_valid(params.scheduler);
          ...
          collective_mainloop.template load<CollectiveMainloop::LoadQ>(
              params.mainloop.tma_load_q, params.problem_size, tile_shape, work_desc, q_pipeline,
              q_smem_pipe_write, storage.tensors.mainloop.smem_q);
        }
        ...
        for (; work_desc.is_valid(params.scheduler);
          ...
          collective_mainloop.template load<CollectiveMainloop::LoadK>(
              params.mainloop.tma_load_k, params.problem_size, tile_shape, work_desc, k_pipeline,
              k_smem_pipe_write, storage.tensors.mainloop.smem_k);
        }
        ...
        for (; work_desc.is_valid(params.scheduler);
          ...
          collective_mainloop.template load<CollectiveMainloop::LoadV>(
              params.mainloop.tma_load_v, params.problem_size, tile_shape, work_desc, v_pipeline,
              v_smem_pipe_write, storage.tensors.mainloop.smem_v);
        }
      ...
      for (; work_desc.is_valid(params.scheduler);
        ...
        collective_mainloop.compute(params.mainloop, params.problem_size, work_desc, q_pipeline,
                                    ...
                                    v_smem_pipe_read, o_pipeline, o_smem_pipe_write,
