...
get_register_requirements(
  ...
  uint32_t mma_registers   = round_down((total_registers - load_registers - aux_registers) / num_state_mma_warp_groups, reg_alloc_granularity);
...
struct FlatKernelTmaWarpSpecializedDeltaRule {
  ...
  operator()(const Params& params, char* smem) {
    ...
    if (warp_group_role == WarpGroupRole::LdSt && ldst_warp_role == LdStWarpRole::LoadAlpha) {
      if constexpr (NeedsAlpha) { alpha_pipeline_params.role = MainloopAlphaPipeline::ThreadCategory::Producer; }
    }
    ...
    MainloopQKPipeline qk_pipeline(storage.qk_pipeline_storage, qk_pipeline_params, /*InitBarriers=*/cute::true_type{});
    MainloopKKPipeline kk_pipeline(storage.kk_pipeline_storage, kk_pipeline_params, /*InitBarriers=*/cute::true_type{});

    MainloopAlphaPipeline alpha_pipeline(storage.alpha_pipeline_storage, alpha_pipeline_params, /*InitBarriers=*/cute::true_type{});
    MainloopBetaPipeline  beta_pipeline(storage.beta_pipeline_storage, beta_pipeline_params, /*InitBarriers=*/cute::true_type{});
    ...
    if constexpr (NeedsAlpha) { alpha_smem_pipe_write = cutlass::make_producer_start_state<MainloopAlphaPipeline>(); }
    BetaPipelineState beta_smem_pipe_read;
    BetaPipelineState beta_smem_pipe_write;
    if constexpr (NeedsBeta) { beta_smem_pipe_write = cutlass::make_producer_start_state<MainloopBetaPipeline>(); }
    ...
    if (warp_group_role == WarpGroupRole::LdSt) {
      ...
      if (ldst_warp_role == LdStWarpRole::LoadQKV) {
        ...
        for (; work_desc.is_valid(params.scheduler); work_desc = scheduler.get_next_work(params.scheduler, params.problem_size)) {
          DPRINTF0_WG("LsSt working on LoadQ/K/V, seq_idx:%d, q/k/v_head_idx:(%d,%d,%d), seq_len:%lld)\n",
                      work_desc.seq_idx, work_desc.q_head_idx(), work_desc.k_head_idx(), work_desc.v_head_idx(), work_desc.seq_len);
          ...
          collective_mainloop.load_qkv(
              ...
              q_pipeline, q_smem_pipe_write,
              k_pipeline, k_smem_pipe_write,
              v_pipeline, v_smem_pipe_write,
        ...
        if constexpr (NeedsAlpha) {
          ...
          for (; work_desc.is_valid(params.scheduler); work_desc = scheduler.get_next_work(params.scheduler, params.problem_size)) {
            DPRINTF0_WG("LsSt working on LoadAlpha, seq_idx:%d, sab_head_idx:%d, seq_len:%lld)\n",
                        work_desc.seq_idx, work_desc.o_head_idx(), work_desc.seq_len);
            auto tile_shape = typename CollectiveMainloop::TileShape{};
            collective_mainloop.load_alpha(
                params.mainloop, params.problem_size, tile_shape, work_desc,
                alpha_pipeline, alpha_smem_pipe_write,
                storage.tensors.mainloop
            );
          }
        }
      ...
      for (; work_desc.is_valid(params.scheduler); work_desc = scheduler.get_next_work(params.scheduler, params.problem_size)) {
        ...
        collective_mainloop.compute(
            ...
            qk_pipeline, qk_smem_pipe_read,
            kk_pipeline, kk_smem_pipe_read,
      ...
      for (; work_desc.is_valid(params.scheduler); work_desc = scheduler.get_next_work(params.scheduler, params.problem_size)) {
        DPRINTF0_WG("Compute[aux]: seq_idx:%d, qk/v/o_head_idx:(%d,%d,%d,%d), seq_len:%lld)\n",
                    work_desc.seq_idx, work_desc.q_head_idx(), work_desc.k_head_idx(), work_desc.v_head_idx(), work_desc.o_head_idx(), work_desc.seq_len);
        collective_mainloop.compute_aux(
            params.mainloop, params.problem_size, work_desc,
            q_pipeline, q_smem_pipe_read,
            k_pipeline, k_smem_pipe_read,
