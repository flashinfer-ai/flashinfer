...
struct OrderedNamedBarriers {
  ...
  OrderedNamedBarriers()
      : mapping_{NBId_t(WGIdToNBIdMapping)...} {}
  ...
  notify_next_blocked(int wg_idx) {  // wg_idx in participants
    ...
    // 0:(2,3), wg0 has not reached on second ordered_or_wait() or (0,3) wg0 wait on second ordered_or_wait() call, unblocked
    // 1:(1,3), wg1 has not reached on second ordered_or_wait() or (2,3) wg1 wait on second ordered_or_wait() call, still block
    // 2:(0,3), unblock wg0 ordered_or_wait() and reset
    //
  }
