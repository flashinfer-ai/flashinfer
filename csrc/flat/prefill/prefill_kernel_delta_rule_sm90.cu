...
#define LAUNCH(is_gva, needs_beta, needs_alpha, init_state)                                    \
  launch_delta_rule_prefill_kernel_gbai<is_gva, needs_beta, needs_alpha, init_state, ArchTag>( \
      stream, output, output_state, q, k, v, input_state, alpha, beta, cu_seqlens, workspace_buffer, num_seqs,   \
...
template void launch_delta_rule_prefill_kernel<cutlass::arch::Sm90, half, half, float>(
    ...
    float const* input_state, float const* alpha, float const* beta, int64_t const* cu_seqlens, uint8_t* workspace_buffer,
...
launch_delta_rule_prefill_kernel<cutlass::arch::Sm90, nv_bfloat16, nv_bfloat16, float>(
    ...
    float const* beta, int64_t const* cu_seqlens, uint8_t* workspace_buffer, int32_t num_seqs, int32_t num_q_heads,
