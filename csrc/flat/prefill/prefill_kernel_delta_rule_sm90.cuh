...
void launch_delta_rule_prefill_kernel_gbai(
  ...
  if constexpr (HopperSupported) {
    ...
    using Options = decltype([&]() {
      ...
      constexpr auto options_1 = add_option(Option<Tag::kIsDeltaRule, cute::true_type>{}, options_0);
      constexpr auto options_2 = add_option(Option<Tag::kIsGVA, std::conditional_t<IsGVA, cute::true_type, cute::false_type>>{}, options_1);
      constexpr auto options_3 = add_option(Option<Tag::kNeedsBeta, std::conditional_t<NeedsBeta, cute::true_type, cute::false_type>>{}, options_2);
      constexpr auto options_4 = add_option(Option<Tag::kNeedsAlpha, std::conditional_t<NeedsAlpha, cute::true_type, cute::false_type>>{}, options_3);
      constexpr auto options_5 = add_option(Option<Tag::kInitStateFromInput, std::conditional_t<InitStateFromInput, cute::true_type, cute::false_type>>{}, options_4);
      return options_5;
    }());
    ...
    using Operation = cutlass::device::Universal<
        typename flat::kernel::FlatBuilderDeltaRule<
            T, float, float,
            TileShape,
