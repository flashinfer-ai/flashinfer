...
struct LinearAttentionPrefillPipeline {
  ...
  attention(
      ...
      TState const* __restrict__ input_state,  // [num_seqs, Hkv, dv, dk], aka, KV initial value, optional
    ...
    auto gKV = make_tensor(
        ...
        make_layout(make_shape(Int<HeadSize>{}, Int<HeadSize>{}, per_head_decay ? num_qo_heads : num_kv_heads, num_seqs))
    ...
    auto tQKrQ_thr_copy = make_tiled_copy_A(typename QK::S2R{}, qk_tiled_mma).get_thread_slice(threadIdx.x);
    auto tQKrK_thr_copy = make_tiled_copy_B(typename QK::S2R{}, qk_tiled_mma).get_thread_slice(threadIdx.x);
    ...
    auto tOrA_thr_copy = make_tiled_copy_A(typename O::S2R{}, o_tiled_mma).get_thread_slice(threadIdx.x);
    auto tOrB_thr_copy = make_tiled_copy_B(typename O::S2R{}, o_tiled_mma).get_thread_slice(threadIdx.x);
    auto tOrV_thr_copy = make_tiled_copy_B(typename O::S2R_V{}, o_tiled_mma).get_thread_slice(threadIdx.x);  // V needs transpose
    ...
    for_each(make_int_sequence<NumPipe>{}, [&](auto pipe) {
      ...
      if (CUDA_UNIFORM_LIKELY(blk < num_blocks - 1 || seq_len % BlockSize == 0)) {
        ...
        load_qkv(
            tQgQ(_, _, _, blk), tQsQ(_, _, _, pipe),
            tKgK(_, _, _, blk), tKsK(_, _, _, pipe),
    ...
    for (; blk < num_blocks - NumPipe; blk++) {
      ...
      __syncthreads();  // all the threads have consumed the target pipe state, we can issue the load now
      ...
      if (CUDA_UNIFORM_LIKELY(blk_to_load < num_blocks - 1 || seq_len % BlockSize == 0)) {
        PIPE_DEBUG_PRINTF("[%d,%d,%d]|| preloading from blk=%d to pipe=%d\n", seq_idx, qo_head_idx, kv_head_idx, blk_to_load, pipe);
        load_qkv(
            tQgQ(_, _, _, blk_to_load), tQsQ(_, _, _, pipe),
            tKgK(_, _, _, blk_to_load), tKsK(_, _, _, pipe),
            tVgV(_, _, _, blk_to_load), tVsV(_, _, _, pipe)
        );
      } else {
        PIPE_DEBUG_PRINTF("[%d,%d,%d]|| preloading from blk=%d to pipe=%d (tail block)\n", seq_idx, qo_head_idx, kv_head_idx, blk_to_load, pipe);
        load_qkv_tail(
            ...
            tKgK(_, _, _, blk_to_load), tKsK(_, _, _, pipe),
            tVgV(_, _, _, blk_to_load), tVsV(_, _, _, pipe),
            tQcQ, remaining_seq_len(blk_to_load)
        );
      }
    }
    ...
    for_each(make_int_rsequence<NumPipe>{}, [&](auto RemainingPipe) {
      ...
      if (blk_idx < 0) { return; }  // means continue here, as we are in a lambda of the for_each
...
public:
  ...
  struct Ldg {
    ...
    using CopyAtom = Copy_Atom<
        ...
        TQKV>;
    using CopyAtomZ = Copy_Atom<
        SM80_CP_ASYNC_CACHEALWAYS_ZFILL<cute::array_aligned<TQKV, QKV_ValD, QKV_ValD * sizeof(TQKV)>>,
        TQKV>;
  ...
  struct Smem {
    using Swizzle_8x8_4x1 = decltype(composition(
        Swizzle<3, 3, 3>{},
        Layout<Shape<Shape<_8, _4>, _8>, Stride<Stride<_1, _64>, _8>>{}
    ));

    using QLayoutAtom = Swizzle_8x8_4x1;
    using QLayout     = decltype(tile_to_shape(QLayoutAtom{}, Shape<Int<HeadSize>, Int<BlockSize>, Int<NumPipe>>{}));

    using KLayoutAtom = Swizzle_8x8_4x1;
    using KLayout     = decltype(tile_to_shape(KLayoutAtom{}, Shape<Int<HeadSize>, Int<BlockSize>, Int<NumPipe>>{}));

    using VLayoutAtom = Swizzle_8x8_4x1;
    using VLayout     = decltype(tile_to_shape(VLayoutAtom{}, Shape<Int<HeadSize>, Int<BlockSize>, Int<NumPipe>>{}));
  ...
  /////////////////////////////////////////////////////////////////////////////
  template <
      typename QSrc, typename QDst,
      typename KSrc, typename KDst,
      typename VSrc, typename VDst>
  __forceinline__ __device__ static void
  load_qkv(
      QSrc&& q_src, QDst&& q_dst,
      KSrc&& k_src, KDst&& k_dst,
      VSrc&& v_src, VDst&& v_dst
  ) {
  ...
  struct QK {
    using TiledMMA = decltype(make_tiled_mma(
        MMA{},
        Layout<Shape<_2, _4>>{},  // 2x4 of atoms to fully cover the threads in a CTA
        Tile<_32, _32, _32>{}
    ));
...
__launch_bounds__(256, 1) __global__ void linear_attention_prefill_kernel(
  ...
  Pipeline::attention(
      output, output_state, q, k, v, input_state, cu_seqlens,
      num_seqs, num_qo_heads, num_kv_heads, scale, decay, per_head_decay, decay_exponent_offset
  );
}
