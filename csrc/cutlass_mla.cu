...
void CutlassMLAPagedAttention(at::Tensor workspace, at::Tensor out,
  ...
  DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(q_nope_pe.scalar_type(), c_type, [&] {
    ...
    auto status = runMla<cutlass_t>(
        workspace.data_ptr(), out.data_ptr(), lse.data_ptr(),
        q_nope_pe.data_ptr(), ckv_kpe_cache.data_ptr(), kv_lens.data_ptr(),
        page_table.data_ptr(), batches, page_count_per_seq, page_count_total,
        page_size, device_index, stream);
    TORCH_CHECK(status == cudaSuccess,
                "Failed to run CutlassMLAPagedAttention: ",
