...
kernel_dsv4_e256_route_pack(int* __restrict__ route_experts, int* __restrict__ route_map, int* __restrict__ tile_expert, int* __restrict__ tile_mn_limit, int* __restrict__ route_slots, int* __restrict__ num_non_exiting_ctas, int* __restrict__ fc1_work_counter, int* __restrict__ fc2_work_counter, int route_count, int top_k, int local_expert_offset, int num_experts, int fc1_initial_work, int fc2_initial_work)
{
    ...
    for (int route = tid; route < route_count; route += THREADS) {
        int expert_0 = route_experts[route] - local_expert_offset;
        if (expert_0 >= 0 && expert_0 < num_experts) {
            atomicAdd(&expert_counts[expert_0], 1);
        }
    }
    __syncthreads();
    ...
    int _shfl_up_1 = __shfl_up_sync(0xFFFFFFFF, first_inclusive, 2, 32);
    int first_peer_0 = _shfl_up_1;
    if (lane >= 2) {
        first_inclusive = first_inclusive + first_peer_0;
    }
    int _shfl_up_2 = __shfl_up_sync(0xFFFFFFFF, first_inclusive, 4, 32);
    int first_peer_1 = _shfl_up_2;
    if (lane >= 4) {
        first_inclusive = first_inclusive + first_peer_1;
    }
    int _shfl_up_3 = __shfl_up_sync(0xFFFFFFFF, first_inclusive, 8, 32);
    int first_peer_2 = _shfl_up_3;
    if (lane >= 8) {
        first_inclusive = first_inclusive + first_peer_2;
    }
    int _shfl_up_4 = __shfl_up_sync(0xFFFFFFFF, first_inclusive, 16, 32);
    ...
    for (int route_1 = tid; route_1 < route_count; route_1 += THREADS) {
        int expert_0_1 = route_experts[route_1] - local_expert_offset;
        if (expert_0_1 >= 0 && expert_0_1 < num_experts) {
            int _atomic_old_0 = atomicAdd(&expert_scatter_offsets[expert_0_1], 1);
            int local_row = _atomic_old_0;
            int tile = expert_tile_offsets[expert_0_1] + local_row / TILE_N;
            int row = local_row % TILE_N;
            int slot = tile * TILE_N + row;
            route_map[slot] = route_1 / top_k;
            route_slots[route_1] = slot;
            if (row == 0) {
                int remaining = expert_counts[expert_0_1] - local_row;
                int valid_rows = TILE_N;
                if (remaining < TILE_N) {
                    valid_rows = remaining;
                }
                tile_expert[tile] = expert_0_1;
                tile_mn_limit[tile] = tile * TILE_N + valid_rows;
                {
                    for (int padding_row = valid_rows; padding_row < TILE_N; padding_row++) {
                        route_map[tile * TILE_N + padding_row] = 0;
                    }
                }
            }
        }
