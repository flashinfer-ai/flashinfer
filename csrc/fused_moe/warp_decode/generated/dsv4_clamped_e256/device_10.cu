...
#define SMEM_MASKS_STAGE_BYTES 112
#define SMEM_MASKS_STRIDE 112
...
kernel_dsv4_sorted_short_route_pack(int* __restrict__ route_experts, int* __restrict__ route_map, int* __restrict__ tile_expert, int* __restrict__ tile_mn_limit, int* __restrict__ route_slots, int* __restrict__ num_non_exiting_ctas, int* __restrict__ fc1_work_counter, int* __restrict__ fc2_work_counter, int route_count, int top_k, int local_expert_offset, int num_experts, int fc1_initial_work, int fc2_initial_work)
{
    ...
    int expert = -1;
    if (tid < route_count) {
        expert = route_experts[tid] - local_expert_offset;
    }
    int valid = 0;
    if (tid < route_count && expert >= 0 && expert < num_experts) {
        valid = 1;
    }
    unsigned int _vote_0 = __ballot_sync(0xFFFFFFFF, (expert & 1) != 0);
    unsigned int bit_mask = _vote_0;
    if (lane == 0) {
        masks[warp * 10] = bit_mask;
    }
    unsigned int _vote_1 = __ballot_sync(0xFFFFFFFF, (expert >> 1 & 1) != 0);
    unsigned int bit_mask_0 = _vote_1;
    if (lane == 0) {
        masks[warp * 10 + 1] = bit_mask_0;
    }
    unsigned int _vote_2 = __ballot_sync(0xFFFFFFFF, (expert >> 2 & 1) != 0);
    unsigned int bit_mask_1 = _vote_2;
    if (lane == 0) {
        masks[warp * 10 + 2] = bit_mask_1;
    }
    unsigned int _vote_3 = __ballot_sync(0xFFFFFFFF, (expert >> 3 & 1) != 0);
    unsigned int bit_mask_2 = _vote_3;
    if (lane == 0) {
        masks[warp * 10 + 3] = bit_mask_2;
    }
    unsigned int _vote_4 = __ballot_sync(0xFFFFFFFF, (expert >> 4 & 1) != 0);
    unsigned int bit_mask_3 = _vote_4;
    if (lane == 0) {
        masks[warp * 10 + 4] = bit_mask_3;
    }
    unsigned int _vote_5 = __ballot_sync(0xFFFFFFFF, (expert >> 5 & 1) != 0);
    unsigned int bit_mask_4 = _vote_5;
    if (lane == 0) {
        masks[warp * 10 + 5] = bit_mask_4;
    }
    unsigned int _vote_6 = __ballot_sync(0xFFFFFFFF, (expert >> 6 & 1) != 0);
    unsigned int bit_mask_5 = _vote_6;
    if (lane == 0) {
        masks[warp * 10 + 6] = bit_mask_5;
    }
    unsigned int _vote_7 = __ballot_sync(0xFFFFFFFF, (expert >> 7 & 1) != 0);
    unsigned int bit_mask_6 = _vote_7;
    if (lane == 0) {
        masks[warp * 10 + 7] = bit_mask_6;
    }
    unsigned int _vote_8 = __ballot_sync(0xFFFFFFFF, (expert >> 8 & 1) != 0);
    unsigned int bit_mask_7 = _vote_8;
    if (lane == 0) {
        masks[warp * 10 + 8] = bit_mask_7;
    }
    unsigned int _vote_9 = __ballot_sync(0xFFFFFFFF, valid != 0);
    unsigned int valid_mask = _vote_9;
    if (lane == 0) {
        masks[warp * 10 + 9] = valid_mask;
    }
    __syncthreads();
    ...
    int _popc_0 = __popc(low);
    int low_count = _popc_0;
    int _popc_1 = __popc(high);
    int count = low_count + _popc_1;
    unsigned int earlier = ((unsigned int)1 << (unsigned int)lane) - (unsigned int)1;
    int _popc_2 = __popc(low & earlier);
    int rank = _popc_2;
    if (warp == 1) {
        int _popc_3 = __popc(high & earlier);
        rank = low_count + _popc_3;
    }
    int tiles = 0;
    if (valid != 0 && rank == 0) {
        tiles = (count + 7) / 8;
    }
    unsigned int _vote_10 = __ballot_sync(0xFFFFFFFF, (tiles & 1) != 0);
    unsigned int tile_mask = _vote_10;
    if (lane == 0) {
        masks[20 + warp * 4] = tile_mask;
    }
    unsigned int _vote_11 = __ballot_sync(0xFFFFFFFF, (tiles >> 1 & 1) != 0);
    unsigned int tile_mask_24 = _vote_11;
    if (lane == 0) {
        masks[20 + warp * 4 + 1] = tile_mask_24;
    }
    unsigned int _vote_12 = __ballot_sync(0xFFFFFFFF, (tiles >> 2 & 1) != 0);
    unsigned int tile_mask_25 = _vote_12;
    if (lane == 0) {
        masks[20 + warp * 4 + 2] = tile_mask_25;
    }
    unsigned int _vote_13 = __ballot_sync(0xFFFFFFFF, (tiles >> 3 & 1) != 0);
    unsigned int tile_mask_26 = _vote_13;
    if (lane == 0) {
        masks[20 + warp * 4 + 3] = tile_mask_26;
    }
    __syncthreads();
    int prefix = 0;
    int total_tiles = 0;
    unsigned int low_tiles = masks[20];
    unsigned int high_tiles = masks[24];
    int _popc_4 = __popc(less_low & low_tiles);
    int _popc_5 = __popc(less_high & high_tiles);
    prefix = prefix + (_popc_4 + _popc_5);
    if (tid == 0) {
        int _popc_6 = __popc(low_tiles);
        int _popc_7 = __popc(high_tiles);
        total_tiles = total_tiles + (_popc_6 + _popc_7);
    }
    unsigned int low_tiles_27 = masks[21];
    unsigned int high_tiles_28 = masks[25];
    int _popc_8 = __popc(less_low & low_tiles_27);
    int _popc_9 = __popc(less_high & high_tiles_28);
    prefix = prefix + (_popc_8 + _popc_9 << 1);
    if (tid == 0) {
        int _popc_10 = __popc(low_tiles_27);
        int _popc_11 = __popc(high_tiles_28);
        total_tiles = total_tiles + (_popc_10 + _popc_11 << 1);
    }
    unsigned int low_tiles_29 = masks[22];
    unsigned int high_tiles_30 = masks[26];
    int _popc_12 = __popc(less_low & low_tiles_29);
    int _popc_13 = __popc(less_high & high_tiles_30);
    prefix = prefix + (_popc_12 + _popc_13 << 2);
    if (tid == 0) {
        int _popc_14 = __popc(low_tiles_29);
        int _popc_15 = __popc(high_tiles_30);
        total_tiles = total_tiles + (_popc_14 + _popc_15 << 2);
    }
    unsigned int low_tiles_31 = masks[23];
    unsigned int high_tiles_32 = masks[27];
    int _popc_16 = __popc(less_low & low_tiles_31);
    int _popc_17 = __popc(less_high & high_tiles_32);
    prefix = prefix + (_popc_16 + _popc_17 << 3);
    if (tid == 0) {
        int _popc_18 = __popc(low_tiles_31);
        int _popc_19 = __popc(high_tiles_32);
        total_tiles = total_tiles + (_popc_18 + _popc_19 << 3);
    }
    if (tid == 0) {
    ...
    __syncthreads();
    if (valid != 0) {
        int tile = prefix + rank / 8;
        int row = rank % 8;
        int slot = tile * 8 + row;
        route_slots[tid] = slot;
        route_map[slot] = tid / top_k;
        if (row == 0) {
            int remaining = count - rank;
            int rows = 8;
            if (remaining < 8) {
                rows = remaining;
            }
            tile_expert[tile] = expert;
            tile_mn_limit[tile] = tile * 8 + rows;
            for (int padding = rows; padding < 8; padding++) {
                route_map[tile * 8 + padding] = 0;
            }
        }
    }
}
