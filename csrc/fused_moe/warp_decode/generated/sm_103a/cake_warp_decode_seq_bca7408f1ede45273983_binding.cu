...
extern "C" __global__ void kernel_cake_warp_decode_fa272ea666b32dbd64d5(__nv_bfloat16* __restrict__ route_outputs, __nv_bfloat16* __restrict__ route_weights, int* __restrict__ route_slots, __nv_bfloat16* __restrict__ output, int top_k, int num_tokens, int route_stride, int M);


namespace cake_host_shim_11d76121c4e49d1c {
...
inline void Submit(PreparedLaunch& prepared, cudaStream_t stream) {
  ...
  config.numAttrs = n;
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_warp_decode_fa272ea666b32dbd64d5), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_warp_decode_fa272ea666b32dbd64d5 failed: "
      << cudaGetErrorString(launch_status);
...
TVM_FFI_DLL_EXPORT int __tvm_ffi_run(
  ...
  (void)self;
  cake_host_shim_11d76121c4e49d1c::RunPacked(
