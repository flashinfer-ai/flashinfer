...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_warp_decode_838c83072f3f6100a5f8'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
inline CUtensorMap EncodeTma_A(const TensorView& t) {
  ...
  uint64_t global_dim[4] = {(uint64_t)(256), (uint64_t)(d2), (uint64_t)((d1 / 128)), (uint64_t)(outer2)};
  ...
  TVM_FFI_CHECK(128u <= global_dim[1] && 2u <= global_dim[2] && 1u <= global_dim[3], ValueError)
      << "TMA box (256, 128, 2, 1) exceeds resolved global dims for 'A'";
...
inline CUtensorMap EncodeTma_B(const TensorView& t) {
  ...
  TVM_FFI_CHECK(8u <= global_dim[1] && 2u <= global_dim[2] && 1u <= global_dim[3], ValueError)
      << "TMA box (256, 8, 2, 1) exceeds resolved global dims for 'B'";
  int64_t carrier_stride_0 = (2 * d1);
  TVM_FFI_CHECK(carrier_stride_0 >= 0, ValueError)
...
inline CUtensorMap EncodeTma_SFB(const TensorView& t) {
  ...
  TVM_FFI_CHECK(1u <= global_dim[3], ValueError)
      << "TMA box (256, 2, 8, 1) exceeds resolved global dims for 'SFB'";
  int64_t carrier_stride_0 = s2;
  TVM_FFI_CHECK(carrier_stride_0 >= 0, ValueError)
...
void Run(TensorView arg_A, TensorView arg_B, TensorView arg_SFA, TensorView arg_SFB, TensorView arg_C_tma, TensorView arg_C, TensorView arg_scale_c, TensorView arg_tile_expert, TensorView arg_tile_mn_limit, TensorView arg_num_non_exiting_ctas, TensorView arg_work_counter, int64_t arg_M, int64_t arg_K, int64_t arg_grid_m, int64_t arg_grid_n, int64_t arg_K_tiles, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_warp_decode_838c83072f3f6100a5f8 failed: "
  ...
  config.dynamicSmemBytes = 218240u;
  config.stream = stream;
  config.attrs = attrs;
  config.numAttrs = n;
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_warp_decode_838c83072f3f6100a5f8), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_warp_decode_838c83072f3f6100a5f8 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_94ef67d9896bdeba

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_94ef67d9896bdeba::Run);
