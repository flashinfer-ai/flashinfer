...
#define SMEM_SMEM_V6_OFF 198960
#define SMEM_SMEM_V6_STAGE_BYTES 4096
#define SMEM_SMEM_V6_STRIDE 4096
#define SMEM_SMEM_V7_OFF 203056
#define SMEM_SMEM_V7_STAGE_BYTES 256
#define SMEM_SMEM_V7_STRIDE 256
#define SMEM_SMEM_V8_OFF 203312
#define SMEM_SMEM_V8_STAGE_BYTES 16
#define SMEM_SMEM_V8_STRIDE 16
#define SMEM_SMEM_V9_OFF 203328
#define SMEM_SMEM_V9_STAGE_BYTES 2048
#define SMEM_SMEM_V9_STRIDE 2048
#define SMEM_SMEM_V10_OFF 205376
#define SMEM_SMEM_V10_STAGE_BYTES 2048
#define SMEM_SMEM_V10_STRIDE 2048
#define SMEM_SMEM_V11_OFF 207424
#define SMEM_SMEM_V11_STAGE_BYTES 4096
#define SMEM_SMEM_V11_STRIDE 4096
#define SMEM_SMEM_V12_OFF 211520
#define SMEM_SMEM_V12_STAGE_BYTES 4096
#define SMEM_SMEM_V12_STRIDE 4096
#define SMEM_SMEM_V13_OFF 215616
#define SMEM_SMEM_V13_STAGE_BYTES 2048
#define SMEM_SMEM_V13_STRIDE 2048
#define SMEM_SMEM_V14_OFF 217664
#define SMEM_SMEM_V14_STAGE_BYTES 2048
#define SMEM_SMEM_V14_STRIDE 2048
#define SMEM_SMEM_V15_OFF 203328
#define SMEM_SMEM_V15_STAGE_BYTES 16384
#define SMEM_SMEM_V15_STRIDE 16384
#define SMEM_SMEM_V16_OFF 203328
#define SMEM_SMEM_V16_STAGE_BYTES 16384
#define SMEM_SMEM_V16_STRIDE 16384
#define SMEM_TOTAL 219776
#define THREADS 512
#define BLOCK_M 128
#define BLOCK_N 8
#define BLOCK_K 512
...
kernel_cake_warp_decode_1bb8b252204839e84d03(const __grid_constant__ CUtensorMap A, uint8_t* __restrict__ B, const __grid_constant__ CUtensorMap SFA, uint8_t* __restrict__ SFB, const __grid_constant__ CUtensorMap C, uint8_t* __restrict__ SFC, int* __restrict__ route_map, int* __restrict__ tile_expert, int* __restrict__ tile_mn_limit, int* __restrict__ num_non_exiting_ctas, int* __restrict__ work_counter, float* __restrict__ scale_c, float* __restrict__ scale_gate, float* __restrict__ clamp_limit, float* __restrict__ act_alpha, float* __restrict__ act_beta, int M_out, int K, int grid_m, int grid_n, int K_tiles, int* __restrict__ route_experts, int* __restrict__ route_slots, float* __restrict__ pack_ready, int* __restrict__ done_counter, int route_count, int top_k, int local_expert_offset, int num_experts, int initial_work, int launch_ctas)
{
    ...
    int* smem_v6 = reinterpret_cast<int*>(smem_raw + 198960);
    const int smem_v6_addr = smem + 198960;
    int* smem_v7 = reinterpret_cast<int*>(smem_raw + 203056);
    const int smem_v7_addr = smem + 203056;
    int* smem_v8 = reinterpret_cast<int*>(smem_raw + 203312);
    const int smem_v8_addr = smem + 203312;
    int* smem_v9 = reinterpret_cast<int*>(smem_raw + 203328);
    const int smem_v9_addr = smem + 203328;
    int* smem_v10 = reinterpret_cast<int*>(smem_raw + 205376);
    const int smem_v10_addr = smem + 205376;
    int* smem_v11 = reinterpret_cast<int*>(smem_raw + 207424);
    const int smem_v11_addr = smem + 207424;
    int* smem_v12 = reinterpret_cast<int*>(smem_raw + 211520);
    const int smem_v12_addr = smem + 211520;
    int* smem_v13 = reinterpret_cast<int*>(smem_raw + 215616);
    const int smem_v13_addr = smem + 215616;
    int* smem_v14 = reinterpret_cast<int*>(smem_raw + 217664);
    const int smem_v14_addr = smem + 217664;
    uint8_t* smem_v15 = reinterpret_cast<uint8_t*>(smem_raw + 203328);
    const int smem_v15_addr = smem + 203328;
    int* smem_v16 = reinterpret_cast<int*>(smem_raw + 203328);
    const int smem_v16_addr = smem + 203328;
    const int tid_0 = tid;
    int _min_0 = ((tid_0) < (route_count - 1) ? (tid_0) : (route_count - 1));
    int raw_expert = route_experts[_min_0];
    int expert_slot = tid_0;
    smem_v6[expert_slot] = 0;
    int expert_slot_1 = tid_0 + 512;
    smem_v6[expert_slot_1] = 0;
    int chunk_word = tid_0;
    smem_v16[chunk_word] = 0;
    int chunk_word_2 = tid_0 + 512;
    smem_v16[chunk_word_2] = 0;
    int chunk_word_3 = tid_0 + 1024;
    smem_v16[chunk_word_3] = 0;
    int chunk_word_4 = tid_0 + 1536;
    smem_v16[chunk_word_4] = 0;
    int chunk_word_5 = tid_0 + 2048;
    smem_v16[chunk_word_5] = 0;
    int chunk_word_6 = tid_0 + 2560;
    smem_v16[chunk_word_6] = 0;
    int chunk_word_7 = tid_0 + 3072;
    smem_v16[chunk_word_7] = 0;
    int chunk_word_8 = tid_0 + 3584;
    smem_v16[chunk_word_8] = 0;
    ...
    const int tid_9 = tid;
    const int lane_10 = lane;
    const int warp_11 = warp;
    int publishing = 0;
    if (blockIdx.x == 0 && blockIdx.y == 0) {
        publishing = 1;
    }
    int my_expert = -1;
    int local_expert = raw_expert - local_expert_offset;
    if (tid_9 < route_count && local_expert >= 0 && local_expert < num_experts) {
        my_expert = local_expert;
    }
    __syncthreads();
    unsigned int _vote_0 = __ballot_sync(0xFFFFFFFF, my_expert >= 0);
    int same = _vote_0;
    unsigned int _vote_1 = __ballot_sync(0xFFFFFFFF, (my_expert & 1) == 1);
    int bit_lanes = _vote_1;
    int my_bit = my_expert & 1;
    same = same & (bit_lanes ^ my_bit - 1);
    unsigned int _vote_2 = __ballot_sync(0xFFFFFFFF, (my_expert >> 1 & 1) == 1);
    int bit_lanes_12 = _vote_2;
    int my_bit_13 = my_expert >> 1 & 1;
    same = same & (bit_lanes_12 ^ my_bit_13 - 1);
    unsigned int _vote_3 = __ballot_sync(0xFFFFFFFF, (my_expert >> 2 & 1) == 1);
    int bit_lanes_14 = _vote_3;
    int my_bit_15 = my_expert >> 2 & 1;
    same = same & (bit_lanes_14 ^ my_bit_15 - 1);
    unsigned int _vote_4 = __ballot_sync(0xFFFFFFFF, (my_expert >> 3 & 1) == 1);
    int bit_lanes_16 = _vote_4;
    int my_bit_17 = my_expert >> 3 & 1;
    same = same & (bit_lanes_16 ^ my_bit_17 - 1);
    unsigned int _vote_5 = __ballot_sync(0xFFFFFFFF, (my_expert >> 4 & 1) == 1);
    int bit_lanes_18 = _vote_5;
    int my_bit_19 = my_expert >> 4 & 1;
    same = same & (bit_lanes_18 ^ my_bit_19 - 1);
    unsigned int _vote_6 = __ballot_sync(0xFFFFFFFF, (my_expert >> 5 & 1) == 1);
    int bit_lanes_20 = _vote_6;
    int my_bit_21 = my_expert >> 5 & 1;
    same = same & (bit_lanes_20 ^ my_bit_21 - 1);
    unsigned int _vote_7 = __ballot_sync(0xFFFFFFFF, (my_expert >> 6 & 1) == 1);
    int bit_lanes_22 = _vote_7;
    int my_bit_23 = my_expert >> 6 & 1;
    same = same & (bit_lanes_22 ^ my_bit_23 - 1);
    unsigned int _vote_8 = __ballot_sync(0xFFFFFFFF, (my_expert >> 7 & 1) == 1);
    int bit_lanes_24 = _vote_8;
    int my_bit_25 = my_expert >> 7 & 1;
    same = same & (bit_lanes_24 ^ my_bit_25 - 1);
    unsigned int _vote_9 = __ballot_sync(0xFFFFFFFF, (my_expert >> 8 & 1) == 1);
    int bit_lanes_26 = _vote_9;
    int my_bit_27 = my_expert >> 8 & 1;
    same = same & (bit_lanes_26 ^ my_bit_27 - 1);
    unsigned int _vote_10 = __ballot_sync(0xFFFFFFFF, (my_expert >> 9 & 1) == 1);
    int bit_lanes_28 = _vote_10;
    int my_bit_29 = my_expert >> 9 & 1;
    same = same & (bit_lanes_28 ^ my_bit_29 - 1);
    int lane_below = (1 << lane_10) - 1;
    int _popc_0 = __popc(same);
    int in_cnt = _popc_0;
    int _popc_1 = __popc(same & lane_below);
    int in_rank = _popc_1;
    int leader = 0;
    if (my_expert >= 0 && in_rank == 0) {
        leader = 1;
    }
    int rank = 0;
    if (leader == 1) {
        atomicAdd(&smem_v6[my_expert], in_cnt);
        smem_v15[warp_11 * 1024 + my_expert] = in_cnt;
    }
    __syncthreads();
    int _ffs_0 = __ffs(same);
    int first = _ffs_0 - 1;
    int prefix = 0;
    if (leader == 1) {
        for (int chunk = 0; chunk < warp_11; chunk++) {
            prefix = prefix + (int)smem_v15[chunk * 1024 + my_expert];
        }
    }
    int _max_0 = ((first) > (0) ? (first) : (0));
    int _shfl_0 = __shfl_sync(0xFFFFFFFF, prefix, _max_0);
    rank = _shfl_0 + in_rank;
    int expert0 = 2 * tid_9;
    int expert1 = expert0 + 1;
    int count0 = 0;
    int count1 = 0;
    if (expert0 < num_experts) {
        count0 = smem_v6[expert0];
    }
    if (expert1 < num_experts) {
        count1 = smem_v6[expert1];
    }
    int tiles0 = (count0 + 8 - 1) / 8;
    int tiles1 = (count1 + 8 - 1) / 8;
    int own_tiles = tiles0 + tiles1;
    int own_count = count0 + count1;
    int tiles_incl = own_tiles;
    int count_incl = own_count;
    int _shfl_up_0 = __shfl_up_sync(0xFFFFFFFF, tiles_incl, 1, 32);
    int peer_tiles = _shfl_up_0;
    int _shfl_up_1 = __shfl_up_sync(0xFFFFFFFF, count_incl, 1, 32);
    int peer_count = _shfl_up_1;
    if (lane_10 >= 1) {
        tiles_incl = tiles_incl + peer_tiles;
        count_incl = count_incl + peer_count;
    }
    int _shfl_up_2 = __shfl_up_sync(0xFFFFFFFF, tiles_incl, 2, 32);
    int peer_tiles_30 = _shfl_up_2;
    int _shfl_up_3 = __shfl_up_sync(0xFFFFFFFF, count_incl, 2, 32);
    int peer_count_31 = _shfl_up_3;
    if (lane_10 >= 2) {
        tiles_incl = tiles_incl + peer_tiles_30;
        count_incl = count_incl + peer_count_31;
    }
    int _shfl_up_4 = __shfl_up_sync(0xFFFFFFFF, tiles_incl, 4, 32);
    int peer_tiles_32 = _shfl_up_4;
    int _shfl_up_5 = __shfl_up_sync(0xFFFFFFFF, count_incl, 4, 32);
    int peer_count_33 = _shfl_up_5;
    if (lane_10 >= 4) {
        tiles_incl = tiles_incl + peer_tiles_32;
        count_incl = count_incl + peer_count_33;
    }
    int _shfl_up_6 = __shfl_up_sync(0xFFFFFFFF, tiles_incl, 8, 32);
    int peer_tiles_34 = _shfl_up_6;
    int _shfl_up_7 = __shfl_up_sync(0xFFFFFFFF, count_incl, 8, 32);
    int peer_count_35 = _shfl_up_7;
    if (lane_10 >= 8) {
        tiles_incl = tiles_incl + peer_tiles_34;
        count_incl = count_incl + peer_count_35;
    }
    int _shfl_up_8 = __shfl_up_sync(0xFFFFFFFF, tiles_incl, 16, 32);
    int peer_tiles_36 = _shfl_up_8;
    int _shfl_up_9 = __shfl_up_sync(0xFFFFFFFF, count_incl, 16, 32);
    int peer_count_37 = _shfl_up_9;
    if (lane_10 >= 16) {
        tiles_incl = tiles_incl + peer_tiles_36;
        count_incl = count_incl + peer_count_37;
    }
    if (lane_10 == 31) {
        smem_v7[warp_11] = tiles_incl;
        smem_v7[16 + warp_11] = count_incl;
    }
    __syncthreads();
    if (warp_11 == 0) {
        int warp_tiles = 0;
        int warp_count = 0;
        if (lane_10 < 16) {
            warp_tiles = smem_v7[lane_10];
            warp_count = smem_v7[16 + lane_10];
        }
        int _shfl_up_10 = __shfl_up_sync(0xFFFFFFFF, warp_tiles, 1, 32);
        int peer_warp_tiles = _shfl_up_10;
        int _shfl_up_11 = __shfl_up_sync(0xFFFFFFFF, warp_count, 1, 32);
        int peer_warp_count = _shfl_up_11;
        if (lane_10 >= 1) {
            warp_tiles = warp_tiles + peer_warp_tiles;
            warp_count = warp_count + peer_warp_count;
        }
        int _shfl_up_12 = __shfl_up_sync(0xFFFFFFFF, warp_tiles, 2, 32);
        int peer_warp_tiles_0 = _shfl_up_12;
        int _shfl_up_13 = __shfl_up_sync(0xFFFFFFFF, warp_count, 2, 32);
        int peer_warp_count_1 = _shfl_up_13;
        if (lane_10 >= 2) {
            warp_tiles = warp_tiles + peer_warp_tiles_0;
            warp_count = warp_count + peer_warp_count_1;
        }
        int _shfl_up_14 = __shfl_up_sync(0xFFFFFFFF, warp_tiles, 4, 32);
        int peer_warp_tiles_2 = _shfl_up_14;
        int _shfl_up_15 = __shfl_up_sync(0xFFFFFFFF, warp_count, 4, 32);
        int peer_warp_count_3 = _shfl_up_15;
        if (lane_10 >= 4) {
            warp_tiles = warp_tiles + peer_warp_tiles_2;
            warp_count = warp_count + peer_warp_count_3;
        }
        int _shfl_up_16 = __shfl_up_sync(0xFFFFFFFF, warp_tiles, 8, 32);
        int peer_warp_tiles_4 = _shfl_up_16;
        int _shfl_up_17 = __shfl_up_sync(0xFFFFFFFF, warp_count, 8, 32);
        int peer_warp_count_5 = _shfl_up_17;
        if (lane_10 >= 8) {
            warp_tiles = warp_tiles + peer_warp_tiles_4;
            warp_count = warp_count + peer_warp_count_5;
        }
        int _shfl_up_18 = __shfl_up_sync(0xFFFFFFFF, warp_tiles, 16, 32);
        int peer_warp_tiles_6 = _shfl_up_18;
        int _shfl_up_19 = __shfl_up_sync(0xFFFFFFFF, warp_count, 16, 32);
        int peer_warp_count_7 = _shfl_up_19;
        if (lane_10 >= 16) {
            warp_tiles = warp_tiles + peer_warp_tiles_6;
            warp_count = warp_count + peer_warp_count_7;
        }
        if (lane_10 < 16) {
            smem_v7[32 + lane_10] = warp_tiles;
            smem_v7[48 + lane_10] = warp_count;
        }
    }
    __syncthreads();
    ...
    if (warp_11 > 0) {
        warp_base_tiles = smem_v7[32 + warp_11 - 1];
        warp_base_count = smem_v7[48 + warp_11 - 1];
    }
    int excl_tiles = warp_base_tiles + tiles_incl - own_tiles;
    int excl_count = warp_base_count + count_incl - own_count;
    int start0 = excl_tiles;
    int start1 = excl_tiles + tiles0;
    int total_tiles = smem_v7[47];
    if (tid_9 == 0) {
        smem_v8[0] = total_tiles;
        if (publishing == 1) {
            num_non_exiting_ctas[0] = total_tiles;
        }
    }
    if (expert0 < num_experts) {
        for (int tile_i0 = 0; tile_i0 < tiles0; tile_i0++) {
            int tile0 = start0 + tile_i0;
            int _min_1 = ((count0 - tile_i0 * 8) < (8) ? (count0 - tile_i0 * 8) : (8));
            int valid_rows0 = _min_1;
            smem_v9[tile0] = expert0;
            smem_v10[tile0] = tile0 * 8 + valid_rows0;
            if (publishing == 1) {
                tile_expert[tile0] = expert0;
                tile_mn_limit[tile0] = tile0 * 8 + valid_rows0;
            }
        }
    }
    if (expert1 < num_experts) {
        for (int tile_i1 = 0; tile_i1 < tiles1; tile_i1++) {
            int tile1 = start1 + tile_i1;
            int _min_2 = ((count1 - tile_i1 * 8) < (8) ? (count1 - tile_i1 * 8) : (8));
            int valid_rows1 = _min_2;
            smem_v9[tile1] = expert1;
            smem_v10[tile1] = tile1 * 8 + valid_rows1;
            if (publishing == 1) {
                tile_expert[tile1] = expert1;
                tile_mn_limit[tile1] = tile1 * 8 + valid_rows1;
            }
        }
    }
    if (expert0 < num_experts) {
        smem_v11[expert0] = start0;
        smem_v12[expert0] = excl_count;
    }
    if (expert1 < num_experts) {
        smem_v11[expert1] = start1;
        smem_v12[expert1] = excl_count + count0;
    }
    __syncthreads();
    if (my_expert >= 0) {
        int position = smem_v12[my_expert] + rank;
        int route_tile = smem_v11[my_expert] + rank / 8;
        int row = rank % 8;
        int token = tid_9 / top_k;
        smem_v13[position] = token;
        if (row == 0) {
            smem_v14[route_tile] = position;
        }
        if (publishing == 1) {
            int slot = route_tile * 8 + row;
            route_map[slot] = token;
            route_slots[tid_9] = slot;
        }
    }
    if (publishing == 1) {
        __threadfence();
    ...
    if (warp <= 3) {
        ...
        { // epilogue_main
            ...
            for (unsigned int _tile_iter = 0; _tile_iter < grid_m * grid_n; _tile_iter++) {
                if (m_tile >= (unsigned int)grid_m || n_tile >= (unsigned int)smem_v8[0]) {
                    break;
                }
                unsigned int tile_work_id = m_tile * (unsigned int)grid_n + n_tile;
                bool tile_profile = _tile_iter < 2;
                int expert = smem_v9[n_tile];
                int valid_rows = (unsigned int)smem_v10[n_tile] - n_tile * (unsigned int)BLOCK_N;
                ...
                for (int token_group = 0; token_group < 1; token_group++) {
                    ...
                    float value11 = 0.0f;
                    {
                        float _max_5 = max_noftz(_tmem_load_0[token_group * 4], neg_cl);
                        float _min_11 = fminf(_max_5, cl);
                        float lin00 = _min_11;
                        float _max_6 = max_noftz(_tmem_load_0[token_group * 4 + 1], neg_cl);
                        float _min_12 = fminf(_max_6, cl);
                        float lin01 = _min_12;
                        float _min_13 = fminf(_tmem_load_0[token_group * 4 + 2], cl);
                        float gate00 = _min_13;
                        float _min_14 = fminf(_tmem_load_0[token_group * 4 + 3], cl);
                        float gate01 = _min_14;
                        float _max_7 = max_noftz(_tmem_load_1[token_group * 4], neg_cl);
                        float _min_15 = fminf(_max_7, cl);
                        float lin10 = _min_15;
                        float _max_8 = max_noftz(_tmem_load_1[token_group * 4 + 1], neg_cl);
                        float _min_16 = fminf(_max_8, cl);
                        float lin11 = _min_16;
                        float _min_17 = fminf(_tmem_load_1[token_group * 4 + 2], cl);
                        float gate10 = _min_17;
                        float _min_18 = fminf(_tmem_load_1[token_group * 4 + 3], cl);
                        float gate11 = _min_18;
                        float _expf_4 = __expf(-(alpha_sg * gate00));
                        value00 = (lin00 * sc * sg + beta_sg) * gate00 / (1.0f + _expf_4);
                        float _expf_5 = __expf(-(alpha_sg * gate01));
                        value01 = (lin01 * sc * sg + beta_sg) * gate01 / (1.0f + _expf_5);
                        float _expf_6 = __expf(-(alpha_sg * gate10));
                        value10 = (lin10 * sc * sg + beta_sg) * gate10 / (1.0f + _expf_6);
                        float _expf_7 = __expf(-(alpha_sg * gate11));
                        value11 = (lin11 * sc * sg + beta_sg) * gate11 / (1.0f + _expf_7);
                    }
                    float _fabs_0 = fabsf(value00);
                    float _fabs_1 = fabsf(value10);
                    float _max_9 = max_noftz(_fabs_0, _fabs_1);
                    float block_max0 = _max_9;
                    float _fabs_2 = fabsf(value01);
                    float _fabs_3 = fabsf(value11);
                    float _max_10 = max_noftz(_fabs_2, _fabs_3);
                    float block_max1 = _max_10;
                    float _shfl_xor_0 = __shfl_xor_sync(0xFFFFFFFF, block_max0, 4);
                    float _max_11 = max_noftz(block_max0, _shfl_xor_0);
                    block_max0 = _max_11;
                    float _shfl_xor_1 = __shfl_xor_sync(0xFFFFFFFF, block_max1, 4);
                    float _max_12 = max_noftz(block_max1, _shfl_xor_1);
                    block_max1 = _max_12;
                    float _shfl_xor_2 = __shfl_xor_sync(0xFFFFFFFF, block_max0, 8);
                    float _max_13 = max_noftz(block_max0, _shfl_xor_2);
                    block_max0 = _max_13;
                    float _shfl_xor_3 = __shfl_xor_sync(0xFFFFFFFF, block_max1, 8);
                    float _max_14 = max_noftz(block_max1, _shfl_xor_3);
                    block_max1 = _max_14;
                    float _shfl_xor_4 = __shfl_xor_sync(0xFFFFFFFF, block_max0, 16);
                    float _max_15 = max_noftz(block_max0, _shfl_xor_4);
                    block_max0 = _max_15;
                    float _shfl_xor_5 = __shfl_xor_sync(0xFFFFFFFF, block_max1, 16);
                    float _max_16 = max_noftz(block_max1, _shfl_xor_5);
                    block_max1 = _max_16;
                    float scale0 = 0.0f;
                    float scale1 = 0.0f;
                    {
                    ...
                    float2 oa_quant0 = _f2_3;
                    float2 _f2_4 = make_float2(0.0f, 0.0f);
                    float2 oa_quant1 = _f2_4;
                    {
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // copy_sfb_main
            ...
            for (unsigned int _tile_iter_1 = 0; _tile_iter_1 < grid_m * grid_n; _tile_iter_1++) {
                if (m_tile_1 >= (unsigned int)grid_m || n_tile_1 >= (unsigned int)smem_v8[0]) {
                    break;
                }
                #pragma unroll 1
    ...
    if (warp >= 8 && warp <= 9) {
        { // load_b_main
            ...
            for (unsigned int _tile_iter_2 = 0; _tile_iter_2 < grid_m * grid_n; _tile_iter_2++) {
                if (m_tile_2 >= (unsigned int)grid_m || n_tile_2 >= (unsigned int)smem_v8[0]) {
                    break;
                }
                int valid_rows_1 = (unsigned int)smem_v10[n_tile_2] - n_tile_2 * (unsigned int)BLOCK_N;
                #pragma unroll 1
                for (int iter_k = 0; iter_k < K_tiles; iter_k++) {
                    ...
                    for (int row_group = 0; row_group < 1; row_group++) {
                        ...
                        int row_1 = elt_offset / 256;
                        int col = elt_offset % 256;
                        int routed = 0;
                        if (row_1 < valid_rows_1) {
                            routed = smem_v13[smem_v14[n_tile_2] + row_1];
                        }
                        int src_base = routed * row_stride_bytes + iter_k * 256 + col / 2;
                        int dst_chunk = elt_offset / 2 ^ row_1 % 8 * 16;
                        asm volatile(
                            "{\n\t"
                            ".reg .pred p;\n\t"
                            "setp.ne.b32 p, %0, 0;\n\t"
                            ...
                            "}"
                            :: "r"((row_1 < valid_rows_1) ? 1 : 0), "r"(dst_base + dst_chunk), "l"(B + src_base));
                        asm volatile(
                            "{\n\t"
                            ".reg .pred p;\n\t"
                            "setp.ne.b32 p, %0, 0;\n\t"
                            "@p cp.async.ca.shared::cta.global [%1], [%2], 16;\n\t"
                            "}"
                            :: "r"((row_1 < valid_rows_1) ? 1 : 0), "r"(dst_base + 1024 + dst_chunk), "l"(B + (src_base + 128)));
                    }
                    asm volatile(
                        "{\n\t"
    ...
    if (warp == 10) {
        { // load_sfb_main
            ...
            for (unsigned int _tile_iter_3 = 0; _tile_iter_3 < grid_m * grid_n; _tile_iter_3++) {
                if (m_tile_3 >= (unsigned int)grid_m || n_tile_3 >= (unsigned int)smem_v8[0]) {
                    break;
                }
                int valid_rows_2 = (unsigned int)smem_v10[n_tile_3] - n_tile_3 * (unsigned int)BLOCK_N;
                #pragma unroll 1
                for (int iter_k_1 = 0; iter_k_1 < K_tiles; iter_k_1++) {
                    ...
                    for (int row_group_1 = 0; row_group_1 < 2; row_group_1++) {
                        int row_2 = row0 + row_group_1 * 4;
                        int routed_1 = 0;
                        if (row_2 < valid_rows_2) {
                            routed_1 = smem_v13[smem_v14[n_tile_3] + row_2];
                        }
                        int dst_offset = row_2 / 8 * 256 + block4 * 32 + row_2 % 8 * 4;
                        asm volatile(
                            "{\n\t"
                            ".reg .pred p;\n\t"
                            "setp.ne.b32 p, %0, 0;\n\t"
                            ...
                            "}"
                            :: "r"((row_2 < valid_rows_2) ? 1 : 0), "r"(smem_sfb_addr + stage_2 * 256 + (unsigned int)dst_offset), "l"(SFB + (routed_1 * sf_stride + sf_col)));
                    }
                    asm volatile(
                        "{\n\t"
                        "cp.async.mbarrier.arrive.shared::cta.b64 [%0];\n\t"
    ...
    if (warp == 11) {
        { // load_a_main
            ...
            for (unsigned int _tile_iter_4 = 0; _tile_iter_4 < grid_m * grid_n; _tile_iter_4++) {
                if (m_tile_4 >= (unsigned int)grid_m || n_tile_4 >= (unsigned int)smem_v8[0]) {
                    break;
                }
                int expert_1 = smem_v9[n_tile_4];
    ...
    if (warp == 12) {
        { // load_sfa_main
            ...
            for (unsigned int _tile_iter_5 = 0; _tile_iter_5 < grid_m * grid_n; _tile_iter_5++) {
                if (m_tile_5 >= (unsigned int)grid_m || n_tile_5 >= (unsigned int)smem_v8[0]) {
                    break;
                }
                int expert_2 = smem_v9[n_tile_5];
                #pragma unroll 1
    ...
    if (warp == 13) {
        { // copy_sfa_main
            ...
            for (unsigned int _tile_iter_6 = 0; _tile_iter_6 < grid_m * grid_n; _tile_iter_6++) {
                if (m_tile_6 >= (unsigned int)grid_m || n_tile_6 >= (unsigned int)smem_v8[0]) {
                    break;
                }
                #pragma unroll 1
    ...
    if (warp == 14) {
        { // mma_main
            ...
            for (unsigned int _tile_iter_7 = 0; _tile_iter_7 < grid_m * grid_n; _tile_iter_7++) {
                if (m_tile_7 >= (unsigned int)grid_m || n_tile_7 >= (unsigned int)smem_v8[0]) {
                    break;
                }
    ...
    if (warp == 15) {
        { // work_id_main
            ...
            for (unsigned int _tile_iter_8 = 0; _tile_iter_8 < grid_m * grid_n; _tile_iter_8++) {
                if (m_tile_8 >= (unsigned int)grid_m || n_tile_8 >= (unsigned int)smem_v8[0]) {
                    break;
                }
                ...
                if (elect_sync()) {
                    ...
                    work_response[response_index_8] = next_linear < grid_m * smem_v8[0];
            ...
            if (elect_sync()) {
                __threadfence();
                int _atomic_old_1 = atomicAdd(done_counter, 1);
                int finished = _atomic_old_1;
                if (finished == launch_ctas - 1) {
                    work_counter[0] = initial_work;
                    done_counter[0] = 0;
                }
            }
        }
