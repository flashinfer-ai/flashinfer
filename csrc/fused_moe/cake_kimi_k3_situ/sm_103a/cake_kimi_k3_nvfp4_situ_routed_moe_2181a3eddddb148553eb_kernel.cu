...
#define NUM_WORK_PIPE_STAGES 8
#define NUM_THROTTLE_PIPE_STAGES 8
...
#define SMEM_SMEM_B_MMA0_STRIDE 8192
#define SMEM_SMEM_B_MMA1_OFF 132128
#define SMEM_SMEM_B_MMA1_STAGE_BYTES 1024
#define SMEM_SMEM_B_MMA1_STRIDE 8192
#define SMEM_SMEM_B_MMA2_OFF 132160
#define SMEM_SMEM_B_MMA2_STAGE_BYTES 1024
#define SMEM_SMEM_B_MMA2_STRIDE 8192
#define SMEM_SMEM_B_MMA3_OFF 132192
#define SMEM_SMEM_B_MMA3_STAGE_BYTES 1024
#define SMEM_SMEM_B_MMA3_STRIDE 8192
#define SMEM_SMEM_B_MMA4_OFF 136192
#define SMEM_SMEM_B_MMA4_STAGE_BYTES 1024
#define SMEM_SMEM_B_MMA4_STRIDE 8192
#define SMEM_SMEM_B_MMA5_OFF 136224
#define SMEM_SMEM_B_MMA5_STAGE_BYTES 1024
#define SMEM_SMEM_B_MMA5_STRIDE 8192
#define SMEM_SMEM_B_MMA6_OFF 136256
#define SMEM_SMEM_B_MMA6_STAGE_BYTES 1024
#define SMEM_SMEM_B_MMA6_STRIDE 8192
#define SMEM_SMEM_B_MMA7_OFF 136288
#define SMEM_SMEM_B_MMA7_STAGE_BYTES 1024
#define SMEM_SMEM_B_MMA7_STRIDE 8192
...
#define SMEM_SMEM_B_MMA0_NEXT_STRIDE 8192
...
#define SMEM_SMEM_B_MMA2_NEXT_STRIDE 8192
...
#define SMEM_WORK_RESPONSE_OFF 193536
...
kernel_cake_kimi_k3_nvfp4_situ_routed_moe_2181a3eddddb148553eb(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap C_tma, __nv_bfloat16* __restrict__ C, float* __restrict__ scale_c, int* __restrict__ tile_expert, int* __restrict__ tile_mn_limit, int* __restrict__ num_non_exiting_ctas, int* __restrict__ work_counter, int M, int K, int grid_m, int grid_n, int K_tiles)
{
    ...
    #define work_empty_addr (mbar_base + 480)
    #define throttle_full_addr (mbar_base + 544)
    #define throttle_empty_addr (mbar_base + 608)

    const int bid = blockIdx.x;
    const int num_bids = gridDim.x;
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 672);
    ...
    uint8_t* smem_b_mma4 = reinterpret_cast<uint8_t*>(smem_raw + 136192);
    const int smem_b_mma4_addr = smem + 136192;
    uint8_t* smem_b_mma5 = reinterpret_cast<uint8_t*>(smem_raw + 136224);
    const int smem_b_mma5_addr = smem + 136224;
    uint8_t* smem_b_mma6 = reinterpret_cast<uint8_t*>(smem_raw + 136256);
    const int smem_b_mma6_addr = smem + 136256;
    uint8_t* smem_b_mma7 = reinterpret_cast<uint8_t*>(smem_raw + 136288);
    const int smem_b_mma7_addr = smem + 136288;
    ...
    uint8_t* smem_sfb = reinterpret_cast<uint8_t*>(smem_raw + 181248);
    const int smem_sfb_addr = smem + 181248;
    ...
    unsigned int* work_response = reinterpret_cast<unsigned int*>(smem_raw + 193536);
    const int work_response_addr = smem + 193536;
    asm volatile("griddepcontrol.wait;" ::: "memory");
    if ((int)blockIdx.y >= num_non_exiting_ctas[0]) return;

    // Mbarrier init (18 pipeline groups, 0 ordered-sequence groups, 84 barriers)
    // Mbarriers at smem_raw[0..672)
    ...
    if (warp == 0) {
        ...
        if (leader) {
            ...
            // --- pipeline 'work_pipe' ---
            // work_full: 8 barriers, init_count=1
            ...
            // work_empty: 8 barriers, init_count=512
            mbarrier_init(smem + 480, 512);
            mbarrier_init(smem + 488, 512);
            mbarrier_init(smem + 496, 512);
            mbarrier_init(smem + 504, 512);
            mbarrier_init(smem + 512, 512);
            mbarrier_init(smem + 520, 512);
            mbarrier_init(smem + 528, 512);
            mbarrier_init(smem + 536, 512);
            // --- pipeline 'throttle_pipe' ---
            // throttle_full: 8 barriers, init_count=32
            mbarrier_init(smem + 544, 32);
            mbarrier_init(smem + 552, 32);
            mbarrier_init(smem + 560, 32);
            mbarrier_init(smem + 568, 32);
            mbarrier_init(smem + 576, 32);
            mbarrier_init(smem + 584, 32);
            mbarrier_init(smem + 592, 32);
            mbarrier_init(smem + 600, 32);
            // throttle_empty: 8 barriers, init_count=32
            mbarrier_init(smem + 608, 32);
            mbarrier_init(smem + 616, 32);
            mbarrier_init(smem + 624, 32);
            mbarrier_init(smem + 632, 32);
            mbarrier_init(smem + 640, 32);
            mbarrier_init(smem + 648, 32);
            mbarrier_init(smem + 656, 32);
            mbarrier_init(smem + 664, 32);
    ...
    if (warp <= 3) {
        ...
        { // epilogue_main
            ...
            for (unsigned int _tile_iter = 0; _tile_iter < grid_m * grid_n; _tile_iter++) {
                ...
                work_stage += 1;
                if (work_stage == 8) { work_stage = 0; _phase_work_full ^= 1; }
        ...
        { // copy_sfb_main
            ...
            for (unsigned int _tile_iter_1 = 0; _tile_iter_1 < grid_m * grid_n; _tile_iter_1++) {
                ...
                for (int _route_k = 0; _route_k < route_k_extent; _route_k++) {
                    mbarrier_wait_hint(sfb_full_addr + (stage) * 8, _phase_sfb_full, 1000);
                    const int q = 0;
                    for (int reg = 0; reg < 1; reg++) {
                        word[q + reg] = 0;
                        if (lane_0 + reg * 32 < 32) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[q + reg])) : "r"(smem_sfb_addr + stage * 1024 + (unsigned int)((reg * 4 + lane_0 / 8) * 256) + (unsigned int)(q * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    const int q_0 = 1;
                    for (int reg_1 = 0; reg_1 < 1; reg_1++) {
                        ...
                        if (lane_0 + reg_1 * 32 < 32) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[q_0 + reg_1])) : "r"(smem_sfb_addr + stage * 1024 + (unsigned int)((reg_1 * 4 + lane_0 / 8) * 256) + (unsigned int)(q_0 * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    const int q_1 = 2;
                    for (int reg_2 = 0; reg_2 < 1; reg_2++) {
                        ...
                        if (lane_0 + reg_2 * 32 < 32) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[q_1 + reg_2])) : "r"(smem_sfb_addr + stage * 1024 + (unsigned int)((reg_2 * 4 + lane_0 / 8) * 256) + (unsigned int)(q_1 * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    const int q_2 = 3;
                    for (int reg_3 = 0; reg_3 < 1; reg_3++) {
                        ...
                        if (lane_0 + reg_3 * 32 < 32) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[q_2 + reg_3])) : "r"(smem_sfb_addr + stage * 1024 + (unsigned int)((reg_3 * 4 + lane_0 / 8) * 256) + (unsigned int)(q_2 * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    const int q_3 = 4;
                    for (int reg_4 = 0; reg_4 < 1; reg_4++) {
                        word[q_3 + reg_4] = 0;
                        if (lane_0 + reg_4 * 32 < 32) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[q_3 + reg_4])) : "r"(smem_sfb_addr + stage * 1024 + (unsigned int)((reg_4 * 4 + lane_0 / 8) * 256) + (unsigned int)(q_3 * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    const int q_4 = 5;
                    for (int reg_5 = 0; reg_5 < 1; reg_5++) {
                        word[q_4 + reg_5] = 0;
                        if (lane_0 + reg_5 * 32 < 32) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[q_4 + reg_5])) : "r"(smem_sfb_addr + stage * 1024 + (unsigned int)((reg_5 * 4 + lane_0 / 8) * 256) + (unsigned int)(q_4 * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    ...
                    asm volatile(
                        ...
                        " [%0], {%1};"
                        :: "r"(taddr + 64 + 128 + stage * 16), "r"((word + q_5)[0]));
                    const int q_6 = 1;
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 64 + 128 + stage * 16 + 2), "r"((word + q_6)[0]));
                    const int q_7 = 2;
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 64 + 128 + stage * 16 + 4), "r"((word + q_7)[0]));
                    const int q_8 = 3;
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 64 + 128 + stage * 16 + 6), "r"((word + q_8)[0]));
                    const int q_9 = 4;
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 64 + 128 + stage * 16 + 8), "r"((word + q_9)[0]));
                    const int q_10 = 5;
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 64 + 128 + stage * 16 + 10), "r"((word + q_10)[0]));
                ...
                work_stage_1 += 1;
                if (work_stage_1 == 8) { work_stage_1 = 0; _phase_work_full_1 ^= 1; }
        ...
        { // load_b_main
            ...
            for (unsigned int _tile_iter_2 = 0; _tile_iter_2 < grid_m * grid_n; _tile_iter_2++) {
                ...
                for (int route_k = 0; route_k < route_k_extent_1; route_k++) {
                    ...
                    if (elect_sync()) {
                        tma_3d_gmem2smem(smem_b_addr + stage_1 * 8192, (&B), iter_k * 512, 0, route_tile, b_full_addr + (stage_1) * 8);
                        tma_3d_gmem2smem(smem_b_addr + stage_1 * 8192 + 4096, (&B), iter_k * 512 + 256, 0, route_tile, b_full_addr + (stage_1) * 8);
                        mbarrier_arrive_expect_tx(b_full_addr + (stage_1) * 8, 8192);
                    }
                    stage_1 += 1;
                ...
                work_stage_2 += 1;
                if (work_stage_2 == 8) { work_stage_2 = 0; _phase_work_full_2 ^= 1; }
        ...
        { // load_sfb_main
            ...
            for (unsigned int _tile_iter_3 = 0; _tile_iter_3 < grid_m * grid_n; _tile_iter_3++) {
                ...
                for (int route_k_1 = 0; route_k_1 < route_k_extent_2; route_k_1++) {
                    ...
                    if (elect_sync()) {
                        tma_3d_gmem2smem(smem_sfb_addr + stage_2 * 1024, (&SFB), 0, iter_k_1 * 8, route_tile_1 * 4, sfb_full_addr + (stage_2) * 8);
                        mbarrier_arrive_expect_tx(sfb_full_addr + (stage_2) * 8, 1024);
                    }
                    stage_2 += 1;
                ...
                work_stage_3 += 1;
                if (work_stage_3 == 8) { work_stage_3 = 0; _phase_work_full_3 ^= 1; }
    ...
    if (warp == 10) {
        { // load_a_main
            ...
            for (unsigned int _tile_iter_4 = 0; _tile_iter_4 < grid_m * grid_n; _tile_iter_4++) {
                ...
                throttle_stage += 1;
                if (throttle_stage == 8) { throttle_stage = 0; _phase_throttle_empty ^= 1; }
                ...
                work_stage_4 += 1;
                if (work_stage_4 == 8) { work_stage_4 = 0; _phase_work_full_4 ^= 1; }
    ...
    if (warp == 11) {
        { // load_sfa_main
            ...
            for (unsigned int _tile_iter_5 = 0; _tile_iter_5 < grid_m * grid_n; _tile_iter_5++) {
                ...
                work_stage_5 += 1;
                if (work_stage_5 == 8) { work_stage_5 = 0; _phase_work_full_5 ^= 1; }
    ...
    if (warp == 12) {
        { // copy_sfa_main
            ...
            for (unsigned int _tile_iter_6 = 0; _tile_iter_6 < grid_m * grid_n; _tile_iter_6++) {
                ...
                work_stage_6 += 1;
                if (work_stage_6 == 8) { work_stage_6 = 0; _phase_work_full_6 ^= 1; }
        ...
        { // mma_main
            ...
            for (unsigned int _tile_iter_7 = 0; _tile_iter_7 < grid_m * grid_n; _tile_iter_7++) {
                ...
                for (int route_k_4 = 0; route_k_4 < route_k_extent_6; route_k_4++) {
                    ...
                    int _mma_b_lo_0 = make_warp_uniform((((smem_b_mma0_addr) >> 4) & 0x3FFF) + (k_stage) * 512);
                    if (elect_sync()) {
                        {
                            ...
                            tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 32)), a_desc + 0, b_desc + 0,
                                0x8080480U, (unsigned int)tmem_sfa + (k_stage * 32 + (unsigned int)sfa_base_col) + 0, (unsigned int)tmem_sfb + (k_stage * 16 + (unsigned int)sfb_base_col) + 0, ((((1) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                        }
                    }
                    int sfa_base_col_0 = 4;
                    int sfb_base_col_1 = 2;
                    int _mma_a_lo_1 = make_warp_uniform((((smem_a_mma1_addr) >> 4) & 0x3FFF) + (k_stage) * 2048);
                    int _mma_b_lo_1 = make_warp_uniform((((smem_b_mma1_addr) >> 4) & 0x3FFF) + (k_stage) * 512);
                    if (elect_sync()) {
                        {
                            ...
                            tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 32)), a_desc + 0, b_desc + 0,
                                0x8080480U, (unsigned int)tmem_sfa + (k_stage * 32 + (unsigned int)sfa_base_col_0) + 0, (unsigned int)tmem_sfb + (k_stage * 16 + (unsigned int)sfb_base_col_1) + 0, ((((0) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                        }
                    }
                    int sfa_base_col_2 = 8;
                    int sfb_base_col_3 = 4;
                    int _mma_a_lo_2 = make_warp_uniform((((smem_a_mma2_addr) >> 4) & 0x3FFF) + (k_stage) * 2048);
                    int _mma_b_lo_2 = make_warp_uniform((((smem_b_mma2_addr) >> 4) & 0x3FFF) + (k_stage) * 512);
                    if (elect_sync()) {
                        {
                            ...
                            tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 32)), a_desc + 0, b_desc + 0,
                                0x8080480U, (unsigned int)tmem_sfa + (k_stage * 32 + (unsigned int)sfa_base_col_2) + 0, (unsigned int)tmem_sfb + (k_stage * 16 + (unsigned int)sfb_base_col_3) + 0, ((((0) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                        }
                    }
                    int sfa_base_col_4 = 12;
                    int sfb_base_col_5 = 6;
                    int _mma_a_lo_3 = make_warp_uniform((((smem_a_mma3_addr) >> 4) & 0x3FFF) + (k_stage) * 2048);
                    int _mma_b_lo_3 = make_warp_uniform((((smem_b_mma3_addr) >> 4) & 0x3FFF) + (k_stage) * 512);
                    if (elect_sync()) {
                        {
                            ...
                            tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 32)), a_desc + 0, b_desc + 0,
                                0x8080480U, (unsigned int)tmem_sfa + (k_stage * 32 + (unsigned int)sfa_base_col_4) + 0, (unsigned int)tmem_sfb + (k_stage * 16 + (unsigned int)sfb_base_col_5) + 0, ((((0) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                        }
                    }
                    int sfa_base_col_6 = 16;
                    int sfb_base_col_7 = 8;
                    int _mma_a_lo_4 = make_warp_uniform((((smem_a_mma4_addr) >> 4) & 0x3FFF) + (k_stage) * 2048);
                    int _mma_b_lo_4 = make_warp_uniform((((smem_b_mma4_addr) >> 4) & 0x3FFF) + (k_stage) * 512);
                    if (elect_sync()) {
                        {
                            ...
                            tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 32)), a_desc + 0, b_desc + 0,
                                0x8080480U, (unsigned int)tmem_sfa + (k_stage * 32 + (unsigned int)sfa_base_col_6) + 0, (unsigned int)tmem_sfb + (k_stage * 16 + (unsigned int)sfb_base_col_7) + 0, ((((0) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                        }
                    }
                    int sfa_base_col_8 = 20;
                    int sfb_base_col_9 = 10;
                    int _mma_a_lo_5 = make_warp_uniform((((smem_a_mma5_addr) >> 4) & 0x3FFF) + (k_stage) * 2048);
                    int _mma_b_lo_5 = make_warp_uniform((((smem_b_mma5_addr) >> 4) & 0x3FFF) + (k_stage) * 512);
                    if (elect_sync()) {
                        {
                            ...
                            tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 32)), a_desc + 0, b_desc + 0,
                                0x8080480U, (unsigned int)tmem_sfa + (k_stage * 32 + (unsigned int)sfa_base_col_8) + 0, (unsigned int)tmem_sfb + (k_stage * 16 + (unsigned int)sfb_base_col_9) + 0, ((((0) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                        }
                    }
                    if (elect_sync()) {
                ...
                work_stage_7 += 1;
                if (work_stage_7 == 8) { work_stage_7 = 0; _phase_work_full_7 ^= 1; }
    ...
    if (warp == 14) {
        { // work_id_main
            ...
            for (unsigned int _tile_iter_8 = 0; _tile_iter_8 < grid_m * grid_n; _tile_iter_8++) {
                ...
                throttle_stage_1 += 1;
                if (throttle_stage_1 == 8) { throttle_stage_1 = 0; _phase_throttle_full ^= 1; }
                if (elect_sync()) {
                    mbarrier_wait(work_empty_addr + (work_stage_8) * 8, _phase_work_empty);
                    if (claim_offset == 8) {
                        int _atomic_old_0 = atomicAdd(work_counter, 8);
                        claim_base = _atomic_old_0;
                        claim_offset = 0;
                    }
                    int next_linear = claim_base + claim_offset;
                    claim_offset = claim_offset + 1;
                ...
                work_stage_8 += 1;
                if (work_stage_8 == 8) { work_stage_8 = 0; _phase_work_empty ^= 1; _phase_work_full_8 ^= 1; }
    ...
    if (warp == 15) {
        { // padding_main
            ...
            for (unsigned int _tile_iter_9 = 0; _tile_iter_9 < grid_m * grid_n; _tile_iter_9++) {
                ...
                {
                    mbarrier_wait_hint(work_full_addr + (work_stage_9) * 8, _phase_work_full_9, 1000);
                }
                ...
                work_stage_9 += 1;
                if (work_stage_9 == 8) { work_stage_9 = 0; _phase_work_full_9 ^= 1; }
