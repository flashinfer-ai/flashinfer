...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_kimi_k3_nvfp4_situ_routed_moe_2181a3eddddb148553eb'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
namespace cake_host_shim_46ab266544175edd {
...
void Run(TensorView arg_A, TensorView arg_B, TensorView arg_SFA, TensorView arg_SFB, TensorView arg_C_tma, TensorView arg_C, TensorView arg_scale_c, TensorView arg_tile_expert, TensorView arg_tile_mn_limit, TensorView arg_num_non_exiting_ctas, TensorView arg_work_counter, int64_t arg_M, int64_t arg_K, int64_t arg_grid_m, int64_t arg_grid_n, int64_t arg_K_tiles, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  void* kargs[] = {&p_A, &p_B, &p_SFA, &p_SFB, &p_C_tma, &p_C, &p_scale_c, &p_tile_expert, &p_tile_mn_limit, &p_num_non_exiting_ctas, &p_work_counter, &v_M, &v_K, &v_grid_m, &v_grid_n, &v_K_tiles};
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_kimi_k3_nvfp4_situ_routed_moe_2181a3eddddb148553eb),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_kimi_k3_nvfp4_situ_routed_moe_2181a3eddddb148553eb failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_kimi_k3_nvfp4_situ_routed_moe_2181a3eddddb148553eb), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_kimi_k3_nvfp4_situ_routed_moe_2181a3eddddb148553eb failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_46ab266544175edd

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_46ab266544175edd::Run);
