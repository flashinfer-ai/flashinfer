...
kernel_cake_kimi_k3_nvfp4_situ_routed_moe_d9854c0d763014b3e55f(__nv_bfloat16* __restrict__ expert_output, __nv_bfloat16* __restrict__ route_weights, int* __restrict__ token_to_permuted, __nv_bfloat16* __restrict__ out, int M)
{
    ...
    if (token < M) {
        int feature = (blockIdx.y * 128 + tid) * 8;
        if (feature < H) {
            float accum[8];
            accum[0] = 0.0f;
            accum[1] = 0.0f;
