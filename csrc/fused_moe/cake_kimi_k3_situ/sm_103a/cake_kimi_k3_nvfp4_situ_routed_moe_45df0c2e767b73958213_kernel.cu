...
kernel_cake_kimi_k3_nvfp4_situ_routed_moe_45df0c2e767b73958213(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap C_tma, __nv_bfloat16* __restrict__ C, float* __restrict__ scale_c, int* __restrict__ tile_expert, int* __restrict__ tile_mn_limit, int* __restrict__ num_non_exiting_ctas, int* __restrict__ work_counter, int M, int K, int grid_m, int grid_n, int K_tiles)
{
    ...
    if (warp == 8) {
        ...
        // Warp-cooperative initialization in physical record order.
        if (lane < 9) {
            mbarrier_init(smem + 72 + lane * 8, 1);
        }
        asm volatile("fence.mbarrier_init.release.cluster;" ::: "memory");
    }
    if (warp == 11) {
        ...
        // Warp-cooperative initialization in physical record order.
        if (lane < 9 || (lane >= 18 && lane < 27)) {
            mbarrier_init(smem + 144 + lane * 8, 1);
        }
        asm volatile("fence.mbarrier_init.release.cluster;" ::: "memory");
    }
    if (warp == 9) {
        ...
        // Warp-cooperative initialization in physical record order.
        if (lane < 9 || (lane >= 18 && lane < 27)) {
            mbarrier_init(smem + 216 + lane * 8, 1);
        }
        asm volatile("fence.mbarrier_init.release.cluster;" ::: "memory");
    }
    if (warp == 12) {
        ...
        // Warp-cooperative initialization in physical record order.
        if (lane < 9) {
            mbarrier_init(smem + 432 + lane * 8, 1);
        }
        asm volatile("fence.mbarrier_init.release.cluster;" ::: "memory");
    }
    if (warp == 4) {
        ...
        // Warp-cooperative initialization in physical record order.
        if (lane < 9) {
            mbarrier_init(smem + 504 + lane * 8, 1);
        }
        asm volatile("fence.mbarrier_init.release.cluster;" ::: "memory");
    }
    if (warp == 0) {
        ...
        // Warp-cooperative initialization in physical record order.
        if (lane < 9) {
            mbarrier_init(smem + 576 + lane * 8, 1);
        }
        asm volatile("fence.mbarrier_init.release.cluster;" ::: "memory");
    }
    if (warp == 1) {
        ...
        // Warp-cooperative initialization in physical record order.
        uint32_t _mbarrier_init_count_1_81 = 4;
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_1_81) : "r"(lane), "n"(2), "r"((uint32_t)(1)));
        if (lane < 4) {
            mbarrier_init(smem + 648 + lane * 8, _mbarrier_init_count_1_81);
        }
        asm volatile("fence.mbarrier_init.release.cluster;" ::: "memory");
    }
    if (warp == 14) {
        ...
        // Warp-cooperative initialization in physical record order.
        uint32_t _mbarrier_init_count_14_85 = 32;
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_14_85) : "r"(lane), "n"(6), "r"((uint32_t)(512)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_14_85) : "r"(lane), "n"(3), "r"((uint32_t)(1)));
        if (lane < 12) {
            mbarrier_init(smem + 680 + lane * 8, _mbarrier_init_count_14_85);
        }
        asm volatile("fence.mbarrier_init.release.cluster;" ::: "memory");
    }
    ...
    // TMEM alloc (256 columns, 232 used)
            ...
            for (unsigned int _tile_iter_1 = 0; _tile_iter_1 < grid_m * grid_n; _tile_iter_1++) {
                ...
                for (int _route_k = 0; _route_k < route_k_extent; _route_k++) {
                    ...
                    asm volatile(
                        ...
                        " [%0], {%1};"
                        :: "r"(taddr + 16 + 144 + stage * 8), "r"(word[0]));
                    const int q_0 = 1;
                    ...
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 16 + 144 + stage * 8 + 2), "r"(word[0]));
                    const int q_1 = 2;
                    ...
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 16 + 144 + stage * 8 + 4), "r"(word[0]));
                    const int q_2 = 3;
                    ...
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 16 + 144 + stage * 8 + 6), "r"(word[0]));
                ...
                for (int route_k_4 = 0; route_k_4 < route_k_extent_6; route_k_4++) {
                    ...
                    asm volatile("tcgen05.fence::after_thread_sync;");
                    {
                        ...
                        int sfb_base_col = 0;
                        {
                            ...
                            int _mma_b_lo_0 = make_warp_uniform((((smem_b_mma0_addr) >> 4) & 0x3FFF) + (k_stage) * 64);
                            if (elect_sync()) {
                                {
                                    ...
                                    tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 8)), a_desc + 0, b_desc + 0,
                                        0x8020480U, (unsigned int)tmem_sfa + (k_stage * 16 + (unsigned int)sfa_base_col) + 0, (unsigned int)tmem_sfb + (k_stage * 8 + (unsigned int)sfb_base_col) + 0, ((((1) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                                }
                            }
                        }
                    }
                    {
                        ...
                        int sfb_base_col_1 = 2;
                        {
                            ...
                            int _mma_b_lo_1 = make_warp_uniform((((smem_b_mma1_addr) >> 4) & 0x3FFF) + (k_stage) * 64);
                            if (elect_sync()) {
                                {
                                    ...
                                    tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 8)), a_desc + 0, b_desc + 0,
                                        0x8020480U, (unsigned int)tmem_sfa + (k_stage * 16 + (unsigned int)sfa_base_col_1) + 0, (unsigned int)tmem_sfb + (k_stage * 8 + (unsigned int)sfb_base_col_1) + 0, ((((0) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                                }
                            }
                        }
                    }
                    {
                        ...
                        if (!1 || route_k_4 == 0) {
                            ...
                            int _mma_b_lo_2 = make_warp_uniform((((smem_b_mma2_addr) >> 4) & 0x3FFF) + (k_stage) * 64);
                            if (elect_sync()) {
                                {
                                    ...
                                    tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 8)), a_desc + 0, b_desc + 0,
                                        0x8020480U, (unsigned int)tmem_sfa + (k_stage * 16 + (unsigned int)sfa_base_col_2) + 0, (unsigned int)tmem_sfb + (k_stage * 8 + (unsigned int)sfb_base_col_2) + 0, ((((0) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                                }
                            }
                        }
                    }
                    {
                        ...
                        if (!1 || route_k_4 == 0) {
                            ...
                            int _mma_b_lo_3 = make_warp_uniform((((smem_b_mma3_addr) >> 4) & 0x3FFF) + (k_stage) * 64);
                            if (elect_sync()) {
                                {
                                    ...
                                    tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 8)), a_desc + 0, b_desc + 0,
                                        0x8020480U, (unsigned int)tmem_sfa + (k_stage * 16 + (unsigned int)sfa_base_col_3) + 0, (unsigned int)tmem_sfb + (k_stage * 8 + (unsigned int)sfb_base_col_3) + 0, ((((0) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                                }
                            }
                        }
                    }
