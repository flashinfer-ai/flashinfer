...
kernel_cake_kimi_k3_nvfp4_situ_routed_moe_2a07a18376bc24431242(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap C_tma, __nv_bfloat16* __restrict__ C, float* __restrict__ scale_c, int* __restrict__ tile_expert, int* __restrict__ tile_mn_limit, int M, int K, int grid_m, int grid_n, int K_tiles, int* __restrict__ total_tiles)
{
    ...
    uint8_t* smem_sfa = reinterpret_cast<uint8_t*>(smem_raw + 166912);
    const int smem_sfa_addr = smem + 166912;
    uint8_t* smem_sfb = reinterpret_cast<uint8_t*>(smem_raw + 185344);
    const int smem_sfb_addr = smem + 185344;
    __nv_bfloat16* epi_staging = reinterpret_cast<__nv_bfloat16*>(smem_raw + 187648);
    const int epi_staging_addr = smem + 187648;
    unsigned long long* epi_staging_u64 = reinterpret_cast<unsigned long long*>(smem_raw + 187648);
    const int epi_staging_u64_addr = smem + 187648;
    unsigned int* work_response = reinterpret_cast<unsigned int*>(smem_raw + 191744);
    const int work_response_addr = smem + 191744;
    ...
    if (warp == 0) {
        ...
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_0_64) : "r"(lane), "n"(27), "r"((uint32_t)(512)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_0_64) : "r"(lane), "n"(24), "r"((uint32_t)(1)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_0_64) : "r"(lane), "n"(21), "r"((uint32_t)(4)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_0_64) : "r"(lane), "n"(19), "r"((uint32_t)(1)));
        mbarrier_init(smem + 512 + lane * 8, _mbarrier_init_count_0_64);
        if (lane < 1) {
            mbarrier_init(smem + 768 + lane * 8, 32);
        }
        ...
        { // epilogue_main
            ...
            for (unsigned int _tile_iter = 0; _tile_iter < grid_m * grid_n; _tile_iter++) {
                ...
                asm volatile("tcgen05.fence::after_thread_sync;");
                {
                    ...
                    tmem_ld_x16(&_tmem_load_2[0], taddr + (unsigned int)(row << 16) + acc_stage * 16);
                    asm volatile("tcgen05.wait::ld.sync.aligned;");
                    ...
                    for (int token = 0; token < 16; token++) {
                        ...
                        if (feature < 64) {
                            ...
                            epi_staging[1024 + token * 64 + feature - 64] = bf;
                        }
                    }
                }
                asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                ...
                if (warp == 0) {
                    if (elect_sync()) {
                        int padding_rows = (16 - _mn_limit % 16) % 16;
                        ...
                        tma_store_4d((&C_tma), off_m + 64, local_token, 1073741824, token_base - padding_rows + 1073741824, epi_staging_addr + 2048);
            ...
            for (unsigned int _tile_iter_1 = 0; _tile_iter_1 < grid_m * grid_n; _tile_iter_1++) {
                ...
                for (int _route_k = 0; _route_k < route_k_extent; _route_k++) {
                    ...
                    for (int reg = 0; reg < 1; reg++) {
                        word[reg] = 0;
                        if (lane_0 + reg * 32 < 16) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[reg])) : "r"(smem_sfb_addr + stage * 256 + (unsigned int)((reg * 4 + lane_0 / 8) * 128) + (unsigned int)(q * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 32 + 144 + stage * 8), "r"(word[0]));
                    const int q_0 = 1;
                    for (int reg_1 = 0; reg_1 < 1; reg_1++) {
                        word[reg_1] = 0;
                        if (lane_0 + reg_1 * 32 < 16) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[reg_1])) : "r"(smem_sfb_addr + stage * 256 + (unsigned int)((reg_1 * 4 + lane_0 / 8) * 128) + (unsigned int)(q_0 * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 32 + 144 + stage * 8 + 2), "r"(word[0]));
                    const int q_1 = 2;
                    for (int reg_2 = 0; reg_2 < 1; reg_2++) {
                        word[reg_2] = 0;
                        if (lane_0 + reg_2 * 32 < 16) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[reg_2])) : "r"(smem_sfb_addr + stage * 256 + (unsigned int)((reg_2 * 4 + lane_0 / 8) * 128) + (unsigned int)(q_1 * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 32 + 144 + stage * 8 + 4), "r"(word[0]));
                    const int q_2 = 3;
                    for (int reg_3 = 0; reg_3 < 1; reg_3++) {
                        word[reg_3] = 0;
                        if (lane_0 + reg_3 * 32 < 16) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[reg_3])) : "r"(smem_sfb_addr + stage * 256 + (unsigned int)((reg_3 * 4 + lane_0 / 8) * 128) + (unsigned int)(q_2 * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 32 + 144 + stage * 8 + 6), "r"(word[0]));
        ...
        { // load_b_main
            ...
            for (unsigned int _tile_iter_2 = 0; _tile_iter_2 < grid_m * grid_n; _tile_iter_2++) {
                ...
                for (int route_k = 0; route_k < route_k_extent_1; route_k++) {
                    ...
                    if (elect_sync()) {
                        tma_3d_gmem2smem(smem_b_addr + stage_1 * 2048, (&B), iter_k * 256, 0, route_tile, b_full_addr + (stage_1) * 8);
        ...
        { // load_sfb_main
            ...
            for (unsigned int _tile_iter_3 = 0; _tile_iter_3 < grid_m * grid_n; _tile_iter_3++) {
                ...
                for (int route_k_1 = 0; route_k_1 < route_k_extent_2; route_k_1++) {
                    ...
                    if (elect_sync()) {
                        tma_3d_gmem2smem(smem_sfb_addr + stage_2 * 256, (&SFB), 0, iter_k_1 * 4, route_tile_1 * 2, sfb_full_addr + (stage_2) * 8);
            ...
            for (unsigned int _tile_iter_6 = 0; _tile_iter_6 < grid_m * grid_n; _tile_iter_6++) {
                ...
                for (int _route_k_1 = 0; _route_k_1 < route_k_extent_5; _route_k_1++) {
                    ...
                    if (elect_sync()) {
                        ...
                        #endif
                        {
                            uint64_t _tcgen05_cp_desc_0 = ((((uint64_t)(smem_sfa_addr + stage_5 * 2048)) & 0x3FFFFULL) >> 4ULL) | (((((uint64_t)(0)) & 0x3FFFFULL) >> 4ULL) << 16ULL) | (((((uint64_t)(128)) & 0x3FFFFULL) >> 4ULL) << 32ULL) | (1ULL << 46ULL) | (0ULL << 61ULL);
                            asm volatile(
                        ...
                        #endif
                        {
                            uint64_t _tcgen05_cp_desc_1 = ((((uint64_t)(smem_sfa_addr + stage_5 * 2048 + 512)) & 0x3FFFFULL) >> 4ULL) | (((((uint64_t)(0)) & 0x3FFFFULL) >> 4ULL) << 16ULL) | (((((uint64_t)(128)) & 0x3FFFFULL) >> 4ULL) << 32ULL) | (1ULL << 46ULL) | (0ULL << 61ULL);
                            asm volatile(
                                "tcgen05.cp.cta_group::1.32x128b.warpx4 [%0], %1;"
                        ...
                        #endif
                        {
                            uint64_t _tcgen05_cp_desc_2 = ((((uint64_t)(smem_sfa_addr + stage_5 * 2048 + 1024)) & 0x3FFFFULL) >> 4ULL) | (((((uint64_t)(0)) & 0x3FFFFULL) >> 4ULL) << 16ULL) | (((((uint64_t)(128)) & 0x3FFFFULL) >> 4ULL) << 32ULL) | (1ULL << 46ULL) | (0ULL << 61ULL);
                            asm volatile(
                                "tcgen05.cp.cta_group::1.32x128b.warpx4 [%0], %1;"
                        ...
                        #endif
                        {
                            uint64_t _tcgen05_cp_desc_3 = ((((uint64_t)(smem_sfa_addr + stage_5 * 2048 + 1536)) & 0x3FFFFULL) >> 4ULL) | (((((uint64_t)(0)) & 0x3FFFFULL) >> 4ULL) << 16ULL) | (((((uint64_t)(128)) & 0x3FFFFULL) >> 4ULL) << 32ULL) | (1ULL << 46ULL) | (0ULL << 61ULL);
                            asm volatile(
                                "tcgen05.cp.cta_group::1.32x128b.warpx4 [%0], %1;"
        ...
        { // mma_main
            ...
            for (unsigned int _tile_iter_7 = 0; _tile_iter_7 < grid_m * grid_n; _tile_iter_7++) {
                ...
                for (int route_k_4 = 0; route_k_4 < route_k_extent_6; route_k_4++) {
                    ...
                    {
                        uint32_t _mbar_token_4 = mbarrier_try_wait(a_full_addr + (k_stage) * 8, k_phase);
                        a_token = _mbar_token_4;
                        uint32_t _mbar_token_5 = mbarrier_try_wait(b_full_addr + (k_stage) * 8, k_phase);
                        b_token = _mbar_token_5;
                        uint32_t _mbar_token_6 = mbarrier_try_wait(tmem_sfa_full_addr + (k_stage) * 8, k_phase);
                        sfa_token = _mbar_token_6;
                        uint32_t _mbar_token_7 = mbarrier_try_wait(tmem_sfb_full_addr + (k_stage) * 8, k_phase);
                            ...
                            if (elect_sync()) {
                                {
                                    ...
                                    tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 16)), a_desc + 0, b_desc + 0,
                                        0x8040480U, (unsigned int)tmem_sfa + (k_stage * 16 + (unsigned int)sfa_base_col) + 0, (unsigned int)tmem_sfb + (k_stage * 8 + (unsigned int)sfb_base_col) + 0, ((((1) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                                }
                            }
                        }
                    }
                            ...
                            if (elect_sync()) {
                                {
                                    ...
                                    tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 16)), a_desc + 0, b_desc + 0,
                                        0x8040480U, (unsigned int)tmem_sfa + (k_stage * 16 + (unsigned int)sfa_base_col_1) + 0, (unsigned int)tmem_sfb + (k_stage * 8 + (unsigned int)sfb_base_col_1) + 0, ((((0) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                                }
                            }
                        }
                    }
                            ...
                            if (elect_sync()) {
                                {
                                    ...
                                    tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 16)), a_desc + 0, b_desc + 0,
                                        0x8040480U, (unsigned int)tmem_sfa + (k_stage * 16 + (unsigned int)sfa_base_col_2) + 0, (unsigned int)tmem_sfb + (k_stage * 8 + (unsigned int)sfb_base_col_2) + 0, ((((0) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                                }
                            }
                        }
                    }
                            ...
                            if (elect_sync()) {
                                {
                                    ...
                                    tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 16)), a_desc + 0, b_desc + 0,
                                        0x8040480U, (unsigned int)tmem_sfa + (k_stage * 16 + (unsigned int)sfa_base_col_3) + 0, (unsigned int)tmem_sfb + (k_stage * 8 + (unsigned int)sfb_base_col_3) + 0, ((((0) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                                }
                            }
                        }
                    }
