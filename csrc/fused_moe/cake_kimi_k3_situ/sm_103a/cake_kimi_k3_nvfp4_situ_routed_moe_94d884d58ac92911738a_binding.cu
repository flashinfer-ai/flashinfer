...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_kimi_k3_nvfp4_situ_routed_moe_94d884d58ac92911738a'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
void Run(TensorView arg_A, TensorView arg_B, TensorView arg_SFA, TensorView arg_SFB, TensorView arg_C_tma, TensorView arg_C, TensorView arg_scale_c, TensorView arg_tile_expert, TensorView arg_tile_mn_limit, int64_t arg_M, int64_t arg_K, int64_t arg_grid_m, int64_t arg_grid_n, int64_t arg_K_tiles, TensorView arg_total_tiles, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_kimi_k3_nvfp4_situ_routed_moe_94d884d58ac92911738a),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_kimi_k3_nvfp4_situ_routed_moe_94d884d58ac92911738a failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_kimi_k3_nvfp4_situ_routed_moe_94d884d58ac92911738a), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_kimi_k3_nvfp4_situ_routed_moe_94d884d58ac92911738a failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_e41d4f8d06952c65

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_e41d4f8d06952c65::Run);
