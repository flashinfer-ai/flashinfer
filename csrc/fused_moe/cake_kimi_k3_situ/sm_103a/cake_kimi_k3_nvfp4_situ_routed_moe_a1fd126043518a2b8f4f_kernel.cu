...
#define SMEM_SMEM_B_MMA0_STAGE_BYTES 512
#define SMEM_SMEM_B_MMA0_STRIDE 4096
#define SMEM_SMEM_B_MMA1_OFF 164896
#define SMEM_SMEM_B_MMA1_STAGE_BYTES 512
#define SMEM_SMEM_B_MMA1_STRIDE 4096
#define SMEM_SMEM_B_MMA2_OFF 164928
#define SMEM_SMEM_B_MMA2_STAGE_BYTES 512
#define SMEM_SMEM_B_MMA2_STRIDE 4096
#define SMEM_SMEM_B_MMA3_OFF 164960
#define SMEM_SMEM_B_MMA3_STAGE_BYTES 512
#define SMEM_SMEM_B_MMA3_STRIDE 4096
#define SMEM_SMEM_B_MMA4_OFF 166912
#define SMEM_SMEM_B_MMA4_STAGE_BYTES 512
#define SMEM_SMEM_B_MMA4_STRIDE 4096
#define SMEM_SMEM_B_MMA5_OFF 166944
#define SMEM_SMEM_B_MMA5_STAGE_BYTES 512
#define SMEM_SMEM_B_MMA5_STRIDE 4096
#define SMEM_SMEM_B_MMA6_OFF 166976
#define SMEM_SMEM_B_MMA6_STAGE_BYTES 512
#define SMEM_SMEM_B_MMA6_STRIDE 4096
#define SMEM_SMEM_B_MMA7_OFF 167008
#define SMEM_SMEM_B_MMA7_STAGE_BYTES 512
...
#define SMEM_SMEM_B_MMA0_NEXT_STAGE_BYTES 512
...
#define SMEM_SMEM_B_MMA2_NEXT_STAGE_BYTES 512
...
#define SMEM_SMEM_B_MMA5_NEXT_STAGE_BYTES 512
...
#define SMEM_SMEM_SFB_OFF 205824
...
kernel_cake_kimi_k3_nvfp4_situ_routed_moe_a1fd126043518a2b8f4f(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap C_tma, __nv_bfloat16* __restrict__ C, float* __restrict__ scale_c, int* __restrict__ tile_expert, int* __restrict__ tile_mn_limit, int* __restrict__ num_non_exiting_ctas, int* __restrict__ work_counter, int M, int K, int grid_m, int grid_n, int K_tiles)
{
    ...
    uint8_t* smem_b_mma4 = reinterpret_cast<uint8_t*>(smem_raw + 166912);
    const int smem_b_mma4_addr = smem + 166912;
    uint8_t* smem_b_mma5 = reinterpret_cast<uint8_t*>(smem_raw + 166944);
    const int smem_b_mma5_addr = smem + 166944;
    uint8_t* smem_b_mma6 = reinterpret_cast<uint8_t*>(smem_raw + 166976);
    const int smem_b_mma6_addr = smem + 166976;
    uint8_t* smem_b_mma7 = reinterpret_cast<uint8_t*>(smem_raw + 167008);
    const int smem_b_mma7_addr = smem + 167008;
    ...
    uint8_t* smem_sfb = reinterpret_cast<uint8_t*>(smem_raw + 205824);
    const int smem_sfb_addr = smem + 205824;
    ...
    if (warp == 0) {
        ...
        if (leader) {
            ...
            mbarrier_init(smem + 544, 512);
            mbarrier_init(smem + 552, 512);
        ...
        { // epilogue_main
            ...
            for (unsigned int _tile_iter = 0; _tile_iter < grid_m * grid_n; _tile_iter++) {
                ...
                asm volatile("tcgen05.fence::after_thread_sync;");
                {
                    ...
                    {
                        int valid_tokens = 16 - (16 - _mn_limit % 16) % 16;
                        ...
                        if (token < valid_tokens) {
                            {
                                ...
                                _pk[0] = __floats2bfloat162_rn(wide_values[0 + 0], wide_values[0 + 1]);
                                _pk[1] = __floats2bfloat162_rn(wide_values[0 + 2], wide_values[0 + 3]);
                                *reinterpret_cast<uint2*>(&((__nv_bfloat16*)(C + (unsigned int)((token_base + token) * M + off_m + wide_feature)))[0]) = _pk2;
                            }
                        }
                        ...
                        if (token_0 < valid_tokens) {
                            {
                                uint2 _pk2;
                                __nv_bfloat162* _pk = reinterpret_cast<__nv_bfloat162*>(&_pk2);
                                _pk[0] = __floats2bfloat162_rn(wide_values[0 + 0], wide_values[0 + 1]);
                                _pk[1] = __floats2bfloat162_rn(wide_values[0 + 2], wide_values[0 + 3]);
                                *reinterpret_cast<uint2*>(&((__nv_bfloat16*)(C + (unsigned int)((token_base + token_0) * M + off_m + wide_feature)))[0]) = _pk2;
                            }
                        }
                        int token_1 = wide_token + 8;
                        wide_values[0] = _tmem_load_0[4] * output_scale;
                        wide_values[1] = _tmem_load_0[6] * output_scale;
                        wide_values[2] = _tmem_load_1[4] * output_scale;
                        wide_values[3] = _tmem_load_1[6] * output_scale;
                        if (token_1 < valid_tokens) {
                            {
                                uint2 _pk2;
                                __nv_bfloat162* _pk = reinterpret_cast<__nv_bfloat162*>(&_pk2);
                                _pk[0] = __floats2bfloat162_rn(wide_values[0 + 0], wide_values[0 + 1]);
                                _pk[1] = __floats2bfloat162_rn(wide_values[0 + 2], wide_values[0 + 3]);
                                *reinterpret_cast<uint2*>(&((__nv_bfloat16*)(C + (unsigned int)((token_base + token_1) * M + off_m + wide_feature)))[0]) = _pk2;
                            }
                        }
                        int token_2 = wide_token + 8 + 1;
                        wide_values[0] = _tmem_load_0[5] * output_scale;
                        wide_values[1] = _tmem_load_0[7] * output_scale;
                        wide_values[2] = _tmem_load_1[5] * output_scale;
                        wide_values[3] = _tmem_load_1[7] * output_scale;
                        if (token_2 < valid_tokens) {
                            {
                                uint2 _pk2;
                                __nv_bfloat162* _pk = reinterpret_cast<__nv_bfloat162*>(&_pk2);
                                _pk[0] = __floats2bfloat162_rn(wide_values[0 + 0], wide_values[0 + 1]);
                                _pk[1] = __floats2bfloat162_rn(wide_values[0 + 2], wide_values[0 + 3]);
                                *reinterpret_cast<uint2*>(&((__nv_bfloat16*)(C + (unsigned int)((token_base + token_2) * M + off_m + wide_feature)))[0]) = _pk2;
                            }
                        }
                    }
                }
            ...
            for (unsigned int _tile_iter_1 = 0; _tile_iter_1 < grid_m * grid_n; _tile_iter_1++) {
                ...
                for (int _route_k = 0; _route_k < route_k_extent; _route_k++) {
                    ...
                    for (int reg = 0; reg < 1; reg++) {
                        word[reg] = 0;
                        if (lane_0 + reg * 32 < 16) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[reg])) : "r"(smem_sfb_addr + stage * 512 + (unsigned int)((reg * 4 + lane_0 / 8) * 256) + (unsigned int)(q * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 32 + 160 + stage * 16), "r"(word[0]));
                    const int q_0 = 1;
                    for (int reg_1 = 0; reg_1 < 1; reg_1++) {
                        word[reg_1] = 0;
                        if (lane_0 + reg_1 * 32 < 16) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[reg_1])) : "r"(smem_sfb_addr + stage * 512 + (unsigned int)((reg_1 * 4 + lane_0 / 8) * 256) + (unsigned int)(q_0 * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 32 + 160 + stage * 16 + 2), "r"(word[0]));
                    const int q_1 = 2;
                    for (int reg_2 = 0; reg_2 < 1; reg_2++) {
                        word[reg_2] = 0;
                        if (lane_0 + reg_2 * 32 < 16) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[reg_2])) : "r"(smem_sfb_addr + stage * 512 + (unsigned int)((reg_2 * 4 + lane_0 / 8) * 256) + (unsigned int)(q_1 * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 32 + 160 + stage * 16 + 4), "r"(word[0]));
                    const int q_2 = 3;
                    for (int reg_3 = 0; reg_3 < 1; reg_3++) {
                        word[reg_3] = 0;
                        if (lane_0 + reg_3 * 32 < 16) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[reg_3])) : "r"(smem_sfb_addr + stage * 512 + (unsigned int)((reg_3 * 4 + lane_0 / 8) * 256) + (unsigned int)(q_2 * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 32 + 160 + stage * 16 + 6), "r"(word[0]));
                    const int q_3 = 4;
                    for (int reg_4 = 0; reg_4 < 1; reg_4++) {
                        word[reg_4] = 0;
                        if (lane_0 + reg_4 * 32 < 16) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[reg_4])) : "r"(smem_sfb_addr + stage * 512 + (unsigned int)((reg_4 * 4 + lane_0 / 8) * 256) + (unsigned int)(q_3 * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 32 + 160 + stage * 16 + 8), "r"(word[0]));
                    const int q_4 = 5;
                    for (int reg_5 = 0; reg_5 < 1; reg_5++) {
                        word[reg_5] = 0;
                        if (lane_0 + reg_5 * 32 < 16) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[reg_5])) : "r"(smem_sfb_addr + stage * 512 + (unsigned int)((reg_5 * 4 + lane_0 / 8) * 256) + (unsigned int)(q_4 * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 32 + 160 + stage * 16 + 10), "r"(word[0]));
                    const int q_5 = 6;
                    for (int reg_6 = 0; reg_6 < 1; reg_6++) {
                        word[reg_6] = 0;
                        if (lane_0 + reg_6 * 32 < 16) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[reg_6])) : "r"(smem_sfb_addr + stage * 512 + (unsigned int)((reg_6 * 4 + lane_0 / 8) * 256) + (unsigned int)(q_5 * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 32 + 160 + stage * 16 + 12), "r"(word[0]));
                    const int q_6 = 7;
                    for (int reg_7 = 0; reg_7 < 1; reg_7++) {
                        word[reg_7] = 0;
                        if (lane_0 + reg_7 * 32 < 16) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&word[reg_7])) : "r"(smem_sfb_addr + stage * 512 + (unsigned int)((reg_7 * 4 + lane_0 / 8) * 256) + (unsigned int)(q_6 * 32) + (unsigned int)(lane_0 % 8 * 4)));
                        }
                    }
                    asm volatile(
                        "tcgen05.st.sync.aligned.32x32b.x1.b32"
                        " [%0], {%1};"
                        :: "r"(taddr + 32 + 160 + stage * 16 + 14), "r"(word[0]));
        ...
        { // load_b_main
            ...
            for (unsigned int _tile_iter_2 = 0; _tile_iter_2 < grid_m * grid_n; _tile_iter_2++) {
                ...
                for (int route_k = 0; route_k < route_k_extent_1; route_k++) {
                    ...
                    if (elect_sync()) {
                        tma_3d_gmem2smem(smem_b_addr + stage_1 * 4096, (&B), iter_k * 512, 0, route_tile, b_full_addr + (stage_1) * 8);
                        tma_3d_gmem2smem(smem_b_addr + stage_1 * 4096 + 2048, (&B), iter_k * 512 + 256, 0, route_tile, b_full_addr + (stage_1) * 8);
        ...
        { // load_sfb_main
            ...
            for (unsigned int _tile_iter_3 = 0; _tile_iter_3 < grid_m * grid_n; _tile_iter_3++) {
                ...
                for (int route_k_1 = 0; route_k_1 < route_k_extent_2; route_k_1++) {
                    ...
                    if (elect_sync()) {
                        tma_3d_gmem2smem(smem_sfb_addr + stage_2 * 512, (&SFB), 0, iter_k_1 * 8, route_tile_1 * 2, sfb_full_addr + (stage_2) * 8);
        ...
        { // mma_main
            ...
            for (unsigned int _tile_iter_7 = 0; _tile_iter_7 < grid_m * grid_n; _tile_iter_7++) {
                ...
                for (int route_k_4 = 0; route_k_4 < route_k_extent_6; route_k_4++) {
                    ...
                    if (elect_sync()) {
                        {
                            ...
                            tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 16)), a_desc + 0, b_desc + 0,
                                0x8040480U, (unsigned int)tmem_sfa + (k_stage * 32 + (unsigned int)sfa_base_col) + 0, (unsigned int)tmem_sfb + (k_stage * 16 + (unsigned int)sfb_base_col) + 0, ((((1) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                        }
                    }
                    ...
                    if (elect_sync()) {
                        {
                            ...
                            tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 16)), a_desc + 0, b_desc + 0,
                                0x8040480U, (unsigned int)tmem_sfa + (k_stage * 32 + (unsigned int)sfa_base_col_0) + 0, (unsigned int)tmem_sfb + (k_stage * 16 + (unsigned int)sfb_base_col_1) + 0, ((((0) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                        }
                    }
                    ...
                    if (elect_sync()) {
                        {
                            ...
                            tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 16)), a_desc + 0, b_desc + 0,
                                0x8040480U, (unsigned int)tmem_sfa + (k_stage * 32 + (unsigned int)sfa_base_col_2) + 0, (unsigned int)tmem_sfb + (k_stage * 16 + (unsigned int)sfb_base_col_3) + 0, ((((0) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                        }
                    }
                    ...
                    if (elect_sync()) {
                        {
                            ...
                            tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 16)), a_desc + 0, b_desc + 0,
                                0x8040480U, (unsigned int)tmem_sfa + (k_stage * 32 + (unsigned int)sfa_base_col_4) + 0, (unsigned int)tmem_sfb + (k_stage * 16 + (unsigned int)sfb_base_col_5) + 0, ((((0) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                        }
                    }
                    int sfa_base_col_6 = 16;
                    int sfb_base_col_7 = 8;
                    int _mma_a_lo_4 = make_warp_uniform((((smem_a_mma4_addr) >> 4) & 0x3FFF) + (k_stage) * 2048);
                    int _mma_b_lo_4 = make_warp_uniform((((smem_b_mma4_addr) >> 4) & 0x3FFF) + (k_stage) * 256);
                    if (elect_sync()) {
                        {
                            ...
                            tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 16)), a_desc + 0, b_desc + 0,
                                0x8040480U, (unsigned int)tmem_sfa + (k_stage * 32 + (unsigned int)sfa_base_col_6) + 0, (unsigned int)tmem_sfb + (k_stage * 16 + (unsigned int)sfb_base_col_7) + 0, ((((0) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                        }
                    }
                    int sfa_base_col_8 = 20;
                    int sfb_base_col_9 = 10;
                    int _mma_a_lo_5 = make_warp_uniform((((smem_a_mma5_addr) >> 4) & 0x3FFF) + (k_stage) * 2048);
                    int _mma_b_lo_5 = make_warp_uniform((((smem_b_mma5_addr) >> 4) & 0x3FFF) + (k_stage) * 256);
                    if (elect_sync()) {
                        {
                            ...
                            tcgen05_mma_mxf4nvf4_bs((tmem_accum + ((acc_stage_1 + (unsigned int)route_slot) * 16)), a_desc + 0, b_desc + 0,
                                0x8040480U, (unsigned int)tmem_sfa + (k_stage * 32 + (unsigned int)sfa_base_col_8) + 0, (unsigned int)tmem_sfb + (k_stage * 16 + (unsigned int)sfb_base_col_9) + 0, ((((0) ? ((0) ? ((route_k_4 % K_tiles == 0) ? 1 : 0) : ((route_k_4 == 0) ? 1 : 0)) : 0)) ? 0 : 1));
                        }
                    }
                    if (elect_sync()) {
                    ...
                    {
                        k_stage += 1;
                        if (k_stage == 5) { k_stage = 0; _phase_a_full ^= 1; _phase_b_full ^= 1; _phase_tmem_sfa_full ^= 1; _phase_tmem_sfb_full ^= 1; }
                    }
                }
                acc_stage_1 += 1;
