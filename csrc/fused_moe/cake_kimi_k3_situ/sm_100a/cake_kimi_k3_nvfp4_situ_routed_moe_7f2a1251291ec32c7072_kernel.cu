...
kernel_cake_kimi_k3_nvfp4_situ_routed_moe_7f2a1251291ec32c7072(const __grid_constant__ CUtensorMap A, uint8_t* __restrict__ B, const __grid_constant__ CUtensorMap SFA, uint8_t* __restrict__ SFB, const __grid_constant__ CUtensorMap C, uint8_t* __restrict__ SFC, int* __restrict__ route_map, int* __restrict__ tile_expert, int* __restrict__ tile_mn_limit, float* __restrict__ scale_c, float* __restrict__ scale_gate, float* __restrict__ clamp_limit, float* __restrict__ act_alpha, float* __restrict__ act_beta, int M_out, int K, int grid_m, int grid_n, int K_tiles, int* __restrict__ total_tiles)
{
    ...
    if (warp <= 3) {
        ...
        { // epilogue_main
            ...
            for (int token_group = 0; token_group < 1; token_group++) {
                ...
                if (lane_1 < 8) {
                    int sf_odd = lane_1 >> 2 & 1;
                    int sf_token = token0 + sf_odd;
                    float sf_value = scale0;
                    if (sf_odd != 0) {
                        sf_value = scale1;
                    }
                    ...
                    if (sf_token < valid_rows) {
                        {
                            ...
                            asm("cvt.rn.satfinite.e4m3x2.f32 %0, 0f00000000, %1;" : "=h"(_fp8_pair) : "f"(sf_value));
    ...
    if (warp == 14) {
        { // mma_main
            ...
            unsigned int ready_phase = 0;
            #pragma unroll 1
            for (int iter_k_4 = 0; iter_k_4 < K_tiles; iter_k_4++) {
                uint32_t _mbar_token_0 = mbarrier_try_wait(a_full_addr + (stage_6) * 8, ready_phase);
                uint32_t _mbar_token_1 = mbarrier_try_wait(b_full_addr + (stage_6) * 8, ready_phase);
                uint32_t _mbar_token_2 = mbarrier_try_wait(tmem_sfa_full_addr + (stage_6) * 8, ready_phase);
                uint32_t _mbar_token_3 = mbarrier_try_wait(tmem_sfb_full_addr + (stage_6) * 8, ready_phase);
                mbarrier_wait_token(a_full_addr + (stage_6) * 8, ready_phase, _mbar_token_0);
                mbarrier_wait_token(b_full_addr + (stage_6) * 8, ready_phase, _mbar_token_1);
                mbarrier_wait_token(tmem_sfa_full_addr + (stage_6) * 8, ready_phase, _mbar_token_2);
                mbarrier_wait_token_hint(tmem_sfb_full_addr + (stage_6) * 8, ready_phase, _mbar_token_3, 10000000);
                asm volatile("tcgen05.fence::after_thread_sync;");
                ...
                stage_6 += 1;
                if (stage_6 == 5) { stage_6 = 0; ready_phase ^= 1; }
            }
        }
    }
    // ---- Role: padding ----
