...
kernel_cake_kimi_k3_nvfp4_situ_routed_moe_f381f4e96597e3db04c2(__nv_bfloat16* __restrict__ x, float* __restrict__ qx, uint8_t* __restrict__ packed, uint8_t* __restrict__ scales, int* __restrict__ topk_ids, int* __restrict__ expert_counts, int* __restrict__ expert_tile_offsets, int* __restrict__ expert_scatter_offsets, int* __restrict__ route_map, int* __restrict__ token_to_permuted, int* __restrict__ tile_expert, int* __restrict__ tile_mn_limit, int* __restrict__ total_tiles, int M)
{
    ...
    for (int slot = 0; slot < 16; slot++) {
        ...
        #pragma unroll
        for (int expert_chunk = 0; expert_chunk < 4; expert_chunk++) {
            int dense_expert = tid + expert_chunk * 256;
            if (peer_expert == dense_expert) {
                dense_counts[expert_chunk] = dense_counts[expert_chunk] + 1;
            }
        }
    }
    int tile_begin = 0;
    ...
    for (int slot_1 = 0; slot_1 < 16; slot_1++) {
        ...
        #pragma unroll
        for (int expert_chunk_1 = 0; expert_chunk_1 < 4; expert_chunk_1++) {
            int dense_expert_1 = tid + expert_chunk_1 * 256;
            if (prefix_expert < dense_expert_1) {
                dense_offsets[expert_chunk_1] = dense_offsets[expert_chunk_1] + prefix_tiles;
            }
        }
    }
    #pragma unroll
    for (int expert_chunk_2 = 0; expert_chunk_2 < 4; expert_chunk_2++) {
        int dense_expert_2 = tid + expert_chunk_2 * 256;
        if (dense_expert_2 < 896) {
            expert_counts[dense_expert_2] = dense_counts[expert_chunk_2];
            expert_scatter_offsets[dense_expert_2] = dense_counts[expert_chunk_2];
            expert_tile_offsets[dense_expert_2] = dense_offsets[expert_chunk_2];
        }
    }
    if (tid == 0) {
