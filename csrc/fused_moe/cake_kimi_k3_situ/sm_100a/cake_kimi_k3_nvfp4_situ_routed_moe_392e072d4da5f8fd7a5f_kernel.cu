...
kernel_cake_kimi_k3_nvfp4_situ_routed_moe_392e072d4da5f8fd7a5f(__nv_bfloat16* __restrict__ expert_output, __nv_bfloat16* __restrict__ route_weights, int* __restrict__ token_to_permuted, __nv_bfloat16* __restrict__ out, int M)
{
    ...
    if (token < M) {
        int feature = blockIdx.y * 128 * 2 + tid * 2;
        if (feature < H) {
            float accum[2];
            accum[0] = 0.0f;
            accum[1] = 0.0f;
            ...
            for (int slot_group = 0; slot_group < 4; slot_group++) {
                ...
                for (int group_slot = 0; group_slot < 4; group_slot++) {
                    ...
                    float _vec_load_2[2];
                    {
                        ...
                        _bf16x2_bits_2 = *reinterpret_cast<const uint32_t*>(expert_output + (unsigned int)(row * H + feature) + 0);
                        asm volatile(
                            "{\n\t"
                            "shl.b32 %0, %2, 16;\n\t"
                            "and.b32 %1, %2, 0xffff0000;\n\t"
            ...
            }
            {
                ...
                *reinterpret_cast<__nv_bfloat162*>(&((__nv_bfloat16*)(out + (token * H + feature)))[0]) = _pk;
            }
        }
    }
}
