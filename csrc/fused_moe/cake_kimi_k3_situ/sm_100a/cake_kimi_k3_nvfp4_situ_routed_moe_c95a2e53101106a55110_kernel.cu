...
kernel_cake_kimi_k3_nvfp4_situ_routed_moe_c95a2e53101106a55110(const __grid_constant__ CUtensorMap A, uint8_t* __restrict__ B, const __grid_constant__ CUtensorMap SFA, uint8_t* __restrict__ SFB, const __grid_constant__ CUtensorMap C, uint8_t* __restrict__ SFC, int* __restrict__ route_map, int* __restrict__ tile_expert, int* __restrict__ tile_mn_limit, float* __restrict__ scale_c, float* __restrict__ scale_gate, float* __restrict__ clamp_limit, float* __restrict__ act_alpha, float* __restrict__ act_beta, int M_out, int K, int grid_m, int grid_n, int K_tiles, int* __restrict__ total_tiles)
{
    ...
    if (warp == 0) {
        ...
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_0_0) : "r"(lane), "n"(30), "r"((uint32_t)(128)));
    ...
    if (warp <= 3) {
        ...
        { // epilogue_main
            ...
            mbarrier_wait_hint(mma_full_addr, _phase_mma_full_0, 10000000);
            _phase_mma_full_0 ^= 1;
            ...
            asm volatile(
                ...
                : "=&r"(*reinterpret_cast<uint32_t*>(&lo[0])), "=&r"(*reinterpret_cast<uint32_t*>(&lo[1])), "=&r"(*reinterpret_cast<uint32_t*>(&lo[2])), "=&r"(*reinterpret_cast<uint32_t*>(&lo[3])), "=&r"(*reinterpret_cast<uint32_t*>(&lo[4])), "=&r"(*reinterpret_cast<uint32_t*>(&lo[5])), "=&r"(*reinterpret_cast<uint32_t*>(&lo[6])), "=&r"(*reinterpret_cast<uint32_t*>(&lo[7])), "=&r"(*reinterpret_cast<uint32_t*>(&hi[0])), "=&r"(*reinterpret_cast<uint32_t*>(&hi[1])), "=&r"(*reinterpret_cast<uint32_t*>(&hi[2])), "=&r"(*reinterpret_cast<uint32_t*>(&hi[3])), "=&r"(*reinterpret_cast<uint32_t*>(&hi[4])), "=&r"(*reinterpret_cast<uint32_t*>(&hi[5])), "=&r"(*reinterpret_cast<uint32_t*>(&hi[6])), "=&r"(*reinterpret_cast<uint32_t*>(&hi[7]))
                : "r"(taddr), "r"(taddr + 1048576)
                : "memory");
            asm volatile("tcgen05.wait::ld.sync.aligned;" ::: "memory");
            int base_row = warp_0 * 16 + lane_1 / 4 * 2;
            asm volatile("cp.async.bulk.wait_group.read 0;");
            asm volatile("barrier.sync 7, 128;" ::: "memory");
            int sf_owner_group = lane_1 / 8;
            int sf_odd = lane_1 / 4 % 2;
            float sf_selected_scale = 0.0f;
            for (int token_group = 0; token_group < 2; token_group++) {
                ...
                float value11 = 0.0f;
                {
                    float up00 = lo[token_group * 4];
                    float up01 = lo[token_group * 4 + 1];
                    float gate00 = lo[token_group * 4 + 2];
                    float gate01 = lo[token_group * 4 + 3];
                    float up10 = hi[token_group * 4];
                    float up11 = hi[token_group * 4 + 1];
                    float gate10 = hi[token_group * 4 + 2];
                    float gate11 = hi[token_group * 4 + 3];
                    ...
                    float2 one2 = _f2_5;
                    float2 _f2_6 = make_float2(up00, up10);
                    float2 up0 = _f2_6;
                    float2 _f2_7 = make_float2(up01, up11);
                    float2 up1 = _f2_7;
                    float2 _f2_8 = make_float2(gate00, gate10);
                    float2 gate0 = _f2_8;
                    float2 _f2_9 = make_float2(gate01, gate11);
                    float2 gate1 = _f2_9;
                    float2 _mul_f32x2_0;
                ...
                if (sf_owner_group == token_group) {
                    sf_selected_scale = scale0;
                    if (sf_odd != 0) {
                        sf_selected_scale = scale1;
                    }
                }
            ...
            if (lane_1 < 16) {
                int sf_token = sf_owner_group * 8 + lane_1 % 4 * 2 + sf_odd;
                int sf_feature = m_tile * 4 + warp_0;
                int sf_token_group = sf_token / 8;
                int sf_tile_stride = 2 * (M_out / 64) * 32;
                int sf_base = n_tile * sf_tile_stride + sf_token_group * (M_out / 64) * 32 + sf_feature / 4 * 32 + sf_token % 8 * 4 + sf_feature % 4;
                if (sf_token < valid_rows) {
                    {
                        unsigned short _fp8_pair;
                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, 0f00000000, %1;" : "=h"(_fp8_pair) : "f"(sf_selected_scale));
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // copy_sfb_main
            ...
            for (int _iter_k = 0; _iter_k < K_tiles; _iter_k++) {
                mbarrier_wait_hint(sfb_full_addr + (stage) * 8, _phase_sfb_full, 10000000);
    ...
    if (warp == 14) {
        { // mma_main
            ...
            for (int iter_k_4 = 0; iter_k_4 < K_tiles; iter_k_4++) {
                ...
                mbarrier_wait_hint(tmem_sfa_full_addr + (stage_6) * 8, _phase_tmem_sfa_full, 10000000);
