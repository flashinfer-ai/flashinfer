...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_kimi_k3_nvfp4_situ_routed_moe_f3c5044b4d59e9b4ff77'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
inline CUtensorMap EncodeTma_SFB(const TensorView& t) {
  ...
  TVM_FFI_CHECK(32u <= global_dim[0] && 4u <= global_dim[2], ValueError)
      << "TMA box (32, 4, 4) exceeds resolved global dims for 'SFB'";
...
void Run(TensorView arg_A, TensorView arg_B, TensorView arg_SFA, TensorView arg_SFB, TensorView arg_C_tma, TensorView arg_C, TensorView arg_scale_c, TensorView arg_tile_expert, TensorView arg_tile_mn_limit, int64_t arg_M, int64_t arg_K, int64_t arg_grid_m, int64_t arg_grid_n, int64_t arg_K_tiles, TensorView arg_total_tiles, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_kimi_k3_nvfp4_situ_routed_moe_f3c5044b4d59e9b4ff77),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_kimi_k3_nvfp4_situ_routed_moe_f3c5044b4d59e9b4ff77 failed: "
  ...
  config.dynamicSmemBytes = 216704u;
  config.stream = stream;
  config.attrs = attrs;
  config.numAttrs = n;
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_kimi_k3_nvfp4_situ_routed_moe_f3c5044b4d59e9b4ff77), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_kimi_k3_nvfp4_situ_routed_moe_f3c5044b4d59e9b4ff77 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_b85da92525a2f9fa

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_b85da92525a2f9fa::Run);
