...
#define SMEM_WORK_RESPONSE_OFF 210432
#define SMEM_WORK_RESPONSE_STAGE_BYTES 16
#define SMEM_WORK_RESPONSE_STRIDE 16
#define SMEM_TOTAL 210560
#define THREADS 512
#define BLOCK_M 128
#define BLOCK_N 16
#define BLOCK_K 512
...
kernel_cake_kimi_k3_nvfp4_situ_routed_moe_1dc27deff743b34ea395(const __grid_constant__ CUtensorMap A, uint8_t* __restrict__ B, const __grid_constant__ CUtensorMap SFA, uint8_t* __restrict__ SFB, const __grid_constant__ CUtensorMap C, uint8_t* __restrict__ SFC, int* __restrict__ route_map, int* __restrict__ tile_expert, int* __restrict__ tile_mn_limit, int* __restrict__ total_tiles, float* __restrict__ scale_c, float* __restrict__ scale_gate, float* __restrict__ clamp_limit, float* __restrict__ act_alpha, float* __restrict__ act_beta, int M_out, int K, int grid_m, int grid_n, int K_tiles)
{
    ...
    unsigned int* work_response = reinterpret_cast<unsigned int*>(smem_raw + 210432);
    const int work_response_addr = smem + 210432;
                ...
                for (int _iter_k = 0; _iter_k < K_tiles; _iter_k++) {
                    ...
                    for (int q = 0; q < 8; q++) {
                        ...
                        asm volatile(
                            ...
                            " [%0], {%1};"
                            :: "r"(taddr + 32 + 160 + stage * 16 + (unsigned int)(q * 2)), "r"(word[0]));
                    }
            ...
            for (unsigned int _tile_iter_2 = 0; _tile_iter_2 < grid_m * grid_n; _tile_iter_2++) {
                ...
                for (int iter_k = 0; iter_k < K_tiles; iter_k++) {
                    ...
                    for (int row_group = 0; row_group < 2; row_group++) {
                        ...
                        int routed = 0;
                        routed = route_map[n_tile_2 * 16 + (unsigned int)row];
            ...
            for (unsigned int _tile_iter_3 = 0; _tile_iter_3 < grid_m * grid_n; _tile_iter_3++) {
                ...
                for (int iter_k_1 = 0; iter_k_1 < K_tiles; iter_k_1++) {
                    ...
                    for (int row_group_1 = 0; row_group_1 < 4; row_group_1++) {
                        ...
                        int routed_1 = 0;
                        routed_1 = route_map[n_tile_3 * 16 + (unsigned int)row_1];
