...
void invokeNoAuxTc(InputT* scores, BiasT* bias, OutputT* topk_values, IdxT* topk_indices, int64_t const num_tokens,
    ...
    double const routed_scaling_factor, bool const launch_with_pdl, cudaStream_t const stream)
{
...
#define INSTANTIATE_NOAUX_TC(InputT, BiasT, OutputT, IdxT)                                                             \
    template void invokeNoAuxTc<InputT, BiasT, OutputT, IdxT>(InputT * scores, BiasT * bias, OutputT * topk_values,    \
        ...
        int64_t const topk_group, int64_t const topk, double const routed_scaling_factor, bool const launch_with_pdl, cudaStream_t const stream);
...
namespace flashinfer::trtllm_dsv3_fused_routing {
    ...
    void NoAuxTc(TensorView scores, TensorView bias, int64_t n_group, int64_t topk_group, int64_t topk, double routed_scaling_factor, TensorView topk_values, TensorView topk_indices, bool launch_with_pdl){
        ...
        TVM_FFI_ICHECK((scores.device().device_type == kDLCUDA) && (bias.device().device_type == kDLCUDA)) << "scores and bias must be CUDA tensors";
        TVM_FFI_ICHECK(scores.device().device_id == bias.device().device_id) << "scores and bias must be on the same device";
        TVM_FFI_ICHECK(bias.dim() == 1 && bias.numel() == num_experts) << "bias must be 1D with length == number of experts (%ld)";
        TVM_FFI_ICHECK(num_experts % n_group == 0) << "num_experts should be divisible by n_group";
        TVM_FFI_ICHECK(n_group <= 32) << "n_group should be smaller than or equal to 32 for now"; //@todo: remove this restriction later
        TVM_FFI_ICHECK(topk <= 32) << "topk should be smaller than or equal to 32 for now";       //@todo: remove this restriction later
        std::cout << "topk_values: " << topk_values.dim() << std::endl;
        ...
        TVM_FFI_ICHECK(topk_values.sizes()[0] == num_tokens) << "topk_values must have the same number of tokens as scores";
        TVM_FFI_ICHECK(topk_indices.sizes()[0] == num_tokens) << "topk_indices must have the same number of tokens as scores";
        TVM_FFI_ICHECK(topk_values.sizes()[1] == topk) << "topk_values must have the same number of topk as scores";
        TVM_FFI_ICHECK(topk_indices.sizes()[1] == topk) << "topk_indices must have the same number of topk as scores";
        TVM_FFI_ICHECK(topk_values.dtype() == data_type) << "topk_values must have the same dtype as scores";
        TVM_FFI_ICHECK(encode_dlpack_dtype(topk_indices.dtype()) == int32_code) << "topk_indices must have the same dtype as scores";
        ...
        case float16_code:
            ...
            {
            case float16_code:
                invokeNoAuxTc<half, half, half, int32_t>(reinterpret_cast<half*>(scores.data_ptr()),
                ...
                break;
            case float32_code:
                invokeNoAuxTc<half, float, half, int32_t>(reinterpret_cast<half*>(scores.data_ptr()),
                ...
                break;
            case bfloat16_code:
                invokeNoAuxTc<half, __nv_bfloat16, half, int32_t>(reinterpret_cast<half*>(scores.data_ptr()),
        ...
        case float32_code:
            ...
            {
            case float32_code:
                invokeNoAuxTc<float, float, float, int32_t>(reinterpret_cast<float*>(scores.data_ptr()),
                    reinterpret_cast<float*>(bias.data_ptr()),
                ...
                break;
            case float16_code:
                invokeNoAuxTc<float, half, float, int32_t>(reinterpret_cast<float*>(scores.data_ptr()),
                    reinterpret_cast<half*>(bias.data_ptr()),
                    reinterpret_cast<float*>(topk_values.data_ptr()),
                    reinterpret_cast<int32_t*>(topk_indices.data_ptr()), num_tokens, num_experts, n_group,
                    topk_group, topk, routed_scaling_factor, launch_with_pdl, stream);
                break;
            case bfloat16_code:
                invokeNoAuxTc<float, __nv_bfloat16, float, int32_t>(reinterpret_cast<float*>(scores.data_ptr()),
                    reinterpret_cast<__nv_bfloat16*>(bias.data_ptr()),
                    reinterpret_cast<float*>(topk_values.data_ptr()),
                    reinterpret_cast<int32_t*>(topk_indices.data_ptr()), num_tokens, num_experts, n_group,
                    topk_group, topk, routed_scaling_factor, launch_with_pdl, stream);
    ...
    TVM_FFI_DLL_EXPORT_TYPED_FUNC(NoAuxTc,
        flashinfer::trtllm_dsv3_fused_routing::NoAuxTc);
} // namespace flashinfer::trtllm_dsv3_fused_routing