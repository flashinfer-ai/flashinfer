...
cudaError_t launch(ScoreT* scores, BiasT* bias, ScoreT* topk_values,
                   int32_t* topk_indices, int16_t* routing_replay_out,
                   int64_t num_tokens, int64_t num_experts, int64_t n_group,
                   int64_t topk_group, int64_t topk,
                   double routed_scaling_factor, bool launch_with_pdl,
                   cudaStream_t stream);
...
#define FLASHINFER_DEFINE_CAKE_DEEPSEEK_ROUTING_LAUNCH(ScoreT, BiasT, ScoreTag, BiasTag)       \
  ...
  inline cudaError_t launch<ScoreT, BiasT>(                                                     \
    ...
    config.blockDim = dim3(                                                                     \
        static_cast<uint32_t>(n_group > 1 ? 256 : (num_experts <= 128 ? 128 : 384)), 1, 1);    \
    ...
    if (n_group > 1) {                                                                          \
      return cudaLaunchKernelEx(                                                                \
          &config, kernel_cake_deepseek_routing_grouped_##ScoreTag##_##BiasTag, scores, bias,   \
          topk_values, topk_indices, replay_bytes, tokens32, experts32, topk32, groups32,       \
          top_groups32, scaling32, has_replay);                                                 \
    }                                                                                            \
    if (num_experts <= 128) {                                                                   \
      return cudaLaunchKernelEx(                                                                \
          &config, kernel_cake_deepseek_routing_single128_##ScoreTag##_##BiasTag, scores, bias, \
          topk_values, topk_indices, replay_bytes, tokens32, experts32, topk32, groups32,       \
          top_groups32, scaling32, has_replay);                                                 \
    }                                                                                            \
    return cudaLaunchKernelEx(                                                                  \
        &config, kernel_cake_deepseek_routing_single384_##ScoreTag##_##BiasTag, scores, bias,   \
        topk_values, topk_indices, replay_bytes, tokens32, experts32, topk32, groups32,         \
        top_groups32, scaling32, has_replay);                                                   \
  }
