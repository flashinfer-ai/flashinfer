...
__global__ void B12xDirectPackRoutesKernel(const int32_t* __restrict__ topk_ids,
                                           int32_t* __restrict__ packed_route_indices,
                                           int32_t* __restrict__ block_expert_ids,
                                           int32_t* __restrict__ packed_route_count,
                                           int32_t routed_rows, int32_t num_experts,
                                           int32_t block_size) {
  __shared__ int32_t route_ids[64];
  __shared__ int32_t present[64];
  const int32_t tid = static_cast<int32_t>(threadIdx.x);
  if (tid < routed_rows) {
    route_ids[tid] = topk_ids[tid];
  }
  __syncthreads();
  ...
  if (tid < num_experts) {
#pragma unroll 1
    for (int32_t route = 0; route < routed_rows; ++route) {
      matches += route_ids[route] == tid;
    }
    present[tid] = matches != 0;
  }
  __syncthreads();

  if (tid < num_experts && matches != 0) {
    int32_t block = 0;
#pragma unroll 1
    for (int32_t expert = 0; expert < tid; ++expert) {
      block += present[expert];
    }
    block_expert_ids[block] = tid;
    int32_t out = block * block_size;
    const int32_t end = out + block_size;
#pragma unroll 1
    for (int32_t route = 0; route < routed_rows; ++route) {
      if (route_ids[route] == tid) {
        packed_route_indices[out++] = route;
      }
    }
    while (out < end) {
...
#pragma unroll 1
    for (int32_t expert = 0; expert < num_experts; ++expert) {
      blocks += present[expert];
    }
    packed_route_count[0] = blocks * block_size;
  }
}

}  // namespace

void B12xDirectPackRoutes(TensorView topk_ids, TensorView packed_route_indices,
                          TensorView block_expert_ids, TensorView packed_route_count,
                          int64_t num_experts, int64_t block_size, int64_t stream_int) {
  CHECK_INPUT_AND_TYPE(topk_ids, dl_int32);
  CHECK_INPUT_AND_TYPE(packed_route_indices, dl_int32);
  CHECK_INPUT_AND_TYPE(block_expert_ids, dl_int32);
  CHECK_INPUT_AND_TYPE(packed_route_count, dl_int32);
  CHECK_DIM(2, topk_ids);
  CHECK_DIM(1, packed_route_indices);
  CHECK_DIM(1, block_expert_ids);
  CHECK_DIM(1, packed_route_count);
  CHECK_DEVICE(packed_route_indices, topk_ids);
  CHECK_DEVICE(block_expert_ids, topk_ids);
  CHECK_DEVICE(packed_route_count, topk_ids);
  const int64_t routed_rows = topk_ids.numel();
  TVM_FFI_ICHECK(routed_rows >= 1 && routed_rows <= 64);
  TVM_FFI_ICHECK(num_experts >= 1 && num_experts <= 64);
  TVM_FFI_ICHECK_EQ(block_size, 8);
  const int64_t max_active_experts = std::min(num_experts, routed_rows);
  ...
  B12xDirectPackRoutesKernel<<<1, 64, 0, stream>>>(
      ...
      static_cast<int32_t*>(packed_route_indices.data_ptr()),
      static_cast<int32_t*>(block_expert_ids.data_ptr()),
      static_cast<int32_t*>(packed_route_count.data_ptr()), static_cast<int32_t>(routed_rows),
      static_cast<int32_t>(num_experts), static_cast<int32_t>(block_size));
  const cudaError_t status = cudaGetLastError();
  TVM_FFI_ICHECK(status == cudaSuccess)
      << "sm120 direct route-pack launch failed: " << cudaGetErrorString(status);
}

void B12xDirectW4A16FusedMoe(TensorView hidden_states, TensorView topk_ids, TensorView topk_weights,
                             TensorView gemm1_weights, TensorView gemm1_scales,
                             TensorView gemm2_weights, TensorView gemm2_scales,
                             TensorView expert_map, TensorView intermediate, TensorView output,
  ...
  status = LaunchB12xDirectW4A16FusedMoe(params, get_stream(hidden_states.device()));
  TVM_FFI_ICHECK(status == cudaSuccess)
      << "b12x_direct_w4a16_fused_moe kernel launch failed: " << cudaGetErrorString(status);
}
