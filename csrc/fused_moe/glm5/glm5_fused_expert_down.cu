...
__device__ __forceinline__ float4 rmsNormBf16x8(float4 const& residual, float4 const& weight,
                                                float inv_rms) {
  float4 out;
#pragma unroll
  for (int i = 0; i < kArElemsPerAccess; ++i) {
...
__device__ __forceinline__ void load_packed_w_down_tile(
  ...
  for (int byte_off = lane * kCopyBytes; byte_off < kFp8BytesPerStage;
       byte_off += kWarpSize * kCopyBytes) {
...
// -----------------------------------------------------------------------------
__device__ __forceinline__ void mma_m16n8k16_f16(uint32_t a0, uint32_t a1, uint32_t a2, uint32_t a3,
...
// -----------------------------------------------------------------------------
__device__ __forceinline__ void cvt_fp8_to_afrag_direct(uint32_t fp8_stage_smem, int ki, int lane,
  ...
  const uint32_t row_lo_base =
      fp8_stage_smem + (uint32_t)(row_lo * (int)kWtKChunk + ki_phys_lo * (int)kMmaK);
  const uint32_t row_hi_base =
      fp8_stage_smem + (uint32_t)(row_hi * (int)kWtKChunk + ki_phys_hi * (int)kMmaK);
...
__global__ __launch_bounds__(kThreadsPerCta, 1) void dsv3_fused_expert_down_kernel(
    __grid_constant__ const CUtensorMap routed_w_down_map,
    __grid_constant__ const CUtensorMap shared_w_down_map,
    __nv_fp8_e4m3 const* __restrict__ routed_w_down_packed,
    __nv_fp8_e4m3 const* __restrict__ shared_w_down_packed,
    ...
    __half const* __restrict__ hidden_in_raw, int32_t const* __restrict__ indices,
    ...
    __nv_bfloat16* __restrict__ hidden_out, float* __restrict__ rms_sums, void** workspace,
    int rank, int nranks, int M, float rms_norm_eps) {
  ...
  if (tid == 0) {
    ...
    int const hidden_prefetch_lines =
        static_cast<int>((hidden_bytes + kPrefetchBytes - 1) / kPrefetchBytes);
  ...
  const size_t partial_base = hidden_bytes + sizeof(int32_t) * (size_t)M * kTopKPlusShared +
...
#pragma unroll
        ...
        float const* const s8_base_pkb =
            (nb8_pkb == nb0_pkb) ? s0_base_pkb
                                 : (shared_w_down_scale + (size_t)nb8_pkb * (size_t)k_blocks);

#pragma unroll
        for (int kb = 0; kb < k_blocks; ++kb) {
          int const stage =
              kUsePackedWeights ? (kb & 1) : ((kFp8Stages == 1) ? 0 : (kb % kFp8Stages));
...
#pragma unroll
          for (int ki = 0; ki < kKItersPerBlock; ++ki) {
            ...
            uint32_t a_frag[4];
            cvt_fp8_to_afrag_direct(fp8_stage_ptr, ki, lane, a_frag[0], a_frag[1], a_frag[2],
                                    a_frag[3]);
            ...
            mma_m16n8k16_f16(a_frag[0], a_frag[1], a_frag[2], a_frag[3], b_frag[0], b_frag[1],
...

        {
          ...
          if (col0 < M && row0 < rows_active) smem_part[slot_off + row0 * kMaxM + col0] = c[0];
          if (col1 < M && row0 < rows_active) smem_part[slot_off + row0 * kMaxM + col1] = c[1];
          if (col0 < M && row1 < rows_active) smem_part[slot_off + row1 * kMaxM + col0] = c[2];
          if (col1 < M && row1 < rows_active) smem_part[slot_off + row1 * kMaxM + col1] = c[3];
        }
      }
    }
  }
...
#pragma unroll
        ...
        float const* const s0_base_pkbR =
            routed_w_down_scale +
            ((size_t)e_id * (kHiddenSize / kBlockN) + nb0_pkbR) * (size_t)k_blocks;
        float const* const s8_base_pkbR =
            (nb8_pkbR == nb0_pkbR)
                ? s0_base_pkbR
                : (routed_w_down_scale +
                   ((size_t)e_id * (kHiddenSize / kBlockN) + nb8_pkbR) * (size_t)k_blocks);
...
#pragma unroll
          for (int ki = 0; ki < kKItersPerBlock; ++ki) {
            ...
            uint32_t b_frag[2];
            {
              ...
              uint32_t b_addr = lane_has_pair ? (smem_hidden_addr + b_row_base_bytes + k_off_bytes)
                                              : warp_mini_addr;
              ldmatrix_x2_b16(b_addr, b_frag[0], b_frag[1]);
            }
...

        {
          ...
          auto write_cell = [&](int p, int row_in_tile, float val) {
            if (p < count && row_in_tile < rows_active) {
              ...
              const int off = s * kRowTilesPerCta * kMmaM * kMaxM + tile * kMmaM * kMaxM +
                              row_in_tile * kMaxM + m;
              smem_part[off] = val;
            }
          };
...
#pragma unroll
    for (int s = 0; s < kTopKPlusShared; ++s) {
      ...
      int const off0 =
          s * kRowTilesPerCta * kMmaM * kMaxM + tile0 * kMmaM * kMaxM + rit0 * kMaxM + m;
      int const off1 =
          s * kRowTilesPerCta * kMmaM * kMaxM + tile1 * kMmaM * kMaxM + rit1 * kMaxM + m;
...
#pragma unroll
  ...
  if constexpr (kEnableArResidual) {
    ...
    comm.update(static_cast<int64_t>(M) * static_cast<int64_t>(kHiddenSize) *
                static_cast<int64_t>(nranks));
    ...
    if constexpr (kEnableRmsNorm) {
      ...
      if (has_ar_idx) {
        ...
        float const inv_rms =
            rsqrtf(rms_sums[token] / static_cast<float>(kHiddenSize) + rms_norm_eps);
        hidden_out_vec[linear_tid] =
            rmsNormBf16x8(residual_sum, norm_weight_vec[hidden_access], inv_rms);
      }
    }
#else
...
static CUtensorMap make_w_down_tmap(void* base_ptr, int num_experts, int K_local,
                                    CUresult* out_err) {
  CUtensorMap map = {};
...
struct WDownTmaDescKey {
  ...
  bool operator==(WDownTmaDescKey const& o) const noexcept {
    return base == o.base && numExperts == o.numExperts && kLocal == o.kLocal &&
           deviceId == o.deviceId;
  }
};
...
static CUtensorMap get_cached_w_down_tmap(void* base_ptr, int num_experts, int K_local,
...
void Glm5FusedExpertDown(TensorView hidden_in, TensorView indices, TensorView scores,
                         TensorView routed_w_down, TensorView routed_w_down_scale,
                         TensorView shared_w_down, TensorView shared_w_down_scale,
                         TensorView output) {
  ...
  TVM_FFI_ICHECK(M >= 1 && M <= kMaxM)
      << "GLM5 fused expert-down supports 1 <= num_tokens <= " << kMaxM << ", got " << M;
  TVM_FFI_ICHECK(hidden_in.size(1) == kTopKPlusShared) << "hidden_in must have shape [M, 9, I]";
  ...
  TVM_FFI_ICHECK(K_local == 256 || K_local == 512)
      << "local intermediate size must be 256 (TP8) or 512 (TP4), got " << K_local;
  ...
  if (use_packed_weights) {
    TVM_FFI_ICHECK(routed_w_down.ndim() == 4 && routed_w_down.size(0) == kSharedExpertIdx &&
  ...
  if (!use_packed_weights) {
    ...
    routed_w_down_map = get_cached_w_down_tmap(routed_w_down.data_ptr(), kSharedExpertIdx, K_local,
                                               device_id, &routed_tma_err);
  ...
  auto compute_smem = [&](int stages, int m_for_smem, bool packed_weights) -> size_t {
    ...
    size_t tables_bytes = sizeof(int32_t) * static_cast<size_t>(m_for_smem) * kTopKPlusShared +
                          sizeof(float) * static_cast<size_t>(m_for_smem) * kTopKPlusShared;
    size_t partial_bytes =
        sizeof(float) * static_cast<size_t>(kTopKPlusShared) * kRowTilesPerCta * kMmaM * kMaxM;
    ...
    fp8_base = (fp8_base + 1023) & ~size_t(1023);
    size_t fp8_bytes = static_cast<size_t>(kNumWarps) * stages * kFp8BytesPerStage;
    return fp8_base + fp8_bytes;
  };

  size_t const smem_bytes = compute_smem(chosen_stages, M, use_packed_weights);
  size_t const max_smem_bytes = compute_smem(chosen_stages, kMaxM, use_packed_weights);
  ...
  using KernelFn =
      void (*)(const CUtensorMap, const CUtensorMap, __nv_fp8_e4m3 const*, __nv_fp8_e4m3 const*,
               __half const*, int32_t const*, float const*, float const*, float const*,
               __nv_bfloat16*, __nv_bfloat16 const*, __nv_bfloat16*, __nv_bfloat16 const*,
               __nv_bfloat16*, float*, void**, int, int, int, float);

  KernelFn kernel = nullptr;
  if (K_local == 512)
    kernel = use_packed_weights ? &dsv3_fused_expert_down_kernel<512, true, false, false>
                                : &dsv3_fused_expert_down_kernel<512, false, false, false>;
  else
    kernel = use_packed_weights ? &dsv3_fused_expert_down_kernel<256, true, false, false>
  ...
  std::once_flag& smem_flag =
      K_local == 512 ? (use_packed_weights ? smem_512_packed[device_id] : smem_512[device_id])
  ...
  std::call_once(smem_flag, [&]() {
    if (max_smem_bytes > 48 * 1024) {
      ...
      TVM_FFI_ICHECK(err == cudaSuccess)
          << "cudaFuncSetAttribute for GLM5 fused expert-down failed: " << cudaGetErrorString(err);
    }
  });

  __nv_fp8_e4m3 const* routed_packed_ptr =
      use_packed_weights ? reinterpret_cast<__nv_fp8_e4m3 const*>(routed_w_down.data_ptr())
                         : nullptr;
  __nv_fp8_e4m3 const* shared_packed_ptr =
      use_packed_weights ? reinterpret_cast<__nv_fp8_e4m3 const*>(shared_w_down.data_ptr())
                         : nullptr;
  ...
  float const* routed_scale_ptr = static_cast<float const*>(routed_w_down_scale.data_ptr());
  float const* shared_scale_ptr = static_cast<float const*>(shared_w_down_scale.data_ptr());
  ...
  cudaError_t launch_err = cudaLaunchKernel(reinterpret_cast<void const*>(kernel), grid, block,
  ...
  TVM_FFI_ICHECK(launch_err == cudaSuccess)
      << "GLM5 fused expert-down reported a CUDA error: " << cudaGetErrorString(launch_err);
}
