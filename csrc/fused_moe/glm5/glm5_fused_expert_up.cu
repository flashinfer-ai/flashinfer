/*
 * SPDX-FileCopyrightText: Copyright (c) 1993-2026 NVIDIA CORPORATION & AFFILIATES. All rights
...
constexpr int kMtileSubslabBytes =
    kKSubsPerThird * kKsubBytes;  // 2048 B per m_tile within a k_sixth
...
constexpr int kCombinedSubslabBytes =
    kCombinedMtilesPerCta * kMtileSubslabBytes;  // 8 * 2048 = 16384 B
constexpr int kCombinedTileBytes =
    kWeightScaleKBlocksPerKIter * kCombinedSubslabBytes;  // 98304 (96 KiB)
...
__device__ __forceinline__ void lds64_b32x2(uint32_t& r0, uint32_t& r1,
...
__device__ __forceinline__ void cp_async_bulk_tensor_3d(void* smem_dst, CUtensorMap const* tmap,
                                                        int32_t coord_x, int32_t coord_y,
                                                        int32_t coord_z, uint64_t* mbar) {
...
// -------------------------------------------------------------------------
__device__ __forceinline__ int fused_expert_up_combined_lane_offset(int m_tile, int k_sub,
                                                                    int lane) {
  int const k_third = k_sub / kKSubsPerThird;         // 0..5
  int const k_sub_in_third = k_sub % kKSubsPerThird;  // 0..3
  return k_third * kCombinedSubslabBytes + m_tile * kMtileSubslabBytes +
...
//
__device__ __forceinline__ void compute_mma_kiter_fused_expert_up(
    __nv_fp8_e4m3 const* __restrict__ smem_tile, __nv_fp8_e4m3 const* __restrict__ smem_act_fp8,
    int k_iter, int my_m, int lane, float const (&gate_block_scale)[kWeightScaleKBlocksPerKIter],
...
#pragma unroll
    for (int ks_in_kb = 0; ks_in_kb < kKSubsPerThird; ++ks_in_kb) {
      ...
      __nv_fp8_e4m3 const* lane_base =
          smem_tile + fused_expert_up_combined_lane_offset(my_m, k_sub, lane);
...
template <int kInterPerTpParam>
__device__ __forceinline__ void issue_packed_weight_tma_fused_expert_up(
    __nv_fp8_e4m3* __restrict__ smem_tile, CUtensorMap const* expert_gate_up_tma, int expert,
    int sub_row, int k_iter, uint64_t* mbar) {
...
__device__ __forceinline__ void quant_act_blocks_fused_expert_up(
    cg::thread_block_tile<kWarpSize> warp, __nv_bfloat16 const* __restrict__ smem_act_bf16,
    __nv_fp8_e4m3* __restrict__ smem_act_fp8, float* __restrict__ smem_act_block_scales, int q_warp,
    int lane) {
...
template <int kInterPerTpParam>
__global__
__launch_bounds__(384, DSV3_FUSED_EXPERT_UP_LB_BLOCKS_PER_SM) void dsv3_fused_expert_up_kernel(
    __grid_constant__ const CUtensorMap expert_gate_up_tma, float const* __restrict__ scores,
    __nv_bfloat16 const* __restrict__ hidden_in, __nv_bfloat16 const* __restrict__ bias,
    __nv_fp8_e4m3 const* __restrict__ expert_gate_up_weight,
...
#pragma unroll
    for (int ii = 0; ii < kActCpAsyncsPerThread; ++ii) {
      ...
      __nv_bfloat16* dst =
          reinterpret_cast<__nv_bfloat16*>(reinterpret_cast<char*>(smem_act_bf16) + byte_off);
...
#pragma unroll
    ...
    mega_topk::reduceTopK<kTopK, float, kMaxNumTopGroups>(warp, top_scores, top_experts, in_value,
                                                          in_idx, kInvalidScore, kTopK);
    if (lane < kTopK) {
    ...
    if (warp_idx == 0) {
      ...
      mega_topk::reduceTopK<kTopK, float>(warp, top_scores, top_experts, cand_val, cand_idx,
                                          kInvalidScore, kTopK);
...
#pragma unroll
  ...
  bool const is_worker =
      (warp_idx >= kWorkerWarpBase && warp_idx < (kWorkerWarpBase + kNumWorkers));
  ...
  if (tidx < kWeightScaleKBlocks) {
    ...
    float const* gate_weight_scale_base =
        expert_gate_up_scale +
        (static_cast<int64_t>(packed_expert) * 2 * kWeightScaleMBlocks + m_block_idx) *
            kWeightScaleKBlocks;
    float const* up_weight_scale_base =
        expert_gate_up_scale + (static_cast<int64_t>(packed_expert) * 2 * kWeightScaleMBlocks +
                                kWeightScaleMBlocks + m_block_idx) *
                                   kWeightScaleKBlocks;
...
#pragma unroll
      for (int kb = 0; kb < kWeightScaleKBlocksPerKIter; ++kb) {
        ...
        gate_block_scales[kb] =
            smem_gate_weight_scales[scale_idx] * smem_act_block_scales[scale_idx];
      ...
      compute_mma_kiter_fused_expert_up(smem_weight_tiles + current_stage * kCombinedTileBytes,
                                        smem_act_fp8, k, my_m, lane, gate_block_scales,
                                        up_block_scales, d_pair);
      if (lane == 0) {
...
static CUtensorMap make_packed_fused_expert_up_tmap(void* base_ptr, int num_tiles,
                                                    CUresult* out_err) {
  CUtensorMap map = {};
  ...
  *out_err =
      cuTensorMapEncodeTiled(&map, CU_TENSOR_MAP_DATA_TYPE_UINT8,
...
struct PackedFusedExpertUpTmaDescCache {
  ...
  std::unordered_map<PackedFusedExpertUpTmaDescKey, ListIt, PackedFusedExpertUpTmaDescKeyHash>
      index;
};

static CUtensorMap get_cached_packed_fused_expert_up_tmap(void* base_ptr, int num_tiles,
                                                          int device_id, CUresult* out_err) {
...
static void glm5_fused_expert_up_impl(TensorView scores, TensorView hidden_in, TensorView bias,
                                      TensorView expert_gate_up_weight,
                                      TensorView expert_gate_up_scale, TensorView topk_weights,
                                      TensorView topk_indices, TensorView hidden_out,
  ...
  std::call_once(smem_attribute_once[device_id], [&]() {
    cudaError_t err = cudaFuncSetAttribute(dsv3_fused_expert_up_kernel<kInterPerTpParam>,
                                           cudaFuncAttributeMaxDynamicSharedMemorySize,
                                           static_cast<int>(smem_bytes));
    TVM_FFI_ICHECK(err == cudaSuccess)
        << "cudaFuncSetAttribute for GLM5 fused expert-up failed: " << cudaGetErrorString(err);
  });

  dsv3_fused_expert_up_kernel<kInterPerTpParam><<<grid, block, smem_bytes, stream>>>(
      expert_gate_up_tma, static_cast<float const*>(scores.data_ptr()),
      ...
      static_cast<float*>(topk_weights.data_ptr()), static_cast<int32_t*>(topk_indices.data_ptr()),
...
void Glm5FusedExpertUp(TensorView scores, TensorView hidden_in, TensorView bias,
                       TensorView expert_gate_up_weight, TensorView expert_gate_up_scale,
                       TensorView topk_weights, TensorView topk_indices, TensorView hidden_out,
                       double routed_scaling_factor) {
  ...
  TVM_FFI_ICHECK(scores.size(1) == kNumExperts) << "scores must have shape [M, 256]";
  TVM_FFI_ICHECK(hidden_in.size(0) == M && hidden_in.size(1) == kHidden)
      << "hidden_in must have shape [M, 6144]";
  TVM_FFI_ICHECK(bias.size(0) == kNumExperts) << "bias must have shape [256]";
  ...
  TVM_FFI_ICHECK(inter_per_tp == kInterPerTp_TP8 || inter_per_tp == kInterPerTp_TP4)
      << "local intermediate size must be 256 (TP8) or 512 (TP4), got " << inter_per_tp;
  ...
  if (inter_per_tp == kInterPerTp_TP8) {
    return glm5_fused_expert_up_impl<kInterPerTp_TP8>(
        scores, hidden_in, bias, expert_gate_up_weight, expert_gate_up_scale, topk_weights,
        topk_indices, hidden_out, routed_scaling_factor);
  }

  return glm5_fused_expert_up_impl<kInterPerTp_TP4>(
      scores, hidden_in, bias, expert_gate_up_weight, expert_gate_up_scale, topk_weights,
      topk_indices, hidden_out, routed_scaling_factor);
}
