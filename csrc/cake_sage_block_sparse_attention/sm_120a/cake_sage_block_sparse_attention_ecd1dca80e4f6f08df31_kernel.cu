...
__global__
__launch_bounds__(128, 3) void kernel_cake_sage_block_sparse_attention_ecd1dca80e4f6f08df31(
        ...
        {
          {
            int _min_2 =
                ((valid_tokens) <
                         (block_sizes[(batch * num_heads + head) * num_k_blocks + current_physical])
                     ? (valid_tokens)
                     : (block_sizes[(batch * num_heads + head) * num_k_blocks + current_physical]));
            valid_tokens = _min_2;
          }
