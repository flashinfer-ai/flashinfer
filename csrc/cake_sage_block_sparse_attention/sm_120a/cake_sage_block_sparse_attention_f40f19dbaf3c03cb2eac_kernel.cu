...
kernel_cake_sage_block_sparse_attention_f40f19dbaf3c03cb2eac(CakeTensorMap const* Q_map, CakeTensorMap const* K_map, CakeTensorMap const* V_map, CakeTensorMap const* O_map, float* __restrict__ q_scale, float* __restrict__ k_scale, float* __restrict__ v_scale, int* __restrict__ q2k_block_index, int* __restrict__ q2k_block_nums, int* __restrict__ block_sizes, int seqlen_q, int seqlen_k, int num_heads, int q2k_capacity, int num_k_blocks, int block_sparse_num, float softmax_scale)
{
    ...
    for (int load_count = 0; load_count < selected_count; load_count++) {
        ...
        int valid_tokens = 64;
        {
            ...
            }
            {
                {
                    {
                        int _min_2 = ((valid_tokens) < (block_sizes[(batch * num_heads + head) * num_k_blocks + current_physical]) ? (valid_tokens) : (block_sizes[(batch * num_heads + head) * num_k_blocks + current_physical]));
                        valid_tokens = _min_2;
                    }
                }
            }
