...
kernel_deepgemm_mega_mhc_sm100a_5d32655fe23e975d07b9(const __grid_constant__ CUtensorMap residual_map, const __grid_constant__ CUtensorMap x_map, const __grid_constant__ CUtensorMap fn_map, const __grid_constant__ CUtensorMap post_map, const __grid_constant__ CUtensorMap comb_map, const __grid_constant__ CUtensorMap prev_map, const __grid_constant__ CUtensorMap new_residual_map, const __grid_constant__ CUtensorMap y_map, float* __restrict__ mix_scales, float* __restrict__ mix_bases, float* __restrict__ new_prev_mix, float* __restrict__ new_post_mix, float* __restrict__ new_comb_res_mix, __nv_bfloat16* __restrict__ rmsnorm_weight, __nv_bfloat16* __restrict__ new_residual, __nv_bfloat16* __restrict__ y_bf16, uint8_t* __restrict__ y_fp8, unsigned int* __restrict__ y_primary_sf, unsigned int* __restrict__ y_shared_sf, float* __restrict__ scratch, unsigned long long* __restrict__ split_barriers, unsigned long long* __restrict__ launch_epochs, unsigned int num_tokens, float hc_norm_eps, float hc_pre_eps, float hc_post_scale, float sinkhorn_eps, unsigned int num_sinkhorn_iters, float rmsnorm_eps, float rmsnorm_scale, unsigned long long primary_sf_stride_token, unsigned long long primary_sf_stride_word, unsigned long long shared_sf_stride_word)
{
    ...
    if (warp == 2) {
        ...
        if (leader) {
            ...
            // empty_stats: 1 barriers, init_count=4
            mbarrier_init(smem + 226952, 4);
                    ...
                    for (int kb_2 = 0; kb_2 < 2 + ((task_2 % 27 < 26) ? 1 : 0); kb_2++) {
                        ...
                        if (elect_sync()) {
                            ...
                            asm volatile(
                                ...
                                " [%0, {%1, %2, %3}], [%4], %5;"
                                :: "l"((&new_residual_map)), "r"(((unsigned int)(task_2 % 27 * 2) + extra_1) * 64 + (unsigned int)(kb_2 * 64)), "r"(mb * 64), "r"(0), "r"(residual_addr + stage_2 * 32768), "l"(0x14F0000000000000ULL) : "memory");
    ...
    for (int attempt = 0; attempt < num_tokens + 1; attempt++) {
        if (warp % 4 == 0) {
            ...
            ticket = _shfl_0;
            token = (unsigned int)bid + ticket * 148;
            if (lane_in_warp == 0) {
        ...
        if (token < num_tokens) {
            ...
            for (int pack_1 = 0; pack_1 < 5; pack_1++) {
                ...
                for (int j_7 = 0; j_7 < 4; j_7++) {
                    ...
                    asm volatile("mul.rn.bf16x2 %0, %1, %2;" : "=r"(_bf16x2_mul_0) : "r"(normalized_words[j_7]), "r"(sf_inv_pair));
                    quantized[2 * j_7] = __uint_as_float(_bf16x2_mul_0 << 16);
                    quantized[2 * j_7 + 1] = __uint_as_float(_bf16x2_mul_0 & 4294901760);
                }
                {
