...
kernel_deepgemm_mega_mhc_sm100a_639e4564284b29a53256(const __grid_constant__ CUtensorMap residual_map, const __grid_constant__ CUtensorMap x_map, const __grid_constant__ CUtensorMap fn_map, const __grid_constant__ CUtensorMap post_map, const __grid_constant__ CUtensorMap comb_map, const __grid_constant__ CUtensorMap prev_map, const __grid_constant__ CUtensorMap new_residual_map, const __grid_constant__ CUtensorMap y_map, float* __restrict__ mix_scales, float* __restrict__ mix_bases, float* __restrict__ new_prev_mix, float* __restrict__ new_post_mix, float* __restrict__ new_comb_res_mix, __nv_bfloat16* __restrict__ rmsnorm_weight, __nv_bfloat16* __restrict__ new_residual, __nv_bfloat16* __restrict__ y_bf16, uint8_t* __restrict__ y_fp8, unsigned int* __restrict__ y_primary_sf, unsigned int* __restrict__ y_shared_sf, float* __restrict__ scratch, unsigned long long* __restrict__ split_barriers, unsigned long long* __restrict__ launch_epochs, unsigned int num_tokens, float hc_norm_eps, float hc_pre_eps, float hc_post_scale, float sinkhorn_eps, unsigned int num_sinkhorn_iters, float rmsnorm_eps, float rmsnorm_scale, unsigned long long primary_sf_stride_token, unsigned long long primary_sf_stride_word, unsigned long long shared_sf_stride_word)
{
    ...
    if (warp == 2) {
        ...
        if (leader) {
            ...
            // full_stats: 1 barriers, init_count=8
            mbarrier_init(smem + 226944, 8);
            // empty_stats: 1 barriers, init_count=5
            mbarrier_init(smem + 226952, 5);
    ...
    if (warp <= 3) {
        { // control_main
            ...
            if (warp < 2) {
                ...
                if (elect_sync()) {
                    #pragma unroll 1
                    for (int task = bid; task < (num_tokens + 63) / 64 * 27; task += 148) {
                        #pragma unroll 1
                        for (int kb = 0; kb < 2 + ((task % 27 < 26) ? 1 : 0); kb++) {
                            unsigned int extra = ((task % 27 < 26) ? (unsigned int)(task % 27) : (unsigned int)26);
                            unsigned int hidden = ((unsigned int)(task % 27 * 2) + extra) * 64 + (unsigned int)(kb * 64);
                            if (warp == 0) {
                                unsigned int stage = seq % 4;
                                mbarrier_wait(empty_io_addr + (stage) * 8, seq / 4 % 2 ^ 1);
                                tma_3d_gmem2smem(residual_addr + stage * 32768, (&residual_map), hidden, task / 27 * 64, 0, full_io_addr + (stage) * 8);
                                tma_2d_gmem2smem(x_addr + stage * 8192, (&x_map), hidden, task / 27 * 64, full_io_addr + (stage) * 8);
                ...
                unsigned int cseq = 0;
                #pragma unroll 1
                for (int task_1 = bid; task_1 < (num_tokens + 63) / 64 * 27; task_1 += 148) {
                    unsigned int cstage = cseq % 2;
                    mbarrier_wait(empty_accum_addr + (cstage) * 8, cseq / 2 % 2 ^ 1);
                    asm volatile("tcgen05.fence::after_thread_sync;");
                    #pragma unroll 1
                    for (int kb_1 = 0; kb_1 < 2 + ((task_1 % 27 < 26) ? 1 : 0); kb_1++) {
                ...
                if ((unsigned int)bid < (num_tokens + 63) / 64 * 27) {
                    mbarrier_wait(empty_coeff_addr, 1);
                    if (elect_sync()) {
                        tma_2d_gmem2smem(post_addr, (&post_map), 0, bid / 27 * 64, full_coeff_addr);
                        tma_2d_gmem2smem(comb_addr, (&comb_map), 0, bid / 27 * 64, full_coeff_addr);
                        tma_2d_gmem2smem(pre_addr, (&prev_map), 0, bid / 27 * 64, full_coeff_addr);
                ...
                #pragma unroll 1
                for (int task_2 = bid; task_2 < (num_tokens + 63) / 64 * 27; task_2 += 148) {
                    unsigned int mb = task_2 / 27;
                    ...
                    } while (static_cast<unsigned long long>(_acquire_observed - static_cast<unsigned long long>(base)) >= static_cast<unsigned long long>(27));
                    }
                    #pragma unroll 1
                    for (int kb_2 = 0; kb_2 < 2 + ((task_2 % 27 < 26) ? 1 : 0); kb_2++) {
                        unsigned int stage_2 = io_seq % 4;
                        mbarrier_wait(store_ready_addr + (stage_2) * 8, io_seq / 4 % 2);
                        unsigned int prefetch = kb_2 + 1 == 2 + ((task_2 % 27 < 26) ? 1 : 0) && (unsigned int)(task_2 + 148) < (num_tokens + 63) / 64 * 27;
                        if (prefetch != 0) {
                            mbarrier_wait(empty_coeff_addr + (coeff_seq % 2) * 8, coeff_seq / 2 % 2 ^ 1);
                        }
                        if (elect_sync()) {
                            unsigned int extra_1 = ((task_2 % 27 < 26) ? (unsigned int)(task_2 % 27) : (unsigned int)26);
                            asm volatile(
                                "cp.async.bulk.tensor.3d.global.shared::cta.tile.bulk_group.L2::cache_hint"
                                " [%0, {%1, %2, %3}], [%4], %5;"
                                :: "l"((&new_residual_map)), "r"(((unsigned int)(task_2 % 27 * 2) + extra_1) * 64 + (unsigned int)(kb_2 * 64)), "r"(mb * 64), "r"(0), "r"(residual_addr + stage_2 * 32768), "l"(0x1000000000000000ULL) : "memory");
                            unsigned int extra_0 = ((task_2 % 27 < 26) ? (unsigned int)(task_2 % 27) : (unsigned int)26);
                            asm volatile(
                                "cp.async.bulk.tensor.2d.global.shared::cta.tile.bulk_group.L2::cache_hint"
                                " [%0, {%1, %2}], [%3], %4;"
                                :: "l"((&y_map)), "r"(((unsigned int)(task_2 % 27 * 2) + extra_0) * 64 + (unsigned int)(kb_2 * 64)), "r"(mb * 64), "r"(x_addr + stage_2 * 8192), "l"(0x14F0000000000000ULL) : "memory");
                            asm volatile("cp.async.bulk.commit_group;");
                            if (prefetch != 0) {
                                tma_2d_gmem2smem(post_addr + coeff_seq % 2 * 1024, (&post_map), 0, (task_2 + 148) / 27 * 64, full_coeff_addr + (coeff_seq % 2) * 8);
                                tma_2d_gmem2smem(comb_addr + coeff_seq % 2 * 4096, (&comb_map), 0, (task_2 + 148) / 27 * 64, full_coeff_addr + (coeff_seq % 2) * 8);
                                tma_2d_gmem2smem(pre_addr + coeff_seq % 2 * 1024, (&prev_map), 0, (task_2 + 148) / 27 * 64, full_coeff_addr + (coeff_seq % 2) * 8);
                                mbarrier_arrive_expect_tx(full_coeff_addr + (coeff_seq % 2) * 8, 6144);
                            }
                            if (kb_2 + 1 < 2 + ((task_2 % 27 < 26) ? 1 : 0)) {
                    ...
                    scratch[(unsigned long long)((num_tokens + 63) / 64 * 27) * 1536 + ((1) ? (unsigned long long)((num_tokens + 63) / 64 * 27) * 64 : (unsigned long long)0) + (unsigned long long)task_2 * 64 + (unsigned long long)lane_in_warp] = x1_sums[lane_in_warp] + x1_sums[64 + lane_in_warp];
                    scratch[(unsigned long long)((num_tokens + 63) / 64 * 27) * 1536 + ((1) ? (unsigned long long)((num_tokens + 63) / 64 * 27) * 64 : (unsigned long long)0) + (unsigned long long)task_2 * 64 + (unsigned long long)lane_in_warp + 32] = x1_sums[lane_in_warp + 32] + x1_sums[96 + lane_in_warp];
                    __syncwarp();
                    if (elect_sync()) {
    ...
    if (warp >= 4 && warp <= 7) {
        { // post0_main
            ...
            #pragma unroll 1
            for (int task_3 = bid; task_3 < (num_tokens + 63) / 64 * 27; task_3 += 148) {
                ...
                #pragma unroll 1
                for (int kb_3 = 0; kb_3 < 2 + ((task_3 % 27 < 26) ? 1 : 0); kb_3++) {
    ...
    if (warp >= 8 && warp <= 11) {
        { // post1_main
            ...
            #pragma unroll 1
            for (int task_4 = bid; task_4 < (num_tokens + 63) / 64 * 27; task_4 += 148) {
                ...
                #pragma unroll 1
                for (int kb_4 = 0; kb_4 < 2 + ((task_4 % 27 < 26) ? 1 : 0); kb_4++) {
    ...
    if (warp >= 12 && warp <= 15) {
        { // workspace_main
            ...
            #pragma unroll 1
            for (int task_5 = bid; task_5 < (num_tokens + 63) / 64 * 27; task_5 += 148) {
                unsigned int mb_1 = task_5 / 27;
                if (warp % 4 == 0) {
                    ...
                    } while (static_cast<unsigned long long>(_acquire_observed - static_cast<unsigned long long>(base_1)) >= static_cast<unsigned long long>(27));
                    }
                }
                ...
                if (lane_in_warp < 16) {
                    ...
                    scratch[(unsigned long long)((num_tokens + 63) / 64 * 27) * 1536 + ((0) ? (unsigned long long)((num_tokens + 63) / 64 * 27) * 64 : (unsigned long long)0) + (unsigned long long)task_5 * 64 + (unsigned long long)row_9] = hc_sums[row_9] + hc_sums[64 + row_9];
                }
                __syncwarp();
                if (elect_sync()) {
                    mbarrier_arrive(empty_stats_addr);
    ...
    if (warp >= 16 && warp <= 19) {
        { // mix_main
            ...
            for (int token = (unsigned int)(bid * 4) + warp % 4; token < num_tokens; token += 592) {
                unsigned long long target = (launch_epochs[bid] + 1) * 128 + 27;
                {
                unsigned long long _acquire_observed;
                do {
                ...
                unsigned int first_task = token / 64 * 27;
                float norm_sum = 0.0f;
                #pragma unroll 1
                for (int split = lane_in_warp; split < 27; split += 32) {
                    norm_sum += scratch[(unsigned long long)((num_tokens + 63) / 64 * 27) * 1536 + ((0) ? (unsigned long long)((num_tokens + 63) / 64 * 27) * 64 : (unsigned long long)0) + (unsigned long long)first_task * 64 + (unsigned long long)(split * 64) + (unsigned long long)row_10];
                }
                ...
                #pragma unroll
                for (int split_1 = 0; split_1 < 27; split_1++) {
        ...
        if (task_6 < pairs * partitions) {
            ...
            while (1) {
                ...
                if (task_6 < pairs * partitions) {
                    ...
                    unsigned long long target_1 = (launch_epochs[bid] + 1) * 128 + 27;
                    {
                    unsigned long long _acquire_observed;
                    do {
                    ...
                    if (lane_in_warp < 2 && token_1 + lane_in_warp < num_tokens) {
                        #pragma unroll
                        for (int split_2 = 0; split_2 < 27; split_2++) {
                            row_sum += scratch[(unsigned long long)((num_tokens + 63) / 64 * 27) * 1536 + ((1) ? (unsigned long long)((num_tokens + 63) / 64 * 27) * 64 : (unsigned long long)0) + (unsigned long long)(token_1 / 64 * 27) * 64 + (unsigned long long)(split_2 * 64) + (unsigned long long)(token_1 % 64) + (unsigned long long)lane_in_warp];
                        }
                    }
                    ...
                    for (int pack = begin; pack < end; pack++) {
                        ...
                        {
                            int4 _iv4 = make_int4(normalized_words[0 + 0], normalized_words[0 + 1], normalized_words[0 + 2], normalized_words[0 + 3]);
                            *reinterpret_cast<int4*>(y_bf16 + off_0_2 + 0) = _iv4;
                        }
                        if (token_1 + 1 < num_tokens) {
                            ...
                            {
                                int4 _iv4 = make_int4(normalized_words[4 + 0], normalized_words[4 + 1], normalized_words[4 + 2], normalized_words[4 + 3]);
                                *reinterpret_cast<int4*>(y_bf16 + off_1 + 0) = _iv4;
                            }
                        }
                        ...
                        unsigned int sf_inv_bits = 254 - sf_bits << 7;
                        unsigned int sf_inv_pair = sf_inv_bits | sf_inv_bits << 16;
                        float quantized[8];
                        #pragma unroll
                        for (int j_4 = 0; j_4 < 4; j_4++) {
                            uint32_t _bf16x2_mul_0;
                            asm volatile("mul.rn.bf16x2 %0, %1, %2;" : "=r"(_bf16x2_mul_0) : "r"(normalized_words[j_4]), "r"(sf_inv_pair));
                            quantized[2 * j_4] = __uint_as_float(_bf16x2_mul_0 << 16);
                            quantized[2 * j_4 + 1] = __uint_as_float(_bf16x2_mul_0 & 4294901760);
                        }
                        {
                        ...
                        unsigned int _shfl_xor_20 = __shfl_xor_sync(0xFFFFFFFF, sf_bits, 4);
                        uint32_t _prmt_b32_0;
                        asm("prmt.b32 %0, %1, %2, 0x1140;" : "=r"(_prmt_b32_0) : "r"(sf_bits), "r"(_shfl_xor_20));
                        unsigned int paired_sf = _prmt_b32_0;
                        unsigned int _shfl_xor_21 = __shfl_xor_sync(0xFFFFFFFF, paired_sf, 8);
                        uint32_t _prmt_b32_1;
                        asm("prmt.b32 %0, %1, %2, 0x5410;" : "=r"(_prmt_b32_1) : "r"(paired_sf), "r"(_shfl_xor_21));
                        ...
                        if (token_1 + 1 < num_tokens) {
                            ...
                            unsigned int sf_bits_2 = ((rounded_1 > 113) ? rounded_1 : (unsigned int)113) - 8;
                            unsigned int sf_inv_bits_3 = 254 - sf_bits_2 << 7;
                            unsigned int sf_inv_pair_4 = sf_inv_bits_3 | sf_inv_bits_3 << 16;
                            float quantized_5[8];
                            #pragma unroll
                            for (int j_5 = 0; j_5 < 4; j_5++) {
                                uint32_t _bf16x2_mul_1;
                                asm volatile("mul.rn.bf16x2 %0, %1, %2;" : "=r"(_bf16x2_mul_1) : "r"(normalized_words[4 + j_5]), "r"(sf_inv_pair_4));
                                quantized_5[2 * j_5] = __uint_as_float(_bf16x2_mul_1 << 16);
                                quantized_5[2 * j_5 + 1] = __uint_as_float(_bf16x2_mul_1 & 4294901760);
                            }
                            {
                                unsigned int _fp8_pk[2];
                                asm("{\n\t"
                            ...
                            unsigned int _shfl_xor_22 = __shfl_xor_sync(0xFFFFFFFF, sf_bits_2, 4);
                            uint32_t _prmt_b32_2;
                            asm("prmt.b32 %0, %1, %2, 0x1140;" : "=r"(_prmt_b32_2) : "r"(sf_bits_2), "r"(_shfl_xor_22));
                            unsigned int paired_sf_6 = _prmt_b32_2;
                            unsigned int _shfl_xor_23 = __shfl_xor_sync(0xFFFFFFFF, paired_sf_6, 8);
                            uint32_t _prmt_b32_3;
                            asm("prmt.b32 %0, %1, %2, 0x5410;" : "=r"(_prmt_b32_3) : "r"(paired_sf_6), "r"(_shfl_xor_23));
                            unsigned int word_7 = _prmt_b32_3;
                            {
                                if (lane_in_warp % 16 == 0) {
    ...
    if (warp == 0) {
        if (elect_sync()) {
            #if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 1000
            asm volatile("red.async.release.gpu.global.add.u64 [%0], %1;" :: "l"((reinterpret_cast<unsigned long long*>(launch_epochs) + (bid))), "l"(static_cast<unsigned long long>(1)) : "memory");
            #elif defined(__CUDA_ARCH__)
            #error "GlobalRedAsyncReleaseAdd requires SM100 or newer"
            #endif
        }
