...
#define SMEM_SMEM_B_OFF 100352
#define SMEM_SMEM_B_STAGE_BYTES 16384
#define SMEM_SMEM_B_STRIDE 16384
#define SMEM_SMEM_SFA_OFF 198656
...
kernel_deepgemm_fp4_k_grouped_gemm_sm103a_957f36ebdd4d493260c6(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap C_tma, int* __restrict__ grouped_layout, int M, int N, int K, int grid_m, int grid_n, int num_groups)
{
    ...
    const int mbar_base = smem + 217088;
    ...
    uint8_t* smem_b = reinterpret_cast<uint8_t*>(smem_raw + 100352);
    const int smem_b_addr = smem + 100352;
    unsigned int* smem_sfa = reinterpret_cast<unsigned int*>(smem_raw + 198656);
    const int smem_sfa_addr = smem + 198656;
    ...
    // Mbarriers at smem_raw[217088..217280)

    if (warp == 1) {
        uint32_t leader = elect_sync();
        if (leader) {
            // Stage-major initialization across barrier declarations.
            mbarrier_init(smem + 217088, 130);
            mbarrier_init(smem + 217136, 1);
            mbarrier_init(smem + 217184, 1);
            mbarrier_init(smem + 217232, 1);
            mbarrier_init(smem + 217248, 256);
            mbarrier_init(smem + 217264, 256);
            mbarrier_init(smem + 217096, 130);
            mbarrier_init(smem + 217144, 1);
            mbarrier_init(smem + 217192, 1);
            mbarrier_init(smem + 217240, 1);
            mbarrier_init(smem + 217256, 256);
            mbarrier_init(smem + 217272, 256);
            mbarrier_init(smem + 217104, 130);
            mbarrier_init(smem + 217152, 1);
            mbarrier_init(smem + 217200, 1);
            mbarrier_init(smem + 217112, 130);
            mbarrier_init(smem + 217160, 1);
            mbarrier_init(smem + 217208, 1);
            mbarrier_init(smem + 217120, 130);
            mbarrier_init(smem + 217168, 1);
            mbarrier_init(smem + 217216, 1);
            mbarrier_init(smem + 217128, 130);
            mbarrier_init(smem + 217176, 1);
            mbarrier_init(smem + 217224, 1);
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 217280);
    if (warp == 2) {
        int _tmem_hold = smem + 217280;
                ...
                for (int s = 0; s < 15; s++) {
                    ...
                    for (int i_2 = 0; i_2 < 2; i_2++) {
                        ...
                        for (int element = 0; element < 4; element++) {
                            ...
                            _tmem_load_1[element] = ((current_shape_k_3 == 0) ? 0.0f : _tmem_load_1[element]);
                        }
                        if (s == 14 && i_2 == 1) {
                    ...
                    if (warp == 4) {
                        if (elect_sync()) {
                            ...
                            for (int atom = 0; atom < 2; atom++) {
                                ...
                                tma_store_3d((&C_tma), off_n_1 + (unsigned int)(atom * 64), off_m_1 + (unsigned int)(s * 16), current_group_3, store_addr);
                            }
