...
#define SMEM_SMEM_SFA_OFF 138240
...
kernel_deepgemm_fp4_k_grouped_gemm_sm103a_7340d5b3bf316801fade(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap C_tma, int* __restrict__ grouped_layout, int M, int N, int K, int grid_m, int grid_n, int num_groups)
{
    ...
    const int mbar_base = smem + 152576;
    ...
    unsigned int* smem_sfa = reinterpret_cast<unsigned int*>(smem_raw + 138240);
    const int smem_sfa_addr = smem + 138240;
    unsigned int* smem_sfb = reinterpret_cast<unsigned int*>(smem_raw + 145408);
    ...
    // Mbarriers at smem_raw[152576..152792)

    if (warp == 1) {
        uint32_t leader = elect_sync();
        if (leader) {
            // Stage-major initialization across barrier declarations.
            mbarrier_init(smem + 152576, 130);
            mbarrier_init(smem + 152632, 1);
            mbarrier_init(smem + 152688, 1);
            mbarrier_init(smem + 152744, 1);
            mbarrier_init(smem + 152760, 256);
            mbarrier_init(smem + 152776, 256);
            mbarrier_init(smem + 152584, 130);
            mbarrier_init(smem + 152640, 1);
            mbarrier_init(smem + 152696, 1);
            mbarrier_init(smem + 152752, 1);
            mbarrier_init(smem + 152768, 256);
            mbarrier_init(smem + 152784, 256);
            mbarrier_init(smem + 152592, 130);
            mbarrier_init(smem + 152648, 1);
            mbarrier_init(smem + 152704, 1);
            mbarrier_init(smem + 152600, 130);
            mbarrier_init(smem + 152656, 1);
            mbarrier_init(smem + 152712, 1);
            mbarrier_init(smem + 152608, 130);
            mbarrier_init(smem + 152664, 1);
            mbarrier_init(smem + 152720, 1);
            mbarrier_init(smem + 152616, 130);
            mbarrier_init(smem + 152672, 1);
            mbarrier_init(smem + 152728, 1);
            mbarrier_init(smem + 152624, 130);
            mbarrier_init(smem + 152680, 1);
            mbarrier_init(smem + 152736, 1);
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 152792);
    if (warp == 2) {
        int _tmem_hold = smem + 152792;
    ...
    const int tmem_scale_b = taddr + 40;
    ...
    if (warp == 0) {
        { // load_main
            ...
            #pragma unroll 1
            for (unsigned int bid_1 = bid; bid_1 < 48; bid_1 += 152) {
                int target_group = bid_1 / 16;
                ...
                int K_tiles = _max_0;
                unsigned int block_idx = bid_1 % 16;
                unsigned int blocks_per_group = 64;
                unsigned int group = block_idx / blocks_per_group;
                unsigned int first_m = group * 8;
                unsigned int in_group = block_idx % blocks_per_group;
                int _min_0 = ((8) < (2 - first_m) ? (8) : (2 - first_m));
                unsigned int m_in_group = _min_0;
                unsigned int off_m = (first_m + in_group % m_in_group) * 128;
                unsigned int off_n = in_group / m_in_group * 16;
                for (int k = 0; k < K_tiles; k++) {
                    mbarrier_wait(empty_addr + (stage) * 8, _phase_empty);
                    if (elect_sync()) {
                        tma_2d_gmem2smem(smem_sfa_addr + stage * 1024, (&SFA), off_m, current_sf_start + k * 2, sf_full_addr + (stage) * 8);
                        tma_2d_gmem2smem(smem_sfb_addr + stage * 1024, (&SFB), off_n, current_sf_start + k * 2, sf_full_addr + (stage) * 8);
                        mbarrier_arrive_expect_tx(sf_full_addr + (stage) * 8, 2048);
                        tma_2d_gmem2smem_cta2(smem_a_addr + stage * 16384, (&A), current_k_start + k * 256, off_m, ((full_addr + (stage) * 8) & 0xFEFFFFFF));
                        tma_2d_gmem2smem_cta2(smem_b_addr + stage * 1024, (&B), current_k_start + k * 256, off_n + (unsigned int)(cta_rank * 8), ((full_addr + (stage) * 8) & 0xFEFFFFFF));
                        if (cta_rank == 0) {
    ...
    if (warp == 1) {
        { // mma_main
            ...
            if (cta_rank == 0) {
                ...
                #pragma unroll 1
                for (unsigned int bid_2 = bid; bid_2 < 48; bid_2 += 152) {
                    int target_group_1 = bid_2 / 16;
                        ...
                        if (elect_sync()) {
                            {
                                ...
                                int _mma_b_lo_2 = (((smem_b_addr) >> 4) & 0x3FFF) + (stage_1) * 64;
                                {
                                    uint64_t a_desc = ((uint64_t)(uint32_t)_mma_a_lo_2) | ((uint64_t)0x40004040 << 32);
                                    uint64_t b_desc = ((uint64_t)(uint32_t)_mma_b_lo_2) | ((uint64_t)0x40004040 << 32);

                                    tcgen05_mma_mxf4_bs_cta2((tmem_accum + (accum_stage * 16)), a_desc + 0, b_desc + 0,
                                        0x10840480U, tmem_scale_a + 0, tmem_scale_b + 0, ((init_flag) ? 0 : 1));
                                    tcgen05_mma_mxf4_bs_cta2((tmem_accum + (accum_stage * 16)), a_desc + 2, b_desc + 2,
                                        0x508404a0U, tmem_scale_a + 0, tmem_scale_b + 0, 1);
                                    tcgen05_mma_mxf4_bs_cta2((tmem_accum + (accum_stage * 16)), a_desc + 4, b_desc + 4,
                                        0x10840480U, tmem_scale_a + 4, tmem_scale_b + 4, 1);
                                    tcgen05_mma_mxf4_bs_cta2((tmem_accum + (accum_stage * 16)), a_desc + 6, b_desc + 6,
                                        0x508404a0U, tmem_scale_a + 4, tmem_scale_b + 4, 1);
                                }
                            }
                        }
                        __syncwarp();
    ...
    if (warp >= 2 && warp <= 3) {
        { // transpose_main
            ...
            #pragma unroll 1
            for (unsigned int bid_3 = bid; bid_3 < 48; bid_3 += 152) {
                int target_group_2 = bid_3 / 16;
                ...
                for (int k_2 = 0; k_2 < K_tiles_2; k_2++) {
                    ...
                    #pragma unroll
                    for (int sf_block_1 = 0; sf_block_1 < 1; sf_block_1++) {
                        unsigned int words_1[4];
                        #pragma unroll
                        for (int i_1 = 0; i_1 < 4; i_1++) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&words_1[i_1])) : "r"(smem_sfb_addr + stage_2 * 1024 + sf_subblock * 128 * 4 + (unsigned int)(sf_block_1 * 512) + ((unsigned int)(i_1 * 32) + lane) * 4));
                        }
                        __syncwarp();
                        asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" ::
                            "r"(smem_sfb_addr + stage_2 * 1024 + sf_subblock * 128 * 4 + (unsigned int)(sf_block_1 * 512) + lane * 16), "r"(*reinterpret_cast<uint32_t*>(&words_1[0])), "r"(*reinterpret_cast<uint32_t*>(&words_1[(0) + 1])), "r"(*reinterpret_cast<uint32_t*>(&words_1[(0) + 2])), "r"(*reinterpret_cast<uint32_t*>(&words_1[(0) + 3])));
                    }
                    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                    asm volatile(
                        "mbarrier.arrive.release.cta.shared::cluster.b64 _, [%0];"
    ...
    if (warp >= 4 && warp <= 7) {
        { // epilogue_main
            ...
            #pragma unroll 1
            for (unsigned int bid_4 = bid; bid_4 < 48; bid_4 += 152) {
                int target_group_3 = bid_4 / 16;
                ...
                for (int s = 0; s < 1; s++) {
                    ...
                    for (int i_2 = 0; i_2 < 4; i_2++) {
                        ...
                        unsigned int address = taddr + (row << 16) + accum_stage_1 * 16 + (unsigned int)(s * 16) + (unsigned int)(i_2 * 4);
                        float _tmem_load_0[4];
                        tmem_ld_x4(&_tmem_load_0[0], address);
                        asm volatile("tcgen05.wait::ld.sync.aligned;");
                        #pragma unroll
                        ...
                        unsigned int write_addr = stage_base + warp_0 * 32 * 16 * 4 + swz_row * 128 + swz_col * 16;
                        asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" ::
                            "r"(write_addr), "r"(*reinterpret_cast<uint32_t*>(&_tmem_load_0[0])), "r"(*reinterpret_cast<uint32_t*>(&_tmem_load_0[(0) + 1])), "r"(*reinterpret_cast<uint32_t*>(&_tmem_load_0[(0) + 2])), "r"(*reinterpret_cast<uint32_t*>(&_tmem_load_0[(0) + 3])));
                    }
                    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                    asm volatile("barrier.sync 0, 128;" ::: "memory");
                    if (warp == 4) {
                        if (elect_sync()) {
                            ...
                            asm volatile(
                                ...
                                " [%0, {%1, %2, %3}], [%4];"
                                :: "l"((&C_tma)), "r"(off_n_1 + (unsigned int)(s * 16)), "r"(off_m_1), "r"(current_group_3), "r"(stage_base) : "memory");
                            #else
