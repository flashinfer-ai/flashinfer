            ...
            for (unsigned int this_bid_3 = bid; this_bid_3 < num_tiles_3; this_bid_3 += num_bids) {
                ...
                if (current_shape_k_3 != 0) {
                    ...
                    for (int epi_pass = 0; epi_pass < 4; epi_pass++) {
                        int store_idx = ((epi_stage == 0) ? 3 - epi_pass : epi_pass);
                        int col_start = store_idx * 64;
                        ...
                        for (int n = 0; n < 8; n++) {
                            ...
                            int col = epi_stage * 232 + (unsigned int)col_start + (unsigned int)(load_idx * 8);
                            ...
                            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"((epi_staging_addr + ((unsigned int)(c_stage_row * 128) + ((unsigned int)(load_idx * 16) ^ c_swz)))), "r"(_tmem_load_0_bf16[0]), "r"(_tmem_load_0_bf16[1]), "r"(_tmem_load_0_bf16[2]), "r"(_tmem_load_0_bf16[3]) : "memory");
                        }
                        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                        asm volatile("barrier.sync 8, 128;" ::: "memory");
                        if (warp == 4) {
                    ...
                    if (warp == 4) {
                        if (elect_sync()) {
                            #pragma unroll
                            for (int store_idx_1 = 0; store_idx_1 < 4; store_idx_1++) {
                                tma_store_3d((&C_tma), off_n_1 + store_idx_1 * 64, off_m_1, current_group_3, zero_smem);
                            }
                            asm volatile("cp.async.bulk.commit_group;");
                        }
                    }
