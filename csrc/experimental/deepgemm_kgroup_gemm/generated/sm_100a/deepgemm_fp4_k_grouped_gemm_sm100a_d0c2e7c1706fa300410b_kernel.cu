...
kernel_deepgemm_fp4_k_grouped_gemm_sm100a_d0c2e7c1706fa300410b(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap C_tma, int* __restrict__ grouped_layout, int M, int N, int K, int grid_m, int grid_n, int num_groups)
{
    ...
    if (warp == 0) {
        { // load_main
            ...
            #pragma unroll 1
            for (unsigned int bid_1 = bid; bid_1 < 48; bid_1 += 148) {
    ...
    if (warp == 1) {
        { // mma_main
            ...
            if (cta_rank == 0) {
                ...
                #pragma unroll 1
                for (unsigned int bid_2 = bid; bid_2 < 48; bid_2 += 148) {
    ...
    if (warp >= 2 && warp <= 3) {
        { // transpose_main
            ...
            #pragma unroll 1
            for (unsigned int bid_3 = bid; bid_3 < 48; bid_3 += 148) {
    ...
    if (warp >= 4 && warp <= 7) {
        { // epilogue_main
            ...
            #pragma unroll 1
            for (unsigned int bid_4 = bid; bid_4 < 48; bid_4 += 148) {
