...
kernel_deepgemm_mega_gate_sm100a_b83e2e7a4b2f281f4666(const __grid_constant__ CUtensorMap X, const __grid_constant__ CUtensorMap W, float* __restrict__ bias, float* __restrict__ image_bias, uint8_t* __restrict__ image_mask, uint8_t* __restrict__ mask, int* __restrict__ physical_map, int* __restrict__ logical_count, long long* __restrict__ topk_idx, long long* __restrict__ unmapped_idx, float* __restrict__ topk_weights, float* __restrict__ scratch, unsigned long long* __restrict__ score_barriers, uint8_t* __restrict__ fixed_mask, uint8_t* __restrict__ random_mask, int num_tokens, int num_shared, int map_width, unsigned int ep_rank, float routed_scale, long long unmapped_stride)
{
    ...
    if (warp == 0) {
        { // load_main
            ...
            if (elect_sync()) {
                #pragma unroll 1
                for (int tile = bid / 6; tile < 47; tile += 24) {
                    int effective_n = ((tile == 46) ? 96 : 176);
                    #pragma unroll 4
    ...
    if (warp == 1) {
        { // mma_main
            ...
            if (cta_rank == 0) {
                #pragma unroll 1
                for (int tile_1 = bid / 6; tile_1 < 47; tile_1 += 24) {
    ...
    if (warp >= 4 && warp <= 31) {
        { // gate_main
            ...
            #pragma unroll 1
            for (int tile_2 = bid / 6; tile_2 < 47; tile_2 += 24) {
                int effective_n_1 = ((tile_2 == 46) ? 96 : 176);
                int _min_0 = ((8192) < ((tile_2 + 1) * 176) ? (8192) : ((tile_2 + 1) * 176));
                    ...
                    if (handled == 0) {
                        ...
                        for (int wave_2 = 0; wave_2 < 3; wave_2++) {
                            ...
                            for (int value_1 = 0; value_1 < 4; value_1++) {
                                int expert_id = (unsigned int)(wave_2 * 128) + lane * 4 + (unsigned int)value_1;
                        ...
                        if (lane < (unsigned int)(6 + num_shared)) {
                            ...
                            topk_weights[(unsigned int)(token * (6 + num_shared)) + lane] = result_1;
                        }
                    }
                }
                ge += 1;
