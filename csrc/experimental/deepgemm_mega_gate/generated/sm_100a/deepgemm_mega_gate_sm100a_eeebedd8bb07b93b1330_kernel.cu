...
kernel_deepgemm_mega_gate_sm100a_eeebedd8bb07b93b1330(DeepgemmTensorMap const* X, DeepgemmTensorMap const* W, float* __restrict__ bias, float* __restrict__ image_bias, uint8_t* __restrict__ image_mask, uint8_t* __restrict__ mask, int* __restrict__ physical_map, int* __restrict__ logical_count, long long* __restrict__ topk_idx, long long* __restrict__ unmapped_idx, float* __restrict__ topk_weights, float* __restrict__ scratch, unsigned long long* __restrict__ score_barriers, uint8_t* __restrict__ fixed_mask, uint8_t* __restrict__ random_mask, int num_tokens, int num_shared, int map_width, unsigned int ep_rank, float routed_scale, long long unmapped_stride)
{
    ...
    if (tid == 0) {
        asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" :: "l"((uint64_t)(X)) : "memory");
    ...
    if (warp == 1) {
        { // mma_main
            ...
            if (cta_rank == 0) {
                #pragma unroll 1
                for (int tile_1 = bid / 12; tile_1 < 24; tile_1 += 12) {
    ...
    if (warp >= 4 && warp <= 31) {
        { // gate_main
            ...
            #pragma unroll 1
            for (int tile_2 = bid / 12; tile_2 < 24; tile_2 += 12) {
                int effective_n_1 = ((tile_2 == 23) ? 48 : 176);
                int _min_0 = ((4096) < ((tile_2 + 1) * 176) ? (4096) : ((tile_2 + 1) * 176));
