...
kernel_cake_mega_gate_sm103a_608178f267c22cb93f02(CakeTensorMap const* X, CakeTensorMap const* W, float* __restrict__ bias, float* __restrict__ image_bias, uint8_t* __restrict__ image_mask, uint8_t* __restrict__ mask, int* __restrict__ physical_map, int* __restrict__ logical_count, long long* __restrict__ topk_idx, long long* __restrict__ unmapped_idx, float* __restrict__ topk_weights, float* __restrict__ scratch, unsigned long long* __restrict__ score_barriers, uint8_t* __restrict__ fixed_mask, uint8_t* __restrict__ random_mask, int num_tokens, int num_shared, int map_width, unsigned int ep_rank, float routed_scale, long long unmapped_stride)
{
    ...
    if (warp == 0) {
        { // load_main
            ...
            if (elect_sync()) {
                #pragma unroll 1
                for (int tile = bid / 6; tile < 47; tile += 25) {
    ...
    if (warp == 1) {
        { // mma_main
            ...
            if (cta_rank == 0) {
                #pragma unroll 1
                for (int tile_1 = bid / 6; tile_1 < 47; tile_1 += 25) {
    ...
    if (warp >= 4 && warp <= 31) {
        { // gate_main
            ...
            #pragma unroll 1
            for (int tile_2 = bid / 6; tile_2 < 47; tile_2 += 25) {
