        ...
        { // mma_main
            ...
            if (cta_rank == 0) {
                ...
                for (int tile_1 = bid / 24; tile_1 < 4; tile_1 += 6) {
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
                    "}\n"
                    :: "r"(_mma_a_lo_0), "r"(_mma_b_lo_0), "r"((tmem_accum + (me * 32))), "r"(((init) ? 0 : 1)));
                        __syncwarp();
                        if (kb_1 == 9) {
    ...
    if (warp >= 4 && warp <= 19) {
        { // gate_main
            ...
            for (int tile_2 = bid / 24; tile_2 < 4; tile_2 += 6) {
                int effective_n_1 = ((tile_2 == 3) ? 32 : 32);
                int _min_0 = ((128) < ((tile_2 + 1) * 32) ? (128) : ((tile_2 + 1) * 32));
                int valid_tokens = _min_0 - tile_2 * 32;
                if (gw == 15 && lane == 31) {
                    if (logical_1 == 0) {
