...
kernel_cake_mega_gate_sm103a_b6726160604650afa858(const __grid_constant__ CUtensorMap X, const __grid_constant__ CUtensorMap W, float* __restrict__ bias, float* __restrict__ image_bias, uint8_t* __restrict__ image_mask, uint8_t* __restrict__ mask, int* __restrict__ physical_map, int* __restrict__ logical_count, long long* __restrict__ topk_idx, long long* __restrict__ unmapped_idx, float* __restrict__ topk_weights, float* __restrict__ scratch, unsigned long long* __restrict__ score_barriers, uint8_t* __restrict__ fixed_mask, uint8_t* __restrict__ random_mask, int num_tokens, int num_shared, int map_width, unsigned int ep_rank, float routed_scale, long long unmapped_stride)
{
    ...
    if (warp == 0) {
        { // load_main
            ...
            if (elect_sync()) {
                #pragma unroll 1
                for (int tile = bid / 12; tile < 12; tile += 12) {
                    int effective_n = ((tile == 11) ? 112 : 176);
                    #pragma unroll 4
    ...
    if (warp == 1) {
        { // mma_main
            ...
            if (cta_rank == 0) {
                #pragma unroll 1
                for (int tile_1 = bid / 12; tile_1 < 12; tile_1 += 12) {
    ...
    if (warp >= 4 && warp <= 27) {
        { // gate_main
            ...
            #pragma unroll 1
            for (int tile_2 = bid / 12; tile_2 < 12; tile_2 += 12) {
                int effective_n_1 = ((tile_2 == 11) ? 112 : 176);
                int _min_0 = ((2048) < ((tile_2 + 1) * 176) ? (2048) : ((tile_2 + 1) * 176));
                ...
                asm volatile("bar.sync 8, 768;" ::: "memory");
                #pragma unroll 1
                for (int token_local = gw * 12 + logical_1; token_local < valid_tokens; token_local += 288) {
