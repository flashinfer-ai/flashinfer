...
kernel_deepgemm_mega_gate_sm103a_03760b12370ab0755fd0(const __grid_constant__ CUtensorMap X, const __grid_constant__ CUtensorMap W, float* __restrict__ bias, float* __restrict__ image_bias, uint8_t* __restrict__ image_mask, uint8_t* __restrict__ mask, int* __restrict__ physical_map, int* __restrict__ logical_count, long long* __restrict__ topk_idx, long long* __restrict__ unmapped_idx, float* __restrict__ topk_weights, float* __restrict__ scratch, unsigned long long* __restrict__ score_barriers, uint8_t* __restrict__ fixed_mask, uint8_t* __restrict__ random_mask, int num_tokens, int num_shared, int map_width, unsigned int ep_rank, float routed_scale, long long unmapped_stride)
{
        ...
        { // load_main
            ...
            if (elect_sync()) {
                ...
                for (int tile = bid / 24; tile < 6; tile += 6) {
                    int effective_n = ((tile == 5) ? 144 : 176);
                    #pragma unroll 4
                    for (int kb = 0; kb < 20; kb++) {
                        mbarrier_wait(empty_addr + (ls) * 8, _phase_empty);
                        int ko = split * 20 * 64 + kb * 64;
                        asm volatile(
    ...
    if (warp >= 4 && warp <= 27) {
        { // gate_main
            ...
            for (int tile_2 = bid / 24; tile_2 < 6; tile_2 += 6) {
                int effective_n_1 = ((tile_2 == 5) ? 144 : 176);
                int _min_0 = ((1024) < ((tile_2 + 1) * 176) ? (1024) : ((tile_2 + 1) * 176));
                int valid_tokens = _min_0 - tile_2 * 176;
                if (gw == 23 && lane == 31) {
                    if (logical_1 == 0) {
                ...
                int num_columns = ((1) ? effective_n_1 / 2 : effective_n_1);
                int token_base = ((1) ? subpartition / 2 * num_columns : 0);
                int expert_atom = ((1) ? subpartition % 2 : subpartition);
                int expert_1 = (unsigned int)(expert_group_1 * 128 + cta_rank * 64 + expert_atom * 32) + lane;
                #pragma unroll 1
                for (int col = gw / 4 * 8; col < num_columns; col += 48) {
                    ...
                    for (int item = 0; item < 8; item++) {
                        ...
                        scratch[((tile_2 * 4 + split_1) * 176 + token_base + col + item) * 384 + expert_1] = stored;
                    }
                }
                ...
                for (int token_local = gw * 24 + logical_1; token_local < valid_tokens; token_local += 576) {
                    ...
                    if (handled == 0) {
                        ...
                        for (int wave_1 = 0; wave_1 < 3; wave_1++) {
                            ...
                            float _vec_load_0[4];
                            {
                                float4 _v4 = *reinterpret_cast<const float4*>(scratch + ((tile_2 * 4 * 176 + token_local) * 384 + expert_base) + 0);
                            ...
                            for (int sp = 1; sp < 4; sp++) {
                                float _vec_load_1[4];
                                {
                                    float4 _v4 = *reinterpret_cast<const float4*>(scratch + (((tile_2 * 4 + sp) * 176 + token_local) * 384 + expert_base) + 0);
