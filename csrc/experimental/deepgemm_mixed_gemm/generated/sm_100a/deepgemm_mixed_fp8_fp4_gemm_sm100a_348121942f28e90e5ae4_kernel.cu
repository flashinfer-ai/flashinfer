...
kernel_deepgemm_mixed_fp8_fp4_gemm_sm100a_348121942f28e90e5ae4(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap C_tma, int M, int N, int K, int grid_m, int grid_n, int K_tiles)
{
    ...
    if (warp == 0) {
        { // load_main
            ...
            if (elect_sync()) {
                ...
                #pragma unroll 1
                for (unsigned int bid_1 = bid; bid_1 < 736; bid_1 += 148) {
    ...
    if (warp == 1) {
        { // mma_main
            ...
            if (cta_rank == 0) {
                #pragma unroll 1
                for (unsigned int bid_2 = bid; bid_2 < 736; bid_2 += 148) {
    ...
    if (warp == 2) {
        { // transpose_main
            ...
            #pragma unroll 1
            for (unsigned int bid_3 = bid; bid_3 < 736; bid_3 += 148) {
    ...
    if (warp >= 4 && warp <= 7) {
        { // epilogue_main
            ...
            #pragma unroll 1
            for (unsigned int bid_4 = bid; bid_4 < 736; bid_4 += 148) {
