...
kernel_cake_mixed_fp8_fp4_gemm_sm103a_924709fdbade6e235685(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap C_tma, int M, int N, int K, int grid_m, int grid_n, int K_tiles)
{
    ...
    if (warp == 0) {
        { // load_main
            ...
            if (elect_sync()) {
                ...
                #pragma unroll 1
                for (unsigned int bid_1 = bid; bid_1 < 736; bid_1 += 152) {
                    unsigned int blocks_per_group = 368;
                    ...
                    for (int k = 0; k < 18; k++) {
                        ...
                        if (cta_rank == 0) {
                            mbarrier_arrive_expect_tx(full_addr + (stage) * 8, 47104);
                        } else {
                            asm volatile(
    ...
    if (warp == 1) {
        { // mma_main
            ...
            if (cta_rank == 0) {
                #pragma unroll 1
                for (unsigned int bid_2 = bid; bid_2 < 736; bid_2 += 152) {
    ...
    if (warp == 2) {
        { // transpose_main
            ...
            #pragma unroll 1
            for (unsigned int bid_3 = bid; bid_3 < 736; bid_3 += 152) {
    ...
    if (warp >= 4 && warp <= 7) {
        { // epilogue_main
            ...
            #pragma unroll 1
            for (unsigned int bid_4 = bid; bid_4 < 736; bid_4 += 152) {
                unsigned int blocks_per_group_1 = 368;
                ...
                for (int s = 0; s < 7; s++) {
                    ...
                    for (int i_2 = 0; i_2 < 4; i_2++) {
                        ...
                        unsigned int shifted = load_idx + lane * 4;
