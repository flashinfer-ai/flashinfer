...
kernel_cake_mixed_fp8_fp4_gemm_sm103a_1227956dc12937d633eb(CakeTensorMap const* SFA, CakeTensorMap const* SFB, CakeTensorMap const* A, CakeTensorMap const* B, CakeTensorMap const* C_tma, int M, int N, int K, int grid_m, int grid_n, int K_tiles)
{
    ...
    unsigned int* smem_sfb = reinterpret_cast<unsigned int*>(smem_raw + 32256);
    const int smem_sfb_addr = smem + 32256;
    ...
    const int tmem_tmem_sfb = taddr + 452;
        ...
        { // load_main
            ...
            for (unsigned int this_bid = bid; this_bid < num_tiles; this_bid += num_bids) {
                ...
                for (int iter_k = 0; iter_k < K_tiles; iter_k++) {
                    ...
                    if (elect_sync()) {
                        if (iter_k % 4 == 0) {
                            tma_2d_gmem2smem(smem_sfa_addr + load_stage * 32256, SFA, off_m, iter_k / 4, tma_full_addr + (load_stage) * 8);
                        }
                        tma_2d_gmem2smem(smem_sfb_addr + load_stage * 32256, SFB, off_n, iter_k, tma_full_addr + (load_stage) * 8);
                        {
                            mbarrier_arrive_expect_tx(tma_full_addr + (load_stage) * 8, 896 + ((iter_k % 4 == 0) ? 512 : 0));
                            tma_3d_gmem2smem_cta2(smem_a_addr + load_stage * 32256, A, 0, off_m, iter_k, ((sf_ready_addr + (load_stage) * 8) & 0xFEFFFFFF));
                            tma_3d_gmem2smem_cta2(smem_b_addr + load_stage * 32256, B, 0, off_n + cta_rank * 112, iter_k, ((sf_ready_addr + (load_stage) * 8) & 0xFEFFFFFF));
                            asm volatile(
                                "mbarrier.arrive.expect_tx.release.cta.shared::cluster.b64 _, [%0], %1;"
                                :: "r"((sf_ready_addr + (load_stage) * 8) & 0xFEFFFFFF), "r"((uint32_t)(23552)) : "memory");
                        }
                    }
                    load_stage += 1;
            ...
            if (cta_rank == 0) {
                ...
                for (unsigned int this_bid_1 = bid; this_bid_1 < num_tiles_1; this_bid_1 += num_bids) {
                    ...
                    for (int iter_k_1 = 0; iter_k_1 < K_tiles; iter_k_1++) {
                        ...
                        if (elect_sync()) {
                            if (iter_k_1 % 4 == 0) {
                                tcgen05_cp_32x128b_warpx4_cta2(tmem_tmem_sfa, make_sf_cp_desc_lo_sbo128((((smem_sfa_addr) >> 4) + (mma_tma_stage) * 2016)));
                            }
                            tcgen05_cp_32x128b_warpx4_cta2(tmem_tmem_sfb, make_sf_cp_desc_lo_sbo128((((smem_sfb_addr) >> 4) + (mma_tma_stage) * 2016)));
                            tcgen05_cp_32x128b_warpx4_cta2((tmem_tmem_sfb + 4), make_sf_cp_desc_lo_sbo128((((smem_sfb_addr) >> 4) + (mma_tma_stage) * 2016 + 32)));
                            int _mma_a_lo_0 = (((smem_a_addr) >> 4) & 0x3FFF) + (mma_tma_stage) * 2016;
                            int _mma_b_lo_0 = (((smem_b_addr) >> 4) & 0x3FFF) + (mma_tma_stage) * 2016;
                            {
                                uint64_t a_desc = ((uint64_t)(uint32_t)_mma_a_lo_0) | ((uint64_t)0x40004040 << 32);
                                uint64_t b_desc = ((uint64_t)(uint32_t)_mma_b_lo_0) | ((uint64_t)0x40004040 << 32);

                                tcgen05_mma_mxf8_bs_cta2((tmem_tmem_acc + (mma_epi_stage * 224)), a_desc + 0, b_desc + 0,
                                    (0x10b81400U | ((((uint32_t)(iter_k_1) % 4U)) << 29) | ((0) << 4)), tmem_tmem_sfa, tmem_tmem_sfb, ((init_flag) ? 0 : 1));
                                tcgen05_mma_mxf8_bs_cta2((tmem_tmem_acc + (mma_epi_stage * 224)), a_desc + 2, b_desc + 2,
                                    (0x10b81400U | ((((uint32_t)(iter_k_1) % 4U)) << 29) | ((1) << 4)), tmem_tmem_sfa, tmem_tmem_sfb, 1);
                                tcgen05_mma_mxf8_bs_cta2((tmem_tmem_acc + (mma_epi_stage * 224)), a_desc + 4, b_desc + 4,
                                    (0x10b81400U | ((((uint32_t)(iter_k_1) % 4U)) << 29) | ((2) << 4)), tmem_tmem_sfa, tmem_tmem_sfb, 1);
                                tcgen05_mma_mxf8_bs_cta2((tmem_tmem_acc + (mma_epi_stage * 224)), a_desc + 6, b_desc + 6,
                                    (0x10b81400U | ((((uint32_t)(iter_k_1) % 4U)) << 29) | ((3) << 4)), tmem_tmem_sfa, tmem_tmem_sfb, 1);
                            }
                        }
        ...
        { // transpose_main
            ...
            for (unsigned int this_bid_2 = bid; this_bid_2 < num_tiles_2; this_bid_2 += num_bids) {
                ...
                for (int iter_k_2 = 0; iter_k_2 < K_tiles; iter_k_2++) {
                    ...
                    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                    int sfa_smem = smem_sfa_addr + transpose_stage * 32256;
                    int sfb_smem = smem_sfb_addr + transpose_stage * 32256;
                    if (iter_k_2 % 4 == 0) {
            ...
            for (unsigned int this_bid_3 = bid; this_bid_3 < num_tiles_3; this_bid_3 += num_bids) {
                ...
                for (int epi_pass = 0; epi_pass < 7; epi_pass++) {
                    ...
                    for (int n = 0; n < 4; n++) {
                        ...
                        int col = epi_stage * 224 + (unsigned int)col_start + (unsigned int)(n * 8);
