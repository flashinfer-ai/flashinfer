...
kernel_deepgemm_mixed_fp8_fp4_gemm_sm103a_3ec1a4a693f361779f26(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap C_tma, int M, int N, int K, int grid_m, int grid_n, int K_tiles)
{
    ...
    const int mbar_base = smem + 211968;
    ...
    uint8_t* smem_b = reinterpret_cast<uint8_t*>(smem_raw + 147456);
    const int smem_b_addr = smem + 147456;
    unsigned int* smem_sfa = reinterpret_cast<unsigned int*>(smem_raw + 204800);
    const int smem_sfa_addr = smem + 204800;
    unsigned int* smem_sfb = reinterpret_cast<unsigned int*>(smem_raw + 208384);
    const int smem_sfb_addr = smem + 208384;
    ...
    // Mbarriers at smem_raw[211968..212184)

    if (warp == 1) {
        uint32_t leader = elect_sync();
        if (leader) {
            // Stage-major initialization across barrier declarations.
            mbarrier_init(smem + 211968, 66);
            mbarrier_init(smem + 212024, 1);
            mbarrier_init(smem + 212080, 1);
            mbarrier_init(smem + 212136, 1);
            mbarrier_init(smem + 212152, 256);
            mbarrier_init(smem + 212168, 256);
            mbarrier_init(smem + 211976, 66);
            mbarrier_init(smem + 212032, 1);
            mbarrier_init(smem + 212088, 1);
            mbarrier_init(smem + 212144, 1);
            mbarrier_init(smem + 212160, 256);
            mbarrier_init(smem + 212176, 256);
            mbarrier_init(smem + 211984, 66);
            mbarrier_init(smem + 212040, 1);
            mbarrier_init(smem + 212096, 1);
            mbarrier_init(smem + 211992, 66);
            mbarrier_init(smem + 212048, 1);
            mbarrier_init(smem + 212104, 1);
            mbarrier_init(smem + 212000, 66);
            mbarrier_init(smem + 212056, 1);
            mbarrier_init(smem + 212112, 1);
            mbarrier_init(smem + 212008, 66);
            mbarrier_init(smem + 212064, 1);
            mbarrier_init(smem + 212120, 1);
            mbarrier_init(smem + 212016, 66);
            mbarrier_init(smem + 212072, 1);
            mbarrier_init(smem + 212128, 1);
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 212184);
    if (warp == 2) {
        int _tmem_hold = smem + 212184;
    ...
    if (warp == 0) {
        { // load_main
            ...
            for (unsigned int bid_1 = bid; bid_1 < grid_m * 36; bid_1 += 152) {
                unsigned int blocks_per_group = 576;
                ...
                unsigned int off_n = in_group / m_in_group * 128;
                for (int k = 0; k < 40; k++) {
                    ...
                    if (elect_sync()) {
                        ...
                        tma_2d_gmem2smem_cta2(smem_b_addr + stage * 8192, (&B), k * 128, off_n + (unsigned int)(cta_rank * 64), ((full_addr + (stage) * 8) & 0xFEFFFFFF));
                        if (cta_rank == 0) {
                    ...
                    for (int k_1 = 0; k_1 < 40; k_1++) {
                        ...
                        if (elect_sync()) {
                            ...
                            int _mma_b_lo_0 = (((smem_b_addr) >> 4) & 0x3FFF) + (stage_1) * 512;
                            {
                                ...
                                tcgen05_mma_mxf8_bs_cta2((tmem_accum + (accum_stage * 128)), a_desc + 0, b_desc + 0,
                                    (0x10a01400U | ((0) << 29) | ((0) << 4)), tmem_scale_a, tmem_scale_b, ((init_flag) ? 0 : 1));
                                tcgen05_mma_mxf8_bs_cta2((tmem_accum + (accum_stage * 128)), a_desc + 2, b_desc + 2,
                                    (0x10a01400U | ((1) << 29) | ((1) << 4)), tmem_scale_a, tmem_scale_b, 1);
                                tcgen05_mma_mxf8_bs_cta2((tmem_accum + (accum_stage * 128)), a_desc + 4, b_desc + 4,
                                    (0x10a01400U | ((2) << 29) | ((2) << 4)), tmem_scale_a, tmem_scale_b, 1);
                                tcgen05_mma_mxf8_bs_cta2((tmem_accum + (accum_stage * 128)), a_desc + 6, b_desc + 6,
                                    (0x10a01400U | ((3) << 29) | ((3) << 4)), tmem_scale_a, tmem_scale_b, 1);
                            }
                        }
                        __syncwarp();
                ...
                for (int k_2 = 0; k_2 < 40; k_2++) {
                    ...
                    for (int sf_block_1 = 0; sf_block_1 < 1; sf_block_1++) {
                        ...
                        for (int i_1 = 0; i_1 < 4; i_1++) {
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&words_1[i_1])) : "r"(smem_sfb_addr + stage_2 * 512 + (unsigned int)(sf_block_1 * 512) + ((unsigned int)(i_1 * 32) + lane) * 4));
                        }
                        __syncwarp();
                        asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" ::
                            "r"(smem_sfb_addr + stage_2 * 512 + (unsigned int)(sf_block_1 * 512) + lane * 16), "r"(*reinterpret_cast<uint32_t*>(&words_1[0])), "r"(*reinterpret_cast<uint32_t*>(&words_1[(0) + 1])), "r"(*reinterpret_cast<uint32_t*>(&words_1[(0) + 2])), "r"(*reinterpret_cast<uint32_t*>(&words_1[(0) + 3])));
                    }
                    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                    asm volatile(
                        "mbarrier.arrive.release.cta.shared::cluster.b64 _, [%0];"
    ...
    if (warp >= 4 && warp <= 7) {
        { // epilogue_main
            ...
            for (unsigned int bid_4 = bid; bid_4 < grid_m * 36; bid_4 += 152) {
                unsigned int blocks_per_group_1 = 576;
                ...
                unsigned int off_n_1 = in_group_1 / m_in_group_1 * 128;
                mbarrier_wait(tmem_full_addr + (accum_stage_1) * 8, _phase_tmem_full);
                asm volatile("tcgen05.fence::after_thread_sync;");
                #pragma unroll
                for (int s = 0; s < 2; s++) {
                    unsigned int store_idx = ((0) ? 1 - s : s);
                    if (warp_0 == 0) {
                    ...
                    for (int i_2 = 0; i_2 < 8; i_2++) {
                        unsigned int load_idx = ((0) ? 7 - i_2 : i_2);
                        unsigned int row = (unsigned int)(cta_rank * 128) + warp_0 * 32;
                        unsigned int address = taddr + (row << 16) + accum_stage_1 * 128 + store_idx * 64 + load_idx * 8;
