...
kernel_cake_mixed_fp8_fp4_gemm_sm103a_63403ebdf1b8575ad206(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap C_tma, int M, int N, int K, int grid_m, int grid_n, int K_tiles)
{
    ...
    const int mbar_base = smem + 229376;
    #define full_addr (mbar_base + 0)
    #define sf_full_addr (mbar_base + 96)
    #define empty_addr (mbar_base + 192)
    #define tmem_full_addr (mbar_base + 288)
    #define tmem_empty_addr (mbar_base + 304)
    #define tmem_overlap_addr (mbar_base + 320)
    ...
    uint8_t* smem_b = reinterpret_cast<uint8_t*>(smem_raw + 20480);
    const int smem_b_addr = smem + 20480;
    unsigned int* smem_sfa = reinterpret_cast<unsigned int*>(smem_raw + 217088);
    const int smem_sfa_addr = smem + 217088;
    unsigned int* smem_sfb = reinterpret_cast<unsigned int*>(smem_raw + 223232);
    const int smem_sfb_addr = smem + 223232;
    ...
    // Mbarriers at smem_raw[229376..229712)

    if (warp == 1) {
        uint32_t leader = elect_sync();
        if (leader) {
            // Stage-major initialization across barrier declarations.
            mbarrier_init(smem + 229376, 66);
            mbarrier_init(smem + 229472, 1);
            mbarrier_init(smem + 229568, 1);
            mbarrier_init(smem + 229664, 1);
            mbarrier_init(smem + 229680, 256);
            mbarrier_init(smem + 229696, 256);
            mbarrier_init(smem + 229384, 66);
            mbarrier_init(smem + 229480, 1);
            mbarrier_init(smem + 229576, 1);
            mbarrier_init(smem + 229672, 1);
            mbarrier_init(smem + 229688, 256);
            mbarrier_init(smem + 229704, 256);
            mbarrier_init(smem + 229392, 66);
            mbarrier_init(smem + 229488, 1);
            mbarrier_init(smem + 229584, 1);
            mbarrier_init(smem + 229400, 66);
            mbarrier_init(smem + 229496, 1);
            mbarrier_init(smem + 229592, 1);
            mbarrier_init(smem + 229408, 66);
            mbarrier_init(smem + 229504, 1);
            mbarrier_init(smem + 229600, 1);
            mbarrier_init(smem + 229416, 66);
            mbarrier_init(smem + 229512, 1);
            mbarrier_init(smem + 229608, 1);
            mbarrier_init(smem + 229424, 66);
            mbarrier_init(smem + 229520, 1);
            mbarrier_init(smem + 229616, 1);
            mbarrier_init(smem + 229432, 66);
            mbarrier_init(smem + 229528, 1);
            mbarrier_init(smem + 229624, 1);
            mbarrier_init(smem + 229440, 66);
            mbarrier_init(smem + 229536, 1);
            mbarrier_init(smem + 229632, 1);
            mbarrier_init(smem + 229448, 66);
            mbarrier_init(smem + 229544, 1);
            mbarrier_init(smem + 229640, 1);
            mbarrier_init(smem + 229456, 66);
            mbarrier_init(smem + 229552, 1);
            mbarrier_init(smem + 229648, 1);
            mbarrier_init(smem + 229464, 66);
            mbarrier_init(smem + 229560, 1);
            mbarrier_init(smem + 229656, 1);
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 229712);
    if (warp == 2) {
        int _tmem_hold = smem + 229712;
    ...
    if (warp == 0) {
        { // load_main
            ...
            #pragma unroll 1
            for (unsigned int bid_1 = bid; bid_1 < grid_m * 40; bid_1 += 152) {
                ...
                for (int k = 0; k < 18; k++) {
                    ...
                    if (elect_sync()) {
                        ...
                        tma_2d_gmem2smem_cta2(smem_a_addr + stage * 1024, (&A), k * 128, off_m + (unsigned int)(cta_rank * 8), ((full_addr + (stage) * 8) & 0xFEFFFFFF));
                        tma_2d_gmem2smem_cta2(smem_b_addr + stage * 16384, (&B), k * 128, off_n, ((full_addr + (stage) * 8) & 0xFEFFFFFF));
                        if (cta_rank == 0) {
                            mbarrier_arrive_expect_tx(full_addr + (stage) * 8, 18432);
                        } else {
                            asm volatile(
    ...
    if (warp == 1) {
        { // mma_main
            ...
            if (cta_rank == 0) {
                #pragma unroll 1
                for (unsigned int bid_2 = bid; bid_2 < grid_m * 40; bid_2 += 152) {
                        ...
                        if (elect_sync()) {
                            ...
                            int _mma_b_lo_0 = (((smem_a_addr) >> 4) & 0x3FFF) + (stage_1) * 64;
                            {
                                ...
                                tcgen05_mma_mxf8_bs_cta2((tmem_accum + (accum_stage * 16)), a_desc + 0, b_desc + 0,
                                    (0x10840280U | ((0) << 29) | ((0) << 4)), tmem_scale_b, tmem_scale_a, ((init_flag) ? 0 : 1));
                                tcgen05_mma_mxf8_bs_cta2((tmem_accum + (accum_stage * 16)), a_desc + 2, b_desc + 2,
                                    (0x10840280U | ((1) << 29) | ((1) << 4)), tmem_scale_b, tmem_scale_a, 1);
                                tcgen05_mma_mxf8_bs_cta2((tmem_accum + (accum_stage * 16)), a_desc + 4, b_desc + 4,
                                    (0x10840280U | ((2) << 29) | ((2) << 4)), tmem_scale_b, tmem_scale_a, 1);
                                tcgen05_mma_mxf8_bs_cta2((tmem_accum + (accum_stage * 16)), a_desc + 6, b_desc + 6,
                                    (0x10840280U | ((3) << 29) | ((3) << 4)), tmem_scale_b, tmem_scale_a, 1);
                            }
                        }
                        __syncwarp();
    ...
    if (warp == 2) {
        { // transpose_main
            ...
            #pragma unroll 1
            for (unsigned int bid_3 = bid; bid_3 < grid_m * 40; bid_3 += 152) {
                #pragma unroll 1
    ...
    if (warp >= 4 && warp <= 7) {
        { // epilogue_main
            ...
            #pragma unroll 1
            for (unsigned int bid_4 = bid; bid_4 < grid_m * 40; bid_4 += 152) {
