...
kernel_cake_source_mega_moe_sm103a_2cd593da73685df5f270(CakeTensorMap const* A1, CakeTensorMap const* A2, CakeTensorMap const* SA1, CakeTensorMap const* SA2, CakeTensorMap const* B1, CakeTensorMap const* B2, CakeTensorMap const* SB1, CakeTensorMap const* SB2, CakeTensorMap const* SFA1, CakeTensorMap const* SFA2, CakeTensorMap const* SSFA1, CakeTensorMap const* SSFA2, CakeTensorMap const* SFB1, CakeTensorMap const* SFB2, CakeTensorMap const* SSFB1, CakeTensorMap const* SSFB2, CakeTensorMap const* L1Output, CakeTensorMap const* SharedL1Output, uint8_t* __restrict__ X, unsigned int* __restrict__ XSF, long long* __restrict__ TopK, float* __restrict__ Weights, uint8_t* __restrict__ L1Acts, unsigned int* __restrict__ L1SF, float* __restrict__ L1Weights, uint8_t* __restrict__ L2SF, uint8_t* __restrict__ SharedL2SF, unsigned int* __restrict__ SourceIndices, unsigned int* __restrict__ TokenMetadata, unsigned int* __restrict__ GridCounters, unsigned int* __restrict__ NvlCounter, unsigned int* __restrict__ NvlSignals, unsigned long long* __restrict__ PeerGrid, unsigned long long* __restrict__ ReadyGrid, unsigned long long* __restrict__ SendCounts, unsigned long long* __restrict__ RecvCounts, unsigned long long* __restrict__ RecvSum, unsigned int* __restrict__ L1Full, unsigned int* __restrict__ L1Empty, unsigned long long* __restrict__ L2Mask, unsigned int* __restrict__ L2Empty, unsigned int* __restrict__ SharedFull, unsigned int* __restrict__ L1Counter, unsigned int* __restrict__ L2Counter, unsigned int* __restrict__ SharedL1Counter, unsigned int* __restrict__ SharedL2Counter, unsigned int* __restrict__ Combine, uint8_t* __restrict__ CombineBytes, uint8_t* __restrict__ Y, unsigned int num_tokens)
{
    ...
    #define task_full_addr (mbar_base + 384)
    #define task_empty_addr (mbar_base + 400)
    ...
    // Mbarriers at smem_raw[227648..228064)
    ...
    if (warp == 2) {
        ...
        if (leader) {
            ...
            mbarrier_init(smem + 228048, 512);
            mbarrier_init(smem + 228056, 512);
            asm volatile("fence.mbarrier_init.release.cluster;" ::: "memory");
        }
    }
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 228064);
    if (warp == 3) {
        int _tmem_hold = smem + 228064;
        ...
        { // epilogue_main
            ...
            for (unsigned int token_chunk = epi_warp_2 * 2 + (unsigned int)bid; token_chunk < num_tokens; token_chunk += 16) {
                ...
                if (mask != 0) {
                    ...
                    if (elect_sync()) {
                        cp_async_bulk_gmem2smem(reusable_addr + (epi_warp_2 + stage * 8) * 512, reinterpret_cast<const void*>(reinterpret_cast<const uint8_t*>(CombineBytes) + ((unsigned long long)(((unsigned long long)slot_1 * 1920 + (unsigned long long)token_1) * 256 * 2 + (unsigned long long)(chunk_1 * 512)) * (unsigned long long)1)), 512, combine_barriers_addr + (epi_warp_2 * 2 + stage) * 8);
                        mbarrier_arrive_expect_tx(combine_barriers_addr + (epi_warp_2 * 2 + stage) * 8, 512);
                    }
                    __syncwarp();
                }
                reduced[0] = 0.0f;
                ...
                if (total_mask != 0) {
                    #pragma unroll 1
                    for (unsigned int selection = 0; selection < 3; selection++) {
                        ...
                        if (has_next != 0) {
                            ...
                            if (elect_sync()) {
                                cp_async_bulk_gmem2smem(reusable_addr + (epi_warp_2 + (stage ^ 1) * 8) * 512, reinterpret_cast<const void*>(reinterpret_cast<const uint8_t*>(CombineBytes) + ((unsigned long long)(((unsigned long long)slot_2 * 1920 + (unsigned long long)token_1) * 256 * 2 + (unsigned long long)(chunk_1 * 512)) * (unsigned long long)1)), 512, combine_barriers_addr + (epi_warp_2 * 2 + (stage ^ 1)) * 8);
                                mbarrier_arrive_expect_tx(combine_barriers_addr + (epi_warp_2 * 2 + (stage ^ 1)) * 8, 512);
                            }
                            __syncwarp();
                        }
                        ...
                        for (unsigned int j_3 = 0; j_3 < 1; j_3++) {
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                ...
                                : "r"(reusable_addr + (epi_warp_2 + stage * 8) * 512 + (j_3 * 32 + lane) * 16));
                ...
                for (unsigned int j_4 = 0; j_4 < 1; j_4++) {
                    ...
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" ::
                        "r"(reusable_addr + (epi_warp_2 + 16) * 512 + (j_4 * 32 + lane) * 16), "r"(*reinterpret_cast<uint32_t*>(&store_values[0])), "r"(*reinterpret_cast<uint32_t*>(&store_values[(0) + 1])), "r"(*reinterpret_cast<uint32_t*>(&store_values[(0) + 2])), "r"(*reinterpret_cast<uint32_t*>(&store_values[(0) + 3])));
                }
                __syncwarp();
                if (elect_sync()) {
                    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                    {
                        ...
                        asm volatile(
                            "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
                            :: "l"(_cpbulk_dst_4), "r"(reusable_addr + (epi_warp_2 + 16) * 512), "r"((uint32_t)(512))
                            : "memory");
                    }
                    asm volatile("cp.async.bulk.commit_group;");
                }
