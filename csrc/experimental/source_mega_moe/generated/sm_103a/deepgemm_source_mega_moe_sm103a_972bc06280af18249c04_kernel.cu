...
kernel_deepgemm_source_mega_moe_sm103a_972bc06280af18249c04(DeepgemmTensorMap const* A1, DeepgemmTensorMap const* A2, DeepgemmTensorMap const* SA1, DeepgemmTensorMap const* SA2, DeepgemmTensorMap const* B1, DeepgemmTensorMap const* B2, DeepgemmTensorMap const* SB1, DeepgemmTensorMap const* SB2, DeepgemmTensorMap const* SFA1, DeepgemmTensorMap const* SFA2, DeepgemmTensorMap const* SSFA1, DeepgemmTensorMap const* SSFA2, DeepgemmTensorMap const* SFB1, DeepgemmTensorMap const* SFB2, DeepgemmTensorMap const* SSFB1, DeepgemmTensorMap const* SSFB2, DeepgemmTensorMap const* L1Output, DeepgemmTensorMap const* SharedL1Output, uint8_t* __restrict__ X, unsigned int* __restrict__ XSF, long long* __restrict__ TopK, float* __restrict__ Weights, uint8_t* __restrict__ L1Acts, unsigned int* __restrict__ L1SF, float* __restrict__ L1Weights, uint8_t* __restrict__ L2SF, uint8_t* __restrict__ SharedL2SF, unsigned int* __restrict__ SourceIndices, unsigned int* __restrict__ TokenMetadata, unsigned int* __restrict__ GridCounters, unsigned int* __restrict__ NvlCounter, unsigned int* __restrict__ NvlSignals, unsigned long long* __restrict__ PeerGrid, unsigned long long* __restrict__ ReadyGrid, unsigned long long* __restrict__ SendCounts, unsigned long long* __restrict__ RecvCounts, unsigned long long* __restrict__ RecvSum, unsigned int* __restrict__ L1Full, unsigned int* __restrict__ L1Empty, unsigned long long* __restrict__ L2Mask, unsigned int* __restrict__ L2Empty, unsigned int* __restrict__ SharedFull, unsigned int* __restrict__ L1Counter, unsigned int* __restrict__ L2Counter, unsigned int* __restrict__ SharedL1Counter, unsigned int* __restrict__ SharedL2Counter, unsigned int* __restrict__ Combine, uint8_t* __restrict__ CombineBytes, uint8_t* __restrict__ Y, unsigned int num_tokens)
{
    ...
    const int mbar_base = smem + 201792;
    #define pull_barriers_addr (mbar_base + 0)
    #define full_addr (mbar_base + 32)
    #define empty_addr (mbar_base + 80)
    #define tmem_full_addr (mbar_base + 128)
    #define tmem_empty_addr (mbar_base + 144)
    #define combine_barriers_addr (mbar_base + 160)
    #define task_full_addr (mbar_base + 736)
    #define task_empty_addr (mbar_base + 752)
    ...
    uint8_t* smem_b = reinterpret_cast<uint8_t*>(smem_raw + 93184);
    const int smem_b_addr = smem + 93184;
    uint8_t* smem_shared_b = reinterpret_cast<uint8_t*>(smem_raw + 93184);
    const int smem_shared_b_addr = smem + 93184;
    unsigned int* smem_sfa = reinterpret_cast<unsigned int*>(smem_raw + 191488);
    const int smem_sfa_addr = smem + 191488;
    unsigned int* smem_sfb = reinterpret_cast<unsigned int*>(smem_raw + 197632);
    const int smem_sfb_addr = smem + 197632;
    float* amax_reduction = reinterpret_cast<float*>(smem_raw + 200704);
    const int amax_reduction_addr = smem + 200704;
    unsigned int* task_infos = reinterpret_cast<unsigned int*>(smem_raw + 201728);
    const int task_infos_addr = smem + 201728;
    ...
    // Mbarrier init (8 pipeline groups, 0 ordered-sequence groups, 96 barriers)
    // Mbarriers at smem_raw[201792..202560)
    ...
    if (warp == 2) {
        ...
        if (leader) {
            // --- pipeline 'pipe' ---
            // full: 6 barriers, init_count=4
            mbarrier_init(smem + 201824, 4);
            mbarrier_init(smem + 201832, 4);
            mbarrier_init(smem + 201840, 4);
            mbarrier_init(smem + 201848, 4);
            mbarrier_init(smem + 201856, 4);
            mbarrier_init(smem + 201864, 4);
            // empty: 6 barriers, init_count=1
            mbarrier_init(smem + 201872, 1);
            mbarrier_init(smem + 201880, 1);
            mbarrier_init(smem + 201888, 1);
            mbarrier_init(smem + 201896, 1);
            mbarrier_init(smem + 201904, 1);
            mbarrier_init(smem + 201912, 1);
            // tmem_full: 2 barriers, init_count=1
            mbarrier_init(smem + 201920, 1);
            mbarrier_init(smem + 201928, 1);
            // tmem_empty: 2 barriers, init_count=512
            mbarrier_init(smem + 201936, 512);
            mbarrier_init(smem + 201944, 512);
            // combine_barriers: 72 barriers, init_count=1
            mbarrier_init(smem + 201952, 1);
            mbarrier_init(smem + 201960, 1);
            mbarrier_init(smem + 201968, 1);
            mbarrier_init(smem + 201976, 1);
            mbarrier_init(smem + 201984, 1);
            mbarrier_init(smem + 201992, 1);
            mbarrier_init(smem + 202000, 1);
            mbarrier_init(smem + 202008, 1);
            mbarrier_init(smem + 202016, 1);
            mbarrier_init(smem + 202024, 1);
            mbarrier_init(smem + 202032, 1);
            mbarrier_init(smem + 202040, 1);
            mbarrier_init(smem + 202048, 1);
            mbarrier_init(smem + 202056, 1);
            mbarrier_init(smem + 202064, 1);
            mbarrier_init(smem + 202072, 1);
            mbarrier_init(smem + 202080, 1);
            mbarrier_init(smem + 202088, 1);
            mbarrier_init(smem + 202096, 1);
            mbarrier_init(smem + 202104, 1);
            mbarrier_init(smem + 202112, 1);
            mbarrier_init(smem + 202120, 1);
            mbarrier_init(smem + 202128, 1);
            mbarrier_init(smem + 202136, 1);
            mbarrier_init(smem + 202144, 1);
            mbarrier_init(smem + 202152, 1);
            mbarrier_init(smem + 202160, 1);
            mbarrier_init(smem + 202168, 1);
            mbarrier_init(smem + 202176, 1);
            mbarrier_init(smem + 202184, 1);
            mbarrier_init(smem + 202192, 1);
            mbarrier_init(smem + 202200, 1);
            mbarrier_init(smem + 202208, 1);
            mbarrier_init(smem + 202216, 1);
            mbarrier_init(smem + 202224, 1);
            mbarrier_init(smem + 202232, 1);
            mbarrier_init(smem + 202240, 1);
            mbarrier_init(smem + 202248, 1);
            mbarrier_init(smem + 202256, 1);
            mbarrier_init(smem + 202264, 1);
            mbarrier_init(smem + 202272, 1);
            mbarrier_init(smem + 202280, 1);
            mbarrier_init(smem + 202288, 1);
            mbarrier_init(smem + 202296, 1);
            mbarrier_init(smem + 202304, 1);
            mbarrier_init(smem + 202312, 1);
            mbarrier_init(smem + 202320, 1);
            mbarrier_init(smem + 202328, 1);
            mbarrier_init(smem + 202336, 1);
            mbarrier_init(smem + 202344, 1);
            mbarrier_init(smem + 202352, 1);
            mbarrier_init(smem + 202360, 1);
            mbarrier_init(smem + 202368, 1);
            mbarrier_init(smem + 202376, 1);
            mbarrier_init(smem + 202384, 1);
            mbarrier_init(smem + 202392, 1);
            mbarrier_init(smem + 202400, 1);
            mbarrier_init(smem + 202408, 1);
            mbarrier_init(smem + 202416, 1);
            mbarrier_init(smem + 202424, 1);
            mbarrier_init(smem + 202432, 1);
            mbarrier_init(smem + 202440, 1);
            mbarrier_init(smem + 202448, 1);
            mbarrier_init(smem + 202456, 1);
            mbarrier_init(smem + 202464, 1);
            mbarrier_init(smem + 202472, 1);
            mbarrier_init(smem + 202480, 1);
            mbarrier_init(smem + 202488, 1);
            mbarrier_init(smem + 202496, 1);
            mbarrier_init(smem + 202504, 1);
            mbarrier_init(smem + 202512, 1);
            mbarrier_init(smem + 202520, 1);
            // task_full: 2 barriers, init_count=1
            mbarrier_init(smem + 202528, 1);
            mbarrier_init(smem + 202536, 1);
            // task_empty: 2 barriers, init_count=512
            mbarrier_init(smem + 202544, 512);
            mbarrier_init(smem + 202552, 512);
            asm volatile("fence.mbarrier_init.release.cluster;" ::: "memory");
        }
    }
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 202560);
    if (warp == 3) {
        int _tmem_hold = smem + 202560;
        ...
        { // load_a_main
            ...
            for (int task_iteration = 0; task_iteration < 401; task_iteration++) {
                ...
                for (int k = 0; k < task[7] / 128; k++) {
                    ...
                    __syncwarp();
                    a_stage += 1;
                    if (a_stage == 6) { a_stage = 0; _phase_empty ^= 1; }
                }
            }
        }
    }
        ...
        { // load_b_main
            ...
            for (int task_iteration_1 = 0; task_iteration_1 < 401; task_iteration_1++) {
                ...
                for (int k_1 = 0; k_1 < task_1[7] / 128; k_1++) {
                    ...
                    __syncwarp();
                    b_stage += 1;
                    if (b_stage == 6) { b_stage = 0; _phase_empty_1 ^= 1; }
                }
            }
        }
    }
            ...
            if (cta_rank == 0) {
                ...
                for (int task_iteration_2 = 0; task_iteration_2 < 401; task_iteration_2++) {
                    ...
                    for (int k_2 = 0; k_2 < task_2[7] / 128; k_2++) {
                        ...
                        __syncwarp();
                        mma_stage += 1;
                        if (mma_stage == 6) { mma_stage = 0; _phase_full ^= 1; }
                    }
                    completed_tasks += 1;
                }
    ...
    if (warp >= 8 && warp <= 15) {
        { // epilogue_main
            ...
            for (unsigned int token_chunk = epi_warp_2 * 2 + (unsigned int)bid; token_chunk < num_tokens; token_chunk += 16) {
                ...
                #pragma unroll 1
                for (unsigned int i_3 = 0; i_3 < 9; i_3++) {
                    if (mask != 0) {
                        int _ffs_1 = __ffs(mask);
                        unsigned int slot_1 = (unsigned int)(_ffs_1 - 1);
                        mask ^= (unsigned int)1 << slot_1;
                        if (elect_sync()) {
                            cp_async_bulk_gmem2smem(reusable_addr + (epi_warp_2 * 9 + i_3) * 1024, reinterpret_cast<const void*>(reinterpret_cast<const uint8_t*>(CombineBytes) + ((unsigned long long)(((unsigned long long)slot_1 * 1920 + (unsigned long long)token_1) * 512 * 2 + (unsigned long long)(chunk_1 * 1024)) * (unsigned long long)1)), 1024, combine_barriers_addr + (epi_warp_2 * 9 + i_3) * 8);
                            mbarrier_arrive_expect_tx(combine_barriers_addr + (epi_warp_2 * 9 + i_3) * 8, 1024);
                        }
                        __syncwarp();
                        issued += 1;
                    }
                ...
                reduced[15] = 0.0f;
                #pragma unroll 1
                for (unsigned int i_4 = 0; i_4 < 9; i_4++) {
                    if (issued > i_4) {
                        mbarrier_wait(combine_barriers_addr + (epi_warp_2 * 9 + i_4) * 8, phases >> i_4 & 1);
                        #pragma unroll
                        for (unsigned int j_3 = 0; j_3 < 2; j_3++) {
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                ...
                                : "r"(reusable_addr + (epi_warp_2 * 9 + i_4) * 1024 + (j_3 * 32 + lane) * 16));
                ...
                for (unsigned int j_4 = 0; j_4 < 2; j_4++) {
                    ...
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" ::
                        "r"(reusable_addr + (72 + epi_warp_2) * 1024 + (j_4 * 32 + lane) * 16), "r"(*reinterpret_cast<uint32_t*>(&store_values[0])), "r"(*reinterpret_cast<uint32_t*>(&store_values[(0) + 1])), "r"(*reinterpret_cast<uint32_t*>(&store_values[(0) + 2])), "r"(*reinterpret_cast<uint32_t*>(&store_values[(0) + 3])));
                }
                __syncwarp();
                if (elect_sync()) {
                    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                    {
                        ...
                        asm volatile(
                            "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
                            :: "l"(_cpbulk_dst_4), "r"(reusable_addr + (72 + epi_warp_2) * 1024), "r"((uint32_t)(1024))
                            : "memory");
                    }
                    asm volatile("cp.async.bulk.commit_group;");
                }
