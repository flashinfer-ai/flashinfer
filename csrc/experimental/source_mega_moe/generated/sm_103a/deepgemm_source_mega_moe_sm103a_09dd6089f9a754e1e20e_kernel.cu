...
kernel_deepgemm_source_mega_moe_sm103a_09dd6089f9a754e1e20e(DeepgemmTensorMap const* A1, DeepgemmTensorMap const* A2, DeepgemmTensorMap const* SA1, DeepgemmTensorMap const* SA2, DeepgemmTensorMap const* B1, DeepgemmTensorMap const* B2, DeepgemmTensorMap const* SB1, DeepgemmTensorMap const* SB2, DeepgemmTensorMap const* SFA1, DeepgemmTensorMap const* SFA2, DeepgemmTensorMap const* SSFA1, DeepgemmTensorMap const* SSFA2, DeepgemmTensorMap const* SFB1, DeepgemmTensorMap const* SFB2, DeepgemmTensorMap const* SSFB1, DeepgemmTensorMap const* SSFB2, DeepgemmTensorMap const* L1Output, DeepgemmTensorMap const* SharedL1Output, uint8_t* __restrict__ X, unsigned int* __restrict__ XSF, long long* __restrict__ TopK, float* __restrict__ Weights, uint8_t* __restrict__ L1Acts, unsigned int* __restrict__ L1SF, float* __restrict__ L1Weights, uint8_t* __restrict__ L2SF, uint8_t* __restrict__ SharedL2SF, unsigned int* __restrict__ SourceIndices, unsigned int* __restrict__ TokenMetadata, unsigned int* __restrict__ GridCounters, unsigned int* __restrict__ NvlCounter, unsigned int* __restrict__ NvlSignals, unsigned long long* __restrict__ PeerGrid, unsigned long long* __restrict__ ReadyGrid, unsigned long long* __restrict__ SendCounts, unsigned long long* __restrict__ RecvCounts, unsigned long long* __restrict__ RecvSum, unsigned int* __restrict__ L1Full, unsigned int* __restrict__ L1Empty, unsigned long long* __restrict__ L2Mask, unsigned int* __restrict__ L2Empty, unsigned int* __restrict__ SharedFull, unsigned int* __restrict__ L1Counter, unsigned int* __restrict__ L2Counter, unsigned int* __restrict__ SharedL1Counter, unsigned int* __restrict__ SharedL2Counter, unsigned int* __restrict__ Combine, uint8_t* __restrict__ CombineBytes, uint8_t* __restrict__ Y, unsigned int num_tokens)
{
    ...
    if (warp == 5) {
        { // load_b_main
            ...
            unsigned int pcounts[12];
            unsigned int ptask[8];
            pcounts[0] = 0;
            pcounts[1] = 0;
            pcounts[2] = 0;
            pcounts[3] = 0;
            ...
            if (lane < 384) {
                num_blocks_1 += (pcounts[0] + 16 - 1) / 16;
            }
            if (32 + lane < 384) {
                num_blocks_1 += (pcounts[1] + 16 - 1) / 16;
            }
            if (64 + lane < 384) {
                num_blocks_1 += (pcounts[2] + 16 - 1) / 16;
            }
            if (96 + lane < 384) {
                num_blocks_1 += (pcounts[3] + 16 - 1) / 16;
            }
            if (128 + lane < 384) {
                num_blocks_1 += (pcounts[4] + 16 - 1) / 16;
            }
            if (160 + lane < 384) {
                num_blocks_1 += (pcounts[5] + 16 - 1) / 16;
            }
            if (192 + lane < 384) {
                num_blocks_1 += (pcounts[6] + 16 - 1) / 16;
            }
            if (224 + lane < 384) {
                num_blocks_1 += (pcounts[7] + 16 - 1) / 16;
            }
            if (256 + lane < 384) {
                num_blocks_1 += (pcounts[8] + 16 - 1) / 16;
            }
            if (288 + lane < 384) {
                num_blocks_1 += (pcounts[9] + 16 - 1) / 16;
            }
            if (320 + lane < 384) {
                num_blocks_1 += (pcounts[10] + 16 - 1) / 16;
            }
            if (352 + lane < 384) {
                num_blocks_1 += (pcounts[11] + 16 - 1) / 16;
            }
            unsigned int _warp_redux_u32_3;
            asm volatile("redux.sync.add.u32 %0, %1, 0xffffffff;" : "=r"(_warp_redux_u32_3) : "r"(num_blocks_1));
            unsigned int pool_blocks = _warp_redux_u32_3;
            unsigned int first_task = cluster_id;
            if (first_task < pool_blocks * 18) {
                unsigned int pool_block_2 = first_task / 18;
                ptask[0] = 1;
                ptask[1] = 0;
                ptask[2] = 0;
                ptask[3] = first_task % 18;
                ptask[4] = pool_block_2;
                ptask[5] = 0;
                ptask[6] = 4608;
                ptask[7] = 5120;
                ...
                if (elect_sync()) {
                    ...
                    for (int k_1 = 0; k_1 < 16; k_1++) {
                        asm volatile("cp.async.bulk.prefetch.tensor.3d.L2.global.tile [%0, {%1, %2, %3}];" :: "l"((uint64_t)(B1)), "r"((int)(0)), "r"((int)(prefetch_row)), "r"((int)(k_1)) : "memory");
                    }
                }
                __syncwarp();
            }
        ...
        { // mma_main
            ...
            if (cta_rank == 0) {
                ...
                for (int task_iteration_2 = 0; task_iteration_2 < 250801; task_iteration_2++) {
                    ...
                    #pragma unroll 2
                    for (int k_3 = 0; k_3 < task_2[7] / 128; k_3++) {
                        ...
                        if ((unsigned int)k_3 == task_2[7] / 128 - 1) {
    ...
    if (warp == 7) {
        { // schedule_main
            ...
            if (cta_rank == 0) {
                ...
                if (lane < 384) {
                    num_blocks_2 += (counts_1[0] + 16 - 1) / 16;
                }
                if (32 + lane < 384) {
                    num_blocks_2 += (counts_1[1] + 16 - 1) / 16;
                }
                if (64 + lane < 384) {
                    num_blocks_2 += (counts_1[2] + 16 - 1) / 16;
                }
                if (96 + lane < 384) {
                    num_blocks_2 += (counts_1[3] + 16 - 1) / 16;
                }
                if (128 + lane < 384) {
                    num_blocks_2 += (counts_1[4] + 16 - 1) / 16;
                }
                if (160 + lane < 384) {
                    num_blocks_2 += (counts_1[5] + 16 - 1) / 16;
                }
                if (192 + lane < 384) {
                    num_blocks_2 += (counts_1[6] + 16 - 1) / 16;
                }
                if (224 + lane < 384) {
                    num_blocks_2 += (counts_1[7] + 16 - 1) / 16;
                }
                if (256 + lane < 384) {
                    num_blocks_2 += (counts_1[8] + 16 - 1) / 16;
                }
                if (288 + lane < 384) {
                    num_blocks_2 += (counts_1[9] + 16 - 1) / 16;
                }
                if (320 + lane < 384) {
                    num_blocks_2 += (counts_1[10] + 16 - 1) / 16;
                }
                if (352 + lane < 384) {
                    num_blocks_2 += (counts_1[11] + 16 - 1) / 16;
                }
                unsigned int _warp_redux_u32_2;
                asm volatile("redux.sync.add.u32 %0, %1, 0xffffffff;" : "=r"(_warp_redux_u32_2) : "r"(num_blocks_2));
            ...
            for (int task_iteration_3 = 0; task_iteration_3 < 250801; task_iteration_3++) {
                ...
                if (task_4[0] == 1 || task_4[0] == 3) {
                    ...
                    if (task_4[0] == 1) {
                        ...
                        } while (static_cast<unsigned int>(_acquire_observed - static_cast<unsigned int>(40 * (pool_block_4 / 960))) >= static_cast<unsigned int>(1));
                        }
                    }
                    #pragma unroll
                    for (int s = 0; s < 1; s++) {
                        ...
                        for (int i = 0; i < 1; i++) {
                            ...
                            __nv_bfloat162 _min_29 = __hmin2(_bf16x2_0, _bf16x2_2);
                            __nv_bfloat162 _max_2 = __hmax2(_bf16x2_1, _bf16x2_3);
                            __nv_bfloat162 _min_30 = __hmin2(_max_2, _bf16x2_2);
                            float2 _cvt_f32_0 = __bfloat1622float2(_min_29);
                            float2 _cvt_f32_1 = __bfloat1622float2(_min_30);
                            float denominator[2];
                            ...
                            __nv_bfloat162 _min_31 = __hmin2(_bf16x2_4, _bf16x2_6);
                            __nv_bfloat162 _max_3 = __hmax2(_bf16x2_5, _bf16x2_7);
                            __nv_bfloat162 _min_32 = __hmin2(_max_3, _bf16x2_6);
                            float2 _cvt_f32_2 = __bfloat1622float2(_min_31);
                            float2 _cvt_f32_3 = __bfloat1622float2(_min_32);
                    ...
                    if (epi_warp == 0) {
                        if (elect_sync()) {
                            if (task_4[0] == 3) {
                                asm volatile("red.release.gpu.global.add.u32 [%0], %1;" :: "l"((reinterpret_cast<unsigned int*>(SharedFull) + (pool_block_4))), "r"(static_cast<unsigned int>(1)) : "memory");
                            } else {
