...
#define TMEM_SCALE_A_OFFSET 256
#define TMEM_SCALE_B_OFFSET 260
...
kernel_cake_source_mega_moe_sm103a_906b5e9aab5134361a7f(CakeTensorMap const* A1, CakeTensorMap const* A2, CakeTensorMap const* SA1, CakeTensorMap const* SA2, CakeTensorMap const* B1, CakeTensorMap const* B2, CakeTensorMap const* SB1, CakeTensorMap const* SB2, CakeTensorMap const* SFA1, CakeTensorMap const* SFA2, CakeTensorMap const* SSFA1, CakeTensorMap const* SSFA2, CakeTensorMap const* SFB1, CakeTensorMap const* SFB2, CakeTensorMap const* SSFB1, CakeTensorMap const* SSFB2, CakeTensorMap const* L1Output, CakeTensorMap const* SharedL1Output, uint8_t* __restrict__ X, unsigned int* __restrict__ XSF, long long* __restrict__ TopK, float* __restrict__ Weights, uint8_t* __restrict__ L1Acts, unsigned int* __restrict__ L1SF, float* __restrict__ L1Weights, uint8_t* __restrict__ L2SF, uint8_t* __restrict__ SharedL2SF, unsigned int* __restrict__ SourceIndices, unsigned int* __restrict__ TokenMetadata, unsigned int* __restrict__ GridCounters, unsigned int* __restrict__ NvlCounter, unsigned int* __restrict__ NvlSignals, unsigned long long* __restrict__ PeerGrid, unsigned long long* __restrict__ ReadyGrid, unsigned long long* __restrict__ SendCounts, unsigned long long* __restrict__ RecvCounts, unsigned long long* __restrict__ RecvSum, unsigned int* __restrict__ L1Full, unsigned int* __restrict__ L1Empty, unsigned long long* __restrict__ L2Mask, unsigned int* __restrict__ L2Empty, unsigned int* __restrict__ SharedFull, unsigned int* __restrict__ L1Counter, unsigned int* __restrict__ L2Counter, unsigned int* __restrict__ SharedL1Counter, unsigned int* __restrict__ SharedL2Counter, unsigned int* __restrict__ Combine, uint8_t* __restrict__ CombineBytes, uint8_t* __restrict__ Y, unsigned int num_tokens)
{
    ...
    const int mbar_base = smem + 219200;
    ...
    uint8_t* smem_a = reinterpret_cast<uint8_t*>(smem_raw + 38912);
    const int smem_a_addr = smem + 38912;
    uint8_t* smem_b = reinterpret_cast<uint8_t*>(smem_raw + 96256);
    const int smem_b_addr = smem + 96256;
    uint8_t* smem_shared_b = reinterpret_cast<uint8_t*>(smem_raw + 96256);
    const int smem_shared_b_addr = smem + 96256;
    unsigned int* smem_sfa = reinterpret_cast<unsigned int*>(smem_raw + 210944);
    const int smem_sfa_addr = smem + 210944;
    unsigned int* smem_sfb = reinterpret_cast<unsigned int*>(smem_raw + 214528);
    const int smem_sfb_addr = smem + 214528;
    float* amax_reduction = reinterpret_cast<float*>(smem_raw + 218112);
    const int amax_reduction_addr = smem + 218112;
    unsigned int* task_infos = reinterpret_cast<unsigned int*>(smem_raw + 219136);
    const int task_infos_addr = smem + 219136;
    ...
    // Mbarriers at smem_raw[219200..219536)

    if (warp == 1) {
        uint32_t leader = elect_sync();
        if (leader) {
            // pull_barriers: 4 barriers, init_count=1
            mbarrier_init(smem + 219200, 1);
            mbarrier_init(smem + 219208, 1);
            mbarrier_init(smem + 219216, 1);
            mbarrier_init(smem + 219224, 1);
    ...
    if (warp == 2) {
        ...
        if (leader) {
            ...
            mbarrier_init(smem + 219232, 4);
            mbarrier_init(smem + 219240, 4);
            mbarrier_init(smem + 219248, 4);
            mbarrier_init(smem + 219256, 4);
            mbarrier_init(smem + 219264, 4);
            mbarrier_init(smem + 219272, 4);
            mbarrier_init(smem + 219280, 4);
            // empty: 7 barriers, init_count=1
            mbarrier_init(smem + 219288, 1);
            mbarrier_init(smem + 219296, 1);
            mbarrier_init(smem + 219304, 1);
            mbarrier_init(smem + 219312, 1);
            mbarrier_init(smem + 219320, 1);
            mbarrier_init(smem + 219328, 1);
            mbarrier_init(smem + 219336, 1);
            // tmem_full: 2 barriers, init_count=1
            mbarrier_init(smem + 219344, 1);
            mbarrier_init(smem + 219352, 1);
            // tmem_empty: 2 barriers, init_count=512
            mbarrier_init(smem + 219360, 512);
            mbarrier_init(smem + 219368, 512);
            // combine_barriers: 16 barriers, init_count=1
            mbarrier_init(smem + 219376, 1);
            mbarrier_init(smem + 219384, 1);
            mbarrier_init(smem + 219392, 1);
            mbarrier_init(smem + 219400, 1);
            mbarrier_init(smem + 219408, 1);
            mbarrier_init(smem + 219416, 1);
            mbarrier_init(smem + 219424, 1);
            mbarrier_init(smem + 219432, 1);
            mbarrier_init(smem + 219440, 1);
            mbarrier_init(smem + 219448, 1);
            mbarrier_init(smem + 219456, 1);
            mbarrier_init(smem + 219464, 1);
            mbarrier_init(smem + 219472, 1);
            mbarrier_init(smem + 219480, 1);
            mbarrier_init(smem + 219488, 1);
            mbarrier_init(smem + 219496, 1);
            // task_full: 2 barriers, init_count=1
            mbarrier_init(smem + 219504, 1);
            mbarrier_init(smem + 219512, 1);
            // task_empty: 2 barriers, init_count=512
            mbarrier_init(smem + 219520, 512);
            mbarrier_init(smem + 219528, 512);
            asm volatile("fence.mbarrier_init.release.cluster;" ::: "memory");
        }
    }
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 219536);
    if (warp == 3) {
        int _tmem_hold = smem + 219536;
    ...
    const int tmem_scale_a = taddr + 256;
    const int tmem_scale_b = taddr + 260;
    ...
    if (warp <= 3) {
        { // dispatch_main
            ...
            for (int token_base_1 = first_token; token_base_1 < num_tokens; token_base_1 += 3040) {
                if ((unsigned int)token_base_1 + lane / 6 < num_tokens && lane < 30) {
                    ...
                    if (expert_2 >= 0) {
                        ...
                        SourceIndices[(unsigned int)(expert_2 * 5760) + slot] = (unsigned int)(token_base_1 * 6) + lane;
                    }
                }
                __syncwarp();
            }
            ...
            if (expert_4 < 384) {
                ...
                } while (((_wait_acquire_mask_2 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
            ...
            if (expert_0 < 384) {
                ...
                do {
                asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_3) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_0))) : "memory");
                } while (((_wait_acquire_mask_3 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                received_1 = _wait_acquire_mask_3;
            }
            ...
            if (expert_2_1 < 384) {
                ...
                do {
                asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_4) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_2_1))) : "memory");
                } while (((_wait_acquire_mask_4 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                received_3 = _wait_acquire_mask_4;
            }
            ...
            if (expert_4_1 < 384) {
                ...
                do {
                asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_5) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_4_1))) : "memory");
                } while (((_wait_acquire_mask_5 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                received_5 = _wait_acquire_mask_5;
            }
            counts[3] = (unsigned int)received_5;
            unsigned int expert_6 = 128 + lane;
            unsigned long long received_7 = 0;
            if (expert_6 < 384) {
                unsigned long long _wait_acquire_mask_6;
                do {
                asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_6) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_6))) : "memory");
                } while (((_wait_acquire_mask_6 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                received_7 = _wait_acquire_mask_6;
            }
            counts[4] = (unsigned int)received_7;
            unsigned int expert_8 = 160 + lane;
            unsigned long long received_9 = 0;
            if (expert_8 < 384) {
                unsigned long long _wait_acquire_mask_7;
                do {
                asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_7) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_8))) : "memory");
                } while (((_wait_acquire_mask_7 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                received_9 = _wait_acquire_mask_7;
            }
            counts[5] = (unsigned int)received_9;
            unsigned int expert_10 = 192 + lane;
            unsigned long long received_11 = 0;
            if (expert_10 < 384) {
                unsigned long long _wait_acquire_mask_8;
                do {
                asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_8) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_10))) : "memory");
                } while (((_wait_acquire_mask_8 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                received_11 = _wait_acquire_mask_8;
            }
            counts[6] = (unsigned int)received_11;
            unsigned int expert_12 = 224 + lane;
            unsigned long long received_13 = 0;
            if (expert_12 < 384) {
                unsigned long long _wait_acquire_mask_9;
                do {
                asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_9) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_12))) : "memory");
                } while (((_wait_acquire_mask_9 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                received_13 = _wait_acquire_mask_9;
            }
            counts[7] = (unsigned int)received_13;
            unsigned int expert_14 = 256 + lane;
            unsigned long long received_15 = 0;
            if (expert_14 < 384) {
                unsigned long long _wait_acquire_mask_10;
                do {
                asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_10) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_14))) : "memory");
                } while (((_wait_acquire_mask_10 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                received_15 = _wait_acquire_mask_10;
            }
            counts[8] = (unsigned int)received_15;
            unsigned int expert_16 = 288 + lane;
            unsigned long long received_17 = 0;
            if (expert_16 < 384) {
                unsigned long long _wait_acquire_mask_11;
                do {
                asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_11) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_16))) : "memory");
                } while (((_wait_acquire_mask_11 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                received_17 = _wait_acquire_mask_11;
            }
            counts[9] = (unsigned int)received_17;
            unsigned int expert_18 = 320 + lane;
            unsigned long long received_19 = 0;
            if (expert_18 < 384) {
                unsigned long long _wait_acquire_mask_12;
                do {
                asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_12) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_18))) : "memory");
                } while (((_wait_acquire_mask_12 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                received_19 = _wait_acquire_mask_12;
            }
            counts[10] = (unsigned int)received_19;
            unsigned int expert_20 = 352 + lane;
            unsigned long long received_21 = 0;
            if (expert_20 < 384) {
                unsigned long long _wait_acquire_mask_13;
                do {
                asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_13) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_20))) : "memory");
                } while (((_wait_acquire_mask_13 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                received_21 = _wait_acquire_mask_13;
            }
            counts[11] = (unsigned int)received_21;
            __syncwarp();
            unsigned int num_blocks = 0;
            if (lane < 384) {
                num_blocks += (counts[0] + 128 - 1) / 128;
            }
            if (32 + lane < 384) {
                num_blocks += (counts[1] + 128 - 1) / 128;
            }
            if (64 + lane < 384) {
                num_blocks += (counts[2] + 128 - 1) / 128;
            }
            if (96 + lane < 384) {
                num_blocks += (counts[3] + 128 - 1) / 128;
            }
            if (128 + lane < 384) {
                num_blocks += (counts[4] + 128 - 1) / 128;
            }
            if (160 + lane < 384) {
                num_blocks += (counts[5] + 128 - 1) / 128;
            }
            if (192 + lane < 384) {
                num_blocks += (counts[6] + 128 - 1) / 128;
            }
            if (224 + lane < 384) {
                num_blocks += (counts[7] + 128 - 1) / 128;
            }
            if (256 + lane < 384) {
                num_blocks += (counts[8] + 128 - 1) / 128;
            }
            if (288 + lane < 384) {
                num_blocks += (counts[9] + 128 - 1) / 128;
            }
            if (320 + lane < 384) {
                num_blocks += (counts[10] + 128 - 1) / 128;
            }
            if (352 + lane < 384) {
                num_blocks += (counts[11] + 128 - 1) / 128;
            }
            ...
            for (int token = (unsigned int)(bid * 4) + warp; token < num_tokens * 6; token += 608) {
                ...
                for (int advance_expert = 0; advance_expert < 385; advance_expert++) {
                    ...
                    pool_offset += (expert_end - expert_start + 128 - 1) / 128;
                ...
                unsigned int source_topk = SourceIndices[(unsigned int)(current_expert * 5760) + in_expert];
                unsigned int source_token = source_topk / 6;
                unsigned int pool_token = pool_offset * 128 + in_expert;
                unsigned int pool_block = pool_token / 128;
                unsigned int ring_block = pool_block % 135;
                unsigned int ring_token = pool_token % 17280;
                unsigned int target_1 = pool_block / 135 * 36;
                if (target_1 > 0) {
                    {
                    unsigned int _acquire_observed;
                    do {
                ...
                unsigned int in_block = in_expert % 128;
                ...
                for (int sf_group = 0; sf_group < 2; sf_group++) {
                    ...
                    if (sf_word < 40) {
                        L1SF[sf_word * 276480 + sf_token] = XSF[source_token * 40 + sf_word];
                    }
                }
                __syncwarp();
                if (elect_sync()) {
                    ...
                    asm volatile("cp.async.bulk.commit_group;");
                    asm volatile("cp.async.bulk.wait_group 0;");
                    unsigned int arrivals = (((unsigned int)token == expert_end - 1) ? 128 - in_block : (unsigned int)1);
            ...
            if (bid == 0) {
                ...
                __syncwarp();
                #pragma unroll
                for (int block = tid; block < 720; block += 128) {
                    SharedFull[block] = 0;
                }
                __syncwarp();
            } else {
                #pragma unroll 1
                for (int expert_7 = bid - 1; expert_7 < 384; expert_7 += 151) {
                    unsigned int num_received = (unsigned int)RecvSum[expert_7];
                    unsigned int blocks = (num_received + 128 - 1) / 128;
                    unsigned int num_blocks_0 = 0;
                    if (lane < (unsigned int)expert_7) {
                        num_blocks_0 += (counts[0] + 128 - 1) / 128;
                    }
                    if (32 + lane < (unsigned int)expert_7) {
                        num_blocks_0 += (counts[1] + 128 - 1) / 128;
                    }
                    if (64 + lane < (unsigned int)expert_7) {
                        num_blocks_0 += (counts[2] + 128 - 1) / 128;
                    }
                    if (96 + lane < (unsigned int)expert_7) {
                        num_blocks_0 += (counts[3] + 128 - 1) / 128;
                    }
                    if (128 + lane < (unsigned int)expert_7) {
                        num_blocks_0 += (counts[4] + 128 - 1) / 128;
                    }
                    if (160 + lane < (unsigned int)expert_7) {
                        num_blocks_0 += (counts[5] + 128 - 1) / 128;
                    }
                    if (192 + lane < (unsigned int)expert_7) {
                        num_blocks_0 += (counts[6] + 128 - 1) / 128;
                    }
                    if (224 + lane < (unsigned int)expert_7) {
                        num_blocks_0 += (counts[7] + 128 - 1) / 128;
                    }
                    if (256 + lane < (unsigned int)expert_7) {
                        num_blocks_0 += (counts[8] + 128 - 1) / 128;
                    }
                    if (288 + lane < (unsigned int)expert_7) {
                        num_blocks_0 += (counts[9] + 128 - 1) / 128;
                    }
                    if (320 + lane < (unsigned int)expert_7) {
                        num_blocks_0 += (counts[10] + 128 - 1) / 128;
                    }
                    if (352 + lane < (unsigned int)expert_7) {
                        num_blocks_0 += (counts[11] + 128 - 1) / 128;
                    }
                    ...
                    for (int block_1 = tid; block_1 < blocks; block_1 += 128) {
                        unsigned int ring = (offset + (unsigned int)block_1) % 135;
                        L1Full[ring] = 0;
                        L1Empty[ring] = 0;
                        L2Mask[ring] = 0;
                        L2Empty[ring] = 0;
            ...
            if (tid == 0) {
                ...
                unsigned int _wait_acquire_mask_14;
                do {
                asm volatile("ld.acquire.gpu.global.u32 %0, [%1];" : "=r"(_wait_acquire_mask_14) : "l"((reinterpret_cast<unsigned int*>(GridCounters) + (0))) : "memory");
                } while (((_wait_acquire_mask_14 ^ static_cast<unsigned int>((old_3 ^ 2147483648) & 2147483648)) & static_cast<unsigned int>(2147483648)) != 0);
            }
            asm volatile("barrier.sync 0, 128;" ::: "memory");
            if (bid == 0) {
    ...
    if (warp == 4) {
        { // load_a_main
            ...
            #pragma unroll 1
            for (int task_iteration = 0; task_iteration < 39331; task_iteration++) {
                ...
                unsigned int ring_block_1 = pool_block_1 % 135;
                ...
                if (task[0] == 1) {
                    ...
                    } while (static_cast<unsigned int>(_acquire_observed - static_cast<unsigned int>(128 * (pool_block_1 / 135 + 1))) >= static_cast<unsigned int>(1));
                    }
                }
                if (task[0] == 4) {
                    {
                ...
                if ((pool_block_1 / 135 & 1) != 0) {
                    expected = 0;
                }
                unsigned long long pending = 68719476735;
                unsigned int token_offset = block_2 * 128 + (unsigned int)cta_rank * ((task[5] + 15) / 16 * 8);
                ...
                for (int k = 0; k < task[7] / 128; k++) {
                    if (task[0] == 2) {
                        ...
                        if ((pending & k_mask) != 0) {
                            unsigned long long _wait_acquire_mask_27;
                            do {
                            asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_27) : "l"((reinterpret_cast<unsigned long long*>(L2Mask) + (ring_block_1))) : "memory");
                            } while (((_wait_acquire_mask_27 ^ static_cast<unsigned long long>(expected)) & static_cast<unsigned long long>(k_mask)) != 0);
                            unsigned long long observed = _wait_acquire_mask_27;
                            pending = observed ^ expected;
                        }
                    }
                    mbarrier_wait(empty_addr + (a_stage) * 8, _phase_empty);
                    if (elect_sync()) {
                        tma_3d_gmem2smem_cta2(smem_a_addr + a_stage * 8192, selected_activation_map, 0, token_offset, k, ((full_addr + (a_stage) * 8) & 0xFEFFFFFF));
                        tma_2d_gmem2smem_cta2(smem_sfa_addr + a_stage * 512, selected_activation_sf_map, sf_offset, k, ((full_addr + (a_stage) * 8) & 0xFEFFFFFF));
                        if (cta_rank == 0) {
                            mbarrier_arrive_expect_tx(full_addr + (a_stage) * 8, 17408);
                        } else {
                            asm volatile(
    ...
    if (warp == 5) {
        { // load_b_main
            ...
            #pragma unroll 1
            for (int task_iteration_1 = 0; task_iteration_1 < 39331; task_iteration_1++) {
    ...
    if (warp == 6) {
        { // mma_main
            ...
            if (cta_rank == 0) {
                #pragma unroll 1
                for (int task_iteration_2 = 0; task_iteration_2 < 39331; task_iteration_2++) {
                    ...
                    unsigned int selected_mma_idesc = (unsigned int)((unsigned int)(((task_2[0] > 2) ? 278921216 : 278921856) & -8257537) | aligned_m >> 3 << 17);
                    ...
                    for (int k_2 = 0; k_2 < task_2[7] / 128; k_2++) {
                        ...
                        { // Pre-election register operand materialization
                            ...
                            int _mma_b_lo_0 = (((smem_a_addr) >> 4) & 0x3FFF) + (mma_stage) * 512;
                            uint32_t a_desc_lo = (uint32_t)_mma_a_lo_0;
                            uint32_t b_desc_lo = (uint32_t)_mma_b_lo_0;

                            uint32_t _election_operand_2_0 = ((tmem_accum + (accum_stage * 128)));
                            uint64_t _election_operand_2_1 = (((uint64_t)(a_desc_lo + 0u) | ((uint64_t)0x40004040 << 32)));
                            uint64_t _election_operand_2_2 = (((uint64_t)(b_desc_lo + 0u) | ((uint64_t)0x40004040 << 32)));
                            ...
                            uint32_t _election_operand_3_0 = ((tmem_accum + (accum_stage * 128)));
                            uint64_t _election_operand_3_1 = (((uint64_t)(a_desc_lo + 2u) | ((uint64_t)0x40004040 << 32)));
                            uint64_t _election_operand_3_2 = (((uint64_t)(b_desc_lo + 2u) | ((uint64_t)0x40004040 << 32)));
                            ...
                            uint32_t _election_operand_4_0 = ((tmem_accum + (accum_stage * 128)));
                            uint64_t _election_operand_4_1 = (((uint64_t)(a_desc_lo + 4u) | ((uint64_t)0x40004040 << 32)));
                            uint64_t _election_operand_4_2 = (((uint64_t)(b_desc_lo + 4u) | ((uint64_t)0x40004040 << 32)));
                            ...
                            uint32_t _election_operand_5_0 = ((tmem_accum + (accum_stage * 128)));
                            uint64_t _election_operand_5_1 = (((uint64_t)(a_desc_lo + 6u) | ((uint64_t)0x40004040 << 32)));
                            uint64_t _election_operand_5_2 = (((uint64_t)(b_desc_lo + 6u) | ((uint64_t)0x40004040 << 32)));
    ...
    if (warp == 7) {
        { // schedule_main
            ...
            unsigned int early_shared_l2 = (unsigned int)((num_tokens + 128 - 1) / 128 * 20 <= 76);
            if (cta_rank == 0) {
                unsigned int total_1 = (num_tokens + 128 - 1) / 128 * 18;
                #pragma unroll 1
                for (int iteration = 0; iteration < 39331; iteration++) {
                    ...
                    task_3[4] = block_3;
                    unsigned int _min_1 = ((num_tokens - block_3 * 128) < (128) ? (num_tokens - block_3 * 128) : (128));
                    task_3[5] = _min_1;
                    task_3[6] = 4608;
                    task_3[7] = 5120;
                    if (lane < 2) {
                ...
                if (early_shared_l2 != 0) {
                    unsigned int total_0 = (num_tokens + 128 - 1) / 128 * 20;
                    #pragma unroll 1
                    for (int iteration_1 = 0; iteration_1 < 39331; iteration_1++) {
                        mbarrier_wait(task_empty_addr + (schedule_iteration[0] % 2) * 8, schedule_iteration[0] / 2 & 1 ^ 1);
                        ...
                        task_3[4] = block_4;
                        unsigned int _min_2 = ((num_tokens - block_4 * 128) < (128) ? (num_tokens - block_4 * 128) : (128));
                        task_3[5] = _min_2;
                        task_3[6] = 5120;
                        task_3[7] = 2304;
                        if (lane < 2) {
                ...
                unsigned int expert_9 = lane;
                unsigned long long received_2 = 0;
                if (expert_9 < 384) {
                    unsigned long long _wait_acquire_mask_15;
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_15) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_9))) : "memory");
                    } while (((_wait_acquire_mask_15 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                    received_2 = _wait_acquire_mask_15;
                }
                counts_1[0] = (unsigned int)received_2;
                unsigned int expert_0_1 = 32 + lane;
                unsigned long long received_1_1 = 0;
                if (expert_0_1 < 384) {
                    unsigned long long _wait_acquire_mask_16;
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_16) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_0_1))) : "memory");
                    } while (((_wait_acquire_mask_16 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                    received_1_1 = _wait_acquire_mask_16;
                }
                counts_1[1] = (unsigned int)received_1_1;
                unsigned int expert_2_2 = 64 + lane;
                unsigned long long received_3_1 = 0;
                if (expert_2_2 < 384) {
                    unsigned long long _wait_acquire_mask_17;
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_17) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_2_2))) : "memory");
                    } while (((_wait_acquire_mask_17 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                    received_3_1 = _wait_acquire_mask_17;
                }
                counts_1[2] = (unsigned int)received_3_1;
                unsigned int expert_4_2 = 96 + lane;
                unsigned long long received_5_1 = 0;
                if (expert_4_2 < 384) {
                    unsigned long long _wait_acquire_mask_18;
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_18) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_4_2))) : "memory");
                    } while (((_wait_acquire_mask_18 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                    received_5_1 = _wait_acquire_mask_18;
                }
                counts_1[3] = (unsigned int)received_5_1;
                unsigned int expert_6_1 = 128 + lane;
                unsigned long long received_7_1 = 0;
                if (expert_6_1 < 384) {
                    unsigned long long _wait_acquire_mask_19;
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_19) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_6_1))) : "memory");
                    } while (((_wait_acquire_mask_19 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                    received_7_1 = _wait_acquire_mask_19;
                }
                counts_1[4] = (unsigned int)received_7_1;
                unsigned int expert_8_1 = 160 + lane;
                unsigned long long received_9_1 = 0;
                if (expert_8_1 < 384) {
                    unsigned long long _wait_acquire_mask_20;
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_20) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_8_1))) : "memory");
                    } while (((_wait_acquire_mask_20 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                    received_9_1 = _wait_acquire_mask_20;
                }
                counts_1[5] = (unsigned int)received_9_1;
                unsigned int expert_10_1 = 192 + lane;
                unsigned long long received_11_1 = 0;
                if (expert_10_1 < 384) {
                    unsigned long long _wait_acquire_mask_21;
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_21) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_10_1))) : "memory");
                    } while (((_wait_acquire_mask_21 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                    received_11_1 = _wait_acquire_mask_21;
                }
                counts_1[6] = (unsigned int)received_11_1;
                unsigned int expert_12_1 = 224 + lane;
                unsigned long long received_13_1 = 0;
                if (expert_12_1 < 384) {
                    unsigned long long _wait_acquire_mask_22;
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_22) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_12_1))) : "memory");
                    } while (((_wait_acquire_mask_22 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                    received_13_1 = _wait_acquire_mask_22;
                }
                counts_1[7] = (unsigned int)received_13_1;
                unsigned int expert_14_1 = 256 + lane;
                unsigned long long received_15_1 = 0;
                if (expert_14_1 < 384) {
                    unsigned long long _wait_acquire_mask_23;
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_23) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_14_1))) : "memory");
                    } while (((_wait_acquire_mask_23 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                    received_15_1 = _wait_acquire_mask_23;
                }
                counts_1[8] = (unsigned int)received_15_1;
                unsigned int expert_16_1 = 288 + lane;
                unsigned long long received_17_1 = 0;
                if (expert_16_1 < 384) {
                    unsigned long long _wait_acquire_mask_24;
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_24) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_16_1))) : "memory");
                    } while (((_wait_acquire_mask_24 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                    received_17_1 = _wait_acquire_mask_24;
                }
                counts_1[9] = (unsigned int)received_17_1;
                unsigned int expert_18_1 = 320 + lane;
                unsigned long long received_19_1 = 0;
                if (expert_18_1 < 384) {
                    unsigned long long _wait_acquire_mask_25;
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_25) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_18_1))) : "memory");
                    } while (((_wait_acquire_mask_25 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                    received_19_1 = _wait_acquire_mask_25;
                }
                counts_1[10] = (unsigned int)received_19_1;
                unsigned int expert_20_1 = 352 + lane;
                unsigned long long received_21_1 = 0;
                if (expert_20_1 < 384) {
                    unsigned long long _wait_acquire_mask_26;
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_26) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_20_1))) : "memory");
                    } while (((_wait_acquire_mask_26 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                    received_21_1 = _wait_acquire_mask_26;
                }
                counts_1[11] = (unsigned int)received_21_1;
                __syncwarp();
                unsigned int num_blocks_1 = 0;
                if (lane < 384) {
                    num_blocks_1 += (counts_1[0] + 128 - 1) / 128;
                }
                if (32 + lane < 384) {
                    num_blocks_1 += (counts_1[1] + 128 - 1) / 128;
                }
                if (64 + lane < 384) {
                    num_blocks_1 += (counts_1[2] + 128 - 1) / 128;
                }
                if (96 + lane < 384) {
                    num_blocks_1 += (counts_1[3] + 128 - 1) / 128;
                }
                if (128 + lane < 384) {
                    num_blocks_1 += (counts_1[4] + 128 - 1) / 128;
                }
                if (160 + lane < 384) {
                    num_blocks_1 += (counts_1[5] + 128 - 1) / 128;
                }
                if (192 + lane < 384) {
                    num_blocks_1 += (counts_1[6] + 128 - 1) / 128;
                }
                if (224 + lane < 384) {
                    num_blocks_1 += (counts_1[7] + 128 - 1) / 128;
                }
                if (256 + lane < 384) {
                    num_blocks_1 += (counts_1[8] + 128 - 1) / 128;
                }
                if (288 + lane < 384) {
                    num_blocks_1 += (counts_1[9] + 128 - 1) / 128;
                }
                if (320 + lane < 384) {
                    num_blocks_1 += (counts_1[10] + 128 - 1) / 128;
                }
                if (352 + lane < 384) {
                    num_blocks_1 += (counts_1[11] + 128 - 1) / 128;
                }
                unsigned int _warp_redux_u32_2;
                asm volatile("redux.sync.add.u32 %0, %1, 0xffffffff;" : "=r"(_warp_redux_u32_2) : "r"(num_blocks_1));
                unsigned int total_22 = _warp_redux_u32_2;
                unsigned int waves_1 = (total_22 * 18 + 76 - 1) / 76;
                ...
                state_1[1] = _min_3;
                #pragma unroll 1
                for (int iteration_2 = 0; iteration_2 < 39331; iteration_2++) {
                    mbarrier_wait(task_empty_addr + (schedule_iteration[0] % 2) * 8, schedule_iteration[0] / 2 & 1 ^ 1);
                    ...
                    if (phase_2 != 0) {
                        ...
                        unsigned int blocks_1 = (tokens + 128 - 1) / 128;
                        ...
                        if (owner_mask != 0) {
                            ...
                            unsigned int _min_4 = ((tokens - local_block * 128) < (128) ? (tokens - local_block * 128) : (128));
                            unsigned int valid = _min_4;
                            unsigned int _shfl_5 = __shfl_sync(0xFFFFFFFF, expert_1_1, owner_lane);
                            task_3[1] = _shfl_5;
                        ...
                        unsigned int lane_offset_16 = block_offset + inclusive_10 - blocks_9;
                        unsigned int _vote_1 = __ballot_sync(0xFFFFFFFF, expert_7_1 < 384 && (pool_block_2 >= lane_offset_16 && pool_block_2 < lane_offset_16 + blocks_9));
                        unsigned int owner_mask_17 = _vote_1;
                        if (owner_mask_17 != 0) {
                            int _ffs_1 = __ffs(owner_mask_17);
                            unsigned int owner_lane_1 = _ffs_1 - 1;
                            unsigned int local_block_1 = pool_block_2 - lane_offset_16;
                            unsigned int _min_5 = ((tokens_8 - local_block_1 * 128) < (128) ? (tokens_8 - local_block_1 * 128) : (128));
                            unsigned int valid_1 = _min_5;
                            unsigned int _shfl_9 = __shfl_sync(0xFFFFFFFF, expert_7_1, owner_lane_1);
                            task_3[1] = _shfl_9;
                        ...
                        unsigned int lane_offset_28 = block_offset + inclusive_22 - blocks_21;
                        unsigned int _vote_2 = __ballot_sync(0xFFFFFFFF, expert_19 < 384 && (pool_block_2 >= lane_offset_28 && pool_block_2 < lane_offset_28 + blocks_21));
                        unsigned int owner_mask_29 = _vote_2;
                        if (owner_mask_29 != 0) {
                            int _ffs_2 = __ffs(owner_mask_29);
                            unsigned int owner_lane_2 = _ffs_2 - 1;
                            unsigned int local_block_2 = pool_block_2 - lane_offset_28;
                            unsigned int _min_6 = ((tokens_20 - local_block_2 * 128) < (128) ? (tokens_20 - local_block_2 * 128) : (128));
                            unsigned int valid_2 = _min_6;
                            unsigned int _shfl_13 = __shfl_sync(0xFFFFFFFF, expert_19, owner_lane_2);
                            task_3[1] = _shfl_13;
                        ...
                        unsigned int lane_offset_39 = block_offset + inclusive_33 - blocks_32;
                        unsigned int _vote_3 = __ballot_sync(0xFFFFFFFF, expert_30 < 384 && (pool_block_2 >= lane_offset_39 && pool_block_2 < lane_offset_39 + blocks_32));
                        unsigned int owner_mask_40 = _vote_3;
                        if (owner_mask_40 != 0) {
                            int _ffs_3 = __ffs(owner_mask_40);
                            unsigned int owner_lane_3 = _ffs_3 - 1;
                            unsigned int local_block_3 = pool_block_2 - lane_offset_39;
                            unsigned int _min_7 = ((tokens_31 - local_block_3 * 128) < (128) ? (tokens_31 - local_block_3 * 128) : (128));
                            unsigned int valid_3 = _min_7;
                            unsigned int _shfl_17 = __shfl_sync(0xFFFFFFFF, expert_30, owner_lane_3);
                            task_3[1] = _shfl_17;
                        ...
                        unsigned int lane_offset_50 = block_offset + inclusive_44 - blocks_43;
                        unsigned int _vote_4 = __ballot_sync(0xFFFFFFFF, expert_41 < 384 && (pool_block_2 >= lane_offset_50 && pool_block_2 < lane_offset_50 + blocks_43));
                        unsigned int owner_mask_51 = _vote_4;
                        if (owner_mask_51 != 0) {
                            int _ffs_4 = __ffs(owner_mask_51);
                            unsigned int owner_lane_4 = _ffs_4 - 1;
                            unsigned int local_block_4 = pool_block_2 - lane_offset_50;
                            unsigned int _min_8 = ((tokens_42 - local_block_4 * 128) < (128) ? (tokens_42 - local_block_4 * 128) : (128));
                            unsigned int valid_4 = _min_8;
                            unsigned int _shfl_21 = __shfl_sync(0xFFFFFFFF, expert_41, owner_lane_4);
                            task_3[1] = _shfl_21;
                        ...
                        unsigned int lane_offset_61 = block_offset + inclusive_55 - blocks_54;
                        unsigned int _vote_5 = __ballot_sync(0xFFFFFFFF, expert_52 < 384 && (pool_block_2 >= lane_offset_61 && pool_block_2 < lane_offset_61 + blocks_54));
                        unsigned int owner_mask_62 = _vote_5;
                        if (owner_mask_62 != 0) {
                            int _ffs_5 = __ffs(owner_mask_62);
                            unsigned int owner_lane_5 = _ffs_5 - 1;
                            unsigned int local_block_5 = pool_block_2 - lane_offset_61;
                            unsigned int _min_9 = ((tokens_53 - local_block_5 * 128) < (128) ? (tokens_53 - local_block_5 * 128) : (128));
                            unsigned int valid_5 = _min_9;
                            unsigned int _shfl_25 = __shfl_sync(0xFFFFFFFF, expert_52, owner_lane_5);
                            task_3[1] = _shfl_25;
                        ...
                        unsigned int lane_offset_72 = block_offset + inclusive_66 - blocks_65;
                        unsigned int _vote_6 = __ballot_sync(0xFFFFFFFF, expert_63 < 384 && (pool_block_2 >= lane_offset_72 && pool_block_2 < lane_offset_72 + blocks_65));
                        unsigned int owner_mask_73 = _vote_6;
                        if (owner_mask_73 != 0) {
                            int _ffs_6 = __ffs(owner_mask_73);
                            unsigned int owner_lane_6 = _ffs_6 - 1;
                            unsigned int local_block_6 = pool_block_2 - lane_offset_72;
                            unsigned int _min_10 = ((tokens_64 - local_block_6 * 128) < (128) ? (tokens_64 - local_block_6 * 128) : (128));
                            unsigned int valid_6 = _min_10;
                            unsigned int _shfl_29 = __shfl_sync(0xFFFFFFFF, expert_63, owner_lane_6);
                            task_3[1] = _shfl_29;
                        ...
                        unsigned int lane_offset_83 = block_offset + inclusive_77 - blocks_76;
                        unsigned int _vote_7 = __ballot_sync(0xFFFFFFFF, expert_74 < 384 && (pool_block_2 >= lane_offset_83 && pool_block_2 < lane_offset_83 + blocks_76));
                        unsigned int owner_mask_84 = _vote_7;
                        if (owner_mask_84 != 0) {
                            int _ffs_7 = __ffs(owner_mask_84);
                            unsigned int owner_lane_7 = _ffs_7 - 1;
                            unsigned int local_block_7 = pool_block_2 - lane_offset_83;
                            unsigned int _min_11 = ((tokens_75 - local_block_7 * 128) < (128) ? (tokens_75 - local_block_7 * 128) : (128));
                            unsigned int valid_7 = _min_11;
                            unsigned int _shfl_33 = __shfl_sync(0xFFFFFFFF, expert_74, owner_lane_7);
                            task_3[1] = _shfl_33;
                        ...
                        unsigned int lane_offset_94 = block_offset + inclusive_88 - blocks_87;
                        unsigned int _vote_8 = __ballot_sync(0xFFFFFFFF, expert_85 < 384 && (pool_block_2 >= lane_offset_94 && pool_block_2 < lane_offset_94 + blocks_87));
                        unsigned int owner_mask_95 = _vote_8;
                        if (owner_mask_95 != 0) {
                            int _ffs_8 = __ffs(owner_mask_95);
                            unsigned int owner_lane_8 = _ffs_8 - 1;
                            unsigned int local_block_8 = pool_block_2 - lane_offset_94;
                            unsigned int _min_12 = ((tokens_86 - local_block_8 * 128) < (128) ? (tokens_86 - local_block_8 * 128) : (128));
                            unsigned int valid_8 = _min_12;
                            unsigned int _shfl_37 = __shfl_sync(0xFFFFFFFF, expert_85, owner_lane_8);
                            task_3[1] = _shfl_37;
                        ...
                        unsigned int lane_offset_105 = block_offset + inclusive_99 - blocks_98;
                        unsigned int _vote_9 = __ballot_sync(0xFFFFFFFF, expert_96 < 384 && (pool_block_2 >= lane_offset_105 && pool_block_2 < lane_offset_105 + blocks_98));
                        unsigned int owner_mask_106 = _vote_9;
                        if (owner_mask_106 != 0) {
                            int _ffs_9 = __ffs(owner_mask_106);
                            unsigned int owner_lane_9 = _ffs_9 - 1;
                            unsigned int local_block_9 = pool_block_2 - lane_offset_105;
                            unsigned int _min_13 = ((tokens_97 - local_block_9 * 128) < (128) ? (tokens_97 - local_block_9 * 128) : (128));
                            unsigned int valid_9 = _min_13;
                            unsigned int _shfl_41 = __shfl_sync(0xFFFFFFFF, expert_96, owner_lane_9);
                            task_3[1] = _shfl_41;
                        ...
                        unsigned int lane_offset_116 = block_offset + inclusive_110 - blocks_109;
                        unsigned int _vote_10 = __ballot_sync(0xFFFFFFFF, expert_107 < 384 && (pool_block_2 >= lane_offset_116 && pool_block_2 < lane_offset_116 + blocks_109));
                        unsigned int owner_mask_117 = _vote_10;
                        if (owner_mask_117 != 0) {
                            int _ffs_10 = __ffs(owner_mask_117);
                            unsigned int owner_lane_10 = _ffs_10 - 1;
                            unsigned int local_block_10 = pool_block_2 - lane_offset_116;
                            unsigned int _min_14 = ((tokens_108 - local_block_10 * 128) < (128) ? (tokens_108 - local_block_10 * 128) : (128));
                            unsigned int valid_10 = _min_14;
                            unsigned int _shfl_45 = __shfl_sync(0xFFFFFFFF, expert_107, owner_lane_10);
                            task_3[1] = _shfl_45;
                        ...
                        unsigned int lane_offset_127 = block_offset + inclusive_121 - blocks_120;
                        unsigned int _vote_11 = __ballot_sync(0xFFFFFFFF, expert_118 < 384 && (pool_block_2 >= lane_offset_127 && pool_block_2 < lane_offset_127 + blocks_120));
                        unsigned int owner_mask_128 = _vote_11;
                        if (owner_mask_128 != 0) {
                            int _ffs_11 = __ffs(owner_mask_128);
                            unsigned int owner_lane_11 = _ffs_11 - 1;
                            unsigned int local_block_11 = pool_block_2 - lane_offset_127;
                            unsigned int _min_15 = ((tokens_119 - local_block_11 * 128) < (128) ? (tokens_119 - local_block_11 * 128) : (128));
                            unsigned int valid_11 = _min_15;
                            unsigned int _shfl_49 = __shfl_sync(0xFFFFFFFF, expert_118, owner_lane_11);
                            task_3[1] = _shfl_49;
                ...
                if (early_shared_l2 == 0) {
                    unsigned int total_0_1 = (num_tokens + 128 - 1) / 128 * 20;
                    #pragma unroll 1
                    for (int iteration_3 = 0; iteration_3 < 39331; iteration_3++) {
                        mbarrier_wait(task_empty_addr + (schedule_iteration[0] % 2) * 8, schedule_iteration[0] / 2 & 1 ^ 1);
                        ...
                        task_3[4] = block_5;
                        unsigned int _min_16 = ((num_tokens - block_5 * 128) < (128) ? (num_tokens - block_5 * 128) : (128));
                        task_3[5] = _min_16;
                        task_3[6] = 5120;
                        task_3[7] = 2304;
                        if (lane < 2) {
    ...
    if (warp >= 8 && warp <= 15) {
        { // epilogue_main
            ...
            asm volatile("barrier.sync 1, 384;" ::: "memory");
            #pragma unroll 1
            for (int task_iteration_3 = 0; task_iteration_3 < 39331; task_iteration_3++) {
                ...
                if (task_4[0] == 1 || task_4[0] == 3) {
                    ...
                    unsigned int ring_block_2 = pool_block_3 % 135;
                    unsigned int block_6 = ring_block_2;
                    unsigned int sf_stride = 1105920;
                    if (task_4[0] == 3) {
                    ...
                    if (task_4[0] == 1) {
                        ...
                        } while (static_cast<unsigned int>(_acquire_observed - static_cast<unsigned int>(40 * (pool_block_3 / 135))) >= static_cast<unsigned int>(1));
                        }
                    }
                    #pragma unroll
                    for (int s = 0; s < 2; s++) {
                        if (valid_m <= epi_wg * 64 + (unsigned int)(s * 32)) {
                        ...
                        for (int i = 0; i < 4; i++) {
                            ...
                            if (task_4[0] == 1 && j * 8 % 32 == 0) {
                                {
                                    cached_weight = L1Weights[ring_block_2 * 128 + epi_wg * 64 + j * 8 + lane_read_0];
                                }
                            }
                            float _shfl_55;
                            asm volatile("shfl.sync.idx.b32 %0, %1, %2, 0x1f, 0xffffffff;" : "=f"(_shfl_55) : "f"(cached_weight), "r"(j * 8 % 32 + lane_read_0 % 4 * 2));
                            float first_weight = _shfl_55;
                            float _shfl_56;
                            asm volatile("shfl.sync.idx.b32 %0, %1, %2, 0x1f, 0xffffffff;" : "=f"(_shfl_56) : "f"(cached_weight), "r"(j * 8 % 32 + lane_read_0 % 4 * 2 + 1));
                            float second_weight = _shfl_56;
                            unsigned int address = accum_stage_1 * 128 + epi_wg * 64 + j * 8;
                            ...
                            if (lane_read_0 < 4) {
                                amax_reduction[epi_warp * 32 + (unsigned int)(i * 8) + lane_read_0 * 2] = first_max;
                                amax_reduction[epi_warp * 32 + (unsigned int)(i * 8) + lane_read_0 * 2 + 1] = second_max;
                            }
                            __syncwarp();
                        }
                        ...
                        for (int i_1 = 0; i_1 < 4; i_1++) {
                            unsigned int paired_index = (epi_warp ^ 1) * 32 + (unsigned int)(i_1 * 8) + lane_read_0 % 4 * 2;
                            ...
                            unsigned int stsm_address = store_address + (unsigned int)(i_1 * 8 * 64) + lane_read_0 * 64 + (warp_in_wg ^ lane_read_0 / 2) * 16;
                            #if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 1000
                            ...
                            #endif
                            if (warp_in_wg % 2 == 0 && lane_read_0 < 4) {
                                unsigned int sf_k = n_block * 2 + warp_in_wg / 2;
                                unsigned int token_base_2 = epi_wg * 64 + (unsigned int)(s * 32) + (unsigned int)(i_1 * 8);
                                unsigned int permuted_base = (token_base_2 & 4294967168) + (token_base_2 & 31) * 4 + (token_base_2 >> 5 & 3);
                                unsigned int sf_token_1 = block_6 * 128 + permuted_base + lane_read_0 * 8;
                        ...
                        if (warp_in_wg == 0) {
                            if (elect_sync()) {
                                asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                                if (task_4[0] == 3) {
                                    tma_store_2d(SharedL1Output, n_block * 64, block_6 * 128 + epi_wg * 64 + (unsigned int)(s * 32), store_address);
                                } else {
                                    tma_store_2d(L1Output, n_block * 64, block_6 * 128 + epi_wg * 64 + (unsigned int)(s * 32), store_address);
                                }
                                asm volatile("cp.async.bulk.commit_group;");
                            }
                        }
                    ...
                    unsigned int pool_m = task_4[4] * 128;
                    ...
                    if (task_4[0] == 2) {
                        if (epi_warp_1 == 0) {
                            if (elect_sync()) {
                                atomicAdd(&L2Empty[task_4[4] % 135], 1);
                            }
                        }
                        __syncwarp();
                    }
                    ...
                    for (int s_1 = 0; s_1 < 2; s_1++) {
                        if (valid_m_1 <= epi_wg_1 * 64 + (unsigned int)(s_1 * 32)) {
                            asm volatile("tcgen05.fence::before_thread_sync;");
                            asm volatile(
                                "mbarrier.arrive.release.cta.shared::cluster.b64 _, [%0];"
                                :: "r"((tmem_empty_addr + (accum_stage_1) * 8) & 0xFEFFFFFF) : "memory");
                        ...
                        for (int j_1 = 0; j_1 < 4; j_1++) {
                            unsigned int row = epi_wg_1 * 64 + (unsigned int)(s_1 * 32) + (unsigned int)(j_1 * 8) + warp_in_wg_1 * 2 + lane_read_0 / 16;
                            if (row < valid_m_1) {
                                if (task_4[0] == 4) {
                        ...
                        for (int i_2 = 0; i_2 < 4; i_2++) {
                            unsigned int address_1 = accum_stage_1 * 128 + epi_wg_1 * 64 + (unsigned int)(s_1 * 32) + (unsigned int)(i_2 * 8);
                            ...
                            unsigned int row_1 = lane_read_0 % 8;
                            unsigned int col = epi_warp_1 % 2 * 4 + lane_read_0 / 8;
                        ...
                        for (int j_2 = 0; j_2 < 4; j_2++) {
                            unsigned int row_in_store = (unsigned int)(j_2 * 8) + warp_in_wg_1 * 2 + lane_read_0 / 16;
                            unsigned int row_2 = epi_wg_1 * 64 + (unsigned int)(s_1 * 32) + row_in_store;
                            if (row_2 >= valid_m_1) {
                                break;
                            }
                            unsigned int row_in_atom = (warp_in_wg_1 * 2 + lane_read_0 / 16) % 8;
                            unsigned int read_address = output_base + lane_read_0 % 16 / 8 * 32 * 128 + row_in_store * 128 + (lane_read_0 % 8 ^ row_in_atom) * 16;
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                ...
                                : "r"(read_address));
                            unsigned long long dst_index = ((unsigned long long)cached_topk[j_2] * 5760 + (unsigned long long)cached_token[j_2]) * 2560 + (unsigned long long)(n_offset_1 / 2) + (unsigned long long)(lane_read_0 % 16 * 4);
            ...
            if (epi_thread == 0) {
                ...
                unsigned int _wait_acquire_mask_28;
                do {
                asm volatile("ld.acquire.gpu.global.u32 %0, [%1];" : "=r"(_wait_acquire_mask_28) : "l"((reinterpret_cast<unsigned int*>(GridCounters) + (1))) : "memory");
                } while (((_wait_acquire_mask_28 ^ static_cast<unsigned int>((old_4 ^ 2147483648) & 2147483648)) & static_cast<unsigned int>(2147483648)) != 0);
            }
            asm volatile("bar.sync 2, 256;" ::: "memory");
            if (bid == 0 && epi_thread == 0) {
            ...
            #pragma unroll 1
            for (unsigned int token_chunk = epi_warp_2 * 152 + (unsigned int)bid; token_chunk < num_tokens * 2; token_chunk += 1216) {
                unsigned int token_1 = token_chunk / 2;
                unsigned int chunk_1 = token_chunk % 2;
                int expert_11 = -1;
                if (lane < 6) {
                    expert_11 = (int)TopK[token_1 * 6 + lane];
                }
                if (lane == 6) {
                    expert_11 = 6;
                }
                unsigned int _vote_12 = __ballot_sync(0xFFFFFFFF, expert_11 >= 0);
                ...
                if (mask != 0) {
                    ...
                    if (elect_sync()) {
                        cp_async_bulk_gmem2smem(reusable_addr + (epi_warp_2 + stage * 8) * 5120, reinterpret_cast<const void*>(reinterpret_cast<const uint8_t*>(CombineBytes) + ((unsigned long long)(((unsigned long long)slot_1 * 5760 + (unsigned long long)token_1) * 5120 * 2 + (unsigned long long)(chunk_1 * 5120)) * (unsigned long long)1)), 5120, combine_barriers_addr + (epi_warp_2 * 2 + stage) * 8);
                        mbarrier_arrive_expect_tx(combine_barriers_addr + (epi_warp_2 * 2 + stage) * 8, 5120);
                    }
                    __syncwarp();
                }
                reduced[0] = 0.0f;
                ...
                if (total_mask != 0) {
                    #pragma unroll 1
                    for (unsigned int selection = 0; selection < 7; selection++) {
                        unsigned int has_next = (unsigned int)(mask != 0);
                        if (has_next != 0) {
                            int _ffs_13 = __ffs(mask);
                            unsigned int slot_2 = (unsigned int)(_ffs_13 - 1);
                            mask ^= (unsigned int)1 << slot_2;
                            if (elect_sync()) {
                                cp_async_bulk_gmem2smem(reusable_addr + (epi_warp_2 + (stage ^ 1) * 8) * 5120, reinterpret_cast<const void*>(reinterpret_cast<const uint8_t*>(CombineBytes) + ((unsigned long long)(((unsigned long long)slot_2 * 5760 + (unsigned long long)token_1) * 5120 * 2 + (unsigned long long)(chunk_1 * 5120)) * (unsigned long long)1)), 5120, combine_barriers_addr + (epi_warp_2 * 2 + (stage ^ 1)) * 8);
                                mbarrier_arrive_expect_tx(combine_barriers_addr + (epi_warp_2 * 2 + (stage ^ 1)) * 8, 5120);
                            }
                            __syncwarp();
                        }
                        mbarrier_wait(combine_barriers_addr + (epi_warp_2 * 2 + stage) * 8, phase_3);
                        #pragma unroll
                        for (unsigned int j_3 = 0; j_3 < 10; j_3++) {
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                ...
                                : "r"(reusable_addr + (epi_warp_2 + stage * 8) * 5120 + (j_3 * 32 + lane) * 16));
                ...
                #pragma unroll
                for (unsigned int j_4 = 0; j_4 < 10; j_4++) {
                    ...
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" ::
                        "r"(reusable_addr + (epi_warp_2 + 16) * 5120 + (j_4 * 32 + lane) * 16), "r"(*reinterpret_cast<uint32_t*>(&store_values[0])), "r"(*reinterpret_cast<uint32_t*>(&store_values[(0) + 1])), "r"(*reinterpret_cast<uint32_t*>(&store_values[(0) + 2])), "r"(*reinterpret_cast<uint32_t*>(&store_values[(0) + 3])));
                }
                __syncwarp();
                if (elect_sync()) {
                    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                    {
                        void* _cpbulk_dst_4 = reinterpret_cast<void*>(Y + ((unsigned long long)token_1 * 5120 * 2 + (unsigned long long)(chunk_1 * 5120)));
                        asm volatile(
                            "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
                            :: "l"(_cpbulk_dst_4), "r"(reusable_addr + (epi_warp_2 + 16) * 5120), "r"((uint32_t)(5120))
                            : "memory");
                    }
                    asm volatile("cp.async.bulk.commit_group;");
                }
