...
#define SMEM_SEND_BUFFER_STAGE_BYTES 1024
#define SMEM_SEND_BUFFER_STRIDE 1024
#define SMEM_SMEM_OUTPUT_OFF 2048
#define SMEM_SMEM_OUTPUT_STAGE_BYTES 4096
#define SMEM_SMEM_OUTPUT_STRIDE 4096
#define SMEM_REUSABLE_OFF 0
#define SMEM_REUSABLE_STAGE_BYTES 227648
#define SMEM_REUSABLE_STRIDE 227648
#define SMEM_SMEM_A_OFF 6144
#define SMEM_SMEM_A_STAGE_BYTES 1024
#define SMEM_SMEM_A_STRIDE 1024
#define SMEM_SMEM_B_OFF 18432
#define SMEM_SMEM_B_STAGE_BYTES 16384
#define SMEM_SMEM_B_STRIDE 16384
#define SMEM_SMEM_SHARED_B_OFF 18432
...
kernel_deepgemm_source_mega_moe_sm103a_8d495c4677c74194347d(DeepgemmTensorMap const* A1, DeepgemmTensorMap const* A2, DeepgemmTensorMap const* SA1, DeepgemmTensorMap const* SA2, DeepgemmTensorMap const* B1, DeepgemmTensorMap const* B2, DeepgemmTensorMap const* SB1, DeepgemmTensorMap const* SB2, DeepgemmTensorMap const* SFA1, DeepgemmTensorMap const* SFA2, DeepgemmTensorMap const* SSFA1, DeepgemmTensorMap const* SSFA2, DeepgemmTensorMap const* SFB1, DeepgemmTensorMap const* SFB2, DeepgemmTensorMap const* SSFB1, DeepgemmTensorMap const* SSFB2, DeepgemmTensorMap const* L1Output, DeepgemmTensorMap const* SharedL1Output, uint8_t* __restrict__ X, unsigned int* __restrict__ XSF, long long* __restrict__ TopK, float* __restrict__ Weights, uint8_t* __restrict__ L1Acts, unsigned int* __restrict__ L1SF, float* __restrict__ L1Weights, uint8_t* __restrict__ L2SF, uint8_t* __restrict__ SharedL2SF, unsigned int* __restrict__ SourceIndices, unsigned int* __restrict__ TokenMetadata, unsigned int* __restrict__ GridCounters, unsigned int* __restrict__ NvlCounter, unsigned int* __restrict__ NvlSignals, unsigned long long* __restrict__ PeerGrid, unsigned long long* __restrict__ ReadyGrid, unsigned long long* __restrict__ SendCounts, unsigned long long* __restrict__ RecvCounts, unsigned long long* __restrict__ RecvSum, unsigned int* __restrict__ L1Full, unsigned int* __restrict__ L1Empty, unsigned long long* __restrict__ L2Mask, unsigned int* __restrict__ L2Empty, unsigned int* __restrict__ SharedFull, unsigned int* __restrict__ L1Counter, unsigned int* __restrict__ L2Counter, unsigned int* __restrict__ SharedL1Counter, unsigned int* __restrict__ SharedL2Counter, unsigned int* __restrict__ Combine, uint8_t* __restrict__ CombineBytes, uint8_t* __restrict__ Y, unsigned int num_tokens)
{
    ...
    const int mbar_base = smem + 227648;
    #define pull_barriers_addr (mbar_base + 0)
    #define full_addr (mbar_base + 32)
    #define empty_addr (mbar_base + 128)
    #define tmem_full_addr (mbar_base + 224)
    #define tmem_empty_addr (mbar_base + 240)
    #define combine_barriers_addr (mbar_base + 256)
    #define task_full_addr (mbar_base + 448)
    #define task_empty_addr (mbar_base + 464)
    ...
    uint8_t* smem_output = reinterpret_cast<uint8_t*>(smem_raw + 2048);
    const int smem_output_addr = smem + 2048;
    uint8_t* reusable = reinterpret_cast<uint8_t*>(smem_raw + 0);
    const int reusable_addr = smem + 0;
    uint8_t* smem_a = reinterpret_cast<uint8_t*>(smem_raw + 6144);
    const int smem_a_addr = smem + 6144;
    uint8_t* smem_b = reinterpret_cast<uint8_t*>(smem_raw + 18432);
    const int smem_b_addr = smem + 18432;
    uint8_t* smem_shared_b = reinterpret_cast<uint8_t*>(smem_raw + 18432);
    const int smem_shared_b_addr = smem + 18432;
    unsigned int* smem_sfa = reinterpret_cast<unsigned int*>(smem_raw + 215040);
    const int smem_sfa_addr = smem + 215040;
    unsigned int* smem_sfb = reinterpret_cast<unsigned int*>(smem_raw + 221184);
    const int smem_sfb_addr = smem + 221184;
    float* amax_reduction = reinterpret_cast<float*>(smem_raw + 227328);
    const int amax_reduction_addr = smem + 227328;
    unsigned int* task_infos = reinterpret_cast<unsigned int*>(smem_raw + 227584);
    const int task_infos_addr = smem + 227584;
    ...
    // Mbarrier init (8 pipeline groups, 0 ordered-sequence groups, 60 barriers)
    // Mbarriers at smem_raw[227648..228128)

    if (warp == 1) {
        uint32_t leader = elect_sync();
        if (leader) {
            // pull_barriers: 4 barriers, init_count=1
            mbarrier_init(smem + 227648, 1);
            mbarrier_init(smem + 227656, 1);
            mbarrier_init(smem + 227664, 1);
            mbarrier_init(smem + 227672, 1);
    ...
    if (warp == 2) {
        ...
        if (leader) {
            // --- pipeline 'pipe' ---
            // full: 12 barriers, init_count=4
            mbarrier_init(smem + 227680, 4);
            mbarrier_init(smem + 227688, 4);
            mbarrier_init(smem + 227696, 4);
            mbarrier_init(smem + 227704, 4);
            mbarrier_init(smem + 227712, 4);
            mbarrier_init(smem + 227720, 4);
            mbarrier_init(smem + 227728, 4);
            mbarrier_init(smem + 227736, 4);
            mbarrier_init(smem + 227744, 4);
            mbarrier_init(smem + 227752, 4);
            mbarrier_init(smem + 227760, 4);
            mbarrier_init(smem + 227768, 4);
            // empty: 12 barriers, init_count=1
            mbarrier_init(smem + 227776, 1);
            mbarrier_init(smem + 227784, 1);
            mbarrier_init(smem + 227792, 1);
            mbarrier_init(smem + 227800, 1);
            mbarrier_init(smem + 227808, 1);
            mbarrier_init(smem + 227816, 1);
            mbarrier_init(smem + 227824, 1);
            mbarrier_init(smem + 227832, 1);
            mbarrier_init(smem + 227840, 1);
            mbarrier_init(smem + 227848, 1);
            mbarrier_init(smem + 227856, 1);
            mbarrier_init(smem + 227864, 1);
            // tmem_full: 2 barriers, init_count=1
            mbarrier_init(smem + 227872, 1);
            mbarrier_init(smem + 227880, 1);
            // tmem_empty: 2 barriers, init_count=512
            mbarrier_init(smem + 227888, 512);
            mbarrier_init(smem + 227896, 512);
            // combine_barriers: 24 barriers, init_count=1
            mbarrier_init(smem + 227904, 1);
            mbarrier_init(smem + 227912, 1);
            mbarrier_init(smem + 227920, 1);
            mbarrier_init(smem + 227928, 1);
            mbarrier_init(smem + 227936, 1);
            mbarrier_init(smem + 227944, 1);
            mbarrier_init(smem + 227952, 1);
            mbarrier_init(smem + 227960, 1);
            mbarrier_init(smem + 227968, 1);
            mbarrier_init(smem + 227976, 1);
            mbarrier_init(smem + 227984, 1);
            mbarrier_init(smem + 227992, 1);
            mbarrier_init(smem + 228000, 1);
            mbarrier_init(smem + 228008, 1);
            mbarrier_init(smem + 228016, 1);
            mbarrier_init(smem + 228024, 1);
            mbarrier_init(smem + 228032, 1);
            mbarrier_init(smem + 228040, 1);
            mbarrier_init(smem + 228048, 1);
            mbarrier_init(smem + 228056, 1);
            mbarrier_init(smem + 228064, 1);
            mbarrier_init(smem + 228072, 1);
            mbarrier_init(smem + 228080, 1);
            mbarrier_init(smem + 228088, 1);
            // task_full: 2 barriers, init_count=1
            mbarrier_init(smem + 228096, 1);
            mbarrier_init(smem + 228104, 1);
            // task_empty: 2 barriers, init_count=512
            mbarrier_init(smem + 228112, 512);
            mbarrier_init(smem + 228120, 512);
            asm volatile("fence.mbarrier_init.release.cluster;" ::: "memory");
        }
    }
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 228128);
    if (warp == 3) {
        int _tmem_hold = smem + 228128;
    ...
    if (warp <= 3) {
        { // dispatch_main
            ...
            unsigned int first_token = ((unsigned int)(bid * 4) + warp) * 16;
            #pragma unroll
            for (int token_base = first_token; token_base < num_tokens; token_base += 128) {
                if ((unsigned int)token_base + lane / 2 < num_tokens && lane < 32) {
                    int expert = (int)TopK[(unsigned int)(token_base * 2) + lane];
                    if (expert >= 0) {
            ...
            asm volatile("barrier.sync 0, 128;" ::: "memory");
            #pragma unroll
            for (int token_base_1 = first_token; token_base_1 < num_tokens; token_base_1 += 128) {
                if ((unsigned int)token_base_1 + lane / 2 < num_tokens && lane < 32) {
                    int expert_2 = (int)TopK[(unsigned int)(token_base_1 * 2) + lane];
                    if (expert_2 >= 0) {
                        ...
                        SourceIndices[(unsigned int)(expert_2 * 1920) + slot] = (unsigned int)(token_base_1 * 2) + lane;
                    }
                }
                __syncwarp();
            }
            ...
            unsigned int interleave_waves = 2 + (total - 1) + 1 - 1 + 1;
            state[0] = total;
            int _max_0 = ((2) > (interleave_waves) ? (2) : (interleave_waves));
            int _min_0 = ((_max_0) < (waves) ? (_max_0) : (waves));
            state[1] = _min_0;
            #pragma unroll 1
            for (int token = (unsigned int)(bid * 4) + warp; token < num_tokens * 2; token += 8) {
                #pragma unroll 1
                ...
                unsigned int source_token = source_topk / 2;
                unsigned int pool_token = pool_offset * 16 + in_expert;
                unsigned int pool_block = pool_token / 16;
                unsigned int ring_block = pool_block % 240;
                unsigned int ring_token = pool_token % 3840;
                unsigned int target_1 = pool_block / 240 * 4;
                if (target_1 > 0) {
                    {
                    unsigned int _acquire_observed;
                    do {
                ...
                unsigned long long source_byte = (unsigned long long)source_token * 256;
                unsigned long long destination_byte = (unsigned long long)ring_token * 256;
                if (elect_sync()) {
                    #pragma unroll
                    for (int chunk = 0; chunk < 1; chunk++) {
                        cp_async_bulk_gmem2smem(send_buffer_addr + warp * 256, reinterpret_cast<const void*>(reinterpret_cast<const uint8_t*>(X) + ((unsigned long long)(source_byte + (unsigned long long)(chunk * 256)) * (unsigned long long)1)), 256, pull_barriers_addr + (warp) * 8);
                        mbarrier_arrive_expect_tx(pull_barriers_addr + (warp) * 8, 256);
                        if (chunk != 0) {
                            mbarrier_wait(pull_barriers_addr + (warp) * 8, pull_phase);
                            pull_phase ^= 1;
                            {
                                void* _cpbulk_dst_0 = reinterpret_cast<void*>(L1Acts + (destination_byte + (unsigned long long)(chunk * 256)));
                                asm volatile(
                                    "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
                                    :: "l"(_cpbulk_dst_0), "r"(send_buffer_addr + warp * 256), "r"((uint32_t)(256))
                                    : "memory");
                            }
                ...
                for (int sf_group = 0; sf_group < 1; sf_group++) {
                    ...
                    if (sf_word < 2) {
                        L1SF[sf_word * 61440 + sf_token] = XSF[source_token * 2 + sf_word];
                    }
                }
                __syncwarp();
                if (elect_sync()) {
                    ...
                    TokenMetadata[pool_token * 3 + 2] = source_topk % 2;
                    mbarrier_wait(pull_barriers_addr + (warp) * 8, pull_phase);
                    pull_phase ^= 1;
                    {
                        ...
                        asm volatile(
                            "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
                            :: "l"(_cpbulk_dst_1), "r"(send_buffer_addr + warp * 256), "r"((uint32_t)(256))
                            : "memory");
                    }
                    asm volatile("cp.async.bulk.commit_group;");
                    asm volatile("cp.async.bulk.wait_group 0;");
            ...
            if (bid == 0) {
                ...
                for (int expert_6 = bid - 1; expert_6 < 8; expert_6++) {
                    ...
                    for (int block_1 = tid; block_1 < blocks; block_1 += 128) {
                        unsigned int ring = (offset + (unsigned int)block_1) % 240;
                        L1Full[ring] = 0;
                        L1Empty[ring] = 0;
                        L2Mask[ring] = 0;
                        L2Empty[ring] = 0;
    ...
    if (warp == 4) {
        { // load_a_main
            ...
            #pragma unroll 1
            for (int task_iteration = 0; task_iteration < 1441; task_iteration++) {
                ...
                unsigned int ring_block_1 = pool_block_1 % 240;
                ...
                if (task[0] == 1) {
                    ...
                    } while (static_cast<unsigned int>(_acquire_observed - static_cast<unsigned int>(16 * (pool_block_1 / 240 + 1))) >= static_cast<unsigned int>(1));
                    }
                }
                if (task[0] == 4) {
                    {
                ...
                for (int k = 0; k < task[7] / 128; k++) {
                    if (task[0] == 2) {
                        ...
                        if ((pending & k_mask) != 0) {
                            ...
                            do {
                            asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_4) : "l"((reinterpret_cast<unsigned long long*>(L2Mask) + (ring_block_1))) : "memory");
                            } while (((_wait_acquire_mask_4 ^ static_cast<unsigned long long>(expected)) & static_cast<unsigned long long>(k_mask)) != 0);
                            unsigned long long observed = _wait_acquire_mask_4;
                    ...
                    __syncwarp();
                    a_stage += 1;
                    if (a_stage == 12) { a_stage = 0; _phase_empty ^= 1; }
                }
            }
        }
    }
    ...
    if (warp == 5) {
        { // load_b_main
            ...
            #pragma unroll 1
            for (int task_iteration_1 = 0; task_iteration_1 < 1441; task_iteration_1++) {
                ...
                for (int k_1 = 0; k_1 < task_1[7] / 128; k_1++) {
                    ...
                    __syncwarp();
                    b_stage += 1;
                    if (b_stage == 12) { b_stage = 0; _phase_empty_1 ^= 1; }
                }
            }
        }
    }
    ...
    if (warp == 6) {
        { // mma_main
            ...
            if (cta_rank == 0) {
                #pragma unroll 1
                for (int task_iteration_2 = 0; task_iteration_2 < 1441; task_iteration_2++) {
                    ...
                    for (int k_2 = 0; k_2 < task_2[7] / 128; k_2++) {
                        ...
                        __syncwarp();
                        mma_stage += 1;
                        if (mma_stage == 12) { mma_stage = 0; _phase_full ^= 1; }
                    }
                    completed_tasks += 1;
                }
    ...
    if (warp == 7) {
        { // schedule_main
            ...
            unsigned int early_shared_l2 = (unsigned int)((num_tokens + 16 - 1) / 16 <= 1);
            if (cta_rank == 0) {
                unsigned int total_1 = (num_tokens + 16 - 1) / 16 * 2;
                #pragma unroll 1
                for (int iteration = 0; iteration < 1441; iteration++) {
                ...
                if (early_shared_l2 != 0) {
                    unsigned int total_0 = (num_tokens + 16 - 1) / 16;
                    #pragma unroll 1
                    for (int iteration_1 = 0; iteration_1 < 1441; iteration_1++) {
                        mbarrier_wait(task_empty_addr + (schedule_iteration[0] % 2) * 8, schedule_iteration[0] / 2 & 1 ^ 1);
                        ...
                        unsigned int block_4 = claimed_0_1;
                        task_3[0] = 4;
                        task_3[1] = 0;
                        task_3[2] = block_4;
                        task_3[3] = 0;
                ...
                if (expert_7 < 8) {
                    ...
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_3) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_7))) : "memory");
                    } while (((_wait_acquire_mask_3 ^ static_cast<unsigned long long>((unsigned long long)2 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                ...
                if (early_shared_l2 == 0) {
                    unsigned int total_1_1 = (num_tokens + 16 - 1) / 16;
                    #pragma unroll 1
                    for (int iteration_3 = 0; iteration_3 < 1441; iteration_3++) {
                        mbarrier_wait(task_empty_addr + (schedule_iteration[0] % 2) * 8, schedule_iteration[0] / 2 & 1 ^ 1);
                        ...
                        unsigned int block_5 = claimed_0_4;
                        task_3[0] = 4;
                        task_3[1] = 0;
                        task_3[2] = block_5;
                        task_3[3] = 0;
                        task_3[4] = block_5;
                        unsigned int _min_5 = ((num_tokens - block_5 * 16) < (16) ? (num_tokens - block_5 * 16) : (16));
                        task_3[5] = _min_5;
                        task_3[6] = 256;
                        task_3[7] = 256;
                        if (lane < 2) {
    ...
    if (warp >= 8 && warp <= 15) {
        { // epilogue_main
            ...
            asm volatile("barrier.sync 1, 384;" ::: "memory");
            #pragma unroll 1
            for (int task_iteration_3 = 0; task_iteration_3 < 1441; task_iteration_3++) {
                ...
                if (task_4[0] == 1 || task_4[0] == 3) {
                    ...
                    unsigned int ring_block_2 = pool_block_3 % 240;
                    unsigned int block_6 = ring_block_2;
                    unsigned int sf_stride = 245760;
                    if (task_4[0] == 3) {
                    ...
                    if (task_4[0] == 1) {
                        ...
                        } while (static_cast<unsigned int>(_acquire_observed - static_cast<unsigned int>(2 * (pool_block_3 / 240))) >= static_cast<unsigned int>(1));
                        }
                    }
                    #pragma unroll
                    for (int s = 0; s < 1; s++) {
                    ...
                    if (task_4[0] == 2) {
                        if (epi_warp_1 == 0) {
                            if (elect_sync()) {
                                atomicAdd(&L2Empty[task_4[4] % 240], 1);
                            }
                        }
                        __syncwarp();
                    }
                    ...
                    for (int s_1 = 0; s_1 < 1; s_1++) {
                        ...
                        for (int j_2 = 0; j_2 < 1; j_2++) {
                            ...
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                ...
                                : "r"(read_address));
                            unsigned long long dst_index = ((unsigned long long)cached_topk[j_2] * 1920 + (unsigned long long)cached_token[j_2]) * 128 + (unsigned long long)(n_offset_1 / 2) + (unsigned long long)(lane % 16 * 4);
            ...
            if (epi_thread == 0) {
                ...
                unsigned int _wait_acquire_mask_5;
                do {
                asm volatile("ld.acquire.gpu.global.u32 %0, [%1];" : "=r"(_wait_acquire_mask_5) : "l"((reinterpret_cast<unsigned int*>(GridCounters) + (1))) : "memory");
                } while (((_wait_acquire_mask_5 ^ static_cast<unsigned int>((old_3 ^ 2147483648) & 2147483648)) & static_cast<unsigned int>(2147483648)) != 0);
            }
            asm volatile("bar.sync 2, 256;" ::: "memory");
            if (bid == 0 && epi_thread == 0) {
            ...
            for (unsigned int token_chunk = epi_warp_2 * 2 + (unsigned int)bid; token_chunk < num_tokens; token_chunk += 16) {
                ...
                int expert_8 = -1;
                if (lane < 2) {
                    expert_8 = (int)TopK[token_1 * 2 + lane];
                }
                if (lane == 2) {
                    expert_8 = 2;
                }
                ...
                #pragma unroll 1
                for (unsigned int i_3 = 0; i_3 < 3; i_3++) {
                    if (mask != 0) {
                        int _ffs_1 = __ffs(mask);
                        unsigned int slot_1 = (unsigned int)(_ffs_1 - 1);
                        mask ^= (unsigned int)1 << slot_1;
                        if (elect_sync()) {
                            cp_async_bulk_gmem2smem(reusable_addr + (epi_warp_2 * 3 + i_3) * 512, reinterpret_cast<const void*>(reinterpret_cast<const uint8_t*>(CombineBytes) + ((unsigned long long)(((unsigned long long)slot_1 * 1920 + (unsigned long long)token_1) * 256 * 2 + (unsigned long long)(chunk_1 * 512)) * (unsigned long long)1)), 512, combine_barriers_addr + (epi_warp_2 * 3 + i_3) * 8);
                            mbarrier_arrive_expect_tx(combine_barriers_addr + (epi_warp_2 * 3 + i_3) * 8, 512);
                        }
                        __syncwarp();
                        issued += 1;
                    }
                ...
                reduced[7] = 0.0f;
                #pragma unroll 1
                for (unsigned int i_4 = 0; i_4 < 3; i_4++) {
                    if (issued > i_4) {
                        mbarrier_wait(combine_barriers_addr + (epi_warp_2 * 3 + i_4) * 8, phases >> i_4 & 1);
                        #pragma unroll
                        for (unsigned int j_3 = 0; j_3 < 1; j_3++) {
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                ...
                                : "r"(reusable_addr + (epi_warp_2 * 3 + i_4) * 512 + (j_3 * 32 + lane) * 16));
                ...
                __syncwarp();
                #pragma unroll
                for (unsigned int j_4 = 0; j_4 < 1; j_4++) {
                    ...
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" ::
                        "r"(reusable_addr + (24 + epi_warp_2) * 512 + (j_4 * 32 + lane) * 16), "r"(*reinterpret_cast<uint32_t*>(&store_values[0])), "r"(*reinterpret_cast<uint32_t*>(&store_values[(0) + 1])), "r"(*reinterpret_cast<uint32_t*>(&store_values[(0) + 2])), "r"(*reinterpret_cast<uint32_t*>(&store_values[(0) + 3])));
                }
                __syncwarp();
                if (elect_sync()) {
                    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                    {
                        void* _cpbulk_dst_4 = reinterpret_cast<void*>(Y + ((unsigned long long)token_1 * 256 * 2 + (unsigned long long)(chunk_1 * 512)));
                        asm volatile(
                            "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
                            :: "l"(_cpbulk_dst_4), "r"(reusable_addr + (24 + epi_warp_2) * 512), "r"((uint32_t)(512))
                            : "memory");
                    }
                    asm volatile("cp.async.bulk.commit_group;");
                }
