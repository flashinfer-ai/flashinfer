...
#define TMEM_SCALE_A_OFFSET 64
#define TMEM_SCALE_B_OFFSET 68
...
#define SMEM_SMEM_OUTPUT_STRIDE 8192
#define SMEM_REUSABLE_OFF 0
#define SMEM_REUSABLE_STAGE_BYTES 225856
#define SMEM_REUSABLE_STRIDE 225856
#define SMEM_SMEM_A_OFF 30720
#define SMEM_SMEM_A_STAGE_BYTES 2048
#define SMEM_SMEM_A_STRIDE 2048
#define SMEM_SMEM_B_OFF 51200
#define SMEM_SMEM_B_STAGE_BYTES 16384
#define SMEM_SMEM_B_STRIDE 16384
#define SMEM_SMEM_SHARED_B_OFF 51200
...
#define SMEM_SMEM_SFB_OFF 220160
#define SMEM_SMEM_SFB_STAGE_BYTES 512
#define SMEM_SMEM_SFB_STRIDE 512
#define SMEM_AMAX_REDUCTION_OFF 225280
#define SMEM_AMAX_REDUCTION_STAGE_BYTES 512
#define SMEM_AMAX_REDUCTION_STRIDE 512
#define SMEM_TASK_INFOS_OFF 225792
...
kernel_deepgemm_source_mega_moe_sm103a_54706a14764b4d257567(DeepgemmTensorMap const* A1, DeepgemmTensorMap const* A2, DeepgemmTensorMap const* SA1, DeepgemmTensorMap const* SA2, DeepgemmTensorMap const* B1, DeepgemmTensorMap const* B2, DeepgemmTensorMap const* SB1, DeepgemmTensorMap const* SB2, DeepgemmTensorMap const* SFA1, DeepgemmTensorMap const* SFA2, DeepgemmTensorMap const* SSFA1, DeepgemmTensorMap const* SSFA2, DeepgemmTensorMap const* SFB1, DeepgemmTensorMap const* SFB2, DeepgemmTensorMap const* SSFB1, DeepgemmTensorMap const* SSFB2, DeepgemmTensorMap const* L1Output, DeepgemmTensorMap const* SharedL1Output, uint8_t* __restrict__ X, unsigned int* __restrict__ XSF, long long* __restrict__ TopK, float* __restrict__ Weights, uint8_t* __restrict__ L1Acts, unsigned int* __restrict__ L1SF, float* __restrict__ L1Weights, uint8_t* __restrict__ L2SF, uint8_t* __restrict__ SharedL2SF, unsigned int* __restrict__ SourceIndices, unsigned int* __restrict__ TokenMetadata, unsigned int* __restrict__ GridCounters, unsigned int* __restrict__ NvlCounter, unsigned int* __restrict__ NvlSignals, unsigned long long* __restrict__ PeerGrid, unsigned long long* __restrict__ ReadyGrid, unsigned long long* __restrict__ SendCounts, unsigned long long* __restrict__ RecvCounts, unsigned long long* __restrict__ RecvSum, unsigned int* __restrict__ L1Full, unsigned int* __restrict__ L1Empty, unsigned long long* __restrict__ L2Mask, unsigned int* __restrict__ L2Empty, unsigned int* __restrict__ SharedFull, unsigned int* __restrict__ L1Counter, unsigned int* __restrict__ L2Counter, unsigned int* __restrict__ SharedL1Counter, unsigned int* __restrict__ SharedL2Counter, unsigned int* __restrict__ Combine, uint8_t* __restrict__ CombineBytes, uint8_t* __restrict__ Y, unsigned int num_tokens)
{
    ...
    const int mbar_base = smem + 225856;
    ...
    #define combine_barriers_addr (mbar_base + 224)
    #define task_full_addr (mbar_base + 672)
    #define task_empty_addr (mbar_base + 688)
    ...
    uint8_t* smem_a = reinterpret_cast<uint8_t*>(smem_raw + 30720);
    const int smem_a_addr = smem + 30720;
    uint8_t* smem_b = reinterpret_cast<uint8_t*>(smem_raw + 51200);
    const int smem_b_addr = smem + 51200;
    uint8_t* smem_shared_b = reinterpret_cast<uint8_t*>(smem_raw + 51200);
    const int smem_shared_b_addr = smem + 51200;
    unsigned int* smem_sfa = reinterpret_cast<unsigned int*>(smem_raw + 215040);
    const int smem_sfa_addr = smem + 215040;
    unsigned int* smem_sfb = reinterpret_cast<unsigned int*>(smem_raw + 220160);
    const int smem_sfb_addr = smem + 220160;
    float* amax_reduction = reinterpret_cast<float*>(smem_raw + 225280);
    const int amax_reduction_addr = smem + 225280;
    unsigned int* task_infos = reinterpret_cast<unsigned int*>(smem_raw + 225792);
    const int task_infos_addr = smem + 225792;
    ...
    // Mbarrier init (8 pipeline groups, 0 ordered-sequence groups, 88 barriers)
    // Mbarriers at smem_raw[225856..226560)

    if (warp == 1) {
        uint32_t leader = elect_sync();
        if (leader) {
            // pull_barriers: 4 barriers, init_count=1
            mbarrier_init(smem + 225856, 1);
            mbarrier_init(smem + 225864, 1);
            mbarrier_init(smem + 225872, 1);
            mbarrier_init(smem + 225880, 1);
    ...
    if (warp == 2) {
        ...
        if (leader) {
            ...
            mbarrier_init(smem + 225888, 4);
            mbarrier_init(smem + 225896, 4);
            mbarrier_init(smem + 225904, 4);
            mbarrier_init(smem + 225912, 4);
            mbarrier_init(smem + 225920, 4);
            mbarrier_init(smem + 225928, 4);
            mbarrier_init(smem + 225936, 4);
            mbarrier_init(smem + 225944, 4);
            mbarrier_init(smem + 225952, 4);
            mbarrier_init(smem + 225960, 4);
            // empty: 10 barriers, init_count=1
            mbarrier_init(smem + 225968, 1);
            mbarrier_init(smem + 225976, 1);
            mbarrier_init(smem + 225984, 1);
            mbarrier_init(smem + 225992, 1);
            mbarrier_init(smem + 226000, 1);
            mbarrier_init(smem + 226008, 1);
            mbarrier_init(smem + 226016, 1);
            mbarrier_init(smem + 226024, 1);
            mbarrier_init(smem + 226032, 1);
            mbarrier_init(smem + 226040, 1);
            // tmem_full: 2 barriers, init_count=1
            mbarrier_init(smem + 226048, 1);
            mbarrier_init(smem + 226056, 1);
            // tmem_empty: 2 barriers, init_count=512
            mbarrier_init(smem + 226064, 512);
            mbarrier_init(smem + 226072, 512);
            // combine_barriers: 56 barriers, init_count=1
            mbarrier_init(smem + 226080, 1);
            mbarrier_init(smem + 226088, 1);
            mbarrier_init(smem + 226096, 1);
            mbarrier_init(smem + 226104, 1);
            mbarrier_init(smem + 226112, 1);
            mbarrier_init(smem + 226120, 1);
            mbarrier_init(smem + 226128, 1);
            mbarrier_init(smem + 226136, 1);
            mbarrier_init(smem + 226144, 1);
            mbarrier_init(smem + 226152, 1);
            mbarrier_init(smem + 226160, 1);
            mbarrier_init(smem + 226168, 1);
            mbarrier_init(smem + 226176, 1);
            mbarrier_init(smem + 226184, 1);
            mbarrier_init(smem + 226192, 1);
            mbarrier_init(smem + 226200, 1);
            mbarrier_init(smem + 226208, 1);
            mbarrier_init(smem + 226216, 1);
            mbarrier_init(smem + 226224, 1);
            mbarrier_init(smem + 226232, 1);
            mbarrier_init(smem + 226240, 1);
            mbarrier_init(smem + 226248, 1);
            mbarrier_init(smem + 226256, 1);
            mbarrier_init(smem + 226264, 1);
            mbarrier_init(smem + 226272, 1);
            mbarrier_init(smem + 226280, 1);
            mbarrier_init(smem + 226288, 1);
            mbarrier_init(smem + 226296, 1);
            mbarrier_init(smem + 226304, 1);
            mbarrier_init(smem + 226312, 1);
            mbarrier_init(smem + 226320, 1);
            mbarrier_init(smem + 226328, 1);
            mbarrier_init(smem + 226336, 1);
            mbarrier_init(smem + 226344, 1);
            mbarrier_init(smem + 226352, 1);
            mbarrier_init(smem + 226360, 1);
            mbarrier_init(smem + 226368, 1);
            mbarrier_init(smem + 226376, 1);
            mbarrier_init(smem + 226384, 1);
            mbarrier_init(smem + 226392, 1);
            mbarrier_init(smem + 226400, 1);
            mbarrier_init(smem + 226408, 1);
            mbarrier_init(smem + 226416, 1);
            mbarrier_init(smem + 226424, 1);
            mbarrier_init(smem + 226432, 1);
            mbarrier_init(smem + 226440, 1);
            mbarrier_init(smem + 226448, 1);
            mbarrier_init(smem + 226456, 1);
            mbarrier_init(smem + 226464, 1);
            mbarrier_init(smem + 226472, 1);
            mbarrier_init(smem + 226480, 1);
            mbarrier_init(smem + 226488, 1);
            mbarrier_init(smem + 226496, 1);
            mbarrier_init(smem + 226504, 1);
            mbarrier_init(smem + 226512, 1);
            mbarrier_init(smem + 226520, 1);
            // task_full: 2 barriers, init_count=1
            mbarrier_init(smem + 226528, 1);
            mbarrier_init(smem + 226536, 1);
            // task_empty: 2 barriers, init_count=512
            mbarrier_init(smem + 226544, 512);
            mbarrier_init(smem + 226552, 512);
            asm volatile("fence.mbarrier_init.release.cluster;" ::: "memory");
        }
    }
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 226560);
    if (warp == 3) {
        int _tmem_hold = smem + 226560;
    ...
    const int tmem_scale_a = taddr + 64;
    const int tmem_scale_b = taddr + 68;
    ...
    if (warp <= 3) {
        { // dispatch_main
            ...
            if (lane < 384) {
                num_blocks += (counts[0] + 32 - 1) / 32;
            }
            if (32 + lane < 384) {
                num_blocks += (counts[1] + 32 - 1) / 32;
            }
            if (64 + lane < 384) {
                num_blocks += (counts[2] + 32 - 1) / 32;
            }
            if (96 + lane < 384) {
                num_blocks += (counts[3] + 32 - 1) / 32;
            }
            if (128 + lane < 384) {
                num_blocks += (counts[4] + 32 - 1) / 32;
            }
            if (160 + lane < 384) {
                num_blocks += (counts[5] + 32 - 1) / 32;
            }
            if (192 + lane < 384) {
                num_blocks += (counts[6] + 32 - 1) / 32;
            }
            if (224 + lane < 384) {
                num_blocks += (counts[7] + 32 - 1) / 32;
            }
            if (256 + lane < 384) {
                num_blocks += (counts[8] + 32 - 1) / 32;
            }
            if (288 + lane < 384) {
                num_blocks += (counts[9] + 32 - 1) / 32;
            }
            if (320 + lane < 384) {
                num_blocks += (counts[10] + 32 - 1) / 32;
            }
            if (352 + lane < 384) {
                num_blocks += (counts[11] + 32 - 1) / 32;
            }
            ...
            for (int token = (unsigned int)(bid * 4) + warp; token < num_tokens * 6; token += 608) {
                ...
                for (int advance_expert = 0; advance_expert < 385; advance_expert++) {
                    ...
                    pool_offset += (expert_end - expert_start + 32 - 1) / 32;
                ...
                unsigned int pool_token = pool_offset * 32 + in_expert;
                unsigned int pool_block = pool_token / 32;
                unsigned int ring_block = pool_block % 480;
                unsigned int ring_token = pool_token % 15360;
                unsigned int target_1 = pool_block / 480 * 36;
                if (target_1 > 0) {
                    {
                    unsigned int _acquire_observed;
                    do {
                ...
                unsigned int in_block = in_expert % 32;
                ...
                if (elect_sync()) {
                    ...
                    asm volatile("cp.async.bulk.commit_group;");
                    asm volatile("cp.async.bulk.wait_group 0;");
                    unsigned int arrivals = (((unsigned int)token == expert_end - 1) ? 32 - in_block : (unsigned int)1);
            ...
            if (bid == 0) {
                ...
                for (int expert_7 = bid - 1; expert_7 < 384; expert_7 += 151) {
                    ...
                    for (int block_1 = tid; block_1 < blocks; block_1 += 128) {
                        unsigned int ring = (offset + (unsigned int)block_1) % 480;
                        L1Full[ring] = 0;
                        L1Empty[ring] = 0;
                        L2Mask[ring] = 0;
                        L2Empty[ring] = 0;
    ...
    if (warp == 4) {
        { // load_a_main
            ...
            #pragma unroll 1
            for (int task_iteration = 0; task_iteration < 125401; task_iteration++) {
                ...
                unsigned int ring_block_1 = pool_block_1 % 480;
                ...
                if (task[0] == 1) {
                    ...
                    } while (static_cast<unsigned int>(_acquire_observed - static_cast<unsigned int>(32 * (pool_block_1 / 480 + 1))) >= static_cast<unsigned int>(1));
                    }
                }
                if (task[0] == 4) {
                    {
                ...
                if ((pool_block_1 / 480 & 1) != 0) {
                    expected = 0;
                }
                unsigned long long pending = 68719476735;
                unsigned int token_offset = block_2 * 32 + (unsigned int)cta_rank * ((task[5] + 15) / 16 * 8);
                ...
                for (int k = 0; k < task[7] / 128; k++) {
                    if (task[0] == 2) {
                        ...
                        if ((pending & k_mask) != 0) {
                            ...
                            do {
                            asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_26) : "l"((reinterpret_cast<unsigned long long*>(L2Mask) + (ring_block_1))) : "memory");
                            } while (((_wait_acquire_mask_26 ^ static_cast<unsigned long long>(expected)) & static_cast<unsigned long long>(k_mask)) != 0);
                            unsigned long long observed = _wait_acquire_mask_26;
                            pending = observed ^ expected;
                        }
                    }
                    mbarrier_wait(empty_addr + (a_stage) * 8, _phase_empty);
                    if (elect_sync()) {
                        tma_3d_gmem2smem_cta2(smem_a_addr + a_stage * 2048, selected_activation_map, 0, token_offset, k, ((full_addr + (a_stage) * 8) & 0xFEFFFFFF));
                        tma_2d_gmem2smem_cta2(smem_sfa_addr + a_stage * 512, selected_activation_sf_map, sf_offset, k, ((full_addr + (a_stage) * 8) & 0xFEFFFFFF));
                        if (cta_rank == 0) {
                            mbarrier_arrive_expect_tx(full_addr + (a_stage) * 8, 5120);
                        } else {
                            asm volatile(
    ...
    if (warp == 5) {
        { // load_b_main
            ...
            #pragma unroll 1
            for (int task_iteration_1 = 0; task_iteration_1 < 125401; task_iteration_1++) {
                ...
                for (int k_1 = 0; k_1 < task_1[7] / 128; k_1++) {
                    ...
                    __syncwarp();
                    b_stage += 1;
                    if (b_stage == 10) { b_stage = 0; _phase_empty_1 ^= 1; }
                }
            }
        }
    }
    ...
    if (warp == 6) {
        { // mma_main
            ...
            if (cta_rank == 0) {
                #pragma unroll 1
                for (int task_iteration_2 = 0; task_iteration_2 < 125401; task_iteration_2++) {
                    ...
                    unsigned int selected_mma_idesc = (unsigned int)((unsigned int)(((task_2[0] > 2) ? 277348352 : 277348992) & -8257537) | aligned_m >> 3 << 17);
                    ...
                    for (int k_2 = 0; k_2 < task_2[7] / 128; k_2++) {
                        ...
                        { // Pre-election register operand materialization
                            ...
                            int _mma_b_lo_0 = (((smem_a_addr) >> 4) & 0x3FFF) + (mma_stage) * 128;
                            uint64_t a_desc = ((uint64_t)(uint32_t)_mma_a_lo_0) | ((uint64_t)0x40004040 << 32);
                            uint64_t b_desc = ((uint64_t)(uint32_t)_mma_b_lo_0) | ((uint64_t)0x40004040 << 32);

                            uint32_t _election_operand_2_0 = ((tmem_accum + (accum_stage * 32)));
                            ...
                            uint32_t _election_operand_3_0 = ((tmem_accum + (accum_stage * 32)));
                            ...
                            uint32_t _election_operand_4_0 = ((tmem_accum + (accum_stage * 32)));
                            ...
                            uint32_t _election_operand_5_0 = ((tmem_accum + (accum_stage * 32)));
                        ...
                        __syncwarp();
                        mma_stage += 1;
                        if (mma_stage == 10) { mma_stage = 0; _phase_full ^= 1; }
                    }
                    completed_tasks += 1;
                }
    ...
    if (warp == 7) {
        { // schedule_main
            ...
            unsigned int early_shared_l2 = (unsigned int)((num_tokens + 32 - 1) / 32 * 20 <= 76);
            if (cta_rank == 0) {
                unsigned int total_1 = (num_tokens + 32 - 1) / 32 * 18;
                #pragma unroll 1
                for (int iteration = 0; iteration < 125401; iteration++) {
                    ...
                    task_3[4] = block_3;
                    unsigned int _min_1 = ((num_tokens - block_3 * 32) < (32) ? (num_tokens - block_3 * 32) : (32));
                    task_3[5] = _min_1;
                    task_3[6] = 4608;
                    task_3[7] = 5120;
                    if (lane < 2) {
                ...
                if (early_shared_l2 != 0) {
                    unsigned int total_0 = (num_tokens + 32 - 1) / 32 * 20;
                    #pragma unroll 1
                    for (int iteration_1 = 0; iteration_1 < 125401; iteration_1++) {
                        mbarrier_wait(task_empty_addr + (schedule_iteration[0] % 2) * 8, schedule_iteration[0] / 2 & 1 ^ 1);
                        ...
                        task_3[4] = block_4;
                        unsigned int _min_2 = ((num_tokens - block_4 * 32) < (32) ? (num_tokens - block_4 * 32) : (32));
                        task_3[5] = _min_2;
                        task_3[6] = 5120;
                        task_3[7] = 2304;
                        if (lane < 2) {
                ...
                if (expert_9 < 384) {
                    unsigned long long _wait_acquire_mask_14;
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_14) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_9))) : "memory");
                    } while (((_wait_acquire_mask_14 ^ static_cast<unsigned long long>((unsigned long long)152 << 32)) & static_cast<unsigned long long>((unsigned long long)4294967295 << 32)) != 0);
                    received_2 = _wait_acquire_mask_14;
                }
                ...
                if (expert_0_1 < 384) {
                    ...
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_15) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_0_1))) : "memory");
                    ...
                    received_1_1 = _wait_acquire_mask_15;
                }
                ...
                if (expert_2_2 < 384) {
                    ...
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_16) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_2_2))) : "memory");
                    ...
                    received_3_1 = _wait_acquire_mask_16;
                }
                ...
                if (expert_4_2 < 384) {
                    ...
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_17) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_4_2))) : "memory");
                    ...
                    received_5_1 = _wait_acquire_mask_17;
                }
                ...
                if (expert_6_1 < 384) {
                    ...
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_18) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_6_1))) : "memory");
                    ...
                    received_7_1 = _wait_acquire_mask_18;
                }
                ...
                if (expert_8_1 < 384) {
                    ...
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_19) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_8_1))) : "memory");
                    ...
                    received_9_1 = _wait_acquire_mask_19;
                }
                ...
                if (expert_10_1 < 384) {
                    ...
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_20) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_10_1))) : "memory");
                    ...
                    received_11_1 = _wait_acquire_mask_20;
                }
                ...
                if (expert_12_1 < 384) {
                    ...
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_21) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_12_1))) : "memory");
                    ...
                    received_13_1 = _wait_acquire_mask_21;
                }
                ...
                if (expert_14_1 < 384) {
                    ...
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_22) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_14_1))) : "memory");
                    ...
                    received_15_1 = _wait_acquire_mask_22;
                }
                ...
                if (expert_16_1 < 384) {
                    ...
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_23) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_16_1))) : "memory");
                    ...
                    received_17_1 = _wait_acquire_mask_23;
                }
                ...
                if (expert_18_1 < 384) {
                    ...
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_24) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_18_1))) : "memory");
                    ...
                    received_19_1 = _wait_acquire_mask_24;
                }
                ...
                if (expert_20_1 < 384) {
                    ...
                    do {
                    asm volatile("ld.acquire.gpu.global.u64 %0, [%1];" : "=l"(_wait_acquire_mask_25) : "l"((reinterpret_cast<unsigned long long*>(RecvSum) + (expert_20_1))) : "memory");
                    ...
                    received_21_1 = _wait_acquire_mask_25;
                }
                counts_1[11] = (unsigned int)received_21_1;
                __syncwarp();
                unsigned int num_blocks_1 = 0;
                if (lane < 384) {
                    num_blocks_1 += (counts_1[0] + 32 - 1) / 32;
                }
                if (32 + lane < 384) {
                    num_blocks_1 += (counts_1[1] + 32 - 1) / 32;
                }
                if (64 + lane < 384) {
                    num_blocks_1 += (counts_1[2] + 32 - 1) / 32;
                }
                if (96 + lane < 384) {
                    num_blocks_1 += (counts_1[3] + 32 - 1) / 32;
                }
                if (128 + lane < 384) {
                    num_blocks_1 += (counts_1[4] + 32 - 1) / 32;
                }
                if (160 + lane < 384) {
                    num_blocks_1 += (counts_1[5] + 32 - 1) / 32;
                }
                if (192 + lane < 384) {
                    num_blocks_1 += (counts_1[6] + 32 - 1) / 32;
                }
                if (224 + lane < 384) {
                    num_blocks_1 += (counts_1[7] + 32 - 1) / 32;
                }
                if (256 + lane < 384) {
                    num_blocks_1 += (counts_1[8] + 32 - 1) / 32;
                }
                if (288 + lane < 384) {
                    num_blocks_1 += (counts_1[9] + 32 - 1) / 32;
                }
                if (320 + lane < 384) {
                    num_blocks_1 += (counts_1[10] + 32 - 1) / 32;
                }
                if (352 + lane < 384) {
                    num_blocks_1 += (counts_1[11] + 32 - 1) / 32;
                }
                ...
                state_1[1] = _min_3;
                #pragma unroll 1
                for (int iteration_2 = 0; iteration_2 < 125401; iteration_2++) {
                    mbarrier_wait(task_empty_addr + (schedule_iteration[0] % 2) * 8, schedule_iteration[0] / 2 & 1 ^ 1);
                    ...
                    if (phase_1 != 0) {
                        ...
                        unsigned int blocks_1 = (tokens + 32 - 1) / 32;
                        ...
                        if (owner_mask != 0) {
                            ...
                            unsigned int _min_4 = ((tokens - local_block * 32) < (32) ? (tokens - local_block * 32) : (32));
                        ...
                        unsigned int blocks_9 = (tokens_8 + 32 - 1) / 32;
                        ...
                        if (owner_mask_17 != 0) {
                            ...
                            unsigned int _min_5 = ((tokens_8 - local_block_1 * 32) < (32) ? (tokens_8 - local_block_1 * 32) : (32));
                        ...
                        unsigned int blocks_21 = (tokens_20 + 32 - 1) / 32;
                        ...
                        if (owner_mask_29 != 0) {
                            ...
                            unsigned int _min_6 = ((tokens_20 - local_block_2 * 32) < (32) ? (tokens_20 - local_block_2 * 32) : (32));
                        ...
                        unsigned int blocks_32 = (tokens_31 + 32 - 1) / 32;
                        ...
                        if (owner_mask_40 != 0) {
                            ...
                            unsigned int _min_7 = ((tokens_31 - local_block_3 * 32) < (32) ? (tokens_31 - local_block_3 * 32) : (32));
                        ...
                        unsigned int blocks_43 = (tokens_42 + 32 - 1) / 32;
                        ...
                        if (owner_mask_51 != 0) {
                            ...
                            unsigned int _min_8 = ((tokens_42 - local_block_4 * 32) < (32) ? (tokens_42 - local_block_4 * 32) : (32));
                        ...
                        unsigned int blocks_54 = (tokens_53 + 32 - 1) / 32;
                        ...
                        if (owner_mask_62 != 0) {
                            ...
                            unsigned int _min_9 = ((tokens_53 - local_block_5 * 32) < (32) ? (tokens_53 - local_block_5 * 32) : (32));
                        ...
                        unsigned int blocks_65 = (tokens_64 + 32 - 1) / 32;
                        ...
                        if (owner_mask_73 != 0) {
                            ...
                            unsigned int _min_10 = ((tokens_64 - local_block_6 * 32) < (32) ? (tokens_64 - local_block_6 * 32) : (32));
                        ...
                        unsigned int blocks_76 = (tokens_75 + 32 - 1) / 32;
                        ...
                        if (owner_mask_84 != 0) {
                            ...
                            unsigned int _min_11 = ((tokens_75 - local_block_7 * 32) < (32) ? (tokens_75 - local_block_7 * 32) : (32));
                        ...
                        unsigned int blocks_87 = (tokens_86 + 32 - 1) / 32;
                        ...
                        if (owner_mask_95 != 0) {
                            ...
                            unsigned int _min_12 = ((tokens_86 - local_block_8 * 32) < (32) ? (tokens_86 - local_block_8 * 32) : (32));
                        ...
                        unsigned int blocks_98 = (tokens_97 + 32 - 1) / 32;
                        ...
                        if (owner_mask_106 != 0) {
                            ...
                            unsigned int _min_13 = ((tokens_97 - local_block_9 * 32) < (32) ? (tokens_97 - local_block_9 * 32) : (32));
                        ...
                        unsigned int blocks_109 = (tokens_108 + 32 - 1) / 32;
                        ...
                        if (owner_mask_117 != 0) {
                            ...
                            unsigned int _min_14 = ((tokens_108 - local_block_10 * 32) < (32) ? (tokens_108 - local_block_10 * 32) : (32));
                        ...
                        unsigned int blocks_120 = (tokens_119 + 32 - 1) / 32;
                        ...
                        if (owner_mask_128 != 0) {
                            ...
                            unsigned int _min_15 = ((tokens_119 - local_block_11 * 32) < (32) ? (tokens_119 - local_block_11 * 32) : (32));
                ...
                if (early_shared_l2 == 0) {
                    unsigned int total_0_1 = (num_tokens + 32 - 1) / 32 * 20;
                    #pragma unroll 1
                    for (int iteration_3 = 0; iteration_3 < 125401; iteration_3++) {
                        mbarrier_wait(task_empty_addr + (schedule_iteration[0] % 2) * 8, schedule_iteration[0] / 2 & 1 ^ 1);
                        ...
                        task_3[4] = block_5;
                        unsigned int _min_16 = ((num_tokens - block_5 * 32) < (32) ? (num_tokens - block_5 * 32) : (32));
                        task_3[5] = _min_16;
                        task_3[6] = 5120;
                        task_3[7] = 2304;
                        if (lane < 2) {
    ...
    if (warp >= 8 && warp <= 15) {
        { // epilogue_main
            ...
            asm volatile("barrier.sync 1, 384;" ::: "memory");
            #pragma unroll 1
            for (int task_iteration_3 = 0; task_iteration_3 < 125401; task_iteration_3++) {
                ...
                if (task_4[0] == 1 || task_4[0] == 3) {
                    ...
                    unsigned int ring_block_2 = pool_block_3 % 480;
                    ...
                    if (task_4[0] == 1) {
                        ...
                        } while (static_cast<unsigned int>(_acquire_observed - static_cast<unsigned int>(40 * (pool_block_3 / 480))) >= static_cast<unsigned int>(1));
                        }
                    }
                    #pragma unroll
                    for (int s = 0; s < 1; s++) {
                        if (valid_m <= epi_wg * 16 + (unsigned int)(s * 16)) {
                        ...
                        #pragma unroll
                        for (int i = 0; i < 2; i++) {
                            unsigned int j = s * 2 + i;
                            if (task_4[0] == 1 && j * 8 % 32 == 0) {
                                if (j * 8 + lane < 16) {
                                    cached_weight = L1Weights[ring_block_2 * 32 + epi_wg * 16 + j * 8 + lane];
                                }
                            }
                            float _shfl_55;
                            asm volatile("shfl.sync.idx.b32 %0, %1, %2, 0x1f, 0xffffffff;" : "=f"(_shfl_55) : "f"(cached_weight), "r"(j * 8 % 32 + lane % 4 * 2));
                            float first_weight = _shfl_55;
                            float _shfl_56;
                            asm volatile("shfl.sync.idx.b32 %0, %1, %2, 0x1f, 0xffffffff;" : "=f"(_shfl_56) : "f"(cached_weight), "r"(j * 8 % 32 + lane % 4 * 2 + 1));
                            float second_weight = _shfl_56;
                            unsigned int address = accum_stage_1 * 32 + epi_wg * 16 + j * 8;
                            ...
                            if (lane < 4) {
                                amax_reduction[epi_warp * 16 + (unsigned int)(i * 8) + lane * 2] = first_max;
                                amax_reduction[epi_warp * 16 + (unsigned int)(i * 8) + lane * 2 + 1] = second_max;
                            }
                            __syncwarp();
                        }
                        unsigned int tma_stage = s % 2;
                        unsigned int store_address = smem_output_addr + (epi_wg * 2 + tma_stage) * 16 * 64;
                        asm volatile("cp.async.bulk.wait_group 1;");
                        asm volatile("bar.sync %0, 128;" :: "r"(3 + epi_wg) : "memory");
                        #pragma unroll
                        for (int i_1 = 0; i_1 < 2; i_1++) {
                            unsigned int paired_index = (epi_warp ^ 1) * 16 + (unsigned int)(i_1 * 8) + lane % 4 * 2;
                            ...
                            if (warp_in_wg % 2 == 0 && lane < 4) {
                                ...
                                unsigned int token_base_2 = epi_wg * 16 + (unsigned int)(s * 16) + (unsigned int)(i_1 * 8);
                        ...
                        if (warp_in_wg == 0) {
                            if (elect_sync()) {
                                asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                                if (task_4[0] == 3) {
                                    tma_store_2d(SharedL1Output, n_block * 64, block_6 * 32 + epi_wg * 16 + (unsigned int)(s * 16), store_address);
                                } else {
                                    tma_store_2d(L1Output, n_block * 64, block_6 * 32 + epi_wg * 16 + (unsigned int)(s * 16), store_address);
                                }
                                asm volatile("cp.async.bulk.commit_group;");
                            }
                        }
                    ...
                    unsigned int pool_m = task_4[4] * 32;
                    unsigned int n_offset_1 = (task_4[3] * 2 + (unsigned int)cta_rank) * 128;
                    unsigned int output_base = smem_output_addr + epi_wg_1 * 16 * 128 * 2;
                    unsigned int cached_token[2];
                    unsigned int cached_topk[2];
                    unsigned int packed[4];
                    if (task_4[0] == 2) {
                        if (epi_warp_1 == 0) {
                            if (elect_sync()) {
                                atomicAdd(&L2Empty[task_4[4] % 480], 1);
                            }
                        }
                        __syncwarp();
                    }
                    ...
                    for (int s_1 = 0; s_1 < 1; s_1++) {
                        if (valid_m_1 <= epi_wg_1 * 16 + (unsigned int)(s_1 * 16)) {
                            asm volatile("tcgen05.fence::before_thread_sync;");
                            asm volatile(
                                "mbarrier.arrive.release.cta.shared::cluster.b64 _, [%0];"
                                :: "r"((tmem_empty_addr + (accum_stage_1) * 8) & 0xFEFFFFFF) : "memory");
                        ...
                        for (int j_1 = 0; j_1 < 2; j_1++) {
                            unsigned int row = epi_wg_1 * 16 + (unsigned int)(s_1 * 16) + (unsigned int)(j_1 * 8) + warp_in_wg_1 * 2 + lane / 16;
                            if (row < valid_m_1) {
                                if (task_4[0] == 4) {
                        ...
                        for (int i_2 = 0; i_2 < 2; i_2++) {
                            unsigned int address_1 = accum_stage_1 * 32 + epi_wg_1 * 16 + (unsigned int)(s_1 * 16) + (unsigned int)(i_2 * 8);
                            ...
                            unsigned int write_address = output_base + warp_in_wg_1 / 2 * 16 * 128 + (unsigned int)(i_2 * 8 * 128) + row_1 * 128 + (col ^ row_1) * 16;
                        ...
                        for (int j_2 = 0; j_2 < 2; j_2++) {
                            ...
                            unsigned int row_2 = epi_wg_1 * 16 + (unsigned int)(s_1 * 16) + row_in_store;
                            if (row_2 >= valid_m_1) {
                                break;
                            }
                            unsigned int row_in_atom = (warp_in_wg_1 * 2 + lane / 16) % 8;
                            unsigned int read_address = output_base + lane % 16 / 8 * 16 * 128 + row_in_store * 128 + (lane % 8 ^ row_in_atom) * 16;
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                ...
                                : "r"(read_address));
                            unsigned long long dst_index = ((unsigned long long)cached_topk[j_2] * 1920 + (unsigned long long)cached_token[j_2]) * 2560 + (unsigned long long)(n_offset_1 / 2) + (unsigned long long)(lane % 16 * 4);
            ...
            if (epi_thread == 0) {
                ...
                unsigned int _wait_acquire_mask_27;
                do {
                asm volatile("ld.acquire.gpu.global.u32 %0, [%1];" : "=r"(_wait_acquire_mask_27) : "l"((reinterpret_cast<unsigned int*>(GridCounters) + (1))) : "memory");
                } while (((_wait_acquire_mask_27 ^ static_cast<unsigned int>((old_3 ^ 2147483648) & 2147483648)) & static_cast<unsigned int>(2147483648)) != 0);
            }
            asm volatile("bar.sync 2, 256;" ::: "memory");
            if (bid == 0 && epi_thread == 0) {
