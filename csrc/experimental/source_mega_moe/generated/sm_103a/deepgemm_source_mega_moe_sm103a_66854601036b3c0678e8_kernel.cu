            ...
            for (unsigned int token_chunk = epi_warp_2 * 152 + (unsigned int)bid; token_chunk < num_tokens * 4; token_chunk += 1216) {
                ...
                for (unsigned int i_3 = 0; i_3 < 7; i_3++) {
                    if (mask != 0) {
                        ...
                        if (elect_sync()) {
                            cp_async_bulk_gmem2smem(reusable_addr + (epi_warp_2 * 7 + i_3) * 2560, reinterpret_cast<const void*>(reinterpret_cast<const uint8_t*>(CombineBytes) + ((unsigned long long)(((unsigned long long)slot_1 * 5760 + (unsigned long long)token_1) * 5120 * 2 + (unsigned long long)(chunk_1 * 2560)) * (unsigned long long)1)), 2560, combine_barriers_addr + (epi_warp_2 * 7 + i_3) * 8);
