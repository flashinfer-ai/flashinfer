                ...
                if (elect_sync()) {
                    ...
                    for (int chunk = 0; chunk < 1; chunk++) {
                        asm volatile(
                            ...
                            " [%0], [%1], %2, [%3], %4;"
                            :: "r"(send_buffer_addr + warp * 512), "l"(reinterpret_cast<const void*>(reinterpret_cast<const uint8_t*>(X) + ((unsigned long long)(source_byte + (unsigned long long)(chunk * 512)) * (unsigned long long)1))), "r"((unsigned)(512)), "r"(pull_barriers_addr + (warp) * 8),
                    ...
                    if (mask != 0) {
                        ...
                        if (elect_sync()) {
                            asm volatile(
                                "cp.async.bulk.shared::cluster.global.mbarrier::complete_tx::bytes.L2::cache_hint"
                                " [%0], [%1], %2, [%3], %4;"
                                :: "r"(reusable_addr + (epi_warp_2 * 9 + i_3) * 1024), "l"(reinterpret_cast<const void*>(reinterpret_cast<const uint8_t*>(CombineBytes) + ((unsigned long long)(((unsigned long long)slot_1 * 1920 + (unsigned long long)token_1) * 512 * 2 + (unsigned long long)(chunk_1 * 1024)) * (unsigned long long)1))), "r"((unsigned)(1024)), "r"(combine_barriers_addr + (epi_warp_2 * 9 + i_3) * 8),
                                   "l"(0x12F0000000000000ULL) : "memory");
