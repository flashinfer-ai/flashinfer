...
kernel_deepgemm_source_mega_moe_sm100a_29fdf4b933419f44d74c(DeepgemmTensorMap const* A1, DeepgemmTensorMap const* A2, DeepgemmTensorMap const* SA1, DeepgemmTensorMap const* SA2, DeepgemmTensorMap const* B1, DeepgemmTensorMap const* B2, DeepgemmTensorMap const* SB1, DeepgemmTensorMap const* SB2, DeepgemmTensorMap const* SFA1, DeepgemmTensorMap const* SFA2, DeepgemmTensorMap const* SSFA1, DeepgemmTensorMap const* SSFA2, DeepgemmTensorMap const* SFB1, DeepgemmTensorMap const* SFB2, DeepgemmTensorMap const* SSFB1, DeepgemmTensorMap const* SSFB2, DeepgemmTensorMap const* L1Output, DeepgemmTensorMap const* SharedL1Output, uint8_t* __restrict__ X, unsigned int* __restrict__ XSF, long long* __restrict__ TopK, float* __restrict__ Weights, uint8_t* __restrict__ L1Acts, unsigned int* __restrict__ L1SF, float* __restrict__ L1Weights, uint8_t* __restrict__ L2SF, uint8_t* __restrict__ SharedL2SF, unsigned int* __restrict__ SourceIndices, unsigned int* __restrict__ TokenMetadata, unsigned int* __restrict__ GridCounters, unsigned int* __restrict__ NvlCounter, unsigned int* __restrict__ NvlSignals, unsigned long long* __restrict__ PeerGrid, unsigned long long* __restrict__ ReadyGrid, unsigned long long* __restrict__ SendCounts, unsigned long long* __restrict__ RecvCounts, unsigned long long* __restrict__ RecvSum, unsigned int* __restrict__ L1Full, unsigned int* __restrict__ L1Empty, unsigned long long* __restrict__ L2Mask, unsigned int* __restrict__ L2Empty, unsigned int* __restrict__ SharedFull, unsigned int* __restrict__ L1Counter, unsigned int* __restrict__ L2Counter, unsigned int* __restrict__ SharedL1Counter, unsigned int* __restrict__ SharedL2Counter, unsigned int* __restrict__ Combine, uint8_t* __restrict__ CombineBytes, uint8_t* __restrict__ Y, unsigned int num_tokens)
{
    ...
    #define task_full_addr (mbar_base + 624)
    #define task_empty_addr (mbar_base + 640)
    ...
    // Mbarriers at smem_raw[219200..219856)
    ...
    if (warp == 2) {
        ...
        if (leader) {
            ...
            mbarrier_init(smem + 219520, 1);
            mbarrier_init(smem + 219528, 1);
            mbarrier_init(smem + 219536, 1);
            mbarrier_init(smem + 219544, 1);
            mbarrier_init(smem + 219552, 1);
            mbarrier_init(smem + 219560, 1);
            mbarrier_init(smem + 219568, 1);
            mbarrier_init(smem + 219576, 1);
            mbarrier_init(smem + 219584, 1);
            mbarrier_init(smem + 219592, 1);
            mbarrier_init(smem + 219600, 1);
            mbarrier_init(smem + 219608, 1);
            mbarrier_init(smem + 219616, 1);
            mbarrier_init(smem + 219624, 1);
            mbarrier_init(smem + 219632, 1);
            mbarrier_init(smem + 219640, 1);
            mbarrier_init(smem + 219648, 1);
            mbarrier_init(smem + 219656, 1);
            mbarrier_init(smem + 219664, 1);
            mbarrier_init(smem + 219672, 1);
            mbarrier_init(smem + 219680, 1);
            mbarrier_init(smem + 219688, 1);
            mbarrier_init(smem + 219696, 1);
            mbarrier_init(smem + 219704, 1);
            mbarrier_init(smem + 219712, 1);
            mbarrier_init(smem + 219720, 1);
            mbarrier_init(smem + 219728, 1);
            mbarrier_init(smem + 219736, 1);
            mbarrier_init(smem + 219744, 1);
            mbarrier_init(smem + 219752, 1);
            mbarrier_init(smem + 219760, 1);
            mbarrier_init(smem + 219768, 1);
            mbarrier_init(smem + 219776, 1);
            mbarrier_init(smem + 219784, 1);
            mbarrier_init(smem + 219792, 1);
            mbarrier_init(smem + 219800, 1);
            mbarrier_init(smem + 219808, 1);
            mbarrier_init(smem + 219816, 1);
            // task_full: 2 barriers, init_count=1
            mbarrier_init(smem + 219824, 1);
            mbarrier_init(smem + 219832, 1);
            // task_empty: 2 barriers, init_count=512
            mbarrier_init(smem + 219840, 512);
            mbarrier_init(smem + 219848, 512);
            asm volatile("fence.mbarrier_init.release.cluster;" ::: "memory");
        }
    }
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 219856);
    if (warp == 3) {
        int _tmem_hold = smem + 219856;
    ...
    if (warp == 7) {
        { // schedule_main
            ...
            unsigned int early_shared_l2 = (unsigned int)((num_tokens + 128 - 1) / 128 * 20 <= 74);
            if (cta_rank == 0) {
                ...
                if (task_4[0] == 1 || task_4[0] == 3) {
                    ...
                    for (int s = 0; s < 2; s++) {
                        ...
                        for (int i = 0; i < 4; i++) {
                            ...
                            if (task_4[0] == 1 && j * 8 % 32 == 0) {
                                {
                                    cached_weight = L1Weights[ring_block_2 * 128 + epi_wg * 64 + j * 8 + lane];
                                }
                            }
                            float _shfl_55;
                    ...
                    for (int s_1 = 0; s_1 < 2; s_1++) {
                        ...
                        for (int j_1 = 0; j_1 < 4; j_1++) {
                            unsigned int row = epi_wg_1 * 64 + (unsigned int)(s_1 * 32) + (unsigned int)(j_1 * 8) + warp_in_wg_1 * 2 + lane / 16;
                            if (row < valid_m_1) {
                                if (task_4[0] == 4) {
                        ...
                        for (int j_2 = 0; j_2 < 4; j_2++) {
                            ...
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                ...
                                : "r"(read_address));
                            unsigned long long dst_index = ((unsigned long long)cached_topk[j_2] * 5760 + (unsigned long long)cached_token[j_2]) * 2560 + (unsigned long long)(n_offset_1 / 2) + (unsigned long long)(lane % 16 * 4);
                    ...
                    if (mask != 0) {
                        ...
                        if (elect_sync()) {
                            asm volatile(
                                "cp.async.bulk.shared::cluster.global.mbarrier::complete_tx::bytes.L2::cache_hint"
                                " [%0], [%1], %2, [%3], %4;"
                                :: "r"(reusable_addr + (epi_warp_2 * 7 + i_3) * 2560), "l"(reinterpret_cast<const void*>(reinterpret_cast<const uint8_t*>(CombineBytes) + ((unsigned long long)(((unsigned long long)slot_1 * 5760 + (unsigned long long)token_1) * 5120 * 2 + (unsigned long long)(chunk_1 * 2560)) * (unsigned long long)1))), "r"((unsigned)(2560)), "r"(combine_barriers_addr + (epi_warp_2 * 7 + i_3) * 8),
                                   "l"(0x12F0000000000000ULL) : "memory");
