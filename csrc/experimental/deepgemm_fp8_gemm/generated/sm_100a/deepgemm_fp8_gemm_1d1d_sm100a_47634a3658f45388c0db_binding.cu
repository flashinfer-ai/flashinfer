...
// tvm-ffi host launcher for Deepgemm kernel 'kernel_deepgemm_fp8_gemm_1d1d_sm100a_47634a3658f45388c0db'.
#include <cuda.h>
...
TVM_FFI_EMBED_CUBIN(deepgemm_fp8_gemm_1d1d_sm100a_47634a3658f45388c0db_sm_100a);

namespace deepgemm_host_shim_df3ad1a0df2fba16 {
...
#define DEEPGEMM_HAS_OVERSIZED_SMEM 1
#else
#define DEEPGEMM_HAS_OVERSIZED_SMEM 0
#endif
...
inline bool DeepgemmConfigureDynamicSmem(tvm::ffi::CubinKernel& kernel, int device_id,
...
#if DEEPGEMM_HAS_OVERSIZED_SMEM
  int oversized_max = 0;
  ...
  {
    using DeepgemmCuKernelSetAttributeFn = int (*)(int attrib, int val, void* kernel, int dev);
    static const auto deepgemm_cu_kernel_set_attribute =
        reinterpret_cast<DeepgemmCuKernelSetAttributeFn>(dlsym(RTLD_DEFAULT, "cuKernelSetAttribute"));
    TVM_FFI_CHECK(deepgemm_cu_kernel_set_attribute != nullptr, RuntimeError)
        << "cuKernelSetAttribute not resolvable while enabling the oversized shared-memory mode";
    int drv_err = deepgemm_cu_kernel_set_attribute(
...
void Run(TensorView arg_A, TensorView arg_B, TensorView arg_SFA, TensorView arg_SFB, TensorView arg_C_tma, int64_t arg_M, int64_t arg_N, int64_t arg_K, int64_t arg_grid_m, int64_t arg_grid_n, int64_t arg_K_tiles, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  static auto kernel = EmbedCubinModule_deepgemm_fp8_gemm_1d1d_sm100a_47634a3658f45388c0db_sm_100a::Global()->mod.GetKernel("kernel_deepgemm_fp8_gemm_1d1d_sm100a_47634a3658f45388c0db");
  static signed char deepgemm_smem_mode_cache[64] = {0};
  const bool use_oversized_smem = DeepgemmConfigureDynamicSmem(
      kernel, (int)arg_A.device().device_id, 226560,
      deepgemm_smem_mode_cache, 64);
  ...
  config.dynamicSmemBytes = 226560u;
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, deepgemm_host_shim_df3ad1a0df2fba16::Run);
