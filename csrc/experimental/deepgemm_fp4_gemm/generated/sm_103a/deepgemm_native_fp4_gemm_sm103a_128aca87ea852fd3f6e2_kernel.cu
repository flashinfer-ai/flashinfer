...
kernel_deepgemm_native_fp4_gemm_sm103a_128aca87ea852fd3f6e2(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap C_tma, int M, int N, int K, int grid_m, int grid_n, int K_tiles, float alpha)
{
    ...
    const int mbar_base = smem + 222208;
    #define full_addr (mbar_base + 0)
    #define sf_full_addr (mbar_base + 88)
    #define empty_addr (mbar_base + 176)
    #define tmem_full_addr (mbar_base + 264)
    #define tmem_empty_addr (mbar_base + 280)
    #define tmem_overlap_addr (mbar_base + 296)
    ...
    uint8_t* smem_b = reinterpret_cast<uint8_t*>(smem_raw + 19456);
    const int smem_b_addr = smem + 19456;
    unsigned int* smem_sfa = reinterpret_cast<unsigned int*>(smem_raw + 199680);
    const int smem_sfa_addr = smem + 199680;
    unsigned int* smem_sfb = reinterpret_cast<unsigned int*>(smem_raw + 210944);
    const int smem_sfb_addr = smem + 210944;
    ...
    // Mbarrier init (6 pipeline groups, 0 ordered-sequence groups, 39 barriers)
    // Mbarriers at smem_raw[222208..222520)

    if (warp == 1) {
        uint32_t leader = elect_sync();
        if (leader) {
            // Stage-major initialization across barrier declarations.
            mbarrier_init(smem + 222208, 130);
            mbarrier_init(smem + 222296, 1);
            mbarrier_init(smem + 222384, 1);
            mbarrier_init(smem + 222472, 1);
            mbarrier_init(smem + 222488, 256);
            mbarrier_init(smem + 222504, 256);
            mbarrier_init(smem + 222216, 130);
            mbarrier_init(smem + 222304, 1);
            mbarrier_init(smem + 222392, 1);
            mbarrier_init(smem + 222480, 1);
            mbarrier_init(smem + 222496, 256);
            mbarrier_init(smem + 222512, 256);
            mbarrier_init(smem + 222224, 130);
            mbarrier_init(smem + 222312, 1);
            mbarrier_init(smem + 222400, 1);
            mbarrier_init(smem + 222232, 130);
            mbarrier_init(smem + 222320, 1);
            mbarrier_init(smem + 222408, 1);
            mbarrier_init(smem + 222240, 130);
            mbarrier_init(smem + 222328, 1);
            mbarrier_init(smem + 222416, 1);
            mbarrier_init(smem + 222248, 130);
            mbarrier_init(smem + 222336, 1);
            mbarrier_init(smem + 222424, 1);
            mbarrier_init(smem + 222256, 130);
            mbarrier_init(smem + 222344, 1);
            mbarrier_init(smem + 222432, 1);
            mbarrier_init(smem + 222264, 130);
            mbarrier_init(smem + 222352, 1);
            mbarrier_init(smem + 222440, 1);
            mbarrier_init(smem + 222272, 130);
            mbarrier_init(smem + 222360, 1);
            mbarrier_init(smem + 222448, 1);
            mbarrier_init(smem + 222280, 130);
            mbarrier_init(smem + 222368, 1);
            mbarrier_init(smem + 222456, 1);
            mbarrier_init(smem + 222288, 130);
            mbarrier_init(smem + 222376, 1);
            mbarrier_init(smem + 222464, 1);
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 222520);
    if (warp == 2) {
        int _tmem_hold = smem + 222520;
    ...
    if (warp == 0) {
        { // load_main
            ...
            #pragma unroll 1
            for (unsigned int bid_1 = bid; bid_1 < grid_m * 36; bid_1 += 152) {
                ...
                int _min_0 = ((8) < (36 - first_n) ? (8) : (36 - first_n));
                unsigned int n_in_group = _min_0;
                unsigned int off_m = in_group / n_in_group * 16;
                ...
                for (int k = 0; k < 20; k++) {
                    ...
                    if (elect_sync()) {
                        ...
                        tma_2d_gmem2smem_cta2(smem_a_addr + stage * 1024, (&A), k * 256, off_m + (unsigned int)(cta_rank * 8), ((full_addr + (stage) * 8) & 0xFEFFFFFF));
                        tma_2d_gmem2smem_cta2(smem_b_addr + stage * 16384, (&B), k * 256, off_n, ((full_addr + (stage) * 8) & 0xFEFFFFFF));
                        if (cta_rank == 0) {
                    ...
                    stage += 1;
                    if (stage == 11) { stage = 0; _phase_empty ^= 1; }
                }
            }
        }
    }
    ...
    if (warp == 1) {
        { // mma_main
            ...
            if (cta_rank == 0) {
                #pragma unroll 1
                for (unsigned int bid_2 = bid; bid_2 < grid_m * 36; bid_2 += 152) {
                    mbarrier_wait(tmem_empty_addr + (accum_stage) * 8, _phase_tmem_empty);
                    asm volatile("tcgen05.fence::after_thread_sync;");
                    #pragma unroll 4
                    for (int k_1 = 0; k_1 < 20; k_1++) {
                        ...
                        __syncwarp();
                        stage_1 += 1;
                        if (stage_1 == 11) { stage_1 = 0; _phase_full ^= 1; }
                    }
                    accum_stage += 1;
    ...
    if (warp >= 2 && warp <= 3) {
        { // transpose_main
            ...
            #pragma unroll 1
            for (unsigned int bid_3 = bid; bid_3 < grid_m * 36; bid_3 += 152) {
                for (int k_2 = 0; k_2 < 20; k_2++) {
                    ...
                    stage_2 += 1;
                    if (stage_2 == 11) { stage_2 = 0; _phase_sf_full ^= 1; }
                }
            }
        }
    }
    ...
    if (warp >= 4 && warp <= 7) {
        { // epilogue_main
            ...
            #pragma unroll 1
            for (unsigned int bid_4 = bid; bid_4 < grid_m * 36; bid_4 += 152) {
                ...
                int _min_1 = ((8) < (36 - first_n_1) ? (8) : (36 - first_n_1));
                unsigned int n_in_group_1 = _min_1;
                unsigned int off_m_1 = in_group_1 / n_in_group_1 * 16;
                ...
                for (int s = 0; s < 1; s++) {
                    ...
                    for (int i_2 = 0; i_2 < 2; i_2++) {
                        unsigned int address = taddr + accum_stage_1 * 16 + (unsigned int)(s * 16) + (unsigned int)(i_2 * 8);
                        ...
                        for (int element = 0; element < 4; element++) {
                            ...
                            _tmem_load_1[element] = _tmem_load_1[element] * alpha;
                        }
