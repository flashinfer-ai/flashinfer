...
kernel_deepgemm_native_fp4_gemm_sm103a_8b24a9828d541f0c6309(const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap C_tma, int M, int N, int K, int grid_m, int grid_n, int K_tiles, float alpha)
{
    ...
    #define overlap_done_addr (mbar_base + 200)
        ...
        { // load_main
            ...
            for (unsigned int this_bid = bid; this_bid < num_tiles; this_bid += num_bids) {
                ...
                for (int iter_k = 0; iter_k < K_tiles; iter_k++) {
                    ...
                    if (elect_sync()) {
                        {
                            tma_2d_gmem2smem(smem_sfa_addr + load_stage * 26624, (&SFA), off_m, iter_k * 2, tma_full_addr + (load_stage) * 8);
                        }
                        tma_2d_gmem2smem(smem_sfb_addr + load_stage * 26624, (&SFB), off_n, iter_k * 2, tma_full_addr + (load_stage) * 8);
                        {
                            mbarrier_arrive_expect_tx(tma_full_addr + (load_stage) * 8, 1024 + ((1) ? 1024 : 0));
                            tma_3d_gmem2smem_cta2(smem_a_addr + load_stage * 26624, (&A), 0, off_m, iter_k, ((sf_ready_addr + (load_stage) * 8) & 0xFEFFFFFF));
                            tma_3d_gmem2smem_cta2(smem_b_addr + load_stage * 26624, (&B), 0, off_n + cta_rank * 64, iter_k, ((sf_ready_addr + (load_stage) * 8) & 0xFEFFFFFF));
                            asm volatile(
            ...
            if (cta_rank == 0) {
                ...
                for (unsigned int this_bid_1 = bid; this_bid_1 < num_tiles_1; this_bid_1 += num_bids) {
                    ...
                    for (int iter_k_1 = 0; iter_k_1 < K_tiles; iter_k_1++) {
                        ...
                        if (elect_sync()) {
                            {
                                int _mma_a_lo_2 = (((smem_a_addr) >> 4) & 0x3FFF) + (mma_tma_stage) * 1664;
                                int _mma_b_lo_2 = (((smem_b_addr) >> 4) & 0x3FFF) + (mma_tma_stage) * 1664;
                                {
        ...
        { // transpose_main
            ...
            for (unsigned int this_bid_2 = bid; this_bid_2 < num_tiles_2; this_bid_2 += num_bids) {
                ...
                for (int iter_k_2 = 0; iter_k_2 < K_tiles; iter_k_2++) {
                    ...
                    int sfb_smem = smem_sfb_addr + transpose_stage * 26624 + (unsigned int)(sf_subblock * 128 * 4);
                    {
        ...
        { // epilogue_main
            ...
            for (unsigned int this_bid_3 = bid; this_bid_3 < num_tiles_3; this_bid_3 += num_bids) {
                ...
                for (int epi_pass = 0; epi_pass < 4; epi_pass++) {
                    int store_idx = ((0) ? 3 - epi_pass : epi_pass);
                    ...
                    for (int n = 0; n < 4; n++) {
                        ...
                        int col = epi_stage * 128 + (unsigned int)col_start + (unsigned int)(load_idx * 8);
