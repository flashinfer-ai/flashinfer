...
kernel_deepgemm_native_fp4_gemm_sm100a_d9c49464a0067902d9c3(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap C_tma, int M, int N, int K, int grid_m, int grid_n, int K_tiles, float alpha)
{
    ...
    uint8_t* smem_b = reinterpret_cast<uint8_t*>(smem_raw + 28672);
    const int smem_b_addr = smem + 28672;
    unsigned int* smem_sfa = reinterpret_cast<unsigned int*>(smem_raw + 192512);
    const int smem_sfa_addr = smem + 192512;
    unsigned int* smem_sfb = reinterpret_cast<unsigned int*>(smem_raw + 202752);
    const int smem_sfb_addr = smem + 202752;
    ...
    const int tmem_scale_b = taddr + 72;
    ...
    if (warp == 0) {
        { // load_main
            ...
            #pragma unroll 1
            for (unsigned int bid_1 = bid; bid_1 < grid_m * 36; bid_1 += 148) {
                ...
                unsigned int off_m = in_group / n_in_group * 32;
                ...
                for (int k = 0; k < 20; k++) {
                    ...
                    if (elect_sync()) {
                        ...
                        tma_2d_gmem2smem_cta2(smem_a_addr + stage * 2048, (&A), k * 256, off_m + (unsigned int)(cta_rank * 16), ((full_addr + (stage) * 8) & 0xFEFFFFFF));
                        tma_2d_gmem2smem_cta2(smem_b_addr + stage * 16384, (&B), k * 256, off_n, ((full_addr + (stage) * 8) & 0xFEFFFFFF));
                        if (cta_rank == 0) {
                            mbarrier_arrive_expect_tx(full_addr + (stage) * 8, 36864);
                        } else {
                            asm volatile(
    ...
    if (warp == 1) {
        { // mma_main
            ...
            if (cta_rank == 0) {
                #pragma unroll 1
                for (unsigned int bid_2 = bid; bid_2 < grid_m * 36; bid_2 += 148) {
                    ...
                    for (int k_1 = 0; k_1 < 20; k_1++) {
                        ...
                        if (elect_sync()) {
                            ...
                            int _mma_b_lo_0 = (((smem_a_addr) >> 4) & 0x3FFF) + (stage_1) * 128;
                            {
                                uint64_t a_desc = ((uint64_t)(uint32_t)_mma_a_lo_0) | ((uint64_t)0x40004040 << 32);
                                uint64_t b_desc = ((uint64_t)(uint32_t)_mma_b_lo_0) | ((uint64_t)0x40004040 << 32);

                                tcgen05_mma_mxf4_bs_cta2((tmem_accum + (accum_stage * 32)), a_desc + 0, b_desc + 0,
                                    0x10880480U, tmem_scale_b + 0, tmem_scale_a + 0, ((init_flag) ? 0 : 1));
                                tcgen05_mma_mxf4_bs_cta2((tmem_accum + (accum_stage * 32)), a_desc + 2, b_desc + 2,
                                    0x508804a0U, tmem_scale_b + 0, tmem_scale_a + 0, 1);
                                tcgen05_mma_mxf4_bs_cta2((tmem_accum + (accum_stage * 32)), a_desc + 4, b_desc + 4,
                                    0x10880480U, tmem_scale_b + 4, tmem_scale_a + 4, 1);
                                tcgen05_mma_mxf4_bs_cta2((tmem_accum + (accum_stage * 32)), a_desc + 6, b_desc + 6,
                                    0x508804a0U, tmem_scale_b + 4, tmem_scale_a + 4, 1);
                            }
                        }
                        __syncwarp();
    ...
    if (warp >= 2 && warp <= 3) {
        { // transpose_main
            ...
            #pragma unroll 1
            for (unsigned int bid_3 = bid; bid_3 < grid_m * 36; bid_3 += 148) {
    ...
    if (warp >= 4 && warp <= 7) {
        { // epilogue_main
            ...
            #pragma unroll 1
            for (unsigned int bid_4 = bid; bid_4 < grid_m * 36; bid_4 += 148) {
                ...
                unsigned int off_m_1 = in_group_1 / n_in_group_1 * 32;
                ...
                for (int s = 0; s < 2; s++) {
                    ...
                    for (int i_2 = 0; i_2 < 2; i_2++) {
                        unsigned int address = taddr + accum_stage_1 * 32 + (unsigned int)(s * 16) + (unsigned int)(i_2 * 8);
