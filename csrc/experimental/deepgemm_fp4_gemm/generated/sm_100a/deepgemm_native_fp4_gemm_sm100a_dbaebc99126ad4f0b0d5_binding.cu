...
// tvm-ffi direct-source launcher for Deepgemm kernel 'kernel_deepgemm_native_fp4_gemm_sm100a_dbaebc99126ad4f0b0d5'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
void Run(TensorView arg_SFA, TensorView arg_SFB, TensorView arg_A, TensorView arg_B, TensorView arg_C_tma, int64_t arg_M, int64_t arg_N, int64_t arg_K, int64_t arg_grid_m, int64_t arg_grid_n, int64_t arg_K_tiles, double arg_alpha, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_deepgemm_native_fp4_gemm_sm100a_dbaebc99126ad4f0b0d5),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_deepgemm_native_fp4_gemm_sm100a_dbaebc99126ad4f0b0d5 failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_deepgemm_native_fp4_gemm_sm100a_dbaebc99126ad4f0b0d5), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_deepgemm_native_fp4_gemm_sm100a_dbaebc99126ad4f0b0d5 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace deepgemm_host_shim_46ac135d2f09fa6d

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, deepgemm_host_shim_46ac135d2f09fa6d::Run);
