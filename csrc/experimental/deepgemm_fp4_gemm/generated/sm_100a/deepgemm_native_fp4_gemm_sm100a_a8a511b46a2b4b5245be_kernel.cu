...
#define TMEM_SCALE_B_OFFSET 264
...
#define SMEM_SMEM_SFB_OFF 211968
...
kernel_deepgemm_native_fp4_gemm_sm100a_a8a511b46a2b4b5245be(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap C_tma, int M, int N, int K, int grid_m, int grid_n, int K_tiles, float alpha)
{
    ...
    unsigned int* smem_sfb = reinterpret_cast<unsigned int*>(smem_raw + 211968);
    const int smem_sfb_addr = smem + 211968;
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 219352);
    if (warp == 2) {
        int _tmem_hold = smem + 219352;
    ...
    const int tmem_scale_b = taddr + 264;
            ...
            for (unsigned int bid_1 = bid; bid_1 < grid_m * 36; bid_1 += 148) {
                ...
                for (int k = 0; k < 20; k++) {
                    ...
                    if (elect_sync()) {
                        ...
                        tma_2d_gmem2smem_cta2(smem_b_addr + stage * 8192, (&B), k * 256, off_n + (unsigned int)(cta_rank * 64), ((full_addr + (stage) * 8) & 0xFEFFFFFF));
                        if (cta_rank == 0) {
                        ...
                        if (elect_sync()) {
                            {
                                ...
                                int _mma_b_lo_2 = (((smem_b_addr) >> 4) & 0x3FFF) + (stage_1) * 512;
                                {
                                    ...
                                    tcgen05_mma_mxf4_bs_cta2((tmem_accum + (accum_stage * 128)), a_desc + 0, b_desc + 0,
                                        0x10a00480U, tmem_scale_a + 0, tmem_scale_b + 0, ((init_flag) ? 0 : 1));
                                    tcgen05_mma_mxf4_bs_cta2((tmem_accum + (accum_stage * 128)), a_desc + 2, b_desc + 2,
                                        0x50a004a0U, tmem_scale_a + 0, tmem_scale_b + 0, 1);
                                    tcgen05_mma_mxf4_bs_cta2((tmem_accum + (accum_stage * 128)), a_desc + 4, b_desc + 4,
                                        0x10a00480U, tmem_scale_a + 4, tmem_scale_b + 4, 1);
                                    tcgen05_mma_mxf4_bs_cta2((tmem_accum + (accum_stage * 128)), a_desc + 6, b_desc + 6,
                                        0x50a004a0U, tmem_scale_a + 4, tmem_scale_b + 4, 1);
                                }
                            }
                        }
                        __syncwarp();
            ...
            for (unsigned int bid_4 = bid; bid_4 < grid_m * 36; bid_4 += 148) {
                ...
                for (int s = 0; s < 2; s++) {
                    ...
                    for (int i_2 = 0; i_2 < 8; i_2++) {
                        ...
                        unsigned int address = taddr + (row << 16) + accum_stage_1 * 128 + (unsigned int)(s * 64) + (unsigned int)(i_2 * 8);
                        ...
                        unsigned int shifted = (unsigned int)i_2 + lane * 8;
                    ...
                    if (warp == 4) {
                        if (elect_sync()) {
                            tma_store_2d((&C_tma), off_n_1 + (unsigned int)(s * 64), off_m_1, stage_base);
