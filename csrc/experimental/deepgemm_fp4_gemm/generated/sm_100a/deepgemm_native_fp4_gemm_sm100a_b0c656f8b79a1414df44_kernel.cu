...
kernel_deepgemm_native_fp4_gemm_sm100a_b0c656f8b79a1414df44(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap C_tma, int M, int N, int K, int grid_m, int grid_n, int K_tiles, float alpha)
{
    ...
    const int mbar_base = smem + 144384;
    ...
    uint8_t* smem_b = reinterpret_cast<uint8_t*>(smem_raw + 122880);
    const int smem_b_addr = smem + 122880;
    unsigned int* smem_sfa = reinterpret_cast<unsigned int*>(smem_raw + 130048);
    const int smem_sfa_addr = smem + 130048;
    unsigned int* smem_sfb = reinterpret_cast<unsigned int*>(smem_raw + 137216);
    const int smem_sfb_addr = smem + 137216;
    ...
    // Mbarriers at smem_raw[144384..144600)

    if (warp == 1) {
        uint32_t leader = elect_sync();
        if (leader) {
            // Stage-major initialization across barrier declarations.
            mbarrier_init(smem + 144384, 130);
            mbarrier_init(smem + 144440, 1);
            mbarrier_init(smem + 144496, 1);
            mbarrier_init(smem + 144552, 1);
            mbarrier_init(smem + 144568, 256);
            mbarrier_init(smem + 144584, 256);
            mbarrier_init(smem + 144392, 130);
            mbarrier_init(smem + 144448, 1);
            mbarrier_init(smem + 144504, 1);
            mbarrier_init(smem + 144560, 1);
            mbarrier_init(smem + 144576, 256);
            mbarrier_init(smem + 144592, 256);
            mbarrier_init(smem + 144400, 130);
            mbarrier_init(smem + 144456, 1);
            mbarrier_init(smem + 144512, 1);
            mbarrier_init(smem + 144408, 130);
            mbarrier_init(smem + 144464, 1);
            mbarrier_init(smem + 144520, 1);
            mbarrier_init(smem + 144416, 130);
            mbarrier_init(smem + 144472, 1);
            mbarrier_init(smem + 144528, 1);
            mbarrier_init(smem + 144424, 130);
            mbarrier_init(smem + 144480, 1);
            mbarrier_init(smem + 144536, 1);
            mbarrier_init(smem + 144432, 130);
            mbarrier_init(smem + 144488, 1);
            mbarrier_init(smem + 144544, 1);
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 144600);
    if (warp == 2) {
        int _tmem_hold = smem + 144600;
    ...
    if (warp == 0) {
        { // load_main
            ...
            #pragma unroll 1
            for (unsigned int bid_1 = bid; bid_1 < grid_m * 8; bid_1 += 148) {
                ...
                int _min_0 = ((8) < ((unsigned int)grid_m - first_m) ? (8) : ((unsigned int)grid_m - first_m));
                ...
                for (int k = 0; k < 8; k++) {
                    ...
                    if (elect_sync()) {
                        ...
                        tma_2d_gmem2smem_cta2(smem_b_addr + stage * 1024, (&B), k * 256, off_n + (unsigned int)(cta_rank * 8), ((full_addr + (stage) * 8) & 0xFEFFFFFF));
                        if (cta_rank == 0) {
    ...
    if (warp == 1) {
        { // mma_main
            ...
            if (cta_rank == 0) {
                #pragma unroll 1
                for (unsigned int bid_2 = bid; bid_2 < grid_m * 8; bid_2 += 148) {
    ...
    if (warp >= 2 && warp <= 3) {
        { // transpose_main
            ...
            #pragma unroll 1
            for (unsigned int bid_3 = bid; bid_3 < grid_m * 8; bid_3 += 148) {
    ...
    if (warp >= 4 && warp <= 7) {
        { // epilogue_main
            ...
            #pragma unroll 1
            for (unsigned int bid_4 = bid; bid_4 < grid_m * 8; bid_4 += 148) {
                ...
                int _min_1 = ((8) < ((unsigned int)grid_m - first_m_1) ? (8) : ((unsigned int)grid_m - first_m_1));
                ...
                for (int s = 0; s < 1; s++) {
                    ...
                    for (int i_2 = 0; i_2 < 2; i_2++) {
                        ...
                        unsigned int address = taddr + (row << 16) + accum_stage_1 * 16 + (unsigned int)(s * 16) + (unsigned int)(i_2 * 8);
                        ...
                        unsigned int shifted = (unsigned int)i_2 + lane * 2;
                        unsigned int swz_row = shifted / 8;
                        unsigned int swz_col = shifted % 8;
                        swz_col = swz_col ^ swz_row % 2;
                        unsigned int write_addr = stage_base + warp_0 * 32 * 16 * 2 + swz_row * 128 + swz_col * 16;
                        asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" ::
                    ...
                    if (warp == 4) {
                        if (elect_sync()) {
                            tma_store_2d((&C_tma), off_n_1 + (unsigned int)(s * 16), off_m_1, stage_base);
