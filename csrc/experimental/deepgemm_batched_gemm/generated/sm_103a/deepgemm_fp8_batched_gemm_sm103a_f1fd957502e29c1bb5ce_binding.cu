...
// tvm-ffi direct-source launcher for Deepgemm kernel 'kernel_deepgemm_fp8_batched_gemm_sm103a_f1fd957502e29c1bb5ce'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_deepgemm_fp8_batched_gemm_sm103a_f1fd957502e29c1bb5ce(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap D, unsigned int* __restrict__ SFD, unsigned int M, unsigned int grid_m, unsigned long long sfd_stride, float alpha);


namespace deepgemm_host_shim_ca5894d08c608b82 {
...
void Run(TensorView arg_A, TensorView arg_B, TensorView arg_SFA, TensorView arg_SFB, TensorView arg_D, TensorView arg_SFD, int64_t arg_M, int64_t arg_grid_m, int64_t arg_sfd_stride, double arg_alpha, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_deepgemm_fp8_batched_gemm_sm103a_f1fd957502e29c1bb5ce),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_deepgemm_fp8_batched_gemm_sm103a_f1fd957502e29c1bb5ce failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_deepgemm_fp8_batched_gemm_sm103a_f1fd957502e29c1bb5ce), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_deepgemm_fp8_batched_gemm_sm103a_f1fd957502e29c1bb5ce failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace deepgemm_host_shim_ca5894d08c608b82

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, deepgemm_host_shim_ca5894d08c608b82::Run);
