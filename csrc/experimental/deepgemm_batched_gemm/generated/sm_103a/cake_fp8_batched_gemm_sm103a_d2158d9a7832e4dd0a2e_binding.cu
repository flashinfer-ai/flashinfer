...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_fp8_batched_gemm_sm103a_d2158d9a7832e4dd0a2e'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_cake_fp8_batched_gemm_sm103a_d2158d9a7832e4dd0a2e(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap D, unsigned int* __restrict__ SFD, unsigned int M, unsigned int grid_m, unsigned long long sfd_stride, float alpha);


namespace cake_host_shim_4298de0dcb940b64 {
...
void Run(TensorView arg_A, TensorView arg_B, TensorView arg_SFA, TensorView arg_SFB, TensorView arg_D, TensorView arg_SFD, int64_t arg_M, int64_t arg_grid_m, int64_t arg_sfd_stride, double arg_alpha, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_fp8_batched_gemm_sm103a_d2158d9a7832e4dd0a2e),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_fp8_batched_gemm_sm103a_d2158d9a7832e4dd0a2e failed: "
  ...
  config.dynamicSmemBytes = 219520u;
  config.stream = stream;
  config.attrs = attrs;
  config.numAttrs = n;
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_fp8_batched_gemm_sm103a_d2158d9a7832e4dd0a2e), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_fp8_batched_gemm_sm103a_d2158d9a7832e4dd0a2e failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_4298de0dcb940b64

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_4298de0dcb940b64::Run);
