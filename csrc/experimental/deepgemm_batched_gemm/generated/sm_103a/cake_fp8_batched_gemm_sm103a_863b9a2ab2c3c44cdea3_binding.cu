...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_fp8_batched_gemm_sm103a_863b9a2ab2c3c44cdea3'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_cake_fp8_batched_gemm_sm103a_863b9a2ab2c3c44cdea3(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap D, unsigned int* __restrict__ SFD, unsigned int M, unsigned int grid_m, unsigned long long sfd_stride, float alpha);


namespace cake_host_shim_22ee5ea394df817b {
...
inline CUtensorMap EncodeTma_D(const TensorView& t) {
  ...
  TVM_FFI_CHECK(64u <= global_dim[0] && 1u <= global_dim[2], ValueError)
      << "TMA box (64, 128, 1) exceeds resolved global dims for 'D'";
  int64_t carrier_stride_0 = (d1 * d2);
  TVM_FFI_CHECK(carrier_stride_0 >= 0, ValueError)
...
void Run(TensorView arg_A, TensorView arg_B, TensorView arg_SFA, TensorView arg_SFB, TensorView arg_D, TensorView arg_SFD, int64_t arg_M, int64_t arg_grid_m, int64_t arg_sfd_stride, double arg_alpha, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_fp8_batched_gemm_sm103a_863b9a2ab2c3c44cdea3),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_fp8_batched_gemm_sm103a_863b9a2ab2c3c44cdea3 failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_fp8_batched_gemm_sm103a_863b9a2ab2c3c44cdea3), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_fp8_batched_gemm_sm103a_863b9a2ab2c3c44cdea3 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_22ee5ea394df817b

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_22ee5ea394df817b::Run);
