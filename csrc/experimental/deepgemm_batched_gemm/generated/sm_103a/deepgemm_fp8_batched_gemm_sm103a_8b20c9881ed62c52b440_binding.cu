...
// tvm-ffi direct-source launcher for Deepgemm kernel 'kernel_deepgemm_fp8_batched_gemm_sm103a_8b20c9881ed62c52b440'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_deepgemm_fp8_batched_gemm_sm103a_8b20c9881ed62c52b440(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap D, unsigned int* __restrict__ SFD, unsigned int M, unsigned int grid_m, unsigned long long sfd_stride, float alpha);


namespace deepgemm_host_shim_767e7df45a29a111 {
...
inline CUtensorMap EncodeTma_A(const TensorView& t) {
  ...
  TVM_FFI_CHECK(128u <= global_dim[0] && 1u <= global_dim[2], ValueError)
      << "TMA box (128, 32, 1) exceeds resolved global dims for 'A'";
  ...
  uint32_t box_dim[3] = {128u, 32u, 1u};
...
inline CUtensorMap EncodeTma_D(const TensorView& t) {
  ...
  TVM_FFI_CHECK(64u <= global_dim[0] && 1u <= global_dim[2], ValueError)
      << "TMA box (64, 16, 1) exceeds resolved global dims for 'D'";
  int64_t carrier_stride_0 = (d1 * d2);
  TVM_FFI_CHECK(carrier_stride_0 >= 0, ValueError)
...
void Run(TensorView arg_A, TensorView arg_B, TensorView arg_SFA, TensorView arg_SFB, TensorView arg_D, TensorView arg_SFD, int64_t arg_M, int64_t arg_grid_m, int64_t arg_sfd_stride, double arg_alpha, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_deepgemm_fp8_batched_gemm_sm103a_8b20c9881ed62c52b440),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_deepgemm_fp8_batched_gemm_sm103a_8b20c9881ed62c52b440 failed: "
  ...
  config.dynamicSmemBytes = 223616u;
  config.stream = stream;
  config.attrs = attrs;
  config.numAttrs = n;
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_deepgemm_fp8_batched_gemm_sm103a_8b20c9881ed62c52b440), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_deepgemm_fp8_batched_gemm_sm103a_8b20c9881ed62c52b440 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace deepgemm_host_shim_767e7df45a29a111

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, deepgemm_host_shim_767e7df45a29a111::Run);
