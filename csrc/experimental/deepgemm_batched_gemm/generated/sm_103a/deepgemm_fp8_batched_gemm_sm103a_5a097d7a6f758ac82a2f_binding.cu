...
// tvm-ffi direct-source launcher for Deepgemm kernel 'kernel_deepgemm_fp8_batched_gemm_sm103a_5a097d7a6f758ac82a2f'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
void Run(TensorView arg_A, TensorView arg_B, TensorView arg_SFA, TensorView arg_SFB, TensorView arg_D, TensorView arg_SFD, int64_t arg_M, int64_t arg_grid_m, int64_t arg_sfd_stride, double arg_alpha, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_deepgemm_fp8_batched_gemm_sm103a_5a097d7a6f758ac82a2f),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_deepgemm_fp8_batched_gemm_sm103a_5a097d7a6f758ac82a2f failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_deepgemm_fp8_batched_gemm_sm103a_5a097d7a6f758ac82a2f), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_deepgemm_fp8_batched_gemm_sm103a_5a097d7a6f758ac82a2f failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace deepgemm_host_shim_a409a88a707d6d00

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, deepgemm_host_shim_a409a88a707d6d00::Run);
