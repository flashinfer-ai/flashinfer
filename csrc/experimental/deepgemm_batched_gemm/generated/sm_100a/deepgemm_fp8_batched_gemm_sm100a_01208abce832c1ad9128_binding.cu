...
// tvm-ffi direct-source launcher for Deepgemm kernel 'kernel_deepgemm_fp8_batched_gemm_sm100a_01208abce832c1ad9128'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
inline CUtensorMap EncodeTma_D(const TensorView& t) {
  ...
  TVM_FFI_CHECK(128u <= global_dim[0] && 1u <= global_dim[2], ValueError)
      << "TMA box (128, 128, 1) exceeds resolved global dims for 'D'";
  int64_t carrier_stride_0 = (d1 * d2);
  TVM_FFI_CHECK(carrier_stride_0 >= 0, ValueError)
...
void Run(TensorView arg_A, TensorView arg_B, TensorView arg_SFA, TensorView arg_SFB, TensorView arg_D, TensorView arg_SFD, int64_t arg_M, int64_t arg_grid_m, int64_t arg_sfd_stride, double arg_alpha, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_deepgemm_fp8_batched_gemm_sm100a_01208abce832c1ad9128),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_deepgemm_fp8_batched_gemm_sm100a_01208abce832c1ad9128 failed: "
  ...
  config.dynamicSmemBytes = 204544u;
  config.stream = stream;
  config.attrs = attrs;
  config.numAttrs = n;
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_deepgemm_fp8_batched_gemm_sm100a_01208abce832c1ad9128), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_deepgemm_fp8_batched_gemm_sm100a_01208abce832c1ad9128 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace deepgemm_host_shim_c0f8742562bb3f14

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, deepgemm_host_shim_c0f8742562bb3f14::Run);
