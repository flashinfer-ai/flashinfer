...
// tvm-ffi direct-source launcher for Deepgemm kernel 'kernel_deepgemm_fp8_batched_gemm_sm100a_0750744246e0e632c22e'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_deepgemm_fp8_batched_gemm_sm100a_0750744246e0e632c22e(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, const __grid_constant__ CUtensorMap D, unsigned int* __restrict__ SFD, unsigned int M, unsigned int grid_m, unsigned long long sfd_stride, float alpha);
...
void Run(TensorView arg_A, TensorView arg_B, TensorView arg_SFA, TensorView arg_SFB, TensorView arg_D, TensorView arg_SFD, int64_t arg_M, int64_t arg_grid_m, int64_t arg_sfd_stride, double arg_alpha, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_deepgemm_fp8_batched_gemm_sm100a_0750744246e0e632c22e),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_deepgemm_fp8_batched_gemm_sm100a_0750744246e0e632c22e failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_deepgemm_fp8_batched_gemm_sm100a_0750744246e0e632c22e), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_deepgemm_fp8_batched_gemm_sm100a_0750744246e0e632c22e failed: "
