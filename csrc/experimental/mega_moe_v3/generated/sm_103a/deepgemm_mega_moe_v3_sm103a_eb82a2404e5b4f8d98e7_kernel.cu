    ...
    if (warp <= 3) {
        { // dispatch_main
            ...
            for (int pair_idx = global_warp_idx; pair_idx < total_pairs; pair_idx += warps_per_grid) {
                ...
                __syncwarp();
                float delayed_rw = topk_weights[pair_idx];
    ...
    if (warp >= 8 && warp <= 15) {
        { // epilogue_main
                ...
                if (task_4[0] == 1) {
                    ...
                    for (int s = 0; s < 1; s++) {
                        ...
                        for (int i_3 = 0; i_3 < 1; i_3++) {
                            ...
                            float2 _f2_5 = make_float2(activation[i_3 * 4], activation[i_3 * 4 + 1]);
                            float2 _mul_f32x2_6;
                            ...
                            float2 _f2_6 = make_float2(activation[i_3 * 4 + 2], activation[i_3 * 4 + 3]);
                            float2 _mul_f32x2_7;
            ...
            for (unsigned int token_chunk = epi_warp_2 * 152 + (unsigned int)bid; token_chunk < num_tokens * 2; token_chunk += 1216) {
                ...
                unsigned int _shfl_56 = __shfl_sync(0xFFFFFFFF, stored_row, 0);
                ...
                reduced[79] = 0.0f;
                #pragma unroll 1
                for (unsigned int selection = 0; selection < 6; selection++) {
                    if (selection + 1 < 6) {
                        unsigned int _shfl_57 = __shfl_sync(0xFFFFFFFF, stored_row, selection + 1);
