        ...
        { // dispatch_main
            ...
            for (int mapping_pair = bid * 4 + disp_warp; mapping_pair < total_pairs; mapping_pair += NUM_CTAS * 4) {
                if (lane == 0) {
                    ...
                    int _atomic_old_0 = atomicAdd(&expert_scatter_offsets[mapping_expert], 1);
                    int mapping_claim = _atomic_old_0;
                ...
                if (elect_sync()) {
                    ...
                    }
                    {
                        ...
                        while (true) {
                            unsigned int _gca_v;
                            asm volatile("ld.acquire.gpu.global.u32 %0, [%1];" : "=r"(_gca_v) : "l"(_gca_p));
                            if (_gca_v >= (unsigned int)(2 * NUM_CTAS)) break;
                        }
                    }
                }
            }
                ...
                if (task_4[0] == 1) {
                    ...
                    for (int s = 0; s < 1; s++) {
                        ...
                        for (int i_3 = 0; i_3 < 1; i_3++) {
                            ...
                            float _max_9 = max_noftz(amax[i_3 * 2], amax_reduction[paired_index]);
                            float first_max_1 = _max_9;
                            float _max_10 = max_noftz(amax[i_3 * 2 + 1], amax_reduction[paired_index + 1]);
