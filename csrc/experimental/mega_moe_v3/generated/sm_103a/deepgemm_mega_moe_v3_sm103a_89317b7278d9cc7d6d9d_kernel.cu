...
#define SMEM_COMBINE_SMEM_STAGE_BYTES 122880
#define SMEM_COMBINE_SMEM_STRIDE 122880
...
kernel_deepgemm_mega_moe_v3_sm103a_89317b7278d9cc7d6d9d(int* __restrict__ topk_idx_i32, float* __restrict__ topk_weights, int* __restrict__ x_fp8, unsigned int* __restrict__ x_sf, DeepgemmTensorMap const* A, int* __restrict__ pool_fp8, unsigned int* __restrict__ pool_sf, float* __restrict__ routing_weight_pool, int* __restrict__ token_to_permuted, int* __restrict__ meta_token, int* __restrict__ meta_slot, int* __restrict__ expert_counts, int* __restrict__ expert_row_offsets, int* __restrict__ expert_scatter_offsets, int* __restrict__ tile_expert, int* __restrict__ tile_m_local, int* __restrict__ total_m_tiles_out, DeepgemmTensorMap const* B1, DeepgemmTensorMap const* SFB1, DeepgemmTensorMap const* I_fp8_w, uint8_t* __restrict__ SF_I_w, __nv_bfloat16* __restrict__ l1_bf16_capture, DeepgemmTensorMap const* A2, unsigned int* __restrict__ SFA2, DeepgemmTensorMap const* B2, DeepgemmTensorMap const* SFB2, __nv_bfloat16* __restrict__ expert_output, __nv_bfloat16* __restrict__ y, unsigned int* __restrict__ histogram_done, unsigned int* __restrict__ prefix_done, unsigned int* __restrict__ dispatch_done, unsigned int* __restrict__ l1_arrival, unsigned int* __restrict__ l2_done, int num_tokens, int top_k, int num_experts, int N1, int K1, int grid_n1, int K1_tiles, int N2, int K2, int grid_n2, int K2_tiles, int total_m_tiles, int M_total, float activation_clamp, unsigned int* __restrict__ PrivateCounters, unsigned long long* __restrict__ PrivateMasks, unsigned int* __restrict__ PublicExpertOutput, unsigned int* __restrict__ PrivateSFBlockOffsets, DeepgemmTensorMap const* PrivateSF1, DeepgemmTensorMap const* PrivateSF2, unsigned int* __restrict__ PrivateSF1Words, uint8_t* __restrict__ PrivateSF2Bytes)
{
    ...
    #define pull_barriers_addr (mbar_base + 368)
    ...
    if (tid == 0) {
        ...
        asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" :: "l"((uint64_t)(I_fp8_w)) : "memory");
        ...
        asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" :: "l"((uint64_t)(PrivateSF1)) : "memory");
        asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" :: "l"((uint64_t)(PrivateSF2)) : "memory");
    }
    __syncthreads();
    ...
    if (warp <= 3) {
        { // dispatch_main
            ...
            if (warp == 0) {
                if (elect_sync()) {
                    {
                        unsigned int* _gca_p = reinterpret_cast<unsigned int*>(histogram_done) + (0);
                        while (true) {
                            unsigned int _gca_v;
                            asm volatile("ld.acquire.gpu.global.u32 %0, [%1];" : "=r"(_gca_v) : "l"(_gca_p));
                            if (_gca_v >= (unsigned int)(NUM_CTAS)) break;
                        }
                    }
                }
            }
            ...
            if (warp == 0) {
                if (elect_sync()) {
                    {
                        unsigned int* _gca_p = reinterpret_cast<unsigned int*>(prefix_done) + (0);
                        while (true) {
                            unsigned int _gca_v;
                            asm volatile("ld.acquire.gpu.global.u32 %0, [%1];" : "=r"(_gca_v) : "l"(_gca_p));
    ...
    if (warp == 4) {
        { // load_a_main
            ...
            while (1) {
                mbarrier_wait_hint(task_full_addr + (task_iteration % 2) * 8, task_iteration / 2 & 1, 10000000);
                ...
                for (int k = 0; k < task[7] / 128; k++) {
                    ...
                    mbarrier_wait_hint(empty_addr + (a_stage) * 8, _phase_empty, 10000000);
                    if (elect_sync()) {
                        if (task[0] == 1) {
    ...
    if (warp == 5) {
        { // load_b_main
            ...
            while (1) {
                mbarrier_wait_hint(task_full_addr + (task_iteration_1 % 2) * 8, task_iteration_1 / 2 & 1, 10000000);
                ...
                for (int k_1 = 0; k_1 < task_1[7] / 128; k_1++) {
                    mbarrier_wait_hint(empty_addr + (b_stage) * 8, _phase_empty_1, 10000000);
    ...
    if (warp == 6) {
        { // mma_main
            ...
            if (cta_rank == 0) {
                ...
                while (1) {
                    mbarrier_wait_hint(task_full_addr + (task_iteration_2 % 2) * 8, task_iteration_2 / 2 & 1, 10000000);
                    ...
                    for (int k_2 = 0; k_2 < task_2[7] / 128; k_2++) {
                        mbarrier_wait_hint(full_addr + (mma_stage) * 8, _phase_full, 10000000);
                        asm volatile("tcgen05.fence::after_thread_sync;");
        ...
        { // schedule_main
            ...
            if (cta_rank == 0) {
                ...
                for (int iteration = 0; iteration < 14821; iteration++) {
                    mbarrier_wait_hint(task_empty_addr + (schedule_iteration % 2) * 8, schedule_iteration / 2 & 1 ^ 1, 10000000);
                    ...
                    if (state[1] != 4294967295 && state[1] != 0) {
                        ...
                        if (lane == 0) {
                            unsigned int _atomic_old_1 = atomicAdd(PrivateCounters, 1);
                            claimed_0 = _atomic_old_1;
                        }
                    ...
                    if (phase == 0) {
                        ...
                        if (lane == 0) {
                            unsigned int _atomic_old_2 = atomicAdd(PrivateCounters + 1, 1);
                            claimed_0_1 = _atomic_old_2;
                        }
    ...
    if (warp >= 8 && warp <= 15) {
        { // epilogue_main
            ...
            while (1) {
                mbarrier_wait_hint(task_full_addr + (task_iteration_3 % 2) * 8, task_iteration_3 / 2 & 1, 10000000);
                ...
                if (task_4[0] == 1) {
                    ...
                    unsigned int block = ((unsigned int)expert_row_offsets[task_4[1]] + task_4[2] * 16) / 16;
                    ...
                    for (int s = 0; s < 1; s++) {
                        ...
                        for (int i_2 = 0; i_2 < 1; i_2++) {
                            unsigned int j = s + i_2;
                            if (task_4[0] == 1 && j * 8 % 32 == 0) {
                                if (j * 8 + lane < 8) {
                                    cached_weight = routing_weight_pool[block * 16 + epi_wg * 8 + j * 8 + lane];
                                }
                            }
                            float _shfl_53;
                            ...
                            activation[i_2 * 4] = _mul_f32x2_2.x;
                            activation[i_2 * 4 + 1] = _mul_f32x2_2.y;
                            ...
                            activation[i_2 * 4 + 2] = _mul_f32x2_5.x;
                            activation[i_2 * 4 + 3] = _mul_f32x2_5.y;
                            float _fabs_0 = fabsf(activation[i_2 * 4]);
                            float _fabs_1 = fabsf(activation[i_2 * 4 + 2]);
                            float _max_3 = max_noftz(_fabs_0, _fabs_1);
                            float first_max = _max_3;
                            float _fabs_2 = fabsf(activation[i_2 * 4 + 1]);
                            float _fabs_3 = fabsf(activation[i_2 * 4 + 3]);
                            ...
                            second_max = _max_10;
                            amax[i_2 * 2] = first_max;
                            amax[i_2 * 2 + 1] = second_max;
                            if (lane < 4) {
                                amax_reduction[epi_warp * 8 + (unsigned int)(i_2 * 8) + lane * 2] = first_max;
                                amax_reduction[epi_warp * 8 + (unsigned int)(i_2 * 8) + lane * 2 + 1] = second_max;
                            }
                            __syncwarp();
                        }
                        ...
                        for (int i_3 = 0; i_3 < 1; i_3++) {
                            unsigned int paired_index = (epi_warp ^ 1) * 8 + (unsigned int)(i_3 * 8) + lane % 4 * 2;
                            float _max_11 = max_noftz(amax[i_3 * 2], amax_reduction[paired_index]);
                            float first_max_1 = _max_11;
                            float _max_12 = max_noftz(amax[i_3 * 2 + 1], amax_reduction[paired_index + 1]);
                            ...
                            quantized[0] = activation[i_3 * 4] * first_inverse;
                            quantized[1] = activation[i_3 * 4 + 1] * second_inverse;
                            quantized[2] = activation[i_3 * 4 + 2] * first_inverse;
                            quantized[3] = activation[i_3 * 4 + 3] * second_inverse;
                            uint32_t _fp8_0[1];
                            {
                                uint32_t _packed;
                                asm volatile("{\n\t"
                            ...
                            unsigned int stsm_address = store_address + (unsigned int)(i_3 * 8 * 64) + lane * 64 + (warp_in_wg ^ lane / 2) * 16;
                            ...
                            if (warp_in_wg % 2 == 0 && lane < 4) {
                                ...
                                unsigned int token_base = epi_wg * 8 + (unsigned int)(s * 8) + (unsigned int)(i_3 * 8);
                                unsigned int sf_address = (block * 16 + token_base + lane * 2) * 72 + sf_k;
                        ...
                        if (warp_in_wg == 0) {
                            if (elect_sync()) {
                                asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                                if (task_4[0] == 3) {
                                    tma_store_2d(I_fp8_w, n_block * 64, block * 16 + epi_wg * 8 + (unsigned int)(s * 8), store_address);
                                } else {
                                    tma_store_2d(I_fp8_w, n_block * 64, block * 16 + epi_wg * 8 + (unsigned int)(s * 8), store_address);
                                }
                                asm volatile("cp.async.bulk.commit_group;");
                    ...
                    for (int s_1 = 0; s_1 < 1; s_1++) {
                        ...
                        for (int i_4 = 0; i_4 < 1; i_4++) {
                            unsigned int address_1 = accum_stage_1 * 16 + epi_wg_1 * 8 + (unsigned int)(s_1 * 8) + (unsigned int)(i_4 * 8);
                            ...
                            unsigned int write_address = output_base + warp_in_wg_1 / 2 * 8 * 128 + (unsigned int)(i_4 * 8 * 128) + row_1 * 128 + (col ^ row_1) * 16;
            ...
            if (warp == 8) {
                if (elect_sync()) {
                    {
                        unsigned int* _gca_p = reinterpret_cast<unsigned int*>(l2_done) + (0);
                        while (true) {
                            unsigned int _gca_v;
                            asm volatile("ld.acquire.gpu.global.u32 %0, [%1];" : "=r"(_gca_v) : "l"(_gca_p));
                            if (_gca_v >= (unsigned int)(NUM_CTAS)) break;
            ...
            for (unsigned int token_chunk = epi_warp_2 * 152 + (unsigned int)bid; token_chunk < num_tokens * 2; token_chunk += 1216) {
                unsigned int token = token_chunk / 2;
                unsigned int chunk = token_chunk % 2;
                unsigned int stored_row = 0;
                if (lane < 6) {
                    stored_row = (unsigned int)token_to_permuted[token * 6 + lane];
                }
                unsigned int _vote_12 = __ballot_sync(0xFFFFFFFF, lane < 6);
                unsigned int mask = _vote_12;
                bool do_reduce = 0;
                if (mask != 0) {
                    int _ffs_12 = __ffs(mask);
                    unsigned int slot = (unsigned int)(_ffs_12 - 1);
                    mask ^= (unsigned int)1 << slot;
                    unsigned int _shfl_56 = __shfl_sync(0xFFFFFFFF, stored_row, slot);
                    unsigned int first_row = _shfl_56;
                    if (elect_sync()) {
                        cp_async_bulk_gmem2smem(combine_smem_addr + (epi_warp_2 + stage * 8) * 5120, expert_output + ((unsigned long long)first_row * 5120 + (unsigned long long)(chunk * 2560)), 5120, combine_barriers_addr + (epi_warp_2 * 2 + stage) * 8);
                ...
                while (do_reduce) {
                    ...
                    if (mask != 0) {
                        ...
                        unsigned int slot_1 = (unsigned int)(_ffs_13 - 1);
                        mask ^= (unsigned int)1 << slot_1;
                        unsigned int _shfl_57 = __shfl_sync(0xFFFFFFFF, stored_row, slot_1);
                        unsigned int next_row = _shfl_57;
                        if (elect_sync()) {
                            cp_async_bulk_gmem2smem(combine_smem_addr + (epi_warp_2 + (stage ^ 1) * 8) * 5120, expert_output + ((unsigned long long)next_row * 5120 + (unsigned long long)(chunk * 2560)), 5120, combine_barriers_addr + (epi_warp_2 * 2 + (stage ^ 1)) * 8);
                            mbarrier_arrive_expect_tx(combine_barriers_addr + (epi_warp_2 * 2 + (stage ^ 1)) * 8, 5120);
                        }
                        __syncwarp();
                        do_reduce = 1;
                    }
                    mbarrier_wait_hint(combine_barriers_addr + (epi_warp_2 * 2 + stage) * 8, phase_1, 10000000);
                    #pragma unroll
                    for (unsigned int j_3 = 0; j_3 < 10; j_3++) {
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                            ...
                            : "r"(combine_smem_addr + (epi_warp_2 + stage * 8) * 5120 + (j_3 * 32 + lane) * 16));
                ...
                for (unsigned int j_4 = 0; j_4 < 10; j_4++) {
                    ...
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" ::
                        "r"(combine_smem_addr + (epi_warp_2 + 16) * 5120 + (j_4 * 32 + lane) * 16), "r"(*reinterpret_cast<uint32_t*>(&store_values[0])), "r"(*reinterpret_cast<uint32_t*>(&store_values[(0) + 1])), "r"(*reinterpret_cast<uint32_t*>(&store_values[(0) + 2])), "r"(*reinterpret_cast<uint32_t*>(&store_values[(0) + 3])));
                }
                __syncwarp();
                if (elect_sync()) {
                    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                    {
                        void* _cpbulk_dst_4 = reinterpret_cast<void*>(y + ((unsigned long long)token * 5120 + (unsigned long long)(chunk * 2560)));
                        asm volatile(
                            "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
                            :: "l"(_cpbulk_dst_4), "r"(combine_smem_addr + (epi_warp_2 + 16) * 5120), "r"((uint32_t)(5120))
                            : "memory");
                    }
                    asm volatile("cp.async.bulk.commit_group;");
                }
