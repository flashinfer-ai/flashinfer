...
#define SMEM_SMEM_SFA_Q1_OFF 50688
#define SMEM_SMEM_SFA_Q1_STAGE_BYTES 512
#define SMEM_SMEM_SFA_Q1_STRIDE 51200
#define SMEM_SMEM_SFB_Q1_OFF 51712
#define SMEM_SMEM_SFB_Q1_STAGE_BYTES 512
#define SMEM_SMEM_SFB_Q1_STRIDE 51200
...
kernel_cake_mega_moe_v3_sm103a_449586525e4a9bc31aa9(CakeTensorMap const* A, CakeTensorMap const* B, CakeTensorMap const* SFA, CakeTensorMap const* SFB, __nv_bfloat16* __restrict__ Out, int* __restrict__ tile_expert, int* __restrict__ tile_m_local, int* __restrict__ expert_row_offsets, int M_total, int N, int K, int grid_n, int K_tiles, int total_m_tiles)
{
    ...
    uint8_t* smem_sfa_q1 = reinterpret_cast<uint8_t*>(smem_raw + 50688);
    const int smem_sfa_q1_addr = smem + 50688;
    uint8_t* smem_sfb_q1 = reinterpret_cast<uint8_t*>(smem_raw + 51712);
    const int smem_sfb_q1_addr = smem + 51712;
    ...
    if (warp == 0) {
        ...
        if (leader) {
            // --- pipeline 'tma_pipe' ---
            // tma_full: 4 barriers, init_count=4
    ...
    if (warp <= 3) {
        { // epilogue_main
            ...
            for (unsigned int this_bid = bid; this_bid < num_tiles; this_bid += num_bids) {
                ...
                int row_off = expert_row_offsets[tile_expert[m_tile]];
                int off_m = row_off + local_m;
                int out_n_base = n_tile * 128;
                ...
                for (int n_chunk = 0; n_chunk < 16; n_chunk++) {
                    ...
                    tmem_ld_x8(&_tmem_load_0[0], taddr + (unsigned int)(tmem_row << 16) + (unsigned int)col);
                    asm volatile("tcgen05.wait::ld.sync.aligned;");
                    if (n_chunk == 15) {
                    ...
                    }
                    {
                        ...
                        *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(Out + (token * N + out_n_base + n_chunk * 8)))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                    }
                }
                epi_stage += 1;
            ...
            if (cta_rank == 0) {
                ...
                for (unsigned int this_bid_1 = bid; this_bid_1 < num_tiles_1; this_bid_1 += num_bids) {
                    ...
                    for (int iter_k = 0; iter_k < K_tiles; iter_k++) {
                        ...
                        if (elect_sync()) {
                            tcgen05_cp_32x128b_warpx4_cta2(tmem_tmem_sfa, make_sf_cp_desc_lo_sbo128((((smem_sfa_addr) >> 4) + (mma_tma_stage) * 3200)));
                            tcgen05_cp_32x128b_warpx4_cta2(tmem_tmem_sfb, make_sf_cp_desc_lo_sbo128((((smem_sfb_addr) >> 4) + (mma_tma_stage) * 3200)));
                            ...
                            tcgen05_cp_32x128b_warpx4_cta2(tmem_tmem_sfa, make_sf_cp_desc_lo_sbo128((((smem_sfa_q1_addr) >> 4) + (mma_tma_stage) * 3200)));
                            tcgen05_cp_32x128b_warpx4_cta2(tmem_tmem_sfb, make_sf_cp_desc_lo_sbo128((((smem_sfb_q1_addr) >> 4) + (mma_tma_stage) * 3200)));
                            ...
                            int _mma_b_lo_1 = (((smem_b_addr + 8192) >> 4) & 0x3FFF) + (mma_tma_stage) * 3200;
                            {
                                ...
                                tcgen05_mma_mxf8_bs_cta2((tmem_tmem_acc + (mma_epi_stage * 128)), a_desc + 0, b_desc + 0,
                                    0x10a01400U, tmem_tmem_sfa, tmem_tmem_sfb, 1);
                                tcgen05_mma_mxf8_bs_cta2((tmem_tmem_acc + (mma_epi_stage * 128)), a_desc + 2, b_desc + 2,
                                    0x30a01410U, tmem_tmem_sfa, tmem_tmem_sfb, 1);
                                tcgen05_mma_mxf8_bs_cta2((tmem_tmem_acc + (mma_epi_stage * 128)), a_desc + 4, b_desc + 4,
                                    0x50a01420U, tmem_tmem_sfa, tmem_tmem_sfb, 1);
            ...
            for (unsigned int this_bid_2 = bid; this_bid_2 < num_tiles_2; this_bid_2 += num_bids) {
                ...
                for (int iter_k_1 = 0; iter_k_1 < K_tiles; iter_k_1++) {
                    ...
                    if (elect_sync()) {
                        ...
                        tma_2d_gmem2smem_cta2(smem_sfa_addr + load_stage * 51200, SFA, off_m_1, iter_k_1 * 2, ((tma_full_addr + (load_stage) * 8) & 0xFEFFFFFF));
                        if (cta_rank == 0) {
                            mbarrier_arrive_expect_tx(tma_full_addr + (load_stage) * 8, 67584);
                        } else {
                            asm volatile(
                                "mbarrier.arrive.release.cta.shared::cluster.b64 _, [%0];"
                                :: "r"((tma_full_addr + (load_stage) * 8) & 0xFEFFFFFF) : "memory");
                        }
                    }
                    __syncwarp();
                    load_stage += 1;
    ...
    if (warp == 6) {
        { // load_b_main
            unsigned int load_b_stage = 0;
            int num_tiles_3 = total_m_tiles * grid_n;
            int sf_words_1 = K / 128;
            unsigned int _phase_mma_done_1 = 1;
            #pragma unroll 1
            for (unsigned int this_bid_3 = bid; this_bid_3 < num_tiles_3; this_bid_3 += num_bids) {
                int m_tile_2 = this_bid_3 / (unsigned int)(grid_n * 2) * 2 + this_bid_3 % 2;
                int n_tile_2 = this_bid_3 / 2 % (unsigned int)grid_n;
                int exp_id_2 = tile_expert[m_tile_2];
                int off_n_1 = n_tile_2 * 128;
                #pragma unroll 1
                for (int iter_k_2 = 0; iter_k_2 < K_tiles; iter_k_2++) {
                    mbarrier_wait(mma_done_addr + (load_b_stage) * 8, _phase_mma_done_1);
                    if (elect_sync()) {
                        tma_4d_gmem2smem_cta2(smem_b_addr + load_b_stage * 51200, B, 0, off_n_1 + cta_rank * 64, iter_k_2 * 2, exp_id_2, ((tma_full_addr + (load_b_stage) * 8) & 0xFEFFFFFF));
                        tma_2d_gmem2smem_cta2(smem_sfb_addr + load_b_stage * 51200, SFB, off_n_1, exp_id_2 * sf_words_1 + iter_k_2 * 2, ((tma_full_addr + (load_b_stage) * 8) & 0xFEFFFFFF));
                        if (cta_rank == 0) {
                            mbarrier_arrive_expect_tx(tma_full_addr + (load_b_stage) * 8, 18432);
                        } else {
                            asm volatile(
                                "mbarrier.arrive.release.cta.shared::cluster.b64 _, [%0];"
                                :: "r"((tma_full_addr + (load_b_stage) * 8) & 0xFEFFFFFF) : "memory");
                        }
                    }
                    __syncwarp();
                    load_b_stage += 1;
                    if (load_b_stage == 4) { load_b_stage = 0; _phase_mma_done_1 ^= 1; }
                }
            }
        }
    }
