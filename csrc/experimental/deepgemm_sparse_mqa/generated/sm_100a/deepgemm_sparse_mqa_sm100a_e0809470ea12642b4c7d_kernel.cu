...
kernel_deepgemm_sparse_mqa_sm100a_e0809470ea12642b4c7d(unsigned int* __restrict__ Starts, unsigned int* __restrict__ Ends, unsigned int* __restrict__ Context, unsigned int* __restrict__ BlockTable, unsigned int* __restrict__ Requests, unsigned int* __restrict__ Sparse, unsigned int* __restrict__ Metadata, unsigned int* __restrict__ Workspace, unsigned int num_q_tokens, unsigned int num_kv_tokens, unsigned int block_table_stride, unsigned int num_ctas)
{
    ...
    for (unsigned int claim_round = 0; claim_round < num_q_tokens + 1; claim_round++) {
        ...
        for (int j_1 = 0; j_1 < 16; j_1++) {
            if (merged_count > (unsigned int)j_1) {
                ...
                if (merged_idx % 80 == 0) {
                    split_bases[merged_idx / 80] = packed_local[j_1] & 2147450879;
                }
            }
        }
        __syncthreads();
        ...
        for (int j_2 = 0; j_2 < 16; j_2++) {
            if (merged_count > (unsigned int)j_2) {
                unsigned int packed_value = packed_local[j_2];
                unsigned int split_off = (lane_sum - merged_count + preceding + (unsigned int)j_2) / 80;
                unsigned int split = split_base + split_off;
                unsigned int block = (lane_sum - merged_count + preceding + (unsigned int)j_2) % 80;
                unsigned int bases = 0;
                bases = split_bases[split_off];
                ...
                physical = BlockTable[(unsigned long long)qbase * (unsigned long long)block_table_stride + (unsigned long long)logical_page] * 8 + logical_block % 8;
                ...
                if (block == 0) {
                    int _min_2 = ((80) < (total - (lane_sum - merged_count + preceding + (unsigned int)j_2)) ? (80) : (total - (lane_sum - merged_count + preceding + (unsigned int)j_2)));
    ...
    if (state[5] != 0) {
        ...
        int _max_1 = ((1) > ((nentries + 148 - 1) / 148) ? (1) : ((nentries + 148 - 1) / 148));
        unsigned int waves = _max_1;
        #pragma unroll 1
        for (unsigned int entry_1 = nentries + (unsigned int)tid; entry_1 < waves * 148; entry_1 += 256) {
            ...
            for (unsigned int wave = warp; wave < waves; wave += 8) {
                ...
                for (int ei = 0; ei < 5; ei++) {
                    ...
                    if (sm < 148) {
                        ...
                        {
                            const uint4* _ivptr_0 = reinterpret_cast<const uint4*>(Metadata + sched + (wave * 148 + sm) * 4);
                            uint4 _ivld_0;
                            _ivld_0 = *_ivptr_0;
                        ...
                        for (int j_4 = 0; j_4 < 4; j_4++) {
                            entry_regs[ei * 4 + j_4] = _vec_load_1[j_4];
                        }
                    }
                }
                #pragma unroll 1
                ...
                for (int ei_1 = 0; ei_1 < 5; ei_1++) {
                    if (lane + (unsigned int)(ei_1 * 32) < 148) {
                ...
                for (int ei_2 = 0; ei_2 < 5; ei_2++) {
                    if (lane + (unsigned int)(ei_2 * 32) < 148) {
                        unsigned int count_3 = entry_regs[ei_2 * 4 + 1] - entry_regs[ei_2 * 4];
                        unsigned int rank = hist[warp * 9 + count_3] + ranks[ei_2];
                        unsigned int dst_sm = ((waves == 2 && wave == 1) ? 147 - rank : (rank + 148 - wave * 148 / waves) % 148);
                        unsigned int entry_store[4];
                        #pragma unroll
                        for (int j_5 = 0; j_5 < 4; j_5++) {
                            entry_store[j_5] = entry_regs[ei_2 * 4 + j_5];
                        }
                        reinterpret_cast<int4*>(Metadata + (sched + (wave * 148 + dst_sm) * 4))[0] = reinterpret_cast<int4*>(entry_store)[0];
                    }
                }
            }
        }
