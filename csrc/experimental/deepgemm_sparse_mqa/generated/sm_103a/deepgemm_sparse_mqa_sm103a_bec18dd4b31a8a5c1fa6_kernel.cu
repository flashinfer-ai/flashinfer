...
#define SMEM_SPLIT_BASES_STAGE_BYTES 256
#define SMEM_SPLIT_BASES_STRIDE 256
    ...
    for (unsigned int claim_round = 0; claim_round < num_q_tokens + 1; claim_round++) {
        ...
        for (int j_1 = 0; j_1 < 16; j_1++) {
            if (merged_count > (unsigned int)j_1) {
                ...
                if (merged_idx % 64 == 0) {
                    split_bases[merged_idx / 64] = packed_local[j_1] & 2147450879;
                }
            }
        }
        __syncthreads();
        ...
        for (int j_2 = 0; j_2 < 16; j_2++) {
            if (merged_count > (unsigned int)j_2) {
                ...
                unsigned int split_off = (lane_sum - merged_count + preceding + (unsigned int)j_2) / 64;
                unsigned int split = split_base + split_off;
                unsigned int block = (lane_sum - merged_count + preceding + (unsigned int)j_2) % 64;
                ...
                if (block == 0) {
                    int _min_2 = ((64) < (total - (lane_sum - merged_count + preceding + (unsigned int)j_2)) ? (64) : (total - (lane_sum - merged_count + preceding + (unsigned int)j_2)));
        ...
        #pragma unroll 1
        for (unsigned int padded = total + (unsigned int)tid; padded < nsplits * 64; padded += 256) {
