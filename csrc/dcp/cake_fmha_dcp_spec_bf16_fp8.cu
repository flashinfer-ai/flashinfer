...
#define SMEM_SMEM_P1_OFF 204800
...
kernel_cake_fmha_dcp_spec_bf16_fp8(LoomTensorMap const* Qt, LoomTensorMap const* K, LoomTensorMap const* V, __nv_bfloat16* __restrict__ O_ptr, float* __restrict__ LSE_ptr, int* __restrict__ page_table, int* __restrict__ seq_lens_kv, int* __restrict__ causal_seqlens_kv_global, int max_pages_per_seq, int max_local_seq_len, float softmax_scale_log2, float output_scale, int cp_rank, int num_q_heads, int num_kv_heads, int batch_size)
{
    ...
    __nv_bfloat16* smem_kv = reinterpret_cast<__nv_bfloat16*>(smem_raw + 71680);
    const int smem_kv_addr = smem + 71680;
    __nv_bfloat16* smem_v = reinterpret_cast<__nv_bfloat16*>(smem_raw + 71680);
    const int smem_v_addr = smem + 71680;
    ...
    if (warp == 0) {
        ...
        mbarrier_init_pred(smem + 128, 256, leader);
    ...
    #define kv_src_full_addr (mbar_base + 16)
    #define kv_full_addr (mbar_base + 48)
    #define kv_empty_addr (mbar_base + 80)
    #define s_full_0_addr (mbar_base + 112)
    #define s_full_1_addr (mbar_base + 120)
    #define p_full_0_addr (mbar_base + 128)
    #define p_full_1_addr (mbar_base + 136)
    #define corr_scale_0_addr (mbar_base + 144)
    #define corr_scale_1_addr (mbar_base + 152)
    #define corr_empty_0_addr (mbar_base + 160)
    #define corr_empty_1_addr (mbar_base + 168)
    #define stats_empty_addr (mbar_base + 176)
    #define o_ready_0_addr (mbar_base + 184)
    #define o_ready_1_addr (mbar_base + 192)
    #define o_empty_0_addr (mbar_base + 200)
    #define o_empty_1_addr (mbar_base + 208)
    ...
    if (warp <= 3) {
        ...
        { // softmax_main
            ...
            for (unsigned int tile_idx_s = blockIdx.x; tile_idx_s < total_tiles_s; tile_idx_s += gridDim.x) {
                ...
                int kv_head_idx = tile_idx_s % (unsigned int)NUM_KV_HEADS;
                int request_row = tile_idx_s / (unsigned int)NUM_KV_HEADS;
    ...
    // ---- Role: correction ----
        ...
        { // correction_main
            ...
            for (unsigned int tile_idx_c = blockIdx.x; tile_idx_c < total_tiles_c; tile_idx_c += gridDim.x) {
                int kv_head_idx_1 = tile_idx_c % (unsigned int)NUM_KV_HEADS;
                int request_row_1 = tile_idx_c / (unsigned int)NUM_KV_HEADS;
                ...
                for (int pair_1 = 1; pair_1 < cta_n_blocks_1 / 2; pair_1++) {
                    ...
                    for (int c_8 = 0; c_8 < 2; c_8++) {
                        float max_diff0 = _tmem_load_0[c_8] - _tmem_load_0[c_8 + 2];
                        float scaled_diff0 = softmax_scale_log2 * max_diff0;
                        float _exp2_2 = approx_exp2(scaled_diff0);
                        acc0_pair[c_8] = ((max_diff0 != 0.0f) ? _exp2_2 : 1.0f);
                    }
                    mbarrier_arrive(corr_empty_0_addr);
                    ...
                    for (int c_9 = 0; c_9 < 2; c_9++) {
                        float max_diff1 = _tmem_load_1[c_9] - _tmem_load_1[c_9 + 2];
                        float scaled_diff1 = softmax_scale_log2 * max_diff1;
                        float _exp2_3 = approx_exp2(scaled_diff1);
                        acc1_pair[c_9] = ((max_diff1 != 0.0f) ? _exp2_3 : 1.0f);
                    }
                    mbarrier_arrive(corr_empty_1_addr);
                ...
                float local_max[8];
                #pragma unroll
                for (int c_10 = 0; c_10 < 8; c_10++) {
                    float m0 = smem_exch0[c_10];
                    float m1 = smem_exch1[c_10];
                    float s0 = smem_corr0[c_10];
                    float s1 = smem_corr1[c_10];
                    ...
                    scale0[c_10] = ((m0 == -LOOM_INF) ? 0.0f : _exp2_4);
                    float _exp2_5 = approx_exp2(d1);
                    scale1[c_10] = ((m1 == -LOOM_INF) ? 0.0f : _exp2_5);
                    local_sum[c_10] = s0 * scale0[c_10] + s1 * scale1[c_10];
                }
                mbarrier_arrive(corr_empty_0_addr);
                mbarrier_arrive(corr_empty_1_addr);
                mbarrier_wait(o_ready_0_addr, _phase_o_ready_0_0);
                ...
                for (int h_6 = 0; h_6 < 8; h_6++) {
                    ...
                    if (local_sum[h_6] > 0.0f && local_sum[h_6] == local_sum[h_6]) {
                        ...
                        final_o = merged * _rcp_0 * output_scale;
                        float _log2_0;
    ...
    // ---- Role: mma_warp ----
        ...
        { // mma_warp_main
            ...
            for (unsigned int tile_idx_m = blockIdx.x; tile_idx_m < total_tiles_m; tile_idx_m += gridDim.x) {
                int kv_head_idx_2 = tile_idx_m % (unsigned int)NUM_KV_HEADS;
                int request_row_2 = tile_idx_m / (unsigned int)NUM_KV_HEADS;
    ...
    // ---- Role: load_warp ----
        ...
        { // load_warp_main
            ...
            for (unsigned int tile_idx_l = blockIdx.x; tile_idx_l < total_tiles_l; tile_idx_l += gridDim.x) {
                int kv_head_idx_3 = tile_idx_l % (unsigned int)NUM_KV_HEADS;
                int request_row_3 = tile_idx_l / (unsigned int)NUM_KV_HEADS;
                ...
                if (elect_sync()) {
                    ...
                    for (int ni = 0; ni < prefill; ni++) {
                        ...
                        mbarrier_arrive_expect_tx(kv_src_full_addr + (kv_stage) * 8, 16384);
                        ...
                        for (int pg_i = 0; pg_i < 2; pg_i++) {
                            ...
                            {
                                tma_4d_gmem2smem(ldst + toff, K, 0, 0, kv_head_idx_3, pg_k[pg_i], kv_src_full_addr + (kv_stage) * 8);
                            }
                        }
                        kv_stage += 1;
                    ...
                    for (int ni_1 = 0; ni_1 < cta_n_blocks_3; ni_1++) {
                        ...
                        mbarrier_arrive_expect_tx(kv_src_full_addr + (stage) * 8, 16384);
                        ...
                        for (int pg_i_1 = 0; pg_i_1 < 2; pg_i_1++) {
                            ...
                            {
                                tma_4d_gmem2smem(vdst + vtoff, V, 0, 0, kv_head_idx_3, pg_v[pg_i_1], kv_src_full_addr + (stage) * 8);
                            }
                        }
                        int next_ni = ni_1 + 2;
                        if (next_ni < cta_n_blocks_3) {
                            ...
                            mbarrier_arrive_expect_tx(kv_src_full_addr + (k_stage) * 8, 16384);
                            ...
                            for (int pg_i_2 = 0; pg_i_2 < 2; pg_i_2++) {
                                ...
                                {
                                    tma_4d_gmem2smem(kdst + ntoff, K, 0, 0, kv_head_idx_3, pg_nk[pg_i_2], kv_src_full_addr + (k_stage) * 8);
                                }
                            }
                        }
                    }
    ...
    // ---- Role: transform ----
        ...
        asm volatile("setmaxnreg.inc.sync.aligned.u32 184;");
        { // transform_main
            unsigned int total_tiles_t = BATCH_SIZE * Q_LEN * NUM_KV_HEADS;
            #pragma unroll 1
            for (unsigned int tile_idx_t = blockIdx.x; tile_idx_t < total_tiles_t; tile_idx_t += gridDim.x) {
                int kv_head_idx_4 = tile_idx_t % (unsigned int)NUM_KV_HEADS;
                int request_row_4 = tile_idx_t / (unsigned int)NUM_KV_HEADS;
                ...
                int inst0_stage_t = 0;
                mbarrier_wait(kv_src_full_addr, 0);
                asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                {
                    const char* _src_ptr = smem_raw + (smem_kv_fp8_addr - smem);
                    char* _dst_ptr = smem_raw + (smem_kv_addr - smem);
                ...
                mbarrier_wait(kv_src_full_addr + 8, 0);
                asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                {
                    const char* _src_ptr = smem_raw + (smem_kv_fp8_addr + 16384 - smem);
                    char* _dst_ptr = smem_raw + (smem_kv_addr + 32768 - smem);
                    const int _tid = (int)threadIdx.x - (12) * 32;
                    uint64_t _src_buf[16];
                    #pragma unroll
                    for (int _outer = 0; _outer < 2; ++_outer) {
                ...
                #pragma unroll 1
                for (int pair_3 = 0; pair_3 < cta_n_blocks_4 / 2 - 1; pair_3++) {
                    int s0_2 = inst0_stage_t;
                    int s1_2 = (inst0_stage_t + 1) % NUM_KV_STAGES;
                    int s0_next_1 = (inst0_stage_t + 2) % NUM_KV_STAGES;
                    int s1_next_1 = (inst0_stage_t + 3) % NUM_KV_STAGES;
                    mbarrier_wait(kv_src_full_addr + (s0_2) * 8, 1);
                    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                    {
                        const char* _src_ptr = smem_raw + (smem_kv_fp8_addr + (unsigned int)(s0_2 * 16384) - smem);
                        char* _dst_ptr = smem_raw + (smem_kv_addr + (unsigned int)(s0_2 * 32768) - smem);
                        const int _tid = (int)threadIdx.x - (12) * 32;
                        uint64_t _src_buf[16];
                        #pragma unroll
                        for (int _outer = 0; _outer < 2; ++_outer) {
                    ...
                    if (warp == 12) {
                        if (elect_sync()) {
                            mbarrier_arrive(kv_full_addr + (s0_2) * 8);
                        }
                    }
                    mbarrier_wait(kv_src_full_addr + (s0_next_1) * 8, 0);
                    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                    {
                        const char* _src_ptr = smem_raw + (smem_kv_fp8_addr + (unsigned int)(s0_next_1 * 16384) - smem);
                        char* _dst_ptr = smem_raw + (smem_kv_addr + (unsigned int)(s0_next_1 * 32768) - smem);
                        const int _tid = (int)threadIdx.x - (12) * 32;
                        uint64_t _src_buf[16];
                        #pragma unroll
                        for (int _outer = 0; _outer < 2; ++_outer) {
                    ...
                    mbarrier_wait(kv_src_full_addr + (s1_2) * 8, 1);
                    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                    {
                        const char* _src_ptr = smem_raw + (smem_kv_fp8_addr + (unsigned int)(s1_2 * 16384) - smem);
                        char* _dst_ptr = smem_raw + (smem_kv_addr + (unsigned int)(s1_2 * 32768) - smem);
                        const int _tid = (int)threadIdx.x - (12) * 32;
                        uint64_t _src_buf[16];
                        #pragma unroll
                        for (int _outer = 0; _outer < 2; ++_outer) {
                    ...
                    if (warp == 12) {
                        if (elect_sync()) {
                            mbarrier_arrive(kv_full_addr + (s1_2) * 8);
                        }
                    }
                    mbarrier_wait(kv_src_full_addr + (s1_next_1) * 8, 0);
                    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                    {
                        const char* _src_ptr = smem_raw + (smem_kv_fp8_addr + (unsigned int)(s1_next_1 * 16384) - smem);
                        char* _dst_ptr = smem_raw + (smem_kv_addr + (unsigned int)(s1_next_1 * 32768) - smem);
                        const int _tid = (int)threadIdx.x - (12) * 32;
                        uint64_t _src_buf[16];
                        #pragma unroll
                        for (int _outer = 0; _outer < 2; ++_outer) {
                ...
                mbarrier_wait(kv_src_full_addr + (s0_last_t) * 8, 1);
                asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                {
                    const char* _src_ptr = smem_raw + (smem_kv_fp8_addr + (unsigned int)(s0_last_t * 16384) - smem);
                    char* _dst_ptr = smem_raw + (smem_kv_addr + (unsigned int)(s0_last_t * 32768) - smem);
                    const int _tid = (int)threadIdx.x - (12) * 32;
                    uint64_t _src_buf[16];
                    #pragma unroll
                    for (int _outer = 0; _outer < 2; ++_outer) {
                ...
                mbarrier_wait(kv_src_full_addr + (s1_last_t) * 8, 1);
                asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                {
                    const char* _src_ptr = smem_raw + (smem_kv_fp8_addr + (unsigned int)(s1_last_t * 16384) - smem);
                    char* _dst_ptr = smem_raw + (smem_kv_addr + (unsigned int)(s1_last_t * 32768) - smem);
                    const int _tid = (int)threadIdx.x - (12) * 32;
                    uint64_t _src_buf[16];
                    #pragma unroll
                    for (int _outer = 0; _outer < 2; ++_outer) {
