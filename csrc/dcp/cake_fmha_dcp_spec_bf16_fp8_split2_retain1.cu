...
__global__ __launch_bounds__(512, 1) void kernel_cake_fmha_dcp_spec_bf16_fp8(
    ...
    float softmax_scale_log2, float output_scale, int cp_rank, int num_q_heads, int num_kv_heads,
    int batch_size) {
...
#pragma unroll 1
      for (unsigned int tile_idx_s = blockIdx.x; tile_idx_s < total_tiles_s;
        ...
        row_max_pair0[0] = -CAKE_INF;
        row_max_pair0[1] = -CAKE_INF;
        row_sum_pair0[0] = 0.0f;
        row_sum_pair0[1] = 0.0f;
        row_max_pair1[0] = -CAKE_INF;
        row_max_pair1[1] = -CAKE_INF;
...
#pragma unroll
          for (int inst_s = 0; inst_s < 2; inst_s++) {
            ...
            int my_tmem_stats = taddr + (unsigned int)(((is_wg1 != 0) ? 48 : 16)) +
...
#pragma unroll
            for (int c_3 = 0; c_3 < 2; c_3++) {
              ...
              acc_scale_pair[c_3] = ((old_max_pair[c_3] > -CAKE_INF) ? _exp2_0 : 1.0f);
            }
            if (is_wg1 != 0) {
            ...
            asm volatile(
                ...
                " [%0], {%1, %2};" ::"r"(my_tmem_stats),
                "f"(acc_scale_pair[0]), "f"(acc_scale_pair[1])
                : "memory");
            float exp_vals[8];
...
#pragma unroll
            for (int c_4 = 0; c_4 < 8; c_4++) {
              float safe_max =
                  ((new_max_pair[c_4 % 2] == -CAKE_INF) ? 0.0f : new_max_pair[c_4 % 2]);
...
#pragma unroll
            ...
            int thr_row_idx = lane % 8;
            {
              ...
              const void* _stmatrix_b8_ptr_5 = reinterpret_cast<const void*>(
                  reinterpret_cast<uint8_t*>(((is_wg1 != 0) ? smem_p1 : smem_p0)) +
                  stsm_offset_fp8);
...
#pragma unroll
        for (int inst_s_1 = 0; inst_s_1 < 2; inst_s_1++) {
          ...
          if (wg_tid < 4) {
            my_corr_ptr[col_pair_base] =
                my_exch_ptr_1[col_pair_base] + my_exch_ptr_1[8 + col_pair_base] +
                my_exch_ptr_1[16 + col_pair_base] + my_exch_ptr_1[24 + col_pair_base];
            my_corr_ptr[col_pair_base + 1] =
                my_exch_ptr_1[col_pair_base + 1] + my_exch_ptr_1[8 + col_pair_base + 1] +
                my_exch_ptr_1[16 + col_pair_base + 1] + my_exch_ptr_1[24 + col_pair_base + 1];
...
#pragma unroll
        for (int h_6 = 0; h_6 < 8; h_6++) {
          ...
          if (group_ratio_rt > h_6) {
            ...
            {
              int partial_o_idx =
                  (((batch_idx_1 * Q_LEN + q_row_idx_1) * NUM_Q_HEADS + q_head) * NUM_SPLIT +
                   split_idx_1) *
                      HEAD_DIM +
                  d_idx;
              *(reinterpret_cast<__nv_bfloat16*>(partial_O_ptr + partial_o_idx) + (0)) =
          ...
          if (split_reduce_flag[0] != 0) {
            ...
            if (d_idx < group_ratio_rt) {
              ...
              float merged_lse = -CAKE_INF;
              {
                ...
                float weight0 = 0.0f;
                float weight1 = 0.0f;
                if (split_lse0 != -CAKE_INF) {
                  float _exp2_4 = approx_exp2(split_lse0 - merged_max);
                  weight0 = _exp2_4;
                }
                if (split_lse1 != -CAKE_INF) {
                  float _exp2_5 = approx_exp2(split_lse1 - merged_max);
                  weight1 = _exp2_5;
                }
            ...
            if (merge_head_in_group < group_ratio_rt) {
              ...
              int partial_o_base =
                  ((batch_idx_1 * Q_LEN + q_row_idx_1) * NUM_Q_HEADS + reduce_q_head_1) *
                      NUM_SPLIT * HEAD_DIM +
                  merge_d_base;
              int final_o_idx =
              ...
              }
              {
                float _vec_load_1[8];
                {
                  const uint4* _vptr_1 = reinterpret_cast<const uint4*>(
                      partial_O_ptr + (partial_o_base + HEAD_DIM) + 0);
                  uint4 _vld_1[1];
#pragma unroll
                  for (int _blk = 0; _blk < 1; _blk++) {
...
#pragma unroll 1
      for (unsigned int tile_idx_l = blockIdx.x; tile_idx_l < total_tiles_l;
        ...
        if (elect_sync()) {
          int off_qt =
              ((batch_idx_3 * Q_LEN + q_row_idx_3) * NUM_KV_HEADS + kv_head_idx_3) * TILE_Q;
          {
            const int group_ratio_l = NUM_Q_HEADS / NUM_KV_HEADS;
            off_qt =
                (batch_idx_3 * Q_LEN + q_row_idx_3) * NUM_Q_HEADS + kv_head_idx_3 * group_ratio_l;
          }
...
#pragma unroll
            for (int pg_i = 0; pg_i < 2; pg_i++) {
              int toff = pg_i * PAGE_SIZE * HEAD_DIM;
              {
                {
                  asm volatile(
                      "cp.async.bulk.tensor.4d.shared::cta.global.mbarrier::complete_tx::bytes.L2::"
                      "cache_hint"
                      " [%0], [%1, {%2, %3, %4, %5}], [%6], %7;" ::"r"(ldst + toff),
                      "l"(K), "r"(0), "r"(0), "r"(kv_head_idx_3), "r"(pg_k[pg_i]),
              ...
              {
                {
                  asm volatile(
                      "cp.async.bulk.tensor.4d.shared::cta.global.mbarrier::complete_tx::bytes.L2::"
                      "cache_hint"
                      " [%0], [%1, {%2, %3, %4, %5}], [%6], %7;" ::"r"(vdst + vtoff),
                      "l"(V), "r"(0), "r"(0), "r"(kv_head_idx_3), "r"(pg_v[pg_i_1]),
                ...
                {
                  {
                    asm volatile(
                        ...
                        "L2::cache_hint"
                        " [%0], [%1, {%2, %3, %4, %5}], [%6], %7;" ::"r"(kdst + ntoff),
                        "l"(K), "r"(0), "r"(0), "r"(kv_head_idx_3), "r"(pg_nk[pg_i_2]),
...
#pragma unroll 1
      for (unsigned int direct_tile_idx_t = blockIdx.x; direct_tile_idx_t < direct_tiles_t;
           direct_tile_idx_t += gridDim.x) {
...
#pragma unroll
        for (int q_xform_row = 0; q_xform_row < TILE_Q; q_xform_row++) {
          ...
          {
            uint16_t _fp8_pair_32704112;
            asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;"
                : "=h"(_fp8_pair_32704112)
                : "f"(0.0f), "f"(q_value));
            uint32_t _byte_32704112 = (uint32_t)(_fp8_pair_32704112 & 0xFF);
            uint32_t _addr_32704112 = static_cast<uint32_t>(
                (smem_qt_addr + (unsigned int)(q_xform_row * 128 + q_xform_tid ^
                                               (q_xform_row * 128 + q_xform_tid >> 7 & 7) << 4)));
            asm volatile("st.shared.u8 [%0], %1;" ::"r"(_addr_32704112), "r"(_byte_32704112)
                         : "memory");
          }
        }
        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
...
#pragma unroll 1
      for (unsigned int tile_idx_t = blockIdx.x; tile_idx_t < total_tiles_t;
           tile_idx_t += gridDim.x) {
...
#pragma unroll 1
        for (int pair_3 = 0; pair_3 < num_pairs_4 - 1; pair_3++) {
          ...
          {
            const char* _src_ptr =
                smem_raw + (smem_kv_transform_src_addr + (unsigned int)(s0_1 * 16384) - smem);
            char* _dst_ptr =
                smem_raw + (smem_kv_transform_dst_addr + (unsigned int)(s0_1 * 32768) - smem);
            const int _tid = (int)threadIdx.x - (12) * 32;
            uint64_t _src_buf[16];
#pragma unroll
            for (int _outer = 0; _outer < 2; ++_outer) {
...
#pragma unroll
                ...
                uint4 _dst4 =
                    make_uint4(_out_x16x2[0], _out_x16x2[1], _out_x16x2[2], _out_x16x2[3]);
                int _elt = _off * 8;
                int _row = (((_elt % 128) / 64) * 128) + (_elt / 128);
                int _byte_off = (_row * 128) + (((_elt % 64) * 16) / 8);
                int _swz_off = _byte_off ^ ((_row % 8) * 16);
          ...
          {
            const char* _src_ptr =
                smem_raw + (smem_kv_transform_src_addr + (unsigned int)(s0_next_1 * 16384) - smem);
            char* _dst_ptr =
                smem_raw + (smem_kv_transform_dst_addr + (unsigned int)(s0_next_1 * 32768) - smem);
            const int _tid = (int)threadIdx.x - (12) * 32;
            uint64_t _src_buf[16];
#pragma unroll
            for (int _outer = 0; _outer < 2; ++_outer) {
...
#pragma unroll
          ...
          {
            const char* _src_ptr =
                smem_raw + (smem_kv_transform_src_addr + (unsigned int)(s1_1 * 16384) - smem);
            char* _dst_ptr =
                smem_raw + (smem_kv_transform_dst_addr + (unsigned int)(s1_1 * 32768) - smem);
            const int _tid = (int)threadIdx.x - (12) * 32;
            uint64_t _src_buf[16];
#pragma unroll
            for (int _outer = 0; _outer < 2; ++_outer) {
...
#pragma unroll
          ...
          {
            const char* _src_ptr =
                smem_raw + (smem_kv_transform_src_addr + (unsigned int)(s1_next_1 * 16384) - smem);
            char* _dst_ptr =
                smem_raw + (smem_kv_transform_dst_addr + (unsigned int)(s1_next_1 * 32768) - smem);
            const int _tid = (int)threadIdx.x - (12) * 32;
            uint64_t _src_buf[16];
#pragma unroll
            for (int _outer = 0; _outer < 2; ++_outer) {
...
#pragma unroll
        ...
        {
          const char* _src_ptr =
              smem_raw + (smem_kv_transform_src_addr + (unsigned int)(s0_last_t * 16384) - smem);
          char* _dst_ptr =
              smem_raw + (smem_kv_transform_dst_addr + (unsigned int)(s0_last_t * 32768) - smem);
          const int _tid = (int)threadIdx.x - (12) * 32;
          uint64_t _src_buf[16];
#pragma unroll
          for (int _outer = 0; _outer < 2; ++_outer) {
...
#pragma unroll
        ...
        {
          const char* _src_ptr =
              smem_raw + (smem_kv_transform_src_addr + (unsigned int)(s1_last_t * 16384) - smem);
          char* _dst_ptr =
              smem_raw + (smem_kv_transform_dst_addr + (unsigned int)(s1_last_t * 32768) - smem);
          const int _tid = (int)threadIdx.x - (12) * 32;
          uint64_t _src_buf[16];
#pragma unroll
          for (int _outer = 0; _outer < 2; ++_outer) {
