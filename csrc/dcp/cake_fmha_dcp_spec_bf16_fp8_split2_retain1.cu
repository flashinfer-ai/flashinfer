...
#pragma unroll
            for (int c_4 = 0; c_4 < 8; c_4++) {
              float safe_max =
                  ((new_max_pair[c_4 % 2] == -LOOM_INF) ? 0.0f : new_max_pair[c_4 % 2]);
            ...
            int thr_row_idx = lane % 8;
            {
              unsigned int regs_p_fp8[2];
              {
                ...
                asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;"
                    : "=h"(_lo)
                    : "f"(exp_vals[1]), "f"(exp_vals[0]));
                asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;"
                    : "=h"(_hi)
                    : "f"(exp_vals[3]), "f"(exp_vals[2]));
              ...
              }
              {
                ...
                asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;"
                    : "=h"(_lo)
                    : "f"(exp_vals[5]), "f"(exp_vals[4]));
                asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;"
                    : "=h"(_hi)
                    : "f"(exp_vals[7]), "f"(exp_vals[6]));
