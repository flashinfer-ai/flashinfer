...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_fmha_dcp_spec_bf16'.
#include <cuda.h>
...
void Run(TensorView arg_Qt, TensorView arg_K, TensorView arg_V, TensorView arg_O_ptr, TensorView arg_LSE_ptr, TensorView arg_page_table, TensorView arg_causal_seqlens_kv_global, int64_t arg_max_pages_per_seq, int64_t arg_max_local_seq_len, double arg_softmax_scale_log2, int64_t arg_cp_rank, int64_t arg_num_q_heads, int64_t arg_num_kv_heads, int64_t arg_batch_size, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  void* kargs[] = {&p_Qt, &p_K, &p_V, &p_O_ptr, &p_LSE_ptr, &p_page_table, &p_causal_seqlens_kv_global, &v_max_pages_per_seq, &v_max_local_seq_len, &v_softmax_scale_log2, &v_cp_rank, &v_num_q_heads, &v_num_kv_heads, &v_batch_size};
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_fmha_dcp_spec_bf16 failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernel(
      reinterpret_cast<const void*>(kernel_cake_fmha_dcp_spec_bf16), grid, block, kargs, 145408u, stream);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernel for kernel_cake_fmha_dcp_spec_bf16 failed: "
