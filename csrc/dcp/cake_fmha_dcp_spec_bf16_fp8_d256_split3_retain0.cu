...
#define tmem_dealloc_addr (mbar_base + 184)
  ...
  if (warp <= 7) {
    ...
    {  // softmax_main
      ...
      int my_tmem_s = taddr + (unsigned int)(((is_stream1 != 0) ? 16 : 0)) +
                      (unsigned int)(tmem_row_base << 16);
      int my_tmem_stats = taddr + (unsigned int)(((is_stream1 != 0) ? 64 : 32)) +
                          (unsigned int)(tmem_row_base << 16);
...
#pragma unroll
        ...
        if (lane < 16) {
          my_total_sum = my_exch_ptr[lane] + my_exch_ptr[16 + lane] + my_exch_ptr[32 + lane] +
                         my_exch_ptr[48 + lane];
        }
        if (is_stream1 != 0) {
          asm volatile("barrier.sync 9, 128;" ::: "memory");
...
#pragma unroll
          for (int q_head = 0; q_head < TILE_Q; q_head++) {
            ...
            asm volatile(
                "ld.shared.b32 %0, [%1];"
                : "=r"(*reinterpret_cast<uint32_t*>(&q_hi_packed[0]))
                : "r"((smem_q_raw_hi_addr +
                       (unsigned int)(q_raw_row * 128 + q_group_col * 2 ^
                                      (q_raw_row * 128 + q_group_col * 2 >> 7 & 7) << 4))));
...
#pragma unroll
          for (int q_head_1 = 0; q_head_1 < TILE_Q; q_head_1++) {
            ...
            asm volatile(
                "ld.shared.b32 %0, [%1];"
                : "=r"(*reinterpret_cast<uint32_t*>(&q_lo_packed[0]))
                : "r"((smem_q_raw_lo_addr +
                       (unsigned int)(q_raw_row_1 * 128 + q_group_col_1 * 2 ^
                                      (q_raw_row_1 * 128 + q_group_col_1 * 2 >> 7 & 7) << 4))));
...
#pragma unroll
        for (int head_base = 0; head_base < 16; head_base += 8) {
          ...
          tmem_ld_x8(&_tmem_load_7[0],
                     taddr + 96 + (unsigned int)head_base + (unsigned int)corr_row);
          asm volatile("tcgen05.wait::ld.sync.aligned;");
          float _tmem_load_8[8];
          tmem_ld_x8(&_tmem_load_8[0],
                     taddr + 128 + (unsigned int)head_base + (unsigned int)corr_row);
          asm volatile("tcgen05.wait::ld.sync.aligned;");
#pragma unroll
          for (int head_lane = 0; head_lane < 8; head_lane++) {
            ...
            if (head_valid != 0) {
              final_o_hi =
                  (_tmem_load_7[head_lane] * head_scale0 + _tmem_load_8[head_lane] * head_scale1) *
                  head_inv_sum * output_scale;
            }
            if (h_8 < group_ratio_rt) {
              int q_head_2 = kv_head_idx_1 * group_ratio_rt + h_8;
              {
                int partial_o_idx =
                    (((batch_idx_1 * Q_LEN + q_row_idx_1) * NUM_Q_HEADS + q_head_2) * NUM_SPLIT +
                     split_idx_1) *
                        HEAD_DIM +
                    d_idx;
                *(reinterpret_cast<__nv_bfloat16*>(partial_O_ptr + partial_o_idx) + (0)) =
                    __float2bfloat16_rn(final_o_hi);
              }
            }
          }
          float _tmem_load_9[8];
          tmem_ld_x8(&_tmem_load_9[0],
                     taddr + 112 + (unsigned int)head_base + (unsigned int)corr_row);
          asm volatile("tcgen05.wait::ld.sync.aligned;");
          float _tmem_load_10[8];
          tmem_ld_x8(&_tmem_load_10[0],
                     taddr + 144 + (unsigned int)head_base + (unsigned int)corr_row);
          asm volatile("tcgen05.wait::ld.sync.aligned;");
#pragma unroll
          for (int head_lane_1 = 0; head_lane_1 < 8; head_lane_1++) {
            ...
            if (head_valid_1 != 0) {
              final_o_lo = (_tmem_load_9[head_lane_1] * head_scale0_1 +
                            _tmem_load_10[head_lane_1] * head_scale1_1) *
                           head_inv_sum_1 * output_scale;
            }
            if (h_9 < group_ratio_rt) {
              int q_head_3 = kv_head_idx_1 * group_ratio_rt + h_9;
              {
                int partial_o_idx_1 =
                    (((batch_idx_1 * Q_LEN + q_row_idx_1) * NUM_Q_HEADS + q_head_3) * NUM_SPLIT +
                     split_idx_1) *
                        HEAD_DIM +
                    HEAD_DIM_HALF + d_idx;
                *(reinterpret_cast<__nv_bfloat16*>(partial_O_ptr + partial_o_idx_1) + (0)) =
                    __float2bfloat16_rn(final_o_lo);
              }
            }
          }
        }
...
#pragma unroll
            for (int source_lane_1 = 0; source_lane_1 < 8; source_lane_1++) {
              float _shfl_11 =
                  __shfl_sync(0xFFFFFFFF, lane_weight_sum, subgroup_lane_base + source_lane_1);
...
#pragma unroll
              for (int vec_chunk = 0; vec_chunk < 4; vec_chunk++) {
                ...
                int partial_o_base =
                    ((batch_idx_1 * Q_LEN + q_row_idx_1) * NUM_Q_HEADS + reduce_q_head_0) *
                        NUM_SPLIT * HEAD_DIM +
                    elem_base;
                int final_o_idx =
                    ((batch_idx_1 * Q_LEN + q_row_idx_1) * NUM_Q_HEADS + reduce_q_head_0) *
                ...
                for (int reduce_split = 1; reduce_split < NUM_SPLIT; reduce_split++) {
                  float _vec_load_1[8];
                  {
                    const uint4* _vptr_1 = reinterpret_cast<const uint4*>(
                        partial_O_ptr + (partial_o_base + reduce_split * HEAD_DIM) + 0);
                    uint4 _vld_1[1];
#pragma unroll
                    for (int _blk = 0; _blk < 1; _blk++) {
...
#pragma unroll
                  for (int elem_1 = 0; elem_1 < 8; elem_1++) {
                    float _fma_0 =
                        __fmaf_rn(_vec_load_1[elem_1], reduce_weight, _vec_load_0[elem_1]);
...
#pragma unroll 1
        for (int pair_2 = 0; pair_2 < num_pairs_2 - 1; pair_2++) {
          ...
          asm volatile("tcgen05.fence::after_thread_sync;");
          int _mma_a_lo_4 =
              make_warp_uniform(((((smem_v_hi_addr) >> 4) & 0x3FFF) | 0x4000000) + (s0) * 1024);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_4),
              "r"(_mma_b_lo_4), "r"(tmem_tmem_o0_hi), "r"(((first_pv0) ? 0 : 1)));
          int _mma_a_lo_5 =
              make_warp_uniform(((((smem_v_lo_addr) >> 4) & 0x3FFF) | 0x4000000) + (s0) * 1024);
          asm volatile(
              "{\n\t"
              ".reg .pred leader, p0, p1;\n\t"
              ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_5),
              "r"(_mma_b_lo_4), "r"(tmem_tmem_o0_lo), "r"(((first_pv0) ? 0 : 1)));
          elect_commit2(kv_empty_addr + (s0) * 8, o_ready_0_addr);
          mbarrier_wait(kv_full_addr + (s0_next) * 8, 0);
          int _mma_a_lo_6 =
              make_warp_uniform((((smem_kv_hi_addr) >> 4) & 0x3FFF) + (s0_next) * 1024);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_6),
              "r"(_mma_b_lo_6), "r"(tmem_tmem_s0), "r"(0));
          int _mma_a_lo_7 =
              make_warp_uniform((((smem_kv_lo_addr) >> 4) & 0x3FFF) + (s0_next) * 1024);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_7),
              "r"(_mma_b_lo_7), "r"(tmem_tmem_s0), "r"(1));
          elect_commit(s_full_0_addr);
          ...
          asm volatile("tcgen05.fence::after_thread_sync;");
          int _mma_a_lo_8 =
              make_warp_uniform(((((smem_v_hi_addr) >> 4) & 0x3FFF) | 0x4000000) + (s1) * 1024);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_8),
              "r"(_mma_b_lo_8), "r"(tmem_tmem_o1_hi), "r"(((first_pv1) ? 0 : 1)));
          int _mma_a_lo_9 =
              make_warp_uniform(((((smem_v_lo_addr) >> 4) & 0x3FFF) | 0x4000000) + (s1) * 1024);
          asm volatile(
              "{\n\t"
              ".reg .pred leader, p0, p1;\n\t"
              ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_9),
              "r"(_mma_b_lo_8), "r"(tmem_tmem_o1_lo), "r"(((first_pv1) ? 0 : 1)));
          elect_commit2(kv_empty_addr + (s1) * 8, o_ready_1_addr);
          mbarrier_wait(kv_full_addr + (s1_next) * 8, 0);
          int _mma_a_lo_10 =
              make_warp_uniform((((smem_kv_hi_addr) >> 4) & 0x3FFF) + (s1_next) * 1024);
          asm volatile(
              "{\n\t"
              ".reg .pred leader, p0, p1;\n\t"
              ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_10),
              "r"(_mma_b_lo_6), "r"(tmem_tmem_s1), "r"(0));
          int _mma_a_lo_11 =
              make_warp_uniform((((smem_kv_lo_addr) >> 4) & 0x3FFF) + (s1_next) * 1024);
          asm volatile(
              "{\n\t"
              ".reg .pred leader, p0, p1;\n\t"
              ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
        ...
        asm volatile("tcgen05.fence::after_thread_sync;");
        int _mma_a_lo_12 =
            make_warp_uniform(((((smem_v_hi_addr) >> 4) & 0x3FFF) | 0x4000000) + (s0_last) * 1024);
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_12),
            "r"(_mma_b_lo_12), "r"(tmem_tmem_o0_hi), "r"(((first_pv0) ? 0 : 1)));
        int _mma_a_lo_13 =
            make_warp_uniform(((((smem_v_lo_addr) >> 4) & 0x3FFF) | 0x4000000) + (s0_last) * 1024);
        asm volatile(
            "{\n\t"
            ".reg .pred leader, p0, p1;\n\t"
            ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_13),
            "r"(_mma_b_lo_12), "r"(tmem_tmem_o0_lo), "r"(((first_pv0) ? 0 : 1)));
        ...
        asm volatile("tcgen05.fence::after_thread_sync;");
        int _mma_a_lo_14 =
            make_warp_uniform(((((smem_v_hi_addr) >> 4) & 0x3FFF) | 0x4000000) + (s1_last) * 1024);
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_14),
            "r"(_mma_b_lo_14), "r"(tmem_tmem_o1_hi), "r"(((first_pv1) ? 0 : 1)));
        int _mma_a_lo_15 =
            make_warp_uniform(((((smem_v_lo_addr) >> 4) & 0x3FFF) | 0x4000000) + (s1_last) * 1024);
        asm volatile(
            "{\n\t"
            ".reg .pred leader, p0, p1;\n\t"
            ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_15),
            "r"(_mma_b_lo_14), "r"(tmem_tmem_o1_lo), "r"(((first_pv1) ? 0 : 1)));
...
#pragma unroll
            for (int pg_i = 0; pg_i < 2; pg_i++) {
              ...
              {
                tma_5d_gmem2smem(hdst + toff, K, 0, 0, 0, kv_head_idx_3, pg_k[pg_i],
                                 kv_full_addr + (kv_stage) * 8);
                tma_5d_gmem2smem(ldst + toff, K, 0, 0, 1, kv_head_idx_3, pg_k[pg_i],
                                 kv_full_addr + (kv_stage) * 8);
              }
            }
            kv_stage += 1;
...
#pragma unroll
            for (int pg_i_1 = 0; pg_i_1 < 2; pg_i_1++) {
              ...
              {
                tma_5d_gmem2smem(vhdst + vtoff, V, 0, 0, 0, kv_head_idx_3, pg_v[pg_i_1],
                                 kv_full_addr + (stage) * 8);
                tma_5d_gmem2smem(vldst + vtoff, V, 0, 0, 1, kv_head_idx_3, pg_v[pg_i_1],
                                 kv_full_addr + (stage) * 8);
              }
            }
            int next_ni = ni_1 + 2;
...
#pragma unroll
              for (int pg_i_2 = 0; pg_i_2 < 2; pg_i_2++) {
                ...
                {
                  tma_5d_gmem2smem(khdst + ntoff, K, 0, 0, 0, kv_head_idx_3, pg_nk[pg_i_2],
                                   kv_full_addr + (k_stage) * 8);
                  tma_5d_gmem2smem(kldst + ntoff, K, 0, 0, 1, kv_head_idx_3, pg_nk[pg_i_2],
                                   kv_full_addr + (k_stage) * 8);
                }
              }
            }
