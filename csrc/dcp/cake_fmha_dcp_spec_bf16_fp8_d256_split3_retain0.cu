                    ...
                    for (int h_3 = 0; h_3 < 16; h_3++) {
                        ...
                        }
                        {
                            ...
                            asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;"
                                : "=h"(_fp8_pair_3493672032) : "f"(0.0f), "f"(exp_value));
                            uint32_t _byte_3493672032 = (uint32_t)(_fp8_pair_3493672032 & 0xFF);
                            const void* _ptr_3493672032 = reinterpret_cast<const void*>((reinterpret_cast<uint8_t*>(my_p_base) + (h_3 * 128 + wg_tid ^ (h_3 * 128 + wg_tid >> 7 & 7) << 4)));
                            uint64_t _addr64_3493672032;
                            asm volatile("cvta.to.shared.u64 %0, %1;" : "=l"(_addr64_3493672032) : "l"(_ptr_3493672032));
                            uint32_t _addr_3493672032;
                            asm volatile("cvt.u32.u64 %0, %1;" : "=r"(_addr_3493672032) : "l"(_addr64_3493672032));
                            asm volatile("st.shared.u8 [%0], %1;" :: "r"(_addr_3493672032), "r"(_byte_3493672032) : "memory");
                        }
                    }
                        ...
                        for (int pair_elem = 0; pair_elem < 2; pair_elem++) {
                            {
                                ...
                                asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;"
                                    : "=h"(_fp8_pair_3494181136) : "f"(0.0f), "f"(q_hi_packed_f32[pair_elem]));
                                uint32_t _byte_3494181136 = (uint32_t)(_fp8_pair_3494181136 & 0xFF);
                                uint32_t _addr_3494181136 = static_cast<uint32_t>((smem_q_hi_addr + (unsigned int)(q_head * 128 + (q_pair_col + pair_elem) ^ (q_head * 128 + (q_pair_col + pair_elem) >> 7 & 7) << 4)));
                                asm volatile("st.shared.u8 [%0], %1;" :: "r"(_addr_3494181136), "r"(_byte_3494181136) : "memory");
                            }
                        }
                    }
                } else {
                        ...
                        for (int pair_elem_1 = 0; pair_elem_1 < 2; pair_elem_1++) {
                            {
                                ...
                                asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;"
                                    : "=h"(_fp8_pair_3493620864) : "f"(0.0f), "f"(q_lo_packed_f32[pair_elem_1]));
                                uint32_t _byte_3493620864 = (uint32_t)(_fp8_pair_3493620864 & 0xFF);
                                uint32_t _addr_3493620864 = static_cast<uint32_t>((smem_q_lo_addr + (unsigned int)(q_head_1 * 128 + (q_pair_col_1 + pair_elem_1) ^ (q_head_1 * 128 + (q_pair_col_1 + pair_elem_1) >> 7 & 7) << 4)));
                                asm volatile("st.shared.u8 [%0], %1;" :: "r"(_addr_3493620864), "r"(_byte_3493620864) : "memory");
                            }
                        }
                    }
                }
                        ...
                        if (merge_head_in_group < group_ratio_rt) {
                            ...
                            for (int vec_chunk = 0; vec_chunk < 4; vec_chunk++) {
                                ...
                                for (int reduce_split = 1; reduce_split < NUM_SPLIT; reduce_split++) {
                                    float _vec_load_1[8];
                                    {
                                        const uint4* _vptr_1 = reinterpret_cast<const uint4*>(partial_O_ptr + (partial_o_base + reduce_split * HEAD_DIM) + 0);
                                        uint4 _vld_1[1];
                                        #pragma unroll
                                        for (int _blk = 0; _blk < 1; _blk++) {
