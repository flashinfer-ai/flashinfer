...
#pragma unroll
                for (int elem = 0; elem < 8; elem++) {
                  _vec_load_0[elem] = _vec_load_0[elem] * reduce_weight0;
                }
#pragma unroll 2
...
#pragma unroll
                  ...
                  float reduce_weight =
                      split_weights[reduce_split_3 * TILE_Q + merge_head_in_group];
#pragma unroll
                  for (int elem_1 = 0; elem_1 < 8; elem_1++) {
                    float _fma_0 =
                        __fmaf_rn(_vec_load_2[elem_1], reduce_weight, _vec_load_0[elem_1]);
...
#pragma unroll
            for (int pg_i = 0; pg_i < 2; pg_i++) {
              ...
              {
                {
                  tma_4d_gmem2smem(ldst + toff, K, 0, 0, kv_head_idx_3, pg_k[pg_i],
...
#pragma unroll
            for (int pg_i_1 = 0; pg_i_1 < 2; pg_i_1++) {
              ...
              {
                {
                  tma_4d_gmem2smem(vdst + vtoff, V, 0, 0, kv_head_idx_3, pg_v[pg_i_1],
...
#pragma unroll
              for (int pg_i_2 = 0; pg_i_2 < 2; pg_i_2++) {
                ...
                {
                  {
                    tma_4d_gmem2smem(kdst + ntoff, K, 0, 0, kv_head_idx_3, pg_nk[pg_i_2],
