...
void launchHopperF8MHAFlashInfer(uint32_t multiProcessorCount, uint32_t nbKHeads, uint32_t slidingWinSize,
    float qScale, OutputHead* output,
#if LOW_PREC_OUTPUT
    float const* rcpOutScale,
#endif
    ...
    uint32_t* semaphores, void* scratch, cudaStream_t stream)
    {
        ...
        uint32_t const nbSubSeqPerSeq = [&]() -> uint32_t
        {
            float const factor = 0.25f;
            return mha::min<uint32_t>(
                mha::max<uint32_t>(1U, (uint32_t) round(multiProcessorCount * 3 / (batchSize * nbKHeads) * factor)),
                divUp(maxSeqLen, gemm0CtaTileNbTokens));
        }();
    #if SPEC_DEC
        ...
        cudaError_t const err = cudaLaunchKernelEx(&launchCfg, kernel_mha, nbKHeads,
            ...
            rcpOutScale,
    #endif
            q, attentionSinks, cacheList, batchSize, kvCacheScale, tensorMap, semaphores, scratch);
    #endif
        checkCuda(err);
    }
#endif
