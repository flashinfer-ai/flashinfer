...
CubinFlashAttention::CubinFlashAttention(const std::string& cubin_path)
    : module_(nullptr), kernel_nl_tiled_(nullptr), kernel_nl_tiled_causal_(nullptr), initialized_(false)
{
    ...
    result = cuModuleGetFunction(&kernel_nl_tiled_, module_, 
    ...
    result = cuModuleGetFunction(&kernel_nl_tiled_causal_, module_, 
        "fmha_v2_flash_attention_e4m3_fp32_64_64_S_q_k_v_192x128_causal_output_bf16_sm120_kernel_nl_tiled");
    if (result != CUDA_SUCCESS)
    {
        cuModuleUnload(module_);
    ...
    cudaStream_t stream)
{
    ...
    CUfunction kernel_to_launch = (launch_params.attention_mask_type == fmha::Attention_mask_type::CAUSAL) 
        ? kernel_nl_tiled_causal_ : kernel_nl_tiled_;
