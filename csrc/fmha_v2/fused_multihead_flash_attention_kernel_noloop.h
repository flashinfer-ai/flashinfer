...
#pragma unroll
  ...
  enum {
    CHECK_NEG_INF = Kernel_traits::BIDIRECTIONAL_SLIDING_WINDOW_ATTENTION ||
                    Kernel_traits::SLIDING_WINDOW_ATTENTION || Kernel_traits::CUSTOM_MASK
  };
  ...
  for (int kv_loop = kv_loop_start; kv_loop < kv_loop_end; kv_loop += Cta_tile_p::N) {
    ...
    bool const apply_sliding_window_mask =
        (mask_sliding_window &&
         (kv_loop <= sliding_window_mask_left || kv_loop >= sliding_window_mask_right));
