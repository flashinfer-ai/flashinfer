...
#define SMEM_EPI_STAGING_STAGE_BYTES 24576
#define SMEM_EPI_STAGING_STRIDE 24576
#define SMEM_SMEM_ASCALE_OFF 222208
#define SMEM_SMEM_ASCALE_STAGE_BYTES 6144
#define SMEM_SMEM_ASCALE_STRIDE 6144
#define SMEM_SMEM_BSCALE_OFF 228352
...
kernel_cake_grouped_fp8_gemm_840c9c693e988f7d33a6(CakeTensorMap const* A, CakeTensorMap const* B, CakeTensorMap const* C_tma, __nv_bfloat16* __restrict__ C, float* __restrict__ a_scale, float* __restrict__ b_scale, int* __restrict__ m_indices, int M, int N, int K, int G)
{
    ...
    #define scale_full_addr (mbar_base + 96)
    ...
    float* smem_ascale = reinterpret_cast<float*>(smem_raw + 222208);
    const int smem_ascale_addr = smem + 222208;
    float* smem_bscale = reinterpret_cast<float*>(smem_raw + 228352);
    const int smem_bscale_addr = smem + 228352;
    ...
    if (warp == 0) {
        ...
        if (leader) {
            ...
            // partial_free: 2 barriers, init_count=8
                ...
                for (int segment_iter = 0; segment_iter < tile_rows; segment_iter++) {
                    ...
                    for (int scale_group = 0; scale_group < scale_groups; scale_group++) {
                        ...
                        for (int k_inner = 0; k_inner < 4; k_inner++) {
                            ...
                            float b_s = smem_bscale[(scale_stage * 2 + (unsigned int)acc_wg) * 4 + (unsigned int)k_inner];
                    ...
                    if (homogeneous != 0) {
                        ...
                        unsigned int epi_a_addr_0 = epi_staging_addr + epi_panel_cursor * 8192;
                        unsigned int epi_b_addr_0 = epi_staging_addr + epi_other_0 * 8192;
                        ...
                        if (acc_wg == 0) {
                            ...
                            for (int chunk = 0; chunk < 4; chunk++) {
                                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"((epi_a_addr_0 + (unsigned int)(row * 64 + chunk * 16 ^ (row * 64 + chunk * 16 >> 7 & 3) << 4))), "r"(acc0_bf16[chunk * 4]), "r"(acc0_bf16[chunk * 4 + 1]), "r"(acc0_bf16[chunk * 4 + 2]), "r"(acc0_bf16[chunk * 4 + 3]) : "memory");
                            }
                        }
                        if (acc_wg == 1) {
                            #pragma unroll
                            for (int chunk_1 = 0; chunk_1 < 4; chunk_1++) {
                                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"((epi_b_addr_0 + (unsigned int)(row * 64 + chunk_1 * 16 ^ (row * 64 + chunk_1 * 16 >> 7 & 3) << 4))), "r"(acc0_bf16[chunk_1 * 4]), "r"(acc0_bf16[chunk_1 * 4 + 1]), "r"(acc0_bf16[chunk_1 * 4 + 2]), "r"(acc0_bf16[chunk_1 * 4 + 3]) : "memory");
                            }
                        }
                        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                        asm volatile("barrier.sync 8, 256;" ::: "memory");
                        ...
                        unsigned int epi_a_addr_1 = epi_staging_addr + epi_panel_cursor * 8192;
                        unsigned int epi_b_addr_1 = epi_staging_addr + epi_other_1 * 8192;
                        ...
                        if (acc_wg == 0) {
                            ...
                            for (int chunk_2 = 0; chunk_2 < 4; chunk_2++) {
                                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"((epi_a_addr_1 + (unsigned int)(row * 64 + chunk_2 * 16 ^ (row * 64 + chunk_2 * 16 >> 7 & 3) << 4))), "r"(acc1_bf16[chunk_2 * 4]), "r"(acc1_bf16[chunk_2 * 4 + 1]), "r"(acc1_bf16[chunk_2 * 4 + 2]), "r"(acc1_bf16[chunk_2 * 4 + 3]) : "memory");
                            }
                        }
                        if (acc_wg == 1) {
                            #pragma unroll
                            for (int chunk_3 = 0; chunk_3 < 4; chunk_3++) {
                                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"((epi_b_addr_1 + (unsigned int)(row * 64 + chunk_3 * 16 ^ (row * 64 + chunk_3 * 16 >> 7 & 3) << 4))), "r"(acc1_bf16[chunk_3 * 4]), "r"(acc1_bf16[chunk_3 * 4 + 1]), "r"(acc1_bf16[chunk_3 * 4 + 2]), "r"(acc1_bf16[chunk_3 * 4 + 3]) : "memory");
                            }
                        }
                        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                        asm volatile("barrier.sync 8, 256;" ::: "memory");
                        if (warp == 0) {
                            if (elect_sync()) {
                                tma_store_2d(C_tma, n_tile * 256 + 32, m_tile * 128, epi_a_addr_1);
                                asm volatile("cp.async.bulk.commit_group;");
                                tma_store_2d(C_tma, n_tile * 256 + 160, m_tile * 128, epi_b_addr_1);
                                asm volatile("cp.async.bulk.commit_group;");
                            }
                        }
                        epi_panel_cursor += 2;
                        ...
                        asm volatile("barrier.sync 8, 256;" ::: "memory");
                        unsigned int epi_other_2 = epi_panel_cursor + 1;
                        if (epi_other_2 == 3) {
                            epi_other_2 = 0;
                        }
                        unsigned int epi_a_addr_2 = epi_staging_addr + epi_panel_cursor * 8192;
                        unsigned int epi_b_addr_2 = epi_staging_addr + epi_other_2 * 8192;
                        ...
                        if (acc_wg == 0) {
                            ...
                            for (int chunk_4 = 0; chunk_4 < 4; chunk_4++) {
                                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"((epi_a_addr_2 + (unsigned int)(row * 64 + chunk_4 * 16 ^ (row * 64 + chunk_4 * 16 >> 7 & 3) << 4))), "r"(acc2_bf16[chunk_4 * 4]), "r"(acc2_bf16[chunk_4 * 4 + 1]), "r"(acc2_bf16[chunk_4 * 4 + 2]), "r"(acc2_bf16[chunk_4 * 4 + 3]) : "memory");
                            }
                        }
                        if (acc_wg == 1) {
                            #pragma unroll
                            for (int chunk_5 = 0; chunk_5 < 4; chunk_5++) {
                                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"((epi_b_addr_2 + (unsigned int)(row * 64 + chunk_5 * 16 ^ (row * 64 + chunk_5 * 16 >> 7 & 3) << 4))), "r"(acc2_bf16[chunk_5 * 4]), "r"(acc2_bf16[chunk_5 * 4 + 1]), "r"(acc2_bf16[chunk_5 * 4 + 2]), "r"(acc2_bf16[chunk_5 * 4 + 3]) : "memory");
                            }
                        }
                        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                        asm volatile("barrier.sync 8, 256;" ::: "memory");
                        if (warp == 0) {
                            if (elect_sync()) {
                                tma_store_2d(C_tma, n_tile * 256 + 64, m_tile * 128, epi_a_addr_2);
                                asm volatile("cp.async.bulk.commit_group;");
                                tma_store_2d(C_tma, n_tile * 256 + 192, m_tile * 128, epi_b_addr_2);
                                asm volatile("cp.async.bulk.commit_group;");
                            }
                        }
                        epi_panel_cursor += 2;
                        ...
                        asm volatile("barrier.sync 8, 256;" ::: "memory");
                        unsigned int epi_other_3 = epi_panel_cursor + 1;
                        if (epi_other_3 == 3) {
                            epi_other_3 = 0;
                        }
                        unsigned int epi_a_addr_3 = epi_staging_addr + epi_panel_cursor * 8192;
                        unsigned int epi_b_addr_3 = epi_staging_addr + epi_other_3 * 8192;
                        ...
                        if (acc_wg == 0) {
                            ...
                            for (int chunk_6 = 0; chunk_6 < 4; chunk_6++) {
                                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"((epi_a_addr_3 + (unsigned int)(row * 64 + chunk_6 * 16 ^ (row * 64 + chunk_6 * 16 >> 7 & 3) << 4))), "r"(acc3_bf16[chunk_6 * 4]), "r"(acc3_bf16[chunk_6 * 4 + 1]), "r"(acc3_bf16[chunk_6 * 4 + 2]), "r"(acc3_bf16[chunk_6 * 4 + 3]) : "memory");
                            }
                        }
                        if (acc_wg == 1) {
                            #pragma unroll
                            for (int chunk_7 = 0; chunk_7 < 4; chunk_7++) {
                                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"((epi_b_addr_3 + (unsigned int)(row * 64 + chunk_7 * 16 ^ (row * 64 + chunk_7 * 16 >> 7 & 3) << 4))), "r"(acc3_bf16[chunk_7 * 4]), "r"(acc3_bf16[chunk_7 * 4 + 1]), "r"(acc3_bf16[chunk_7 * 4 + 2]), "r"(acc3_bf16[chunk_7 * 4 + 3]) : "memory");
                            }
                        }
                        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                        asm volatile("barrier.sync 8, 256;" ::: "memory");
                        if (warp == 0) {
                            if (elect_sync()) {
                                tma_store_2d(C_tma, n_tile * 256 + 96, m_tile * 128, epi_a_addr_3);
                                asm volatile("cp.async.bulk.commit_group;");
                                tma_store_2d(C_tma, n_tile * 256 + 224, m_tile * 128, epi_b_addr_3);
                                asm volatile("cp.async.bulk.commit_group;");
                            }
                        }
                        epi_panel_cursor += 2;
                ...
                for (int segment_iter_1 = 0; segment_iter_1 < tile_rows_1; segment_iter_1++) {
                    ...
                    for (int iter_k = 0; iter_k < k_blocks_1; iter_k++) {
                        ...
                        int _mma_b_lo_0 = make_warp_uniform((((smem_b_addr) >> 4) & 0x3FFF) + (ab_stage) * 2048);
                        {
                            ...
                            if (elect_sync()) {
                                tcgen05_mma_f8f6f4((tmem_partials + (partial_stage_1 * 256)), _mma_ss_a_desc_0, _mma_ss_b_desc_0, 138412048, 0);
                            }
                            incr_smem_desc_lo(_mma_ss_a_desc_0, 2U);
                            incr_smem_desc_lo(_mma_ss_b_desc_0, 2U);
                            if (elect_sync()) {
                                tcgen05_mma_f8f6f4((tmem_partials + (partial_stage_1 * 256)), _mma_ss_a_desc_0, _mma_ss_b_desc_0, 138412048, 1);
                            }
                            incr_smem_desc_lo(_mma_ss_a_desc_0, 2U);
                            incr_smem_desc_lo(_mma_ss_b_desc_0, 2U);
                            if (elect_sync()) {
    ...
    if (warp == 9) {
        { // tma_role_main
            ...
            for (int tile_id_1 = bid; tile_id_1 < total_tiles_2; tile_id_1 += num_bids) {
                int m_tile_2 = tile_id_1 / n_tiles_2;
                int n_tile_1 = tile_id_1 % n_tiles_2;
                int tile_rows_2 = 128;
    ...
    if (warp == 10) {
        { // scale_role_main
            ...
            for (int tile_id_2 = bid; tile_id_2 < total_tiles_3; tile_id_2 += num_bids) {
                int m_tile_3 = tile_id_2 / n_tiles_3;
                int n_tile_2 = tile_id_2 % n_tiles_3;
                int tile_rows_3 = 128;
                    ...
                    for (int scale_group_1 = 0; scale_group_1 < scale_groups_1; scale_group_1++) {
                        ...
                        for (int n_half = 0; n_half < 2; n_half++) {
                            asm volatile(
                                ...
                                "}"
                                :: "r"((lane == 0) ? 1 : 0), "r"(smem_bscale_addr + (scale_stage_1 * 2 + (unsigned int)n_half) * 4 * 4), "l"(b_scale + ((group_3 * n_blocks + n_tile_2 * 2 + n_half) * k_blocks_3 + scale_group_1 * 4)));
                        }
                        asm volatile(
                            "{\n\t"
    ...
    if (warp == 11) {
        { // tma_b_role_main
            ...
            for (int tile_id_b = bid; tile_id_b < total_tiles_b; tile_id_b += num_bids) {
                int m_tile_b = tile_id_b / n_tiles_b;
                ...
                for (int segment_iter_4 = 0; segment_iter_4 < tile_rows_4; segment_iter_4++) {
                    ...
                    if (elect_sync()) {
                        ...
                        for (int iter_k_b = 0; iter_k_b < k_blocks_b; iter_k_b++) {
                            ...
                            tma_4d_gmem2smem(smem_b_addr + ab_stage_b * 32768, B, 0, n_tile_b * 256, iter_k_b, group_b, ab_full_addr + (ab_stage_b) * 8);
                            ab_stage_b += 1;
