...
kernel_cake_grouped_fp8_gemm_fd349a3926d8ae06cece(CakeTensorMap const* A, CakeTensorMap const* B, CakeTensorMap const* C_tma, __nv_bfloat16* __restrict__ C, float* __restrict__ a_scale, float* __restrict__ b_scale, int* __restrict__ m_indices, int M, int N, int K, int G)
{
        ...
        { // acc_epi_main
            ...
            for (int tile_id = cluster_id; tile_id < total_tiles; tile_id += num_clusters) {
                ...
                for (int segment_iter = 0; segment_iter < tile_rows; segment_iter++) {
                    ...
                    if (homogeneous != 0) {
                        unsigned int epi_first_slot = epi_staging_0_addr + output_ab_stage * 16384;
                        unsigned int epi_second_slot = epi_staging_2_addr + output_ab_stage * 16384;
                        if (acc_wg == 1) {
                            epi_first_slot = epi_staging_1_addr + output_ab_stage * 16384;
                            epi_second_slot = epi_staging_3_addr + output_ab_stage * 16384;
                        }
                        if (acc_warp_in_wg == 0) {
                            if (elect_sync()) {
                        ...
                        unsigned int epi_addr_0 = epi_first_slot + epi_panel_cursor / 2 * (epi_second_slot - epi_first_slot);
                        ...
                        unsigned int epi_addr_1 = epi_first_slot + epi_panel_cursor / 2 * (epi_second_slot - epi_first_slot);
                        ...
                        unsigned int epi_addr_2 = epi_first_slot + epi_panel_cursor / 2 * (epi_second_slot - epi_first_slot);
                        ...
                        unsigned int epi_addr_3 = epi_first_slot + epi_panel_cursor / 2 * (epi_second_slot - epi_first_slot);
    ...
    if (warp == 11) {
        { // tma_b_role_main
            ...
            int cluster_m_tile_b = cluster_id / (unsigned int)n_tiles_b;
            int n_tile_b = cluster_id % (unsigned int)n_tiles_b;
            int step_m_b = num_clusters / (unsigned int)n_tiles_b;
            int step_n_b = num_clusters % (unsigned int)n_tiles_b;
