...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_grouped_fp8_gemm_47c6a3211c2c38184a50'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_cake_grouped_fp8_gemm_47c6a3211c2c38184a50(CakeTensorMap const* A, CakeTensorMap const* B, CakeTensorMap const* C_tma, __nv_bfloat16* __restrict__ C, float* __restrict__ a_scale, float* __restrict__ b_scale, int* __restrict__ m_indices, int M, int N, int K, int G);


namespace cake_host_shim_2d32ef7fc49957fc {
...
void Run(TensorView arg_A, TensorView arg_B, TensorView arg_C_tma, TensorView arg_C, TensorView arg_a_scale, TensorView arg_b_scale, TensorView arg_m_indices, int64_t arg_M, int64_t arg_N, int64_t arg_K, int64_t arg_G, tvm::ffi::Tensor arg_tma_descriptor_workspace, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_grouped_fp8_gemm_47c6a3211c2c38184a50),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_grouped_fp8_gemm_47c6a3211c2c38184a50 failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_grouped_fp8_gemm_47c6a3211c2c38184a50), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_grouped_fp8_gemm_47c6a3211c2c38184a50 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_2d32ef7fc49957fc

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_2d32ef7fc49957fc::Run);
