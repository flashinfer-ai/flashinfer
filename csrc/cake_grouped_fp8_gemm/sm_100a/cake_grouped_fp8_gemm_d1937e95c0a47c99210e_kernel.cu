...
kernel_cake_grouped_fp8_gemm_d1937e95c0a47c99210e(CakeTensorMap const* A, CakeTensorMap const* B, CakeTensorMap const* C_tma, __nv_bfloat16* __restrict__ C, float* __restrict__ a_scale, float* __restrict__ b_scale, int* __restrict__ m_indices, int M, int N, int K, int G)
{
    ...
    __nv_bfloat16* epi_staging_0 = reinterpret_cast<__nv_bfloat16*>(smem_raw + 164864);
    const int epi_staging_0_addr = smem + 164864;
    __nv_bfloat16* epi_staging_1 = reinterpret_cast<__nv_bfloat16*>(smem_raw + 173056);
    const int epi_staging_1_addr = smem + 173056;
    __nv_bfloat16* epi_staging_2 = reinterpret_cast<__nv_bfloat16*>(smem_raw + 181248);
    const int epi_staging_2_addr = smem + 181248;
    __nv_bfloat16* epi_staging_3 = reinterpret_cast<__nv_bfloat16*>(smem_raw + 189440);
                ...
                for (int segment_iter = 0; segment_iter < tile_rows; segment_iter++) {
                    ...
                    for (int scale_group = 0; scale_group < scale_groups; scale_group++) {
                        ...
                        for (int k_inner = 0; k_inner < 1; k_inner++) {
                            ...
                            int _trl_addr_3 = tmem_partials + (partial_stage * 128 + (unsigned int)wg_col_base + 16) + (row_base << 16);
                            ...
                            int _trl_addr_5 = tmem_partials + (partial_stage * 128 + (unsigned int)wg_col_base + 32) + (row_base << 16);
                            ...
                            int _trl_addr_7 = tmem_partials + (partial_stage * 128 + (unsigned int)wg_col_base + 32 + 16) + (row_base << 16);
                ...
                for (int segment_iter_1 = 0; segment_iter_1 < tile_rows_1; segment_iter_1++) {
                    ...
                    for (int iter_k = 0; iter_k < k_blocks_1; iter_k++) {
                        ...
                        int _mma_b_lo_0 = make_warp_uniform((((smem_b_addr) >> 4) & 0x3FFF) + (ab_stage) * 1024);
                        {
                            ...
                            if (elect_sync()) {
                                tcgen05_mma_f8f6f4((tmem_partials + (partial_stage_1 * 128)), _mma_ss_a_desc_0, _mma_ss_b_desc_0, 136314896, 0);
                            }
                            incr_smem_desc_lo(_mma_ss_a_desc_0, 2U);
                            incr_smem_desc_lo(_mma_ss_b_desc_0, 2U);
                            if (elect_sync()) {
                                tcgen05_mma_f8f6f4((tmem_partials + (partial_stage_1 * 128)), _mma_ss_a_desc_0, _mma_ss_b_desc_0, 136314896, 1);
                            }
                            incr_smem_desc_lo(_mma_ss_a_desc_0, 2U);
                            incr_smem_desc_lo(_mma_ss_b_desc_0, 2U);
                            if (elect_sync()) {
                ...
                for (int segment_iter_2 = 0; segment_iter_2 < tile_rows_2; segment_iter_2++) {
                    ...
                    if (elect_sync()) {
                        ...
                        for (int iter_k_1 = 0; iter_k_1 < k_blocks_2; iter_k_1++) {
                            ...
                            tma_3d_gmem2smem(smem_a_addr + ab_stage_1 * 16384, A, 0, m_tile_2 * 128, iter_k_1, ab_full_addr + (ab_stage_1) * 8);
                            tma_4d_gmem2smem(smem_b_addr + ab_stage_1 * 16384, B, 0, n_tile_1 * 128, iter_k_1, group_2, ab_full_addr + (ab_stage_1) * 8);
