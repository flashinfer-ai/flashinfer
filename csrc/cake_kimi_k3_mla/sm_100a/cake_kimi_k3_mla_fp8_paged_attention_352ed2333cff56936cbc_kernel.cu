...
kernel_cake_kimi_k3_mla_fp8_paged_attention_352ed2333cff56936cbc(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_qr, const __grid_constant__ CUtensorMap tmap_k, const __grid_constant__ CUtensorMap tmap_kr, __nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_max, float* __restrict__ partial_sum, int* __restrict__ seq_lens, int* __restrict__ cum_seq_lens_q, int* __restrict__ page_table, float softmax_scale_log2, float bmm2_scale, int num_heads, int num_split, int max_pages_per_seq)
{
    ...
    #define rope_full_addr (mbar_base + 168)
    #define rope_empty_addr (mbar_base + 184)
    #define s_full_addr (mbar_base + 200)
    #define s_free_addr (mbar_base + 216)
    #define p_full_addr (mbar_base + 232)
    #define pt_free_addr (mbar_base + 248)
    #define pv_done_addr (mbar_base + 264)
    #define o_done_addr (mbar_base + 272)
    ...
    uint8_t* smem_qr = reinterpret_cast<uint8_t*>(smem_raw + 9216);
    const int smem_qr_addr = smem + 9216;
    uint8_t* smem_k = reinterpret_cast<uint8_t*>(smem_raw + 10240);
    const int smem_k_addr = smem + 10240;
    uint8_t* smem_vt = reinterpret_cast<uint8_t*>(smem_raw + 10240);
    const int smem_vt_addr = smem + 10240;
    uint8_t* smem_kr = reinterpret_cast<uint8_t*>(smem_raw + 174080);
    const int smem_kr_addr = smem + 174080;
    uint8_t* smem_pt = reinterpret_cast<uint8_t*>(smem_raw + 190464);
    const int smem_pt_addr = smem + 190464;
    unsigned int* smem_pt_words = reinterpret_cast<unsigned int*>(smem_raw + 190464);
    const int smem_pt_words_addr = smem + 190464;
    float* smem_stats = reinterpret_cast<float*>(smem_raw + 223232);
    const int smem_stats_addr = smem + 223232;
    int* smem_vis = reinterpret_cast<int*>(smem_raw + 223808);
    const int smem_vis_addr = smem + 223808;
    int* smem_flags = reinterpret_cast<int*>(smem_raw + 223872);
    const int smem_flags_addr = smem + 223872;
    unsigned int* smem_stats_w = reinterpret_cast<unsigned int*>(smem_raw + 223232);
    const int smem_stats_w_addr = smem + 223232;
    unsigned int* smem_vis_w = reinterpret_cast<unsigned int*>(smem_raw + 223808);
    const int smem_vis_w_addr = smem + 223808;
    unsigned int* smem_flags_w = reinterpret_cast<unsigned int*>(smem_raw + 223872);
    const int smem_flags_w_addr = smem + 223872;

    // Mbarrier init (12 pipeline groups, 0 ordered-sequence groups, 36 barriers)
    ...
    if (warp <= 3) {
        ...
        { // softmax_wg0_main
            ...
            int red_col = warp_in_wg * 2 + lane;
            int is_reducer = lane < 2;
            ...
            if (is_reducer != 0) {
                ...
                smem_stats[96 + red_col] = 1031.8073549220576f;
            }
            ...
            for (int tile = 0; tile < my_n_tiles; tile++) {
                ...
                tmem_ld_x4(&_tmem_load_0[0], taddr + (unsigned int)(sphase * 32) + (unsigned int)(lane_base << 16));
                float _tmem_load_1[4];
                tmem_ld_x4(&_tmem_load_1[0], taddr + (unsigned int)(sphase * 32) + 4 + (unsigned int)(lane_base << 16));
                ...
                if (tile_last >= vis_min) {
                    ...
                    #pragma unroll
                    for (int k_1 = 0; k_1 < 4; k_1 += 4) {
                        uint32_t _smem_vis_w_reg_1[4];
                        __int128_t _smem_b128_1;
                        asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_1) : "r"(smem_vis_w_addr + (4 + k_1) * 4));
                ...
                float pm_0 = -CAKE_INF;
                #pragma unroll
                for (int k_3 = 0; k_3 < 4; k_3 += 4) {
                    uint32_t _smem_stats_w_reg_1[4];
                    __int128_t _smem_b128_3;
                    asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_3) : "r"(smem_stats_w_addr + (100 + k_3) * 4));
                ...
                if (wg_exceed != 0) {
                    ...
                    for (int j_3 = 0; j_3 < 2; j_3++) {
                        float send = ((upper != 0) ? red[j_3] : red[j_3 + 2]);
                        float keep = ((upper != 0) ? red[j_3 + 2] : red[j_3]);
                    ...
                    for (int j_4 = 0; j_4 < 1; j_4++) {
                        float send_1 = ((upper_0 != 0) ? red[j_4] : red[j_4 + 1]);
                        float keep_1 = ((upper_0 != 0) ? red[j_4 + 1] : red[j_4]);
                        float _shfl_xor_1 = __shfl_xor_sync(0xFFFFFFFF, send_1, 8);
                        float recv_1 = _shfl_xor_1;
                        float _max_4 = max_noftz(keep_1, recv_1);
                        red[j_4] = _max_4;
                    }
                    float _shfl_xor_2 = __shfl_xor_sync(0xFFFFFFFF, red[0], 4);
                    float other = _shfl_xor_2;
                    float _max_5 = max_noftz(red[0], other);
                    red[0] = _max_5;
                    float _shfl_xor_3 = __shfl_xor_sync(0xFFFFFFFF, red[0], 2);
                    float other_1 = _shfl_xor_3;
                    float _max_6 = max_noftz(red[0], other_1);
                    red[0] = _max_6;
                    ...
                    red[0] = _max_7;
                    if ((lane & 7) == 0) {
                        smem_stats[32 + warp_in_wg * 8 + (lane >> 3)] = red[0];
                    }
                    float red_3[4];
                    #pragma unroll
                    for (int j_5 = 0; j_5 < 4; j_5++) {
                        red_3[j_5] = _tmem_load_1[j_5];
                    }
                    int upper_4 = (lane & 16) != 0;
                    #pragma unroll
                    for (int j_6 = 0; j_6 < 2; j_6++) {
                        float send_2 = ((upper_4 != 0) ? red_3[j_6] : red_3[j_6 + 2]);
                        float keep_2 = ((upper_4 != 0) ? red_3[j_6 + 2] : red_3[j_6]);
                        float _shfl_xor_5 = __shfl_xor_sync(0xFFFFFFFF, send_2, 16);
                        float recv_2 = _shfl_xor_5;
                        float _max_8 = max_noftz(keep_2, recv_2);
                        red_3[j_6] = _max_8;
                    }
                    int upper_5 = (lane & 8) != 0;
                    #pragma unroll
                    for (int j_7 = 0; j_7 < 1; j_7++) {
                        float send_3 = ((upper_5 != 0) ? red_3[j_7] : red_3[j_7 + 1]);
                        float keep_3 = ((upper_5 != 0) ? red_3[j_7 + 1] : red_3[j_7]);
                        float _shfl_xor_6 = __shfl_xor_sync(0xFFFFFFFF, send_3, 8);
                        float recv_3 = _shfl_xor_6;
                        float _max_9 = max_noftz(keep_3, recv_3);
                        red_3[j_7] = _max_9;
                    }
                    float _shfl_xor_7 = __shfl_xor_sync(0xFFFFFFFF, red_3[0], 4);
                    float other_6 = _shfl_xor_7;
                    float _max_10 = max_noftz(red_3[0], other_6);
                    red_3[0] = _max_10;
                    ...
                    red_3[0] = _max_12;
                    if ((lane & 7) == 0) {
                        smem_stats[32 + warp_in_wg * 8 + 4 + (lane >> 3)] = red_3[0];
                    }
                    asm volatile("barrier.sync 8, 128;" ::: "memory");
                    if (is_reducer != 0) {
                        float m0 = smem_stats[32 + red_col];
                        float m1 = smem_stats[40 + red_col];
                        float m2 = smem_stats[48 + red_col];
                        float m3 = smem_stats[56 + red_col];
                    ...
                    asm volatile("barrier.sync 8, 128;" ::: "memory");
                    #pragma unroll
                    for (int k_4 = 0; k_4 < 4; k_4 += 4) {
                        uint32_t _smem_stats_w_reg_2[4];
                        __int128_t _smem_b128_5;
                        asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_5) : "r"(smem_stats_w_addr + (16 + k_4) * 4));
                    ...
                    #pragma unroll
                    for (int k_5 = 0; k_5 < 4; k_5 += 4) {
                        uint32_t _smem_stats_w_reg_4[4];
                        __int128_t _smem_b128_7;
                        asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_7) : "r"(smem_stats_w_addr + (20 + k_5) * 4));
                        ...
                        asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_8) : "r"(smem_stats_w_addr + (4 + k_5) * 4));
                ...
                for (int j_8 = 0; j_8 < 4; j_8++) {
                    float _exp2_1 = approx_exp2(_tmem_load_0[j_8]);
                    _tmem_load_0[j_8] = _exp2_1;
                    psum0[j_8] = psum0[j_8] + _tmem_load_0[j_8];
                }
                ...
                int swz_rel = row_rel + ((0 ^ my_tok & 7) << 4);
                smem_pt_words[swz_rel >> 2] = packed[0];
                #pragma unroll
                for (int j_9 = 0; j_9 < 4; j_9++) {
                    float _exp2_2 = approx_exp2(_tmem_load_1[j_9]);
                    _tmem_load_1[j_9] = _exp2_2;
                    psum1[j_9] = psum1[j_9] + _tmem_load_1[j_9];
                }
                unsigned int packed_2[1];
                {
                    uint32_t _packed;
                    asm volatile("{\n\t"
                        ".reg .b16 _lo;\n\t"
                ...
                int swz_rel_5 = row_rel_4 + ((0 ^ my_tok & 7) << 4) + 4;
                smem_pt_words[swz_rel_5 >> 2] = packed_2[0];
                asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                if (wg_exceed != 0) {
                    if (tile > 0) {
                        ...
                        for (int vs = 0; vs < 4; vs++) {
                            ...
                            int o_b = taddr + 64 + (unsigned int)(vs * 32) + 4 + (unsigned int)(lane_base << 16);
                            float _tmem_load_2[4];
                            tmem_ld_x4(&_tmem_load_2[0], o_a);
                            float _tmem_load_3[4];
                            tmem_ld_x4(&_tmem_load_3[0], o_b);
                            float vals[4];
                            #pragma unroll
                            for (int k_6 = 0; k_6 < 4; k_6 += 4) {
                            ...
                            float vals_0[4];
                            #pragma unroll
                            for (int k_7 = 0; k_7 < 4; k_7 += 4) {
                                uint32_t _smem_stats_w_reg_7[4];
                                __int128_t _smem_b128_10;
                                asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_10) : "r"(smem_stats_w_addr + (4 + k_7) * 4));
                            ...
                            for (int j_10 = 0; j_10 < 4; j_10++) {
                                _tmem_load_2[j_10] = _tmem_load_2[j_10] * vals[j_10];
                            }
                            #pragma unroll
                            for (int j_11 = 0; j_11 < 4; j_11++) {
                                _tmem_load_3[j_11] = _tmem_load_3[j_11] * vals_0[j_11];
                            }
                            tmem_st_x4_f32(o_a, _tmem_load_2);
                            tmem_st_x4_f32(o_b, _tmem_load_3);
                        }
            ...
            for (int j_12 = 0; j_12 < 4; j_12++) {
                float _warp_reduce_0 = psum0[j_12];
                #pragma unroll
                for (int offset = 16; offset > 0; offset >>= 1)
                    _warp_reduce_0 += __shfl_xor_sync(0xFFFFFFFF, _warp_reduce_0, offset);
                float wsum_j = _warp_reduce_0;
                if (lane == j_12 % 32) {
                    smem_stats[32 + warp_in_wg * 8 + j_12] = wsum_j;
                }
            }
            #pragma unroll
            for (int j_13 = 0; j_13 < 4; j_13++) {
                float _warp_reduce_1 = psum1[j_13];
                #pragma unroll
                for (int offset = 16; offset > 0; offset >>= 1)
                    _warp_reduce_1 += __shfl_xor_sync(0xFFFFFFFF, _warp_reduce_1, offset);
                float wsum_j_1 = _warp_reduce_1;
                if (lane == (4 + j_13) % 32) {
                    smem_stats[32 + warp_in_wg * 8 + 4 + j_13] = wsum_j_1;
                }
            }
            asm volatile("barrier.sync 8, 128;" ::: "memory");
            if (is_reducer != 0) {
                float s0 = smem_stats[32 + red_col];
                float s1 = smem_stats[40 + red_col];
                float s2 = smem_stats[48 + red_col];
                float s3 = smem_stats[56 + red_col];
                float csum = s0 + s1 + (s2 + s3);
                smem_stats[112 + red_col] = csum;
                float safe_sum = ((csum > 0.0f) ? csum : 1.0f);
                float out_scale = ((num_split == 1) ? bmm2_scale : 1.0f);
                float _rcp_0 = approx_rcp(safe_sum);
                smem_stats[128 + red_col] = _rcp_0 * out_scale;
            ...
            if (my_n_tiles > 0) {
                ...
                for (int k_8 = 0; k_8 < 8; k_8 += 4) {
                    ...
                    asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_11) : "r"(smem_stats_w_addr + (128 + k_8) * 4));
                ...
                for (int vs_1 = 0; vs_1 < 4; vs_1++) {
                    float _tmem_load_4[4];
                    tmem_ld_x4(&_tmem_load_4[0], taddr + 64 + (unsigned int)(vs_1 * 32) + (unsigned int)(lane_base << 16));
                    #pragma unroll
                    for (int j_14 = 0; j_14 < 4; j_14++) {
                        int c = j_14;
                        if (c < rows_valid) {
                            *(reinterpret_cast<__nv_bfloat16*>(partial_O + (out_base + c * row_stride + vs_1 * 128)) + (0)) = __float2bfloat16_rn(_tmem_load_4[j_14] * vals_1[j_14]);
                        }
                    }
                    float _tmem_load_5[4];
                    tmem_ld_x4(&_tmem_load_5[0], taddr + 64 + (unsigned int)(vs_1 * 32) + 4 + (unsigned int)(lane_base << 16));
                    #pragma unroll
                    for (int j_15 = 0; j_15 < 4; j_15++) {
                        int c_1 = 4 + j_15;
                        if (c_1 < rows_valid) {
                            *(reinterpret_cast<__nv_bfloat16*>(partial_O + (out_base + c_1 * row_stride + vs_1 * 128)) + (0)) = __float2bfloat16_rn(_tmem_load_5[j_15] * vals_1[4 + j_15]);
                        }
                    }
                }
            }
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // softmax_wg1_main
            ...
            int red_col_1 = 8 + warp_in_wg_1 * 2 + lane;
            int is_reducer_1 = lane < 2;
            ...
            if (is_reducer_1 != 0) {
                ...
                smem_stats[96 + red_col_1] = 1031.8073549220576f;
            }
            asm volatile("barrier.sync 9, 128;" ::: "memory");
            int vis_min_1 = smem_vis[8];
            #pragma unroll 1
            for (int tile_1 = 0; tile_1 < my_n_tiles_1; tile_1++) {
                ...
                tmem_ld_x4(&_tmem_load_6[0], taddr + (unsigned int)(sphase_1 * 32) + 8 + (unsigned int)(lane_base_1 << 16));
                float _tmem_load_7[4];
                tmem_ld_x4(&_tmem_load_7[0], taddr + (unsigned int)(sphase_1 * 32) + 8 + 4 + (unsigned int)(lane_base_1 << 16));
                asm volatile("tcgen05.wait::ld.sync.aligned;" ::: "memory");
                asm volatile("tcgen05.fence::before_thread_sync;");
                ...
                if (tile_last_1 >= vis_min_1) {
                    #pragma unroll
                    for (int k_9 = 0; k_9 < 4; k_9 += 4) {
                        uint32_t _smem_vis_w_reg_2[4];
                        __int128_t _smem_b128_0;
                        asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_0) : "r"(smem_vis_w_addr + (8 + k_9) * 4));
                    ...
                    #pragma unroll
                    for (int k_10 = 0; k_10 < 4; k_10 += 4) {
                        uint32_t _smem_vis_w_reg_3[4];
                        __int128_t _smem_b128_1;
                        asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_1) : "r"(smem_vis_w_addr + (12 + k_10) * 4));
                ...
                float pm_2 = -CAKE_INF;
                #pragma unroll
                for (int k_11 = 0; k_11 < 4; k_11 += 4) {
                    uint32_t _smem_stats_w_reg_9[4];
                    __int128_t _smem_b128_2;
                    asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_2) : "r"(smem_stats_w_addr + (104 + k_11) * 4));
                ...
                #pragma unroll
                for (int k_12 = 0; k_12 < 4; k_12 += 4) {
                    uint32_t _smem_stats_w_reg_10[4];
                    __int128_t _smem_b128_3;
                    asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_3) : "r"(smem_stats_w_addr + (108 + k_12) * 4));
                ...
                if (wg_exceed_1 != 0) {
                    ...
                    for (int j_18 = 0; j_18 < 4; j_18++) {
                        red_1[j_18] = _tmem_load_6[j_18];
                    }
                    int upper_1 = (lane & 16) != 0;
                    #pragma unroll
                    for (int j_19 = 0; j_19 < 2; j_19++) {
                        float send_4 = ((upper_1 != 0) ? red_1[j_19] : red_1[j_19 + 2]);
                        float keep_4 = ((upper_1 != 0) ? red_1[j_19 + 2] : red_1[j_19]);
                        float _shfl_xor_10 = __shfl_xor_sync(0xFFFFFFFF, send_4, 16);
                        float recv_4 = _shfl_xor_10;
                        float _max_19 = max_noftz(keep_4, recv_4);
                        red_1[j_19] = _max_19;
                    }
                    int upper_0_1 = (lane & 8) != 0;
                    #pragma unroll
                    for (int j_20 = 0; j_20 < 1; j_20++) {
                        float send_5 = ((upper_0_1 != 0) ? red_1[j_20] : red_1[j_20 + 1]);
                        float keep_5 = ((upper_0_1 != 0) ? red_1[j_20 + 1] : red_1[j_20]);
                        float _shfl_xor_11 = __shfl_xor_sync(0xFFFFFFFF, send_5, 8);
                        float recv_5 = _shfl_xor_11;
                        float _max_20 = max_noftz(keep_5, recv_5);
                        red_1[j_20] = _max_20;
                    }
                    float _shfl_xor_12 = __shfl_xor_sync(0xFFFFFFFF, red_1[0], 4);
                    float other_3 = _shfl_xor_12;
                    float _max_21 = max_noftz(red_1[0], other_3);
                    red_1[0] = _max_21;
                    float _shfl_xor_13 = __shfl_xor_sync(0xFFFFFFFF, red_1[0], 2);
                    float other_1_1 = _shfl_xor_13;
                    float _max_22 = max_noftz(red_1[0], other_1_1);
                    red_1[0] = _max_22;
                    ...
                    red_1[0] = _max_23;
                    if ((lane & 7) == 0) {
                        smem_stats[64 + warp_in_wg_1 * 8 + (lane >> 3)] = red_1[0];
                    }
                    float red_3_1[4];
                    #pragma unroll
                    for (int j_21 = 0; j_21 < 4; j_21++) {
                        red_3_1[j_21] = _tmem_load_7[j_21];
                    }
                    int upper_4_1 = (lane & 16) != 0;
                    #pragma unroll
                    for (int j_22 = 0; j_22 < 2; j_22++) {
                        float send_6 = ((upper_4_1 != 0) ? red_3_1[j_22] : red_3_1[j_22 + 2]);
                        float keep_6 = ((upper_4_1 != 0) ? red_3_1[j_22 + 2] : red_3_1[j_22]);
                        float _shfl_xor_15 = __shfl_xor_sync(0xFFFFFFFF, send_6, 16);
                        float recv_6 = _shfl_xor_15;
                        float _max_24 = max_noftz(keep_6, recv_6);
                        red_3_1[j_22] = _max_24;
                    }
                    int upper_5_1 = (lane & 8) != 0;
                    #pragma unroll
                    for (int j_23 = 0; j_23 < 1; j_23++) {
                        float send_7 = ((upper_5_1 != 0) ? red_3_1[j_23] : red_3_1[j_23 + 1]);
                        float keep_7 = ((upper_5_1 != 0) ? red_3_1[j_23 + 1] : red_3_1[j_23]);
                        float _shfl_xor_16 = __shfl_xor_sync(0xFFFFFFFF, send_7, 8);
                        float recv_7 = _shfl_xor_16;
                        float _max_25 = max_noftz(keep_7, recv_7);
                        red_3_1[j_23] = _max_25;
                    }
                    float _shfl_xor_17 = __shfl_xor_sync(0xFFFFFFFF, red_3_1[0], 4);
                    float other_6_1 = _shfl_xor_17;
                    float _max_26 = max_noftz(red_3_1[0], other_6_1);
                    red_3_1[0] = _max_26;
                    ...
                    red_3_1[0] = _max_28;
                    if ((lane & 7) == 0) {
                        smem_stats[64 + warp_in_wg_1 * 8 + 4 + (lane >> 3)] = red_3_1[0];
                    }
                    asm volatile("barrier.sync 9, 128;" ::: "memory");
                    if (is_reducer_1 != 0) {
                        float m0_1 = smem_stats[64 + (red_col_1 - 8)];
                        float m1_1 = smem_stats[72 + (red_col_1 - 8)];
                        float m2_1 = smem_stats[80 + (red_col_1 - 8)];
                        float m3_1 = smem_stats[88 + (red_col_1 - 8)];
                        float _max_29 = max_noftz(m0_1, m1_1);
                        float _max_30 = max_noftz(m2_1, m3_1);
                        float _max_31 = max_noftz(_max_29, _max_30);
                        float tm_1 = _max_31;
                        float c_old_1 = smem_stats[96 + red_col_1];
                        ...
                        smem_stats[96 + red_col_1] = c_new_1;
                        smem_stats[16 + red_col_1] = delta_1;
                        smem_stats[red_col_1] = alpha_1;
                    }
                    asm volatile("barrier.sync 9, 128;" ::: "memory");
                    #pragma unroll
                    for (int k_13 = 0; k_13 < 4; k_13 += 4) {
                        uint32_t _smem_stats_w_reg_11[4];
                        __int128_t _smem_b128_5;
                        asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_5) : "r"(smem_stats_w_addr + (24 + k_13) * 4));
                        ...
                        __int128_t _smem_b128_6;
                        asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_6) : "r"(smem_stats_w_addr + (8 + k_13) * 4));
                    ...
                    #pragma unroll
                    for (int k_14 = 0; k_14 < 4; k_14 += 4) {
                        uint32_t _smem_stats_w_reg_13[4];
                        __int128_t _smem_b128_7;
                        asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_7) : "r"(smem_stats_w_addr + (28 + k_14) * 4));
                        ...
                        __int128_t _smem_b128_8;
                        asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_8) : "r"(smem_stats_w_addr + (12 + k_14) * 4));
                ...
                for (int j_24 = 0; j_24 < 4; j_24++) {
                    float _exp2_4 = approx_exp2(_tmem_load_6[j_24]);
                    _tmem_load_6[j_24] = _exp2_4;
                    psum0_1[j_24] = psum0_1[j_24] + _tmem_load_6[j_24];
                }
                unsigned int packed_1[1];
                {
                    uint32_t _packed;
                    asm volatile("{\n\t"
                        ".reg .b16 _lo;\n\t"
                ...
                int swz_rel_1 = row_rel_1 + ((0 ^ my_tok_1 & 7) << 4) + 8;
                smem_pt_words[swz_rel_1 >> 2] = packed_1[0];
                #pragma unroll
                for (int j_25 = 0; j_25 < 4; j_25++) {
                    float _exp2_5 = approx_exp2(_tmem_load_7[j_25]);
                    _tmem_load_7[j_25] = _exp2_5;
                    psum1_1[j_25] = psum1_1[j_25] + _tmem_load_7[j_25];
                }
                unsigned int packed_2_1[1];
                {
                    uint32_t _packed;
                    asm volatile("{\n\t"
                        ".reg .b16 _lo;\n\t"
                ...
                int swz_rel_5_1 = row_rel_4_1 + ((0 ^ my_tok_1 & 7) << 4) + 12;
                smem_pt_words[swz_rel_5_1 >> 2] = packed_2_1[0];
                asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                if (wg_exceed_1 != 0) {
                    if (tile_1 > 0) {
                        ...
                        for (int vs_2 = 0; vs_2 < 4; vs_2++) {
                            int o_a_1 = taddr + 64 + (unsigned int)(vs_2 * 32) + 8 + (unsigned int)(lane_base_1 << 16);
                            int o_b_1 = taddr + 64 + (unsigned int)(vs_2 * 32) + 8 + 4 + (unsigned int)(lane_base_1 << 16);
                            float _tmem_load_8[4];
                            tmem_ld_x4(&_tmem_load_8[0], o_a_1);
                            float _tmem_load_9[4];
                            tmem_ld_x4(&_tmem_load_9[0], o_b_1);
                            float vals_2[4];
                            #pragma unroll
                            for (int k_15 = 0; k_15 < 4; k_15 += 4) {
                                uint32_t _smem_stats_w_reg_15[4];
                                __int128_t _smem_b128_9;
                                asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_9) : "r"(smem_stats_w_addr + (8 + k_15) * 4));
                            ...
                            float vals_0_1[4];
                            #pragma unroll
                            for (int k_16 = 0; k_16 < 4; k_16 += 4) {
                                uint32_t _smem_stats_w_reg_16[4];
                                __int128_t _smem_b128_10;
                                asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_10) : "r"(smem_stats_w_addr + (12 + k_16) * 4));
                            ...
                            for (int j_26 = 0; j_26 < 4; j_26++) {
                                _tmem_load_8[j_26] = _tmem_load_8[j_26] * vals_2[j_26];
                            }
                            #pragma unroll
                            for (int j_27 = 0; j_27 < 4; j_27++) {
                                _tmem_load_9[j_27] = _tmem_load_9[j_27] * vals_0_1[j_27];
                            }
                            tmem_st_x4_f32(o_a_1, _tmem_load_8);
                            tmem_st_x4_f32(o_b_1, _tmem_load_9);
                        }
                        asm volatile("tcgen05.wait::st.sync.aligned;" ::: "memory");
                    }
                }
            ...
            if (my_n_tiles_1 > 0) {
                ...
                for (int k_17 = 0; k_17 < 8; k_17 += 4) {
                    ...
                    __int128_t _smem_b128_11;
                    asm volatile("ld.shared.b128 %0, [%1];" : "=q"(_smem_b128_11) : "r"(smem_stats_w_addr + (136 + k_17) * 4));
                ...
                for (int vs_3 = 0; vs_3 < 4; vs_3++) {
                    float _tmem_load_10[4];
                    tmem_ld_x4(&_tmem_load_10[0], taddr + 64 + (unsigned int)(vs_3 * 32) + 8 + (unsigned int)(lane_base_1 << 16));
                    #pragma unroll
                    for (int j_30 = 0; j_30 < 4; j_30++) {
                        int c_2 = 8 + j_30;
                        if (c_2 < rows_valid_1) {
                            *(reinterpret_cast<__nv_bfloat16*>(partial_O + (out_base_1 + c_2 * row_stride_1 + vs_3 * 128)) + (0)) = __float2bfloat16_rn(_tmem_load_10[j_30] * vals_3[j_30]);
                        }
                    }
                    float _tmem_load_11[4];
                    tmem_ld_x4(&_tmem_load_11[0], taddr + 64 + (unsigned int)(vs_3 * 32) + 8 + 4 + (unsigned int)(lane_base_1 << 16));
                    #pragma unroll
                    for (int j_31 = 0; j_31 < 4; j_31++) {
                        int c_3 = 12 + j_31;
                        if (c_3 < rows_valid_1) {
                            *(reinterpret_cast<__nv_bfloat16*>(partial_O + (out_base_1 + c_3 * row_stride_1 + vs_3 * 128)) + (0)) = __float2bfloat16_rn(_tmem_load_11[j_31] * vals_3[4 + j_31]);
                        }
                    }
                }
            }
        ...
        { // mma_warp_main
            ...
            for (int tile_2 = 0; tile_2 < my_n_tiles_2; tile_2++) {
                ...
                for (int st = 0; st < 4; st++) {
                    ...
                    int _mma_b_lo_0 = make_warp_uniform((((smem_q_addr) >> 4) & 0x3FFF) + (st) * 128);
                    {
                        ...
                        if (elect_sync()) {
                            tcgen05_mma_f8f6f4((tmem_tmem + (sphase_2 * 32)), _mma_ss_a_desc_0, _mma_ss_b_desc_0, 134479888, ((st == 0) ? 0 : 1));
                        }
                    ...
                    mma_stage += 1;
                    if (mma_stage == 10) { mma_stage = 0; mma_phase ^= 1; }
                }
                ...
                int _mma_b_lo_1 = make_warp_uniform(((smem_qr_addr) >> 4) & 0x3FFF);
                {
                    ...
                    if (elect_sync()) {
                        tcgen05_mma_f8f6f4((tmem_tmem + (sphase_2 * 32)), _mma_ss_a_desc_1, _mma_ss_b_desc_1, 134479888, 1);
                    }
        ...
        { // pv_warp_main
            ...
            for (int tile_3 = 0; tile_3 < my_n_tiles_3; tile_3++) {
                ...
                for (int vs_4 = 0; vs_4 < 4; vs_4++) {
                    ...
                    unsigned int slot = ((slot_raw >= 10) ? slot_raw - 10 : slot_raw);
                    ...
                    int _mma_b_lo_2 = make_warp_uniform(((((smem_pt_addr) >> 4) & 0x3FFF) | 0x4000000) + (pv_pp) * 1024);
                    {
                        ...
                        if (elect_sync()) {
                            tcgen05_mma_f8f6f4((tmem_tmem + (64 + vs_4 * 32)), _mma_ss_a_desc_0, _mma_ss_b_desc_0, 134578192, ((first_pv) ? 0 : 1));
                        }
                        incr_smem_desc_lo(_mma_ss_a_desc_0, 256U);
                        incr_smem_desc_lo(_mma_ss_b_desc_0, 256U);
                        if (elect_sync()) {
                            tcgen05_mma_f8f6f4((tmem_tmem + (64 + vs_4 * 32)), _mma_ss_a_desc_0, _mma_ss_b_desc_0, 134578192, 1);
                        }
                        incr_smem_desc_lo(_mma_ss_a_desc_0, 256U);
                        incr_smem_desc_lo(_mma_ss_b_desc_0, 256U);
                        if (elect_sync()) {
                ...
                base = ((base_raw >= 10) ? base_raw - 10 : base_raw);
            }
            if (my_n_tiles_3 > 0) {
    ...
    if (warp == 10) {
        { // load_warp_main
            ...
            if (elect_sync()) {
                mbarrier_arrive_expect_tx(q_full_addr, 9216);
                #pragma unroll
                for (int st_1 = 0; st_1 < 4; st_1++) {
                    tma_2d_gmem2smem(smem_q_addr + (unsigned int)(st_1 * 2048), (&tmap_q), st_1 * 128, row_base_global_4, q_full_addr);
                }
            ...
            for (int tile_4 = 0; tile_4 < my_n_tiles_4; tile_4++) {
                ...
                for (int st_3 = 0; st_3 < 4; st_3++) {
                    ...
                    load_stage += 1;
                    if (load_stage == 10) { load_stage = 0; load_phase ^= 1; }
                }
