...
bool use_rowwise_kernel(uint32_t rows, uint32_t vocab_size, ParameterKind parameter_kind) {
  ...
  const bool dense_aligned_mid_row =
      rows > 128 && rows <= 384 && vocab_size >= 24576 && vocab_size <= 256000 &&
      vocab_size % 4 == 0 && parameter_kind == ParameterKind::kNone;
  const bool dense_aligned_high_row_narrow =
      rows > 384 && rows <= 1024 && vocab_size >= 24576 && vocab_size <= 32000 &&
      vocab_size % 4 == 0 && parameter_kind == ParameterKind::kNone;
...
cudaError_t launch_blackwell_softmax(float* logits, float* output, float* temperature_arr,
  ...
  cudaError_t status = cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      &active_blocks_per_sm, kernel_flashinfer_blackwell_softmax_bootstrap_seed,
      kBootstrapThreads, kDynamicSmemBytes);
  ...
  const uint64_t tiles_per_row = ceil_div(static_cast<uint64_t>(vocab_size),
                                          static_cast<uint64_t>(kBootstrapThreads));
  const uint64_t cooperative_capacity =
      static_cast<uint64_t>(active_blocks_per_sm) * static_cast<uint64_t>(sm_count);
  const uint64_t provisional_grid =
      std::max<uint64_t>(1, std::min<uint64_t>(static_cast<uint64_t>(rows) * tiles_per_row,
  ...
  const uint64_t grid =
      std::max<uint64_t>(1, std::min<uint64_t>(provisional_grid,
                                              static_cast<uint64_t>(rows) * splits));
  ...
  int splits_i = splits;
  void* args[] = {&logits,          &parameter,       &output,        &partial_max,
                  &partial_sum,     &rows_i,          &vocab_size_i,  &splits_i,
                  &parameter_kind_i, &temperature_val};
  return cudaLaunchCooperativeKernel(
      reinterpret_cast<const void*>(kernel_flashinfer_blackwell_softmax_bootstrap_seed),
      dim3(static_cast<uint32_t>(grid)), dim3(kBootstrapThreads), args, kDynamicSmemBytes,
      stream);
}
...
void blackwell_softmax(TensorView workspace_buffer, TensorView logits, TensorView output,
  ...
  cudaError_t status =
      launch_blackwell_softmax(logits_ptr, output_ptr, temperature_ptr,
                               static_cast<float>(temperature_val), parameter_kind, rows,
                               vocab_size, workspace_buffer.data_ptr(), workspace_bytes, stream);
  if (status == cudaErrorNotSupported) {
    status = sampling::OnlineSoftmax<float>(
        logits_ptr, output_ptr, rows, vocab_size, temperature_ptr,
        static_cast<float>(temperature_val), workspace_buffer.data_ptr(), workspace_bytes,
        enable_pdl, stream);
  }
