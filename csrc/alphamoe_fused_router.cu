...
// Both include a forced shared logit in selected softmax. Generated function
// bodies are unchanged; CUDA headers are hoisted and helpers/macros isolated.
...
inline RouterLaunchConfig GetRouterLaunchConfig(int32_t device_id) {
  ...
  CheckCuda(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
                ...
                static_cast<int>(kSmallThreads),
                static_cast<size_t>(kSmallDynamicSmemBytes)),
  ...
  const RouterLaunchConfig config{sm_count, active_blocks_per_sm, small_active_blocks_per_sm};
...
void Run(TensorView logits, TensorView topk_weights, TensorView topk_ids,
  ...
  const int64_t grid_x = use_small ? std::max<int64_t>(1, std::min<int64_t>((m + kSmallGeneratedWarps - 1) / kSmallGeneratedWarps, config.sm_count)) : std::max<int64_t>(
      1, std::min<int64_t>((m + kGeneratedWarps - 1) / kGeneratedWarps, config.sm_count));
  const int64_t cooperative_capacity =
      static_cast<int64_t>(use_small ? config.small_active_blocks_per_sm
                                     : config.active_blocks_per_sm) * config.sm_count;
  ...
  if (use_small) {
    CheckCuda(
        cudaLaunchCooperativeKernel(reinterpret_cast<const void*>(kernel_alpha_moe_fused_router_small),
