...
void selective_state_update(
    ...
    Optional<TensorView> intermediate_states_buffer,  // (cache_entries, buffer_steps, nheads, dim,
                                                      // dstate)
    Optional<TensorView> intermediate_state_indices,  // (batch,)
    Optional<TensorView> intermediate_state_scales,   // float32: matching intermediate states
