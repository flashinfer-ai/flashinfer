...
// Bundle: Blackwell all-gather matmul, world sizes 2 and 4.
    ...
    if (elect_sync()) {
        // nvlink_barrier(pg_flags) phase=0
        {
            ...
            for (int __r = 0; __r < __ws; ++__r) {
                ...
                asm volatile("red.async.relaxed.sys.global.add.u32 [%0], %1;"
                    :: "l"(__peer_flag), "r"(1u) : "memory");
            }
            asm volatile("fence.proxy.async.global;");
            unsigned* __local_flag = pg_flags[__me] + __slot;
            while (true) {
                unsigned __v;
                asm volatile("ld.acquire.sys.global.u32 %0, [%1];" : "=r"(__v) : "l"(__local_flag));
                if (__v >= (unsigned)__ws) break;
            }
            asm volatile("st.relaxed.sys.global.u32 [%0], %1;" :: "l"(__local_flag), "r"(0u));
        }
    }
}
...
kernel_cake_blackwell_all_gather_matmul_float16_ws2(CakeTensorMap const* A_local, CakeTensorMap const* A_scratch, CakeTensorMap const* B, __half* __restrict__ C, __half* __restrict__ scratch_payload, unsigned int* __restrict__ ready, int rank, int M)
{
    const int tid = threadIdx.x;
    const int warp = make_warp_uniform(tid / 32);
    const int lane = tid % 32;
                ...
                for (int chunk_idx_2 = 0; chunk_idx_2 < (M + ((M < 2432) ? M : 2432) - 1) / ((M < 2432) ? M : 2432); chunk_idx_2++) {
                    ...
                    if (active_tiles_2 > bid) {
                        ...
                        for (int n_chunk = 0; n_chunk < 32; n_chunk++) {
                            ...
                            reinterpret_cast<int4*>(C + ((out_m + epi_tid) * 2048 + (off_n_1 + n_chunk * 8)))[0] = reinterpret_cast<int4*>(_tmem_load_0_f16)[0];
                        }
                        if (elect_sync()) {
...
__global__ __launch_bounds__(192) void
kernel_cake_blackwell_all_gather_matmul_bfloat16_ws2(CakeTensorMap const* A_local, CakeTensorMap const* A_scratch, CakeTensorMap const* B, __nv_bfloat16* __restrict__ C, __nv_bfloat16* __restrict__ scratch_payload, unsigned int* __restrict__ ready, int rank, int M)
{
    const int tid = threadIdx.x;
    const int warp = make_warp_uniform(tid / 32);
    const int lane = tid % 32;
...
__global__ __launch_bounds__(192) void
kernel_cake_blackwell_all_gather_matmul_float16_ws4(CakeTensorMap const* A_local, CakeTensorMap const* A_scratch, CakeTensorMap const* B, __half* __restrict__ C, __half* __restrict__ scratch_payload, unsigned int* __restrict__ ready, int rank, int M)
{
    const int tid = threadIdx.x;
    const int warp = make_warp_uniform(tid / 32);
    const int lane = tid % 32;
...
__global__ __launch_bounds__(192) void
kernel_cake_blackwell_all_gather_matmul_bfloat16_ws4(CakeTensorMap const* A_local, CakeTensorMap const* A_scratch, CakeTensorMap const* B, __nv_bfloat16* __restrict__ C, __nv_bfloat16* __restrict__ scratch_payload, unsigned int* __restrict__ ready, int rank, int M)
{
    const int tid = threadIdx.x;
    const int warp = make_warp_uniform(tid / 32);
    const int lane = tid % 32;
