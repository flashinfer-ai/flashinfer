...
kernel_cake_bf16_bmm_bc0d392cafbe0e91f5aa(__nv_bfloat16* __restrict__ A, __nv_bfloat16* __restrict__ B_storage, uint8_t* __restrict__ out_bytes, int M, int N, int a_stride_b, int a_stride_m, int a_stride_k, int b_stride_b, int b_stride_n, int b_stride_k, int out_type)
{
    ...
    int effective_m = 128;
    int effective_n = 80;
    int effective_a_stride_b = 8192;
    int effective_a_stride_m = 64;
    int effective_a_stride_k = 1;
    int effective_b_stride_b = 5120;
    int effective_b_stride_n = 64;
        ...
        if (effective_m > m_base + 15 && effective_n > n_warp_base + 7) {
            ...
            for (int frag_row = 0; frag_row < 2; frag_row++) {
                ...
                if (effective_out_type == 0) {
                    ...
                    if ((pair_addr & 3) == 0) {
                        {
                            __half2 _pk = __floats2half2_rn(accum[value_idx + 0], accum[value_idx + 1]);
                            *reinterpret_cast<__half2*>(&((__half*)(reinterpret_cast<unsigned int*>(pair_dst)))[0]) = _pk;
                        }
                    } else {
                        *(reinterpret_cast<__half*>(pair_dst) + (0)) = __float2half_rn(accum[value_idx]);
                        *(reinterpret_cast<__half*>(pair_dst + 2) + (0)) = __float2half_rn(accum[value_idx + 1]);
                    }
                } else {
                    {
            ...
            for (int frag_row_1 = 0; frag_row_1 < 2; frag_row_1++) {
                int m_idx_1 = (unsigned int)m_base + lane / 4 + (unsigned int)(frag_row_1 * 8);
                ...
                if (m_idx_1 < effective_m && n_idx_1 < effective_n) {
                    ...
                    if (effective_n > n_idx_1 + 1) {
                        if (effective_out_type == 0) {
                            ...
                            if ((pair_addr_1 & 3) == 0) {
                                {
                                    __half2 _pk = __floats2half2_rn(accum[value_idx_1 + 0], accum[value_idx_1 + 1]);
                                    *reinterpret_cast<__half2*>(&((__half*)(reinterpret_cast<unsigned int*>(pair_dst_1)))[0]) = _pk;
                                }
                            } else {
                                *(reinterpret_cast<__half*>(pair_dst_1) + (0)) = __float2half_rn(accum[value_idx_1]);
                                *(reinterpret_cast<__half*>(pair_dst_1 + 2) + (0)) = __float2half_rn(accum[value_idx_1 + 1]);
                            }
                        } else {
                            {
