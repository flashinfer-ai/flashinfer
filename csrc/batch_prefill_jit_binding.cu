...
// Same signature as `plan`, but plans for the ragged kernel: FP8 KV at large
// head dims needs a smaller CTA_TILE_Q there (non-VO-split shared-memory
// layout), which must not affect the paged kernel planned via `plan`.
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(ragged_plan, BatchPrefillWithRaggedKVCachePlan);
