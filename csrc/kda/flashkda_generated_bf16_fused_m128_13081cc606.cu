            ...
            for (int chunk_idx = 0; chunk_idx < num_chunks_0; chunk_idx++) {
                ...
                for (int residual_half = 0; residual_half < 2; residual_half++) {
                    ...
                    }
                    {
                        #pragma unroll
                        for (int _ls = 0; _ls < 8; _ls++)
                            sub_f32x2_inplace(&reinterpret_cast<float2*>(residual_v)[_ls], reinterpret_cast<const float2*>((_tmem_load_0 + residual_half * 16))[_ls]);
                    }
                    ...
                    {
                        {
                            ...
                            *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(final_state + (state_base + (long long)(state_col_block_1 * 32))))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                            *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(final_state + (state_base + (long long)(state_col_block_1 * 32))))[8]) = *reinterpret_cast<uint4*>(&_pk[4]);
                        }
                        {
                            __nv_bfloat162 _pk[8];
                            ...
                            *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(final_state + (state_base + (long long)(state_col_block_1 * 32) + 16)))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                            *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(final_state + (state_base + (long long)(state_col_block_1 * 32) + 16)))[8]) = *reinterpret_cast<uint4*>(&_pk[4]);
                        }
                    }
                }
            }
