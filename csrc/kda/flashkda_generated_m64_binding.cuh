...
inline void LaunchM64(M64Args args, const StatePointerSlots& state, dim3 grid,
                      cudaStream_t stream) {
...
inline void RunM64(
  ...
  TVM_FFI_ICHECK(grid_y == 1 && grid_z == 1)
      << "M64 requires a one-dimensional grid";
  const int64_t expected_grid_x = prepared.num_sequences * num_heads * 2;
  TVM_FFI_ICHECK(grid_x == expected_grid_x)
      << "M64 grid_x must equal 2 * num_sequences * num_heads";
  M64Args args{};
  args.q = q.data_ptr();
  ...
  args.use_state_indices = use_state_indices;
  ...
  args.lower_bound = static_cast<float>(lower_bound);
  LaunchM64(args, prepared.state, CheckedGrid(grid_x, grid_y, grid_z),
            prepared.stream);
}
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(run,
                              flashinfer::flash_kda_generated::RunM64);
