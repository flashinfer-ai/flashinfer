...
// Round-151 (KDA_FN16 single-launch fused N16 experiment): device-side packed
// planning. Under KDA_BT16_PLAN_INLINE the per-sequence 16-token chunk base
// and count are derived from cu_seqlens with an O(seq) scan (<= 32
// sequences), eliminating the Bt16BuildMetadata launch and cu_chunks tensor.
#ifdef KDA_BT16_PLAN_INLINE
__device__ __forceinline__ void bt16_inline_seqplan(const long long* __restrict__ cu_seqlens,
                                                    int seq, int& chunk_base_out,
                                                    int& num_chunks_out) {
  int acc = 0;
  #pragma unroll 1
  for (int s = 0; s < seq; ++s) {
    acc += (int)((cu_seqlens[s + 1] - cu_seqlens[s] + 15) / 16);
  }
  chunk_base_out = acc;
  num_chunks_out = (int)((cu_seqlens[seq + 1] - cu_seqlens[seq] + 15) / 16);
}
#endif
                          ...
                          )
{
    ...
    if (warp <= 3) {
        ...
        { // epilogue_main
            ...
            #ifdef KDA_BT16_PLAN_INLINE
            int num_chunks, chunk_base;
            bt16_inline_seqplan(cu_seqlens, seq_idx, chunk_base, num_chunks);
#else
            int num_chunks = cu_chunks[seq_idx + 1] - cu_chunks[seq_idx];
            int chunk_base = cu_chunks[seq_idx];
#endif
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // compute_left_main
            ...
            #ifdef KDA_BT16_PLAN_INLINE
            int num_chunks_1, chunk_base_1;
            bt16_inline_seqplan(cu_seqlens, seq_idx_1, chunk_base_1, num_chunks_1);
#else
            int num_chunks_1 = cu_chunks[seq_idx_1 + 1] - cu_chunks[seq_idx_1];
            int chunk_base_1 = cu_chunks[seq_idx_1];
#endif
    ...
    if (warp >= 8 && warp <= 11) {
        ...
        { // compute_right_main
            ...
            #ifdef KDA_BT16_PLAN_INLINE
            int num_chunks_2, chunk_base_2;
            bt16_inline_seqplan(cu_seqlens, seq_idx_2, chunk_base_2, num_chunks_2);
#else
            int num_chunks_2 = cu_chunks[seq_idx_2 + 1] - cu_chunks[seq_idx_2];
            int chunk_base_2 = cu_chunks[seq_idx_2];
#endif
    ...
    if (warp == 12) {
        { // mma_q_main
            ...
            #ifdef KDA_BT16_PLAN_INLINE
            int num_chunks_3, chunk_base_3;
            bt16_inline_seqplan(cu_seqlens, seq_idx_3, chunk_base_3, num_chunks_3);
#else
            int num_chunks_3 = cu_chunks[seq_idx_3 + 1] - cu_chunks[seq_idx_3];
            int chunk_base_3 = cu_chunks[seq_idx_3];
#endif
    ...
    if (warp == 13) {
        { // mma_state_main
            ...
            #ifdef KDA_BT16_PLAN_INLINE
            int num_chunks_4, chunk_base_4;
            bt16_inline_seqplan(cu_seqlens, seq_idx_4, chunk_base_4, num_chunks_4);
#else
            int num_chunks_4 = cu_chunks[seq_idx_4 + 1] - cu_chunks[seq_idx_4];
            int chunk_base_4 = cu_chunks[seq_idx_4];
#endif
    ...
    if (warp == 14) {
        { // factors_main
            ...
            #ifdef KDA_BT16_PLAN_INLINE
            int num_chunks_5, chunk_base_5;
            bt16_inline_seqplan(cu_seqlens, seq_idx_5, chunk_base_5, num_chunks_5);
#else
            int num_chunks_5 = cu_chunks[seq_idx_5 + 1] - cu_chunks[seq_idx_5];
            int chunk_base_5 = cu_chunks[seq_idx_5];
#endif
            int chunk_base = chunk_base_5;
