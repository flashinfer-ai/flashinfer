...
#if !defined(FLASHINFER_CAKE_KDA_TARGET_MINOR)
#error "FLASHINFER_CAKE_KDA_TARGET_MINOR must be defined by the JIT/AOT spec"
#endif
#if defined(FLASHINFER_CAKE_KDA_TARGET_FAMILY)
#error "CakeKDA prefill requires an exact-SM target, not a family target"
#endif

constexpr int kCakeKDATargetMinor = FLASHINFER_CAKE_KDA_TARGET_MINOR;
static_assert(kCakeKDATargetMinor == 0 || kCakeKDATargetMinor == 3,
              "CakeKDA target must be exact SM100a or exact SM103a");

inline void CheckCakeKDATarget(int32_t device_id) {
  int major = 0;
  int minor = 0;
  ...
  TVM_FFI_ICHECK(major == 10 && minor == kCakeKDATargetMinor)
      << "this CakeKDA module was compiled for exact compute capability 10." << kCakeKDATargetMinor
      << ", got " << major << "." << minor;
}
...
inline int64_t CheckCommonInputs(const TensorView& q, const TensorView& k, const TensorView& v,
  ...
  if (unbounded_softplus) {
    TVM_FFI_ICHECK(lower_bound == 0.0)
        << "the unbounded-softplus variant requires its unused lower_bound ABI scalar to be zero";
  } else {
...
inline void CheckDynamicSmemCapacity(int32_t device_id, int32_t smem_bytes) {
  ...
  TVM_FFI_ICHECK(max_optin >= smem_bytes)
      << "device exposes only " << max_optin << " bytes of opt-in shared memory; CakeKDA requires "
      << smem_bytes;
}
