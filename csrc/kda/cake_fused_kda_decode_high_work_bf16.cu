...
static_assert(alignof(FlashInferTensorMap) == 128, "kernel tensor-map ABI must be 128-byte aligned");
#include <cuda_bf16.h>
#include <cuda_fp8.h>
...
#define SMEM_SASYNCSTATE_STAGE_BYTES 16384
#define SMEM_SASYNCSTATE_STRIDE 16384
...
kernel_cake_fused_kda_decode_high_work_bf16(__nv_bfloat16* __restrict__ x, float* __restrict__ weight, __nv_bfloat16* __restrict__ conv_state, __nv_bfloat16* __restrict__ raw_gate, __nv_bfloat16* __restrict__ raw_beta, float* __restrict__ A_log, float* __restrict__ dt_bias, int* __restrict__ state_indices, __nv_bfloat16* __restrict__ state, __nv_bfloat16* __restrict__ output_gate, float* __restrict__ norm_weight, __nv_bfloat16* __restrict__ output, int x_row_stride, int conv_slot_stride, int beta_row_stride, int state_slot_stride, int output_gate_row_stride, int H, int use_lower_bound, float lower_bound_log2, float norm_eps)
{
    ...
    }
    {
        if (tid_0 < 128) {
            if (warp % 2 != 0) {
                ...
                int early_async_target_group = group;
                int early_async_state_group_row = early_async_target_group * 2;
                int early_async_state_src_base = slot * state_slot_stride + head * 128 * 128 + state_owner_row_base * 128 + k_start + 8192;
                #pragma unroll
                for (int local_row = 0; local_row < 2; local_row++) {
                    int early_async_state_source_row = local_row;
                    int early_async_state_elem = (early_async_state_group_row + local_row) * 128 + k_start;
                    asm volatile("cp.async.ca.shared::cta.global [%0], [%1], 16;"
                        :: "r"(sAsyncState_addr + (unsigned int)(early_async_state_elem * 2)), "l"(state + (early_async_state_src_base + early_async_state_source_row * 128)));
                }
            }
        }
    }
    if (tid_0 >= 128) {
        ...
        sGateDecay[gate_smem_idx] = _exp2_0;
        {
            if (warp % 2 != 0) {
                asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                int late_async_target_group = group;
                int late_async_state_group_row = late_async_target_group * 2;
                int late_async_state_src_base = slot * state_slot_stride + head * 128 * 128 + state_owner_row_base * 128 + k_start + 8192;
                #pragma unroll
                for (int local_row_1 = 0; local_row_1 < 2; local_row_1++) {
                    int late_async_state_source_row = local_row_1;
                    int late_async_state_elem = (late_async_state_group_row + local_row_1) * 128 + k_start;
                    asm volatile("cp.async.ca.shared::cta.global [%0], [%1], 16;"
                        :: "r"(sAsyncState_addr + (unsigned int)(late_async_state_elem * 2)), "l"(state + (late_async_state_src_base + late_async_state_source_row * 128)));
                }
            }
        }
    ...
    int state_group_base = state_head_base + state_owner_row_base * 128 + k_start;
    {
        if (warp % 2 == 0) {
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            int rotating_async_target_group = group;
            int rotating_async_state_group_row = rotating_async_target_group * 2;
            int rotating_async_state_src_base = state_head_base + state_owner_row_base * 128 + k_start + 8192;
            #pragma unroll
            for (int rotating_local_row = 0; rotating_local_row < 2; rotating_local_row++) {
                int rotating_async_source_row = rotating_local_row;
                int rotating_async_elem = (rotating_async_state_group_row + rotating_local_row) * 128 + k_start;
                asm volatile("cp.async.ca.shared::cta.global [%0], [%1], 16;"
                    :: "r"(sAsyncState_addr + (unsigned int)(rotating_async_elem * 2)), "l"(state + (rotating_async_state_src_base + rotating_async_source_row * 128)));
            }
            asm volatile("cp.async.commit_group;");
        }
    }
    ...
    for (int local_row_2 = 0; local_row_2 < 4; local_row_2++) {
        {
            const uint4* _vptr_2 = reinterpret_cast<const uint4*>(state + state_group_base + local_row_2 * 128);
        ...
        for (int _pair = 0; _pair < 4; _pair++) {
            asm volatile(
                ...
                "and.b32 %1, %2, 0xffff0000;\n\t"
                "}\n"
                : "=f"((&state_regs[local_row_2 * 8 + _pair * 2])[0]), "=f"((&state_regs[local_row_2 * 8 + _pair * 2])[1])
    ...
    for (int value_tile = 0; value_tile < 2; value_tile++) {
        ...
        if (value_tile > 0) {
            #pragma unroll
            for (int local_row_3 = 0; local_row_3 < 2; local_row_3++) {
                int async_state_local_row = local_row_3;
                int async_state_elem = (group * 2 + local_row_3) * 128 + k_start;
                asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                    ...
                    : "r"(sAsyncState_addr + (unsigned int)(async_state_elem * 2)));
                #pragma unroll
                for (int _pair = 0; _pair < 4; _pair++) {
                    asm volatile(
                        "{\n\t"
            ...
            #pragma unroll
            for (int local_row_4 = 2; local_row_4 < 4; local_row_4++) {
                {
                    const uint4* _vptr_3 = reinterpret_cast<const uint4*>(state + state_tile_base + local_row_4 * 128);
                ...
                for (int _pair = 0; _pair < 4; _pair++) {
                    asm volatile(
                        ...
                        "and.b32 %1, %2, 0xffff0000;\n\t"
                        "}\n"
                        : "=f"((&state_regs[local_row_4 * 8 + _pair * 2])[0]), "=f"((&state_regs[local_row_4 * 8 + _pair * 2])[1])
                        : "r"(state_carriers[_pair]));
                }
            }
        }
        ...
        for (int row_group = 0; row_group < 4 / ((1) ? 4 : 2); row_group++) {
            ...
            float2 state_key_pair_c = _f2_22;
            float2 _f2_23 = make_float2(0.0f, 0.0f);
            float2 state_key_pair_d = _f2_23;
            #pragma unroll
            for (int i_pair_2 = 0; i_pair_2 < 4; i_pair_2++) {
                ...
                float2 _f2_24 = make_float2(r_decay[i0_2], r_decay[i1_2]);
                float2 decay_pair = _f2_24;
                float2 _f2_25 = make_float2(r_k[i0_2], r_k[i1_2]);
                float2 key_pair = _f2_25;
                float2 _f2_26 = make_float2(state_regs[reg_offset_a], state_regs[reg_offset_a + 1]);
                float2 _mul_f32x2_2;
                asm("mul.rn.ftz.f32x2 %0, %1, %2;" : "=l"(*(unsigned long long*)&_mul_f32x2_2) : "l"(*(const unsigned long long*)&_f2_26), "l"(*(const unsigned long long*)&decay_pair));
                float2 state_pair_a = _mul_f32x2_2;
                float2 _f2_27 = make_float2(state_regs[reg_offset_c], state_regs[reg_offset_c + 1]);
                float2 _mul_f32x2_3;
                asm("mul.rn.ftz.f32x2 %0, %1, %2;" : "=l"(*(unsigned long long*)&_mul_f32x2_3) : "l"(*(const unsigned long long*)&_f2_27), "l"(*(const unsigned long long*)&decay_pair));
                ...
                float2 _f2_28 = make_float2(state_regs[reg_offset_b], state_regs[reg_offset_b + 1]);
                float2 _mul_f32x2_4;
                asm("mul.rn.ftz.f32x2 %0, %1, %2;" : "=l"(*(unsigned long long*)&_mul_f32x2_4) : "l"(*(const unsigned long long*)&_f2_28), "l"(*(const unsigned long long*)&decay_pair));
                float2 state_pair_b = _mul_f32x2_4;
                float2 _f2_29 = make_float2(state_regs[reg_offset_d], state_regs[reg_offset_d + 1]);
                float2 _mul_f32x2_5;
                asm("mul.rn.ftz.f32x2 %0, %1, %2;" : "=l"(*(unsigned long long*)&_mul_f32x2_5) : "l"(*(const unsigned long long*)&_f2_29), "l"(*(const unsigned long long*)&decay_pair));
            ...
            float _shfl_xor_10 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_c, 8);
            state_key_dot_c += _shfl_xor_10;
            float _shfl_xor_11 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_d, 8);
            state_key_dot_d += _shfl_xor_11;
            float _shfl_xor_12 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_a, 4);
            state_key_dot_a += _shfl_xor_12;
            float _shfl_xor_13 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_b, 4);
            state_key_dot_b += _shfl_xor_13;
            float _shfl_xor_14 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_c, 4);
            state_key_dot_c += _shfl_xor_14;
            float _shfl_xor_15 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_d, 4);
            ...
            float _shfl_xor_20 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_a, 1);
            state_key_dot_a += _shfl_xor_20;
            float _shfl_xor_21 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_b, 1);
            state_key_dot_b += _shfl_xor_21;
            float _shfl_xor_22 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_c, 1);
            state_key_dot_c += _shfl_xor_22;
            float _shfl_xor_23 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_d, 1);
            ...
            for (int i_pair_3 = 0; i_pair_3 < 4; i_pair_3++) {
                ...
                float2 _f2_34 = make_float2(r_k[i0_3], r_k[i1_3]);
                float2 key_pair_1 = _f2_34;
                float2 _f2_35 = make_float2(r_q[i0_3], r_q[i1_3]);
                float2 query_pair = _f2_35;
                float2 _f2_36 = make_float2(delta_key_scale_a, delta_key_scale_a);
                float2 _f2_37 = make_float2(state_regs[reg_offset_a_1], state_regs[reg_offset_a_1 + 1]);
                float2 updated_pair_a = fma_f32x2_rn_ftz(_f2_36, key_pair_1, _f2_37);
                float2 _f2_38 = make_float2(delta_key_scale_c, delta_key_scale_c);
                float2 _f2_39 = make_float2(state_regs[reg_offset_c_1], state_regs[reg_offset_c_1 + 1]);
                float2 updated_pair_c = fma_f32x2_rn_ftz(_f2_38, key_pair_1, _f2_39);
            ...
            for (int post_i_pair = 0; post_i_pair < 4; post_i_pair++) {
                ...
                float2 _f2_40 = make_float2(r_k[post_i0], r_k[post_i1]);
                float2 post_key_pair = _f2_40;
                float2 _f2_41 = make_float2(r_q[post_i0], r_q[post_i1]);
                float2 post_query_pair = _f2_41;
                float2 _f2_42 = make_float2(delta_key_scale_b, delta_key_scale_b);
                float2 _f2_43 = make_float2(state_regs[post_reg_offset_b], state_regs[post_reg_offset_b + 1]);
                float2 post_updated_pair_b = fma_f32x2_rn_ftz(_f2_42, post_key_pair, _f2_43);
                float2 _f2_44 = make_float2(delta_key_scale_d, delta_key_scale_d);
                float2 _f2_45 = make_float2(state_regs[post_reg_offset_d], state_regs[post_reg_offset_d + 1]);
                float2 post_updated_pair_d = fma_f32x2_rn_ftz(_f2_44, post_key_pair, _f2_45);
            ...
            if (is_live != 0) {
                ...
                {
                    __nv_bfloat162 _pk[4];
                    _pk[0] = __floats2bfloat162_rn(state_regs[local_row_c * 8 + 0], state_regs[local_row_c * 8 + 1]);
                    _pk[1] = __floats2bfloat162_rn(state_regs[local_row_c * 8 + 2], state_regs[local_row_c * 8 + 3]);
                    _pk[2] = __floats2bfloat162_rn(state_regs[local_row_c * 8 + 4], state_regs[local_row_c * 8 + 5]);
                    _pk[3] = __floats2bfloat162_rn(state_regs[local_row_c * 8 + 6], state_regs[local_row_c * 8 + 7]);
                    *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(state))[state_tile_base + local_row_c * 128 + 0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                }
                {
                    __nv_bfloat162 _pk[4];
                    _pk[0] = __floats2bfloat162_rn(state_regs[local_row_d * 8 + 0], state_regs[local_row_d * 8 + 1]);
                    _pk[1] = __floats2bfloat162_rn(state_regs[local_row_d * 8 + 2], state_regs[local_row_d * 8 + 3]);
                    _pk[2] = __floats2bfloat162_rn(state_regs[local_row_d * 8 + 4], state_regs[local_row_d * 8 + 5]);
                    _pk[3] = __floats2bfloat162_rn(state_regs[local_row_d * 8 + 6], state_regs[local_row_d * 8 + 7]);
                    *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(state))[state_tile_base + local_row_d * 128 + 0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                }
            }
            ...
            float _shfl_xor_24 = __shfl_xor_sync(0xFFFFFFFF, recurrence_value_a, 8);
            recurrence_value_a += _shfl_xor_24;
            float _shfl_xor_25 = __shfl_xor_sync(0xFFFFFFFF, recurrence_value_b, 8);
            recurrence_value_b += _shfl_xor_25;
            float _shfl_xor_26 = __shfl_xor_sync(0xFFFFFFFF, recurrence_value_c, 8);
            recurrence_value_c += _shfl_xor_26;
            float _shfl_xor_27 = __shfl_xor_sync(0xFFFFFFFF, recurrence_value_d, 8);
            recurrence_value_d += _shfl_xor_27;
            float _shfl_xor_28 = __shfl_xor_sync(0xFFFFFFFF, recurrence_value_a, 4);
            recurrence_value_a += _shfl_xor_28;
            float _shfl_xor_29 = __shfl_xor_sync(0xFFFFFFFF, recurrence_value_b, 4);
            recurrence_value_b += _shfl_xor_29;
            float _shfl_xor_30 = __shfl_xor_sync(0xFFFFFFFF, recurrence_value_c, 4);
            recurrence_value_c += _shfl_xor_30;
            float _shfl_xor_31 = __shfl_xor_sync(0xFFFFFFFF, recurrence_value_d, 4);
            recurrence_value_d += _shfl_xor_31;
            float _shfl_xor_32 = __shfl_xor_sync(0xFFFFFFFF, recurrence_value_a, 2);
            recurrence_value_a += _shfl_xor_32;
            float _shfl_xor_33 = __shfl_xor_sync(0xFFFFFFFF, recurrence_value_b, 2);
            recurrence_value_b += _shfl_xor_33;
            float _shfl_xor_34 = __shfl_xor_sync(0xFFFFFFFF, recurrence_value_c, 2);
            recurrence_value_c += _shfl_xor_34;
            float _shfl_xor_35 = __shfl_xor_sync(0xFFFFFFFF, recurrence_value_d, 2);
            recurrence_value_d += _shfl_xor_35;
            float _shfl_xor_36 = __shfl_xor_sync(0xFFFFFFFF, recurrence_value_a, 1);
            recurrence_value_a += _shfl_xor_36;
            float _shfl_xor_37 = __shfl_xor_sync(0xFFFFFFFF, recurrence_value_b, 1);
            recurrence_value_b += _shfl_xor_37;
            float _shfl_xor_38 = __shfl_xor_sync(0xFFFFFFFF, recurrence_value_c, 1);
            recurrence_value_c += _shfl_xor_38;
            float _shfl_xor_39 = __shfl_xor_sync(0xFFFFFFFF, recurrence_value_d, 1);
            recurrence_value_d += _shfl_xor_39;
    ...
    if (warp == 0) {
        ...
        float _shfl_xor_40 = __shfl_xor_sync(0xFFFFFFFF, sum_squares, 16);
        sum_squares += _shfl_xor_40;
        float _shfl_xor_41 = __shfl_xor_sync(0xFFFFFFFF, sum_squares, 8);
        sum_squares += _shfl_xor_41;
        float _shfl_xor_42 = __shfl_xor_sync(0xFFFFFFFF, sum_squares, 4);
        sum_squares += _shfl_xor_42;
        float _shfl_xor_43 = __shfl_xor_sync(0xFFFFFFFF, sum_squares, 2);
        sum_squares += _shfl_xor_43;
        float _shfl_xor_44 = __shfl_xor_sync(0xFFFFFFFF, sum_squares, 1);
