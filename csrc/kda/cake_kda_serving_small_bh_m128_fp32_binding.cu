...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_kda_serving_small_bh_m128_fp32'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
// 2D TMA descriptor for buffer 'packet_workspace' — compiled from the
// descriptor's std.Expr global_dim/global_strides/checks record.
inline CUtensorMap EncodeTma_packet_workspace(const TensorView& t) {
  TVM_FFI_CHECK(t.ndim() >= 2, ValueError)
      << "TMA source 'packet_workspace' must have at least 2 dimensions, got ndim=" << t.ndim();
  TVM_FFI_CHECK(t.stride(-1) == 1, ValueError)
      << "TMA source 'packet_workspace' must have unit innermost stride, got " << t.stride(-1);
  int64_t d1 = t.size(t.ndim() - 1);
  TVM_FFI_CHECK(d1 > 0, ValueError)
      << "TMA source 'packet_workspace' trailing dims must be positive";
  int64_t outer1 = t.numel() / (d1);
  CheckDenseLeadingFold(t, 1, "packet_workspace");
  int64_t s2 = t.stride(t.ndim() - 2) * 1;
  TVM_FFI_CHECK(s2 > 0, ValueError)
      << "TMA source 'packet_workspace' physical strides must be positive";
  uint64_t global_dim[2] = {(uint64_t)(d1), (uint64_t)(outer1)};
  TVM_FFI_CHECK(global_dim[0] > 0 && global_dim[1] > 0, ValueError)
      << "TMA descriptor for 'packet_workspace' resolved a non-positive global dim";
  TVM_FFI_CHECK(128u <= global_dim[0] && 123u <= global_dim[1], ValueError)
      << "TMA box (128, 123) exceeds resolved global dims for 'packet_workspace'";
  uint64_t global_strides[1] = {
      (uint64_t)((s2 * 16) / 8),
  };
  uint32_t box_dim[2] = {128u, 123u};
  uint32_t elem_strides[2] = {1u, 1u};
  CUtensorMap tm{};
  CUresult r = cuTensorMapEncodeTiled(
      &tm, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, 2, t.data_ptr(), global_dim, global_strides, box_dim, elem_strides,
      ...
      CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
  TVM_FFI_CHECK(r == CUDA_SUCCESS, RuntimeError)
      << "cuTensorMapEncodeTiled (2D, 'packet_workspace') failed: CUresult=" << (int)r;
  return tm;
}

void Run(TensorView arg_q, TensorView arg_q_tma, TensorView arg_k, TensorView arg_k_tma, TensorView arg_v, TensorView arg_v_tma, TensorView arg_g, TensorView arg_g_tma, TensorView arg_beta, TensorView arg_beta_tma, TensorView arg_A_log, TensorView arg_dt_bias, TensorView arg_cu_seqlens, TensorView arg_seq_order, TensorView arg_initial_state, TensorView arg_out, TensorView arg_out_tma, TensorView arg_final_state, int64_t arg_num_heads, int64_t arg_use_initial_state, int64_t arg_store_final_state, double arg_scale, double arg_lower_bound, int64_t arg_state_indices_addr, int64_t arg_state_checkpoints_addr, int64_t arg_checkpoint_cu_starts_addr, int64_t arg_beta_token_stride, int64_t arg_state_slot_stride, int64_t arg_use_state_indices, int64_t arg_checkpoint_every_n_tokens, TensorView arg_packet_workspace, TensorView arg_packet_ready, TensorView arg_packet_consumed, TensorView arg_helper_done, TensorView arg_initial_state_f32, TensorView arg_final_state_f32, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  TVM_FFI_CHECK(arg_checkpoint_every_n_tokens >= -2147483648LL && arg_checkpoint_every_n_tokens <= 2147483647LL, ValueError)
      ...
      << " is outside i32 range [-2147483648, 2147483647]";
  CheckCudaTensor(arg_packet_workspace, "packet_workspace");
  CheckDtype(arg_packet_workspace, "packet_workspace", 4, 16, 1);
  CheckCudaTensor(arg_packet_ready, "packet_ready");
  CheckDtype(arg_packet_ready, "packet_ready", 1, 32, 1);
  CheckContiguous(arg_packet_ready, "packet_ready");
  CheckCudaTensor(arg_packet_consumed, "packet_consumed");
  CheckDtype(arg_packet_consumed, "packet_consumed", 1, 32, 1);
  CheckContiguous(arg_packet_consumed, "packet_consumed");
  CheckCudaTensor(arg_helper_done, "helper_done");
  CheckDtype(arg_helper_done, "helper_done", 1, 32, 1);
  CheckContiguous(arg_helper_done, "helper_done");
  ...
  CheckSameCudaDevice(arg_packet_workspace, arg_q, "packet_workspace", "q");
  CheckSameCudaDevice(arg_packet_ready, arg_q, "packet_ready", "q");
  CheckSameCudaDevice(arg_packet_consumed, arg_q, "packet_consumed", "q");
  CheckSameCudaDevice(arg_helper_done, arg_q, "helper_done", "q");
  ...
  CUtensorMap p_packet_workspace = EncodeTma_packet_workspace(arg_packet_workspace);
  unsigned int* p_packet_ready = static_cast<unsigned int*>(arg_packet_ready.data_ptr());
  unsigned int* p_packet_consumed = static_cast<unsigned int*>(arg_packet_consumed.data_ptr());
  unsigned int* p_helper_done = static_cast<unsigned int*>(arg_helper_done.data_ptr());
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_kda_serving_small_bh_m128_fp32),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_kda_serving_small_bh_m128_fp32 failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_kda_serving_small_bh_m128_fp32), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_kda_serving_small_bh_m128_fp32 failed: "
