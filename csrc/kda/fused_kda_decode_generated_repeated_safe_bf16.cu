...
#define SMEM_SBETA_STAGE_BYTES 4
#define SMEM_SBETA_STRIDE 4
...
#define SMEM_SSCHEDULER_OFF 3588
#define SMEM_SSCHEDULER_STAGE_BYTES 12
#define SMEM_SSCHEDULER_STRIDE 12
...
kernel_kernel(__nv_bfloat16* __restrict__ x, float* __restrict__ weight, __nv_bfloat16* __restrict__ conv_state, __nv_bfloat16* __restrict__ raw_gate, __nv_bfloat16* __restrict__ raw_beta, float* __restrict__ A_log, float* __restrict__ dt_bias, int* __restrict__ state_indices, __nv_bfloat16* __restrict__ state, __nv_bfloat16* __restrict__ output_gate, float* __restrict__ norm_weight, __nv_bfloat16* __restrict__ output, int x_row_stride, int conv_slot_stride, int beta_row_stride, int state_slot_stride, int output_gate_row_stride, int H, int rows, int use_lower_bound, float lower_bound_log2, float norm_eps)
{
    ...
    int* sScheduler = reinterpret_cast<int*>(smem_raw + 3588);
    const int sScheduler_addr = smem + 3588;

    // === Task calls (dependency order) ===
    int origin_row = blockIdx.y;
    int head = blockIdx.x;
    int tid_0 = tid;
    int lane_1 = lane;
    int group = tid_0 / 16;
    ...
    int lane_requested_slot = 0;
    if (warp == 0) {
        if (lane_1 == 0) {
            lane_requested_slot = state_indices[origin_row];
        }
        int _shfl_0 = __shfl_sync(0xFFFFFFFF, lane_requested_slot, 0);
        lane_requested_slot = _shfl_0;
        int scanned_first_row = rows;
        int scanned_match_count = 0;
        #pragma unroll 1
        for (int scan_base = 0; scan_base < rows; scan_base += 32) {
            int scan_row = scan_base + lane_1;
            int scan_slot = 0;
            if (scan_row < rows) {
                scan_slot = state_indices[scan_row];
            }
            unsigned int _vote_0 = __ballot_sync(0xFFFFFFFF, lane_requested_slot > 0 && scan_row < rows && scan_slot == lane_requested_slot);
            unsigned int match_mask = _vote_0;
            if (scanned_first_row == rows && match_mask != 0) {
                int _ffs_0 = __ffs(match_mask);
                scanned_first_row = scan_base + _ffs_0 - 1;
            }
            int _popc_0 = __popc(match_mask);
            scanned_match_count += _popc_0;
        }
        if (lane_requested_slot <= 0) {
            scanned_first_row = origin_row;
            scanned_match_count = 1;
        }
        if (lane_1 == 0) {
            sScheduler[0] = lane_requested_slot;
            sScheduler[1] = scanned_first_row;
            sScheduler[2] = scanned_match_count;
        }
    }
    __syncthreads();
    int requested_slot = make_warp_uniform(sScheduler[0]);
    int first_row = make_warp_uniform(sScheduler[1]);
    int match_count = make_warp_uniform(sScheduler[2]);
    __syncthreads();
    int chain_iterations = make_warp_uniform(0);
    int slot = make_warp_uniform(0);
    int is_live = make_warp_uniform(0);
    if (requested_slot <= 0) {
        chain_iterations = 1;
    } else if (origin_row == first_row) {
        chain_iterations = match_count;
        slot = requested_slot;
        is_live = 1;
    }
    int row = make_warp_uniform(origin_row);
    #pragma unroll 1
    for (int chain_iteration = 0; chain_iteration < chain_iterations; chain_iteration++) {
        int writes_state = make_warp_uniform(0);
        if (is_live != 0) {
            if (chain_iteration + 1 == chain_iterations) {
                writes_state = 1;
            }
        }
        float state_regs[32];
        ...
        int state_group_base = state_head_base + group * 4 * 128 + k_start;
        #pragma unroll
        ...
        float _shfl_1 = __shfl_sync(0xFFFFFFFF, q_scale, lane_group);
        q_scale = _shfl_1;
        float _shfl_2 = __shfl_sync(0xFFFFFFFF, k_scale, lane_group);
        k_scale = _shfl_2;
        #pragma unroll
        for (int i_1 = 0; i_1 < 8; i_1++) {
            float _shfl_3 = __shfl_sync(0xFFFFFFFF, r_q[i_1], lane_group);
            r_q[i_1] = _shfl_3;
            float _shfl_4 = __shfl_sync(0xFFFFFFFF, r_k[i_1], lane_group);
            r_k[i_1] = _shfl_4;
        }
        float beta = sBeta[0];
        #pragma unroll
        ...
        for (int value_tile = 0; value_tile < 2; value_tile++) {
            ...
            if (value_tile > 0) {
                ...
                for (int local_row_1 = 0; local_row_1 < 4; local_row_1++) {
                    {
                        const uint4* _vptr_3 = reinterpret_cast<const uint4*>(state + state_tile_base + local_row_1 * 128);
                    ...
                    for (int _pair = 0; _pair < 4; _pair++) {
                        asm volatile(
                            ...
                            "and.b32 %1, %2, 0xffff0000;\n\t"
                            "}\n"
                            : "=f"((&state_regs[local_row_1 * 8 + _pair * 2])[0]), "=f"((&state_regs[local_row_1 * 8 + _pair * 2])[1])
                            : "r"(state_carriers[_pair]));
                    }
                }
            }
            ...
            for (int row_group = 0; row_group < 4 / ((0) ? 4 : 2); row_group++) {
                ...
                int value_row_a = tile_base + group * 4 + local_row_a;
                int value_row_b = tile_base + group * 4 + local_row_b;
                float value_a = sMixed[384 + value_row_a];
                float value_b = sMixed[384 + value_row_b];
        ...
        if (chain_iterations > 1 && chain_iterations > chain_iteration + 1) {
            int next_row = rows;
            if (warp == 0) {
                #pragma unroll 1
                for (int successor_base = 0; successor_base < rows; successor_base += 32) {
                    int successor_row = successor_base + lane_1;
                    int successor_slot = 0;
                    if (successor_row < rows) {
                        successor_slot = state_indices[successor_row];
                    }
                    unsigned int _vote_1 = __ballot_sync(0xFFFFFFFF, successor_row > row && successor_row < rows && successor_slot == requested_slot);
                    unsigned int successor_mask = _vote_1;
                    if (next_row == rows && successor_mask != 0) {
                        int _ffs_1 = __ffs(successor_mask);
                        next_row = successor_base + _ffs_1 - 1;
                    }
                }
                if (lane_1 == 0) {
                    sScheduler[0] = next_row;
                }
            }
            __syncthreads();
            row = sScheduler[0];
