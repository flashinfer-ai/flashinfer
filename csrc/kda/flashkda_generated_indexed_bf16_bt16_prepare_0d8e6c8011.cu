...
kernel_flashkda_bf16_bt16_prepare(__nv_bfloat16* __restrict__ q, CudaTensorMap const* q_tma, __nv_bfloat16* __restrict__ k, CudaTensorMap const* k_tma, __nv_bfloat16* __restrict__ raw_gate, CudaTensorMap const* raw_gate_tma, __nv_bfloat16* __restrict__ beta_logits, CudaTensorMap const* beta_logits_tma, float* __restrict__ a_log, float* __restrict__ dt_bias, long long* __restrict__ cu_seqlens, int* __restrict__ cu_chunks, int* __restrict__ chunk_to_seq, __nv_bfloat16* __restrict__ ws_qd, CudaTensorMap const* ws_qd_tma, __nv_bfloat16* __restrict__ ws_kd, CudaTensorMap const* ws_kd_tma, __nv_bfloat16* __restrict__ ws_w, CudaTensorMap const* ws_w_tma, __nv_bfloat16* __restrict__ ws_qk_t, float* __restrict__ ws_diag, int total_chunks, int num_heads, float gate_lower_bound)
{
    ...
    if (warp == 3) {
        uint64_t __cuda_tensormap_acquire_addr = (uint64_t)(q_tma);
        if (lane == 1) __cuda_tensormap_acquire_addr = (uint64_t)(k_tma);
        if (lane == 2) __cuda_tensormap_acquire_addr = (uint64_t)(raw_gate_tma);
        if (lane == 3) __cuda_tensormap_acquire_addr = (uint64_t)(beta_logits_tma);
        if (lane == 4) __cuda_tensormap_acquire_addr = (uint64_t)(ws_qd_tma);
        if (lane == 5) __cuda_tensormap_acquire_addr = (uint64_t)(ws_kd_tma);
        if (lane == 6) __cuda_tensormap_acquire_addr = (uint64_t)(ws_w_tma);
        if (lane < 7) {
            asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" :: "l"(__cuda_tensormap_acquire_addr) : "memory");
        }
    }
