...
void Run(TensorView q, TensorView k, TensorView v, TensorView g, TensorView beta,
  ...
  const flash_kda::TmaPointers tma = flash_kda::EncodeTmaPointers<kValueRows, 32>(
      q, k, v, g, beta_tma, out, descriptor_storage, prepare_descriptors, stream);
  flash_kda::PackBetaForTmaIfNeeded(beta, beta_tma, num_heads,
                                    beta.stride(beta.ndim() - 2), stream);
