...
#define uint8_t cake_kda_bt64_generated_uint8_t
#define uint16_t cake_kda_bt64_generated_uint16_t
#define uint32_t cake_kda_bt64_generated_uint32_t
#define uint64_t cake_kda_bt64_generated_uint64_t
#define int32_t cake_kda_bt64_generated_int32_t
#define int16_t cake_kda_bt64_generated_int16_t
#define CakeKDATensorMap cake_kda_bt64_generated_CakeKDATensorMap
#define CakeKDATensorMapPack cake_kda_bt64_generated_CakeKDATensorMapPack
#define CUtensorMap cake_kda_bt64_generated_CUtensorMap
#include "cake_kda_bf16_fused_m128_bt64_unbounded_softplus.cu"
#undef CUtensorMap
...
void RunM128BT64UnboundedSoftplus(TensorView q, TensorView k, TensorView v, TensorView g,
  ...
  TVM_FFI_ICHECK(num_heads == 4)
      << "checkpoint-aligned BT64 CakeKDA requires exactly four local heads";
  TVM_FFI_ICHECK(checkpoint_every_n_tokens > 0 && checkpoint_every_n_tokens % 64 == 0)
      << "checkpoint-aligned BT64 CakeKDA requires a nonzero checkpoint cadence divisible by 64";
  ...
  TVM_FFI_ICHECK(grid_x_i64 > 0 && grid_x_i64 <= std::numeric_limits<uint32_t>::max())
      << "unbounded-softplus BT64 M128 CakeKDA grid.x is out of range: " << grid_x_i64;
  ...
  const TmaPointers tma = EncodeTmaPointers<128, 64>(q, k, v, g, beta_tma, out, descriptor_storage,
  ...
  kernel_cake_kda_bf16_fused_m128<<<grid, block, kSmemBytes, stream>>>(
      ...
      reinterpret_cast<cake_kda_bt64_generated_CakeKDATensorMap const*>(tma.q),
      reinterpret_cast<__nv_bfloat16*>(k.data_ptr()),
      reinterpret_cast<cake_kda_bt64_generated_CakeKDATensorMap const*>(tma.k),
      reinterpret_cast<__nv_bfloat16*>(v.data_ptr()),
      reinterpret_cast<cake_kda_bt64_generated_CakeKDATensorMap const*>(tma.v),
      reinterpret_cast<__nv_bfloat16*>(g.data_ptr()),
      reinterpret_cast<cake_kda_bt64_generated_CakeKDATensorMap const*>(tma.g),
      reinterpret_cast<__nv_bfloat16*>(beta.data_ptr()),
      reinterpret_cast<cake_kda_bt64_generated_CakeKDATensorMap const*>(tma.beta),
      ...
      reinterpret_cast<cake_kda_bt64_generated_CakeKDATensorMap const*>(tma.out),
  ...
  CheckCuda(cudaGetLastError(), "unbounded-softplus BT64 M128 CakeKDA launch");
}

}  // namespace cake_kda
}  // namespace flashinfer

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, flashinfer::cake_kda::RunM128BT64UnboundedSoftplus);
