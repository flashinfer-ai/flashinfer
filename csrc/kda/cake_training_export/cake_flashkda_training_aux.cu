...
kernel_flashkda_refine_forgetting_horizons(__nv_bfloat16* __restrict__ g, float* __restrict__ A_log, float* __restrict__ dt_bias, int* __restrict__ work_items, int* __restrict__ boundaries, unsigned int* __restrict__ persistent_counters, int num_heads, float lower_bound, float log2_threshold)
{
        ...
        if (warmup[0] == 0) {
            ...
            if (left_chunk >= 0) {
                ...
                for (int sample = 0; sample < 4; sample++) {
                    ...
                    if (left_token < eos) {
                        long long left_index = ((long long)left_token * (long long)num_heads + (long long)head) * 128 + (long long)channel;
                        float _tanh_approx_0;
                        asm volatile("tanh.approx.f32 %0, %1;" : "=f"(_tanh_approx_0) : "f"(gate_rate * ((float)g[left_index] + gate_bias) * 0.5f));
        ...
        if (warmup[1] == 0) {
            ...
            if (right_chunk < num_chunks) {
                ...
                for (int sample_1 = 0; sample_1 < 4; sample_1++) {
                    ...
                    if (right_token < eos) {
                        long long right_index = ((long long)right_token * (long long)num_heads + (long long)head) * 128 + (long long)channel;
                        float _tanh_approx_1;
                        asm volatile("tanh.approx.f32 %0, %1;" : "=f"(_tanh_approx_1) : "f"(gate_rate * ((float)g[right_index] + gate_bias) * 0.5f));
