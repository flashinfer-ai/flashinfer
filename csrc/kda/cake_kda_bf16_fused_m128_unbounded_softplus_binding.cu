...
void RunM128UnboundedSoftplus(TensorView q, TensorView k, TensorView v, TensorView g,
  ...
  const int64_t num_seqs = CheckCommonInputs(
      ...
      store_final_state, scale, lower_bound, true, state_pool_slots, true);
  ...
  CheckCuda(cudaFuncSetAttribute(kernel_cake_kda_bf16_fused_m128,
                                 cudaFuncAttributeMaxDynamicSharedMemorySize, kSmemBytes),
            "cudaFuncSetAttribute(kernel_cake_kda_bf16_fused_m128)");

  const int64_t grid_x_i64 = num_seqs * num_heads;
  TVM_FFI_ICHECK(grid_x_i64 > 0 && grid_x_i64 <= std::numeric_limits<uint32_t>::max())
      << "unbounded-softplus M128 CakeKDA grid.x is out of range: " << grid_x_i64;
  ...
  CheckCuda(cudaGetLastError(), "unbounded-softplus M128 CakeKDA launch");
}

}  // namespace cake_kda
}  // namespace flashinfer

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, flashinfer::cake_kda::RunM128UnboundedSoftplus);
