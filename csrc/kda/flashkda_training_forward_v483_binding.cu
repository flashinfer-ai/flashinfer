...
#define uint8_t flashkda_training_generated_uint8_t
#define uint16_t flashkda_training_generated_uint16_t
#define uint32_t flashkda_training_generated_uint32_t
#define uint64_t flashkda_training_generated_uint64_t
#define int32_t flashkda_training_generated_int32_t
#define int16_t flashkda_training_generated_int16_t
#define FlashKDATensorMap flashkda_training_generated_FlashKDATensorMap
#define FlashKDATensorMapPack flashkda_training_generated_FlashKDATensorMapPack
#define CUtensorMap flashkda_training_generated_CUtensorMap
#include "flashkda_training_forward_v483.cu"
#undef CUtensorMap
...
void RunForward(TensorView q, TensorView k, TensorView v, TensorView g, TensorView beta,
  ...
  if (prepare_descriptors != 0) {
    ...
    TVM_FFI_ICHECK(capture_status == cudaStreamCaptureStatusNone)
        << "training-forward descriptors must be warmed before CUDA graph capture";
  ...
  kernel_flashkda_forward_checkpoint_c16<<<grid, 512, kSmemBytes, stream>>>(
      ...
      reinterpret_cast<flashkda_training_generated_FlashKDATensorMap const*>(descriptor_bytes +
                                                                             0 * stride),
      reinterpret_cast<flashkda_training_generated_FlashKDATensorMap const*>(descriptor_bytes +
                                                                             1 * stride),
      reinterpret_cast<flashkda_training_generated_FlashKDATensorMap const*>(descriptor_bytes +
      ...
      reinterpret_cast<__nv_bfloat16*>(final_state_bf16.data_ptr()), kWorkItems, 1, kHeads, 16,
