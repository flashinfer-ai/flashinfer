...
kernel_flashkda_blackwell_evolution_m128_h64_p1_s114(__nv_bfloat16* __restrict__ q, CakeTensorMap const* q_tma, __nv_bfloat16* __restrict__ k, CakeTensorMap const* k_tma, __nv_bfloat16* __restrict__ v, CakeTensorMap const* v_tma, __nv_bfloat16* __restrict__ g, CakeTensorMap const* g_tma, __nv_bfloat16* __restrict__ beta, CakeTensorMap const* beta_tma, float* __restrict__ A_log, float* __restrict__ dt_bias, long long* __restrict__ cu_seqlens, int* __restrict__ seq_order, int* __restrict__ tile_schedule, int* __restrict__ tile_schedule_counts, __nv_bfloat16* __restrict__ initial_state, __nv_bfloat16* __restrict__ out, CakeTensorMap const* out_tma, __nv_bfloat16* __restrict__ final_state, int num_heads, int use_initial_state, int store_final_state, float scale, float lower_bound)
{
        ...
        { // compute_main
            ...
            for (int global_chunk_idx = 0; global_chunk_idx < total_chunks; global_chunk_idx++) {
                ...
                long long current_state_base = state_base;
                {
                    ...
                    current_state_base = (((long long)seq_idx * (long long)NUM_HEADS + (long long)head_idx) * 128 + (long long)state_row) * 128;
                    if (global_chunk_idx != 0 && chunk_idx == 0) {
                        ...
                        for (int state_col_block_1 = 0; state_col_block_1 < 4; state_col_block_1++) {
                            ...
                            {
                                {
                                    const uint4* _vptr_4 = reinterpret_cast<const uint4*>(initial_state + current_state_base + (long long)(state_col_block_1 * 32));
                                    uint4 _vld_4[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
                                        ...
                                        for (int _pair = 0; _pair < 4; _pair++) {
                                            asm volatile(
                                                ...
                                                "and.b32 %1, %2, 0xffff0000;\n\t"
                                                "}\n"
                                                : "=f"((&state_frag_1[0 + _blk * 8 + _pair * 2])[0]), "=f"((&state_frag_1[0 + _blk * 8 + _pair * 2])[1])
                                                : "r"(_vpairs_4[_pair]));
                                        }
                                    }
                                }
                                {
                                    const uint4* _vptr_5 = reinterpret_cast<const uint4*>(initial_state + current_state_base + (long long)(state_col_block_1 * 32) + 16);
                                    uint4 _vld_5[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
                                        ...
                                        for (int _pair = 0; _pair < 4; _pair++) {
                                            asm volatile(
                                                ...
                                                "and.b32 %1, %2, 0xffff0000;\n\t"
                                                "}\n"
                                                : "=f"((&state_frag_1[16 + _blk * 8 + _pair * 2])[0]), "=f"((&state_frag_1[16 + _blk * 8 + _pair * 2])[1])
                                                : "r"(_vpairs_5[_pair]));
                                        }
                                    }
                                }
                ...
                if (lane < 16) {
                    ...
                    {
                        __nv_bfloat16 _bval_6 = __float2bfloat16_rn(diag_scale_0);
                        uint16_t _bits_6 = *(uint16_t*)&_bval_6;
                        uint32_t _addr_6 = static_cast<uint32_t>((diag_base_0 + (lane / 16 * 512 + lane * 32 + lane % 16 * 2 ^ (lane / 16 * 512 + lane * 32 + lane % 16 * 2 >> 7 & 1) << 4)));
                        asm volatile("st.shared.b16 [%0], %1;" :: "r"(_addr_6), "h"(_bits_6) : "memory");
                    }
                    {
                        __nv_bfloat16 _bval_7 = __float2bfloat16_rn(diag_scale_1);
                        uint16_t _bits_7 = *(uint16_t*)&_bval_7;
                        uint32_t _addr_7 = static_cast<uint32_t>((diag_base_1 + (lane / 16 * 512 + lane * 32 + lane % 16 * 2 ^ (lane / 16 * 512 + lane * 32 + lane % 16 * 2 >> 7 & 1) << 4)));
                ...
                for (int residual_half = 0; residual_half < 2; residual_half++) {
                    ...
                    int residual_beta_addr = smem_qd_addr + compute_stage * 41984 + 41488 + (unsigned int)(residual_half * 16 * 4);
                    #pragma unroll
                    for (int residual_beta_vec = 0; residual_beta_vec < 4; residual_beta_vec++) {
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                            : "=r"(*reinterpret_cast<uint32_t*>(&residual_beta[residual_beta_vec * 4])), "=r"(*reinterpret_cast<uint32_t*>(&residual_beta[(residual_beta_vec * 4) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&residual_beta[(residual_beta_vec * 4) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&residual_beta[(residual_beta_vec * 4) + 3]))
                            : "r"(residual_beta_addr + residual_beta_vec * 16));
                    }
                    #pragma unroll
                    for (int residual_col = 0; residual_col < 16; residual_col++) {
                        int token_col = residual_half * 16 + residual_col;
                        __nv_bfloat16 v_value = smem_v[token_col * 128 + state_row];
                ...
                if (PERSISTENT_SCHEDULE && STORE_FINAL_STATE && chunk_idx + 1 == num_chunks) {
                    ...
                    for (int state_col_block_3 = 0; state_col_block_3 < 4; state_col_block_3++) {
                            ...
                            : "r"(taddr + 64 + (unsigned int)tmem_row_base + (unsigned int)(state_col_block_3 * 32)));
                        {
                            ...
                            *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(final_state + (current_state_base + (long long)(state_col_block_3 * 32))))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                            *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(final_state + (current_state_base + (long long)(state_col_block_3 * 32))))[8]) = *reinterpret_cast<uint4*>(&_pk[4]);
                        }
                        {
                            __nv_bfloat162 _pk[8];
                            ...
                            *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(final_state + (current_state_base + (long long)(state_col_block_3 * 32) + 16)))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                            *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(final_state + (current_state_base + (long long)(state_col_block_3 * 32) + 16)))[8]) = *reinterpret_cast<uint4*>(&_pk[4]);
                        }
                    }
                }
                compute_stage += 1;
    ...
    if (warp == 9) {
        { // mma_main
            ...
            for (int _global_chunk_idx = 0; _global_chunk_idx < total_chunks_2; _global_chunk_idx++) {
                ...
                {
                    int _mma_b_lo_2 = make_warp_uniform((((smem_qd_addr) >> 4) & 0x3FFF) + (mma_stage) * 2624);
                    asm volatile(
                    "{\n\t"
                    ...
                    int _mma_b_lo_3 = make_warp_uniform((((smem_kd_addr) >> 4) & 0x3FFF) + (mma_stage) * 2624);
                    asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 dhi, blo, ta, id;\n\t"
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f16 [%0], [ta], db, id, p1;\n\t"
                    "}\n"
                    :: "r"(tmem_tmem_u_acc), "r"(_mma_b_lo_3), "r"(tmem_tmem_state_inp), "r"(0));
                ...
                int _mma_b_lo_4 = make_warp_uniform(((((smem_state_diag0_addr) >> 4) & 0x3FFF) | 0x200000) + (0) * 32);
                mma_ts_step(tmem_tmem_state, tmem_tmem_state_inp, _mma_b_lo_4, 0xC0004010, 134546576, 0);
                int _mma_b_lo_5 = make_warp_uniform(((((smem_state_diag1_addr) >> 4) & 0x3FFF) | 0x200000) + (0) * 32);
                mma_ts_step((tmem_tmem_state + (16)), tmem_tmem_state_inp + 8, _mma_b_lo_5, 0xC0004010, 134546576, 0);
                int _mma_b_lo_6 = make_warp_uniform(((((smem_state_diag2_addr) >> 4) & 0x3FFF) | 0x200000) + (0) * 32);
                mma_ts_step((tmem_tmem_state + (32)), tmem_tmem_state_inp + 16, _mma_b_lo_6, 0xC0004010, 134546576, 0);
                int _mma_b_lo_7 = make_warp_uniform(((((smem_state_diag3_addr) >> 4) & 0x3FFF) | 0x200000) + (0) * 32);
                mma_ts_step((tmem_tmem_state + (48)), tmem_tmem_state_inp + 24, _mma_b_lo_7, 0xC0004010, 134546576, 0);
                int _mma_b_lo_8 = make_warp_uniform(((((smem_state_diag4_addr) >> 4) & 0x3FFF) | 0x200000) + (0) * 32);
                mma_ts_step((tmem_tmem_state + (64)), tmem_tmem_state_inp + 32, _mma_b_lo_8, 0xC0004010, 134546576, 0);
                int _mma_b_lo_9 = make_warp_uniform(((((smem_state_diag5_addr) >> 4) & 0x3FFF) | 0x200000) + (0) * 32);
                mma_ts_step((tmem_tmem_state + (80)), tmem_tmem_state_inp + 40, _mma_b_lo_9, 0xC0004010, 134546576, 0);
                int _mma_b_lo_10 = make_warp_uniform(((((smem_state_diag6_addr) >> 4) & 0x3FFF) | 0x200000) + (0) * 32);
                mma_ts_step((tmem_tmem_state + (96)), tmem_tmem_state_inp + 48, _mma_b_lo_10, 0xC0004010, 134546576, 0);
                int _mma_b_lo_11 = make_warp_uniform(((((smem_state_diag7_addr) >> 4) & 0x3FFF) | 0x200000) + (0) * 32);
                mma_ts_step((tmem_tmem_state + (112)), tmem_tmem_state_inp + 56, _mma_b_lo_11, 0xC0004010, 134546576, 0);
                mbarrier_wait(u_inp_ready_addr + (mma_stage) * 8, _phase_u_inp_ready);
                int _mma_b_lo_12 = make_warp_uniform((((smem_inv_addr) >> 4) & 0x3FFF) + (mma_stage) * 2624);
                asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 dhi, blo, ta, id;\n\t"
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f16 [%0], [ta], db, id, p1;\n\t"
                    "}\n"
                    :: "r"(tmem_tmem_u2_acc), "r"(_mma_b_lo_12), "r"(tmem_tmem_u2_inp), "r"(0));
                elect_commit(u2_acc_ready_addr + (mma_stage) * 8);
                mbarrier_wait(u2_inp_ready_addr + (mma_stage) * 8, _phase_u2_inp_ready);
                int _mma_b_lo_13 = make_warp_uniform(((((smem_final_trans_addr) >> 4) & 0x3FFF) | 0x1000000) + (mma_stage) * 2624);
                asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 dhi, blo, ta, id;\n\t"
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f16 [%0], [ta], db, id, p1;\n\t"
                    "}\n"
                    :: "r"(tmem_tmem_state_out), "r"(_mma_b_lo_13), "r"(tmem_tmem_u2_inp), "r"(1));
            ...
            for (int global_chunk_idx_2 = 0; global_chunk_idx_2 < total_chunks_3; global_chunk_idx_2++) {
                ...
                if (elect_sync()) {
                    if (chunk_is_full_1 != 0) {
                        ...
                        tma_3d_gmem2smem(smem_v_addr, v_tma, 0, head_idx_2, (int)(bos_3 + (long long)(chunk_idx_2 * 32)), v_full_addr);
                    }
                }
                if (chunk_is_full_1 == 0) {
                    #pragma unroll
                    for (int v_load_iter = 0; v_load_iter < 16; v_load_iter++) {
                        ...
                        asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16, %2;"
                            :: "r"(smem_v_addr + (unsigned int)((row * 128 + segment * 8) * 2)), "l"(v + v_src), "r"((token_valid != 0) ? 16 : 0));
                    }
    ...
    if (warp >= 12 && warp <= 31) {
        ...
        { // prep_main
            ...
            for (int prep_iter = 0; prep_iter < num_prep_iters; prep_iter++) {
                ...
                } else if (PERSISTENT_SCHEDULE && prep_local_warp == 0) {
                    if (elect_sync()) {
                    ...
                    if (chunk_is_full_2 != 0) {
                        ...
                        for (int gate_block_idx = 0; gate_block_idx < 4; gate_block_idx++) {
                            ...
                            for (int gate_row_in_block = 0; gate_row_in_block < 8; gate_row_in_block++) {
                                ...
                                if (1 + gate_block_idx * 8 + gate_row_in_block < 32) {
                                    ...
                                    gate_block[gate_row_in_block] = specialized_lower_bound * 1.4426950408889634f * gate_sigmoid;
                                }
                            }
                        ...
                        for (int gate_row = 0; gate_row < 32; gate_row++) {
                            ...
                            if (gate_token < eos_4) {
                                ...
                                float gate_sigmoid_1 = _tanh_approx_4 * 0.5f + 0.5f;
                                gate_log2 = specialized_lower_bound * 1.4426950408889634f * gate_sigmoid_1;
                            }
                ...
                if (prep_tid < 128) {
                    int segment_3 = prep_tid / 8;
                    int elem_3 = prep_tid & 7;
                    int half_3 = elem_3 / 4;
                    int vector_idx_2 = segment_3 * 2 + half_3;
                    int cycle_2 = vector_idx_2 / 8;
                    int group_2 = (segment_3 & 3) + (segment_3 / 4 + half_3 & 1) * 4;
                    float total_log2 = smem_gate_all[stage_f32 + 3968 + (cycle_2 * 32 + group_2 * 4 + (elem_3 & 3))];
                    float _exp2_0 = approx_exp2(total_log2 - specialized_lower_bound * 1.4426950408889634f * 16.0f);
                    smem_restore_factor_all[stage_f32 + prep_tid] = _exp2_0;
                }
                if (prep_tid == 0) {
                    float _exp2_1 = approx_exp2(specialized_lower_bound * 1.4426950408889634f * 16.0f);
                ...
                for (int work_pass = 0; work_pass < 4; work_pass++) {
                    ...
                    int segment_4 = work_item % 16;
                    long long token_1 = bos_4 + (long long)(chunk_idx_3 * 32 + row_1);
                    int token_valid_1 = ((token_1 < eos_4) ? 1 : 0);
                    long long gmem_base = (token_1 * (long long)NUM_HEADS + (long long)head_idx_3) * 128 + (long long)(segment_4 * 8);
                    float q_raw_vec[8];
                    float k_raw_vec[8];
                    q_raw_vec[0] = 0.0f;
                    q_raw_vec[1] = 0.0f;
                    ...
                    if (chunk_is_full_2 != 0) {
                        ...
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                            ...
                            : "r"((smem_q_raw_prefetch_addr + prep_stage * 41984 + (unsigned int)(segment_4 * 8 / 64 * 4096 + row_1 * 128 + segment_4 * 8 % 64 * 2 ^ (segment_4 * 8 / 64 * 4096 + row_1 * 128 + segment_4 * 8 % 64 * 2 >> 7 & 7) << 4))));
                        float packed_f32[8];
                        #pragma unroll
                        for (int _pair = 0; _pair < 4; _pair++) {
                            asm volatile(
                        ...
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                            ...
                            : "r"((smem_kd_addr + prep_stage * 41984 + (unsigned int)(segment_4 * 8 / 64 * 4096 + row_1 * 128 + segment_4 * 8 % 64 * 2 ^ (segment_4 * 8 / 64 * 4096 + row_1 * 128 + segment_4 * 8 % 64 * 2 >> 7 & 7) << 4))));
                    ...
                    int segment_0_1 = segment_4 * 8 / 8;
                    int elem_4 = segment_4 * 8 & 7;
                    int half_4 = elem_4 / 4;
                    int vector_idx_4 = segment_0_1 * 2 + half_4;
                    int cycle_3 = vector_idx_4 / 8;
                    int group_3 = (segment_0_1 & 3) + (segment_0_1 / 4 + half_4 & 1) * 4;
                    asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                        ...
                        : "r"(smem_gate_all_addr + (unsigned int)((stage_f32 + row_1 * 128 + (cycle_3 * 32 + group_3 * 4 + (elem_4 & 3))) * 4)));
                    int segment_1_1 = (segment_4 * 8 + 4) / 8;
                    int elem_2_1 = segment_4 * 8 + 4 & 7;
                    int half_3_1 = elem_2_1 / 4;
                    int vector_idx_4_1 = segment_1_1 * 2 + half_3_1;
                    int cycle_5 = vector_idx_4_1 / 8;
                    ...
                    for (int word = 0; word < 4; word++) {
                        asm volatile("st.shared.b32 [%0], %1;" :: "r"((smem_qd_addr + prep_stage * 41984 + (unsigned int)(segment_4 * 8 / 64 * 4096 + row_1 * 128 + segment_4 * 8 % 64 * 2 ^ (segment_4 * 8 / 64 * 4096 + row_1 * 128 + segment_4 * 8 % 64 * 2 >> 7 & 7) << 4)) + (unsigned int)(word * 4)), "r"((packed_1[word])));
                    }
                    ...
                    for (int word_1 = 0; word_1 < 4; word_1++) {
                        asm volatile("st.shared.b32 [%0], %1;" :: "r"((smem_kd_addr + prep_stage * 41984 + (unsigned int)(segment_4 * 8 / 64 * 4096 + row_1 * 128 + segment_4 * 8 % 64 * 2 ^ (segment_4 * 8 / 64 * 4096 + row_1 * 128 + segment_4 * 8 % 64 * 2 >> 7 & 7) << 4)) + (unsigned int)(word_1 * 4)), "r"((packed_7[word_1])));
                    }
                    ...
                    for (int word_2 = 0; word_2 < 4; word_2++) {
                        asm volatile("st.shared.b32 [%0], %1;" :: "r"((smem_ki_addr + prep_stage * 41984 + (unsigned int)(segment_4 * 8 / 64 * 4096 + row_1 * 128 + segment_4 * 8 % 64 * 2 ^ (segment_4 * 8 / 64 * 4096 + row_1 * 128 + segment_4 * 8 % 64 * 2 >> 7 & 7) << 4)) + (unsigned int)(word_2 * 4)), "r"((packed_8[word_2])));
                    }
                }
                if (prep_instance == 0) {
                    asm volatile("barrier.sync 11, 128;" ::: "memory");
                ...
                if (prep_tid < 128) {
                    ...
                    int half_5 = elem_5 / 4;
                    int vector_idx_5 = segment_5 * 2 + half_5;
                    int cycle_6 = vector_idx_5 / 8;
                ...
                if (prep_local_warp == 0) {
                    ...
                    for (int word_6 = 0; word_6 < 4; word_6++) {
                        asm volatile("st.shared.b32 [%0], %1;" :: "r"(smem_inv_work_addr + prep_stage * 41984 + (unsigned int)swizzled_off_2 + (unsigned int)(word_6 * 4)), "r"((packed_0_2[word_6])));
                    }
                }
                if (prep_local_warp < 2) {
                    if (elect_sync()) {
                ...
                for (int _advance = 0; _advance < 5; _advance++) {
                    prep_stage += 1;
                    if (prep_stage == 5) { prep_stage = 0; _phase_raw_inputs_free ^= 1; _phase_gate_raw_full ^= 1; _phase_smem_free ^= 1; _phase_qk_raw_full ^= 1; _phase_prep_diag_ready ^= 1; _phase_prep_inv16_ready ^= 1; }
                }
            }
        }
    }
