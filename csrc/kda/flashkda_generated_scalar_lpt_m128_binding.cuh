...
inline void LaunchScalarLptM128(ScalarLptM128Args args,
  ...
  void* kernel_args[] = {
      ...
      &args.dt_bias, &args.cu_seqlens, &args.seq_order, &args.tile_schedule,
      &args.tile_schedule_counts, &args.schedule_stride, &args.initial_state,
      &args.out, &args.out_tma, &args.final_state, &args.state_indices_addr,
      &args.state_slot_stride, &args.use_state_indices, &args.initial_state_f32,
...
inline void RunScalarLptM128(
    ...
    TensorView cu_seqlens, TensorView seq_order, TensorView tile_schedule,
    TensorView tile_schedule_counts, TensorView state_indices,
    TensorView initial_state, TensorView out, TensorView final_state,
    TensorView descriptor_storage, int64_t prepare_descriptors,
    int64_t schedule_stride, int64_t num_heads, int64_t beta_token_stride,
  ...
  args.tile_schedule = CheckedBufferPointer(tile_schedule, "tile_schedule",
                                            prepared.device_id, dl_int32);
  args.tile_schedule_counts = CheckedBufferPointer(
      tile_schedule_counts, "tile_schedule_counts", prepared.device_id, dl_int32);
  args.schedule_stride = CheckedInt32(schedule_stride, "schedule_stride");
  TVM_FFI_ICHECK(args.schedule_stride > 0) << "schedule_stride must be positive";
  ...
  LaunchScalarLptM128(args, prepared.state, CheckedGrid(grid_x, grid_y, grid_z),
                      prepared.stream);
}
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(
    run, flashinfer::flash_kda_generated::RunScalarLptM128);
