...
kernel_cake_fused_kda_decode_high_work_f32_wide_slot_offsets(__nv_bfloat16* __restrict__ x, float* __restrict__ weight, __nv_bfloat16* __restrict__ conv_state, __nv_bfloat16* __restrict__ raw_gate, __nv_bfloat16* __restrict__ raw_beta, float* __restrict__ A_log, float* __restrict__ dt_bias, int* __restrict__ state_indices, float* __restrict__ state, __nv_bfloat16* __restrict__ output_gate, float* __restrict__ norm_weight, __nv_bfloat16* __restrict__ output, int x_row_stride, int conv_slot_stride, int beta_row_stride, int state_slot_stride, int output_gate_row_stride, int H, int use_lower_bound, float lower_bound_log2, float norm_eps)
{
    ...
    float _shfl_xor_0 = __shfl_xor_sync(0xFFFFFFFF, k_sq, 8);
    k_sq += _shfl_xor_0;
    float _shfl_xor_1 = __shfl_xor_sync(0xFFFFFFFF, k_sq, 4);
    k_sq += _shfl_xor_1;
    float _shfl_xor_2 = __shfl_xor_sync(0xFFFFFFFF, k_sq, 2);
    k_sq += _shfl_xor_2;
    float _shfl_xor_3 = __shfl_xor_sync(0xFFFFFFFF, k_sq, 1);
    k_sq += _shfl_xor_3;
    float q_scale = 0.0f;
    float k_scale = 0.0f;
    if (lane_1 < 16) {
        float _rsqrt_0 = rsqrtf(k_sq + 1e-06f);
        k_scale = _rsqrt_0;
    }
    ...
    for (int value_tile = 0; value_tile < 2; value_tile++) {
        ...
        for (int row_group = 0; row_group < 4 / ((1) ? 4 : 2); row_group++) {
            ...
            float _shfl_xor_4 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_a, 8);
            state_key_dot_a += _shfl_xor_4;
            float _shfl_xor_5 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_b, 8);
            state_key_dot_b += _shfl_xor_5;
            float _shfl_xor_6 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_c, 8);
            state_key_dot_c += _shfl_xor_6;
            float _shfl_xor_7 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_d, 8);
            state_key_dot_d += _shfl_xor_7;
            if (value_tile == 0) {
                float _shfl_xor_8 = __shfl_xor_sync(0xFFFFFFFF, q_sq, 8);
                q_sq += _shfl_xor_8;
            }
            float _shfl_xor_9 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_a, 4);
            state_key_dot_a += _shfl_xor_9;
            float _shfl_xor_10 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_b, 4);
            state_key_dot_b += _shfl_xor_10;
            float _shfl_xor_11 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_c, 4);
            state_key_dot_c += _shfl_xor_11;
            float _shfl_xor_12 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_d, 4);
            state_key_dot_d += _shfl_xor_12;
            if (value_tile == 0) {
                float _shfl_xor_13 = __shfl_xor_sync(0xFFFFFFFF, q_sq, 4);
                q_sq += _shfl_xor_13;
            }
            float _shfl_xor_14 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_a, 2);
            state_key_dot_a += _shfl_xor_14;
            float _shfl_xor_15 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_b, 2);
            state_key_dot_b += _shfl_xor_15;
            float _shfl_xor_16 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_c, 2);
            state_key_dot_c += _shfl_xor_16;
            float _shfl_xor_17 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_d, 2);
            state_key_dot_d += _shfl_xor_17;
            if (value_tile == 0) {
                float _shfl_xor_18 = __shfl_xor_sync(0xFFFFFFFF, q_sq, 2);
                q_sq += _shfl_xor_18;
            }
            float _shfl_xor_19 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_a, 1);
            state_key_dot_a += _shfl_xor_19;
            float _shfl_xor_20 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_b, 1);
            state_key_dot_b += _shfl_xor_20;
            float _shfl_xor_21 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_c, 1);
            state_key_dot_c += _shfl_xor_21;
            float _shfl_xor_22 = __shfl_xor_sync(0xFFFFFFFF, state_key_dot_d, 1);
            state_key_dot_d += _shfl_xor_22;
            if (value_tile == 0) {
                float _shfl_xor_23 = __shfl_xor_sync(0xFFFFFFFF, q_sq, 1);
                q_sq += _shfl_xor_23;
                if (lane_1 < 16) {
