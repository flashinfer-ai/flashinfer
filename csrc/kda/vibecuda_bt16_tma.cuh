...
inline void PublishMaps(const std::array<CUtensorMap, kTensorMapCount>& host_maps,
  ...
  CheckCuda(cudaMemcpyAsync(descriptor_storage, host_maps.data(), kDescriptorStorageBytes,
                            cudaMemcpyHostToDevice, stream),
            "PublishMaps memcpy");
}

inline void CheckEncode(CUresult result, const char* name) {
  KDA_BT16_CHECK(result == CUDA_SUCCESS, std::string("cuTensorMapEncodeTiled failed for ") + name);
}

// EncodeQkTma<16> recipe for q/k: flattened [tokens, H, 128] bf16.
inline CUtensorMap EncodeQk16(const void* base, int64_t tokens, int64_t heads, const char* name) {
  const int64_t d1 = kHeadDim;
  const int64_t d2 = heads;
  KDA_BT16_CHECK(d2 >= 1 && tokens >= 1, "q/k dims invalid for BT16 TMA box");
  uint64_t global_dim[4] = {64, static_cast<uint64_t>(tokens), static_cast<uint64_t>(d2), 2};
  ...
  CUtensorMap map{};
  CheckEncode(
      cuTensorMapEncodeTiled(&map, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, 4, const_cast<void*>(base),
...
inline CUtensorMap EncodeGate16(const void* base, int64_t tokens, int64_t heads) {
  ...
  CUtensorMap map{};
  CheckEncode(
      cuTensorMapEncodeTiled(&map, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, 3, const_cast<void*>(base),
                             global_dim, global_strides, box_dim, elem_strides,
...
inline CUtensorMap EncodeFactor16(const void* base, int64_t heads, int64_t padded_tokens,
                                  int box_value_splits, const char* name) {
  const int64_t d1 = kHeadDim;
  uint64_t global_dim[4] = {64, static_cast<uint64_t>(padded_tokens), static_cast<uint64_t>(heads),
                            2};
...
inline CUtensorMap EncodeQkWs(const void* base, int64_t heads, int64_t total_chunks) {
  ...
  uint64_t global_strides[4] = {
      16 * sizeof(__nv_bfloat16), 16 * 16 * sizeof(__nv_bfloat16),
      static_cast<uint64_t>(total_chunks) * 16 * 16 * sizeof(__nv_bfloat16),
      static_cast<uint64_t>(heads) * static_cast<uint64_t>(total_chunks) * 16 * 16 *
          sizeof(__nv_bfloat16)};
  uint32_t box_dim[5] = {16, 16, 1, 1, 1};
  uint32_t elem_strides[5] = {1, 1, 1, 1, 1};
  CUtensorMap map{};
  CheckEncode(
      cuTensorMapEncodeTiled(&map, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, 5, const_cast<void*>(base),
                             global_dim, global_strides, box_dim, elem_strides,
...
inline CUtensorMap EncodeDiagWs(const void* base, int64_t heads, int64_t total_chunks) {
  uint64_t global_dim[4] = {128, static_cast<uint64_t>(total_chunks), static_cast<uint64_t>(heads),
                            1};
  uint64_t global_strides[3] = {
      128 * sizeof(float), static_cast<uint64_t>(total_chunks) * 128 * sizeof(float),
      static_cast<uint64_t>(heads) * static_cast<uint64_t>(total_chunks) * 128 * sizeof(float)};
  uint32_t box_dim[4] = {128, 1, 1, 1};
  uint32_t elem_strides[4] = {1, 1, 1, 1};
  CUtensorMap map{};
  CheckEncode(
      cuTensorMapEncodeTiled(&map, CU_TENSOR_MAP_DATA_TYPE_FLOAT32, 4, const_cast<void*>(base),
                             global_dim, global_strides, box_dim, elem_strides,
                             CU_TENSOR_MAP_INTERLEAVE_NONE, CU_TENSOR_MAP_SWIZZLE_NONE,
                             CU_TENSOR_MAP_L2_PROMOTION_NONE, CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE),
      "ws_diag");
