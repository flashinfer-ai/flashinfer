// Round-182 (supervisor directive): fam v2 head-family path, H=64
// instantiation. Compiles the EVOLVED slab image
// (flashkda_bf16_fam2_gen.cu, a clone of
// flashkda_bf16_fused_m128slab_gen.cu transformed exactly like the round-148
// slabh12 specialization — runtime head count renamed num_heads_rt and a
// compile-time constexpr num_heads = KDA_FAM_H) so every measured R10-R88
...
#define kernel_flashkda_bf16_fused_m128 kernel_flashkda_fam2_h64
#define kernel_flashkda_split_scan_m128 kernel_flashkda_split_scan_m128_fam2h64
#define kernel_flashkda_split_scan_bf16_m128 kernel_flashkda_split_scan_bf16_m128_fam2h64
#define kernel_flashkda_split_lookback_m128 kernel_flashkda_split_lookback_m128_fam2h64
#define kernel_flashkda_split_out_add_m128 kernel_flashkda_split_out_add_m128_fam2h64
#define KDA_FAM
#define KDA_FAM_H 64
#include "vibecuda_flashkda_fam2_gen.cu"
...
void RunFam2H64(const void* q_ptr, const void* k_ptr, const void* v_ptr, const void* g_ptr,
  ...
  static const bool kSmemAttrOnce = [] {
    CheckCuda(cudaFuncSetAttribute(kernel_flashkda_fam2_h64,
                                   cudaFuncAttributeMaxDynamicSharedMemorySize, kSmemBytes),
              "cudaFuncSetAttribute(kernel_flashkda_fam2_h64)");
    return true;
  }();
  (void)kSmemAttrOnce;
  ...
  if (kPdlOn) {
    ...
    pdl_cfg.numAttrs = 1;
    CheckCuda(cudaLaunchKernelEx(&pdl_cfg, kernel_flashkda_fam2_h64,
        ...
        "kernel_flashkda_fam2_h64 PDL launch");
    return;
  }
#endif
  kernel_flashkda_fam2_h64<<<grid, block, SMEM_TOTAL, stream>>>(
      reinterpret_cast<__nv_bfloat16*>(const_cast<void*>(q_ptr)), tma.q,
      reinterpret_cast<__nv_bfloat16*>(const_cast<void*>(k_ptr)), tma.k,
      reinterpret_cast<__nv_bfloat16*>(const_cast<void*>(v_ptr)), tma.v,
      reinterpret_cast<__nv_bfloat16*>(const_cast<void*>(g_ptr)), tma.g,
      ...
      /*fixup_mode=*/0, /*progress_flags=*/nullptr,
      /*map_state_bf16=*/nullptr, static_cast<int>(ft_slab));
  CheckCuda(cudaGetLastError(), "kernel_flashkda_fam2_h64 launch");
}
