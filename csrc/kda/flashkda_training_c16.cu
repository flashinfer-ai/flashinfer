...
kernel_flashkda_forward_checkpoint_c16(unsigned int* __restrict__ dynamic_counter, const __grid_constant__ CUtensorMap q_tma, const __grid_constant__ CUtensorMap k_tma, const __grid_constant__ CUtensorMap v_tma, const __grid_constant__ CUtensorMap g_tma, __nv_bfloat16* __restrict__ g, const __grid_constant__ CUtensorMap out_tma, __nv_bfloat16* __restrict__ out, const __grid_constant__ CUtensorMap checkpoint_tma, __nv_bfloat16* __restrict__ beta, __nv_bfloat16* __restrict__ beta_active_out, float* __restrict__ A_log, float* __restrict__ dt_bias, long long* __restrict__ cu_seqlens, long long* __restrict__ checkpoint_cu_starts, int* __restrict__ work_items, float* __restrict__ initial_state, float* __restrict__ final_state, int total_work_items, int uniform_work_items, int num_qk_heads, int num_heads, int beta_active_stride, int checkpoint_every_n_tokens, float scale, float lower_bound)
{
            ...
            for (int _ = 0; _ < total_work_items + 1; _++) {
                ...
                for (int chunk_cg0 = cg0_instance; chunk_cg0 < chunks_cg0; chunk_cg0 += 2) {
                    ...
                    for (int dim_half_cg0 = 0; dim_half_cg0 < 2; dim_half_cg0++) {
                        ...
                        long long valid_qk_row_cg0 = bos_cg0 + (long long)logical_chunk_cg0 * 16 + (long long)decay_row_cg0;
                        #pragma unroll
                            ...
                            if (eos_epi >= output_token_base_epi + 16) {
                                if (elect_sync()) {
                                    ...
                                    for (int output_segment_epi = 0; output_segment_epi < 2; output_segment_epi++) {
                                        tma_store_3d((&out_tma), output_segment_epi * 64, head_epi, (int)output_token_base_epi, smem_o_addr + output_stage_epi * 4096 + (unsigned int)(output_segment_epi * 16 * 64 * 2));
                                    }
                                }
                                asm volatile("cp.async.bulk.commit_group;");
                                asm volatile("cp.async.bulk.wait_group.read 0;");
                                ...
                                #pragma unroll 1
                                for (int output_linear_epi = lane; output_linear_epi < 2048; output_linear_epi += 32) {
                                    ...
                                    if (output_token_epi < eos_epi) {
                                        ...
                                        out[(output_token_epi * (long long)num_heads + (long long)head_epi) * 128 + (long long)output_dim_epi] = smem_o_all[(int)output_stage_epi * 16 * 128 + segment_7 * 16 * 64 + output_row_epi * 64 + swizzled_col_7];
                                    }
                                }
                            }
                        }
                    ...
                    if (eos_epi >= last_token_base_epi + 16) {
                        if (elect_sync()) {
                            ...
                            for (int last_segment_epi = 0; last_segment_epi < 2; last_segment_epi++) {
                                tma_store_3d((&out_tma), last_segment_epi * 64, head_epi, (int)last_token_base_epi, smem_o_addr + last_o_stage_epi * 4096 + (unsigned int)(last_segment_epi * 16 * 64 * 2));
                            }
                        }
                        asm volatile("cp.async.bulk.commit_group;");
                        asm volatile("cp.async.bulk.wait_group.read 0;");
                        ...
                        for (int last_linear_epi = lane; last_linear_epi < 2048; last_linear_epi += 32) {
                            ...
                            if (last_token_epi < eos_epi) {
                                ...
                                out[(last_token_epi * (long long)num_heads + (long long)head_epi) * 128 + (long long)last_dim_epi] = smem_o_all[(int)last_o_stage_epi * 16 * 128 + segment_8 * 16 * 64 + last_row_epi * 64 + swizzled_col_8];
                            }
                        }
                    }
            ...
            for (int _ = 0; _ < total_work_items; _++) {
                ...
                for (int reverse0 = 0; reverse0 < compute_end0 - write_start0; reverse0++) {
                    ...
                    if (eos0 >= bos0 + (long long)(raw_chunk0 + 1) * 16) {
                        ...
                        raw_tail_stage0 += 1;
                        if (raw_tail_stage0 == 2) { raw_tail_stage0 = 0; _phase_raw_tail_ready ^= 1; }
                    }
        ...
        { // compute1_main
            ...
            for (int __1 = 0; __1 < total_work_items; __1++) {
                ...
                for (int reverse1 = 0; reverse1 < compute_end1 - write_start1; reverse1++) {
                    ...
                    if (eos1 < raw_token_base1 + 16) {
                        ...
                        for (int raw_tail_row1 = 0; raw_tail_row1 < 16; raw_tail_row1++) {
                            if (eos1 <= raw_token_base1 + (long long)raw_tail_row1) {
                                ...
                                int raw_tail_index1 = (int)raw_stage1 * 16 * 128 + segment_5 * 16 * 64 + raw_tail_row1 * 64 + swizzled_col_6;
                        ...
                        raw_tail_stage1 += 1;
                        if (raw_tail_stage1 == 2) { raw_tail_stage1 = 0; }
                    }
                    ...
                    unsigned int v_addr_lo1 = raw_v_all_addr + (unsigned int)(((int)raw_stage1 * 16 * 128 + segment_6 * 16 * 64 + ov_token1 * 64 + swizzled_col_7) * 2);
                    ...
                    unsigned int do_addr_hi1 = raw_do_all_addr + (unsigned int)(((int)raw_stage1 * 16 * 128 + segment_6_1 * 16 * 64 + ov_token1 * 64 + swizzled_col_8) * 2);
                    asm volatile("ldmatrix.sync.aligned.m8n8.x4.trans.shared.b16 {%0, %1, %2, %3}, [%4];\n"
                    ...
                    long long boundary_checkpoint1 = checkpoint_cu_starts[sequence1] + (long long)boundary_chunk1;
                    #pragma unroll
                    ...
                    if (boundary_chunk1 < write_end1) {
                        ...
                        long long dgate_output_index1 = (boundary_token1 * (long long)num_heads + (long long)head1) * 128 + (long long)role_tid1;
            ...
            for (int __2 = 0; __2 < total_work_items; __2++) {
                ...
                for (int reverse2 = 0; reverse2 < compute_end2 - write_start2; reverse2++) {
                    ...
                    for (int token_gate2 = 0; token_gate2 < 16; token_gate2++) {
                        ...
                        eg_values2[token_gate2] = g_prefix_all[(int)g_prefix_stage2 * 16 * 128 + segment_7 * 16 * 32 + token_gate2 * 32 + swizzled_col_9];
                    }
                    ...
                    if (output_chunk2 < write_end2) {
                        ...
                        for (int output_token2 = 0; output_token2 < 16; output_token2++) {
                            ...
                            if (output_token_global2 < eos2) {
                                long long output_index2 = (output_token_global2 * (long long)num_heads + (long long)head2) * 128 + (long long)role_tid2;
            ...
            for (int __3 = 0; __3 < total_work_items; __3++) {
                ...
                for (int reverse3 = 0; reverse3 < compute_end3 - write_start3; reverse3++) {
                    ...
                    if (eos3 >= bos3 + (long long)(raw_chunk3 + 1) * 16) {
                        ...
                        raw_tail_stage3 += 1;
                        if (raw_tail_stage3 == 2) { raw_tail_stage3 = 0; _phase_raw_tail_ready_1 ^= 1; }
                    }
                    ...
                    if (enable_kk != 0) {
                        ...
                        for (int k_block3 = 0; k_block3 < 8; k_block3++) {
                            ...
                            asm volatile("ldmatrix.sync.aligned.m8n8.x4.shared.b16 {%0, %1, %2, %3}, [%4];\n"
                                ...
                                : "r"(k_decay_all_addr + (unsigned int)(((int)operand_stage3 * 16 * 128 + segment_8 * 16 * 64 + lhs_row3 * 64 + swizzled_col_10) * 2))
                                : "memory");
                    ...
                    for (int k_block_dm3 = 0; k_block_dm3 < 8; k_block_dm3++) {
                        ...
                        asm volatile("ldmatrix.sync.aligned.m8n8.x4.shared.b16 {%0, %1, %2, %3}, [%4];\n"
                            ...
                            : "r"(dy_smem_all_addr + (unsigned int)((segment_10 * 16 * 64 + lhs_row3 * 64 + swizzled_col_12) * 2))
                            : "memory");
            ...
            for (int __4 = 0; __4 < total_work_items; __4++) {
                ...
                for (int reverse4 = 0; reverse4 < compute_end4 - write_start4; reverse4++) {
                    ...
                    if (eos4 >= bos4 + (long long)(raw_chunk4 + 1) * 16) {
                        ...
                        raw_tail_stage4 += 1;
                        if (raw_tail_stage4 == 2) { raw_tail_stage4 = 0; _phase_raw_tail_ready_2 ^= 1; }
                    }
            ...
            for (int __5 = 0; __5 < total_work_items; __5++) {
                ...
                for (int reverse5 = 0; reverse5 < compute_end5 - write_start5; reverse5++) {
                    ...
                    if (eos5 >= bos5 + (long long)(raw_chunk5 + 1) * 16) {
                        ...
                        raw_tail_stage5 += 1;
                        if (raw_tail_stage5 == 2) { raw_tail_stage5 = 0; _phase_raw_tail_ready_3 ^= 1; }
                    }
                    ...
                    for (int k_block5 = 0; k_block5 < 8; k_block5++) {
                        ...
                        asm volatile("ldmatrix.sync.aligned.m8n8.x4.shared.b16 {%0, %1, %2, %3}, [%4];\n"
                            ...
                            : "r"(q_decay_all_addr + (unsigned int)(((int)operand_stage5 * 16 * 128 + segment_11 * 16 * 64 + lhs_row5 * 64 + swizzled_col_13) * 2))
                            : "memory");
                        ...
                        if (eos5 >= dv_token_base5 + 16) {
                            ...
                            for (int dv_linear5 = lane; dv_linear5 < 2048; dv_linear5 += 32) {
                                ...
                                if (dv_token_global5 < eos5) {
                                    ...
                                    dv[(dv_token_global5 * (long long)num_heads + (long long)head5) * 128 + (long long)dv_dim5] = beta_dy_smem_all[(int)(dv_stage5 % 2) * 16 * 128 + (segment_13 * 16 * 64 + dv_row5 * 64 + swizzled_col_15)];
                                }
                            }
                        }
                    }
                    ...
                    for (int k_block_da5 = 0; k_block_da5 < 8; k_block_da5++) {
                        ...
                        asm volatile("ldmatrix.sync.aligned.m8n8.x4.shared.b16 {%0, %1, %2, %3}, [%4];\n"
                            ...
                            : "r"(raw_do_all_addr + (unsigned int)(((int)raw_stage5 * 16 * 128 + segment_14 * 16 * 64 + lhs_row5 * 64 + swizzled_col_16) * 2))
                            : "memory");
