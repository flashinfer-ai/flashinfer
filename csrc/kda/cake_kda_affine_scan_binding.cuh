...
static_assert(FLASHINFER_CAKE_KDA_AFFINE_ROLE == FLASHINFER_CAKE_KDA_AFFINE_ROLE_SCAN,
...
static_assert(FLASHINFER_CAKE_KDA_AFFINE_USE_PDL == 1, "sealed Cake KDA affine scan requires PDL");
...
inline void RunCakeKDAAffineScan(TensorView split_state, TensorView map_state_bf16,
                                 TensorView carry, int64_t num_heads, int64_t num_parts,
  ...
  TVM_FFI_ICHECK(num_parts >= 2) << "Cake KDA affine scan requires at least two parts";
  CakeKDAAffineCheckCompactState(split_state, "split_state", device_id, dl_float32, num_parts,
                                 num_heads);
  CakeKDAAffineCheckCompactState(map_state_bf16, "map_state_bf16", device_id, dl_bfloat16,
                                 num_parts - 1, num_heads);
  CakeKDAAffineCheckCompactState(carry, "carry", device_id, dl_float32, num_parts - 1, num_heads);
  CheckNoOverlap(split_state, "split_state", map_state_bf16, "map_state_bf16");
  ...
  void* kernel_args[] = {&args.split_state, &args.map_state_bf16, &args.carry, &args.num_heads,
                         &args.num_parts};
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, flashinfer::cake_kda::RunCakeKDAAffineScan);
