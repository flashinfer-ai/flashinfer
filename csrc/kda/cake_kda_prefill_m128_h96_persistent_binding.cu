...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_kda_prefill_m128_h96_persistent'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
void Run(TensorView arg_q, TensorView arg_q_tma, TensorView arg_k, TensorView arg_k_tma, TensorView arg_v, TensorView arg_v_tma, TensorView arg_g, TensorView arg_g_tma, TensorView arg_beta, TensorView arg_beta_tma, TensorView arg_A_log, TensorView arg_dt_bias, TensorView arg_cu_seqlens, TensorView arg_seq_order, TensorView arg_tile_schedule, TensorView arg_tile_schedule_counts, int64_t arg_schedule_stride, TensorView arg_initial_state, TensorView arg_out, TensorView arg_out_tma, TensorView arg_final_state, int64_t arg_state_indices_addr, int64_t arg_state_slot_stride, int64_t arg_use_state_indices, TensorView arg_initial_state_f32, TensorView arg_final_state_f32, int64_t arg_num_heads, int64_t arg_use_initial_state, int64_t arg_store_final_state, double arg_scale, double arg_lower_bound, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  CheckCudaTensor(arg_tile_schedule, "tile_schedule");
  CheckDtype(arg_tile_schedule, "tile_schedule", 0, 32, 1);
  CheckContiguous(arg_tile_schedule, "tile_schedule");
  CheckCudaTensor(arg_tile_schedule_counts, "tile_schedule_counts");
  CheckDtype(arg_tile_schedule_counts, "tile_schedule_counts", 0, 32, 1);
  CheckContiguous(arg_tile_schedule_counts, "tile_schedule_counts");
  TVM_FFI_CHECK(arg_schedule_stride >= -2147483648LL && arg_schedule_stride <= 2147483647LL, ValueError)
      << "scalar 'schedule_stride' value " << arg_schedule_stride
  ...
  CheckSameCudaDevice(arg_tile_schedule, arg_q, "tile_schedule", "q");
  CheckSameCudaDevice(arg_tile_schedule_counts, arg_q, "tile_schedule_counts", "q");
  ...
  int* p_tile_schedule = static_cast<int*>(arg_tile_schedule.data_ptr());
  int* p_tile_schedule_counts = static_cast<int*>(arg_tile_schedule_counts.data_ptr());
  int32_t v_schedule_stride = (int32_t)arg_schedule_stride;
  ...
  void* kargs[] = {&p_q, &p_q_tma, &p_k, &p_k_tma, &p_v, &p_v_tma, &p_g, &p_g_tma, &p_beta, &p_beta_tma, &p_A_log, &p_dt_bias, &p_cu_seqlens, &p_seq_order, &p_tile_schedule, &p_tile_schedule_counts, &v_schedule_stride, &p_initial_state, &p_out, &p_out_tma, &p_final_state, &v_state_indices_addr, &v_state_slot_stride, &v_use_state_indices, &p_initial_state_f32, &p_final_state_f32, &v_num_heads, &v_use_initial_state, &v_store_final_state, &v_scale, &v_lower_bound};
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_kda_prefill_m128_h96_persistent),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_kda_prefill_m128_h96_persistent failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_kda_prefill_m128_h96_persistent), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_kda_prefill_m128_h96_persistent failed: "
