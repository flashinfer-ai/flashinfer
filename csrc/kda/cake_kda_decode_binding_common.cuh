...
struct VariantTraits {
  ...
  static_assert(GateKind == 0 || GateKind == 1 || GateKind == 2);
...
struct LaunchContext {
  ...
  int32_t k_token_stride;
  int32_t v_token_stride;
  int32_t gate_token_stride;
  int32_t beta_token_stride;
  int32_t state_slot_stride;
  int32_t beta_is_logit;
  cudaStream_t stream;
};
...
#ifndef FLASHINFER_CAKE_KDA_DECODE_TARGET_KIND
#error "FLASHINFER_CAKE_KDA_DECODE_TARGET_KIND must be defined by the JIT/AOT spec"
#endif

constexpr int kCakeKDADecodeTargetKind = FLASHINFER_CAKE_KDA_DECODE_TARGET_KIND;
constexpr int kCakeKDADecodeFamilyTarget = 100;
constexpr int kCakeKDADecodeExactSM100aTarget = 1000;
constexpr int kCakeKDADecodeExactSM103aTarget = 1003;
static_assert(kCakeKDADecodeTargetKind == kCakeKDADecodeFamilyTarget ||
                  kCakeKDADecodeTargetKind == kCakeKDADecodeExactSM100aTarget ||
                  kCakeKDADecodeTargetKind == kCakeKDADecodeExactSM103aTarget,
              "CakeKDA decode target must be SM100f, exact SM100a, or exact SM103a");

inline void CheckCakeKDADecodeTarget(int32_t device_id) {
  int major = 0;
  int minor = 0;
  ...
  if (kCakeKDADecodeTargetKind == kCakeKDADecodeFamilyTarget) {
    TVM_FFI_ICHECK(major == 10 && (minor == 0 || minor == 3))
        << "this frozen CakeKDA decode module was compiled for the SM100 family "
           "(compute capability 10.0 or 10.3), got "
        << major << "." << minor;
    return;
  }

  const int expected_minor = kCakeKDADecodeTargetKind == kCakeKDADecodeExactSM100aTarget ? 0 : 3;
  TVM_FFI_ICHECK(major == 10 && minor == expected_minor)
      << "this frozen CakeKDA decode module was compiled for exact compute capability 10."
...
inline void CheckCompactTokenRows(const TensorView& tensor, const char* name, int64_t num_heads,
                                  int64_t head_dim, bool vector_aligned_stride = true) {
  ...
  if (vector_aligned_stride) {
    TVM_FFI_ICHECK(tensor.stride(1) % 4 == 0)
        << name << " token stride must preserve eight-byte vector alignment";
  }
}

inline void CheckCompactBetaRows(const TensorView& beta, int64_t num_value_heads) {
  TVM_FFI_ICHECK(beta.ndim() == 3) << "beta must be rank 3";
  TVM_FFI_ICHECK(beta.stride(2) == 1) << "beta must have a unit head stride";
  CheckedInt32(beta.stride(1), "beta token stride", true);
  TVM_FFI_ICHECK(beta.stride(1) >= num_value_heads)
      << "beta token stride must not overlap adjacent tokens";
}
...
LaunchContext CheckInputs(const TensorView& q, const TensorView& k, const TensorView& v,
                          ...
                          int64_t beta_is_logit, int64_t cuda_stream) {
  ...
  CheckCakeKDADecodeTarget(device_id);
  ...
  if constexpr (kGateKind == 2 && kTokens == 1) {
    CheckCompactTokenRows(q, "q", num_heads, kHeadDim);
    CheckCompactTokenRows(k, "k", num_heads, kHeadDim);
    CheckCompactTokenRows(v, "v", num_value_heads, kHeadDim);
    CheckCompactBetaRows(beta, num_value_heads);
  } else {
    CheckContiguous(q, "q");
    CheckContiguous(k, "k");
    CheckContiguous(v, "v");
  ...
  if constexpr (kGateKind == 0) {
    ...
    CheckContiguous(dt_bias, "dt_bias");
    TVM_FFI_ICHECK(A_log.numel() >= num_heads)
        << "unbounded-softplus gate variants require at least H A_log values";
    TVM_FFI_ICHECK(dt_bias.numel() >= num_heads * kHeadDim)
        << "unbounded-softplus gate variants require at least H * D dt_bias values";
    TVM_FFI_ICHECK(lower_bound == 0.0) << "unbounded-softplus gate variants require their unused "
                                          "lower_bound ABI scalar to be zero";
  }
  TVM_FFI_ICHECK(beta_is_logit == 0 || beta_is_logit == 1)
      << "beta_is_logit must be exactly zero or one";
  if constexpr (!(kGateKind == 2 && kTokens == 1)) {
    TVM_FFI_ICHECK(beta_is_logit == 0)
        << "only the T1 unbounded-softplus variant accepts beta logits";
  }
  TVM_FFI_ICHECK(std::isfinite(scale) && std::isfinite(static_cast<float>(scale)))
      << "scale must be finite and representable as float32";

  CheckedInt32Product({total_tokens, q.stride(1)}, "q strided linear index extent");
  CheckedInt32Product({total_tokens, k.stride(1)}, "k strided linear index extent");
  CheckedInt32Product({total_tokens, v.stride(1)}, "v strided linear index extent");
  CheckedInt32Product({total_tokens, g.stride(1)}, "g strided linear index extent");
  CheckedInt32Product({total_tokens, beta.stride(1)}, "beta strided linear index extent");
  ...
  return {
      ...
      static_cast<int32_t>(CheckedInt32(q.stride(1), "q token stride", true)),
      static_cast<int32_t>(CheckedInt32(k.stride(1), "k token stride", true)),
      static_cast<int32_t>(CheckedInt32(v.stride(1), "v token stride", true)),
      static_cast<int32_t>(CheckedInt32(g.stride(1), "g token stride", true)),
      static_cast<int32_t>(CheckedInt32(beta.stride(1), "beta token stride", true)),
      static_cast<int32_t>(CheckedInt32(state.stride(0), "state slot stride", true)),
      static_cast<int32_t>(beta_is_logit),
...
inline void CheckDynamicSmemCapacity(int32_t device_id, int32_t required) {
  ...
  TVM_FFI_ICHECK(required <= capacity)
      << "frozen CakeKDA decode kernel requires " << required
