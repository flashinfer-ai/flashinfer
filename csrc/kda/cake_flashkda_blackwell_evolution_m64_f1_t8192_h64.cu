...
kernel_flashkda_blackwell_evolution_m64_f1_t8192_h64(__nv_bfloat16* __restrict__ q, CakeTensorMap const* q_tma, __nv_bfloat16* __restrict__ k, CakeTensorMap const* k_tma, __nv_bfloat16* __restrict__ v, CakeTensorMap const* v_tma, __nv_bfloat16* __restrict__ g, CakeTensorMap const* g_tma, __nv_bfloat16* __restrict__ beta, CakeTensorMap const* beta_tma, float* __restrict__ A_log, float* __restrict__ dt_bias, long long* __restrict__ cu_seqlens, int* __restrict__ seq_order, __nv_bfloat16* __restrict__ initial_state, __nv_bfloat16* __restrict__ out, CakeTensorMap const* out_tma, __nv_bfloat16* __restrict__ final_state, int num_heads, int use_initial_state, int store_final_state, float scale, float lower_bound)
{
        ...
        { // compute_main
            ...
            for (int chunk_idx = 0; chunk_idx < num_chunks; chunk_idx++) {
                ...
                if (lane < 16) {
                    ...
                    {
                        __nv_bfloat16 _bval_2 = __float2bfloat16_rn(diag_scale_0);
                        uint16_t _bits_2 = *(uint16_t*)&_bval_2;
                        uint32_t _addr_2 = static_cast<uint32_t>((diag_base_0 + (lane / 16 * 512 + lane * 32 + lane % 16 * 2 ^ (lane / 16 * 512 + lane * 32 + lane % 16 * 2 >> 7 & 1) << 4)));
                        asm volatile("st.shared.b16 [%0], %1;" :: "r"(_addr_2), "h"(_bits_2) : "memory");
                    }
                    {
                        __nv_bfloat16 _bval_3 = __float2bfloat16_rn(diag_scale_1);
                        uint16_t _bits_3 = *(uint16_t*)&_bval_3;
                        uint32_t _addr_3 = static_cast<uint32_t>((diag_base_1 + (lane / 16 * 512 + lane * 32 + lane % 16 * 2 ^ (lane / 16 * 512 + lane * 32 + lane % 16 * 2 >> 7 & 1) << 4)));
            ...
            }
            {
                ...
                for (int state_col_half_2 = 0; state_col_half_2 < 2; state_col_half_2++) {
                    ...
                    for (int state_col_group_1 = 0; state_col_group_1 < 8; state_col_group_1++) {
                        ...
                        final_state[state_base_top + (long long)state_col_pair_1] = _tmem_load_3[state_reg_base_1];
                        final_state[state_base_top + (long long)state_col_pair_1 + 1] = _tmem_load_3[state_reg_base_1 + 1];
                        final_state[state_base_bot + (long long)state_col_pair_1] = _tmem_load_3[state_reg_base_1 + 2];
                        final_state[state_base_bot + (long long)state_col_pair_1 + 1] = _tmem_load_3[state_reg_base_1 + 3];
                    }
                }
            }
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // epilogue_main
            ...
            long long bos_1 = (long long)seq_idx_1 * (long long)SEQ_LEN;
            long long eos_1 = bos_1 + (long long)SEQ_LEN;
    ...
    if (warp == 9) {
        { // mma_main
            ...
            long long bos_2 = (long long)seq_idx_2 * (long long)SEQ_LEN;
            long long eos_2 = bos_2 + (long long)SEQ_LEN;
    ...
    if (warp == 10) {
        { // load_main
            ...
            long long bos_3 = (long long)seq_idx_3 * (long long)SEQ_LEN;
            long long eos_3 = bos_3 + (long long)SEQ_LEN;
    ...
    if (warp >= 12 && warp <= 31) {
        ...
        { // prep_main
            ...
            long long bos_4 = (long long)seq_idx_4 * (long long)SEQ_LEN;
            long long eos_4 = bos_4 + (long long)SEQ_LEN;
            int seq_len_4 = SEQ_LEN;
