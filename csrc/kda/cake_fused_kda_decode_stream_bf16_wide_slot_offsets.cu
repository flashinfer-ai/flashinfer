...
#define SMEM_SOUTPUTSCALE_OFF 3600
#define SMEM_SOUTPUTSCALE_STAGE_BYTES 1024
#define SMEM_SOUTPUTSCALE_STRIDE 1024
...
#define SMEM_SSTAGE_OFF 4736
#define SMEM_SSTAGE_STAGE_BYTES 65536
#define SMEM_SSTAGE_STRIDE 65536
...
kernel_cake_fused_kda_decode_stream_bf16_wide_slot_offsets(__nv_bfloat16* __restrict__ x, float* __restrict__ weight, __nv_bfloat16* __restrict__ conv_state, __nv_bfloat16* __restrict__ raw_gate, __nv_bfloat16* __restrict__ raw_beta, float* __restrict__ A_log, float* __restrict__ dt_bias, int* __restrict__ state_indices, __nv_bfloat16* __restrict__ state, __nv_bfloat16* __restrict__ output_gate, float* __restrict__ norm_weight, __nv_bfloat16* __restrict__ output, int x_row_stride, int conv_slot_stride, int beta_row_stride, int state_slot_stride, int output_gate_row_stride, int H, int rows, int use_lower_bound, float lower_bound_log2, float norm_eps)
{
    ...
    float* sOutputScale = reinterpret_cast<float*>(smem_raw + 3600);
    const int sOutputScale_addr = smem + 3600;
    ...
    unsigned int* sStage = reinterpret_cast<unsigned int*>(smem_raw + 4736);
    const int sStage_addr = smem + 4736;
    ...
    int lane_stage_byte = state_owner_row_base * 256 + k_start * 2;
    int qkv_idx = tid_0 / 32;
    int channel_start = (tid_0 - qkv_idx * 32) * 4;
    int k_idx = tid_0 - 128;
    int total_items = rows * H;
    int grid_size = gridDim.x;
    int first_item = blockIdx.x;
    int item_count = make_warp_uniform((total_items - first_item + grid_size - 1) / grid_size);
    float state_regs[32];
    unsigned int state_carriers[4];
    float r_q[8];
    float r_k[8];
    float r_decay[8];
    float gate_in_a = 0.0f;
    float gate_in_gate = 0.0f;
    float gate_in_bias = 0.0f;
    float gate_in_out = 0.0f;
    float gate_in_weight = 0.0f;
    float gate_in_beta = 0.0f;
    int row = first_item / H;
    int head = first_item - row * H;
    int slot = state_indices[row];
    long long prologue_base = (long long)slot * (long long)state_slot_stride + (long long)(head * 128 * 128) + (long long)(state_owner_row_base * 128) + (long long)k_start;
    #pragma unroll
    for (int stage_tile = 0; stage_tile < 2; stage_tile++) {
        #pragma unroll
        for (int stage_row = 0; stage_row < 4; stage_row++) {
            asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16;"
                :: "r"(sStage_addr + (unsigned int)lane_stage_byte + (unsigned int)((stage_tile * 64 + stage_row) * 256)), "l"(state + (prologue_base + (long long)((stage_tile * 64 + stage_row) * 128))));
        }
    }
    asm volatile("cp.async.commit_group;");
    if (tid_0 < 96) {
        int prologue_channel_base = qkv_idx * hidden + head * 128 + channel_start;
        #pragma unroll
        for (int width_idx = 0; width_idx < 4; width_idx++) {
            {
                float4 _v4 = *reinterpret_cast<const float4*>(weight + (qkv_idx * 4 + width_idx) * hidden + head * 128 + channel_start);
        ...
        long long prologue_conv_base = (long long)slot * (long long)conv_slot_stride + (long long)prologue_channel_base;
        #pragma unroll
        for (int history_idx = 0; history_idx < 3; history_idx++) {
            {
                uint2 _vld_1;
                _vld_1 = *reinterpret_cast<const uint2*>(conv_state + prologue_conv_base + (long long)(history_idx * qkv_size));
        ...
        int prologue_x_base = row * x_row_stride + prologue_channel_base;
        #pragma unroll
        for (int channel_idx = 0; channel_idx < 4; channel_idx++) {
            r_q[channel_idx] = (float)x[prologue_x_base + channel_idx];
        }
    }
    if (tid_0 >= 128) {
        gate_in_a = A_log[head];
        gate_in_gate = (float)raw_gate[(row * H + head) * 128 + k_idx];
        gate_in_bias = dt_bias[head * 128 + k_idx];
        gate_in_out = (float)output_gate[row * output_gate_row_stride + head * 128 + k_idx];
        gate_in_weight = norm_weight[k_idx];
        if (tid_0 == 128) {
            gate_in_beta = (float)raw_beta[row * beta_row_stride + head];
        }
    }
    #pragma unroll 1
    for (int item_step = 0; item_step < item_count; item_step++) {
        ...
        int next_slot = slot;
        if (has_next != 0) {
            next_row = next_item / H;
            next_head = next_item - next_row * H;
            next_slot = state_indices[next_row];
        }
        if (tid_0 < 96) {
        ...
        if (tid_0 >= 128) {
            float _expf_0 = __expf(gate_in_a);
            float A = _expf_0;
            float gate = gate_in_gate + gate_in_bias;
            ...
            asm volatile("tanh.approx.f32 %0, %1;" : "=f"(_tanh_approx_5) : "f"(gate_in_out * 0.5f));
            sOutputScale[parity * 128 + k_idx] = gate_in_weight * (_tanh_approx_5 * 0.5f + 0.5f);
            if (tid_0 == 128) {
                float _tanh_approx_6;
                asm volatile("tanh.approx.f32 %0, %1;" : "=f"(_tanh_approx_6) : "f"(gate_in_beta * 0.5f));
        ...
        for (int value_tile = 0; value_tile < 2; value_tile++) {
            ...
            for (int local_row = 0; local_row < 4; local_row++) {
                asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                    ...
                    : "r"(stage_byte + (tile_base + local_row) * 256));
                #pragma unroll
            ...
            for (int row_group = 0; row_group < 2; row_group++) {
                ...
                float recurrence_value_a = state_query_pair_a.x + state_query_pair_a.y;
                float recurrence_value_b = state_query_pair_b.x + state_query_pair_b.y;
        ...
        if (warp == 3) {
            ...
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                ...
                : "r"(sOutputScale_addr + (unsigned int)(parity * 512) + (unsigned int)(lane_1 * 16)));
            #pragma unroll
        ...
        if (has_next != 0) {
            if (tid_0 < 96) {
                int next_channel_base = qkv_idx * hidden + next_head * 128 + channel_start;
                #pragma unroll
                for (int width_idx_1 = 0; width_idx_1 < 4; width_idx_1++) {
                    {
                        float4 _v4 = *reinterpret_cast<const float4*>(weight + (qkv_idx * 4 + width_idx_1) * hidden + next_head * 128 + channel_start);
                        state_regs[width_idx_1 * 4 + 0] = _v4.x;
                        state_regs[width_idx_1 * 4 + 1] = _v4.y;
                        state_regs[width_idx_1 * 4 + 2] = _v4.z;
                        state_regs[width_idx_1 * 4 + 3] = _v4.w;
                    }
                }
                long long next_conv_base = (long long)next_slot * (long long)conv_slot_stride + (long long)next_channel_base;
                #pragma unroll
                for (int history_idx_1 = 0; history_idx_1 < 3; history_idx_1++) {
                    {
                        uint2 _vld_3;
                        _vld_3 = *reinterpret_cast<const uint2*>(conv_state + next_conv_base + (long long)(history_idx_1 * qkv_size));
                        ...
                        for (int _pair = 0; _pair < 2; _pair++) {
                            asm volatile(
                                ...
                                "and.b32 %1, %2, 0xffff0000;\n\t"
                                "}\n"
                                : "=f"((&state_regs[16 + history_idx_1 * 4 + _pair * 2])[0]), "=f"((&state_regs[16 + history_idx_1 * 4 + _pair * 2])[1])
                                : "r"(_vpairs_3[_pair]));
                        }
                    }
                }
                int next_x_base = next_row * x_row_stride + next_channel_base;
                #pragma unroll
                for (int channel_idx_3 = 0; channel_idx_3 < 4; channel_idx_3++) {
                    r_q[channel_idx_3] = (float)x[next_x_base + channel_idx_3];
                }
            }
            if (tid_0 >= 128) {
                gate_in_a = A_log[next_head];
                gate_in_gate = (float)raw_gate[(next_row * H + next_head) * 128 + k_idx];
                gate_in_bias = dt_bias[next_head * 128 + k_idx];
                gate_in_out = (float)output_gate[next_row * output_gate_row_stride + next_head * 128 + k_idx];
                if (tid_0 == 128) {
                    gate_in_beta = (float)raw_beta[next_row * beta_row_stride + next_head];
                }
            }
        }
        row = next_row;
