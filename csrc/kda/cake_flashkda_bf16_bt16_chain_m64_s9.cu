...
kernel_flashkda_bf16_bt16_chain_m64(__nv_bfloat16* __restrict__ ws_qd, CakeTensorMap const* ws_qd_tma, __nv_bfloat16* __restrict__ ws_kd, CakeTensorMap const* ws_kd_tma, __nv_bfloat16* __restrict__ ws_w, CakeTensorMap const* ws_w_tma, __nv_bfloat16* __restrict__ ws_qk, CakeTensorMap const* ws_qk_tma, float* __restrict__ ws_diag, CakeTensorMap const* ws_diag_tma, __nv_bfloat16* __restrict__ v, CakeTensorMap const* v_tma, long long* __restrict__ cu_seqlens, int* __restrict__ cu_chunks, int* __restrict__ seq_order, __nv_bfloat16* __restrict__ initial_state, __nv_bfloat16* __restrict__ out, CakeTensorMap const* out_tma, __nv_bfloat16* __restrict__ final_state, int num_heads, int use_initial_state, int store_final_state, float scale)
{
    ...
    // Mbarrier init (10 groups, 76 barriers)
    ...
    if (warp == 0) {
        ...
        for (int _bar = lane; _bar < 9; _bar += 32) {
            mbarrier_init(smem + 288 + _bar * 8, 8);
        }
        for (int _bar = lane; _bar < 18; _bar += 32) {
            mbarrier_init(smem + 360 + _bar * 8, 2);
        }
        for (int _bar = lane; _bar < 9; _bar += 32) {
    ...
    #define old_out_ready_addr (mbar_base + 216)
    #define u2_inp_ready_addr (mbar_base + 288)
    #define recurrence_left_done_addr (mbar_base + 360)
    #define recurrence_right_done_addr (mbar_base + 432)
    #define output_ready_addr (mbar_base + 504)
    ...
    if (warp == 13) {
        { // mma_state_main
            ...
            for (int _chunk_idx_2 = 0; _chunk_idx_2 < num_chunks_4; _chunk_idx_2++) {
                ...
                int _mma_b_lo_2 = make_warp_uniform((((smem_kd_addr) >> 4) & 0x3FFF) + (mma_stage) * 960);
                asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 dhi, blo, ta, id;\n\t"
                ...
                int _mma_b_lo_3 = make_warp_uniform(((((smem_kr_trans_left_addr) >> 4) & 0x3FFF) | 0x800000) + (mma_stage) * 960);
                mma_ts_step(tmem_tmem_state, tmem_tmem_u2_inp, _mma_b_lo_3, 0x40004040, 68224144, 1);
                elect_commit(recurrence_left_done_addr + (mma_stage) * 8);
                int _mma_b_lo_4 = make_warp_uniform(((((smem_kr_trans_right_addr) >> 4) & 0x3FFF) | 0x800000) + (mma_stage) * 960);
                mma_ts_step((tmem_tmem_state + (64)), tmem_tmem_u2_inp, _mma_b_lo_4, 0x40004040, 68224144, 1);
                elect_commit2(recurrence_right_done_addr + (mma_stage) * 8, smem_free_addr + (mma_stage) * 8);
                mma_stage += 1;
                if (mma_stage == 9) { mma_stage = 0; _phase_state_inp_ready_1 ^= 1; _phase_u2_inp_ready_1 ^= 1; }
                mma_tma_stage += 1;
