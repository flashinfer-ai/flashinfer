...
#define NUM_CHUNK_PIPE_STAGES 9
...
#define SMEM_SMEM_OUT_OFF 139264
#define SMEM_SMEM_OUT_STAGE_BYTES 2048
#define SMEM_SMEM_OUT_STRIDE 2048
#define SMEM_SMEM_GT_ALL_OFF 15872
#define SMEM_SMEM_GT_ALL_STAGE_BYTES 123392
#define SMEM_SMEM_GT_ALL_STRIDE 123392
...
kernel_flashkda_bf16_bt16_chain_m64(__nv_bfloat16* __restrict__ ws_qd, CakeTensorMap const* ws_qd_tma, __nv_bfloat16* __restrict__ ws_kd, CakeTensorMap const* ws_kd_tma, __nv_bfloat16* __restrict__ ws_w, CakeTensorMap const* ws_w_tma, __nv_bfloat16* __restrict__ ws_qk, CakeTensorMap const* ws_qk_tma, float* __restrict__ ws_diag, CakeTensorMap const* ws_diag_tma, __nv_bfloat16* __restrict__ v, CakeTensorMap const* v_tma, long long* __restrict__ cu_seqlens, int* __restrict__ cu_chunks, int* __restrict__ seq_order, __nv_bfloat16* __restrict__ initial_state, __nv_bfloat16* __restrict__ out, CakeTensorMap const* out_tma, __nv_bfloat16* __restrict__ final_state, int num_heads, int use_initial_state, int store_final_state, float scale)
{
    ...
    __nv_bfloat16* smem_out = reinterpret_cast<__nv_bfloat16*>(smem_raw + 139264);
    const int smem_out_addr = smem + 139264;
    float* smem_gt_all = reinterpret_cast<float*>(smem_raw + 15872);
    const int smem_gt_all_addr = smem + 15872;

    // Mbarrier init (11 groups, 85 barriers)
    // Mbarriers at smem_raw[0..680)

    if (warp == 0) {
        // --- pipeline 'tma_pipe' ---
        // qk_full: 9 barriers, init_count=1
        // --- pipeline 'chunk_pipe' ---
        // smem_free: 9 barriers, init_count=2
        // state_inp_ready: 9 barriers, init_count=8
        // state_inp_left_ready: 9 barriers, init_count=4
        // old_out_ready: 9 barriers, init_count=1
        // u2_inp_ready: 9 barriers, init_count=8
        // recurrence_left_done: 9 barriers, init_count=2
        // recurrence_right_done: 9 barriers, init_count=2
        // output_ready: 9 barriers, init_count=1
        ...
        for (int _bar = lane; _bar < 9; _bar += 32) {
            mbarrier_init(smem + 72 + _bar * 8, 2);
        }
        for (int _bar = lane; _bar < 9; _bar += 32) {
            mbarrier_init(smem + 144 + _bar * 8, 8);
        }
        for (int _bar = lane; _bar < 9; _bar += 32) {
        ...
        for (int _bar = lane; _bar < 9; _bar += 32) {
            mbarrier_init(smem + 360 + _bar * 8, 8);
        }
        for (int _bar = lane; _bar < 18; _bar += 32) {
            mbarrier_init(smem + 432 + _bar * 8, 2);
        }
        for (int _bar = lane; _bar < 9; _bar += 32) {
            mbarrier_init(smem + 576 + _bar * 8, 1);
        }
        for (int _bar = lane; _bar < 3; _bar += 32) {
            mbarrier_init(smem + 648 + _bar * 8, 4);
        }
        for (int _bar = lane; _bar < 1; _bar += 32) {
            mbarrier_init(smem + 672 + _bar * 8, 3);
        }
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 680);
    if (warp == 0) {
        int _tmem_hold = smem + 680;
    ...
    #define smem_free_addr (mbar_base + 72)
    #define state_inp_ready_addr (mbar_base + 144)
    #define state_inp_left_ready_addr (mbar_base + 216)
    #define old_out_ready_addr (mbar_base + 288)
    #define u2_inp_ready_addr (mbar_base + 360)
    #define recurrence_left_done_addr (mbar_base + 432)
    #define recurrence_right_done_addr (mbar_base + 504)
    #define output_ready_addr (mbar_base + 576)
    #define out_empty_addr (mbar_base + 648)
    #define tmem_dealloc_ready_addr (mbar_base + 672)
    ...
    if (warp <= 3) {
        ...
        { // epilogue_main
            ...
            for (int chunk_idx = 0; chunk_idx < num_chunks; chunk_idx++) {
                ...
                epilogue_stage += 1;
                if (epilogue_stage == 9) { epilogue_stage = 0; _phase_output_ready ^= 1; }
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // compute_left_main
            ...
            for (int _chunk_idx = 0; _chunk_idx < num_chunks_1; _chunk_idx++) {
                ...
                if (elect_sync()) {
                    {
                        mbarrier_arrive(state_inp_left_ready_addr + (compute_left_stage) * 8);
                    }
                ...
                if (compute_left_stage == 9) { compute_left_stage = 0; _phase_recurrence_left_done ^= 1; }
                compute_left_tma_stage += 1;
                if (compute_left_tma_stage == 9) { compute_left_tma_stage = 0; compute_left_tma_phase ^= 1; }
            }
    ...
    if (warp >= 8 && warp <= 11) {
        ...
        { // compute_right_main
            ...
            for (int chunk_idx_1 = 0; chunk_idx_1 < num_chunks_2; chunk_idx_1++) {
                ...
                compute_stage += 1;
                if (compute_stage == 9) { compute_stage = 0; _phase_old_out_ready ^= 1; _phase_recurrence_right_done ^= 1; }
                compute_tma_stage += 1;
                if (compute_tma_stage == 9) { compute_tma_stage = 0; compute_tma_phase ^= 1; }
            }
            if (store_final_state != 0) {
                #pragma unroll
    ...
    if (warp == 12) {
        { // mma_q_main
            ...
            for (int _chunk_idx_1 = 0; _chunk_idx_1 < num_chunks_3; _chunk_idx_1++) {
                ...
                mma_q_stage += 1;
                if (mma_q_stage == 9) { mma_q_stage = 0; _phase_state_inp_ready ^= 1; _phase_u2_inp_ready ^= 1; }
                mma_q_output_acc_stage += 1;
                if (mma_q_output_acc_stage == 3) { mma_q_output_acc_stage = 0; _phase_out_empty ^= 1; }
                mma_q_tma_stage += 1;
                if (mma_q_tma_stage == 9) { mma_q_tma_stage = 0; mma_q_tma_phase ^= 1; }
            }
        }
    }
    // ---- Role: mma_state ----
        ...
        { // mma_state_main
            ...
            for (int _chunk_idx_2 = 0; _chunk_idx_2 < num_chunks_4; _chunk_idx_2++) {
                mbarrier_wait(qk_full_addr + (mma_tma_stage) * 8, mma_tma_phase);
                {
                    ...
                    int _mma_b_lo_2 = make_warp_uniform((((smem_kd_left_addr) >> 4) & 0x3FFF) + (mma_stage) * 960);
                    asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 dhi, blo, ta, id;\n\t"
                    ...
                    int _mma_b_lo_3 = make_warp_uniform((((smem_kd_right_addr) >> 4) & 0x3FFF) + (mma_stage) * 960);
                    asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 dhi, blo, ta, id;\n\t"
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f16 [%0], [ta], db, id, p1;\n\t"
                    "}\n"
                    :: "r"(tmem_tmem_u_acc), "r"(_mma_b_lo_3), "r"(tmem_tmem_state_inp + 32), "r"(1));
                }
                ...
                mma_stage += 1;
                if (mma_stage == 9) { mma_stage = 0; _phase_state_inp_left_ready ^= 1; _phase_state_inp_ready_1 ^= 1; _phase_u2_inp_ready_1 ^= 1; }
                mma_tma_stage += 1;
                if (mma_tma_stage == 9) { mma_tma_stage = 0; mma_tma_phase ^= 1; }
            }
    ...
    if (warp == 14) {
        { // factors_main
            ...
            for (int chunk_idx_2 = 0; chunk_idx_2 < num_chunks_5; chunk_idx_2++) {
                ...
                factor_stage += 1;
                if (factor_stage == 9) { factor_stage = 0; _phase_smem_free ^= 1; }
                factor_tma_stage += 1;
                if (factor_tma_stage == 9) { factor_tma_stage = 0; factor_tma_phase ^= 1; }
            }
        }
    }
    // ---- Role: idle ----
