...
__global__ __launch_bounds__(512, 1) void kernel_cake_flashkda_forward_checkpoint_c16_aligned(
        ...
        if (chunks_cg1 > 0) {
          ...
          if (ENABLE_CHECKPOINTS != 0 && checkpoint_chunk_cg1) {
            mbarrier_wait(checkpoint_done_addr + (checkpoint_stage_cg1) * 8,
                          (unsigned int)(checkpoint_event_cg1 / 2 + 1 & 1));
          }
        ...
        for (int chunk_cg1 = 1; chunk_cg1 < chunks_cg1; chunk_cg1++) {
          ...
          if (ENABLE_CHECKPOINTS != 0 && checkpoint_chunk_cg1_1) {
            mbarrier_wait(checkpoint_done_addr + (checkpoint_stage_cg1_1) * 8,
                          (unsigned int)(checkpoint_event_cg1_1 / 2 + 1 & 1));
          }
...
#pragma unroll
                  for (int checkpoint_segment_loop_epi = 0; checkpoint_segment_loop_epi < 2;
                    ...
                    tma_store_4d((&checkpoint_tma), checkpoint_segment_loop_epi * 64, 0, head_epi,
                                 checkpoint_base_epi +
                                     (long long)(logical_chunk_epi / CHECKPOINT_STRIDE_CHUNKS),
