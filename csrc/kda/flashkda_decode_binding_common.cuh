...
inline void CheckCudaTensor(const TensorView& tensor, const char* name,
  ...
  TVM_FFI_ICHECK(tensor.device().device_id == device_id)
      << name << " must be on CUDA device " << device_id << ", got "
      << tensor.device().device_id;
}

inline void CheckDtype(const TensorView& tensor, const char* name,
                       DLDataType expected) {
...
inline int64_t CheckedInt32(int64_t value, const char* name,
                            bool positive = false) {
  TVM_FFI_ICHECK(value >= (positive ? 1 : 0) &&
                 value <= std::numeric_limits<int32_t>::max())
      << name << " is outside the supported int32 range: " << value;
  return value;
}

inline int64_t CheckedInt32Product(std::initializer_list<int64_t> factors,
                                   const char* name) {
  int64_t product = 1;
  for (const int64_t factor : factors) {
    TVM_FFI_ICHECK(
        factor > 0 &&
        product <= std::numeric_limits<int32_t>::max() / factor)
...
inline void CheckAlignment(const TensorView& tensor, const char* name,
                           uintptr_t alignment) {
  TVM_FFI_ICHECK(reinterpret_cast<uintptr_t>(tensor.data_ptr()) % alignment ==
                 0)
      << name << " data pointer must be aligned to " << alignment << " bytes";
}

inline void CheckCompactGate(const TensorView& gate,
                             int64_t num_value_heads, int64_t head_dim) {
...
inline void CheckStateLayout(const TensorView& state,
                             int64_t num_value_heads, int64_t head_dim) {
  TVM_FFI_ICHECK(state.ndim() == 4) << "initial_state must be rank 4";
  TVM_FFI_ICHECK(state.size(0) > 0 &&
                 state.size(1) == num_value_heads &&
                 state.size(2) == head_dim && state.size(3) == head_dim)
      << "initial_state must have shape [slots, HV, 128, 128]";
  TVM_FFI_ICHECK(
      state.stride(3) == 1 && state.stride(2) == head_dim &&
...
inline std::pair<uintptr_t, uintptr_t> TensorByteRange(
    const TensorView& tensor, const char* name) {
  const DLDataType dtype = tensor.dtype();
  const uint64_t bits = static_cast<uint64_t>(dtype.bits) * dtype.lanes;
  TVM_FFI_ICHECK(bits > 0 && bits % 8 == 0)
      << name << " has a non-byte dtype";
  ...
  for (int32_t i = 0; i < tensor.ndim(); ++i) {
    ...
    if (tensor.size(i) > 0) {
      last_element += static_cast<uint64_t>(tensor.size(i) - 1) *
                      static_cast<uint64_t>(tensor.stride(i));
    }
  }

  const uint64_t bytes =
      tensor.numel() == 0 ? 0 : (last_element + 1) * (bits / 8);
...
inline void CheckNoOverlap(const TensorView& lhs, const char* lhs_name,
  ...
  TVM_FFI_ICHECK(lhs_range.first >= rhs_range.second ||
                 rhs_range.first >= lhs_range.second)
      << lhs_name << " must not overlap " << rhs_name
      << ": the frozen kernel uses __restrict__ pointers";
}

template <int kValueSplit>
LaunchContext CheckInputs(
    const TensorView& q, const TensorView& k, const TensorView& v,
    const TensorView& g, const TensorView& beta, const TensorView& A_log,
    const TensorView& dt_bias, const TensorView& state,
    const TensorView& out, const TensorView& cu_seqlens,
    const TensorView& ssm_state_indices,
    const TensorView& num_accepted_tokens, double scale,
    double lower_bound, int64_t cuda_stream) {
  static_assert(kValueSplit == 1 || kValueSplit == 2 ||
                kValueSplit == 4 || kValueSplit == 8);
  ...
  for (const auto& named :
       {std::pair<const TensorView*, const char*>(&q, "q"),
        ...
        std::pair<const TensorView*, const char*>(&cu_seqlens,
                                                 "cu_seqlens"),
        std::pair<const TensorView*, const char*>(&ssm_state_indices,
                                                 "ssm_state_indices"),
        std::pair<const TensorView*, const char*>(&num_accepted_tokens,
                                                 "num_accepted_tokens")}) {
  ...
  TVM_FFI_ICHECK(k.ndim() == 4 && k.size(0) == 1 &&
                 k.size(1) == total_tokens && k.size(2) == num_heads &&
                 k.size(3) == kHeadDim)
      << "k must match q";

  const int64_t num_value_heads = v.ndim() == 4 ? v.size(2) : 0;
  TVM_FFI_ICHECK(v.ndim() == 4 && v.size(0) == 1 &&
                 v.size(1) == total_tokens && v.size(3) == kHeadDim)
      << "v must have shape [1, total_tokens, HV, 128]";
  TVM_FFI_ICHECK(num_value_heads >= num_heads &&
                 num_value_heads % num_heads == 0)
      << "HV must be a positive multiple of H";
  TVM_FFI_ICHECK(g.ndim() == 4 && g.size(0) == 1 &&
                 g.size(1) == total_tokens &&
  ...
  TVM_FFI_ICHECK(out.ndim() == 4 && out.size(0) == 1 &&
                 out.size(1) == total_tokens &&
                 out.size(2) == num_value_heads &&
                 out.size(3) == kHeadDim)
      << "output must match v";
  ...
  TVM_FFI_ICHECK(num_sequences <= 65535)
      << "number of sequences exceeds CUDA grid.y limit";
  TVM_FFI_ICHECK(total_tokens == num_sequences * kTokens)
      << "packed speculative input must contain exactly N * 5 tokens";
  TVM_FFI_ICHECK(ssm_state_indices.numel() ==
                 num_sequences * kTokens)
      << "ssm_state_indices must contain N * 5 elements";
  TVM_FFI_ICHECK(num_accepted_tokens.ndim() == 1 &&
                 num_accepted_tokens.numel() == num_sequences)
  ...
  TVM_FFI_ICHECK(lower_bound == 0.0)
      << "precomputed-gate variants require lower_bound=0";
  TVM_FFI_ICHECK(std::isfinite(scale) &&
                 std::isfinite(static_cast<float>(scale)))
      << "scale must be finite and representable as float32";

  CheckedInt32Product({total_tokens, num_heads, kHeadDim},
                      "q/k linear index extent");
  CheckedInt32Product({total_tokens, num_value_heads, kHeadDim},
                      "v/g/output linear index extent");
  CheckedInt32Product({total_tokens, g.stride(1)},
                      "g strided linear index extent");
  CheckedInt32Product({num_sequences, kTokens},
                      "metadata linear index extent");
  CheckedInt32Product({state.size(0), state.stride(0)},
                      "state linear index extent");
  CheckedInt32Product({num_value_heads, kValueSplit},
                      "decode grid.x extent");
  ...
  return {
      device_id,
      static_cast<int32_t>(
          CheckedInt32(num_sequences, "number of sequences", true)),
      ...
      static_cast<int32_t>(
          CheckedInt32(g.stride(1), "g token stride", true)),
      static_cast<int32_t>(
          CheckedInt32(state.stride(0), "state slot stride", true)),
...
inline void CheckDynamicSmemCapacity(int32_t device_id, int32_t required) {
  ...
  CheckCuda(cudaDeviceGetAttribute(
                &capacity, cudaDevAttrMaxSharedMemoryPerBlockOptin,
                device_id),
