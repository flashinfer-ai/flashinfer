...
#define PERSISTENT_SCHEDULE 0
#define SCHEDULE_STRIDE 1
...
kernel_flashkda_blackwell_evolution_m128_h96_p0_s1(__nv_bfloat16* __restrict__ q, CakeTensorMap const* q_tma, __nv_bfloat16* __restrict__ k, CakeTensorMap const* k_tma, __nv_bfloat16* __restrict__ v, CakeTensorMap const* v_tma, __nv_bfloat16* __restrict__ g, CakeTensorMap const* g_tma, __nv_bfloat16* __restrict__ beta, CakeTensorMap const* beta_tma, float* __restrict__ A_log, float* __restrict__ dt_bias, long long* __restrict__ cu_seqlens, int* __restrict__ seq_order, int* __restrict__ tile_schedule, int* __restrict__ tile_schedule_counts, __nv_bfloat16* __restrict__ initial_state, __nv_bfloat16* __restrict__ out, CakeTensorMap const* out_tma, __nv_bfloat16* __restrict__ final_state, int num_heads, int use_initial_state, int store_final_state, float scale, float lower_bound)
{
                        ...
                        {
                            {
                                ...
                                asm volatile(
                                    ...
                                    :: "l"((void*)(&((__nv_bfloat16*)(final_state + (current_state_base + (long long)(state_col_block_2 * 32))))[0])), "r"(_pk_u0), "r"(_pk_u1), "r"(_pk_u2), "r"(_pk_u3), "r"(_pk_u4), "r"(_pk_u5), "r"(_pk_u6), "r"(_pk_u7) : "memory");
                            }
                            {
                                ...
                                asm volatile(
                                    "st.global.v8.b32 [%0], {%1, %2, %3, %4, %5, %6, %7, %8};"
                                    :: "l"((void*)(&((__nv_bfloat16*)(final_state + (current_state_base + (long long)(state_col_block_2 * 32))))[16])), "r"(_pk_u0), "r"(_pk_u1), "r"(_pk_u2), "r"(_pk_u3), "r"(_pk_u4), "r"(_pk_u5), "r"(_pk_u6), "r"(_pk_u7) : "memory");
                            }
                        }
                    }
                }
                    ...
                    {
                        {
                            ...
                            asm volatile(
                                "st.global.v8.b32 [%0], {%1, %2, %3, %4, %5, %6, %7, %8};"
                                :: "l"((void*)(&((__nv_bfloat16*)(final_state + (state_base + (long long)(state_col_block_3 * 32))))[0])), "r"(_pk_u0), "r"(_pk_u1), "r"(_pk_u2), "r"(_pk_u3), "r"(_pk_u4), "r"(_pk_u5), "r"(_pk_u6), "r"(_pk_u7) : "memory");
                        }
                        {
                            ...
                            asm volatile(
                                "st.global.v8.b32 [%0], {%1, %2, %3, %4, %5, %6, %7, %8};"
                                :: "l"((void*)(&((__nv_bfloat16*)(final_state + (state_base + (long long)(state_col_block_3 * 32))))[16])), "r"(_pk_u0), "r"(_pk_u1), "r"(_pk_u2), "r"(_pk_u3), "r"(_pk_u4), "r"(_pk_u5), "r"(_pk_u6), "r"(_pk_u7) : "memory");
                        }
                    }
                }
            }
    ...
    // ---- Role: mma ----
        ...
        { // mma_main
            ...
            for (int _global_chunk_idx = 0; _global_chunk_idx < total_chunks_2; _global_chunk_idx++) {
                ...
                {
                    {
                        int _mma_b_lo_2 = make_warp_uniform((((smem_kd_addr) >> 4) & 0x3FFF) + (mma_stage) * 2624);
                        asm volatile(
                    "{\n\t"
                    ...
                    :: "r"(tmem_tmem_u_acc), "r"(_mma_b_lo_2), "r"(tmem_tmem_state_inp), "r"(0));
                        ...
                        int _mma_b_lo_3 = make_warp_uniform((((smem_qd_addr) >> 4) & 0x3FFF) + (mma_stage) * 2624);
                        asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 dhi, blo, ta, id;\n\t"
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f16 [%0], [ta], db, id, p1;\n\t"
                    "}\n"
                    :: "r"(tmem_tmem_out), "r"(_mma_b_lo_3), "r"(tmem_tmem_state_inp), "r"(0));
    ...
    // ---- Role: prep ----
        ...
        { // prep_main
            ...
            for (int prep_iter = 0; prep_iter < num_prep_iters; prep_iter++) {
                ...
                {
                    gate_rate_cached = smem_gate_rate_all[prep_stage * 10496];
                }
                ...
                if (prep_local_warp == 1) {
                    ...
                    for (int restore_pass = 0; restore_pass < 4; restore_pass++) {
                        ...
                        int restore_publish_col = restore_col;
                        {
                            ...
                            restore_publish_col = cycle_4_1 * 32 + group_5_1 * 4 + (elem_1_1 & 3);
                        }
                ...
                if (prep_local_warp == 0) {
                    ...
                    int restore_segment_2 = lane & 15;
                    {
                        ...
                        int segment_8 = (restore_segment_2 * 8 + restore_half_id_2 * 4) / 8;
                        int elem_8 = restore_segment_2 * 8 + restore_half_id_2 * 4 & 7;
                        int half_8 = elem_8 / 4;
                        int vector_idx_9 = segment_8 * 2 + half_8;
                        int cycle_9 = vector_idx_9 / 8;
                        int group_9 = (segment_8 & 3) + (segment_8 / 4 + half_8 & 1) * 4;
                        int restore_vector_col_2 = cycle_9 * 32 + group_9 * 4 + (elem_8 & 3);
