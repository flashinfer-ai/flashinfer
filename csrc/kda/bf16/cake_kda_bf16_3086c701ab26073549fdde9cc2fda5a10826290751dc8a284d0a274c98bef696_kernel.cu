...
#define SMEM_SMEM_V18_OFF 154624
...
kernel_cake_kda_bf16_3086c701ab26073549fdde9cc2fda5a10826290751dc8a284d0a274c98bef696(__nv_bfloat16* __restrict__ q, CakeTensorMap const* q_tma, __nv_bfloat16* __restrict__ k, CakeTensorMap const* k_tma, __nv_bfloat16* __restrict__ v, CakeTensorMap const* v_tma, __nv_bfloat16* __restrict__ g, CakeTensorMap const* g_tma, __nv_bfloat16* __restrict__ beta, CakeTensorMap const* beta_tma, float* __restrict__ A_log, float* __restrict__ dt_bias, long long* __restrict__ cu_seqlens, int* __restrict__ seq_order, __nv_bfloat16* __restrict__ initial_state, __nv_bfloat16* __restrict__ out, CakeTensorMap const* out_tma, __nv_bfloat16* __restrict__ final_state, int num_heads, int use_initial_state, int store_final_state, float scale, float lower_bound, unsigned long long state_indices_addr, unsigned long long state_checkpoints_addr, unsigned long long checkpoint_cu_starts_addr, long long beta_token_stride, long long state_slot_stride, int use_state_indices, int checkpoint_every_n_tokens, long long* __restrict__ cu_chunk_offsets, __nv_bfloat16* __restrict__ chunk_state, unsigned int* __restrict__ state_checkpoint_needed, __nv_bfloat16* __restrict__ tape_qd, __nv_bfloat16* __restrict__ tape_kd, __nv_bfloat16* __restrict__ tape_kr, __nv_bfloat16* __restrict__ tape_j, float* __restrict__ tape_restore_factor, __nv_bfloat16* __restrict__ tape_e, __nv_bfloat16* __restrict__ tape_x, __nv_bfloat16* __restrict__ tape_r, float* __restrict__ norm_inv_out, __nv_bfloat16* __restrict__ decay_out, float* __restrict__ beta_active_out, float* __restrict__ initial_state_f32, unsigned int* __restrict__ zero_workspace, int zero_words, int num_sequences, CakeTensorMap const* state_checkpoints_tma, float* __restrict__ final_state_f32, long long g_token_stride)
{
    ...
    #define checkpoint_snapshot_done_addr (mbar_base + 608)
    #define work_item_ready_addr (mbar_base + 640)
    #define aux_pairwise_inputs_ready_addr (mbar_base + 648)
    #define aux_pairwise_consumed_addr (mbar_base + 688)
    #define aux_inverse_ready_addr (mbar_base + 728)
    #define short_beta_ready_addr (mbar_base + 768)
    ...
    __nv_bfloat16* smem_v18 = reinterpret_cast<__nv_bfloat16*>(smem_raw + 154624);
    const int smem_v18_addr = smem + 154624;
    ...
    // Mbarrier init (24 pipeline groups, 0 ordered-sequence groups, 101 barriers)
    // Mbarriers at smem_raw[0..808)
    ...
    if (warp == 10) {
        ...
        uint32_t _mbarrier_init_count_10_64 = 32;
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_10_64) : "r"(lane), "n"(22), "r"((uint32_t)(128)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_10_64) : "r"(lane), "n"(17), "r"((uint32_t)(1)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_10_64) : "r"(lane), "n"(16), "r"((uint32_t)(4)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_10_64) : "r"(lane), "n"(12), "r"((uint32_t)(1)));
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 808);
    if (warp == 0) {
        int _tmem_hold = smem + 808;
        ...
        { // compute_main
            ...
            for (int chunk_idx = 0; chunk_idx < num_chunks_0; chunk_idx++) {
                ...
                for (int state_col_block_2 = 0; state_col_block_2 < ((1) ? 4 : 3); state_col_block_2++) {
                    ...
                    if (!0 && checkpoint_entering != 0 && chunk_idx != 0) {
                        long long entering_checkpoint_idx = checkpoint_row_start + (long long)(chunk_idx * 16 / checkpoint_every_n_tokens);
                    ...
                    }
                    {
                        ...
                        for (int residual_row_half_1 = 0; residual_row_half_1 < 2; residual_row_half_1++) {
                            ...
                            for (int token_group_1 = 0; token_group_1 < 2; token_group_1++) {
                                ...
                                {
                                    beta_pair = smem_prep_beta_u32_all[compute_stage * 5376 + (unsigned int)(token_group_1 * 4) + (unsigned int)(lane & 3)];
                                }
                                #pragma unroll
            ...
            unsigned int _phase_aux_pairwise_inputs_ready = 0;
            {
                ...
                for (int aux_iter = 0; aux_iter < num_aux_iters; aux_iter++) {
                    ...
                    float beta1 = 0.0f;
                    {
                        ...
                        float _cvt_f32_27 = __bfloat162float(beta0_bf16);
                        beta0 = _cvt_f32_27;
                        float _cvt_f32_28 = __bfloat162float(beta1_bf16);
                        beta1 = _cvt_f32_28;
                    }
                    float l_values[8];
                    l_values[0] = 0.0f;
                    ...
                    if (row0 > col0) {
                        ...
                        float _cvt_f32_29 = __bfloat162float(_cvt_bf16_14);
                        l_values[0] = _cvt_f32_29;
                    }
                    if (row0 > col0 + 1) {
                        __nv_bfloat16 _cvt_bf16_15 = __float2bfloat16(acc[1] * beta0);
                        float _cvt_f32_30 = __bfloat162float(_cvt_bf16_15);
                        l_values[1] = _cvt_f32_30;
                    }
                    if (row1 > col0) {
                        __nv_bfloat16 _cvt_bf16_16 = __float2bfloat16(acc[2] * beta1);
                        float _cvt_f32_31 = __bfloat162float(_cvt_bf16_16);
                        l_values[2] = _cvt_f32_31;
                    }
                    if (row1 > col0 + 1) {
                        __nv_bfloat16 _cvt_bf16_17 = __float2bfloat16(acc[3] * beta1);
                        float _cvt_f32_32 = __bfloat162float(_cvt_bf16_17);
                        l_values[3] = _cvt_f32_32;
                    }
                    if (row0 > col0 + 8) {
                        __nv_bfloat16 _cvt_bf16_18 = __float2bfloat16(acc[4] * beta0);
                        float _cvt_f32_33 = __bfloat162float(_cvt_bf16_18);
                        l_values[4] = _cvt_f32_33;
                    }
                    if (row0 > col0 + 9) {
                        __nv_bfloat16 _cvt_bf16_19 = __float2bfloat16(acc[5] * beta0);
                        float _cvt_f32_34 = __bfloat162float(_cvt_bf16_19);
                        l_values[5] = _cvt_f32_34;
                    }
                    if (row1 > col0 + 8) {
                        __nv_bfloat16 _cvt_bf16_20 = __float2bfloat16(acc[6] * beta1);
                        float _cvt_f32_35 = __bfloat162float(_cvt_bf16_20);
                        l_values[6] = _cvt_f32_35;
                    }
                    if (row1 > col0 + 9) {
                        __nv_bfloat16 _cvt_bf16_21 = __float2bfloat16(acc[7] * beta1);
                        float _cvt_f32_36 = __bfloat162float(_cvt_bf16_21);
                        l_values[7] = _cvt_f32_36;
                    }
            ...
            for (int prep_iter = 0; prep_iter < num_prep_iters; prep_iter++) {
                ...
                if (prep_tid < 128) {
                    ...
                    float prefix_log2 = 0.0f;
                    {
                        ...
                        long long first_gate_token = bos_4 + (long long)(chunk_idx_3 * 16);
                        ...
                        if (chunk_is_full_1 == 0) {
                            ...
                            float first_gate_value = lower_bound * 1.4426950408889634f * gate_sigmoid;
                            first_gate_log2 = ((first_gate_token < eos_4) ? first_gate_value : 0.0f);
                        }
                        prefix_log2 += first_gate_log2;
                        smem_gate_all[stage_f32_1 + gate_col] = prefix_log2;
                        for (int gate_pair = 0; gate_pair < 7; gate_pair++) {
                            int pair_row0 = gate_pair * 2 + 1;
                            int pair_row1 = pair_row0 + 1;
                            long long pair_token0 = bos_4 + (long long)(chunk_idx_3 * 16 + pair_row0);
                            ...
                            float gate_arg_1 = gate_rate * (_cvt_f32_8 + gate_bias);
                            float _tanh_approx_9;
                            asm volatile("tanh.approx.f32 %0, %1;" : "=f"(_tanh_approx_9) : "f"(gate_arg_1 * 0.5f));
                            float gate_sigmoid_1 = _tanh_approx_9 * 0.5f + 0.5f;
                            float _cvt_f32_9 = __bfloat162float(pair_raw1);
                            float gate_arg_0 = gate_rate * (_cvt_f32_9 + gate_bias);
                            float _tanh_approx_10;
                            asm volatile("tanh.approx.f32 %0, %1;" : "=f"(_tanh_approx_10) : "f"(gate_arg_0 * 0.5f));
                            float gate_sigmoid_1_1 = _tanh_approx_10 * 0.5f + 0.5f;
                            float pair_log0 = ((pair_token0 < eos_4) ? lower_bound * 1.4426950408889634f * gate_sigmoid_1 : 0.0f);
                            float pair_log1 = ((pair_token1 < eos_4) ? lower_bound * 1.4426950408889634f * gate_sigmoid_1_1 : 0.0f);
                            prefix_log2 += pair_log0;
                            smem_gate_all[stage_f32_1 + pair_row0 * 128 + gate_col] = prefix_log2;
                            prefix_log2 += pair_log1;
                            smem_gate_all[stage_f32_1 + pair_row1 * 128 + gate_col] = prefix_log2;
                        }
                        long long last_gate_token = bos_4 + (long long)(chunk_idx_3 * 16 + 15);
                        __nv_bfloat16 last_gate_raw = smem_g_raw_all[stage_bf16 + 1920 + gate_col];
                        float _cvt_f32_10 = __bfloat162float(last_gate_raw);
                        float gate_arg_2 = gate_rate * (_cvt_f32_10 + gate_bias);
                        float _tanh_approx_11;
                        asm volatile("tanh.approx.f32 %0, %1;" : "=f"(_tanh_approx_11) : "f"(gate_arg_2 * 0.5f));
                        float gate_sigmoid_2 = _tanh_approx_11 * 0.5f + 0.5f;
                        float last_gate_value = lower_bound * 1.4426950408889634f * gate_sigmoid_2;
                        float last_gate_log2 = ((last_gate_token < eos_4) ? last_gate_value : 0.0f);
                        prefix_log2 += last_gate_log2;
                        smem_gate_all[stage_f32_1 + 1920 + gate_col] = prefix_log2;
                    }
                }
                asm volatile("barrier.sync %0, 128;" :: "r"(10 + prep_instance) : "memory");
                ...
                for (int work_pass = 0; work_pass < 2; work_pass++) {
                    ...
                    const float2 _scale2_3 = {_cvt_f32_11, _cvt_f32_11};
                    #pragma unroll
                    for (int _ls = 0; _ls < 4; _ls++)
                ...
                }
                {
                    ...
                    for (int restore_k_pass = 0; restore_k_pass < 2; restore_k_pass++) {
                        ...
                        for (int restore_elem = 0; restore_elem < 8; restore_elem++) {
                            ...
                            float _cvt_f32_24 = __bfloat162float(_cvt_bf16_11);
                            float restore_ki_carrier = _cvt_f32_24;
                            ...
                            float _cvt_f32_25 = __bfloat162float(_cvt_bf16_12);
                            float restore_total_carrier = _cvt_f32_25;
                            __nv_bfloat16 _cvt_bf16_13 = __float2bfloat16(restore_ki_carrier * restore_total_carrier);
                            float _cvt_f32_26 = __bfloat162float(_cvt_bf16_13);
                            restore_kr_values[restore_elem] = _cvt_f32_26;
                        }
