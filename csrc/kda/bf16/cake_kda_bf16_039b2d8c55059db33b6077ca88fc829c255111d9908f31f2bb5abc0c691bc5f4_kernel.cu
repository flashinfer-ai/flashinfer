...
kernel_cake_kda_bf16_039b2d8c55059db33b6077ca88fc829c255111d9908f31f2bb5abc0c691bc5f4(__nv_bfloat16* __restrict__ q, CakeTensorMap const* q_tma, __nv_bfloat16* __restrict__ k, CakeTensorMap const* k_tma, __nv_bfloat16* __restrict__ v, CakeTensorMap const* v_tma, __nv_bfloat16* __restrict__ g, CakeTensorMap const* g_tma, __nv_bfloat16* __restrict__ beta, CakeTensorMap const* beta_tma, float* __restrict__ A_log, float* __restrict__ dt_bias, long long* __restrict__ cu_seqlens, int* __restrict__ seq_order, __nv_bfloat16* __restrict__ initial_state, __nv_bfloat16* __restrict__ out, CakeTensorMap const* out_tma, __nv_bfloat16* __restrict__ final_state, int num_heads, int use_initial_state, int store_final_state, float scale, float lower_bound, unsigned long long state_indices_addr, unsigned long long state_checkpoints_addr, unsigned long long checkpoint_cu_starts_addr, long long beta_token_stride, long long state_slot_stride, int use_state_indices, int checkpoint_every_n_tokens, long long* __restrict__ cu_chunk_offsets, __nv_bfloat16* __restrict__ chunk_state, unsigned int* __restrict__ state_checkpoint_needed, __nv_bfloat16* __restrict__ tape_qd, __nv_bfloat16* __restrict__ tape_kd, __nv_bfloat16* __restrict__ tape_kr, __nv_bfloat16* __restrict__ tape_j, float* __restrict__ tape_restore_factor, __nv_bfloat16* __restrict__ tape_e, __nv_bfloat16* __restrict__ tape_x, __nv_bfloat16* __restrict__ tape_r, float* __restrict__ norm_inv_out, __nv_bfloat16* __restrict__ decay_out, float* __restrict__ beta_active_out, float* __restrict__ initial_state_f32, unsigned int* __restrict__ zero_workspace, int zero_words, int num_sequences, CakeTensorMap const* state_checkpoints_tma, float* __restrict__ final_state_f32, long long g_token_stride)
{
    ...
    if (warp == 10) {
        ...
        // smem_free: 5 barriers, init_count=129
        // raw_inputs_free: 5 barriers, init_count=2
        ...
        uint32_t _mbarrier_init_count_10_0 = 2;
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_10_0) : "r"(lane), "n"(30), "r"((uint32_t)(129)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_10_0) : "r"(lane), "n"(25), "r"((uint32_t)(128)));
        ...
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_10_32) : "r"(lane), "n"(3), "r"((uint32_t)(2)));
    ...
    // ---- Role: mma ----
        ...
        { // mma_main
            ...
            for (int _chunk_idx = 0; _chunk_idx < num_chunks_0_2; _chunk_idx++) {
                ...
                {
                    long long op_chunk = cu_chunk_offsets[seq_idx_2] + (long long)_chunk_idx;
                    long long op_vec_lane = (op_chunk * (long long)num_heads + (long long)head_idx_2) * 292;
                    unsigned int beta_stage_f32 = mma_stage * 10496;
                    tape_restore_factor[op_vec_lane + (long long)(257 + lane)] = smem_prep_beta_all[beta_stage_f32 + (unsigned int)lane];
                    if (elect_sync()) {
                        long long op_chunk_0 = cu_chunk_offsets[seq_idx_2] + (long long)_chunk_idx;
                        long long op_slab = op_chunk_0 * (long long)num_heads + (long long)head_idx_2;
                        long long op_vec = op_slab * 292;
                        unsigned int op_stage_bytes = mma_stage * 41984;
                        tape_restore_factor[op_vec + 256] = smem_restore_factor_all[beta_stage_f32 + 128];
                        tape_restore_factor[op_vec + 289] = smem_gate_rate_all[beta_stage_f32];
                        {
                            void* _cpbulk_dst_0 = reinterpret_cast<void*>(tape_qd + (op_slab * 4096));
                            asm volatile(
                                "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
                                :: "l"(_cpbulk_dst_0), "r"(smem_qd_addr + mma_stage * 41984), "r"((uint32_t)(8192))
                                : "memory");
                        }
                        {
                            void* _cpbulk_dst_1 = reinterpret_cast<void*>(tape_kd + (op_slab * 4096));
                            asm volatile(
                                "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
                                :: "l"(_cpbulk_dst_1), "r"(smem_kd_addr + mma_stage * 41984), "r"((uint32_t)(8192))
                                : "memory");
                        }
                        {
                            void* _cpbulk_dst_2 = reinterpret_cast<void*>(tape_restore_factor + op_vec);
                            asm volatile(
                                "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
                                :: "l"(_cpbulk_dst_2), "r"(smem_gt_all_addr + op_stage_bytes), "r"((uint32_t)(512))
                                : "memory");
                        }
                        {
                            void* _cpbulk_dst_3 = reinterpret_cast<void*>(tape_restore_factor + (op_vec + 128));
                            asm volatile(
                                "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
                                :: "l"(_cpbulk_dst_3), "r"(smem_restore_factor_all_addr + op_stage_bytes), "r"((uint32_t)(512))
                                : "memory");
                        }
                        asm volatile("cp.async.bulk.commit_group;");
                    }
                ...
                {
                    if (elect_sync()) {
                        long long op_chunk_1 = cu_chunk_offsets[seq_idx_2] + (long long)_chunk_idx;
                        {
                            void* _cpbulk_dst_4 = reinterpret_cast<void*>(tape_j + ((op_chunk_1 * (long long)num_heads + (long long)head_idx_2) * 1024));
                            asm volatile(
                                "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
                                :: "l"(_cpbulk_dst_4), "r"(smem_inv_addr + mma_stage * 41984), "r"((uint32_t)(2048))
                                : "memory");
                        }
                        asm volatile("cp.async.bulk.commit_group;");
                    }
                ...
                {
                    if (elect_sync()) {
                        long long op_chunk_2 = cu_chunk_offsets[seq_idx_2] + (long long)_chunk_idx;
                        long long op_slab_final = (op_chunk_2 * (long long)num_heads + (long long)head_idx_2) * 6144;
                        {
                            void* _cpbulk_dst_5 = reinterpret_cast<void*>(tape_kr + op_slab_final);
                            asm volatile(
                                "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
                                :: "l"(_cpbulk_dst_5), "r"(smem_kr_trans_addr + mma_stage * 41984), "r"((uint32_t)(8192))
                                : "memory");
                        }
                        {
                            void* _cpbulk_dst_6 = reinterpret_cast<void*>(tape_kr + (op_slab_final + 4096));
                            asm volatile(
                                "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
                                :: "l"(_cpbulk_dst_6), "r"(smem_final_mqk_slab_addr + mma_stage * 41984), "r"((uint32_t)(4096))
                                : "memory");
                        }
                        asm volatile("cp.async.bulk.commit_group;");
                    }
                ...
                elect_commit(final_ready_addr + (mma_stage) * 8);
                {
                    if (elect_sync()) {
                        asm volatile("cp.async.bulk.wait_group.read 0;");
                        mbarrier_arrive(raw_inputs_free_addr + (mma_stage) * 8);
                        mbarrier_arrive(smem_free_addr + (mma_stage) * 8);
                    }
                }
                mma_stage += 1;
