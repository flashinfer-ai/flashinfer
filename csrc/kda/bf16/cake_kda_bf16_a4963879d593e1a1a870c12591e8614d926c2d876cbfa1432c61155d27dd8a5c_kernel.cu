        ...
        { // compute_main
            ...
            for (int chunk_idx = 0; chunk_idx < num_chunks; chunk_idx++) {
                ...
                for (int token_group = 0; token_group < 4; token_group++) {
                    ...
                    {
                        residual_values[residual_reg_base] = (v_ld_bits_f32[0] - _tmem_load_3[residual_reg_base]) * beta_0;
                        residual_values[residual_reg_base + 1] = (v_ld_bits_f32[1] - _tmem_load_3[residual_reg_base + 1]) * beta_1;
                        residual_values[residual_reg_base + 2] = (v_ld_bits_f32[2] - _tmem_load_3[residual_reg_base + 2]) * beta_0;
                        residual_values[residual_reg_base + 3] = (v_ld_bits_f32[3] - _tmem_load_3[residual_reg_base + 3]) * beta_1;
                    }
                }
            ...
            for (int prep_iter = 0; prep_iter < num_prep_iters; prep_iter++) {
                ...
                if (chunk_is_full_2 != 0) {
                    if (prep_local_warp == 0) {
                        if (elect_sync()) {
                            mbarrier_arrive_expect_tx(gate_raw_full_addr + (prep_stage) * 8, 32 * (128 + ((0) ? 0 : 8)) * 2);
                ...
                if (chunk_is_full_2 != 0) {
                    ...
                    if (!0 && prep_local_warp == 2 && lane < 32) {
                ...
                if (prep_local_warp == 2 && lane < 32) {
                    ...
                    if (beta_token < eos_4) {
                        {
                            if (chunk_is_full_2 != 0) {
                                beta_value = early_beta_value;
                            } else {
                                float beta_logit_1 = (float)beta[beta_token * beta_token_stride + (long long)head_idx_3];
