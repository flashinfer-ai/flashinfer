                    ...
                    if (prep_local_warp == 2 && lane < 32) {
                        {
                            float beta_logit = 0.0f;
                            {
                                ...
                                __nv_bfloat16 beta_pair_value = smem_beta_raw_all[(unsigned int)stage_bf16 + ((unsigned int)beta_start_parity + lane) * 12 + (unsigned int)head_idx_3];
