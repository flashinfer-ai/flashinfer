...
#define SMEM_SMEM_V25_OFF 211968
#define SMEM_SMEM_V25_STAGE_BYTES 8192
#define SMEM_SMEM_V25_STRIDE 8192
...
kernel_cake_kda_bf16_2e0287ba84d4f2efe3b651e83023390d4fb843e4982ea5e18d616438a2f9718f(__nv_bfloat16* __restrict__ q, CakeTensorMap const* q_tma, __nv_bfloat16* __restrict__ k, CakeTensorMap const* k_tma, __nv_bfloat16* __restrict__ v, CakeTensorMap const* v_tma, __nv_bfloat16* __restrict__ g, CakeTensorMap const* g_tma, __nv_bfloat16* __restrict__ beta, CakeTensorMap const* beta_tma, float* __restrict__ A_log, float* __restrict__ dt_bias, long long* __restrict__ cu_seqlens, int* __restrict__ seq_order, __nv_bfloat16* __restrict__ initial_state, __nv_bfloat16* __restrict__ out, CakeTensorMap const* out_tma, __nv_bfloat16* __restrict__ final_state, int num_heads, int use_initial_state, int store_final_state, float scale, float lower_bound, unsigned long long state_indices_addr, unsigned long long state_checkpoints_addr, unsigned long long checkpoint_cu_starts_addr, long long beta_token_stride, long long state_slot_stride, int use_state_indices, int checkpoint_every_n_tokens, long long* __restrict__ cu_chunk_offsets, __nv_bfloat16* __restrict__ chunk_state, unsigned int* __restrict__ state_checkpoint_needed, __nv_bfloat16* __restrict__ tape_qd, __nv_bfloat16* __restrict__ tape_kd, __nv_bfloat16* __restrict__ tape_kr, __nv_bfloat16* __restrict__ tape_j, float* __restrict__ tape_restore_factor, __nv_bfloat16* __restrict__ tape_e, __nv_bfloat16* __restrict__ tape_x, __nv_bfloat16* __restrict__ tape_r, float* __restrict__ norm_inv_out, __nv_bfloat16* __restrict__ decay_out, float* __restrict__ beta_active_out, float* __restrict__ initial_state_f32, unsigned int* __restrict__ zero_workspace, int zero_words, int num_sequences, CakeTensorMap const* state_checkpoints_tma, float* __restrict__ final_state_f32, long long g_token_stride)
{
    ...
    #define checkpoint_snapshot_done_addr (mbar_base + 592)
    #define work_item_ready_addr (mbar_base + 624)
    #define short_beta_ready_addr (mbar_base + 632)
    #define page_prefix_ready_addr (mbar_base + 664)
    #define page_total_ready_addr (mbar_base + 680)
    #define page_local_ready_addr (mbar_base + 696)
    #define page_aux_operands_ready_addr (mbar_base + 728)
    #define page_cross_ready_addr (mbar_base + 744)
    #define page_scratch_free_addr (mbar_base + 760)
    #define page_operand_ready_addr (mbar_base + 768)
    #define page_operand_free_addr (mbar_base + 776)
    #define page_result_ready_addr (mbar_base + 784)
    #define page_result_free_addr (mbar_base + 792)
    ...
    __nv_bfloat16* smem_v25 = reinterpret_cast<__nv_bfloat16*>(smem_raw + 211968);
    const int smem_v25_addr = smem + 211968;
    ...
    // Mbarrier init (34 pipeline groups, 0 ordered-sequence groups, 100 barriers)
    // Mbarriers at smem_raw[0..800)
    ...
    if (warp == 10) {
        ...
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_10_64) : "r"(lane), "n"(31), "r"((uint32_t)(64)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_10_64) : "r"(lane), "n"(27), "r"((uint32_t)(128)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_10_64) : "r"(lane), "n"(19), "r"((uint32_t)(1)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_10_64) : "r"(lane), "n"(14), "r"((uint32_t)(4)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_10_64) : "r"(lane), "n"(10), "r"((uint32_t)(1)));
        ...
        uint32_t _mbarrier_init_count_10_96 = 4;
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_10_96) : "r"(lane), "n"(3), "r"((uint32_t)(1)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_10_96) : "r"(lane), "n"(1), "r"((uint32_t)(4)));
        if (lane < 4) {
            mbarrier_init(smem + 768 + lane * 8, _mbarrier_init_count_10_96);
        }
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 800);
    if (warp == 0) {
        int _tmem_hold = smem + 800;
        ...
        { // compute_main
            ...
            for (int page_idx = 0; page_idx < num_pages; page_idx++) {
                ...
                for (int page_token_2 = 0; page_token_2 < 32; page_token_2++) {
                    ...
                    float _cvt_f32_33 = __bfloat162float(page_v_value);
                    beta_v[page_token_2] = _cvt_f32_33 * page_beta_value;
                }
                mbarrier_wait(page_operand_free_addr, _phase_page_operand_free_0);
                _phase_page_operand_free_0 ^= 1;
                ...
                for (int page_token_6 = 0; page_token_6 < 32; page_token_6++) {
                    ...
                    float _cvt_f32_34 = __bfloat162float(page_v_value_1);
                    beta_v_10[page_token_6] = _cvt_f32_34 * page_beta_value_1;
                }
                mbarrier_wait(page_operand_free_addr, _phase_page_operand_free_0);
                _phase_page_operand_free_0 ^= 1;
                ...
                for (int page_token_7 = 0; page_token_7 < 32; page_token_7++) {
                    ...
                    float _cvt_f32_35 = __bfloat162float(page_v_value_2);
                    beta_v_12[page_token_7] = _cvt_f32_35 * page_beta_value_2;
                }
                mbarrier_wait(page_operand_free_addr, _phase_page_operand_free_0);
                _phase_page_operand_free_0 ^= 1;
                ...
                if (checkpoint_every_n_tokens != 0 && page_checkpoint_token < seq_len && page_checkpoint_token % checkpoint_every_n_tokens == 0) {
                    long long page_checkpoint_idx = checkpoint_row_start + (long long)(page_checkpoint_token / checkpoint_every_n_tokens);
                ...
                first_page_stage += 1;
                if (first_page_stage == 4) { first_page_stage = 0; _phase_qk_full ^= 1; _phase_v_full ^= 1; }
                first_page_stage += 1;
                if (first_page_stage == 4) { first_page_stage = 0; _phase_qk_full ^= 1; _phase_v_full ^= 1; }
                second_page_stage += 1;
                if (second_page_stage == 4) { second_page_stage = 0; _phase_qk_full_1 ^= 1; _phase_v_full_1 ^= 1; _phase_final_ready ^= 1; }
                second_page_stage += 1;
                if (second_page_stage == 4) { second_page_stage = 0; _phase_qk_full_1 ^= 1; _phase_v_full_1 ^= 1; _phase_final_ready ^= 1; }
            }
            ...
            for (int chunk_idx = 0; chunk_idx < num_chunks_0; chunk_idx++) {
                ...
                for (int residual_half = 0; residual_half < 2; residual_half++) {
                    ...
                    for (int residual_col = 0; residual_col < 16; residual_col++) {
                        int token_col = residual_half * 16 + residual_col;
                        {
                            ...
                            float _cvt_f32_36 = __bfloat162float(v_value);
                            residual_v[residual_col] = _cvt_f32_36;
                ...
                compute_stage += 1;
                if (compute_stage == 4) { compute_stage = 0; _phase_qk_full_2 ^= 1; _phase_v_full_2 ^= 1; _phase_old_out_ready ^= 1; _phase_w_acc_ready ^= 1; _phase_prediction_acc_ready ^= 1; _phase_u2_acc_ready ^= 1; _phase_final_ready_1 ^= 1; }
            }
    ...
    // ---- Role: aux_mma ----
        ...
        { // aux_mma_main
            ...
            for (int page_aux_idx = 0; page_aux_idx < page_aux_num_pages; page_aux_idx++) {
                ...
                if (page_aux_first_stage == 4) { page_aux_first_stage = 0; _phase_page_local_ready ^= 1; }
                page_aux_first_stage += 1;
                if (page_aux_first_stage == 4) { page_aux_first_stage = 0; _phase_page_local_ready ^= 1; }
                page_aux_second_stage += 1;
                if (page_aux_second_stage == 4) { page_aux_second_stage = 0; _phase_page_local_ready_1 ^= 1; }
                page_aux_second_stage += 1;
                if (page_aux_second_stage == 4) { page_aux_second_stage = 0; _phase_page_local_ready_1 ^= 1; }
    ...
    // ---- Role: mma ----
        ...
        { // mma_main
            ...
            for (int _page_idx = 0; _page_idx < num_pages_1; _page_idx++) {
                ...
                if (first_page_stage_1 == 4) { first_page_stage_1 = 0; _phase_qk_full_3 ^= 1; }
                first_page_stage_1 += 1;
                if (first_page_stage_1 == 4) { first_page_stage_1 = 0; _phase_qk_full_3 ^= 1; }
                second_page_stage_1 += 1;
                if (second_page_stage_1 == 4) { second_page_stage_1 = 0; _phase_qk_full_4 ^= 1; _phase_final_ready_3 ^= 1; }
                second_page_stage_1 += 1;
                if (second_page_stage_1 == 4) { second_page_stage_1 = 0; _phase_qk_full_4 ^= 1; _phase_final_ready_3 ^= 1; }
            }
            num_chunks_0_2 = 0;
            ...
            for (int _chunk_idx = 0; _chunk_idx < num_chunks_0_2; _chunk_idx++) {
                ...
                mma_stage += 1;
                if (mma_stage == 4) { mma_stage = 0; _phase_qk_full_5 ^= 1; _phase_state_inp_ready ^= 1; _phase_w_inp_ready ^= 1; _phase_prediction_inp_ready ^= 1; _phase_u_inp_ready ^= 1; _phase_u2_inp_ready ^= 1; _phase_final_ready_4 ^= 1; }
            }
