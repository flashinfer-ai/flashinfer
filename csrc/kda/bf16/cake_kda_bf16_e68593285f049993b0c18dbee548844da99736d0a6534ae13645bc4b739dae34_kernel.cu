            ...
            for (int chunk_idx = 0; chunk_idx < num_chunks; chunk_idx++) {
                ...
                mbarrier_wait(qk_full_addr + (compute_stage) * 8, _phase_qk_full);
                {
                    if (lane < 16) {
                        ...
                        float diag_scale_0 = smem_gt_all[(unsigned int)(diag_stage_f32 + diag_block_0 * 16) + lane];
                        float diag_scale_1 = smem_gt_all[(unsigned int)(diag_stage_f32 + diag_block_1 * 16) + lane];
                        {
                            __nv_bfloat16 _bval_4 = __float2bfloat16_rn(diag_scale_0);
                            uint16_t _bits_4 = *(uint16_t*)&_bval_4;
                            uint32_t _addr_4 = static_cast<uint32_t>(((unsigned int)diag_base_0 + (lane / 16 * 512 + lane * 32 + lane % 16 * 2 ^ (lane / 16 * 512 + lane * 32 + lane % 16 * 2 >> 7 & 1) << 4)));
                        ...
                        }
                        {
                            ...
                            uint32_t _addr_5 = static_cast<uint32_t>(((unsigned int)diag_base_1 + (lane / 16 * 512 + lane * 32 + lane % 16 * 2 ^ (lane / 16 * 512 + lane * 32 + lane % 16 * 2 >> 7 & 1) << 4)));
