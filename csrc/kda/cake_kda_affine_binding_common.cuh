...
inline dim3 CakeKDAAffineCheckedGrid(int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  for (const auto& named : {std::pair<int64_t, const char*>{grid_x, "grid_x"},
    ...
    TVM_FFI_ICHECK(named.first > 0 && named.first <= std::numeric_limits<uint32_t>::max())
...
inline void CakeKDAAffineCheckInactiveTensor(const TensorView& tensor, const char* name,
                                             int32_t device_id, DLDataType dtype) {
  CheckCudaTensor(tensor, name, device_id);
  CheckDtype(tensor, name, dtype);
  TVM_FFI_ICHECK(tensor.numel() == 0) << name << " must be empty for this Cake KDA affine role";
}

inline void CakeKDAAffineCheckCompactState(const TensorView& state, const char* name,
                                           int32_t device_id, DLDataType dtype, int64_t slots,
                                           int64_t num_heads) {
  CheckCudaTensor(state, name, device_id);
  CheckDtype(state, name, dtype);
  TVM_FFI_ICHECK(slots > 0 && num_heads > 0);
  TVM_FFI_ICHECK(state.ndim() == 4 && state.size(0) == slots && state.size(1) == num_heads &&
                 state.size(2) == kHeadDim && state.size(3) == kHeadDim)
      << name << " must have shape [" << slots << ", " << num_heads << ", 128, 128]";
  TVM_FFI_ICHECK(state.stride(0) == num_heads * kHeadDim * kHeadDim &&
                 state.stride(1) == kHeadDim * kHeadDim && state.stride(2) == kHeadDim &&
                 state.stride(3) == 1)
...
inline void CakeKDAAffineCheckBFloat16StatePool(const TensorView& state, const char* name,
                                                int32_t device_id, int64_t num_heads,
  ...
  TVM_FFI_ICHECK(state.ndim() == 4 && state.size(0) > 0 && state.size(1) == num_heads &&
                 state.size(2) == kHeadDim && state.size(3) == kHeadDim)
      << name << " must have shape [N_pool, " << num_heads << ", 128, 128]";
  TVM_FFI_ICHECK(state_slot_stride >= compact_slot_stride && state.stride(0) == state_slot_stride &&
                 state.stride(1) == kHeadDim * kHeadDim && state.stride(2) == kHeadDim &&
                 state.stride(3) == 1)
      << name << " must be contiguous inside each slot and match state_slot_stride";
  TVM_FFI_ICHECK(reinterpret_cast<uintptr_t>(state.data_ptr()) % 16 == 0 &&
...
constexpr void CakeKDAAffineCheckArgumentCount(void* (&)[Actual]) {
  static_assert(Expected == Actual, "Cake KDA affine kernel ABI argument count changed");
}

inline void CakeKDAAffineConfigureAndLaunch(const void* kernel, dim3 grid, int32_t device_id,
  ...
  CheckDynamicSmemCapacity(device_id, FLASHINFER_CAKE_KDA_AFFINE_SMEM_BYTES);
  ...
  config.gridDim = grid;
  config.blockDim = dim3(FLASHINFER_CAKE_KDA_AFFINE_THREADS, 1, 1);
...
#else
  CheckCuda(cudaLaunchKernel(kernel, grid, dim3(FLASHINFER_CAKE_KDA_AFFINE_THREADS, 1, 1), args,
