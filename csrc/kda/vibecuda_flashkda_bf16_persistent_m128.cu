...
kernel_flashkda_bf16_persistent_m128(__nv_bfloat16* __restrict__ q, const void* __restrict__ q_tma, __nv_bfloat16* __restrict__ k, const void* __restrict__ k_tma, __nv_bfloat16* __restrict__ v, const void* __restrict__ v_tma, __nv_bfloat16* __restrict__ g, const void* __restrict__ g_tma, __nv_bfloat16* __restrict__ beta, const void* __restrict__ beta_tma, float* __restrict__ A_log, float* __restrict__ dt_bias, long long* __restrict__ cu_seqlens, int* __restrict__ seq_order, int* __restrict__ task_ids, int* __restrict__ task_offsets, __nv_bfloat16* __restrict__ initial_state, __nv_bfloat16* __restrict__ out, const void* __restrict__ out_tma, __nv_bfloat16* final_state, int num_heads, int use_initial_state, int store_final_state, float scale, float lower_bound)
        ...
        { // compute_main
            ...
            unsigned int _phase_final_ready = 0;
            {
                ...
                for (int task_pos = task_begin; task_pos < task_end; task_pos++) {
                    int seq_idx = seq_order[task_ids[task_pos] / num_heads];
                    int head_idx = task_ids[task_pos] % num_heads;
                    ...
                    for (int chunk_idx = 0; chunk_idx < num_chunks; chunk_idx++) {
                        ...
                        // R37-02: pre-issue the per-lane v stream inside the
                        // old_out_ready wait window (see fused gen notes;
                        // register-free under setmaxnreg 168).
                        __nv_bfloat16 _res_v_pre[32];
                        #pragma unroll
                        for (int _pc = 0; _pc < 32; _pc++)
                            _res_v_pre[_pc] = smem_v_all[compute_stage * 20992 + (unsigned int)(_pc * 128) + (unsigned int)state_row];
                        mbarrier_wait(old_out_ready_addr + (compute_stage) * 8, _phase_old_out_ready);
                        // R37: slice the u_acc drain into two 16-column
                        // tcgen05.ld chunks; the (v,beta) smem stream issues
                        // inside TMEM-load latency instead of serializing
                        // behind the full x32 drain.
                        ...
                        for (int residual_half = 0; residual_half < 2; residual_half++) {
                            ...
                            for (int residual_col = 0; residual_col < 16; residual_col++) {
                                ...
                                float _cvt_f32_6 = __bfloat162float(v_value);
                                residual_v[residual_col] = _cvt_f32_6;
        ...
        { // epilogue_main
            ...
            unsigned int _phase_final_ready_1 = 0;
            {
                ...
                for (int task_pos_1 = task_begin_1; task_pos_1 < task_end_1; task_pos_1++) {
                    int seq_idx_1 = seq_order[task_ids[task_pos_1] / num_heads];
                    int head_idx_1 = task_ids[task_pos_1] % num_heads;
                    ...
                    for (int chunk_idx_1 = 0; chunk_idx_1 < num_chunks_1; chunk_idx_1++) {
                        ...
                        if (chunk_is_full != 0) {
                            ...
                            tmem_ld_x32(&_tmem_load_10[0], taddr + 192 + (unsigned int)tmem_row_base_1);
                            asm volatile("tcgen05.wait::ld.sync.aligned;" ::: "memory");
                            asm volatile("barrier.sync 9, 128;" ::: "memory");
                            if (epilogue_local_warp == 0) {
                                if (elect_sync()) {
        ...
        { // mma_main
            ...
            unsigned int _phase_u2_inp_ready = 0;
            {
                ...
                for (int task_pos_2 = task_begin_2; task_pos_2 < task_end_2; task_pos_2++) {
                    int seq_idx_2 = seq_order[task_ids[task_pos_2] / num_heads];
                    ...
                    for (int _chunk_idx = 0; _chunk_idx < num_chunks_2; _chunk_idx++) {
                        ...
                        // Round-36 UMMA-1/UMMA-2 issue swap (persistent port of
                        // the m128 change): only UMMA-2's D (u_acc) feeds the
                        // residual chain; UMMA-1's D (tmem_out) is consumed
                        // solely by the epilogue after final_ready. old_out_ready
                        // now commits right after u_acc so the residual produce
                        // overlaps UMMA-1's tail; raw_inputs_free still commits
                        // after both (qd dead after UMMA-1).
        ...
        { // load_main
            ...
            unsigned int _phase_qk_full_2 = 0;
            {
                ...
                for (int task_pos_3 = task_begin_3; task_pos_3 < task_end_3; task_pos_3++) {
                    int seq_idx_3 = seq_order[task_ids[task_pos_3] / num_heads];
                    int head_idx_2 = task_ids[task_pos_3] % num_heads;
        ...
        { // prep_main
            ...
            unsigned int _phase_prep_inv16_ready = 0;
            {
                ...
                for (int task_pos_4 = task_begin_4; task_pos_4 < task_end_4; task_pos_4++) {
                    ...
                    int seq_idx_4 = seq_order[task_ids[task_pos_4] / num_heads];
                    int head_idx_3 = task_ids[task_pos_4] % num_heads;
                        ...
                        if (prep_tid < 128) {
                            ...
                            for (int gate_row = 0; gate_row < 32; gate_row++) {
                                ...
                                int gate_needs_compute = 1;
                                {
                                    if (gate_row < 8) {
                                        if (chunk_is_full_2 != 0) {
                                            gate_log2 = early_gate_log2_values[gate_row];
