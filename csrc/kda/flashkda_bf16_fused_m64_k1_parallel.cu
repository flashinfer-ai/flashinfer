...
// FlashInfer integration: K1 prep CTAs publish bounded global-mailbox packets to
// two persistent M64 recurrent owners within the same cluster launch.
...
__device__ __forceinline__ void publish_k1_to_global(
    uint32_t local_stage, unsigned char* packet, unsigned int* flag,
    unsigned int ready_value, int prep_tid) {
    if (prep_tid != 0) {
        return;
    }
    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
    asm volatile(
        ...
        :: "l"(packet), "r"(local_stage), "n"(28672) : "memory");
    asm volatile(
        "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
        :: "l"(packet + 28672), "r"(local_stage + 28672), "n"(2688)
        : "memory");
    asm volatile(
        "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
        :: "l"(packet + 31360), "r"(local_stage + 41472), "n"(160)
        : "memory");
    asm volatile("cp.async.bulk.commit_group;" ::: "memory");
    asm volatile("cp.async.bulk.wait_group 0;" ::: "memory");
    asm volatile("st.global.release.gpu.u32 [%0], %1;"
        :: "l"(flag), "r"(ready_value) : "memory");
}

__device__ __forceinline__ void wait_k1_global_flag(
    const unsigned int* flag, unsigned int expected) {
...
__device__ __forceinline__ void wait_k1_global_ready(
    const unsigned int* flag, unsigned int ready_value) {
    unsigned int state = 0;
    do {
        asm volatile("ld.global.acquire.gpu.u32 %0, [%1];"
            : "=r"(state) : "l"(flag) : "memory");
    } while ((state & ~6u) != ready_value);
}

__device__ __forceinline__ void acknowledge_k1_global_packet(
    unsigned int* flag, unsigned int owner_rank) {
    unsigned int old;
    const unsigned int mask = 1u << (owner_rank + 1u);
    asm volatile("atom.global.release.gpu.or.b32 %0, [%1], %2;"
        : "=r"(old) : "l"(flag), "r"(mask) : "memory");
}

__device__ __forceinline__ void load_k1_from_global(
    uint32_t local_stage, uint32_t qk_barrier,
    const unsigned char* packet) {
    mbarrier_arrive_expect_tx(qk_barrier, kK1PacketBytes);
    asm volatile(
        "cp.async.bulk.shared::cta.global.mbarrier::complete_tx::bytes"
        " [%0], [%1], %2, [%3];"
        :: "r"(local_stage), "l"(packet), "n"(28672), "r"(qk_barrier)
        : "memory");
    asm volatile(
        "cp.async.bulk.shared::cta.global.mbarrier::complete_tx::bytes"
        " [%0], [%1], %2, [%3];"
        :: "r"(local_stage + 28672), "l"(packet + 28672), "n"(2688),
           "r"(qk_barrier) : "memory");
    asm volatile(
        "cp.async.bulk.shared::cta.global.mbarrier::complete_tx::bytes"
        " [%0], [%1], %2, [%3];"
        :: "r"(local_stage + 41472), "l"(packet + 31360), "n"(160),
           "r"(qk_barrier) : "memory");
}
...
// FLASHINFER INTEGRATION END: allow exact state alias
{
    ...
    constexpr int kOwnerCount = 2;
    constexpr int kProducerFirstRank = kOwnerCount;
    const int kProducerCount = kClusterSize - kOwnerCount;
    ...
    if (warp == 0) {
        ...
        const int producer_free_arrivals = 1;
        // smem_free: 5 barriers
        mbarrier_init_pred(smem + 200, producer_free_arrivals, leader);
        mbarrier_init_pred(smem + 208, producer_free_arrivals, leader);
        mbarrier_init_pred(smem + 216, producer_free_arrivals, leader);
        mbarrier_init_pred(smem + 224, producer_free_arrivals, leader);
        mbarrier_init_pred(smem + 232, producer_free_arrivals, leader);
        // raw_inputs_free: 5 barriers
        mbarrier_init_pred(smem + 240, producer_free_arrivals, leader);
        mbarrier_init_pred(smem + 248, producer_free_arrivals, leader);
        mbarrier_init_pred(smem + 256, producer_free_arrivals, leader);
        mbarrier_init_pred(smem + 264, producer_free_arrivals, leader);
        mbarrier_init_pred(smem + 272, producer_free_arrivals, leader);
        ...
        mbarrier_init_pred(smem + 520, 4, leader);
    ...
    if (cta_rank < kOwnerCount && warp == 0) {
        int _tmem_hold = smem + 656;
        asm volatile("tcgen05.alloc.cta_group::1.sync.aligned.shared::cta.b32 [%0], %1;" :: "r"(_tmem_hold), "r"(256) : "memory");
    } else if (cta_rank >= kOwnerCount && tid == 0) {
        tmem_addr_storage[0] = 0;
    }

    __syncthreads();
    if (cta_rank < kOwnerCount) {
    ...
    if (cta_rank < kOwnerCount && warp >= 8) {
        asm volatile("setmaxnreg.dec.sync.aligned.u32 48;");
    }

    // ---- Role: compute ----
    if (cta_rank < kOwnerCount && warp <= 3) {
        asm volatile("setmaxnreg.inc.sync.aligned.u32 168;");
        { // compute_main
            int task_idx = bid;
            int value_row_offset = cta_rank * 64;
            ...
            for (int chunk_idx = 0; chunk_idx < num_chunks; chunk_idx++) {
                mbarrier_wait_cluster(qk_full_addr + (compute_stage) * 8, _phase_qk_full);
                #pragma unroll
    ...
    // ---- Role: epilogue ----
    } else if (cta_rank < kOwnerCount && warp >= 4 && warp <= 7) {
        asm volatile("setmaxnreg.dec.sync.aligned.u32 48;");
        { // epilogue_main
            int task_idx_1 = bid;
            int value_split_idx_1 = cta_rank;
            ...
            for (int chunk_idx_1 = 0; chunk_idx_1 < num_chunks_1; chunk_idx_1++) {
                ...
                if (chunk_is_full != 0) {
                    ...
                    if (epilogue_local_warp == 0) {
                        if (elect_sync()) {
                            tma_store_4d(out_tma, 0, (int)(bos_1 + (long long)(chunk_idx_1 * 32)), head_idx_1, value_split_idx_1, out_stage_addr);
                        }
                ...
                if (epilogue_local_warp == 0 && elect_sync()) {
                    ...
                    acknowledge_k1_global_packet(
                        k1_flags + packet_idx, (unsigned int)cta_rank);
                    mbarrier_arrive(
    ...
    // ---- Role: mma ----
    } else if (cta_rank < kOwnerCount && warp == 9) {
        { // mma_main
            int task_idx_2 = bid;
            ...
            for (int _chunk_idx = 0; _chunk_idx < num_chunks_2; _chunk_idx++) {
                mbarrier_wait_cluster(qk_full_addr + (mma_stage) * 8, _phase_qk_full_1);
    ...
    // ---- Role: load ----
    } else if (cta_rank < kOwnerCount && warp == 10) {
        { // load_main
            int task_idx_3 = bid;
            int value_row_offset_2 = cta_rank * 64;
            ...
            for (int chunk_idx_2 = 0; chunk_idx_2 < num_chunks_3; chunk_idx_2++) {
                ...
                mbarrier_wait_cluster(qk_full_addr + (load_stage) * 8, _phase_qk_full_2);
    ...
    // ---- Role: persistent owner mailbox ingress coordinator ----
    } else if (cta_rank < kOwnerCount && warp == 12) {
        int task_idx_4 = bid;
        ...
        for (int chunk_idx_3 = 0; chunk_idx_3 < num_chunks_4;
            ...
            const unsigned int generation =
                (unsigned int)(chunk_idx_3 / mailbox_depth);
            wait_k1_global_ready(
                k1_flags + packet_idx, (generation << 3) | 1u);
            if (elect_sync()) {
                load_k1_from_global(
    ...
    // ---- Role: prep ----
    } else if (cta_rank >= kProducerFirstRank && warp >= 12 && warp <= 31) {
        asm volatile("setmaxnreg.dec.sync.aligned.u32 48;");
        { // prep_main
            int task_idx_4 = bid;
            int seq_idx_4 = seq_order[task_idx_4 / num_heads];
            ...
            for (int prep_iter = 0; prep_iter < num_prep_iters; prep_iter++) {
                ...
                if (prep_tid == 0) {
                    const unsigned int generation =
                        (unsigned int)(chunk_idx_3 / mailbox_depth);
                    const unsigned int reusable_state = generation == 0
                        ? 0u
                        : ((generation - 1u) << 3) | 7u;
                    wait_k1_global_flag(
                        k1_flags + packet_idx, reusable_state);
                }
                const unsigned int ready_value =
                    ((unsigned int)(chunk_idx_3 / mailbox_depth) << 3) | 1u;
                publish_k1_to_global(
                    smem_qd_addr + prep_stage * 41984,
                    k1_workspace + packet_idx * kK1PacketBytes,
                    k1_flags + packet_idx, ready_value, prep_tid);
                if (prep_tid == 0) {
                    mbarrier_arrive(
