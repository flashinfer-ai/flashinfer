...
kernel_kernel(__nv_bfloat16* __restrict__ x, float* __restrict__ weight, __nv_bfloat16* __restrict__ conv_state, __nv_bfloat16* __restrict__ raw_gate, __nv_bfloat16* __restrict__ raw_beta, float* __restrict__ A_log, float* __restrict__ dt_bias, int* __restrict__ state_indices, __nv_bfloat16* __restrict__ state, __nv_bfloat16* __restrict__ output_gate, float* __restrict__ norm_weight, __nv_bfloat16* __restrict__ output, int x_row_stride, int conv_slot_stride, int beta_row_stride, int state_slot_stride, int output_gate_row_stride, int H, int use_lower_bound, float lower_bound_log2, float norm_eps)
{
    ...
    }
    {
        if (tid_0 < 128) {
            if (warp % 2 != 0) {
                ...
                long long early_async_state_src_base = (long long)slot * (long long)state_slot_stride + (long long)(head * 128 * 128) + (long long)(state_owner_row_base * 128) + (long long)k_start + 8192;
                #pragma unroll
                for (int local_row = 0; local_row < 2; local_row++) {
                    ...
                    asm volatile("cp.async.ca.shared::cta.global [%0], [%1], 16;"
                        :: "r"(sAsyncState_addr + (unsigned int)(early_async_state_elem * 2)), "l"(state + (early_async_state_src_base + (long long)(early_async_state_source_row * 128))));
                }
            }
        }
    }
    if (tid_0 >= 128) {
        ...
        sGateDecay[gate_smem_idx] = _exp2_0;
        {
            if (warp % 2 != 0) {
                ...
                long long late_async_state_src_base = (long long)slot * (long long)state_slot_stride + (long long)(head * 128 * 128) + (long long)(state_owner_row_base * 128) + (long long)k_start + 8192;
                #pragma unroll
                for (int local_row_1 = 0; local_row_1 < 2; local_row_1++) {
                    ...
                    asm volatile("cp.async.ca.shared::cta.global [%0], [%1], 16;"
                        :: "r"(sAsyncState_addr + (unsigned int)(late_async_state_elem * 2)), "l"(state + (late_async_state_src_base + (long long)(late_async_state_source_row * 128))));
                }
            }
        }
    ...
    long long state_group_base = state_head_base + (long long)(state_owner_row_base * 128) + (long long)k_start;
    {
        if (warp % 2 == 0) {
            ...
            long long rotating_async_state_src_base = state_head_base + (long long)(state_owner_row_base * 128) + (long long)k_start + 8192;
            #pragma unroll
            for (int rotating_local_row = 0; rotating_local_row < 2; rotating_local_row++) {
                ...
                asm volatile("cp.async.ca.shared::cta.global [%0], [%1], 16;"
                    :: "r"(sAsyncState_addr + (unsigned int)(rotating_async_elem * 2)), "l"(state + (rotating_async_state_src_base + (long long)(rotating_async_source_row * 128))));
            }
            asm volatile("cp.async.commit_group;");
        }
    }
    ...
    for (int local_row_2 = 0; local_row_2 < 4; local_row_2++) {
        {
            const uint4* _vptr_2 = reinterpret_cast<const uint4*>(state + state_group_base + (long long)(local_row_2 * 128));
    ...
    for (int value_tile = 0; value_tile < 2; value_tile++) {
        ...
        if (value_tile > 0) {
            ...
            for (int local_row_4 = 2; local_row_4 < 4; local_row_4++) {
                {
                    const uint4* _vptr_3 = reinterpret_cast<const uint4*>(state + state_tile_base + (long long)(local_row_4 * 128));
