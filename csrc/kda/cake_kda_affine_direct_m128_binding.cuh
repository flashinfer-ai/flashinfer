...
#if FLASHINFER_CAKE_KDA_AFFINE_ROLE != FLASHINFER_CAKE_KDA_AFFINE_ROLE_MAIN && \
    FLASHINFER_CAKE_KDA_AFFINE_ROLE != FLASHINFER_CAKE_KDA_AFFINE_ROLE_MAP &&  \
    FLASHINFER_CAKE_KDA_AFFINE_ROLE != FLASHINFER_CAKE_KDA_AFFINE_ROLE_CORRECTION
...
inline CakeKDAAffineDirectPreparedInputs CakeKDAAffinePrepareDirectInputs(
    ...
    int64_t num_heads, int64_t beta_token_stride, double scale, double lower_bound,
    int64_t cuda_stream) {
  ...
  TVM_FFI_ICHECK(q.ndim() == 4 && q.size(0) == 1 && q.size(2) == num_heads && q.size(3) == kHeadDim)
  ...
  for (const auto& named :
       {std::pair<const TensorView*, const char*>{&k, "k"}, {&v, "v"}, {&g, "g"}, {&out, "out"}}) {
    const TensorView& tensor = *named.first;
    TVM_FFI_ICHECK(tensor.ndim() == 4 && tensor.size(0) == 1 && tensor.size(1) == token_count &&
                   tensor.size(2) == num_heads && tensor.size(3) == kHeadDim)
        << named.second << " must match q's [1, tokens, H, 128] shape";
  }
  TVM_FFI_ICHECK(beta.ndim() == 3 && beta.size(0) == 1 && beta.size(1) == token_count &&
  ...
  TVM_FFI_ICHECK(beta_tma_heads > 0) << "beta_tma must have a positive head dimension";
  const bool direct_beta = beta_tma_heads == num_heads && num_heads >= 8;
  TVM_FFI_ICHECK(beta_tma.ndim() >= 2 && (direct_beta || beta_tma_heads == padded_heads) &&
  ...
  TVM_FFI_ICHECK(beta_tma.data_ptr() != nullptr &&
                 ...
                 beta_tma.stride(beta_tma.ndim() - 2) * sizeof(__nv_bfloat16) % 16 == 0)
  ...
  constexpr int64_t kMinimumCuSeqlens =
      ...
      2;
#endif
  TVM_FFI_ICHECK(cu_seqlens.ndim() == 1 && cu_seqlens.numel() >= kMinimumCuSeqlens)
      << "affine direct role has too few token parts";
  const int64_t num_sequences = cu_seqlens.numel() - 1;
  TVM_FFI_ICHECK(seq_order.ndim() == 1 && seq_order.numel() == num_sequences)
  ...
  TVM_FFI_ICHECK(descriptor_storage.numel() >= static_cast<int64_t>(kDescriptorStorageBytes) &&
                 reinterpret_cast<uintptr_t>(descriptor_storage.data_ptr()) % kTensorMapAlignment ==
                     0)
...
inline void CakeKDAAffineLaunchDirectM128(CakeKDAAffineDirectKernelArgs& args, dim3 grid,
  ...
  CakeKDAAffineConfigureAndLaunch(reinterpret_cast<const void*>(FLASHINFER_CAKE_KDA_AFFINE_KERNEL),
...
inline void RunCakeKDAAffineDirectM128(
    ...
    TensorView state_indices, TensorView initial_state_bf16, TensorView out,
    TensorView final_state_bf16, TensorView initial_state_f32, TensorView final_state_f32,
    TensorView descriptor_storage, int64_t prepare_descriptors, int64_t num_heads,
    int64_t beta_token_stride, int64_t state_slot_stride, double scale, double lower_bound,
    int64_t grid_x, int64_t grid_y, int64_t grid_z, int64_t cuda_stream) {
  const auto prepared = CakeKDAAffinePrepareDirectInputs(
      q, k, v, g, beta, beta_tma, A_log, dt_bias, cu_seqlens, seq_order, out, descriptor_storage,
      prepare_descriptors, num_heads, beta_token_stride, scale, lower_bound, cuda_stream);
  ffi::CUDADeviceGuard device_guard(prepared.device_id);
  const int64_t compact_state_stride = num_heads * kHeadDim * kHeadDim;
  TVM_FFI_ICHECK(grid_x == prepared.num_sequences * num_heads && grid_y == 1 && grid_z == 1)
  ...
  args.num_sequences = CakeKDAAffineCheckedInt32(prepared.num_sequences, "num_sequences");
  ...
  const auto check_no_data_overlap = [&](const TensorView& state, const char* state_name) {
    for (const auto& named : {std::pair<const TensorView*, const char*>{&q, "q"},
                              {&k, "k"},
                              {&v, "v"},
                              {&g, "g"},
  ...
  CakeKDAAffineCheckBFloat16StatePool(initial_state_bf16, "initial_state_bf16", prepared.device_id,
                                      num_heads, state_slot_stride);
  CakeKDAAffineCheckInactiveTensor(final_state_bf16, "final_state_bf16", prepared.device_id,
                                   dl_bfloat16);
  CakeKDAAffineCheckInactiveTensor(initial_state_f32, "initial_state_f32", prepared.device_id,
                                   dl_float32);
  CakeKDAAffineCheckCompactState(final_state_f32, "final_state_f32", prepared.device_id, dl_float32,
  ...
  CheckNoOverlap(initial_state_bf16, "initial_state_bf16", final_state_f32, "final_state_f32");
...
#elif FLASHINFER_CAKE_KDA_AFFINE_ROLE == FLASHINFER_CAKE_KDA_AFFINE_ROLE_MAP
  CakeKDAAffineCheckInactiveTensor(state_indices, "state_indices", prepared.device_id, dl_int32);
  TVM_FFI_ICHECK(state_slot_stride == compact_state_stride)
      << "Cake KDA affine map requires compact state_slot_stride";
  CakeKDAAffineCheckCompactState(initial_state_bf16, "initial_state_bf16", prepared.device_id,
                                 dl_bfloat16, prepared.num_sequences, num_heads);
  CakeKDAAffineCheckCompactState(final_state_bf16, "final_state_bf16", prepared.device_id,
                                 dl_bfloat16, prepared.num_sequences, num_heads);
  CakeKDAAffineCheckCompactState(initial_state_f32, "initial_state_f32", prepared.device_id,
                                 dl_float32, prepared.num_sequences + 1, num_heads);
  CakeKDAAffineCheckInactiveTensor(final_state_f32, "final_state_f32", prepared.device_id,
                                   dl_float32);
  check_no_data_overlap(initial_state_bf16, "initial_state_bf16");
  check_no_data_overlap(final_state_bf16, "final_state_bf16");
  check_no_data_overlap(initial_state_f32, "initial_state_f32");
  CheckNoOverlap(initial_state_bf16, "initial_state_bf16", final_state_bf16, "final_state_bf16");
  CheckNoOverlap(initial_state_bf16, "initial_state_bf16", initial_state_f32, "initial_state_f32");
  CheckNoOverlap(final_state_bf16, "final_state_bf16", initial_state_f32, "initial_state_f32");
  args.initial_state = initial_state_bf16.data_ptr();
  ...
  CakeKDAAffineCheckInactiveTensor(initial_state_bf16, "initial_state_bf16", prepared.device_id,
                                   dl_bfloat16);
  CakeKDAAffineCheckInactiveTensor(final_state_bf16, "final_state_bf16", prepared.device_id,
                                   dl_bfloat16);
  CakeKDAAffineCheckCompactState(initial_state_f32, "initial_state_f32", prepared.device_id,
  ...
  check_no_data_overlap(initial_state_f32, "initial_state_f32");
  check_no_data_overlap(final_state_f32, "final_state_f32");
  CheckNoOverlap(initial_state_f32, "initial_state_f32", final_state_f32, "final_state_f32");
  args.initial_state_f32 = initial_state_f32.data_ptr();
  args.final_state_f32 = final_state_f32.data_ptr();
#endif

  CakeKDAAffineLaunchDirectM128(args, CakeKDAAffineCheckedGrid(grid_x, grid_y, grid_z),
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, flashinfer::cake_kda::RunCakeKDAAffineDirectM128);
