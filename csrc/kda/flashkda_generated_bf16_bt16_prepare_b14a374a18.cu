...
kernel_flashkda_bf16_bt16_prepare(__nv_bfloat16* __restrict__ q, FlashKDATensorMap const* q_tma, __nv_bfloat16* __restrict__ k, FlashKDATensorMap const* k_tma, __nv_bfloat16* __restrict__ raw_gate, FlashKDATensorMap const* raw_gate_tma, __nv_bfloat16* __restrict__ beta_logits, FlashKDATensorMap const* beta_logits_tma, float* __restrict__ a_log, float* __restrict__ dt_bias, long long* __restrict__ cu_seqlens, int* __restrict__ cu_chunks, int* __restrict__ chunk_to_seq, __nv_bfloat16* __restrict__ ws_qd, FlashKDATensorMap const* ws_qd_tma, __nv_bfloat16* __restrict__ ws_kd, FlashKDATensorMap const* ws_kd_tma, __nv_bfloat16* __restrict__ ws_w, FlashKDATensorMap const* ws_w_tma, __nv_bfloat16* __restrict__ ws_qk_t, float* __restrict__ ws_diag, int total_chunks, int num_heads, float gate_lower_bound)
{
    ...
    if (warp == 3) {
        uint64_t __flashkda_tensormap_acquire_addr = (uint64_t)(q_tma);
        if (lane == 1) __flashkda_tensormap_acquire_addr = (uint64_t)(k_tma);
        if (lane == 2) __flashkda_tensormap_acquire_addr = (uint64_t)(raw_gate_tma);
        if (lane == 3) __flashkda_tensormap_acquire_addr = (uint64_t)(beta_logits_tma);
        if (lane == 4) __flashkda_tensormap_acquire_addr = (uint64_t)(ws_qd_tma);
        if (lane == 5) __flashkda_tensormap_acquire_addr = (uint64_t)(ws_kd_tma);
        if (lane == 6) __flashkda_tensormap_acquire_addr = (uint64_t)(ws_w_tma);
        if (lane < 7) {
            asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" :: "l"(__flashkda_tensormap_acquire_addr) : "memory");
        }
    }
    ...
    for (int cta_chunk = 0; cta_chunk < my_chunks; cta_chunk++) {
        ...
        if (warp == 0) {
            ...
            int solve_row = lane & 15;
            int solve_row_group = solve_row & 7;
            float inverse_row[16];
            #pragma unroll
            for (int solve_col = 0; solve_col < 16; solve_col++) {
                int source_lane = solve_row_group * 4 + (solve_col & 7) / 2;
                int top_slot = solve_col / 8 * 4 + solve_col % 2;
                int bottom_slot = top_slot + 2;
                float _shfl_0 = __shfl_sync(0xFFFFFFFF, l_values[top_slot], source_lane);
                float top_value = _shfl_0;
                float _shfl_1 = __shfl_sync(0xFFFFFFFF, l_values[bottom_slot], source_lane);
                float bottom_value = _shfl_1;
                inverse_row[solve_col] = top_value;
                if (solve_row >= 8) {
                    inverse_row[solve_col] = bottom_value;
                }
                if (solve_row == solve_col) {
                    inverse_row[solve_col] = 1.0f;
                }
            }
            int solve_half_base = lane / 16 * 16;
            #pragma unroll
            for (int source_row = 0; source_row < 15; source_row++) {
                float row_scale = -inverse_row[source_row];
                #pragma unroll
                for (int previous_col = 0; previous_col < source_row; previous_col++) {
                    float _shfl_2 = __shfl_sync(0xFFFFFFFF, inverse_row[previous_col], solve_half_base + source_row);
                    float pivot = _shfl_2;
                    if (solve_row > source_row) {
                        float _fma_2 = __fmaf_rn(row_scale, pivot, inverse_row[previous_col]);
                        inverse_row[previous_col] = _fma_2;
                    }
                }
                if (solve_row > source_row) {
                    inverse_row[source_row] = row_scale;
                }
            }
            unsigned int inverse_row_packed[8];
            #pragma unroll
            for (int _lp = 0; _lp < 8; _lp++) {
                __nv_bfloat162 _bf2 = __float22bfloat162_rn(make_float2(inverse_row[_lp*2 + 0], inverse_row[_lp*2+1 + 0]));
                inverse_row_packed[_lp] = *(uint32_t*)&_bf2;
            }
            if (lane < 16) {
                #pragma unroll
                for (int inverse_segment = 0; inverse_segment < 2; inverse_segment++) {
                    #pragma unroll
                    for (int inverse_word = 0; inverse_word < 4; inverse_word++) {
                        asm volatile("st.shared.b32 [%0], %1;" :: "r"((smem_abt_addr + (unsigned int)(inverse_segment * 8 / 16 * 512 + lane * 32 + inverse_segment * 8 % 16 * 2 ^ (inverse_segment * 8 / 16 * 512 + lane * 32 + inverse_segment * 8 % 16 * 2 >> 7 & 1) << 4)) + (unsigned int)(inverse_word * 4)), "r"((inverse_row_packed[inverse_segment * 4 + inverse_word])));
                    }
                }
            }
            __syncwarp();
            ...
            asm volatile("ldmatrix.sync.aligned.m8n8.x4.trans.shared.b16 {%0, %1, %2, %3}, [%4];\n"
                : "=r"(inverse_trans_frag[0]), "=r"(inverse_trans_frag[1]), "=r"(inverse_trans_frag[2]), "=r"(inverse_trans_frag[3])
                : "r"((smem_abt_addr + (unsigned int)(lane % 16 / 8 * 8 / 16 * 512 + (8 * (lane / 16) + lane % 8) * 32 + lane % 16 / 8 * 8 % 16 * 2 ^ (lane % 16 / 8 * 8 / 16 * 512 + (8 * (lane / 16) + lane % 8) * 32 + lane % 16 / 8 * 8 % 16 * 2 >> 7 & 1) << 4)))
                : "memory");
