...
// Round-151 (supervisor directive, KDA_FN16 single-launch fused N16
// experiment): device-side packed planning. When KDA_BT16_PLAN_INLINE is
// defined the kernel derives (seq, local chunk, bos, eos) for any global
// 16-token chunk directly from cu_seqlens with an O(N) scan (N <= 32
// sequences), eliminating the separate Bt16BuildMetadata launch and the
// cu_chunks/chunk_to_seq tensors. Capacity-pad chunks (c >= actual total)
// map to the last sequence exactly like the metadata kernel's sentinel rule,
// so TMA OOB zero-fill and capacity-clamped stores behave identically.
#ifdef KDA_BT16_PLAN_INLINE
__device__ __forceinline__ void bt16_inline_plan(const long long* __restrict__ cu_seqlens,
                                                 int num_seqs, int chunk, int& seq_out,
                                                 long long& bos_out, long long& eos_out,
                                                 int& local_chunk_out) {
  int acc = 0;
  int seq = num_seqs - 1;
  int base = 0;
  ...
  for (int s = 0; s < num_seqs; ++s) {
    const int nchunks = (int)((cu_seqlens[s + 1] - cu_seqlens[s] + 15) / 16);
    // Capacity-pad chunks (c >= actual total) sentinel-map to the last
    // sequence with base = chunk count before it, exactly like the metadata
    // kernel's rules, so always break at s == num_seqs - 1.
    if (chunk < acc + nchunks || s == num_seqs - 1) {
      seq = s;
      base = acc;
      break;
    }
  ...
  seq_out = seq;
  local_chunk_out = chunk - base;
  bos_out = cu_seqlens[seq];
  eos_out = cu_seqlens[seq + 1];
}
#endif
                                                  ...
                                                  )
{
    ...
    if (my_chunks > 0) {
#ifdef KDA_BT16_PLAN_INLINE
        int first_seq, first_local_chunk;
        long long first_bos;
        bt16_inline_plan(cu_seqlens, num_seqs, chunk_lo, first_seq, first_bos,
                         current_eos, first_local_chunk);
...
#ifdef KDA_BT16_PLAN_INLINE
            int next_seq, next_local_chunk;
            long long next_bos, next_eos;
            bt16_inline_plan(cu_seqlens, num_seqs, next_gchunk, next_seq, next_bos,
                             next_eos, next_local_chunk);
#else
