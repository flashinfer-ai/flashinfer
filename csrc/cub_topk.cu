...
//
// CUB_TOPK_MAX_LEN is the top tier and doubles as the hard input limit enforced at the entry
// point: 2^21 is DeviceBatchedTopK's own per-segment cap, enforced by a static_assert on the
// bound at its public entry (cluster backend, SM 9.0+; "larger segments are future work" per
// the CUB docs). Rows longer than this cannot be served by this backend on any architecture,
// so the Python dispatcher must route them to the radix backend.
...
cudaError_t CubBatchedTopKRun(const DType* input, int32_t* output_indices, DType* output_values,
  ...
  cudaError_t status =
      cub::DeviceBatchedTopK::MaxPairs(d_temp_storage, temp_storage_bytes, d_keys_in, d_keys_out,
                                       d_values_in, d_values_out, segment_sizes, k_arg, num_segs,
                                       env);
...
cudaError_t CubBatchedTopKDispatchBound(const DType* input, int32_t* output_indices,
  ...
  if (max_len <= 8192) {
    return CubBatchedTopKRun<int64_t{8192}>(input, output_indices, output_values,
    ...
    return CubBatchedTopKRun<CUB_TOPK_MAX_LEN>(input, output_indices, output_values,
                                               maybe_temp_storage, num_rows, max_len, top_k,
                                               requirements, stream);
  }
}
...
cudaError_t CubBatchedTopKDispatch(const DType* input, int32_t* output_indices,
  ...
  auto run = [&](auto requirements) {
    return CubBatchedTopKDispatchBound(input, output_indices, output_values, maybe_temp_storage,
                                       num_rows, max_len, top_k, requirements, stream);
  };
...
void cub_topk(TensorView input, TensorView output_indices, TensorView output_values,
              Optional<TensorView> maybe_temp_storage, int64_t top_k, bool deterministic,
              int64_t tie_break) {
  CHECK_INPUT(input);
  ...
  DISPATCH_DLPACK_DTYPE_TO_CTYPE_FP32_FP16(input.dtype(), c_type, [&] {
    ...
    status = CubBatchedTopKDispatch(input_ptr, indices_ptr, values_ptr, maybe_temp_storage,
                                    num_rows, max_len, top_k, deterministic, tie_break, stream);
