...
__global__ __launch_bounds__(128) void kernel_alpha_moe_fused_router_medium(
...
#pragma unroll
  ...
  if (MAX_EXPERTS == 512 && PUBLIC_SHARED_SOFTMAX == 0 && E == 512 && has_shared_expert == 0 &&
      M <= num_bids * NUM_WARPS) {
    preload_single_row = 1;
    int preload_token = (unsigned int)(bid * NUM_WARPS) + warp;
    if (preload_token < M) {
      unsigned long long preload_row_base =
          (unsigned long long)preload_token * (unsigned long long)E;
...
#pragma unroll
      for (int preload_slot = 0; preload_slot < MAX_EXPERTS / 32; preload_slot++) {
        row_values[preload_slot] =
            logits[preload_lane_base + (unsigned long long)(preload_slot * 32)];
      }
    }
  }
  if (private_raw_hist == 0) {
...
#pragma unroll
      ...
      __syncthreads();
#pragma unroll
      for (int private_load_slot = 0; private_load_slot < MAX_EXPERTS / THREADS;
           private_load_slot++) {
...
#pragma unroll
      for (int expert_slot_count_load = 0; expert_slot_count_load < MAX_EXPERTS / THREADS;
           expert_slot_count_load++) {
...
#pragma unroll
      for (int expert_slot_full_prefix = 0; expert_slot_full_prefix < MAX_EXPERTS / THREADS;
           expert_slot_full_prefix++) {
