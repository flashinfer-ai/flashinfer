...
kernel_cake_nvfp4_svdquant_gemm_sm103a_8f1364cc5fb82aad53ed(CakeTensorMap const* A, CakeTensorMap const* B, CakeTensorMap const* SFA, CakeTensorMap const* SFB, CakeTensorMap const* D, CakeTensorMap const* L1, float* __restrict__ alpha, __nv_bfloat16* __restrict__ bias, __nv_bfloat16* __restrict__ out, int M, int N, int K_tiles, int rank, int rank_chunks, int grid_n, int has_bias)
{
    ...
    if (warp >= 4 && warp <= 7) {
        { // epilogue_main
            ...
            for (int n_chunk = 0; n_chunk < 16; n_chunk++) {
                int tmem_addr = taddr + (unsigned int)(epi_warp * 32 << 16) + (unsigned int)(n_chunk * 8);
                ...
                for (int j = 0; j < 8; j++) {
                    ...
                    if (has_bias != 0) {
                        bias_value = (float)bias[off_n_1 + n_chunk * 8 + j];
                    }
                    result[j] = _tmem_load_0[j] * alpha_value + bias_value;
                }
                if (out_row < M) {
                    uint32_t result_bf16[4];
                    #pragma unroll
                    for (int _lp = 0; _lp < 4; _lp++) {
                        __nv_bfloat162 _bf2 = __float22bfloat162_rn(make_float2(result[_lp*2 + 0], result[_lp*2+1 + 0]));
                        result_bf16[_lp] = *(uint32_t*)&_bf2;
                    }
                    reinterpret_cast<int4*>(out + (out_row * N + off_n_1 + n_chunk * 8))[0] = reinterpret_cast<int4*>(result_bf16)[0];
                }
            }
        }
    }
