...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_nvfp4_svdquant_gemm_sm100a_83ad737b1da2b792d17c'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
inline CUtensorMap EncodeTma_SFA(const TensorView& t) {
  ...
  TVM_FFI_CHECK(64u <= global_dim[0] && 4u <= global_dim[1] && 4u <= global_dim[2] && 1u <= global_dim[3], ValueError)
      << "TMA box (64, 4, 4, 1) exceeds resolved global dims for 'SFA'";
  ...
  uint32_t box_dim[4] = {64u, 4u, 4u, 1u};
...
inline CUtensorMap EncodeTma_SFB(const TensorView& t) {
  ...
  TVM_FFI_CHECK(64u <= global_dim[0] && 4u <= global_dim[1] && 4u <= global_dim[2] && 1u <= global_dim[3], ValueError)
      << "TMA box (64, 4, 4, 1) exceeds resolved global dims for 'SFB'";
  int64_t carrier_stride_0 = s3;
  TVM_FFI_CHECK(carrier_stride_0 >= 0, ValueError)
...
inline CUtensorMap EncodeTma_L1(const TensorView& t) {
  ...
  TVM_FFI_CHECK(64u <= global_dim[1], ValueError)
      << "TMA box (64, 64) exceeds resolved global dims for 'L1'";
  int64_t carrier_stride_0 = s2;
  TVM_FFI_CHECK(carrier_stride_0 >= 0, ValueError)
...
void Run(TensorView arg_A, TensorView arg_B, TensorView arg_SFA, TensorView arg_SFB, TensorView arg_D, TensorView arg_L1, TensorView arg_alpha, TensorView arg_out, int64_t arg_K_tiles, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  void* kargs[] = {&p_A, &p_B, &p_SFA, &p_SFB, &p_D, &p_L1, &p_alpha, &p_out, &v_K_tiles};
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_nvfp4_svdquant_gemm_sm100a_83ad737b1da2b792d17c),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_nvfp4_svdquant_gemm_sm100a_83ad737b1da2b792d17c failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_nvfp4_svdquant_gemm_sm100a_83ad737b1da2b792d17c), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_nvfp4_svdquant_gemm_sm100a_83ad737b1da2b792d17c failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_022bedcbe5e21577

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_022bedcbe5e21577::Run);
