...
kernel_cake_fmha_request_ordered_paged_decode_fp8q_split_0c9d93efc571f7435cec(CakeTensorMap const* Qt, CakeTensorMap const* K, CakeTensorMap const* V, __nv_bfloat16* __restrict__ O, float* __restrict__ LSE, int* __restrict__ request_order, int use_request_order, int* __restrict__ page_table, int* __restrict__ seq_lens_kv, float* __restrict__ bmm1_scale_ptr, float* __restrict__ bmm2_scale_ptr, float* __restrict__ partial_O, float* __restrict__ partial_max, float* __restrict__ partial_sum, int pt_batch_stride, int pt_v_offset, int bmm1_is_log2, int num_splits, int blocks_per_split)
{
    ...
    if (warp <= 3) {
        ...
        { // softmax_main
            ...
            #pragma unroll 1
            for (unsigned int _tile_iter_s = 0; _tile_iter_s < BATCH_SIZE * NUM_KV_HEADS * NUM_SPLITS; _tile_iter_s++) {
                int split_s = tile_idx_s % (unsigned int)NUM_SPLITS;
                int batch_s = tile_idx_s / (unsigned int)(NUM_SPLITS * NUM_KV_HEADS);
                if (use_request_order != 0) {
                    batch_s = request_order[batch_s];
                }
                int kv_len_s = seq_lens_kv[batch_s];
                int blocks = (kv_len_s + 127) / 128;
                int base = blocks / NUM_SPLITS;
                int extra = blocks % NUM_SPLITS;
                int count = base + ((split_s < extra) ? 1 : 0);
                int start = split_s * base + ((split_s < extra) ? split_s : extra);
                if (count > 0) {
                    float row_max_s = -3.4028235e+38f;
                    float row_sum_s = 0.0f;
                    mbarrier_wait(stats_empty_addr + (stats_slot_s) * 8, stats_phase_s);
                    #pragma unroll 1
                    for (int block_s = 0; block_s < count; block_s++) {
                        ...
                        if ((start + block_s) * 128 + 128 > kv_len_s - Q_LEN) {
                            int causal_s = kv_len_s - Q_LEN + logical_row_s / 16;
                            #pragma unroll
                            for (int j_s = 0; j_s < 128; j_s++) {
                                if (causal_s < (start + block_s) * 128 + first_col_s + j_s) {
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // correction_main
            ...
            #pragma unroll 1
            for (unsigned int _tile_iter_c = 0; _tile_iter_c < BATCH_SIZE * NUM_KV_HEADS * NUM_SPLITS; _tile_iter_c++) {
                int split_c = tile_idx_c % (unsigned int)NUM_SPLITS;
                int batch_c = tile_idx_c / (unsigned int)(NUM_SPLITS * NUM_KV_HEADS);
                int kv_head_c = tile_idx_c / (unsigned int)NUM_SPLITS % (unsigned int)NUM_KV_HEADS;
                if (use_request_order != 0) {
                    batch_c = request_order[batch_c];
                }
                int kv_len_c = seq_lens_kv[batch_c];
                int blocks_1 = (kv_len_c + 127) / 128;
                int base_1 = blocks_1 / NUM_SPLITS;
                int extra_1 = blocks_1 % NUM_SPLITS;
                int count_1 = base_1 + ((split_c < extra_1) ? 1 : 0);
                int start_1 = split_c * base_1 + ((split_c < extra_1) ? split_c : extra_1);
                if (count_1 > 0) {
                    ...
                    #pragma unroll 1
                    for (int block_c = 0; block_c < count_1 - 1; block_c++) {
                        mbarrier_wait(stats_full_addr + (stat_slot_c) * 8, stat_phase_c);
                        asm volatile(
                    ...
                    float normalization_c = 1.0f / total_sum_c;
                    int row_c = (batch_c * Q_LEN + logical_row_c / 16) * NUM_Q_HEADS + kv_head_c * 16 + logical_row_c % 16;
                    int stat_index_c = row_c * NUM_SPLITS + split_c;
                    int visible_c = kv_len_c - Q_LEN + logical_row_c / 16;
                    #pragma unroll
                    for (int half_c_1 = 0; half_c_1 < 2; half_c_1++) {
                        ...
                        for (int chunk_c = 0; chunk_c < 128; chunk_c += 8) {
                            ...
                            if (logical_row_c < Q_LEN * 16) {
                                if (visible_c >= start_1 * 128) {
                                    int offset_c = stat_index_c * HEAD_DIM + half_c_1 * 128 + chunk_c;
                                    {
                                        float4 _v4 = make_float4(out_c[0 + 0], out_c[0 + 1], out_c[0 + 2], out_c[0 + 3]);
                                        *reinterpret_cast<float4*>(partial_O + offset_c) = _v4;
                                    }
                                    {
                                        float4 _v4 = make_float4(out_c[4 + 0], out_c[4 + 1], out_c[4 + 2], out_c[4 + 3]);
                                        *reinterpret_cast<float4*>(partial_O + offset_c + 4) = _v4;
                                    }
                                }
                            }
                        }
                    ...
                    if (logical_row_c < Q_LEN * 16) {
                        if (visible_c >= start_1 * 128) {
                            *(reinterpret_cast<float*>(partial_max + stat_index_c) + (0)) = stat_c[1] * scale_c - 8.8073549f;
                            *(reinterpret_cast<float*>(partial_sum + stat_index_c) + (0)) = total_sum_c;
                        } else {
                            *(reinterpret_cast<float*>(partial_max + stat_index_c) + (0)) = -CAKE_INF;
                            *(reinterpret_cast<float*>(partial_sum + stat_index_c) + (0)) = 0.0f;
                        }
                    }
                    mbarrier_arrive(o_empty_addr);
                } else if (logical_row_c < Q_LEN * 16) {
                    int row_empty_c = (batch_c * Q_LEN + logical_row_c / 16) * NUM_Q_HEADS + kv_head_c * 16 + logical_row_c % 16;
                    int empty_stat_c = row_empty_c * NUM_SPLITS + split_c;
                    *(reinterpret_cast<float*>(partial_max + empty_stat_c) + (0)) = -CAKE_INF;
                    *(reinterpret_cast<float*>(partial_sum + empty_stat_c) + (0)) = 0.0f;
                }
    ...
    if (warp == 8) {
        { // mma_warp_main
            ...
            #pragma unroll 1
            for (unsigned int _tile_iter_m = 0; _tile_iter_m < BATCH_SIZE * NUM_KV_HEADS * NUM_SPLITS; _tile_iter_m++) {
                int split_m = tile_idx_m % (unsigned int)NUM_SPLITS;
                int batch_m = tile_idx_m / (unsigned int)(NUM_SPLITS * NUM_KV_HEADS);
                if (use_request_order != 0) {
                    batch_m = request_order[batch_m];
                }
                int kv_len_m = seq_lens_kv[batch_m];
                int blocks_2 = (kv_len_m + 127) / 128;
                int base_2 = blocks_2 / NUM_SPLITS;
                int extra_2 = blocks_2 % NUM_SPLITS;
                int count_2 = base_2 + ((split_m < extra_2) ? 1 : 0);
                int start_2 = split_m * base_2 + ((split_m < extra_2) ? split_m : extra_2);
                if (count_2 > 0) {
                    ...
                    #pragma unroll 1
                    for (int i_m = 0; i_m < count_2 - 1; i_m++) {
                        mbarrier_wait(kv_full_addr + (kv_slot_m) * 8, kv_phase_m);
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%0], [ta], db, id, p1;\n\t"
                    "}\n"
                    :: "r"(tmem_output_hi), "r"(_mma_b_lo_6), "r"(tmem_probabilities + p_slot_m * 128), "r"(((count_2 == 1) ? 0 : 1)));
                    elect_commit(kv_empty_addr + (kv_slot_m) * 8);
                    kv_slot_m += 1;
                    if (kv_slot_m == 9) { kv_slot_m = 0; kv_phase_m ^= 1; }
                    mbarrier_wait(kv_full_addr + (kv_slot_m) * 8, kv_phase_m);
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%0], [ta], db, id, p1;\n\t"
                    "}\n"
                    :: "r"(tmem_output_lo), "r"(_mma_b_lo_7), "r"(tmem_probabilities + p_slot_m * 128), "r"(((count_2 == 1) ? 0 : 1)));
                    elect_commit2(kv_empty_addr + (kv_slot_m) * 8, o_full_addr);
                    kv_slot_m += 1;
                    if (kv_slot_m == 9) { kv_slot_m = 0; kv_phase_m ^= 1; }
                    p_slot_m = (p_slot_m + 1) % 2;
    ...
    if (warp == 9) {
        { // load_pgoff_main
            ...
            #pragma unroll 1
            for (unsigned int _tile_iter_p = 0; _tile_iter_p < BATCH_SIZE * NUM_KV_HEADS * NUM_SPLITS; _tile_iter_p++) {
                int split_p = tile_idx_p % (unsigned int)NUM_SPLITS;
                int batch_idx_p = tile_idx_p / (unsigned int)(NUM_SPLITS * NUM_KV_HEADS);
                if (use_request_order != 0) {
                    batch_idx_p = request_order[batch_idx_p];
                }
                int seqlen_kv_p = seq_lens_kv[batch_idx_p];
                int blocks_3 = (seqlen_kv_p + 127) / 128;
                int base_3 = blocks_3 / NUM_SPLITS;
                int extra_3 = blocks_3 % NUM_SPLITS;
                int count_3 = base_3 + ((split_p < extra_3) ? 1 : 0);
                int start_3 = split_p * base_3 + ((split_p < extra_3) ? split_p : extra_3);
                int page_upper_p = (seqlen_kv_p + 63) / 64 - 1;
                #pragma unroll 1
                for (int group_base_p = 0; group_base_p < count_3; group_base_p += 16) {
                    int page_idx_p = (unsigned int)((start_3 + group_base_p) * 2) + lane;
    ...
    if (warp == 10) {
        { // scheduler_main
            ...
            unsigned int total_tiles_h = BATCH_SIZE * NUM_KV_HEADS * NUM_SPLITS;
    ...
    if (warp == 11) {
        { // load_warp_main
            ...
            for (unsigned int _tile_iter_l = 0; _tile_iter_l < total_tiles_l; _tile_iter_l++) {
                ...
                int blocks_4 = (seqlen_kv + 127) / 128;
                int base_4 = blocks_4 / NUM_SPLITS;
                int extra_4 = blocks_4 % NUM_SPLITS;
                int count_4 = base_4 + ((split_l < extra_4) ? 1 : 0);
                int start_4 = split_l * base_4 + ((split_l < extra_4) ? split_l : extra_4);
                if (count_4 > 0) {
                    ...
                    if (elect_sync()) {
                        ...
                        for (int action_l = 0; action_l < 2 * count_4; action_l++) {
                            ...
                            if (block_l % 16 == 15 || block_l + 1 == count_4) {
