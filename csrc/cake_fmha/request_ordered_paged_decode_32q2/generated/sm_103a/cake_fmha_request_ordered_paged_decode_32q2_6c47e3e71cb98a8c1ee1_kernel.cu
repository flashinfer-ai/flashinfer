...
#define SMEM_SMEM_CORR_OFF 201088
...
#define SMEM_SPLIT_WEIGHTS_STAGE_BYTES 2304
#define SMEM_SPLIT_WEIGHTS_STRIDE 2304
#define SMEM_SMEM_PAGE_OFFSETS_OFF 202624
#define SMEM_SMEM_PAGE_OFFSETS_STAGE_BYTES 768
#define SMEM_SMEM_PAGE_OFFSETS_STRIDE 768
#define SMEM_SMEM_Q6_SCALES_OFF 203392
#define SMEM_SMEM_Q6_SCALES_STAGE_BYTES 1024
#define SMEM_SMEM_Q6_SCALES_STRIDE 1024
#define SMEM_SMEM_Q6_FINAL_OFF 206464
...
kernel_cake_fmha_request_ordered_paged_decode_32q2_6c47e3e71cb98a8c1ee1(CakeTensorMap const* Qt, CakeTensorMap const* K, CakeTensorMap const* V, __nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_LSE, unsigned int* __restrict__ split_completion, __nv_bfloat16* __restrict__ O, float* __restrict__ LSE, int* __restrict__ page_table, int* __restrict__ seq_lens_kv, int* __restrict__ request_order, int max_pages_per_seq, int page_table_v_offset, float softmax_scale_log2, float output_scale, float* __restrict__ bmm1_scale_ptr, float* __restrict__ bmm2_scale_ptr, int bmm1_is_log2, int num_q_heads, int num_kv_heads, int batch_size, unsigned int total_tiles)
{
    ...
    // Kernel setup ops
    float* smem_corr = reinterpret_cast<float*>(smem_raw + 201088);
    const int smem_corr_addr = smem + 201088;
    ...
    int* smem_page_offsets = reinterpret_cast<int*>(smem_raw + 202624);
    const int smem_page_offsets_addr = smem + 202624;
    float* smem_q6_scales = reinterpret_cast<float*>(smem_raw + 203392);
    const int smem_q6_scales_addr = smem + 203392;
    float* smem_q6_final = reinterpret_cast<float*>(smem_raw + 206464);
    const int smem_q6_final_addr = smem + 206464;
        ...
        { // softmax_main
            ...
            for (unsigned int _tile_iter_s = 0; _tile_iter_s < total_tiles; _tile_iter_s++) {
                ...
                for (int n = 0; n < num_pairs * 2; n++) {
                    ...
                    int _min_0 = ((128) < (visible_qm - (split_start_pair * 2 + n) * 128) ? (128) : (visible_qm - (split_start_pair * 2 + n) * 128));
                    int _max_0 = ((0) > (_min_0) ? (0) : (_min_0));
                    int valid_qm = _max_0;
                    if (wg_tid >= 96) {
                        valid_qm = 0;
                    }
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // correction_main
            ...
            for (unsigned int _tile_iter_c = 0; _tile_iter_c < total_tiles; _tile_iter_c++) {
                ...
                if (corr_tid < 96) {
                    float lse_final_qm = -CAKE_INF;
                    if (sum_final_qm > 0.0f) {
                        float _log2_0;
                        asm volatile("lg2.approx.ftz.f32 %0, %1;" : "=f"(_log2_0) : "f"(sum_final_qm));
                        lse_final_qm = _log2_0 + max_final_qm * bmm1_scale_log2_c;
                    }
                    *(reinterpret_cast<float*>(partial_LSE + (output_row_qm * NUM_SPLIT + split_idx_c)) + (0)) = lse_final_qm;
                }
                float normalizer_qm = 0.0f;
                if (sum_final_qm > 0.0f) {
                    float _rcp_0 = approx_rcp(sum_final_qm);
                    normalizer_qm = _rcp_0 * bmm2_scale_c;
                }
                #pragma unroll
                for (int output_half_qm_1 = 0; output_half_qm_1 < 2; output_half_qm_1++) {
                    ...
                    for (int epilogue_chunk_qm = 0; epilogue_chunk_qm < 8; epilogue_chunk_qm++) {
                        ...
                        if (corr_tid < 96) {
                            int partial_offset_qm = (output_row_qm * NUM_SPLIT + split_idx_c) * HEAD_DIM + output_half_qm_1 * 128 + epilogue_chunk_qm * 16;
                            {
                ...
                if (USE_SEGMENTED_CLC != 0 && part_count_c > 1) {
                    ...
                    if (split_reduce_flag[0] != 0) {
                        __threadfence();
                        #pragma unroll
                        for (int reduce_head_slice = 0; reduce_head_slice < 6; reduce_head_slice++) {
                            ...
                            reduce_q_head = kv_head_idx_c * 16 + reduce_head / 16 * num_q_heads + reduce_head % 16;
                            ...
                            float _max_4 = max_noftz(lse0, lse1);
                            float lane_max = _max_4;
                            int subgroup_lane_base = lane / 8 * 8;
                            float merged_max = -CAKE_INF;
                            #pragma unroll
                            for (int source_lane = 0; source_lane < 8; source_lane++) {
                                float _shfl_2 = __shfl_sync(0xFFFFFFFF, lane_max, subgroup_lane_base + source_lane);
                                float source_max = _shfl_2;
                                float _max_5 = max_noftz(merged_max, source_max);
                                merged_max = _max_5;
                            }
                            float weight0 = 0.0f;
                            float weight1 = 0.0f;
                            ...
                            for (int source_lane_1 = 0; source_lane_1 < 8; source_lane_1++) {
                                float _shfl_3 = __shfl_sync(0xFFFFFFFF, lane_weight_sum, subgroup_lane_base + source_lane_1);
                                weight_sum = weight_sum + _shfl_3;
                            }
                            ...
                            if (merge_head_valid != 0) {
                                ...
                                merge_q_head = kv_head_idx_c * 16 + merge_head / 16 * num_q_heads + merge_head % 16;
                                #pragma unroll
                                for (int vec_chunk = 0; vec_chunk < 4; vec_chunk++) {
                                    ...
                                    float _vec_load_2[8];
                                    {
                                        const uint4* _vptr_4 = reinterpret_cast<const uint4*>(partial_O + partial_o_base + 0);
                                        uint4 _vld_4[1];
                                        #pragma unroll
                                        for (int _blk = 0; _blk < 1; _blk++) {
                                    ...
                                    for (int reduce_split = 1; reduce_split < NUM_SPLIT; reduce_split++) {
                                        float _vec_load_3[8];
                                        {
                                            const uint4* _vptr_5 = reinterpret_cast<const uint4*>(partial_O + (partial_o_base + reduce_split * HEAD_DIM) + 0);
                                            uint4 _vld_5[1];
                                            #pragma unroll
                                            for (int _blk = 0; _blk < 1; _blk++) {
