    ...
    if (warp <= 3) {
        ...
        { // softmax_main
            ...
            for (unsigned int _tile_iter_s = 0; _tile_iter_s < total_tiles; _tile_iter_s++) {
                int visible_keys = UNIFORM_KV_LEN - gridDim.x + q_row_idx_s + 1;
                {
                    visible_keys = seq_lens_kv[batch_idx_s] - gridDim.x + q_row_idx_s + 1;
                }
                if (visible_keys < 0) {
                    visible_keys = 0;
                }
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // correction_main
            ...
            for (unsigned int _tile_iter_c = 0; _tile_iter_c < total_tiles; _tile_iter_c++) {
                int visible_keys_1 = UNIFORM_KV_LEN - gridDim.x + q_row_idx_c + 1;
                {
                    visible_keys_1 = seq_lens_kv[batch_idx_c] - gridDim.x + q_row_idx_c + 1;
                }
                if (visible_keys_1 < 0) {
                    visible_keys_1 = 0;
                }
                ...
                if (corr_tid < 8) {
                    ...
                    if (stats_head_c < group_ratio_rt) {
                        ...
                        int stats_output_row_c = (batch_idx_c * gridDim.x + q_row_idx_c) * num_q_heads + stats_q_head_c;
                        {
                            {
                                {
                                    float _log2_2;
                ...
                for (int h_2 = 0; h_2 < 8; h_2++) {
                    ...
                    if (group_ratio_rt > h_2) {
                        ...
                        int output_row = (batch_idx_c * gridDim.x + q_row_idx_c) * num_q_heads + q_head;
                        {
                            {
                ...
                if (USE_SEGMENTED_CLC != 0 && part_count_c > 1) {
                    int base_tile_idx_seg = (batch_idx_c * gridDim.x + q_row_idx_c) * (num_kv_heads * Q_GROUPS_PER_KV) + kv_head_idx_c;
                    __threadfence();
                    asm volatile("barrier.sync 9, 128;" ::: "memory");
                    if (d_idx == 0) {
                    ...
                    if (split_reduce_flag[0] != 0) {
                        ...
                        int reduce_stat_base_seg = ((batch_idx_c * gridDim.x + q_row_idx_c) * num_q_heads + reduce_q_head_seg) * 16;
                            ...
                            if (reduce_lane_seg == 0) {
                                {
                                    ...
                                    int final_lse_idx_seg = (batch_idx_c * gridDim.x + q_row_idx_c) * num_q_heads + reduce_q_head_seg;
                        ...
                        if (merge_head_valid_seg != 0) {
                            ...
                            for (int vec_chunk_seg = 0; vec_chunk_seg < 4; vec_chunk_seg++) {
                                ...
                                int partial_o_base_seg = ((batch_idx_c * gridDim.x + q_row_idx_c) * num_q_heads + merge_q_head_seg) * 16 * HEAD_DIM + elem_base_seg;
                                int final_o_idx_seg = ((batch_idx_c * gridDim.x + q_row_idx_c) * num_q_heads + merge_q_head_seg) * HEAD_DIM + elem_base_seg;
                                float _vec_load_0[8];
                                {
                    ...
                    int base_tile_idx = (batch_idx_c * gridDim.x + q_row_idx_c) * (num_kv_heads * Q_GROUPS_PER_KV) + kv_head_idx_c;
                    __threadfence();
                    asm volatile("barrier.sync 9, 128;" ::: "memory");
                    if (d_idx == 0) {
                    ...
                    if (split_reduce_flag[0] != 0) {
                        ...
                        int reduce_stat_base = ((batch_idx_c * gridDim.x + q_row_idx_c) * num_q_heads + reduce_q_head) * NUM_SPLIT;
                            ...
                            if (reduce_lane == 0) {
                                {
                                    ...
                                    int final_lse_idx = (batch_idx_c * gridDim.x + q_row_idx_c) * num_q_heads + reduce_q_head;
                        ...
                        if (merge_head_valid != 0) {
                            ...
                            for (int vec_chunk = 0; vec_chunk < 4; vec_chunk++) {
                                ...
                                int partial_o_base = ((batch_idx_c * gridDim.x + q_row_idx_c) * num_q_heads + merge_q_head) * NUM_SPLIT * HEAD_DIM + elem_base;
                                int final_o_idx = ((batch_idx_c * gridDim.x + q_row_idx_c) * num_q_heads + merge_q_head) * HEAD_DIM + elem_base;
                                float _vec_load_2[8];
                                {
    ...
    if (warp == 8) {
        ...
        { // mma_warp_main
            ...
            for (unsigned int _tile_iter_m = 0; _tile_iter_m < total_tiles; _tile_iter_m++) {
                int visible_keys_2 = UNIFORM_KV_LEN - gridDim.x + q_row_idx_m + 1;
                {
                    visible_keys_2 = seq_lens_kv[batch_idx_m] - gridDim.x + q_row_idx_m + 1;
                }
                if (visible_keys_2 < 0) {
                    visible_keys_2 = 0;
                }
    ...
    if (warp == 9) {
        ...
        { // page_offsets_main
            ...
            for (unsigned int _tile_iter_p = 0; _tile_iter_p < total_tiles; _tile_iter_p++) {
                int visible_keys_3 = UNIFORM_KV_LEN - gridDim.x + q_row_idx_p + 1;
                {
                    visible_keys_3 = seq_lens_kv[batch_idx_p] - gridDim.x + q_row_idx_p + 1;
                }
                if (visible_keys_3 < 0) {
                    visible_keys_3 = 0;
                }
    ...
    if (warp == 11) {
        ...
        { // load_warp_main
            ...
            for (unsigned int _tile_iter_l = 0; _tile_iter_l < total_tiles; _tile_iter_l++) {
                ...
                int visible_keys_4 = UNIFORM_KV_LEN - gridDim.x + q_row_idx_1_1 + 1;
                {
                    visible_keys_4 = seq_lens_kv[batch_idx_0] - gridDim.x + q_row_idx_1_1 + 1;
                }
                if (visible_keys_4 < 0) {
                    visible_keys_4 = 0;
                }
                ...
                if (elect_sync()) {
                    ...
                    int off_qt = (batch_idx_0 * gridDim.x + q_row_idx_1_1) * num_q_heads + kv_head_idx_2_1 * group_ratio_l;
    ...
    if (warp >= 12 && warp <= 15) {
        ...
        { // transform_main
            ...
            for (unsigned int _tile_iter_t = 0; _tile_iter_t < total_tiles; _tile_iter_t++) {
                int visible_keys_5 = UNIFORM_KV_LEN - gridDim.x + q_row_idx_t + 1;
                {
                    visible_keys_5 = seq_lens_kv[batch_idx_t] - gridDim.x + q_row_idx_t + 1;
                }
                if (visible_keys_5 < 0) {
                    visible_keys_5 = 0;
                }
