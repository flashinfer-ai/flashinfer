...
CUtensorMap EncodeTmaQt(TensorView tensor) {
  ...
  CUresult result = cuTensorMapEncodeTiled(
      &tm, CU_TENSOR_MAP_DATA_TYPE_FLOAT16, 3, tensor.data_ptr(), global_dim, global_strides,
...
CUtensorMap EncodeTmaPagedKv(TensorView tensor, const char* name) {
  ...
  // decode.py normalizes logical NHD [page, token, head, dim] into a
  // zero-copy [page, head, token, dim] view. Preserve those non-contiguous
  // token/head strides when encoding the physical NHD pool.
  uint64_t global_dim[5] = {64u, 32u, 2u, static_cast<uint64_t>(tensor.size(1)),
  ...
  uint32_t box_dim[5] = {64u, 32u, 1u, 1u, 1u};
  uint32_t elem_strides[5] = {1u, 1u, 1u, 1u, 1u};
  CUtensorMap tm;
  CUresult result = cuTensorMapEncodeTiled(
      &tm, CU_TENSOR_MAP_DATA_TYPE_FLOAT16, 5, tensor.data_ptr(), global_dim, global_strides,
      box_dim, elem_strides, CU_TENSOR_MAP_INTERLEAVE_NONE, CU_TENSOR_MAP_SWIZZLE_128B,
      CU_TENSOR_MAP_L2_PROMOTION_NONE, CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
...
__global__ void PrepareDecodeMetadata(const int* seq_lens, int* causal_prefix,
  ...
  if (padded_page_table != nullptr && index < batch_size * 2 * padded_pages) {
...
void cake_paged_attention_decode(
  ...
  int64_t required_pages = even_kv_blocks * 4;
  int64_t source_pages = block_tables.size(2);
  int64_t padded_pages = AlignUp(std::max(required_pages, source_pages), 4);
  ...
  if (needs_page_padding) {
    cursor += batch_size * 2 * padded_pages * static_cast<int64_t>(sizeof(int));
  ...
  TVM_FFI_ICHECK_GE(actual_workspace_bytes, cursor)
      << "Cake FMHA decode-native FP16 NHD workspace requires " << cursor << " bytes";
  ...
  int64_t metadata_items =
      std::max<int64_t>(batch_size, needs_page_padding ? batch_size * 2 * padded_pages : 0);
  ...
  int max_pages_per_seq = static_cast<int>(padded_pages);
  ...
  TVM_FFI_ICHECK(scale_tensor.has_value())
      << "decode-native FP16 NHD scale-pointer specialization requires a tensor";
