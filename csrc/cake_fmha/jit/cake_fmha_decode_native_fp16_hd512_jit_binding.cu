...
CUtensorMap EncodeTmaQt(TensorView tensor) {
  ...
  TVM_FFI_ICHECK_EQ(tensor.size(2), 512);
  uint64_t global_dim[3] = {64u, static_cast<uint64_t>(tensor.size(0) * tensor.size(1)), 8u};
  uint64_t global_strides[2] = {1024u, 128u};
...
CUtensorMap EncodeTmaPagedKv(TensorView tensor, const char* name) {
  ...
  TVM_FFI_ICHECK_EQ(tensor.dtype(), dl_float16);
  TVM_FFI_ICHECK_EQ(tensor.size(2), 64);
  TVM_FFI_ICHECK_EQ(tensor.size(3), 512);
  ...
  uint64_t global_dim[5] = {64u, 64u, 8u, static_cast<uint64_t>(tensor.size(1)),
...
void cake_paged_attention_decode(
  ...
  TVM_FFI_ICHECK_EQ(query.size(2), 512);
  ...
  int64_t required_pages = even_kv_blocks * 2;
  ...
  TVM_FFI_ICHECK_GE(actual_workspace_bytes, cursor)
      << "Cake FMHA decode-native FP16 head-dim-512 workspace requires " << cursor << " bytes";
  ...
  TVM_FFI_ICHECK(scale_tensor.has_value())
      << "decode-native FP16 head-dim-512 scale-pointer specialization requires a tensor";
