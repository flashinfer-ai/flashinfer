...
void cake_paged_attention_decode(
  ...
  auto const* p_q =
      reinterpret_cast<CakeFmhaTensorMap const*>(q_slot.pointer);
  auto const* p_k =
  ...
  auto ksf_slot = TmaDeviceSlot(h_ksf, query.device().device_id, stream);
  auto vsf_slot = TmaDeviceSlot(h_vsf, query.device().device_id, stream);
  auto const* p_ksf =
      reinterpret_cast<CakeFmhaTensorMap const*>(ksf_slot.pointer);
  auto const* p_vsf =
      reinterpret_cast<CakeFmhaTensorMap const*>(vsf_slot.pointer);
#endif
...
#endif
#if CAKE_FMHA_NVFP4
  RecordTmaDeviceSlotUses(
      {q_slot, k_slot, v_slot, ksf_slot, vsf_slot}, stream);
#else
