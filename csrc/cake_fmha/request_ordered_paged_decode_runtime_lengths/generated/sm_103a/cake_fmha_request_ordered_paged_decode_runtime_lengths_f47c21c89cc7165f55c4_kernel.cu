...
#define SMEM_SPLIT_WEIGHTS_STAGE_BYTES 8192
#define SMEM_SPLIT_WEIGHTS_STRIDE 8192
    ...
    if (warp == 10) {
        if (elect_sync()) {
            ...
            for (int runtime_slot = 0; runtime_slot < batch_size; runtime_slot++) {
                ...
                {
                    runtime_request = request_order[runtime_slot];
                }
            ...
            for (int runtime_slot_p = 0; runtime_slot_p < batch_size; runtime_slot_p++) {
                ...
                int _min_2 = ((256) < (_max_5) ? (256) : (_max_5));
                runtime_p = _min_2;
    ...
    if (warp <= 3) {
        ...
        { // softmax_main
            ...
            if (runtime_virtual < runtime_tile_total) {
                ...
                {
                    runtime_batch = request_order[runtime_batch];
                }
            }
            ...
            for (unsigned int _tile_iter_s = 0; _tile_iter_s < runtime_work; _tile_iter_s++) {
                ...
                if (corr_prod_stage == 2) { corr_prod_stage = 0; corr_prod_phase ^= 1; }
                {
                    ...
                    if (runtime_virtual_0 < runtime_tile_total_1) {
                        ...
                        {
                            runtime_batch_3 = request_order[runtime_batch_3];
                        }
                    }
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // correction_main
            ...
            if (runtime_virtual_1 < runtime_tile_total_2) {
                ...
                {
                    runtime_batch_1 = request_order[runtime_batch_1];
                }
            }
            ...
            for (unsigned int _tile_iter_c = 0; _tile_iter_c < runtime_work_1; _tile_iter_c++) {
                    ...
                    if (stats_head_c < group_ratio_rt) {
                        ...
                        long long stats_output_row_c = ((long long)batch_idx_c * (long long)Q_LEN + (long long)q_row_idx_c) * (long long)num_q_heads + (long long)stats_q_head_c;
                        {
                            if (part_count_c == 1) {
                                ...
                                long long partial_lse_idx = (long long)stats_output_row_c * 256 + (long long)split_idx_c;
                        ...
                        long long output_row = ((long long)batch_idx_c * (long long)Q_LEN + (long long)q_row_idx_c) * (long long)num_q_heads + (long long)q_head;
                        {
                            if (part_count_c == 1) {
                                ...
                                *(reinterpret_cast<__nv_bfloat16*>(O + direct_out_idx_lo_seg) + (0)) = __float2bfloat16_rn(final_o_lo);
                                ...
                                long long partial_row = (long long)output_row * 256 + (long long)split_idx_c;
                ...
                if (USE_SEGMENTED_CLC != 0 && part_count_c > 1) {
                    ...
                    if (split_reduce_flag[0] != 0) {
                        ...
                        long long reduce_stat_base_seg = (((long long)batch_idx_c * (long long)Q_LEN + (long long)q_row_idx_c) * (long long)num_q_heads + (long long)reduce_q_head_seg) * 256;
                        int split0_seg = reduce_lane_seg;
                        int split1_seg = reduce_lane_seg + 8;
                        float lse0_seg = -CAKE_INF;
                        float lse1_seg = -CAKE_INF;
                        float cached_lse_seg[16];
                        cached_lse_seg[0] = -CAKE_INF;
                        cached_lse_seg[1] = -CAKE_INF;
                        cached_lse_seg[2] = -CAKE_INF;
                        cached_lse_seg[3] = -CAKE_INF;
                        cached_lse_seg[4] = -CAKE_INF;
                        cached_lse_seg[5] = -CAKE_INF;
                        cached_lse_seg[6] = -CAKE_INF;
                        cached_lse_seg[7] = -CAKE_INF;
                        cached_lse_seg[8] = -CAKE_INF;
                        cached_lse_seg[9] = -CAKE_INF;
                        cached_lse_seg[10] = -CAKE_INF;
                        cached_lse_seg[11] = -CAKE_INF;
                        cached_lse_seg[12] = -CAKE_INF;
                        cached_lse_seg[13] = -CAKE_INF;
                        cached_lse_seg[14] = -CAKE_INF;
                        cached_lse_seg[15] = -CAKE_INF;
                        #pragma unroll
                        for (int cache_chunk_seg = 0; cache_chunk_seg < 16; cache_chunk_seg++) {
                        ...
                        #pragma unroll
                        for (int stats_chunk_seg = 0; stats_chunk_seg < 16; stats_chunk_seg++) {
                            float _max_17 = max_noftz(lane_max_seg, cached_lse_seg[stats_chunk_seg]);
                            lane_max_seg = _max_17;
                        }
                        int subgroup_lane_base_seg = lane / 16 * 16;
                        ...
                        #pragma unroll
                        for (int weight_chunk_seg = 0; weight_chunk_seg < 16; weight_chunk_seg++) {
                        ...
                        for (int wide_head_batch_seg = 0; wide_head_batch_seg < wide_head_batches_seg; wide_head_batch_seg++) {
                            ...
                            long long wide_partial_base_seg = wide_final_row_seg * 256 * (long long)HEAD_DIM + (long long)wide_elem_seg;
                            float _vec_load_0[8];
                            {
                        ...
                        if (assisted_reduce_seg != 0) {
                            if (split_reduce_flag[0] == 2) {
                                ...
                                float _vec_load_2[8];
                                {
                                    const uint4* _vptr_4 = reinterpret_cast<const uint4*>(partial_O + (copied_row_seg * 256 * (long long)HEAD_DIM + (long long)copied_elem_seg) + 0);
                                    uint4 _vld_4[1];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 1; _blk++) {
                ...
                }
                {
                    ...
                    if (runtime_virtual_0_1 < runtime_tile_total_1_1) {
                        ...
                        {
                            runtime_batch_3_1 = request_order[runtime_batch_3_1];
                        }
                    }
    ...
    if (warp == 8) {
        ...
        { // mma_warp_main
            ...
            if (runtime_virtual_2 < runtime_tile_total_3) {
                ...
                {
                    runtime_batch_2 = request_order[runtime_batch_2];
                }
            }
            ...
            for (unsigned int _tile_iter_m = 0; _tile_iter_m < runtime_work_2; _tile_iter_m++) {
                ...
                if (runtime_virtual_0_2 < runtime_tile_total_1_2) {
                    ...
                    {
                        runtime_batch_3_2 = request_order[runtime_batch_3_2];
                    }
                }
    ...
    if (warp == 9) {
        ...
        { // page_offsets_main
            ...
            if (runtime_virtual_3 < runtime_tile_total_4) {
                ...
                {
                    runtime_batch_4 = request_order[runtime_batch_4];
                }
            }
            ...
            for (unsigned int _tile_iter_p = 0; _tile_iter_p < runtime_work_3; _tile_iter_p++) {
                ...
                if (runtime_virtual_0_3 < runtime_tile_total_1_3) {
                    ...
                    {
                        runtime_batch_3_3 = request_order[runtime_batch_3_3];
                    }
                }
    ...
    if (warp == 11) {
        ...
        { // load_warp_main
            ...
            if (runtime_virtual_4 < runtime_tile_total_5) {
                ...
                {
                    runtime_batch_5 = request_order[runtime_batch_5];
                }
            }
            ...
            for (unsigned int _tile_iter_l = 0; _tile_iter_l < runtime_work_4; _tile_iter_l++) {
                ...
                if (runtime_virtual_0_4 < runtime_tile_total_1_4) {
                    ...
                    {
                        runtime_batch_3_4 = request_order[runtime_batch_3_4];
                    }
                }
    ...
    if (warp >= 12 && warp <= 15) {
        ...
        { // transform_main
            ...
            if (runtime_virtual_5 < runtime_tile_total_6) {
                ...
                {
                    runtime_batch_6 = request_order[runtime_batch_6];
                }
            }
            ...
            for (unsigned int _tile_iter_t = 0; _tile_iter_t < runtime_work_5; _tile_iter_t++) {
                ...
                if (runtime_virtual_0_5 < runtime_tile_total_1_5) {
                    ...
                    {
                        runtime_batch_3_5 = request_order[runtime_batch_3_5];
                    }
                }
