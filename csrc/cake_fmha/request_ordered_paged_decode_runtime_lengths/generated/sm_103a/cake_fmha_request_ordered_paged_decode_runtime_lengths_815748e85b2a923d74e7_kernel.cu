...
kernel_cake_fmha_request_ordered_paged_decode_runtime_lengths_815748e85b2a923d74e7(CakeTensorMap const* Qt, CakeTensorMap const* K, CakeTensorMap const* V, __nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_LSE, unsigned int* __restrict__ split_completion, __nv_bfloat16* __restrict__ O, float* __restrict__ LSE, int* __restrict__ page_table, int* __restrict__ seq_lens_kv, int* __restrict__ request_order, int max_pages_per_seq, int page_table_v_offset, float softmax_scale_log2, float output_scale, float* __restrict__ bmm1_scale_ptr, float* __restrict__ bmm2_scale_ptr, int bmm1_is_log2, int num_q_heads, int num_kv_heads, int batch_size, unsigned int total_tiles)
{
    ...
    if (warp == 10) {
        ...
        #pragma unroll
        for (int runtime_lane_mask = 0; runtime_lane_mask < 5; runtime_lane_mask++) {
            unsigned int _shfl_xor_0 = __shfl_xor_sync(0xFFFFFFFF, runtime_sum, 1 << runtime_lane_mask);
            int _shfl_xor_1 = __shfl_xor_sync(0xFFFFFFFF, runtime_min_len, 1 << runtime_lane_mask);
            int _shfl_xor_2 = __shfl_xor_sync(0xFFFFFFFF, runtime_max_len, 1 << runtime_lane_mask);
            runtime_sum += _shfl_xor_0;
            int _min_1 = ((runtime_min_len) < (_shfl_xor_1) ? (runtime_min_len) : (_shfl_xor_1));
            runtime_min_len = _min_1;
            int _max_3 = ((runtime_max_len) > (_shfl_xor_2) ? (runtime_max_len) : (_shfl_xor_2));
            runtime_max_len = _max_3;
        }
        __syncwarp();
        if (warp == 10) {
            if (elect_sync()) {
                ...
                if (batch_size > 0) {
                    if (runtime_min_len == runtime_max_len) {
                        ...
                        int _min_2 = ((gridDim.z / batch_size) < (runtime_uniform_units) ? (gridDim.z / batch_size) : (runtime_uniform_units));
                        runtime_uniform = _min_2;
                    }
                }
                int runtime_total = batch_size * runtime_uniform;
                if (runtime_uniform == 0) {
                    int _max_4 = ((batch_size) > (304) ? (batch_size) : (304));
                    unsigned int runtime_target = (unsigned int)_max_4;
                    unsigned int runtime_pairs = runtime_sum / runtime_target;
                    if (runtime_sum % runtime_target != 0) {
                        runtime_pairs += 1;
                    }
                    unsigned int _max_5 = ((runtime_pairs) > (4) ? (runtime_pairs) : (4));
                    runtime_pairs = _max_5;
                    runtime_prefix[0] = 0;
                    runtime_total = 0;
                    #pragma unroll 1
                    for (int runtime_slot_p = 0; runtime_slot_p < batch_size; runtime_slot_p++) {
                        ...
                        int _max_6 = ((runtime_p) > (1) ? (runtime_p) : (1));
                        int _min_3 = ((16) < (_max_6) ? (16) : (_max_6));
                        runtime_p = _min_3;
            ...
            for (unsigned int _tile_iter_c = 0; _tile_iter_c < runtime_work_1; _tile_iter_c++) {
                    ...
                    if (stats_head_c < group_ratio_rt) {
                        ...
                        long long stats_output_row_c = ((long long)batch_idx_c * (long long)Q_LEN + (long long)q_row_idx_c) * (long long)num_q_heads + (long long)stats_q_head_c;
                        {
                            if (part_count_c == 1) {
                                ...
                                long long partial_lse_idx = (long long)stats_output_row_c * 16 + (long long)split_idx_c;
                        ...
                        long long output_row = ((long long)batch_idx_c * (long long)Q_LEN + (long long)q_row_idx_c) * (long long)num_q_heads + (long long)q_head;
                        {
                            if (part_count_c == 1) {
                                ...
                                *(reinterpret_cast<__nv_bfloat16*>(O + direct_out_idx_lo_seg) + (0)) = __float2bfloat16_rn(final_o_lo);
                                ...
                                long long partial_row = (long long)output_row * 16 + (long long)split_idx_c;
                ...
                if (USE_SEGMENTED_CLC != 0 && part_count_c > 1) {
                    ...
                    if (split_reduce_flag[0] != 0) {
                        ...
                        long long reduce_stat_base_seg = (((long long)batch_idx_c * (long long)Q_LEN + (long long)q_row_idx_c) * (long long)num_q_heads + (long long)reduce_q_head_seg) * 16;
                        ...
                        if (reduce_head_valid_seg != 0) {
                            if (split0_seg < part_count_c) {
                                lse0_seg = partial_LSE[reduce_stat_base_seg + (long long)split0_seg];
                            }
                            if (split1_seg < part_count_c) {
                                lse1_seg = partial_LSE[reduce_stat_base_seg + (long long)split1_seg];
                            }
                        }
                        ...
                        for (int source_lane_seg = 0; source_lane_seg < 8; source_lane_seg++) {
                            float _shfl_6;
                            asm volatile("shfl.sync.idx.b32 %0, %1, %2, 0x1f, 0xffffffff;" : "=f"(_shfl_6) : "f"(lane_max_seg), "r"(subgroup_lane_base_seg + source_lane_seg));
                            float source_max_seg = _shfl_6;
                            float _max_19 = max_noftz(merged_max_seg, source_max_seg);
                            merged_max_seg = _max_19;
                        }
                        ...
                        for (int source_lane_seg_1 = 0; source_lane_seg_1 < 8; source_lane_seg_1++) {
                            float _shfl_7;
                            asm volatile("shfl.sync.idx.b32 %0, %1, %2, 0x1f, 0xffffffff;" : "=f"(_shfl_7) : "f"(lane_weight_sum_seg), "r"(subgroup_lane_base_seg + source_lane_seg_1));
                        ...
                        if (merge_head_valid_seg != 0) {
                            ...
                            for (int vec_chunk_seg = 0; vec_chunk_seg < 4; vec_chunk_seg++) {
                                ...
                                long long partial_o_base_seg = (((long long)batch_idx_c * (long long)Q_LEN + (long long)q_row_idx_c) * (long long)num_q_heads + (long long)merge_q_head_seg) * 16 * (long long)HEAD_DIM + (long long)elem_base_seg;
                                long long final_o_idx_seg = (((long long)batch_idx_c * (long long)Q_LEN + (long long)q_row_idx_c) * (long long)num_q_heads + (long long)merge_q_head_seg) * (long long)HEAD_DIM + (long long)elem_base_seg;
                                float _vec_load_0[8];
                                {
                                ...
                                for (int reduce_part_seg = 1; reduce_part_seg < part_count_c; reduce_part_seg++) {
                                    float _vec_load_1[8];
                                    {
                                        const uint4* _vptr_1 = reinterpret_cast<const uint4*>(partial_O + (partial_o_base_seg + (long long)(reduce_part_seg * HEAD_DIM)) + 0);
                                        uint4 _vld_1[1];
                                        #pragma unroll
                                        for (int _blk = 0; _blk < 1; _blk++) {
                    ...
                    if (split_reduce_flag[0] != 0) {
                        ...
                        for (int source_lane = 0; source_lane < 8; source_lane++) {
                            float _shfl_8;
                            asm volatile("shfl.sync.idx.b32 %0, %1, %2, 0x1f, 0xffffffff;" : "=f"(_shfl_8) : "f"(lane_max), "r"(subgroup_lane_base + source_lane));
                            float source_max = _shfl_8;
                            float _max_21 = max_noftz(merged_max, source_max);
                            merged_max = _max_21;
                        }
                        float weight0 = 0.0f;
                        float weight1 = 0.0f;
                        ...
                        for (int source_lane_1 = 0; source_lane_1 < 8; source_lane_1++) {
                            float _shfl_9;
                            asm volatile("shfl.sync.idx.b32 %0, %1, %2, 0x1f, 0xffffffff;" : "=f"(_shfl_9) : "f"(lane_weight_sum), "r"(subgroup_lane_base + source_lane_1));
