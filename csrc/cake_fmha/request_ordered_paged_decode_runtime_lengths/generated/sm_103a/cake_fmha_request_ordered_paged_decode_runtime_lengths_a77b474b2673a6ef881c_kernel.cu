...
#define SMEM_SPLIT_WEIGHTS_STAGE_BYTES 4096
#define SMEM_SPLIT_WEIGHTS_STRIDE 4096
    ...
    if (warp == 10) {
        if (elect_sync()) {
            ...
            for (int runtime_slot_p = 0; runtime_slot_p < batch_size; runtime_slot_p++) {
                ...
                int _min_2 = ((128) < (_max_5) ? (128) : (_max_5));
                runtime_p = _min_2;
            ...
            for (unsigned int _tile_iter_c = 0; _tile_iter_c < runtime_work_1; _tile_iter_c++) {
                    ...
                    if (stats_head_c < group_ratio_rt) {
                        ...
                        long long stats_output_row_c = ((long long)batch_idx_c * (long long)Q_LEN + (long long)q_row_idx_c) * (long long)num_q_heads + (long long)stats_q_head_c;
                        {
                            if (part_count_c == 1) {
                                ...
                                long long partial_lse_idx = (long long)stats_output_row_c * 128 + (long long)split_idx_c;
                        ...
                        long long output_row = ((long long)batch_idx_c * (long long)Q_LEN + (long long)q_row_idx_c) * (long long)num_q_heads + (long long)q_head;
                        {
                            if (part_count_c == 1) {
                                ...
                                *(reinterpret_cast<__nv_bfloat16*>(O + direct_out_idx_lo_seg) + (0)) = __float2bfloat16_rn(final_o_lo);
                                ...
                                long long partial_row = (long long)output_row * 128 + (long long)split_idx_c;
                ...
                if (USE_SEGMENTED_CLC != 0 && part_count_c > 1) {
                    ...
                    if (split_reduce_flag[0] != 0) {
                        ...
                        long long reduce_stat_base_seg = (((long long)batch_idx_c * (long long)Q_LEN + (long long)q_row_idx_c) * (long long)num_q_heads + (long long)reduce_q_head_seg) * 128;
                        int split0_seg = reduce_lane_seg;
                        int split1_seg = reduce_lane_seg + 8;
                        float lse0_seg = -CAKE_INF;
                        float lse1_seg = -CAKE_INF;
                        float cached_lse_seg[8];
                        ...
                        #pragma unroll
                        for (int cache_chunk_seg = 0; cache_chunk_seg < 8; cache_chunk_seg++) {
                        ...
                        #pragma unroll
                        for (int stats_chunk_seg = 0; stats_chunk_seg < 8; stats_chunk_seg++) {
                        ...
                        #pragma unroll
                        for (int weight_chunk_seg = 0; weight_chunk_seg < 8; weight_chunk_seg++) {
                        ...
                        for (int wide_head_batch_seg = 0; wide_head_batch_seg < wide_head_batches_seg; wide_head_batch_seg++) {
                            ...
                            long long wide_partial_base_seg = wide_final_row_seg * 128 * (long long)HEAD_DIM + (long long)wide_elem_seg;
                            float _vec_load_0[8];
                            {
                        ...
                        if (assisted_reduce_seg != 0) {
                            if (split_reduce_flag[0] == 2) {
                                ...
                                float _vec_load_2[8];
                                {
                                    const uint4* _vptr_4 = reinterpret_cast<const uint4*>(partial_O + (copied_row_seg * 128 * (long long)HEAD_DIM + (long long)copied_elem_seg) + 0);
                                    uint4 _vld_4[1];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 1; _blk++) {
                                ...
                                {
                                    if (lane == 0) {
                                        float copied_lse_seg = partial_LSE[copied_row_seg * 128];
