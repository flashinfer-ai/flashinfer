...
extern "C" __global__ void kernel_cake_fmha_context_fp8(
...
extern "C" cudaError_t cake_fmha_launch_context_fp8(
  ...
  cudaError_t status =
      cudaFuncSetAttribute(reinterpret_cast<const void*>(kernel_cake_fmha_context_fp8),
                           cudaFuncAttributeMaxDynamicSharedMemorySize, 86528);
  ...
  return cudaLaunchKernel(reinterpret_cast<const void*>(kernel_cake_fmha_context_fp8),
                          dim3(grid_x, grid_y, grid_z), dim3(512, 1, 1), kernel_args, 86528,
                          stream);
}
