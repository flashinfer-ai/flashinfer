...
extern "C" __global__ void kernel_cake_fmha_decode_quant_nvfp4(
    ...
    int num_splits, int blocks_per_split);
...
extern "C" cudaError_t cake_fmha_launch_decode_quant_nvfp4(
  ...
  cudaError_t status =
      cudaFuncSetAttribute(reinterpret_cast<const void*>(kernel_cake_fmha_decode_quant_nvfp4),
                           cudaFuncAttributeMaxDynamicSharedMemorySize, 147456);
  if (status != cudaSuccess) {
    return status;
  }
  void* kernel_args[] = {const_cast<void*>(reinterpret_cast<const void*>(&Qt)),
  ...
  return cudaLaunchKernel(reinterpret_cast<const void*>(kernel_cake_fmha_decode_quant_nvfp4),
                          dim3(grid_x, grid_y, grid_z), dim3(512, 1, 1), kernel_args, 147456,
                          stream);
}
