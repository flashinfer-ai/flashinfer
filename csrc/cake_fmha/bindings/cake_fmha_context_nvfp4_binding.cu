...
extern "C" __global__ void kernel_cake_fmha_context_nvfp4(
    ...
    int page_row_stride, int num_ctas, uint32_t* dynamic_counter);
...
extern "C" cudaError_t cake_fmha_launch_context_nvfp4(
  ...
  cudaError_t status =
      cudaFuncSetAttribute(reinterpret_cast<const void*>(kernel_cake_fmha_context_nvfp4),
  ...
  return cudaLaunchKernel(reinterpret_cast<const void*>(kernel_cake_fmha_context_nvfp4),
                          dim3(grid_x, grid_y, grid_z), dim3(512, 1, 1), kernel_args, 104960,
                          stream);
}
