...
extern "C" __global__ void kernel_cake_fmha_context_fp16_hd256(
...
extern "C" cudaError_t cake_fmha_launch_context_fp16_hd256(
  ...
  cudaError_t status =
      cudaFuncSetAttribute(reinterpret_cast<const void*>(kernel_cake_fmha_context_fp16_hd256),
                           cudaFuncAttributeMaxDynamicSharedMemorySize, 200960);
  ...
  return cudaLaunchKernel(reinterpret_cast<const void*>(kernel_cake_fmha_context_fp16_hd256),
                          dim3(grid_x, grid_y, grid_z), dim3(384, 1, 1), kernel_args, 200960,
                          stream);
}
