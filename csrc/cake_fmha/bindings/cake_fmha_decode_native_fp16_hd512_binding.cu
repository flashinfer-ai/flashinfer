...
extern "C" __global__ void kernel_cake_fmha_decode_native_fp16_hd512(
    ...
    int window_left, int num_q_heads, int num_kv_heads, int batch_size);
...
extern "C" cudaError_t cake_fmha_launch_decode_native_fp16_hd512(
  ...
  cudaError_t status =
      cudaFuncSetAttribute(reinterpret_cast<const void*>(kernel_cake_fmha_decode_native_fp16_hd512),
                           cudaFuncAttributeMaxDynamicSharedMemorySize, 149504);
  ...
  return cudaLaunchKernel(reinterpret_cast<const void*>(kernel_cake_fmha_decode_native_fp16_hd512),
                          dim3(grid_x, grid_y, grid_z), dim3(512, 1, 1), kernel_args, 149504,
                          stream);
}
