...
extern "C" cudaError_t cake_fmha_launch_decode_quant_bf16q(
  ...
  cudaError_t status =
      cudaFuncSetAttribute(reinterpret_cast<const void*>(kernel_cake_fmha_decode_quant_bf16q),
                           cudaFuncAttributeMaxDynamicSharedMemorySize, 139904);
  if (status != cudaSuccess) {
    return status;
  }
  void* kernel_args[] = {const_cast<void*>(reinterpret_cast<const void*>(&Q_ptr)),
  ...
  return cudaLaunchKernel(reinterpret_cast<const void*>(kernel_cake_fmha_decode_quant_bf16q),
                          dim3(grid_x, grid_y, grid_z), dim3(512, 1, 1), kernel_args, 139904,
                          stream);
}
