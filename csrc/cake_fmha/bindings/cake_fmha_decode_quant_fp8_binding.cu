...
extern "C" __global__ void kernel_cake_fmha_decode_quant_fp8(
    ...
    int bmm1_is_log2, int num_splits, int blocks_per_split);
...
extern "C" cudaError_t cake_fmha_launch_decode_quant_fp8(
  ...
  cudaError_t status =
      cudaFuncSetAttribute(reinterpret_cast<const void*>(kernel_cake_fmha_decode_quant_fp8),
                           cudaFuncAttributeMaxDynamicSharedMemorySize, 157952);
  ...
  return cudaLaunchKernel(reinterpret_cast<const void*>(kernel_cake_fmha_decode_quant_fp8),
                          dim3(grid_x, grid_y, grid_z), dim3(512, 1, 1), kernel_args, 157952,
                          stream);
}
