...
extern "C" __global__ void kernel_cake_fmha_dcp_spec_bf16_fp8(
    ...
    int cp_rank, int num_q_heads, int num_kv_heads, int batch_size);
...
extern "C" cudaError_t cake_fmha_launch_dcp_spec_bf16_fp8(
  ...
  cudaError_t status =
      cudaFuncSetAttribute(reinterpret_cast<const void*>(kernel_cake_fmha_dcp_spec_bf16_fp8),
  ...
  return cudaLaunchKernel(reinterpret_cast<const void*>(kernel_cake_fmha_dcp_spec_bf16_fp8),
                          dim3(grid_x, grid_y, grid_z), dim3(512, 1, 1), kernel_args, 208896,
                          stream);
}
