...
extern "C" __global__ void kernel_cake_fmha_context_bf16(
...
extern "C" cudaError_t cake_fmha_launch_context_bf16(
  ...
  cudaError_t status =
      cudaFuncSetAttribute(reinterpret_cast<const void*>(kernel_cake_fmha_context_bf16),
                           cudaFuncAttributeMaxDynamicSharedMemorySize, 168448);
  ...
  return cudaLaunchKernel(reinterpret_cast<const void*>(kernel_cake_fmha_context_bf16),
                          dim3(grid_x, grid_y, grid_z), dim3(512, 1, 1), kernel_args, 168448,
                          stream);
}
