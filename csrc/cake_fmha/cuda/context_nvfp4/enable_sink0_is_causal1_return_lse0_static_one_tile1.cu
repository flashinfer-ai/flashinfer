...
__global__ __launch_bounds__(512, 1) void kernel_cake_fmha_context_nvfp4(
    CakeFmhaTensorMap const* Q, CakeFmhaTensorMap const* Kp, CakeFmhaTensorMap const* Vp,
    CakeFmhaTensorMap const* Ksf, CakeFmhaTensorMap const* Vsf, uint8_t* __restrict__ O_ptr,
    float* __restrict__ LSE_ptr, float* __restrict__ sinks, int* __restrict__ page_table_k,
    int* __restrict__ page_table_v, int* __restrict__ seq_lens_q, int* __restrict__ seq_lens_kv,
    int* __restrict__ cu_seq_lens_q, float softmax_scale_log2, float output_scale, int total_bh,
    int page_row_stride, int num_ctas, unsigned int* __restrict__ dynamic_counter) {
...
#define work_id_empty_addr (mbar_base + 320)
  ...
  if (tid == 0) {
    ...
    mbarrier_init_owner_lane(smem_raw + 16, 4);
    mbarrier_init_owner_lane(smem_raw + 24, 4);
    mbarrier_init_owner_lane(smem_raw + 32, 4);
    ...
    mbarrier_init_owner_lane(smem_raw + 80, 4);
    mbarrier_init_owner_lane(smem_raw + 88, 4);
    // s_full: 2 barriers, init_count=1
    mbarrier_init_owner_lane(smem_raw + 96, 1);
    mbarrier_init_owner_lane(smem_raw + 104, 1);
    // p_full: 2 barriers, init_count=256
    mbarrier_init_owner_lane(smem_raw + 112, 256);
    mbarrier_init_owner_lane(smem_raw + 120, 256);
    // p_full_2: 2 barriers, init_count=128
    mbarrier_init_owner_lane(smem_raw + 128, 128);
    mbarrier_init_owner_lane(smem_raw + 136, 128);
    // corr_sig: 2 barriers, init_count=128
    mbarrier_init_owner_lane(smem_raw + 144, 128);
    mbarrier_init_owner_lane(smem_raw + 152, 128);
    // corr_done: 2 barriers, init_count=128
    mbarrier_init_owner_lane(smem_raw + 160, 128);
    mbarrier_init_owner_lane(smem_raw + 168, 128);
    // order_p01_0: 1 barriers, init_count=128
    mbarrier_init_owner_lane(smem_raw + 176, 128);
    // order_p01_1: 1 barriers, init_count=128
    mbarrier_init_owner_lane(smem_raw + 184, 128);
    ...
    mbarrier_init_owner_lane(smem_raw + 216, 8);
    mbarrier_init_owner_lane(smem_raw + 224, 8);
    mbarrier_init_owner_lane(smem_raw + 232, 8);
    mbarrier_init_owner_lane(smem_raw + 240, 8);
    mbarrier_init_owner_lane(smem_raw + 248, 8);
    mbarrier_init_owner_lane(smem_raw + 256, 8);
    // page_empty: 6 barriers, init_count=1
    mbarrier_init_owner_lane(smem_raw + 264, 1);
    mbarrier_init_owner_lane(smem_raw + 272, 1);
    mbarrier_init_owner_lane(smem_raw + 280, 1);
    mbarrier_init_owner_lane(smem_raw + 288, 1);
    ...
    mbarrier_init_owner_lane(smem_raw + 320, 15);
  }
      ...
      for (unsigned int tile_iter_1 = 0; tile_iter_1 < max_rounds_1; tile_iter_1++) {
        ...
        unsigned int ksf_words[2];
        {
          asm volatile("ld.shared.b32 %0, [%1];"
                       : "=r"(*reinterpret_cast<uint32_t*>(&ksf_words[0]))
                       : "r"(sf_base + dq_tok * 8));
          asm volatile("ld.shared.b32 %0, [%1];"
                       : "=r"(*reinterpret_cast<uint32_t*>(&ksf_words[1]))
                       : "r"(sf_base + dq_tok * 8 + 4));
        }
#pragma unroll
        for (int chunk_wave = 0; chunk_wave < 4; chunk_wave++) {
          ...
          asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                       : "=r"(*reinterpret_cast<uint32_t*>(&packed_words[0])),
                         "=r"(*reinterpret_cast<uint32_t*>(&packed_words[(0) + 1])),
                         "=r"(*reinterpret_cast<uint32_t*>(&packed_words[(0) + 2])),
                         "=r"(*reinterpret_cast<uint32_t*>(&packed_words[(0) + 3]))
...
#pragma unroll
          for (int in_pair = 0; in_pair < 2; in_pair++) {
            ...
            {
              encoded[0] = cake_fmha_qmul4_portable<5>(packed_words[packed_idx], sf);
            }
            {
              encoded[1] = cake_fmha_qmul4_portable<6>(packed_words[packed_idx], sf);
            }
            {
              encoded[2] = cake_fmha_qmul4_portable<5>(packed_words[packed_idx + 1], sf);
            }
            {
              encoded[3] = cake_fmha_qmul4_portable<6>(packed_words[packed_idx + 1], sf);
            }
        ...
        unsigned int vsf_words[8];
        {
          ...
          asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                       : "=r"(*reinterpret_cast<uint32_t*>(&vsf_words[0])),
                         "=r"(*reinterpret_cast<uint32_t*>(&vsf_words[(0) + 1])),
                         "=r"(*reinterpret_cast<uint32_t*>(&vsf_words[(0) + 2])),
                         "=r"(*reinterpret_cast<uint32_t*>(&vsf_words[(0) + 3]))
                       : "r"(sf_base_1 + vsf_row));
          asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                       : "=r"(*reinterpret_cast<uint32_t*>(&vsf_words[4])),
                         "=r"(*reinterpret_cast<uint32_t*>(&vsf_words[(4) + 1])),
                         "=r"(*reinterpret_cast<uint32_t*>(&vsf_words[(4) + 2])),
                         "=r"(*reinterpret_cast<uint32_t*>(&vsf_words[(4) + 3]))
...
#pragma unroll
        for (int chunk_wave_1 = 0; chunk_wave_1 < 4; chunk_wave_1++) {
          ...
          asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                       : "=r"(*reinterpret_cast<uint32_t*>(&packed_words_1[0])),
                         "=r"(*reinterpret_cast<uint32_t*>(&packed_words_1[(0) + 1])),
                         "=r"(*reinterpret_cast<uint32_t*>(&packed_words_1[(0) + 2])),
                         "=r"(*reinterpret_cast<uint32_t*>(&packed_words_1[(0) + 3]))
...
#pragma unroll
          for (int in_pair_1 = 0; in_pair_1 < 2; in_pair_1++) {
            ...
            {
              encoded_1[0] = cake_fmha_qmul4_portable<5>(packed_words_1[packed_idx_1], sf_1);
            }
            {
              encoded_1[1] = cake_fmha_qmul4_portable<6>(packed_words_1[packed_idx_1], sf_1);
            }
            {
              encoded_1[2] = cake_fmha_qmul4_portable<5>(packed_words_1[packed_idx_1 + 1], sf_1);
            }
            {
              encoded_1[3] = cake_fmha_qmul4_portable<6>(packed_words_1[packed_idx_1 + 1], sf_1);
            }
        ...
        if (num_n_blocks_1 > 1) {
          ...
          {
            asm volatile("ld.shared.b32 %0, [%1];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&ksf_words_4[0]))
                         : "r"(sf_base_2 + dq_tok * 8));
            asm volatile("ld.shared.b32 %0, [%1];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&ksf_words_4[1]))
                         : "r"(sf_base_2 + dq_tok * 8 + 4));
          }
#pragma unroll
          for (int chunk_wave_2 = 0; chunk_wave_2 < 4; chunk_wave_2++) {
            ...
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&packed_words_2[0])),
                           "=r"(*reinterpret_cast<uint32_t*>(&packed_words_2[(0) + 1])),
                           "=r"(*reinterpret_cast<uint32_t*>(&packed_words_2[(0) + 2])),
                           "=r"(*reinterpret_cast<uint32_t*>(&packed_words_2[(0) + 3]))
...
#pragma unroll
            for (int in_pair_2 = 0; in_pair_2 < 2; in_pair_2++) {
              ...
              {
                encoded_2[0] = cake_fmha_qmul4_portable<5>(packed_words_2[packed_idx_2], sf_2);
              }
              {
                encoded_2[1] = cake_fmha_qmul4_portable<6>(packed_words_2[packed_idx_2], sf_2);
              }
              {
                encoded_2[2] = cake_fmha_qmul4_portable<5>(packed_words_2[packed_idx_2 + 1], sf_2);
              }
              {
                encoded_2[3] = cake_fmha_qmul4_portable<6>(packed_words_2[packed_idx_2 + 1], sf_2);
              }
          ...
          unsigned int vsf_words_8[8];
          {
            ...
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&vsf_words_8[0])),
                           "=r"(*reinterpret_cast<uint32_t*>(&vsf_words_8[(0) + 1])),
                           "=r"(*reinterpret_cast<uint32_t*>(&vsf_words_8[(0) + 2])),
                           "=r"(*reinterpret_cast<uint32_t*>(&vsf_words_8[(0) + 3]))
                         : "r"(sf_base_6 + vsf_row_1));
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&vsf_words_8[4])),
                           "=r"(*reinterpret_cast<uint32_t*>(&vsf_words_8[(4) + 1])),
                           "=r"(*reinterpret_cast<uint32_t*>(&vsf_words_8[(4) + 2])),
                           "=r"(*reinterpret_cast<uint32_t*>(&vsf_words_8[(4) + 3]))
...
#pragma unroll
          for (int chunk_wave_3 = 0; chunk_wave_3 < 4; chunk_wave_3++) {
            ...
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&packed_words_3[0])),
                           "=r"(*reinterpret_cast<uint32_t*>(&packed_words_3[(0) + 1])),
                           "=r"(*reinterpret_cast<uint32_t*>(&packed_words_3[(0) + 2])),
                           "=r"(*reinterpret_cast<uint32_t*>(&packed_words_3[(0) + 3]))
...
#pragma unroll
            for (int in_pair_3 = 0; in_pair_3 < 2; in_pair_3++) {
              ...
              {
                encoded_3[0] = cake_fmha_qmul4_portable<5>(packed_words_3[packed_idx_3], sf_3);
              }
              {
                encoded_3[1] = cake_fmha_qmul4_portable<6>(packed_words_3[packed_idx_3], sf_3);
              }
              {
                encoded_3[2] = cake_fmha_qmul4_portable<5>(packed_words_3[packed_idx_3 + 1], sf_3);
              }
              {
                encoded_3[3] = cake_fmha_qmul4_portable<6>(packed_words_3[packed_idx_3 + 1], sf_3);
              }
...
#pragma unroll
          ...
          if (n_iter_2 < num_n_blocks_1 - 1) {
            ...
            {
              asm volatile("ld.shared.b32 %0, [%1];"
                           : "=r"(*reinterpret_cast<uint32_t*>(&ksf_words_4_1[0]))
                           : "r"(sf_base_2_1 + dq_tok * 8));
              asm volatile("ld.shared.b32 %0, [%1];"
                           : "=r"(*reinterpret_cast<uint32_t*>(&ksf_words_4_1[1]))
                           : "r"(sf_base_2_1 + dq_tok * 8 + 4));
            }
#pragma unroll
            for (int chunk_wave_4 = 0; chunk_wave_4 < 4; chunk_wave_4++) {
              ...
              asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                           : "=r"(*reinterpret_cast<uint32_t*>(&packed_words_4[0])),
                             "=r"(*reinterpret_cast<uint32_t*>(&packed_words_4[(0) + 1])),
                             "=r"(*reinterpret_cast<uint32_t*>(&packed_words_4[(0) + 2])),
                             "=r"(*reinterpret_cast<uint32_t*>(&packed_words_4[(0) + 3]))
...
#pragma unroll
              for (int in_pair_4 = 0; in_pair_4 < 2; in_pair_4++) {
                ...
                {
                  encoded_4[0] = cake_fmha_qmul4_portable<5>(packed_words_4[packed_idx_4], sf_4);
                }
                {
                  encoded_4[1] = cake_fmha_qmul4_portable<6>(packed_words_4[packed_idx_4], sf_4);
                }
                {
                  encoded_4[2] =
                      cake_fmha_qmul4_portable<5>(packed_words_4[packed_idx_4 + 1], sf_4);
                }
                {
                  encoded_4[3] =
                      cake_fmha_qmul4_portable<6>(packed_words_4[packed_idx_4 + 1], sf_4);
                }
                ...
                asm volatile("st.shared.b32 [%0], %1;" ::"r"(out_group_4 + 12),
                             "r"((encoded_4[3])));
              }
            }
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
...
#pragma unroll
          ...
          if (n_iter_2 < num_n_blocks_1 - 1) {
            ...
            unsigned int vsf_words_4[8];
            {
              ...
              asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                           : "=r"(*reinterpret_cast<uint32_t*>(&vsf_words_4[0])),
                             "=r"(*reinterpret_cast<uint32_t*>(&vsf_words_4[(0) + 1])),
                             "=r"(*reinterpret_cast<uint32_t*>(&vsf_words_4[(0) + 2])),
                             "=r"(*reinterpret_cast<uint32_t*>(&vsf_words_4[(0) + 3]))
                           : "r"(sf_base_2_2 + vsf_row_2));
              asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                           : "=r"(*reinterpret_cast<uint32_t*>(&vsf_words_4[4])),
                             "=r"(*reinterpret_cast<uint32_t*>(&vsf_words_4[(4) + 1])),
                             "=r"(*reinterpret_cast<uint32_t*>(&vsf_words_4[(4) + 2])),
                             "=r"(*reinterpret_cast<uint32_t*>(&vsf_words_4[(4) + 3]))
...
#pragma unroll
            for (int chunk_wave_5 = 0; chunk_wave_5 < 4; chunk_wave_5++) {
              ...
              asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                           : "=r"(*reinterpret_cast<uint32_t*>(&packed_words_5[0])),
                             "=r"(*reinterpret_cast<uint32_t*>(&packed_words_5[(0) + 1])),
                             "=r"(*reinterpret_cast<uint32_t*>(&packed_words_5[(0) + 2])),
                             "=r"(*reinterpret_cast<uint32_t*>(&packed_words_5[(0) + 3]))
...
#pragma unroll
              for (int in_pair_5 = 0; in_pair_5 < 2; in_pair_5++) {
                ...
                {
                  encoded_5[0] = cake_fmha_qmul4_portable<5>(packed_words_5[packed_idx_5], sf_5);
                }
                {
                  encoded_5[1] = cake_fmha_qmul4_portable<6>(packed_words_5[packed_idx_5], sf_5);
                }
                {
                  encoded_5[2] =
                      cake_fmha_qmul4_portable<5>(packed_words_5[packed_idx_5 + 1], sf_5);
                }
                {
                  encoded_5[3] =
                      cake_fmha_qmul4_portable<6>(packed_words_5[packed_idx_5 + 1], sf_5);
                }
                ...
                asm volatile("st.shared.b32 [%0], %1;" ::"r"(out_group_5 + 12),
                             "r"((encoded_5[3])));
              }
            }
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
...
#pragma unroll 1
        for (unsigned int ni = 0; ni < num_n_blocks_3; ni++) {
          ...
          int pg_lane = lane;
          if (pg_lane < 8) {
            int _min_0 = (((base_tok + pg_lane * PAGE_SIZE) / PAGE_SIZE) < (last_slot)
                              ? ((base_tok + pg_lane * PAGE_SIZE) / PAGE_SIZE)
                              : (last_slot));
...
#pragma unroll
            for (int chunk = 0; chunk < 8; chunk++) {
              ...
              tma_5d_gmem2smem(smem_pk_addr + load_pk_stage * 9216 + (unsigned int)(chunk * 1024),
                               Kp, 0, w_k, 0, kv_head, pg_k, pk_full_addr + (load_pk_stage) * 8);
              tma_3d_gmem2smem(
                  smem_pk_addr + load_pk_stage * 9216 + (unsigned int)(8192 + chunk * 128), Ksf,
                  w_k * 8, kv_head, pg_k, pk_full_addr + (load_pk_stage) * 8);
            }
          }
          load_pk_stage += 1;
          if (load_pk_stage == 2) {
...
#pragma unroll
            for (int chunk_1 = 0; chunk_1 < 8; chunk_1++) {
              ...
              tma_5d_gmem2smem(smem_pk_addr + load_pk_stage * 9216 + (unsigned int)(chunk_1 * 1024),
                               Vp, 0, w_v, 0, kv_head, pg_v, pk_full_addr + (load_pk_stage) * 8);
              tma_3d_gmem2smem(
                  smem_pk_addr + load_pk_stage * 9216 + (unsigned int)(8192 + chunk_1 * 128), Vsf,
                  w_v * 8, kv_head, pg_v, pk_full_addr + (load_pk_stage) * 8);
            }
          }
          load_pk_stage += 1;
          if (load_pk_stage == 2) {
