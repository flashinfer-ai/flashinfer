...
__global__ __launch_bounds__(512) void kernel_cake_fmha_decode_native_fp16_hd512(
    CakeFmhaTensorMap const* Qt, CakeFmhaTensorMap const* K, CakeFmhaTensorMap const* V,
    __half* __restrict__ O_ptr, float* __restrict__ LSE_ptr, int* __restrict__ page_table,
    int* __restrict__ causal_seqlens_kv_global, float* __restrict__ scale_log2_ptr,
    int max_pages_per_seq, int max_local_seq_len, float softmax_scale_log2, int window_left,
    int num_q_heads, int num_kv_heads, int batch_size) {
...
#define tmem_dealloc_addr (mbar_base + 176)
  ...
  if (tid == 0) {
    asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" ::"l"((uint64_t)(Qt))
                 : "memory");
...
#pragma unroll 1
      for (unsigned int tile_idx_s = blockIdx.x; tile_idx_s < total_tiles_s;
        ...
        int win_start = 0;
        {
          int win_lo = global_q_pos - window_left;
          if (win_lo > 0) {
            win_start = win_lo;
          }
        }
...
#pragma unroll 1
        for (int pair = 0; pair < cta_n_blocks / 2; pair++) {
          ...
          {
            if (kv_pos0 < win_start) {
              sv[0] = -3.4028235e+38f;
              sv[1] = -3.4028235e+38f;
            }
            if (kv_pos1 < win_start) {
              sv[2] = -3.4028235e+38f;
              sv[3] = -3.4028235e+38f;
            }
            if (kv_pos2 < win_start) {
              sv[4] = -3.4028235e+38f;
              sv[5] = -3.4028235e+38f;
            }
            if (kv_pos3 < win_start) {
              sv[6] = -3.4028235e+38f;
              sv[7] = -3.4028235e+38f;
            }
          }
...
#pragma unroll
          for (int c_4 = 0; c_4 < 8; c_4++) {
            float safe_max =
                ((new_max_pair[c_4 % 2] == -CAKE_FMHA_INF) ? 0.0f : new_max_pair[c_4 % 2]);
...
#pragma unroll
          for (int _lp = 0; _lp < 4; _lp++) {
            __half2 _h2 =
                __float22half2_rn(make_float2(exp_vals[_lp * 2 + 0], exp_vals[_lp * 2 + 1 + 0]));
          ...
          asm volatile(
              "stmatrix.sync.aligned.m8n8.x4.trans.shared.b16 [%0], {%1, %2, %3, %4};\n" ::"r"(
                  _stmatrix_addr_5),
              "r"(*reinterpret_cast<const uint32_t*>(&regs_p[0])),
              "r"(*reinterpret_cast<const uint32_t*>(&regs_p[1])),
              "r"(*reinterpret_cast<const uint32_t*>(&regs_p[2])),
              "r"(*reinterpret_cast<const uint32_t*>(&regs_p[3]))
              : "memory");
          asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
          if (is_wg1 != 0) {
...
#pragma unroll
        ...
        if (wg_tid < 4) {
          my_corr_ptr[col_pair_base] = my_exch_ptr[col_pair_base] + my_exch_ptr[8 + col_pair_base] +
                                       my_exch_ptr[16 + col_pair_base] +
                                       my_exch_ptr[24 + col_pair_base];
          my_corr_ptr[col_pair_base + 1] =
              my_exch_ptr[col_pair_base + 1] + my_exch_ptr[8 + col_pair_base + 1] +
              my_exch_ptr[16 + col_pair_base + 1] + my_exch_ptr[24 + col_pair_base + 1];
...
#pragma unroll 1
      for (unsigned int tile_idx_c = blockIdx.x; tile_idx_c < total_tiles_c;
        ...
        int win_start_1 = 0;
        {
          int win_lo_1 = global_q_pos_1 - window_left;
          if (win_lo_1 > 0) {
            win_start_1 = win_lo_1;
          }
        }
...
#pragma unroll
            ...
            asm volatile(
                "tcgen05.st.sync.aligned.16x256b.x1.b32"
                " [%0], {%1, %2, %3, %4};" ::"r"(taddr + 64 + 8 + 1048576),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_hi[0])),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_hi[1])),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_hi[2])),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_hi[3])));
...
#pragma unroll
            ...
            asm volatile(
                "tcgen05.st.sync.aligned.16x256b.x1.b32"
                " [%0], {%1, %2, %3, %4};" ::"r"(taddr + 64 + 16 + 1048576),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_hi[0])),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_hi[1])),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_hi[2])),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_hi[3])));
...
#pragma unroll
            ...
            asm volatile(
                "tcgen05.st.sync.aligned.16x256b.x1.b32"
                " [%0], {%1, %2, %3, %4};" ::"r"(taddr + 64 + 24 + 1048576),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_hi[0])),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_hi[1])),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_hi[2])),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_hi[3])));
...
#pragma unroll
            ...
            asm volatile(
                "tcgen05.st.sync.aligned.16x256b.x1.b32"
                " [%0], {%1, %2, %3, %4};" ::"r"(taddr + 96 + 8 + 1048576),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_hi[0])),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_hi[1])),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_hi[2])),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_hi[3])));
...
#pragma unroll
            ...
            asm volatile(
                "tcgen05.st.sync.aligned.16x256b.x1.b32"
                " [%0], {%1, %2, %3, %4};" ::"r"(taddr + 96 + 16 + 1048576),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_hi[0])),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_hi[1])),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_hi[2])),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_hi[3])));
...
#pragma unroll
            ...
            asm volatile(
                "tcgen05.st.sync.aligned.16x256b.x1.b32"
                " [%0], {%1, %2, %3, %4};" ::"r"(taddr + 96 + 24 + 1048576),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_hi[0])),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_hi[1])),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_hi[2])),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_hi[3])));
...
#pragma unroll
        for (int h_24 = 0; h_24 < 8; h_24++) {
          ...
          if (group_ratio_rt > h_24) {
            ...
            int o_idx =
                ((batch_idx_1 * Q_LEN + q_row_idx_1) * NUM_Q_HEADS + q_head) * HEAD_DIM + d_idx;
...
#pragma unroll
        for (int h_25 = 0; h_25 < 8; h_25++) {
          ...
          if (group_ratio_rt > h_25) {
            ...
            int o_idx_1 =
                ((batch_idx_1 * Q_LEN + q_row_idx_1) * NUM_Q_HEADS + q_head_1) * HEAD_DIM + 128 +
                d_idx;
...
#pragma unroll
        for (int h_26 = 0; h_26 < 8; h_26++) {
          ...
          if (group_ratio_rt > h_26) {
            ...
            int o_idx_2 =
                ((batch_idx_1 * Q_LEN + q_row_idx_1) * NUM_Q_HEADS + q_head_2) * HEAD_DIM + 256 +
                d_idx;
...
#pragma unroll
        for (int h_27 = 0; h_27 < 8; h_27++) {
          ...
          if (group_ratio_rt > h_27) {
            ...
            int o_idx_3 =
                ((batch_idx_1 * Q_LEN + q_row_idx_1) * NUM_Q_HEADS + q_head_3) * HEAD_DIM + 384 +
                d_idx;
...
#pragma unroll 1
      for (unsigned int tile_idx_m = blockIdx.x; tile_idx_m < total_tiles_m;
        ...
        int win_start_2 = 0;
        {
          int win_lo_2 = global_q_pos_2 - window_left;
          if (win_lo_2 > 0) {
            win_start_2 = win_lo_2;
          }
        }
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_2),
            "r"(_mma_b_lo_2), "r"(tmem_tmem_s0), "r"(1));
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_3),
            "r"(_mma_b_lo_3), "r"(tmem_tmem_s0), "r"(1));
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_6),
            "r"(_mma_b_lo_6), "r"(tmem_tmem_s1), "r"(1));
        elect_commit(kv_empty_addr + 16);
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_7),
            "r"(_mma_b_lo_7), "r"(tmem_tmem_s1), "r"(1));
        elect_commit(kv_empty_addr + 24);
...
#pragma unroll 1
        for (int pair_2 = 0; pair_2 < cta_n_blocks_2 / 2 - 1; pair_2++) {
          ...
          asm volatile("tcgen05.fence::after_thread_sync;");
          int _mma_a_lo_8 =
              make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (0) * 2048);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_8),
              "r"(_mma_b_lo_8), "r"(tmem_tmem_o0_c0), "r"(((first_pv0) ? 0 : 1)));
          elect_commit(kv_empty_addr);
          mbarrier_wait(kv_full_addr + 8, 0);
          int _mma_a_lo_9 =
              make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (1) * 2048);
          asm volatile(
              "{\n\t"
              ".reg .pred leader, p0, p1;\n\t"
              ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_9),
              "r"(_mma_b_lo_8), "r"(tmem_tmem_o0_c1), "r"(((first_pv0) ? 0 : 1)));
          {
            elect_commit(kv_empty_addr + 8);
          }
          mbarrier_wait(kv_full_addr + 16, 0);
          int _mma_a_lo_10 =
              make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (2) * 2048);
          asm volatile(
              "{\n\t"
              ".reg .pred leader, p0, p1;\n\t"
              ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_10),
              "r"(_mma_b_lo_8), "r"(tmem_tmem_o0_c2), "r"(((first_pv0) ? 0 : 1)));
          {
            elect_commit(kv_empty_addr + 16);
          }
          mbarrier_wait(kv_full_addr + 24, 0);
          int _mma_a_lo_11 =
              make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (3) * 2048);
          asm volatile(
              "{\n\t"
              ".reg .pred leader, p0, p1;\n\t"
              ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_11),
              "r"(_mma_b_lo_8), "r"(tmem_tmem_o0_c3), "r"(((first_pv0) ? 0 : 1)));
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_12),
              "r"(_mma_b_lo_12), "r"(tmem_tmem_s0), "r"(0));
          elect_commit(kv_empty_addr);
          mbarrier_wait(kv_full_addr + 8, 1);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_13),
              "r"(_mma_b_lo_13), "r"(tmem_tmem_s0), "r"(1));
          elect_commit(kv_empty_addr + 8);
          mbarrier_wait(kv_full_addr + 16, 1);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_14),
              "r"(_mma_b_lo_14), "r"(tmem_tmem_s0), "r"(1));
          elect_commit(kv_empty_addr + 16);
          mbarrier_wait(kv_full_addr + 24, 1);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_15),
              "r"(_mma_b_lo_15), "r"(tmem_tmem_s0), "r"(1));
          elect_commit(kv_empty_addr + 24);
          elect_commit(s_full_0_addr);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_16),
              "r"(_mma_b_lo_16), "r"(tmem_tmem_o1_c0), "r"(((first_pv1) ? 0 : 1)));
          elect_commit(kv_empty_addr);
          mbarrier_wait(kv_full_addr + 8, 0);
          int _mma_a_lo_17 =
              make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (1) * 2048);
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_17),
              "r"(_mma_b_lo_16), "r"(tmem_tmem_o1_c1), "r"(((first_pv1) ? 0 : 1)));
          {
            elect_commit(kv_empty_addr + 8);
          }
          mbarrier_wait(kv_full_addr + 16, 0);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_18),
              "r"(_mma_b_lo_16), "r"(tmem_tmem_o1_c2), "r"(((first_pv1) ? 0 : 1)));
          {
            elect_commit(kv_empty_addr + 16);
          }
          mbarrier_wait(kv_full_addr + 24, 0);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_19),
              "r"(_mma_b_lo_16), "r"(tmem_tmem_o1_c3), "r"(((first_pv1) ? 0 : 1)));
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_20),
              "r"(_mma_b_lo_20), "r"(tmem_tmem_s1), "r"(0));
          elect_commit(kv_empty_addr);
          mbarrier_wait(kv_full_addr + 8, 1);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_21),
              "r"(_mma_b_lo_21), "r"(tmem_tmem_s1), "r"(1));
          elect_commit(kv_empty_addr + 8);
          mbarrier_wait(kv_full_addr + 16, 1);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_22),
              "r"(_mma_b_lo_22), "r"(tmem_tmem_s1), "r"(1));
          elect_commit(kv_empty_addr + 16);
          mbarrier_wait(kv_full_addr + 24, 1);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_23),
              "r"(_mma_b_lo_23), "r"(tmem_tmem_s1), "r"(1));
          elect_commit(kv_empty_addr + 24);
          elect_commit(s_full_1_addr);
          first_pv0 = 0;
          first_pv1 = 0;
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_24),
            "r"(_mma_b_lo_24), "r"(tmem_tmem_o0_c0), "r"(((first_pv0) ? 0 : 1)));
        elect_commit(kv_empty_addr);
        mbarrier_wait(kv_full_addr + 8, 0);
        int _mma_a_lo_25 =
            make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (1) * 2048);
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_25),
            "r"(_mma_b_lo_24), "r"(tmem_tmem_o0_c1), "r"(((first_pv0) ? 0 : 1)));
        {
          elect_commit(kv_empty_addr + 8);
        }
        mbarrier_wait(kv_full_addr + 16, 0);
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_26),
            "r"(_mma_b_lo_24), "r"(tmem_tmem_o0_c2), "r"(((first_pv0) ? 0 : 1)));
        {
          elect_commit(kv_empty_addr + 16);
        }
        mbarrier_wait(kv_full_addr + 24, 0);
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_27),
            "r"(_mma_b_lo_24), "r"(tmem_tmem_o0_c3), "r"(((first_pv0) ? 0 : 1)));
        elect_commit2(kv_empty_addr + 24, o_ready_0_addr);
        mbarrier_wait(o_empty_1_addr, _phase_o_empty_1_0);
        _phase_o_empty_1_0 ^= 1;
        mbarrier_wait(kv_full_addr, 1);
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_28),
            "r"(_mma_b_lo_28), "r"(tmem_tmem_o1_c0), "r"(((first_pv1) ? 0 : 1)));
        elect_commit(kv_empty_addr);
        mbarrier_wait(kv_full_addr + 8, 1);
        int _mma_a_lo_29 =
            make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (1) * 2048);
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_29),
            "r"(_mma_b_lo_28), "r"(tmem_tmem_o1_c1), "r"(((first_pv1) ? 0 : 1)));
        {
          elect_commit(kv_empty_addr + 8);
        }
        mbarrier_wait(kv_full_addr + 16, 1);
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_30),
            "r"(_mma_b_lo_28), "r"(tmem_tmem_o1_c2), "r"(((first_pv1) ? 0 : 1)));
        {
          elect_commit(kv_empty_addr + 16);
        }
        mbarrier_wait(kv_full_addr + 24, 1);
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_31),
            "r"(_mma_b_lo_28), "r"(tmem_tmem_o1_c3), "r"(((first_pv1) ? 0 : 1)));
        elect_commit2(kv_empty_addr + 24, o_ready_1_addr);
      }
...
#pragma unroll 1
      for (unsigned int tile_idx_la = blockIdx.x; tile_idx_la < total_tiles_la;
           tile_idx_la += gridDim.x) {
        ...
        int win_start_3 = 0;
        {
          int win_lo_3 = global_q_pos_3 - window_left;
          if (win_lo_3 > 0) {
            win_start_3 = win_lo_3;
          }
        }
          ...
          for (int ni_la = 0; ni_la < prefill_la; ni_la++) {
            ...
            mbarrier_arrive_expect_tx(kv_full_addr, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr),
                  "l"(K), "r"(0), "r"(0), "r"(0), "r"(kv_head_idx_3), "r"(pg_k_la),
                  "r"(kv_full_addr), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  "cp.async.bulk.tensor.5d.shared::cta.global.mbarrier::complete_tx::bytes.L2::"
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 16384),
                  "l"(K), "r"(0), "r"(0), "r"(1), "r"(kv_head_idx_3), "r"(pg_k_la),
                  "r"(kv_full_addr), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 8, 16384);
            {
              asm volatile(
                  "cp.async.bulk.tensor.5d.shared::cta.global.mbarrier::complete_tx::bytes.L2::"
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 32768),
                  "l"(K), "r"(0), "r"(0), "r"(2), "r"(kv_head_idx_3), "r"(pg_k_la),
                  "r"(kv_full_addr + 8), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 32768 +
                                                                       16384),
                  "l"(K), "r"(0), "r"(0), "r"(3), "r"(kv_head_idx_3), "r"(pg_k_la),
                  "r"(kv_full_addr + 8), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 16, 16384);
            {
              asm volatile(
                  "cp.async.bulk.tensor.5d.shared::cta.global.mbarrier::complete_tx::bytes.L2::"
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 65536),
                  "l"(K), "r"(0), "r"(0), "r"(4), "r"(kv_head_idx_3), "r"(pg_k_la),
                  "r"(kv_full_addr + 16), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 65536 +
                                                                       16384),
                  "l"(K), "r"(0), "r"(0), "r"(5), "r"(kv_head_idx_3), "r"(pg_k_la),
                  "r"(kv_full_addr + 16), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 24, 16384);
            {
              asm volatile(
                  "cp.async.bulk.tensor.5d.shared::cta.global.mbarrier::complete_tx::bytes.L2::"
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 98304),
                  "l"(K), "r"(0), "r"(0), "r"(6), "r"(kv_head_idx_3), "r"(pg_k_la),
                  "r"(kv_full_addr + 24), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 98304 +
                                                                       16384),
                  "l"(K), "r"(0), "r"(0), "r"(7), "r"(kv_head_idx_3), "r"(pg_k_la),
                  "r"(kv_full_addr + 24), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
          }
          ...
          for (int ni_la_1 = 0; ni_la_1 < main_ni_la; ni_la_1++) {
            ...
            mbarrier_arrive_expect_tx(kv_full_addr, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr),
                  "l"(V), "r"(0), "r"(0), "r"(0), "r"(kv_head_idx_3), "r"(pg_v_la),
                  "r"(kv_full_addr), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 16384),
                  "l"(V), "r"(0), "r"(0), "r"(1), "r"(kv_head_idx_3), "r"(pg_v_la),
                  "r"(kv_full_addr), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 8, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 32768),
                  "l"(V), "r"(0), "r"(0), "r"(2), "r"(kv_head_idx_3), "r"(pg_v_la),
                  "r"(kv_full_addr + 8), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 32768 +
                                                                       16384),
                  "l"(V), "r"(0), "r"(0), "r"(3), "r"(kv_head_idx_3), "r"(pg_v_la),
                  "r"(kv_full_addr + 8), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 16, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 65536),
                  "l"(V), "r"(0), "r"(0), "r"(4), "r"(kv_head_idx_3), "r"(pg_v_la),
                  "r"(kv_full_addr + 16), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 65536 +
                                                                       16384),
                  "l"(V), "r"(0), "r"(0), "r"(5), "r"(kv_head_idx_3), "r"(pg_v_la),
                  "r"(kv_full_addr + 16), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 24, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 98304),
                  "l"(V), "r"(0), "r"(0), "r"(6), "r"(kv_head_idx_3), "r"(pg_v_la),
                  "r"(kv_full_addr + 24), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 98304 +
                                                                       16384),
                  "l"(V), "r"(0), "r"(0), "r"(7), "r"(kv_head_idx_3), "r"(pg_v_la),
                  "r"(kv_full_addr + 24), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr),
                  "l"(K), "r"(0), "r"(0), "r"(0), "r"(kv_head_idx_3), "r"(pg_nk_la),
                  "r"(kv_full_addr), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 16384),
                  "l"(K), "r"(0), "r"(0), "r"(1), "r"(kv_head_idx_3), "r"(pg_nk_la),
                  "r"(kv_full_addr), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 8, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 32768),
                  "l"(K), "r"(0), "r"(0), "r"(2), "r"(kv_head_idx_3), "r"(pg_nk_la),
                  "r"(kv_full_addr + 8), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 32768 +
                                                                       16384),
                  "l"(K), "r"(0), "r"(0), "r"(3), "r"(kv_head_idx_3), "r"(pg_nk_la),
                  "r"(kv_full_addr + 8), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 16, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 65536),
                  "l"(K), "r"(0), "r"(0), "r"(4), "r"(kv_head_idx_3), "r"(pg_nk_la),
                  "r"(kv_full_addr + 16), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 65536 +
                                                                       16384),
                  "l"(K), "r"(0), "r"(0), "r"(5), "r"(kv_head_idx_3), "r"(pg_nk_la),
                  "r"(kv_full_addr + 16), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 24, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 98304),
                  "l"(K), "r"(0), "r"(0), "r"(6), "r"(kv_head_idx_3), "r"(pg_nk_la),
                  "r"(kv_full_addr + 24), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 98304 +
                                                                       16384),
                  "l"(K), "r"(0), "r"(0), "r"(7), "r"(kv_head_idx_3), "r"(pg_nk_la),
                  "r"(kv_full_addr + 24), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
          }
          ...
          for (int tail_ni_la = main_ni_la; tail_ni_la < cta_n_blocks_3; tail_ni_la++) {
            ...
            mbarrier_arrive_expect_tx(kv_full_addr, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr),
                  "l"(V), "r"(0), "r"(0), "r"(0), "r"(kv_head_idx_3), "r"(tpg_v_la),
                  "r"(kv_full_addr), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 16384),
                  "l"(V), "r"(0), "r"(0), "r"(1), "r"(kv_head_idx_3), "r"(tpg_v_la),
                  "r"(kv_full_addr), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 8, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 32768),
                  "l"(V), "r"(0), "r"(0), "r"(2), "r"(kv_head_idx_3), "r"(tpg_v_la),
                  "r"(kv_full_addr + 8), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 32768 +
                                                                       16384),
                  "l"(V), "r"(0), "r"(0), "r"(3), "r"(kv_head_idx_3), "r"(tpg_v_la),
                  "r"(kv_full_addr + 8), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 16, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 65536),
                  "l"(V), "r"(0), "r"(0), "r"(4), "r"(kv_head_idx_3), "r"(tpg_v_la),
                  "r"(kv_full_addr + 16), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 65536 +
                                                                       16384),
                  "l"(V), "r"(0), "r"(0), "r"(5), "r"(kv_head_idx_3), "r"(tpg_v_la),
                  "r"(kv_full_addr + 16), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 24, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 98304),
                  "l"(V), "r"(0), "r"(0), "r"(6), "r"(kv_head_idx_3), "r"(tpg_v_la),
                  "r"(kv_full_addr + 24), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 98304 +
                                                                       16384),
                  "l"(V), "r"(0), "r"(0), "r"(7), "r"(kv_head_idx_3), "r"(tpg_v_la),
                  "r"(kv_full_addr + 24), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
          }
...
#pragma unroll 1
      for (unsigned int tile_idx_l = blockIdx.x; tile_idx_l < total_tiles_l;
        ...
        int win_start_4 = 0;
        {
          int win_lo_4 = global_q_pos_4 - window_left;
          if (win_lo_4 > 0) {
            win_start_4 = win_lo_4;
          }
        }
        ...
        if (elect_sync()) {
          ...
          int off_qt =
              (batch_idx_4 * Q_LEN + q_row_idx_4) * NUM_Q_HEADS + kv_head_idx_4 * group_ratio_l;
          ...
          for (int ni = 0; ni < prefill; ni++) {
            ...
            mbarrier_arrive_expect_tx(kv_full_addr, 16384);
            {
              asm volatile(
                  "cp.async.bulk.tensor.5d.shared::cta.global.mbarrier::complete_tx::bytes.L2::"
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 8192),
                  "l"(K), "r"(0), "r"(0), "r"(0), "r"(kv_head_idx_4), "r"(pg_k), "r"(kv_full_addr),
            ...
            }
            {
              asm volatile(
                  "cp.async.bulk.tensor.5d.shared::cta.global.mbarrier::complete_tx::bytes.L2::"
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 16384 + 8192),
                  "l"(K), "r"(0), "r"(0), "r"(1), "r"(kv_head_idx_4), "r"(pg_k), "r"(kv_full_addr),
                  "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 8, 16384);
            {
              asm volatile(
                  "cp.async.bulk.tensor.5d.shared::cta.global.mbarrier::complete_tx::bytes.L2::"
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 32768 + 8192),
                  "l"(K), "r"(0), "r"(0), "r"(2), "r"(kv_head_idx_4), "r"(pg_k),
                  "r"(kv_full_addr + 8), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 32768 +
                                                                       16384 + 8192),
                  "l"(K), "r"(0), "r"(0), "r"(3), "r"(kv_head_idx_4), "r"(pg_k),
                  "r"(kv_full_addr + 8), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 16, 16384);
            {
              asm volatile(
                  "cp.async.bulk.tensor.5d.shared::cta.global.mbarrier::complete_tx::bytes.L2::"
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 65536 + 8192),
                  "l"(K), "r"(0), "r"(0), "r"(4), "r"(kv_head_idx_4), "r"(pg_k),
                  "r"(kv_full_addr + 16), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 65536 +
                                                                       16384 + 8192),
                  "l"(K), "r"(0), "r"(0), "r"(5), "r"(kv_head_idx_4), "r"(pg_k),
                  "r"(kv_full_addr + 16), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 24, 16384);
            {
              asm volatile(
                  "cp.async.bulk.tensor.5d.shared::cta.global.mbarrier::complete_tx::bytes.L2::"
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 98304 + 8192),
                  "l"(K), "r"(0), "r"(0), "r"(6), "r"(kv_head_idx_4), "r"(pg_k),
                  "r"(kv_full_addr + 24), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 98304 +
                                                                       16384 + 8192),
                  "l"(K), "r"(0), "r"(0), "r"(7), "r"(kv_head_idx_4), "r"(pg_k),
                  "r"(kv_full_addr + 24), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
          }
          ...
          for (int ni_1 = 0; ni_1 < main_ni; ni_1++) {
            ...
            mbarrier_arrive_expect_tx(kv_full_addr, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 8192),
                  "l"(V), "r"(0), "r"(0), "r"(0), "r"(kv_head_idx_4), "r"(pg_v), "r"(kv_full_addr),
                  "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 16384 + 8192),
                  "l"(V), "r"(0), "r"(0), "r"(1), "r"(kv_head_idx_4), "r"(pg_v), "r"(kv_full_addr),
                  "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            mbarrier_wait(kv_empty_addr + 8, 1);
            mbarrier_arrive_expect_tx(kv_full_addr + 8, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 32768 + 8192),
                  "l"(V), "r"(0), "r"(0), "r"(2), "r"(kv_head_idx_4), "r"(pg_v),
                  "r"(kv_full_addr + 8), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 32768 +
                                                                       16384 + 8192),
                  "l"(V), "r"(0), "r"(0), "r"(3), "r"(kv_head_idx_4), "r"(pg_v),
                  "r"(kv_full_addr + 8), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            mbarrier_wait(kv_empty_addr + 16, 1);
            mbarrier_arrive_expect_tx(kv_full_addr + 16, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 65536 + 8192),
                  "l"(V), "r"(0), "r"(0), "r"(4), "r"(kv_head_idx_4), "r"(pg_v),
                  "r"(kv_full_addr + 16), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 65536 +
                                                                       16384 + 8192),
                  "l"(V), "r"(0), "r"(0), "r"(5), "r"(kv_head_idx_4), "r"(pg_v),
                  "r"(kv_full_addr + 16), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            mbarrier_wait(kv_empty_addr + 24, 1);
            mbarrier_arrive_expect_tx(kv_full_addr + 24, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 98304 + 8192),
                  "l"(V), "r"(0), "r"(0), "r"(6), "r"(kv_head_idx_4), "r"(pg_v),
                  "r"(kv_full_addr + 24), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 98304 +
                                                                       16384 + 8192),
                  "l"(V), "r"(0), "r"(0), "r"(7), "r"(kv_head_idx_4), "r"(pg_v),
                  "r"(kv_full_addr + 24), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            int next_ni = ni_1 + 2;
            ...
            mbarrier_arrive_expect_tx(kv_full_addr, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 8192),
                  "l"(K), "r"(0), "r"(0), "r"(0), "r"(kv_head_idx_4), "r"(pg_nk), "r"(kv_full_addr),
                  "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 16384 + 8192),
                  "l"(K), "r"(0), "r"(0), "r"(1), "r"(kv_head_idx_4), "r"(pg_nk), "r"(kv_full_addr),
                  "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            mbarrier_wait(kv_empty_addr + 8, 0);
            mbarrier_arrive_expect_tx(kv_full_addr + 8, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 32768 + 8192),
                  "l"(K), "r"(0), "r"(0), "r"(2), "r"(kv_head_idx_4), "r"(pg_nk),
                  "r"(kv_full_addr + 8), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 32768 +
                                                                       16384 + 8192),
                  "l"(K), "r"(0), "r"(0), "r"(3), "r"(kv_head_idx_4), "r"(pg_nk),
                  "r"(kv_full_addr + 8), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            mbarrier_wait(kv_empty_addr + 16, 0);
            mbarrier_arrive_expect_tx(kv_full_addr + 16, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 65536 + 8192),
                  "l"(K), "r"(0), "r"(0), "r"(4), "r"(kv_head_idx_4), "r"(pg_nk),
                  "r"(kv_full_addr + 16), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 65536 +
                                                                       16384 + 8192),
                  "l"(K), "r"(0), "r"(0), "r"(5), "r"(kv_head_idx_4), "r"(pg_nk),
                  "r"(kv_full_addr + 16), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            mbarrier_wait(kv_empty_addr + 24, 0);
            mbarrier_arrive_expect_tx(kv_full_addr + 24, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 98304 + 8192),
                  "l"(K), "r"(0), "r"(0), "r"(6), "r"(kv_head_idx_4), "r"(pg_nk),
                  "r"(kv_full_addr + 24), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 98304 +
                                                                       16384 + 8192),
                  "l"(K), "r"(0), "r"(0), "r"(7), "r"(kv_head_idx_4), "r"(pg_nk),
                  "r"(kv_full_addr + 24), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
          }
          ...
          for (int tail_ni = main_ni; tail_ni < cta_n_blocks_4; tail_ni++) {
            ...
            mbarrier_arrive_expect_tx(kv_full_addr, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 8192),
                  "l"(V), "r"(0), "r"(0), "r"(0), "r"(kv_head_idx_4), "r"(tpg_v), "r"(kv_full_addr),
                  "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 16384 + 8192),
                  "l"(V), "r"(0), "r"(0), "r"(1), "r"(kv_head_idx_4), "r"(tpg_v), "r"(kv_full_addr),
                  "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 8, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 32768 + 8192),
                  "l"(V), "r"(0), "r"(0), "r"(2), "r"(kv_head_idx_4), "r"(tpg_v),
                  "r"(kv_full_addr + 8), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 32768 +
                                                                       16384 + 8192),
                  "l"(V), "r"(0), "r"(0), "r"(3), "r"(kv_head_idx_4), "r"(tpg_v),
                  "r"(kv_full_addr + 8), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 16, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 65536 + 8192),
                  "l"(V), "r"(0), "r"(0), "r"(4), "r"(kv_head_idx_4), "r"(tpg_v),
                  "r"(kv_full_addr + 16), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 65536 +
                                                                       16384 + 8192),
                  "l"(V), "r"(0), "r"(0), "r"(5), "r"(kv_head_idx_4), "r"(tpg_v),
                  "r"(kv_full_addr + 16), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            ...
            mbarrier_arrive_expect_tx(kv_full_addr + 24, 16384);
            {
              asm volatile(
                  ...
                  "cache_hint"
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 98304 + 8192),
                  "l"(V), "r"(0), "r"(0), "r"(6), "r"(kv_head_idx_4), "r"(tpg_v),
                  "r"(kv_full_addr + 24), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
            {
              asm volatile(
                  ...
                  " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;" ::"r"(smem_kv_addr + 98304 +
                                                                       16384 + 8192),
                  "l"(V), "r"(0), "r"(0), "r"(7), "r"(kv_head_idx_4), "r"(tpg_v),
                  "r"(kv_full_addr + 24), "l"(0x14F0000000000000ULL)
                  : "memory");
            }
          }
