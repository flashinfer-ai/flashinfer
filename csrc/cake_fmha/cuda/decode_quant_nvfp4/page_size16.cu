...
__device__ __forceinline__ uint32_t cake_fmha_qmul4_portable(uint32_t src, uint32_t scale) {
  static_assert(kVariant >= 0 && kVariant <= 6, "invalid Cake FMHA QMUL4 variant");
  uint16_t a01;
  uint16_t a23;
  if constexpr (kVariant <= 4) {
    a01 = static_cast<uint16_t>((src & 0x0000000Fu) | ((src >> 4) & 0x000000F0u));
    a23 = static_cast<uint16_t>(((src >> 16) & 0x0000000Fu) | ((src >> 20) & 0x000000F0u));
  ...
  if constexpr (kVariant == 0) {
    ...
    constexpr int kByteIndex = kVariant == 1   ? 0
                               : kVariant == 2 ? 1
                               : kVariant == 3 ? 2
                               : kVariant == 4 ? 3
                                               : 0;
    const uint16_t scale_byte = static_cast<uint16_t>((scale >> (kByteIndex * 8)) & 0xFFu);
...
__global__ __launch_bounds__(512, 1) void kernel_cake_fmha_decode_quant_nvfp4(
    CakeFmhaTensorMap const* Qt, CakeFmhaTensorMap const* Kp, CakeFmhaTensorMap const* Vp,
    CakeFmhaTensorMap const* Ksf, CakeFmhaTensorMap const* Vsf, uint8_t* __restrict__ O,
    int* __restrict__ page_table, int* __restrict__ seq_lens_kv, float* __restrict__ bmm1_scale_ptr,
    float* __restrict__ bmm2_scale_ptr, float* __restrict__ partial_O,
    float* __restrict__ partial_max, float* __restrict__ partial_sum, int pt_batch_stride,
...
#define tmem_dealloc_addr (mbar_base + 312)
  ...
  if (tid == 0) {
    asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" ::"l"((uint64_t)(Qt))
                 : "memory");
    asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" ::"l"((uint64_t)(Kp))
                 : "memory");
    asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" ::"l"((uint64_t)(Vp))
                 : "memory");
    asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" ::"l"((uint64_t)(Ksf))
                 : "memory");
    asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" ::"l"((uint64_t)(Vsf))
                 : "memory");
  }
  __syncthreads();
...
#pragma unroll
          for (int inst_s = 0; inst_s < 2; inst_s++) {
            ...
            int my_tmem_stats = taddr + (unsigned int)(((is_inst1 != 0) ? 48 : 16)) +
                                (unsigned int)(tmem_row_base_v << 16);
...
#pragma unroll
            ...
            unsigned int regs_p_s[2];
            {
              ...
              asm volatile(
                  ...
                  "}"
                  : "=r"(_packed)
                  : "f"(exp_vals_s[0]), "f"(exp_vals_s[1]), "f"(exp_vals_s[2]), "f"(exp_vals_s[3]));
              regs_p_s[0] = _packed;
            }
            {
              uint32_t _packed;
              asm volatile(
                  ...
                  "}"
                  : "=r"(_packed)
                  : "f"(exp_vals_s[4]), "f"(exp_vals_s[5]), "f"(exp_vals_s[6]), "f"(exp_vals_s[7]));
              regs_p_s[1] = _packed;
            }
            ...
            const void* _stmatrix_b8_ptr_5 = reinterpret_cast<const void*>(
                reinterpret_cast<uint8_t*>(my_p_base) + stsm_offset_s);
...
#pragma unroll
        for (int inst_s_1 = 0; inst_s_1 < 2; inst_s_1++) {
          ...
          int my_tmem_stats_1 = taddr + (unsigned int)(((is_inst1_1 != 0) ? 48 : 16)) +
                                (unsigned int)(tmem_row_base_v << 16);
          if (is_inst1_1 != 0) {
            mbarrier_wait(corr_empty_1_addr, _phase_corr_empty_1_0);
            _phase_corr_empty_1_0 ^= 1;
...
#pragma unroll
        ...
        if (num_splits > 1) {
          ...
          if (q_even < group_ratio_rt) {
            *(reinterpret_cast<float*>(partial_O + (po_even + (corr_row_e ^ 80))) + (0)) =
                part_vals[0];
            *(reinterpret_cast<float*>(partial_O + (po_even + (corr_row_e + 8 ^ 80))) + (0)) =
                part_vals[2];
            *(reinterpret_cast<float*>(partial_O + (po_even + (corr_row_e + 16 ^ 80))) + (0)) =
                part_vals[4];
            *(reinterpret_cast<float*>(partial_O + (po_even + (corr_row_e + 24 ^ 80))) + (0)) =
                part_vals[6];
          }
          if (q_odd < group_ratio_rt) {
            *(reinterpret_cast<float*>(partial_O + (po_odd + (corr_row_e ^ corr_x_odd))) + (0)) =
                part_vals[1];
            *(reinterpret_cast<float*>(partial_O + (po_odd + (corr_row_e + 8 ^ corr_x_odd))) +
              (0)) = part_vals[3];
            *(reinterpret_cast<float*>(partial_O + (po_odd + (corr_row_e + 16 ^ corr_x_odd))) +
              (0)) = part_vals[5];
            *(reinterpret_cast<float*>(partial_O + (po_odd + (corr_row_e + 24 ^ corr_x_odd))) +
              (0)) = part_vals[7];
          }
          if (corr_tid < 4) {
          ...
          const void* _stmatrix_b8_ptr_0 =
              reinterpret_cast<const void*>(reinterpret_cast<uint8_t*>(smem_o) + o_stsm_offset);
          uint64_t _stmatrix_b8_addr64_0;
          asm volatile("cvta.to.shared.u64 %0, %1;"
                       : "=l"(_stmatrix_b8_addr64_0)
                       : "l"(_stmatrix_b8_ptr_0));
          uint32_t _stmatrix_b8_addr_0;
          asm volatile("cvt.u32.u64 %0, %1;"
                       : "=r"(_stmatrix_b8_addr_0)
                       : "l"(_stmatrix_b8_addr64_0));
          asm volatile("stmatrix.sync.aligned.m16n8.x2.trans.shared.b8 [%0], {%1, %2};\n" ::"r"(
                           _stmatrix_b8_addr_0),
                       "r"(regs_o[0]), "r"(regs_o[1])
                       : "memory");
          asm volatile("barrier.sync 10, 128;" ::: "memory");
          ...
          if (copy_row < group_ratio_rt) {
            ...
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&copy_vec[0])),
                           "=r"(*reinterpret_cast<uint32_t*>(&copy_vec[(0) + 1])),
                           "=r"(*reinterpret_cast<uint32_t*>(&copy_vec[(0) + 2])),
                           "=r"(*reinterpret_cast<uint32_t*>(&copy_vec[(0) + 3]))
                         : "r"(smem_o_addr + (unsigned int)copy_smem_offset));
            int o_base = (batch_idx * NUM_Q_HEADS + kv_head_idx * group_ratio_rt) * HEAD_DIM;
            reinterpret_cast<int4*>(O + (o_base + copy_row * HEAD_DIM + copy_col))[0] =
                reinterpret_cast<int4*>(copy_vec)[0];
          }
          if (elect_sync()) {
            mbarrier_arrive(stats_empty_addr);
          }
            ...
            if (issue_is_v_m == 0) {
              ...
              if (issue_blk_m % 2 == 0) {
                ...
                asm volatile(
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%0], [ta], db, id, p1;\n\t"
                    "}\n" ::"r"(tmem_tmem_s0),
                    "r"(_mma_b_lo_0), "r"(tmem_tmem_kv + issue_stage_m * 32), "r"(0));
                ...
                asm volatile(
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%0], [ta], db, id, p1;\n\t"
                    "}\n" ::"r"(tmem_tmem_s1),
                    "r"(_mma_b_lo_1), "r"(tmem_tmem_kv + issue_stage_m * 32), "r"(0));
                elect_commit(kv_empty_addr + (issue_stage_m) * 8);
              ...
              asm volatile(
                  ...
                  "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%0], [ta], db, id, p1;\n\t"
                  "}\n" ::"r"(tmem_tmem_o0),
                  "r"(_mma_b_lo_2), "r"(tmem_tmem_kv + issue_stage_m * 32),
                  "r"(((first_pv0_m) ? 0 : 1)));
              ...
              asm volatile(
                  ...
                  "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%0], [ta], db, id, p1;\n\t"
                  "}\n" ::"r"(tmem_tmem_o1),
                  "r"(_mma_b_lo_3), "r"(tmem_tmem_kv + issue_stage_m * 32),
                  "r"(((first_pv1_m) ? 0 : 1)));
...
#pragma unroll 1
        for (int round_p = 0; round_p < cta_n_blocks_p; round_p++) {
          ...
          if (lane < 2 * pages_per_block_p) {
            ...
            int pg_dst_p = smem_pg_addr + (unsigned int)(pg_slot_p * 64) +
                           (unsigned int)((task_i_p * 8 + page_i_p) * 4);
...
#pragma unroll
            for (int task_i_l = 0; task_i_l < 2; task_i_l++) {
              ...
              int pg_l[8];
              int task_pg_base_l =
                  smem_pg_addr + (unsigned int)(pg_slot_l * 64) + (unsigned int)(task_i_l * 8 * 4);
              asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                           : "=r"(*reinterpret_cast<uint32_t*>(&pg_l[0])),
                             "=r"(*reinterpret_cast<uint32_t*>(&pg_l[(0) + 1])),
                             "=r"(*reinterpret_cast<uint32_t*>(&pg_l[(0) + 2])),
                             "=r"(*reinterpret_cast<uint32_t*>(&pg_l[(0) + 3]))
                           : "r"(task_pg_base_l));
              {
                asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                             : "=r"(*reinterpret_cast<uint32_t*>(&pg_l[4])),
                               "=r"(*reinterpret_cast<uint32_t*>(&pg_l[(4) + 1])),
                               "=r"(*reinterpret_cast<uint32_t*>(&pg_l[(4) + 2])),
                               "=r"(*reinterpret_cast<uint32_t*>(&pg_l[(4) + 3]))
                             : "r"(task_pg_base_l + 16));
              }
...
#pragma unroll
                for (int pg_i_l = 0; pg_i_l < 8; pg_i_l++) {
                  ...
                  tma_5d_gmem2smem(task_dst_l + pg_i_l * 2048, Vp, 0, vsub_l, 0, kv_head_idx_l,
                                   vpg_l, pk_full_addr + (pk_slot_l) * 8);
                  tma_3d_gmem2smem(task_dst_l + 16384 + pg_i_l * 128, Vsf, vsub_l * 8,
                                   kv_head_idx_l, vpg_l, pk_full_addr + (pk_slot_l) * 8);
                }
              } else {
#pragma unroll
                for (int pg_i_l_1 = 0; pg_i_l_1 < 8; pg_i_l_1++) {
                  ...
                  tma_5d_gmem2smem(task_dst_l + pg_i_l_1 * 2048, Kp, 0, ksub_l, 0, kv_head_idx_l,
                                   kpg_l, pk_full_addr + (pk_slot_l) * 8);
                  tma_3d_gmem2smem(task_dst_l + 16384 + pg_i_l_1 * 128, Ksf, ksub_l * 8,
                                   kv_head_idx_l, kpg_l, pk_full_addr + (pk_slot_l) * 8);
                }
              }
            }
...
#pragma unroll 1
      for (unsigned int tile_idx_w = blockIdx.x; tile_idx_w < total_tiles_w;
           tile_idx_w += gridDim.x) {
...
#pragma unroll
          for (int work_task_index = 0; work_task_index < 2; work_task_index++) {
            ...
            uint32_t _mbar_token_0 =
                mbarrier_try_wait(kv_full_addr + (work_stage) * 8, ((work_is_v == 1) ? 0 : 1));
            unsigned int work_reuse_ready = _mbar_token_0;
            mbarrier_wait(kv_empty_addr + (work_stage) * 8, ((work_is_v == 1) ? 0 : 1));
            mbarrier_wait_token(kv_full_addr + (work_stage) * 8, ((work_is_v == 1) ? 0 : 1),
                                work_reuse_ready);
            int work_pk_base = smem_pk_addr + (unsigned int)(pk_slot_c * 34816) +
                               (unsigned int)(work_task_index * 17408);
...
#pragma unroll
              for (int atom_i = 0; atom_i < 2; atom_i++) {
                ...
                asm volatile("ld.shared.b32 %0, [%1];"
                             : "=r"(*reinterpret_cast<uint32_t*>(&ksf_words0[atom_i * 2]))
                             : "r"(work_sf_base + ksf_row0 * 8));
                asm volatile("ld.shared.b32 %0, [%1];"
                             : "=r"(*reinterpret_cast<uint32_t*>(&ksf_words0[atom_i * 2 + 1]))
                             : "r"(work_sf_base + ksf_row0 * 8 + 4));
                asm volatile("ld.shared.b32 %0, [%1];"
                             : "=r"(*reinterpret_cast<uint32_t*>(&ksf_words1[atom_i * 2]))
                             : "r"(work_sf_base + (ksf_row0 + 8) * 8));
                asm volatile("ld.shared.b32 %0, [%1];"
                             : "=r"(*reinterpret_cast<uint32_t*>(&ksf_words1[atom_i * 2 + 1]))
                             : "r"(work_sf_base + (ksf_row0 + 8) * 8 + 4));
#pragma unroll
                for (int ldsm_stage = 0; ldsm_stage < 8; ldsm_stage++) {
                  ...
                  asm volatile(
                      "ldmatrix.sync.aligned.shared::cta.m8n16.x2.b8x16.b4x16_p64 {%0, %1}, [%2];\n"
...
#pragma unroll
                for (int ldsm_stage_1 = 0; ldsm_stage_1 < 8; ldsm_stage_1++) {
                  ...
                  if (ldsm_stage_1 % 4 == 0) {
                    {
                      dst_word0 =
                          cake_fmha_qmul4_portable<1>(k_src_word0[slot_1], ksf_words0[scale_slot]);
                    }
                    {
                      dst_word1 =
                          cake_fmha_qmul4_portable<1>(k_src_word1[slot_1], ksf_words1[scale_slot]);
                    }
                  } else if (ldsm_stage_1 % 4 == 1) {
                    {
                      dst_word0 =
                          cake_fmha_qmul4_portable<2>(k_src_word0[slot_1], ksf_words0[scale_slot]);
                    }
                    {
                      dst_word1 =
                          cake_fmha_qmul4_portable<2>(k_src_word1[slot_1], ksf_words1[scale_slot]);
                    }
                  } else {
                    if (ldsm_stage_1 % 4 == 2) {
                      {
                        dst_word0 = cake_fmha_qmul4_portable<3>(k_src_word0[slot_1],
                                                                ksf_words0[scale_slot]);
                      }
                      {
                        dst_word1 = cake_fmha_qmul4_portable<3>(k_src_word1[slot_1],
                                                                ksf_words1[scale_slot]);
                      }
                    } else {
                      {
                        dst_word0 = cake_fmha_qmul4_portable<4>(k_src_word0[slot_1],
                                                                ksf_words0[scale_slot]);
                      }
                      {
                        dst_word1 = cake_fmha_qmul4_portable<4>(k_src_word1[slot_1],
                                                                ksf_words1[scale_slot]);
                      }
                    }
                  }
                  ...
                  int taddr_atom = taddr + 96 + (unsigned int)(atom_i_1 * 16 << 16) +
                                   (unsigned int)(work_stage * 32 + ldsm_stage_1 * 4);
                  asm volatile(
                      "tcgen05.st.sync.aligned.16x128b.x1.b32"
                      " [%0], {%1, %2};" ::"r"(taddr_atom),
                      "r"(*reinterpret_cast<const uint32_t*>(&store_words[0])),
                      "r"(*reinterpret_cast<const uint32_t*>(&store_words[1])));
                }
              }
            } else {
...
#pragma unroll
                for (int atom_i_2 = 0; atom_i_2 < 2; atom_i_2++) {
                  ...
                  asm volatile(
                      "ldmatrix.sync.aligned.shared::cta.m16n16.x1.trans.b8x16.b4x16_p64 {%0, %1}, "
                      "[%2];\n"
                ...
                asm volatile("ld.shared.b32 %0, [%1];"
                             : "=r"(*reinterpret_cast<uint32_t*>(&vsf_words[ldsm_stage_2 * 2]))
                             : "r"(vsf_addr));
                asm volatile("ld.shared.b32 %0, [%1];"
                             : "=r"(*reinterpret_cast<uint32_t*>(&vsf_words[ldsm_stage_2 * 2 + 1]))
                             : "r"(vsf_addr + 4));
              }
#pragma unroll
...
#pragma unroll
                for (int ldsm_stage_3 = 0; ldsm_stage_3 < 8; ldsm_stage_3++) {
                  ...
                  {
                    dst_word0_1 =
                        cake_fmha_qmul4_portable<0>(v_src_word0[slot_3], vsf_words[scale_slot_1]);
                  }
                  {
                    dst_word1_1 =
                        cake_fmha_qmul4_portable<0>(v_src_word1[slot_3], vsf_words[scale_slot_1]);
                  }
                  unsigned int store_words_1[2];
                  store_words_1[0] = dst_word0_1;
                  store_words_1[1] = dst_word1_1;
                  int taddr_atom_1 = taddr + 96 + (unsigned int)(atom_i_3 * 16 << 16) +
                                     (unsigned int)(work_stage * 32 + ldsm_stage_3 * 4);
                  asm volatile(
                      "tcgen05.st.sync.aligned.16x128b.x1.b32"
                      " [%0], {%1, %2};" ::"r"(taddr_atom_1),
                      "r"(*reinterpret_cast<const uint32_t*>(&store_words_1[0])),
                      "r"(*reinterpret_cast<const uint32_t*>(&store_words_1[1])));
                }
              }
            }
            asm volatile("tcgen05.wait::st.sync.aligned;" ::: "memory");
