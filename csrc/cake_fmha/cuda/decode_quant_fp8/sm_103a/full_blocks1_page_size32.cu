...
#pragma unroll 1
      for (unsigned int _tile_iter_m = 0; _tile_iter_m < total_tiles_m; _tile_iter_m++) {
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_1),
            "r"(_mma_b_lo_1), "r"(tmem_tmem_s1), "r"(0));
...
#pragma unroll 1
      for (unsigned int _tile_iter_h = 0; _tile_iter_h < total_tiles_h; _tile_iter_h++) {
        ...
        throttle_stage_h += 1;
        if (throttle_stage_h == 2) {
          throttle_stage_h = 0;
          _phase_throttle_full ^= 1;
        }
        if (elect_sync()) {
          ...
          asm volatile(
              ...
              " [%0], [%1];" ::"r"(work_response_addr + work_stage_h * 16),
              "r"(work_full_addr + work_stage_h * 8)
              : "memory");
        }
...
#pragma unroll 1
      for (unsigned int _tile_iter_l = 0; _tile_iter_l < total_tiles_l; _tile_iter_l++) {
        ...
        throttle_stage_l += 1;
        if (throttle_stage_l == 2) {
          throttle_stage_l = 0;
          _phase_throttle_empty ^= 1;
        }
        ...
        if (elect_sync()) {
          ...
          tma_3d_gmem2smem(smem_qt_addr + (unsigned int)(q_slot_l * 1024), Qt, 0, off_qt, 0,
                           q_full_addr + (q_slot_l) * 8);
...
#pragma unroll
              for (int pg_i_1 = 0; pg_i_1 < 8; pg_i_1++) {
                ...
                tma_5d_gmem2smem(kdst + ntoff, K, 0, pg_i_1 % (PAGE_SIZE / 16) * 16, 0,
                                 kv_head_idx_1, npg0, kv_full_addr + (kv_slot_l) * 8);
              }
              kv_slot_l += 1;
              if (kv_slot_l == 9) {
                kv_slot_l = 0;
