...
#define SMEM_SMEM_PG_OFF 139904
...
kernel_cake_fmha_decode_quant_fp8(const void* __restrict__ Qt, const void* __restrict__ K, const void* __restrict__ V, uint8_t* __restrict__ O, int* __restrict__ page_table, int* __restrict__ seq_lens_kv, float* __restrict__ bmm1_scale_ptr, float* __restrict__ bmm2_scale_ptr, float* __restrict__ partial_O, float* __restrict__ partial_max, float* __restrict__ partial_sum, int pt_batch_stride, int pt_v_offset, int bmm1_is_log2, int num_splits, int blocks_per_split)
{
    ...
    int* smem_pg = reinterpret_cast<int*>(smem_raw + 139904);
    const int smem_pg_addr = smem + 139904;
    ...
    // Mbarrier init (22 groups, 50 barriers)
    ...
    if (warp == 0) {
        ...
        // pg_full: 8 barriers, init_count=32
        ...
        mbarrier_init_pred(smem + 192, 32, leader);
        mbarrier_init_pred(smem + 200, 32, leader);
        // pg_empty: 8 barriers, init_count=1
        ...
        mbarrier_init_pred(smem + 304, 128, leader);
        // o_free_1: 1 barriers, init_count=128
        mbarrier_init_pred(smem + 312, 128, leader);
    ...
    #define pg_full_addr (mbar_base + 144)
    ...
    #define corr_scale_0_addr (mbar_base + 336)
    #define corr_scale_1_addr (mbar_base + 344)
            ...
            for (unsigned int tile_idx_c = blockIdx.x; tile_idx_c < total_tiles_c; tile_idx_c += gridDim.x) {
                ...
                if (num_splits > 1) {
                    ...
                    if (q_even < group_ratio_rt) {
                        *((float*)(partial_O + (po_even + (corr_row_e ^ 80)))) = part_vals[0];
                        *((float*)(partial_O + (po_even + (corr_row_e + 8 ^ 80)))) = part_vals[2];
                        *((float*)(partial_O + (po_even + (corr_row_e + 16 ^ 80)))) = part_vals[4];
                        *((float*)(partial_O + (po_even + (corr_row_e + 24 ^ 80)))) = part_vals[6];
                    }
                    if (q_odd < group_ratio_rt) {
                        *((float*)(partial_O + (po_odd + (corr_row_e ^ corr_x_odd)))) = part_vals[1];
                        *((float*)(partial_O + (po_odd + (corr_row_e + 8 ^ corr_x_odd)))) = part_vals[3];
                        *((float*)(partial_O + (po_odd + (corr_row_e + 16 ^ corr_x_odd)))) = part_vals[5];
                        *((float*)(partial_O + (po_odd + (corr_row_e + 24 ^ corr_x_odd)))) = part_vals[7];
                    }
                    if (corr_tid < 4) {
                    ...
                    {
                        unsigned short _lo, _hi;
                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(out_vals[1]), "f"(out_vals[0]));
                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(out_vals[3]), "f"(out_vals[2]));
                        regs_o[0] = (unsigned)_lo | ((unsigned)_hi << 16);
                    }
                    {
                        unsigned short _lo, _hi;
                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_lo) : "f"(out_vals[5]), "f"(out_vals[4]));
                        asm("cvt.rn.satfinite.e4m3x2.f32 %0, %1, %2;" : "=h"(_hi) : "f"(out_vals[7]), "f"(out_vals[6]));
                        regs_o[1] = (unsigned)_lo | ((unsigned)_hi << 16);
                    }
    ...
    // ---- Role: mma_warp ----
        ...
        { // mma_warp_main
            ...
            for (unsigned int tile_idx_m = blockIdx.x; tile_idx_m < total_tiles_m; tile_idx_m += gridDim.x) {
                ...
                int _mma_a_addr_1 = smem_kv_addr + 16384;
                ...
                for (int pair_2 = 0; pair_2 < num_pairs_2 - 1; pair_2++) {
                    ...
                    int _mma_a_addr_2 = smem_kv_addr + (unsigned int)(s0_next * 16384);
                    ...
                    asm volatile("tcgen05.fence::after_thread_sync;");
                    int _mma_a_addr_3 = smem_v_addr + (unsigned int)(s0 * 16384);
                    int _mma_a_lo_3 = make_warp_uniform(((_mma_a_addr_3 >> 4) & 0x3FFF) | 0x4000000);
                    ...
                    int _mma_a_addr_4 = smem_kv_addr + (unsigned int)(s1_next * 16384);
                    ...
                    asm volatile("tcgen05.fence::after_thread_sync;");
                    int _mma_a_addr_5 = smem_v_addr + (unsigned int)(s1 * 16384);
                    int _mma_a_lo_5 = make_warp_uniform(((_mma_a_addr_5 >> 4) & 0x3FFF) | 0x4000000);
                ...
                asm volatile("tcgen05.fence::after_thread_sync;");
                int _mma_a_addr_6 = smem_v_addr + (unsigned int)(s0_last * 16384);
                ...
                asm volatile("tcgen05.fence::after_thread_sync;");
                int _mma_a_addr_7 = smem_v_addr + (unsigned int)(s1_last * 16384);
    ...
    // ---- Role: load_pgoff ----
        ...
        { // load_pgoff_main
            ...
            for (unsigned int tile_idx_p = blockIdx.x; tile_idx_p < total_tiles_p; tile_idx_p += gridDim.x) {
                ...
                int split_start_block_p = split_p * blocks_per_split;
                ...
                for (int ni_p = 0; ni_p < cta_n_blocks_p; ni_p++) {
                    int n_block_p = split_start_block_p + cta_n_blocks_p - 1 - ni_p;
                    int page_base_p = n_block_p * pages_per_block_p;
                    mbarrier_wait(pg_empty_addr + (pg_slot_p) * 8, pg_phase_p);
                    if (lane < 2 * pages_per_block_p) {
                        int side_p = lane / pages_per_block_p;
                        int page_i_p = lane % pages_per_block_p;
                        int pt_row_p = ((side_p == 0) ? pt_base_k_p : pt_base_v_p);
                        int page_id_p = page_table[pt_row_p + page_base_p + page_i_p];
                        int pg_dst_p = smem_pg_addr + (unsigned int)(pg_slot_p * 64) + (unsigned int)((side_p * 8 + page_i_p) * 4);
                        asm volatile("st.shared.b32 [%0], %1;" :: "r"(pg_dst_p), "r"(page_id_p));
                    }
                    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                    mbarrier_arrive(pg_full_addr + (pg_slot_p) * 8);
                    pg_slot_p = (pg_slot_p + 1) % 8;
                    if (pg_slot_p == 0) { pg_phase_p ^= 1; }
                }
            }
        }
    // ---- Role: scheduler ----
            ...
            for (unsigned int tile_idx_l = blockIdx.x; tile_idx_l < total_tiles_l; tile_idx_l += gridDim.x) {
                ...
                if (elect_sync()) {
                    ...
                    for (int ni = 0; ni < prefill; ni++) {
                        ...
                        int pg_k[8];
                        int pg_k_base = smem_pg_addr + (unsigned int)(pg_slot_l * 64);
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                        ...
                        pg_slot_l = (pg_slot_l + 1) % 8;
                        if (pg_slot_l == 0) { pg_phase_l ^= 1; }
                    }
                    #pragma unroll 1
                    for (int ni_1 = 0; ni_1 < cta_n_blocks_3; ni_1++) {
                        int stage = ni_1 % 8;
                        int pg_v_stage = (pg_tile_base_l + ni_1) % 8;
                        int next_ni = ni_1 + 2;
                        if (next_ni < cta_n_blocks_3) {
                            ...
                            int pg_nk[8];
                            int pg_nk_base = smem_pg_addr + (unsigned int)(pg_slot_l * 64);
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                        ...
                        int pg_v[8];
                        int pg_v_base = smem_pg_addr + (unsigned int)(pg_v_stage * 64) + 32;
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
