...
__global__ __launch_bounds__(512, 1) void kernel_cake_fmha_decode_quant_fp8(
    CakeFmhaTensorMap const* Qt, CakeFmhaTensorMap const* K, CakeFmhaTensorMap const* V,
    uint8_t* __restrict__ O, int* __restrict__ page_table, int* __restrict__ seq_lens_kv,
...
#define corr_scale_0_addr (mbar_base + 320)
#define corr_scale_1_addr (mbar_base + 328)
...
#define order_p01_1_addr (mbar_base + 376)
  ...
  // Mbarrier init (22 groups, 48 barriers)
  ...
  if (warp <= 7) {
    ...
    {  // softmax_main
      ...
      int op01_stage = 0;
      unsigned int total_tiles_s =
          BATCH_SIZE * NUM_KV_HEADS * ((NUM_SPLITS != 0) ? NUM_SPLITS : num_splits);
...
#pragma unroll 1
      for (unsigned int tile_idx_s = blockIdx.x; tile_idx_s < total_tiles_s;
        ...
        int split_start_block =
            split_s * ((BLOCKS_PER_SPLIT != 0) ? BLOCKS_PER_SPLIT : blocks_per_split);
        int rem_blocks = even_n_blocks - split_start_block;
        int capped_blocks =
            ((rem_blocks > ((BLOCKS_PER_SPLIT != 0) ? BLOCKS_PER_SPLIT : blocks_per_split))
                 ? ((BLOCKS_PER_SPLIT != 0) ? BLOCKS_PER_SPLIT : blocks_per_split)
                 : rem_blocks);
...
#pragma unroll 1
      for (unsigned int tile_idx_c = blockIdx.x; tile_idx_c < total_tiles_c;
        ...
        int split_start_block_1 =
            split_c * ((BLOCKS_PER_SPLIT != 0) ? BLOCKS_PER_SPLIT : blocks_per_split);
        int rem_blocks_1 = even_n_blocks_1 - split_start_block_1;
        int capped_blocks_1 =
            ((rem_blocks_1 > ((BLOCKS_PER_SPLIT != 0) ? BLOCKS_PER_SPLIT : blocks_per_split))
                 ? ((BLOCKS_PER_SPLIT != 0) ? BLOCKS_PER_SPLIT : blocks_per_split)
                 : rem_blocks_1);
...
#pragma unroll
        ...
        if (((NUM_SPLITS != 0) ? NUM_SPLITS : num_splits) > 1) {
          ...
          int po_even =
              ((head_base + q_even) * ((NUM_SPLITS != 0) ? NUM_SPLITS : num_splits) + split_c) *
              HEAD_DIM;
          int po_odd =
              ((head_base + q_odd) * ((NUM_SPLITS != 0) ? NUM_SPLITS : num_splits) + split_c) *
              HEAD_DIM;
...
#pragma unroll 1
      for (unsigned int tile_idx_m = blockIdx.x; tile_idx_m < total_tiles_m;
        ...
        int split_start_block_2 =
            split_m * ((BLOCKS_PER_SPLIT != 0) ? BLOCKS_PER_SPLIT : blocks_per_split);
        int rem_blocks_2 = even_n_blocks_2 - split_start_block_2;
        int capped_blocks_2 =
            ((rem_blocks_2 > ((BLOCKS_PER_SPLIT != 0) ? BLOCKS_PER_SPLIT : blocks_per_split))
                 ? ((BLOCKS_PER_SPLIT != 0) ? BLOCKS_PER_SPLIT : blocks_per_split)
                 : rem_blocks_2);
...
#pragma unroll 1
        for (int pair_2 = 0; pair_2 < num_pairs_2 - 1; pair_2++) {
          ...
          mbarrier_wait(kv_full_addr + (kv_slot_m) * 8, kv_phase_m);
          int _mma_a_lo_2 =
              make_warp_uniform((((smem_kv_addr) >> 4) & 0x3FFF) + (kv_slot_m) * 1024);
          ...
          asm volatile("tcgen05.fence::after_thread_sync;");
          int _mma_a_lo_3 =
              make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (kv_slot_m) * 1024);
...
#pragma unroll 1
      for (unsigned int tile_idx_p = blockIdx.x; tile_idx_p < total_tiles_p;
        ...
        int split_start_block_p =
            split_p * ((BLOCKS_PER_SPLIT != 0) ? BLOCKS_PER_SPLIT : blocks_per_split);
        int rem_blocks_p = even_n_blocks_p - split_start_block_p;
        int capped_blocks_p =
            ((rem_blocks_p > ((BLOCKS_PER_SPLIT != 0) ? BLOCKS_PER_SPLIT : blocks_per_split))
                 ? ((BLOCKS_PER_SPLIT != 0) ? BLOCKS_PER_SPLIT : blocks_per_split)
                 : rem_blocks_p);
...
#pragma unroll 1
        for (int group_base_p = 0; group_base_p < cta_n_blocks_p; group_base_p += 4) {
          ...
          int n_block_p =
              split_start_block_p + cta_n_blocks_p - 1 - group_base_p - safe_group_block_p;
          ...
          asm volatile(
              ...
              "}" ::"r"((active_pg_p) ? 1 : 0),
              "r"(pg_stage_addr_p + lane * 4),
              "l"(page_table + (pt_base_k_p + page_base_p + page_i_p)));
          asm volatile(
              "{\n\t"
              ".reg .pred p;\n\t"
              "setp.ne.b32 p, %0, 0;\n\t"
              "@p cp.async.ca.shared::cta.global [%1], [%2], 4;\n\t"
              "}" ::"r"((active_pg_p) ? 1 : 0),
              "r"(pg_stage_addr_p + (32 + lane) * 4),
              "l"(page_table + (pt_base_v_p + page_base_p + page_i_p)));
          asm volatile(
              "{\n\t"
              "cp.async.mbarrier.arrive.shared::cta.b64 [%0];\n\t"
              "}" ::"r"(pg_full_addr + (pg_slot_p) * 8)
              : "memory");
...
#pragma unroll 1
      for (unsigned int tile_idx_l = blockIdx.x; tile_idx_l < total_tiles_l;
        ...
        int split_start_block_3 =
            split_l * ((BLOCKS_PER_SPLIT != 0) ? BLOCKS_PER_SPLIT : blocks_per_split);
        int rem_blocks_3 = even_n_blocks_3 - split_start_block_3;
        int capped_blocks_3 =
            ((rem_blocks_3 > ((BLOCKS_PER_SPLIT != 0) ? BLOCKS_PER_SPLIT : blocks_per_split))
                 ? ((BLOCKS_PER_SPLIT != 0) ? BLOCKS_PER_SPLIT : blocks_per_split)
                 : rem_blocks_3);
...
#pragma unroll 1
          for (int ni = 0; ni < prefill; ni++) {
            ...
            int pg_k_base =
                smem_pg_addr + (unsigned int)(pg_k_stage * 256) + (unsigned int)(ni % 4 * 8 * 4);
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
...
#pragma unroll
            for (int pg_i = 0; pg_i < 8; pg_i++) {
              ...
              int toff = pg_i * 2048;
              tma_4d_gmem2smem(ldst + toff, K, 0, pg_i % (PAGE_SIZE / 16) * 16, kv_head_idx_1, pg0,
                               kv_full_addr + (kv_slot_l) * 8);
            }
            kv_slot_l += 1;
            if (kv_slot_l == 9) {
              kv_slot_l = 0;
...
#pragma unroll 1
          for (int ni_1 = 0; ni_1 < cta_n_blocks_3; ni_1++) {
            ...
            if (next_ni < cta_n_blocks_3) {
              ...
              int pg_nk_base = smem_pg_addr + (unsigned int)(pg_nk_stage * 256) +
                               (unsigned int)(next_ni % 4 * 8 * 4);
              asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
...
#pragma unroll
              for (int pg_i_1 = 0; pg_i_1 < 8; pg_i_1++) {
                ...
                tma_4d_gmem2smem(kdst + ntoff, K, 0, pg_i_1 % (PAGE_SIZE / 16) * 16, kv_head_idx_1,
                                 npg0, kv_full_addr + (kv_slot_l) * 8);
              }
              kv_slot_l += 1;
              if (kv_slot_l == 9) {
                kv_slot_l = 0;
            ...
            int pg_v[8];
            int pg_v_base = smem_pg_addr + (unsigned int)(pg_v_stage * 256) +
                            (unsigned int)((32 + ni_1 % 4 * 8) * 4);
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
...
#pragma unroll
            for (int pg_i_2 = 0; pg_i_2 < 8; pg_i_2++) {
              ...
              tma_4d_gmem2smem(vdst + vtoff, V, 0, pg_i_2 % (PAGE_SIZE / 16) * 16, kv_head_idx_1,
                               vpg0, kv_full_addr + (kv_slot_l) * 8);
            }
            kv_slot_l += 1;
            if (kv_slot_l == 9) {
              kv_slot_l = 0;
