                    ...
                    int split_start_block_la = 0;
                    {
                        ...
                        for (int ni_la = 0; ni_la < prefill_la; ni_la++) {
                            ...
                            if (ni_la % 4 == 0) {
                                ...
                                page_slot_la += 1;
                                if (page_slot_la == 6) { page_slot_la = 0; page_phase_la ^= 1; }
                            }
                            int pg_k_la[4];
                            int page_stage_k_la = (page_tile_base_la + ni_la / 4) % 6;
                            int page_addr = smem_page_offsets_addr + (unsigned int)(page_stage_k_la * 128) + (unsigned int)(ni_la % 4 * 8 * 4);
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                : "=r"(*reinterpret_cast<uint32_t*>(&pg_k_la[0])), "=r"(*reinterpret_cast<uint32_t*>(&pg_k_la[(0) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&pg_k_la[(0) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&pg_k_la[(0) + 3]))
                                : "r"(page_addr));
                            ...
                            for (int pg_half_la = 0; pg_half_la < 4; pg_half_la++) {
                                ...
                                for (int hg_la = 0; hg_la < 2; hg_la++) {
                                    ...
                                    {
                                        tma_5d_gmem2smem(ldst_la + toff_la, K, 0, 0, hg_la, kv_head_idx_3, pg0_la, kv_full_addr + (kv_stage_la) * 8);
                                    }
                                }
                            }
                            kv_stage_la += 1;
                        ...
                        for (int ni_la_1 = 0; ni_la_1 < main_ni_la; ni_la_1++) {
                            ...
                            int page_addr_1 = smem_page_offsets_addr + (unsigned int)(page_stage_v_la * 128) + (unsigned int)(ni_la_1 % 4 * 8 * 4);
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                : "=r"(*reinterpret_cast<uint32_t*>(&pg_v_la[0])), "=r"(*reinterpret_cast<uint32_t*>(&pg_v_la[(0) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&pg_v_la[(0) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&pg_v_la[(0) + 3]))
                                : "r"(page_addr_1));
                            ...
                            for (int pg_half_la_1 = 0; pg_half_la_1 < 4; pg_half_la_1++) {
                                ...
                                for (int hg_la_1 = 0; hg_la_1 < 2; hg_la_1++) {
                                    ...
                                    {
                                        tma_5d_gmem2smem(vdst_la + vtoff_la, V, 0, 0, hg_la_1, kv_head_idx_3, vpg0_la, kv_full_addr + (stage_la) * 8);
                                    }
                                }
                            }
                            ...
                            int page_addr_0 = smem_page_offsets_addr + (unsigned int)(page_stage_nk_la * 128) + (unsigned int)(next_ni_la % 4 * 8 * 4);
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                : "=r"(*reinterpret_cast<uint32_t*>(&pg_nk_la[0])), "=r"(*reinterpret_cast<uint32_t*>(&pg_nk_la[(0) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&pg_nk_la[(0) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&pg_nk_la[(0) + 3]))
                                : "r"(page_addr_0));
                            ...
                            for (int pg_half_la_2 = 0; pg_half_la_2 < 4; pg_half_la_2++) {
                                ...
                                for (int hg_la_2 = 0; hg_la_2 < 2; hg_la_2++) {
                                    ...
                                    {
                                        tma_5d_gmem2smem(kdst_la + ntoff_la, K, 0, 0, hg_la_2, kv_head_idx_3, npg0_la, kv_full_addr + (k_stage_la) * 8);
                                    }
                                }
                            }
                        }
                        ...
                        for (int tail_ni_la = main_ni_la; tail_ni_la < cta_n_blocks_3; tail_ni_la++) {
                            ...
                            int page_addr_2 = smem_page_offsets_addr + (unsigned int)(page_stage_tv_la * 128) + (unsigned int)(tail_ni_la % 4 * 8 * 4);
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                : "=r"(*reinterpret_cast<uint32_t*>(&tpg_v_la[0])), "=r"(*reinterpret_cast<uint32_t*>(&tpg_v_la[(0) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&tpg_v_la[(0) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&tpg_v_la[(0) + 3]))
                                : "r"(page_addr_2));
                            int page_group_done_tail_la = ((tail_ni_la % 4 == 3) ? 1 : 0);
                            page_group_done_tail_la = page_group_done_tail_la | ((tail_ni_la + 1 == cta_n_blocks_3) ? 1 : 0);
                            ...
                            for (int pg_half_la_3 = 0; pg_half_la_3 < 4; pg_half_la_3++) {
                                ...
                                for (int hg_la_3 = 0; hg_la_3 < 2; hg_la_3++) {
                                    ...
                                    {
                                        tma_5d_gmem2smem(tvdst_la + tvtoff_la, V, 0, 0, hg_la_3, kv_head_idx_3, tvpg0_la, kv_full_addr + (tstage_la) * 8);
                                    }
                                }
                            }
                        }
            ...
            for (unsigned int tile_idx_p = blockIdx.x; tile_idx_p < total_tiles_p; tile_idx_p += gridDim.x) {
                ...
                for (int group_base_p = 0; group_base_p < cta_n_blocks_4; group_base_p += 4) {
                    ...
                    asm volatile("cp.async.ca.shared::cta.global [%0], [%1], 4, %2;"
                        :: "r"(page_stage_addr_p + lane * 4), "l"(page_table + (pt_base_p + n_block_p * 8 + page_i_p)), "r"((active_page_p) ? 4 : 0));
                    asm volatile(
                        "{\n\t"
        ...
        { // load_warp_main
            ...
            for (unsigned int tile_idx_l = blockIdx.x; tile_idx_l < total_tiles_l; tile_idx_l += gridDim.x) {
                ...
                if (elect_sync()) {
                    ...
                    int off_qt = (batch_idx_5 * Q_LEN + q_row_idx_5) * NUM_Q_HEADS + kv_head_idx_5 * group_ratio_l;
                    ...
                    for (int ni = 0; ni < prefill; ni++) {
                        ...
                        int page_addr_3 = smem_page_offsets_addr + (unsigned int)(page_stage_k * 128) + (unsigned int)((ni % 4 * 8 + 4) * 4);
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                        ...
                        for (int pg_half = 0; pg_half < 4; pg_half++) {
                            ...
                            for (int hg = 0; hg < 2; hg++) {
                                ...
                                {
                                    tma_5d_gmem2smem(ldst + toff, K, 0, 0, hg, kv_head_idx_5, pg0, kv_full_addr + (kv_stage) * 8);
                                }
                            }
                        }
                        kv_stage += 1;
                    ...
                    for (int ni_1 = 0; ni_1 < main_ni; ni_1++) {
                        ...
                        int page_addr_4 = smem_page_offsets_addr + (unsigned int)(page_stage_v * 128) + (unsigned int)((ni_1 % 4 * 8 + 4) * 4);
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                        ...
                        for (int pg_half_1 = 0; pg_half_1 < 4; pg_half_1++) {
                            ...
                            for (int hg_1 = 0; hg_1 < 2; hg_1++) {
                                ...
                                {
                                    tma_5d_gmem2smem(vdst + vtoff, V, 0, 0, hg_1, kv_head_idx_5, vpg0, kv_full_addr + (stage) * 8);
                                }
                            }
                        }
                        int next_ni = ni_1 + 2;
                        ...
                        int page_addr_0_1 = smem_page_offsets_addr + (unsigned int)(page_stage_nk * 128) + (unsigned int)((next_ni % 4 * 8 + 4) * 4);
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                        ...
                        for (int pg_half_2 = 0; pg_half_2 < 4; pg_half_2++) {
                            ...
                            for (int hg_2 = 0; hg_2 < 2; hg_2++) {
                                ...
                                {
                                    tma_5d_gmem2smem(kdst + ntoff, K, 0, 0, hg_2, kv_head_idx_5, npg0, kv_full_addr + (k_stage) * 8);
                                }
                            }
                        }
                    }
                    ...
                    for (int tail_ni = main_ni; tail_ni < cta_n_blocks_5; tail_ni++) {
                        ...
                        int page_addr_5 = smem_page_offsets_addr + (unsigned int)(page_stage_tv * 128) + (unsigned int)((tail_ni % 4 * 8 + 4) * 4);
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                        ...
                        for (int pg_half_3 = 0; pg_half_3 < 4; pg_half_3++) {
                            ...
                            for (int hg_3 = 0; hg_3 < 2; hg_3++) {
                                ...
                                {
                                    tma_5d_gmem2smem(tvdst + tvtoff, V, 0, 0, hg_3, kv_head_idx_5, tvpg0, kv_full_addr + (tstage) * 8);
                                }
                            }
                        }
                    }
