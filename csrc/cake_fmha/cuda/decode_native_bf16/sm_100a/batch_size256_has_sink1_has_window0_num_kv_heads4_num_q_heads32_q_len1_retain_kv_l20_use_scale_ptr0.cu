...
__global__ __launch_bounds__(512) void kernel_cake_fmha_decode_native_bf16(
    CakeFmhaTensorMap const* Qt, CakeFmhaTensorMap const* K, CakeFmhaTensorMap const* V,
    __nv_bfloat16* __restrict__ O_ptr, float* __restrict__ LSE_ptr, int* __restrict__ page_table,
    int* __restrict__ causal_seqlens_kv_global, float* __restrict__ scale_log2_ptr,
    float* __restrict__ sinks_ptr, int max_pages_per_seq, int max_local_seq_len,
...
#pragma unroll
          for (int _lp = 0; _lp < 4; _lp++) {
            __nv_bfloat162 _bf2 = __float22bfloat162_rn(
                make_float2(exp_vals[_lp * 2 + 0], exp_vals[_lp * 2 + 1 + 0]));
        ...
        for (int h = 0; h < 8; h++) {
          ...
          float local_lse = -LOOM_INF;
          {
            ...
            if (local_sum[h] > 0.0f && local_sum[h] == local_sum[h]) {
              ...
              {
                float _log2_0;
                asm volatile("lg2.approx.ftz.f32 %0, %1;" : "=f"(_log2_0) : "f"(local_sum[h]));
                local_lse = local_max[h] * smx_scale_c + _log2_0;
              }
            }
            if (group_ratio_rt > h) {
...
#pragma unroll 1
        for (int pair_2 = 0; pair_2 < cta_n_blocks_2 / 2 - 1; pair_2++) {
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_2),
              "r"(_mma_b_lo_2), "r"(tmem_tmem_s0), "r"(0));
          elect_commit(s_full_0_addr);
          ...
          asm volatile("tcgen05.fence::after_thread_sync;");
          int _mma_a_lo_3 =
              make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (s0_1) * 2048);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_3),
              "r"(_mma_b_lo_3), "r"(tmem_tmem_o0), "r"(((first_pv0) ? 0 : 1)));
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_4),
              "r"(_mma_b_lo_2), "r"(tmem_tmem_s1), "r"(0));
          elect_commit(s_full_1_addr);
          ...
          asm volatile("tcgen05.fence::after_thread_sync;");
          int _mma_a_lo_5 =
              make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (s1_1) * 2048);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_5),
              "r"(_mma_b_lo_5), "r"(tmem_tmem_o1), "r"(((first_pv1) ? 0 : 1)));
        ...
        asm volatile("tcgen05.fence::after_thread_sync;");
        int _mma_a_lo_6 =
            make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (s0_last) * 2048);
        ...
        asm volatile("tcgen05.fence::after_thread_sync;");
        int _mma_a_lo_7 =
            make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (s1_last) * 2048);
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_7),
            "r"(_mma_b_lo_7), "r"(tmem_tmem_o1), "r"(((first_pv1) ? 0 : 1)));
...
#pragma unroll 1
        for (unsigned int tile_idx_la = blockIdx.x; tile_idx_la < total_tiles_la;
          ...
          {
            int page_tile_base_la = page_slot_la;
            int kv_stage_la = 0;
            int kv_phase_la = 1;
...
#pragma unroll 1
            for (int ni_la = 0; ni_la < prefill_la; ni_la++) {
              ...
              if (ni_la % 4 == 0) {
                mbarrier_wait(page_full_addr + (page_slot_la) * 8, page_phase_la);
                page_slot_la += 1;
                if (page_slot_la == 6) {
                  page_slot_la = 0;
                  page_phase_la ^= 1;
              ...
              int pg_k_la[4];
              int page_stage_k_la = (page_tile_base_la + ni_la / 4) % 6;
              int page_addr = smem_page_offsets_addr + (unsigned int)(page_stage_k_la * 128) +
                              (unsigned int)(ni_la % 4 * 8 * 4);
              asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                           : "=r"(*reinterpret_cast<uint32_t*>(&pg_k_la[0])),
                             "=r"(*reinterpret_cast<uint32_t*>(&pg_k_la[(0) + 1])),
                             "=r"(*reinterpret_cast<uint32_t*>(&pg_k_la[(0) + 2])),
                             "=r"(*reinterpret_cast<uint32_t*>(&pg_k_la[(0) + 3]))
                           : "r"(page_addr));
...
#pragma unroll 1
            for (int ni_la_1 = 0; ni_la_1 < main_ni_la; ni_la_1++) {
              ...
              int pg_v_la[4];
              int page_stage_v_la = (page_tile_base_la + ni_la_1 / 4) % 6;
              int page_addr_1 = smem_page_offsets_addr + (unsigned int)(page_stage_v_la * 128) +
                                (unsigned int)(ni_la_1 % 4 * 8 * 4);
              asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                           : "=r"(*reinterpret_cast<uint32_t*>(&pg_v_la[0])),
                             "=r"(*reinterpret_cast<uint32_t*>(&pg_v_la[(0) + 1])),
                             "=r"(*reinterpret_cast<uint32_t*>(&pg_v_la[(0) + 2])),
                             "=r"(*reinterpret_cast<uint32_t*>(&pg_v_la[(0) + 3]))
                           : "r"(page_addr_1));
              int page_group_done_la = ((ni_la_1 % 4 == 3) ? 1 : 0);
              page_group_done_la = page_group_done_la | ((ni_la_1 + 1 == cta_n_blocks_3) ? 1 : 0);
              if (page_group_done_la != 0) {
                mbarrier_arrive(page_empty_addr + (page_stage_v_la) * 8);
              }
...
#pragma unroll
              ...
              int pg_nk_la[4];
              int page_stage_nk_la = (page_tile_base_la + next_ni_la / 4) % 6;
              int page_addr_0 = smem_page_offsets_addr + (unsigned int)(page_stage_nk_la * 128) +
                                (unsigned int)(next_ni_la % 4 * 8 * 4);
              asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                           : "=r"(*reinterpret_cast<uint32_t*>(&pg_nk_la[0])),
                             "=r"(*reinterpret_cast<uint32_t*>(&pg_nk_la[(0) + 1])),
                             "=r"(*reinterpret_cast<uint32_t*>(&pg_nk_la[(0) + 2])),
                             "=r"(*reinterpret_cast<uint32_t*>(&pg_nk_la[(0) + 3]))
                           : "r"(page_addr_0));
...
#pragma unroll 1
            for (int tail_ni_la = main_ni_la; tail_ni_la < cta_n_blocks_3; tail_ni_la++) {
              ...
              int tpg_v_la[4];
              int page_stage_tv_la = (page_tile_base_la + tail_ni_la / 4) % 6;
              int page_addr_2 = smem_page_offsets_addr + (unsigned int)(page_stage_tv_la * 128) +
                                (unsigned int)(tail_ni_la % 4 * 8 * 4);
              asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                           : "=r"(*reinterpret_cast<uint32_t*>(&tpg_v_la[0])),
                             "=r"(*reinterpret_cast<uint32_t*>(&tpg_v_la[(0) + 1])),
                             "=r"(*reinterpret_cast<uint32_t*>(&tpg_v_la[(0) + 2])),
                             "=r"(*reinterpret_cast<uint32_t*>(&tpg_v_la[(0) + 3]))
                           : "r"(page_addr_2));
              int page_group_done_tail_la = ((tail_ni_la % 4 == 3) ? 1 : 0);
              page_group_done_tail_la =
                  page_group_done_tail_la | ((tail_ni_la + 1 == cta_n_blocks_3) ? 1 : 0);
              if (page_group_done_tail_la != 0) {
                mbarrier_arrive(page_empty_addr + (page_stage_tv_la) * 8);
              }
...
#pragma unroll
                for (int hg_la_3 = 0; hg_la_3 < 2; hg_la_3++) {
                  ...
                  {
                    tma_5d_gmem2smem(tvdst_la + tvtoff_la, V, 0, 0, hg_la_3, kv_head_idx_3,
                                     tvpg0_la, kv_full_addr + (tstage_la) * 8);
                  }
                }
              }
            }
...
#pragma unroll 1
      for (unsigned int tile_idx_p = blockIdx.x; tile_idx_p < total_tiles_p;
           tile_idx_p += gridDim.x) {
        const int tiles_per_batch_4 = Q_LEN * NUM_KV_HEADS;
        int batch_idx_4 = tile_idx_p / (unsigned int)tiles_per_batch_4;
        int tile_in_batch_4 = tile_idx_p % (unsigned int)tiles_per_batch_4;
        ...
        int pt_base_p = batch_idx_4 * max_pages_per_seq;
#pragma unroll 1
        for (int group_base_p = 0; group_base_p < cta_n_blocks_4; group_base_p += 4) {
          int group_rem_p = cta_n_blocks_4 - group_base_p;
          int group_blocks_p = ((group_rem_p > 4) ? 4 : group_rem_p);
          int group_block_p = lane / 8;
          int active_page_p = ((group_block_p < group_blocks_p) ? 1 : 0);
          int safe_group_block_p = ((active_page_p != 0) ? group_block_p : 0);
          int n_block_p = group_base_p + safe_group_block_p;
          int page_i_p = lane % 8;
          mbarrier_wait(page_empty_addr + (page_slot_p) * 8, page_phase_p);
          int page_stage_addr_p = smem_page_offsets_addr + (unsigned int)(page_slot_p * 128);
          asm volatile("cp.async.ca.shared::cta.global [%0], [%1], 4, %2;" ::"r"(page_stage_addr_p +
                                                                                 lane * 4),
                       "l"(page_table + (pt_base_p + n_block_p * 8 + page_i_p)),
                       "r"((active_page_p) ? 4 : 0));
          asm volatile(
              "{\n\t"
              "cp.async.mbarrier.arrive.shared::cta.b64 [%0];\n\t"
              "}" ::"r"(page_full_addr + (page_slot_p) * 8)
              : "memory");
...
#pragma unroll 1
      for (unsigned int tile_idx_l = blockIdx.x; tile_idx_l < total_tiles_l;
        ...
        const int tiles_per_batch_5 = Q_LEN * NUM_KV_HEADS;
        int batch_idx_5 = tile_idx_l / (unsigned int)tiles_per_batch_5;
        int tile_in_batch_5 = tile_idx_l % (unsigned int)tiles_per_batch_5;
        int q_row_idx_5 = tile_in_batch_5 / NUM_KV_HEADS;
        int kv_head_idx_5 = tile_in_batch_5 % NUM_KV_HEADS;
        int global_q_pos_5 = causal_seqlens_kv_global[batch_idx_5] + q_row_idx_5;
        int visible_local_keys_5 = global_q_pos_5 + 1;
        int win_start_5 = 0;
        int num_n_blocks_total_5 = (max_local_seq_len + BLOCK_N - 1) / BLOCK_N;
        int cta_n_blocks_5 = num_n_blocks_total_5 + num_n_blocks_total_5 % 2;
        if (cta_n_blocks_5 < 4) {
          cta_n_blocks_5 = 4;
        }
        ...
        if (elect_sync()) {
          ...
          int off_qt =
              (batch_idx_5 * Q_LEN + q_row_idx_5) * NUM_Q_HEADS + kv_head_idx_5 * group_ratio_l;
          ...
          int kv_stage = 0;
          int kv_phase = 1;
          int prefill = ((cta_n_blocks_5 < 2) ? cta_n_blocks_5 : 2);
#pragma unroll 1
          for (int ni = 0; ni < prefill; ni++) {
            int n_block = split_start_block_1 + cta_n_blocks_5 - 1 - ni;
            if (ni % 4 == 0) {
            ...
            int page_addr_3 = smem_page_offsets_addr + (unsigned int)(page_stage_k * 128) +
                              (unsigned int)((ni % 4 * 8 + 4) * 4);
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&pg_k[0])),
                           "=r"(*reinterpret_cast<uint32_t*>(&pg_k[(0) + 1])),
                           "=r"(*reinterpret_cast<uint32_t*>(&pg_k[(0) + 2])),
                           "=r"(*reinterpret_cast<uint32_t*>(&pg_k[(0) + 3]))
                         : "r"(page_addr_3));
...
#pragma unroll
              for (int hg = 0; hg < 2; hg++) {
                ...
                {
                  tma_5d_gmem2smem(ldst + toff, K, 0, 0, hg, kv_head_idx_5, pg0,
          ...
          int main_ni = cta_n_blocks_5 - 2;
#pragma unroll 1
          for (int ni_1 = 0; ni_1 < main_ni; ni_1++) {
            int stage = ni_1 % 4;
            int n_block_1 = split_start_block_1 + cta_n_blocks_5 - 1 - ni_1;
            int pg_v[4];
            int page_stage_v = (page_tile_base_l + ni_1 / 4) % 6;
            int page_addr_4 = smem_page_offsets_addr + (unsigned int)(page_stage_v * 128) +
                              (unsigned int)((ni_1 % 4 * 8 + 4) * 4);
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&pg_v[0])),
                           "=r"(*reinterpret_cast<uint32_t*>(&pg_v[(0) + 1])),
                           "=r"(*reinterpret_cast<uint32_t*>(&pg_v[(0) + 2])),
                           "=r"(*reinterpret_cast<uint32_t*>(&pg_v[(0) + 3]))
                         : "r"(page_addr_4));
            int page_group_done = ((ni_1 % 4 == 3) ? 1 : 0);
            page_group_done = page_group_done | ((ni_1 + 1 == cta_n_blocks_5) ? 1 : 0);
...
#pragma unroll
              for (int hg_1 = 0; hg_1 < 2; hg_1++) {
                ...
                {
                  tma_5d_gmem2smem(vdst + vtoff, V, 0, 0, hg_1, kv_head_idx_5, vpg0,
            ...
            int next_n = split_start_block_1 + cta_n_blocks_5 - 1 - next_ni;
            if (next_ni % 4 == 0) {
              mbarrier_wait(page_full_addr + (page_slot_l) * 8, page_phase_l);
              page_slot_l += 1;
              if (page_slot_l == 6) {
            ...
            int page_addr_0_1 = smem_page_offsets_addr + (unsigned int)(page_stage_nk * 128) +
                                (unsigned int)((next_ni % 4 * 8 + 4) * 4);
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&pg_nk[0])),
                           "=r"(*reinterpret_cast<uint32_t*>(&pg_nk[(0) + 1])),
                           "=r"(*reinterpret_cast<uint32_t*>(&pg_nk[(0) + 2])),
                           "=r"(*reinterpret_cast<uint32_t*>(&pg_nk[(0) + 3]))
                         : "r"(page_addr_0_1));
...
#pragma unroll
              for (int hg_2 = 0; hg_2 < 2; hg_2++) {
                ...
                {
                  tma_5d_gmem2smem(kdst + ntoff, K, 0, 0, hg_2, kv_head_idx_5, npg0,
...
#pragma unroll 1
          for (int tail_ni = main_ni; tail_ni < cta_n_blocks_5; tail_ni++) {
            int tstage = tail_ni % 4;
            int tn_block = split_start_block_1 + cta_n_blocks_5 - 1 - tail_ni;
            int tpg_v[4];
            int page_stage_tv = (page_tile_base_l + tail_ni / 4) % 6;
            int page_addr_5 = smem_page_offsets_addr + (unsigned int)(page_stage_tv * 128) +
                              (unsigned int)((tail_ni % 4 * 8 + 4) * 4);
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&tpg_v[0])),
                           "=r"(*reinterpret_cast<uint32_t*>(&tpg_v[(0) + 1])),
                           "=r"(*reinterpret_cast<uint32_t*>(&tpg_v[(0) + 2])),
                           "=r"(*reinterpret_cast<uint32_t*>(&tpg_v[(0) + 3]))
                         : "r"(page_addr_5));
            int page_group_done_tail = ((tail_ni % 4 == 3) ? 1 : 0);
            page_group_done_tail = page_group_done_tail | ((tail_ni + 1 == cta_n_blocks_5) ? 1 : 0);
...
#pragma unroll
              for (int hg_3 = 0; hg_3 < 2; hg_3++) {
                ...
                {
                  tma_5d_gmem2smem(tvdst + tvtoff, V, 0, 0, hg_3, kv_head_idx_5, tvpg0,
                                   kv_full_addr + (tstage) * 8);
                }
              }
            }
          }
