...
#define K_LOOKAHEAD (NUM_KV_STAGES - 2)
#define KV_SUFFIX (5120 + (NUM_KV_STAGES * 32768))
#define SMEM_BYTES (KV_SUFFIX + 8192)
...
kernel_cake_fmha_decode_native_bf16(CakeFmhaTensorMap const* Qt, CakeFmhaTensorMap const* K, CakeFmhaTensorMap const* V, __nv_bfloat16* __restrict__ O_ptr, float* __restrict__ LSE_ptr, int* __restrict__ page_table, int* __restrict__ causal_seqlens_kv_global, float* __restrict__ scale_log2_ptr, float* __restrict__ sinks_ptr, int max_pages_per_seq, int max_local_seq_len, float softmax_scale_log2, int window_left, int num_q_heads, int num_kv_heads, int batch_size)
{
    ...
    #define s_full_0_addr (mbar_base + 224)
    #define s_full_1_addr (mbar_base + 232)
    #define s_empty_0_addr (mbar_base + 240)
    #define s_empty_1_addr (mbar_base + 248)
    #define p_full_0_addr (mbar_base + 256)
    #define p_full_1_addr (mbar_base + 264)
    #define corr_scale_addr (mbar_base + 272)
    #define corr_empty_0_addr (mbar_base + 288)
    #define corr_empty_1_addr (mbar_base + 296)
    #define stats_empty_addr (mbar_base + 304)
    #define o_ready_0_addr (mbar_base + 312)
    #define o_ready_1_addr (mbar_base + 320)
    #define o_empty_0_addr (mbar_base + 328)
    #define o_empty_1_addr (mbar_base + 336)
    ...
    __nv_bfloat16* smem_p0 = reinterpret_cast<__nv_bfloat16*>(smem_raw + 169984);
    const int smem_p0_addr = smem + 169984;
    __nv_bfloat16* smem_p1 = reinterpret_cast<__nv_bfloat16*>(smem_raw + 174080);
    const int smem_p1_addr = smem + 174080;
    int* smem_page_offsets = reinterpret_cast<int*>(smem_raw + 176128);
    const int smem_page_offsets_addr = smem + 176128;
    unsigned int* work_response = reinterpret_cast<unsigned int*>(smem_raw + 177152);
    const int work_response_addr = smem + 177152;
    ...
    // Mbarrier init (25 pipeline groups, 0 ordered-sequence groups, 52 barriers)
    ...
    if (tid == 0) {
        ...
        mbarrier_init_owner_lane(smem_raw + 80, 1);
        mbarrier_init_owner_lane(smem_raw + 88, 1);
        ...
        mbarrier_init_owner_lane(smem_raw + 144, 32);
        mbarrier_init_owner_lane(smem_raw + 152, 32);
        ...
        mbarrier_init_owner_lane(smem_raw + 224, 1);
        // s_full_1: 1 barriers, init_count=1
        mbarrier_init_owner_lane(smem_raw + 232, 1);
        ...
        mbarrier_init_owner_lane(smem_raw + 256, 256);
        // p_full_1: 1 barriers, init_count=256
        mbarrier_init_owner_lane(smem_raw + 264, 256);
        ...
        mbarrier_init_owner_lane(smem_raw + 400, 32);
        mbarrier_init_owner_lane(smem_raw + 408, 32);
    }
        ...
        { // softmax_main
            ...
            for (unsigned int _tile_iter_s = 0; _tile_iter_s < total_tiles_s; _tile_iter_s++) {
                ...
                for (int pair = 0; pair < cta_n_blocks / 2; pair++) {
                    ...
                    float2 _mul_f32x2_1;
                    asm("mul.rn.ftz.f32x2 %0, %1, %2;" : "=l"(*(unsigned long long*)&_mul_f32x2_1) : "l"(*(const unsigned long long*)&_f2_3), "l"(*(const unsigned long long*)&_f2_4));
                    float2 neg_max_scaled_pair_f2 = _mul_f32x2_1;
                    ...
                    float2 affine01_pair_f2 = fma_f32x2_rn_ftz(_f2_5, _f2_2, neg_max_scaled_pair_f2);
                    float2 affine23_pair_f2 = fma_f32x2_rn_ftz(_f2_6, _f2_2, neg_max_scaled_pair_f2);
                    float2 affine45_pair_f2 = fma_f32x2_rn_ftz(_f2_7, _f2_2, neg_max_scaled_pair_f2);
                    float2 affine67_pair_f2 = fma_f32x2_rn_ftz(_f2_8, _f2_2, neg_max_scaled_pair_f2);
                    ...
                    float2 row_sum_next_f2 = fma_f32x2_rn_ftz(_f2_16, _f2_17, pair_sum_reduced_f2);
        ...
        { // correction_main
            ...
            for (unsigned int _tile_iter_c = 0; _tile_iter_c < total_tiles_c; _tile_iter_c++) {
                ...
                for (int pair_1 = 1; pair_1 < cta_n_blocks_1 / 2; pair_1++) {
                    ...
                    float2 _mul_f32x2_2;
                    asm("mul.rn.ftz.f32x2 %0, %1, %2;" : "=l"(*(unsigned long long*)&_mul_f32x2_2) : "l"(*(const unsigned long long*)&_f2_20), "l"(*(const unsigned long long*)&max_diff0_pair_f2));
                    ...
                    if (_vote_0 != 0) {
                        ...
                        float2 _mul_f32x2_3;
                        asm("mul.rn.ftz.f32x2 %0, %1, %2;" : "=l"(*(unsigned long long*)&_mul_f32x2_3) : "l"(*(const unsigned long long*)&_f2_22), "l"(*(const unsigned long long*)&_f2_21));
                        float2 o0_lo01_scaled_f2 = _mul_f32x2_3;
                        float2 _mul_f32x2_4;
                        asm("mul.rn.ftz.f32x2 %0, %1, %2;" : "=l"(*(unsigned long long*)&_mul_f32x2_4) : "l"(*(const unsigned long long*)&_f2_23), "l"(*(const unsigned long long*)&_f2_21));
                        float2 o0_lo23_scaled_f2 = _mul_f32x2_4;
                        float2 _mul_f32x2_5;
                        asm("mul.rn.ftz.f32x2 %0, %1, %2;" : "=l"(*(unsigned long long*)&_mul_f32x2_5) : "l"(*(const unsigned long long*)&_f2_24), "l"(*(const unsigned long long*)&_f2_21));
                        float2 o0_hi01_scaled_f2 = _mul_f32x2_5;
                        float2 _mul_f32x2_6;
                        asm("mul.rn.ftz.f32x2 %0, %1, %2;" : "=l"(*(unsigned long long*)&_mul_f32x2_6) : "l"(*(const unsigned long long*)&_f2_25), "l"(*(const unsigned long long*)&_f2_21));
                    ...
                    float2 _mul_f32x2_7;
                    asm("mul.rn.ftz.f32x2 %0, %1, %2;" : "=l"(*(unsigned long long*)&_mul_f32x2_7) : "l"(*(const unsigned long long*)&_f2_20), "l"(*(const unsigned long long*)&max_diff1_pair_f2));
                ...
                {
                    if ((unsigned int)group_ratio_rt > lane) {
                        int sink_head_lane = (unsigned int)(kv_head_idx_1 * group_ratio_rt) + lane;
                ...
                for (int hp = 0; hp < 4; hp++) {
                    ...
                    asm("mul.rn.ftz.f32x2 %0, %1, %2;" : "=l"(*(unsigned long long*)&_mul_f32x2_12) : "l"(*(const unsigned long long*)&_f2_34), "l"(*(const unsigned long long*)&_f2_36));
                    float2 o1_scaled_pair_f2 = _mul_f32x2_12;
                    float2 merged_pair_f2 = fma_f32x2_rn_ftz(_f2_33, _f2_35, o1_scaled_pair_f2);
            ...
            for (unsigned int _tile_iter_p = 0; _tile_iter_p < total_tiles_p; _tile_iter_p++) {
                ...
                for (int group_pair_base_p = 0; group_pair_base_p < 32; group_pair_base_p += 8) {
                    ...
                    asm volatile("cp.async.ca.shared::cta.global [%0], [%1], 4;"
                        :: "r"((unsigned int)page_stage_addr_p0 + lane * 4), "l"(page_table + (pt_base_p + n_block_p0 * 8 + page_i_p)));
                    asm volatile(
                        "{\n\t"
                    ...
                    asm volatile("cp.async.ca.shared::cta.global [%0], [%1], 4;"
                        :: "r"((unsigned int)page_stage_addr_p1 + lane * 4), "l"(page_table + (pt_base_p + n_block_p1 * 8 + page_i_p)));
                    asm volatile(
                        "{\n\t"
                        "cp.async.mbarrier.arrive.shared::cta.b64 [%0];\n\t"
                        "}"
        ...
        { // scheduler_main
            ...
            for (unsigned int _tile_iter_sched = 0; _tile_iter_sched < total_tiles_sched; _tile_iter_sched++) {
                ...
                if (elect_sync()) {
                    ...
                    asm volatile(
                        ...
                        "clusterlaunchcontrol.try_cancel.async.shared::cta"
                            ...
                            " [%0], [%1];"
                        :: "r"(work_response_addr + work_stage_sched * 16 + 0 * 16), "r"(work_full_addr + work_stage_sched * 8)
                        : "memory");
                }
                ...
                asm volatile(
                    ...
                    "}\n"
                    : "=r"(_clc_valid_1)
                    : "r"(work_response_addr + work_stage_sched * 16 + 0 * 16)
                    : "memory");
            ...
            for (unsigned int _tile_iter_l = 0; _tile_iter_l < total_tiles_l; _tile_iter_l++) {
                ...
                for (int ni = 0; ni < prefill; ni++) {
                    ...
                    if (elect_sync()) {
                        ...
                        int page_ids[4];
                        int page_addr_1 = smem_page_offsets_addr + (unsigned int)(page_stage_k * 128) + (unsigned int)(ni % 4 * 8 * 4);
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                ...
                int main_ni = cta_n_blocks_3 - K_LOOKAHEAD;
                #pragma unroll 1
                for (int ni_1 = 0; ni_1 < main_ni; ni_1++) {
                    ...
                    if (elect_sync()) {
                        int page_addr_2 = smem_page_offsets_addr + (unsigned int)(page_stage_v * 128) + (unsigned int)((ni_1 % 4 * 8 + 4) * 4);
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                    ...
                    if (elect_sync()) {
                        ...
                        int page_ids_1[4];
                        int page_addr_3 = smem_page_offsets_addr + (unsigned int)(page_stage_v * 128) + (unsigned int)(ni_1 % 4 * 8 * 4);
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                    ...
                    if (elect_sync()) {
                        int page_addr_4 = smem_page_offsets_addr + (unsigned int)(page_stage_nk * 128) + (unsigned int)((next_ni % 4 * 8 + 4) * 4);
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                    ...
                    if (elect_sync()) {
                        ...
                        int page_ids_2[4];
                        int page_addr_5 = smem_page_offsets_addr + (unsigned int)(page_stage_nk * 128) + (unsigned int)(next_ni % 4 * 8 * 4);
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                ...
                for (int tail_ni = main_ni; tail_ni < cta_n_blocks_3; tail_ni++) {
                    int tstage = tail_ni % NUM_KV_STAGES;
                    int tn_block = split_start_block_1 + cta_n_blocks_3 - 1 - tail_ni;
                    int tpg_v[4];
                    int page_stage_tv = (page_tile_base_l + tail_ni / 4) % 8;
                    if (elect_sync()) {
                        int page_addr_6 = smem_page_offsets_addr + (unsigned int)(page_stage_tv * 128) + (unsigned int)((tail_ni % 4 * 8 + 4) * 4);
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                    ...
                    if (elect_sync()) {
                        ...
                        int page_ids_3[4];
                        int page_addr_7 = smem_page_offsets_addr + (unsigned int)(page_stage_tv * 128) + (unsigned int)(tail_ni % 4 * 8 * 4);
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
