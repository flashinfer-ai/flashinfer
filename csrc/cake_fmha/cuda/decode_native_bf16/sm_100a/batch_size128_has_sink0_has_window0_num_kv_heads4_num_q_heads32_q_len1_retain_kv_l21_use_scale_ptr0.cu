...
kernel_cake_fmha_decode_native_bf16(CakeFmhaTensorMap const* Qt, CakeFmhaTensorMap const* K, CakeFmhaTensorMap const* V, __nv_bfloat16* __restrict__ O_ptr, float* __restrict__ LSE_ptr, int* __restrict__ page_table, int* __restrict__ causal_seqlens_kv_global, float* __restrict__ scale_log2_ptr, float* __restrict__ sinks_ptr, int max_pages_per_seq, int max_local_seq_len, float softmax_scale_log2, int window_left, int num_q_heads, int num_kv_heads, int batch_size)
{
    ...
    #define corr_empty_0_addr (mbar_base + 216)
    #define corr_empty_1_addr (mbar_base + 224)
    #define stats_empty_addr (mbar_base + 232)
    #define o_ready_0_addr (mbar_base + 240)
    #define o_ready_1_addr (mbar_base + 248)
    #define o_empty_0_addr (mbar_base + 256)
    #define o_empty_1_addr (mbar_base + 264)
    #define tmem_dealloc_addr (mbar_base + 272)
    ...
    // Mbarrier init (19 pipeline groups, 0 ordered-sequence groups, 35 barriers)
        ...
        { // scheduler_main
            ...
            for (unsigned int tile_idx_p = blockIdx.x; tile_idx_p < total_tiles_p; tile_idx_p += gridDim.x) {
                ...
                for (int group_base_p = 0; group_base_p < cta_n_blocks_4; group_base_p += 4) {
                    ...
                    int n_block_p = cta_n_blocks_4 - 1 - group_base_p - safe_group_block_p;
                    int page_i_p = lane % 8;
                    ...
                    asm volatile("cp.async.ca.shared::cta.global [%0], [%1], 4, %2;"
                        :: "r"((unsigned int)page_stage_addr_p + lane * 4), "l"(page_table + (pt_base_p + n_block_p * 8 + page_i_p)), "r"((active_page_p) ? 4 : 0));
                    asm volatile(
                        "{\n\t"
                            ...
                            for (int hg = 0; hg < 2; hg++) {
                                int toff = hg * 16384 + pg_i * 2048;
                                {
                                    asm volatile(
                                        "cp.async.bulk.tensor.5d.shared::cta.global.mbarrier::complete_tx::bytes.L2::cache_hint"
                                        " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;"
                                        :: "r"(ldst + toff), "l"(K), "r"(0), "r"(0), "r"(hg), "r"(kv_head_idx_5), "r"(pg0),
                            ...
                            for (int hg_1 = 0; hg_1 < 2; hg_1++) {
                                int vtoff = hg_1 * 16384 + pg_i_1 * 2048;
                                {
                                    asm volatile(
                                        "cp.async.bulk.tensor.5d.shared::cta.global.mbarrier::complete_tx::bytes.L2::cache_hint"
                                        " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;"
                                        :: "r"(vdst + vtoff), "l"(V), "r"(0), "r"(0), "r"(hg_1), "r"(kv_head_idx_5), "r"(vpg0),
                            ...
                            for (int hg_2 = 0; hg_2 < 2; hg_2++) {
                                int ntoff = hg_2 * 16384 + pg_i_2 * 2048;
                                {
                                    asm volatile(
                                        "cp.async.bulk.tensor.5d.shared::cta.global.mbarrier::complete_tx::bytes.L2::cache_hint"
                                        " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;"
                                        :: "r"(kdst + ntoff), "l"(K), "r"(0), "r"(0), "r"(hg_2), "r"(kv_head_idx_5), "r"(npg0),
                            ...
                            for (int hg_3 = 0; hg_3 < 2; hg_3++) {
                                int tvtoff = hg_3 * 16384 + pg_i_3 * 2048;
                                {
                                    asm volatile(
                                        "cp.async.bulk.tensor.5d.shared::cta.global.mbarrier::complete_tx::bytes.L2::cache_hint"
                                        " [%0], [%1, {%2, %3, %4, %5, %6}], [%7], %8;"
                                        :: "r"(tvdst + tvtoff), "l"(V), "r"(0), "r"(0), "r"(hg_3), "r"(kv_head_idx_5), "r"(tvpg0),
