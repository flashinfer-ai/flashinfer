...
#define SMEM_SMEM_P0_OFF 136192
#define SMEM_SMEM_P0_STAGE_BYTES 2048
#define SMEM_SMEM_P0_STRIDE 2048
#define SMEM_SMEM_P1_OFF 138240
#define SMEM_SMEM_P1_STAGE_BYTES 2048
#define SMEM_SMEM_P1_STRIDE 2048
#define SMEM_SMEM_PAGE_OFFSETS_OFF 140288
#define SMEM_SMEM_PAGE_OFFSETS_STAGE_BYTES 128
#define SMEM_SMEM_PAGE_OFFSETS_STRIDE 128
#define SMEM_SMEM_CGA_O_OFF 141056
#define SMEM_SMEM_CGA_O_STAGE_BYTES 32768
#define SMEM_SMEM_CGA_O_STRIDE 32768
#define SMEM_SMEM_CGA_STATS_OFF 145152
#define SMEM_SMEM_CGA_STATS_STAGE_BYTES 128
#define SMEM_SMEM_CGA_STATS_STRIDE 128
#define SMEM_SMEM_EXCH0_OFF 174848
#define SMEM_SMEM_EXCH0_STAGE_BYTES 128
#define SMEM_SMEM_EXCH0_STRIDE 128
#define SMEM_SMEM_EXCH1_OFF 174976
#define SMEM_SMEM_EXCH1_STAGE_BYTES 128
#define SMEM_SMEM_EXCH1_STRIDE 128
#define SMEM_SMEM_EXCH0_U32_OFF 174848
#define SMEM_SMEM_EXCH0_U32_STAGE_BYTES 128
#define SMEM_SMEM_EXCH0_U32_STRIDE 128
#define SMEM_SMEM_EXCH1_U32_OFF 174976
#define SMEM_SMEM_EXCH1_U32_STAGE_BYTES 128
#define SMEM_SMEM_EXCH1_U32_STRIDE 128
#define SMEM_SMEM_CORR0_OFF 175104
#define SMEM_SMEM_CORR0_STAGE_BYTES 64
#define SMEM_SMEM_CORR0_STRIDE 64
#define SMEM_SMEM_CORR1_OFF 175168
#define SMEM_SMEM_CORR1_STAGE_BYTES 64
#define SMEM_SMEM_CORR1_STRIDE 64
#define SMEM_TOTAL 175360
#define THREADS 512
#define BLOCK_N 128
#define HEAD_DIM 128
#define TILE_Q 8
...
__global__ __launch_bounds__(512)
    __cluster_dims__(2, 1, 1) void kernel_cake_fmha_decode_native_bf16(
        ...
        int* __restrict__ page_table, int* __restrict__ causal_seqlens_kv_global,
        float* __restrict__ scale_log2_ptr, float* __restrict__ sinks_ptr, int max_pages_per_seq,
        int max_local_seq_len, float softmax_scale_log2, int window_left, int num_q_heads,
...
#define s_full_0_addr (mbar_base + 192)
#define s_full_1_addr (mbar_base + 200)
#define p_full_0_addr (mbar_base + 208)
#define p_full_1_addr (mbar_base + 216)
#define corr_scale_0_addr (mbar_base + 224)
#define corr_empty_0_addr (mbar_base + 232)
#define corr_empty_1_addr (mbar_base + 240)
#define stats_empty_addr (mbar_base + 248)
#define o_ready_addr (mbar_base + 256)
...
#define reduction_bar_addr (mbar_base + 312)

  const int bid = blockIdx.x;
  const int num_bids = gridDim.x;
  const unsigned int clusters_x = gridDim.x / 2;
  const unsigned int cluster_id =
      ((blockIdx.z * gridDim.y + blockIdx.y) * clusters_x) + blockIdx.x / 2;
  ...
  // Kernel setup ops
  __nv_bfloat16* smem_qt = reinterpret_cast<__nv_bfloat16*>(smem_raw + 1024);
  const int smem_qt_addr = smem + 1024;
  __nv_bfloat16* smem_kv = reinterpret_cast<__nv_bfloat16*>(smem_raw + 5120);
  const int smem_kv_addr = smem + 5120;
  __nv_bfloat16* smem_v = reinterpret_cast<__nv_bfloat16*>(smem_raw + 5120);
  const int smem_v_addr = smem + 5120;
  __nv_bfloat16* smem_p0 = reinterpret_cast<__nv_bfloat16*>(smem_raw + 136192);
  const int smem_p0_addr = smem + 136192;
  __nv_bfloat16* smem_p1 = reinterpret_cast<__nv_bfloat16*>(smem_raw + 138240);
  const int smem_p1_addr = smem + 138240;
  int* smem_page_offsets = reinterpret_cast<int*>(smem_raw + 140288);
  const int smem_page_offsets_addr = smem + 140288;
  __nv_bfloat16* smem_cga_o = reinterpret_cast<__nv_bfloat16*>(smem_raw + 141056);
  const int smem_cga_o_addr = smem + 141056;
  float* smem_cga_stats = reinterpret_cast<float*>(smem_raw + 145152);
  const int smem_cga_stats_addr = smem + 145152;
  float* smem_exch0 = reinterpret_cast<float*>(smem_raw + 174848);
  const int smem_exch0_addr = smem + 174848;
  float* smem_exch1 = reinterpret_cast<float*>(smem_raw + 174976);
  const int smem_exch1_addr = smem + 174976;
  unsigned int* smem_exch0_u32 = reinterpret_cast<unsigned int*>(smem_raw + 174848);
  const int smem_exch0_u32_addr = smem + 174848;
  unsigned int* smem_exch1_u32 = reinterpret_cast<unsigned int*>(smem_raw + 174976);
  const int smem_exch1_u32_addr = smem + 174976;
  float* smem_corr0 = reinterpret_cast<float*>(smem_raw + 175104);
  const int smem_corr0_addr = smem + 175104;
  float* smem_corr1 = reinterpret_cast<float*>(smem_raw + 175168);
  const int smem_corr1_addr = smem + 175168;
  ...
  // Mbarrier init (20 groups, 40 barriers)
  ...
  if (warp == 0) {
    ...
    if (leader) {
      ...
      // reduction_bar: 1 barriers, init_count=1
...
#pragma unroll 1
      for (unsigned int tile_idx_s = blockIdx.x / 2; tile_idx_s < total_tiles_s;
           tile_idx_s += gridDim.x / 2) {
        ...
        int split_start_block = cta_rank * 4;
...
#pragma unroll 1
        for (int pair = 0; pair < 2; pair++) {
          int my_block = split_start_block + 4 - 1 - 2 * pair - is_wg1;
...
#pragma unroll
          for (int c_4 = 0; c_4 < 8; c_4++) {
            ...
            exp_vals[c_4] = sv[c_4] * smx_scale - max_scaled;
          }
          if (is_wg1 != 0) {
          ...
          float _exp2_1 = approx_exp2(exp_vals[0]);
          exp_vals[0] = _exp2_1;
          float _exp2_2 = approx_exp2(exp_vals[1]);
          exp_vals[1] = _exp2_2;
          if (is_wg1 != 0) {
          ...
          op01_phase ^= 1;
#pragma unroll
          for (int c_5 = 2; c_5 < 8; c_5++) {
            float _exp2_3 = approx_exp2(exp_vals[c_5]);
            exp_vals[c_5] = _exp2_3;
          }
          if (kv_pos0 >= visible_local_keys) {
            exp_vals[0] = 0.0f;
            exp_vals[1] = 0.0f;
...
#pragma unroll
  ...
  if (warp >= 8 && warp <= 11) {
    ...
    {  // correction_main
      ...
      if (warp_in_wg_c == 0) {
        if (elect_sync()) {
          mbarrier_arrive_expect_tx(reduction_bar_addr, 4224);
        }
      }
      float smx_scale_c = softmax_scale_log2;
      unsigned int _phase_corr_scale_0_0 = 0;
      unsigned int _phase_o_ready_0 = 0;
      unsigned int _phase_o_ready_1 = 0;
      unsigned int _phase_reduction_bar_0 = 0;
#pragma unroll 1
      for (unsigned int tile_idx_c = blockIdx.x / 2; tile_idx_c < total_tiles_c;
           tile_idx_c += gridDim.x / 2) {
...
#pragma unroll 1
        for (int pair_1 = 1; pair_1 < 2; pair_1++) {
          mbarrier_wait(corr_scale_0_addr, _phase_corr_scale_0_0);
          _phase_corr_scale_0_0 ^= 1;
          asm volatile("tcgen05.fence::after_thread_sync;");
...
#pragma unroll
          for (int c_10 = 0; c_10 < 2; c_10++) {
            ...
            float _exp2_4 = approx_exp2(scaled_diff0);
            acc0_pair[c_10] = ((max_diff0 != 0.0f) ? _exp2_4 : 1.0f);
          }
          mbarrier_arrive(corr_empty_0_addr);
...
#pragma unroll
          for (int c_11 = 0; c_11 < 2; c_11++) {
            ...
            float _exp2_5 = approx_exp2(scaled_diff1);
            acc1_pair[c_11] = ((max_diff1 != 0.0f) ? _exp2_5 : 1.0f);
          }
          mbarrier_arrive(corr_empty_1_addr);
          mbarrier_wait(o_ready_addr + 8, _phase_o_ready_1);
          _phase_o_ready_1 ^= 1;
          asm volatile("tcgen05.fence::after_thread_sync;");
...
#pragma unroll
        for (int c_12 = 0; c_12 < 8; c_12++) {
          float m0 = smem_exch0[c_12];
          float m1 = smem_exch1[c_12];
          float s0 = smem_corr0[c_12];
          float s1 = smem_corr1[c_12];
          float _max_9 = max_noftz(m0, m1);
          float fm = _max_9;
          local_max[c_12] = fm;
          float d0 = smx_scale_c * (m0 - fm);
          float d1 = smx_scale_c * (m1 - fm);
          float _exp2_6 = approx_exp2(d0);
          scale0[c_12] = ((m0 == -CAKE_FMHA_INF) ? 0.0f : _exp2_6);
          float _exp2_7 = approx_exp2(d1);
          scale1[c_12] = ((m1 == -CAKE_FMHA_INF) ? 0.0f : _exp2_7);
          local_sum[c_12] = s0 * scale0[c_12] + s1 * scale1[c_12];
        }
        mbarrier_arrive(corr_empty_0_addr);
        mbarrier_arrive(corr_empty_1_addr);
        mbarrier_wait(o_ready_addr, _phase_o_ready_0);
        ...
        asm volatile(
            "tcgen05.ld.sync.aligned.16x256b.x1.b32"
            " {%0, %1, %2, %3}, [%4];"
            : "=r"(*reinterpret_cast<uint32_t*>(&o0_epi_lo[0])),
              "=r"(*reinterpret_cast<uint32_t*>(&o0_epi_lo[1])),
              "=r"(*reinterpret_cast<uint32_t*>(&o0_epi_lo[2])),
              "=r"(*reinterpret_cast<uint32_t*>(&o0_epi_lo[3]))
            : "r"(taddr + 80));
        asm volatile(
            "tcgen05.ld.sync.aligned.16x256b.x1.b32"
            " {%0, %1, %2, %3}, [%4];"
            : "=r"(*reinterpret_cast<uint32_t*>(&o0_epi_hi[0])),
              "=r"(*reinterpret_cast<uint32_t*>(&o0_epi_hi[1])),
              "=r"(*reinterpret_cast<uint32_t*>(&o0_epi_hi[2])),
              "=r"(*reinterpret_cast<uint32_t*>(&o0_epi_hi[3]))
            : "r"(taddr + 80 + 1048576));
        asm volatile(
            "tcgen05.ld.sync.aligned.16x256b.x1.b32"
            " {%0, %1, %2, %3}, [%4];"
            : "=r"(*reinterpret_cast<uint32_t*>(&o1_epi_lo[0])),
              "=r"(*reinterpret_cast<uint32_t*>(&o1_epi_lo[1])),
              "=r"(*reinterpret_cast<uint32_t*>(&o1_epi_lo[2])),
              "=r"(*reinterpret_cast<uint32_t*>(&o1_epi_lo[3]))
            : "r"(taddr + 88));
        asm volatile(
            "tcgen05.ld.sync.aligned.16x256b.x1.b32"
            " {%0, %1, %2, %3}, [%4];"
            : "=r"(*reinterpret_cast<uint32_t*>(&o1_epi_hi[0])),
              "=r"(*reinterpret_cast<uint32_t*>(&o1_epi_hi[1])),
              "=r"(*reinterpret_cast<uint32_t*>(&o1_epi_hi[2])),
              "=r"(*reinterpret_cast<uint32_t*>(&o1_epi_hi[3]))
            : "r"(taddr + 88 + 1048576));
        asm volatile("tcgen05.wait::ld.sync.aligned;" ::: "memory");
        int head_pair_slot_o = lane % 4;
        float selected_scale0[2];
        float selected_scale1[2];
        selected_scale0[0] = scale0[0];
        selected_scale0[1] = scale0[1];
        selected_scale1[0] = scale1[0];
        selected_scale1[1] = scale1[1];
        if (head_pair_slot_o == 1) {
          selected_scale0[0] = scale0[2];
          selected_scale0[1] = scale0[3];
          selected_scale1[0] = scale1[2];
          selected_scale1[1] = scale1[3];
        }
        if (head_pair_slot_o == 2) {
          selected_scale0[0] = scale0[4];
          selected_scale0[1] = scale0[5];
          selected_scale1[0] = scale1[4];
          selected_scale1[1] = scale1[5];
        }
        if (head_pair_slot_o == 3) {
          selected_scale0[0] = scale0[6];
          selected_scale0[1] = scale0[7];
          selected_scale1[0] = scale1[6];
          selected_scale1[1] = scale1[7];
        }
        float merged_o[8];
#pragma unroll
        for (int h_6 = 0; h_6 < 4; h_6++) {
          merged_o[h_6] =
              o0_epi_lo[h_6] * selected_scale0[h_6 % 2] + o1_epi_lo[h_6] * selected_scale1[h_6 % 2];
          merged_o[h_6 + 4] =
              o0_epi_hi[h_6] * selected_scale0[h_6 % 2] + o1_epi_hi[h_6] * selected_scale1[h_6 % 2];
        }
        uint32_t _mapa_0;
        asm volatile("mapa.shared::cluster.u32 %0, %1, %2;"
                     : "=r"(_mapa_0)
                     : "r"(reduction_bar_addr), "r"(0));
        uint32_t _mapa_1;
        asm volatile("mapa.shared::cluster.u32 %0, %1, %2;"
                     : "=r"(_mapa_1)
                     : "r"(smem_cga_o_addr), "r"(0));
        uint32_t _mapa_2;
        asm volatile("mapa.shared::cluster.u32 %0, %1, %2;"
                     : "=r"(_mapa_2)
                     : "r"(smem_cga_stats_addr), "r"(0));
        unsigned int packed_o[4];
#pragma unroll
        for (int _lp = 0; _lp < 4; _lp++) {
          __nv_bfloat162 _bf2 =
              __float22bfloat162_rn(make_float2(merged_o[_lp * 2 + 0], merged_o[_lp * 2 + 1 + 0]));
          packed_o[_lp] = *(uint32_t*)&_bf2;
        }
        int slice_idx_o = warp_in_wg_c / 2;
        int warp_idx_in_slice_o = warp_in_wg_c % 2;
        int mtx_idx_o = lane / 8;
        int thr_row_idx_o = lane % 8;
        int mtx_col_idx_o = warp_idx_in_slice_o * 4 + mtx_idx_o;
        int seg_col_idx_o = mtx_col_idx_o ^ thr_row_idx_o;
        int stsm_offset_o = slice_idx_o * TILE_Q * 128 + thr_row_idx_o * 128 + seg_col_idx_o * 16;
        const void* _stmatrix_ptr_0 =
            reinterpret_cast<const void*>(reinterpret_cast<uint8_t*>(smem_p0) + stsm_offset_o);
        uint64_t _stmatrix_addr64_0;
        asm volatile("cvta.to.shared.u64 %0, %1;"
                     : "=l"(_stmatrix_addr64_0)
                     : "l"(_stmatrix_ptr_0));
        ...
        asm volatile(
            "stmatrix.sync.aligned.m8n8.x4.trans.shared.b16 [%0], {%1, %2, %3, %4};\n" ::"r"(
                _stmatrix_addr_0),
            "r"(*reinterpret_cast<const uint32_t*>(&packed_o[0])),
            "r"(*reinterpret_cast<const uint32_t*>(&packed_o[1])),
            "r"(*reinterpret_cast<const uint32_t*>(&packed_o[2])),
            "r"(*reinterpret_cast<const uint32_t*>(&packed_o[3]))
            : "memory");
        asm volatile("barrier.sync 3, 128;" ::: "memory");
        int copy_base_o = wg_tid_c * 16;
        int copy_smem_row_o = copy_base_o / 128;
        int copy_smem_col_o = copy_base_o % 128;
        int copy_smem_offset_o = copy_base_o ^ copy_smem_row_o % TILE_Q * 16;
        int copy_dst_row_o = copy_smem_row_o % TILE_Q;
        int copy_dst_col_o = copy_smem_row_o / TILE_Q * 128 + copy_smem_col_o;
        unsigned int copy_vec_o[4];
        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                     : "=r"(*reinterpret_cast<uint32_t*>(&copy_vec_o[0])),
                       "=r"(*reinterpret_cast<uint32_t*>(&copy_vec_o[(0) + 1])),
                       "=r"(*reinterpret_cast<uint32_t*>(&copy_vec_o[(0) + 2])),
                       "=r"(*reinterpret_cast<uint32_t*>(&copy_vec_o[(0) + 3]))
                     : "r"(smem_p0_addr + (unsigned int)copy_smem_offset_o));
        int cga_o_offset = (cta_rank * TILE_Q + copy_dst_row_o) * HEAD_DIM * 2 + copy_dst_col_o;
        asm volatile(
            "st.async.weak.shared::cluster.mbarrier::complete_tx::bytes.v4.b32 [%0], {%1, %2, %3, "
            "%4}, [%5];" ::"r"(_mapa_1 + (unsigned int)cga_o_offset),
            "r"(copy_vec_o[0]), "r"(copy_vec_o[1]), "r"(copy_vec_o[2]), "r"(copy_vec_o[3]),
            "r"(_mapa_0)
            : "memory");
#pragma unroll
        for (int pair_2 = 0; pair_2 < TILE_Q / 2; pair_2++) {
          if (wg_tid_c == pair_2) {
            int cga_stats_offset = (cta_rank * TILE_Q * 2 + pair_2 * 4) * 4;
            asm volatile(
                "st.async.weak.shared::cluster.mbarrier::complete_tx::bytes.v4.b32 [%0], {%1, %2, "
                "%3, %4}, [%5];" ::"r"(_mapa_2 + (unsigned int)cga_stats_offset),
                "r"(__float_as_uint(local_max[pair_2 * 2])),
                "r"(__float_as_uint(local_sum[pair_2 * 2])),
                "r"(__float_as_uint(local_max[pair_2 * 2 + 1])),
                "r"(__float_as_uint(local_sum[pair_2 * 2 + 1])), "r"(_mapa_0)
                : "memory");
          }
        }
        if (cta_rank == 0) {
          mbarrier_wait_cluster(reduction_bar_addr, _phase_reduction_bar_0);
          _phase_reduction_bar_0 ^= 1;
          asm volatile("tcgen05.fence::after_thread_sync;");
#pragma unroll
          for (int h_7 = 0; h_7 < TILE_Q; h_7++) {
            __nv_bfloat16 partial0_o_b = smem_cga_o[h_7 * HEAD_DIM + d_idx];
            __nv_bfloat16 partial1_o_b = smem_cga_o[(TILE_Q + h_7) * HEAD_DIM + d_idx];
            float _cvt_f32_0 = __bfloat162float(partial0_o_b);
            float partial0_o = _cvt_f32_0;
            float _cvt_f32_1 = __bfloat162float(partial1_o_b);
            float partial1_o = _cvt_f32_1;
            float partial0_max = smem_cga_stats[h_7 * 2];
            float partial0_sum = smem_cga_stats[h_7 * 2 + 1];
            float partial1_max = smem_cga_stats[TILE_Q * 2 + h_7 * 2];
            float partial1_sum = smem_cga_stats[TILE_Q * 2 + h_7 * 2 + 1];
            float _max_10 = max_noftz(partial0_max, partial1_max);
            float global_max = _max_10;
            float partial0_weight = 0.0f;
            float partial1_weight = 0.0f;
            if (partial0_max != -CAKE_FMHA_INF) {
              float _exp2_8 = approx_exp2(smx_scale_c * (partial0_max - global_max));
              partial0_weight = _exp2_8;
            }
            if (partial1_max != -CAKE_FMHA_INF) {
              float _exp2_9 = approx_exp2(smx_scale_c * (partial1_max - global_max));
              partial1_weight = _exp2_9;
            }
            float global_sum = partial0_sum * partial0_weight + partial1_sum * partial1_weight;
            float global_o = partial0_o * partial0_weight + partial1_o * partial1_weight;
            float final_o = 0.0f;
            float local_lse = -CAKE_FMHA_INF;
            {
              if (global_sum > 0.0f && global_sum == global_sum) {
                float _rcp_1 = approx_rcp(global_sum);
                final_o = global_o * _rcp_1;
              }
            }
            if (group_ratio_rt > h_7) {
              int q_head = kv_head_idx_1 * group_ratio_rt + h_7;
              int o_idx =
...
#pragma unroll 1
      for (unsigned int tile_idx_m = blockIdx.x / 2; tile_idx_m < total_tiles_m;
           tile_idx_m += gridDim.x / 2) {
        ...
        int _mma_b_lo_0 = make_warp_uniform((((smem_qt_addr) >> 4) & 0x3FFF) + (q_stage_m) * 128);
        asm volatile(
            "{\n\t"
        ...
        int _mma_b_lo_1 = make_warp_uniform((((smem_qt_addr) >> 4) & 0x3FFF) + (q_stage_m) * 128);
        asm volatile(
            "{\n\t"
            ".reg .pred leader, p0, p1;\n\t"
            ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
...
#pragma unroll 1
        for (int pair_3 = 0; pair_3 < 1; pair_3++) {
          ...
          elect_commit2(kv_empty_addr + (s0_1) * 8, o_ready_addr);
          mbarrier_wait(kv_full_addr + (s0_next) * 8, 0);
          int _mma_a_lo_3 = make_warp_uniform((((smem_kv_addr) >> 4) & 0x3FFF) + (s0_next) * 2048);
          int _mma_b_lo_3 = make_warp_uniform((((smem_qt_addr) >> 4) & 0x3FFF) + (q_stage_m) * 128);
          asm volatile(
              "{\n\t"
              ".reg .pred leader, p0, p1;\n\t"
              ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
          ...
          elect_commit2(kv_empty_addr + (s1_1) * 8, o_ready_addr + 8);
          mbarrier_wait(kv_full_addr + (s1_next) * 8, 0);
          int _mma_a_lo_5 = make_warp_uniform((((smem_kv_addr) >> 4) & 0x3FFF) + (s1_next) * 2048);
          int _mma_b_lo_5 = make_warp_uniform((((smem_qt_addr) >> 4) & 0x3FFF) + (q_stage_m) * 128);
          asm volatile(
              "{\n\t"
              ".reg .pred leader, p0, p1;\n\t"
              ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_5),
              "r"(_mma_b_lo_5), "r"(tmem_tmem_s1), "r"(0));
          elect_commit(s_full_1_addr);
        ...
        elect_commit(q_empty_addr + (q_stage_m) * 8);
        q_stage_m += 1;
        if (q_stage_m == 2) {
          q_stage_m = 0;
          q_phase_m ^= 1;
        ...
        elect_commit2(kv_empty_addr + (s0_last) * 8, o_ready_addr);
        mbarrier_wait(o_empty_addr + 8, _phase_o_empty_1);
        _phase_o_empty_1 ^= 1;
        ...
        elect_commit2(kv_empty_addr + (s1_last) * 8, o_ready_addr + 8);
      }
...
#pragma unroll 1
      for (unsigned int tile_idx_p = blockIdx.x / 2; tile_idx_p < total_tiles_p;
           tile_idx_p += gridDim.x / 2) {
...
#pragma unroll 1
        for (int group_base_p = 0; group_base_p < 4; group_base_p += 4) {
          int group_rem_p = 4 - group_base_p;
          ...
          int n_block_p = cta_rank * 4 + 4 - 1 - group_base_p - safe_group_block_p;
          int page_i_p = lane % 8;
...
#pragma unroll 1
      for (unsigned int tile_idx_l = blockIdx.x / 2; tile_idx_l < total_tiles_l;
           tile_idx_l += gridDim.x / 2) {
        ...
        int split_start_block_1 = cta_rank * 4;
        mbarrier_wait(q_empty_addr + (q_stage_l) * 8, q_phase_l);
        if (elect_sync()) {
          const int group_ratio_l = NUM_Q_HEADS / NUM_KV_HEADS;
          int off_qt =
              (batch_idx_4 * Q_LEN + q_row_idx_4) * NUM_Q_HEADS + kv_head_idx_4 * group_ratio_l;
          mbarrier_arrive_expect_tx(q_full_addr + (q_stage_l) * 8, TILE_Q * HEAD_DIM * 2);
          tma_3d_gmem2smem(smem_qt_addr + (unsigned int)(q_stage_l * 2048), Qt, 0, off_qt, 0,
                           q_full_addr + (q_stage_l) * 8);
          q_stage_l += 1;
          if (q_stage_l == 2) {
            q_stage_l = 0;
            q_phase_l ^= 1;
...
#pragma unroll 1
          for (int ni = 0; ni < prefill; ni++) {
            int n_block = split_start_block_1 + 4 - 1 - ni;
            ...
            int page_addr = smem_page_offsets_addr + (unsigned int)(page_stage_k * 128) +
                            (unsigned int)(ni % 4 * 8 * 4);
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
...
#pragma unroll 1
          for (int ni_1 = 0; ni_1 < main_ni; ni_1++) {
            int stage = ni_1 % 4;
            int n_block_1 = split_start_block_1 + 4 - 1 - ni_1;
            ...
            int page_addr_1 = smem_page_offsets_addr + (unsigned int)(page_stage_v * 128) +
                              (unsigned int)(ni_1 % 4 * 8 * 4);
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
...
#pragma unroll
            ...
            int next_n = split_start_block_1 + 4 - 1 - next_ni;
            ...
            int page_addr_0 = smem_page_offsets_addr + (unsigned int)(page_stage_nk * 128) +
...
#pragma unroll 1
          for (int tail_ni = main_ni; tail_ni < 4; tail_ni++) {
            int tstage = tail_ni % 4;
            int tn_block = split_start_block_1 + 4 - 1 - tail_ni;
            ...
            int page_addr_2 = smem_page_offsets_addr + (unsigned int)(page_stage_tv * 128) +
                              (unsigned int)(tail_ni % 4 * 8 * 4);
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                         ...
                         : "r"(page_addr_2));
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&tpg_v[4])),
                           "=r"(*reinterpret_cast<uint32_t*>(&tpg_v[(4) + 1])),
                           "=r"(*reinterpret_cast<uint32_t*>(&tpg_v[(4) + 2])),
                           "=r"(*reinterpret_cast<uint32_t*>(&tpg_v[(4) + 3]))
                         : "r"(page_addr_2 + 16));
            int page_group_done_tail = ((tail_ni % 4 == 3) ? 1 : 0);
            page_group_done_tail = page_group_done_tail | ((tail_ni + 1 == 4) ? 1 : 0);
            if (page_group_done_tail != 0) {
              mbarrier_arrive(page_empty_addr + (page_stage_tv) * 8);
            }
#pragma unroll
            for (int pg_i_3 = 0; pg_i_3 < 8; pg_i_3++) {
              int tvpg0 = tpg_v[pg_i_3];
#pragma unroll
