...
kernel_cake_fmha_decode_native_bf16(CakeFmhaTensorMap const* Qt, CakeFmhaTensorMap const* K, CakeFmhaTensorMap const* V, __nv_bfloat16* __restrict__ O_ptr, float* __restrict__ LSE_ptr, int* __restrict__ page_table, int* __restrict__ causal_seqlens_kv_global, float* __restrict__ scale_log2_ptr, float* __restrict__ sinks_ptr, int max_pages_per_seq, int max_local_seq_len, float softmax_scale_log2, int window_left, int num_q_heads, int num_kv_heads, int batch_size)
{
    ...
    // Mbarrier init (20 pipeline groups, 0 ordered-sequence groups, 40 barriers)
    ...
    if (warp <= 7) {
        ...
        { // softmax_main
            ...
            #pragma unroll 1
            for (unsigned int tile_idx_s = blockIdx.x / 2; tile_idx_s < total_tiles_s; tile_idx_s += gridDim.x / 2) {
    ...
    if (warp >= 8 && warp <= 11) {
        ...
        { // correction_main
            ...
            #pragma unroll 1
            for (unsigned int tile_idx_c = blockIdx.x / 2; tile_idx_c < total_tiles_c; tile_idx_c += gridDim.x / 2) {
                ...
                asm volatile(
                    "tcgen05.ld.sync.aligned.16x256b.x1.b32"
                    " {%0, %1, %2, %3}, [%4];"
                    : "=r"(*reinterpret_cast<uint32_t*>(&o0_epi_lo[0])), "=r"(*reinterpret_cast<uint32_t*>(&o0_epi_lo[1])), "=r"(*reinterpret_cast<uint32_t*>(&o0_epi_lo[2])), "=r"(*reinterpret_cast<uint32_t*>(&o0_epi_lo[3]))
                    : "r"(taddr + 80));
                asm volatile(
                    "tcgen05.ld.sync.aligned.16x256b.x1.b32"
                    " {%0, %1, %2, %3}, [%4];"
                    : "=r"(*reinterpret_cast<uint32_t*>(&o0_epi_hi[0])), "=r"(*reinterpret_cast<uint32_t*>(&o0_epi_hi[1])), "=r"(*reinterpret_cast<uint32_t*>(&o0_epi_hi[2])), "=r"(*reinterpret_cast<uint32_t*>(&o0_epi_hi[3]))
                    : "r"(taddr + 80 + 1048576));
                asm volatile(
                    "tcgen05.ld.sync.aligned.16x256b.x1.b32"
                    " {%0, %1, %2, %3}, [%4];"
                    : "=r"(*reinterpret_cast<uint32_t*>(&o1_epi_lo[0])), "=r"(*reinterpret_cast<uint32_t*>(&o1_epi_lo[1])), "=r"(*reinterpret_cast<uint32_t*>(&o1_epi_lo[2])), "=r"(*reinterpret_cast<uint32_t*>(&o1_epi_lo[3]))
                    : "r"(taddr + 88));
                asm volatile(
                    "tcgen05.ld.sync.aligned.16x256b.x1.b32"
                    " {%0, %1, %2, %3}, [%4];"
                    : "=r"(*reinterpret_cast<uint32_t*>(&o1_epi_hi[0])), "=r"(*reinterpret_cast<uint32_t*>(&o1_epi_hi[1])), "=r"(*reinterpret_cast<uint32_t*>(&o1_epi_hi[2])), "=r"(*reinterpret_cast<uint32_t*>(&o1_epi_hi[3]))
                    : "r"(taddr + 88 + 1048576));
                ...
                for (int h_6 = 0; h_6 < 4; h_6++) {
                    merged_o[h_6] = o0_epi_lo[h_6] * selected_scale0[h_6 % 2] + o1_epi_lo[h_6] * selected_scale1[h_6 % 2];
                    merged_o[h_6 + 4] = o0_epi_hi[h_6] * selected_scale0[h_6 % 2] + o1_epi_hi[h_6] * selected_scale1[h_6 % 2];
                }
                uint32_t _mapa_0;
                asm volatile(
                    "mapa.shared::cluster.u32 %0, %1, %2;"
                    : "=r"(_mapa_0) : "r"(reduction_bar_addr), "r"(0));
                uint32_t _mapa_1;
                asm volatile(
                    "mapa.shared::cluster.u32 %0, %1, %2;"
                    : "=r"(_mapa_1) : "r"(smem_cga_o_addr), "r"(0));
                uint32_t _mapa_2;
                asm volatile(
                    "mapa.shared::cluster.u32 %0, %1, %2;"
                    : "=r"(_mapa_2) : "r"(smem_cga_stats_addr), "r"(0));
                unsigned int packed_o[4];
                #pragma unroll
                for (int _lp = 0; _lp < 4; _lp++) {
                    __nv_bfloat162 _bf2 = __float22bfloat162_rn(make_float2(merged_o[_lp*2 + 0], merged_o[_lp*2+1 + 0]));
                ...
                const void* _stmatrix_ptr_0 = reinterpret_cast<const void*>(reinterpret_cast<uint8_t*>(smem_p0) + stsm_offset_o);
                ...
                asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                    : "=r"(*reinterpret_cast<uint32_t*>(&copy_vec_o[0])), "=r"(*reinterpret_cast<uint32_t*>(&copy_vec_o[(0) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&copy_vec_o[(0) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&copy_vec_o[(0) + 3]))
                ...
                asm volatile(
                    ...
                    :: "r"(_mapa_1 + (unsigned int)cga_o_offset), "r"(copy_vec_o[0]), "r"(copy_vec_o[1]), "r"(copy_vec_o[2]), "r"(copy_vec_o[3]), "r"(_mapa_0) : "memory");
                #pragma unroll
                for (int pair_2 = 0; pair_2 < TILE_Q / 2; pair_2++) {
                    if (wg_tid_c == pair_2) {
                        ...
                        asm volatile(
                            "st.async.weak.shared::cluster.mbarrier::complete_tx::bytes.v4.b32 [%0], {%1, %2, %3, %4}, [%5];"
                            :: "r"(_mapa_2 + (unsigned int)cga_stats_offset), "r"(__float_as_uint(local_max[pair_2 * 2])), "r"(__float_as_uint(local_sum[pair_2 * 2])), "r"(__float_as_uint(local_max[pair_2 * 2 + 1])), "r"(__float_as_uint(local_sum[pair_2 * 2 + 1])), "r"(_mapa_0) : "memory");
                    }
                }
                if (cta_rank == 0) {
                    mbarrier_wait_cluster_hint(reduction_bar_addr, _phase_reduction_bar_0, 10000000);
    ...
    if (warp == 12) {
        { // mma_warp_main
            ...
            #pragma unroll 1
            for (unsigned int tile_idx_m = blockIdx.x / 2; tile_idx_m < total_tiles_m; tile_idx_m += gridDim.x / 2) {
                ...
                for (int pair_3 = 0; pair_3 < 1; pair_3++) {
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
                    "}\n"
                    :: "r"(_mma_a_lo_5), "r"(_mma_b_lo_5), "r"(tmem_tmem_s1), "r"(0));
                    elect_commit(s_full_1_addr);
                ...
                q_stage_m += 1;
                if (q_stage_m == 2) { q_stage_m = 0; q_phase_m ^= 1; }
    ...
    if (warp == 13) {
        { // load_pgoff_main
            ...
            #pragma unroll 1
            for (unsigned int tile_idx_p = blockIdx.x / 2; tile_idx_p < total_tiles_p; tile_idx_p += gridDim.x / 2) {
    ...
    if (warp == 15) {
        { // load_warp_main
            ...
            #pragma unroll 1
            for (unsigned int tile_idx_l = blockIdx.x / 2; tile_idx_l < total_tiles_l; tile_idx_l += gridDim.x / 2) {
                ...
                if (elect_sync()) {
                    ...
                    tma_3d_gmem2smem(smem_qt_addr + (unsigned int)(q_stage_l * 2048), Qt, 0, off_qt, 0, q_full_addr + (q_stage_l) * 8);
                    q_stage_l += 1;
                    if (q_stage_l == 2) { q_stage_l = 0; q_phase_l ^= 1; }
                    ...
                    for (int ni = 0; ni < prefill; ni++) {
                        ...
                        int page_addr = smem_page_offsets_addr + (unsigned int)(page_stage_k * 128) + (unsigned int)(ni % 4 * 8 * 4);
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                    ...
                    for (int ni_1 = 0; ni_1 < main_ni; ni_1++) {
                        ...
                        int page_addr_1 = smem_page_offsets_addr + (unsigned int)(page_stage_v * 128) + (unsigned int)(ni_1 % 4 * 8 * 4);
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                        ...
                        int page_addr_0 = smem_page_offsets_addr + (unsigned int)(page_stage_nk * 128) + (unsigned int)(next_ni % 4 * 8 * 4);
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                    ...
                    for (int tail_ni = main_ni; tail_ni < 4; tail_ni++) {
                        ...
                        int page_addr_2 = smem_page_offsets_addr + (unsigned int)(page_stage_tv * 128) + (unsigned int)(tail_ni % 4 * 8 * 4);
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                            ...
                            : "r"(page_addr_2));
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                            : "=r"(*reinterpret_cast<uint32_t*>(&tpg_v[4])), "=r"(*reinterpret_cast<uint32_t*>(&tpg_v[(4) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&tpg_v[(4) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&tpg_v[(4) + 3]))
                            : "r"(page_addr_2 + 16));
