...
__global__ __launch_bounds__(512, 1) void kernel_cake_fmha_decode_native_bf16(
...
#pragma unroll
        ...
        tile_idx_s =
            (_clc_ctaid_14 * (unsigned int)Q_LEN + _clc_ctaid_12) * (unsigned int)NUM_KV_HEADS +
            _clc_ctaid_13;
      }
    }
  }
...
#pragma unroll
        ...
        tile_idx_c =
            (_clc_ctaid_17 * (unsigned int)Q_LEN + _clc_ctaid_15) * (unsigned int)NUM_KV_HEADS +
            _clc_ctaid_16;
      }
...
#pragma unroll 1
        ...
        tile_idx_m =
            (_clc_ctaid_11 * (unsigned int)Q_LEN + _clc_ctaid_9) * (unsigned int)NUM_KV_HEADS +
            _clc_ctaid_10;
      }
...
#pragma unroll 1
        for (int group_pair_base_p = 0; group_pair_base_p < 32; group_pair_base_p += 8) {
          ...
          asm volatile(
              "cp.async.ca.shared::cta.global [%0], [%1], 4;" ::"r"(page_stage_addr_p0 + lane * 4),
              "l"(page_table + (pt_base_p + n_block_p0 * 8 + page_i_p)));
          asm volatile(
              "{\n\t"
          ...
          asm volatile(
              "cp.async.ca.shared::cta.global [%0], [%1], 4;" ::"r"(page_stage_addr_p1 + lane * 4),
              "l"(page_table + (pt_base_p + n_block_p1 * 8 + page_i_p)));
          asm volatile(
              "{\n\t"
              "cp.async.mbarrier.arrive.shared::cta.b64 [%0];\n\t"
              "}" ::"r"(page_full_addr + (page_slot_p) * 8)
        ...
        tile_idx_p =
            (_clc_ctaid_2 * (unsigned int)Q_LEN + _clc_ctaid_0) * (unsigned int)NUM_KV_HEADS +
            _clc_ctaid_1;
      }
    }
  }
...
#pragma unroll 1
      for (unsigned int _tile_iter_sched = 0; _tile_iter_sched < total_tiles_sched;
           _tile_iter_sched++) {
        ...
        if (throttle_stage_sched == 2) {
          throttle_stage_sched = 0;
        ...
        if (elect_sync()) {
          ...
          asm volatile(
              ...
              " [%0], [%1];" ::"r"(work_response_addr + work_stage_sched * 16),
              "r"(work_full_addr + work_stage_sched * 8)
              : "memory");
        }
...
#pragma unroll 1
          for (int ni = 0; ni < prefill; ni++) {
            ...
            int page_ids[4];
            int page_addr_0 = smem_page_offsets_addr + (unsigned int)(page_stage_k * 128) +
                              (unsigned int)(ni % 4 * 8 * 4);
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&page_ids[0])),
                           "=r"(*reinterpret_cast<uint32_t*>(&page_ids[(0) + 1])),
                           "=r"(*reinterpret_cast<uint32_t*>(&page_ids[(0) + 2])),
                           "=r"(*reinterpret_cast<uint32_t*>(&page_ids[(0) + 3]))
                         : "r"(page_addr_0));
#pragma unroll
...
#pragma unroll
              for (int head_dim_half = 0; head_dim_half < 2; head_dim_half++) {
                ...
                {
                  tma_5d_gmem2smem(ldst + byte_offset, K, 0, 0, head_dim_half, kv_head_idx_4,
                                   page_id, kv_full_addr + (kv_stage) * 8);
                }
              }
            }
#pragma unroll
...
#pragma unroll 1
          for (int ni_1 = 0; ni_1 < main_ni; ni_1++) {
            ...
            int page_ids_1[4];
            int page_addr_0_1 = smem_page_offsets_addr + (unsigned int)(page_stage_v * 128) +
                                (unsigned int)(ni_1 % 4 * 8 * 4);
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&page_ids_1[0])),
                           "=r"(*reinterpret_cast<uint32_t*>(&page_ids_1[(0) + 1])),
                           "=r"(*reinterpret_cast<uint32_t*>(&page_ids_1[(0) + 2])),
                           "=r"(*reinterpret_cast<uint32_t*>(&page_ids_1[(0) + 3]))
                         : "r"(page_addr_0_1));
#pragma unroll
...
#pragma unroll
              for (int head_dim_half_1 = 0; head_dim_half_1 < 2; head_dim_half_1++) {
                ...
                {
                  tma_5d_gmem2smem(vdst + byte_offset_1, V, 0, 0, head_dim_half_1, kv_head_idx_4,
                                   page_id_1, kv_full_addr + (stage) * 8);
                }
              }
            }
#pragma unroll
...
#pragma unroll
            ...
            int page_addr_1_1 = smem_page_offsets_addr + (unsigned int)(page_stage_nk * 128) +
            ...
            int page_ids_2[4];
            int page_addr_3 = smem_page_offsets_addr + (unsigned int)(page_stage_nk * 128) +
                              (unsigned int)(next_ni % 4 * 8 * 4);
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&page_ids_2[0])),
                           "=r"(*reinterpret_cast<uint32_t*>(&page_ids_2[(0) + 1])),
                           "=r"(*reinterpret_cast<uint32_t*>(&page_ids_2[(0) + 2])),
                           "=r"(*reinterpret_cast<uint32_t*>(&page_ids_2[(0) + 3]))
                         : "r"(page_addr_3));
#pragma unroll
...
#pragma unroll
              for (int head_dim_half_2 = 0; head_dim_half_2 < 2; head_dim_half_2++) {
                ...
                {
                  tma_5d_gmem2smem(kdst + byte_offset_2, K, 0, 0, head_dim_half_2, kv_head_idx_4,
                                   page_id_2, kv_full_addr + (k_stage) * 8);
                }
              }
            }
#pragma unroll
...
#pragma unroll 1
          for (int tail_ni = main_ni; tail_ni < cta_n_blocks_3; tail_ni++) {
            ...
            int page_ids_3[4];
            int page_addr_0_2 = smem_page_offsets_addr + (unsigned int)(page_stage_tv * 128) +
                                (unsigned int)(tail_ni % 4 * 8 * 4);
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&page_ids_3[0])),
                           "=r"(*reinterpret_cast<uint32_t*>(&page_ids_3[(0) + 1])),
                           "=r"(*reinterpret_cast<uint32_t*>(&page_ids_3[(0) + 2])),
                           "=r"(*reinterpret_cast<uint32_t*>(&page_ids_3[(0) + 3]))
                         : "r"(page_addr_0_2));
#pragma unroll
...
#pragma unroll
              for (int head_dim_half_3 = 0; head_dim_half_3 < 2; head_dim_half_3++) {
                ...
                {
                  tma_5d_gmem2smem(tvdst + byte_offset_3, V, 0, 0, head_dim_half_3, kv_head_idx_4,
                                   page_id_3, kv_full_addr + (tstage) * 8);
                }
              }
            }
#pragma unroll
...
#pragma unroll
        ...
        tile_idx_l =
            (_clc_ctaid_8 * (unsigned int)Q_LEN + _clc_ctaid_6) * (unsigned int)NUM_KV_HEADS +
            _clc_ctaid_7;
      }
    }
  }
