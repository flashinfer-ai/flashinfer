...
kernel_cake_fmha_context_fp8_hd256(CakeFmhaTensorMap const* Q, CakeFmhaTensorMap const* K, CakeFmhaTensorMap const* V, uint8_t* __restrict__ O_ptr, int* __restrict__ page_table, int* __restrict__ seq_lens_q, int* __restrict__ seq_lens_kv, int* __restrict__ cu_seq_lens_q, float softmax_scale_log2, float output_scale, int total_bh, int max_pages_per_seq, unsigned int* __restrict__ dynamic_counter)
{
    ...
    // Mbarrier init (14 groups, 30 barriers)
        ...
        { // softmax_main
            ...
            for (unsigned int tile_iter = 0; tile_iter < total_tiles; tile_iter++) {
                ...
                unsigned int num_masked_iters;
                {
                    ...
                    causal_row = m_block * (unsigned int)BLOCK_M + (unsigned int)(warp % 4 * 32 + lane) + (unsigned int)causal_shift_1;
