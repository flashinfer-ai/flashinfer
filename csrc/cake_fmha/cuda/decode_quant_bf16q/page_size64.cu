...
#define SMEM_SMEM_QT_LO_OFF 2688
...
__global__ __launch_bounds__(512, 1) void kernel_cake_fmha_decode_quant_bf16q(
    unsigned int* __restrict__ Q_ptr, CakeFmhaTensorMap const* K, CakeFmhaTensorMap const* V,
    __nv_bfloat16* __restrict__ O, int* __restrict__ page_table, int* __restrict__ seq_lens_kv,
    float* __restrict__ bmm1_scale_ptr, float* __restrict__ bmm2_scale_ptr,
    float* __restrict__ partial_O, float* __restrict__ partial_max, float* __restrict__ partial_sum,
    int pt_batch_stride, int pt_v_offset, int bmm1_is_log2, int num_splits, int blocks_per_split) {
...
#define s_full_0_addr (mbar_base + 144)
#define s_full_1_addr (mbar_base + 152)
#define s_empty_0_addr (mbar_base + 160)
#define s_empty_1_addr (mbar_base + 168)
#define o_free_0_addr (mbar_base + 176)
#define o_free_1_addr (mbar_base + 184)
#define o_done_0_addr (mbar_base + 192)
#define o_done_1_addr (mbar_base + 200)
#define corr_scale_0_addr (mbar_base + 208)
#define corr_scale_1_addr (mbar_base + 216)
#define corr_empty_0_addr (mbar_base + 224)
#define corr_empty_1_addr (mbar_base + 232)
#define stats_empty_addr (mbar_base + 240)
#define tmem_dealloc_addr (mbar_base + 248)
#define order_p01_0_addr (mbar_base + 256)
#define order_p01_1_addr (mbar_base + 264)
  ...
  uint8_t* smem_qt_lo = reinterpret_cast<uint8_t*>(smem_raw + 2688);
  const int smem_qt_lo_addr = smem + 2688;
  ...
  // Mbarrier init (20 groups, 34 barriers)
  // Mbarriers at smem_raw[0..272)

  if (warp == 0) {
    uint32_t leader = elect_sync();
    if (leader) {
      // q_full: 1 barriers, init_count=32
  ...
  if (warp <= 7) {
    ...
    {  // softmax_main
      ...
      int my_tmem_stats =
          taddr + (unsigned int)(((is_wg1 != 0) ? 48 : 16)) + (unsigned int)(tmem_row_base_v << 16);
...
#pragma unroll 1
      for (unsigned int tile_idx_s = blockIdx.x; tile_idx_s < total_tiles_s;
           tile_idx_s += gridDim.x) {
        ...
        int split_start_block = split_s * blocks_per_split;
        ...
        asm volatile(
            ...
            " {%0, %1, %2, %3}, [%4];"
            : "=r"(*reinterpret_cast<uint32_t*>(&sv_lo[0])),
              "=r"(*reinterpret_cast<uint32_t*>(&sv_lo[1])),
              "=r"(*reinterpret_cast<uint32_t*>(&sv_lo[2])),
              "=r"(*reinterpret_cast<uint32_t*>(&sv_lo[3]))
            : "r"(my_tmem_s_base));
        asm volatile(
            "tcgen05.ld.sync.aligned.16x256b.x1.b32"
            " {%0, %1, %2, %3}, [%4];"
            : "=r"(*reinterpret_cast<uint32_t*>(&sv_hi[0])),
              "=r"(*reinterpret_cast<uint32_t*>(&sv_hi[1])),
              "=r"(*reinterpret_cast<uint32_t*>(&sv_hi[2])),
              "=r"(*reinterpret_cast<uint32_t*>(&sv_hi[3]))
...
#pragma unroll 1
        for (int pair = 0; pair < num_pairs; pair++) {
          ...
          if (seqlen_kv <= block_base + (kv_row0 ^ 80)) {
            sv[0] = -3.4028235e+38f;
          }
          if (seqlen_kv <= block_base + (kv_row0 ^ mask_x_odd)) {
            sv[1] = -3.4028235e+38f;
          }
          if (seqlen_kv <= block_base + (kv_row1 ^ 80)) {
            sv[2] = -3.4028235e+38f;
          }
          if (seqlen_kv <= block_base + (kv_row1 ^ mask_x_odd)) {
            sv[3] = -3.4028235e+38f;
          }
          if (seqlen_kv <= block_base + (kv_row2 ^ 80)) {
            sv[4] = -3.4028235e+38f;
          }
          if (seqlen_kv <= block_base + (kv_row2 ^ mask_x_odd)) {
            sv[5] = -3.4028235e+38f;
          }
          if (seqlen_kv <= block_base + (kv_row3 ^ 80)) {
            sv[6] = -3.4028235e+38f;
          }
          if (seqlen_kv <= block_base + (kv_row3 ^ mask_x_odd)) {
            sv[7] = -3.4028235e+38f;
          }
          float pair_max[2];
...
#pragma unroll
          ...
          unsigned int regs_p[2];
          {
            ...
            asm volatile(
                ...
                "}"
                : "=r"(_packed)
                : "f"(exp_vals[0]), "f"(exp_vals[1]), "f"(exp_vals[2]), "f"(exp_vals[3]));
            regs_p[0] = _packed;
          }
          {
            uint32_t _packed;
            asm volatile(
                ...
                "}"
                : "=r"(_packed)
                : "f"(exp_vals[4]), "f"(exp_vals[5]), "f"(exp_vals[6]), "f"(exp_vals[7]));
            regs_p[1] = _packed;
          }
          int mtx_idx = lane / 8;
          ...
          const void* _stmatrix_b8_ptr_5 =
              reinterpret_cast<const void*>(reinterpret_cast<uint8_t*>(my_p_base) + stsm_offset);
          uint64_t _stmatrix_b8_addr64_5;
          asm volatile("cvta.to.shared.u64 %0, %1;"
                       : "=l"(_stmatrix_b8_addr64_5)
                       : "l"(_stmatrix_b8_ptr_5));
          uint32_t _stmatrix_b8_addr_5;
          asm volatile("cvt.u32.u64 %0, %1;"
                       : "=r"(_stmatrix_b8_addr_5)
                       : "l"(_stmatrix_b8_addr64_5));
          asm volatile("stmatrix.sync.aligned.m16n8.x2.trans.shared.b8 [%0], {%1, %2};\n" ::"r"(
                           _stmatrix_b8_addr_5),
                       "r"(regs_p[0]), "r"(regs_p[1])
                       : "memory");
          asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
...
#pragma unroll 1
      for (unsigned int tile_idx_c = blockIdx.x; tile_idx_c < total_tiles_c;
           tile_idx_c += gridDim.x) {
        ...
        int split_start_block_1 = split_c * blocks_per_split;
        ...
        for (int pair_1 = 1; pair_1 < num_pairs_1; pair_1++) {
          ...
          if (_vote_0 != 0) {
            ...
            asm volatile(
                "tcgen05.ld.sync.aligned.16x256b.x1.b32"
                " {%0, %1, %2, %3}, [%4];"
                : "=r"(*reinterpret_cast<uint32_t*>(&o0_lo[0])),
                  "=r"(*reinterpret_cast<uint32_t*>(&o0_lo[1])),
                  "=r"(*reinterpret_cast<uint32_t*>(&o0_lo[2])),
                  "=r"(*reinterpret_cast<uint32_t*>(&o0_lo[3]))
                : "r"(taddr + 80));
            asm volatile(
                "tcgen05.ld.sync.aligned.16x256b.x1.b32"
                " {%0, %1, %2, %3}, [%4];"
                : "=r"(*reinterpret_cast<uint32_t*>(&o0_hi[0])),
                  "=r"(*reinterpret_cast<uint32_t*>(&o0_hi[1])),
                  "=r"(*reinterpret_cast<uint32_t*>(&o0_hi[2])),
                  "=r"(*reinterpret_cast<uint32_t*>(&o0_hi[3]))
                : "r"(taddr + 80 + 1048576));
            float o0[8];
#pragma unroll
            for (int h = 0; h < 4; h++) {
...
#pragma unroll
            ...
            asm volatile(
                ...
                " [%0], {%1, %2, %3, %4};" ::"r"(taddr + 80),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_lo[0])),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_lo[1])),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_lo[2])),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_lo[3])));
            asm volatile(
                "tcgen05.st.sync.aligned.16x256b.x1.b32"
                " [%0], {%1, %2, %3, %4};" ::"r"(taddr + 80 + 1048576),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_hi[0])),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_hi[1])),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_hi[2])),
                "r"(*reinterpret_cast<const uint32_t*>(&o0_hi[3])));
            asm volatile("tcgen05.wait::st.sync.aligned;" ::: "memory");
          }
          mbarrier_arrive(o_free_0_addr);
          mbarrier_wait(corr_scale_1_addr, _phase_corr_scale_1_0);
          ...
          if (_vote_1 != 0) {
            ...
            asm volatile(
                "tcgen05.ld.sync.aligned.16x256b.x1.b32"
                " {%0, %1, %2, %3}, [%4];"
                : "=r"(*reinterpret_cast<uint32_t*>(&o1_lo[0])),
                  "=r"(*reinterpret_cast<uint32_t*>(&o1_lo[1])),
                  "=r"(*reinterpret_cast<uint32_t*>(&o1_lo[2])),
                  "=r"(*reinterpret_cast<uint32_t*>(&o1_lo[3]))
                : "r"(taddr + 88));
            asm volatile(
                "tcgen05.ld.sync.aligned.16x256b.x1.b32"
                " {%0, %1, %2, %3}, [%4];"
                : "=r"(*reinterpret_cast<uint32_t*>(&o1_hi[0])),
                  "=r"(*reinterpret_cast<uint32_t*>(&o1_hi[1])),
                  "=r"(*reinterpret_cast<uint32_t*>(&o1_hi[2])),
                  "=r"(*reinterpret_cast<uint32_t*>(&o1_hi[3]))
                : "r"(taddr + 88 + 1048576));
            float o1[8];
#pragma unroll
...
#pragma unroll
            ...
            asm volatile(
                "tcgen05.st.sync.aligned.16x256b.x1.b32"
                " [%0], {%1, %2, %3, %4};" ::"r"(taddr + 88),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_lo[0])),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_lo[1])),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_lo[2])),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_lo[3])));
            asm volatile(
                "tcgen05.st.sync.aligned.16x256b.x1.b32"
                " [%0], {%1, %2, %3, %4};" ::"r"(taddr + 88 + 1048576),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_hi[0])),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_hi[1])),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_hi[2])),
                "r"(*reinterpret_cast<const uint32_t*>(&o1_hi[3])));
            asm volatile("tcgen05.wait::st.sync.aligned;" ::: "memory");
          }
          mbarrier_arrive(o_free_1_addr);
        }
...
#pragma unroll
        ...
        asm volatile(
            "tcgen05.ld.sync.aligned.16x256b.x1.b32"
            " {%0, %1, %2, %3}, [%4];"
            : "=r"(*reinterpret_cast<uint32_t*>(&o0_lo_epi[0])),
              "=r"(*reinterpret_cast<uint32_t*>(&o0_lo_epi[1])),
              "=r"(*reinterpret_cast<uint32_t*>(&o0_lo_epi[2])),
              "=r"(*reinterpret_cast<uint32_t*>(&o0_lo_epi[3]))
            : "r"(taddr + 80));
        asm volatile(
            "tcgen05.ld.sync.aligned.16x256b.x1.b32"
            " {%0, %1, %2, %3}, [%4];"
            : "=r"(*reinterpret_cast<uint32_t*>(&o0_hi_epi[0])),
              "=r"(*reinterpret_cast<uint32_t*>(&o0_hi_epi[1])),
              "=r"(*reinterpret_cast<uint32_t*>(&o0_hi_epi[2])),
              "=r"(*reinterpret_cast<uint32_t*>(&o0_hi_epi[3]))
            : "r"(taddr + 80 + 1048576));
        asm volatile(
            "tcgen05.ld.sync.aligned.16x256b.x1.b32"
            " {%0, %1, %2, %3}, [%4];"
            : "=r"(*reinterpret_cast<uint32_t*>(&o1_lo_epi[0])),
              "=r"(*reinterpret_cast<uint32_t*>(&o1_lo_epi[1])),
              "=r"(*reinterpret_cast<uint32_t*>(&o1_lo_epi[2])),
              "=r"(*reinterpret_cast<uint32_t*>(&o1_lo_epi[3]))
            : "r"(taddr + 88));
        asm volatile(
            "tcgen05.ld.sync.aligned.16x256b.x1.b32"
            " {%0, %1, %2, %3}, [%4];"
            : "=r"(*reinterpret_cast<uint32_t*>(&o1_hi_epi[0])),
              "=r"(*reinterpret_cast<uint32_t*>(&o1_hi_epi[1])),
              "=r"(*reinterpret_cast<uint32_t*>(&o1_hi_epi[2])),
              "=r"(*reinterpret_cast<uint32_t*>(&o1_hi_epi[3]))
            : "r"(taddr + 88 + 1048576));
        if (num_splits > 1) {
          float part_vals[8];
          ...
          if (q_even < group_ratio_rt) {
            *(reinterpret_cast<float*>(partial_O + (po_even + corr_row_e)) + (0)) = part_vals[0];
            *(reinterpret_cast<float*>(partial_O + (po_even + (corr_row_e + 8))) + (0)) =
                part_vals[2];
            *(reinterpret_cast<float*>(partial_O + (po_even + (corr_row_e + 16))) + (0)) =
                part_vals[4];
            *(reinterpret_cast<float*>(partial_O + (po_even + (corr_row_e + 24))) + (0)) =
                part_vals[6];
          }
          if (q_odd < group_ratio_rt) {
            *(reinterpret_cast<float*>(partial_O + (po_odd + corr_row_e)) + (0)) = part_vals[1];
            *(reinterpret_cast<float*>(partial_O + (po_odd + (corr_row_e + 8))) + (0)) =
                part_vals[3];
            *(reinterpret_cast<float*>(partial_O + (po_odd + (corr_row_e + 16))) + (0)) =
                part_vals[5];
            *(reinterpret_cast<float*>(partial_O + (po_odd + (corr_row_e + 24))) + (0)) =
                part_vals[7];
          }
          if (corr_tid < 4) {
          ...
          int o_row_e = tmem_row_base_v_1 + lane / 4;
          int oq_even = col_pair_base_c;
          int oq_odd = col_pair_base_c + 1;
          int o_head_base = batch_idx_1 * NUM_Q_HEADS + kv_head_idx * group_ratio_rt;
          int o_even_off = (o_head_base + oq_even) * HEAD_DIM;
          int o_odd_off = (o_head_base + oq_odd) * HEAD_DIM;
          if (oq_even < group_ratio_rt) {
            *(reinterpret_cast<__nv_bfloat16*>(O + (o_even_off + o_row_e)) + (0)) =
                __float2bfloat16_rn(out_vals[0]);
            *(reinterpret_cast<__nv_bfloat16*>(O + (o_even_off + (o_row_e + 8))) + (0)) =
                __float2bfloat16_rn(out_vals[2]);
            *(reinterpret_cast<__nv_bfloat16*>(O + (o_even_off + (o_row_e + 16))) + (0)) =
                __float2bfloat16_rn(out_vals[4]);
            *(reinterpret_cast<__nv_bfloat16*>(O + (o_even_off + (o_row_e + 24))) + (0)) =
                __float2bfloat16_rn(out_vals[6]);
          }
          if (oq_odd < group_ratio_rt) {
            *(reinterpret_cast<__nv_bfloat16*>(O + (o_odd_off + o_row_e)) + (0)) =
                __float2bfloat16_rn(out_vals[1]);
            *(reinterpret_cast<__nv_bfloat16*>(O + (o_odd_off + (o_row_e + 8))) + (0)) =
                __float2bfloat16_rn(out_vals[3]);
            *(reinterpret_cast<__nv_bfloat16*>(O + (o_odd_off + (o_row_e + 16))) + (0)) =
                __float2bfloat16_rn(out_vals[5]);
            *(reinterpret_cast<__nv_bfloat16*>(O + (o_odd_off + (o_row_e + 24))) + (0)) =
                __float2bfloat16_rn(out_vals[7]);
          }
          asm volatile("barrier.sync 11, 128;" ::: "memory");
          if (elect_sync()) {
            mbarrier_arrive(stats_empty_addr);
...
#pragma unroll 1
      for (unsigned int tile_idx_m = blockIdx.x; tile_idx_m < total_tiles_m;
           tile_idx_m += gridDim.x) {
        ...
        int split_start_block_2 = split_m * blocks_per_split;
        int rem_blocks_2 = even_n_blocks_2 - split_start_block_2;
        int capped_blocks_2 = ((rem_blocks_2 > blocks_per_split) ? blocks_per_split : rem_blocks_2);
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_0),
            "r"(_mma_b_lo_0), "r"(tmem_tmem_s0), "r"(0));
        int _mma_a_lo_1 = make_warp_uniform((((smem_kv_addr) >> 4) & 0x3FFF) + (0) * 1024);
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_1),
            "r"(_mma_b_lo_1), "r"(tmem_tmem_s0), "r"(1));
        ...
        int _mma_a_lo_2 = make_warp_uniform((((smem_kv_addr) >> 4) & 0x3FFF) + (1) * 1024);
        asm volatile(
            "{\n\t"
            ".reg .pred leader, p0, p1;\n\t"
            ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_2),
            "r"(_mma_b_lo_0), "r"(tmem_tmem_s1), "r"(0));
        int _mma_a_lo_3 = make_warp_uniform((((smem_kv_addr) >> 4) & 0x3FFF) + (1) * 1024);
        asm volatile(
            "{\n\t"
            ".reg .pred leader, p0, p1;\n\t"
            ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_3),
            "r"(_mma_b_lo_1), "r"(tmem_tmem_s1), "r"(1));
...
#pragma unroll 1
        for (int pair_2 = 0; pair_2 < num_pairs_2 - 1; pair_2++) {
          ...
          int _mma_b_lo_4 = make_warp_uniform(((smem_qt_addr) >> 4) & 0x3FFF);
          asm volatile(
              "{\n\t"
              ".reg .pred leader, p0, p1;\n\t"
              ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_4),
              "r"(_mma_b_lo_4), "r"(tmem_tmem_s0), "r"(0));
          int _mma_a_lo_5 = make_warp_uniform((((smem_kv_addr) >> 4) & 0x3FFF) + (s0_next) * 1024);
          int _mma_b_lo_5 = make_warp_uniform(((smem_qt_lo_addr) >> 4) & 0x3FFF);
          asm volatile(
              "{\n\t"
              ".reg .pred leader, p0, p1;\n\t"
              ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_5),
              "r"(_mma_b_lo_5), "r"(tmem_tmem_s0), "r"(1));
          ...
          asm volatile("tcgen05.fence::after_thread_sync;");
          int _mma_a_lo_6 =
              make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (s0) * 1024);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_6),
              "r"(_mma_b_lo_6), "r"(tmem_tmem_o0), "r"(((first_pv0) ? 0 : 1)));
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_7),
              "r"(_mma_b_lo_4), "r"(tmem_tmem_s1), "r"(0));
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_8),
              "r"(_mma_b_lo_5), "r"(tmem_tmem_s1), "r"(1));
          ...
          asm volatile("tcgen05.fence::after_thread_sync;");
          int _mma_a_lo_9 =
              make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (s1) * 1024);
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_9),
              "r"(_mma_b_lo_9), "r"(tmem_tmem_o1), "r"(((first_pv1) ? 0 : 1)));
        ...
        asm volatile("tcgen05.fence::after_thread_sync;");
        int _mma_a_lo_10 =
            make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (s0_last) * 1024);
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_10),
            "r"(_mma_b_lo_10), "r"(tmem_tmem_o0), "r"(((first_pv0) ? 0 : 1)));
        ...
        asm volatile("tcgen05.fence::after_thread_sync;");
        int _mma_a_lo_11 =
            make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (s1_last) * 1024);
        ...
        asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f8f6f4 [%2], da, db, id, p1;\n\t"
            "}\n" ::"r"(_mma_a_lo_11),
            "r"(_mma_b_lo_11), "r"(tmem_tmem_o1), "r"(((first_pv1) ? 0 : 1)));
...
#pragma unroll 1
      for (unsigned int tile_idx_l = blockIdx.x; tile_idx_l < total_tiles_l;
           tile_idx_l += gridDim.x) {
        ...
        int split_start_block_3 = split_l * blocks_per_split;
        int rem_blocks_3 = even_n_blocks_3 - split_start_block_3;
        int capped_blocks_3 = ((rem_blocks_3 > blocks_per_split) ? blocks_per_split : rem_blocks_3);
        int cta_n_blocks_3 = ((capped_blocks_3 < 2) ? 2 : capped_blocks_3);
        mbarrier_wait(q_empty_addr, _phase_q_empty_0);
        _phase_q_empty_0 ^= 1;
        int q_word_base = (batch_idx_3 * NUM_KV_HEADS + kv_head_idx_1) * (TILE_Q * HEAD_DIM / 2);
        int q_words[8];
        float q_f32[16];
        float q_res[16];
        unsigned int q_packed[4];
        unsigned int q_packed_lo[4];
        int q_wbits = 0;
        float q_lo_f = 0.0f;
        float q_hi_f = 0.0f;
        float q_trunc_f = 0.0f;
#pragma unroll
        for (int qc = 0; qc < 2; qc++) {
          int q_chunk = lane * 2 + qc;
          int q_row = q_chunk / 8;
          int q_col16 = q_chunk % 8;
          {
            int4 _iv4 =
                *reinterpret_cast<const int4*>(Q_ptr + q_word_base + q_row * 64 + q_col16 * 8);
            q_words[0 + 0] = _iv4.x;
            q_words[0 + 1] = _iv4.y;
            q_words[0 + 2] = _iv4.z;
            q_words[0 + 3] = _iv4.w;
          ...
          {
            int4 _iv4 =
                *reinterpret_cast<const int4*>(Q_ptr + q_word_base + q_row * 64 + q_col16 * 8 + 4);
            q_words[4 + 0] = _iv4.x;
            q_words[4 + 1] = _iv4.y;
            q_words[4 + 2] = _iv4.z;
            q_words[4 + 3] = _iv4.w;
...
#pragma unroll
          for (int qw = 0; qw < 8; qw++) {
            q_wbits = q_words[qw] << 16;
            q_lo_f = reinterpret_cast<float*>(&q_wbits)[0];
            q_wbits = (q_words[qw] & 65520) << 16;
            q_trunc_f = reinterpret_cast<float*>(&q_wbits)[0];
            q_f32[2 * qw] = q_trunc_f;
            q_res[2 * qw] = q_lo_f - q_trunc_f;
            q_wbits = q_words[qw] >> 16 << 16;
            q_hi_f = reinterpret_cast<float*>(&q_wbits)[0];
            q_wbits = (q_words[qw] >> 16 & 65520) << 16;
            q_trunc_f = reinterpret_cast<float*>(&q_wbits)[0];
            q_f32[2 * qw + 1] = q_trunc_f;
            q_res[2 * qw + 1] = q_hi_f - q_trunc_f;
          }
          {
            uint32_t _packed;
            asm volatile(
                ...
                "}"
                : "=r"(_packed)
                : "f"(q_f32[0]), "f"(q_f32[1]), "f"(q_f32[2]), "f"(q_f32[3]));
            q_packed[0] = _packed;
          }
          {
            uint32_t _packed;
            asm volatile(
                ...
                "}"
                : "=r"(_packed)
                : "f"(q_f32[4]), "f"(q_f32[5]), "f"(q_f32[6]), "f"(q_f32[7]));
            q_packed[1] = _packed;
          }
          {
            uint32_t _packed;
            asm volatile(
                ...
                "}"
                : "=r"(_packed)
                : "f"(q_f32[8]), "f"(q_f32[9]), "f"(q_f32[10]), "f"(q_f32[11]));
            q_packed[2] = _packed;
          }
          {
            uint32_t _packed;
            asm volatile(
                ...
                "}"
                : "=r"(_packed)
                : "f"(q_f32[12]), "f"(q_f32[13]), "f"(q_f32[14]), "f"(q_f32[15]));
            q_packed[3] = _packed;
          }
          {
            uint32_t _packed;
            asm volatile(
                ...
                "}"
                : "=r"(_packed)
                : "f"(q_res[0]), "f"(q_res[1]), "f"(q_res[2]), "f"(q_res[3]));
            q_packed_lo[0] = _packed;
          }
          {
            uint32_t _packed;
            asm volatile(
                ...
                "}"
                : "=r"(_packed)
                : "f"(q_res[4]), "f"(q_res[5]), "f"(q_res[6]), "f"(q_res[7]));
            q_packed_lo[1] = _packed;
          }
          {
            uint32_t _packed;
            asm volatile(
                ...
                "}"
                : "=r"(_packed)
                : "f"(q_res[8]), "f"(q_res[9]), "f"(q_res[10]), "f"(q_res[11]));
            q_packed_lo[2] = _packed;
          }
          {
            uint32_t _packed;
            asm volatile(
                ...
                "}"
                : "=r"(_packed)
                : "f"(q_res[12]), "f"(q_res[13]), "f"(q_res[14]), "f"(q_res[15]));
            q_packed_lo[3] = _packed;
          }
          int q_dst = q_row * 128 + (q_col16 ^ q_row % 8) * 16;
          asm volatile("st.shared.b32 [%0], %1;" ::"r"(smem_qt_addr + (unsigned int)q_dst),
                       "r"((q_packed[0])));
          asm volatile("st.shared.b32 [%0], %1;" ::"r"(smem_qt_addr + (unsigned int)q_dst + 4),
                       "r"((q_packed[1])));
          asm volatile("st.shared.b32 [%0], %1;" ::"r"(smem_qt_addr + (unsigned int)q_dst + 8),
                       "r"((q_packed[2])));
          asm volatile("st.shared.b32 [%0], %1;" ::"r"(smem_qt_addr + (unsigned int)q_dst + 12),
                       "r"((q_packed[3])));
          asm volatile("st.shared.b32 [%0], %1;" ::"r"(smem_qt_lo_addr + (unsigned int)q_dst),
                       "r"((q_packed_lo[0])));
          asm volatile("st.shared.b32 [%0], %1;" ::"r"(smem_qt_lo_addr + (unsigned int)q_dst + 4),
                       "r"((q_packed_lo[1])));
          asm volatile("st.shared.b32 [%0], %1;" ::"r"(smem_qt_lo_addr + (unsigned int)q_dst + 8),
                       "r"((q_packed_lo[2])));
          asm volatile("st.shared.b32 [%0], %1;" ::"r"(smem_qt_lo_addr + (unsigned int)q_dst + 12),
                       "r"((q_packed_lo[3])));
        }
        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
        mbarrier_arrive(q_full_addr);
        if (elect_sync()) {
          int pt_base_k = batch_idx_3 * pt_batch_stride;
          int pt_base_v = pt_base_k + pt_v_offset;
          int kv_stage = 0;
          int kv_phase = 1;
          int prefill = ((cta_n_blocks_3 < 2) ? cta_n_blocks_3 : 2);
#pragma unroll 1
          for (int ni = 0; ni < prefill; ni++) {
            int n_block = split_start_block_3 + cta_n_blocks_3 - 1 - ni;
            int pg_base = n_block * (BLOCK_N / PAGE_SIZE);
            ...
            int pg_k[8];
            {
              int4 _iv4 = *reinterpret_cast<const int4*>(page_table + pt_base_k + pg_base);
              pg_k[0 + 0] = _iv4.x;
              pg_k[0 + 1] = _iv4.y;
              pg_k[0 + 2] = _iv4.z;
              pg_k[0 + 3] = _iv4.w;
...
#pragma unroll
            for (int pg_i = 0; pg_i < 8; pg_i++) {
              ...
              int toff = pg_i * 2048;
              tma_5d_gmem2smem(ldst + toff, K, 0, pg_i % (PAGE_SIZE / 16) * 16, 0, kv_head_idx_1,
                               pg0, kv_full_addr + (kv_stage) * 8);
            }
            kv_stage += 1;
            if (kv_stage == 8) {
              kv_stage = 0;
...
#pragma unroll 1
          for (int ni_1 = 0; ni_1 < cta_n_blocks_3; ni_1++) {
            int stage = ni_1 % 8;
            int n_block_1 = split_start_block_3 + cta_n_blocks_3 - 1 - ni_1;
            int vpg_base = n_block_1 * (BLOCK_N / PAGE_SIZE);
            int next_ni = ni_1 + 2;
            if (next_ni < cta_n_blocks_3) {
              int k_stage = next_ni % 8;
              int next_n = split_start_block_3 + cta_n_blocks_3 - 1 - next_ni;
              int npg_base = next_n * (BLOCK_N / PAGE_SIZE);
              ...
              int pg_nk[8];
              {
                int4 _iv4 = *reinterpret_cast<const int4*>(page_table + pt_base_k + npg_base);
                pg_nk[0 + 0] = _iv4.x;
                pg_nk[0 + 1] = _iv4.y;
                pg_nk[0 + 2] = _iv4.z;
                pg_nk[0 + 3] = _iv4.w;
...
#pragma unroll
              for (int pg_i_1 = 0; pg_i_1 < 8; pg_i_1++) {
                ...
                tma_5d_gmem2smem(kdst + ntoff, K, 0, pg_i_1 % (PAGE_SIZE / 16) * 16, 0,
                                 kv_head_idx_1, npg0, kv_full_addr + (k_stage) * 8);
              }
            }
            int pg_v[8];
            {
              int4 _iv4 = *reinterpret_cast<const int4*>(page_table + pt_base_v + vpg_base);
              pg_v[0 + 0] = _iv4.x;
              pg_v[0 + 1] = _iv4.y;
              pg_v[0 + 2] = _iv4.z;
              pg_v[0 + 3] = _iv4.w;
...
#pragma unroll
            for (int pg_i_2 = 0; pg_i_2 < 8; pg_i_2++) {
              ...
              tma_5d_gmem2smem(vdst + vtoff, V, 0, pg_i_2 % (PAGE_SIZE / 16) * 16, 0, kv_head_idx_1,
                               vpg0, kv_full_addr + (stage) * 8);
            }
          }
        }
      }
