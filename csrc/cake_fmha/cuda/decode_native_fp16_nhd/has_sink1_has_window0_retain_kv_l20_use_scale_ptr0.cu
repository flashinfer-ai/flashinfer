...
kernel_cake_fmha_decode_native_fp16_nhd(const void* __restrict__ Qt, const void* __restrict__ K, const void* __restrict__ V, __half* __restrict__ O_ptr, float* __restrict__ LSE_ptr, int* __restrict__ page_table, int* __restrict__ causal_seqlens_kv_global, float* __restrict__ scale_log2_ptr, float* __restrict__ sinks_ptr, int max_pages_per_seq, int max_local_seq_len, float softmax_scale_log2, int window_left, int num_q_heads, int num_kv_heads, int batch_size)
{
    ...
    // ---- Role: load_pgoff ----
        ...
        { // load_pgoff_main
            ...
            for (unsigned int tile_idx_la = blockIdx.x; tile_idx_la < total_tiles_la; tile_idx_la += gridDim.x) {
                ...
                if (elect_sync()) {
                    int pt_k_base_la = batch_idx_3 * (2 * max_pages_per_seq);
                    int pt_v_base_la = pt_k_base_la + max_pages_per_seq;
                    int kv_stage_la = 0;
                    int kv_phase_la = 1;
                    ...
                    for (int ni_la = 0; ni_la < prefill_la; ni_la++) {
                        ...
                        int pg_k_la[4];
                        {
                            int4 _iv4 = *reinterpret_cast<const int4*>(page_table + pt_k_base_la + pg_base_la);
                            pg_k_la[0 + 0] = _iv4.x;
                            pg_k_la[0 + 1] = _iv4.y;
                            pg_k_la[0 + 2] = _iv4.z;
                            pg_k_la[0 + 3] = _iv4.w;
                        ...
                        #pragma unroll
                        for (int pg_half_la = 0; pg_half_la < 2; pg_half_la++) {
                            int pg0_la = pg_k_la[pg_half_la];
                            #pragma unroll
                            for (int hg_la = 0; hg_la < 2; hg_la++) {
                                int toff_la = hg_la * 16384 + pg_half_la * 4096;
                                {
                    ...
                    for (int ni_la_1 = 0; ni_la_1 < main_ni_la; ni_la_1++) {
                        ...
                        int vpg_base_la = n_block_la_1 * 4;
                        ...
                        int pg_v_la[4];
                        {
                            int4 _iv4 = *reinterpret_cast<const int4*>(page_table + pt_v_base_la + vpg_base_la);
                            pg_v_la[0 + 0] = _iv4.x;
                            pg_v_la[0 + 1] = _iv4.y;
                            pg_v_la[0 + 2] = _iv4.z;
                            pg_v_la[0 + 3] = _iv4.w;
                        ...
                        #pragma unroll
                        for (int pg_half_la_1 = 0; pg_half_la_1 < 2; pg_half_la_1++) {
                            int vpg0_la = pg_v_la[pg_half_la_1];
                            #pragma unroll
                            for (int hg_la_1 = 0; hg_la_1 < 2; hg_la_1++) {
                                int vtoff_la = hg_la_1 * 16384 + pg_half_la_1 * 4096;
                                {
                        ...
                        int pg_nk_la[4];
                        {
                            int4 _iv4 = *reinterpret_cast<const int4*>(page_table + pt_k_base_la + npg_base_la);
                            pg_nk_la[0 + 0] = _iv4.x;
                            pg_nk_la[0 + 1] = _iv4.y;
                            pg_nk_la[0 + 2] = _iv4.z;
                            pg_nk_la[0 + 3] = _iv4.w;
                        ...
                        #pragma unroll
                        for (int pg_half_la_2 = 0; pg_half_la_2 < 2; pg_half_la_2++) {
                            int npg0_la = pg_nk_la[pg_half_la_2];
                            #pragma unroll
                            for (int hg_la_2 = 0; hg_la_2 < 2; hg_la_2++) {
                                int ntoff_la = hg_la_2 * 16384 + pg_half_la_2 * 4096;
                                {
                    ...
                    for (int tail_ni_la = main_ni_la; tail_ni_la < cta_n_blocks_3; tail_ni_la++) {
                        ...
                        int tvpg_base_la = tn_block_la * 4;
                        ...
                        int tpg_v_la[4];
                        {
                            int4 _iv4 = *reinterpret_cast<const int4*>(page_table + pt_v_base_la + tvpg_base_la);
                            tpg_v_la[0 + 0] = _iv4.x;
                            tpg_v_la[0 + 1] = _iv4.y;
                            tpg_v_la[0 + 2] = _iv4.z;
                            tpg_v_la[0 + 3] = _iv4.w;
                        ...
                        #pragma unroll
                        for (int pg_half_la_3 = 0; pg_half_la_3 < 2; pg_half_la_3++) {
                            int tvpg0_la = tpg_v_la[pg_half_la_3];
                            #pragma unroll
                            for (int hg_la_3 = 0; hg_la_3 < 2; hg_la_3++) {
                                int tvtoff_la = hg_la_3 * 16384 + pg_half_la_3 * 4096;
                                {
        ...
        { // load_warp_main
            ...
            for (unsigned int tile_idx_l = blockIdx.x; tile_idx_l < total_tiles_l; tile_idx_l += gridDim.x) {
                ...
                if (elect_sync()) {
                    ...
                    int pt_k_base = batch_idx_4 * (2 * max_pages_per_seq);
                    int pt_v_base = pt_k_base + max_pages_per_seq;
                    int kv_stage = 0;
                    int kv_phase = 1;
                    ...
                    for (int ni = 0; ni < prefill; ni++) {
                        ...
                        int pg_k[4];
                        {
                            int4 _iv4 = *reinterpret_cast<const int4*>(page_table + pt_k_base + pg_base);
                            pg_k[0 + 0] = _iv4.x;
                            pg_k[0 + 1] = _iv4.y;
                            pg_k[0 + 2] = _iv4.z;
                            pg_k[0 + 3] = _iv4.w;
                        ...
                        #pragma unroll
                        for (int pg_half = 0; pg_half < 2; pg_half++) {
                            int pg0 = pg_k[pg_half + 2];
                            const int pg_i = pg_half + 2;
                            #pragma unroll
                    ...
                    for (int ni_1 = 0; ni_1 < main_ni; ni_1++) {
                        ...
                        int pg_v[4];
                        {
                            int4 _iv4 = *reinterpret_cast<const int4*>(page_table + pt_v_base + vpg_base);
                            pg_v[0 + 0] = _iv4.x;
                            pg_v[0 + 1] = _iv4.y;
                            pg_v[0 + 2] = _iv4.z;
                            pg_v[0 + 3] = _iv4.w;
                        ...
                        #pragma unroll
                        for (int pg_half_1 = 0; pg_half_1 < 2; pg_half_1++) {
                            int vpg0 = pg_v[pg_half_1 + 2];
                            const int pg_i_1 = pg_half_1 + 2;
                            #pragma unroll
                            for (int hg_1 = 0; hg_1 < 2; hg_1++) {
                                int vtoff = hg_1 * 16384 + pg_i_1 * 4096;
                                {
                        ...
                        int pg_nk[4];
                        {
                            int4 _iv4 = *reinterpret_cast<const int4*>(page_table + pt_k_base + npg_base);
                            pg_nk[0 + 0] = _iv4.x;
                            pg_nk[0 + 1] = _iv4.y;
                            pg_nk[0 + 2] = _iv4.z;
                            pg_nk[0 + 3] = _iv4.w;
                        ...
                        #pragma unroll
                        for (int pg_half_2 = 0; pg_half_2 < 2; pg_half_2++) {
                            int npg0 = pg_nk[pg_half_2 + 2];
                            const int pg_i_2 = pg_half_2 + 2;
                            #pragma unroll
                            for (int hg_2 = 0; hg_2 < 2; hg_2++) {
                                int ntoff = hg_2 * 16384 + pg_i_2 * 4096;
                                {
                    ...
                    for (int tail_ni = main_ni; tail_ni < cta_n_blocks_4; tail_ni++) {
                        ...
                        int tpg_v[4];
                        {
                            int4 _iv4 = *reinterpret_cast<const int4*>(page_table + pt_v_base + tvpg_base);
                            tpg_v[0 + 0] = _iv4.x;
                            tpg_v[0 + 1] = _iv4.y;
                            tpg_v[0 + 2] = _iv4.z;
                            tpg_v[0 + 3] = _iv4.w;
                        ...
                        #pragma unroll
                        for (int pg_half_3 = 0; pg_half_3 < 2; pg_half_3++) {
                            int tvpg0 = tpg_v[pg_half_3 + 2];
                            const int pg_i_3 = pg_half_3 + 2;
                            #pragma unroll
                            for (int hg_3 = 0; hg_3 < 2; hg_3++) {
                                int tvtoff = hg_3 * 16384 + pg_i_3 * 4096;
                                {
