...
__global__ __launch_bounds__(32) void kernel_cake_fmha_decode_quant_fp8_reduce(
    float* __restrict__ partial_O, float* __restrict__ partial_max, float* __restrict__ partial_sum,
    uint8_t* __restrict__ O, float* __restrict__ bmm2_scale_ptr, int num_split) {
...
#pragma unroll 8
  ...
  out_quad[3] = out_pair1[1];
  {
    ...
    asm("{\n\t"
        ...
        "}\n"
        : "=r"(_fp8_pk[0])
        : "f"(out_quad[0 + 0]), "f"(out_quad[0 + 1]), "f"(out_quad[0 + 2]), "f"(out_quad[0 + 3]));
    *reinterpret_cast<unsigned int*>(reinterpret_cast<unsigned char*>(O + o_off) + (0)) =
        *reinterpret_cast<unsigned int*>(_fp8_pk);
  }
}
