...
__device__ __forceinline__ float e2m1_value(unsigned code) {
  constexpr float levels[16] = {0.0f,  0.5f,  1.0f,  1.5f,  2.0f,  3.0f,  4.0f,  6.0f,
...
__device__ __forceinline__ long long output_scale_swizzled_index(int row, int column,
                                                                 int rounded_columns) {
...
__device__ __forceinline__ long long kv_index(int page, int token, int head, int dim, int layout,
...
__device__ __forceinline__ long long v_scale_swizzled_index(int page, int token, int head,
                                                            int block, int layout, int page_size,
                                                            int num_heads, int scale_dim,
                                                            long long page_stride) {
  ...
  if (layout == 0) {
    within =
        (((((static_cast<long long>(head) * (page_size >> 2) + token_group) * 4 + scale_outer) *
               scale_groups +
           scale_inner) *
          4) +
         token_inner);
  } else {
    within = (((((static_cast<long long>(token_group) * 4 + scale_outer) * num_heads + head) *
                    scale_groups +
                scale_inner) *
               4) +
              token_inner);
...
__device__ __forceinline__ float load_nvfp4(const void* packed, const void* scales, int page,
                                            int token, int head, int dim, int layout,
                                            bool value_side, int page_size, int num_heads,
                                            int head_dim, long long p0, long long p1, long long p2,
                                            long long p3, long long sf0, long long sf1,
                                            long long sf2, long long sf3) {
  ...
  long long sf_at;
  if (value_side) {
    sf_at = v_scale_swizzled_index(page, token, head, block, layout, page_size, num_heads,
                                   head_dim >> 4, sf0);
  } else {
...
extern "C" __global__ __launch_bounds__(256) void kernel_cake_fmha_compat_v1(
  ...
  const float q_value = tid < head_dim
                            ? load_scalar(q,
                                          static_cast<long long>(token_global) * q_s0 +
                                              static_cast<long long>(q_head) * q_s1 + tid,
                                          q_dtype) *
                                  q_scale
                            : 0.0f;
  ...
  for (int kv_pos = kv_begin; kv_pos < kv_end; ++kv_pos) {
    ...
    if (tid < head_dim) {
      if (kv_dtype == 3) {
        k_value = load_nvfp4(k, k_scales, page_k, page_offset, kv_head, tid, kv_layout, false,
                             page_size, num_kv_heads, head_dim, k_s0, k_s1, k_s2, k_s3, ksf_s0,
                             ksf_s1, ksf_s2, ksf_s3);
      } else {
        const long long at =
            kv_index(page_k, page_offset, kv_head, tid, kv_layout, k_s0, k_s1, k_s2, k_s3);
    ...
    if (tid < head_dim) {
      float v_value;
      if (kv_dtype == 3) {
        v_value = load_nvfp4(v, v_scales, page_v, page_offset, kv_head, tid, kv_layout, true,
                             page_size, num_kv_heads, head_dim, v_s0, v_s1, v_s2, v_s3, vsf_s0,
                             vsf_s1, vsf_s2, vsf_s3);
      } else {
        const long long at =
            kv_index(page_v, page_offset, kv_head, tid, kv_layout, v_s0, v_s1, v_s2, v_s3);
...
#pragma unroll
    ...
    if (tid < head_dim && lane16 == 0) {
      ...
      const long long sf_at =
          output_scale_swizzled_index(token_global + o_sf_start, scale_column, o_sf_columns);
    ...
    if (tid < head_dim && (tid & 1) == 0) {
      ...
      reinterpret_cast<unsigned char*>(o)[static_cast<long long>(token_global) * o_s0 +
                                          static_cast<long long>(q_head) * o_s1 + (tid >> 1)] =
