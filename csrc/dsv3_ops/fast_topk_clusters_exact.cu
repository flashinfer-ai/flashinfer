...
__device__ __forceinline__ void fast_topk_cuda_v4(
    ...
    const int overflow_stride, const int seq_len, const int num_cached) {
      ...
      if (bin == threshold_bin) {
        ...
        if (cached_offset < num_cached) {
          ...
          if (g_off < overflow_stride) {
            PackedCachedData cached_res = {bits, i};
...
__global__ __launch_bounds__(1024) void __cluster_dims__(NClusters, 1, 1)
    fast_topk_clusters_exact_kernel(
        const float* __restrict__ logits,  // [batchsize, max_num_pages * 64]
        int* __restrict__ output_indices, int* __restrict__ seq_lens, int* __restrict__ pre_hist,
        int* __restrict__ cached_overflow, int overflow_stride, int logit_stride,
  ...
  if (seq_len <= TopK) {
    ...
    fast_topk_cuda_v4<NClusters, PDL_ENABLED, PRE_HISTOGRAM>(
        logits + logit_offset, output_indices + ind_offset, pre_hist + cluster_id * 256,
        (PackedCachedData*)block_overflow, overflow_stride, seq_len, num_cached);
  }
}

constexpr int MAX_SMEM_CARVEOUT = 227 * 1000;

#define DISPATCH_TOPK_EXACT(CLUSTERS, PDL)                                          \
  if (num_clusters == CLUSTERS && pdl_enabled == PDL) {                             \
    if (pre_hist == nullptr) {                                                      \
      setup_kernel_smem_once<fast_topk_clusters_exact_kernel<CLUSTERS, PDL, false>, \
                             MAX_SMEM_CARVEOUT>();                                  \
      kernel = (void*)&fast_topk_clusters_exact_kernel<CLUSTERS, PDL, false>;       \
    } else {                                                                        \
      setup_kernel_smem_once<fast_topk_clusters_exact_kernel<CLUSTERS, PDL, true>,  \
                             MAX_SMEM_CARVEOUT>();                                  \
      kernel = (void*)&fast_topk_clusters_exact_kernel<CLUSTERS, PDL, true>;        \
    }                                                                               \
  }
...
void launch_fast_topk_clusters_exact(const float* logits, int* indices, int* seq_lens,
  ...
  void* args[9] = {&logits,          &indices,      &seq_lens,       &pre_hist,  &cached_overflow,
                   &overflow_stride, &logit_stride, &indices_stride, &num_cached};

  void* kernel = nullptr;
  DISPATCH_TOPK_EXACT(1, false);
  DISPATCH_TOPK_EXACT(2, false);
  DISPATCH_TOPK_EXACT(4, false);
  DISPATCH_TOPK_EXACT(8, false);
  DISPATCH_TOPK_EXACT(1, true);
  DISPATCH_TOPK_EXACT(2, true);
  DISPATCH_TOPK_EXACT(4, true);
  DISPATCH_TOPK_EXACT(8, true);
