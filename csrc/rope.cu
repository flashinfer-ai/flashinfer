...
void mla_rope_quantize(TensorView q_rope_in, TensorView k_rope_in, TensorView q_nope_in,
  ...
  TVM_FFI_ICHECK_EQ(q_rope_in.size(q_rope_in.ndim() - 1), 64);
  TVM_FFI_ICHECK_EQ(k_rope_in.size(k_rope_in.ndim() - 1), 64);
  TVM_FFI_ICHECK_EQ(q_nope_in.size(q_nope_in.ndim() - 1), 512);
  TVM_FFI_ICHECK_EQ(k_nope_in.size(k_nope_in.ndim() - 1), 512);
  TVM_FFI_ICHECK_EQ(q_rope_out.size(q_rope_out.ndim() - 1), 64);
  TVM_FFI_ICHECK_EQ(k_rope_out.size(k_rope_out.ndim() - 1), 64);
  TVM_FFI_ICHECK_EQ(q_nope_out.size(q_nope_out.ndim() - 1), 512);
  TVM_FFI_ICHECK_EQ(k_nope_out.size(k_nope_out.ndim() - 1), 512);
  ...
  TVM_FFI_ICHECK_EQ(q_rope_in.size(1), num_heads);
  TVM_FFI_ICHECK_EQ(q_nope_in.size(1), num_heads);
  TVM_FFI_ICHECK_EQ(q_rope_out.size(1), num_heads);
  TVM_FFI_ICHECK_EQ(q_nope_out.size(1), num_heads);
