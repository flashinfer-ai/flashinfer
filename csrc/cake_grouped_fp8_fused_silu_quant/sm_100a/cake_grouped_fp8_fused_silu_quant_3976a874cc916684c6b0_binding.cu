...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_grouped_fp8_fused_silu_quant_3976a874cc916684c6b0'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_cake_grouped_fp8_fused_silu_quant_3976a874cc916684c6b0(const __nv_bfloat16* __restrict__ y, uint8_t* __restrict__ out_q, float* __restrict__ out_s, int M, int H);


namespace cake_host_shim_67ef1881e5da1de7 {
...
void Run(TensorView arg_y, TensorView arg_out_q, TensorView arg_out_s, int64_t arg_M, int64_t arg_H, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  DLDevice dev = arg_y.device();
  ...
  TVM_FFI_CHECK(arg_H >= -2147483648LL && arg_H <= 2147483647LL, ValueError)
      << "scalar 'H' value " << arg_H
      << " is outside i32 range [-2147483648, 2147483647]";
  CheckSameCudaDevice(arg_out_q, arg_y, "out_q", "y");
  CheckSameCudaDevice(arg_out_s, arg_y, "out_s", "y");
  ...
  int32_t v_H = (int32_t)arg_H;
  void* kargs[] = {&p_y, &p_out_q, &p_out_s, &v_M, &v_H};
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_grouped_fp8_fused_silu_quant_3976a874cc916684c6b0), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_grouped_fp8_fused_silu_quant_3976a874cc916684c6b0 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_67ef1881e5da1de7

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_67ef1881e5da1de7::Run);
