...
kernel_cake_grouped_fp8_fused_silu_quant_3976a874cc916684c6b0(const __nv_bfloat16* __restrict__ y, uint8_t* __restrict__ out_q, float* __restrict__ out_s, int M, int H)
{
    ...
    for (int item = first_item; item < total_items; item += num_bids * 4) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            float s0 = _rcp_0;
            __nv_bfloat16 _cvt_bf16_0 = __float2bfloat16(g0 * s0 * u0);
        ...
        float _fmax_1 = fmaxf(absmax, 1e-10f);
