...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_grouped_fp8_fused_silu_quant_c5c857d1d7fd38445fd2'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_cake_grouped_fp8_fused_silu_quant_c5c857d1d7fd38445fd2(CakeTensorMap const* A, CakeTensorMap const* B, uint8_t* __restrict__ out_q, float* __restrict__ out_s, float* __restrict__ a_scale, float* __restrict__ b_scale, int* __restrict__ m_indices, int M, int N, int K, int G);


namespace cake_host_shim_b518870afd0ca553 {
...
inline CUtensorMap EncodeTma_B(const TensorView& t) {
  ...
  TVM_FFI_CHECK(128u <= global_dim[0] && 64u <= global_dim[1] && 1u <= global_dim[2] && 1u <= global_dim[3], ValueError)
      << "TMA box (128, 64, 1, 1) exceeds resolved global dims for 'B'";
  int64_t carrier_stride_0 = d1;
  TVM_FFI_CHECK(carrier_stride_0 >= 0, ValueError)
...
void Run(TensorView arg_A, TensorView arg_B, TensorView arg_out_q, TensorView arg_out_s, TensorView arg_a_scale, TensorView arg_b_scale, TensorView arg_m_indices, int64_t arg_M, int64_t arg_N, int64_t arg_K, int64_t arg_G, tvm::ffi::Tensor arg_tma_descriptor_workspace, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  CheckDtype(arg_out_s, "out_s", 2, 32, 1);
  ...
  CheckSameCudaDevice(arg_out_s, arg_A, "out_s", "A");
  ...
  float* p_out_s = static_cast<float*>(arg_out_s.data_ptr());
  float* p_a_scale = static_cast<float*>(arg_a_scale.data_ptr());
  float* p_b_scale = static_cast<float*>(arg_b_scale.data_ptr());
  ...
  void* kargs[] = {&p_A, &p_B, &p_out_q, &p_out_s, &p_a_scale, &p_b_scale, &p_m_indices, &v_M, &v_N, &v_K, &v_G};
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_grouped_fp8_fused_silu_quant_c5c857d1d7fd38445fd2),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_grouped_fp8_fused_silu_quant_c5c857d1d7fd38445fd2 failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_grouped_fp8_fused_silu_quant_c5c857d1d7fd38445fd2), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_grouped_fp8_fused_silu_quant_c5c857d1d7fd38445fd2 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_b518870afd0ca553

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_b518870afd0ca553::Run);
