...
// ---------------------------------------------------------------------------
// SINGLE-RANK deployment geometry.  These are the compile-time constants the
// `pinned` family bakes in and the page map a tensor-parallel size of 1
// produces.  `general` reads every one of them at runtime instead, which is why
// the host binding derives the head counts and the page map FROM THE TENSORS
// (see `page_layout` below) rather than asserting these: a tensor-parallel rank
// holds `max(1, num_key_value_heads / tp)` KV heads and vLLM sizes its cache
// from that per-rank number, so the page is 36,864 B at tp 2 and 18,432 B at
// tp 4, not 73,728.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// The planar page map for an arbitrary KV-head count, in bytes.  ONE function,
// host-side, mirrored in `flashinfer/msa_ops/_nvfp4_decode_sm100.py` as
// `page_layout`, because the map is a pure function of the KV-head count and
// the head dimension and of nothing else.  The four constants above are this
// function evaluated at kNumKVHeads.
// ---------------------------------------------------------------------------
struct PageLayout {
  int64_t k_scale_byte_offset;
  int64_t v_data_byte_offset;
  int64_t v_scale_byte_offset;
  int64_t page_bytes;
};

inline PageLayout page_layout(int num_kv_heads) {
  const int64_t k_scale = static_cast<int64_t>(num_kv_heads) * kDataHeadStride;
  const int64_t v_data = k_scale + static_cast<int64_t>(num_kv_heads) * kScaleHeadStride;
  const int64_t v_scale = v_data + k_scale;
  return PageLayout{k_scale, v_data, v_scale,
                    v_scale + static_cast<int64_t>(num_kv_heads) * kScaleHeadStride};
}
...
// THE SPLIT AND THE CHUNK WIDTH `pinned::launch` WOULD CHOOSE -- ONE COPY.
// The family predicate below has to reject a configuration the launcher would
// take, so it cannot restate the launcher's arithmetic; `launch` calls these.
inline int pinned_split_for(int total_q, int num_pages) {
  const int tiles = total_q * kNumKVHeads;
  int split = tiles <= 32 ? 8 : (tiles <= 64 ? 4 : (tiles <= 128 ? 2 : 1));
  ...
  if (total_q == 8 && !(num_pages < 32 * total_q)) split = 4;
  return split;
}
...
cudaError_t launch(const void* q, const void* k_data, const void* v_data, const void* k_scale,
  ...
  const int tiles = total_q * kNumKVHeads;
  // ONE copy of the split arithmetic, shared with `selects_pinned_path` -- the
  // family predicate now REJECTS a configuration this function would take, so
...
// The KV-head count and the page byte size are ARGUMENTS here, not
// `geom::` constants.  A tensor-parallel rank holds
// `max(1, num_key_value_heads / tp)` KV heads and vLLM sizes its paged cache
// from that same per-rank number, so the page SHRINKS with the rank: at tp 2 it
// is 36,864 B and at tp 4 it is 18,432 B, not 73,728.  Both kernel families
// already take `page_bytes`, `data_head_stride` and `scale_head_stride` as
// launch arguments and derive every address from them, so what pinned the
// geometry was this check and the offsets below, never the kernel.
void check_page_region(const TensorView& t, const char* name, int64_t num_pages,
                       int64_t num_kv_heads, int64_t page_bytes, int64_t inner,
                       int64_t head_stride) {
  CHECK_DIM(4, t);
  TVM_FFI_ICHECK_EQ(t.size(0), num_pages) << name << " must cover the whole page pool";
  TVM_FFI_ICHECK_EQ(t.size(1), num_kv_heads)
      << name << " must have " << num_kv_heads << " kv heads";
  ...
  TVM_FFI_ICHECK_EQ(t.stride(0), page_bytes) << name << " page stride must be " << page_bytes;
...
void msa_decode_nvfp4_specialized(TensorView q, TensorView k_data, TensorView v_data,
  ...
  // head_dim is a compile-time constant of BOTH kernel bodies; the head COUNTS
  // are not.  They come off the tensors, and the GQA group they imply is
  // checked against the parametric family's own capacity below.
  TVM_FFI_ICHECK_EQ(q.size(2), geom::kHeadDim) << "head_dim must be 128";
  CHECK_DIM(4, k_data);
  const int64_t num_kv_heads_i64 = k_data.size(1);
  const int64_t num_q_heads_i64 = q.size(1);
  TVM_FFI_ICHECK_GT(num_kv_heads_i64, 0) << "the cache must hold at least one kv head";
  TVM_FFI_ICHECK(num_q_heads_i64 > 0 && num_q_heads_i64 % num_kv_heads_i64 == 0)
      << "num_qo_heads (" << num_q_heads_i64 << ") must be a positive multiple of num_kv_heads ("
      << num_kv_heads_i64 << ")";
  const int64_t gqa_group = num_q_heads_i64 / num_kv_heads_i64;
  TVM_FFI_ICHECK_LE(gqa_group, geom::general::kHeadCapacity)
      << "GQA group " << gqa_group << " exceeds the parametric kernel's head capacity "
      << geom::general::kHeadCapacity
      << " -- one warp per query head in a 512-thread CTA sizes four shared-memory arrays";
  // The kernel writes `output` with q's dimensions and never re-derives them
  // from `output` itself, so this is the only thing standing between a
  // mis-shaped destination and an out-of-bounds store. It is asserted AFTER the
  // head counts because those are what q's dimensions now mean.
  CHECK_SHAPE(output, q);
  const geom::PageLayout page = geom::page_layout(static_cast<int>(num_kv_heads_i64));

  TVM_FFI_ICHECK_EQ(q2k_indices.size(0), num_kv_heads_i64);
  ...
  TVM_FFI_ICHECK_LE(
      (num_kv_heads_i64 - 1) * q2k_head_stride + (total_q - 1) * q2k_token_stride + topk,
      0x7fffffffLL)
  ...
  check_page_region(k_data, "k_data", num_pages, num_kv_heads_i64, page.page_bytes, geom::kDataDim,
                    geom::kDataHeadStride);
  check_page_region(v_data, "v_data", num_pages, num_kv_heads_i64, page.page_bytes, geom::kDataDim,
                    geom::kDataHeadStride);
  check_page_region(k_scale, "k_scale", num_pages, num_kv_heads_i64, page.page_bytes,
                    geom::kScaleDim, geom::kScaleHeadStride);
  check_page_region(v_scale, "v_scale", num_pages, num_kv_heads_i64, page.page_bytes,
                    geom::kScaleDim, geom::kScaleHeadStride);
  ...
  TVM_FFI_ICHECK_EQ(byte_offset(k_scale, k_data), page.k_scale_byte_offset)
      << "k_scale must be the K-scale region of the same page as k_data";
  TVM_FFI_ICHECK_EQ(byte_offset(v_data, k_data), page.v_data_byte_offset)
      << "v_data must be the V-data region of the same page as k_data";
  TVM_FFI_ICHECK_EQ(byte_offset(v_scale, k_data), page.v_scale_byte_offset)
