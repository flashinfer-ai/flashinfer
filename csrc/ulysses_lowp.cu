...
// Shared TVM-FFI bindings for global-grid Ulysses statistics, packing and
// unpack. SM89/SM120 compile this file directly; the SM90 translation unit
// includes it with its layout selected below. Kernel templates stay shared.
...
#ifdef FLASHINFER_ULYSSES_LOWP_SM90
constexpr uint32_t Q_GROUP = 16;
constexpr uint32_t K_GROUP = 128;
#else
constexpr uint32_t Q_GROUP = 32;
constexpr uint32_t K_GROUP = 64;
#endif

// Both dimensions are compiled into each architecture module.
bool supports_head_dim(int64_t head_dim) { return head_dim == 64 || head_dim == 128; }

template <typename F>
void dispatch_head_dim(int64_t head_dim, F&& fn) {
  TVM_FFI_ICHECK(supports_head_dim(head_dim)) << "head dimension must be 64 or 128";
  if (head_dim == 64) {
    fn(std::integral_constant<uint32_t, 64>{});
  } else {
    fn(std::integral_constant<uint32_t, 128>{});
  }
}
...
void check_v2g_shard(const TensorView& tensor, const char* name) {
  ...
  // Heads are parametric; kernels specialize the two supported head dimensions.
  TVM_FFI_ICHECK(tensor.size(0) > 0 && tensor.size(1) > 0 && tensor.size(2) > 0 &&
                 supports_head_dim(tensor.size(3)))
      << name << " must be a non-empty [B,L,H,D] tensor with D=64 or 128";
  // float4 loads require every row to stay aligned, including projection views.
  TVM_FFI_ICHECK_EQ(reinterpret_cast<uintptr_t>(tensor.data_ptr()) % 16, 0)
      << name << " must keep 16-byte alignment of every head_dim row";
  for (int i = 0; i < 3; ++i) {
    TVM_FFI_ICHECK_EQ(tensor.stride(i) % 8, 0)
        << name << " must keep 16-byte alignment of every head_dim row";
  }
}
...
// [B, H, ceil(L/256), D] fp32 and passes them through.
void ulysses_lowp_k_sum_v_amax(TensorView k, TensorView v, TensorView k_sum, TensorView v_amax,
  ...
  TVM_FFI_ICHECK(supports_head_dim(k.size(3))) << "head dimension must be 64 or 128";
  ...
  dispatch_head_dim(k.size(3), [&](auto dim) {
    constexpr uint32_t HEAD_DIM = decltype(dim)::value;
    ...
    DISPATCH_DLPACK_DTYPE_TO_CTYPE_FP16(k.dtype(), c_type, [&] {
      launch_kernel("KSumVAmaxPartialKernel", enable_pdl,
                    lowp::KSumVAmaxPartialKernel<c_type, HEAD_DIM, CHUNK_TOKENS>, grid, block,
                    stream, static_cast<const c_type*>(k.data_ptr()),
                    static_cast<const c_type*>(v.data_ptr()),
                    static_cast<float*>(k_partial.data_ptr()),
                    static_cast<float*>(v_partial.data_ptr()), static_cast<uint32_t>(tokens),
                    static_cast<uint32_t>(heads), static_cast<uint32_t>(chunks), k.stride(0),
    ...
    launch_kernel("KSumVAmaxCombineKernel", enable_pdl, lowp::KSumVAmaxCombineKernel<HEAD_DIM>,
                  grid2, block2, stream, static_cast<const float*>(k_partial.data_ptr()),
...
void ulysses_lowp_q_grouped_amax(TensorView q, TensorView amax_out, int64_t rank,
  ...
  const int64_t slots_alloc = lowp::grid::slots(local_sequence, Q_GROUP);
  const int64_t group_first = lowp::grid::group_first(rank, local_sequence, Q_GROUP);
  const int64_t touched = lowp::grid::group_last(rank, local_sequence, Q_GROUP) - group_first + 1;
  check_shape_3d(amax_out, "amax_out", q.size(0), q.size(2), slots_alloc);

  ffi::CUDADeviceGuard device_guard(q.device().device_id);
  const cudaStream_t stream = get_stream(q.device());
  dispatch_head_dim(q.size(3), [&](auto dim) {
    constexpr uint32_t HEAD_DIM = decltype(dim)::value;
    DISPATCH_DLPACK_DTYPE_TO_CTYPE_FP16(q.dtype(), c_type, [&] {
      constexpr uint32_t GROUP = Q_GROUP;
...
void ulysses_lowp_k_grouped_amax(TensorView k, TensorView k_mean, TensorView amax_out, int64_t rank,
  ...
  const int64_t slots_alloc = lowp::grid::slots(local_sequence, K_GROUP);
  const int64_t group_first = lowp::grid::group_first(rank, local_sequence, K_GROUP);
  const int64_t touched = lowp::grid::group_last(rank, local_sequence, K_GROUP) - group_first + 1;
  ...
  dispatch_head_dim(k.size(3), [&](auto dim) {
    constexpr uint32_t HEAD_DIM = decltype(dim)::value;
    DISPATCH_DLPACK_DTYPE_TO_CTYPE_FP16(k.dtype(), c_type, [&] {
      constexpr uint32_t GROUP = K_GROUP;
      ...
      launch_kernel("GroupedAmaxKernel(K)", enable_pdl,
                    ...
                    static_cast<const c_type*>(k.data_ptr()),
...
void ulysses_lowp_quant_q_int8_pack(TensorView q, TensorView q_amax_final, TensorView output,
  ...
  const lowp::grid::ChunkSpec spec =
      lowp::grid::chunk_spec(batch_size, local_sequence, local_heads, head_dim, Q_GROUP, K_GROUP);
  const int64_t group_first = lowp::grid::group_first(rank, local_sequence, Q_GROUP);
  const int64_t touched = lowp::grid::group_last(rank, local_sequence, Q_GROUP) - group_first + 1;
  ...
  dispatch_head_dim(q.size(3), [&](auto dim) {
    ...
    DISPATCH_DLPACK_DTYPE_TO_CTYPE_FP16(q.dtype(), c_type, [&] {
      ...
      launch_kernel("QuantInt8GroupScalePackKernel(Q)", enable_pdl,
                    lowp::QuantInt8GroupScalePackKernel<HEAD_DIM, GROUP, false, c_type>, grid,
                    block, stream, static_cast<const c_type*>(q.data_ptr()),
                    static_cast<const c_type*>(nullptr),
...
// General receiver: each token resolves its source, even if a tile crosses
// several shards. Scale slots beyond the communication sequence are zero;
// extra Q slots beyond the live prefix may hold initialized nonzero scales.
void ulysses_lowp_unpack_for_sage_unaligned(TensorView input, TensorView q, TensorView k,
  ...
  TVM_FFI_ICHECK(q.size(0) > 0 && q.size(2) > 0 && supports_head_dim(q.size(3)))
      << "q must have non-empty [B,S,h,D] shape with D=64 or 128";

  const int64_t batch_size = q.size(0);
  const int64_t logical_sequence = local_sequence * world_size;
  const int64_t local_heads = q.size(2);
  const int64_t head_dim = q.size(3);
  // V storage must cover complete Sage CTA_K tiles on each architecture.
  const int64_t padded_sequence = (logical_sequence + K_GROUP - 1) / K_GROUP * K_GROUP;
  // Scale tensors are emitted at the width the Sage consumer derives from the
  // rows it is given (four Q groups per CTA_Q, one K group per CTA_K); a
  ...
  const int64_t q_scale_alloc = (scale_rows + 4 * Q_GROUP - 1) / (4 * Q_GROUP) * 4;
  const int64_t k_scale_alloc = (scale_rows + K_GROUP - 1) / K_GROUP;
  const lowp::grid::ChunkSpec spec =
      lowp::grid::chunk_spec(batch_size, local_sequence, local_heads, head_dim, Q_GROUP, K_GROUP);
  ...
  dispatch_head_dim(q.size(3), [&](auto dim) {
    ...
    launch_kernel("UnpackForSageUnalignedKernel", enable_pdl,
                  (lowp::UnpackForSageUnalignedKernel<HEAD_DIM, CTA_SIZE, Q_GROUP, K_GROUP>), grid,
                  block, stream, static_cast<const uint8_t*>(input.data_ptr()),
...
int64_t ulysses_lowp_compiled_q_group() { return Q_GROUP; }
int64_t ulysses_lowp_compiled_k_group() { return K_GROUP; }
// Retain the original default-dimension query for existing callers.
int64_t ulysses_lowp_compiled_head_dim() { return 128; }
bool ulysses_lowp_supports_head_dim(int64_t head_dim) { return supports_head_dim(head_dim); }
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(ulysses_lowp_supports_head_dim, ulysses_lowp_supports_head_dim);
