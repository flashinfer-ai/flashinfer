...
// Generator source sha256:ce4bf0aba8a398979b70df863658baa9b3700fa840b4be957997fd0bee64cf0b.
...
#define CAKE_SOFTMAX_INF CUDART_INF_F
...
kernel_mr474_manual_softmax_exp2_t512_vec4(float* __restrict__ x, float* __restrict__ temperature, float* __restrict__ y, float temperature_scalar)
{
    ...
    float local_max = -CAKE_SOFTMAX_INF;
    #pragma unroll
    ...
    __syncthreads();
    float block_max = ((lane < 16) ? reduce_smem[lane] : -CAKE_SOFTMAX_INF);
    ...
    for (int tile_1 = 0; tile_1 < VECTORS_PER_THREAD; tile_1++) {
        if ((tile_1 * 512 + tid) * 4 < VOCAB_SIZE) {
            ...
            for (int j_1 = 0; j_1 < 4; j_1++) {
                ...
                float exp_value = ((row_max == -CAKE_SOFTMAX_INF) ? 0.0f : _exp2_0);
