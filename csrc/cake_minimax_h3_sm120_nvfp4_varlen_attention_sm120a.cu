...
// call = four launches over packed THD tensors q, k, v [T, H, 128] BF16 with int32 cu_seqlens:
...
__global__ __launch_bounds__(256, 2) void
kernel_minimax_h3_sm120_varlen_quantize_nvfp4(__nv_bfloat16* __restrict__ Q, __nv_bfloat16* __restrict__ K, __nv_bfloat16* __restrict__ V, int* __restrict__ cu_seqlens, float* __restrict__ mean_k, float* __restrict__ v_scale, unsigned int* __restrict__ q4, unsigned int* __restrict__ k4, uint8_t* __restrict__ q_sf, uint8_t* __restrict__ k_sf, unsigned int* __restrict__ vt8, float* __restrict__ q_scale, float* __restrict__ k_scale, int total_tokens, int total_padded, int num_segments, int num_heads, int num_kblocks)
{
    const int tid = threadIdx.x;
    const int warp = make_warp_uniform(tid / 32);
    const int lane = tid % 32;
    ...
    for (int work = bid; work < num_kblocks * num_heads; work += num_bids) {
        ...
        for (int ps = 0; ps < 4; ps++) {
            ...
            uint16_t _e4m3x2_decode_1 = (uint16_t)((unsigned int)_e4m3x2_f32_0 & 0xFFu);
            ...
            asm("cvt.f32.f16 %0, %1;" : "=f"(_fp8_decode_0) : "h"(_f16_decode_1));
            ...
            {
                const uint4* _vptr_2 = reinterpret_cast<const uint4*>(K + base + 0);
                uint4 _vld_2[2];
                #pragma unroll
                for (int _blk = 0; _blk < 2; _blk++) {
            ...
            for (int quad = 0; quad < 4; quad++) {
                ...
                for (int e_3 = 0; e_3 < 4; e_3++) {
                    ...
                    float kc = kf - _vec_load_2[e_3];
                    kvals[ps * 16 + quad * 4 + e_3] = ((valid == 1) ? kc : 0.0f);
                }
            }
            tmp[0] = kvals[ps * 16] + kvals[ps * 16 + 1];
            tmp[1] = kvals[ps * 16] - kvals[ps * 16 + 1];
            tmp[2] = kvals[ps * 16 + 2] + kvals[ps * 16 + 3];
            tmp[3] = kvals[ps * 16 + 2] - kvals[ps * 16 + 3];
            tmp[4] = kvals[ps * 16 + 4] + kvals[ps * 16 + 5];
            tmp[5] = kvals[ps * 16 + 4] - kvals[ps * 16 + 5];
            tmp[6] = kvals[ps * 16 + 6] + kvals[ps * 16 + 7];
            tmp[7] = kvals[ps * 16 + 6] - kvals[ps * 16 + 7];
            tmp[8] = kvals[ps * 16 + 8] + kvals[ps * 16 + 9];
            tmp[9] = kvals[ps * 16 + 8] - kvals[ps * 16 + 9];
            tmp[10] = kvals[ps * 16 + 10] + kvals[ps * 16 + 11];
            tmp[11] = kvals[ps * 16 + 10] - kvals[ps * 16 + 11];
            tmp[12] = kvals[ps * 16 + 12] + kvals[ps * 16 + 13];
            tmp[13] = kvals[ps * 16 + 12] - kvals[ps * 16 + 13];
            tmp[14] = kvals[ps * 16 + 14] + kvals[ps * 16 + 15];
            tmp[15] = kvals[ps * 16 + 14] - kvals[ps * 16 + 15];
            kvals[ps * 16] = tmp[0];
            kvals[ps * 16 + 1] = tmp[1];
            kvals[ps * 16 + 2] = tmp[2];
            kvals[ps * 16 + 3] = tmp[3];
            ...
            kvals[ps * 16 + 8] = tmp[8];
            kvals[ps * 16 + 9] = tmp[9];
            kvals[ps * 16 + 10] = tmp[10];
            kvals[ps * 16 + 11] = tmp[11];
            kvals[ps * 16 + 12] = tmp[12];
            kvals[ps * 16 + 13] = tmp[13];
            kvals[ps * 16 + 14] = tmp[14];
            kvals[ps * 16 + 15] = tmp[15];
            tmp[0] = kvals[ps * 16] + kvals[ps * 16 + 2];
            tmp[1] = kvals[ps * 16 + 1] + kvals[ps * 16 + 3];
            tmp[2] = kvals[ps * 16] - kvals[ps * 16 + 2];
            tmp[3] = kvals[ps * 16 + 1] - kvals[ps * 16 + 3];
            tmp[4] = kvals[ps * 16 + 4] + kvals[ps * 16 + 6];
            tmp[5] = kvals[ps * 16 + 5] + kvals[ps * 16 + 7];
            tmp[6] = kvals[ps * 16 + 4] - kvals[ps * 16 + 6];
            tmp[7] = kvals[ps * 16 + 5] - kvals[ps * 16 + 7];
            tmp[8] = kvals[ps * 16 + 8] + kvals[ps * 16 + 10];
            tmp[9] = kvals[ps * 16 + 9] + kvals[ps * 16 + 11];
            tmp[10] = kvals[ps * 16 + 8] - kvals[ps * 16 + 10];
            tmp[11] = kvals[ps * 16 + 9] - kvals[ps * 16 + 11];
            tmp[12] = kvals[ps * 16 + 12] + kvals[ps * 16 + 14];
            tmp[13] = kvals[ps * 16 + 13] + kvals[ps * 16 + 15];
            tmp[14] = kvals[ps * 16 + 12] - kvals[ps * 16 + 14];
            tmp[15] = kvals[ps * 16 + 13] - kvals[ps * 16 + 15];
            kvals[ps * 16] = tmp[0];
            kvals[ps * 16 + 1] = tmp[1];
            kvals[ps * 16 + 2] = tmp[2];
            kvals[ps * 16 + 3] = tmp[3];
            ...
            kvals[ps * 16 + 14] = tmp[14];
            kvals[ps * 16 + 15] = tmp[15];
            tmp[0] = kvals[ps * 16] + kvals[ps * 16 + 4];
            tmp[1] = kvals[ps * 16 + 1] + kvals[ps * 16 + 5];
            tmp[2] = kvals[ps * 16 + 2] + kvals[ps * 16 + 6];
            tmp[3] = kvals[ps * 16 + 3] + kvals[ps * 16 + 7];
            tmp[4] = kvals[ps * 16] - kvals[ps * 16 + 4];
            tmp[5] = kvals[ps * 16 + 1] - kvals[ps * 16 + 5];
            tmp[6] = kvals[ps * 16 + 2] - kvals[ps * 16 + 6];
            tmp[7] = kvals[ps * 16 + 3] - kvals[ps * 16 + 7];
            tmp[8] = kvals[ps * 16 + 8] + kvals[ps * 16 + 12];
            tmp[9] = kvals[ps * 16 + 9] + kvals[ps * 16 + 13];
            tmp[10] = kvals[ps * 16 + 10] + kvals[ps * 16 + 14];
            tmp[11] = kvals[ps * 16 + 11] + kvals[ps * 16 + 15];
            tmp[12] = kvals[ps * 16 + 8] - kvals[ps * 16 + 12];
            tmp[13] = kvals[ps * 16 + 9] - kvals[ps * 16 + 13];
            tmp[14] = kvals[ps * 16 + 10] - kvals[ps * 16 + 14];
            tmp[15] = kvals[ps * 16 + 11] - kvals[ps * 16 + 15];
            kvals[ps * 16] = tmp[0];
            kvals[ps * 16 + 1] = tmp[1];
            kvals[ps * 16 + 2] = tmp[2];
            kvals[ps * 16 + 3] = tmp[3];
            ...
            kvals[ps * 16 + 14] = tmp[14];
            kvals[ps * 16 + 15] = tmp[15];
            tmp[0] = kvals[ps * 16] + kvals[ps * 16 + 8];
            tmp[1] = kvals[ps * 16 + 1] + kvals[ps * 16 + 9];
            tmp[2] = kvals[ps * 16 + 2] + kvals[ps * 16 + 10];
            tmp[3] = kvals[ps * 16 + 3] + kvals[ps * 16 + 11];
            tmp[4] = kvals[ps * 16 + 4] + kvals[ps * 16 + 12];
            tmp[5] = kvals[ps * 16 + 5] + kvals[ps * 16 + 13];
            tmp[6] = kvals[ps * 16 + 6] + kvals[ps * 16 + 14];
            tmp[7] = kvals[ps * 16 + 7] + kvals[ps * 16 + 15];
            tmp[8] = kvals[ps * 16] - kvals[ps * 16 + 8];
            tmp[9] = kvals[ps * 16 + 1] - kvals[ps * 16 + 9];
            tmp[10] = kvals[ps * 16 + 2] - kvals[ps * 16 + 10];
            tmp[11] = kvals[ps * 16 + 3] - kvals[ps * 16 + 11];
            tmp[12] = kvals[ps * 16 + 4] - kvals[ps * 16 + 12];
            tmp[13] = kvals[ps * 16 + 5] - kvals[ps * 16 + 13];
            tmp[14] = kvals[ps * 16 + 6] - kvals[ps * 16 + 14];
            tmp[15] = kvals[ps * 16 + 7] - kvals[ps * 16 + 15];
            kvals[ps * 16] = tmp[0];
            kvals[ps * 16 + 1] = tmp[1];
            kvals[ps * 16 + 2] = tmp[2];
            kvals[ps * 16 + 3] = tmp[3];
            ...
            kvals[ps * 16 + 14] = tmp[14];
            kvals[ps * 16 + 15] = tmp[15];
            float _shfl_xor_53 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16], 1);
            tmp[0] = _shfl_xor_53;
            float _shfl_xor_54 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 1], 1);
            tmp[1] = _shfl_xor_54;
            float _shfl_xor_55 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 2], 1);
            tmp[2] = _shfl_xor_55;
            float _shfl_xor_56 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 3], 1);
            tmp[3] = _shfl_xor_56;
            float _shfl_xor_57 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 4], 1);
            tmp[4] = _shfl_xor_57;
            float _shfl_xor_58 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 5], 1);
            tmp[5] = _shfl_xor_58;
            float _shfl_xor_59 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 6], 1);
            tmp[6] = _shfl_xor_59;
            float _shfl_xor_60 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 7], 1);
            tmp[7] = _shfl_xor_60;
            float _shfl_xor_61 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 8], 1);
            tmp[8] = _shfl_xor_61;
            float _shfl_xor_62 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 9], 1);
            tmp[9] = _shfl_xor_62;
            float _shfl_xor_63 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 10], 1);
            tmp[10] = _shfl_xor_63;
            float _shfl_xor_64 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 11], 1);
            tmp[11] = _shfl_xor_64;
            float _shfl_xor_65 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 12], 1);
            tmp[12] = _shfl_xor_65;
            float _shfl_xor_66 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 13], 1);
            tmp[13] = _shfl_xor_66;
            float _shfl_xor_67 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 14], 1);
            tmp[14] = _shfl_xor_67;
            float _shfl_xor_68 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 15], 1);
            tmp[15] = _shfl_xor_68;
            kvals[ps * 16] = (((sub & 1) == 1) ? tmp[0] - kvals[ps * 16] : kvals[ps * 16] + tmp[0]);
            kvals[ps * 16 + 1] = (((sub & 1) == 1) ? tmp[1] - kvals[ps * 16 + 1] : kvals[ps * 16 + 1] + tmp[1]);
            kvals[ps * 16 + 2] = (((sub & 1) == 1) ? tmp[2] - kvals[ps * 16 + 2] : kvals[ps * 16 + 2] + tmp[2]);
            kvals[ps * 16 + 3] = (((sub & 1) == 1) ? tmp[3] - kvals[ps * 16 + 3] : kvals[ps * 16 + 3] + tmp[3]);
            kvals[ps * 16 + 4] = (((sub & 1) == 1) ? tmp[4] - kvals[ps * 16 + 4] : kvals[ps * 16 + 4] + tmp[4]);
            kvals[ps * 16 + 5] = (((sub & 1) == 1) ? tmp[5] - kvals[ps * 16 + 5] : kvals[ps * 16 + 5] + tmp[5]);
            kvals[ps * 16 + 6] = (((sub & 1) == 1) ? tmp[6] - kvals[ps * 16 + 6] : kvals[ps * 16 + 6] + tmp[6]);
            kvals[ps * 16 + 7] = (((sub & 1) == 1) ? tmp[7] - kvals[ps * 16 + 7] : kvals[ps * 16 + 7] + tmp[7]);
            kvals[ps * 16 + 8] = (((sub & 1) == 1) ? tmp[8] - kvals[ps * 16 + 8] : kvals[ps * 16 + 8] + tmp[8]);
            kvals[ps * 16 + 9] = (((sub & 1) == 1) ? tmp[9] - kvals[ps * 16 + 9] : kvals[ps * 16 + 9] + tmp[9]);
            kvals[ps * 16 + 10] = (((sub & 1) == 1) ? tmp[10] - kvals[ps * 16 + 10] : kvals[ps * 16 + 10] + tmp[10]);
            kvals[ps * 16 + 11] = (((sub & 1) == 1) ? tmp[11] - kvals[ps * 16 + 11] : kvals[ps * 16 + 11] + tmp[11]);
            kvals[ps * 16 + 12] = (((sub & 1) == 1) ? tmp[12] - kvals[ps * 16 + 12] : kvals[ps * 16 + 12] + tmp[12]);
            kvals[ps * 16 + 13] = (((sub & 1) == 1) ? tmp[13] - kvals[ps * 16 + 13] : kvals[ps * 16 + 13] + tmp[13]);
            kvals[ps * 16 + 14] = (((sub & 1) == 1) ? tmp[14] - kvals[ps * 16 + 14] : kvals[ps * 16 + 14] + tmp[14]);
            kvals[ps * 16 + 15] = (((sub & 1) == 1) ? tmp[15] - kvals[ps * 16 + 15] : kvals[ps * 16 + 15] + tmp[15]);
            float _shfl_xor_69 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16], 2);
            tmp[0] = _shfl_xor_69;
            float _shfl_xor_70 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 1], 2);
            tmp[1] = _shfl_xor_70;
            float _shfl_xor_71 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 2], 2);
            tmp[2] = _shfl_xor_71;
            float _shfl_xor_72 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 3], 2);
            tmp[3] = _shfl_xor_72;
            float _shfl_xor_73 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 4], 2);
            tmp[4] = _shfl_xor_73;
            float _shfl_xor_74 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 5], 2);
            tmp[5] = _shfl_xor_74;
            float _shfl_xor_75 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 6], 2);
            tmp[6] = _shfl_xor_75;
            float _shfl_xor_76 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 7], 2);
            tmp[7] = _shfl_xor_76;
            float _shfl_xor_77 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 8], 2);
            tmp[8] = _shfl_xor_77;
            float _shfl_xor_78 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 9], 2);
            tmp[9] = _shfl_xor_78;
            float _shfl_xor_79 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 10], 2);
            tmp[10] = _shfl_xor_79;
            float _shfl_xor_80 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 11], 2);
            tmp[11] = _shfl_xor_80;
            float _shfl_xor_81 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 12], 2);
            tmp[12] = _shfl_xor_81;
            float _shfl_xor_82 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 13], 2);
            tmp[13] = _shfl_xor_82;
            float _shfl_xor_83 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 14], 2);
            tmp[14] = _shfl_xor_83;
            float _shfl_xor_84 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 15], 2);
            tmp[15] = _shfl_xor_84;
            kvals[ps * 16] = (((sub >> 1 & 1) == 1) ? tmp[0] - kvals[ps * 16] : kvals[ps * 16] + tmp[0]);
            kvals[ps * 16 + 1] = (((sub >> 1 & 1) == 1) ? tmp[1] - kvals[ps * 16 + 1] : kvals[ps * 16 + 1] + tmp[1]);
            kvals[ps * 16 + 2] = (((sub >> 1 & 1) == 1) ? tmp[2] - kvals[ps * 16 + 2] : kvals[ps * 16 + 2] + tmp[2]);
            kvals[ps * 16 + 3] = (((sub >> 1 & 1) == 1) ? tmp[3] - kvals[ps * 16 + 3] : kvals[ps * 16 + 3] + tmp[3]);
            kvals[ps * 16 + 4] = (((sub >> 1 & 1) == 1) ? tmp[4] - kvals[ps * 16 + 4] : kvals[ps * 16 + 4] + tmp[4]);
            kvals[ps * 16 + 5] = (((sub >> 1 & 1) == 1) ? tmp[5] - kvals[ps * 16 + 5] : kvals[ps * 16 + 5] + tmp[5]);
            kvals[ps * 16 + 6] = (((sub >> 1 & 1) == 1) ? tmp[6] - kvals[ps * 16 + 6] : kvals[ps * 16 + 6] + tmp[6]);
            kvals[ps * 16 + 7] = (((sub >> 1 & 1) == 1) ? tmp[7] - kvals[ps * 16 + 7] : kvals[ps * 16 + 7] + tmp[7]);
            kvals[ps * 16 + 8] = (((sub >> 1 & 1) == 1) ? tmp[8] - kvals[ps * 16 + 8] : kvals[ps * 16 + 8] + tmp[8]);
            kvals[ps * 16 + 9] = (((sub >> 1 & 1) == 1) ? tmp[9] - kvals[ps * 16 + 9] : kvals[ps * 16 + 9] + tmp[9]);
            kvals[ps * 16 + 10] = (((sub >> 1 & 1) == 1) ? tmp[10] - kvals[ps * 16 + 10] : kvals[ps * 16 + 10] + tmp[10]);
            kvals[ps * 16 + 11] = (((sub >> 1 & 1) == 1) ? tmp[11] - kvals[ps * 16 + 11] : kvals[ps * 16 + 11] + tmp[11]);
            kvals[ps * 16 + 12] = (((sub >> 1 & 1) == 1) ? tmp[12] - kvals[ps * 16 + 12] : kvals[ps * 16 + 12] + tmp[12]);
            kvals[ps * 16 + 13] = (((sub >> 1 & 1) == 1) ? tmp[13] - kvals[ps * 16 + 13] : kvals[ps * 16 + 13] + tmp[13]);
            kvals[ps * 16 + 14] = (((sub >> 1 & 1) == 1) ? tmp[14] - kvals[ps * 16 + 14] : kvals[ps * 16 + 14] + tmp[14]);
            kvals[ps * 16 + 15] = (((sub >> 1 & 1) == 1) ? tmp[15] - kvals[ps * 16 + 15] : kvals[ps * 16 + 15] + tmp[15]);
            float _shfl_xor_85 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16], 4);
            tmp[0] = _shfl_xor_85;
            float _shfl_xor_86 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 1], 4);
            tmp[1] = _shfl_xor_86;
            float _shfl_xor_87 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 2], 4);
            tmp[2] = _shfl_xor_87;
            float _shfl_xor_88 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 3], 4);
            tmp[3] = _shfl_xor_88;
            float _shfl_xor_89 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 4], 4);
            tmp[4] = _shfl_xor_89;
            float _shfl_xor_90 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 5], 4);
            tmp[5] = _shfl_xor_90;
            float _shfl_xor_91 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 6], 4);
            tmp[6] = _shfl_xor_91;
            float _shfl_xor_92 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 7], 4);
            tmp[7] = _shfl_xor_92;
            float _shfl_xor_93 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 8], 4);
            tmp[8] = _shfl_xor_93;
            float _shfl_xor_94 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 9], 4);
            tmp[9] = _shfl_xor_94;
            float _shfl_xor_95 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 10], 4);
            tmp[10] = _shfl_xor_95;
            float _shfl_xor_96 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 11], 4);
            tmp[11] = _shfl_xor_96;
            float _shfl_xor_97 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 12], 4);
            tmp[12] = _shfl_xor_97;
            float _shfl_xor_98 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 13], 4);
            tmp[13] = _shfl_xor_98;
            float _shfl_xor_99 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 14], 4);
            tmp[14] = _shfl_xor_99;
            float _shfl_xor_100 = __shfl_xor_sync(0xFFFFFFFF, kvals[ps * 16 + 15], 4);
            tmp[15] = _shfl_xor_100;
            kvals[ps * 16] = (((sub >> 2 & 1) == 1) ? tmp[0] - kvals[ps * 16] : kvals[ps * 16] + tmp[0]);
            kvals[ps * 16 + 1] = (((sub >> 2 & 1) == 1) ? tmp[1] - kvals[ps * 16 + 1] : kvals[ps * 16 + 1] + tmp[1]);
            kvals[ps * 16 + 2] = (((sub >> 2 & 1) == 1) ? tmp[2] - kvals[ps * 16 + 2] : kvals[ps * 16 + 2] + tmp[2]);
            kvals[ps * 16 + 3] = (((sub >> 2 & 1) == 1) ? tmp[3] - kvals[ps * 16 + 3] : kvals[ps * 16 + 3] + tmp[3]);
            kvals[ps * 16 + 4] = (((sub >> 2 & 1) == 1) ? tmp[4] - kvals[ps * 16 + 4] : kvals[ps * 16 + 4] + tmp[4]);
            kvals[ps * 16 + 5] = (((sub >> 2 & 1) == 1) ? tmp[5] - kvals[ps * 16 + 5] : kvals[ps * 16 + 5] + tmp[5]);
            kvals[ps * 16 + 6] = (((sub >> 2 & 1) == 1) ? tmp[6] - kvals[ps * 16 + 6] : kvals[ps * 16 + 6] + tmp[6]);
            kvals[ps * 16 + 7] = (((sub >> 2 & 1) == 1) ? tmp[7] - kvals[ps * 16 + 7] : kvals[ps * 16 + 7] + tmp[7]);
            kvals[ps * 16 + 8] = (((sub >> 2 & 1) == 1) ? tmp[8] - kvals[ps * 16 + 8] : kvals[ps * 16 + 8] + tmp[8]);
            kvals[ps * 16 + 9] = (((sub >> 2 & 1) == 1) ? tmp[9] - kvals[ps * 16 + 9] : kvals[ps * 16 + 9] + tmp[9]);
            kvals[ps * 16 + 10] = (((sub >> 2 & 1) == 1) ? tmp[10] - kvals[ps * 16 + 10] : kvals[ps * 16 + 10] + tmp[10]);
            kvals[ps * 16 + 11] = (((sub >> 2 & 1) == 1) ? tmp[11] - kvals[ps * 16 + 11] : kvals[ps * 16 + 11] + tmp[11]);
            kvals[ps * 16 + 12] = (((sub >> 2 & 1) == 1) ? tmp[12] - kvals[ps * 16 + 12] : kvals[ps * 16 + 12] + tmp[12]);
            kvals[ps * 16 + 13] = (((sub >> 2 & 1) == 1) ? tmp[13] - kvals[ps * 16 + 13] : kvals[ps * 16 + 13] + tmp[13]);
            kvals[ps * 16 + 14] = (((sub >> 2 & 1) == 1) ? tmp[14] - kvals[ps * 16 + 14] : kvals[ps * 16 + 14] + tmp[14]);
            kvals[ps * 16 + 15] = (((sub >> 2 & 1) == 1) ? tmp[15] - kvals[ps * 16 + 15] : kvals[ps * 16 + 15] + tmp[15]);
            kvals[ps * 16] = kvals[ps * 16] * (sgn_hi * 0.08838834764831843f);
            kvals[ps * 16 + 1] = kvals[ps * 16 + 1] * (sgn_hi * -0.08838834764831843f);
            kvals[ps * 16 + 2] = kvals[ps * 16 + 2] * (sgn_hi * 0.08838834764831843f);
            kvals[ps * 16 + 3] = kvals[ps * 16 + 3] * (sgn_hi * 0.08838834764831843f);
            kvals[ps * 16 + 4] = kvals[ps * 16 + 4] * (sgn_hi * -0.08838834764831843f);
            kvals[ps * 16 + 5] = kvals[ps * 16 + 5] * (sgn_hi * 0.08838834764831843f);
            kvals[ps * 16 + 6] = kvals[ps * 16 + 6] * (sgn_hi * -0.08838834764831843f);
            kvals[ps * 16 + 7] = kvals[ps * 16 + 7] * (sgn_hi * -0.08838834764831843f);
            kvals[ps * 16 + 8] = kvals[ps * 16 + 8] * (sgn_hi * 0.08838834764831843f);
            kvals[ps * 16 + 9] = kvals[ps * 16 + 9] * (sgn_hi * 0.08838834764831843f);
            kvals[ps * 16 + 10] = kvals[ps * 16 + 10] * (sgn_hi * -0.08838834764831843f);
            kvals[ps * 16 + 11] = kvals[ps * 16 + 11] * (sgn_hi * 0.08838834764831843f);
            kvals[ps * 16 + 12] = kvals[ps * 16 + 12] * (sgn_hi * -0.08838834764831843f);
            kvals[ps * 16 + 13] = kvals[ps * 16 + 13] * (sgn_hi * -0.08838834764831843f);
            kvals[ps * 16 + 14] = kvals[ps * 16 + 14] * (sgn_hi * 0.08838834764831843f);
            kvals[ps * 16 + 15] = kvals[ps * 16 + 15] * (sgn_hi * -0.08838834764831843f);
            for (int e_4 = 0; e_4 < 16; e_4++) {
                float _fabs_1 = fabsf(kvals[ps * 16 + e_4]);
                float _fmax_5 = fmaxf(kmax, _fabs_1);
                kmax = _fmax_5;
            }
        }
        for (int stage = 0; stage < 5; stage++) {
            float _shfl_xor_101 = __shfl_xor_sync(0xFFFFFFFF, kmax, 16 >> stage);
            float _fmax_6 = fmaxf(kmax, _shfl_xor_101);
            kmax = _fmax_6;
        }
        if (lane == 0) {
            amax_part[warp] = kmax;
        ...
        for (int stage_1 = 0; stage_1 < 3; stage_1++) {
            float _shfl_xor_102 = __shfl_xor_sync(0xFFFFFFFF, kmax, 4 >> stage_1);
            float _fmax_7 = fmaxf(kmax, _shfl_xor_102);
            kmax = _fmax_7;
        }
        float _fmax_8 = fmaxf(kmax, 1e-12f);
        kmax = _fmax_8;
        float _fdiv_rn_3 = __fdiv_rn(2688.0f, kmax);
        float kg = _fdiv_rn_3;
        if (tid == 0) {
            float _fdiv_rn_4 = __fdiv_rn(kmax, 2688.0f);
            *(reinterpret_cast<float*>(k_scale + (head * num_kblocks + blk)) + (0)) = _fdiv_rn_4;
        }
        for (int ps_1 = 0; ps_1 < 4; ps_1++) {
            int tok2 = tok_base + ps_1 * 32 + trow;
            float kbmax = 0.0f;
            for (int e_5 = 0; e_5 < 16; e_5++) {
                float _fabs_2 = fabsf(kvals[ps_1 * 16 + e_5]);
                float _fmax_9 = fmaxf(kbmax, _fabs_2);
                kbmax = _fmax_9;
            }
            ...
            uint16_t _e4m3x2_decode_4 = (uint16_t)((unsigned int)_e4m3x2_f32_1 & 0xFFu);
            ...
            asm("cvt.f32.f16 %0, %1;" : "=f"(_fp8_decode_1) : "h"(_f16_decode_4));
            float ksf_dec = _fp8_decode_1;
            float _fdiv_rn_5 = __fdiv_rn(kg, ksf_dec);
            float kout = ((ksf_dec != 0.0f) ? _fdiv_rn_5 : 0.0f);
            float kn[16];
            for (int e_6 = 0; e_6 < 16; e_6++) {
                kn[e_6] = kvals[ps_1 * 16 + e_6] * kout;
            }
            unsigned int kw[2];
            ...
            unsigned int _shfl_xor_103 = __shfl_xor_sync(0xFFFFFFFF, kw[0], 1);
            kstore[0] = _shfl_xor_103;
            unsigned int _shfl_xor_104 = __shfl_xor_sync(0xFFFFFFFF, kw[1], 1);
            kstore[1] = _shfl_xor_104;
            kstore[2] = kw[0];
            kstore[3] = kw[1];
            {
                unsigned short _sf_pair;
                asm("cvt.rn.satfinite.e4m3x2.f32 %0, 0f00000000, %1;" : "=h"(_sf_pair) : "f"(ksf_val));
                *(reinterpret_cast<unsigned char*>(k_sf + (((head * num_kblocks + blk) * 128 + ps_1 * 32 + trow) * 8 + sub)) + (0)) = (unsigned char)(_sf_pair & 0x7F);
            }
            if (tok2 < total_tokens) {
                if ((sub & 1) == 1) {
                    reinterpret_cast<int4*>(k4 + (((tok2 * num_heads + head) * 64 + (sub - 1) * 8) / 4))[0] = reinterpret_cast<int4*>(kstore)[0];
                }
            }
        }
        for (int pass2 = 0; pass2 < 1; pass2++) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                {
                    const uint4* _vptr_5 = reinterpret_cast<const uint4*>(V + ((vtok_c * num_heads + head) * 128 + cg * 16) + 0);
                    uint4 _vld_5[2];
                    #pragma unroll
                    for (int _blk = 0; _blk < 2; _blk++) {
                ...
                for (int quad_1 = 0; quad_1 < 4; quad_1++) {
                    ...
                    for (int e_7 = 0; e_7 < 4; e_7++) {
                        float vf = _vec_load_3[quad_1 * 4 + e_7];
                        float _fdiv_rn_6 = __fdiv_rn(vf, _vec_load_4[e_7]);
                        vq[i * 16 + quad_1 * 4 + e_7] = ((vvalid == 1) ? _fdiv_rn_6 : 0.0f);
                    }
                }
            }
            int vt_word = (head * 128 + cg * 16) * total_padded + tok_base + chunk * 16 + 4 * t8 >> 2;
            for (int e_8 = 0; e_8 < 16; e_8++) {
                float four[4];
                for (int i_1 = 0; i_1 < 4; i_1++) {
                    four[i_1] = vq[i_1 * 16 + e_8];
                }
                unsigned int word[1];
                {
                    uint32_t _packed;
                ...
                *(reinterpret_cast<unsigned int*>(vt8 + (vt_word + e_8 * (total_padded >> 2))) + (0)) = word[0];
            }
        }
        __syncthreads();
    }
...
kernel_minimax_h3_sm120_varlen_attention_nvfp4(const __grid_constant__ CUtensorMap Q_map, const __grid_constant__ CUtensorMap K_map, const __grid_constant__ CUtensorMap KSF_map, const __grid_constant__ CUtensorMap V_map, __nv_bfloat16* __restrict__ O, float* __restrict__ q_scale, float* __restrict__ k_scale, float* __restrict__ v_scale, unsigned int* __restrict__ q_sf, int* __restrict__ seg_begin, int* __restrict__ seg_len, int* __restrict__ unit_table, int total_units, int num_heads, int num_kblocks, float softmax_scale_log2)
{
    const int tid = threadIdx.x;
    const int warp = make_warp_uniform(tid / 32);
    const int lane = tid % 32;
...
//   [>= tokens * heads], k_scale f32 [>= heads * num_kblocks], v_scale and mean_k f32
//   [>= segments * heads * 128], partials f32 [>= num_tiles * heads * 256].
void minimax_h3_sm120_varlen_attention_nvfp4(TensorView q, TensorView k, TensorView v, TensorView cu_seqlens,
                                             ...
                                             int64_t num_segments, int64_t num_tiles, int64_t num_units,
                                             int64_t attention_grid, double softmax_scale) {
  ...
  h3_varlen_quantize_nvfp4_sm120a::kernel_minimax_h3_sm120_varlen_quantize_nvfp4<<<quant_grid, kQuantThreads,
  ...
  h3_varlen_attention_nvfp4_sm120a::kernel_minimax_h3_sm120_varlen_attention_nvfp4<<<grid, kAttentionThreads,
