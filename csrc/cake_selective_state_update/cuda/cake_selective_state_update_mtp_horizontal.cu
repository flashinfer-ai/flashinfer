...
__global__ __launch_bounds__(288, 4) void kernel_cake_selective_state_update_mtp_horizontal(
    CakeTensorMap const* state_tma, CakeTensorMap const* x_tma, CakeTensorMap const* b_tma,
    CakeTensorMap const* c_tma, float* __restrict__ dt, float* __restrict__ A,
  ...
  if (tid == 0) {
    asm volatile(
        ...
        : "memory");
    asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" ::"l"((uint64_t)(x_tma))
                 : "memory");
    asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" ::"l"((uint64_t)(b_tma))
                 : "memory");
    asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" ::"l"((uint64_t)(c_tma))
                 : "memory");
  }
  __syncthreads();
