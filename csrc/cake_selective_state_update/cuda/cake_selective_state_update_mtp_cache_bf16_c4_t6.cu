...
__global__ __launch_bounds__(128, 7) void kernel_cake_selective_state_update_mtp_cache_bf16_c4_t6(
    ...
    long long* __restrict__ intermediate_state_indices, int nheads, int ngroups,
    unsigned long long state_stride_slot, unsigned long long intermediate_stride_slot,
...
#pragma unroll
  for (int pack_turn = 0; pack_turn < 3; pack_turn++) {
    ...
    int step = pack / 16;
    int col = pack % 16 * 8;
    int source_index = ((batch * 6 + step) * ngroups + group) * 128 + col;
    if (warp == 0) {
  ...
  for (int step_1 = warp; step_1 < 6; step_1 += 4) {
    for (int col_1 = lane * 8; col_1 < 16; col_1 += 256) {
      int source_index_1 = ((token_base + step_1) * nheads + head) * 64 + dim_base + col_1;
      asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16;" ::"r"(
...
#pragma unroll
  for (int pack_turn_1 = 0; pack_turn_1 < 2; pack_turn_1++) {
    ...
    unsigned long long state_index =
        (unsigned long long)source_slot * state_stride_slot + (unsigned long long)state_row;
    asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16;" ::"r"(
  ...
  if (tid < 6) {
    int step_2 = tid;
    float dt_value = dt[(token_base + step_2) * nheads + head];
...
#pragma unroll
  for (int dim_pass = 0; dim_pass < 1; dim_pass++) {
    ...
    unsigned long long intermediate_row_base = 0;
    {
      ...
      intermediate_row_base = (unsigned long long)cache_slot * intermediate_stride_slot +
...
#pragma unroll
    for (int step_3 = 0; step_3 < 6; step_3++) {
      ...
      float _expf_1 = __expf(a_value * dt_value_1);
      float decay = _expf_1;
...
#pragma unroll
      ...
      partial += _shfl_xor_2;
      if (member == 0) {
        int output_index = ((token_base + step_3) * nheads + head) * 64 + dim_index;
          ...
          {
            {
              ...
              _pk[0] =
                  __floats2bfloat162_rn(state_values[tile_2 * 8 + 0], state_values[tile_2 * 8 + 1]);
              _pk[1] =
                  __floats2bfloat162_rn(state_values[tile_2 * 8 + 2], state_values[tile_2 * 8 + 3]);
              _pk[2] =
                  __floats2bfloat162_rn(state_values[tile_2 * 8 + 4], state_values[tile_2 * 8 + 5]);
              _pk[3] =
                  __floats2bfloat162_rn(state_values[tile_2 * 8 + 6], state_values[tile_2 * 8 + 7]);
...
#pragma unroll
      for (int pack_turn_2 = 0; pack_turn_2 < 2; pack_turn_2++) {
        ...
        unsigned long long state_index_1 =
            (unsigned long long)source_slot * state_stride_slot + (unsigned long long)state_row_1;
        asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16;" ::"r"(
