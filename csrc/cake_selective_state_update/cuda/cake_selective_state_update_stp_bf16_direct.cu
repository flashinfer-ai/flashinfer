...
__global__ __launch_bounds__(288, 4) void kernel_cake_selective_state_update_stp_bf16_direct(
    CakeTensorMap const* state_tma, __nv_bfloat16* __restrict__ x, float* __restrict__ dt,
    float* __restrict__ A, __nv_bfloat16* __restrict__ B, __nv_bfloat16* __restrict__ C,
    float* __restrict__ D, __nv_bfloat16* __restrict__ z, float* __restrict__ dt_bias,
    __nv_bfloat16* __restrict__ output, long long* __restrict__ state_batch_indices,
    long long* __restrict__ dst_state_batch_indices, int nheads, int ngroups, int head_tiles,
    int dt_softplus, int has_z, int disable_state_update) {
  ...
  if (tid == 0) {
    asm volatile(
        "fence.proxy.tensormap::generic.acquire.sys [%0], 128;" ::"l"((uint64_t)(state_tma))
        : "memory");
  }
  __syncthreads();
    ...
    {  // consumers_main
      ...
      if (local_warp == 0) {
        if (lane < 16) {
          ...
          asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16;" ::"r"(
                           s_b_addr + (unsigned int)(col * 2)),
                       "l"(B + ((batch * ngroups + group) * 128 + col)));
        }
      ...
      if (local_warp == 1) {
        if (lane < 16) {
          int col_1 = lane * 8;
          asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16;" ::"r"(
                           s_c_addr + (unsigned int)(col_1 * 2)),
                       "l"(C + ((batch * ngroups + group) * 128 + col_1)));
        }
        asm volatile("cp.async.commit_group;");
        asm volatile("cp.async.wait_group 0;");
      }
...
#pragma unroll
          for (int pair = 0; pair < 8; pair++) {
            ...
            asm volatile("ld.shared.b32 %0, [%1];"
                         ...
                         : "r"(s_state_addr + (unsigned int)(state_index * 2)));
            asm volatile("ld.shared.b32 %0, [%1];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&b_carrier[0]))
                         : "r"(s_b_addr + (unsigned int)(global_col * 2)));
            asm volatile("ld.shared.b32 %0, [%1];"
                         : "=r"(*reinterpret_cast<uint32_t*>(&c_carrier[0]))
                         : "r"(s_c_addr + (unsigned int)(global_col * 2)));
#pragma unroll
...
#pragma unroll
            ...
            asm volatile(
                "st.shared.b32 [%0], %1;" ::"r"(s_state_addr + (unsigned int)(state_index * 2)),
                "r"((state_values_bf16[0])));
          }
          asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
          if (elect_sync()) {
...
#pragma unroll
        for (int stage_1 = 0; stage_1 < 4; stage_1++) {
          tma_4d_gmem2smem(s_state_addr + (unsigned int)(stage_1 * 8192), state_tma, stage_1 * 32,
                           0, first_head, (int)source_slot, state_full_addr + (stage_1) * 8);
...
#pragma unroll
          for (int stage_2 = 0; stage_2 < 4; stage_2++) {
            ...
            if (disable_state_update == 0) {
              asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
              tma_store_4d(state_tma, stage_2 * 32, 0, head_1, (int)destination_slot,
                           s_state_addr + (unsigned int)(stage_2 * 8192));
              asm volatile("cp.async.bulk.commit_group;");
              asm volatile("cp.async.bulk.wait_group.read 0;");
            }
            if (next_head_valid != 0) {
              int next_head = head_1 + 1;
              tma_4d_gmem2smem(s_state_addr + (unsigned int)(stage_2 * 8192), state_tma,
                               stage_2 * 32, 0, next_head, (int)source_slot,
