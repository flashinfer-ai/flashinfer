...
__global__ __launch_bounds__(128, 8) void kernel_cake_selective_state_update_dynamic_checkpoint7(
    ...
    int batch_size, int token_steps, int previous_tokens, unsigned long long state_stride_slot,
...
#pragma unroll
    for (int element = 0; element < 4; element++) {
      state_values[tile * 4 + element] = s_state[local_row * 128 + state_col + element];
    }
  }
...
#pragma unroll
        for (int element_1 = 0; element_1 < 4; element_1++) {
          int col_4 = state_col_1 + element_1;
          ...
          state_values[tile_1 * 4 + element_1] =
              state_values[tile_1 * 4 + element_1] * decay + b_value * dtx;
          partial += state_values[tile_1 * 4 + element_1] * c_value;
        }
      }
