...
kernel_gated_act_mxfp8_fwd_col_staged_64x64(const __grid_constant__ CUtensorMap gate_tma, const __grid_constant__ CUtensorMap up_tma, const __grid_constant__ CUtensorMap col_output_tma, uint8_t* __restrict__ col_scales, int M, int K)
{
    ...
    for (int stage = 0; stage < 1; stage++) {
        ...
        if (ty == 0) {
            *(reinterpret_cast<unsigned char*>(col_scales + scale_index) + (0)) = (unsigned char)(scale);
        }
