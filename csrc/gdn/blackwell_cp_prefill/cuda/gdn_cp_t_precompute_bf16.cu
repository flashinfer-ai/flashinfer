...
kernel_flashinfer_blackwell_gdn_cp_prefill_t_precompute_bf16_v1(const __grid_constant__ CUtensorMap K, float* __restrict__ beta, __nv_bfloat16* __restrict__ t, long long* __restrict__ cu_seqlens, int num_k_heads, int num_sab_heads, int total_t_blocks, int num_seqs)
{
