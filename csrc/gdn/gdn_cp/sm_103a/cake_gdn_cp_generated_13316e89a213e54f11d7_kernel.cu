...
kernel_cake_gdn_cp_13316e89a213e54f11d7(CakeTensorMap const* K, CakeTensorMap const* V, CakeTensorMap const* T, float* __restrict__ alpha, float* __restrict__ local_transfer, float* __restrict__ local_state, long long* __restrict__ cu_seqlens, int chunk_len, int num_k_heads, int num_v_heads, int num_sab_heads, int total_cp_chunks, int num_seqs)
{
    ...
    if (tid == 0) {
        ...
        asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" :: "l"((uint64_t)(T)) : "memory");
    }
    __syncthreads();
    ...
    // Mbarrier init (34 pipeline groups, 0 ordered-sequence groups, 54 barriers)
    ...
    if (warp <= 3) {
        ...
        { // compute_group_0_main
            ...
            if (blockIdx.x / num_sab_heads < ((int)cu_seqlens[blockIdx.y + 1] - (int)cu_seqlens[blockIdx.y] + chunk_len - 1) / chunk_len) {
                ...
                int row = (unsigned int)(warp_in_wg * 32) + lane;
                #pragma unroll
                ...
                int row_1 = (unsigned int)(warp_in_wg * 32) + lane;
                #pragma unroll
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // compute_group_1_main
            ...
            if (blockIdx.x / num_sab_heads < ((int)cu_seqlens[blockIdx.y + 1] - (int)cu_seqlens[blockIdx.y] + chunk_len - 1) / chunk_len) {
                ...
                int row_2 = (unsigned int)(warp_in_wg_1 * 32) + lane;
                #pragma unroll
