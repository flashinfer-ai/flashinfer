...
kernel_cake_gdn_cp_7c846a200567707f263b(const __grid_constant__ CUtensorMap Q, const __grid_constant__ CUtensorMap K, const __grid_constant__ CUtensorMap V, const __grid_constant__ CUtensorMap T, const __grid_constant__ CUtensorMap O, float* __restrict__ alpha, long long* __restrict__ cu_seqlens, float* __restrict__ fixed_state, float* __restrict__ initial_state_workspace, uint8_t* __restrict__ tensormap_workspace, int cp_chunk_len, int source_cp_chunk_len, int num_q_heads, int num_k_heads, int num_v_heads, int num_sab_heads, CakeFastDivmod cp_chunk_len_fdd, CakeFastDivmod num_sab_heads_fdd, float scale)
{
    ...
    // Mbarrier init (29 pipeline groups, 0 ordered-sequence groups, 59 barriers)
    ...
    if (warp <= 3) {
        ...
        { // compute_group_0_main
            unsigned int flat_work = blockIdx.x;
            uint32_t _fast_div_q_12 = (num_sab_heads_fdd.divisor != 1) ? (__umulhi((uint32_t)(flat_work), num_sab_heads_fdd.multiplier) >> num_sab_heads_fdd.shift_right) : (uint32_t)(flat_work);
            uint32_t _fast_div_r_12 = (uint32_t)(flat_work) - _fast_div_q_12 * (uint32_t)(num_sab_heads_fdd.divisor);
            int sab_head = _fast_div_r_12;
            int chunk_in_seq = _fast_div_q_12;
            int seq_idx = blockIdx.y;
            int seq_start = (int)cu_seqlens[seq_idx];
            int seq_end = (int)cu_seqlens[seq_idx + 1];
            int seq_len = seq_end - seq_start;
            uint32_t _fast_div_q_13 = (cp_chunk_len_fdd.divisor != 1) ? (__umulhi((uint32_t)((unsigned int)(seq_len + cp_chunk_len - 1)), cp_chunk_len_fdd.multiplier) >> cp_chunk_len_fdd.shift_right) : (uint32_t)((unsigned int)(seq_len + cp_chunk_len - 1));
            uint32_t _fast_div_r_13 = (uint32_t)((unsigned int)(seq_len + cp_chunk_len - 1)) - _fast_div_q_13 * (uint32_t)(cp_chunk_len_fdd.divisor);
            int num_cp_chunks = _fast_div_q_13;
            int chunk_len = 0;
            ...
            uint32_t _fast_div_q_14 = (cp_chunk_len_fdd.divisor != 1) ? (__umulhi((uint32_t)((unsigned int)(seq_start - prefix_items)), cp_chunk_len_fdd.multiplier) >> cp_chunk_len_fdd.shift_right) : (uint32_t)((unsigned int)(seq_start - prefix_items));
            uint32_t _fast_div_r_14 = (uint32_t)((unsigned int)(seq_start - prefix_items)) - _fast_div_q_14 * (uint32_t)(cp_chunk_len_fdd.divisor);
            int cp_chunk = prefix_items + (int)_fast_div_q_14 + chunk_in_seq;
            ...
            if (chunk_in_seq < num_cp_chunks) {
                ...
                int t_ld_row_cg0 = (unsigned int)(warp_cg0 * 16) + (lane & 7) + (lane & 8);
                ...
                for (int block_cg0 = 0; block_cg0 < num_padded_blocks; block_cg0++) {
                    ...
                    {
                        valid_tokens_cg0 = chunk_end - (chunk_start + block_cg0 * 64);
                        if (valid_tokens_cg0 > 64) {
                            valid_tokens_cg0 = 64;
                        }
                        if (valid_tokens_cg0 < 0) {
                            valid_tokens_cg0 = 0;
                        }
                    }
                    int source_block_end_cg0 = chunk_start - seq_start + block_cg0 * 64 + valid_tokens_cg0;
                    int source_chunk_end_cg0 = (source_block_end_cg0 + source_cp_chunk_len - 1) / source_cp_chunk_len * source_cp_chunk_len;
                    int source_final_block_cg0 = ((valid_tokens_cg0 == 64 && source_block_end_cg0 == seq_len) ? 1 : 0);
                    {
                    ...
                    for (int t_col_tile_cg0 = 0; t_col_tile_cg0 < 4; t_col_tile_cg0++) {
                                    ...
                                    {
                                        {
                                            ...
                                            for (int t_item_cg0 = 0; t_item_cg0 < 8; t_item_cg0++) {
                                                ...
                                                _unpack_f16_0[t_item_cg0] = (-gamma_cg0) * _unpack_f16_0[t_item_cg0];
                                                int qk_valid_cg0 = 0;
                        ...
                        for (int _lp = 0; _lp < 4; _lp++) {
                            __half2 _h2 = __float22half2_rn(make_float2(_unpack_f16_0[_lp*2 + 0], _unpack_f16_0[_lp*2+1 + 0]));
                            t_signed_bits_cg0[_lp] = *(uint32_t*)&_h2;
                        }
                        int t_store_row_cg0 = (unsigned int)(t_col_tile_cg0 * 16) + (lane & 7) + (lane & 16) / 2;
                        int t_store_col_lane_cg0 = (unsigned int)(warp_cg0 * 16) + (lane & 8);
                    ...
                    int qk_store_row_cg0 = (unsigned int)qk_logical_row_base_cg0 + (lane & 7) + (lane & 8);
    ...
    // ---- Role: compute_group_1 ----
        ...
        { // compute_group_1_main
            unsigned int flat_work_1 = blockIdx.x;
            uint32_t _fast_div_q_15 = (num_sab_heads_fdd.divisor != 1) ? (__umulhi((uint32_t)(flat_work_1), num_sab_heads_fdd.multiplier) >> num_sab_heads_fdd.shift_right) : (uint32_t)(flat_work_1);
            uint32_t _fast_div_r_15 = (uint32_t)(flat_work_1) - _fast_div_q_15 * (uint32_t)(num_sab_heads_fdd.divisor);
            int sab_head_1 = _fast_div_r_15;
            int chunk_in_seq_1 = _fast_div_q_15;
            ...
            uint32_t _fast_div_q_16 = (cp_chunk_len_fdd.divisor != 1) ? (__umulhi((uint32_t)((unsigned int)(seq_len_1 + cp_chunk_len - 1)), cp_chunk_len_fdd.multiplier) >> cp_chunk_len_fdd.shift_right) : (uint32_t)((unsigned int)(seq_len_1 + cp_chunk_len - 1));
            uint32_t _fast_div_r_16 = (uint32_t)((unsigned int)(seq_len_1 + cp_chunk_len - 1)) - _fast_div_q_16 * (uint32_t)(cp_chunk_len_fdd.divisor);
            int num_cp_chunks_1 = _fast_div_q_16;
            int chunk_len_1 = 0;
            if (chunk_in_seq_1 < num_cp_chunks_1) {
            ...
            uint32_t _fast_div_q_17 = (cp_chunk_len_fdd.divisor != 1) ? (__umulhi((uint32_t)((unsigned int)(seq_start_1 - prefix_items_1)), cp_chunk_len_fdd.multiplier) >> cp_chunk_len_fdd.shift_right) : (uint32_t)((unsigned int)(seq_start_1 - prefix_items_1));
            uint32_t _fast_div_r_17 = (uint32_t)((unsigned int)(seq_start_1 - prefix_items_1)) - _fast_div_q_17 * (uint32_t)(cp_chunk_len_fdd.divisor);
            int cp_chunk_1 = prefix_items_1 + (int)_fast_div_q_17 + chunk_in_seq_1;
            ...
            if (chunk_in_seq_1 < num_cp_chunks_1) {
                ...
                int state_slot_cg1 = ((chunk_in_seq_1 > 0) ? (cp_chunk_1 - 1) * num_sab_heads + sab_head_1 : seq_idx_1 * num_sab_heads + sab_head_1);
                int state_row_cg1 = (unsigned int)(warp_cg1 * 32) + lane;
                ...
                for (int block_cg1 = 0; block_cg1 < num_padded_blocks_1; block_cg1++) {
                    ...
                    int source_block_end_cg1 = chunk_start_1 - seq_start_1 + block_cg1 * 64 + valid_tokens_cg1;
                    int source_chunk_end_cg1 = (source_block_end_cg1 + source_cp_chunk_len - 1) / source_cp_chunk_len * source_cp_chunk_len;
                    int source_final_block_cg1 = ((valid_tokens_cg1 == 64 && source_block_end_cg1 == seq_len_1) ? 1 : 0);
                    {
                    ...
                    int _trl_addr_2 = tmem_tmem_state + (warp_cg1 * 32 << 16);
                    tmem_ld_x32(&state_values_cg1[0], _trl_addr_2);
                    int _trl_addr_3 = tmem_tmem_state + (32) + (warp_cg1 * 32 << 16);
                    tmem_ld_x32(&state_values_cg1[32], _trl_addr_3);
                    int _trl_addr_4 = tmem_tmem_state + (64) + (warp_cg1 * 32 << 16);
                    tmem_ld_x32(&state_values_cg1[64], _trl_addr_4);
                    int _trl_addr_5 = tmem_tmem_state + (96) + (warp_cg1 * 32 << 16);
                    tmem_ld_x32(&state_values_cg1[96], _trl_addr_5);
                    asm volatile("tcgen05.wait::ld.sync.aligned;" ::: "memory");
                    {
                        ...
                        for (int state_col_block_cg1_1 = 0; state_col_block_cg1_1 < 4; state_col_block_cg1_1++) {
                            asm volatile(
                                ...
                                :: "r"(taddr + 192 + (unsigned int)tmem_row_base_cg1 + (unsigned int)(state_col_block_cg1_1 * 16)), "r"((state_input_bits_cg1 + state_col_block_cg1_1 * 16)[0]), "r"((state_input_bits_cg1 + state_col_block_cg1_1 * 16)[1]), "r"((state_input_bits_cg1 + state_col_block_cg1_1 * 16)[2]), "r"((state_input_bits_cg1 + state_col_block_cg1_1 * 16)[3]), "r"((state_input_bits_cg1 + state_col_block_cg1_1 * 16)[4]), "r"((state_input_bits_cg1 + state_col_block_cg1_1 * 16)[5]), "r"((state_input_bits_cg1 + state_col_block_cg1_1 * 16)[6]), "r"((state_input_bits_cg1 + state_col_block_cg1_1 * 16)[7]), "r"((state_input_bits_cg1 + state_col_block_cg1_1 * 16)[8]), "r"((state_input_bits_cg1 + state_col_block_cg1_1 * 16)[9]), "r"((state_input_bits_cg1 + state_col_block_cg1_1 * 16)[10]), "r"((state_input_bits_cg1 + state_col_block_cg1_1 * 16)[11]), "r"((state_input_bits_cg1 + state_col_block_cg1_1 * 16)[12]), "r"((state_input_bits_cg1 + state_col_block_cg1_1 * 16)[13]), "r"((state_input_bits_cg1 + state_col_block_cg1_1 * 16)[14]), "r"((state_input_bits_cg1 + state_col_block_cg1_1 * 16)[15]));
                        }
                        asm volatile("tcgen05.wait::st.sync.aligned;" ::: "memory");
                    }
                    mbarrier_arrive(state_input_ready_addr + (state_input_stage_cg1) * 8);
                    state_input_empty_phase_cg1 ^= 1;
                    const float2 _scale2_6 = {state_decay_cg1, state_decay_cg1};
                    #pragma unroll
                    for (int _ls = 0; _ls < 64; _ls++)
                        mul_f32x2_inplace(&reinterpret_cast<float2*>(state_values_cg1)[_ls], _scale2_6);
                    #pragma unroll
                    ...
                    for (int v_repeat_cg1 = 0; v_repeat_cg1 < 8; v_repeat_cg1++) {
                        ...
                        int v_token_cg1 = (unsigned int)(v_repeat_cg1 * 8) + (lane & 7);
                        ...
                        for (int vks_pair_cg1 = 0; vks_pair_cg1 < 16; vks_pair_cg1++) {
                            {
                                if (source_final_block_cg1 != 0) {
                                    ...
                                    uint32_t _packed16x2_sub_2;
                                    asm volatile("sub.rn.f16x2 %0, %1, %2;" : "=r"(_packed16x2_sub_2) : "r"(v_frag_lo_cg1[vks_pair_cg1]), "r"(ks_frag_lo_cg1_f16[vks_pair_cg1]));
                                    vks_bits_lo_cg1[vks_pair_cg1] = _packed16x2_sub_2;
                                    uint32_t _packed16x2_sub_3;
                                    asm volatile("sub.rn.f16x2 %0, %1, %2;" : "=r"(_packed16x2_sub_3) : "r"(v_frag_hi_cg1[vks_pair_cg1]), "r"(ks_frag_hi_cg1_f16[vks_pair_cg1]));
                                    vks_bits_hi_cg1[vks_pair_cg1] = _packed16x2_sub_3;
                                }
                            }
                        }
                    }
                    ...
                    mbarrier_arrive(vks_ready_addr);
                    {
                        asm volatile("tcgen05.wait::ld.sync.aligned;" ::: "memory");
                        {
                            ...
                            const float2 _scale2_7 = {scale, scale};
                            #pragma unroll
                            for (int _ls = 0; _ls < 16; _ls++)
                                mul_f32x2_inplace(&reinterpret_cast<float2*>(qs_frag_early_cg1)[_ls], _scale2_7);
                        }
                        if (source_final_block_cg1 != 0) {
                        ...
                        asm volatile("tcgen05.wait::ld.sync.aligned;");
                        {
                            ...
                            #pragma unroll
                            for (int _ls = 0; _ls < 16; _ls++)
                                mul_f32x2_inplace(&reinterpret_cast<float2*>(_tmem_load_1)[_ls], _scale2_8);
                        }
                        if (source_final_block_cg1 != 0) {
                            _tmem_load_1[0] = 0.0f;
                            _tmem_load_1[1] = 0.0f;
                    ...
                    int o_stage_addr_cg1 = smem_o_addr + o_stage_cg1 * 16384;
                    {
                        ...
                        for (int o_half_cg1 = 0; o_half_cg1 < 2; o_half_cg1++) {
                            ...
                            for (int o_group_cg1 = 0; o_group_cg1 < 4; o_group_cg1++) {
                                ...
                                uint32_t _stmatrix_addr_9 = static_cast<uint32_t>((unsigned long long)(o_stage_addr_cg1 + o_offset_cg1));
                                asm volatile("stmatrix.sync.aligned.m8n8.x4.trans.shared.b16 [%0], {%1, %2, %3, %4};\n"
                                    :: "r"(_stmatrix_addr_9), "r"(*reinterpret_cast<const uint32_t*>(&o_bits_cg1[o_word_cg1])), "r"(*reinterpret_cast<const uint32_t*>(&o_bits_cg1[o_word_cg1 + 1])), "r"(*reinterpret_cast<const uint32_t*>(&o_bits_cg1[o_word_cg1 + 2])), "r"(*reinterpret_cast<const uint32_t*>(&o_bits_cg1[o_word_cg1 + 3]))
                                    : "memory");
                            }
                        }
                    }
                ...
                if (chunk_in_seq_1 == num_cp_chunks_1 - 1) {
                    ...
                    int _trl_addr_10 = tmem_tmem_state + (warp_cg1 * 32 << 16);
                    tmem_ld_x32(&final_state_values_cg1[0], _trl_addr_10);
                    int _trl_addr_11 = tmem_tmem_state + (32) + (warp_cg1 * 32 << 16);
                    tmem_ld_x32(&final_state_values_cg1[32], _trl_addr_11);
                    int _trl_addr_12 = tmem_tmem_state + (64) + (warp_cg1 * 32 << 16);
                    tmem_ld_x32(&final_state_values_cg1[64], _trl_addr_12);
                    int _trl_addr_13 = tmem_tmem_state + (96) + (warp_cg1 * 32 << 16);
                    tmem_ld_x32(&final_state_values_cg1[96], _trl_addr_13);
                    asm volatile("tcgen05.wait::ld.sync.aligned;" ::: "memory");
                }
                mbarrier_arrive(kv_acc_empty_addr + (kv_stage_cg1) * 8);
                #pragma unroll
    ...
    // ---- Role: qk_mma ----
        ...
        { // qk_mma_main
            ...
            unsigned int flat_work_2 = blockIdx.x;
            uint32_t _fast_div_q_3 = (num_sab_heads_fdd.divisor != 1) ? (__umulhi((uint32_t)(flat_work_2), num_sab_heads_fdd.multiplier) >> num_sab_heads_fdd.shift_right) : (uint32_t)(flat_work_2);
            uint32_t _fast_div_r_3 = (uint32_t)(flat_work_2) - _fast_div_q_3 * (uint32_t)(num_sab_heads_fdd.divisor);
            int sab_head_2 = _fast_div_r_3;
            int chunk_in_seq_2 = _fast_div_q_3;
            ...
            uint32_t _fast_div_q_4 = (cp_chunk_len_fdd.divisor != 1) ? (__umulhi((uint32_t)((unsigned int)(seq_len_2 + cp_chunk_len - 1)), cp_chunk_len_fdd.multiplier) >> cp_chunk_len_fdd.shift_right) : (uint32_t)((unsigned int)(seq_len_2 + cp_chunk_len - 1));
            uint32_t _fast_div_r_4 = (uint32_t)((unsigned int)(seq_len_2 + cp_chunk_len - 1)) - _fast_div_q_4 * (uint32_t)(cp_chunk_len_fdd.divisor);
            int num_cp_chunks_2 = _fast_div_q_4;
            int chunk_len_2 = 0;
            if (chunk_in_seq_2 < num_cp_chunks_2) {
            ...
            uint32_t _fast_div_q_5 = (cp_chunk_len_fdd.divisor != 1) ? (__umulhi((uint32_t)((unsigned int)(seq_start_2 - prefix_items_2)), cp_chunk_len_fdd.multiplier) >> cp_chunk_len_fdd.shift_right) : (uint32_t)((unsigned int)(seq_start_2 - prefix_items_2));
            uint32_t _fast_div_r_5 = (uint32_t)((unsigned int)(seq_start_2 - prefix_items_2)) - _fast_div_q_5 * (uint32_t)(cp_chunk_len_fdd.divisor);
            int cp_chunk_2 = prefix_items_2 + (int)_fast_div_q_5 + chunk_in_seq_2;
    ...
    // ---- Role: tma_qkvt ----
        ...
        asm volatile("setmaxnreg.dec.sync.aligned.u32 24;");
        { // tma_qkvt_main
            unsigned int flat_work_3 = blockIdx.x;
            uint32_t _fast_div_q_0 = (num_sab_heads_fdd.divisor != 1) ? (__umulhi((uint32_t)(flat_work_3), num_sab_heads_fdd.multiplier) >> num_sab_heads_fdd.shift_right) : (uint32_t)(flat_work_3);
            uint32_t _fast_div_r_0 = (uint32_t)(flat_work_3) - _fast_div_q_0 * (uint32_t)(num_sab_heads_fdd.divisor);
            int sab_head_3 = _fast_div_r_0;
            int chunk_in_seq_3 = _fast_div_q_0;
            ...
            uint32_t _fast_div_q_1 = (cp_chunk_len_fdd.divisor != 1) ? (__umulhi((uint32_t)((unsigned int)(seq_len_3 + cp_chunk_len - 1)), cp_chunk_len_fdd.multiplier) >> cp_chunk_len_fdd.shift_right) : (uint32_t)((unsigned int)(seq_len_3 + cp_chunk_len - 1));
            uint32_t _fast_div_r_1 = (uint32_t)((unsigned int)(seq_len_3 + cp_chunk_len - 1)) - _fast_div_q_1 * (uint32_t)(cp_chunk_len_fdd.divisor);
            int num_cp_chunks_3 = _fast_div_q_1;
            int chunk_len_3 = 0;
            if (chunk_in_seq_3 < num_cp_chunks_3) {
            ...
            uint32_t _fast_div_q_2 = (cp_chunk_len_fdd.divisor != 1) ? (__umulhi((uint32_t)((unsigned int)(seq_start_3 - prefix_items_3)), cp_chunk_len_fdd.multiplier) >> cp_chunk_len_fdd.shift_right) : (uint32_t)((unsigned int)(seq_start_3 - prefix_items_3));
            uint32_t _fast_div_r_2 = (uint32_t)((unsigned int)(seq_start_3 - prefix_items_3)) - _fast_div_q_2 * (uint32_t)(cp_chunk_len_fdd.divisor);
            int cp_chunk_3 = prefix_items_3 + (int)_fast_div_q_2 + chunk_in_seq_3;
    ...
    // ---- Role: state_mma ----
        ...
        asm volatile("setmaxnreg.dec.sync.aligned.u32 24;");
        { // state_mma_main
            unsigned int flat_work_4 = blockIdx.x;
            uint32_t _fast_div_q_6 = (num_sab_heads_fdd.divisor != 1) ? (__umulhi((uint32_t)(flat_work_4), num_sab_heads_fdd.multiplier) >> num_sab_heads_fdd.shift_right) : (uint32_t)(flat_work_4);
            uint32_t _fast_div_r_6 = (uint32_t)(flat_work_4) - _fast_div_q_6 * (uint32_t)(num_sab_heads_fdd.divisor);
            int sab_head_4 = _fast_div_r_6;
            int chunk_in_seq_4 = _fast_div_q_6;
            ...
            uint32_t _fast_div_q_7 = (cp_chunk_len_fdd.divisor != 1) ? (__umulhi((uint32_t)((unsigned int)(seq_len_4 + cp_chunk_len - 1)), cp_chunk_len_fdd.multiplier) >> cp_chunk_len_fdd.shift_right) : (uint32_t)((unsigned int)(seq_len_4 + cp_chunk_len - 1));
            uint32_t _fast_div_r_7 = (uint32_t)((unsigned int)(seq_len_4 + cp_chunk_len - 1)) - _fast_div_q_7 * (uint32_t)(cp_chunk_len_fdd.divisor);
            int num_cp_chunks_4 = _fast_div_q_7;
            int chunk_len_4 = 0;
            if (chunk_in_seq_4 < num_cp_chunks_4) {
            ...
            uint32_t _fast_div_q_8 = (cp_chunk_len_fdd.divisor != 1) ? (__umulhi((uint32_t)((unsigned int)(seq_start_4 - prefix_items_4)), cp_chunk_len_fdd.multiplier) >> cp_chunk_len_fdd.shift_right) : (uint32_t)((unsigned int)(seq_start_4 - prefix_items_4));
            uint32_t _fast_div_r_8 = (uint32_t)((unsigned int)(seq_start_4 - prefix_items_4)) - _fast_div_q_8 * (uint32_t)(cp_chunk_len_fdd.divisor);
            int cp_chunk_4 = prefix_items_4 + (int)_fast_div_q_8 + chunk_in_seq_4;
            ...
            if (chunk_in_seq_4 < num_cp_chunks_4) {
                ...
                for (int __8 = 0; __8 < num_padded_blocks_4; __8++) {
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f16 [%0], [ta], db, id, p1;\n\t"
                    "}\n"
                    :: "r"(tmem_tmem_state), "r"(_mma_b_lo_13), "r"(tmem_tmem_shared_input + 32), "r"(1));
                    }
                    ...
                    q_state_empty_phase_state ^= 1;
                    kv_empty_phase_state ^= 1;
                    state_input_phase_state ^= 1;
                    vks_phase_state ^= 1;
                    nv_phase_state ^= 1;
                    decay_v_phase_state ^= 1;
                }
                mbarrier_wait(cg1_acc_empty_addr, cg1_acc_empty_phase_state);
                mbarrier_wait(q_state_acc_empty_addr + (q_state_stage_state) * 8, q_state_empty_phase_state);
                mbarrier_wait(kv_acc_empty_addr + (kv_stage_state) * 8, kv_empty_phase_state);
    ...
    // ---- Role: gate_epilogue ----
        ...
        asm volatile("setmaxnreg.dec.sync.aligned.u32 24;");
        { // gate_epilogue_main
            unsigned int flat_work_5 = blockIdx.x;
            uint32_t _fast_div_q_9 = (num_sab_heads_fdd.divisor != 1) ? (__umulhi((uint32_t)(flat_work_5), num_sab_heads_fdd.multiplier) >> num_sab_heads_fdd.shift_right) : (uint32_t)(flat_work_5);
            uint32_t _fast_div_r_9 = (uint32_t)(flat_work_5) - _fast_div_q_9 * (uint32_t)(num_sab_heads_fdd.divisor);
            int sab_head_5 = _fast_div_r_9;
            int chunk_in_seq_5 = _fast_div_q_9;
            ...
            uint32_t _fast_div_q_10 = (cp_chunk_len_fdd.divisor != 1) ? (__umulhi((uint32_t)((unsigned int)(seq_len_5 + cp_chunk_len - 1)), cp_chunk_len_fdd.multiplier) >> cp_chunk_len_fdd.shift_right) : (uint32_t)((unsigned int)(seq_len_5 + cp_chunk_len - 1));
            uint32_t _fast_div_r_10 = (uint32_t)((unsigned int)(seq_len_5 + cp_chunk_len - 1)) - _fast_div_q_10 * (uint32_t)(cp_chunk_len_fdd.divisor);
            int num_cp_chunks_5 = _fast_div_q_10;
            int chunk_len_5 = 0;
            if (chunk_in_seq_5 < num_cp_chunks_5) {
            ...
            uint32_t _fast_div_q_11 = (cp_chunk_len_fdd.divisor != 1) ? (__umulhi((uint32_t)((unsigned int)(seq_start_5 - prefix_items_5)), cp_chunk_len_fdd.multiplier) >> cp_chunk_len_fdd.shift_right) : (uint32_t)((unsigned int)(seq_start_5 - prefix_items_5));
            uint32_t _fast_div_r_11 = (uint32_t)((unsigned int)(seq_start_5 - prefix_items_5)) - _fast_div_q_11 * (uint32_t)(cp_chunk_len_fdd.divisor);
            int cp_chunk_5 = prefix_items_5 + (int)_fast_div_q_11 + chunk_in_seq_5;
