...
// tvm-ffi host launcher for GDN non-CP kernel 'kernel_flashinfer_blackwell_gdn_prefill_dvsplit_initial_bf16state_indexed'.
#include <cuda.h>
...
TVM_FFI_EMBED_CUBIN(flashinfer_blackwell_gdn_prefill_dvsplit_initial_bf16state_indexed_1721a68fd1);

namespace gdn_noncp_host_shim_ec5900fc34322923 {
...
void Run(TensorView arg_Q, TensorView arg_K, TensorView arg_V, TensorView arg_O, TensorView arg_gate, TensorView arg_beta, TensorView arg_cu_seqlens, TensorView arg_state_indices, TensorView arg_initial_state, TensorView arg_output_state, TensorView arg_checkpoint_state, TensorView arg_cu_checkpoints, TensorView arg_tensormap_workspace, int64_t arg_initial_state_stride_slot, int64_t arg_output_state_stride_slot, int64_t arg_checkpoint_every_n_tokens, double arg_scale, int64_t arg_num_seqs, int64_t arg_num_q_heads, int64_t arg_num_v_heads, int64_t arg_total_tiles, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  CheckDtype(arg_output_state, "output_state", 4, 16, 1);
  CheckCudaTensor(arg_checkpoint_state, "checkpoint_state");
  CheckDtype(arg_checkpoint_state, "checkpoint_state", 4, 16, 1);
  ...
  TVM_FFI_CHECK(arg_total_tiles >= -2147483648LL && arg_total_tiles <= 2147483647LL, ValueError)
      ...
      << " is outside i32 range [-2147483648, 2147483647]";
  TVM_FFI_CHECK(arg_num_seqs > 0 && arg_num_q_heads == 4LL && arg_num_v_heads == 8LL, ValueError)
      << "prefill sequence/head counts do not match the selected GDN non-CP variant";
  TVM_FFI_CHECK(arg_total_tiles == arg_num_seqs * 16LL, ValueError)
  ...
  TVM_FFI_CHECK(arg_Q.ndim() == 3 && arg_Q.size(0) > 0 && arg_Q.size(1) == 4 && arg_Q.size(2) == 128LL, ValueError)
      << "Q shape does not match [tokens,Hq,128]";
  TVM_FFI_CHECK(arg_K.ndim() == 3 && arg_K.size(0) == arg_Q.size(0) && arg_K.size(1) == 4 && arg_K.size(2) == 128LL, ValueError)
      << "K shape does not match [tokens,min(Hq,Hv),128]";
  TVM_FFI_CHECK(arg_V.ndim() == 3 && arg_V.size(0) == arg_Q.size(0) && arg_V.size(1) == 8 && arg_V.size(2) == 128LL, ValueError)
      << "V shape does not match [tokens,Hv,128]";
  TVM_FFI_CHECK(arg_O.ndim() == 3 && arg_O.size(0) == arg_Q.size(0) && arg_O.size(1) == 8 && arg_O.size(2) == 128LL, ValueError)
      << "O shape does not match [tokens,max(Hq,Hv),128]";
  TVM_FFI_CHECK(arg_gate.ndim() == 2 && arg_gate.size(0) == arg_Q.size(0) && arg_gate.size(1) == 8, ValueError)
      << "gate shape does not match [tokens,max(Hq,Hv)]";
  TVM_FFI_CHECK(arg_beta.ndim() == 2 && arg_beta.size(0) == arg_Q.size(0) && arg_beta.size(1) == 8, ValueError)
  ...
  TVM_FFI_CHECK(arg_state_indices.ndim() == 1 && arg_state_indices.size(0) == arg_num_seqs, ValueError)
      << "state_indices must contain one slot per sequence";
  TVM_FFI_CHECK(arg_initial_state.ndim() == 4 && arg_initial_state.size(0) >= arg_num_seqs && arg_initial_state.size(1) == 8 && arg_initial_state.size(2) == 128LL && arg_initial_state.size(3) == 128LL, ValueError)
      << "initial_state shape does not match the selected state-pool contract";
  TVM_FFI_CHECK(arg_initial_state.stride(3) == 1 && arg_initial_state.stride(2) == 128LL && arg_initial_state.stride(1) == 16384LL && arg_initial_state_stride_slot == arg_initial_state.stride(0) && arg_initial_state_stride_slot >= 131072LL, ValueError)
      << "initial_state inner layout or slot stride is invalid";
  TVM_FFI_CHECK(arg_output_state.ndim() == 4 && arg_output_state.size(0) >= arg_num_seqs && arg_output_state.size(1) == 8 && arg_output_state.size(2) == 128LL && arg_output_state.size(3) == 128LL, ValueError)
      << "output_state shape does not match the selected state-pool contract";
  TVM_FFI_CHECK(arg_output_state.stride(3) == 1 && arg_output_state.stride(2) == 128LL && arg_output_state.stride(1) == 16384LL && arg_output_state_stride_slot == arg_output_state.stride(0) && arg_output_state_stride_slot >= 131072LL, ValueError)
      << "output_state inner layout or slot stride is invalid";
  TVM_FFI_CHECK(arg_checkpoint_every_n_tokens > 0 && arg_checkpoint_every_n_tokens % 64 == 0, ValueError)
      << "checkpoint interval must be a positive multiple of 64";
  TVM_FFI_CHECK(arg_checkpoint_state.ndim() == 4 && arg_checkpoint_state.size(1) == 8 && arg_checkpoint_state.size(2) == 128LL && arg_checkpoint_state.size(3) == 128LL, ValueError)
      << "checkpoint_state shape does not match [checkpoints,H,128,128]";
  TVM_FFI_CHECK(arg_cu_checkpoints.ndim() == 1 && arg_cu_checkpoints.size(0) == arg_num_seqs + 1, ValueError)
      << "cu_checkpoints must contain num_seqs+1 entries";
  ...
  int64_t v_initial_state_size_p0 = arg_initial_state.size(0);
  int64_t v_output_state_size_p0 = arg_output_state.size(0);
  void* kargs[] = {&p_Q, &p_K, &p_V, &p_O, &p_gate, &p_beta, &p_cu_seqlens, &p_state_indices, &p_initial_state, &p_output_state, &p_checkpoint_state, &p_cu_checkpoints, &p_tensormap_workspace, &v_initial_state_stride_slot, &v_output_state_stride_slot, &v_checkpoint_every_n_tokens, &v_scale, &v_num_seqs, &v_num_q_heads, &v_num_v_heads, &v_total_tiles, &v_initial_state_size_p0, &v_output_state_size_p0};

  static auto kernel = EmbedCubinModule_flashinfer_blackwell_gdn_prefill_dvsplit_initial_bf16state_indexed_1721a68fd1::Global()->mod.GetKernel("kernel_flashinfer_blackwell_gdn_prefill_dvsplit_initial_bf16state_indexed");
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(run_prefill_flashinfer_blackwell_gdn_prefill_dvsplit_initial_bf16state_1c987612a6c1, gdn_noncp_host_shim_ec5900fc34322923::Run);
// clang-format on
