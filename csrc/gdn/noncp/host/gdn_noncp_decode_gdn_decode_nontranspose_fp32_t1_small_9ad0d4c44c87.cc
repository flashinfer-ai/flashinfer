...
TVM_FFI_EMBED_CUBIN(gdn_decode_nontranspose_fp32_t1_small_2d008af47b);

namespace gdn_noncp_host_shim_7f987b4c0a240834 {
...
void Run(TensorView arg_q, TensorView arg_k, TensorView arg_v, TensorView arg_state, TensorView arg_A_log, TensorView arg_a, TensorView arg_dt_bias, TensorView arg_b, TensorView arg_out, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  TVM_FFI_CHECK(arg_state.ndim() == 4 && arg_state.size(0) == arg_q.size(0) && arg_state.size(1) == 8LL && arg_state.size(2) == 128LL && arg_state.size(3) == 128LL, ValueError)
      << "state shape does not match the frozen FlashInfer GDN non-CP launch contract";
  TVM_FFI_CHECK(arg_q.size(0) > 0 && arg_q.size(0) < 32 && grid_y == 1 && grid_z == 1 && grid_x == arg_q.size(0) * 64LL, ValueError)
  ...
  TVM_FFI_CHECK(arg_a.ndim() == 3 && arg_a.size(0) == arg_q.size(0) && arg_a.size(1) == 1LL && arg_a.size(2) == 8LL, ValueError)
      << "a shape does not match [batch,1,8]";
  TVM_FFI_CHECK(arg_a.stride(0) > 0 && arg_a.stride(1) > 0 && arg_a.stride(2) == 1 && arg_a.stride(1) >= 8LL && arg_a.stride(0) / 1LL >= arg_a.stride(1) && (arg_q.size(0) == 1 || arg_a.stride(0) <= (9223372036854775807LL - (0LL * arg_a.stride(1) + 7LL)) / (arg_q.size(0) - 1LL)), ValueError)
      << "a strides do not cover the reachable [batch,token,head] span";
  TVM_FFI_CHECK(arg_b.ndim() == 3 && arg_b.size(0) == arg_q.size(0) && arg_b.size(1) == 1LL && arg_b.size(2) == 8LL, ValueError)
      << "b shape does not match [batch,1,8]";
  TVM_FFI_CHECK(arg_b.stride(0) > 0 && arg_b.stride(1) > 0 && arg_b.stride(2) == 1 && arg_b.stride(1) >= 8LL && arg_b.stride(0) / 1LL >= arg_b.stride(1) && (arg_q.size(0) == 1 || arg_b.stride(0) <= (9223372036854775807LL - (0LL * arg_b.stride(1) + 7LL)) / (arg_q.size(0) - 1LL)), ValueError)
  ...
  static auto kernel = TVM_FFI_EMBED_CUBIN_GET_KERNEL(gdn_decode_nontranspose_fp32_t1_small_2d008af47b, "kernel_gdn_decode_nontranspose_fp32_t1_small");
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(run_decode_gdn_decode_nontranspose_fp32_t1_small_9ad0d4c44c87, gdn_noncp_host_shim_7f987b4c0a240834::Run);
// clang-format on
