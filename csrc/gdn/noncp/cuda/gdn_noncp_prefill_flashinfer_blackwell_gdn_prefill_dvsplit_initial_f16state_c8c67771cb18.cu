...
kernel_flashinfer_blackwell_gdn_prefill_dvsplit_initial_f16state(const __grid_constant__ CUtensorMap Q, const __grid_constant__ CUtensorMap K, const __grid_constant__ CUtensorMap V, const __grid_constant__ CUtensorMap O, float* __restrict__ gate, float* __restrict__ beta, int* __restrict__ cu_seqlens, int* __restrict__ state_indices, __half* __restrict__ initial_state, __half* __restrict__ output_state, __half* __restrict__ checkpoint_state, int* __restrict__ cu_checkpoints, uint8_t* __restrict__ tensormap_workspace, long long initial_state_stride_slot, long long output_state_stride_slot, int checkpoint_every_n_tokens, float scale, int num_seqs, int num_q_heads, int num_v_heads, int total_tiles)
{
                        ...
                        if (output_warp_cg1 < 4) {
                            ...
                            for (int token_group_cg1 = 0; token_group_cg1 < 4; token_group_cg1++) {
                                ...
                                asm volatile("stmatrix.sync.aligned.m8n8.x4.trans.shared.b16 [%0], {%1, %2, %3, %4};\n"
                                    :: "r"(_stmatrix_addr_3), "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_7_bf16[o_pack_base_cg1])), "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_7_bf16[o_pack_base_cg1 + 1])), "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_7_bf16[o_pack_base_cg1 + 2])), "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_7_bf16[o_pack_base_cg1 + 3]))
                                    : "memory");
                            }
                        }
                        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                ...
                for (int chunk_idx_1 = 1; chunk_idx_1 < active_chunks_b; chunk_idx_1++) {
                    ...
                    if (output_warp_cg1_1 < 4) {
                        ...
                        for (int token_group_cg1_1 = 0; token_group_cg1_1 < 4; token_group_cg1_1++) {
                            ...
                            asm volatile("stmatrix.sync.aligned.m8n8.x4.trans.shared.b16 [%0], {%1, %2, %3, %4};\n"
                                :: "r"(_stmatrix_addr_7), "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_15_bf16[o_pack_base_cg1_1])), "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_15_bf16[o_pack_base_cg1_1 + 1])), "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_15_bf16[o_pack_base_cg1_1 + 2])), "r"(*reinterpret_cast<const uint32_t*>(&_tmem_load_15_bf16[o_pack_base_cg1_1 + 3]))
                                : "memory");
                        }
                    }
                    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                    ...
                    if (state_warp_id_in_role_cg1 < 4) {
                        ...
                        for (int state_col_half_cg1 = 0; state_col_half_cg1 < 2; state_col_half_cg1++) {
                            ...
                            for (int state_col_group_cg1 = 0; state_col_group_cg1 < 8; state_col_group_cg1++) {
                                ...
                                {
                                    *(reinterpret_cast<__half*>(output_state + (state_base_idx_top_cg1 + (long long)state_col_pair_cg1)) + (0)) = __float2half_rn(_tmem_load_16[state_reg_base_cg1]);
                                    *(reinterpret_cast<__half*>(output_state + (state_base_idx_top_cg1 + (long long)state_col_pair_cg1 + 1)) + (0)) = __float2half_rn(_tmem_load_16[state_reg_base_cg1 + 1]);
                                    *(reinterpret_cast<__half*>(output_state + (state_base_idx_bot_cg1 + (long long)state_col_pair_cg1)) + (0)) = __float2half_rn(_tmem_load_16[state_reg_base_cg1 + 2]);
                                    *(reinterpret_cast<__half*>(output_state + (state_base_idx_bot_cg1 + (long long)state_col_pair_cg1 + 1)) + (0)) = __float2half_rn(_tmem_load_16[state_reg_base_cg1 + 3]);
                                }
                            }
                        }
                    }
                        ...
                        for (int empty_col_group = 0; empty_col_group < 8; empty_col_group++) {
                            ...
                            long long empty_output_bot_base = output_state_head_base_cg1 + (long long)(value_split_idx_1 * 64 + empty_state_row_bot) * 128;
                            {
                                ...
                                for (int empty_element_1 = 0; empty_element_1 < 4; empty_element_1++) {
                                    *(reinterpret_cast<__half*>(output_state + (empty_output_top_base + (long long)empty_state_col + (long long)empty_element_1)) + (0)) = __float2half_rn(empty_top_vec[empty_element_1]);
                                    *(reinterpret_cast<__half*>(output_state + (empty_output_bot_base + (long long)empty_state_col + (long long)empty_element_1)) + (0)) = __float2half_rn(empty_bot_vec[empty_element_1]);
                                }
                            }
                        }
                    }
