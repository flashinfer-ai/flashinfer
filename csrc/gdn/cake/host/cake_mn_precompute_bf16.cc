...
TVM_FFI_EMBED_CUBIN(flashinfer_blackwell_gdn_cp_prefill_mn_precompute_bf16_v1_02211680ce);
...
void Run(TensorView arg_K, TensorView arg_V, TensorView arg_T, TensorView arg_alpha, TensorView arg_local_transfer, TensorView arg_local_state, TensorView arg_cu_seqlens, int64_t arg_chunk_len, int64_t arg_num_k_heads, int64_t arg_num_v_heads, int64_t arg_num_sab_heads, int64_t arg_total_cp_chunks, int64_t arg_num_seqs, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  static auto kernel = EmbedCubinModule_flashinfer_blackwell_gdn_cp_prefill_mn_precompute_bf16_v1_02211680ce::Global()->mod.GetKernel("kernel_flashinfer_blackwell_gdn_cp_prefill_mn_precompute_bf16_v1");
  static signed char cake_smem_mode_cache[64] = {0};
  const bool use_oversized_smem = CakeConfigureDynamicSmem(
      kernel, (int)arg_K.device().device_id, 192512,
  ...
  TVM_FFI_CHECK_CUBIN_LAUNCHER_CUDA_ERROR(kernel.Launch(kargs, grid, block, stream, 192512u));
}
