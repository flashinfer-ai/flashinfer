...
// tvm-ffi host launcher for Cake kernel 'kernel_gdn_decode_pretranspose_splitv8'.
#include <cuda.h>
...
TVM_FFI_EMBED_CUBIN(gdn_decode_pretranspose_splitv8_48992a0adc);
...
void Run(TensorView arg_q, TensorView arg_k, TensorView arg_v, TensorView arg_state, TensorView arg_A_log, TensorView arg_a, TensorView arg_dt_bias, TensorView arg_b, TensorView arg_out, TensorView arg_initial_state_indices, TensorView arg_output_state_indices, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  TVM_FFI_CHECK(arg_state.ndim() == 4 && arg_state.size(0) > 0 && arg_state.size(1) == 32LL && arg_state.size(2) == 128LL && arg_state.size(3) == 128LL, ValueError)
      << "indexed FP32 state pool shape is invalid";
  TVM_FFI_CHECK(arg_initial_state_indices.ndim() == 1 && arg_initial_state_indices.size(0) == arg_q.size(0) && arg_output_state_indices.ndim() == 1 && arg_output_state_indices.size(0) == arg_q.size(0), ValueError)
      << "indexed FP32 decode requires one input/output slot per batch row";
  TVM_FFI_CHECK(arg_q.size(0) > 0 && grid_y == 1 && grid_z == 1 && grid_x == arg_q.size(0) * 256LL, ValueError)
      << "indexed FP32 decode grid does not match batch and state heads";
  ...
  void* kargs[] = {&p_q, &p_k, &p_v, &p_state, &p_A_log, &p_a, &p_dt_bias, &p_b, &p_out, &p_initial_state_indices, &p_output_state_indices};

  static auto kernel = TVM_FFI_EMBED_CUBIN_GET_KERNEL(gdn_decode_pretranspose_splitv8_48992a0adc, "kernel_gdn_decode_pretranspose_splitv8");
  tvm::ffi::dim3 grid((uint32_t)grid_x, (uint32_t)grid_y, (uint32_t)grid_z);
  tvm::ffi::dim3 block(128u, 1u, 1u);

  TVM_FFI_CHECK_CUBIN_LAUNCHER_CUDA_ERROR(kernel.Launch(kargs, grid, block, stream, 9088u));
}

}  // namespace cake_host_shim

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run_decode_gdn_decode_pretranspose_splitv8_23d4a6f854ff, cake_host_shim::Run);
// clang-format on
