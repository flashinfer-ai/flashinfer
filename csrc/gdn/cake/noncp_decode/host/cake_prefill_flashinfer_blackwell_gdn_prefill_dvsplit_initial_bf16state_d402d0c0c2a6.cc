...
TVM_FFI_EMBED_CUBIN(flashinfer_blackwell_gdn_prefill_dvsplit_initial_bf16state_92cabb292f);
...
void Run(TensorView arg_Q, TensorView arg_K, TensorView arg_V, TensorView arg_O, TensorView arg_gate, TensorView arg_beta, TensorView arg_cu_seqlens, TensorView arg_state_indices, TensorView arg_initial_state, TensorView arg_output_state, TensorView arg_checkpoint_state, TensorView arg_cu_checkpoints, TensorView arg_tensormap_workspace, int64_t arg_initial_state_stride_slot, int64_t arg_output_state_stride_slot, int64_t arg_checkpoint_every_n_tokens, double arg_scale, int64_t arg_num_seqs, int64_t arg_num_q_heads, int64_t arg_num_v_heads, int64_t arg_total_tiles, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  TVM_FFI_CHECK(arg_initial_state.ndim() == 4 && arg_initial_state.size(0) == arg_num_seqs && arg_initial_state.size(1) == 1 && arg_initial_state.size(2) == 128LL && arg_initial_state.size(3) == 128LL, ValueError)
      << "initial_state shape does not match the selected state-pool contract";
  TVM_FFI_CHECK(arg_initial_state.stride(3) == 1 && arg_initial_state.stride(2) == 128LL && arg_initial_state.stride(1) == 16384LL && arg_initial_state_stride_slot == arg_initial_state.stride(0) && arg_initial_state_stride_slot >= 16384LL, ValueError)
  ...
  static auto kernel = EmbedCubinModule_flashinfer_blackwell_gdn_prefill_dvsplit_initial_bf16state_92cabb292f::Global()->mod.GetKernelWithMaxDynamicSharedMemory("kernel_flashinfer_blackwell_gdn_prefill_dvsplit_initial_bf16state", 226048);
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(run_prefill_flashinfer_blackwell_gdn_prefill_dvsplit_initial_bf16state_d402d0c0c2a6, cake_host_shim::Run);
// clang-format on
