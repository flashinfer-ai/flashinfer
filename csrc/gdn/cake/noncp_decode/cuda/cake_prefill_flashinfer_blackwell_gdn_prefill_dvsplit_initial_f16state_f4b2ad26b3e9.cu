...
kernel_flashinfer_blackwell_gdn_prefill_dvsplit_initial_f16state(LoomTensorMap const* Q, LoomTensorMap const* K, LoomTensorMap const* V, LoomTensorMap const* O, float* __restrict__ gate, float* __restrict__ beta, int* __restrict__ cu_seqlens, int* __restrict__ state_indices, __half* __restrict__ initial_state, __half* __restrict__ output_state, __half* __restrict__ checkpoint_state, int* __restrict__ cu_checkpoints, uint8_t* __restrict__ tensormap_workspace, long long initial_state_stride_slot, long long output_state_stride_slot, int checkpoint_every_n_tokens, float scale, int num_seqs, int num_q_heads, int num_v_heads, int total_tiles)
{
