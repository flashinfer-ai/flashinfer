                    ...
                    if (state_warp_init < 4) {
                        ...
                        for (int state_col_half_init = 0; state_col_half_init < 2; state_col_half_init++) {
                            ...
                            for (int state_col_group_init = 0; state_col_group_init < 8; state_col_group_init++) {
                                ...
                                {
                                    {
                                        uint16_t _fp8x2_0 = *reinterpret_cast<const uint16_t*>(initial_state + state_base_top_init + (long long)state_col_pair_init);
                                        uint32_t _f16x2_0;
                                        asm volatile("cvt.rn.f16x2.e4m3x2 %0, %1;" : "=r"(_f16x2_0) : "h"(_fp8x2_0));
                                    ...
                                    {
                                        uint16_t _fp8x2_1 = *reinterpret_cast<const uint16_t*>(initial_state + state_base_bot_init + (long long)state_col_pair_init);
                                        uint32_t _f16x2_1;
                                        asm volatile("cvt.rn.f16x2.e4m3x2 %0, %1;" : "=r"(_f16x2_1) : "h"(_fp8x2_1));
                    ...
                    if (empty_state_warp < 4) {
                        ...
                        for (int empty_col_group = 0; empty_col_group < 8; empty_col_group++) {
                            ...
                            for (int empty_pair = 0; empty_pair < 2; empty_pair++) {
                                {
                                    uint16_t _fp8x2_10 = *reinterpret_cast<const uint16_t*>(initial_state + empty_initial_top_base + (long long)empty_state_col + (long long)(empty_pair * 2));
                                    uint32_t _f16x2_10;
                                    asm volatile("cvt.rn.f16x2.e4m3x2 %0, %1;" : "=r"(_f16x2_10) : "h"(_fp8x2_10));
                                ...
                                {
                                    uint16_t _fp8x2_11 = *reinterpret_cast<const uint16_t*>(initial_state + empty_initial_bot_base + (long long)empty_state_col + (long long)(empty_pair * 2));
                                    uint32_t _f16x2_11;
                                    asm volatile("cvt.rn.f16x2.e4m3x2 %0, %1;" : "=r"(_f16x2_11) : "h"(_fp8x2_11));
