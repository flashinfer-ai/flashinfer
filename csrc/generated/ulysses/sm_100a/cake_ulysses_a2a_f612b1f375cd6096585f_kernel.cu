...
kernel_cake_ulysses_a2a_f612b1f375cd6096585f(__half* __restrict__ inp, CakePeerPointerTable<__half> staging_peers, CakePeerPointerTable<unsigned int> signals_signals, unsigned int* __restrict__ signals_self, int32_t pg_rank, int B, int S_local, int H_local, int D)
{
    ...
    unsigned int num_rows = batches * 6 * sequences;
    ...
    if (row_lo < row_hi) {
        ...
        if (vector_aligned != 0) {
            unsigned int units_magic = (unsigned int)0;
            unsigned int sequences_magic = (unsigned int)0;
            units_magic = (unsigned int)4294967295 / units_per_row;
            sequences_magic = (unsigned int)4294967295 / sequences;
            ...
            for (unsigned int unit_index = (unsigned int)tid; unit_index < total_units; unit_index += 512) {
                ...
                unsigned int peer = row_quotient % 6;
                ...
                unsigned int dst_offset = slab_offset + (sequence * 5 + rank) * block_len - peer_span_offset;
            ...
            for (unsigned int row_1 = row_lo; row_1 < row_hi; row_1++) {
                ...
                unsigned int peer_1 = row_quotient_1 % 6;
                unsigned int batch = row_quotient_1 / 6;
                unsigned int src_offset_1 = (batch * (sequences * 6) + peer_1 * sequences + sequence_1) * block_len;
                unsigned int dst_offset_1 = ((batch * sequences + sequence_1) * (local_heads * 6) + rank * local_heads) * head_dim;
                #pragma unroll 1
