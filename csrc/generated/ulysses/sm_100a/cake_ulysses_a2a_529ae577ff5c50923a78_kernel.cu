...
kernel_cake_ulysses_a2a_529ae577ff5c50923a78(__nv_bfloat16* __restrict__ inp, CakePeerPointerTable<__nv_bfloat16> staging_peers, CakePeerPointerTable<unsigned int> signals_signals, unsigned int* __restrict__ signals_self, int32_t pg_rank, int B, int S_local, int H_local, int D)
{
    ...
    if (row_lo < row_hi) {
        ...
        if (vector_aligned != 0) {
            ...
            if (units_per_row == 128 && sequences == 5460) {
                units_magic = (unsigned int)33554431;
                sequences_magic = (unsigned int)786624;
            } else {
                units_magic = (unsigned int)4294967295 / units_per_row;
                sequences_magic = (unsigned int)4294967295 / sequences;
            }
            int w6_bf16_performance_shape = (int)(batches == 1 && sequences == 5460 && units_per_row == 128 && grid == 36);
            ...
            for (unsigned int unit_index = (unsigned int)tid; unit_index < total_units; unit_index += 512) {
                ...
                if (w6_bf16_performance_shape != 0) {
                    local_row = unit_index >> 7;
                    unit_in_row = unit_index & 127;
                    row = row_lo + local_row;
                    unsigned int block_peer = block / 6;
                    unsigned int block_in_peer = block - block_peer * 6;
                    row_quotient = block_peer;
                    sequence = block_in_peer * 910 + local_row;
                } else {
                    ...
                    scatter_batch = row_quotient / 6;
                }
                unsigned int peer = row_quotient % 6;
                {
                    unsigned int vector_element = unit_in_row * 8;
                    unsigned int batch = scatter_batch;
                    unsigned int scatter_peer = row_quotient - batch * 6;
                    unsigned int src_offset = ((batch * sequences + sequence) * (local_heads * 6) + scatter_peer * local_heads) * head_dim;
                    unsigned int dst_offset = (batch * (sequences * 6) + rank * sequences + sequence) * block_len;
            ...
            for (unsigned int row_1 = row_lo; row_1 < row_hi; row_1++) {
                ...
                unsigned int batch_1 = row_quotient_1 / 6;
                unsigned int src_offset_1 = ((batch_1 * sequences + sequence_1) * (local_heads * 6) + peer_1 * local_heads) * head_dim;
                unsigned int dst_offset_1 = (batch_1 * (sequences * 6) + rank * sequences + sequence_1) * block_len;
                #pragma unroll 1
