...
kernel_cake_ulysses_a2a_db6770e67a1e623ddc47(__half* __restrict__ inp, CakePeerPointerTable<__half> staging_peers, CakePeerPointerTable<unsigned int> signals_signals, unsigned int* __restrict__ signals_self, int32_t pg_rank, int B, int S_local, int H_local, int D)
{
        ...
        if (vector_aligned != 0) {
            ...
            for (unsigned int unit_index = (unsigned int)tid; unit_index < total_units; unit_index += 512) {
                ...
                unsigned int peer = row_quotient % 2;
                {
                    ...
                    unsigned int batch = row_quotient / 2;
                    unsigned int src_offset = ((batch * sequences + sequence) * (local_heads * 2) + peer * local_heads) * head_dim;
            ...
            for (unsigned int row_1 = row_lo; row_1 < row_hi; row_1++) {
                ...
                unsigned int batch_1 = row_quotient_1 / 2;
                unsigned int src_offset_1 = ((batch_1 * sequences + sequence_1) * (local_heads * 2) + peer_1 * local_heads) * head_dim;
                unsigned int dst_offset_1 = (batch_1 * (sequences * 2) + rank * sequences + sequence_1) * block_len;
                #pragma unroll 1
