...
kernel_cake_ulysses_a2a_f492358b1a26be5cc57e(__nv_bfloat16* __restrict__ inp, CakePeerPointerTable<__nv_bfloat16> staging_peers, CakePeerPointerTable<unsigned int> signals_signals, unsigned int* __restrict__ signals_self, int32_t pg_rank, int B, int S_local, int H_local, int D)
{
        ...
        if (vector_aligned != 0) {
            ...
            for (unsigned int unit_index = (unsigned int)tid; unit_index < total_units; unit_index += 512) {
                ...
                if (w2_bf16_performance_shape != 0) {
                    ...
                    w2_src_offset = (peer * 16380 + (w2_block_in_peer * 910 + unit_index / 320)) * block_len + vector_element;
                    w2_dst_offset = ((w2_block_in_peer * 910 + unit_index / 320) * 2 + rank) * block_len + vector_element;
                } else {
                    ...
                    unsigned int slab_offset = row * block_len + vector_element_1;
                    unsigned int peer_span_offset = peer * sequences * block_len;
                    w2_src_offset = slab_offset;
                    w2_dst_offset = slab_offset + (sequence + rank) * block_len - peer_span_offset;
                }
            ...
            for (unsigned int row_1 = row_lo; row_1 < row_hi; row_1++) {
                ...
                unsigned int src_offset = (batch * (sequences * 2) + peer_1 * sequences + sequence_1) * block_len;
                unsigned int dst_offset = ((batch * sequences + sequence_1) * (local_heads * 2) + rank * local_heads) * head_dim;
                #pragma unroll 1
