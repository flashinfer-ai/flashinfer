...
kernel_cake_ulysses_a2a_54deb052ac6ae2b15662(float* __restrict__ inp, CakePeerPointerTable<float> staging_peers, CakePeerPointerTable<unsigned int> signals_signals, unsigned int* __restrict__ signals_self, int32_t pg_rank, int B, int S_local, int H_local, int D)
{
    ...
    if (row_lo < row_hi) {
        ...
        if (vector_aligned != 0) {
            ...
            for (unsigned int unit_index = (unsigned int)tid; unit_index < total_units; unit_index += 512) {
                ...
                unsigned int slab_offset = row * block_len + unit_in_row * 4;
