...
kernel_cake_ulysses_a2a_b22486e951c99364ee15(float* __restrict__ inp, CakePeerPointerTable<float> staging_peers, CakePeerPointerTable<unsigned int> signals_signals, unsigned int* __restrict__ signals_self, int32_t pg_rank, int B, int S_local, int H_local, int D)
{
    ...
    unsigned int units_per_row = block_len / 4;
    if (tid == 0) {
        if (block_len % 4 == 0) {
    ...
    unsigned int num_rows = batches * 8 * sequences;
    ...
    if (row_lo < row_hi) {
        int vector_aligned = (int)(block_len % 4 == 0 && (unsigned long long)inp % 16 == 0);
        if (vector_aligned != 0) {
            ...
            for (unsigned int unit_index = (unsigned int)tid; unit_index < total_units; unit_index += 512) {
                ...
                unsigned int peer = row_quotient % 8;
                {
                    unsigned int vector_element = unit_in_row * 4;
                    unsigned int batch = row_quotient / 8;
                    unsigned int src_offset = ((batch * sequences + sequence) * (local_heads * 8) + peer * local_heads) * head_dim;
                    unsigned int dst_offset = (row - peer * sequences + rank * sequences) * block_len;
                    *reinterpret_cast<uint4*>((staging_peers.ptrs[peer] + (dst_offset + vector_element))) = *reinterpret_cast<const uint4*>((inp + (src_offset + vector_element)));
                }
            }
        } else {
            ...
            for (unsigned int row_1 = row_lo; row_1 < row_hi; row_1++) {
                ...
                unsigned int peer_1 = row_quotient_1 % 8;
                unsigned int batch_1 = row_quotient_1 / 8;
                unsigned int src_offset_1 = ((batch_1 * sequences + sequence_1) * (local_heads * 8) + peer_1 * local_heads) * head_dim;
                unsigned int dst_offset_1 = (batch_1 * (sequences * 8) + rank * sequences + sequence_1) * block_len;
                #pragma unroll 1
