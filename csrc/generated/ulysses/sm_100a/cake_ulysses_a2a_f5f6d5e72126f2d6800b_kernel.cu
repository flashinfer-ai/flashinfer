...
kernel_cake_ulysses_a2a_f5f6d5e72126f2d6800b(__nv_bfloat16* __restrict__ inp, CakePeerPointerTable<__nv_bfloat16> staging_peers, CakePeerPointerTable<unsigned int> signals_signals, unsigned int* __restrict__ signals_self, int32_t pg_rank, int B, int S_local, int H_local, int D)
{
    ...
    if (row_lo < row_hi) {
        ...
        if (vector_aligned != 0) {
            ...
            int w2_bf16_performance_shape = (int)(batches == 1 && sequences == 16380 && units_per_row == 320 && grid == 36);
            unsigned int w2_block_peer = block / 18;
            unsigned int w2_peer = w2_block_peer % 2;
            unsigned int w2_block_in_peer = block - w2_block_peer * 18;
            ...
            for (unsigned int unit_index = (unsigned int)tid; unit_index < total_units; unit_index += 512) {
                ...
                unsigned int w2_src_offset = (unsigned int)0;
                unsigned int w2_dst_offset = (unsigned int)0;
                if (w2_bf16_performance_shape != 0) {
                    peer = w2_peer;
                    unsigned int vector_element = (unit_index - unit_index / 320 * 320) * 8;
                    w2_src_offset = ((w2_block_in_peer * 910 + unit_index / 320) * 2 + peer) * block_len + vector_element;
                    w2_dst_offset = (w2_block_in_peer * 910 + unit_index / 320 + rank * 16380) * block_len + vector_element;
                } else {
                    ...
                    peer = row_quotient % 2;
                    unsigned int vector_element_1 = unit_in_row * 8;
                    unsigned int peer_row = row - peer * sequences;
                    w2_src_offset = (peer_row + sequence + peer) * block_len + vector_element_1;
                    w2_dst_offset = (peer_row + rank * sequences) * block_len + vector_element_1;
                }
                *reinterpret_cast<uint4*>((staging_peers.ptrs[peer] + w2_dst_offset)) = *reinterpret_cast<const uint4*>((inp + w2_src_offset));
            }
        } else {
            ...
            for (unsigned int row_1 = row_lo; row_1 < row_hi; row_1++) {
                ...
                unsigned int src_offset = ((batch * sequences + sequence_1) * (local_heads * 2) + peer_1 * local_heads) * head_dim;
                unsigned int dst_offset = (batch * (sequences * 2) + rank * sequences + sequence_1) * block_len;
                #pragma unroll 1
                for (unsigned int element = (unsigned int)tid; element < block_len; element += 512) {
                    staging_peers.ptrs[peer_1][dst_offset + element] = inp[src_offset + element];
                }
            }
        }
    }
