...
kernel_cake_ulysses_a2a_81eb3da8eb3723c0e75a(__nv_bfloat16* __restrict__ inp, CakePeerPointerTable<__nv_bfloat16> staging_peers, CakePeerPointerTable<unsigned int> signals_signals, unsigned int* __restrict__ signals_self, int32_t pg_rank, int B, int S_local, int H_local, int D)
{
    ...
    if (row_lo < row_hi) {
        ...
        if (vector_aligned != 0) {
            ...
            for (unsigned int unit_index = (unsigned int)tid; unit_index < total_units; unit_index += 512) {
                ...
                unsigned int dst_offset = slab_offset + (sequence * 7 + rank) * block_len - peer_span_offset;
            ...
            for (unsigned int row_1 = row_lo; row_1 < row_hi; row_1++) {
                ...
                unsigned int batch = row_quotient_1 / 8;
                unsigned int src_offset_1 = (batch * (sequences * 8) + peer_1 * sequences + sequence_1) * block_len;
                unsigned int dst_offset_1 = ((batch * sequences + sequence_1) * (local_heads * 8) + rank * local_heads) * head_dim;
                #pragma unroll 1
