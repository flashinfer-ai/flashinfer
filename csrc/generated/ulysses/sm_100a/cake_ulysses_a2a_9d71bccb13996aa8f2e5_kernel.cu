...
kernel_cake_ulysses_a2a_9d71bccb13996aa8f2e5(__nv_bfloat16* __restrict__ inp, CakePeerPointerTable<__nv_bfloat16> staging_peers, CakePeerPointerTable<unsigned int> signals_signals, unsigned int* __restrict__ signals_self, int32_t pg_rank, int B, int S_local, int H_local, int D)
{
    ...
    if (row_lo < row_hi) {
        ...
        if (vector_aligned != 0) {
            ...
            for (unsigned int unit_index = (unsigned int)tid; unit_index < total_units; unit_index += 512) {
                ...
                unsigned int scatter_batch = (unsigned int)0;
                if (w4_bf16_performance_shape != 0) {
                    ...
                    peer = row_quotient % 4;
                    scatter_batch = row_quotient / 4;
                }
                unsigned int vector_element = unit_in_row * 8;
                unsigned int batch = scatter_batch;
                unsigned int src_offset = ((batch * sequences + sequence) * (local_heads * 4) + peer * local_heads) * head_dim;
            ...
            for (unsigned int row_1 = row_lo; row_1 < row_hi; row_1++) {
                ...
                unsigned int batch_1 = row_quotient_1 / 4;
                unsigned int src_offset_1 = ((batch_1 * sequences + sequence_1) * (local_heads * 4) + peer_1 * local_heads) * head_dim;
                unsigned int dst_offset_1 = (batch_1 * (sequences * 4) + rank * sequences + sequence_1) * block_len;
                #pragma unroll 1
