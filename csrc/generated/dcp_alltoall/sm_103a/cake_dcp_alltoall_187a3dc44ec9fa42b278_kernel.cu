...
#define SMEM_PACKET_STAGE_BYTES 2048
#define SMEM_PACKET_STRIDE 2048
#define SMEM_TOTAL 2112
#define THREADS 128
...
kernel_cake_dcp_alltoall_187a3dc44ec9fa42b278(__nv_bfloat16* __restrict__ partial_o, float* __restrict__ softmax_stats, __nv_bfloat16* __restrict__ partial_o_out, float* __restrict__ softmax_stats_out, uint8_t* __restrict__ workspace, unsigned long long workspace_stride_in_u64, int cp_rank, int entry_count, int max_channel_count)
{
    ...
    unsigned long long fifo_region_bytes = 2097152 * (unsigned long long)max_channel_count;
    unsigned long long info_plane_bytes = 1024 * (unsigned long long)max_channel_count;
    ...
    if (is_sender != 0) {
        ...
        int fifo_index = -1;
        #pragma unroll 1
        for (int entry = channel; entry < entry_count; entry += run_channels) {
            long long data_index = (long long)entry * 4 + (long long)peer;
            if (elect_sync()) {
                cp_async_bulk_gmem2smem(group_smem_addr, partial_o + (data_index * 128), 256, ready_addr + (group) * 8);
            }
            asm volatile(
                "{\n\t"
                ".reg .pred p;\n\t"
                ...
                "}"
                :: "r"((lane_1 == 0) ? 1 : 0), "r"(group_smem_addr + 256), "l"(softmax_stats + (data_index * 2)));
        ...
        for (int entry_1 = channel; entry_1 < entry_count; entry_1 += run_channels) {
            if (entry_1 + run_channels >= entry_count) {
                asm volatile("griddepcontrol.launch_dependents;" ::: "memory");
            }
            ...
            __syncwarp();
            long long data_index_1 = (long long)entry_1 * 4 + (long long)peer;
            if (elect_sync()) {
                {
                    void* _cpbulk_dst_0 = reinterpret_cast<void*>(partial_o_out + (data_index_1 * 128));
                    asm volatile(
                        "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
                        :: "l"(_cpbulk_dst_0), "r"(group_smem_addr), "r"((uint32_t)(256))
                        : "memory");
                }
                float stats[2];
                    ...
                    : "r"(group_smem_addr + 256));
                {
                    ...
                    *reinterpret_cast<float2*>(softmax_stats_out + (data_index_1 * 2) + 0) = _v2;
                }
            }
