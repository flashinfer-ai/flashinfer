...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_dsv4_bf16_h128_swa128'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
void Run(TensorView arg_tmap_q, TensorView arg_tmap_swa_kv, TensorView arg_O, TensorView arg_sparse_indices, TensorView arg_sparse_topk_lens, TensorView arg_sinks, TensorView arg_bmm1_scale, TensorView arg_bmm2_scale, int64_t arg_num_heads, int64_t arg_num_head_tiles, int64_t arg_has_sinks, TensorView arg_tma_descriptor_workspace, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_dsv4_bf16_h128_swa128),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_dsv4_bf16_h128_swa128 failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_dsv4_bf16_h128_swa128), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_dsv4_bf16_h128_swa128 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_d67f012bdae641ea

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run_bf16_h128_swa128, cake_host_shim_d67f012bdae641ea::Run);
