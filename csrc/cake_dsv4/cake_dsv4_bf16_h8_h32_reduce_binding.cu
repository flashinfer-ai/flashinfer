...
#include "cake_dsv4_bf16_h8_h32_reduce.cu"
#undef CUtensorMap
#undef CakeTensorMap
#undef uint8_t
#undef uint16_t
...
void Run_bf16_h8_h32_reduce(TensorView arg_partial_O, TensorView arg_partial_lse, TensorView arg_O, int64_t arg_num_heads, int64_t arg_num_splits, int64_t grid_x, int64_t grid_y, int64_t grid_z, int64_t cuda_stream) {
  ...
  status = cudaLaunchKernel(
      reinterpret_cast<const void*>(kernel_cake_dsv4_bf16_h8_h32_reduce), grid, block, kargs,
      128u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_cake_dsv4_bf16_h8_h32_reduce launch failed: " << cudaGetErrorString(status);
}
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(
    run_bf16_h8_h32_reduce, flashinfer::cake_dsv4::Run_bf16_h8_h32_reduce);
