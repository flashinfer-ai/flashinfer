...
#include "cake_dsv4_fp8_lowhead_prefill.cu"
#undef CUtensorMap
#undef CakeTensorMap
#undef uint8_t
#undef uint16_t
...
inline CUtensorMap EncodeTma_tmap_q(const TensorView& t) {
  ...
  TVM_FFI_CHECK(128u <= global_dim[0] && 1u <= global_dim[2] && 1u <= global_dim[3], ValueError)
      << "TMA box (128, 128, 1, 1) exceeds resolved global dims for 'tmap_q'";
...
void Run_fp8_lowhead_prefill(TensorView arg_tmap_q, TensorView arg_tmap_swa_kv, TensorView arg_tmap_compressed_kv, TensorView arg_O, TensorView arg_partial_lse, TensorView arg_sparse_indices, TensorView arg_sparse_topk_lens, TensorView arg_sinks, TensorView arg_bmm1_scale, TensorView arg_bmm2_scale, int64_t arg_num_heads, int64_t arg_num_query_tokens, int64_t arg_sparse_topk, int64_t arg_has_sinks, int64_t arg_total_work_items, int64_t grid_x, int64_t grid_y, int64_t grid_z, int64_t cuda_stream) {
  ...
  status = cudaFuncSetAttribute(kernel_cake_dsv4_fp8_lowhead_prefill,
      cudaFuncAttributeMaxDynamicSharedMemorySize,
      205824);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_cake_dsv4_fp8_lowhead_prefill) failed: "
      << cudaGetErrorString(status);
  status = cudaLaunchKernel(
      reinterpret_cast<const void*>(kernel_cake_dsv4_fp8_lowhead_prefill), grid, block, kargs,
      205824u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_cake_dsv4_fp8_lowhead_prefill launch failed: " << cudaGetErrorString(status);
}
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(
    run_fp8_lowhead_prefill, flashinfer::cake_dsv4::Run_fp8_lowhead_prefill);
