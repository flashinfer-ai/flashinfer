...
kernel_cake_dsv4_bf16_h128_topk128x(CakeTensorMap const* tmap_q, CakeTensorMap const* tmap_swa_kv, CakeTensorMap const* tmap_compressed_kv, __nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_lse, int* __restrict__ sparse_indices, int* __restrict__ sparse_topk_lens, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int num_head_tiles, int sparse_topk, int num_splits, int has_sinks)
{
    ...
    if (warp <= 3) {
        ...
        { // softmax_main
            ...
            int split_head_work = blockIdx.x >> 2;
            int split_idx = split_head_work % num_splits;
            int head_work = split_head_work / num_splits;
            int head_tile = head_work % num_head_tiles;
            int query_idx = head_work / num_head_tiles;
            ...
            int global_head_idx = head_tile * 64 + my_row;
            ...
            row_max = _max_0;
            if (has_sinks != 0 && split_idx == 0 && global_head_idx < num_heads) {
                float sink_unscaled = sinks[global_head_idx] * 1.4426950408889634f / softmax_scale_log2;
            ...
            if (has_sinks != 0 && split_idx == 0 && global_head_idx < num_heads) {
                float _exp2_8 = approx_exp2(sinks[global_head_idx] * 1.4426950408889634f - max_scaled);
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // epilogue_main
            ...
            int split_head_work_1 = work_idx >> 2;
            int split_idx_1 = split_head_work_1 % num_splits;
            int head_work_1 = split_head_work_1 / num_splits;
            int head_tile_1 = head_work_1 % num_head_tiles;
            int query_idx_1 = head_work_1 / num_head_tiles;
            ...
            int global_head_idx_1 = head_tile_1 * 64 + my_row_1;
            ...
            int output_base = ((query_idx_1 * num_heads + global_head_idx_1) * num_splits + split_idx_1) * 512 + v_chunk * 128;
            ...
            if (global_head_idx_1 < num_heads) {
                #pragma unroll
                ...
                if (v_chunk == 0 && col_half_1 == 0) {
                    int lse_offset = (query_idx_1 * num_heads + global_head_idx_1) * num_splits + split_idx_1;
    ...
    if (warp >= 12 && warp <= 15) {
        ...
        { // load_warp_main
            ...
            int split_head_work_2 = work_idx_1 >> 2;
            int split_idx_2 = split_head_work_2 % num_splits;
            int head_work_2 = split_head_work_2 / num_splits;
            int head_tile_2 = head_work_2 % num_head_tiles;
            int query_idx_2 = head_work_2 / num_head_tiles;
