...
#include "cake_dsv4_bf16_h128_prefill.cu"
#undef CUtensorMap
#undef CakeTensorMap
#undef uint8_t
#undef uint16_t
...
void Run_bf16_h128_prefill(TensorView arg_tmap_q, TensorView arg_tmap_swa_k, TensorView arg_tmap_compressed_k, TensorView arg_tmap_swa_v, TensorView arg_tmap_compressed_v, TensorView arg_O, TensorView arg_sparse_indices, TensorView arg_sparse_topk_lens, TensorView arg_sinks, TensorView arg_bmm1_scale, TensorView arg_bmm2_scale, int64_t arg_num_heads, int64_t arg_num_query_tokens, int64_t arg_sparse_topk, int64_t arg_has_sinks, int64_t arg_total_work_items, int64_t grid_x, int64_t grid_y, int64_t grid_z, int64_t cuda_stream) {
  ...
  void* kargs[] = {&p_tmap_q, &p_tmap_swa_k, &p_tmap_compressed_k, &p_tmap_swa_v, &p_tmap_compressed_v, &p_O, &p_sparse_indices, &p_sparse_topk_lens, &p_sinks, &p_bmm1_scale, &p_bmm2_scale, &v_num_heads, &v_num_query_tokens, &v_sparse_topk, &v_has_sinks, &v_total_work_items};

  dim3 grid((uint32_t)grid_x, (uint32_t)grid_y, (uint32_t)grid_z);
  dim3 block(512u, 1u, 1u);
  cudaError_t status = cudaSuccess;
  status = cudaFuncSetAttribute(kernel_cake_dsv4_bf16_h128_prefill,
      cudaFuncAttributeMaxDynamicSharedMemorySize,
      219648);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_cake_dsv4_bf16_h128_prefill) failed: "
  ...
  status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_dsv4_bf16_h128_prefill), kargs);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_cake_dsv4_bf16_h128_prefill launch failed: " << cudaGetErrorString(status);
}
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(
    run_bf16_h128_prefill, flashinfer::cake_dsv4::Run_bf16_h128_prefill);
