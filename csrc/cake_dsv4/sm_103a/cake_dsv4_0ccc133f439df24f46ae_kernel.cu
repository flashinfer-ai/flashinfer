    ...
    if (warp <= 3) {
        ...
        { // softmax_main
            ...
            for (int h_3 = 0; h_3 < 8; h_3++) {
                float _shfl_0;
                asm volatile("shfl.sync.idx.b32 %0, %1, %2, 0x1f, 0xffffffff;" : "=f"(_shfl_0) : "f"(tile_max[h_3]), "r"(h_3));
                        ...
                        if (global_head_4 < num_heads) {
                            ...
                            for (int merge_chunk = 0; merge_chunk < 2; merge_chunk++) {
                                ...
                                for (int split = 0; split < num_splits; split++) {
                                    ...
                                    float _shfl_1;
                                    asm volatile("shfl.sync.idx.b32 %0, %1, %2, 0x1f, 0xffffffff;" : "=f"(_shfl_1) : "f"(normalized_weight), "r"(split));
