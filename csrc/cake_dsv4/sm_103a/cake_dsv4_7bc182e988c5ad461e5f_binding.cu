...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_dsv4_7bc182e988c5ad461e5f'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
void Run(TensorView arg_tmap_q, TensorView arg_tmap_swa_kv, TensorView arg_tmap_compressed_kv, TensorView arg_O, TensorView arg_sparse_indices, TensorView arg_sparse_topk_lens, TensorView arg_sinks, TensorView arg_bmm1_scale, TensorView arg_bmm2_scale, int64_t arg_num_heads, int64_t arg_sparse_topk, int64_t arg_total_work_items, int64_t arg_has_sinks, tvm::ffi::Tensor arg_tma_descriptor_workspace, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  void* kargs[] = {&p_tmap_q, &p_tmap_swa_kv, &p_tmap_compressed_kv, &p_O, &p_sparse_indices, &p_sparse_topk_lens, &p_sinks, &p_bmm1_scale, &p_bmm2_scale, &v_num_heads, &v_sparse_topk, &v_total_work_items, &v_has_sinks};
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_dsv4_7bc182e988c5ad461e5f),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_dsv4_7bc182e988c5ad461e5f failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_dsv4_7bc182e988c5ad461e5f), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_dsv4_7bc182e988c5ad461e5f failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_cc1a0694d720aa97

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_cc1a0694d720aa97::Run);
