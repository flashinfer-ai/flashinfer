...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_dsv4_eea345a0ced493c7a6b1'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_cake_dsv4_eea345a0ced493c7a6b1(CakeTensorMap const* tmap_q, CakeTensorMap const* tmap_swa_kv, CakeTensorMap const* tmap_compressed_kv, __nv_bfloat16* __restrict__ O, int* __restrict__ sparse_indices, int* __restrict__ sparse_topk_lens, int* __restrict__ seq_lens, int* __restrict__ cum_seq_lens_q, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int sparse_topk, int batch_size, int max_q_len, int ragged_query, int has_sinks);


namespace cake_host_shim_a111f219510b1892 {
...
inline CUtensorMap EncodeTma_tmap_q(const TensorView& t) {
  ...
  TVM_FFI_CHECK(64u <= global_dim[0] && 8u <= global_dim[2] && 1u <= global_dim[3], ValueError)
      << "TMA box (64, 64, 8, 1) exceeds resolved global dims for 'tmap_q'";
  ...
  uint32_t box_dim[4] = {64u, 64u, 8u, 1u};
...
void Run(TensorView arg_tmap_q, TensorView arg_tmap_swa_kv, TensorView arg_tmap_compressed_kv, TensorView arg_O, TensorView arg_sparse_indices, TensorView arg_sparse_topk_lens, TensorView arg_seq_lens, TensorView arg_cum_seq_lens_q, TensorView arg_sinks, TensorView arg_bmm1_scale, TensorView arg_bmm2_scale, int64_t arg_num_heads, int64_t arg_sparse_topk, int64_t arg_batch_size, int64_t arg_max_q_len, int64_t arg_ragged_query, int64_t arg_has_sinks, tvm::ffi::Tensor arg_tma_descriptor_workspace, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_dsv4_eea345a0ced493c7a6b1),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_dsv4_eea345a0ced493c7a6b1 failed: "
  ...
  config.dynamicSmemBytes = 198144u;
  config.stream = stream;
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_dsv4_eea345a0ced493c7a6b1), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_dsv4_eea345a0ced493c7a6b1 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_a111f219510b1892

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_a111f219510b1892::Run);
