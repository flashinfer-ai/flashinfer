...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_dsv4_1dde79eadd77e010b5e6'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
inline CUtensorMap EncodeTma_tmap_q(const TensorView& t) {
  ...
  TVM_FFI_CHECK(64u <= global_dim[0] && 2u <= global_dim[2] && 1u <= global_dim[3], ValueError)
      << "TMA box (64, 8, 2, 1) exceeds resolved global dims for 'tmap_q'";
...
void Run(TensorView arg_tmap_q, TensorView arg_tmap_swa_kv, TensorView arg_tmap_compressed_kv, TensorView arg_partial_O, TensorView arg_partial_lse, TensorView arg_O, TensorView arg_partition_arrivals, TensorView arg_sparse_indices, TensorView arg_sparse_topk_lens, TensorView arg_sinks, TensorView arg_bmm1_scale, TensorView arg_bmm2_scale, int64_t arg_num_heads, int64_t arg_sparse_topk, int64_t arg_num_splits, int64_t arg_num_head_tiles, int64_t arg_has_sinks, int64_t arg_completion_base, TensorView arg_tma_descriptor_workspace, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  CheckCudaTensor(arg_partition_arrivals, "partition_arrivals");
  CheckDtype(arg_partition_arrivals, "partition_arrivals", 1, 32, 1);
  CheckContiguous(arg_partition_arrivals, "partition_arrivals");
  ...
  TVM_FFI_CHECK(arg_num_splits >= -2147483648LL && arg_num_splits <= 2147483647LL, ValueError)
      ...
      << " is outside i32 range [-2147483648, 2147483647]";
  TVM_FFI_CHECK(arg_num_head_tiles >= -2147483648LL && arg_num_head_tiles <= 2147483647LL, ValueError)
      << "scalar 'num_head_tiles' value " << arg_num_head_tiles
      << " is outside i32 range [-2147483648, 2147483647]";
  TVM_FFI_CHECK(arg_has_sinks >= -2147483648LL && arg_has_sinks <= 2147483647LL, ValueError)
      << "scalar 'has_sinks' value " << arg_has_sinks
      << " is outside i32 range [-2147483648, 2147483647]";
  TVM_FFI_CHECK(arg_completion_base >= -2147483648LL && arg_completion_base <= 2147483647LL, ValueError)
      << "scalar 'completion_base' value " << arg_completion_base
      << " is outside i32 range [-2147483648, 2147483647]";
  ...
  CheckSameCudaDevice(arg_partition_arrivals, arg_tmap_q, "partition_arrivals", "tmap_q");
  ...
  CakeTensorMap const* p_tmap_q = static_cast<CakeTensorMap const*>(CallerTmaWorkspaceSlot(arg_tma_descriptor_workspace, 0u));
  CUtensorMap h_tmap_swa_kv = EncodeTma_tmap_swa_kv(arg_tmap_swa_kv);
  CakeTensorMap const* p_tmap_swa_kv = static_cast<CakeTensorMap const*>(CallerTmaWorkspaceSlot(arg_tma_descriptor_workspace, 1u));
  CUtensorMap h_tmap_compressed_kv = EncodeTma_tmap_compressed_kv(arg_tmap_compressed_kv);
  CakeTensorMap const* p_tmap_compressed_kv = static_cast<CakeTensorMap const*>(CallerTmaWorkspaceSlot(arg_tma_descriptor_workspace, 2u));
  ...
  unsigned int* p_partition_arrivals = static_cast<unsigned int*>(arg_partition_arrivals.data_ptr());
  ...
  int32_t v_num_head_tiles = (int32_t)arg_num_head_tiles;
  int32_t v_has_sinks = (int32_t)arg_has_sinks;
  int32_t v_completion_base = (int32_t)arg_completion_base;
  void* kargs[] = {&p_tmap_q, &p_tmap_swa_kv, &p_tmap_compressed_kv, &p_partial_O, &p_partial_lse, &p_O, &p_partition_arrivals, &p_sparse_indices, &p_sparse_topk_lens, &p_sinks, &p_bmm1_scale, &p_bmm2_scale, &v_num_heads, &v_sparse_topk, &v_num_splits, &v_num_head_tiles, &v_has_sinks, &v_completion_base};
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_dsv4_1dde79eadd77e010b5e6),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_dsv4_1dde79eadd77e010b5e6 failed: "
      << cudaGetErrorString(smem_status);
  dim3 grid((uint32_t)grid_x, (uint32_t)grid_y, (uint32_t)grid_z);
  dim3 block(256u, 1u, 1u);
  const CUtensorMap workspace_maps[] = {h_tmap_q, h_tmap_swa_kv, h_tmap_compressed_kv};
  UploadCallerTmaWorkspace(arg_tma_descriptor_workspace.data_ptr(), workspace_maps, stream);

  cudaLaunchConfig_t config{};
  config.gridDim = grid;
  config.blockDim = block;
  config.dynamicSmemBytes = 143104u;
  config.stream = stream;
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_dsv4_1dde79eadd77e010b5e6), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_dsv4_1dde79eadd77e010b5e6 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_4b3046134d6d6817

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_4b3046134d6d6817::Run);
