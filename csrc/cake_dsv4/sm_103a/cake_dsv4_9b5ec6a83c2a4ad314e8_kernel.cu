...
kernel_cake_dsv4_9b5ec6a83c2a4ad314e8(CakeTensorMap const* tmap_q, CakeTensorMap const* tmap_swa_kv, CakeTensorMap const* tmap_compressed_kv, __nv_bfloat16* __restrict__ O, int* __restrict__ sparse_indices, int* __restrict__ sparse_topk_lens, int* __restrict__ seq_lens, int* __restrict__ cum_seq_lens_q, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int sparse_topk, int batch_size, int max_q_len, int ragged_query, int has_sinks)
{
    ...
    #define pv_done_addr (mbar_base + 72)
    ...
    if (warp == 0) {
        ...
        if (leader) {
            ...
            // tmem_dealloc: 1 barriers, init_count=7
            mbarrier_init(smem + 80, 7);
        ...
        { // softmax_main
            ...
            for (int tile = 0; tile < 1; tile++) {
                if (tile_count > tile) {
                    ...
                    if (tile == 0) {
                        int valid_swa_cols = swa_valid_columns - col_half * 64;
                        if (valid_swa_cols < 0) {
                            valid_swa_cols = 0;
                        }
                        if (valid_swa_cols < valid_cols) {
                            valid_cols = valid_swa_cols;
                        }
                    }
            ...
            if (sparse_topk == 128) {
                mbarrier_wait(pv_done_addr, 0);
                asm volatile("tcgen05.fence::after_thread_sync;");
                float output_scale = bmm2_scale[0];
                float _rcp_0 = approx_rcp(row_sum);
                float inv_sum = _rcp_0;
                int output_base = (query_idx * num_heads + my_row) * 512 + blockIdx.y * 256;
                #pragma unroll
                for (int v_stage = 0; v_stage < 2; v_stage++) {
                    int output_col = 256 + v_stage * 128;
                    int output_addr = taddr + (unsigned int)output_col + (unsigned int)(tmem_row_origin << 16);
                    ...
                    asm volatile(
                        ...
                        : "r"(output_addr + 32));
                    int col_base = v_stage * 128 + col_half * 64;
                    if (my_row < num_heads) {
                        #pragma unroll
                        for (int offset = 0; offset < 64; offset += 8) {
                            {
                                const float2 _prescale2_6 = {inv_sum * output_scale, inv_sum * output_scale};
                                #if __CUDA_ARCH__ >= 1000
                                #pragma unroll
                                for (int _ps = 0; _ps < 4; _ps++)
                                    mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_1[offset])[_ps], _prescale2_6);
                                #else
                                #pragma unroll
        ...
        { // mma_warp_main
            ...
            for (int tile_1 = 0; tile_1 < 1; tile_1++) {
                if (tile_count_1 > tile_1) {
                    ...
                    int score_col_1 = ((phase_1 != 0) ? 128 : 0);
                    #pragma unroll
                    for (int k_stage = 0; k_stage < 4; k_stage++) {
                        mbarrier_wait(kv_full_addr + (k_stage) * 8, phase_1);
                        asm volatile("tcgen05.fence::after_thread_sync;");
                        ...
                        int _mma_b_lo_1 = make_warp_uniform((((smem_kv_addr + 16384) >> 4) & 0x3FFF) + (k_stage) * 2048);
                        {
                            ...
                            if (elect_sync()) {
                                tcgen05_mma_f16((tmem_tmem + (score_col_1)), _mma_ss_a_desc_1, _mma_ss_b_desc_1, 69207184, 1);
                            }
                        ...
                        if (k_stage == 3) {
                            elect_commit(s_full_addr + (phase_1) * 8);
                        }
                    }
                    int wait_phase_1 = tile_1 >> 1 & 1;
                    mbarrier_wait(p_full_addr + (phase_1) * 8, wait_phase_1);
                    asm volatile("tcgen05.fence::after_thread_sync;");
                    #pragma unroll
                    for (int v_stage_1 = 0; v_stage_1 < 2; v_stage_1++) {
                        int _mma_b_lo_2 = make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (blockIdx.y * 2 + v_stage_1) * 2048);
                        asm volatile(
                    "{\n\t"
                    ...
                    "@leader tcgen05.mma.cta_group::1.kind::f16 [%0], [ta], db, id, p1;\n\t"
                    "}\n"
                    :: "r"((tmem_tmem + (256 + v_stage_1 * 128))), "r"(_mma_b_lo_2), "r"(tmem_tmem + (score_col_1 + 64)), "r"(0));
                    }
    ...
    if (warp >= 5 && warp <= 7) {
        { // load_warp_main
            const int load_warp_rank = warp - 5;
            ...
            int group = (unsigned int)load_warp_rank + lane * 3;
            int row0 = 0;
            int row1 = 0;
            int row2 = 0;
            int row3 = 0;
            if (group < 32) {
                int index_base = group * 4;
                int raw_rows[4];
                int _vec_load_0[4];
                {
                    const int4* _ivptr_0 = reinterpret_cast<const int4*>(sparse_indices + (sparse_base + index_base) + 0);
                    int4 _ivld_0;
                    _ivld_0 = *_ivptr_0;
                ...
                #pragma unroll
                for (int i = 0; i < 4; i++) {
                    raw_rows[i] = _vec_load_0[i];
                }
                row0 = ((raw_rows[0] >= 0) ? raw_rows[0] : 0);
                row1 = ((raw_rows[1] >= 0) ? raw_rows[1] : 0);
                row2 = ((raw_rows[2] >= 0) ? raw_rows[2] : 0);
                row3 = ((raw_rows[3] >= 0) ? raw_rows[3] : 0);
            }
            #pragma unroll
            for (int tile_2 = 0; tile_2 < 1; tile_2++) {
