...
kernel_cake_dsv4_285a982d8a32811fdae6(CakeTensorMap const* tmap_q, CakeTensorMap const* tmap_swa_kv, __nv_bfloat16* __restrict__ O, int* __restrict__ sparse_indices, int* __restrict__ sparse_topk_lens, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int has_sinks)
{
    ...
    if (warp <= 1) {
        { // softmax_main
            ...
            asm volatile(
                ...
                : "r"(score_addr + 32));
            int valid_cols = active_topk - col_half * 64;
    ...
    if (warp >= 8 && warp <= 11) {
        ...
        { // load_warp_main
            const int load_warp_rank = warp - 8;
            int work_idx_1 = blockIdx.x;
            int query_idx_2 = work_idx_1 >> 2;
            int v_chunk_1 = work_idx_1 & 3;
            int sparse_base = query_idx_2 * 128;
            if (load_warp_rank == 0) {
                if (elect_sync()) {
            ...
            int sparse_row = sparse_indices[sparse_base + index_offset];
