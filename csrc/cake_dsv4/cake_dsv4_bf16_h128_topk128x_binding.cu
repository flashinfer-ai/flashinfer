...
#include "cake_dsv4_bf16_h128_topk128x.cu"
#undef CUtensorMap
#undef CakeTensorMap
#undef uint8_t
#undef uint16_t
...
void Run_bf16_h128_topk128x(TensorView arg_tmap_q, TensorView arg_tmap_swa_kv, TensorView arg_tmap_compressed_kv, TensorView arg_partial_O, TensorView arg_partial_lse, TensorView arg_sparse_indices, TensorView arg_sparse_topk_lens, TensorView arg_sinks, TensorView arg_bmm1_scale, TensorView arg_bmm2_scale, int64_t arg_num_heads, int64_t arg_num_head_tiles, int64_t arg_sparse_topk, int64_t arg_num_splits, int64_t arg_has_sinks, int64_t grid_x, int64_t grid_y, int64_t grid_z, int64_t cuda_stream) {
  ...
  void* kargs[] = {&p_tmap_q, &p_tmap_swa_kv, &p_tmap_compressed_kv, &p_partial_O, &p_partial_lse, &p_sparse_indices, &p_sparse_topk_lens, &p_sinks, &p_bmm1_scale, &p_bmm2_scale, &v_num_heads, &v_num_head_tiles, &v_sparse_topk, &v_num_splits, &v_has_sinks};

  dim3 grid((uint32_t)grid_x, (uint32_t)grid_y, (uint32_t)grid_z);
  dim3 block(512u, 1u, 1u);
  cudaError_t status = cudaSuccess;
  status = cudaFuncSetAttribute(kernel_cake_dsv4_bf16_h128_topk128x,
      cudaFuncAttributeMaxDynamicSharedMemorySize,
      198656);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_cake_dsv4_bf16_h128_topk128x) failed: "
      << cudaGetErrorString(status);
  status = cudaLaunchKernel(
      reinterpret_cast<const void*>(kernel_cake_dsv4_bf16_h128_topk128x), grid, block, kargs,
      198656u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_cake_dsv4_bf16_h128_topk128x launch failed: " << cudaGetErrorString(status);
}
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(
    run_bf16_h128_topk128x, flashinfer::cake_dsv4::Run_bf16_h128_topk128x);
