...
kernel_cake_dsv4_d74fbd280a9f5576be33(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_swa_kv, __nv_bfloat16* __restrict__ O, int* __restrict__ sparse_indices, int* __restrict__ sparse_topk_lens, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int num_head_tiles, int has_sinks)
{
    ...
    if (warp <= 3) {
        ...
        { // softmax_main
            ...
            if (has_sinks != 0 && head_idx < num_heads) {
                float _exp2_8 = approx_exp2(sinks[head_idx] * 1.4426950408889634f - max_scaled);
