...
kernel_cake_dsv4_d9a6f0b6d78de4242818(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_swa_kv, const __grid_constant__ CUtensorMap tmap_compressed_kv, __nv_bfloat16* __restrict__ O, float* __restrict__ partial_lse, int* __restrict__ sparse_indices, int* __restrict__ sparse_topk_lens, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int num_query_tokens, int sparse_topk, int has_sinks, int total_work_items)
{
            ...
            for (unsigned int work_idx_1 = 0; work_idx_1 < 1; work_idx_1++) {
                ...
                for (int tile_1 = 0; tile_1 < num_kv_tiles_1; tile_1++) {
                    ...
                    mbarrier_arrive(p_full_addr + (phase) * 8);
                    {
                        {
                            asm volatile(".pragma \"next knob FenceCode\";\n" ::: "memory");
                            float _fma_3 = __fmaf_rn(row_sum_val, acc_scale, block_sum);
                            row_sum_val = _fma_3;
                        }
                    }
                }
    ...
    if (warp == 4 || warp == 6) {
        { // correction_wg_main
            ...
            for (unsigned int work_idx_2 = 0; work_idx_2 < 1; work_idx_2++) {
                ...
                int partial_o_offset = (query_idx_3 * num_heads + head_idx + split_idx_3) * 512 + ((0) ? v_chunk_1 : n_half_1) * 256;
