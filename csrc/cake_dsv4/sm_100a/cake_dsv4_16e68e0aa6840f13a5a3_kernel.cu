...
kernel_cake_dsv4_16e68e0aa6840f13a5a3(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_swa_kv, __nv_bfloat16* __restrict__ O, int* __restrict__ sparse_indices, int* __restrict__ sparse_topk_lens, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int has_sinks)
{
    ...
    if (warp == 0) {
        ...
        if (leader) {
            ...
            // sum_ready: 1 barriers, init_count=32
            mbarrier_init(smem + 88, 32);
            // o_done: 1 barriers, init_count=1
            mbarrier_init(smem + 96, 1);
            // tmem_dealloc: 1 barriers, init_count=64
    ...
    // ---- Role: empty ----
    if (warp == 1 || warp == 3 || warp == 5 || warp == 6 || warp == 7) {
