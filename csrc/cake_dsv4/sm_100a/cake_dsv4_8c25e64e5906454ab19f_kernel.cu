...
kernel_cake_dsv4_8c25e64e5906454ab19f(__nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_lse, __nv_bfloat16* __restrict__ O, int num_q_heads, int num_split)
{
    ...
    if (lane < 5) {
        local_m = partial_lse[(unsigned int)stat_base + lane];
    }
