...
kernel_cake_dsv4_b312710763724940c7bb(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_swa_kv, const __grid_constant__ CUtensorMap tmap_compressed_kv, __nv_bfloat16* __restrict__ O, float* __restrict__ partial_lse, int* __restrict__ sparse_indices, int* __restrict__ sparse_topk_lens, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int num_query_tokens, int sparse_topk, int has_sinks, int total_work_items)
{
                ...
                }
                {
                    ...
                    for (int index_pass = load_warp_rank; index_pass < num_index_passes; index_pass += 4) {
                        ...
                        if (global_index_offset < sparse_topk) {
                            int _vec_load_0[4];
                            {
                                const int4* _ivptr_0 = reinterpret_cast<const int4*>(sparse_indices + (sparse_base + global_index_offset) + 0);
                                int4 _ivld_0;
                                _ivld_0 = *_ivptr_0;
