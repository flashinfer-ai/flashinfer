...
kernel_cake_dsv4_36cd52d1d0ac280bdc5b(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_swa_kv, const __grid_constant__ CUtensorMap tmap_compressed_kv, __nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_lse, __nv_bfloat16* __restrict__ O, unsigned int* __restrict__ partition_arrivals, int* __restrict__ sparse_indices, int* __restrict__ sparse_topk_lens, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int sparse_topk, int num_splits, int num_head_tiles, int has_sinks, int completion_base)
{
    ...
    #define o_full_addr (mbar_base + 88)
    #define o_empty_addr (mbar_base + 96)
    ...
    // Mbarrier init (8 pipeline groups, 0 ordered-sequence groups, 14 barriers)
    ...
    if (warp <= 3) {
        ...
        { // compute_main
            ...
            int merge_group = query_idx * num_head_tiles + head_tile;
            const int d_lane = warp * 32 + lane;
            const int row_addr = warp * 32 << 16;
            ...
            for (int v_stage = 0; v_stage < 4; v_stage++) {
                ...
                const int d_idx = v_stage * 128 + d_lane;
                #pragma unroll
                for (int h_7 = 0; h_7 < 8; h_7++) {
                    int global_head_2 = head_base + h_7;
                    if (global_head_2 < num_heads) {
                        ...
                        int output_offset = ((query_idx * num_heads + global_head_2) * num_splits + split_idx) * 512 + d_idx;
            ...
            if (num_splits > 1) {
                ...
                if (split_idx == 0) {
                    ...
                    int merge_d_base = lane * 16;
                    #pragma unroll
                    for (int local_h = 0; local_h < 2; local_h++) {
                        int global_head_3 = (unsigned int)head_base + warp * 2 + (unsigned int)local_h;
                        if (global_head_3 < num_heads) {
                            int lse_base = (query_idx * num_heads + global_head_3) * num_splits;
                            ...
                            float _rcp_1 = approx_rcp(global_sum);
                            float normalized_weight = ((global_sum > 0.0f) ? local_weight * _rcp_1 : 0.0f);
                            float merged[16];
                            #pragma unroll
                            for (int elem = 0; elem < 16; elem++) {
                                merged[elem] = 0.0f;
                            }
                            int partial_base = (query_idx * num_heads + global_head_3) * num_splits * 512;
                            #pragma unroll 3
                            for (int split = 0; split < num_splits; split++) {
                                float _vec_load_0[16];
                                {
                                    const uint4* _vptr_2 = reinterpret_cast<const uint4*>(partial_O + (partial_base + split * 512 + merge_d_base) + 0);
                                    uint4 _vld_2[2];
                                    #pragma unroll
                            ...
                            int output_base = (query_idx * num_heads + global_head_3) * 512 + merge_d_base;
                            {
                                ...
                                _pk[4] = __floats2bfloat162_rn(merged[0 + 8], merged[0 + 9]);
                                _pk[5] = __floats2bfloat162_rn(merged[0 + 10], merged[0 + 11]);
                                _pk[6] = __floats2bfloat162_rn(merged[0 + 12], merged[0 + 13]);
                                _pk[7] = __floats2bfloat162_rn(merged[0 + 14], merged[0 + 15]);
                                *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(O + output_base))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                                *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(O + output_base))[8]) = *reinterpret_cast<uint4*>(&_pk[4]);
                            }
                        }
                    }
                }
    ...
    if (warp == 4) {
        { // mma_warp_main
            ...
            for (int v_stage_1 = 0; v_stage_1 < 4; v_stage_1++) {
                mbarrier_wait(kv_full_addr + (v_stage_1) * 8, 1);
                mbarrier_wait(o_empty_addr, _phase_o_empty_0);
                _phase_o_empty_0 ^= 1;
                asm volatile("tcgen05.fence::after_thread_sync;");
                int _mma_a_lo_1 = make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (v_stage_1) * 2048);
                int _mma_b_lo_1 = make_warp_uniform((((smem_p_addr) >> 4) & 0x3FFF) | 0x400000);
                {
                    ...
                    if (elect_sync()) {
                        tcgen05_mma_f16(tmem_tmem_o, _mma_ss_a_desc_1, _mma_ss_b_desc_1, 134382736, 0);
                    }
                    incr_smem_desc_lo(_mma_ss_a_desc_1, 128U);
                    incr_smem_desc_lo(_mma_ss_b_desc_1, 2U);
                    if (elect_sync()) {
                        tcgen05_mma_f16(tmem_tmem_o, _mma_ss_a_desc_1, _mma_ss_b_desc_1, 134382736, 1);
                    }
                    incr_smem_desc_lo(_mma_ss_a_desc_1, 128U);
                    incr_smem_desc_lo(_mma_ss_b_desc_1, 2U);
                    if (elect_sync()) {
    ...
    if (warp >= 5 && warp <= 7) {
        { // load_warp_main
            ...
            int merge_group_1 = query_idx_1 * num_head_tiles + head_tile_1;
            int sparse_base = query_idx_1 * sparse_topk;
            ...
            if (load_rank == 0) {
                if (elect_sync()) {
                    ...
                    for (int q_stage = 0; q_stage < 4; q_stage++) {
                        tma_4d_gmem2smem(smem_q_addr + (unsigned int)(q_stage * 2048), (&tmap_q), 0, head_tile_1 * 8, q_stage * 2, query_idx_1, q_full_addr);
                    }
                }
                int global_vector_offset = split_idx_1 * 128 + vector_offset;
            ...
            for (int k_stage_1 = 0; k_stage_1 < 4; k_stage_1++) {
                if (k_stage_1 % 3 == load_rank) {
                    ...
                    if (elect_sync()) {
                        mbarrier_arrive_expect_tx(kv_full_addr + (k_stage_1) * 8, 32768);
                    }
            ...
            for (int v_stage_2 = 0; v_stage_2 < 4; v_stage_2++) {
                if (v_stage_2 % 3 == load_rank) {
                    mbarrier_wait(kv_empty_addr + (v_stage_2) * 8, 0);
                    if (elect_sync()) {
                        mbarrier_arrive_expect_tx(kv_full_addr + (v_stage_2) * 8, 32768);
                    }
                    int dst_v = smem_v_addr + (unsigned int)(v_stage_2 * 32768);
                    if (split_idx_1 == 0) {
                        tma_gather4_gmem2smem((unsigned int)dst_v + lane * 512, (&tmap_swa_kv), v_stage_2 * 128, row0, row1, row2, row3, kv_full_addr + (v_stage_2) * 8);
                        tma_gather4_gmem2smem((unsigned int)(dst_v + 16384) + lane * 512, (&tmap_swa_kv), v_stage_2 * 128 + 64, row0, row1, row2, row3, kv_full_addr + (v_stage_2) * 8);
                    } else {
                        tma_gather4_gmem2smem((unsigned int)dst_v + lane * 512, (&tmap_compressed_kv), v_stage_2 * 128, row0, row1, row2, row3, kv_full_addr + (v_stage_2) * 8);
                        tma_gather4_gmem2smem((unsigned int)(dst_v + 16384) + lane * 512, (&tmap_compressed_kv), v_stage_2 * 128 + 64, row0, row1, row2, row3, kv_full_addr + (v_stage_2) * 8);
                    }
                }
            }
        }
