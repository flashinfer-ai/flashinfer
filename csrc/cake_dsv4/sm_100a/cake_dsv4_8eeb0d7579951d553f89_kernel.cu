...
#define SMEM_SMEM_INDEX_FLAGS_OFF 198656
...
kernel_cake_dsv4_8eeb0d7579951d553f89(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_swa_kv, const __grid_constant__ CUtensorMap tmap_compressed_kv, __nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_lse, int* __restrict__ sparse_indices, int* __restrict__ sparse_topk_lens, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int sparse_topk, int num_splits, int has_sinks)
{
    ...
    #define o_done_0_addr (mbar_base + 96)
    #define o_done_1_addr (mbar_base + 104)
    #define o_empty_0_addr (mbar_base + 112)
    #define o_empty_1_addr (mbar_base + 120)
    #define tmem_dealloc_addr (mbar_base + 128)
    ...
    int* smem_index_flags = reinterpret_cast<int*>(smem_raw + 198656);
    const int smem_index_flags_addr = smem + 198656;
    int* smem_indices = reinterpret_cast<int*>(smem_raw + 198144);
    const int smem_indices_addr = smem + 198144;

    // Mbarrier init (11 pipeline groups, 0 ordered-sequence groups, 17 barriers)
        ...
        { // epilogue_main
            ...
            for (int v_stage = 0; v_stage < 1; v_stage++) {
                ...
                for (int v_half = 0; v_half < 2; v_half++) {
                    ...
                    int v_offset = v_owner * 256 + v_half * 128 + col_base;
            ...
            if (v_owner == 0 && my_row_1 < num_heads && col_half_1 == 0) {
    ...
    if (warp == 8) {
        { // mma_warp_main
            ...
            #pragma unroll
            for (int v_stage_1 = 0; v_stage_1 < 1; v_stage_1++) {
                asm volatile("tcgen05.fence::after_thread_sync;");
                int _mma_b_lo_2 = make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (v_owner_1 * 2) * 2048);
                asm volatile(
                    "{\n\t"
                ...
                int _mma_b_lo_3 = make_warp_uniform(((((smem_v_addr) >> 4) & 0x3FFF) | 0x4000000) + (v_owner_1 * 2 + 1) * 2048);
                asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 dhi, blo, ta, id;\n\t"
        ...
        { // load_warp_main
            ...
            if (load_warp_rank == 0) {
                if (elect_sync()) {
                    ...
                    #pragma unroll
                    for (int q_stage = 0; q_stage < 8; q_stage += 8) {
            ...
            #pragma unroll
            for (int k_stage_2 = 0; k_stage_2 < 4; k_stage_2++) {
                mbarrier_wait(kv_empty_addr + (k_stage_2) * 8, _phase_kv_empty);
                if (elect_sync()) {
                    mbarrier_arrive_expect_tx(kv_full_addr + (k_stage_2) * 8, 8192);
                }
                #pragma unroll
                for (int local_group = 0; local_group < 8; local_group++) {
                    int dst_k = smem_kv_addr + (unsigned int)(k_stage_2 * 32768);
                    ...
                    if (elect_sync()) {
                        if (split_idx_2 == 0) {
                            ...
                            tma_gather4_gmem2smem(dst_k + group * 512, (&tmap_compressed_kv), k_stage_2 * 128, row0, row1, row2, row3, kv_full_addr + (k_stage_2) * 8);
                            tma_gather4_gmem2smem(dst_k + 16384 + group * 512, (&tmap_compressed_kv), k_stage_2 * 128 + 64, row0, row1, row2, row3, kv_full_addr + (k_stage_2) * 8);
                        }
                    }
                }
            }
