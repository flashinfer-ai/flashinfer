...
kernel_cake_dsv4_13fbb52aea3227dd009d(__nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_lse, __nv_bfloat16* __restrict__ O, int num_q_heads)
{
    ...
    #pragma unroll
    for (int i = 0; i < 1; i++) {
        int split_idx = lane + (unsigned int)(i * 32);
        local_m[i] = -CAKE_INF;
        local_w[i] = 0.0f;
        if (split_idx < 2) {
    ...
    for (int i_1 = 0; i_1 < 1; i_1++) {
        int split_idx_1 = lane + (unsigned int)(i_1 * 32);
        if (split_idx_1 < 2) {
    ...
    float local_weight = local_w[0] * inv_sum;
    #pragma unroll
    for (int split_idx_2 = 0; split_idx_2 < 2; split_idx_2++) {
        float _shfl_0 = __shfl_sync(0xFFFFFFFF, local_weight, split_idx_2);
        split_weights[split_idx_2] = _shfl_0;
    }
