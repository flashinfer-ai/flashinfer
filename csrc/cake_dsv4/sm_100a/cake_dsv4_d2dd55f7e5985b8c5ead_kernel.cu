...
kernel_cake_dsv4_d2dd55f7e5985b8c5ead(__nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_lse, __nv_bfloat16* __restrict__ O, int num_heads, int num_splits)
{
    ...
    if (num_splits == 4) {
        ...
        for (int split = 0; split < num_splits; split++) {
            ...
            float _shfl_4;
            asm volatile("shfl.sync.idx.b32 %0, %1, %2, 0x1f, 0xffffffff;" : "=f"(_shfl_4) : "f"(normalized_weight), "r"(split));
