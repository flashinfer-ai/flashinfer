...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_dsv4_2e0c00ebcbc54711cbfd'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_cake_dsv4_2e0c00ebcbc54711cbfd(__nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_lse, __nv_bfloat16* __restrict__ O, int num_heads, int num_splits);


namespace cake_host_shim_1e9d5c188d22251c {
...
void Run(TensorView arg_partial_O, TensorView arg_partial_lse, TensorView arg_O, int64_t arg_num_heads, int64_t arg_num_splits, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_dsv4_2e0c00ebcbc54711cbfd), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_dsv4_2e0c00ebcbc54711cbfd failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_1e9d5c188d22251c

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_1e9d5c188d22251c::Run);
