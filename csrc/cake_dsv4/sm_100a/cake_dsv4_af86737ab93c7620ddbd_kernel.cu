...
#define SMEM_SPLIT_WEIGHTS_STAGE_BYTES 8
#define SMEM_SPLIT_WEIGHTS_STRIDE 8
...
kernel_cake_dsv4_af86737ab93c7620ddbd(__nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_lse, __nv_bfloat16* __restrict__ O, int num_q_heads)
{
    ...
    if (warp == 0) {
        ...
        for (int i_2 = 0; i_2 < 1; i_2++) {
            int split_idx_2 = lane + (unsigned int)(i_2 * 32);
            if (split_idx_2 < 2) {
