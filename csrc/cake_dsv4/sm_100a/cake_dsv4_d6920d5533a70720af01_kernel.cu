...
#define SMEM_SMEM_INDICES_STAGE_BYTES 2048
#define SMEM_SMEM_INDICES_STRIDE 2048
...
kernel_cake_dsv4_d6920d5533a70720af01(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_swa_k, const __grid_constant__ CUtensorMap tmap_compressed_k, const __grid_constant__ CUtensorMap tmap_swa_v, const __grid_constant__ CUtensorMap tmap_compressed_v, __nv_bfloat16* __restrict__ O, float* __restrict__ partial_lse, int* __restrict__ sparse_indices, int* __restrict__ sparse_topk_lens, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int num_query_tokens, int sparse_topk, int has_sinks, int total_work_items)
{
    ...
    if (warp <= 3) {
        ...
        { // softmax_wg_main
            ...
            for (unsigned int work_idx = cluster_id; work_idx < total_work_items; work_idx += num_clusters) {
                int split_idx = work_idx % (unsigned int)(((1) ? 4 : 1));
                int query_idx = work_idx / (unsigned int)(((1) ? 4 : 1));
    ...
    if (warp >= 4 && warp <= 7) {
        ...
        { // correction_wg_main
            ...
            for (unsigned int work_idx_1 = cluster_id; work_idx_1 < total_work_items; work_idx_1 += num_clusters) {
                int split_idx_1 = work_idx_1 % (unsigned int)(((1) ? 4 : 1));
                int query_idx_1 = work_idx_1 / (unsigned int)(((1) ? 4 : 1));
                int num_kv_tiles_1 = ((sparse_topk + 128 - 1) / 128 + ((1) ? 4 : 1) - 1) / ((1) ? 4 : 1);
                ...
                int output_offset = ((1) ? ((query_idx_1 * num_heads + head_idx_1) * ((1) ? 4 : 1) + split_idx_1) * 512 : (query_idx_1 * num_heads + head_idx_1) * 512);
                {
                    if (n_half_1 == 0 && head_idx_1 < num_heads) {
                        int stat_offset = (query_idx_1 * num_heads + head_idx_1) * ((1) ? 4 : 1) + split_idx_1;
                        float _log2_0;
    ...
    if (warp == 8) {
        { // mma_warp_main
            ...
            for (unsigned int work_idx_2 = cluster_id; work_idx_2 < total_work_items; work_idx_2 += num_clusters) {
                int num_kv_tiles_2 = ((sparse_topk + 128 - 1) / 128 + ((1) ? 4 : 1) - 1) / ((1) ? 4 : 1);
                if (cta_rank == 0) {
    ...
    if (warp >= 9 && warp <= 12) {
        { // load_wg_main
            ...
            for (unsigned int work_idx_3 = cluster_id; work_idx_3 < total_work_items; work_idx_3 += num_clusters) {
                int split_idx_2 = work_idx_3 % (unsigned int)(((1) ? 4 : 1));
                int query_idx_2 = work_idx_3 / (unsigned int)(((1) ? 4 : 1));
                int num_kv_tiles_3 = ((sparse_topk + 128 - 1) / 128 + ((1) ? 4 : 1) - 1) / ((1) ? 4 : 1);
    ...
    if (warp == 13) {
        { // index_warp_main
            ...
            for (unsigned int work_idx_4 = cluster_id; work_idx_4 < total_work_items; work_idx_4 += num_clusters) {
                int split_idx_3 = work_idx_4 % (unsigned int)(((1) ? 4 : 1));
                int query_idx_3 = work_idx_4 / (unsigned int)(((1) ? 4 : 1));
                int sparse_base = query_idx_3 * sparse_topk + split_idx_3 * (4 / ((1) ? 4 : 1)) * 128;
                #pragma unroll
                for (int index_pass = 0; index_pass < 4 / ((1) ? 4 : 1); index_pass++) {
