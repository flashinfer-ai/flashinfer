...
kernel_cake_dsv4_d1cd994220423bc1c7cc(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_swa_kv, const __grid_constant__ CUtensorMap tmap_compressed_kv, __nv_bfloat16* __restrict__ O, float* __restrict__ partial_lse, int* __restrict__ sparse_indices, int* __restrict__ sparse_topk_lens, int* __restrict__ seq_lens, int* __restrict__ cum_seq_lens_q, float* __restrict__ sinks, float* __restrict__ bmm1_scale, float* __restrict__ bmm2_scale, int num_heads, int num_query_tokens, int sparse_topk, int has_sinks, int total_work_items, int max_q_len, int batch_size)
{
                                ...
                                if (first_tile_1 + tile == 0) {
                                    ...
                                    for (int group_local_1 = 0; group_local_1 < 8; group_local_1++) {
                                        ...
                                        for (int feature_part_1 = 0; feature_part_1 < 4; feature_part_1++) {
                                            ...
                                            tma_gather4_gmem2smem_mc_cta2(k_dst_1 + group_1 * 512, (&tmap_compressed_kv), feature_chunk_1 * 128, ((raw_rows_1[0] >= 0) ? raw_rows_1[0] : 0), ((raw_rows_1[1] >= 0) ? raw_rows_1[1] : 0), ((raw_rows_1[2] >= 0) ? raw_rows_1[2] : 0), ((raw_rows_1[3] >= 0) ? raw_rows_1[3] : 0), ((k_full_addr + (load_k_stage) * 8) & 0xFEFFFFFF), 1 << bid % 2);
                                        }
                                    }
                                }
                            }
                                ...
                                if (first_tile_1 + tile_1 == 0) {
                                    ...
                                    for (int v_group_local = 0; v_group_local < 16; v_group_local++) {
                                        ...
                                        for (int v_feature = 0; v_feature < 2; v_feature++) {
                                            ...
                                            tma_gather4_gmem2smem_mc_cta2(v_dst + group_2 * 512, (&tmap_swa_kv), v_col, ((raw_rows_2[0] >= 0) ? raw_rows_2[0] : 0), ((raw_rows_2[1] >= 0) ? raw_rows_2[1] : 0), ((raw_rows_2[2] >= 0) ? raw_rows_2[2] : 0), ((raw_rows_2[3] >= 0) ? raw_rows_2[3] : 0), ((v_full_addr + (load_v_stage) * 8) & 0xFEFFFFFF), 1 << bid % 2);
                                        }
                                    }
                                } else {
                                    #pragma unroll
                                    for (int v_group_local_1 = 0; v_group_local_1 < 16; v_group_local_1++) {
                                        ...
                                        for (int v_feature_1 = 0; v_feature_1 < 2; v_feature_1++) {
                                            ...
                                            tma_gather4_gmem2smem_mc_cta2(v_dst_1 + group_3 * 512, (&tmap_compressed_kv), v_col_1, ((raw_rows_3[0] >= 0) ? raw_rows_3[0] : 0), ((raw_rows_3[1] >= 0) ? raw_rows_3[1] : 0), ((raw_rows_3[2] >= 0) ? raw_rows_3[2] : 0), ((raw_rows_3[3] >= 0) ? raw_rows_3[3] : 0), ((v_full_addr + (load_v_stage) * 8) & 0xFEFFFFFF), 1 << bid % 2);
                                        }
                                    }
                                }
                            }
                    ...
                    for (int tile_2 = 0; tile_2 < num_kv_tiles_1; tile_2++) {
                        ...
                        float sv_split[64];
                        {
                            if (valid_sparse_cols == 64) {
                                ...
                                float _max_0 = max_noftz(_max3_27, sv_split[63]);
                                float _max3_30;
                                #if defined(__CUDA_ARCH__) && (__CUDA_ARCH__ < 1000)
                                #error "Max3 requires PTX three-input max.f32 support on sm_100+"
                                #endif
