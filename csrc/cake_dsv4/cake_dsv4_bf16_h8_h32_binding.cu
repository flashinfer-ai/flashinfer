...
#include "cake_dsv4_bf16_h8_h32.cu"
#undef CUtensorMap
#undef CakeTensorMap
#undef uint8_t
#undef uint16_t
...
void Run_bf16_h8_h32(TensorView arg_tmap_q, TensorView arg_tmap_swa_kv, TensorView arg_tmap_compressed_kv, TensorView arg_partial_O, TensorView arg_partial_lse, TensorView arg_sparse_indices, TensorView arg_sparse_topk_lens, TensorView arg_sinks, TensorView arg_bmm1_scale, TensorView arg_bmm2_scale, int64_t arg_num_heads, int64_t arg_sparse_topk, int64_t arg_num_splits, int64_t arg_has_sinks, int64_t grid_x, int64_t grid_y, int64_t grid_z, int64_t cuda_stream) {
  ...
  status = cudaFuncSetAttribute(kernel_cake_dsv4_bf16_h8_h32,
      cudaFuncAttributeMaxDynamicSharedMemorySize,
      198656);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute(kernel_cake_dsv4_bf16_h8_h32) failed: "
      << cudaGetErrorString(status);
  status = cudaLaunchKernel(
      reinterpret_cast<const void*>(kernel_cake_dsv4_bf16_h8_h32), grid, block, kargs,
      198656u, stream);
  TVM_FFI_CHECK(status == cudaSuccess, RuntimeError)
      << "kernel_cake_dsv4_bf16_h8_h32 launch failed: " << cudaGetErrorString(status);
}
...
TVM_FFI_DLL_EXPORT_TYPED_FUNC(
    run_bf16_h8_h32, flashinfer::cake_dsv4::Run_bf16_h8_h32);
