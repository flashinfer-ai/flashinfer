...
__device__ __forceinline__ void tcgen05_mma_mxf4nvf4_bs_cta2(int taddr, uint64_t a_desc,
                                                             uint64_t b_desc, uint32_t i_desc,
                                                             int sfa_taddr, int sfb_taddr,
                                                             int enable_input_d) {
  asm volatile(
      "{\n\t"
      ".reg .pred p;\n\t"
...
__device__ __forceinline__ void tcgen05_mma_mxf4nvf4_bs_ts_cta2(int taddr, int a_taddr,
                                                                uint64_t b_desc, uint32_t i_desc,
                                                                int sfa_taddr, int sfb_taddr,
                                                                int enable_input_d) {
  asm volatile(
...
__device__ __forceinline__ void tcgen05_cp_32x128b_warpx4_cta2(int taddr, uint64_t s_desc) {
...
__device__ __forceinline__ void tma_2d_gmem2smem_cta2(int dst, const void* tmap_ptr, int x, int y,
                                                      int mbar_addr) {
  asm volatile(
...
__device__ __forceinline__ void tma_5d_gmem2smem_cta2(int dst, const void* tmap_ptr, int x, int y,
                                                      int z, int w, int v, int mbar_addr) {
  asm volatile(
...
__global__ __launch_bounds__(512, 1) __cluster_dims__(2, 1, 1) void kernel_wan_hybrid_attention(
    WanHybridTensorMap const* Q, WanHybridTensorMap const* K, WanHybridTensorMap const* Vt,
    WanHybridTensorMap const* SFQ, WanHybridTensorMap const* SFK, WanHybridTensorMap const* SFVtLo,
    WanHybridTensorMap const* SFVtHi, float* __restrict__ QKCorrection, WanHybridTensorMap const* O,
    int seqlen_q, int seqlen_kv, int q_stride, int kv_stride, float softmax_scale_log2, int heads,
    int total_bh) {
  ...
  if (tid == 0) {
    ...
    asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" ::"l"((uint64_t)(Vt))
                 : "memory");
    asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" ::"l"((uint64_t)(SFQ))
                 : "memory");
    asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" ::"l"((uint64_t)(SFK))
                 : "memory");
  ...
  if (warp == 0) {
    ...
    asm volatile(
        "tcgen05.alloc.cta_group::2.sync.aligned.shared::cta.b32 [%0], %1;" ::"r"(_tmem_hold),
        "r"(512)
        : "memory");
...
#pragma unroll 1
      for (unsigned int tile_idx = cluster_id; tile_idx < total_tiles; tile_idx += num_clusters) {
        ...
        int causal_row =
            (m_block + stage) * (unsigned int)BLOCK_M + (unsigned int)(warp % 4 * 32 + lane);
...
#pragma unroll 1
        for (unsigned int n_iter = 0; n_iter < num_n_blocks; n_iter++) {
          ...
          _phase_s_full ^= 1;
          int s_addr = taddr + (unsigned int)TMEM_SCORES_OFFSET + stage * 128 +
          ...
          asm volatile(
              ...
              : "=f"(sv[0]), "=f"(sv[1]), "=f"(sv[2]), "=f"(sv[3]), "=f"(sv[4]), "=f"(sv[5]),
                ...
                "=f"(sv[60]), "=f"(sv[61]), "=f"(sv[62]), "=f"(sv[63])
              : "r"(s_addr)
              : "memory");
          asm volatile(
              "tcgen05.ld.sync.aligned.32x32b.x64.b32"
...
#pragma unroll
            ...
            const float2 _fma_c2_11 = {2.584962500721156f - block_max_scaled,
...
#pragma unroll
            ...
            float block_max_scaled_3 =
                ((block_max_2 > -WAN_HYBRID_INF) ? block_max_2 * softmax_scale_log2 : 0.0f);
            ...
            const float2 _fma_c2_13 = {2.584962500721156f - block_max_scaled_3,
                                       2.584962500721156f - block_max_scaled_3};
#pragma unroll
            for (int _lf = 0; _lf < 8; _lf++)
...
#pragma unroll
            ...
            float block_max_scaled_7 =
                ((block_max_6 > -WAN_HYBRID_INF) ? block_max_6 * softmax_scale_log2 : 0.0f);
            ...
            const float2 _fma_c2_15 = {2.584962500721156f - block_max_scaled_7,
                                       2.584962500721156f - block_max_scaled_7};
#pragma unroll
            for (int _lf = 0; _lf < 8; _lf++)
...
#pragma unroll
            ...
            float block_max_scaled_11 =
                ((block_max_10 > -WAN_HYBRID_INF) ? block_max_10 * softmax_scale_log2 : 0.0f);
            ...
            const float2 _fma_c2_17 = {2.584962500721156f - block_max_scaled_11,
                                       2.584962500721156f - block_max_scaled_11};
#pragma unroll
            for (int _lf = 0; _lf < 8; _lf++)
...
#pragma unroll
              for (int owner_col = 0; owner_col < HEAD_DIM / 16; owner_col++) {
                int owner_output_addr = taddr + (unsigned int)owner_output_off +
                                        (unsigned int)(warp % 4 * 32 << 16) +
                                        (unsigned int)(owner_col * 16);
...
#pragma unroll
              ...
              asm volatile(
                  ...
                  " } \n"
                  : "=r"(_fp4_0[1])
                  : "f"(sv[8]), "f"(sv[9]), "f"(sv[10]), "f"(sv[11]), "f"(sv[12]), "f"(sv[13]),
                    "f"(sv[14]), "f"(sv[15]));
              block_sum2 = add_f32x2(block_sum2, raw_sum2);
              asm volatile(
                  "tcgen05.st.sync.aligned.32x32b.x2.b32"
                  " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)TMEM_SCORES_OFFSET + stage * 128 +
                                           (unsigned int)(warp % 4 * 32 << 16)),
...
#pragma unroll
              ...
              asm volatile(
                  ...
                  " } \n"
                  : "=r"(_fp4_4[0])
                  : "f"(sv[16]), "f"(sv[17]), "f"(sv[18]), "f"(sv[19]), "f"(sv[20]), "f"(sv[21]),
                    "f"(sv[22]), "f"(sv[23]));
              asm volatile(
                  " { .reg .b8 __b0, __b1, __b2, __b3; \n"
                  " cvt.rn.satfinite.e2m1x2.f32 __b0, %2, %1; \n"
...
#pragma unroll
              ...
              asm volatile(
                  ...
                  " } \n"
                  : "=r"(_fp4_8[0])
                  : "f"(sv[32]), "f"(sv[33]), "f"(sv[34]), "f"(sv[35]), "f"(sv[36]), "f"(sv[37]),
                    "f"(sv[38]), "f"(sv[39]));
              asm volatile(
                  " { .reg .b8 __b0, __b1, __b2, __b3; \n"
                  " cvt.rn.satfinite.e2m1x2.f32 __b0, %2, %1; \n"
                  ...
                  " } \n"
                  : "=r"(_fp4_8[1])
                  : "f"(sv[40]), "f"(sv[41]), "f"(sv[42]), "f"(sv[43]), "f"(sv[44]), "f"(sv[45]),
                    "f"(sv[46]), "f"(sv[47]));
...
#pragma unroll
              ...
              asm volatile(
                  ...
                  " } \n"
                  : "=r"(_fp4_12[1])
                  : "f"(sv[56]), "f"(sv[57]), "f"(sv[58]), "f"(sv[59]), "f"(sv[60]), "f"(sv[61]),
                    "f"(sv[62]), "f"(sv[63]));
          ...
          int pv_sfa_lo_col =
              ((stage == 0) ? TMEM_TMEM_SFA_PV0_LO_OFFSET : TMEM_TMEM_SFA_PV1_LO_OFFSET) + warp % 4;
          asm volatile(
              "tcgen05.st.sync.aligned.32x32b.x1.b32"
              " [%0], {%1};" ::"r"(taddr + (unsigned int)pv_sfa_lo_col),
          ...
          asm volatile("tcgen05.wait::st.sync.aligned;" ::: "memory");
          asm volatile("tcgen05.fence::before_thread_sync;");
          asm volatile("mbarrier.arrive.release.cta.shared::cluster.b64 _, [%0];" ::"r"(
                           (p_full_addr + (stage) * 8) & 0xFEFFFFFF)
                       : "memory");
...
#pragma unroll
          ...
          float _exp2_9 = approx_exp2(
              ((sv_max_0_2 > -WAN_HYBRID_INF) ? sv_max_0_2 * softmax_scale_log2 : 0.0f) -
...
#pragma unroll
              ...
              float2 _f2_99 = make_float2(((sv_max_0_2 > -WAN_HYBRID_INF) ? _exp2_9 : 0.0f),
                                          ((sv_max_0_2 > -WAN_HYBRID_INF) ? _exp2_9 : 0.0f));
              ...
              asm volatile(
                  ...
                  " } \n"
                  : "=r"(_fp4_16[1])
                  : "f"(sv[72]), "f"(sv[73]), "f"(sv[74]), "f"(sv[75]), "f"(sv[76]), "f"(sv[77]),
                    "f"(sv[78]), "f"(sv[79]));
              block_sum2 = add_f32x2(block_sum2, raw_sum2_4);
              asm volatile(
                  "tcgen05.st.sync.aligned.32x32b.x2.b32"
                  " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)TMEM_SCORES_OFFSET + stage * 128 +
                                           (unsigned int)(warp % 4 * 32 << 16) + 8),
...
#pragma unroll
              ...
              float2 _f2_121 = make_float2(((sv_max_1_1 > -WAN_HYBRID_INF) ? _exp2_10 : 0.0f),
                                           ((sv_max_1_1 > -WAN_HYBRID_INF) ? _exp2_10 : 0.0f));
              ...
              asm volatile(
                  ...
                  " } \n"
                  : "=r"(_fp4_20[0])
                  : "f"(sv[80]), "f"(sv[81]), "f"(sv[82]), "f"(sv[83]), "f"(sv[84]), "f"(sv[85]),
                    "f"(sv[86]), "f"(sv[87]));
              asm volatile(
                  " { .reg .b8 __b0, __b1, __b2, __b3; \n"
                  " cvt.rn.satfinite.e2m1x2.f32 __b0, %2, %1; \n"
...
#pragma unroll
          ...
          float _exp2_11 =
              approx_exp2(((sv_max_2 > -WAN_HYBRID_INF) ? sv_max_2 * softmax_scale_log2 : 0.0f) -
                          new_max_scaled - 2.584962500721156f);
          sf_values[2] = ((sv_max_2 > -WAN_HYBRID_INF) ? _exp2_11 : 0.0f);
          const float2 _fma_b2_23 = {softmax_scale_log2, softmax_scale_log2};
          const float2 _fma_c2_24 = {
              2.584962500721156f -
                  ((sv_max_2 > -WAN_HYBRID_INF) ? sv_max_2 * softmax_scale_log2 : 0.0f),
              2.584962500721156f -
                  ((sv_max_2 > -WAN_HYBRID_INF) ? sv_max_2 * softmax_scale_log2 : 0.0f)};
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
...
#pragma unroll
              ...
              float2 _f2_143 = make_float2(((sv_max_2 > -WAN_HYBRID_INF) ? _exp2_11 : 0.0f),
                                           ((sv_max_2 > -WAN_HYBRID_INF) ? _exp2_11 : 0.0f));
              ...
              asm volatile(
                  ...
                  " } \n"
                  : "=r"(_fp4_24[0])
                  : "f"(sv[96]), "f"(sv[97]), "f"(sv[98]), "f"(sv[99]), "f"(sv[100]), "f"(sv[101]),
                    "f"(sv[102]), "f"(sv[103]));
              asm volatile(
                  " { .reg .b8 __b0, __b1, __b2, __b3; \n"
                  " cvt.rn.satfinite.e2m1x2.f32 __b0, %2, %1; \n"
                  ...
                  " } \n"
                  : "=r"(_fp4_24[1])
                  : "f"(sv[104]), "f"(sv[105]), "f"(sv[106]), "f"(sv[107]), "f"(sv[108]),
                    "f"(sv[109]), "f"(sv[110]), "f"(sv[111]));
              block_sum2 = add_f32x2(block_sum2, raw_sum2_6);
              asm volatile(
                  "tcgen05.st.sync.aligned.32x32b.x2.b32"
                  " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)TMEM_SCORES_OFFSET + stage * 128 +
                                           (unsigned int)(warp % 4 * 32 << 16) + 12),
                  "r"(*reinterpret_cast<const uint32_t*>(&_fp4_24[0])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_fp4_24[1]))
                  : "memory");
            }
          }
...
#pragma unroll
          ...
          float _exp2_12 =
              approx_exp2(((sv_max_3 > -WAN_HYBRID_INF) ? sv_max_3 * softmax_scale_log2 : 0.0f) -
                          new_max_scaled - 2.584962500721156f);
          sf_values[3] = ((sv_max_3 > -WAN_HYBRID_INF) ? _exp2_12 : 0.0f);
          const float2 _fma_b2_25 = {softmax_scale_log2, softmax_scale_log2};
          const float2 _fma_c2_26 = {
              2.584962500721156f -
                  ((sv_max_3 > -WAN_HYBRID_INF) ? sv_max_3 * softmax_scale_log2 : 0.0f),
              2.584962500721156f -
                  ((sv_max_3 > -WAN_HYBRID_INF) ? sv_max_3 * softmax_scale_log2 : 0.0f)};
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
...
#pragma unroll
              ...
              float2 _f2_165 = make_float2(((sv_max_3 > -WAN_HYBRID_INF) ? _exp2_12 : 0.0f),
                                           ((sv_max_3 > -WAN_HYBRID_INF) ? _exp2_12 : 0.0f));
              ...
              asm volatile(
                  ...
                  " } \n"
                  : "=r"(_fp4_28[0])
                  : "f"(sv[112]), "f"(sv[113]), "f"(sv[114]), "f"(sv[115]), "f"(sv[116]),
                    "f"(sv[117]), "f"(sv[118]), "f"(sv[119]));
              asm volatile(
                  " { .reg .b8 __b0, __b1, __b2, __b3; \n"
                  " cvt.rn.satfinite.e2m1x2.f32 __b0, %2, %1; \n"
                  " cvt.rn.satfinite.e2m1x2.f32 __b1, %4, %3; \n"
              ...
              asm volatile(
                  "tcgen05.st.sync.aligned.32x32b.x2.b32"
                  " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)TMEM_SCORES_OFFSET + stage * 128 +
                                           (unsigned int)(warp % 4 * 32 << 16) + 14),
                  "r"(*reinterpret_cast<const uint32_t*>(&_fp4_28[0])),
                  "r"(*reinterpret_cast<const uint32_t*>(&_fp4_28[1]))
                  : "memory");
            }
          }
          uint32_t _fp8_2[1];
          ...
          int pv_sfa_hi_col =
              ((stage == 0) ? TMEM_TMEM_SFA_PV0_HI_OFFSET : TMEM_TMEM_SFA_PV1_HI_OFFSET) + warp % 4;
          asm volatile(
              "tcgen05.st.sync.aligned.32x32b.x1.b32"
              " [%0], {%1};" ::"r"(taddr + (unsigned int)pv_sfa_hi_col),
          ...
          asm volatile("tcgen05.fence::before_thread_sync;");
          asm volatile("mbarrier.arrive.release.cta.shared::cluster.b64 _, [%0];" ::"r"(
                           (p_full_2_addr + (stage) * 8) & 0xFEFFFFFF)
                       : "memory");
...
#pragma unroll
          for (int col = 0; col < HEAD_DIM / 32; col++) {
            int addr = taddr + (unsigned int)TMEM_OUTPUT_0_OFFSET +
                       (unsigned int)(warp % 4 * 32 << 16) + (unsigned int)(col * 16);
...
#pragma unroll
            ...
            asm volatile(
                "st.shared.v4.b32 [%0], {%1,%2,%3,%4};" ::"r"(
                    (smem_o00_addr +
                     (unsigned int)((warp % 4 * 32 + lane) * 128 + col * 32 ^
                                    ((warp % 4 * 32 + lane) * 128 + col * 32 >> 7 & 7) << 4))),
                "r"(_tmem_load_1_bf16[0]), "r"(_tmem_load_1_bf16[1]), "r"(_tmem_load_1_bf16[2]),
                "r"(_tmem_load_1_bf16[3])
                : "memory");
            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" ::"r"(
                             (smem_o00_addr +
                              (unsigned int)((warp % 4 * 32 + lane) * 128 + (col * 32 + 16) ^
                                             ((warp % 4 * 32 + lane) * 128 + (col * 32 + 16) >> 7 &
                                              7) << 4))),
                         "r"(_tmem_load_1_bf16[4]), "r"(_tmem_load_1_bf16[5]),
                         "r"(_tmem_load_1_bf16[6]), "r"(_tmem_load_1_bf16[7])
                         : "memory");
          }
#pragma unroll
          for (int col_1 = 0; col_1 < HEAD_DIM / 32; col_1++) {
            int addr_1 = taddr + (unsigned int)TMEM_OUTPUT_0_OFFSET +
                         (unsigned int)(warp % 4 * 32 << 16) + (unsigned int)(HEAD_DIM / 2) +
                         (unsigned int)(col_1 * 16);
...
#pragma unroll
            ...
            asm volatile(
                "st.shared.v4.b32 [%0], {%1,%2,%3,%4};" ::"r"(
                    (smem_o01_addr +
                     (unsigned int)((warp % 4 * 32 + lane) * 128 + col_1 * 32 ^
                                    ((warp % 4 * 32 + lane) * 128 + col_1 * 32 >> 7 & 7) << 4))),
                "r"(_tmem_load_2_bf16[0]), "r"(_tmem_load_2_bf16[1]), "r"(_tmem_load_2_bf16[2]),
                "r"(_tmem_load_2_bf16[3])
                : "memory");
            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" ::"r"((
                             smem_o01_addr +
                             (unsigned int)((warp % 4 * 32 + lane) * 128 + (col_1 * 32 + 16) ^
                                            ((warp % 4 * 32 + lane) * 128 + (col_1 * 32 + 16) >> 7 &
                                             7) << 4))),
                         "r"(_tmem_load_2_bf16[4]), "r"(_tmem_load_2_bf16[5]),
                         "r"(_tmem_load_2_bf16[6]), "r"(_tmem_load_2_bf16[7])
                         : "memory");
          }
...
#pragma unroll
          for (int col_2 = 0; col_2 < HEAD_DIM / 32; col_2++) {
            int addr_2 = taddr + (unsigned int)TMEM_OUTPUT_1_OFFSET +
                         (unsigned int)(warp % 4 * 32 << 16) + (unsigned int)(col_2 * 16);
...
#pragma unroll
            ...
            asm volatile(
                "st.shared.v4.b32 [%0], {%1,%2,%3,%4};" ::"r"(
                    (smem_o10_addr +
                     (unsigned int)((warp % 4 * 32 + lane) * 128 + col_2 * 32 ^
                                    ((warp % 4 * 32 + lane) * 128 + col_2 * 32 >> 7 & 7) << 4))),
                "r"(_tmem_load_3_bf16[0]), "r"(_tmem_load_3_bf16[1]), "r"(_tmem_load_3_bf16[2]),
                "r"(_tmem_load_3_bf16[3])
                : "memory");
            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" ::"r"((
                             smem_o10_addr +
                             (unsigned int)((warp % 4 * 32 + lane) * 128 + (col_2 * 32 + 16) ^
                                            ((warp % 4 * 32 + lane) * 128 + (col_2 * 32 + 16) >> 7 &
                                             7) << 4))),
                         "r"(_tmem_load_3_bf16[4]), "r"(_tmem_load_3_bf16[5]),
                         "r"(_tmem_load_3_bf16[6]), "r"(_tmem_load_3_bf16[7])
                         : "memory");
          }
#pragma unroll
          for (int col_3 = 0; col_3 < HEAD_DIM / 32; col_3++) {
            int addr_3 = taddr + (unsigned int)TMEM_OUTPUT_1_OFFSET +
                         (unsigned int)(warp % 4 * 32 << 16) + (unsigned int)(HEAD_DIM / 2) +
                         (unsigned int)(col_3 * 16);
...
#pragma unroll
            ...
            asm volatile(
                "st.shared.v4.b32 [%0], {%1,%2,%3,%4};" ::"r"(
                    (smem_o11_addr +
                     (unsigned int)((warp % 4 * 32 + lane) * 128 + col_3 * 32 ^
                                    ((warp % 4 * 32 + lane) * 128 + col_3 * 32 >> 7 & 7) << 4))),
                "r"(_tmem_load_4_bf16[0]), "r"(_tmem_load_4_bf16[1]), "r"(_tmem_load_4_bf16[2]),
                "r"(_tmem_load_4_bf16[3])
                : "memory");
            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" ::"r"((
                             smem_o11_addr +
                             (unsigned int)((warp % 4 * 32 + lane) * 128 + (col_3 * 32 + 16) ^
                                            ((warp % 4 * 32 + lane) * 128 + (col_3 * 32 + 16) >> 7 &
                                             7) << 4))),
                         "r"(_tmem_load_4_bf16[4]), "r"(_tmem_load_4_bf16[5]),
                         "r"(_tmem_load_4_bf16[6]), "r"(_tmem_load_4_bf16[7])
                         : "memory");
          }
          asm volatile("fence.proxy.async;");
        ...
        asm volatile("mbarrier.arrive.release.cta.shared::cluster.b64 _, [%0];" ::"r"(
                         (o_empty_addr + (stage) * 8) & 0xFEFFFFFF)
                     : "memory");
...
#pragma unroll 1
      for (unsigned int tile_idx_1 = cluster_id; tile_idx_1 < correction_total_tiles;
           tile_idx_1 += num_clusters) {
...
#pragma unroll 1
        for (unsigned int tile_idx_2 = cluster_id; tile_idx_2 < total_tiles_2;
             tile_idx_2 += num_clusters) {
          ...
          asm volatile(
              ...
              "mov.b32 m5, 0; mov.b32 m6, 0; mov.b32 m7, 0;\n\t"
              ...
              "mov.b64 da, {alo, adhi};\n\t"
              "mov.b64 db, {blo, bdhi};\n\t"
              "@leader tcgen05.mma.cta_group::2.kind::f16 [%2], da, db, id, {m0, m1, m2, m3, m4, "
              "m5, m6, m7}, p0;\n\t"
              "add.u32 alo, alo, 2;\n\t"
              "add.u32 blo, blo, 2;\n\t"
              "mov.b64 da, {alo, adhi};\n\t"
          ...
          asm volatile(
              ...
              "m5, m6, m7}, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_1),
              "r"(_mma_b_lo_1), "r"((tmem_scores + (128))), "r"(0));
...
#pragma unroll 1
          for (unsigned int n_iter_2 = 0; n_iter_2 < num_n_blocks_2 - 1; n_iter_2++) {
            ...
            mbarrier_wait(k_full_addr + (k_stage) * 8, k_phase);
            {
              ...
              _phase_p_full_0 ^= 1;
              if (elect_sync()) {
                tcgen05_cp_32x128b_warpx4_cta2(
                    tmem_tmem_sfb_pv_lo,
                    make_sf_cp_desc_sbo128(smem_sfvt_lo_addr + v_stage * 1024));
              }
              if (elect_sync()) {
                tcgen05_cp_32x128b_warpx4_cta2(
                    tmem_tmem_sfa_pv0_res_lo,
                    make_sf_cp_desc_sbo128(smem_sfvt_residual_lo_addr + v_stage * 1024));
              }
              if (elect_sync()) {
                tcgen05_cp_32x128b_warpx4_cta2(
                    tmem_tmem_sfb_pv_hi,
                    make_sf_cp_desc_sbo128(smem_sfvt_hi_addr + v_stage * 1024));
              }
              if (elect_sync()) {
                tcgen05_cp_32x128b_warpx4_cta2(
                    tmem_tmem_sfa_pv0_res_hi,
                    make_sf_cp_desc_sbo128(smem_sfvt_residual_hi_addr + v_stage * 1024));
              }
              asm volatile("tcgen05.fence::after_thread_sync;");
              ...
              if (elect_sync()) {
                {
                  ...
                  tcgen05_mma_mxf4nvf4_bs_ts_cta2(
                      tmem_output_0, tmem_scores + 0, b_desc + 0, 0x10200480U,
                      tmem_tmem_sfa_pv0_lo + 0, tmem_tmem_sfb_pv_lo + 0, ((first_pv_flag) ? 0 : 1));
                }
              }
              ...
              if (elect_sync()) {
                {
                  ...
                  tcgen05_mma_mxf4nvf4_bs_ts_cta2(tmem_output_0, tmem_scores + 0, b_desc + 0,
                                                  0x10200480U, tmem_tmem_sfa_pv0_lo + 0,
                                                  tmem_tmem_sfa_pv0_res_lo + 0, 1);
                }
              }
              ...
              if (elect_sync()) {
                {
                  ...
                  tcgen05_mma_mxf4nvf4_bs_ts_cta2(tmem_output_0, tmem_scores + 8 + 0, b_desc + 0,
                                                  0x10200480U, tmem_tmem_sfa_pv0_hi + 0,
                                                  tmem_tmem_sfb_pv_hi + 0, 1);
                }
              }
              ...
              if (elect_sync()) {
                {
                  ...
                  tcgen05_mma_mxf4nvf4_bs_ts_cta2(
                      tmem_output_1, tmem_scores + 128 + 0, b_desc + 0, 0x10200480U,
                      tmem_tmem_sfa_pv1_lo + 0, tmem_tmem_sfb_pv_lo + 0, ((first_pv_flag) ? 0 : 1));
                }
              }
              ...
              if (elect_sync()) {
                {
                  ...
                  tcgen05_mma_mxf4nvf4_bs_ts_cta2(tmem_output_1, tmem_scores + 128 + 0, b_desc + 0,
                                                  0x10200480U, tmem_tmem_sfa_pv1_lo + 0,
                                                  tmem_tmem_sfa_pv0_res_lo + 0, 1);
                }
              }
              ...
              if (elect_sync()) {
                {
                  ...
                  tcgen05_mma_mxf4nvf4_bs_ts_cta2(tmem_output_1, tmem_scores + 136 + 0, b_desc + 0,
                                                  0x10200480U, tmem_tmem_sfa_pv1_hi + 0,
                                                  tmem_tmem_sfb_pv_hi + 0, 1);
                }
              }
              ...
              asm volatile(
                  ...
                  "mov.b64 da, {alo, adhi};\n\t"
                  "mov.b64 db, {blo, bdhi};\n\t"
                  "@leader tcgen05.mma.cta_group::2.kind::f16 [%2], da, db, id, {m0, m1, m2, m3, "
                  "m4, m5, m6, m7}, p0;\n\t"
                  "add.u32 alo, alo, 2;\n\t"
                  "add.u32 blo, blo, 2;\n\t"
                  "mov.b64 da, {alo, adhi};\n\t"
                  ...
                  "m4, m5, m6, m7}, p1;\n\t"
                  "}\n" ::"r"(_mma_a_lo_10),
                  "r"(_mma_b_lo_10), "r"(tmem_scores), "r"(0));
              elect_commit_cg2_multicast(s_full_addr, (uint16_t)(3));
              ...
              asm volatile(
                  ...
                  "m4, m5, m6, m7}, p1;\n\t"
                  "}\n" ::"r"(_mma_a_lo_11),
                  "r"(_mma_b_lo_11), "r"((tmem_scores + (128))), "r"(0));
              elect_commit_cg2_multicast(s_full_addr + 8, (uint16_t)(3));
              elect_commit_cg2_multicast(k_empty_addr + (k_stage) * 8, (uint16_t)(3));
              k_stage += 1;
              if (k_stage == 2) {
          ...
          if (elect_sync()) {
            tcgen05_cp_32x128b_warpx4_cta2(
                tmem_tmem_sfb_pv_lo, make_sf_cp_desc_sbo128(smem_sfvt_lo_addr + v_stage * 1024));
          }
          if (elect_sync()) {
            tcgen05_cp_32x128b_warpx4_cta2(
                tmem_tmem_sfa_pv0_res_lo,
          ...
          if (elect_sync()) {
            tcgen05_cp_32x128b_warpx4_cta2(
                tmem_tmem_sfb_pv_hi, make_sf_cp_desc_sbo128(smem_sfvt_hi_addr + v_stage * 1024));
          }
          if (elect_sync()) {
            tcgen05_cp_32x128b_warpx4_cta2(
                tmem_tmem_sfa_pv0_res_hi,
          ...
          if (elect_sync()) {
            {
              ...
              tcgen05_mma_mxf4nvf4_bs_ts_cta2(tmem_output_0, tmem_scores + 0, b_desc + 0,
                                              0x10200480U, tmem_tmem_sfa_pv0_lo + 0,
                                              tmem_tmem_sfb_pv_lo + 0, ((first_pv_flag_1) ? 0 : 1));
            }
          }
          {
...
#pragma unroll 1
      for (unsigned int tile_idx_3 = cluster_id; tile_idx_3 < total_tiles_3;
           tile_idx_3 += num_clusters) {
        ...
        if (elect_sync()) {
          tma_5d_gmem2smem_cta2(smem_q_addr, Q, 0, q_row, head_1, batch_idx_1, 0,
                                ((q_full_addr) & 0xFEFFFFFF));
          tma_5d_gmem2smem_cta2(smem_q_addr + 32768, Q, 0, q_row + BLOCK_M, head_1, batch_idx_1, 0,
                                ((q_full_addr + 8) & 0xFEFFFFFF));
          asm volatile(
              "mbarrier.arrive.expect_tx.release.cta.shared::cluster.b64 _, [%0], %1;" ::"r"(
                  (q_full_addr) & 0xFEFFFFFF),
              "r"((uint32_t)(32768))
              : "memory");
          asm volatile(
...
#pragma unroll 1
        for (unsigned int ni = 0; ni < num_n_blocks_3; ni++) {
          ...
          if (elect_sync()) {
            tma_5d_gmem2smem_cta2(smem_k_addr + k_load_stage * 16384, K, 0,
                                  n * (unsigned int)BLOCK_N + (unsigned int)(cta_rank * 64), head_1,
                                  batch_idx_1, 0,
                                  ((k_full_addr + (k_load_stage) * 8) & 0xFEFFFFFF));
            asm volatile(
                "mbarrier.arrive.expect_tx.release.cta.shared::cluster.b64 _, [%0], %1;" ::"r"(
                    (k_full_addr + (k_load_stage) * 8) & 0xFEFFFFFF),
                "r"((uint32_t)(16384))
          ...
          if (elect_sync()) {
            tma_2d_gmem2smem_cta2(smem_vt_addr + v_load_stage * 4096, Vt, vt_col,
                                  vt_row + cta_rank * 64,
                                  ((v_full_addr + (v_load_stage) * 8) & 0xFEFFFFFF));
            {
              tma_2d_gmem2smem_cta2(
                  smem_vt_residual_addr + v_load_stage * 4096, Vt, vt_col,
                  v_level_row_stride + (unsigned int)vt_row + (unsigned int)(cta_rank * 64),
                  ((v_full_addr + (v_load_stage) * 8) & 0xFEFFFFFF));
            }
            tma_2d_gmem2smem_cta2(smem_sfvt_lo_addr + v_load_stage * 1024, SFVtLo, 0,
                                  kv_sf_tile * 16,
                                  ((v_full_addr + (v_load_stage) * 8) & 0xFEFFFFFF));
            {
              tma_2d_gmem2smem_cta2(smem_sfvt_residual_lo_addr + v_load_stage * 1024, SFVtLo, 0,
                                    v_scale_level_stride + (unsigned int)(kv_sf_tile * 16),
                                    ((v_full_addr + (v_load_stage) * 8) & 0xFEFFFFFF));
            }
            tma_2d_gmem2smem_cta2(smem_sfvt_hi_addr + v_load_stage * 1024, SFVtHi, 0,
                                  kv_sf_tile * 16,
                                  ((v_full_addr + (v_load_stage) * 8) & 0xFEFFFFFF));
            {
              tma_2d_gmem2smem_cta2(smem_sfvt_residual_hi_addr + v_load_stage * 1024, SFVtHi, 0,
                                    v_scale_level_stride + (unsigned int)(kv_sf_tile * 16),
                                    ((v_full_addr + (v_load_stage) * 8) & 0xFEFFFFFF));
            }
            asm volatile(
  ...
  if (warp == 0) {
    asm volatile(
        "tcgen05.dealloc.cta_group::2.sync.aligned.b32 %0, %1;" ::"r"(tmem_addr_storage[0]),
        "r"(512));
  }
}
