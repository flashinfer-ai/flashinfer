...
struct __align__(128) WanHybridTensorMap {
  uint64_t opaque[16];
};
template <int N>
struct __align__(128) WanHybridTensorMapPack {
  WanHybridTensorMap maps[N];
};
...
__device__ __forceinline__ void tcgen05_mma_mxf4nvf4_bs(int taddr, uint64_t a_desc, uint64_t b_desc,
                                                        uint32_t i_desc, int sfa_taddr,
                                                        int sfb_taddr, int enable_input_d) {
  asm volatile(
      "{\n\t"
      ".reg .pred p;\n\t"
      "setp.ne.b32 p, %6, 0;\n\t"
...
__device__ __forceinline__ void tcgen05_mma_mxf4nvf4_bs_ts(int taddr, int a_taddr, uint64_t b_desc,
                                                           uint32_t i_desc, int sfa_taddr,
                                                           int sfb_taddr, int enable_input_d) {
  asm volatile(
      "{\n\t"
      ...
      " [%0], [%1], %2, %3, [%4], [%5], p;\n\t"
      "}\n" ::"r"(taddr),
      "r"(a_taddr), "l"(b_desc), "r"(i_desc), "r"(sfa_taddr), "r"(sfb_taddr), "r"(enable_input_d));
}
...
__global__ __launch_bounds__(512, 1) void kernel_wan_hybrid_attention(
    WanHybridTensorMap const* Q, WanHybridTensorMap const* K, WanHybridTensorMap const* Vt,
    WanHybridTensorMap const* SFVtLo, WanHybridTensorMap const* SFVtHi, WanHybridTensorMap const* O,
    int seqlen_q, int seqlen_kv, float softmax_scale_log2, int heads, int total_bh,
    int physical_num_blocks) {
  ...
  if (tid == 0) {
    ...
    asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" ::"l"((uint64_t)(K))
                 : "memory");
    asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" ::"l"((uint64_t)(Vt))
                 : "memory");
    asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" ::"l"((uint64_t)(SFVtLo))
                 : "memory");
    asm volatile("fence.proxy.tensormap::generic.acquire.sys [%0], 128;" ::"l"((uint64_t)(SFVtHi))
                 : "memory");
  ...
  if (warp == 0) {
    asm volatile("prefetch.tensormap [%0];" ::"l"((uint64_t)(SFVtLo)) : "memory");
  }
  if (warp == 0) {
    asm volatile("prefetch.tensormap [%0];" ::"l"((uint64_t)(SFVtHi)) : "memory");
  }
  if (warp == 0) {
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_6)
                     : "f"(max9ab), "f"(maxcde), "f"(_tmem_load_0[15]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_8)
                     : "f"(_tmem_load_0[19]), "f"(_tmem_load_0[20]), "f"(_tmem_load_0[21]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_9)
                     : "f"(_tmem_load_0[22]), "f"(_tmem_load_0[23]), "f"(_tmem_load_0[24]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_10)
                     : "f"(_tmem_load_0[25]), "f"(_tmem_load_0[26]), "f"(_tmem_load_0[27]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_12)
                     : "f"(max012_1), "f"(max345_2), "f"(max678_3));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_13)
                     : "f"(max9ab_4), "f"(maxcde_5), "f"(_tmem_load_0[31]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_19)
                     : "f"(max012_9), "f"(max345_10), "f"(max678_11));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_20)
                     : "f"(max9ab_12), "f"(maxcde_13), "f"(_tmem_load_0[47]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_26)
                     : "f"(max012_17), "f"(max345_18), "f"(max678_19));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_27)
                     : "f"(max9ab_20), "f"(maxcde_21), "f"(_tmem_load_0[63]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_33)
                     : "f"(max012_25), "f"(max345_26), "f"(max678_27));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_34)
                     : "f"(max9ab_28), "f"(maxcde_29), "f"(_tmem_load_0[79]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_36)
                     : "f"(_tmem_load_0[83]), "f"(_tmem_load_0[84]), "f"(_tmem_load_0[85]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_37)
                     : "f"(_tmem_load_0[86]), "f"(_tmem_load_0[87]), "f"(_tmem_load_0[88]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_38)
                     : "f"(_tmem_load_0[89]), "f"(_tmem_load_0[90]), "f"(_tmem_load_0[91]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_40)
                     : "f"(max012_33), "f"(max345_34), "f"(max678_35));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_41)
                     : "f"(max9ab_36), "f"(maxcde_37), "f"(_tmem_load_0[95]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_47)
                     : "f"(max012_41), "f"(max345_42), "f"(max678_43));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_48)
                     : "f"(max9ab_44), "f"(maxcde_45), "f"(_tmem_load_0[111]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_50)
                     : "f"(_tmem_load_0[115]), "f"(_tmem_load_0[116]), "f"(_tmem_load_0[117]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_51)
                     : "f"(_tmem_load_0[118]), "f"(_tmem_load_0[119]), "f"(_tmem_load_0[120]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_52)
                     : "f"(_tmem_load_0[121]), "f"(_tmem_load_0[122]), "f"(_tmem_load_0[123]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_54)
                     : "f"(max012_49), "f"(max345_50), "f"(max678_51));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_55)
                     : "f"(max9ab_52), "f"(maxcde_53), "f"(_tmem_load_0[127]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_56)
                     : "f"(group_max0_0), "f"(group_max1_8), "f"(group_max2_16));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_57)
                     : "f"(group_max3_24), "f"(group_max4_32), "f"(group_max5_40));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
        ...
        asm volatile("max.f32 %0, %1, %2, %3;"
                     : "=f"(_max3_58)
                     : "f"(max012_57), "f"(max345_58), "f"(max67));
        new_max = _max3_58;
        ...
        float block_max_scaled =
            ((block_max > -WAN_HYBRID_INF) ? block_max * softmax_scale_log2 : 0.0f);
        ...
        const float2 _fma_c2_9 = {2.584962500721156f - block_max_scaled,
                                  2.584962500721156f - block_max_scaled};
#pragma unroll
        for (int _lf = 0; _lf < 8; _lf++)
          fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_0 + 0))[_lf], _fma_b2_8,
                            _fma_c2_9);
#pragma unroll
...
#pragma unroll
        ...
        asm volatile(
            ...
            : "f"(_tmem_load_0[8]), "f"(_tmem_load_0[9]), "f"(_tmem_load_0[10]),
              "f"(_tmem_load_0[11]), "f"(_tmem_load_0[12]), "f"(_tmem_load_0[13]),
              "f"(_tmem_load_0[14]), "f"(_tmem_load_0[15]));
        block_sum2 = add_f32x2(block_sum2, raw_sum2);
        asm volatile(
            "tcgen05.st.sync.aligned.32x32b.x2.b32"
            " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_stage_off + 64 +
                                     (unsigned int)(warp % 4 * 32 << 16)),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp4_0[0])),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp4_0[1]))
            : "memory");
        float block_max_59 = group_max1;
        float block_max_scaled_60 =
            ((block_max_59 > -WAN_HYBRID_INF) ? block_max_59 * softmax_scale_log2 : 0.0f);
        ...
        const float2 _fma_c2_11 = {2.584962500721156f - block_max_scaled_60,
                                   2.584962500721156f - block_max_scaled_60};
#pragma unroll
        for (int _lf = 0; _lf < 8; _lf++)
          fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_0 + 16))[_lf], _fma_b2_10,
                            _fma_c2_11);
#pragma unroll
        for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
        ...
        asm volatile(
            "tcgen05.st.sync.aligned.32x32b.x2.b32"
            " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_stage_off + 64 +
                                     (unsigned int)(warp % 4 * 32 << 16) + 2),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp4_1[0])),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp4_1[1]))
            : "memory");
        float block_max_66 = group_max2;
        float block_max_scaled_67 =
            ((block_max_66 > -WAN_HYBRID_INF) ? block_max_66 * softmax_scale_log2 : 0.0f);
        ...
        const float2 _fma_c2_13 = {2.584962500721156f - block_max_scaled_67,
                                   2.584962500721156f - block_max_scaled_67};
#pragma unroll
        for (int _lf = 0; _lf < 8; _lf++)
          fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_0 + 32))[_lf], _fma_b2_12,
                            _fma_c2_13);
#pragma unroll
        for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
        ...
        asm volatile(
            ...
            : "f"(_tmem_load_0[40]), "f"(_tmem_load_0[41]), "f"(_tmem_load_0[42]),
              "f"(_tmem_load_0[43]), "f"(_tmem_load_0[44]), "f"(_tmem_load_0[45]),
              "f"(_tmem_load_0[46]), "f"(_tmem_load_0[47]));
        block_sum2 = add_f32x2(block_sum2, raw_sum2_72);
        asm volatile(
            "tcgen05.st.sync.aligned.32x32b.x2.b32"
            " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_stage_off + 64 +
                                     (unsigned int)(warp % 4 * 32 << 16) + 4),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp4_2[0])),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp4_2[1]))
            : "memory");
        float block_max_73 = group_max3;
        float block_max_scaled_74 =
            ((block_max_73 > -WAN_HYBRID_INF) ? block_max_73 * softmax_scale_log2 : 0.0f);
        ...
        const float2 _fma_c2_15 = {2.584962500721156f - block_max_scaled_74,
                                   2.584962500721156f - block_max_scaled_74};
#pragma unroll
        for (int _lf = 0; _lf < 8; _lf++)
          fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_0 + 48))[_lf], _fma_b2_14,
                            _fma_c2_15);
#pragma unroll
        for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
        ...
        asm volatile(
            "tcgen05.st.sync.aligned.32x32b.x2.b32"
            " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_stage_off + 64 +
                                     (unsigned int)(warp % 4 * 32 << 16) + 6),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp4_3[0])),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp4_3[1]))
            : "memory");
        uint32_t _fp8_0[1];
        {
          uint32_t _packed;
          asm volatile(
              ...
              "}"
              : "=r"(_packed)
              : "f"(sf_values[0]), "f"(sf_values[1]), "f"(sf_values[2]), "f"(sf_values[3]));
          _fp8_0[0] = _packed;
        }
        asm volatile(
            "tcgen05.st.sync.aligned.32x32b.x1.b32"
            " [%0], {%1};" ::"r"(taddr + (unsigned int)p_stage_off + 80 + (unsigned int)(warp % 4)),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp8_0[0]))
            : "memory");
        ...
        float _exp2_4 =
            approx_exp2(((group_max4 > -WAN_HYBRID_INF) ? group_max4 * softmax_scale_log2 : 0.0f) -
                        new_max_scaled - 2.584962500721156f);
        sf_values[0] = ((group_max4 > -WAN_HYBRID_INF) ? _exp2_4 : 0.0f);
        const float2 _fma_b2_16 = {softmax_scale_log2, softmax_scale_log2};
        const float2 _fma_c2_17 = {
            2.584962500721156f -
                ((group_max4 > -WAN_HYBRID_INF) ? group_max4 * softmax_scale_log2 : 0.0f),
            2.584962500721156f -
                ((group_max4 > -WAN_HYBRID_INF) ? group_max4 * softmax_scale_log2 : 0.0f)};
#pragma unroll
        for (int _lf = 0; _lf < 8; _lf++)
          fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_0 + 64))[_lf], _fma_b2_16,
                            _fma_c2_17);
#pragma unroll
        for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
        ...
        float2 _f2_15 = make_float2(((group_max4 > -WAN_HYBRID_INF) ? _exp2_4 : 0.0f),
                                    ((group_max4 > -WAN_HYBRID_INF) ? _exp2_4 : 0.0f));
        ...
        asm volatile(
            ...
            : "f"(_tmem_load_0[72]), "f"(_tmem_load_0[73]), "f"(_tmem_load_0[74]),
              "f"(_tmem_load_0[75]), "f"(_tmem_load_0[76]), "f"(_tmem_load_0[77]),
              "f"(_tmem_load_0[78]), "f"(_tmem_load_0[79]));
        block_sum2 = add_f32x2(block_sum2, raw_sum2_83);
        asm volatile(
            "tcgen05.st.sync.aligned.32x32b.x2.b32"
            " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_hi_off +
                                     (unsigned int)(warp % 4 * 32 << 16)),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp4_4[0])),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp4_4[1]))
            : "memory");
        float _exp2_5 =
            approx_exp2(((group_max5 > -WAN_HYBRID_INF) ? group_max5 * softmax_scale_log2 : 0.0f) -
                        new_max_scaled - 2.584962500721156f);
        sf_values[1] = ((group_max5 > -WAN_HYBRID_INF) ? _exp2_5 : 0.0f);
        const float2 _fma_b2_18 = {softmax_scale_log2, softmax_scale_log2};
        const float2 _fma_c2_19 = {
            2.584962500721156f -
                ((group_max5 > -WAN_HYBRID_INF) ? group_max5 * softmax_scale_log2 : 0.0f),
            2.584962500721156f -
                ((group_max5 > -WAN_HYBRID_INF) ? group_max5 * softmax_scale_log2 : 0.0f)};
#pragma unroll
        for (int _lf = 0; _lf < 8; _lf++)
          fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_0 + 80))[_lf], _fma_b2_18,
                            _fma_c2_19);
#pragma unroll
        for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
        ...
        float2 _f2_18 = make_float2(((group_max5 > -WAN_HYBRID_INF) ? _exp2_5 : 0.0f),
                                    ((group_max5 > -WAN_HYBRID_INF) ? _exp2_5 : 0.0f));
        ...
        asm volatile(
            ...
            " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_hi_off +
                                     (unsigned int)(warp % 4 * 32 << 16) + 2),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp4_5[0])),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp4_5[1]))
            : "memory");
        float _exp2_6 =
            approx_exp2(((group_max6 > -WAN_HYBRID_INF) ? group_max6 * softmax_scale_log2 : 0.0f) -
                        new_max_scaled - 2.584962500721156f);
        sf_values[2] = ((group_max6 > -WAN_HYBRID_INF) ? _exp2_6 : 0.0f);
        const float2 _fma_b2_20 = {softmax_scale_log2, softmax_scale_log2};
        const float2 _fma_c2_21 = {
            2.584962500721156f -
                ((group_max6 > -WAN_HYBRID_INF) ? group_max6 * softmax_scale_log2 : 0.0f),
            2.584962500721156f -
                ((group_max6 > -WAN_HYBRID_INF) ? group_max6 * softmax_scale_log2 : 0.0f)};
#pragma unroll
        for (int _lf = 0; _lf < 8; _lf++)
          fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_0 + 96))[_lf], _fma_b2_20,
                            _fma_c2_21);
#pragma unroll
        for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
        ...
        float2 _f2_21 = make_float2(((group_max6 > -WAN_HYBRID_INF) ? _exp2_6 : 0.0f),
                                    ((group_max6 > -WAN_HYBRID_INF) ? _exp2_6 : 0.0f));
        ...
        asm volatile(
            ...
            : "f"(_tmem_load_0[104]), "f"(_tmem_load_0[105]), "f"(_tmem_load_0[106]),
              "f"(_tmem_load_0[107]), "f"(_tmem_load_0[108]), "f"(_tmem_load_0[109]),
        ...
        asm volatile(
            ...
            " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_hi_off +
                                     (unsigned int)(warp % 4 * 32 << 16) + 4),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp4_6[0])),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp4_6[1]))
            : "memory");
        float _exp2_7 =
            approx_exp2(((group_max7 > -WAN_HYBRID_INF) ? group_max7 * softmax_scale_log2 : 0.0f) -
                        new_max_scaled - 2.584962500721156f);
        sf_values[3] = ((group_max7 > -WAN_HYBRID_INF) ? _exp2_7 : 0.0f);
        const float2 _fma_b2_22 = {softmax_scale_log2, softmax_scale_log2};
        const float2 _fma_c2_23 = {
            2.584962500721156f -
                ((group_max7 > -WAN_HYBRID_INF) ? group_max7 * softmax_scale_log2 : 0.0f),
            2.584962500721156f -
                ((group_max7 > -WAN_HYBRID_INF) ? group_max7 * softmax_scale_log2 : 0.0f)};
#pragma unroll
        for (int _lf = 0; _lf < 8; _lf++)
          fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_0 + 112))[_lf], _fma_b2_22,
                            _fma_c2_23);
#pragma unroll
        for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
        for (int pair_7 = 2; pair_7 < 16; pair_7 += 2) {
          float2 _f2_23 =
              make_float2((_tmem_load_0 + 112)[pair_7], (_tmem_load_0 + 112)[pair_7 + 1]);
          partial_92 = add_f32x2(partial_92, _f2_23);
        }
        float2 frag_sum2_93 = partial_92;
        float2 _f2_24 = make_float2(((group_max7 > -WAN_HYBRID_INF) ? _exp2_7 : 0.0f),
                                    ((group_max7 > -WAN_HYBRID_INF) ? _exp2_7 : 0.0f));
        ...
        asm volatile(
            ...
            " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_hi_off +
                                     (unsigned int)(warp % 4 * 32 << 16) + 6),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp4_7[0])),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp4_7[1]))
            : "memory");
        uint32_t _fp8_1[1];
        {
          uint32_t _packed;
        ...
        asm volatile(
            "tcgen05.st.sync.aligned.32x32b.x1.b32"
            " [%0], {%1};" ::"r"(taddr + (unsigned int)sfa_hi_off + (unsigned int)(warp % 4)),
            "r"(*reinterpret_cast<const uint32_t*>(&_fp8_1[0]))
            : "memory");
        asm volatile("tcgen05.wait::st.sync.aligned;" ::: "memory");
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_64)
                       : "f"(max012_2), "f"(max345_3), "f"(max678_4));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_65)
                       : "f"(max9ab_5), "f"(maxcde_6), "f"(_tmem_load_1[15]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_67)
                       : "f"(_tmem_load_1[19]), "f"(_tmem_load_1[20]), "f"(_tmem_load_1[21]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_68)
                       : "f"(_tmem_load_1[22]), "f"(_tmem_load_1[23]), "f"(_tmem_load_1[24]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_69)
                       : "f"(_tmem_load_1[25]), "f"(_tmem_load_1[26]), "f"(_tmem_load_1[27]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_71)
                       : "f"(max012_10), "f"(max345_11), "f"(max678_12));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_72)
                       : "f"(max9ab_13), "f"(maxcde_14), "f"(_tmem_load_1[31]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_78)
                       : "f"(max012_18), "f"(max345_19), "f"(max678_20));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_79)
                       : "f"(max9ab_21), "f"(maxcde_22), "f"(_tmem_load_1[47]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_85)
                       : "f"(max012_26), "f"(max345_27), "f"(max678_28));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_86)
                       : "f"(max9ab_29), "f"(maxcde_30), "f"(_tmem_load_1[63]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_92)
                       : "f"(max012_34), "f"(max345_35), "f"(max678_36));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_93)
                       : "f"(max9ab_37), "f"(maxcde_38), "f"(_tmem_load_1[79]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_95)
                       : "f"(_tmem_load_1[83]), "f"(_tmem_load_1[84]), "f"(_tmem_load_1[85]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_96)
                       : "f"(_tmem_load_1[86]), "f"(_tmem_load_1[87]), "f"(_tmem_load_1[88]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_97)
                       : "f"(_tmem_load_1[89]), "f"(_tmem_load_1[90]), "f"(_tmem_load_1[91]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_99)
                       : "f"(max012_42), "f"(max345_43), "f"(max678_44));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_100)
                       : "f"(max9ab_45), "f"(maxcde_46), "f"(_tmem_load_1[95]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_106)
                       : "f"(max012_50), "f"(max345_51), "f"(max678_52));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_107)
                       : "f"(max9ab_53), "f"(maxcde_54), "f"(_tmem_load_1[111]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_109)
                       : "f"(_tmem_load_1[115]), "f"(_tmem_load_1[116]), "f"(_tmem_load_1[117]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_110)
                       : "f"(_tmem_load_1[118]), "f"(_tmem_load_1[119]), "f"(_tmem_load_1[120]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_111)
                       : "f"(_tmem_load_1[121]), "f"(_tmem_load_1[122]), "f"(_tmem_load_1[123]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_113)
                       : "f"(max012_58), "f"(max345_59), "f"(max678_60));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_114)
                       : "f"(max9ab_61), "f"(maxcde_62), "f"(_tmem_load_1[127]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_115)
                       : "f"(group_max0_9), "f"(group_max1_17), "f"(group_max2_25));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_116)
                       : "f"(group_max3_33), "f"(group_max4_41), "f"(group_max5_49));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_117)
                       : "f"(max012_66), "f"(max345_67), "f"(max67_68));
          new_max = _max3_117;
          ...
          new_max = _max_18;
          float new_max_scaled_69 =
              ((new_max == -WAN_HYBRID_INF) ? 0.0f : new_max) * softmax_scale_log2;
          float acc_scale;
          float selected_max;
          if ((row_max_val - ((new_max == -WAN_HYBRID_INF) ? 0.0f : new_max)) *
                  softmax_scale_log2 >=
              -8.0f) {
            selected_max = row_max_val;
            acc_scale = 1.0f;
            new_max_scaled_69 =
                ((row_max_val == -WAN_HYBRID_INF) ? 0.0f : row_max_val) * softmax_scale_log2;
          } else {
            selected_max = new_max;
            if (row_max_val > -WAN_HYBRID_INF) {
              float _exp2_8 =
                  approx_exp2((row_max_val - ((new_max == -WAN_HYBRID_INF) ? 0.0f : new_max)) *
                              softmax_scale_log2);
              acc_scale = _exp2_8;
            } else {
              acc_scale = 1.0f;
          ...
          float block_max_scaled_75 =
              ((block_max_74 > -WAN_HYBRID_INF) ? block_max_74 * softmax_scale_log2 : 0.0f);
          ...
          const float2 _fma_c2_33 = {2.584962500721156f - block_max_scaled_75,
                                     2.584962500721156f - block_max_scaled_75};
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
            fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_1 + 0))[_lf], _fma_b2_32,
                              _fma_c2_33);
#pragma unroll
          for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
          ...
          asm volatile(
              ...
              : "f"(_tmem_load_1[8]), "f"(_tmem_load_1[9]), "f"(_tmem_load_1[10]),
                "f"(_tmem_load_1[11]), "f"(_tmem_load_1[12]), "f"(_tmem_load_1[13]),
                "f"(_tmem_load_1[14]), "f"(_tmem_load_1[15]));
          block_sum2_70 = add_f32x2(block_sum2_70, raw_sum2_80);
          asm volatile(
              "tcgen05.st.sync.aligned.32x32b.x2.b32"
              " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_stage_off_72 + 64 +
                                       (unsigned int)(warp % 4 * 32 << 16)),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_8[0])),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_8[1]))
              : "memory");
          float block_max_81 = group_max1;
          float block_max_scaled_82 =
              ((block_max_81 > -WAN_HYBRID_INF) ? block_max_81 * softmax_scale_log2 : 0.0f);
          float _exp2_10 =
              approx_exp2(block_max_scaled_82 - new_max_scaled_69 - 2.584962500721156f);
          ...
          const float2 _fma_c2_35 = {2.584962500721156f - block_max_scaled_82,
                                     2.584962500721156f - block_max_scaled_82};
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
            fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_1 + 16))[_lf], _fma_b2_34,
                              _fma_c2_35);
#pragma unroll
          for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
          for (int pair_9 = 2; pair_9 < 16; pair_9 += 2) {
            float2 _f2_30 =
                make_float2((_tmem_load_1 + 16)[pair_9], (_tmem_load_1 + 16)[pair_9 + 1]);
          ...
          asm volatile(
              ...
              " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_stage_off_72 + 64 +
                                       (unsigned int)(warp % 4 * 32 << 16) + 2),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_9[0])),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_9[1]))
              : "memory");
          float block_max_89 = group_max2;
          float block_max_scaled_90 =
              ((block_max_89 > -WAN_HYBRID_INF) ? block_max_89 * softmax_scale_log2 : 0.0f);
          float _exp2_11 =
              approx_exp2(block_max_scaled_90 - new_max_scaled_69 - 2.584962500721156f);
          ...
          const float2 _fma_c2_37 = {2.584962500721156f - block_max_scaled_90,
                                     2.584962500721156f - block_max_scaled_90};
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
            fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_1 + 32))[_lf], _fma_b2_36,
                              _fma_c2_37);
#pragma unroll
          for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
          for (int pair_10 = 2; pair_10 < 16; pair_10 += 2) {
            float2 _f2_33 =
                make_float2((_tmem_load_1 + 32)[pair_10], (_tmem_load_1 + 32)[pair_10 + 1]);
          ...
          asm volatile(
              ...
              : "f"(_tmem_load_1[40]), "f"(_tmem_load_1[41]), "f"(_tmem_load_1[42]),
                "f"(_tmem_load_1[43]), "f"(_tmem_load_1[44]), "f"(_tmem_load_1[45]),
          ...
          asm volatile(
              ...
              " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_stage_off_72 + 64 +
                                       (unsigned int)(warp % 4 * 32 << 16) + 4),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_10[0])),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_10[1]))
              : "memory");
          float block_max_97 = group_max3;
          float block_max_scaled_98 =
              ((block_max_97 > -WAN_HYBRID_INF) ? block_max_97 * softmax_scale_log2 : 0.0f);
          float _exp2_12 =
              approx_exp2(block_max_scaled_98 - new_max_scaled_69 - 2.584962500721156f);
          ...
          const float2 _fma_c2_39 = {2.584962500721156f - block_max_scaled_98,
                                     2.584962500721156f - block_max_scaled_98};
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
            fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_1 + 48))[_lf], _fma_b2_38,
                              _fma_c2_39);
#pragma unroll
          for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
          for (int pair_11 = 2; pair_11 < 16; pair_11 += 2) {
            float2 _f2_36 =
                make_float2((_tmem_load_1 + 48)[pair_11], (_tmem_load_1 + 48)[pair_11 + 1]);
          ...
          asm volatile(
              ...
              " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_stage_off_72 + 64 +
                                       (unsigned int)(warp % 4 * 32 << 16) + 6),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_11[0])),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_11[1]))
              : "memory");
          uint32_t _fp8_2[1];
          {
            uint32_t _packed;
            asm volatile(
                ...
                "}"
                : "=r"(_packed)
                : "f"(sf_values_71[0]), "f"(sf_values_71[1]), "f"(sf_values_71[2]),
                  "f"(sf_values_71[3]));
            _fp8_2[0] = _packed;
          }
          asm volatile(
              "tcgen05.st.sync.aligned.32x32b.x1.b32"
              " [%0], {%1};" ::"r"(taddr + (unsigned int)p_stage_off_72 + 80 +
                                   (unsigned int)(warp % 4)),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp8_2[0]))
              : "memory");
          asm volatile("tcgen05.wait::st.sync.aligned;" ::: "memory");
          mbarrier_arrive(p_full_addr + (stage) * 8);
          if (stage == 0) {
          ...
          int p_hi_off_104 =
              ((stage == 0) ? TMEM_SCORES_1_OFFSET + 104 : TMEM_SCORES_1_OFFSET + 72);
          int sfa_hi_off_105 =
              ((stage == 0) ? TMEM_TMEM_SFA_PV0_HI_OFFSET : TMEM_TMEM_SFA_PV1_HI_OFFSET);
          float _exp2_13 = approx_exp2(
              ((group_max4 > -WAN_HYBRID_INF) ? group_max4 * softmax_scale_log2 : 0.0f) -
              new_max_scaled_69 - 2.584962500721156f);
          sf_values_71[0] = ((group_max4 > -WAN_HYBRID_INF) ? _exp2_13 : 0.0f);
          const float2 _fma_b2_40 = {softmax_scale_log2, softmax_scale_log2};
          const float2 _fma_c2_41 = {
              2.584962500721156f -
                  ((group_max4 > -WAN_HYBRID_INF) ? group_max4 * softmax_scale_log2 : 0.0f),
              2.584962500721156f -
                  ((group_max4 > -WAN_HYBRID_INF) ? group_max4 * softmax_scale_log2 : 0.0f)};
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
            fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_1 + 64))[_lf], _fma_b2_40,
                              _fma_c2_41);
#pragma unroll
          for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
          for (int pair_12 = 2; pair_12 < 16; pair_12 += 2) {
            float2 _f2_39 =
                make_float2((_tmem_load_1 + 64)[pair_12], (_tmem_load_1 + 64)[pair_12 + 1]);
            partial_106 = add_f32x2(partial_106, _f2_39);
          }
          float2 frag_sum2_107 = partial_106;
          float2 _f2_40 = make_float2(((group_max4 > -WAN_HYBRID_INF) ? _exp2_13 : 0.0f),
                                      ((group_max4 > -WAN_HYBRID_INF) ? _exp2_13 : 0.0f));
          ...
          asm volatile(
              ...
              : "f"(_tmem_load_1[72]), "f"(_tmem_load_1[73]), "f"(_tmem_load_1[74]),
                "f"(_tmem_load_1[75]), "f"(_tmem_load_1[76]), "f"(_tmem_load_1[77]),
                "f"(_tmem_load_1[78]), "f"(_tmem_load_1[79]));
          block_sum2_70 = add_f32x2(block_sum2_70, raw_sum2_109);
          asm volatile(
              "tcgen05.st.sync.aligned.32x32b.x2.b32"
              " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_hi_off_104 +
                                       (unsigned int)(warp % 4 * 32 << 16)),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_12[0])),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_12[1]))
              : "memory");
          float _exp2_14 = approx_exp2(
              ((group_max5 > -WAN_HYBRID_INF) ? group_max5 * softmax_scale_log2 : 0.0f) -
              new_max_scaled_69 - 2.584962500721156f);
          sf_values_71[1] = ((group_max5 > -WAN_HYBRID_INF) ? _exp2_14 : 0.0f);
          const float2 _fma_b2_42 = {softmax_scale_log2, softmax_scale_log2};
          const float2 _fma_c2_43 = {
              2.584962500721156f -
                  ((group_max5 > -WAN_HYBRID_INF) ? group_max5 * softmax_scale_log2 : 0.0f),
              2.584962500721156f -
                  ((group_max5 > -WAN_HYBRID_INF) ? group_max5 * softmax_scale_log2 : 0.0f)};
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
            fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_1 + 80))[_lf], _fma_b2_42,
                              _fma_c2_43);
#pragma unroll
          for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
          for (int pair_13 = 2; pair_13 < 16; pair_13 += 2) {
            float2 _f2_42 =
                make_float2((_tmem_load_1 + 80)[pair_13], (_tmem_load_1 + 80)[pair_13 + 1]);
            partial_110 = add_f32x2(partial_110, _f2_42);
          }
          float2 frag_sum2_111 = partial_110;
          float2 _f2_43 = make_float2(((group_max5 > -WAN_HYBRID_INF) ? _exp2_14 : 0.0f),
                                      ((group_max5 > -WAN_HYBRID_INF) ? _exp2_14 : 0.0f));
          ...
          asm volatile(
              ...
              " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_hi_off_104 +
                                       (unsigned int)(warp % 4 * 32 << 16) + 2),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_13[0])),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_13[1]))
              : "memory");
          float _exp2_15 = approx_exp2(
              ((group_max6 > -WAN_HYBRID_INF) ? group_max6 * softmax_scale_log2 : 0.0f) -
              new_max_scaled_69 - 2.584962500721156f);
          sf_values_71[2] = ((group_max6 > -WAN_HYBRID_INF) ? _exp2_15 : 0.0f);
          const float2 _fma_b2_44 = {softmax_scale_log2, softmax_scale_log2};
          const float2 _fma_c2_45 = {
              2.584962500721156f -
                  ((group_max6 > -WAN_HYBRID_INF) ? group_max6 * softmax_scale_log2 : 0.0f),
              2.584962500721156f -
                  ((group_max6 > -WAN_HYBRID_INF) ? group_max6 * softmax_scale_log2 : 0.0f)};
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
            fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_1 + 96))[_lf], _fma_b2_44,
                              _fma_c2_45);
#pragma unroll
          for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
          for (int pair_14 = 2; pair_14 < 16; pair_14 += 2) {
            float2 _f2_45 =
                make_float2((_tmem_load_1 + 96)[pair_14], (_tmem_load_1 + 96)[pair_14 + 1]);
            partial_114 = add_f32x2(partial_114, _f2_45);
          }
          float2 frag_sum2_115 = partial_114;
          float2 _f2_46 = make_float2(((group_max6 > -WAN_HYBRID_INF) ? _exp2_15 : 0.0f),
                                      ((group_max6 > -WAN_HYBRID_INF) ? _exp2_15 : 0.0f));
          ...
          asm volatile(
              ...
              : "f"(_tmem_load_1[104]), "f"(_tmem_load_1[105]), "f"(_tmem_load_1[106]),
                "f"(_tmem_load_1[107]), "f"(_tmem_load_1[108]), "f"(_tmem_load_1[109]),
          ...
          asm volatile(
              ...
              " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_hi_off_104 +
                                       (unsigned int)(warp % 4 * 32 << 16) + 4),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_14[0])),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_14[1]))
              : "memory");
          float _exp2_16 = approx_exp2(
              ((group_max7 > -WAN_HYBRID_INF) ? group_max7 * softmax_scale_log2 : 0.0f) -
              new_max_scaled_69 - 2.584962500721156f);
          sf_values_71[3] = ((group_max7 > -WAN_HYBRID_INF) ? _exp2_16 : 0.0f);
          const float2 _fma_b2_46 = {softmax_scale_log2, softmax_scale_log2};
          const float2 _fma_c2_47 = {
              2.584962500721156f -
                  ((group_max7 > -WAN_HYBRID_INF) ? group_max7 * softmax_scale_log2 : 0.0f),
              2.584962500721156f -
                  ((group_max7 > -WAN_HYBRID_INF) ? group_max7 * softmax_scale_log2 : 0.0f)};
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
            fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_1 + 112))[_lf], _fma_b2_46,
                              _fma_c2_47);
#pragma unroll
          for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
          for (int pair_15 = 2; pair_15 < 16; pair_15 += 2) {
            float2 _f2_48 =
                make_float2((_tmem_load_1 + 112)[pair_15], (_tmem_load_1 + 112)[pair_15 + 1]);
            partial_118 = add_f32x2(partial_118, _f2_48);
          }
          float2 frag_sum2_119 = partial_118;
          float2 _f2_49 = make_float2(((group_max7 > -WAN_HYBRID_INF) ? _exp2_16 : 0.0f),
                                      ((group_max7 > -WAN_HYBRID_INF) ? _exp2_16 : 0.0f));
          ...
          asm volatile(
              ...
              " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_hi_off_104 +
                                       (unsigned int)(warp % 4 * 32 << 16) + 6),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_15[0])),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_15[1]))
              : "memory");
          uint32_t _fp8_3[1];
          {
            uint32_t _packed;
          ...
          asm volatile(
              "tcgen05.st.sync.aligned.32x32b.x1.b32"
              " [%0], {%1};" ::"r"(taddr + (unsigned int)sfa_hi_off_105 + (unsigned int)(warp % 4)),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp8_3[0]))
              : "memory");
          asm volatile("tcgen05.wait::st.sync.aligned;" ::: "memory");
          mbarrier_arrive(p_full_2_addr + (stage) * 8);
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_123)
                       : "f"(max012_2_1), "f"(max345_3_1), "f"(max678_4_1));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_124)
                       : "f"(max9ab_5_1), "f"(maxcde_6_1), "f"(_tmem_load_2[15]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_125)
                       : "f"(_tmem_load_2[16]), "f"(_tmem_load_2[17]), "f"(_tmem_load_2[18]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_126)
                       : "f"(_tmem_load_2[19]), "f"(_tmem_load_2[20]), "f"(_tmem_load_2[21]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_127)
                       : "f"(_tmem_load_2[22]), "f"(_tmem_load_2[23]), "f"(_tmem_load_2[24]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_128)
                       : "f"(_tmem_load_2[25]), "f"(_tmem_load_2[26]), "f"(_tmem_load_2[27]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_129)
                       : "f"(_tmem_load_2[28]), "f"(_tmem_load_2[29]), "f"(_tmem_load_2[30]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_130)
                       : "f"(max012_10_1), "f"(max345_11_1), "f"(max678_12_1));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_131)
                       : "f"(max9ab_13_1), "f"(maxcde_14_1), "f"(_tmem_load_2[31]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_132)
                       : "f"(_tmem_load_2[32]), "f"(_tmem_load_2[33]), "f"(_tmem_load_2[34]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_133)
                       : "f"(_tmem_load_2[35]), "f"(_tmem_load_2[36]), "f"(_tmem_load_2[37]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_134)
                       : "f"(_tmem_load_2[38]), "f"(_tmem_load_2[39]), "f"(_tmem_load_2[40]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_135)
                       : "f"(_tmem_load_2[41]), "f"(_tmem_load_2[42]), "f"(_tmem_load_2[43]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_136)
                       : "f"(_tmem_load_2[44]), "f"(_tmem_load_2[45]), "f"(_tmem_load_2[46]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_137)
                       : "f"(max012_18_1), "f"(max345_19_1), "f"(max678_20_1));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_138)
                       : "f"(max9ab_21_1), "f"(maxcde_22_1), "f"(_tmem_load_2[47]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_144)
                       : "f"(max012_26_1), "f"(max345_27_1), "f"(max678_28_1));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_145)
                       : "f"(max9ab_29_1), "f"(maxcde_30_1), "f"(_tmem_load_2[63]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_146)
                       : "f"(_tmem_load_2[64]), "f"(_tmem_load_2[65]), "f"(_tmem_load_2[66]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_147)
                       : "f"(_tmem_load_2[67]), "f"(_tmem_load_2[68]), "f"(_tmem_load_2[69]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_148)
                       : "f"(_tmem_load_2[70]), "f"(_tmem_load_2[71]), "f"(_tmem_load_2[72]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_149)
                       : "f"(_tmem_load_2[73]), "f"(_tmem_load_2[74]), "f"(_tmem_load_2[75]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_150)
                       : "f"(_tmem_load_2[76]), "f"(_tmem_load_2[77]), "f"(_tmem_load_2[78]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_151)
                       : "f"(max012_34_1), "f"(max345_35_1), "f"(max678_36_1));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_152)
                       : "f"(max9ab_37_1), "f"(maxcde_38_1), "f"(_tmem_load_2[79]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_153)
                       : "f"(_tmem_load_2[80]), "f"(_tmem_load_2[81]), "f"(_tmem_load_2[82]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_154)
                       : "f"(_tmem_load_2[83]), "f"(_tmem_load_2[84]), "f"(_tmem_load_2[85]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_155)
                       : "f"(_tmem_load_2[86]), "f"(_tmem_load_2[87]), "f"(_tmem_load_2[88]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_156)
                       : "f"(_tmem_load_2[89]), "f"(_tmem_load_2[90]), "f"(_tmem_load_2[91]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_157)
                       : "f"(_tmem_load_2[92]), "f"(_tmem_load_2[93]), "f"(_tmem_load_2[94]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_158)
                       : "f"(max012_42_1), "f"(max345_43_1), "f"(max678_44_1));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_159)
                       : "f"(max9ab_45_1), "f"(maxcde_46_1), "f"(_tmem_load_2[95]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_160)
                       : "f"(_tmem_load_2[96]), "f"(_tmem_load_2[97]), "f"(_tmem_load_2[98]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_161)
                       : "f"(_tmem_load_2[99]), "f"(_tmem_load_2[100]), "f"(_tmem_load_2[101]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_162)
                       : "f"(_tmem_load_2[102]), "f"(_tmem_load_2[103]), "f"(_tmem_load_2[104]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_163)
                       : "f"(_tmem_load_2[105]), "f"(_tmem_load_2[106]), "f"(_tmem_load_2[107]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_164)
                       : "f"(_tmem_load_2[108]), "f"(_tmem_load_2[109]), "f"(_tmem_load_2[110]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_165)
                       : "f"(max012_50_1), "f"(max345_51_1), "f"(max678_52_1));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_166)
                       : "f"(max9ab_53_1), "f"(maxcde_54_1), "f"(_tmem_load_2[111]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_167)
                       : "f"(_tmem_load_2[112]), "f"(_tmem_load_2[113]), "f"(_tmem_load_2[114]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_168)
                       : "f"(_tmem_load_2[115]), "f"(_tmem_load_2[116]), "f"(_tmem_load_2[117]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_169)
                       : "f"(_tmem_load_2[118]), "f"(_tmem_load_2[119]), "f"(_tmem_load_2[120]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_170)
                       : "f"(_tmem_load_2[121]), "f"(_tmem_load_2[122]), "f"(_tmem_load_2[123]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_171)
                       : "f"(_tmem_load_2[124]), "f"(_tmem_load_2[125]), "f"(_tmem_load_2[126]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_172)
                       : "f"(max012_58_1), "f"(max345_59_1), "f"(max678_60_1));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_173)
                       : "f"(max9ab_61_1), "f"(maxcde_62_1), "f"(_tmem_load_2[127]));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_174)
                       : "f"(group_max0_9_1), "f"(group_max1_17_1), "f"(group_max2_25_1));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_175)
                       : "f"(group_max3_33_1), "f"(group_max4_41_1), "f"(group_max5_49_1));
...
#error "Max3 requires PTX three-input max.f32 support on sm_100+"
          ...
          asm volatile("max.f32 %0, %1, %2, %3;"
                       : "=f"(_max3_176)
                       : "f"(max012_66_1), "f"(max345_67_1), "f"(max67_68_1));
          new_max = _max3_176;
          ...
          new_max = _max_28;
          float new_max_scaled_69_1 =
              ((new_max == -WAN_HYBRID_INF) ? 0.0f : new_max) * softmax_scale_log2;
          float acc_scale_1;
          float selected_max_1;
          selected_max_1 = (((row_max_val - ((new_max == -WAN_HYBRID_INF) ? 0.0f : new_max)) *
                                 softmax_scale_log2 >=
                             -8.0f)
                                ? row_max_val
                                : new_max);
          float _exp2_17 = approx_exp2(
              (row_max_val - ((new_max == -WAN_HYBRID_INF) ? 0.0f : new_max)) * softmax_scale_log2);
          acc_scale_1 = (((row_max_val - ((new_max == -WAN_HYBRID_INF) ? 0.0f : new_max)) *
                              softmax_scale_log2 >=
                          -8.0f)
                             ? 1.0f
                             : _exp2_17);
          new_max_scaled_69_1 = (((row_max_val - ((new_max == -WAN_HYBRID_INF) ? 0.0f : new_max)) *
                                      softmax_scale_log2 >=
                                  -8.0f)
                                     ? row_max_val
                                     : ((new_max == -WAN_HYBRID_INF) ? 0.0f : new_max)) *
                                softmax_scale_log2;
          ...
          float block_max_scaled_75_1 =
              ((block_max_74_1 > -WAN_HYBRID_INF) ? block_max_74_1 * softmax_scale_log2 : 0.0f);
          float _exp2_18 =
              approx_exp2(block_max_scaled_75_1 - new_max_scaled_69_1 - 2.584962500721156f);
          ...
          const float2 _fma_c2_49 = {2.584962500721156f - block_max_scaled_75_1,
                                     2.584962500721156f - block_max_scaled_75_1};
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
            fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_2 + 0))[_lf], _fma_b2_48,
                              _fma_c2_49);
#pragma unroll
          for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
          for (int pair_16 = 2; pair_16 < 16; pair_16 += 2) {
            float2 _f2_52 =
                make_float2((_tmem_load_2 + 0)[pair_16], (_tmem_load_2 + 0)[pair_16 + 1]);
          ...
          asm volatile(
              ...
              " } \n"
              : "=r"(_fp4_16[1])
              : "f"(_tmem_load_2[8]), "f"(_tmem_load_2[9]), "f"(_tmem_load_2[10]),
                "f"(_tmem_load_2[11]), "f"(_tmem_load_2[12]), "f"(_tmem_load_2[13]),
                "f"(_tmem_load_2[14]), "f"(_tmem_load_2[15]));
          block_sum2_70_1 = add_f32x2(block_sum2_70_1, raw_sum2_80_1);
          asm volatile(
              "tcgen05.st.sync.aligned.32x32b.x2.b32"
              " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_stage_off_72_1 + 64 +
                                       (unsigned int)(warp % 4 * 32 << 16)),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_16[0])),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_16[1]))
              : "memory");
          float block_max_81_1 = group_max1;
          float block_max_scaled_82_1 =
              ((block_max_81_1 > -WAN_HYBRID_INF) ? block_max_81_1 * softmax_scale_log2 : 0.0f);
          float _exp2_19 =
              approx_exp2(block_max_scaled_82_1 - new_max_scaled_69_1 - 2.584962500721156f);
          ...
          const float2 _fma_c2_51 = {2.584962500721156f - block_max_scaled_82_1,
                                     2.584962500721156f - block_max_scaled_82_1};
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
            fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_2 + 16))[_lf], _fma_b2_50,
                              _fma_c2_51);
#pragma unroll
          for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
          for (int pair_17 = 2; pair_17 < 16; pair_17 += 2) {
            float2 _f2_55 =
                make_float2((_tmem_load_2 + 16)[pair_17], (_tmem_load_2 + 16)[pair_17 + 1]);
          ...
          asm volatile(
              ...
              " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_stage_off_72_1 + 64 +
                                       (unsigned int)(warp % 4 * 32 << 16) + 2),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_17[0])),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_17[1]))
              : "memory");
          float block_max_89_1 = group_max2;
          float block_max_scaled_90_1 =
              ((block_max_89_1 > -WAN_HYBRID_INF) ? block_max_89_1 * softmax_scale_log2 : 0.0f);
          float _exp2_20 =
              approx_exp2(block_max_scaled_90_1 - new_max_scaled_69_1 - 2.584962500721156f);
          ...
          const float2 _fma_c2_53 = {2.584962500721156f - block_max_scaled_90_1,
                                     2.584962500721156f - block_max_scaled_90_1};
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
            fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_2 + 32))[_lf], _fma_b2_52,
                              _fma_c2_53);
#pragma unroll
          for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
          for (int pair_18 = 2; pair_18 < 16; pair_18 += 2) {
            float2 _f2_58 =
                make_float2((_tmem_load_2 + 32)[pair_18], (_tmem_load_2 + 32)[pair_18 + 1]);
          ...
          asm volatile(
              ...
              " } \n"
              : "=r"(_fp4_18[1])
              : "f"(_tmem_load_2[40]), "f"(_tmem_load_2[41]), "f"(_tmem_load_2[42]),
                "f"(_tmem_load_2[43]), "f"(_tmem_load_2[44]), "f"(_tmem_load_2[45]),
          ...
          asm volatile(
              ...
              " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_stage_off_72_1 + 64 +
                                       (unsigned int)(warp % 4 * 32 << 16) + 4),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_18[0])),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_18[1]))
              : "memory");
          float block_max_97_1 = group_max3;
          float block_max_scaled_98_1 =
              ((block_max_97_1 > -WAN_HYBRID_INF) ? block_max_97_1 * softmax_scale_log2 : 0.0f);
          float _exp2_21 =
              approx_exp2(block_max_scaled_98_1 - new_max_scaled_69_1 - 2.584962500721156f);
          ...
          const float2 _fma_c2_55 = {2.584962500721156f - block_max_scaled_98_1,
                                     2.584962500721156f - block_max_scaled_98_1};
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
            fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_2 + 48))[_lf], _fma_b2_54,
                              _fma_c2_55);
#pragma unroll
          for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
          for (int pair_19 = 2; pair_19 < 16; pair_19 += 2) {
            float2 _f2_61 =
                make_float2((_tmem_load_2 + 48)[pair_19], (_tmem_load_2 + 48)[pair_19 + 1]);
          ...
          asm volatile(
              ...
              " } \n"
              : "=r"(_fp4_19[1])
              : "f"(_tmem_load_2[56]), "f"(_tmem_load_2[57]), "f"(_tmem_load_2[58]),
                "f"(_tmem_load_2[59]), "f"(_tmem_load_2[60]), "f"(_tmem_load_2[61]),
          ...
          asm volatile(
              ...
              " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_stage_off_72_1 + 64 +
                                       (unsigned int)(warp % 4 * 32 << 16) + 6),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_19[0])),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_19[1]))
              : "memory");
          uint32_t _fp8_4[1];
          {
            uint32_t _packed;
            asm volatile(
                ...
                "}"
                : "=r"(_packed)
                : "f"(sf_values_71_1[0]), "f"(sf_values_71_1[1]), "f"(sf_values_71_1[2]),
                  "f"(sf_values_71_1[3]));
            _fp8_4[0] = _packed;
          }
          asm volatile(
              "tcgen05.st.sync.aligned.32x32b.x1.b32"
              " [%0], {%1};" ::"r"(taddr + (unsigned int)p_stage_off_72_1 + 80 +
                                   (unsigned int)(warp % 4)),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp8_4[0]))
              : "memory");
          asm volatile("tcgen05.wait::st.sync.aligned;" ::: "memory");
          mbarrier_arrive(p_full_addr + (stage) * 8);
          if (stage == 0) {
...
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
            fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_2 + 64))[_lf], _fma_b2_56,
                              _fma_c2_57);
#pragma unroll
          for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
          for (int pair_20 = 2; pair_20 < 16; pair_20 += 2) {
            float2 _f2_64 =
                make_float2((_tmem_load_2 + 64)[pair_20], (_tmem_load_2 + 64)[pair_20 + 1]);
            partial_106_1 = add_f32x2(partial_106_1, _f2_64);
          }
          float2 frag_sum2_107_1 = partial_106_1;
          float2 _f2_65 = make_float2(((group_max4 > -WAN_HYBRID_INF) ? _exp2_22 : 0.0f),
                                      ((group_max4 > -WAN_HYBRID_INF) ? _exp2_22 : 0.0f));
          ...
          asm volatile(
              ...
              : "f"(_tmem_load_2[64]), "f"(_tmem_load_2[65]), "f"(_tmem_load_2[66]),
                "f"(_tmem_load_2[67]), "f"(_tmem_load_2[68]), "f"(_tmem_load_2[69]),
                "f"(_tmem_load_2[70]), "f"(_tmem_load_2[71]));
          asm volatile(
              " { .reg .b8 __b0, __b1, __b2, __b3; \n"
              " cvt.rn.satfinite.e2m1x2.f32 __b0, %2, %1; \n"
              " cvt.rn.satfinite.e2m1x2.f32 __b1, %4, %3; \n"
              ...
              " } \n"
              : "=r"(_fp4_20[1])
              : "f"(_tmem_load_2[72]), "f"(_tmem_load_2[73]), "f"(_tmem_load_2[74]),
                "f"(_tmem_load_2[75]), "f"(_tmem_load_2[76]), "f"(_tmem_load_2[77]),
                "f"(_tmem_load_2[78]), "f"(_tmem_load_2[79]));
          block_sum2_70_1 = add_f32x2(block_sum2_70_1, raw_sum2_109_1);
          asm volatile(
              "tcgen05.st.sync.aligned.32x32b.x2.b32"
              " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_hi_off_104_1 +
                                       (unsigned int)(warp % 4 * 32 << 16)),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_20[0])),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_20[1]))
              : "memory");
          float _exp2_23 = approx_exp2(
              ((group_max5 > -WAN_HYBRID_INF) ? group_max5 * softmax_scale_log2 : 0.0f) -
              new_max_scaled_69_1 - 2.584962500721156f);
          sf_values_71_1[1] = ((group_max5 > -WAN_HYBRID_INF) ? _exp2_23 : 0.0f);
          const float2 _fma_b2_58 = {softmax_scale_log2, softmax_scale_log2};
          const float2 _fma_c2_59 = {
              2.584962500721156f -
                  ((group_max5 > -WAN_HYBRID_INF) ? group_max5 * softmax_scale_log2 : 0.0f),
              2.584962500721156f -
                  ((group_max5 > -WAN_HYBRID_INF) ? group_max5 * softmax_scale_log2 : 0.0f)};
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
            fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_2 + 80))[_lf], _fma_b2_58,
                              _fma_c2_59);
#pragma unroll
          for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
          for (int pair_21 = 2; pair_21 < 16; pair_21 += 2) {
            float2 _f2_67 =
                make_float2((_tmem_load_2 + 80)[pair_21], (_tmem_load_2 + 80)[pair_21 + 1]);
            partial_110_1 = add_f32x2(partial_110_1, _f2_67);
          }
          float2 frag_sum2_111_1 = partial_110_1;
          float2 _f2_68 = make_float2(((group_max5 > -WAN_HYBRID_INF) ? _exp2_23 : 0.0f),
                                      ((group_max5 > -WAN_HYBRID_INF) ? _exp2_23 : 0.0f));
          ...
          asm volatile(
              ...
              : "f"(_tmem_load_2[80]), "f"(_tmem_load_2[81]), "f"(_tmem_load_2[82]),
                "f"(_tmem_load_2[83]), "f"(_tmem_load_2[84]), "f"(_tmem_load_2[85]),
                "f"(_tmem_load_2[86]), "f"(_tmem_load_2[87]));
          asm volatile(
              " { .reg .b8 __b0, __b1, __b2, __b3; \n"
              " cvt.rn.satfinite.e2m1x2.f32 __b0, %2, %1; \n"
              " cvt.rn.satfinite.e2m1x2.f32 __b1, %4, %3; \n"
              ...
              " } \n"
              : "=r"(_fp4_21[1])
              : "f"(_tmem_load_2[88]), "f"(_tmem_load_2[89]), "f"(_tmem_load_2[90]),
                "f"(_tmem_load_2[91]), "f"(_tmem_load_2[92]), "f"(_tmem_load_2[93]),
                "f"(_tmem_load_2[94]), "f"(_tmem_load_2[95]));
          block_sum2_70_1 = add_f32x2(block_sum2_70_1, raw_sum2_113_1);
          asm volatile(
              "tcgen05.st.sync.aligned.32x32b.x2.b32"
              " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_hi_off_104_1 +
                                       (unsigned int)(warp % 4 * 32 << 16) + 2),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_21[0])),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_21[1]))
              : "memory");
          float _exp2_24 = approx_exp2(
              ((group_max6 > -WAN_HYBRID_INF) ? group_max6 * softmax_scale_log2 : 0.0f) -
              new_max_scaled_69_1 - 2.584962500721156f);
          sf_values_71_1[2] = ((group_max6 > -WAN_HYBRID_INF) ? _exp2_24 : 0.0f);
          const float2 _fma_b2_60 = {softmax_scale_log2, softmax_scale_log2};
          const float2 _fma_c2_61 = {
              2.584962500721156f -
                  ((group_max6 > -WAN_HYBRID_INF) ? group_max6 * softmax_scale_log2 : 0.0f),
              2.584962500721156f -
                  ((group_max6 > -WAN_HYBRID_INF) ? group_max6 * softmax_scale_log2 : 0.0f)};
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
            fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_2 + 96))[_lf], _fma_b2_60,
                              _fma_c2_61);
#pragma unroll
          for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
          for (int pair_22 = 2; pair_22 < 16; pair_22 += 2) {
            float2 _f2_70 =
                make_float2((_tmem_load_2 + 96)[pair_22], (_tmem_load_2 + 96)[pair_22 + 1]);
            partial_114_1 = add_f32x2(partial_114_1, _f2_70);
          }
          float2 frag_sum2_115_1 = partial_114_1;
          float2 _f2_71 = make_float2(((group_max6 > -WAN_HYBRID_INF) ? _exp2_24 : 0.0f),
                                      ((group_max6 > -WAN_HYBRID_INF) ? _exp2_24 : 0.0f));
          ...
          asm volatile(
              ...
              : "f"(_tmem_load_2[96]), "f"(_tmem_load_2[97]), "f"(_tmem_load_2[98]),
                "f"(_tmem_load_2[99]), "f"(_tmem_load_2[100]), "f"(_tmem_load_2[101]),
                "f"(_tmem_load_2[102]), "f"(_tmem_load_2[103]));
          asm volatile(
              " { .reg .b8 __b0, __b1, __b2, __b3; \n"
              " cvt.rn.satfinite.e2m1x2.f32 __b0, %2, %1; \n"
              " cvt.rn.satfinite.e2m1x2.f32 __b1, %4, %3; \n"
              ...
              " } \n"
              : "=r"(_fp4_22[1])
              : "f"(_tmem_load_2[104]), "f"(_tmem_load_2[105]), "f"(_tmem_load_2[106]),
                "f"(_tmem_load_2[107]), "f"(_tmem_load_2[108]), "f"(_tmem_load_2[109]),
                "f"(_tmem_load_2[110]), "f"(_tmem_load_2[111]));
          block_sum2_70_1 = add_f32x2(block_sum2_70_1, raw_sum2_117_1);
          asm volatile(
              "tcgen05.st.sync.aligned.32x32b.x2.b32"
              " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_hi_off_104_1 +
                                       (unsigned int)(warp % 4 * 32 << 16) + 4),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_22[0])),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_22[1]))
              : "memory");
          float _exp2_25 = approx_exp2(
              ((group_max7 > -WAN_HYBRID_INF) ? group_max7 * softmax_scale_log2 : 0.0f) -
              new_max_scaled_69_1 - 2.584962500721156f);
          sf_values_71_1[3] = ((group_max7 > -WAN_HYBRID_INF) ? _exp2_25 : 0.0f);
          const float2 _fma_b2_62 = {softmax_scale_log2, softmax_scale_log2};
          const float2 _fma_c2_63 = {
              2.584962500721156f -
                  ((group_max7 > -WAN_HYBRID_INF) ? group_max7 * softmax_scale_log2 : 0.0f),
              2.584962500721156f -
                  ((group_max7 > -WAN_HYBRID_INF) ? group_max7 * softmax_scale_log2 : 0.0f)};
#pragma unroll
          for (int _lf = 0; _lf < 8; _lf++)
            fma_f32x2_inplace(&reinterpret_cast<float2*>((_tmem_load_2 + 112))[_lf], _fma_b2_62,
                              _fma_c2_63);
#pragma unroll
          for (int _le = 0; _le < 16; _le++) {
...
#pragma unroll
          for (int pair_23 = 2; pair_23 < 16; pair_23 += 2) {
            float2 _f2_73 =
                make_float2((_tmem_load_2 + 112)[pair_23], (_tmem_load_2 + 112)[pair_23 + 1]);
            partial_118_1 = add_f32x2(partial_118_1, _f2_73);
          }
          float2 frag_sum2_119_1 = partial_118_1;
          float2 _f2_74 = make_float2(((group_max7 > -WAN_HYBRID_INF) ? _exp2_25 : 0.0f),
                                      ((group_max7 > -WAN_HYBRID_INF) ? _exp2_25 : 0.0f));
          ...
          asm volatile(
              ...
              : "f"(_tmem_load_2[112]), "f"(_tmem_load_2[113]), "f"(_tmem_load_2[114]),
                "f"(_tmem_load_2[115]), "f"(_tmem_load_2[116]), "f"(_tmem_load_2[117]),
                "f"(_tmem_load_2[118]), "f"(_tmem_load_2[119]));
          asm volatile(
              " { .reg .b8 __b0, __b1, __b2, __b3; \n"
              " cvt.rn.satfinite.e2m1x2.f32 __b0, %2, %1; \n"
              " cvt.rn.satfinite.e2m1x2.f32 __b1, %4, %3; \n"
              ...
              " } \n"
              : "=r"(_fp4_23[1])
              : "f"(_tmem_load_2[120]), "f"(_tmem_load_2[121]), "f"(_tmem_load_2[122]),
                "f"(_tmem_load_2[123]), "f"(_tmem_load_2[124]), "f"(_tmem_load_2[125]),
                "f"(_tmem_load_2[126]), "f"(_tmem_load_2[127]));
          block_sum2_70_1 = add_f32x2(block_sum2_70_1, raw_sum2_121_1);
          asm volatile(
              "tcgen05.st.sync.aligned.32x32b.x2.b32"
              " [%0], {%1, %2};" ::"r"(taddr + (unsigned int)p_hi_off_104_1 +
                                       (unsigned int)(warp % 4 * 32 << 16) + 6),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_23[0])),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp4_23[1]))
              : "memory");
          uint32_t _fp8_5[1];
          {
            uint32_t _packed;
          ...
          asm volatile(
              "tcgen05.st.sync.aligned.32x32b.x1.b32"
              " [%0], {%1};" ::"r"(taddr + (unsigned int)sfa_hi_off_105_1 +
                                   (unsigned int)(warp % 4)),
              "r"(*reinterpret_cast<const uint32_t*>(&_fp8_5[0]))
              : "memory");
          asm volatile("tcgen05.wait::st.sync.aligned;" ::: "memory");
          mbarrier_arrive(p_full_2_addr + (stage) * 8);
...
#pragma unroll 1
        for (unsigned int n_iter_2 = 1; n_iter_2 < num_n_blocks_1; n_iter_2++) {
          ...
          if (corr_p0_high_stage == 2) {
            corr_p0_high_stage = 0;
            corr_p0_high_phase ^= 1;
          }
...
#pragma unroll
            for (int ce_col_group = 0; ce_col_group < HEAD_DIM / 32; ce_col_group++) {
              ...
              int ce_addr = taddr + (unsigned int)tmem_o_off + (unsigned int)ce_tmem_row +
                            (unsigned int)ce_col_base;
              ...
              asm volatile(
                  ...
                  : "=r"(*reinterpret_cast<uint32_t*>(&_tmem_load_5[0])),
                    ...
                    "=r"(*reinterpret_cast<uint32_t*>(&_tmem_load_5[8])),
                    "=r"(*reinterpret_cast<uint32_t*>(&_tmem_load_5[9])),
                    "=r"(*reinterpret_cast<uint32_t*>(&_tmem_load_5[10])),
                    "=r"(*reinterpret_cast<uint32_t*>(&_tmem_load_5[11])),
                    "=r"(*reinterpret_cast<uint32_t*>(&_tmem_load_5[12])),
                    "=r"(*reinterpret_cast<uint32_t*>(&_tmem_load_5[13])),
                    "=r"(*reinterpret_cast<uint32_t*>(&_tmem_load_5[14])),
                    "=r"(*reinterpret_cast<uint32_t*>(&_tmem_load_5[15]))
                  : "r"(ce_addr)
                  : "memory");
...
#pragma unroll
                for (int _ls = 0; _ls < 1; _ls++)
                  mul_f32x2_inplace(
                      &reinterpret_cast<float2*>((_tmem_load_5 + ce_repeat_base))[_ls], _scale2_2);
                const float2 _scale2_3 = {ce_scale_bottom, ce_scale_bottom};
#pragma unroll
                for (int _ls = 0; _ls < 1; _ls++)
                  mul_f32x2_inplace(
                      &reinterpret_cast<float2*>((_tmem_load_5 + ce_repeat_base + 2))[_ls],
                      _scale2_3);
              }
...
#pragma unroll
              ...
              uint32_t _stmatrix_addr_4 = static_cast<uint32_t>(
                  (unsigned long long)(ce_stage_base +
                                       (ce_atom_row * 128 + ce_atom_col_bytes ^
                                        (ce_atom_row * 128 + ce_atom_col_bytes >> 7 & 7) << 4)));
              asm volatile(
                  "stmatrix.sync.aligned.m8n8.x4.shared.b16 [%0], {%1, %2, %3, %4};\n" ::"r"(
                      _stmatrix_addr_4),
                  "r"(*reinterpret_cast<const uint32_t*>(&ce_packed[0])),
                  "r"(*reinterpret_cast<const uint32_t*>(&ce_packed[2])),
                  "r"(*reinterpret_cast<const uint32_t*>(&ce_packed[4])),
                  "r"(*reinterpret_cast<const uint32_t*>(&ce_packed[6]))
                  : "memory");
              uint32_t _stmatrix_addr_5 = static_cast<uint32_t>(
                  (unsigned long long)(ce_stage_base +
                                       ((ce_atom_row + 8) * 128 + ce_atom_col_bytes ^
                                        ((ce_atom_row + 8) * 128 + ce_atom_col_bytes >> 7 & 7)
                                            << 4)));
              asm volatile(
                  "stmatrix.sync.aligned.m8n8.x4.shared.b16 [%0], {%1, %2, %3, %4};\n" ::"r"(
                      _stmatrix_addr_5),
                  "r"(*reinterpret_cast<const uint32_t*>(&ce_packed[1])),
                  "r"(*reinterpret_cast<const uint32_t*>(&ce_packed[3])),
                  "r"(*reinterpret_cast<const uint32_t*>(&ce_packed[5])),
                  "r"(*reinterpret_cast<const uint32_t*>(&ce_packed[7]))
                  : "memory");
            }
          }
          asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
...
#pragma unroll 1
        for (unsigned int n_iter_3 = 0; n_iter_3 < num_n_blocks_2 - 1; n_iter_3++) {
          ...
          _phase_p_full_0 ^= 1;
          if (elect_sync()) {
            tcgen05_cp_32x128b_warpx4(tmem_tmem_sfb_pv0_lo,
                                      make_sf_cp_desc_sbo128(smem_sfvt_lo_addr + v_stage * 32768));
          }
          if (elect_sync()) {
            tcgen05_cp_32x128b_warpx4(
                tmem_tmem_sfb_pv0_res_lo,
                make_sf_cp_desc_sbo128(smem_sfvt_residual_lo_addr + v_stage * 32768));
          }
          asm volatile("tcgen05.fence::after_thread_sync;");
          ...
          if (elect_sync()) {
            {
              ...
              tcgen05_mma_mxf4nvf4_bs_ts(tmem_output_0, tmem_scores_0 + 64 + 0, b_desc + 0,
                                         0x8200480U, tmem_tmem_sfa_pv0_lo + 0,
                                         tmem_tmem_sfb_pv0_lo + 0, ((first_pv_flag) ? 0 : 1));
            }
          }
          int _mma_b_lo_3 =
              make_warp_uniform((((smem_vt_residual_addr) >> 4) & 0x3FFF) + (v_stage) * 2048);
          if (elect_sync()) {
            {
              uint64_t b_desc = ((uint64_t)_mma_b_lo_3) | ((uint64_t)0x80004020 << 32);
              tcgen05_mma_mxf4nvf4_bs_ts(tmem_output_0, tmem_scores_0 + 64 + 0, b_desc + 0,
                                         0x8200480U, tmem_tmem_sfa_pv0_lo + 0,
                                         tmem_tmem_sfb_pv0_res_lo + 0, 1);
            }
          }
          elect_commit(pv_stage0_reads_done_addr + (pv_reads_stage) * 8);
          mbarrier_wait(pv_stage0_reads_done_addr + (pv_reads_stage) * 8,
                        _phase_pv_stage0_reads_done);
          asm volatile("tcgen05.fence::after_thread_sync;");
          ...
          if (elect_sync()) {
            tcgen05_cp_32x128b_warpx4(tmem_tmem_sfb_pv0_hi,
                                      make_sf_cp_desc_sbo128(smem_sfvt_hi_addr + v_stage * 32768));
          }
          if (elect_sync()) {
            tcgen05_cp_32x128b_warpx4(
                tmem_tmem_sfb_pv0_res_hi,
                make_sf_cp_desc_sbo128(smem_sfvt_residual_hi_addr + v_stage * 32768));
          }
          mbarrier_wait(p_full_2_addr, _phase_p_full_2_0);
          _phase_p_full_2_0 ^= 1;
          asm volatile("tcgen05.fence::after_thread_sync;");
          int _mma_b_lo_5 =
              make_warp_uniform((((smem_vt_addr + 32) >> 4) & 0x3FFF) + (v_stage) * 2048);
          if (elect_sync()) {
            {
              uint64_t b_desc = ((uint64_t)_mma_b_lo_5) | ((uint64_t)0x80004020 << 32);
              tcgen05_mma_mxf4nvf4_bs_ts(tmem_output_0, tmem_scores_1 + 104 + 0, b_desc + 0,
                                         0x8200480U, tmem_tmem_sfa_pv0_hi + 0,
                                         tmem_tmem_sfb_pv0_hi + 0, 1);
            }
          }
          int _mma_b_lo_6 =
              make_warp_uniform((((smem_vt_residual_addr + 32) >> 4) & 0x3FFF) + (v_stage) * 2048);
          if (elect_sync()) {
            {
              uint64_t b_desc = ((uint64_t)_mma_b_lo_6) | ((uint64_t)0x80004020 << 32);
              tcgen05_mma_mxf4nvf4_bs_ts(tmem_output_0, tmem_scores_1 + 104 + 0, b_desc + 0,
                                         0x8200480U, tmem_tmem_sfa_pv0_hi + 0,
                                         tmem_tmem_sfb_pv0_res_hi + 0, 1);
            }
          }
          ...
          _phase_p_full_1 ^= 1;
          if (elect_sync()) {
            tcgen05_cp_32x128b_warpx4(tmem_tmem_sfb_pv1_lo,
                                      make_sf_cp_desc_sbo128(smem_sfvt_lo_addr + v_stage * 32768));
          }
          if (elect_sync()) {
            tcgen05_cp_32x128b_warpx4(
                tmem_tmem_sfb_pv1_res_lo,
                make_sf_cp_desc_sbo128(smem_sfvt_residual_lo_addr + v_stage * 32768));
          }
          asm volatile("tcgen05.fence::after_thread_sync;");
          ...
          if (elect_sync()) {
            {
              ...
              tcgen05_mma_mxf4nvf4_bs_ts(tmem_output_1, tmem_scores_1 + 64 + 0, b_desc + 0,
                                         0x8200480U, tmem_tmem_sfa_pv1_lo + 0,
                                         tmem_tmem_sfb_pv1_lo + 0, ((first_pv_flag) ? 0 : 1));
            }
          }
          int _mma_b_lo_8 =
              make_warp_uniform((((smem_vt_residual_addr) >> 4) & 0x3FFF) + (v_stage) * 2048);
          if (elect_sync()) {
            {
              ...
              tcgen05_mma_mxf4nvf4_bs_ts(tmem_output_1, tmem_scores_1 + 64 + 0, b_desc + 0,
                                         0x8200480U, tmem_tmem_sfa_pv1_lo + 0,
                                         tmem_tmem_sfb_pv1_res_lo + 0, 1);
            }
          }
          if (elect_sync()) {
            tcgen05_cp_32x128b_warpx4(tmem_tmem_sfb_pv1_hi,
                                      make_sf_cp_desc_sbo128(smem_sfvt_hi_addr + v_stage * 32768));
          }
          if (elect_sync()) {
            tcgen05_cp_32x128b_warpx4(
                tmem_tmem_sfb_pv1_res_hi,
                make_sf_cp_desc_sbo128(smem_sfvt_residual_hi_addr + v_stage * 32768));
          }
          ...
          if (elect_sync()) {
            {
              ...
              tcgen05_mma_mxf4nvf4_bs_ts(tmem_output_1, tmem_scores_1 + 72 + 0, b_desc + 0,
                                         0x8200480U, tmem_tmem_sfa_pv1_hi + 0,
                                         tmem_tmem_sfb_pv1_hi + 0, 1);
            }
          }
          int _mma_b_lo_10 =
              make_warp_uniform((((smem_vt_residual_addr + 32) >> 4) & 0x3FFF) + (v_stage) * 2048);
          if (elect_sync()) {
            {
              ...
              tcgen05_mma_mxf4nvf4_bs_ts(tmem_output_1, tmem_scores_1 + 72 + 0, b_desc + 0,
                                         0x8200480U, tmem_tmem_sfa_pv1_hi + 0,
                                         tmem_tmem_sfb_pv1_res_hi + 0, 1);
            }
          }
          elect_commit(pv_stage1_reads_done_addr + (pv_reads_stage) * 8);
          mbarrier_wait(pv_stage1_reads_done_addr + (pv_reads_stage) * 8,
                        _phase_pv_stage1_reads_done);
          pv_reads_stage += 1;
          if (pv_reads_stage == 2) {
            pv_reads_stage = 0;
            _phase_pv_stage0_reads_done ^= 1;
            _phase_pv_stage1_reads_done ^= 1;
          }
          asm volatile("tcgen05.fence::after_thread_sync;");
          ...
          asm volatile(
              ...
              "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
              "}\n" ::"r"(_mma_a_lo_11),
              "r"(_mma_b_lo_11), "r"(tmem_scores_1), "r"(0));
          elect_commit(s_full_addr + 8);
        ...
        if (elect_sync()) {
          tcgen05_cp_32x128b_warpx4(
              tmem_tmem_sfb_pv0_lo,
              make_sf_cp_desc_sbo128(smem_sfvt_lo_addr + mma_kv_stage * 32768));
        }
        if (elect_sync()) {
          tcgen05_cp_32x128b_warpx4(
              tmem_tmem_sfb_pv0_res_lo,
              make_sf_cp_desc_sbo128(smem_sfvt_residual_lo_addr + mma_kv_stage * 32768));
        }
        asm volatile("tcgen05.fence::after_thread_sync;");
        int _mma_b_lo_12 =
            make_warp_uniform((((smem_vt_addr) >> 4) & 0x3FFF) + (mma_kv_stage) * 2048);
        if (elect_sync()) {
          {
            uint64_t b_desc = ((uint64_t)_mma_b_lo_12) | ((uint64_t)0x80004020 << 32);
            tcgen05_mma_mxf4nvf4_bs_ts(tmem_output_0, tmem_scores_0 + 64 + 0, b_desc + 0,
                                       0x8200480U, tmem_tmem_sfa_pv0_lo + 0,
                                       tmem_tmem_sfb_pv0_lo + 0, ((first_pv_flag_1) ? 0 : 1));
          }
        }
        int _mma_b_lo_13 =
            make_warp_uniform((((smem_vt_residual_addr) >> 4) & 0x3FFF) + (mma_kv_stage) * 2048);
        if (elect_sync()) {
          {
        ...
        if (elect_sync()) {
          tcgen05_cp_32x128b_warpx4(
              tmem_tmem_sfb_pv0_hi,
              make_sf_cp_desc_sbo128(smem_sfvt_hi_addr + mma_kv_stage * 32768));
        }
        if (elect_sync()) {
          tcgen05_cp_32x128b_warpx4(
              tmem_tmem_sfb_pv0_res_hi,
              make_sf_cp_desc_sbo128(smem_sfvt_residual_hi_addr + mma_kv_stage * 32768));
        }
        mbarrier_wait(p_full_2_addr, _phase_p_full_2_0);
        _phase_p_full_2_0 ^= 1;
        asm volatile("tcgen05.fence::after_thread_sync;");
        int _mma_b_lo_14 =
            make_warp_uniform((((smem_vt_addr + 32) >> 4) & 0x3FFF) + (mma_kv_stage) * 2048);
        if (elect_sync()) {
          {
        ...
        int _mma_b_lo_15 = make_warp_uniform((((smem_vt_residual_addr + 32) >> 4) & 0x3FFF) +
                                             (mma_kv_stage) * 2048);
        if (elect_sync()) {
          {
        ...
        if (elect_sync()) {
          {
            ...
            tcgen05_mma_mxf4nvf4_bs_ts(tmem_output_1, tmem_scores_1 + 64 + 0, b_desc + 0,
                                       0x8200480U, tmem_tmem_sfa_pv1_lo + 0,
                                       tmem_tmem_sfb_pv1_lo + 0, ((first_pv_flag_1) ? 0 : 1));
          }
        }
        int _mma_b_lo_17 =
            make_warp_uniform((((smem_vt_residual_addr) >> 4) & 0x3FFF) + (mma_kv_stage) * 2048);
        ...
        int _mma_b_lo_19 = make_warp_uniform((((smem_vt_residual_addr + 32) >> 4) & 0x3FFF) +
                                             (mma_kv_stage) * 2048);
        if (elect_sync()) {
          {
...
#pragma unroll
        for (int stage_2 = 0; stage_2 < 2; stage_2++) {
          ...
          if (elect_sync()) {
            tma_store_5d(O, 0, q_row, head, batch_idx, 0,
                         smem_o_addr + (unsigned int)(stage_2 * 32768));
          }
...
#pragma unroll 1
      for (unsigned int tile_idx_4 = bid; tile_idx_4 < total_tiles_4; tile_idx_4 += num_bids) {
        ...
        if (elect_sync()) {
          ...
          tma_5d_gmem2smem(smem_kv_addr + load_kv_stage * 32768, K, 0, first_kv_off, head_1,
                           batch_idx_1, 0, kv_full_addr + (load_kv_stage) * 8);
        }
        load_kv_stage += 1;
        if (load_kv_stage == 3) {
          load_kv_stage = 0;
        ...
        if (elect_sync()) {
          ...
          tma_2d_gmem2smem(smem_vt_addr + load_kv_stage * 32768, Vt, first_vt_col, first_vt_row,
                           kv_full_addr + (load_kv_stage) * 8);
          tma_2d_gmem2smem(smem_vt_residual_addr + load_kv_stage * 32768, Vt, first_vt_col,
                           v_level_row_stride + (unsigned int)first_vt_row,
                           kv_full_addr + (load_kv_stage) * 8);
          tma_2d_gmem2smem(smem_sfvt_lo_addr + load_kv_stage * 32768, SFVtLo, 0,
                           first_kv_sf_tile * 16, kv_full_addr + (load_kv_stage) * 8);
          tma_2d_gmem2smem(smem_sfvt_residual_lo_addr + load_kv_stage * 32768, SFVtLo, 0,
                           v_scale_level_stride + (unsigned int)(first_kv_sf_tile * 16),
                           kv_full_addr + (load_kv_stage) * 8);
          tma_2d_gmem2smem(smem_sfvt_hi_addr + load_kv_stage * 32768, SFVtHi, 0,
                           first_kv_sf_tile * 16, kv_full_addr + (load_kv_stage) * 8);
          tma_2d_gmem2smem(smem_sfvt_residual_hi_addr + load_kv_stage * 32768, SFVtHi, 0,
                           v_scale_level_stride + (unsigned int)(first_kv_sf_tile * 16),
                           kv_full_addr + (load_kv_stage) * 8);
...
#pragma unroll 1
        for (unsigned int ni = 1; ni < num_n_blocks_4; ni++) {
          ...
          if (elect_sync()) {
            mbarrier_arrive_expect_tx(kv_full_addr + (load_kv_stage) * 8, 32768);
            tma_5d_gmem2smem(smem_kv_addr + load_kv_stage * 32768, K, 0, kv_off, head_1,
                             batch_idx_1, 0, kv_full_addr + (load_kv_stage) * 8);
          }
          load_kv_stage += 1;
          if (load_kv_stage == 3) {
          ...
          if (elect_sync()) {
            ...
            tma_2d_gmem2smem(smem_vt_addr + load_kv_stage * 32768, Vt, vt_col, vt_row,
                             kv_full_addr + (load_kv_stage) * 8);
            tma_2d_gmem2smem(smem_vt_residual_addr + load_kv_stage * 32768, Vt, vt_col,
                             v_level_row_stride + (unsigned int)vt_row,
                             kv_full_addr + (load_kv_stage) * 8);
            tma_2d_gmem2smem(smem_sfvt_lo_addr + load_kv_stage * 32768, SFVtLo, 0, kv_sf_tile * 16,
                             kv_full_addr + (load_kv_stage) * 8);
            tma_2d_gmem2smem(smem_sfvt_residual_lo_addr + load_kv_stage * 32768, SFVtLo, 0,
                             v_scale_level_stride + (unsigned int)(kv_sf_tile * 16),
                             kv_full_addr + (load_kv_stage) * 8);
            tma_2d_gmem2smem(smem_sfvt_hi_addr + load_kv_stage * 32768, SFVtHi, 0, kv_sf_tile * 16,
                             kv_full_addr + (load_kv_stage) * 8);
            tma_2d_gmem2smem(smem_sfvt_residual_hi_addr + load_kv_stage * 32768, SFVtHi, 0,
                             v_scale_level_stride + (unsigned int)(kv_sf_tile * 16),
                             kv_full_addr + (load_kv_stage) * 8);
