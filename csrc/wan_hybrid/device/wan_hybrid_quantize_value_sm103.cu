...
#define SMEM_V_SMEM_STAGE_BYTES 32800
#define SMEM_V_SMEM_STRIDE 32800
...
__global__ __launch_bounds__(256, 1) void kernel_wan_hybrid_quantize_value(
    __nv_bfloat16* __restrict__ v, uint8_t* __restrict__ v_fp4_base_t,
    uint8_t* __restrict__ v_fp4_residual_t, uint8_t* __restrict__ v_scale_base_lo,
    uint8_t* __restrict__ v_scale_base_hi, uint8_t* __restrict__ v_scale_residual_lo,
    uint8_t* __restrict__ v_scale_residual_hi, int heads, int seq_len, int padded_seq_len,
    int logical_num_blocks, int physical_num_blocks) {
...
#pragma unroll
  for (int iteration = 0; iteration < 8; iteration++) {
    ...
    long long input_offset =
        (((long long)batch_idx * (long long)seq_len + (long long)token) * (long long)heads +
         (long long)head) *
            128 +
        (long long)dim_base;
    float values[8];
...
#pragma unroll
          for (int _pair = 0; _pair < 4; _pair++) {
            asm volatile(
                ...
                "}\n"
                : "=f"((&values[0 + _blk * 8 + _pair * 2])[0]),
                  "=f"((&values[0 + _blk * 8 + _pair * 2])[1])
                : "r"(_vpairs_0[_pair]));
          }
        }
      }
...
#pragma unroll
    for (int _lp = 0; _lp < 4; _lp++) {
      __nv_bfloat162 _bf2 =
          __float22bfloat162_rn(make_float2(values[_lp * 2 + 0], values[_lp * 2 + 1 + 0]));
      packed_input[_lp] = *(uint32_t*)&_bf2;
    }
    int row_group = row / 16;
    int smem_offset = row * 128 + row_group * 2 + dim_base;
#pragma unroll
    for (int word = 0; word < 4; word++) {
      asm volatile("st.shared.b32 [%0], %1;" ::"r"(v_smem_addr +
                                                   (unsigned int)((smem_offset + word * 2) * 2)),
                   "r"((packed_input[word])));
    }
  }
  __syncthreads();
...
#pragma unroll
    for (int element = 0; element < 16; element++) {
      int row_1 = group * 16 + element;
      int smem_index = row_1 * 128 + group * 2 + dim;
...
#pragma unroll
    ...
    asm volatile(
        ...
        " } \n"
        : "=r"(_fp4_0[0])
        : "f"(work[0]), "f"(work[1]), "f"(work[2]), "f"(work[3]), "f"(work[4]), "f"(work[5]),
          "f"(work[6]), "f"(work[7]));
    asm volatile(
        " { .reg .b8 __b0, __b1, __b2, __b3; \n"
        " cvt.rn.satfinite.e2m1x2.f32 __b0, %2, %1; \n"
        ...
        " } \n"
        : "=r"(_fp4_0[1])
        : "f"(work[8]), "f"(work[9]), "f"(work[10]), "f"(work[11]), "f"(work[12]), "f"(work[13]),
          "f"(work[14]), "f"(work[15]));
    {
    ...
    (__CUDACC_VER_MAJOR__ > 13 || (__CUDACC_VER_MAJOR__ == 13 && __CUDACC_VER_MINOR__ >= 2))
      ...
      asm volatile(
          ...
          " } \n"
          : "=r"(_fp4_decode_pairs[0]), "=r"(_fp4_decode_pairs[1]), "=r"(_fp4_decode_pairs[2]),
      ...
      asm volatile(
          ...
          " cvt.f32.f16 %7, __h1; \n"
          " } \n"
          : "=f"(work[0]), "=f"(work[1]), "=f"(work[2]), "=f"(work[3]), "=f"(work[4]),
            "=f"(work[5]), "=f"(work[6]), "=f"(work[7])
          : "r"(_fp4_0[0]));
#endif
    }
    {
    ...
    (__CUDACC_VER_MAJOR__ > 13 || (__CUDACC_VER_MAJOR__ == 13 && __CUDACC_VER_MINOR__ >= 2))
      ...
      asm volatile(
          ...
          " cvt.f32.f16 %7, __h1; \n"
          " } \n"
          : "=f"(work[8]), "=f"(work[9]), "=f"(work[10]), "=f"(work[11]), "=f"(work[12]),
            "=f"(work[13]), "=f"(work[14]), "=f"(work[15])
          : "r"(_fp4_0[1]));
#endif
    }
#pragma unroll
...
#pragma unroll
    ...
    long long output_offset =
        ((long long)bh * 128 + (long long)dim) * (long long)(padded_seq_len / 2) +
        (long long)(block * 64) + (long long)(group * 8);
    ...
    if (group < 4) {
      {
        ...
        asm("cvt.rn.satfinite.e4m3x2.f32 %0, 0f00000000, %1;"
            : "=h"(_sf_pair)
            : "f"(raw_base_scale));
        *(reinterpret_cast<unsigned char*>(v_scale_base_lo +
                                           (scale_tile_offset + (long long)scale_offset)) +
      ...
      }
      {
        ...
        asm("cvt.rn.satfinite.e4m3x2.f32 %0, 0f00000000, %1;"
            : "=h"(_sf_pair)
            : "f"(raw_residual_scale));
        *(reinterpret_cast<unsigned char*>(v_scale_residual_lo +
                                           (scale_tile_offset + (long long)scale_offset)) +
          (0)) = (unsigned char)(_sf_pair & 0x7F);
      }
    } else {
      {
        ...
        asm("cvt.rn.satfinite.e4m3x2.f32 %0, 0f00000000, %1;"
            : "=h"(_sf_pair)
            : "f"(raw_base_scale));
        *(reinterpret_cast<unsigned char*>(v_scale_base_hi +
                                           (scale_tile_offset + (long long)scale_offset)) +
          (0)) = (unsigned char)(_sf_pair & 0x7F);
      }
      {
        ...
        asm("cvt.rn.satfinite.e4m3x2.f32 %0, 0f00000000, %1;"
            : "=h"(_sf_pair)
            : "f"(raw_residual_scale));
        *(reinterpret_cast<unsigned char*>(v_scale_residual_hi +
                                           (scale_tile_offset + (long long)scale_offset)) +
          (0)) = (unsigned char)(_sf_pair & 0x7F);
      }
    }
