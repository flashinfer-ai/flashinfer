...
__global__ __launch_bounds__(512, 1) void kernel_mamba_ssd_q_tmem_alias_bf16_batched(
    ...
    int* __restrict__ seq_chunk_cumsum, __nv_bfloat16* __restrict__ out_native, int nheads,
    int ngroups, int batch, int seqlen, int nchunks, int sequence_count, int num_logical_chunks,
    int mode_varlen, int has_seq_chunk_cumsum, int seq_idx_int64, int D_mode, int has_z,
    int has_initial, int dt_softplus, float dt_min, float dt_max, int write_final_states) {
...
#pragma unroll
              for (int state_local_3 = 0; state_local_3 < 16; state_local_3++) {
                ...
                final_states[state_head_base + dim_1 * 128 + state_row_1] =
...
#pragma unroll
            for (int dim_chunk_4 = 0; dim_chunk_4 < 2; dim_chunk_4++) {
              ...
              tmem_ld_x32(&_tmem_load_2[0], taddr_0_2 + row_tmem_base_1 + 320 + dim_chunk_4 * 32);
              asm volatile("tcgen05.wait::ld.sync.aligned;");
              float _tmem_load_3[32];
              tmem_ld_x32(&_tmem_load_3[0], taddr_0_2 + row_tmem_base_1 + 448 + dim_chunk_4 * 32);
              asm volatile("tcgen05.wait::ld.sync.aligned;");
...
#pragma unroll
                for (int local_pair = 0; local_pair < 16; local_pair++) {
                  int dim_pair_base = dim_chunk_4 * 32 + local_pair * 2;
...
#pragma unroll
                  for (int pair_lane = 0; pair_lane < 2; pair_lane++) {
                    ...
                    int dim_2 = dim_pair_base + pair_lane;
                    float _fma_0 =
                    ...
                    if (D_mode == 2) {
                      float _fma_2 = __fmaf_rn(x_value, (float)D[head_3 * 64 + dim_2], value);
                      value = _fma_2;
                    }
                    if (has_z != 0) {
                      float z_value = (float)z[token * nheads * 64 + head_3 * 64 + dim_2];
                      float _expf_0 = __expf(-z_value);
                      float _rcp_0 = approx_rcp(1.0f + _expf_0);
                      value *= z_value * _rcp_0;
                    }
                    int out_index = (((physical_batch_2 * nheads + head_3) * 64 + dim_2) * nchunks +
                                     physical_chunk_6) *
                                        128 +
                                    row_1;
                    {
                      {
                        ...
                        uint32_t _addr_0 = static_cast<uint32_t>(
                            (smem_y_addr + output_stage * 16384 +
                             (unsigned int)((row_1 ^ 32) / 64 * 8192 + dim_2 * 128 +
                                                (row_1 ^ 32) % 64 * 2 ^
                                            ((row_1 ^ 32) / 64 * 8192 + dim_2 * 128 +
                                                     (row_1 ^ 32) % 64 * 2 >>
                                                 7 &
                                             7) << 4)));
