...
__global__ __launch_bounds__(128) void kernel_factorized_persistent_segment_preprocess(
  ...
  if (tile < total_tiles) {
    ...
    int start = segment_starts[segment];
    int length = segment_lengths[segment];
