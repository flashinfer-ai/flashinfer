...
#define NUM_INPUT_PIPE_STAGES 1
...
kernel_mamba_ssd_direct_preprocess_warp_sync_1212_f16_varlen(const __grid_constant__ CUtensorMap x_map, const __grid_constant__ CUtensorMap b_map, const __grid_constant__ CUtensorMap c_map, const __grid_constant__ CUtensorMap out_map, __nv_bfloat16* __restrict__ x, float* __restrict__ dt, __nv_bfloat16* __restrict__ delta_precomputed, float* __restrict__ cumsum_precomputed, float* __restrict__ A, __nv_bfloat16* __restrict__ B_tensor, __nv_bfloat16* __restrict__ C, __nv_bfloat16* __restrict__ D, __nv_bfloat16* __restrict__ z, float* __restrict__ dt_bias, __half* __restrict__ initial_states, __half* __restrict__ final_states, __half* __restrict__ checkpoint_states, int* __restrict__ checkpoint_token_indices, int* __restrict__ checkpoint_state_slots, int* __restrict__ seq_idx_i32, long long* __restrict__ seq_idx_i64, int* __restrict__ chunk_indices, int* __restrict__ chunk_offsets, int* __restrict__ seq_chunk_cumsum, __nv_bfloat16* __restrict__ out_native, int nheads, int ngroups, int batch, int seqlen, int nchunks, int sequence_count, int num_logical_chunks, int mode_varlen, int has_seq_chunk_cumsum, int seq_idx_int64, int D_mode, int has_z, int has_initial, int dt_softplus, float dt_min, float dt_max, int write_final_states, int checkpoint_state_count)
{
    ...
    __nv_bfloat16* smem_x_tma = reinterpret_cast<__nv_bfloat16*>(smem_raw + 66560);
    const int smem_x_tma_addr = smem + 66560;
    __nv_bfloat16* smem_x = reinterpret_cast<__nv_bfloat16*>(smem_raw + 66560);
    const int smem_x_addr = smem + 66560;
    __nv_bfloat16* smem_scaled_b = reinterpret_cast<__nv_bfloat16*>(smem_raw + 82944);
    ...
    if (warp == 0) {
        ...
        if (leader) {
            // --- pipeline 'input_pipe' ---
            // bc_full: 1 barriers, init_count=1
    ...
    if (warp <= 3) {
        ...
        { // pre_inter_main
            ...
            for (unsigned int work = bid; work < sequence_count * nheads; work += num_bids) {
                ...
                int head = work % (unsigned int)nheads;
                int state_head_base = (sequence * nheads + head) * 64 * 128;
                #pragma unroll
                ...
                int _uniform_10 = make_warp_uniform(first_logical);
                first_logical = _uniform_10;
                int _uniform_11 = make_warp_uniform(logical_end);
                logical_end = _uniform_11;
                #pragma unroll 1
                for (int logical = first_logical; logical < logical_end; logical++) {
                    ...
                    segment_offset = chunk_offsets[logical];
                    if (logical + 1 < num_logical_chunks) {
                        int next_chunk = chunk_indices[logical + 1];
                        if (next_chunk == physical_chunk) {
                            segment_limit = chunk_offsets[logical + 1];
                        }
                    }
                    {
                        #pragma unroll
                        ...
                        for (int b_col_iter = 0; b_col_iter < 16; b_col_iter++) {
                            ...
                            if (col >= segment_offset && col < segment_limit) {
                                ...
                                asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                    ...
                                    : "r"((smem_b_addr + input_stage * 32768 + (unsigned int)(b_state_base / 64 * 16384 + col * 128 + b_state_base % 64 * 2 ^ (b_state_base / 64 * 16384 + col * 128 + b_state_base % 64 * 2 >> 7 & 7) << 4))));
                                #pragma unroll
                        ...
                        if (elect_sync()) {
                            ...
                            mbarrier_arrive(bc_empty_addr + (input_stage) * 8);
                        }
                        ...
                        if (elect_sync()) {
                            mbarrier_arrive(aux_empty_addr + (input_stage) * 8);
                        }
                        input_stage += 1;
                        if (input_stage == 1) { input_stage = 0; _phase_aux_full ^= 1; _phase_bc_full ^= 1; }
                    }
                    if (checkpoint_state_count > 0) {
                        int checkpoint_token = checkpoint_token_indices[sequence];
                        int segment_end = physical_chunk * 128 + segment_limit;
                        if (checkpoint_token == segment_end) {
                            ...
                            if (checkpoint_slot >= 0 && checkpoint_slot < checkpoint_state_count) {
                                int checkpoint_head_base = (checkpoint_slot * nheads + head) * 64 * 128;
                                #pragma unroll
        ...
        { // mma_inter_main
            ...
            for (unsigned int work_1 = bid; work_1 < sequence_count * nheads; work_1 += num_bids) {
                ...
                for (int logical_1 = first_logical_1; logical_1 < logical_end_1; logical_1++) {
                    ...
                    asm volatile("tcgen05.fence::after_thread_sync;");
                    int _mma_a_lo_2 = make_warp_uniform((((smem_c_addr) >> 4) & 0x3FFF) + (input_stage_1) * 2048);
                    ...
                    mbarrier_wait(x_full_addr + (input_stage_1) * 8, _phase_x_full);
                    asm volatile("tcgen05.fence::after_thread_sync;");
                    int _mma_a_lo_3 = make_warp_uniform((((smem_scaled_b_addr) >> 4) & 0x3FFF) + (0) * 2048);
                    int _mma_b_lo_3 = make_warp_uniform(((((smem_x_addr) >> 4) & 0x3FFF) | 0x4000000) + (input_stage_1) * 1024);
                    asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
                    ...
                    input_stage_1 += 1;
                    if (input_stage_1 == 1) { input_stage_1 = 0; _phase_bc_full_1 ^= 1; _phase_x_full ^= 1; }
                }
            }
        }
    }
    ...
    if (warp == 5) {
        { // mma_intra_main
            ...
            for (unsigned int work_2 = bid; work_2 < sequence_count * nheads; work_2 += num_bids) {
                ...
                int _uniform_4 = make_warp_uniform(first_logical_2);
                first_logical_2 = _uniform_4;
                int _uniform_5 = make_warp_uniform(logical_end_2);
                logical_end_2 = _uniform_5;
                #pragma unroll 1
                for (int logical_2 = first_logical_2; logical_2 < logical_end_2; logical_2++) {
                    ...
                    mbarrier_wait(bc_full_addr + (input_stage_2) * 8, _phase_bc_full_2);
                    mbarrier_wait(cb_empty_addr + (acc_stage) * 8, _phase_cb_empty);
                    asm volatile("tcgen05.fence::after_thread_sync;");
                    int _mma_a_lo_0 = make_warp_uniform((((smem_c_addr) >> 4) & 0x3FFF) + (input_stage_2) * 2048);
                    int _mma_b_lo_0 = make_warp_uniform((((smem_b_addr) >> 4) & 0x3FFF) + (input_stage_2) * 2048);
                    asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
                    ...
                    elect_commit(bc_empty_addr + (input_stage_2) * 8);
                    mbarrier_wait(q_full_addr, _phase_q_full_0);
                    _phase_q_full_0 ^= 1;
                    mbarrier_wait(outputs_empty_addr, _phase_outputs_empty_0_1);
                    _phase_outputs_empty_0_1 ^= 1;
                    mbarrier_wait(x_full_addr + (input_stage_2) * 8, _phase_x_full_1);
                    asm volatile("tcgen05.fence::after_thread_sync;");
                    int _mma_b_lo_1 = make_warp_uniform(((((smem_x_addr) >> 4) & 0x3FFF) | 0x4000000) + (input_stage_2) * 1024);
                    asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ...
                    elect_commit(x_empty_addr + (input_stage_2) * 8);
                    input_stage_2 += 1;
                    if (input_stage_2 == 1) { input_stage_2 = 0; _phase_bc_full_2 ^= 1; _phase_x_full_1 ^= 1; }
                    acc_stage += 1;
    ...
    if (warp == 6) {
        { // load_bc_main
            ...
            for (unsigned int work_3 = bid; work_3 < sequence_count * nheads; work_3 += num_bids) {
                ...
                int group = head_1 * ngroups / nheads;
                ...
                int _uniform_0 = make_warp_uniform(first_logical_3);
                first_logical_3 = _uniform_0;
                int _uniform_1 = make_warp_uniform(logical_end_3);
                logical_end_3 = _uniform_1;
                #pragma unroll 1
                for (int logical_3 = first_logical_3; logical_3 < logical_end_3; logical_3++) {
                    ...
                    mbarrier_wait(bc_empty_addr + (input_stage_3) * 8, _phase_bc_empty);
                    int physical_batch = 0;
                    int token_in_batch = physical_chunk_3 * 128;
                    if (elect_sync()) {
                        tma_5d_gmem2smem(smem_b_addr + input_stage_3 * 32768, (&b_map), 0, 0, group, token_in_batch, physical_batch, bc_full_addr + (input_stage_3) * 8);
                        tma_5d_gmem2smem(smem_b_addr + input_stage_3 * 32768 + 16384, (&b_map), 0, 1, group, token_in_batch, physical_batch, bc_full_addr + (input_stage_3) * 8);
                        tma_5d_gmem2smem(smem_c_addr + input_stage_3 * 32768, (&c_map), 0, 0, group, token_in_batch, physical_batch, bc_full_addr + (input_stage_3) * 8);
                        tma_5d_gmem2smem(smem_c_addr + input_stage_3 * 32768 + 16384, (&c_map), 0, 1, group, token_in_batch, physical_batch, bc_full_addr + (input_stage_3) * 8);
                        mbarrier_arrive_expect_tx(bc_full_addr + (input_stage_3) * 8, 65536);
                    }
                    input_stage_3 += 1;
                    if (input_stage_3 == 1) { input_stage_3 = 0; _phase_bc_empty ^= 1; }
                }
            }
        }
    }
    ...
    if (warp == 7) {
        { // load_x_dt_main
            ...
            for (unsigned int work_4 = bid; work_4 < sequence_count * nheads; work_4 += num_bids) {
                ...
                int _uniform_2 = make_warp_uniform(first_logical_4);
                first_logical_4 = _uniform_2;
                int _uniform_3 = make_warp_uniform(logical_end_4);
                logical_end_4 = _uniform_3;
                #pragma unroll 1
                for (int logical_4 = first_logical_4; logical_4 < logical_end_4; logical_4++) {
                    ...
                    mbarrier_wait(aux_empty_addr + (input_stage_4) * 8, _phase_aux_empty);
                    mbarrier_wait(x_empty_addr + (input_stage_4) * 8, _phase_x_empty);
                    int physical_batch_1 = 0;
                    int token_in_batch_1 = physical_chunk_4 * 128;
                    int physical_start = physical_batch_1 * seqlen + token_in_batch_1;
                    if (elect_sync()) {
                        tma_4d_gmem2smem(smem_x_tma_addr, (&x_map), 0, head_2, token_in_batch_1, physical_batch_1, x_full_addr + (input_stage_4) * 8);
                        mbarrier_arrive_expect_tx(x_full_addr + (input_stage_4) * 8, 16384);
                    }
                    int tile = logical_4 * nheads + head_2;
                    ...
                    for (int local = 0; local < 4; local++) {
                        ...
                        smem_cumsum_all[input_stage_4 * 128 + (unsigned int)physical_token] = cumsum_value;
                        smem_delta_all[input_stage_4 * 128 + (unsigned int)physical_token] = delta_value;
                    }
                    __syncwarp();
                    if (elect_sync()) {
                        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            ...
            for (unsigned int work_5 = bid; work_5 < sequence_count * nheads; work_5 += num_bids) {
                ...
                for (int logical_5 = first_logical_5; logical_5 < logical_end_5; logical_5++) {
                    ...
                    }
                    {
                        ...
                        input_stage_5 += 1;
                        if (input_stage_5 == 1) { input_stage_5 = 0; _phase_aux_full_1 ^= 1; }
                        acc_stage_1 += 1;
            ...
            for (unsigned int work_6 = bid; work_6 < sequence_count * nheads; work_6 += num_bids) {
                ...
                for (int logical_6 = first_logical_6; logical_6 < logical_end_6; logical_6++) {
                    ...
                    }
                    {
                        ...
                        input_stage_6 += 1;
                        if (input_stage_6 == 1) { input_stage_6 = 0; _phase_aux_full_2 ^= 1; _phase_x_full_2 ^= 1; }
                    }
                }
            }
        }
