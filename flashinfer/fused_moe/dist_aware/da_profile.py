...
def _robust_per_tile_tactics(
    latencies: Dict[Tuple[int, int], Dict[Tuple[int, int], float]],
    distribution_count: int,
) -> Dict[int, Dict[int, Tuple[int, int]]]:
    """Choose fixed tile tactics that were measured on every distribution."""

    expected_distributions = set(range(int(distribution_count)))
    per_tactic: Dict[int, Dict[Tuple[int, int], Dict[int, float]]] = {}
    for (bucket, distribution), tactic_latencies in latencies.items():
        for tactic, latency in tactic_latencies.items():
            per_tactic.setdefault(int(bucket), {}).setdefault(
                (int(tactic[0]), int(tactic[1])), {}
            )[int(distribution)] = float(latency)

    selected: Dict[int, Dict[int, Tuple[int, int]]] = {}
    for bucket, tactic_distributions in per_tactic.items():
        per_tile_scores: Dict[int, list[Tuple[float, float, int]]] = {}
        for (tile, config), distribution_latencies in tactic_distributions.items():
            if set(distribution_latencies) != expected_distributions:
                continue
            values = list(distribution_latencies.values())
            # A DA body is fixed across distributions. Prefer the tactic with
            # the lowest worst-distribution latency, then its mean latency;
            # the config index makes exact ties deterministic. Requiring the
            # complete distribution set also gives the post-selection guard
            # matched timings without changing its candidate assignments.
            per_tile_scores.setdefault(tile, []).append(
                (max(values), float(np.mean(values)), config)
            )
        for tile, scores in per_tile_scores.items():
            _worst, _mean, config = min(scores)
            selected.setdefault(bucket, {})[tile] = (tile, config)
    return selected
...
def populate_per_tile_tactics_from_autotune(
    ...
    *,
    ...
    robust_tiles = _robust_per_tile_tactics(
        cached_profile_tactic_latencies(
            da_context,
            da_context.top_k,
            runner_hash=runner_hash,
            config=config,
        ),
        len(active_auto_distributions(config)),
    )
    for bucket, tile_map in robust_tiles.items():
        per_bucket_tiles.setdefault(bucket, {}).update(tile_map)
    ...
    if verbose:
        ...
        print(
            ...
            f"sample_tiles={sample} robust_buckets={sorted(robust_tiles.keys())[:8]}",
            flush=True,
        )
