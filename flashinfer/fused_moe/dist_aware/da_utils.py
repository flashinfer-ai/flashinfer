...
def exp_floor_probs_for_target_eff(target_eff: float, n_experts: int) -> np.ndarray:
    """Return deterministic exponential-floor probabilities at a target skew."""
...
def symmetric_dirichlet_probs_for_target_eff(
    target_eff: float, n_experts: int
) -> np.ndarray:
    """Return a fixed-seed symmetric-Dirichlet sample at a target skew."""
    target_eff = _clamp_effective_experts(target_eff, n_experts)

    def expected(alpha: float) -> float:
        n = float(n_experts)
        second = (alpha + 1.0) / (n * alpha + 1.0)
        floor = _EXP_FLOOR_FRAC
        return float(1.0 / ((1.0 - floor) ** 2 * second + (2.0 * floor - floor**2) / n))
...
def sparse_probs(kind: str, param: Any, num_local_experts: int) -> np.ndarray:
    """Return probabilities with an explicitly bounded active-expert set."""
    active, target_eff = _sparse_active_eff(kind, param, num_local_experts)
    probs = np.zeros(num_local_experts, dtype=np.float64)
    probs[:active] = symmetric_dirichlet_probs_for_target_eff(target_eff, active)
    return probs
...
def da_distribution_target_effective_experts(
    ...
    """Resolve a synthetic distribution specification to effective experts."""
...
def generate_da_distribution_assignments(
    ...
    if kind == "uniform":
        ...
        probs = sparse_probs(kind, param, num_local_experts)
    else:
        raise ValueError(f"Unknown DA distribution kind: {kind!r}")
