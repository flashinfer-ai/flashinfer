...
def _concrete_cuda_device(device: torch.device) -> torch.device:
    """Resolve an unindexed CUDA device against the current CUDA context."""
    device = torch.device(device)
    if device.type != "cuda":
        raise ValueError(f"DA capture requires a CUDA device, got {device}")
...
def _validated_capture_streams(
    *,
    ...
    device = _concrete_cuda_device(device)
    for name, stream in (
        ("outer", outer_stream),
        ("side", side_stream),
        ...
        if stream_device is not None and torch.device(stream_device) != device:
            raise RuntimeError(
                f"DA {name}_stream device {stream_device} does not match {device}"
            )
