...
def resolve_static_fallback(
    ...
    *,
    ...
    if tactic != -1 or not execution.config.enabled or tuner.is_tuning_mode:
        return tactic
    call = execution.invocation
    da_bucket = upload_bucket(call.num_tokens, call.tune_max_num_tokens)
    decision = da_state.BASELINE_GUARD_DECISIONS.get(
        da_state.cache_key(call.da_context, da_bucket), {}
    )
    final_policy = decision.get("final_policy", decision.get("policy"))
    if final_policy == "noda_baseline_guard":
        baseline_tactic = decision.get("baseline_tactic")
        if baseline_tactic is None or len(baseline_tactic) != 2:
            raise RuntimeError(
                "guarded NoDA fallback is missing its recorded baseline tactic "
                f"for DA bucket {da_bucket}"
            )
        tactic = tuple(int(value) for value in baseline_tactic)
        if execution.config.verbose:
            print(
                f"[DA guarded-static-fallback] op={custom_op} "
                f"bucket={da_bucket} tactic={tactic}",
                flush=True,
            )
        return tactic

    # Ordinary eager misses belong to the shape-only autotuner domain, whose
    # token buckets use the public runner's hybrid ceil mapper.
    profile_bucket = map_to_hybrid_bucket(
        int(call.num_tokens), int(call.tune_max_num_tokens)
    )
...
def tune_and_publish_tactic(
    ...
    **kwargs: Any,
    ...
    if execution.config.enabled:
        tuning_kwargs, value_specs_active = tuning_config_kwargs(
            execution,
            tuner,
            pack_topk_ids=pack_topk_ids,
        )
    ...
    kwargs.setdefault("routing_input_mode", execution.invocation.routing_input_mode)
    _, tactic = choose_one(
        execution,
        tuner,
        custom_op=custom_op,
        ...
        inputs=runner_inputs.to_list(),
        da_value_specs_active=value_specs_active,
        **kwargs,
    )
    tactic = resolve_static_fallback(
        execution,
        tuner,
        custom_op=custom_op,
        runner=runner,
    ...
    if value_specs_active and tuner.is_tuning_mode:
        maybe_prepare_bundle(execution, tuner)
    return normalize_tactic(execution, tactic)
