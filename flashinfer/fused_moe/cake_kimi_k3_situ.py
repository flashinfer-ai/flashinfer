...
def _workspace_layout(num_tokens, arch=None):
    ...
    if num_tokens in (8, 16) or (num_tokens in (32, 64, 128) and arch in (None, "sm_100a", "sm_103a")) or (num_tokens == 256 and arch in (None, "sm_100a", "sm_103a")) or (num_tokens in (512, 1024) and arch in (None, *_N32_CLAIM8_ARCHES)):
...
def cutlass_fused_moe_prepare_workspace(
    ...
    with torch.cuda.device(workspace_buffer.device):
        ...
        state["shapes"][num_tokens] = {
            ...
            "feature_finalize": feature_finalize, "m64_claim8": m64_claim8, "mid_work5fd": mid_work5fd, "n32_claim8": n32_claim8, "m256_c12": m256_c12,
            "fc2_device_workfeed": fc2_device_workfeed, "fc2_grid_n": fc2_grid_n,
...
def _cake_situ_stage_bindings(options, prepared):
    ...
    if num_tokens == 1:
        stages = {
            "quant_route": dict(
                grid=(1, 1, 1), x=x, qx=qx,
                packed=views["x_packed"], scales=views["x_scales"],
                topk_ids=flat_ids,
                **{name: views[name] for name in (
                    "expert_counts", "expert_tile_offsets", "expert_scatter_offsets",
                    "route_map", "token_to_permuted", "tile_expert",
                    "tile_mn_limit", "total_tiles")}, M=num_tokens),
        }
        ...
        stages = {
            ...
            "fused_router": dict(
                grid=(1, 1, 1), topk_ids=flat_ids,
                **{name: views[name] for name in (
                    "expert_counts", "expert_tile_offsets", "expert_scatter_offsets",
                    "route_map", "token_to_permuted", "tile_expert",
                    "tile_mn_limit", "total_tiles")},
                total_pairs=total_pairs, num_experts=_E, max_tiles=max_tiles,
                top_k=_TOP_K, tile_n=tile_n,
    ...
    stages.update({
        ...
        "fc2": dict(grid=(_H // 128, prepared["fc2_grid_n"], 1), A=w2,
            ...
            **({"num_non_exiting_ctas": views["total_tiles"],
               ...
               if fc2_device_workfeed else {"total_tiles": views["total_tiles"]}),
            M=_H, K=_I, grid_m=_H // 128, grid_n=max_tiles,
            K_tiles=1 if prepared["m64_claim8"] or prepared["n32_claim8"] else 2),
        "finalize": dict(grid=(num_tokens, 1 if prepared["n32_claim8"] else 4 if prepared["feature_finalize"] else
            (_H + 128 * (2 if tile_n == 8 else 8) - 1) // (128 * (2 if tile_n == 8 else 8)), 1), expert_output=views["expert_output"],
