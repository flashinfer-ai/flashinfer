...
class SkinnyDecodeGatherKernel:
    def __init__(
        ...
        *,
        topk=16,
        runtime_situ_linear_beta=False,
    ...
    def wrapper(
        ...
        weight_scales = cute.make_tensor(
            cute.recast_ptr(b_sf_ptr, dtype=cutlass.Uint8),
            cute.make_layout(l * n * k // 32),
        )
        x = cute.make_tensor(a_ptr, cute.make_layout((orig_m, k), stride=(k, 1)))
        xs = cute.make_tensor(
            cute.recast_ptr(a_sf_ptr, dtype=cutlass.Uint8),
        ...
        beta = cute.make_tensor(
            situ_beta_ptr, cute.make_layout(l, stride=situ_beta_stride)
        )
        linear = None
        if cutlass.const_expr(self.runtime_situ_linear_beta):
            linear = cute.make_tensor(
    ...
    def _kernel_body(
        ...
        b_nks = storage.b.get_tensor(
            cute.composition(
                bl.outer, cute.make_layout((8, 512, 3), stride=(1, 8, 4096))
            ),
            swizzle=bl.inner,
        )
        sfa = storage.sa.get_tensor(sal)
        raw_sfa = cute.make_tensor(
            cute.recast_ptr(sfa.iterator, dtype=cutlass.Uint8), cute.make_layout(6144)
        )
        raw_sfb = storage.sb.get_tensor(cute.make_layout(384))
        ...
        gates = storage.gates.get_tensor(cute.make_layout((8, 64), stride=(64, 1)))
        ...
        if warp == 11:
            ...
            while work[0] >= 0:
                ...
                for kt in cutlass.range(k // 512, unroll=1):
                    ...
                    if kt + 3 < k // 512:
                        cute.prefetch(
                            tma, ga_copy[(None, bm, cutlass.Int32(kt + 3), expert)]
                        )
                    a_state.advance()
                self.read_work(mp, meta_a, records, work)
                meta_a.advance()
                    ...
                    for vr in cutlass.range_constexpr(4):
                        ...
                        if cutlass.const_expr(self.enable_b_vector_load):
                            ...
                            source = cute.make_tensor(
                                x.iterator + offset, cute.make_layout((16,))
                            )
                            ...
                            if row_base + row < limit:
                                ...
                                cute.autovec_copy(
                                    cute.local_tile(
                                        x, (1, 16), (token, kt * 32 + chunk)
                                    ),
                                    values,
                                )
                            cute.autovec_copy(
                                values, cute.local_tile(b_nk, (1, 16), (row, chunk))
                            )
                    cute.arch.fence_proxy("async.shared", space="cta")
            ...
            while work[0] >= 0:
                ...
                for kt in cutlass.range(k // 512, unroll=1):
                    ...
                    for kc in cutlass.range_constexpr(4):
                        ...
                        source = cute.make_tensor(
                            ws.iterator + global_offset, cute.make_layout((16,))
                        )
                        destination = cute.make_tensor(
                            raw_sfa.iterator + shared_offset, cute.make_layout((16,))
                        )
        ...
        # Warp 10: exactly 128 SFB bytes/K512, no padded 128-row expansion.
        if warp == 10:
            ...
            while work[0] >= 0:
                ...
                for kt in cutlass.range(k // 512, unroll=1):  # noqa: B007
                    ...
                    for kc in cutlass.range_constexpr(4):
                        ...
                        if lane < 8:
                            word = compact_words[sb_cons.index * 32 + kc * 8 + lane].to(
                                cutlass.Uint32
                            )
                        registers.fill(word)
            ...
            while work[0] >= 0:
                ...
                if tid < 64:
                    ...
                    for vi in cutlass.range_constexpr(0, cute.size(values), 2):
                        ...
                        activated[vi], activated[vi + 1] = cute.arch.mul_packed_f32x2(
                            (u0, u1), (v0, v1)
                        )
                    ...
                    for vi in cutlass.range_constexpr(0, cute.size(values), 2):
                        channel0, slot0 = coords[vi]
                        channel1, slot1 = coords[vi + 1]
                        max0 = cute.arch.warp_redux_sync(
                            abs(scale_values[vi]),
                            kind="fmax",
                            mask_and_clamp=0xFFFFFFFF,
                            nan=False,
                        )
                        max1 = cute.arch.warp_redux_sync(
                            abs(scale_values[vi + 1]),
                            kind="fmax",
                            mask_and_clamp=0xFFFFFFFF,
                            nan=False,
                        )
                        ...
                        code0, code1 = (
                            float_to_ue8m0_fast(scale0),
                            float_to_ue8m0_fast(scale1),
                        )
