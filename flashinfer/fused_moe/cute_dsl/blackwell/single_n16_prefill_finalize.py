...
from .utils import (
    UnalignedNamedBarrier, blk_reduce_bf16, blk_copy,
...
class SingleN16PrefillFinalizeKernel:
    ...
    def clamped_b_partition(self, atom, mapped, tma_shared, row_base, limit):
        valid = cutlass.min(cutlass.Int32(16),
    ...
    def wrapper(
        ...
        output_tokens = num_tokens * top_k if cutlass.const_expr(self.write_expanded_weighted) else num_tokens
        out = cute.make_tensor(c_ptr, cute.make_layout((output_tokens, n), stride=(n, 1)))
        ...
        assert Storage.size_in_bytes() == self.expected_shared_storage_bytes
        self.shared_storage = Storage
        self.kernel(mma, tma, ma, sfa_tma, sfa_ma, b_tma, b_mapped, x, xs, routes, route_expert,
    ...
    @cute.kernel
    def kernel(self, mma, tma, ma, sfa_tma, sfa_ma, b_tma, b_mapped, x, xs, routes, route_expert,
               route_limit, active_tiles, alpha, route_weights, out,
               al, bl, sal, sbl):
                    ...
                    for kc in cutlass.range_constexpr(4):
                        for row_half in cutlass.range_constexpr(2):
                            ...
                            if row_base + row < limit:
                                r = row_base + row
                                value = xs[(r % 32, (r % 128) // 32, r // 128,
                                            g, kt * 4 + kc, 0)].to(cutlass.Uint8)
            ...
            while work[0] >= 0:
                ...
                for row_group in cutlass.range_constexpr(2):
                    for vi in cutlass.range_constexpr(cute.size(values)):
                        ...
                        if slot >= row_group * 8 and slot < (row_group + 1) * 8:
                            if row_base + slot < limit:
                                output_rows[(slot - row_group * 8, channel)] = (scale * values[vi]).to(cutlass.BFloat16)
                    cute.arch.fence_proxy("async.shared", space="cta")
                    ...
                    if cutlass.const_expr(row_group == 0):
                        ...
                        acc_cons.advance()
                    group_token = cute.arch.shuffle_sync(output_row, cutlass.Int32(lane + row_group * 8) % 32)
                    if tid < 8:
                        if row_base + row_group * 8 + tid < limit:
                            destination = cute.domain_offset((group_token, work[0] * 128), out)
                            if cutlass.const_expr(self.write_expanded_weighted):
                                blk_copy(destination, output_rows[tid, None], cutlass.Int32(256))
                            else:
