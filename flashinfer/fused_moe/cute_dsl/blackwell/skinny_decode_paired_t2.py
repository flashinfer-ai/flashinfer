    ...
    def wrapper(
        ...
        class Storage:
            ...
            sb: cute.struct.Align[
                cute.struct.MemRange[cutlass.Uint8, self.sfb_shared_bytes], 128
            ]
    ...
    def _kernel_body(
        ...
        tbp = pipeline.PipelineAsyncUmma.create(
            ...
            producer_group=pipeline.CooperativeGroup(
                pipeline.Agent.Thread, self.sfb_publisher_threads
            ),
            consumer_group=pipeline.CooperativeGroup(pipeline.Agent.Thread, 1),
            cta_layout_vmnk=None,
            defer_sync=True,
                    ...
                    if cutlass.const_expr(self.enable_sfb_cp):
                        ...
                        for half in cutlass.range_constexpr(2):
                            ...
                            target = cute.make_tensor(
                                cute.recast_ptr(
                                    acc_ptr + 64 + tb_prod.index * 8 + half * 4,
                                    dtype=cutlass.Uint32,
                                ),
                                cute.make_layout(
                                    ((32, 4), 4), stride=((65536, 32 * 65536), 1)
                                ),
                            )
                            atom = cute.make_copy_atom(
                                tcgen05.Cp4x32x128bOp(self.cta_group), cutlass.Uint32
                            )
                            copy = tcgen05.make_s2t_copy(atom, target)
                            part = copy.get_slice(0)
                            source_desc = tcgen05.get_s2t_smem_desc_tensor(
                                copy, part.partition_S(source)
                            )
                        ...
                        for kc in cutlass.range_constexpr(4):
                            ...
                            registers = cute.make_rmem_tensor(
                                coords.shape, cutlass.Uint32
                            )
                            assert cute.size(registers) == 1
                            word = cutlass.Uint32(0)
                            if lane < 8:
                                word = compact_words[
                                    sb_cons.index * 32 + kc * 8 + lane
                                ].to(cutlass.Uint32)
                            registers.fill(word)
        ...
        if warp < 4:
            ...
            while work[0] >= 0:
                ...
                tc = tcgen05.make_tmem_copy(
                    cute.make_copy_atom(
                        tcgen05.Ld16x256bOp(tcgen05.Repetition.x1), cutlass.Float32
                    ),
                    acc_mn,
                )
                ...
                for pair in cutlass.range_constexpr(2):
                    ...
                    activated[2 * pair], activated[2 * pair + 1] = (
                        cute.arch.mul_packed_f32x2((u0, u1), (v0, v1))
                    )
                ...
                for shift in cutlass.range_constexpr(3):
                    offset = 16 >> shift
                    max0 = cute.arch.fmax(
                        max0, cute.arch.shuffle_sync_bfly(max0, offset=offset)
                    )
                    max1 = cute.arch.fmax(
                        max1, cute.arch.shuffle_sync_bfly(max1, offset=offset)
                    )
                slot = 2 * (lane % 4)
                if lane < 4:
