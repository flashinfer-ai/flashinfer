...
from .mxfp4_combine import _plan_expanded_weighted_combine
...
class Mxfp4MoEPlan:
    ...
    def __init__(
        ...
        *,
        ...
        self._kwargs = kwargs
        self._fused_prefill_sort = _fused_prefill_sort
    ...
    def _prepare(self):
        ...
        expanded = self._kwargs.get("_expanded_weighted_output")
        if expanded is not None:
            self._expanded_combine = _plan_expanded_weighted_combine(
                expanded,
        ...
        if decode or prefill_route_conversion:
            self._route_preprocess = _plan_route_preprocess(
                ...
                _fused_prefill_sort=self._fused_prefill_sort,
                _clear_output=self._expanded_combine is None,
            )
...
class CuteDslMxfp4MoEWrapper:
    ...
    def plan(
        ...
        *,
        ...
        paired_decode_weights: Optional[tuple[torch.Tensor, torch.Tensor]] = None,
        workspace: torch.Tensor,
        output: torch.Tensor,
    ) -> Mxfp4MoEPlan:
        """Bind buffers and prepare kernels; all tensor contents must be valid.

        Weights use ``prepare_cute_dsl_mxfp4_weights`` layouts. Optionally pass
        the exact pair returned by ``prepare_cute_dsl_mxfp4_decode_weights`` as
        ``paired_decode_weights``. Planning selects it for the supported B300
        T=2/4/16 SiTU decode tactic (H=7168, I=3072, local E=112, top_k=16);
        other plans use the four canonical prepared weights. Omission keeps
        the canonical path. Selection copies no weights. Refresh both prepared
        representations together when changing weights, then create new plans.
        ...
        with torch.cuda.device(x.device):
            ...
            # The optional representation is prepared explicitly at load time.
            # Select before validation/overlap checks and before binding pointers.
            if (
                paired_decode_weights is not None
                and decode_specialization
                and num_tokens in (2, 4, 16)
                and self.hidden_size == 7168
                and self.intermediate_size == 3072
                and self.num_local_experts == 112
                and self.top_k == 16
                ...
                if (
                    not isinstance(paired_decode_weights, tuple)
                    or len(paired_decode_weights) != 2
                    or any(
                        not isinstance(tensor, torch.Tensor)
                        or getattr(tensor, "_private_paired_t2_layout", None)
                        != "paired_t2_w1_sf32"
                        for tensor in paired_decode_weights
                    )
                ):
                    raise ValueError(
                        "paired_decode_weights must be the exact pair returned by "
                        "prepare_cute_dsl_mxfp4_decode_weights"
                    )
                w1, w1_sf = paired_decode_weights
            expected = {
            ...
            # Prefill's device tile limits select the existing narrow A layout.
            # Keep this provenance separate from static decode routing/finalize.
            sparse_prefill_narrow_a = (
                (major, minor) == (10, 3)
                and num_tokens > 16
                and self.activation_type == ActivationType.Situ
                and not self.enable_pdl
            ...
            # Device row limits choose the T512 G1 epilogue.
            ...
            write_expanded_weighted = (
                (major, minor) == (10, 3)
                and "expanded_contributions_bf16" in buffers
                and sparse_prefill_narrow_a
                and self.activation_type == ActivationType.Situ
                and gemm1 == ((128, 128), (1, 1), False)
            )
            ...
            plan = Mxfp4MoEPlan(
                ...
                _fused_prefill_sort=fused_prefill_sort,
            )
            plan._prepare()
            return plan
