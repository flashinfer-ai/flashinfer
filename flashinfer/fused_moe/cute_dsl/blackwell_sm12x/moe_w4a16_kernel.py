    ...
    def kernel(
        ...
        if cta == Int32(0):
            ...
            if tid < routed_rows:
                ...
                shared[Int32(64) + route_expert * Int32(self.block_size) + slot] = tid
            cute.arch.sync_threads()
...
def _w4a16_fused_persistent_grid_x(
    *,
    ...
    if bool(getattr(fused, "schedule_whole_tiles", False)):
        ...
        return min(
            range(1, int(cap) + 1),
            key=lambda grid: (whole_tile_critical_path(grid), grid),
        )
...
@torch.library.custom_op(
    "flashinfer::w4a16_topk_sum_launch",
    mutates_args=("output",),
)
