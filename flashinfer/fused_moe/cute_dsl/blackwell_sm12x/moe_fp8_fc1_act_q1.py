...
def out_sf_shape(m: int, n: int, num_experts: int):
    return (ceil_div(n, GRAN_K), compute_padded_offset(m, num_experts, SF_M_ALIGN))


class CuteDslSm120GroupedFp8Fc1ActQ1Op:
    OUT_DTYPES = (torch.float8_e4m3fn,)
    TILES = ((64, 128), (32, 128), (8, 128))
    ACTIVATIONS = (ActivationType.Swiglu,)
    ...
    def is_valid_dtypes(out_dtype) -> bool:
        return out_dtype in CuteDslSm120GroupedFp8Fc1ActQ1Op.OUT_DTYPES

    @staticmethod
    def is_valid_tile(tile) -> bool:
        bm, bn, bk = tile
        return (bm, bn) in CuteDslSm120GroupedFp8Fc1ActQ1Op.TILES and bk == BK
    ...
    def build(self, grid_x: int) -> CuteDslSm120MoeFp8Fc1ActQ1:
        return CuteDslSm120MoeFp8Fc1ActQ1(self.cfg, grid_x)
...
TACTICS = tuple(
    (bm, bn, e)
    for bm, bn in CuteDslSm120GroupedFp8Fc1ActQ1Op.TILES
    for e in epi_tactics((bm, bn, BK))
    if CuteDslSm120GroupedFp8Fc1ActQ1Op.is_constructible((bm, bn, BK), e)
)
...
def select_tile(*, total_rows: int, n: int, k: int, num_experts: int, num_sms: int):
    return select_fc1_act_tile(
        ...
        num_sms=num_sms,
        tiles=CuteDslSm120GroupedFp8Fc1ActQ1Op.TILES,
        gran_k=BK,
    )
