...
REG_PROD_BY_TACTIC = {
    (64, 128, EpiMethod.WG_S2R_QUANT): 40,
    (64, 128, EpiMethod.WG_QUANT_R2S): 40,
    (32, 128, EpiMethod.WG_S2R_QUANT): 40,
    (32, 128, EpiMethod.WG_QUANT_R2S): 40,
    (8, 128, EpiMethod.WG_QUANT_SWAP): 40,
}
...
def make_cfg(
    ...
    *,
    ...
    tile = (bm, bn, bk)
    store_threads = (
        WgQuantSwapEpiConfig.store_threads_for(bf16, (bm, bn))
        if swap
...
_DEFAULT_QCFG = QuantConst(is_ue8m0=False)


class CuteDslSm120MoeFp8Fc1ActQ1:
    def __init__(self, cfg, grid_x, qcfg=_DEFAULT_QCFG):
        ...
        group_extent = cfg.TILE[0] if cfg.mma.swap_ab else cfg.TILE[1]
        assert group_extent == GRAN_K, (
            f"{'bm' if cfg.mma.swap_ab else 'bn'} {group_extent} != gran_k {GRAN_K}: one work item "
            f"must own a whole quant group, or amax would have to reduce across CTAs"
        )
    ...
    def kernel(
        ...
        if cfg.is_prod_wg(warp_idx):
            ...
            cons = moe_scheduler.MoeSchedConsumer.create(cfg.sched_stages)
            tile = cons.get_next_tile(sWork, sfull, sempty)
            read_stage, ab_phase = i32(0), i32(0)
            while tile.valid != i32(0):
                if cutlass.const_expr(swap):
