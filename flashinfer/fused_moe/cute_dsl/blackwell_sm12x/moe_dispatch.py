...
def _static_max_chunks(rows: int) -> int:
    """Virtual-expert chunks the static route can open for ``rows`` routed rows."""
    return (rows + _STATIC_SLOT_ROWS - 1) // _STATIC_SLOT_ROWS
...
# tile-M-row virtual-expert split keeps every scheduled tile inside the
# 32x128 kernel's single computed M step, making the full compact band
# correct at any per-expert row count.
_STATIC_COMPACT_CUTOVER_PAIRS_DEFAULT = 640
# The retained NVFP4 path uses the 32x128 schedule through 1024 routed pairs.
...
# Versioned static/dynamic cutover registry.  Key =
...
# Entries come only from accepted same-GPU boundary measurements (forced static
# under the covering plan vs forced dynamic, seeded random and zipf(s=0.8)
# routes, three interleaved repeats); when the two routing families require
# incompatible boundaries the entry records the conservative fallback.  Every
# key part is exact: a missing part, an unmeasured shape, another SM count or
...
_STATIC_CUTOVER_REGISTRY: Dict[Tuple[str, str, int, int, int, int, int, int], int] = {
    # Qwen3.5-35B TP1 (E256 / H2048 / I512 / top-k 8), 110-SM SM120, capacity 8192 tokens:
    # r=7 (1792 pairs).  On the final kernel (prologue L2 prefetch limited to 512-expert
    # shapes) the neighbourhood M224-M448 gives r=7 the smallest worst-case regret (0.58 %:
    # seeded random would gain 0.58 % from static at M256, zipf loses 0.25 % to static at
    # M224) while r=8 leaves zipf 3.5 % behind dynamic at M256 (adjust-down at the exact
    # M256 / M257 boundary; seeded random confirms it).  A revision with the prefetch still
    # active on 256-expert shapes had moved the crossover to M256-M288 and would have taken
    # r=8; the entry is decided on the kernel that ships (exact-boundary measurements).
...
# Max-active-cluster policy for the retained2 32x128 static band, keyed by
# routed-row range after the tile-M-row virtual-expert split.
...
# (halving the task count starves the 110 resident CTAs) and won 1-8 % from
# 1920 pairs upwards (half the route-scratch traffic, no phantom slice).
...
# counter restore costs ~1 us, which at M8 (80 routed pairs) is a 0.7-1.2 %
# loss and at M16 (160 pairs) is neutral.  Launches below this many routed
# pairs keep the prologue clear (and withdraw the workspace's clean marker).
...
def _get_static_compact_cutover_pairs(
    ...
    *,
    ...
    versioned registry entry of the full key (quant mode, activation, E, H,
...
class Sm120StaticMoEWorkspace:
    ...
    # tile-M-row virtual-expert split scratch: [weight_E] monotone row allocators
...
def allocate_sm120_static_workspace(
    *,
    ...
    # Per-slot activation planes: _STATIC_SLOT_ROWS routed rows (the tile-M
    # chunk size) and one 128-row block-scale atom.  max_rows stays the
    # routed-row capacity.
    slot_rows = _STATIC_SLOT_ROWS
    rows_pad_k = _align_up(slot_rows, 128)
    cols_pad_k = _align_up(k // sf_vec_size, 4)
    # The tile-M-row virtual-expert split may open extra compact expert slots
    # for experts with more than one chunk of routed rows.
    # Sum(ceil(rows_e/tile_m)) <= actives + total_rows/tile_m, so
    # ceil(max_rows/tile_m) extra slots always suffice.
    max_chunks = _static_max_chunks(max_rows)
...
class _WeightViews:
    ...
    static_w13_fp4: Optional[torch.Tensor] = None
    static_down_fp4: Optional[torch.Tensor] = None
    intermediate_size: int | None = None
    ...
    # built by this callable on first use.
...
def static_source_scales(
    ...
    complete K atoms. The static kernel and eligible dynamic FC2 consume source
    scales; dynamic FC1 and fallback consumers still materialize padded scales
    lazily on first use.
    """
...
def _get_weight_views(
    ...
    quant_mode: str = "nvfp4",
    intermediate_size: int | None = None,
    w1_fp4_unpadded: torch.Tensor | None = None,
    w2_fp4_unpadded: torch.Tensor | None = None,
    ...
    * a single N128 slice (``intermediate_size <= 128``): the retained2 static
    ...
    w1_static = w1_fp4 if w1_fp4_unpadded is None else w1_fp4_unpadded
    w2_static = w2_fp4 if w2_fp4_unpadded is None else w2_fp4_unpadded
    if w1_static.shape[1] != branches * n_true or w2_static.shape[2] != n_true // 2:
        raise ValueError(
            "intermediate_size does not match the unpadded weights: "
            f"n_true={n_true}, w1 rows={w1_static.shape[1]}, "
            f"w2 packed cols={w2_static.shape[2]}"
        )
    static_w13 = w1_static.reshape(
...
def _get_static_kernel(
    ...
    *,
    ...
    # One virtual expert occupies exactly one mathematical M tile.  Its
    # packed activation planes and token map use the same row extent.
    mma_tiler_mn = (_STATIC_TILE_M, _LEVEL_TILE_N)
    ...
    virt_route_scratch_fake = cute.runtime.make_fake_compact_tensor(
        cutlass.Int32,
        (weight_E * (1 + _static_max_chunks(route_rows)) + 8,),
        assumed_align=4,
    )
...
def _get_micro_kernel(
    ...
    *,
    ...
    # Micro has its own route packing and MMA loops; changing the static
    # virtual-expert tile must not change this kernel's geometry.
    mma_tiler_mn = (64, _LEVEL_TILE_N)
...
def _get_direct_micro_kernel(
    ...
    *,
    ...
    Returns (compiled, grid_x, accepts_block_dim). The default preserves the
    low-level direct multiplier contract; API callers pass their scale mode.
    """
...
def launch_sm120_static_moe(
    *,
    ...
    # tile padding. Only the wide FC2 body masks activation tail reads; keep
    # <=256 extents on the padded path. Its raw pointers also require the
    # original packed backing order; TMA views may otherwise carry strides.
    direct_source_scales = (
        weights.source_scales
        and int(weights.intermediate_size or n) > 256
        and weights.static_family_w1_storage is None
        and weights.static_w13_fp4 is not None
        and weights.static_down_fp4 is not None
        and weights.static_w13_fp4.permute(2, 0, 1).is_contiguous()
        and weights.static_down_fp4.permute(2, 0, 1).is_contiguous()
    )
    if direct_source_scales:
        family_n = int(weights.intermediate_size or n)
    # The Qwen E512/H2560/I320/top-10 M4 source path benefits from direct
    # micro beyond the usual routed-row cutover; keep other shapes unchanged.
    direct_m4_source_shape = (
        num_tokens == 4
        and k == 2560
        and family_n == 320
        and top_k == 10
    ...
    use_direct_micro = (
        ...
        and (routed_rows < _DIRECT_MICRO_CUTOVER_PAIRS or direct_m4_source_shape)
    ...
    if use_direct_micro:
        ...
        if not block_ok:
            ...
            use_direct_micro = False
    if use_direct_micro:
        # The kernel converts ordinary API scales when loading them, so graph
        # replay reads live tensors without separate reciprocal/copy kernels.
        # The source views retain the contiguous [E, 2I, K/2] / [E, K, I/2]
        # backing storage. launch() takes their base pointers; their permuted
        # tensor strides are not consumed by the direct CUDA-core kernel.
        if direct_source_scales:
            dm_w1, dm_w2 = weights.static_w13_fp4, weights.static_down_fp4
            dm_w1_sf, dm_w2_sf = weights._w13_sf_storage, weights._down_sf_storage
        elif weights.static_family_w1_storage is not None:
            dm_w1, dm_w1_sf = (
    ...
    if use_micro:
        ...
        # top-k=1 keeps the retained2 schedule; the merged rule is calibrated
        # for multi-top-k routes only.
...
def select_sm120_moe_backend(
    *,
    ...
    comes from the versioned registry when the full key is known)."""
...
def _get_dynamic_kernel(
    ...
    *,
    ...
    source_down_scales = bool(source_down_scales)
    if source_down_scales and not (
        branch_major_extent is not None
        and quant_mode == "nvfp4"
        and n_true > 128
        and n_true % 128 == 64
    ):
        raise ValueError("source down scales require the true-extent gated NVFP4 path")
    ...
    cache_key = _dynamic_kernel_cache_key(
        ...
        source_down_scales=source_down_scales,
    )
...
def launch_sm120_dynamic_moe(
    *,
    ...
    # Select the prepared storage and its compiled extent together.
    source_down_scales = bool(
        weights.source_scales
        and branch_major_extent is not None
        and weights.branch_major_w13_fp4 is not None
    )
