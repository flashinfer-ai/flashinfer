...
# Calibrated NVFP4 fallback, expressed as routed rows per weight expert so it
# scales with E. These are performance heuristics, not layout requirements or
# a guarantee of the optimal crossover on an unseen shape/GPU. In particular,
# the wider tail band is not implied by padding bytes: eligible gated dynamic
# kernels also stream true-extent weights.
...
def _get_static_compact_cutover_pairs(
    ...
    *,
    ...
    _normalize_activation_precision(activation_precision)
    # This policy is cheap to evaluate; do not cache environment-dependent
    # answers or let one wrapper's geometry/capacity affect another's choice.
    cutover = _first_env(
    ...
    registry_pairs = _static_cutover_registry_lookup(
        quant_mode,
        activation,
        num_experts,
        hidden_size,
    ...
    # Generic/MXFP4 keeps its original boundary. A single N128 slice also
    # keeps the flat NVFP4 boundary: retained2 static needs N256 operands.
    if quant_mode != "nvfp4":
        return _STATIC_COMPACT_CUTOVER_PAIRS_DEFAULT
    floor = _STATIC_COMPACT_CUTOVER_PAIRS_NVFP4_DEFAULT
    if num_experts is None or (
        intermediate_size is not None and static_needs_256_extent(intermediate_size)
    ):
        return floor
    rows_per_expert = (
        _STATIC_COMPACT_CUTOVER_ROWS_PER_EXPERT_PADDED
        if intermediate_size is not None and int(intermediate_size) % _LEVEL_TILE_N
    ...
    if (
        ...
        and _static_merged_groups(intermediate_size, rows_per_expert * int(num_experts))
    ):
        rows_per_expert = _STATIC_MERGED_CUTOVER_ROWS_PER_EXPERT
    return max(floor, rows_per_expert * int(num_experts))
...
def allocate_sm120_static_workspace(
    *,
    ...
    max_chunks = _align_up(max_rows, _STATIC_SLOT_ROWS) // _STATIC_SLOT_ROWS
...
class _WeightViews:
    ...
    # Single-slice shapes: static and direct micro consume N256 copies;
    # MMA micro keeps N128 operands. Retain the copies' backing storages.
    ...
    def ensure_legacy(self) -> None:
        ...
        self.w13_fp4, self.down_fp4 = _legacy_fp4_views(w1_padded, w2_padded)
...
_STATIC_MIN_EXTENT = _STATIC_RETAINED_GROUP_N
...
def static_source_scales(
    ...
    return (
        ...
        and n > _LEVEL_TILE_N
        # Half-atom branch offset also guarantees a legal 32-element TMA stride.
        and n % _LEVEL_TILE_N == _LEVEL_TILE_N // 2
    )
...
def _legacy_fp4_views(
    w1: torch.Tensor, w2: torch.Tensor
) -> Tuple[torch.Tensor, torch.Tensor]:
    """Expert-last FP4 views of legacy operands; no allocation or copy."""
    return (
        w1.permute(1, 2, 0).view(torch.float4_e2m1fn_x2),
        w2.permute(1, 2, 0).view(torch.float4_e2m1fn_x2),
    )


def _convert_weight_scales(
    w1_sf: torch.Tensor,
    w2_sf: torch.Tensor,
    *,
    n: int,
    ...
    """Convert both scales at the caller's chosen extent (true, padded or N256).

    Cache ownership and capture guards stay with the preparation path, which
    decides whether this is a source-layout view or a materialized conversion.
    """
    return (
        convert_sf_from_mma_layout(
            w1_sf,
...
def _prepare_weight_views(
    *,
    ...
    pad_args = (w1_fp4, w1_blockscale, w2_fp4, w2_blockscale)
    static_override = None
    if static_needs_256_extent(n):
        static_override = _pad_intermediate_to_tile(
...
def _get_weight_views(
    ...
    * a single N128 slice (``intermediate_size <= 128``): the retained2 static
    ...
    if cached is None:
        ...
        cached = (
            *_convert_weight_scales(
                w1_blockscale,
                w2_blockscale,
                n=scale_n,
                k=k,
                branches=branches,
                experts=w1_fp4.shape[0],
                sf_vec_size=sf_vec_size,
            ),
    ...
    if padded_fp4_present:
        w13, down = _legacy_fp4_views(w1_fp4, w2_fp4)
    else:
        w13 = down = None
    ...
    if not true_extent_views_supported(n_true):
        if w13 is None:
            ...
            w13, down = _legacy_fp4_views(w1_p, w2_p)
    ...
    if static_override is not None:
        ...
        if cached_sf is None:
            ...
            cached_sf = _convert_weight_scales(
                w1_sf_s,
                w2_sf_s,
                n=static_extent,
                k=k,
                branches=branches,
                experts=w1_s.shape[0],
                sf_vec_size=sf_vec_size,
            )
...
# Unified dispatch
# ==========================================================================
# Legacy padded block scales and FP4 copies have separate caches: source-scale
# or true-extent backends need not materialize operands for a different backend.
...
def _pad_intermediate_to_tile(
    ...
    if n_pad == n:
        return w1_weight, w1_weight_sf, w2_weight, w2_weight_sf, n
    E = int(num_experts)
    ...
    if not pad_fp4:
        return w1_weight, w1_sf_p, w2_weight, w2_sf_p, n_pad
    ...
    w1p, w2p = fp4
    return w1p, w1_sf_p, w2p, w2_sf_p, n_pad
