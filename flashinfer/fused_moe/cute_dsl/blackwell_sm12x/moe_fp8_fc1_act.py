...
"""fp8 fused fc1_gate_up + SiLU on CuteDSL: what one compiled kernel is specialized on, and the entry."""
...
class CuteDslSm120GroupedFp8Fc1ActOp:
    ...
    def is_valid_dtypes(out_dtype) -> bool:
        return out_dtype in CuteDslSm120GroupedFp8Fc1ActOp.OUT_DTYPES

    @staticmethod
    def is_valid_tile(tile) -> bool:
        bm, bn, bk = tile
        return (bm, bn) in CuteDslSm120GroupedFp8Fc1ActOp.TILES and bk == GRAN_K
    ...
    def build(self, grid_x: int) -> CuteDslSm120MoeFp8Fc1Act:
        return CuteDslSm120MoeFp8Fc1Act(self.cfg, grid_x)


TACTICS = tuple(
    (bm, bn, epi)
    for bm, bn in CuteDslSm120GroupedFp8Fc1ActOp.TILES
    for epi in epi_tactics((bm, bn, GRAN_K))
    if CuteDslSm120GroupedFp8Fc1ActOp.is_constructible((bm, bn, GRAN_K), epi)
)
...
def _op(
    ...
    situ_beta=SITU_BETA,
    situ_linear_beta=SITU_LINEAR_BETA,
) -> CuteDslSm120GroupedFp8Fc1ActOp:
    return CuteDslSm120GroupedFp8Fc1ActOp(
        n=n,
        k=k,
        tile=tile,
        out_dtype=out_dtype,
...
def select_tile(*, total_rows: int, n: int, k: int, num_experts: int, num_sms: int):
    return select_fc1_act_tile(
        ...
        num_sms=num_sms,
        tiles=CuteDslSm120GroupedFp8Fc1ActOp.TILES,
        gran_k=GRAN_K,
    )
...
def cute_dsl_sm12x_fc1_act_fp8(
    a_q,
    a_scale,
    b_q,
    b_scale,
...
class _Fc1ActRunner(TunableRunner):
    ...
    def valid_tactics(self, inputs):
        ...
        n, k = int(b_q.shape[1]) // 2, int(b_q.shape[2])
        return [
            t
            for t in TACTICS
            if CuteDslSm120GroupedFp8Fc1ActOp.can_implement(
                n=n,
                k=k,
