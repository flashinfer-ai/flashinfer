...
def decode_block(
    block_idx, gemm_type, num_m_blocks, num_n_blocks, k_1d, min_fn, max_fn
):
    if gemm_type == GemmType.BATCHED:
        blocks_per_batch = num_m_blocks * num_n_blocks
        batch = block_idx // blocks_per_batch
        m_block, n_block = swizzle_block(
            block_idx % blocks_per_batch,
            num_n_blocks,
            num_m_blocks,
            k_1d,
            min_fn,
    ...
    m_block, n_block = swizzle_block(
        block_idx, num_n_blocks, num_m_blocks, k_1d, min_fn, max_fn
    )
...
class MoeSchedProducer:
    ...
    def __new_from_mlir_values__(self, values):
        return MoeSchedProducer(
            self.stages, new_from_mlir_values(self.sched_iter, values)
        )

    @dsl_user_op
    @cute.jit
    def publish(
        self, work: cute.Tensor, full, empty, tile: MoeWorkTile, *, loc=None, ip=None
    ):
        sched_stage = self.sched_iter % self.stages
        cute.arch.mbarrier_wait(
            empty + sched_stage, ((self.sched_iter // self.stages) & 1) ^ 1
        )
...
class MoeSchedConsumer:
    ...
    @cute.jit
    def get_next_tile(
        self, work: cute.Tensor, full, empty, *, loc=None, ip=None
    ) -> MoeWorkTile:
        sched_stage = self.sched_iter % self.stages
        cute.arch.mbarrier_wait(
            full + sched_stage, (self.sched_iter // self.stages) & 1
        )
        tile = MoeWorkTile.load(work, sched_stage)
        cute.arch.sync_warp()
        if tile.valid != Int32(0):
            with cute.arch.elect_one():
