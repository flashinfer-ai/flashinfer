...
class _ExpandedWeightedCombine:
    ...
    def __call__(
        self,
        contributions_ptr: cute.Pointer,
        mapping_ptr: cute.Pointer,
        output_ptr: cute.Pointer,
        stream: cuda.CUstream,
    ):
        contributions = cute.make_tensor(
            contributions_ptr, cute.make_layout((8192, 7168), stride=(7168, 1))
        )
        mapping = cute.make_tensor(
            mapping_ptr, cute.make_layout((512, 16), stride=(16, 1))
        )
        output = cute.make_tensor(
            output_ptr, cute.make_layout((512, 7168), stride=(7168, 1))
        )
    ...
    @cute.kernel
    def kernel(
        self, contributions: cute.Tensor, mapping: cute.Tensor, output: cute.Tensor
    ):
                ...
                for slot in cutlass.range_constexpr(16):
                    if (written_mask & (cutlass.Int32(1) << slot)) != 0:
                        for j in cutlass.range_constexpr(7):
                            ...
                            pair_word[0] = contribution_words[
                                token * 16 + slot, channel_pair
                            ]
                            totals[2 * j] = totals[2 * j] + pair[0].to(cutlass.Float32)
                            totals[2 * j + 1] = totals[2 * j + 1] + pair[1].to(
                                cutlass.Float32
                            )
            for j in cutlass.range_constexpr(7):
                channel_pair = tid + 512 * j
                ...
                for slot in cutlass.range_constexpr(16):
                    ...
                    if (written_mask & (cutlass.Int32(1) << slot)) != 0:
                        for j in cutlass.range_constexpr(14):
                            channel = tid + 512 * j
                            totals[j] = totals[j] + contributions[
                                token * 16 + slot, channel
                            ].to(cutlass.Float32)
            for j in cutlass.range_constexpr(14):
                channel = tid + 512 * j
...
def _plan_expanded_weighted_combine(contributions, mapping, output):
    ...
    with torch.cuda.device(output.device):
        ...
        arguments = tuple(
            ...
            for tensor, dtype, alignment in zip(
                owners,
        ...
        compiled = cute.compile(
            _ExpandedWeightedCombine(paired_io), *arguments, stream=stream
        )
