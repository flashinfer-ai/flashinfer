...
"""Fused MXFP4 MoE preprocessing; minimum architecture SM100 (B300: SM103).

Unpack global expert IDs, convert BF16 router weights to FP32, and clear the
combined BF16 output in one launch. Fused T512 sorting requires B300/SM103.
T=1..16 can additionally sort assignments
into compact expert tiles; prefill retains native sorting. Planning compiles
and binds caller-owned buffers outside capture; execution uses the supplied
CUDA stream.
"""
...
class _RoutePreprocess:
    def __init__(self, mode, threads, clear_words_per_thread=1, _vector_clear=False):
        self.packed = mode == "packed"
        self.convert_weights = mode != "separate_fp32"
        self.threads = threads
        self.clear_words_per_thread = clear_words_per_thread
        self.vector_clear = _vector_clear
    ...
    def kernel(
        ...
        if cutlass.const_expr(self.convert_weights):
            ...
            if cutlass.const_expr(self.vector_clear):
                # Selected T1024/top16 factor4: stride917504 >16384 routes.
                # Preserve caller strides and signed packed expert extraction.
                if first < cute.size(weights_dst):
                    token = first >> 4
                    slot = first & 15
                    if cutlass.const_expr(self.packed):
                        ids_dst[first] = ids_src[(token, slot)] >> 16
                    weights_dst[first] = weights_src[(token, slot)].to(cutlass.Float32)
            else:
        ...
        if cutlass.const_expr(self.vector_clear):
            zeros = cute.make_rmem_tensor(cute.make_layout((4,)), cutlass.Uint32)
            zeros.fill(cutlass.Uint32(0))
            atom = cute.make_copy_atom(
                cute.nvgpu.CopyUniversalOp(), cutlass.Uint32, num_bits_per_copy=128
            )
            # Selected factor4 gives one16-byte group per lane for128/256 threads.
            if first < cute.size(output_words) // 4:
                word = cute.assume(first * 4, divby=4)
                destination = cute.make_tensor(
                    output_words.iterator + word, cute.make_layout((4,))
                )
                cute.copy_atom_call(atom, zeros, destination)
        else:
    ...
    def kernel(
        ...
        if block == 0:
            ...
            if cutlass.const_expr(self.fused_prefill_sort):
                for route in cutlass.range(tid, cute.size(expanded), self.threads):
                    token = route // ids_src.shape[1]
                    slot = route % ids_src.shape[1]
            ...
            if cutlass.const_expr(self.fused_prefill_sort):
                for route in cutlass.range(tid, cute.size(expanded), self.threads):
                    relocated_rank = expanded[route]
                    if relocated_rank >= 0:
                        relocated_token = route // ids_src.shape[1]
                        relocated_slot = route % ids_src.shape[1]
                        relocated_expert = ids_src[(relocated_token, relocated_slot)]
                        if cutlass.const_expr(self.packed):
                            relocated_expert = relocated_expert >> 16
                        relocated_local = relocated_expert - local_offset
                        row = bases[relocated_local] + relocated_rank
                        expanded[route] = row
                        permuted[row] = route
            else:
                if rank >= 0:
...
def _plan_route_preprocess(
    ...
    *,
    ...
    buffers are contiguous. Conversion/clearing supports positive token counts;
    fused sorting remains limited to T=1..16. Both modes require top-k from
    ...
    count, device, architecture, single-tile prefix mode and private clear-work
    setting. Sorting uses 256..1024 threads, selected during planning.
    The private single-tile option relies on the
    ...
    if sorts_tokens:
        if _fused_prefill_sort:
            ...
            if tokens > 16:
                raise ValueError("fused sorting requires T=1..16")
        if not (
    ...
    with torch.cuda.device(device):
        ...
        if _fused_prefill_sort and (not sorts_tokens or arch != (10, 3)):
            raise ValueError("private T512 fused sorting requires sorting on B300")
        # Private T1024 store-width specialization. Keep the existing 4-byte
        # output contract: other alignments use the unchanged scalar clear.
        vector_clear = (
            arch == (10, 3)
            and not sorts_tokens
            and tokens == 1024
            and hidden_size == 7168
            and top_k == 16
            and num_experts == 896
            and num_local_experts == 112
            and mode in ("packed", "separate_bf16")
        ...
        pointers = (
            ...
            make_ptr(
                ...
                output.data_ptr(),
                cute.AddressSpace.gmem,
                assumed_align=16 if vector_clear else 4,
            ),
        )
        if sorts_tokens:
            pointers += tuple(
        ...
        # Coarsen only nonsorting prefill; both device loops are grid-stride.
        clear_words_per_thread = 4 if not sorts_tokens and tokens > 16 else 1
        cache_key = (
            mode,
            threads,
            device.index,
            ...
            sorts_tokens,
            single_tile_per_expert,
            clear_words_per_thread,
            _clear_output,
        )
        if _fused_prefill_sort:
            cache_key += ("t512_fused_sort",)
        if vector_clear:
            cache_key += ("t1024_vector_clear", "t1024_single_pass_route_clear")
