...
def _moe_core_impl(
    ...
    2. GEMM1 + activation: First projection with fused activation
...
class CuteDslMoEWrapper:
    ...
    def __init__(
        ...
        self.activation_type: ActivationType = normalized_activation_type
        self.gated = gated
...
def cute_dsl_fused_moe_nvfp4(
    ...
    *,
    ...
    torch.Tensor
        Output tensor of shape ``[num_tokens, hidden_size]``.
    """
    normalized_activation_type, _ = normalize_cute_dsl_moe_activation_type(
        activation_type
    )
    validate_cute_dsl_moe_situ_config(
        normalized_activation_type, situ_beta, situ_linear_beta
    ...
    activation_name = (
        "Situ" if situ_beta is not None else normalized_activation_type.name
    )
    _, best_tactic = tuner.choose_one(
