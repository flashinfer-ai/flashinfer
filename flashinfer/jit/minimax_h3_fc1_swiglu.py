...
MiniMaxH3Fc1SwigluTarget = Literal["sm100a", "sm103a"]
...
def _minimax_h3_fc1_swiglu_cuda_source(target: MiniMaxH3Fc1SwigluTarget) -> Path:
    ...
    raise FileNotFoundError(
        f"MiniMax-H3 FC1+SwiGLU CUDA source not found. Checked:\n  - {packaged}\n  - {source_tree}"
    )


def minimax_h3_fc1_swiglu_target(
...
def gen_minimax_h3_fc1_swiglu_module(target: MiniMaxH3Fc1SwigluTarget) -> JitSpec:
    """JIT spec for the fused MiniMax-H3 RMSNorm + AdaLN + FC1 + SwiGLU operator (BF16, MXFP8
    and NVFP4 variants) on one exact Blackwell target.

    The module exposes ``minimax_h3_fc1_swiglu``, ``minimax_h3_fc1_swiglu_mxfp8`` and
    ``minimax_h3_fc1_swiglu_nvfp4``.  The device code is tcgen05 + TMEM + 2-CTA MMA and does not
    compile for SM90 or SM120.
    """

    if target not in _TARGETS:
        raise ValueError(f"unsupported MiniMax-H3 FC1+SwiGLU target: {target!r}")
    if not is_cuda_version_at_least("12.9"):
        raise RuntimeError("SM100a/SM103a compilation requires CUDA 12.9 or newer")
    source = _minimax_h3_fc1_swiglu_cuda_source(target)
    return gen_jit_spec(
        f"minimax_h3_fc1_swiglu_{target}_v1",
        [source],
