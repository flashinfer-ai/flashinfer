...
"""Checksum-verified JIT loader for the source-only GDN non-CP GDN backend."""
...
GDNNonCPArch = Literal["sm_100a", "sm_103a"]

_EXPORT_SCHEMA = "flashinfer-gdn-noncp-decode-standalone-export-v1"
_MANIFEST_SHA256 = "2f280959a0128344c67891f7afdb9a4cd06d43c61b9c5d724570ff661184bd97"
_ARCH_ACTIVE_CLUSTERS: dict[GDNNonCPArch, int] = {
    "sm_100a": 148,
    "sm_103a": 160,
}


class GDNNonCPUnsupportedError(NotImplementedError):
    """The explicit GDN non-CP backend cannot serve the requested GDN contract."""


class GDNNonCPRoute(NamedTuple):
    route_id: str
    variant_name: str
...
def _nvcc_identity() -> tuple[Path, str]:
    nvcc = (Path(get_cuda_path()) / "bin" / "nvcc").resolve()
    if not nvcc.is_file():
    ...
    if result.returncode != 0:
        raise RuntimeError(f"failed to identify nvcc at {nvcc}:\n{result.stdout}")
    return nvcc, result.stdout


def _compile_cache_digest(
    *,
    arch: GDNNonCPArch,
    cuda_sha256: str,
    header_sha256s: tuple[str, ...],
...
def _source_dir() -> Path:
    installed = jit_env.FLASHINFER_CSRC_DIR / "gdn" / "noncp"
    if installed.exists():
        return installed
    checkout = Path(__file__).resolve().parents[2] / "csrc" / "gdn" / "noncp"
    if checkout.exists():
        return checkout
    raise FileNotFoundError(
        "frozen GDN non-CP GDN non-CP/decode sources were not found; checked "
...
def _manifest() -> dict[str, Any]:
    ...
    if observed_digest != _MANIFEST_SHA256:
        raise RuntimeError(
            f"GDN non-CP GDN manifest drift at {path}: "
    ...
    if observed != expected:
        raise RuntimeError(
            "GDN non-CP GDN manifest does not match the frozen support contract: "
...
def _kernel_record(name: str) -> dict[str, Any]:
    ...
    if len(records) != 1:
        raise ValueError(f"unknown GDN non-CP GDN kernel: {name!r}")
    return records[0]


def _cuda_record(record: dict[str, Any], arch: GDNNonCPArch) -> dict[str, Any]:
    outputs = [
...
def _compile_cubin(
    ...
    *,
    ...
    cache_dir = jit_env.FLASHINFER_JIT_DIR / "gdn_noncp" / arch
...
@functools.cache
def load_gdn_noncp_kernel(name: str, arch: GDNNonCPArch):
    """Compile and load one checksum-verified GDN non-CP GDN host entrypoint."""

    if arch not in _ARCH_ACTIVE_CLUSTERS:
        raise GDNNonCPUnsupportedError(
            f"unsupported GDN non-CP GDN architecture: {arch!r}"
        )
    ...
    for path, expected in sources:
        ...
        if observed != expected:
            raise RuntimeError(
                f"GDN non-CP GDN source drift at {path}: expected {expected}, got {observed}"
            )
    compile_options = tuple(record.get("compile_options", ()))
    unsupported_options = set(compile_options) - {"--use_fast_math"}
    if unsupported_options:
        raise RuntimeError(
            f"unsupported GDN non-CP GDN compile options: {sorted(unsupported_options)!r}"
        )
    ...
    module = cpp.load_inline(
        f"flashinfer_gdn_noncp_{name}_{arch}_{module_digest[:12]}",
...
def _prefill_schedule_attr(
    *,
    ...
    if gatepipe4:
        return "flashinfer_blackwell_gdn_prefill_dvsplit_initial_f16io_gatepipe4"
    if fullgrid_vhold:
        return "flashinfer_blackwell_gdn_prefill_dvsplit_initial_f16io_fullgrid_vhold"
    stem = (
...
def _variant_for(
    *, domain: str, schedule_attr: str, specializations: dict[str, int | float]
    ...
    source_schedule = {
        "decode": "flashinfer.gdn_noncp.generated.decode",
        "prefill": "flashinfer.gdn_noncp.generated.prefill",
    }[domain]
    expected_source = f"{source_schedule}:{schedule_attr}"
    expected_tma_abi = "grid_constant" if domain == "prefill" else "pointer"
    matches = [
        record
    ...
    if len(matches) != 1:
        raise GDNNonCPUnsupportedError(
            "no exact frozen GDN non-CP GDN variant for "
...
@functools.cache
def select_gdn_noncp_prefill_variant(
    *,
    arch: GDNNonCPArch,
    io_dtype: str,
    state_dtype: str,
    ...
    """Resolve host-visible non-CP metadata to one frozen GDN non-CP variant."""

    if arch not in _ARCH_ACTIVE_CLUSTERS:
        raise GDNNonCPUnsupportedError(f"unsupported architecture {arch}")
    if io_dtype not in {"float16", "bfloat16"}:
        raise GDNNonCPUnsupportedError(f"unsupported I/O dtype {io_dtype}")
    if state_dtype not in {
        ...
        raise GDNNonCPUnsupportedError(f"unsupported state dtype {state_dtype}")
    min_heads = min(num_q_heads, num_v_heads)
    num_o_heads = max(num_q_heads, num_v_heads)
    if min_heads <= 0 or num_k_heads != min_heads or num_o_heads % min_heads:
        raise GDNNonCPUnsupportedError("unsupported GDN head mapping")
    if (
        num_seqs <= 0
        or total_seq_len <= 0
        or max_seq_len <= 0
        ...
        raise GDNNonCPUnsupportedError(
            "GDN non-CP GDN prefill requires positive, consistent sequence metadata"
        )
    ...
    if use_state_indices and (not use_initial_state or not store_final_state):
        raise GDNNonCPUnsupportedError("indexed state requires initial and final state")
    ...
    sglang_tp4_checkpoint_route = (
        ...
        and len(seq_lens) == num_seqs
        and all(length >= 0 for length in seq_lens)
        and sum(seq_lens) == total_seq_len
        and max(seq_lens, default=0) == max_seq_len
        and dvsplit
    )
    if enable_checkpoints and not (
        ...
        raise GDNNonCPUnsupportedError(
            ...
            "the SGLang TP4 BF16 indexed DV-split contract"
        )
    ...
    if single_chunk:
        route = "flashinfer.gdn_prefill.noncp.single_chunk"
    elif enable_checkpoints:
        route = "flashinfer.gdn_prefill.noncp.checkpoints"
    else:
        route = "flashinfer.gdn_prefill.noncp"
    return GDNNonCPRoute(
        f"{route}.{'dvsplit' if dvsplit else 'full_dv'}", record["name"]
    )


@functools.cache
def select_gdn_noncp_decode_variant(
    *,
    arch: GDNNonCPArch,
    batch_size: int,
    io_dtype: str,
    ...
    if io_dtype != "bfloat16" or state_dtype not in {"float32", "bfloat16"}:
        raise GDNNonCPUnsupportedError(
            "GDN non-CP decode requires BF16 I/O and FP32 or BF16 state"
        )
    if head_size != 128:
        raise GDNNonCPUnsupportedError("GDN non-CP decode requires K=V=128")
    if not use_qk_l2norm:
        raise GDNNonCPUnsupportedError(
            "GDN non-CP T=1 child contract requires in-kernel Q/K L2 normalization"
        )
    if (
        num_q_heads <= 0
        ...
        raise GDNNonCPUnsupportedError("unsupported decode head mapping")
    if batch_size <= 0:
        raise GDNNonCPUnsupportedError("decode batch size must be positive")
    if state_dtype == "bfloat16":
        promoted = {
        ...
        if num_q_heads == 4 and num_v_heads == 8:
            ...
            return GDNNonCPRoute(route, record["name"])
        ...
        if seq_len == 1:
            ...
            route = f"flashinfer.gdn_decode.indexed_bf16_update_t{seq_len}"
        return GDNNonCPRoute(f"{route}.wide{tile_v}", record["name"])
    ...
    if seq_len != 1:
        promoted_fp32_mtp = {
            ...
            (16, 4, True, False, 4): (
                ...
                "flashinfer.gdn_decode.indexed_fp32_mtp_t4.tile64_update_cache",
            ),
            (64, 4, True, False, 4): (
                "gdn_decode_pretranspose_mtp_t4_splitv2_tile64",
                "flashinfer.gdn_decode.indexed_fp32_mtp_t4.tile64_update_cache",
    ...
    if layout == "pretranspose":
        ...
        route = "flashinfer.gdn_decode.indexed_fp32_t1_splitv8"
    elif layout == "nontranspose":
        if batch_size < 32:
            schedule_attr = "gdn_decode_nontranspose_fp32_t1_small"
            route = "flashinfer.gdn_decode.direct_fp32_t1_nontranspose_small"
        else:
            schedule_attr = "gdn_decode_nontranspose_fp32_t1"
            route = "flashinfer.gdn_decode.direct_fp32_t1_nontranspose_large"
    else:
        raise GDNNonCPUnsupportedError(f"unsupported decode state layout {layout}")
    record = _variant_for(
        domain="decode",
        schedule_attr=schedule_attr,
...
def arch_for_compute_capability(major: int, minor: int) -> GDNNonCPArch:
    if (major, minor) == (10, 0):
        return "sm_100a"
    if (major, minor) == (10, 3):
        return "sm_103a"
    raise GDNNonCPUnsupportedError(
        f"GDN non-CP GDN supports only SM100a/SM103a, got compute capability {major}.{minor}"
    )
...
__all__ = [
    "GDNNonCPArch",
    "GDNNonCPRoute",
    "GDNNonCPUnsupportedError",
    "arch_for_compute_capability",
    "load_gdn_noncp_kernel",
    "select_gdn_noncp_decode_variant",
    "select_gdn_noncp_prefill_variant",
]
