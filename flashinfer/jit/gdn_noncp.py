...
_MANIFEST_SHA256 = "53a5c9a73dd98d1dbdcd6b83690d9682cf14903d84c7a2bbc8790b54225c363f"
...
def _nvcc_identity() -> tuple[Path, str]:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"failed to identify nvcc at {nvcc}:\n{result.stdout}"
        )
...
def _source_dir() -> Path:
    ...
    if installed.exists():
        return installed
    checkout = (
        Path(__file__).resolve().parents[2] / "csrc" / "gdn" / "noncp"
    )
    if checkout.exists():
        return checkout
...
def load_gdn_noncp_kernel(name: str, arch: GDNNonCPArch):
    ...
    if arch not in _ARCH_ACTIVE_CLUSTERS:
        raise GDNNonCPUnsupportedError(f"unsupported GDN non-CP GDN architecture: {arch!r}")
    ...
    *,
    ...
    if state_dtype == "bfloat16":
        ...
        if (
            ...
            raise GDNNonCPUnsupportedError(
                "BF16 decode is limited to the sixteen exact promoted indexed/verify rows"
            )
