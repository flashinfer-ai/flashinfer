...
_VARIANT_SPECS: tuple[dict[str, Any], ...] = ({'name': 'repeated_safe_f32_wide_slot_offsets',
  'target': 'sm100a',
 ...
 {'name': 'compact_async_pr_eval_h96_f32_wide_slot_offsets',
  ...
  'eligibility': ({'heads': [96],
                   'minimum_rows': 4,
                   'maximum_rows': 4,
                   'state_indices_modes': ['positive_unique'],
                   'lower_bound_values': [-5.0],
                   'norm_eps_values': [1e-05],
                   'strides': {'x_row_stride': 36881,
 ...
 {'name': 'high_work_positive_pr_eval_h24_f32_wide_slot_offsets',
  ...
  'threads': 256,
  'dynamic_smem_bytes': 28288,
  'eligibility': ({'heads': [24],
                   'minimum_rows': 50,
                   'maximum_rows': None,
                   'state_indices_modes': ['positive_unique'],
                   'lower_bound_values': [-5.0],
                   'norm_eps_values': [1e-05],
                   'strides': {'x_row_stride': 9233,
 ...
 {'name': 'high_work_positive_pr_eval_h32_f32_wide_slot_offsets',
  ...
  'threads': 256,
  'dynamic_smem_bytes': 28288,
  'eligibility': ({'heads': [32],
                   'minimum_rows': 32,
                   'maximum_rows': None,
                   'state_indices_modes': ['positive_unique'],
                   'lower_bound_values': [-5.0],
                   'norm_eps_values': [1e-05],
                   'strides': {'x_row_stride': 12305,
 ...
 {'name': 'high_work_positive_pr_eval_h48_f32_wide_slot_offsets',
  ...
  'threads': 256,
  'dynamic_smem_bytes': 28288,
  'eligibility': ({'heads': [48],
                   'minimum_rows': 25,
                   'maximum_rows': None,
                   'state_indices_modes': ['positive_unique'],
                   'lower_bound_values': [-5.0],
                   'norm_eps_values': [1e-05],
                   'strides': {'x_row_stride': 18449,
...
def get_cake_fused_kda_decode_variants() -> tuple[CakeFusedKDADecodeVariant, ...]:
    ...
    for item in _VARIANT_SPECS:
        ...
        if body.parent != csrc_dir or not body.name.startswith("cake_fused_kda_decode_"):
...
def _variant_build_identity_payload(
    ...
    return {
        ...
        "binding_header_sha256": hashlib.sha256(binding_header.read_bytes()).hexdigest(),
...
def get_cake_fused_kda_decode_uri(
    name: str, target: CakeFusedKDADecodeTarget
) -> str:
    variant = get_cake_fused_kda_decode_variant(name, target)
    return f"cake_fused_kda_decode_{name}_{_variant_build_identity(variant)[:16]}_{target}"
...
def gen_cake_fused_kda_decode_module(
    ...
    logger.info(f"Generated Cake fused KDA decode {name} {target} JIT spec: {spec.name}")
    return spec


@functools.cache
def load_cake_fused_kda_decode_module(
    name: str, target: CakeFusedKDADecodeTarget
):
