...
def gen_cutlass_fused_moe_sm121_module(use_fast_build: bool = False) -> JitSpec:
    """SM121 (GB10): same compile flags as SM120 but passes device_arch='121'
    so generate_gemm_operations only emits the 128x128x64B tile that fits
    in GB10's ~99 KB SMEM."""
    nvcc_flags = [
        "-DCOMPILE_BLACKWELL_TMA_GEMMS",
        "-DCOMPILE_BLACKWELL_SM120_TMA_GROUPED_GEMMS",
        "-DENABLE_BF16",
    ...
    return gen_cutlass_fused_moe_module(nvcc_flags, "121", use_fast_build)
