...
"""JIT loader for the source-only Blackwell GDN CP-prefill backend."""
...
_MANIFEST_SHA256 = "13966b45d56d947df0429eb621ed39c042e22126c87eef26b6d397a4564f467a"
_BASELINE_REVISION = "6cb2e70995d92edbc443b1bfc317ecacac907640"
_FOCUS_CONTRACT = (
    150,
    "f64bc1f07c03379ed002ecf9d624809d6ecc46dfbda38df806475d40e3a4f539",
)
_FULL_CONTRACT = (
    822,
    "aea1d3199a1add1035ca172688a82ae49e93260b79af564399b99db4a486843b",
)
...
def _source_dir() -> Path:
    ...
    raise FileNotFoundError(
        "frozen Blackwell GDN CP-prefill sources were not found; checked "
        f"{installed} and {checkout}"
    )


@functools.cache
def gdn_cp_nvcc_version() -> tuple[int, int]:
    """Return the version of the nvcc executable used by the Blackwell JIT."""
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"failed to query the Blackwell GDN CP-prefill nvcc at {nvcc}:\n{result.stdout}"
        )
    match = re.search(r"\brelease\s+(\d+)\.(\d+)\b", result.stdout)
    if match is None:
        raise RuntimeError(
            f"could not parse the Blackwell GDN CP-prefill nvcc version at {nvcc}:\n"
            f"{result.stdout}"
        )
...
def _manifest() -> dict[str, Any]:
    ...
    if observed_digest != _MANIFEST_SHA256:
        raise RuntimeError(
            f"Blackwell GDN CP-prefill manifest drift at {path}: "
    ...
    if observed != expected:
        raise RuntimeError(
            "Blackwell GDN CP-prefill manifest does not match the ratified v3 "
...
def _kernel_record(name: str) -> dict[str, Any]:
    ...
    if len(records) != 1:
        raise ValueError(f"unknown Blackwell GDN CP-prefill kernel: {name!r}")
    return records[0]
...
def _compile_cubin(source: Path, *, arch: GDNCPArch, digest: str) -> bytes:
    cache_dir = jit_env.FLASHINFER_JIT_DIR / "gdn_cp_prefill" / arch
...
def load_gdn_cp_kernel(name: str, arch: GDNCPArch):
    """Compile and load one checksum-verified Blackwell kernel binding."""

    if arch not in ("sm_100a", "sm_103a"):
        raise ValueError(f"unsupported Blackwell GDN CP-prefill architecture: {arch!r}")
    ...
    for path, expected in sources:
        ...
        if observed != expected:
            raise RuntimeError(
                f"Blackwell GDN CP-prefill source drift at {path}: expected {expected}, got {observed}"
            )
