...
def _source_dir() -> Path:
    ...
    if packaged.is_dir():
        return packaged
    checkout = Path(__file__).resolve().parents[3] / "csrc" / "cake_mamba_ssd_combined"
    return checkout
...
class CakeSSDCombined:
    ...
    def run(
        ...
        if not dt_softplus:
            raise ValueError("Cake SSDCombined requires dt_softplus=True")
        ...
        if not mode_varlen and any(value is not None for value in metadata):
            raise ValueError("batched mode does not accept varlen metadata")
        ...
        final = workspace["final"] if return_final_states else None
