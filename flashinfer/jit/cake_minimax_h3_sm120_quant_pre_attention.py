...
_CUDA_SOURCE_NAME = "cake_minimax_h3_sm120_quant_pre_attention_sm120a.cu"
...
def _cuda_source() -> Path:
    ...
    raise FileNotFoundError(
        "MiniMax-H3 SM120 quantized pre-attention CUDA source not found. Checked:\n"
        f"  - {packaged}\n  - {source_tree}"
    )


def gen_minimax_h3_sm120_quant_pre_attention_module() -> JitSpec:
    """JIT spec for the SM120 (GB202) FP8 / NVFP4 fused MiniMax-H3 pre-attention kernels.
    ...
    return gen_jit_spec(
        "minimax_h3_sm120_quant_pre_attention_sm120a_v1",
        [_cuda_source()],
        extra_cuda_cflags=list(sm120a_nvcc_flags),
    )


__all__ = ["gen_minimax_h3_sm120_quant_pre_attention_module"]
