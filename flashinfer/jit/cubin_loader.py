...
def get_cubin(file_name: str, sha256: str, session=None) -> bytes:
    ...
    # Source-compiled cubins: load directly without sha256 verification
    # (their hash won't match the pre-built hash in flashinferMetaInfo.h)
    if file_name in _source_compiled_cubins:
        try:
            with open(cubin_path, "rb") as f:
                return f.read()
        except FileNotFoundError:
            logger.warning(
                f"Source-compiled cubin registered but not found: {cubin_path}"
            )
