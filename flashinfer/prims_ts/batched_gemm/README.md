...
| Activations | TRT-LLM Gen APIs expose SwiGLU, GeGLU, ReLU2, and Identity combinations across the tested dtype paths; BF16 and MxFP8 OA params are restricted to SwiGLU. | PrimsTS activation support is config-driven. The raw enum is wider than the local MoE JSON config set, so missing BF16/FP4/FP8 activation combinations are usually JSON config/tensor-adapter work. DeepSeek FP8 supports SwiGLU and SiTU through its separate activation/quantization kernel; MXFP8 applies them in the fused FC1 epilogue. |
