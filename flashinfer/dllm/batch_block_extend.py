...
class BatchBlockExtendAttentionWrapper:
    ...
    def run(
        ...
        return self._inner_wrapper.run(q, k, v, effective_sm_scale, self._dllm_block_size, return_lse=return_lse)
    
    def run_return_lse(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        return self.run(q, k, v, return_lse=True)
...
class BatchBlockExtendPagedAttentionWrapper:
    ...
    def run(
        ...
        assert self._inner_wrapper is not None, "Must call plan() before run()"
        effective_sm_scale = sm_scale if sm_scale is not None else self._sm_scale
        return self._inner_wrapper.run(q, paged_kv_cache, effective_sm_scale, self._dllm_block_size, return_lse=return_lse)
