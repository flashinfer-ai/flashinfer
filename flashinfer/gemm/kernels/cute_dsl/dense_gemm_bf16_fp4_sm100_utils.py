...
"""Low-level conversion helpers for the SM100 NVFP4/BF16 GEMM kernel."""
...
from cutlass.cutlass_dsl import BFloat16, T, Uint32, dsl_user_op
...
def fmin(
    ...
    *,
    ...
    """CUDA-version-independent NVVM minimum used by the epilogue template."""
...
@dsl_user_op
def blk_reduce_bf16(dst_gmem, src_smem, size, *, loc=None, ip=None):
    """Emit the BF16 bulk-reduction primitive used by the epilogue template."""
    llvm.inline_asm(
        None,
        [
            dst_gmem.iterator.llvm_ptr,
