...
The cuTile kernel and autotune logic follow NVIDIA TileGym
(https://github.com/NVIDIA/TileGym, ``src/tilegym/ops/cutile/fp8_quantization_matmul.py``),
using the public ``cuda.tile`` API only (no TileGym runtime dependency).

Implementation notes:

* ``from __future__ import annotations`` must NOT be used here — it would convert
  the ``ct.Constant[int]`` annotations into strings at definition time and break
  ``cuda.tile``'s runtime introspection of the
  ``Annotated[int, ConstantAnnotation()]`` metadata.

* Two kernel variants live here: ``_w8a8_block_fp8_matmul_kernel`` (the
  ``ct.gather``/``ct.scatter`` baseline, kept for verification/fallback) and
  ``_w8a8_block_fp8_matmul_kernel_tma`` (tiled ``ct.load``/``ct.store``, TMA). The
  launcher defaults to the TMA kernel (``use_tma=True``).
"""
