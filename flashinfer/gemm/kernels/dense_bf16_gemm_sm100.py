...
class Sm100PersistentDenseGemmKernel:
    ...
    Supports BF16/FP16 inputs with FP32 accumulator and BF16/FP16/FP32 output.
    ...
    Example:
        >>> gemm = Sm100PersistentDenseGemmKernel(
        ...     acc_dtype=cutlass.Float32,
        ...     use_2cta_instrs=False,
        ...     mma_tiler_mn=(128, 128),
        ...     cluster_shape_mn=(1, 1),
        ... )
        >>> gemm.wrapper(m, n, k, 1, a_ptr, b_ptr, c_tensor, max_active_clusters, stream)
    """
    ...
    def is_valid_tensor_alignment(
        ...
        if (
            not check_contiguous_16B_alignment(
                ab_dtype, a_major == "m", (m, k, batch_size)
            )
            or not check_contiguous_16B_alignment(
                ab_dtype, b_major == "n", (n, k, batch_size)
            )
            or not check_contiguous_16B_alignment(
                c_dtype, c_major == "m", (m, n, batch_size)
            )
        ):
            return False
        return True
    ...
    def can_implement(
        ...
        if not Sm100PersistentDenseGemmKernel.check_supported_dtypes(
            ...
            return False
        if not Sm100PersistentDenseGemmKernel.is_valid_mma_tiler_and_cluster_shape(
            use_2cta_instrs, mma_tiler_mn, cluster_shape_mn
        ):
            return False
        if not Sm100PersistentDenseGemmKernel.is_valid_tensor_alignment(
    ...
    def wrapper(
        ...
        """Execute the GEMM with dynamic M/N/K from raw pointers.

        Args:
            m: M dimension of the GEMM problem.
            n: N dimension of the GEMM problem.
            k: K dimension of the GEMM problem.
            batch_size: Batch dimension (typically 1 for mm_bf16).
            a_ptr: Pointer to A tensor (M x K x batch_size, K innermost / row-major).
            b_ptr: Pointer to B tensor (N x K x batch_size, K innermost).
            c_tensor: Output tensor (M x N x batch_size) as cute.Tensor.
