...
from ..jit.gemm import gen_gemm_sm120_module_cute_mxfp8
...
def get_gemm_sm120_module_cute_mxfp8():
    """MXFP8 group GEMM module accessor for SM120."""
...
def quantize_mxfp8_for_zero_padding(
    ...
    scale tensor matching the input layout expected by ``group_gemm_mxfp8_nt_groupwise``
    with ``mode="zero_padding"``.
...
def group_gemm_mxfp8_nt_groupwise_zero_padding(
    ...
    r"""Perform group GEMM with MXFP8 inputs in zero-padding mode using groupwise UE8M0
    ...
    get_gemm_sm120_module_cute_mxfp8().group_gemm_mxfp8_nt_groupwise_zero_padding(
        a,
        b,
        a_scale,
        b_scale,
...
def group_gemm_mxfp8_nt_groupwise(
    ...
    r"""Perform group GEMM with MXFP8 inputs using groupwise UE8M0 scaling, with the
    DeepGEMM-style contiguous group layout. Signature aligned with
    ``flashinfer.deep_gemm.m_grouped_fp8_gemm_nt_contiguous``. Currently only supported on
    NVIDIA RTX PRO 6000 Blackwell (SM120) architecture.

    Parameters
    ----------
    a: torch.Tensor
        Row-major input tensor shape ``(m, k)``, data type is ``torch.float8_e4m3fn``.
    ...
    use_psum_layout: bool
        If ``True``, dispatch to the psum-layout kernel variant which accepts pre-padded
        per-expert M segments. Default ``False`` (contiguous layout).

    out: Optional[torch.Tensor]
        The output tensor, shape ``(m, n)``. If not specified, an output tensor will be
        created.
    ...
    get_gemm_sm120_module_cute_mxfp8().group_gemm_mxfp8_nt_groupwise(
        a,
        b,
        a_scale,
        b_scale,
...
def group_gemm_mxfp8_nt_groupwise_masked(
    ...
    r"""Perform masked grouped GEMM with MXFP8 inputs using groupwise UE8M0 scaling.
    Signature aligned with ``flashinfer.deep_gemm.m_grouped_fp8_gemm_nt_masked``.
    Currently only supported on NVIDIA RTX PRO 6000 Blackwell (SM120) architecture.

    Each expert's A slice ``a[i]`` is multiplied with ``b[i]``; only the first
    ``masked_m[i]`` rows of ``a[i]`` are considered valid (the rest are ignored).

    Parameters
    ----------
    a: torch.Tensor
        Per-expert padded input tensor shape ``(num_experts, max_m, k)``, data type
        is ``torch.float8_e4m3fn``.

    b: torch.Tensor
        Per-expert weight tensor shape ``(num_experts, n, k)``, data type is
        ``torch.float8_e4m3fn``.
    ...
    masked_m: torch.Tensor
        ...
        ``torch.int32``. ``masked_m[i] = j`` means only the first ``j`` rows of
        ``a[i]`` are computed; the rest are ignored. Mirrors DG / 6KD canonical
        naming.

    scale_granularity_mnk: Tuple[int, int, int]
        Accepted values: ``(1, 1, 128)`` (default) or ``(1, 1, 32)``.

    out: Optional[torch.Tensor]
        Output tensor, shape ``(num_experts, max_m, n)``. If not specified, created.

    out_dtype: Optional[torch.dtype]
        Output dtype. Currently only ``torch.bfloat16`` is supported.
    ...
    num_experts, max_m, _ = a.shape
    n = b.shape[1]
    if out is None:
        out = torch.empty((num_experts, max_m, n), dtype=out_dtype, device=a.device)

    get_gemm_sm120_module_cute_mxfp8().group_gemm_mxfp8_nt_groupwise_masked(
        a,
        b,
        a_scale,
        b_scale,
...
def gemm_mxfp8_nt_groupwise(
    ...
    r"""Perform single MXFP8 matmul with groupwise UE8M0 scaling (dense Normal entry).
    Currently only supported on NVIDIA RTX PRO 6000 Blackwell (SM120) architecture.
    ...
    scale_granularity_mnk: Tuple[int, int, int]
        Accepted values: ``(1, 1, 128)`` (default) or ``(1, 1, 32)``. Per-row contract
        (cute backend) — distinct from DG legacy ``(1, 128, 128)`` 2D-block contract.

    out: Optional[torch.Tensor]
        Output tensor, shape ``(m, n)``. If not specified, created internally.
    ...
    get_gemm_sm120_module_cute_mxfp8().gemm_mxfp8_nt_groupwise(
        a,
        b,
        a_scale,
        b_scale,
...
def batch_gemm_mxfp8_nt_groupwise(
    ...
    r"""Perform batched MXFP8 matmul with groupwise UE8M0 scaling (dense Batched entry).
    Currently only supported on NVIDIA RTX PRO 6000 Blackwell (SM120) architecture.

    Each batch ``a[i]`` is multiplied with ``b[i]``.

    Parameters
    ----------
    a: torch.Tensor
        Input tensor shape ``(num_groups, m, k)``, k-major (last stride must be 1),
        data type is ``torch.float8_e4m3fn``.

    b: torch.Tensor
        Weight tensor shape ``(num_groups, n, k)``, k-major (last stride must be 1),
        data type is ``torch.float8_e4m3fn``.

    a_scale: torch.Tensor
        Int32-packed UE8M0 scale tensor for ``a``, shape ``(num_groups, m, k_align)``.
        Data type ``torch.int32``.

    b_scale: torch.Tensor
        Int32-packed UE8M0 scale tensor for ``b``, shape ``(num_groups, n, k_align)``.
        Data type ``torch.int32``.

    scale_granularity_mnk: Tuple[int, int, int]
        Accepted values: ``(1, 1, 128)`` (default) or ``(1, 1, 32)``.

    lda, stride_a, ldb, stride_b, ldd, stride_d: Optional[int]
        Optional explicit strides passed to the 6KD runner. If ``None``, defaults are
        read from ``a.strides()`` / ``b.strides()`` / ``out.strides()`` (row stride for
        ``ld*``, batch stride for ``stride_*``). Explicit pass overrides.

    out: Optional[torch.Tensor]
        Output tensor, shape ``(num_groups, m, n)``. If not specified, created.
    ...
    n = b.shape[1]
    if out is None:
        out = torch.empty((num_groups, m, n), dtype=out_dtype, device=a.device)

    _lda = a.stride(1) if lda is None else lda
    _stride_a = a.stride(0) if stride_a is None else stride_a
    _ldb = b.stride(1) if ldb is None else ldb
    _stride_b = b.stride(0) if stride_b is None else stride_b
    _ldd = out.stride(1) if ldd is None else ldd
    _stride_d = out.stride(0) if stride_d is None else stride_d

    get_gemm_sm120_module_cute_mxfp8().batch_gemm_mxfp8_nt_groupwise(
        a,
        b,
        a_scale,
        b_scale,
