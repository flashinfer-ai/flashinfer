...
Gated Delta Net (GDN-2) Chunked Prefill - Blackwell SM100 Adapter
=================================================================

Bridges FlashInfer's PyTorch-based API to the CuTe DSL chunked GDN-2 kernel
for SM100 (Blackwell).

GDN-2 differs from GDN at the interface by adding a second update gate ``w``
and making both ``beta`` and ``w`` *channel-wise* tensors (last dim is the
head dimension rather than a per-head scalar):

    beta: (total_tokens, H, DK)   w: (total_tokens, H, DV)
...
from .gated_delta_net_chunked2 import GatedDeltaNetChunkedKernel2
...
def chunk_gated_delta_rule2_sm100(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    gate: torch.Tensor,
    ...
    """Execute the Blackwell chunked GDN-2 prefill kernel.
    ...
    Args:
        ...
        beta: ``(total_tokens, HO, DK)`` bf16, channel-wise erase gate
        w: ``(total_tokens, HO, DV)`` bf16, channel-wise write gate
    ...
    if "compiled" not in cache:
        ...
        gdn = GatedDeltaNetChunkedKernel2(
            io_dtype=io_dtype,
