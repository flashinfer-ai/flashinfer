...
"""Source-only four-stage context-parallel GDN prefill for SM100/SM103.
...
from ...jit.source_gdn_cp_prefill import (
    GDNCPArch,
    gdn_cp_nvcc_version as _gdn_cp_nvcc_version,
    load_gdn_cp_kernel,
)
...
def _arch_for(device: torch.device) -> GDNCPArch:
    ...
    if capability == (10, 3):
        return "sm_103a"
    raise ValueError(
        "Blackwell GDN CP-prefill requires exact compute capability 10.0 or 10.3, "
...
def _build_plan(
    ...
    *,
    ...
    if head_dim != _HEAD_DIM or not _head_mapping_is_legal(hq, hk, hv):
        raise ValueError(
            "Blackwell PR4078 CP-prefill requires D=128 and equal-head, GQA, or GVA "
    ...
    # barrier topology.  The Blackwell SIMT fixup owns mixed empty/nonempty batches.
    fixup_kernel = (
...
class GDNCPPrefill:
    """Internal fixed-address Blackwell replay.
    ...
    def __init__(
        ...
        *,
        ...
        if needs_gather:
            ...
            self._gather = load_gdn_cp_kernel(f"state_gather_{suffix}", plan.arch)
...
def prepare_gdn_cp_prefill(
    ...
    *,
    ...
    """Run once and optionally capture one internal fixed-address Blackwell graph."""

    if not q.is_cuda:
        raise ValueError("Blackwell GDN CP-prefill requires CUDA tensors")
...
def chunk_gated_delta_rule_source_sm100(
    output: torch.Tensor,
    output_state: torch.Tensor | None,
    q: torch.Tensor,
    k: torch.Tensor,
    ...
    *,
    ...
    """Public dispatcher target; every accepted SM100/SM103 route uses Blackwell."""
    ...
    if capturing:
        if not (
            ...
            raise RuntimeError(
                "Blackwell GDN CP-prefill metadata must be warmed with the same "
    ...
    if _public_prepared is None or _public_key != key:
        if capturing:
            raise RuntimeError(
                "Blackwell GDN CP-prefill plan must be warmed before CUDA graph capture"
            )
...
__all__ = [
    ...
    "chunk_gated_delta_rule_source_sm100",
