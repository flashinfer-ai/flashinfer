...
def run_gdn_verify_kernel_mtp(
    ...
    # B comes from the (dynamic) runtime shape so one compiled kernel serves
    # every batch size that maps to the same config.
...
def run_mtp_decode(
    ...
    # cu_seqlens is a dummy (is_varlen=False) but must match the runtime B.
