...
def cp_delta_rule_t_precompute_dsl_sm120(
    ...
    *,
    ...
    if not _skip_check:
        ...
        if k.shape[-1] != 128:
            raise RuntimeError(f"CPDeltaRuleTPrecomputeSm120 only supports D=128, got {k.shape[-1]}")
        if k.dtype not in (torch.float16, torch.bfloat16):
            raise RuntimeError(f"CPDeltaRuleTPrecomputeSm120 only supports fp16/bf16 inputs, got {k.dtype}")
...
class CPDeltaRuleMNPrecomputeSm120(KeyedCompileMixin):
    ...
    def store_acc_to_smem(
        ...
        stsm_atom = cute.make_copy_atom(warp.StMatrix8x8x16bOp(transpose=transpose, num_matrices=4), self.dtype)
    ...
    def store_acc_to_gmem(
        ...
        copy_atom  = cute.make_copy_atom(cute.nvgpu.CopyR2GOp(), self.acc_dtype, num_bits_per_copy=64)
    ...
    def mask_alpha_oob_block(
        ...
        for i in cutlass.range_constexpr(2):
            ...
            tok = blk * self.BLK + row
            if tok >= chunk_len:
                sAlpha_stage[row, AlphaProcessor.CUMPROD_NEG_END_RCP] = cutlass.Float32(0.0)
    ...
    def compute_loop_body(
        ...
        tXrK = load_tensor_as_a(sK, x_tiled_mma, thread_idx, (self.D, self.BLK), self.dtype, False)
        tXrT = load_tensor_as_b(sT_blk, x_tiled_mma, thread_idx, (self.BLK, self.BLK), self.dtype, True)
        ...
        ldsm_n4 = cute.make_copy_atom(warp.LdMatrix8x8x16bOp(transpose=False, num_matrices=4), self.dtype)
        ...
        tZrK    = cute.make_rmem_tensor(z_thr_mma.partition_shape_B(cute.slice_(z_tile_shape, (0, None, None))), self.dtype)
        ...
        tTRANSrX = load_tensor_as_b(sX_DS, trans_tiled_mma, thread_idx, (self.D, self.BLK), self.dtype, True)
        ...
        tSTATErX = load_tensor_as_b(sX_DS, state_tiled_mma, thread_idx, (self.D, self.BLK), self.dtype, True)
    ...
    def run_load_alpha_role(
        ...
        for blk in cutlass.range(num_blocks, unroll=1):
            ...
            self.mask_alpha_oob_block(sAlpha, blk, chunk_len, alpha_producer_state.index, tidx)
            cute.arch.fence_view_async_shared()
...
def cp_delta_rule_mn_precompute_dsl_sm120(
    ...
    *,
    ...
    if not _skip_check:
        if k.shape[-1] != 128:
            raise RuntimeError(f"CPDeltaRuleMNPrecomputeSm120 only supports D=128, got {k.shape[-1]}")
        if k.dtype not in (torch.float16, torch.bfloat16):
            raise RuntimeError(f"CPDeltaRuleMNPrecomputeSm120 only supports fp16/bf16 inputs, got {k.dtype}")
        if t.dtype != k.dtype:
...
class CPDeltaRuleFixupSm120(KeyedCompileMixin):
    ...
    def run_fixup_loop(
        ...
        for chunk_idx in cutlass.range(start, num_chunks, unroll=1):
            ...
            TF32.load_tf32_kpermuted_b(tSMrM, sM_NK[None, None, m_consumer_state.index], lane_idx)
            for iter_k in cutlass.range_constexpr(self.k_tiles):
                ...
                if cutlass.const_expr(self.use_3xtf32):
                    tSMrM_l = TF32.convert_fp32_to_tf32_residual(tSMrM[None, None, iter_k])
                    cute.gemm(tiled_mma, tSMrState, tSMrS_h, tSMrM_l, tSMrState)
                    tSMrS_l = TF32.convert_fp32_to_tf32_residual(tSMrS[None, None, iter_k])
                    tSMrM_h = cute.recast_tensor(tSMrM[None, None, iter_k], cutlass.TFloat32)
    ...
    def kernel(
        ...
        gLocalState = cute.make_tensor(g_local_state_t.iterator.align(128), workspace_layout)
        ...
        tiled_copy_m = cute.make_tiled_copy_tv(copy_atom, copy_thr_layout, copy_m_val_layout)
        tiled_copy_n = cute.make_tiled_copy_tv(copy_atom, copy_thr_layout, copy_n_val_layout)
            ...
            if num_chunks > 0:
                ...
                if cutlass.const_expr(self.needs_initial_state):
                    self.load_fixup_register_fragment(
                        tiled_copy_n, gLocalState_cta[None, None, chunk_start], tNrN, local_tidx)
                    self.load_fixup_register_fragment(
                        tiled_copy_m, gTransfer_seq[None, None, chunk_start], tMrM, local_tidx)
                    for chunk_idx in cutlass.range(0, num_chunks - 1, 1, unroll=1):
                        ...
                        self.load_fixup_register_fragment(
                            tiled_copy_n, gLocalState_cta[None, None, chunk_start + chunk_idx + 1], tNrN, local_tidx)
                        m_producer_state = self.store_fixup_register_fragment(
                            tiled_copy_m, tMrM, sM, m_pipeline, m_producer_state, local_tidx)
                        self.load_fixup_register_fragment(
                            tiled_copy_m, gTransfer_seq[None, None, chunk_start + chunk_idx + 1], tMrM, local_tidx)
                    n_producer_state = self.store_fixup_register_fragment(
                        tiled_copy_n, tNrN, sN, n_pipeline, n_producer_state, local_tidx)
                    m_producer_state = self.store_fixup_register_fragment(
                        tiled_copy_m, tMrM, sM, m_pipeline, m_producer_state, local_tidx)
                    ...
                    if num_chunks > 1:
                        self.load_fixup_register_fragment(
                            tiled_copy_m, gTransfer_seq[None, None, chunk_start + 1], tMrM, local_tidx)
                        for chunk_idx in cutlass.range(0, num_chunks - 1, 1, unroll=1):
                            n_producer_state = self.store_fixup_register_fragment(
                                tiled_copy_n, tNrN, sN, n_pipeline, n_producer_state, local_tidx)
                            self.load_fixup_register_fragment(
                            ...
                            if chunk_idx + 2 < num_chunks:
                                self.load_fixup_register_fragment(
                                    tiled_copy_m, gTransfer_seq[None, None, chunk_start + chunk_idx + 2], tMrM, local_tidx)
                    n_producer_state = self.store_fixup_register_fragment(
                        tiled_copy_n, tNrN, sN, n_pipeline, n_producer_state, local_tidx)
        else:
            if num_chunks > 0:
    ...
    *,
    ...
    if not _skip_check:
        ...
        if local_transfer.shape[-2:] != (128, 128):
            raise RuntimeError(f"CPDeltaRuleFixupSm120 only supports D=128, got {tuple(local_transfer.shape[-2:])}")
...
class CPDeltaRulePrefillSm120(KeyedCompileMixin):
    ...
    def run_aux_loop_body(
        ...
        tQKrQ = load_tensor_as_a(sQ, qk_tiled_mma, aux_tidx, (self.BLK_Q, self.D), self.dtype, True)
        tQKrK = load_tensor_as_b(sK, qk_tiled_mma, aux_tidx, (self.BLK_KV, self.D), self.dtype, True)
        tQKrQK = qk_thr_mma.make_fragment_C(qk_thr_mma.partition_shape_C((self.BLK_Q, self.BLK_KV)))
        ...
        tQKrQK_cvt = self.cp_qk_epi_converted(
            tQKrQK, tQKcMqk, sAlpha, alpha_consumer_state.index,
    ...
    def compute_loop_body(
        ...
        if cutlass.const_expr(self.needs_alpha):
            alpha_pipeline.consumer_wait(alpha_consumer_state)
            cute.arch.fence_view_async_shared()
        tOrO = cute.make_rmem_tensor(o1_thr_mma.partition_shape_C((d, blk_q)), self.acc_dtype)
        ...
        k_pipeline.consumer_wait(k_consumer_state)
        tSKrSK = cute.make_rmem_tensor(sk_thr_mma.partition_shape_C((d, blk_kv)), self.acc_dtype)
        ...
        tNewVrC = cute.make_rmem_tensor(newv_thr_mma.partition_shape_C((d, blk_kv)), self.acc_dtype)
        ...
        self._math_order_wait(wg_idx)
        cute.copy(newv_tiled_copy_B, tNewVsB[None, None, None, kk_consumer_state.index], tNewVrB_cv)
        ...
        tOrQK    = o2_thr_mma.make_fragment_B(o2_thr_mma.partition_B(sQK[None, None, qk_consumer_state.index]))
        tOrQK_cv = o2_thr_copy_B.retile(tOrQK)
        tOsQK    = o2_thr_copy_B.partition_S(sQK)
        self._math_order_wait(wg_idx)
        cute.copy(o2_tiled_copy_B, tOsQK[None, None, None, qk_consumer_state.index], tOrQK_cv)
        ...
        self.o_store(tOrO, tOsO[None, None, None, o_stage], o_tiled_copy_r2s, o_thr_copy_r2s)
        o_pipeline.producer_commit(o_producer_state)
        o_producer_state.advance()

        block_coeff = cutlass.Float32(1.0)
        if cutlass.const_expr(self.needs_alpha):
            block_coeff = cutlass.Float32(sAlpha[B - cutlass.Int32(1), AlphaProcessor.CUMPROD, alpha_stage])
        for i in cutlass.range(cute.size(tKVrKV), unroll_full=True):
    ...
    def run_cp_state_math_role(
        ...
        if cutlass.const_expr(produce_aux):
            aux_q_consumer_state = pipeline.make_pipeline_state(pipeline.PipelineUserType.Consumer, self.q_stage)
            aux_k_consumer_state = pipeline.make_pipeline_state(pipeline.PipelineUserType.Consumer, self.k_stage)
            aux_t_consumer_state = pipeline.make_pipeline_state(pipeline.PipelineUserType.Consumer, self.t_stage)
            aux_qk_producer_state = pipeline.make_pipeline_state(pipeline.PipelineUserType.Producer, self.qk_stage)
            aux_kk_producer_state = pipeline.make_pipeline_state(pipeline.PipelineUserType.Producer, self.kk_stage)
            ...
            qk_tiled_mma = cute.make_tiled_mma(
                aux_mma_atom, cute.make_layout((4, 1, 1)),
        ...
        if cutlass.const_expr(produce_aux):
            (
                ...
                q_pipeline, aux_q_consumer_state,
                k_pipeline, aux_k_consumer_state,
                t_pipeline, aux_t_consumer_state,
                qk_pipeline, aux_qk_producer_state,
                kk_pipeline, aux_kk_producer_state,
                alpha_pipeline, aux_alpha_consumer_state,
                work_desc, scale, cutlass.Int32(0), True,
                qk_tiled_mma, kk_tiled_mma,
                tQKcMqk, tKKcMkk, aux_tidx,
            )
        ...
        if cutlass.const_expr(self.needs_initial_state):
            (
                ...
                sQ_SD, sK_SD, sK_DS, sV_DS, sQK, sKK_opd, sO, sAlpha,
                kv_tiled_mma,
            ...
            if warp_group_idx == CPPrefillWarpGroupRole.LDST:
                ...
                if warp_group_idx == CPPrefillWarpGroupRole.MATH_STATE0:
                    self.run_cp_state_math_role(
                        sQ_SD, sK_SD, sK_DS, sV_DS, sT, sQK, sKK_opd,
                        sO, sAlpha,
                        q_pipeline, k_pipeline, v_pipeline, t_pipeline, o_pipeline,
...
def cp_delta_rule_prefill_dsl_sm120(
    ...
    *,
    ...
    if not _skip_check:
        ...
        if not _FullyFusedDeltaRuleSm120.can_implement(num_q_heads, num_k_heads, num_v_heads, d, q.element_size()):
            raise RuntimeError(
        ...
        if q.shape[-1] != 128:
            raise RuntimeError(f"CPDeltaRulePrefillSm120 only supports D=128, got {q.shape[-1]}")
        if q.dtype not in (torch.float16, torch.bfloat16):
            raise RuntimeError(f"CPDeltaRulePrefillSm120 only supports fp16/bf16 inputs, got {q.dtype}")
    ...
    kernel = CPDeltaRulePrefillSm120(kernel_dtype, needs_initial_state=needs_initial_state)
    kernel_args = (
...
def cp_delta_rule_dsl_sm120(
    ...
    *,
    ...
    if q.dtype not in (torch.float16, torch.bfloat16):
        raise RuntimeError(f"CPDeltaRuleSm120 only supports fp16/bf16 inputs, got {q.dtype}")
