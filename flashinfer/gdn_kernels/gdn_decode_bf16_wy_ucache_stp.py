"""Gated Delta-Net (GDN) decode — DEDICATED STP (T=1) kernel with
vLLM-Triton-compatible FLUSH SEMANTICS (fold ABSORBS the current token).

Fork of ``gdn_decode_bf16_wy_ucache_flush.py`` (read its docstring first —
the TMA/MMA engine, precision modes, and verify math are identical),
specialized for single-token prediction:

  - SINGLE-ROW ring appends: only the 1 real token's (normed-k, u, g) entry
    is written per step (the T=4 tile's 3 zero filler rows are never
    appended), so the ring never needs filler headroom.
  - FOLD-ABSORBS-TOKEN flush (vLLM ReplaySSM semantics): a flush commits
        S_new = a0 * (e^{G_P} S0 + sum_{j<P} w_j u_j k_j^T) + u_0 k_0^T
    (a0 = e^{la_0}) — the CURRENT token is folded into the checkpoint and
    the window restarts EMPTY (hist_len <- 0), matching the Triton kernel's
    ``is_flush`` fold exactly. Implemented as a pure RE-WEIGHTING of the
    parent's fold GEMM: the fold-local w_j are scaled by a0, the new token's
    (u_0, k_0) ride as GEMM row P with weight 1.0 (u_0 read back from its
    own ring append; k_0 patched into the khist tile from the CTA's own
    packed normed-k row — sibling CTAs never race on the leader's k append),
    and bdec is scaled by a0. The MMA/epilogue machinery is untouched.
  - FLAT 16-slot buffer (RING_SLOTS = 16, half the parent's ring memory,
    ~324 KB/request-slot at Qwen3.5-122B geometry — parity with vLLM):
    cache_base stays 0 forever; appends land at slot P <= 15.
  - flush_min in [1, 15]; default 15 == vLLM's flush-at-(L-1): 14 verify
    steps + 1 fold step per 16-token cycle, replay depth <= 15.
  - Cursor commit (fused kernel): flush rows hist_len <- 0; verify rows
    hist_len <- P + 1. Outputs y are IDENTICAL to the parent's for the same
    inputs (the fold change only affects what is written to the pool).

Entry point: ``gated_delta_rule_stp_ucache_flush`` (same signature/contract
as the parent module's STP wrapper, incl. the ``prepadded`` zero-copy path;
inputs are still the [B, 4, ...] tile with rows 1..3 zero).
"""
...
# Physical buffer: FLAT 16 slots (STP fork; see module docstring). The
# parent's rotating-32 headroom exists for multi-row appends + accept
# rewind; this fork appends 1 row/step and never appends on flush, so
# slot P <= 15 always suffices and cache_base stays 0.
RING_SLOTS = 16
    ...
    def kernel(
        ...
        if const_expr(self._t_input <= 8):
            ...
            if warp_id == Int32(1):
                ...
                # (stp) publish a0 = e^{la_0} for the fold-absorb
                # re-weighting (sGamma[0] = the token's log-decay; published
                # by warp 3 before this norm phase, same ordering as the
                # g-append read below). Slot 33 is unused in the parent.
                if lane_id == Int32(0):
                    sGhist.iterator[33] = _exp_approx_f32(sGamma.iterator[0])
                if lane_id < Int32(1):
                    # (stp) SINGLE-ROW ring append (token 0 only; the 3
                    # filler rows are never appended). Flush rows append
                    # too — the entry is dead as history (hist resets to 0)
                    # but the u row doubles as the fold's u_0 source.
                    _g_app_val = (
        ...
        if const_expr(_RING_MIXED and not _PACK_MIXED):
            ...
            _kr3 = _kv3
        if (pid_hv % (HV // H)) == 0:
            # (stp) SINGLE-ROW ring append: token 0's normed k only.
            if tidx < Int32(K_DIM // 8):
                _st_global_v4_b32(
        ...
        if P_hist >= flush_min:
            ...
            # (stp fold-absorb) row P of the fold's khist operand must be
            # the CURRENT token's normed k. Source it from the CTA's OWN
            # packed sK row 0 (still resident: k_buf is re-tenanted only at
            # the tail u-reload) — NOT from the gmem ring, whose row P was
            # appended by the k-group leader CTA only (cross-CTA visibility
            # within a launch is not guaranteed). Ring-dtype repack matches
            # the append path (compile-time no-op in the default mode).
            if (tidx // Int32(8)) == P_hist:
                _ksrc_off = (tidx % Int32(8)) * Int32(8)
                _khs0 = _sK_i32.iterator[_ksrc_off + 0]
                _khs1 = _sK_i32.iterator[_ksrc_off + 1]
                _khs2 = _sK_i32.iterator[_ksrc_off + 2]
                _khs3 = _sK_i32.iterator[_ksrc_off + 3]
                _khs4 = _sK_i32.iterator[_ksrc_off + 4]
                _khs5 = _sK_i32.iterator[_ksrc_off + 5]
                _khs6 = _sK_i32.iterator[_ksrc_off + 6]
                _khs7 = _sK_i32.iterator[_ksrc_off + 7]
                if const_expr(_RING_MIXED and not _PACK_MIXED):
                    _khs0 = _repack_io2_to_rg2(_khs0)
                    _khs1 = _repack_io2_to_rg2(_khs1)
                    _khs2 = _repack_io2_to_rg2(_khs2)
                    _khs3 = _repack_io2_to_rg2(_khs3)
                    _khs4 = _repack_io2_to_rg2(_khs4)
                    _khs5 = _repack_io2_to_rg2(_khs5)
                    _khs6 = _repack_io2_to_rg2(_khs6)
                    _khs7 = _repack_io2_to_rg2(_khs7)
            sync_threads()
        ...
        for _fl_pass in cutlass.range_constexpr(2 if self._t_input > 8 else 1):
            ...
            if const_expr(self._t_input >= 8):
                ...
                # (stp) STG only the 1 REAL output row — rows 1..3 are the
                # zero filler tokens' outputs; the parent writes all
                # t_input rows (4x the T=1 write traffic for nothing).
                if _fl_row < Int32(1):
                    _st_global_v4_b32(_gOut_base, _fl_off, _v0, _v1, _v2, _v3)
        ...
        # (stp) SINGLE-ROW ring append: token 0's correction u only.
        if tidx < Int32(V_DIM_C // 8):
        ...
        if P_hist >= flush_min:
            ...
            for _fr in cutlass.range_constexpr(2):
                ...
                # (stp fold-absorb) also reload row P — the CURRENT token's
                # u, appended by THIS CTA above (same-CTA gmem RAW, ordered
                # by the fold-entry sync_threads; .cg reads through L2).
                if _fr_row < P_hist + Int32(1):
                    _cp_async_bf16x8_cg(
                        _gUC_base,
                        _uc_base
            ...
            for _ws in cutlass.range_constexpr(9):
                ...
                if _ws_r < Int32(W_RING):
                    # (stp fold-absorb) window rows carry a0*w_j (the token's
                    # decay applied to the folded history); row P is the
                    # token itself with weight 1.0; rows > P keep w=0.
                    _a0 = sGhist.iterator[33]
                    _w_row = sGhist.iterator[W_RING + _ws_r] * _a0
                    if _ws_r == P_hist:
                        _w_row = f32(1.0)
            ...
            #        16-B chunks. ---
            # (stp fold-absorb) bdec' = a0 * e^{G_P}: the token's decay
            # applied to the checkpoint term as well.
            _bdec_t = sGhist.iterator[32] * sGhist.iterator[33]
...
def gated_delta_rule_mtp_ucache_flush(
    ...
    if restart_hist_on_flush:
        ...
        if not torch.cuda.is_current_stream_capturing():
            # (stp) single-row appends: slot P must exist -> P <= 15.
            _hl_cap = RING_SLOTS - 1
            _hl_min, _hl_max = (int(x.item()) for x in hist_len.aminmax())
            assert _hl_min >= 0 and _hl_max <= _hl_cap, (
                f"hist_len out of legal range [0, {_hl_cap}] "
                f"(W_RING={W_RING}, RING_SLOTS={RING_SLOTS}, T={T}): "
    ...
    # (stp) cap = 15: single-row appends need slot P <= 15, and the fold
    # (which absorbs the current token) replays at most 15 window rows —
    # exactly vLLM's flush-at-(L-1) operating point.
    _fm_cap = RING_SLOTS - 1
    if flush_min is None:
        flush_min = _fm_cap  # lazy: flush exactly when [P, P+T) overflows
    assert 1 <= flush_min <= _fm_cap, (
        f"flush_min={flush_min} out of range [1, {_fm_cap}] for T={T}, "
        f"RING_SLOTS={RING_SLOTS} (above the cap, a request could neither "
        "append nor flush without corrupting the live window)."
    )
...
# ============================================================================
# gated_delta_rule_stp_ucache_flush — single-token-prediction (STP,
# non-speculative) decode: the T=1 case of the ucache verify+flush contract.
#
# There is no dedicated T=1 compile-time specialization: the kernel's native
# masking (n_valid < t_input) does not correctly gate its warp-specialized
# output-write path below T=4 (some batch rows silently write nothing — see
# the T=1 kernel-request writeup for the repro). Instead this wraps the
# VALIDATED T=4 kernel: the 1 real token is placed at row 0 of a 4-row tile,
# rows 1..3 are zero-padded no-op filler tokens (same zero-fill the base
# kernel already uses for any T < T_KERNEL — "load-bearing zero rows", see
# the module docstring), and only row 0 of the output is real.
#
# Why the filler rows are provably inert:
#   - output: y[t] is computed and written before token t+1 is processed, so
#     row 0's output cannot depend on rows 1..3.
#   - fold: the state written back to the pool on a flush is S_after_history
#     (computed from the RING only, before any of the T new tokens) — rows
#     1..3 never reach the pool.
#   - ring: rows 1..3 append REAL (non-garbage, zero-derived) k/u/g entries
#     at ring slots (base+P+1)..(base+P+3) — physically past the live window
#     [base, base+P), so they are never read as history. This function
#     commits hist_len = P + 1 (not +4), so the NEXT call's row-0 append
#     lands at exactly slot (base+P+1) and overwrites row 1's filler entry
#     before it could ever be read; rows P+2/P+3 are overwritten the same
#     way two/three calls later. No sibling CTA reads ahead of hist_len, so
#     this is safe under the same ring invariant the T>=4 paths rely on.
#
# Trade-off vs a true native T=1 kernel: flush_min is capped at the T=4
# kernel's own bound (W_RING - 4 + 1 = 13, vs the 16 a dedicated T=1 tile
# could reach), so this path folds ~2 calls earlier per window on average.
# Correctness is unaffected; only fold frequency (and thus amortized fold
# cost) changes.
# ============================================================================
_STP_PAD: dict = {}


# --- fused STP cursor commit -------------------------------------------------
# The "accept exactly 1 token" commit (flush rows: base'=(base+P)&MASK,
# len'=1; verify rows: len'=P+1) expressed as torch elementwise ops costs
# ~8 tiny kernel launches per step (~15 us under CUDA-graph replay) — larger
# than the T=4 kernel itself at small B. This single [B]-element kernel
# replaces the whole chain with one graph node.
@cute.kernel
def _stp_commit_kernel(
    hist_len: cute.Tensor,
    cache_base: cute.Tensor,
    flush_min: cutlass.Constexpr[int],
):
    tidx, _, _ = cute.arch.thread_idx()
    bidx, _, _ = cute.arch.block_idx()
    i = bidx * 128 + tidx
    B = cute.size(hist_len.shape[0])
    if i < B:
        P = hist_len[i]
        # (stp fold-absorb) a flush folded the window AND the current token
        # into the checkpoint: the buffer restarts EMPTY and, with flat
        # addressing, cache_base stays 0 forever.
        if flush_min <= P:
            hist_len[i] = cutlass.Int32(0)
        else:
            hist_len[i] = P + 1
...
def _run_stp_commit(
    ...
    B = cute.size(hist_len.shape[0])
    grid = (B + 127) // 128
    _stp_commit_kernel(hist_len, cache_base, flush_min).launch(
        grid=(grid, 1, 1), block=[128, 1, 1], stream=stream
    )


_STP_COMMIT_CACHE: dict = {}


def _stp_commit(hist_len: torch.Tensor, cache_base: torch.Tensor, flush_min: int):
    device = hist_len.device
    key = (str(device), int(flush_min))
    stream = cuda.CUstream(torch.cuda.current_stream(device=device).cuda_stream)
    fn = _STP_COMMIT_CACHE.get(key)
    if fn is None:
        _ph = torch.zeros(1, dtype=torch.int32, device=device)
        ...
        _STP_COMMIT_CACHE[key] = fn
    fn(hist_len, cache_base, stream)


def gated_delta_rule_stp_ucache_flush(
    A_log: torch.Tensor,
    a: torch.Tensor,
    dt_bias: torch.Tensor,
    softplus_beta: float = 1.0,
    ...
    """GDN STP (T=1) decode output + u-cache append + per-request state flush.

    Same tensor contract as ``gated_delta_rule_mtp_ucache_flush`` except q/k
    are ``[B, 1, H, K]``, v/a/b are ``[B, 1, HV, {V,1}]``-shaped (T=1), and
    ``output`` is ``[B, 1, HV, V]``. hist_len/cache_base are ALWAYS updated
    in place (this path always owns its cursor commit — the T=1 caller has
    no external accept/reject decision to make; every call accepts exactly
    the 1 real token it decoded); the commit is a single fused [B]-element
    kernel (graph-capturable). flush_min defaults to the max legal value
    for the underlying T=4 kernel: ``min(W_RING - 4 + 1, RING_SLOTS - 4)``
    — 13 at the default 32-slot ring, 12 in the half-memory
    ``GDN_UCACHE_RING_SLOTS=16`` STP-mode layout; pass a smaller value for
    more predictive flushing. Legal hist_len at call time: [0, flush_min].

    ``prepadded=True`` is the zero-copy serving fast path: q/k/v/a/b are the
    PERSISTENT ``[B, 4, ...]`` buffers with the real token in row 0 and rows
    1..3 ZERO (the producer writes its projections straight into row 0 each
    step — the same fixed-buffer pattern as the module's ``_RESTAGE=False``
    mode). This removes the five per-step row-0 staging copies (~18 us of
    graph nodes at small B). The zero tail rows are LOAD-BEARING and are
    validated on eager (non-captured) calls only; the kernel never writes
    the input buffers, so the tail stays zero across steps.
    """
    assert q is not None and k is not None and v is not None
    assert b is not None and initial_state_source is not None
    assert hist_len is not None and cache_base is not None, (
        "gated_delta_rule_stp_ucache_flush: hist_len/cache_base are required "
        "(this path always self-commits; there is no accept/reject step at "
        "T=1)."
    )
    ...
    if prepadded:
        assert T_in == T4, (
            f"prepadded=True expects [B, {T4}, ...] inputs (row 0 real, rows "
            f"1..{T4 - 1} zero); got T={T_in}."
        )
    else:
        assert T_in == 1, f"gated_delta_rule_stp_ucache_flush: T must be 1, got {T_in}."
    HV = v.shape[2]
    V_dim = v.shape[3]
    HK = k.shape[2]
    device = q.device
    # 15: single-row appends + fold-absorbs-token flush (vLLM parity;
    # 14 verify steps + 1 fold per 16-token cycle).
    _flush_min4 = RING_SLOTS - 1
    if flush_min is None:
        flush_min = _flush_min4
    assert 1 <= flush_min <= _flush_min4, (
        f"flush_min={flush_min} out of range [1, {_flush_min4}] (capped by "
        "the underlying T=4 kernel's ring-headroom bound; a dedicated "
        f"native T=1 tile could reach {W_RING})."
    )
    if not torch.cuda.is_current_stream_capturing():
        _hl_min, _hl_max = (int(x.item()) for x in hist_len.aminmax())
        assert _hl_min >= 0 and _hl_max <= flush_min, (
            f"hist_len out of legal range [0, {flush_min}] for this STP path: "
            f"min={_hl_min} max={_hl_max}"
        )
        if prepadded:
            # The zero tail rows are load-bearing (the kernel's ldmatrix/MMA
            # reads the full tile). Validate on eager calls only — the three
            # warmup calls before any graph capture check the same buffers,
            # and the kernel never writes its inputs.
            _tail = sum(float(t[:, 1:].abs().sum().item()) for t in (q, k, v, a, b))
            assert _tail == 0.0, (
                "prepadded=True requires rows 1..3 of q/k/v/a/b to be ZERO "
                f"(got |tail| sum {_tail:.3e}); the producer must write only "
                "row 0 of the persistent buffers."
            )

    if prepadded:
        # Zero-copy fast path: the caller's persistent [B, 4, ...] buffers go
        # straight to the T=4 kernel (no per-step staging graph nodes).
        q4, k4, v4, a4, b4 = q, k, v, a, b
    else:
        # --- pad the single real token to the validated T=4 native tile; rows
        # 1..3 are zero filler (see the block comment above for why they are
        # provably inert). Persistent per-shape buffers, like the module's own
        # _STAGE cache, so steady-state calls pay only a 1-row copy-in.
        pkey = (str(device), B, H, HK, HV, K_dim, V_dim, str(q.dtype))
        bufs = _STP_PAD.get(pkey)
        if bufs is None:
            with torch.inference_mode(False):
                bufs = (
                    torch.zeros(B, T4, H, K_dim, dtype=q.dtype, device=device),
                    torch.zeros(B, T4, HK, K_dim, dtype=q.dtype, device=device),
                    torch.zeros(B, T4, HV, V_dim, dtype=q.dtype, device=device),
                    torch.zeros(B, T4, HV, dtype=q.dtype, device=device),
                    torch.zeros(B, T4, HV, dtype=q.dtype, device=device),
                )
            _STP_PAD[pkey] = bufs
        q4, k4, v4, a4, b4 = bufs
    ...
    # restart_hist_on_flush=False: the T=4 wrapper leaves the cursors
    # untouched (no defensive clone needed) — the fused commit below is the
    # only mutator.
    out4 = gated_delta_rule_mtp_ucache_flush(
        A_log,
        a4,
        dt_bias,
        ...
        flush_min=flush_min,
        restart_hist_on_flush=False,  # we own the +1-token commit below
        pdl_trigger=pdl_trigger,
    )

    # Cursor commit for "accept exactly 1 real token" (see docstring): a
    # fold restarts the window at base'=(base+P)&MASK with the just-appended
    # real token already inside it (len'=1); otherwise the window just grows
    # by the 1 real token (len'=P+1). One fused kernel (stream-ordered after
    # the main kernel, which reads the cursors).
    _stp_commit(hist_len, cache_base, flush_min)

    y = out4[:, 0:1]  # [B, 1, HV, V] — the only real output row
    if output is None:
        return y
    output.copy_(y)
    return output
