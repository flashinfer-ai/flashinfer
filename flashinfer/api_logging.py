...
def _format_value(value: Any, level: int, indent: int = 0) -> str:
    ...
    if isinstance(value, enum.Enum):
        ...
        return f"{indent_str}{value.__class__.__name__}.{value.name} (value={value.value})"
            ...
            try:
                ...
                if value.is_cuda and hasattr(torch.cuda, 'is_current_stream_capturing'):
                    try:
                        ...
                        pass  # Fallback if detection fails
                
                if is_capturing:
                    lines.append(f"{indent_str}  [statistics skipped: CUDA graph capture in progress]")
                elif value.numel() > 0:
                    ...
                    if value.dtype in [torch.float16, torch.float32, torch.float64, 
                        ...
                        lines.append(f"{indent_str}  mean={value.float().mean().item():.6f}")
    ...
    if hasattr(value, '__class__') and value.__class__.__name__ == 'FP4Tensor':
        ...
        lines.append(f"{indent_str}  data={_format_value(value.data, level, indent + 1)}")
        lines.append(f"{indent_str}  scale={_format_value(value.scale, level, indent + 1)}")
    ...
    if isinstance(value, list):
        ...
        for i, item in enumerate(value):
            lines.append(f"{indent_str}  [{i}]: {_format_value(item, level, indent + 1)}")
        lines.append(f"{indent_str}]")
        return "\n".join(lines)
    ...
    if isinstance(value, dict):
        ...
        for key, val in value.items():
            lines.append(f"{indent_str}  {repr(key)}: {_format_value(val, level, indent + 1)}")
        lines.append(f"{indent_str}}}")
        return "\n".join(lines)
...
def _get_default_params(func: Callable, args: tuple, kwargs: dict) -> dict:
    ...
    try:
        ...
        for i, (param_name, param) in enumerate(sig.parameters.items()):
            ...
            # Check positional args (accounting for 'self' in methods)
            if i < len(args):
                provided = True
            # Check keyword args
...
def _log_function_inputs(func: Callable, func_name: str, args: tuple, kwargs: dict, level: int) -> None:
    """
        ...
        def wrapper(*args, **kwargs):
            ...
            if args and hasattr(args[0], '__class__'):
                try:
                    ...
                    if 'Wrapper' in class_name or class_name in ['BatchMLAPagedAttentionWrapper']:
