...
_HEAD_DIM = 128
_PAGE_SIZE = 128
# The SINGLE-RANK (tensor-parallel size 1) head geometry. A rank presents
# `num_attention_heads // tp` query heads over `max(1, num_key_value_heads //
# tp)` KV heads; the kernel has a compiled body per KV-head count in
# `_COMPILED_KV_HEAD_COUNTS`, all of them at the SAME GQA group.
_NUM_QO_HEADS = 64
_NUM_KV_HEADS = 4
# The GQA group is what this kernel body is actually built around, and unlike
# the head counts it is not a free parameter: `kQueriesPerTile = kTileM /
# kGroupSize` is 8, and `kQueriesPerTile * kTopK <= kHashSize` holds with EXACT
# equality at 8 * 16 == 128. Halving the group doubles kQueriesPerTile and needs
# a 256-entry union table, which breaks `kHashSize == 4 * 32` -- the
# ballot-ranked compaction is four warps, one word each. So group 16 is refused
# to move, and the KV-head count is not.
_GQA_GROUP = 16
# The KV-head counts with a compiled body: MiniMax-M3 at tensor-parallel size
# 1, 2 and 4. Mirrored from `is_compiled_kv_head_count` in
# csrc/msa_prefill_nvfp4_specialized.cu, which the binding checks again.
_COMPILED_KV_HEAD_COUNTS = (4, 2, 1)
_TOPK = 16
_SCALE_VEC = 16
...
def page_layout(num_kv_heads: int) -> Dict[str, int]:
    """The planar page map for ``num_kv_heads`` KV heads, in bytes.

    A pure function of the KV-head count and the head dimension. vLLM sizes the
    NVFP4 cache from the PER-RANK KV-head count (``num_head_slots = 2 *
    spec.num_kv_heads``), so under tensor parallelism the page is 36,864 B at
    tp 2 and 18,432 B at tp 4 -- which is why these cannot be module constants.
    """

    k_scale = num_kv_heads * _DATA_HEAD_STRIDE
    v_data = k_scale + num_kv_heads * _SCALE_HEAD_STRIDE
    v_scale = v_data + k_scale
    return {
        "k_scale_byte_offset": k_scale,
        "v_data_byte_offset": v_data,
        "v_scale_byte_offset": v_scale,
        "page_bytes": v_scale + num_kv_heads * _SCALE_HEAD_STRIDE,
    }


_TP1_LAYOUT = page_layout(_NUM_KV_HEADS)
_K_SCALE_BYTE_OFFSET = _TP1_LAYOUT["k_scale_byte_offset"]
_V_DATA_BYTE_OFFSET = _TP1_LAYOUT["v_data_byte_offset"]
_V_SCALE_BYTE_OFFSET = _TP1_LAYOUT["v_scale_byte_offset"]
_PAGE_BYTES = _TP1_LAYOUT["page_bytes"]
...
def _page_region_reason(
    ...
    if shape != (num_pages, num_kv_heads, _PAGE_SIZE, inner):
        return f"{name} shape {shape} is not (num_pages, {num_kv_heads}, 128, {inner})"
    ...
    if stride != (page_bytes, head_stride, inner, 1):
        return (
            f"{name} stride {stride} is not ({page_bytes}, {head_stride}, {inner}, 1)"
        )
    return None
...
def check_surface(
    *,
    ...
    if head_dim != _HEAD_DIM:
        # head_dim is a compile-time constant of the kernel body -- the MMA K
        # extent, the swizzle, the shared-memory map -- so it is refused
        # separately from the head COUNTS, which select an instantiation.
        return _reject(f"this route serves head_dim 128, got {head_dim}")
    # The KV-head count comes from the CACHE, and it is what selects the
    # compiled body and the page map. The GQA GROUP is what stays fixed.
    num_kv_heads = (
        int(k.shape[1]) if isinstance(k, torch.Tensor) and k.ndim == 4 else -1
    )
    if num_kv_heads not in _COMPILED_KV_HEAD_COUNTS:
        return _reject(
            f"this route has compiled bodies for kv-head counts "
            f"{list(_COMPILED_KV_HEAD_COUNTS)} (MiniMax-M3 at tensor-parallel "
            f"size 1, 2 and 4), got {num_kv_heads}"
        )
    if num_qo_heads != num_kv_heads * _GQA_GROUP:
        return _reject(
            f"this route serves GQA group {_GQA_GROUP}, so {num_kv_heads} kv "
            f"heads require {num_kv_heads * _GQA_GROUP} query heads, got "
            f"{num_qo_heads}"
        )
    ...
    if (
        ...
        # reason.
        return _reject(f"q2k_indices must be int32 ({num_kv_heads}, total_q, {_TOPK})")
    ...
    if (
        (num_kv_heads - 1) * int(q2k_indices.stride(0))
    ...
    for tensor, name, offset in (
        (k_scale, "k_scale", layout["k_scale_byte_offset"]),
        (v, "v", layout["v_data_byte_offset"]),
        (v_scale, "v_scale", layout["v_scale_byte_offset"]),
    ):
    ...
    # ---- allowlist ------------------------------------------------------
    signature = (num_qo_heads, num_kv_heads, head_dim, _PAGE_SIZE, _TOPK)
...
def _warm_inputs(
    device: torch.device, num_kv_heads: int = _NUM_KV_HEADS
) -> Dict[str, Any]:
    """The smallest call that satisfies :func:`check_surface` exactly.

    One page, one request, one query token, a one-wide block table. The KV-head
    count is an ARGUMENT defaulting to the single-rank geometry: there is one
    compiled body per count, and the first launch of a body is what makes the
    driver resolve it, so each has to be warmed on its own page.
    ...
    layout = page_layout(num_kv_heads)
    page_bytes = layout["page_bytes"]
    pool = torch.zeros(page_bytes, dtype=torch.uint8, device=device)
    data_shape = (1, num_kv_heads, _PAGE_SIZE, _DATA_DIM)
    scale_shape = (1, num_kv_heads, _PAGE_SIZE, _SCALE_DIM)
    data_stride = (page_bytes, _DATA_HEAD_STRIDE, _DATA_DIM, 1)
    scale_stride = (page_bytes, _SCALE_HEAD_STRIDE, _SCALE_DIM, 1)
    q = torch.zeros(
        (1, num_kv_heads * _GQA_GROUP, _HEAD_DIM), dtype=torch.bfloat16, device=device
    )
    ...
    q2k_indices = torch.full(
        (num_kv_heads, 1, _TOPK), -1, dtype=torch.int32, device=device
    )
    q2k_indices[:, :, 0] = 0
    return dict(
        q=q,
        ...
        k_scale=torch.as_strided(
            pool, scale_shape, scale_stride, layout["k_scale_byte_offset"]
        ),
        v_scale=torch.as_strided(
            pool, scale_shape, scale_stride, layout["v_scale_byte_offset"]
        ),
        q2k_indices=q2k_indices,
...
def warm(device: torch.device | str) -> None:
    ...
    try:
        ...
        # EVERY compiled body, not just the single-rank one. Each KV-head count
        # is a separate kernel image with its own launch attribute, and the
        # first launch of an image is what makes the driver resolve it -- so
        # warming only 4 would leave a tp 2 or tp 4 rank's body cold, which is
        # the exact condition this function exists to remove.
        for kv in _COMPILED_KV_HEAD_COUNTS:
            _dispatch(module, **_warm_inputs(device, num_kv_heads=kv))
        ...
        return
    _warm_dispatch_count += len(_COMPILED_KV_HEAD_COUNTS)
...
def reference(
    *,
    ...
    # From the tensors, not from module constants: an oracle that reads 4 KV
    # heads out of a constant does not follow the kernel onto a tensor-parallel
    # rank -- it indexes past the end of that rank's cache, or silently checks a
    # quarter of the answer.
    num_kv_heads = int(k_data.shape[1])
    group = int(q.shape[1]) // num_kv_heads
    ...
    for request in range(len(boundaries) - 1):
        ...
        for head in range(num_kv_heads):
            for low in range(q_lo, q_hi, chunk):
                ...
                rows_q = q[low:high, head * group : (head + 1) * group].float()
                scores = torch.einsum("bgd,bnd->bgn", rows_q, keys) * float(
                    softmax_scale
                )
                scores = scores.masked_fill(~mask.unsqueeze(1), float("-inf"))
                weights = torch.nan_to_num(torch.softmax(scores, dim=-1), nan=0.0)
                out[low:high, head * group : (head + 1) * group] = torch.einsum(
