...
# family and the CuTe-DSL scored fast path are compiled for, and it is one row
# of the allowlist rather than the whole of it: a tensor-parallel rank presents
# `num_attention_heads // tp` query heads over `max(1, num_key_value_heads //
# tp)` KV heads, and the parametric family reads both at runtime.
_NUM_QO_HEADS = 64
_NUM_KV_HEADS = 4
# GQA group ceiling of the parametric kernel body: one warp per query head in a
# 512-thread CTA, and it sizes four shared-memory arrays. Mirrored from
...
def page_layout(num_kv_heads: int) -> Dict[str, int]:
    ...
    ONE function, because the page map is a pure function of the KV-head count
    and the head dimension and nothing else. A tensor-parallel rank holds
    ``max(1, num_key_value_heads // tp)`` KV heads and vLLM sizes its cache from
    that same number (``num_head_slots = 2 * spec.num_kv_heads``), so the page
    SHRINKS with the rank rather than staying at the tp=1 size -- which is why
    the four byte offsets cannot be module constants.
    """
...
def check_surface(
    *,
    ...
    if head_dim != _HEAD_DIM:
        # head_dim is a compile-time constant of BOTH kernel bodies -- the MMA K
        # extent, the 128 B shared-memory swizzle, the output row stride and the
        # whole shared-memory map -- so it is refused separately from the head
        # COUNTS, which the parametric body reads at runtime.
        return _reject(f"this route serves head_dim 128, got {head_dim}")
    # The KV-head count comes from the CACHE, not from a module constant: it is
    # what the page map, the selection tensor and the grid are all derived from,
    # and under tensor parallelism every one of them shrinks with the rank.
    num_kv_heads = int(k.shape[1]) if k.ndim == 4 else -1
    if num_kv_heads < 1:
        return _reject("k must be a 4-D (num_pages, num_kv_heads, page_size, dim)")
    if num_qo_heads < 1 or num_qo_heads % num_kv_heads:
        return _reject(
            f"num_qo_heads ({num_qo_heads}) must be a positive multiple of "
            f"num_kv_heads ({num_kv_heads})"
        )
    group = num_qo_heads // num_kv_heads
    if group > _MAX_GQA_GROUP:
        # A REAL structural bound of the parametric body, not a policy: it runs
        # one warp per query head inside a 512-thread CTA and sizes four
        # shared-memory arrays by this number.
        return _reject(
            f"GQA group {group} exceeds the kernel's head capacity "
            f"{_MAX_GQA_GROUP} -- one warp per query head in a 512-thread CTA"
        )
    ...
    if (
        ...
        return _reject(
            f"q2k_indices must be int32 ({num_kv_heads}, total_q, topk) with "
            f"total_q={total_q}"
        )
    ...
    # ---- allowlist ------------------------------------------------------
    signature = (num_qo_heads, num_kv_heads, head_dim, _PAGE_SIZE)
...
def _warm_inputs(
    device: torch.device,
    num_qo_heads: int = _NUM_QO_HEADS,
    num_kv_heads: int = _NUM_KV_HEADS,
) -> Dict[str, Any]:
    """The smallest call that satisfies :func:`check_surface` exactly.

    One page and one decode token. Zero bytes are a valid NVFP4 encoding --
    e2m1 code 0 and an e4m3 scale of 0 -- so the launch is defined without
    quantizing anything; the output is discarded. The four K/V views are cut out
    of one allocation at the documented byte offsets because the guard proves
    that relationship and the kernel asserts it again.

    The head counts are ARGUMENTS, defaulting to the single-rank geometry: a
    tensor-parallel rank's page is smaller, so warming it means building its
    page, not the tp=1 one.
    """
    ...
    q = torch.zeros((1, num_qo_heads, _HEAD_DIM), dtype=torch.bfloat16, device=device)
...
def warm(device: torch.device | str) -> None:
    ...
    try:
        ...
        _dispatch(module, warming=True, **general_inputs)
        ...
        _dispatch(module, warming=True, **pinned_inputs)
        # EVERY ALLOWLISTED HEAD GEOMETRY, not just the single-rank one. Under
        # tensor parallelism a rank's page is a different size and its selection
        # tensor a different shape, so warming only 64/4 would leave the
        # geometry a TP rank actually calls with unlaunched -- and the first
        # launch of an instantiation is the whole point of this function. The
        # extra geometries are one page each (18-37 KB) and all take the
        # parametric family, which the tp=1 dispatch above does not exercise.
        extra = 0
        for qo, kv, hd, page in sorted(_load_allowlist()):
            if (qo, kv, hd, page) == (
                _NUM_QO_HEADS,
                _NUM_KV_HEADS,
                _HEAD_DIM,
                ...
                continue
            rank_inputs = _warm_inputs(device, num_qo_heads=qo, num_kv_heads=kv)
            _dispatch(module, warming=True, **rank_inputs)
            extra += 1
...
def reference(
    *,
    ...
    topk = int(q2k_indices.shape[2])
    # Same reason for the head counts. An oracle that reads 4 KV heads out of a
    # module constant does not follow the kernel onto a tensor-parallel rank; it
    # indexes past the end of that rank's cache, or silently checks a quarter of
    # the answer.
    ...
    for head in range(num_kv_heads):
        for low in range(0, total_q, chunk):
            ...
            scores = torch.einsum("bgd,bnd->bgn", rows_q, keys) * float(softmax_scale)
...
def _pinned_split_for(total_q: int, num_pages: int) -> int:
    """The split ``geom::pinned::launch`` would choose. Mirrors the C++ copy."""

    tiles = total_q * _NUM_KV_HEADS
    split = 8 if tiles <= 32 else (4 if tiles <= 64 else (2 if tiles <= 128 else 1))
...
def _dispatch(
    ...
    *,
    ...
    if miss is not None and not warming:
        ...
        #
        # SUPPRESSED WHILE WARMING, and that is not a softening. `warm` launches
        # EVERY allowlisted head geometry so that no tensor-parallel rank meets
        # a cold instantiation inside a CUDA-graph capture, so on a tp=1 rank it
        # necessarily misses the pin twice -- and `info_once` is keyed on the
        # message, so the first miss would win and a real pin drift later would
        # then be silent. Warming is not traffic; only traffic can tell you the
        # pin stopped matching.
        logger.info_once(
