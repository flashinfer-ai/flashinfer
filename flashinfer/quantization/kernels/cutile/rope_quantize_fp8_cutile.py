...
# Source: ocean/src/tilegym/suites/flashinfer/cutile/rope_quantize_fp8.py
# Changes vs Ocean source:
#   - Removed `from tilegym.autotune import is_autotune_disabled` (TileGym-private)
#     Replaced is_autotune_disabled() with env var: FLASHINFER_CUTILE_AUTOTUNE_DISABLED
#   - Removed `from tilegym.backend import register_impl` and @register_impl decorator
#   - Removed `from tilegym.ops.cutile.utils import cached_replace_hints`
#     Replaced cached_replace_hints(kernel, ...) with kernel.replace_hints(...)
...
def rope_quantize_fp8_cutile(
    ...
    fused pass. Migrated from Ocean cuTile suite using public cuda.tile API only.
