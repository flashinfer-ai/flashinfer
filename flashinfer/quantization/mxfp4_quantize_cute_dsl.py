...
class MXFP4QuantizeSwizzledKernel:
    ...
    Key characteristics:
    - Each thread processes 32 elements (one full SF block)
    ...
    def __init__(
        ...
        # Each thread handles 1 SF block (32 elements)
        # Want enough threads to cover all SF blocks in a row
        num_sf_blocks = self.num_sf_blocks_per_row
        if num_sf_blocks <= 16:
            self.num_threads = 64
        elif num_sf_blocks <= 32:
            self.num_threads = 128
        elif num_sf_blocks <= 64:
            self.num_threads = 256
        elif num_sf_blocks <= 128:
    ...
    def __call__(
        ...
        num_blocks = cutlass.min(padded_M, self.target_grid)
    ...
    def kernel(
        ...
        while row_idx < padded_M:
            ...
            if is_padding_row:
                ...
                # sf_idx already initialized to tidx above
                ...
                num_sf_iters = (num_sf_blocks_per_row + num_threads - 1) // num_threads
