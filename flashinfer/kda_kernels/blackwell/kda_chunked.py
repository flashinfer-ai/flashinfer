"""Chunked KDA (Kimi Delta Attention) prefill kernel for Blackwell SM100.

KDA == GDN-2 with beta == w and beta a per-token scalar (per sab head):
  S = exp(g)[:, None] * S + beta_t * outer(k, v - k @ S);  o = q @ S
The decay gate stays per-channel; beta broadcasts over both key and value axes.

Algorithm overview (per chunk c, tokens [cC, (c+1)C)):
  Inputs : Q[BT,DK], K[BT,DK], V[BT,DV] (fp16/bf16), channel-wise decay
           gate[BT,DK] (fp32), per-token scalar beta[BT] (fp32)
  ...
  GEMM 5 - new v   : NV[BT,DV] = A_inv @ V       (corrected value vectors)
                      where A_inv = (I + M_kk)^{-1},  M_kk[i,j] = T[i,j]*beta_i*W_kk[i,j]  (lower-tri, hierarchical blockwise inverse)
...
SMEM buffers (per stage):
  q, k (x2, raw K double-buffered), sE (x2, E fold output), v,
  A_inverse/new_v, W_qkv scores, O staging, raw gate/cumsumlog (fp32,
  overwritten in place), cumprod_total, beta (fp32 [BT] scalars).
...
Warp assignments (12 warps = 384 threads):
  ...
  warps 4-7     : compute group 1 - K_bar fold, S staging, beta*V - KS,
                                    state*q_epi, new_v_epi, dS combine,
                                    deferred qkv_epilogue
  warp  8       : MMA warp       - issues all 7 GEMMs
  warp  9       : TMA load warp  - loads q, k, v, raw gate
  warp  10      : beta load warp - cp.async-loads beta[BT] scalars
...
from ...gdn_kernels.blackwell.gated_delta_net_tile_scheduler import (
...
class KDAChunkedKernel:
    """
    Configuration and execution class for the Chunked KDA kernel.
    ...
    def kernel(
        ...
        # ------------------------------------------------------------------
        # TMA descriptor GMEM workspace - one set of 4 ptrs per CTA
        ...
        load_beta_consumer_cg1 = load_beta_consumer.clone()
        ...
        # ==============================================================
        # BETA LOAD WARP (warp 10) -- cp.async-loads beta[BT] scalars
        # ==============================================================
        if warp_idx == self.load_gate_beta_warp_id:
            ...
            while work.is_valid_tile:
                ...
                for chunk_idx in cutlass.range(num_chunks_b - 1):
                    chunk_offset = batch_start + chunk_idx * self.b_t
                    load_beta_producer = self.load_beta_warp(
                        tidx,
                        (mBeta,),
                        (sBeta,),
                        (load_beta_producer,),
    ...
    def tma_qkv_warp(
        ...
        """Warp 9: load Q, K (double-buffered), V, raw gate for the chunk.
        ...
        tma_q, tma_k, tma_v, tma_gate = tma_args
        sQ, sK, sKb, sV, sGate = smem_args
        (
            load_q_producer,
            load_k_cg0_producer,
            load_k_cg1_producer,
        ...
        # sGate -- CG0's per-channel cumsum is critical-path.
        # ------------------------------------------------------------------
    ...
    def load_beta_warp(
        ...
        """Warp 10: load beta[BT] scalars for the current chunk.

        Beta is loaded via ldgsts (async G->S, cp.async) into sBeta SMEM
        (GDN-1's load_gate_beta_warp pattern, beta-only; the gate stays TMA).

        The last tile uses predicated copies: elements with linear index >=
        batch_end are out-of-bounds and receive the neutral value beta=0.
        """
        (beta,) = gmem_args
        (sBeta,) = smem_args
        (load_beta_producer,) = pipeline_args
        ...
        cBeta = cute.domain_offset(
            (chunk_offset,), cute.make_identity_tensor(beta[None, head_idx].shape)
        )
        gBeta = cute.flat_divide(gBeta, (self.b_t,))[None, 0]
        cBeta = cute.flat_divide(cBeta, (self.b_t,))[None, 0]
        ...
        if cutlass.const_expr(is_last_tile):
            tBcBeta = thr_copy_beta_g2s.partition_S(cBeta)
            tBpBeta = cute.make_rmem_tensor(
                ((tBcBeta.shape[0][1],), tBcBeta.shape[1]), cutlass.Boolean
            )
            for i in range(cute.size(tBpBeta)):
                tBpBeta[i] = cute.elem_less(tBcBeta[i][0], batch_end)
    ...
    def compute_group_0(
        ...
        # Phase 2: K -> E = exp2(L) * beta_t * K.  Beta is a per-token scalar
        ...
        for p in cutlass.range_constexpr(cute.size(tRT_rK_i32)):
            ...
            b0 = sBeta[tCcE[2 * p][0], 0, beta_handle.index]
            b1 = sBeta[tCcE[2 * p + 1][0], 0, beta_handle.index]
    ...
    def compute_group_1(
        ...
        sV, sCumprodTotal, sBeta, sO, sCumprod, sCumprodBank, sE, sK = smem_args
        ...
        beta_cg1_handle = load_beta_consumer_cg1.wait_and_advance()
        ...
        tRT_rBetaV = cute.make_rmem_tensor_like(tRT_tCcV, self.io_dtype)
        for k in cutlass.range(cute.size(tRT_rBetaV)):
            coord = tRT_tCcV[k]
            tRT_rBetaV[k] = sBeta[coord[1], 0, beta_cg1_handle.index].to(self.io_dtype)
        for k in cutlass.range(cute.size(tRT_rV), vectorize=True):
            tRT_rV[k] = tRT_rV[k] * tRT_rBetaV[k]
    ...
    def get_workspace_size(num_sm: int, B: int, HQ: int, HV: int, is_persistent: bool):
        ...
        if is_persistent:
            return (
                KDAChunkedKernel.bytes_per_tensormap
                * KDAChunkedKernel.num_tensormaps
                * num_sm
            )
