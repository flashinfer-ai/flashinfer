...
KDA (Kimi Delta Attention) Chunked Prefill - Blackwell SM100 Adapter
====================================================================

Bridges FlashInfer's PyTorch-based API to the CuTe DSL chunked KDA kernel
for SM100 (Blackwell).

KDA is GDN-2 with beta == w and beta a per-token scalar:

    gate: (total_tokens, H, DK) fp32 raw decay   beta: (total_tokens, H) fp32

Follows the same compile-once-cache-and-replay pattern used by
``gdn_prefill2.py``.
...
from .kda_chunked import KDAChunkedKernel
...
def chunk_kda_sm100(
    ...
    """Execute the Blackwell chunked KDA prefill kernel.
    ...
    Args:
        ...
        gate: ``(total_tokens, HO, DK)`` float32, channel-wise forget/decay gate
              (raw decay in (0, 1], NOT log space)
        beta: ``(total_tokens, HO)`` float32, per-token scalar erase/write gate
    ...
    if "compiled" not in cache:
        ...
        workspace_size = KDAChunkedKernel.get_workspace_size(num_sm, B, HQ, HV, True)
    ...
    if ws_key not in cache or cache[ws_key].size(0) < workspace_size:
        cache[ws_key] = torch.zeros(workspace_size, dtype=torch.int8, device=q.device)
