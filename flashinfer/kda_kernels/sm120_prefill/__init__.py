...
def run_kda_prefill_sm120(
    *,
    ...
    ``lookup_only`` asks for the memoized plan and nothing else: a warm call
    is executed and returned, a miss returns ``None`` without validating,
    canonicalizing or building anything.  The public dispatcher probes with it
    before running its own eligibility checks, so a warm serving loop pays
    neither those checks nor this module's validation on the hot path -- both
    ran when the plan was built, for tensors the memo verifies by identity.
    ``call_key`` is folded into the memo key for whatever the caller's own
    contract depends on beyond these arguments, so a warm hit implies the
    caller validated an identical call.

    A packed ``cu_seqlens`` is matched by *content* as well as identity: a
    serving loop that allocates fresh offsets every scheduler step and passes
    them to every layer keeps its plan across steps, at the price of one
    device-to-host read per new offsets tensor -- the read the first call
    ...
    scalars: tuple = (resolved_scale, float(lower_bound), variant, bool(safe_gate))
    if call_key:
        scalars = scalars + (call_key,)
    ...
    memo_tensors = tensors
    offsets = None
    if resolved is None and cu_seqlens is not None and q.dim() == 4 and q.shape[0] == 1:
        # A new offsets tensor with the content of one already canonicalized:
        # the plan built on that canonical buffer is this call's plan.  The
        # memo keys the offsets slot on the canonical tensor for that reason.
        # Refused offsets are not an answer here; the full path below reports
        # them, and a lookup-only probe must not raise where eligibility would
        # have fallen back.
        try:
            offsets = validate_packed_offsets(cu_seqlens, q.shape[1])
        except KDAPrefillValidationError:
            if lookup_only:
                return None
            raise
        if offsets.canonical is not cu_seqlens:
            memo_tensors = tensors[:-1] + (offsets.canonical,)
            resolved = _resolved_call(
                q.device, tensors, scalars, resources, memo_tensors=memo_tensors
            )

    # Plans that can be re-pointed: keyed on layouts and offsets content, not
    # on addresses, so a caller handing over fresh buffers on every call --
    # a serving loop's conv outputs, gathered state and output view -- keeps
    # its plan and pays a descriptor refresh instead of a rebuild.
    shape_key = None
    if resolved is None and resources is None and q.dim() == 4:
        if cu_seqlens is None:
            shape_key = _shape_key(q.device, tensors, scalars, None)
        elif offsets is not None:
            shape_key = _shape_key(q.device, tensors, scalars, offsets.host)
    if shape_key is not None:
        with _RESOLVED_LOCK:
            shaped = _SHAPE_PLANS.get(shape_key)
            if shaped is not None:
                _SHAPE_PLANS.move_to_end(shape_key)
        if shaped is not None:
            module, plan = shaped
            # The public contract forbids an exact out/v alias that this
            # backend's schedule allows; a lookup-only probe answers for the
            # public dispatcher, so it declines and lets eligibility decide.
            if lookup_only and is_exact_alias(out, v):
                return None
            try:
                # Re-point and launch under the plan's launch lock: the plan is
                # shared by every call of this shape, and another thread must
                # not re-point it between the two steps.
                module.run_rebound(
                    plan,
                    q=q,
                    k=k,
                ...
                if lookup_only:
                    return None
                raise
            # The plan is now pointed at exactly these tensors, so the next
            # call with the same objects may replay it from the fast path.
            # Every re-point refreshes this entry, which is what keeps an
            # identity hit and the plan's binding in step.
            _remember_last(
                q.device, tensors, scalars, resources, (module.execute, plan)
            )
    ...
    if (
        ...
        and plan is not STATE_ONLY_PLAN
        and getattr(module, "run_rebound", None) is not None
    ):
        # A re-pointable plan is memoized by shape only.  It is deliberately
        # kept out of the identity memo: an identity hit replays a plan
        # without re-pointing it, and this plan may since have been pointed
        # at another call's buffers.
        with _RESOLVED_LOCK:
            _SHAPE_PLANS[shape_key] = (module, plan)
            while len(_SHAPE_PLANS) > SHAPE_PLAN_MAX_ENTRIES:
                _SHAPE_PLANS.popitem(last=False)
        _remember_last(q.device, tensors, scalars, resources, (module.execute, plan))
        return out, final_state
    _remember_call(
        q.device,
        memo_tensors,
...
#: Re-pointable plans by shape key -> ``(module, plan)``.  One entry per
#: distinct call shape; a model with one KDA shape per chunk length needs a
#: handful.  Guarded by :data:`_RESOLVED_LOCK`.
SHAPE_PLAN_MAX_ENTRIES = 16
_SHAPE_PLANS: "OrderedDict[tuple, tuple]" = OrderedDict()


def _shape_key(device, tensors, scalars, offsets_host) -> tuple:
    return (
        tuple(tensor_shape_identity(t) for t in tensors),
        scalars,
        offsets_host,
...
def _resolved_call(device, tensors, scalars, resources, memo_tensors=None):
    """The memoized ``(execute, plan)`` for this call, or ``None`` on a miss.

    ``tensors`` are what the caller passed and what the one-entry fast path
    compares against; ``memo_tensors`` is the tuple the LRU was keyed on when
    it differs -- the offsets slot holding the canonical tensor rather than the
    caller's -- and defaults to ``tensors``.  An LRU hit re-arms the fast path
    with the caller's tensors, so the next call with the same objects takes
    the cheap comparison instead of the key build.
    ...
    if not _RESOLVED:
        return None
    if memo_tensors is None:
        memo_tensors = tensors
    key = _resolved_key(device, memo_tensors, scalars, resources)
    with _RESOLVED_LOCK:
        ...
        for ref, tensor in zip(refs, memo_tensors, strict=True):
            if ref is None:
                if tensor is not None:
                    break
            elif ref is not _BY_ADDRESS and ref() is not tensor:
            ...
            _remember_last(device, tensors, scalars, resources, value, refs)
            return value
...
def _remember_last(device, tensors, scalars, resources, value, refs=None) -> None:
    """Arm the one-entry fast path for ``tensors``.

    ``refs`` are the LRU entry's holders when the call came through it; a slot
    the entry verifies by address stays by-address here, since the object in
    that slot is by construction a fresh buffer every call.
    """
    ...
    _RESOLVED_LAST = (
        tuple(
            ...
            if refs is not None and refs[i] is _BY_ADDRESS
            else weakref.ref(t)
...
def _remember_call(
    device, tensors, scalars, resources, value, private=(), last_tensors=None
) -> None:
    """Memoize ``value`` for ``tensors`` in both levels.

    ``last_tensors`` are the caller's own tensors when ``tensors`` is the LRU
    key tuple with the offsets slot canonicalized; the fast path compares the
    next call against what the caller passes, not against the key.
    """
    ...
    global _RESOLVED_LAST
    if last_tensors is None:
        last_tensors = tensors
    _RESOLVED_LAST = (
        tuple(_hold(i, t) for i, t in enumerate(last_tensors)),
        tuple(tensor_identity(t) for t in last_tensors),
        scalars,
        resource_cache_token(resources),
        current_stream_ptr(device),
        value,
