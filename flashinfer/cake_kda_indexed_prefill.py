...
Source-only loader for the generated FP32-indexed Cake KDA portfolio.
"""
...
CakeKDAIndexedTarget = Literal["sm100a", "sm103a"]

_CATALOG_KIND = "flashinfer.cake_kda_indexed_prefill.source_catalog"
_IMPORT_RECEIPT_KIND = "flashinfer.cake_kda_indexed_prefill.import_receipt"
...
class CakeKDAIndexedPrefillError(RuntimeError):
...
def _require(condition: bool, message: str) -> None:
    if not condition:
        raise CakeKDAIndexedPrefillError(message)
...
def _source_root() -> Path:
    ...
    candidates = (
        jit_env.FLASHINFER_CSRC_DIR / "cake_kda_indexed_prefill",
        Path(__file__).resolve().parents[1] / "csrc" / "cake_kda_indexed_prefill",
    )
    for candidate in candidates:
        if candidate.is_dir() and not candidate.is_symlink():
            return candidate.resolve(strict=True)
    raise CakeKDAIndexedPrefillError(
        "Cake KDA indexed-prefill source package is not installed"
    )
...
def _safe_relative(value: object, label: str) -> PurePosixPath:
    ...
    _require(
        relative.name.startswith("cake_"),
        f"{label} basename must start with cake_",
    )
    return relative
...
def _source_record(root: Path, value: object, label: str) -> _SourceRecord:
    ...
    try:
        ...
        raise CakeKDAIndexedPrefillError(f"{label} escapes the source root") from error
...
def _verify_catalog_receipt(root: Path, catalog_payload: bytes) -> None:
    ...
    _require(
        ...
        "Cake KDA import receipt is unavailable",
    )
    try:
        receipt = json.loads(receipt_path.read_bytes())
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise CakeKDAIndexedPrefillError("Cake KDA import receipt is invalid") from error
    receipt = _object(
        receipt,
    ...
    _require(
        ...
        "Cake KDA import receipt identity differs",
    )
    _require(
...
def _read_catalog(root: Path) -> tuple[_TargetRecord, ...]:
    ...
    catalog_path = root / "cake_kda_source_catalog.json"
    _require(
        catalog_path.is_file() and not catalog_path.is_symlink(),
        "Cake KDA source catalog is unavailable",
    )
    catalog_payload = catalog_path.read_bytes()
    _verify_catalog_receipt(root, catalog_payload)
    try:
        catalog = json.loads(catalog_payload)
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise CakeKDAIndexedPrefillError("Cake KDA source catalog is invalid") from error
    ...
    _require(
        ...
        "Cake KDA source catalog identity differs",
    )
    ...
    for target_index, raw_target in enumerate(raw_targets):
        ...
        for module_index, raw_module in enumerate(raw_modules):
            ...
            for field in ("id", "module_ident", "entry_point", "kernel_name"):
                _require(
                    isinstance(module[field], str) and bool(module[field]),
                    f"{module_label}.{field} is invalid",
                )
            _require(
            ...
            modules.append(
                _ModuleRecord(
                    id=str(module["id"]),
                    module_ident=str(module["module_ident"]),
                    entry_point=str(module["entry_point"]),
                    kernel_name=str(module["kernel_name"]),
...
def _target_record(target: CakeKDAIndexedTarget) -> _TargetRecord:
    for record in _catalog():
        if record.target == target:
            return record
    raise CakeKDAIndexedPrefillError(f"Cake KDA target {target!r} is unavailable")


def _target_for_device(device: torch.device) -> CakeKDAIndexedTarget:
    capability = get_compute_capability(device)
    try:
        return _TARGET_COMPUTE_CAPABILITIES[capability]
    except KeyError as error:
        raise CakeKDAIndexedPrefillError(
            f"Cake KDA indexed prefill requires CC 10.0 or 10.3, got {capability}"
        ) from error
...
def _nvrtc_check(error: object, operation: str) -> None:
    code = getattr(error, "value", error)
    if code != 0:
        raise CakeKDAIndexedPrefillError(f"{operation} failed with NVRTC result {code}")
...
def _compile_exact_cubin(module: _ModuleRecord, target: _TargetRecord) -> bytes:
    ...
    _nvrtc_check(error, "nvrtcCreateProgram")
    try:
        options = [
            f"--gpu-architecture={target.architecture}",
            "-std=c++17",
            "-default-device",
        ]
        ...
        options.extend(module.compile_options)
        ...
        if getattr(error, "value", error) != 0:
            ...
            raise CakeKDAIndexedPrefillError(
                f"NVRTC compilation failed for {target.target}/{module.id}:\n"
                + log.decode(errors="replace").rstrip("\0")
            )
        error, size = nvrtc.nvrtcGetCUBINSize(program)
        _nvrtc_check(error, "nvrtcGetCUBINSize")
        cubin = b"\0" * size
        (error,) = nvrtc.nvrtcGetCUBIN(program, cubin)
        _nvrtc_check(error, "nvrtcGetCUBIN")
    finally:
...
def _module_build_directory(module: _ModuleRecord, target: _TargetRecord) -> Path:
    ...
    return jit_env.FLASHINFER_JIT_DIR / (
        f"cake_kda_indexed_{target.target}_{module.module_ident}_{identity}"
    )
...
@functools.cache
def _load_entrypoint(target_name: CakeKDAIndexedTarget, module_id: str) -> Callable[..., object]:
    from tvm_ffi import cpp

    target = _target_record(target_name)
    module = next((record for record in target.modules if record.id == module_id), None)
    _require(module is not None, f"unknown Cake KDA module id {module_id!r}")
...
class _LazyEntrypoint:
    ...
    def __init__(self, target: CakeKDAIndexedTarget, module_id: str) -> None:
        self._target = target
...
def _load_dispatcher_source(record: _TargetRecord) -> dict[str, Callable[..., object]]:
    ...
    namespace: dict[str, Any] = {
        ...
        "__name__": f"cake_kda_indexed_dispatcher_{record.target}",
    }
    try:
...
@functools.cache
def get_cake_kda_indexed_prefill_dispatcher(
    target: CakeKDAIndexedTarget,
...
def cake_kda_indexed_prefill_is_eligible(
    *,
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
...
def run_cake_kda_indexed_prefill(
    *,
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
...
__all__ = [
    "CakeKDAIndexedPrefillError",
    "cake_kda_indexed_prefill_is_eligible",
    "get_cake_kda_indexed_prefill_dispatcher",
