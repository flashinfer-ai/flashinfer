...
from .cute_utils import as_tensor, signal_multimem, wait_loop, multimem_ld_reduce, multimem_st
...
class MultimemAllReduce:
    ...
    def __init__(
        ...
        num_warps_per_block = 8 # for fp16 and bf16
        num_ctas = 128 # max is the number of sms
        tensor_size = max_buffer_elements
        numel_per_thread = 8

        max_work_per_iter = numel_per_thread * num_warps_per_block * 32 * num_ctas * world_size
        num_iter_per_thread = tensor_size//max_work_per_iter
        ...
        #     self.device,
        # )
        self.cute_tensor_barrier_start, self.nvshmem_tensor_barrier_start, self.tensor_mc_memerf_barrier_start = self.create_barrier_flags([num_ctas], cutlass.Int32, device)

        self.cute_tensor_barrier_end, self.nvshmem_tensor_barrier_end, self.tensor_mc_memerf_barrier_end = self.create_barrier_flags([num_ctas], cutlass.Int32, device)
    ...
    def create_barrier_flags(
        ...
        nvshmem_tensor = self.nvshmem_modulenvshmem_malloc(shape, torch_dtype, device)
        nvshmem_tensor.fill_(0)
        tensor_mc = self.nvshmem_modulenvshmem_multicast_ptr(nvshmem_tensor)
        cute_tensor = from_dlpack(nvshmem_tensor)
        cute_tensor.element_type = dtype
        cute_tensor.mark_layout_dynamic()
        tensor_mc_memerf = from_dlpack(
            as_tensor(tensor_mc, nvshmem_tensor.shape, nvshmem_tensor.dtype),
        )
        #NOTE(asamani): check if should align to 16 bytes?
        tensor_mc_memerf.mark_layout_dynamic()
        return cute_tensor, nvshmem_tensor, tensor_mc_memerf
    ...
    def create_and_permute_tensor(
        ...
        if is_mc:
            ...
            tensor_mc = from_dlpack(as_tensor(mc_ptr,
                                            torch_tensor.shape,
                                            torch_tensor.dtype),
                                    assumed_align=16)
            tensor_mc = tensor_mc.mark_layout_dynamic()
        cute_tensor = from_dlpack(torch_tensor, assumed_align=16)
        cute_tensor.element_type = dtype
        cute_tensor.mark_layout_dynamic()
        # TODO(asamani): if fp8 handle differently
        return cute_tensor, torch_tensor, tensor_mc
