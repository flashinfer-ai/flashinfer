            ...
            for (int phase = 0; phase < 2; phase++) {
                ...
                for (int work = work_begin; work < work_end; work += (int)num_clusters) {
                    ...
                    for (int row_tile = 0; row_tile < 1; row_tile++) {
                        int packed_token = (unsigned int)(row_tile * 32) + lane;
                        ...
                        if (phase == 0) {
                            ...
                            int warp_row_ptr_1 = taddr + (unsigned int)acc_offset + (warp * 32 << 16);
                    ...
                    for (int tail = 0; tail < 2; tail++) {
                        if (route_count > 32 + tail * 16) {
                            int packed_token_1 = (unsigned int)(32 + tail * 16) + lane;
                            int expert_row_tile_2 = (tail + 1) * 32 + expert_1;
                            int acc_offset_1 = (acc_stage * 3 + (unsigned int)tail + 1) * 32;
                            int warp_row_ptr_2 = taddr + (unsigned int)acc_offset_1 + (warp * 32 << 16);
                            if (phase == 0) {
                                int feature_base_1 = (unsigned int)(weight_m_tile * 128 + cta_rank * 64) + warp * 16;
                                ...
                                int hidden_base_1 = (unsigned int)(weight_m_tile * 256 + cta_rank * 128) + warp * 32;
