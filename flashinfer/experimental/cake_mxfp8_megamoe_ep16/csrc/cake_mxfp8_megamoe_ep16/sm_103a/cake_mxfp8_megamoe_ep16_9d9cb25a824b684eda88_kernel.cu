...
#define SMEM_B_SMEM_STAGE_BYTES 2048
#define SMEM_B_SMEM_STRIDE 2048
...
kernel_cake_mxfp8_megamoe_ep16_9d9cb25a824b684eda88(__nv_bfloat16* __restrict__ source_hidden_bf16, long long* __restrict__ source_topk_ids_i64, float* __restrict__ source_topk_weights_f32, CakeTensorMap const* fc1_weight_e4m3, CakeTensorMap const* fc1_blocked_e8m0, CakeTensorMap const* activation_bf16, CakeTensorMap const* activation_bf16_n16, __nv_bfloat16* __restrict__ activation_bf16_ptr, CakeTensorMap const* fc2_weight_e4m3, CakeTensorMap const* fc2_blocked_e8m0, CakeTensorMap const* fc1_workspace_bf16_tma, CakeTensorMap const* fc1_workspace_bf16_tma_n16, __nv_bfloat16* __restrict__ fc1_workspace_bf16, __nv_bfloat16* __restrict__ fc2_output_bf16, int* __restrict__ route_map_i32, float* __restrict__ route_scale_f32, unsigned int* __restrict__ route_counts_u32, unsigned int* __restrict__ fc1_done, unsigned int* __restrict__ publication_done, unsigned int* __restrict__ publication_visible, unsigned int* __restrict__ dispatch_done, unsigned int* __restrict__ compute_done, unsigned int* __restrict__ return_done, unsigned int* __restrict__ return_visible, int launch_epoch, int tokens_per_rank, int32_t mega_pg_world, int32_t mega_pg_rank, unsigned* const* __restrict__ mega_pg_flags, __nv_bfloat16* __restrict__ published_hidden_bf16, __nv_bfloat16* const* __restrict__ published_hidden_bf16_peers, int* __restrict__ published_topk_ids_i32, int* const* __restrict__ published_topk_ids_i32_peers, float* __restrict__ published_topk_weights_f32, float* const* __restrict__ published_topk_weights_f32_peers, __nv_bfloat16* __restrict__ route_terms_bf16, __nv_bfloat16* const* __restrict__ route_terms_bf16_peers)
{
    ...
    if (warp <= 3) {
        ...
        { // epilogue_main
            ...
            #pragma unroll 1
            for (int expert_row_tile = linear_thread; expert_row_tile < 128; expert_row_tile += thread_stride) {
                {
            ...
            for (int phase = 0; phase < 2; phase++) {
                ...
                for (int work = work_begin; work < work_end; work += (int)num_clusters) {
                    ...
                    int _max_2 = ((1) > ((route_count + 16 - 1) / 16) ? (1) : ((route_count + 16 - 1) / 16));
                    ...
                    for (int segment = 0; segment < active_segments; segment++) {
                        int packed_token = (unsigned int)(segment * 16) + lane;
                        int expert_row_tile_1 = segment * 32 + expert_1;
                        int acc_offset = (acc_stage * 4 + (unsigned int)segment) * 32;
                        ...
                        if (phase == 0) {
                            ...
                            #pragma unroll
                            for (int elem_1 = 8; elem_1 < 16; elem_1++) {
                                folded[elem_1] = 0.0f;
                            }
                            float transposed[16];
                            float r1[16];
                            ...
                            for (int elem_2 = 0; elem_2 < 8; elem_2++) {
                                r3_raw[elem_2] = _tmem_load_3[elem_2];
                                r3_raw[elem_2 + 8] = _tmem_load_4[elem_2];
                            }
                            float r3[16];
                            r3[0] = r3_raw[0];
                            r3[1] = r3_raw[1];
                            ...
                            for (int elem_3 = 0; elem_3 < 8; elem_3++) {
                                r4_raw[elem_3] = _tmem_load_5[elem_3];
                                r4_raw[elem_3 + 8] = _tmem_load_6[elem_3];
                            }
                            ...
                            if (lane < 16) {
                                ...
                                if (valid_route >= 0) {
                                    ...
                                    for (int pair_idx_1 = 0; pair_idx_1 < 8; pair_idx_1++) {
                                        int elem_4 = pair_idx_1 * 2;
                                        float2 _f2_6 = make_float2(transposed[elem_4], transposed[elem_4 + 1]);
                                        float2 values = _f2_6;
                                        ...
                                        scaled[elem_4] = product.x;
                                        scaled[elem_4 + 1] = product.y;
                                    }
                                } else {
                                    #pragma unroll
                            ...
                            asm volatile(
                                ...
                                : "r"(warp_row_ptr + 1048576));
                            asm volatile("tcgen05.wait::ld.sync.aligned;");
                            float casted_input[16];
                            #pragma unroll
                            for (int elem_5 = 0; elem_5 < 8; elem_5++) {
                                casted_input[elem_5] = _tmem_load_7[elem_5];
                                casted_input[8 + elem_5] = _tmem_load_8[elem_5];
                            }
                            unsigned int casted_bf16[8];
                            #pragma unroll
                            ...
                            for (int pair = 0; pair < 4; pair++) {
                                ...
                                asm("prmt.b32 %0, %1, %2, 0x5410;" : "=r"(_prmt_b32_4) : "r"(casted_bf16[pair]), "r"(casted_bf16[4 + pair]));
                                packed[pair * 2] = __uint_as_float(_prmt_b32_4);
                                uint32_t _prmt_b32_5;
                                asm("prmt.b32 %0, %1, %2, 0x7632;" : "=r"(_prmt_b32_5) : "r"(casted_bf16[pair]), "r"(casted_bf16[4 + pair]));
                                packed[pair * 2 + 1] = __uint_as_float(_prmt_b32_5);
                            }
                            #pragma unroll
                            for (int elem_6 = 8; elem_6 < 16; elem_6++) {
                                packed[elem_6] = 0.0f;
                            }
                            ...
                            asm volatile(
                                ...
                                : "r"(warp_row_ptr + 1048576));
                            #pragma unroll
                            for (int elem_7 = 0; elem_7 < 8; elem_7++) {
                                r3_raw_1[elem_7] = _tmem_load_10[elem_7];
                                r3_raw_1[elem_7 + 8] = _tmem_load_11[elem_7];
                            }
                            float r3_1[16];
                            r3_1[0] = r3_raw_1[0];
                            r3_1[1] = r3_raw_1[1];
                            ...
                            asm volatile(
                                ...
                                : "r"(warp_row_ptr + 1048576));
                            #pragma unroll
                            for (int elem_8 = 0; elem_8 < 8; elem_8++) {
                                r4_raw_1[elem_8] = _tmem_load_12[elem_8];
                                r4_raw_1[elem_8 + 8] = _tmem_load_13[elem_8];
                            }
    ...
    if (warp == 4) {
        { // mma_main
            ...
            #pragma unroll 1
            for (int expert_row_tile_2 = linear_thread_1; expert_row_tile_2 < 128; expert_row_tile_2 += thread_stride_1) {
                {
            ...
            if (cta_rank == 0) {
                ...
                for (int phase_1 = 0; phase_1 < 2; phase_1++) {
                    ...
                    for (int work_1 = work_begin_1; work_1 < work_end_1; work_1 += (int)num_clusters) {
                        ...
                        int _max_1 = ((1) > ((route_count_1 + 16 - 1) / 16) ? (1) : ((route_count_1 + 16 - 1) / 16));
                        ...
                        for (int iter_k = 0; iter_k < num_k_tiles_1; iter_k++) {
                            ...
                            for (int segment_1 = 0; segment_1 < active_segments_1; segment_1++) {
                                ...
                                asm volatile("tcgen05.fence::after_thread_sync;");
                                int _mma_b_lo_0 = (((b_smem_addr) >> 4) & 0x3FFF) + (b_stage) * 128;
                                asm volatile(
                    "{\n\t"
                    ...
                    "@leader tcgen05.mma.cta_group::2.kind::f16 [%0], [ta], db, id, {m0, m1, m2, m3, m4, m5, m6, m7}, p1;\n\t"
                    "}\n"
                    :: "r"((tmem_accum + ((acc_stage_1 * 4 + (unsigned int)segment_1) * 32))), "r"(_mma_b_lo_0), "r"((unsigned int)tmem_transformed_a + transformed_stage * 64), "r"(((((iter_k == 0) ? 1 : 0)) ? 0 : 1)));
    ...
    if (warp == 5) {
        { // tma_a_scale_main
            ...
            #pragma unroll 1
            for (int expert_row_tile_3 = linear_thread_2; expert_row_tile_3 < 128; expert_row_tile_3 += thread_stride_2) {
                {
    ...
    if (warp == 6) {
        { // tma_b_main
            ...
            #pragma unroll 1
            for (int expert_row_tile_4 = linear_thread_3; expert_row_tile_4 < 128; expert_row_tile_4 += thread_stride_3) {
                {
            ...
            for (int phase_3 = 0; phase_3 < 2; phase_3++) {
                ...
                for (int work_3 = work_begin_3; work_3 < work_end_3; work_3 += (int)num_clusters) {
                    ...
                    int _max_0 = ((1) > ((route_count_2 + 16 - 1) / 16) ? (1) : ((route_count_2 + 16 - 1) / 16));
                    ...
                    for (int iter_k_2 = 0; iter_k_2 < num_k_tiles_3; iter_k_2++) {
                        ...
                        for (int segment_3 = 0; segment_3 < active_segments_2; segment_3++) {
                            ...
                            if (elect_sync()) {
                                int packed_row = segment_3 * 16 + cta_rank * 8;
                                if (phase_3 == 0) {
                                    tma_4d_gmem2smem_cta2(b_smem_addr + b_stage_1 * 2048, activation_bf16, 0, packed_row, iter_k_2 * 2, expert_7, ((b_full_addr + (b_stage_1) * 8) & 0xFEFFFFFF));
                                } else {
                                    tma_4d_gmem2smem_cta2(b_smem_addr + b_stage_1 * 2048, fc1_workspace_bf16_tma, 0, packed_row, iter_k_2 * 2, expert_7, ((b_full_addr + (b_stage_1) * 8) & 0xFEFFFFFF));
                                }
                                asm volatile(
    ...
    if (warp == 7) {
        { // scheduler_main
            ...
            #pragma unroll 1
            for (int expert_row_tile_5 = linear_thread_4; expert_row_tile_5 < 128; expert_row_tile_5 += thread_stride_4) {
                {
    ...
    if (warp >= 8 && warp <= 11) {
        ...
        { // transform_main
            ...
            #pragma unroll 1
            for (int expert_row_tile_6 = linear_thread_5; expert_row_tile_6 < 128; expert_row_tile_6 += thread_stride_5) {
                {
