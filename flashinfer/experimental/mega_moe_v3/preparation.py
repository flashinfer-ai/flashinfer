...
def _mega_dispatch_reference(topk_idx, num_experts: int, align_m: int = ALIGN_M, block_m: int = BLOCK_M):
...
def _sf_float_to_u32_words(sf_float):
    ...
    sf_bytes = ((sf_float.contiguous().view(torch.int32) >> 23) & 0xFF).to(torch.uint8).contiguous()
...
def build_tile_lists(per_expert_M, num_experts: int):
    ...
    for e in range(num_experts):
        ...
        m_e = per_expert_M[e]
        assert m_e % BLOCK_M == 0, f"expert {e} M={m_e} not aligned to BLOCK_M={BLOCK_M}"
...
def prepare_pipeline_bindings(inputs):
    ...
    capture_l1 = False
    import torch
    E, TK, T, H, I = (inputs[k] for k in ('num_experts', 'top_k', 'num_tokens', 'hidden', 'intermediate'))
    ...
    if not 1 <= E <= DISP_NUM_EXPERTS_MAX or not 1 <= TK <= min(E, 32):
        raise ValueError('MegaMoE requires 1..384 experts and 1..min(experts,32) routes')
    if T < 1 or H % BLOCK_K or I % BLOCK_K:
        raise ValueError('MegaMoE requires positive T and H/I divisible by 256')
    source_schedule = bool(pr432_optimizations) and (not capture_l1) and ((E, TK, H, I) == (384, 6, 5120, 2304))
    ...
    x_fp8 = inputs['x_fp8_packed'].view(torch.uint8).reshape(T, H).view(torch.int32).contiguous()
    ...
    SFB1 = _sf_float_to_u32_words(_interleave_gate_up(inputs['w1_sf'])).view(torch.uint32)
    ...
    residual_readiness = source_schedule and persistent_grid in (148, 152) and ((float('inf') if inputs.get('activation_clamp') is None else float(inputs['activation_clamp'])) == 10.0) and (weight_fmt == 'fp4' and T in (16, 128, 512) or (weight_fmt == 'fp8' and T == 16))
    arrival_words = total_m_tiles * K2_tiles
    if source_schedule:
        arrival_words = max(arrival_words, 2 + (3 if T == 1 or residual_readiness else 2) * source_blocks)
    reset_storage = torch.zeros(2 * E + 4 + arrival_words, dtype=torch.uint32, device='cuda') if source_schedule and (T == 1 or residual_readiness) else None
    expert_counts = reset_storage[:E].view(torch.int32) if reset_storage is not None else torch.zeros(E, dtype=torch.int32, device='cuda')
    expert_row_offsets = torch.zeros(E, dtype=torch.int32, device='cuda')
    expert_scatter_offsets = reset_storage[E:2 * E].view(torch.int32) if reset_storage is not None else torch.zeros(E, dtype=torch.int32, device='cuda')
    ...
    l1_bf16 = torch.full((M_total, N1), float('nan'), dtype=torch.bfloat16, device='cuda') if capture_l1 else expert_output
    ...
    combine_chunks = 2 if pr432_optimizations and H > 4096 else 1
    launch_kwargs = dict(grid=grid, topk_idx_i32=topk_idx_i32, topk_weights=topk_weights_flat, x_fp8=x_fp8, x_sf=x_sf, A=pool_fp8.view(torch.uint8), pool_fp8=pool_fp8, pool_sf=pool_sf.view(torch.uint32), routing_weight_pool=routing_weight_pool, token_to_permuted=token_to_permuted, meta_token=meta_token, meta_slot=meta_slot, expert_counts=expert_counts, expert_row_offsets=expert_row_offsets, expert_scatter_offsets=expert_scatter_offsets, tile_expert=tile_expert, tile_m_local=tile_m_local, total_m_tiles_out=total_m_tiles_out, B1=B1, SFB1=SFB1, I_fp8_w=I_fp8, SF_I_w=SF_I, l1_bf16_capture=l1_bf16, A2=I_fp8, SFA2=SF_I.view(torch.uint32), B2=B2, SFB2=SFB2, expert_output=expert_output, y=y, histogram_done=histogram_done, prefix_done=prefix_done, dispatch_done=dispatch_done, l1_arrival=l1_arrival, l2_done=l2_done, num_tokens=T, top_k=TK, num_experts=E, N1=N1, K1=K1, grid_n1=grid_n1, K1_tiles=K1_tiles, N2=N2, K2=K2, grid_n2=grid_n2, K2_tiles=K2_tiles, total_m_tiles=total_m_tiles, M_total=M_total, activation_clamp=float('inf') if inputs.get('activation_clamp') is None else float(inputs['activation_clamp']))
    if source_schedule:
        for name in ('SFB1', 'SFB2'):
            original = launch_kwargs[name]
            groups, rows, words_k = original.shape
            packed = original.reshape(groups, rows // 128, 4, 32, words_k).transpose(2, 3)
            launch_kwargs[name] = packed.reshape(groups, rows, words_k).permute(0, 2, 1).contiguous().reshape(-1, rows)
        for name in ('B1', 'B2'):
            original = launch_kwargs[name]
            launch_kwargs[name] = original.reshape(-1, original.shape[-1])
        source_rows = source_blocks * 128
        source_sf1 = torch.zeros((H // 128, source_rows), dtype=torch.uint32, device='cuda')
        source_sf2 = torch.zeros((I // 128, source_rows), dtype=torch.uint32, device='cuda')
        launch_kwargs.update(PrivateCounters=l1_arrival[:2], PrivateMasks=l1_arrival[2:2 + 2 * source_blocks].view(torch.uint64), PublicExpertOutput=expert_output.view(torch.uint32), PrivateSFBlockOffsets=torch.empty((E,), dtype=torch.uint32, device='cuda'), PrivateSF1=source_sf1, PrivateSF2=source_sf2, PrivateSF1Words=source_sf1.reshape(-1), PrivateSF2Bytes=source_sf2.view(torch.uint8).reshape(-1))
    reset_buffers = (expert_counts, expert_scatter_offsets, histogram_done, prefix_done, dispatch_done, l1_arrival, l2_done)
    return dict(bindings=launch_kwargs, reset_storage=reset_storage, reset_buffers=reset_buffers, output=y, expert_output=expert_output, source_schedule=source_schedule, grid=grid, M_total=M_total, total_m_tiles=total_m_tiles)
