...
kernel_cake_mla_varq_dcp_decode_252672ef0d898f524376(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_k, const __grid_constant__ CUtensorMap tmap_v, const __grid_constant__ CUtensorMap tmap_o, const __grid_constant__ CUtensorMap tmap_po, __nv_bfloat16* __restrict__ O, float* __restrict__ LSE, __nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_lse, int* __restrict__ page_table, int* __restrict__ seq_lens, int* __restrict__ cum_seq_lens_q, int* __restrict__ causal_global, unsigned int* __restrict__ sched_counters, unsigned int* __restrict__ unit_flags, unsigned int* __restrict__ split_meta, unsigned int* __restrict__ merge_ctl, float softmax_scale_log2, int tiles_max, int num_heads, int max_pages, int cp_world, int cp_rank, int num_items, int unit_min, int static_tiles, int unit_num, int unit_den, int max_units, int static_only, unsigned long long* __restrict__ dbg, int partial_slots, CakeFastDivmod fd_tiles_max, CakeFastDivmod fd_num_heads, CakeFastDivmod fd_cp_world, CakeFastDivmod fd_num_items, CakeFastDivmod fd_unit_min, CakeFastDivmod fd_static_tiles, CakeFastDivmod fd_unit_den, CakeFastDivmod fd_clusters)
{
    ...
    #define kv_empty_addr (mbar_base + 248)
    #define s_full_addr (mbar_base + 400)
    #define s_empty_addr (mbar_base + 416)
    #define p_full_addr (mbar_base + 432)
    #define p_empty_addr (mbar_base + 448)
    #define o_empty_addr (mbar_base + 464)
    #define stats_addr (mbar_base + 472)
    #define stats_empty_addr (mbar_base + 488)
    #define o_full_addr (mbar_base + 504)
    #define o_first_slice_seeded_addr (mbar_base + 512)
    #define tmem_dealloc_peer_addr (mbar_base + 520)
    ...
    uint8_t* smem_kv = reinterpret_cast<uint8_t*>(smem_raw + 41984);
    ...
    // Mbarrier init (22 pipeline groups, 0 ordered-sequence groups, 66 barriers)
    // Mbarriers at smem_raw[0..528)
    ...
    if (warp == 0) {
        ...
        // --- pipeline 'kv_pipe' ---
        // kv_full: 19 barriers, init_count=1
        // kv_empty: 19 barriers, init_count=1
        ...
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_0_32) : "r"(lane), "n"(31), "r"((uint32_t)(128)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_0_32) : "r"(lane), "n"(27), "r"((uint32_t)(256)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_0_32) : "r"(lane), "n"(26), "r"((uint32_t)(1)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_0_32) : "r"(lane), "n"(24), "r"((uint32_t)(256)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_0_32) : "r"(lane), "n"(20), "r"((uint32_t)(1)));
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 528);
    if (warp == 0) {
        int _tmem_hold = smem + 528;
            ...
            for (unsigned int _unit_iter_softmax = 0; _unit_iter_softmax < max_units + 1; _unit_iter_softmax++) {
                ...
                if (num_kv_tiles > 0) {
                    ...
                    for (int tile = 0; tile < num_kv_tiles; tile++) {
                        ...
                        #pragma unroll
                        for (int ps_local = 0; ps_local < 1; ps_local++) {
                            int ps = n_half + ps_local;
                            int p_stage = phase * 2 + ps;
                            int p_base_smem = smem_p_addr + (unsigned int)(p_stage * 4096);
                            #pragma unroll
                            for (int vec = 0; vec < 4; vec++) {
                                int pv_off = (ps_local * 4 + vec) * 4;
                                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"((p_base_smem + (p_row * 64 + vec * 16 ^ (p_row * 64 + vec * 16 >> 7 & 3) << 4))), "r"(_fp8_0[pv_off]), "r"(_fp8_0[pv_off + 1]), "r"(_fp8_0[pv_off + 2]), "r"(_fp8_0[pv_off + 3]) : "memory");
                            }
                        }
                        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                        asm volatile(
            ...
            for (unsigned int _unit_iter_mma = 0; _unit_iter_mma < max_units + 1; _unit_iter_mma++) {
                ...
                if (num_kv_tiles_2 > 0) {
                    ...
                    :: "r"(_mma_a_lo_0), "r"(_mma_b_lo_0), "r"((tmem_tmem_scratch + (sc_col))), "r"(((n_m == 0) ? 0 : 1)));
                            ...
                            mma_kv_stage += 1;
                            if (mma_kv_stage == 19) { mma_kv_stage = 0; _phase_kv_full ^= 1; }
                        }
                        mbarrier_wait(kv_full_addr + (mma_kv_stage) * 8, _phase_kv_full);
                        int _mma_a_lo_1 = (((smem_q_addr) >> 4) & 0x3FFF) + (4) * 512;
                    ...
                    :: "r"(_mma_a_lo_2), "r"(_mma_b_lo_2), "r"((tmem_tmem_scratch + (sc_col_3))), "r"(((n_m_1 == 0) ? 0 : 1)));
                                ...
                                mbarrier_wait(kv_full_addr + (mma_kv_stage) * 8, _phase_kv_full);
                                int _mma_a_lo_3 = (((smem_q_addr) >> 4) & 0x3FFF) + (4) * 512;
                    ...
                    :: "r"(_mma_a_lo_3), "r"(_mma_b_lo_3), "r"((tmem_tmem_scratch + (sc_col_3))), "r"(1));
                            ...
                            asm volatile("tcgen05.fence::after_thread_sync;");
                            #pragma unroll
                            for (int ps_m = 0; ps_m < 2; ps_m++) {
                                int p_stage_m = ph_p * 2 + ps_m;
                                #pragma unroll
                                for (int acc_m = 0; acc_m < 2; acc_m++) {
                                    mbarrier_wait(kv_full_addr + (mma_kv_stage) * 8, _phase_kv_full);
                                    int out_col_m = 128 + acc_m * 128;
                                    int _mma_a_lo_4 = (((smem_p_addr) >> 4) & 0x3FFF) + (p_stage_m) * 256;
                                    int _mma_b_lo_4 = ((((smem_v_addr) >> 4) & 0x3FFF) | 0x2000000) + (mma_kv_stage) * 512;
                                    asm volatile(
                    "{\n\t"
                    ".reg .pred leader, p0, p1;\n\t"
                    ".reg .b32 adhi, bdhi, alo, blo, id, m0, m1, m2, m3, m4, m5, m6, m7;\n\t"
            ...
            for (unsigned int _unit_iter_load = 0; _unit_iter_load < max_units + 1; _unit_iter_load++) {
                ...
                if (num_kv_tiles_4 > 0) {
                        ...
                        if (cta_rank == 0) {
                            if (elect_sync()) {
                                asm volatile(
                                    ...
                                    :: "r"((q_full_addr) & 0xFEFFFFFF), "r"((uint32_t)(81920)) : "memory");
                            }
                        }
                        if (elect_sync()) {
                            #pragma unroll
                            for (int s = 0; s < 5; s++) {
                                ...
                                tma_2d_gmem2smem_cta2(q_dst, (&tmap_q), s * 128, q_row_global, ((q_full_addr) & 0xFEFFFFFF));
                            }
                        }
                        int pg_k0[1];
                        int off_k0[1];
                        ...
                        for (int n_l = 0; n_l < 5; n_l++) {
                            ...
                            if (elect_sync()) {
                                ...
                                for (int kb_l = 0; kb_l < 1; kb_l++) {
                                    ...
                                    tma_3d_gmem2smem_cta2(dst_kl + kb_l * 8192, (&tmap_k), n_l * 128, off_kbl, pg_kbl, ((kv_full_addr + (load_kv_stage) * 8) & 0xFEFFFFFF));
                                }
                            }
                            load_kv_stage += 1;
                            if (load_kv_stage == 19) { load_kv_stage = 0; _phase_kv_empty ^= 1; }
                        }
                    }
                    spec_left = 0;
                    int pg_kn[1];
                    ...
                    if (num_kv_tiles_4 > 0) {
                        ...
                        #pragma unroll
                        for (int psv_l = 0; psv_l < 2; psv_l++) {
                            #pragma unroll
                            for (int vbv_l = 0; vbv_l < 1; vbv_l++) {
                                int tok_vl = tok_vb + psv_l * 64 + vbv_l * 64;
                    ...
                    for (int tile_3 = 0; tile_3 < num_kv_tiles_4; tile_3++) {
                        ...
                        if (num_kv_tiles_4 > tile_3 + 1) {
                            ...
                            #pragma unroll
                            for (int psv_l_1 = 0; psv_l_1 < 2; psv_l_1++) {
                                #pragma unroll
                                for (int vbv_l_1 = 0; vbv_l_1 < 1; vbv_l_1++) {
                                    int tok_vl_1 = tok_vb_1 + psv_l_1 * 64 + vbv_l_1 * 64;
                            ...
                            for (int n_l_1 = 0; n_l_1 < 5; n_l_1++) {
                                ...
                                if (elect_sync()) {
                                    ...
                                    for (int kb_l_1 = 0; kb_l_1 < 1; kb_l_1++) {
                                        ...
                                        tma_3d_gmem2smem_cta2(dst_kl_1 + kb_l_1 * 8192, (&tmap_k), n_l_1 * 128, off_kbl_1, pg_kbl_1, ((kv_full_addr + (load_kv_stage) * 8) & 0xFEFFFFFF));
                                    }
                                }
                                load_kv_stage += 1;
                                if (load_kv_stage == 19) { load_kv_stage = 0; _phase_kv_empty ^= 1; }
                            ...
                            for (int acc_l = 0; acc_l < 2; acc_l++) {
                                ...
                                if (elect_sync()) {
                                    ...
                                    for (int vb_l = 0; vb_l < 1; vb_l++) {
                                        ...
                                        int v_col_l = acc_l * 256 + cta_rank * 128;
                                        tma_3d_gmem2smem_cta2(dst_vl + vb_l * 8192, (&tmap_v), v_col_l, off_vl, pg_vl, ((kv_full_addr + (load_kv_stage) * 8) & 0xFEFFFFFF));
                                    }
                                }
                                load_kv_stage += 1;
                                if (load_kv_stage == 19) { load_kv_stage = 0; _phase_kv_empty ^= 1; }
