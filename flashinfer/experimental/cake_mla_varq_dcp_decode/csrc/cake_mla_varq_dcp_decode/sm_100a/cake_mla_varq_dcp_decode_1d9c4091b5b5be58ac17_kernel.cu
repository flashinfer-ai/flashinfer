...
kernel_cake_mla_varq_dcp_decode_1d9c4091b5b5be58ac17(const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_k, const __grid_constant__ CUtensorMap tmap_v, const __grid_constant__ CUtensorMap tmap_o, const __grid_constant__ CUtensorMap tmap_po, __nv_bfloat16* __restrict__ O, float* __restrict__ LSE, __nv_bfloat16* __restrict__ partial_O, float* __restrict__ partial_lse, int* __restrict__ page_table, int* __restrict__ seq_lens, int* __restrict__ cum_seq_lens_q, int* __restrict__ causal_global, unsigned int* __restrict__ sched_counters, unsigned int* __restrict__ unit_flags, unsigned int* __restrict__ split_meta, unsigned int* __restrict__ merge_ctl, float softmax_scale_log2, int tiles_max, int num_heads, int max_pages, int cp_world, int cp_rank, int num_items, int unit_min, int static_tiles, int unit_num, int unit_den, int max_units, int static_only, unsigned long long* __restrict__ dbg, int partial_slots, CakeFastDivmod fd_tiles_max, CakeFastDivmod fd_num_heads, CakeFastDivmod fd_cp_world, CakeFastDivmod fd_num_items, CakeFastDivmod fd_unit_min, CakeFastDivmod fd_static_tiles, CakeFastDivmod fd_unit_den, CakeFastDivmod fd_clusters)
{
    ...
    #define kv_empty_addr (mbar_base + 360)
    #define s_full_addr (mbar_base + 512)
    #define s_empty_addr (mbar_base + 528)
    #define p_full_addr (mbar_base + 544)
    #define p_empty_addr (mbar_base + 560)
    #define o_empty_addr (mbar_base + 576)
    #define stats_addr (mbar_base + 584)
    #define stats_empty_addr (mbar_base + 600)
    #define o_full_addr (mbar_base + 616)
    #define o_first_slice_seeded_addr (mbar_base + 624)
    #define tmem_dealloc_peer_addr (mbar_base + 632)
    ...
    // Mbarrier init (22 pipeline groups, 0 ordered-sequence groups, 80 barriers)
    ...
    if (warp == 0) {
        ...
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_0_64) : "r"(lane), "n"(15), "r"((uint32_t)(256)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_0_64) : "r"(lane), "n"(14), "r"((uint32_t)(1)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_0_64) : "r"(lane), "n"(13), "r"((uint32_t)(128)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_0_64) : "r"(lane), "n"(9), "r"((uint32_t)(256)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_0_64) : "r"(lane), "n"(8), "r"((uint32_t)(1)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_0_64) : "r"(lane), "n"(6), "r"((uint32_t)(256)));
        asm volatile("{ .reg .pred p; setp.lt.u32 p, %1, %2; selp.u32 %0, %3, %0, p; }" : "+r"(_mbarrier_init_count_0_64) : "r"(lane), "n"(2), "r"((uint32_t)(1)));
        if (lane < 16) {
        ...
        { // load_warp_main
            ...
            if (cluster_id < (unsigned int)num_items) {
                ...
                if (n_w2 > 0) {
                    ...
                    if (elect_sync()) {
                        #pragma unroll
                        for (int s_q = 0; s_q < 5; s_q++) {
                            int q_dst_q = smem_q_addr + (unsigned int)(s_q * 8192);
                            tma_2d_gmem2smem_cta2(q_dst_q, (&tmap_q), s_q * 128, q_begin_6 * num_heads + sm_w2 * 128 + cta_rank * 64, ((q_full_addr) & 0xFEFFFFFF));
                        }
                    }
                    int pg_k0b[1];
                    int off_k0b[1];
            ...
            if (spec_left == 0) {
                if (hit_sp >= 0) {
                    if (end_sp > start_sp) {
                        ...
                        if (elect_sync()) {
                            #pragma unroll
                            for (int s_q_1 = 0; s_q_1 < 5; s_q_1++) {
                                int q_dst_q_1 = smem_q_addr + (unsigned int)(s_q_1 * 8192);
                                tma_2d_gmem2smem_cta2(q_dst_q_1, (&tmap_q), s_q_1 * 128, qb_sp * num_heads + sm_sp * 128 + cta_rank * 64, ((q_full_addr) & 0xFEFFFFFF));
                            }
                        }
                        int pg_k0e[1];
                        int off_k0e[1];
            ...
            for (unsigned int _unit_iter_load = 0; _unit_iter_load < max_units + 1; _unit_iter_load++) {
                ...
                if (num_kv_tiles_4 > 0) {
                        ...
                        for (int n_l_2 = 0; n_l_2 < 5; n_l_2++) {
                            ...
                            if (elect_sync()) {
                                ...
                                for (int kb_l_2 = 0; kb_l_2 < 1; kb_l_2++) {
                                    ...
                                    tma_3d_gmem2smem_cta2(dst_kl_2 + kb_l_2 * 8192, (&tmap_k), n_l_2 * 128, off_kbl_2, pg_kbl_2, ((kv_full_addr + (load_kv_stage) * 8) & 0xFEFFFFFF));
                                }
                            }
                            load_kv_stage += 1;
                            if (load_kv_stage == 19) { load_kv_stage = 0; _phase_kv_empty ^= 1; }
                    ...
                    for (int tile_3 = 0; tile_3 < num_kv_tiles_4; tile_3++) {
                        ...
                        if (has_k != 0) {
                            #pragma unroll
                            for (int n_l_3 = 0; n_l_3 < 5; n_l_3++) {
                                if (early_t != 0) {
                                    if (load_kv_stage >= 10) {
                                        mbarrier_wait(kv_empty_addr + (load_kv_stage - 10) * 8, 0);
                                    }
                                }
                                mbarrier_wait(kv_empty_addr + (load_kv_stage) * 8, _phase_kv_empty);
                                if (cta_rank == 0) {
                                ...
                                if (elect_sync()) {
                                    ...
                                    for (int kb_l_3 = 0; kb_l_3 < 1; kb_l_3++) {
                                        ...
                                        tma_3d_gmem2smem_cta2(dst_kl_3 + kb_l_3 * 8192, (&tmap_k), n_l_3 * 128, off_kbl_3, pg_kbl_3, ((kv_full_addr + (load_kv_stage) * 8) & 0xFEFFFFFF));
                                    }
                                }
                                load_kv_stage += 1;
                                if (load_kv_stage == 19) { load_kv_stage = 0; _phase_kv_empty ^= 1; }
