        ...
        { // load_warp_main
            ...
            if (cluster_id < (unsigned int)num_items) {
                ...
                for (int kbb = 0; kbb < 2; kbb++) {
                    pg_pf[kbb] = page_table[sb_w2 * max_pages + (tok_b + kbb * 32) / 32];
                }
                ...
                if (n_w2 > 0) {
                    ...
                    for (int kbl = 0; kbl < 2; kbl++) {
                        int tok_kbl = tok_kl + kbl * 32;
                        off_k0b[kbl] = tok_kbl % 32;
                        if (one_sp != 0) {
                            pg_k0b[kbl] = pg_pf[kbl];
                        } else {
                            pg_k0b[kbl] = page_table[pt_sp + tok_kbl / 32];
                        }
                    }
                    #pragma unroll
                    for (int n_l = 0; n_l < 9; n_l++) {
                        ...
                        if (elect_sync()) {
                            ...
                            for (int kb_l = 0; kb_l < 2; kb_l++) {
                                ...
                                tma_4d_gmem2smem_cta2(dst_kl + kb_l * 4096, (&tmap_k), 0, off_kbl, n_l, pg_kbl, ((kv_full_addr + (load_kv_stage) * 8) & 0xFEFFFFFF));
                            }
                        }
                        load_kv_stage += 1;
            ...
            if (spec_left == 0) {
                if (hit_sp >= 0) {
                    if (end_sp > start_sp) {
                        ...
                        #pragma unroll
                        for (int kbl_1 = 0; kbl_1 < 2; kbl_1++) {
                            int tok_kbl_1 = tok_kl_1 + kbl_1 * 32;
                            off_k0e[kbl_1] = tok_kbl_1 % 32;
                            {
                                pg_k0e[kbl_1] = page_table[pt_sp + tok_kbl_1 / 32];
                            }
                        }
                        #pragma unroll
                        for (int n_l_1 = 0; n_l_1 < 9; n_l_1++) {
                            ...
                            if (elect_sync()) {
                                ...
                                #pragma unroll
                                for (int kb_l_1 = 0; kb_l_1 < 2; kb_l_1++) {
                                    int pg_kbl_1 = pg_k0e[kb_l_1];
                                    int off_kbl_1 = off_k0e[kb_l_1];
                                    tma_4d_gmem2smem_cta2(dst_kl_1 + kb_l_1 * 4096, (&tmap_k), 0, off_kbl_1, n_l_1, pg_kbl_1, ((kv_full_addr + (load_kv_stage) * 8) & 0xFEFFFFFF));
                                }
                            }
                            load_kv_stage += 1;
                            if (load_kv_stage == 15) { load_kv_stage = 0; _phase_kv_empty ^= 1; }
            ...
            for (unsigned int _unit_iter_load = 0; _unit_iter_load < max_units + 1; _unit_iter_load++) {
                ...
                if (num_kv_tiles_4 > 0) {
                    ...
                    if (spec_left == 0) {
                        ...
                        #pragma unroll
                        for (int kbl_2 = 0; kbl_2 < 2; kbl_2++) {
                            int tok_kbl_2 = tok_kl_2 + kbl_2 * 32;
                            off_k0[kbl_2] = tok_kbl_2 % 32;
                            if (is_static_3 != 0) {
                                pg_k0[kbl_2] = pg_pf[kbl_2];
                            } else {
                                pg_k0[kbl_2] = page_table[pt_base + tok_kbl_2 / 32];
                            }
                        }
                        #pragma unroll
                        for (int n_l_2 = 0; n_l_2 < 9; n_l_2++) {
                            ...
                            if (elect_sync()) {
                                ...
                                #pragma unroll
                                for (int kb_l_2 = 0; kb_l_2 < 2; kb_l_2++) {
                                    int pg_kbl_2 = pg_k0[kb_l_2];
                                    int off_kbl_2 = off_k0[kb_l_2];
                                    tma_4d_gmem2smem_cta2(dst_kl_2 + kb_l_2 * 4096, (&tmap_k), 0, off_kbl_2, n_l_2, pg_kbl_2, ((kv_full_addr + (load_kv_stage) * 8) & 0xFEFFFFFF));
                                }
                            }
                            load_kv_stage += 1;
                            if (load_kv_stage == 15) { load_kv_stage = 0; _phase_kv_empty ^= 1; }
                    ...
                    if (num_kv_tiles_4 > 1) {
                        ...
                        #pragma unroll
                        for (int kbl_3 = 0; kbl_3 < 2; kbl_3++) {
                            int tok_kbl_3 = tok_kl_3 + kbl_3 * 32;
                            off_kn[kbl_3] = tok_kbl_3 % 32;
                            {
                                pg_kn[kbl_3] = page_table[pt_base + tok_kbl_3 / 32];
                            }
                        }
                    }
                    if (num_kv_tiles_4 > 0) {
                        ...
                        for (int psv_l = 0; psv_l < 4; psv_l++) {
                            ...
                            for (int vbv_l = 0; vbv_l < 1; vbv_l++) {
                                ...
                                pg_vn[psv_l + vbv_l] = page_table[pt_base + tok_vl / 32];
                                off_vn[psv_l + vbv_l] = tok_vl % 32;
                            }
                        }
                    }
                    #pragma unroll 1
                    for (int tile_3 = 0; tile_3 < num_kv_tiles_4; tile_3++) {
                        ...
                        if (num_kv_tiles_4 > tile_3 + 1 + 1) {
                            ...
                            #pragma unroll
                            for (int kbl_4 = 0; kbl_4 < 2; kbl_4++) {
                                int tok_kbl_4 = tok_kl_4 + kbl_4 * 32;
                                off_kn[kbl_4] = tok_kbl_4 % 32;
                                {
                                    pg_kn[kbl_4] = page_table[pt_base + tok_kbl_4 / 32];
                                }
                            }
                        }
                        if (num_kv_tiles_4 > tile_3 + 1) {
                            ...
                            for (int psv_l_1 = 0; psv_l_1 < 4; psv_l_1++) {
                                ...
                                for (int vbv_l_1 = 0; vbv_l_1 < 1; vbv_l_1++) {
                                    ...
                                    pg_vn[psv_l_1 + vbv_l_1] = page_table[pt_base + tok_vl_1 / 32];
                                    off_vn[psv_l_1 + vbv_l_1] = tok_vl_1 % 32;
                                }
                            }
                        }
                        if (has_k != 0) {
                            ...
                            for (int n_l_3 = 0; n_l_3 < 9; n_l_3++) {
                                ...
                                if (elect_sync()) {
                                    ...
                                    #pragma unroll
                                    for (int kb_l_3 = 0; kb_l_3 < 2; kb_l_3++) {
                                        int pg_kbl_3 = pg_kt[kb_l_3];
                                        int off_kbl_3 = off_kt[kb_l_3];
                                        tma_4d_gmem2smem_cta2(dst_kl_3 + kb_l_3 * 4096, (&tmap_k), 0, off_kbl_3, n_l_3, pg_kbl_3, ((kv_full_addr + (load_kv_stage) * 8) & 0xFEFFFFFF));
                                    }
                                }
                                load_kv_stage += 1;
                                if (load_kv_stage == 15) { load_kv_stage = 0; _phase_kv_empty ^= 1; }
