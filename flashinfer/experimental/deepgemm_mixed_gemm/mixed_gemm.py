...
def route_key(options):
    return ':'.join(str(options.get(key,default)) for key,default in (
        ('M',None),('N',None),('K',None),('num_sms',None),('variant',None),
        ('block_n',128),('gran_k_a',32)))
...
class MixedGemmPlan:
    ...
    """
    def __init__(self,a,b,a_scales,b_scales,*,m,out=None,block_n=128,gran_k_a=32,
                 variant=None,descriptor_workspace=None):
        import torch
        if a.device.type!='cuda' or torch.cuda.get_device_capability(a.device)!=(10,3):
            raise RuntimeError('Mixed FP8×FP4 GEMM requires the validated SM103a target')
        if a.ndim!=2 or b.ndim!=2 or a.dtype not in (torch.float8_e4m3fn,torch.uint8) or b.dtype not in (torch.int8,torch.uint8):
            raise ValueError('A must be E4M3 FP8 (or raw uint8); B must be packed E2M1 bytes')
        ...
        self.options=dict(M=m,N=n,K=k,num_sms=sms,block_n=block_n,gran_k_a=gran_k_a,variant=variant)
        try:
            route=_catalog()['routes'][route_key(self.options)]
        except KeyError as error:
            raise NotImplementedError(f'No exported mixed GEMM schedule for {self.options}') from error
        cfg=route['config']
        if a.shape[0]!=cfg['input_m']:
            raise ValueError(f"A must have {cfg['input_m']} rows for this production route")
        for tensor,shape in ((a_scales,(cfg['sfa_words'],cfg['sfa_mn'])),(b_scales,(cfg['sfb_words'],cfg['sfb_mn']))):
            if tensor.dtype not in (torch.int32,torch.uint32) or tuple(tensor.shape)!=shape:
                raise ValueError(f'Packed scale shape must be {shape} in int32/uint32 words')
        if out is None:
        ...
        self.bindings=dict(A=a.view(torch.uint8),B=b.view(torch.uint8),SFA=a_scales.view(torch.uint32),
            ...
            grid_m=cfg['grid_m'],grid_n=cfg['grid_n'],K_tiles=cfg['K_tiles'],
