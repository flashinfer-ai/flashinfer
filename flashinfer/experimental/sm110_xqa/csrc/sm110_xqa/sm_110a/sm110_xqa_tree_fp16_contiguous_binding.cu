...
void run_tree(
  ...
  CheckCuda(cudaFuncSetAttribute(reinterpret_cast<const void*>(kernel_sm110_xqa_tree_fp16_contiguous),
  ...
  CheckCuda(cudaLaunchKernel(reinterpret_cast<const void*>(kernel_sm110_xqa_tree_fp16_contiguous),
      dim3(static_cast<uint32_t>(grid_x), static_cast<uint32_t>(grid_y), static_cast<uint32_t>(grid_z)),
      dim3(256, 1, 1), arguments, 164864, stream));
}
