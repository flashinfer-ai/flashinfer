...
#define SMEM_RAW_K_OFF 163840
#define SMEM_RAW_K_STAGE_BYTES 32768
#define SMEM_RAW_K_STRIDE 32768
#define SMEM_RAW_V_OFF 196608
#define SMEM_RAW_V_STAGE_BYTES 16384
#define SMEM_RAW_V_STRIDE 16384
...
kernel_sm110_xqa_tree_fp8_contiguous(unsigned int q_seq_len, unsigned int num_kv_heads, unsigned int head_group_size, const unsigned int* __restrict__ q_cu_seq_lens, float attention_scale, __half* __restrict__ output, const __half* __restrict__ q, const unsigned int* __restrict__ mask, const uint8_t* __restrict__ cache, const int* __restrict__ sequence_lengths, const int* __restrict__ page_list, unsigned int capacity, unsigned int max_pages, float k_cache_scale, float v_cache_scale)
{
    ...
    uint8_t* raw_k = reinterpret_cast<uint8_t*>(smem_raw + 163840);
    const int raw_k_addr = smem + 163840;
    uint8_t* raw_v = reinterpret_cast<uint8_t*>(smem_raw + 196608);
    const int raw_v_addr = smem + 196608;

    // Mbarrier init (1 pipeline groups, 0 ordered-sequence groups, 1 barriers)
    // Mbarriers at smem_raw[212992..213000)

    if (warp == 0) {
        uint32_t leader = elect_sync();
        if (leader) {
            // mma_done: 1 barriers, init_count=1
            mbarrier_init(smem + 212992, 1);
    ...
    volatile int* tmem_addr_storage = (volatile int*)(smem_raw + 213000);
    if (warp == 0) {
        int _tmem_hold = smem + 213000;
    ...
    scale = scale * k_cache_scale;
    value_scale = v_cache_scale;
    #pragma unroll 1
    ...
    if (num_blocks > 0) {
        ...
        for (int copy_1 = 0; copy_1 < 8; copy_1++) {
            ...
            unsigned int row_1 = grain_1 / 32;
            unsigned int col_1 = grain_1 % 32 * 16;
            unsigned int token = row_1;
            unsigned int source_row = 0;
            source_row = ((unsigned int)(blockIdx.z * 2) * num_kv_heads + head) * capacity + token;
            asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16, %2;"
                :: "r"(raw_k_addr + grain_1 * 16), "l"(reinterpret_cast<const uint8_t*>(cache) + (source_row * 512 + col_1)), "r"((token < length) ? 16 : 0));
        }
        #pragma unroll 1
        for (int copy_2 = 0; copy_2 < 4; copy_2++) {
            unsigned int grain_2 = copy_2 * 256 + tid;
            unsigned int row_2 = grain_2 / 16;
            unsigned int col_2 = grain_2 % 16 * 16;
            unsigned int token_1 = row_2;
            unsigned int source_row_1 = 0;
            source_row_1 = ((unsigned int)(blockIdx.z * 2 + 1) * num_kv_heads + head) * capacity + token_1;
            asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16, %2;"
                :: "r"(raw_v_addr + grain_2 * 16), "l"(reinterpret_cast<const uint8_t*>(cache) + (source_row_1 * 512 + output_col + col_2)), "r"((token_1 < length) ? 16 : 0));
        }
        asm volatile("cp.async.commit_group;");
    }
    #pragma unroll 1
    for (unsigned int tile = 0; tile < num_blocks; tile++) {
        ...
        asm volatile("barrier.sync 8, 256;" ::: "memory");
        #pragma unroll 1
        for (int copy_3 = 0; copy_3 < 8; copy_3++) {
            unsigned int grain_3 = copy_3 * 256 + tid;
            unsigned int row_3 = grain_3 / 32;
            unsigned int col_3 = grain_3 % 32 * 16;
            ...
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                ...
                : "r"(raw_k_addr + grain_3 * 16));
            unsigned int widened[8];
            #pragma unroll
            for (int pair = 0; pair < 8; pair++) {
                uint32_t _fp8_f16x2_0;
                asm("cvt.rn.f16x2.e4m3x2 %0, %1;" : "=r"(_fp8_f16x2_0) : "h"((uint16_t)(words[pair / 2] >> (unsigned int)(pair % 2 * 16))));
                widened[pair] = _fp8_f16x2_0;
            }
            #pragma unroll
            for (int half = 0; half < 2; half++) {
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(sk_addr + col_3 / 64 * 8192 + row_3 * 128 + (unsigned int)((col_3 % 64 + (unsigned int)(half * 8)) * 2 ^ (row_3 & 7) << 4)), "r"(widened[half * 4]), "r"(widened[half * 4 + 1]), "r"(widened[half * 4 + 2]), "r"(widened[half * 4 + 3]) : "memory");
            }
        }
        #pragma unroll 1
        for (int copy_4 = 0; copy_4 < 4; copy_4++) {
            unsigned int grain_4 = copy_4 * 256 + tid;
            unsigned int row_4 = grain_4 / 16;
            unsigned int col_4 = grain_4 % 16 * 16;
            ...
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                ...
                : "r"(raw_v_addr + grain_4 * 16));
            unsigned int widened_1[8];
            #pragma unroll
            for (int pair_1 = 0; pair_1 < 8; pair_1++) {
                uint32_t _fp8_f16x2_1;
                asm("cvt.rn.f16x2.e4m3x2 %0, %1;" : "=r"(_fp8_f16x2_1) : "h"((uint16_t)(words_1[pair_1 / 2] >> (unsigned int)(pair_1 % 2 * 16))));
                widened_1[pair_1] = _fp8_f16x2_1;
            }
            #pragma unroll
            for (int half_1 = 0; half_1 < 2; half_1++) {
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(sv_0_addr + col_4 / 64 * 8192 + row_4 * 128 + (unsigned int)((col_4 % 64 + (unsigned int)(half_1 * 8)) * 2 ^ (row_4 & 7) << 4)), "r"(widened_1[half_1 * 4]), "r"(widened_1[half_1 * 4 + 1]), "r"(widened_1[half_1 * 4 + 2]), "r"(widened_1[half_1 * 4 + 3]) : "memory");
            }
        }
        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
        asm volatile("barrier.sync 8, 256;" ::: "memory");
        if (num_blocks > tile + 1) {
            #pragma unroll 1
            for (int copy_5 = 0; copy_5 < 8; copy_5++) {
                unsigned int grain_5 = copy_5 * 256 + tid;
                unsigned int row_5 = grain_5 / 32;
                unsigned int col_5 = grain_5 % 32 * 16;
                unsigned int token_2 = (tile + 1) * 64 + row_5;
                unsigned int source_row_2 = 0;
                source_row_2 = ((unsigned int)(blockIdx.z * 2) * num_kv_heads + head) * capacity + token_2;
                asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16, %2;"
                    :: "r"(raw_k_addr + grain_5 * 16), "l"(reinterpret_cast<const uint8_t*>(cache) + (source_row_2 * 512 + col_5)), "r"((token_2 < length) ? 16 : 0));
            }
            #pragma unroll 1
            for (int copy_6 = 0; copy_6 < 4; copy_6++) {
                unsigned int grain_6 = copy_6 * 256 + tid;
                unsigned int row_6 = grain_6 / 16;
                unsigned int col_6 = grain_6 % 16 * 16;
                unsigned int token_3 = (tile + 1) * 64 + row_6;
                unsigned int source_row_3 = 0;
                source_row_3 = ((unsigned int)(blockIdx.z * 2 + 1) * num_kv_heads + head) * capacity + token_3;
                asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16, %2;"
                    :: "r"(raw_v_addr + grain_6 * 16), "l"(reinterpret_cast<const uint8_t*>(cache) + (source_row_3 * 512 + output_col + col_6)), "r"((token_3 < length) ? 16 : 0));
            }
            asm volatile("cp.async.commit_group;");
        }
        if (warp == 0) {
        ...
        if (warp < 4) {
            ...
            if (prefix > tile * 64 + 63) {
                ...
                #pragma unroll
                for (int col_7 = 0; col_7 < 32; col_7++) {
                    unsigned int token_4 = tile * 64 + col_half * 32 + (unsigned int)col_7;
                    bool visible = row_valid && token_4 < length;
                    if (visible && token_4 >= prefix) {
                        unsigned int tree_col = token_4 - prefix;
                        unsigned int bits = mask[(request_offset + query_token) * mask_stride + tree_col / 32];
                        visible = (bits >> tree_col % 32 & 1) != 0;
                    }
                    if (!visible) {
                        _tmem_load_0[col_7] = -SM110_XQA_INF;
                    }
                }
            }
    ...
    if (warp < 4) {
        {
            ...
            if (row_valid) {
                ...
                #pragma unroll
                for (int col_8 = 0; col_8 < 64; col_8 += 8) {
                    {
                        const float2 _prescale2_23 = {inverse, inverse};
                        #if __CUDA_ARCH__ >= 1000
                        #pragma unroll
                        for (int _ps = 0; _ps < 4; _ps++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_3[col_8])[_ps], _prescale2_23);
                        #else
                        #pragma unroll
                        for (int _ps = 0; _ps < 8; _ps++)
                            _tmem_load_3[col_8 + _ps] *= inverse;
                        #endif
                        __half2 _pk[4];
                        _pk[0] = __floats2half2_rn(_tmem_load_3[col_8 + 0], _tmem_load_3[col_8 + 1]);
                        _pk[1] = __floats2half2_rn(_tmem_load_3[col_8 + 2], _tmem_load_3[col_8 + 3]);
                        _pk[2] = __floats2half2_rn(_tmem_load_3[col_8 + 4], _tmem_load_3[col_8 + 5]);
                        _pk[3] = __floats2half2_rn(_tmem_load_3[col_8 + 6], _tmem_load_3[col_8 + 7]);
                        *reinterpret_cast<uint4*>(&((__half*)(output + (out_head * 512 + output_col + col_half * 64 + (unsigned int)col_8)))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                    }
                }
            }
        }
        {
            ...
            if (row_valid) {
                ...
                #pragma unroll
                for (int col_9 = 0; col_9 < 64; col_9 += 8) {
                    {
                        const float2 _prescale2_24 = {inverse_1, inverse_1};
                        #if __CUDA_ARCH__ >= 1000
                        #pragma unroll
                        for (int _ps = 0; _ps < 4; _ps++)
                            mul_f32x2_inplace(&reinterpret_cast<float2*>(&_tmem_load_4[col_9])[_ps], _prescale2_24);
                        #else
                        #pragma unroll
                        for (int _ps = 0; _ps < 8; _ps++)
                            _tmem_load_4[col_9 + _ps] *= inverse_1;
                        #endif
                        __half2 _pk[4];
                        _pk[0] = __floats2half2_rn(_tmem_load_4[col_9 + 0], _tmem_load_4[col_9 + 1]);
                        _pk[1] = __floats2half2_rn(_tmem_load_4[col_9 + 2], _tmem_load_4[col_9 + 3]);
                        _pk[2] = __floats2half2_rn(_tmem_load_4[col_9 + 4], _tmem_load_4[col_9 + 5]);
                        _pk[3] = __floats2half2_rn(_tmem_load_4[col_9 + 6], _tmem_load_4[col_9 + 7]);
                        *reinterpret_cast<uint4*>(&((__half*)(output + (out_head_1 * 512 + output_col + 128 + col_half * 64 + col_9)))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                    }
                }
            }
        }
