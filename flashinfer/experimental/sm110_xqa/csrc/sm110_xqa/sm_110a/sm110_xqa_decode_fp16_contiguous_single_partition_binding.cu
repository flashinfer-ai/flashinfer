...
void run_decode(
  ...
  TVM_FFI_CHECK(partitions == 1, ValueError) << "single-partition route requires one partition";

  DeviceGuard guard(device);
  CheckArch(device);
  cudaStream_t stream = get_stream(device);
  CheckCuda(cudaFuncSetAttribute(reinterpret_cast<const void*>(kernel_sm110_xqa_decode_fp16_contiguous_single_partition),
  ...
  CheckCuda(cudaLaunchKernel(reinterpret_cast<const void*>(kernel_sm110_xqa_decode_fp16_contiguous_single_partition),
