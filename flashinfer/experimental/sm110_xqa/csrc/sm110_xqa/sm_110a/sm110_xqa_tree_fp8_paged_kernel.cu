...
kernel_sm110_xqa_tree_fp8_paged(unsigned int q_seq_len, unsigned int num_kv_heads, unsigned int head_group_size, const unsigned int* __restrict__ q_cu_seq_lens, float attention_scale, __half* __restrict__ output, const __half* __restrict__ q, const unsigned int* __restrict__ mask, const uint8_t* __restrict__ cache, const int* __restrict__ sequence_lengths, const int* __restrict__ page_list, unsigned int capacity, unsigned int max_pages, float k_cache_scale, float v_cache_scale)
{
    ...
    if (num_blocks > 0) {
        ...
        int tile_page_0 = 0;
        if (length > 0) {
            tile_page_0 = page_list[(unsigned int)(blockIdx.z * 2 + 1) * max_pages];
        }
        #pragma unroll 1
        for (int copy_2 = 0; copy_2 < 4; copy_2++) {
            ...
            if (token_1 < length) {
                source_row_1 = ((unsigned int)tile_page_0 * 128 + row_2) * num_kv_heads + head;
            }
            asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16, %2;"
    ...
    for (unsigned int tile = 0; tile < num_blocks; tile++) {
        ...
        if (num_blocks > tile + 1) {
            int tile_page_1 = 0;
            if (length > (tile + 1) * 64) {
                tile_page_1 = page_list[(unsigned int)(blockIdx.z * 2) * max_pages + (tile + 1) * 64 / 128];
            }
            #pragma unroll 1
            for (int copy_5 = 0; copy_5 < 8; copy_5++) {
                ...
                if (token_2 < length) {
                    source_row_2 = ((unsigned int)tile_page_1 * 128 + (tile + 1) * 64 % 128 + row_5) * num_kv_heads + head;
                }
                asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16, %2;"
            ...
            int tile_page_0_1 = 0;
            if (length > (tile + 1) * 64) {
                tile_page_0_1 = page_list[(unsigned int)(blockIdx.z * 2 + 1) * max_pages + (tile + 1) * 64 / 128];
            }
            #pragma unroll 1
            for (int copy_6 = 0; copy_6 < 4; copy_6++) {
                ...
                if (token_3 < length) {
                    source_row_3 = ((unsigned int)tile_page_0_1 * 128 + (tile + 1) * 64 % 128 + row_6) * num_kv_heads + head;
                }
                asm volatile("cp.async.cg.shared::cta.global [%0], [%1], 16, %2;"
