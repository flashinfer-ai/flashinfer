...
// tvm-ffi direct-source launcher for SM110 GQA decode kernel 'kernel_sm110_gqa_decode_short'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_sm110_gqa_decode_short(const __grid_constant__ Sm110GqaTensorMap64 Q, const __grid_constant__ Sm110GqaTensorMap64 K, const __grid_constant__ Sm110GqaTensorMap64 V, __half* __restrict__ O, int* __restrict__ sequence_lengths, float softmax_scale_log2);


namespace sm110_gqa_host_shim_36f368750879e67d {
...
void Run(TensorView arg_Q, TensorView arg_K, TensorView arg_V, TensorView arg_O, TensorView arg_sequence_lengths, double arg_softmax_scale_log2, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_sm110_gqa_decode_short),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_sm110_gqa_decode_short failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_sm110_gqa_decode_short), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_sm110_gqa_decode_short failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace sm110_gqa_host_shim_36f368750879e67d

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run_short, sm110_gqa_host_shim_36f368750879e67d::Run);
// clang-format on
