...
// tvm-ffi direct-source launcher for Sm110 kernel 'kernel_sm110_gqa_decode_n32_disjoint_s10'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
struct alignas(64) Sm110TensorMap64 { uint64_t opaque[16]; };
extern "C" __global__ void kernel_sm110_gqa_decode_n32_disjoint_s10(const __grid_constant__ Sm110TensorMap64 Q, const __grid_constant__ Sm110TensorMap64 K, const __grid_constant__ Sm110TensorMap64 V, float* __restrict__ partial_O, float* __restrict__ partial_max, float* __restrict__ partial_sum, unsigned int* __restrict__ completed, __half* __restrict__ O, int* __restrict__ sequence_lengths, float softmax_scale_log2);


namespace sm110_host_shim_4f24b974d7eebab6 {
...
inline int64_t Sm110DeviceMultiprocessorCount(int device_id) {
...
inline CUtensorMap EncodeTma_K(const TensorView& t) {
  ...
  TVM_FFI_CHECK(64u <= global_dim[0] && 1u <= global_dim[2] && 1u <= global_dim[3], ValueError)
      << "TMA box (64, 32, 1, 1) exceeds resolved global dims for 'K'";
...
inline CUtensorMap EncodeTma_V(const TensorView& t) {
  ...
  TVM_FFI_CHECK(64u <= global_dim[0] && 1u <= global_dim[2] && 1u <= global_dim[3], ValueError)
      << "TMA box (64, 32, 1, 1) exceeds resolved global dims for 'V'";
  int64_t carrier_stride_0 = s2;
  TVM_FFI_CHECK(carrier_stride_0 >= 0, ValueError)
...
void Run(TensorView arg_Q, TensorView arg_K, TensorView arg_V, TensorView arg_partial_O, TensorView arg_partial_max, TensorView arg_partial_sum, TensorView arg_completed, TensorView arg_O, TensorView arg_sequence_lengths, double arg_softmax_scale_log2, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  CheckCudaTensor(arg_completed, "completed");
  CheckDtype(arg_completed, "completed", 1, 32, 1);
  CheckContiguous(arg_completed, "completed");
  ...
  CheckSameCudaDevice(arg_partial_max, arg_Q, "partial_max", "Q");
  CheckSameCudaDevice(arg_partial_sum, arg_Q, "partial_sum", "Q");
  CheckSameCudaDevice(arg_completed, arg_Q, "completed", "Q");
  ...
  unsigned int* p_completed = static_cast<unsigned int*>(arg_completed.data_ptr());
  ...
  config.dynamicSmemBytes = 42112u;
  config.stream = stream;
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_sm110_gqa_decode_n32_disjoint_s10), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_sm110_gqa_decode_n32_disjoint_s10 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace sm110_host_shim_4f24b974d7eebab6

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, sm110_host_shim_4f24b974d7eebab6::Run);
