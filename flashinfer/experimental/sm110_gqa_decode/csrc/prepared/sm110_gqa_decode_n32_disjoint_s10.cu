...
#define SMEM_TICKET_SMEM_OFF 41984
...
kernel_sm110_gqa_decode_n32_disjoint_s10(const __grid_constant__ Sm110TensorMap64 Q, const __grid_constant__ Sm110TensorMap64 K, const __grid_constant__ Sm110TensorMap64 V, float* __restrict__ partial_O, float* __restrict__ partial_max, float* __restrict__ partial_sum, unsigned int* __restrict__ completed, __half* __restrict__ O, int* __restrict__ sequence_lengths, float softmax_scale_log2)
{
    ...
    unsigned int* ticket_smem = reinterpret_cast<unsigned int*>(smem_raw + 41984);
    const int ticket_smem_addr = smem + 41984;
    ...
    if (warp == 0) {
        { // softmax_main
            ...
            _phase_o_full_0 ^= 1;
            asm volatile("tcgen05.fence::after_thread_sync;");
            int total_blocks_0 = (seqlen_kv + 32 - 1) / 32;
    ...
    if (warp == 2) {
        { // load_warp_main
            ...
            asm volatile("tcgen05.wait::st.sync.aligned;" ::: "memory");
            mbarrier_arrive(padding_ready_addr);
            int total_blocks_0_1 = (seqlen_kv_2 + 32 - 1) / 32;
            ...
            for (int n_block_2 = 0; n_block_2 < num_n_blocks_2; n_block_2++) {
                ...
                if (elect_sync()) {
                    ...
                    if (first_block_2 + n_block_2 >= total_blocks_0_1 - 32 && total_blocks_0_1 > first_block_2 + n_block_2) {
                        asm volatile(
