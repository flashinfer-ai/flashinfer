...
// tvm-ffi direct-source launcher for Sm110 kernel 'kernel_sm110_gqa_decode_n32_b4_direct'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_sm110_gqa_decode_n32_b4_direct(const __grid_constant__ Sm110TensorMap64 Q, const __grid_constant__ Sm110TensorMap64 K, const __grid_constant__ Sm110TensorMap64 V, __half* __restrict__ O, int* __restrict__ sequence_lengths, float softmax_scale_log2);


namespace sm110_host_shim_aa825100d898a459 {
...
void Run(TensorView arg_Q, TensorView arg_K, TensorView arg_V, TensorView arg_O, TensorView arg_sequence_lengths, double arg_softmax_scale_log2, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  config.dynamicSmemBytes = 41984u;
  config.stream = stream;
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_sm110_gqa_decode_n32_b4_direct), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_sm110_gqa_decode_n32_b4_direct failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace sm110_host_shim_aa825100d898a459

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, sm110_host_shim_aa825100d898a459::Run);
