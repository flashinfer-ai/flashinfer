...
// tvm-ffi direct-source launcher for Sm110 kernel 'kernel_sm110_gqa_decode_n64_kvlast_s10'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
void Run(TensorView arg_Q, TensorView arg_K, TensorView arg_V, TensorView arg_partial_O, TensorView arg_partial_max, TensorView arg_partial_sum, TensorView arg_completed, TensorView arg_O, TensorView arg_sequence_lengths, double arg_softmax_scale_log2, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_sm110_gqa_decode_n64_kvlast_s10),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_sm110_gqa_decode_n64_kvlast_s10 failed: "
  ...
  config.dynamicSmemBytes = 50304u;
  config.stream = stream;
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_sm110_gqa_decode_n64_kvlast_s10), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_sm110_gqa_decode_n64_kvlast_s10 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace sm110_host_shim_491fd998bb88b500

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, sm110_host_shim_491fd998bb88b500::Run);
