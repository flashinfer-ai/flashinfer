...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_minimax_h3_varlen_attention_530411d4fd6f12553109'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_cake_minimax_h3_varlen_attention_530411d4fd6f12553109(__nv_bfloat16* __restrict__ v, float* __restrict__ v_amax_partial, int num_vectors, int num_chunks);


namespace cake_host_shim_5ea8913db942402a {
...
void Run(TensorView arg_v, TensorView arg_v_amax_partial, int64_t arg_num_vectors, int64_t arg_num_chunks, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_minimax_h3_varlen_attention_530411d4fd6f12553109), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_minimax_h3_varlen_attention_530411d4fd6f12553109 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_5ea8913db942402a

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_5ea8913db942402a::Run);
