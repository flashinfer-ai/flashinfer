...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_minimax_h3_varlen_attention_4b58552420c22471767c'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
void Run(TensorView arg_Q, TensorView arg_Q_raw, TensorView arg_K, TensorView arg_V, TensorView arg_O, TensorView arg_seg_begin, TensorView arg_seg_len, TensorView arg_unit_table, TensorView arg_partial_O, TensorView arg_partial_ML, int64_t arg_total_tiles, int64_t arg_num_heads, double arg_softmax_scale_log2, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  void* kargs[] = {&p_Q, &p_Q_raw, &p_K, &p_V, &p_O, &p_seg_begin, &p_seg_len, &p_unit_table, &p_partial_O, &p_partial_ML, &v_total_tiles, &v_num_heads, &v_softmax_scale_log2};
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_minimax_h3_varlen_attention_4b58552420c22471767c),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_minimax_h3_varlen_attention_4b58552420c22471767c failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_minimax_h3_varlen_attention_4b58552420c22471767c), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_minimax_h3_varlen_attention_4b58552420c22471767c failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_1c0458279ae2c177

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_1c0458279ae2c177::Run);
