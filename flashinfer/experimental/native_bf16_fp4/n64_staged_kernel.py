...
class NativeN64Bf16Fp4Kernel:
    ...
    def load_stage(self, a, b, sf, sa, sb, ssf, bn, tile, stage, stop):
        ...
        for i in cutlass.range_constexpr(
            cute.ceil_div(sa.shape[0] * a_cols, threads * 4)
        ):
            idx = tid * 4 + i * threads * 4
            if idx < sa.shape[0] * a_cols:
        ...
        for i in cutlass.range_constexpr(cute.ceil_div(64 * b_cols, threads * 4)):
            idx = tid * 4 + i * threads * 4
            if idx < 64 * b_cols:
                row, col = idx // b_cols, idx % b_cols
                valid = Int32(0)
                if bn * 64 + row < n and tile * self.tile_k < k and tile < stop:
                    valid = Int32(16)
                src = get_ptr_as_int64(
                    b, (bn * 64 + row) * (k // 8) + tile * b_cols + col
                )
        ...
        for i in cutlass.range_constexpr(cute.ceil_div(sf_bytes, threads * 16)):
            ...
            if idx < sf_bytes:
                ...
                if tile * self.tile_k < k and tile < stop:
                    valid = Int32(16)
                src = get_ptr_as_int64(
                    sf, (bn // 2) * (k // 64) * 512 + tile * sf_bytes + idx
                )
    ...
    def kernel(
        ...
        sa = smem.allocate_tensor(
            cutlass.Int32,
            cute.make_layout(
                (a_rows, a_cols + 4, self.stages),
                stride=(a_cols + 4, 1, a_rows * (a_cols + 4)),
            ),
            byte_alignment=16,
        )
        sb = smem.allocate_tensor(
            cutlass.Int32,
            cute.make_layout(
                (64, b_cols + 4, self.stages),
                stride=(b_cols + 4, 1, 64 * (b_cols + 4)),
            ),
            byte_alignment=16,
        )
        ...
        for offset in cutlass.range(per_split):
            ...
            for fragment in cutlass.range_constexpr(self.tile_k // 16):
                addr = get_smem_ptr_as_int32(
                    sa,
                    sa.layout((a_row, fragment * 8 + (lane // 16) * 4, stage)),
                )
                ...
                for nt in cutlass.range_constexpr(self.n_tiles):
                    ...
                    scale_row = ni + (bn % 2) * 64
                    si = (scale_row % 32) * 16 + (scale_row // 32) * 4
        ...
        for nt in cutlass.range_constexpr(self.n_tiles):
            for row in cutlass.range_constexpr(2):
                for col in cutlass.range_constexpr(2):
                    mi = group + row * 8
                    ni = bn * 64 + warp * self.n_tiles * 8 + nt * 8 + pair * 2 + col
