...
class VsaFwdSm90:
    def __init__(self, kv_stage=12, q_stage=1, epi_stage=2):
    ...
    def __call__(
        ...
        tma_atom_v, tma_tensor_v = self._make_tma_atoms_and_tensors(
            v, v_smem_layout_staged, (self.pv_mma_tiler[1], self.pv_mma_tiler[2])
        )
        ...
        grid = ((q.shape[0] // 64) * q.shape[2], 1, 1)
    ...
    def kernel(
        ...
        if warp_group_idx == self.math_warp_group_id:
            ...
            gD = cute.local_tile(mO_qdl, (64, 128), (bidx, 0, bidy))
...
def run_prepared(q, k, v, idx_gpu, cnt_gpu, order, sm_scale, out):
    ...
    compile_key = (q.device.index, use_order)
