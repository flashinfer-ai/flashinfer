...
"""Prepared MXFP8 grouped FC1/activation and FC2 from frozen Frost templates.

Inputs are already routed. Data are E4M3, scales are E8M0 bytes with one
...
def discover(root: Path | None = None) -> tuple[Mxfp8Kernel, ...]:
    ...
    if payload.get("schema_version") != 2 or not isinstance(
        ...
        raise RuntimeError(f"invalid MXFP8 manifest: {path}")
    result, seen = [], set()
    for raw in payload["kernels"]:
        op, identity = raw.get("op"), raw.get("id")
        if op not in _OPS:
            raise RuntimeError(f"unsupported MXFP8 op: {op}")
        ...
        if (
            ...
            raise RuntimeError(f"invalid MXFP8 launch tail: {identity}")
...
def _launch_arguments(
    ...
    weight = [w.permute(1, 2, 0) for w in weights]
...
class PreparedMxfp8GroupedGemm:
    ...
    (7). Each weight is contiguous [E,N,K]. Scale blobs must
    ...
    def __init__(
        ...
        *,
        ...
        if tokens.device.type != "cuda":
            raise ValueError("MXFP8 grouped GEMM requires CUDA tensors")
        with torch.cuda.device(tokens.device):
            if torch.cuda.is_current_stream_capturing():
                raise RuntimeError("prepare MXFP8 grouped GEMM outside capture")
        count = 2 if kernel.gated else 1
        if len(weights) != count or len(weight_scales) != count:
            raise ValueError(f"this MXFP8 kernel requires {count} weight/scale pairs")
        if tokens.ndim != 2 or weights[0].ndim != 3:
            raise ValueError("expected tokens[S,K] and weights[E,N,K]")
        s, k = tokens.shape
        e, n, wk = weights[0].shape
        if min(s, k, e, n) <= 0 or n % 128 or k % 128 or wk != k:
            raise ValueError(
                "positive dimensions and N/K divisible by 128 are required"
            )
        if max(s, k, n) >= 2**31:
            raise ValueError("MXFP8 grouped dimensions must fit int32")
        if any(w.shape != weights[0].shape for w in weights):
            raise ValueError("all weights must share shape [E,N,K]")
        ...
        if kernel.arch != runtime._arch_for(tokens.device) or not all(
            ...
            raise ValueError("MXFP8 artifact geometry/architecture mismatch")
        if any(t.dtype != torch.float8_e4m3fn for t in (tokens, *weights)):
        ...
        if (
            ...
            raise ValueError(
                "MXFP8 workspace must be a sufficiently large uint8 buffer"
            )
        tensors = (
            tokens,
            *weights,
        ...
        if any(t.device != tokens.device or not t.is_contiguous() for t in tensors):
            raise ValueError("MXFP8 tensors must be contiguous on one CUDA device")
        if workspace.data_ptr() % 128 or any(
            t.data_ptr() % 16
            for t in (tokens, *weights, token_scales, *weight_scales, out)
        ):
            raise ValueError(
                "MXFP8 data/scales require 16B and workspace 128B alignment"
            )
        writes = {
        ...
        if len(writes) != 2 or writes & reads:
            raise ValueError(
                "MXFP8 output/workspace must not alias inputs or each other"
            )
