...
kernel_cake_kimi_k3_fused_router_07cdba1397d2b574f330(float* __restrict__ logits, float* __restrict__ bias, float* __restrict__ topk_weights, int* __restrict__ topk_ids, int* __restrict__ sorted_token_ids, int* __restrict__ expert_ids, int* __restrict__ num_tokens_post_padded, int* __restrict__ expert_counts, int* __restrict__ expert_offsets, int* __restrict__ expert_scatter_offsets, int M)
{
    ...
    int token_stride = num_bids * NUM_WARPS;
    int cta_first = bid * NUM_WARPS;
    int n_it = 0;
    if (cta_first < M) {
        n_it = (M - cta_first + token_stride - 1) / token_stride;
    }
    #pragma unroll 1
    for (int it = 0; it < n_it; it++) {
        int token = cta_first + warp + it * token_stride;
        if (token < M) {
            ...
            int shift0 = 24;
            unsigned int lane_top = 0;
            #pragma unroll
            for (int item_top = 0; item_top < 28; item_top++) {
                unsigned int top_byte = keys[item_top] >> 24;
                if (top_byte > lane_top) {
                    lane_top = top_byte;
                }
            }
            unsigned int _warp_redux_u32_0;
            asm volatile("redux.sync.max.u32 %0, %1, 0xffffffff;" : "=r"(_warp_redux_u32_0) : "r"(lane_top));
            unsigned int warp_top = _warp_redux_u32_0;
            unsigned int lane_top_count = 0;
            #pragma unroll
            for (int item_cnt = 0; item_cnt < 28; item_cnt++) {
                if (keys[item_cnt] >> 24 == warp_top) {
                    lane_top_count += 1;
                }
            }
            unsigned int _warp_redux_u32_1;
            asm volatile("redux.sync.add.u32 %0, %1, 0xffffffff;" : "=r"(_warp_redux_u32_1) : "r"(lane_top_count));
            unsigned int warp_top_count = _warp_redux_u32_1;
            unsigned int r0_match = warp_top | (warp_top_count & 1023) << 18;
            if (warp_top_count < (unsigned int)TOP_K) {
                __syncwarp();
                #pragma unroll
                ...
                for (int item_hist = 0; item_hist < 28; item_hist++) {
                    if ((keys[item_hist] & examined_mask) == threshold) {
                        int digit = (int)(keys[item_hist] >> (unsigned int)shift0 & 255);
                ...
                asm volatile("redux.sync.max.u32 %0, %1, 0xffffffff;" : "=r"(_warp_redux_u32_2) : "r"(warp_match_record));
                r0_match = _warp_redux_u32_2;
            }
            int chosen_bin0 = (int)(r0_match & 255);
            remaining -= (int)(r0_match >> 8 & 1023);
            active_total = (int)(r0_match >> 18 & 1023);
            threshold |= (unsigned int)chosen_bin0 << (unsigned int)shift0;
            examined_mask |= (unsigned int)255 << (unsigned int)shift0;
            if (remaining == active_total) {
                take_all_equals = 1;
            }
            if (take_all_equals == 0) {
                #pragma unroll
                for (int radix_round = 1; radix_round < 4; radix_round++) {
                    int shift = 24 - radix_round * 8;
                    __syncwarp();
                    #pragma unroll
                    for (int clear_slot_1 = 0; clear_slot_1 < 8; clear_slot_1++) {
                        warp_hist[lane_hist_base + clear_slot_1] = 0;
                    }
                    __syncwarp();
                    #pragma unroll
                    for (int item_hist_1 = 0; item_hist_1 < 28; item_hist_1++) {
                        if ((keys[item_hist_1] & examined_mask) == threshold) {
                            int digit_1 = (int)(keys[item_hist_1] >> (unsigned int)shift & 255);
                            atomicAdd(&warp_hist[hist_base + digit_1 + (digit_1 >> 3)], 1);
                        }
                    }
                    __syncwarp();
                    int scan_counts_1[8];
                    int lane_count_1 = 0;
                    #pragma unroll
                    for (int scan_offset_2 = 0; scan_offset_2 < 8; scan_offset_2++) {
                        int scan_count_1 = warp_hist[lane_hist_base + scan_offset_2];
                        scan_counts_1[scan_offset_2] = scan_count_1;
                        lane_count_1 += scan_count_1;
                    }
                    int inclusive_1 = lane_count_1;
                    int _shfl_up_5 = __shfl_up_sync(0xFFFFFFFF, inclusive_1, 1, 32);
                    int scan_peer_4 = _shfl_up_5;
                    if (lane >= 1) {
                        inclusive_1 += scan_peer_4;
                    }
                    int _shfl_up_6 = __shfl_up_sync(0xFFFFFFFF, inclusive_1, 2, 32);
                    int scan_peer_0_1 = _shfl_up_6;
                    if (lane >= 2) {
                        inclusive_1 += scan_peer_0_1;
                    }
                    int _shfl_up_7 = __shfl_up_sync(0xFFFFFFFF, inclusive_1, 4, 32);
                    int scan_peer_1_1 = _shfl_up_7;
                    if (lane >= 4) {
                        inclusive_1 += scan_peer_1_1;
                    }
                    int _shfl_up_8 = __shfl_up_sync(0xFFFFFFFF, inclusive_1, 8, 32);
                    int scan_peer_2_1 = _shfl_up_8;
                    if (lane >= 8) {
                        inclusive_1 += scan_peer_2_1;
                    }
                    int _shfl_up_9 = __shfl_up_sync(0xFFFFFFFF, inclusive_1, 16, 32);
                    int scan_peer_3_1 = _shfl_up_9;
                    if (lane >= 16) {
                        inclusive_1 += scan_peer_3_1;
                    }
                    int bin_inclusive_1 = inclusive_1 - lane_count_1;
                    unsigned int warp_match_record_1 = 0;
                    #pragma unroll
                    for (int scan_offset_3 = 0; scan_offset_3 < 8; scan_offset_3++) {
                        int bin_count_1 = scan_counts_1[scan_offset_3];
                        bin_inclusive_1 += bin_count_1;
                        int bin_above_1 = active_total - bin_inclusive_1;
                        if (bin_above_1 < remaining && remaining <= bin_above_1 + bin_count_1) {
                            int chosen_scan_bin_1 = scan_bin_base + scan_offset_3;
                            warp_match_record_1 = (unsigned int)(chosen_scan_bin_1 & 255) | (unsigned int)(bin_above_1 & 1023) << 8 | (unsigned int)(bin_count_1 & 1023) << 18;
                        }
                    }
                    unsigned int _warp_redux_u32_3;
                    asm volatile("redux.sync.max.u32 %0, %1, 0xffffffff;" : "=r"(_warp_redux_u32_3) : "r"(warp_match_record_1));
                    unsigned int match_record = _warp_redux_u32_3;
    ...
    int phase2_scan_peer = _shfl_up_10;
    if (lane >= 1) {
        warp_inclusive += phase2_scan_peer;
    }
    int _shfl_up_11 = __shfl_up_sync(0xFFFFFFFF, warp_inclusive, 2, 32);
    int phase2_scan_peer_0 = _shfl_up_11;
    if (lane >= 2) {
        warp_inclusive += phase2_scan_peer_0;
    }
    int _shfl_up_12 = __shfl_up_sync(0xFFFFFFFF, warp_inclusive, 4, 32);
    int phase2_scan_peer_1 = _shfl_up_12;
    if (lane >= 4) {
        warp_inclusive += phase2_scan_peer_1;
    }
    int _shfl_up_13 = __shfl_up_sync(0xFFFFFFFF, warp_inclusive, 8, 32);
    int phase2_scan_peer_2 = _shfl_up_13;
    if (lane >= 8) {
        warp_inclusive += phase2_scan_peer_2;
    }
    int _shfl_up_14 = __shfl_up_sync(0xFFFFFFFF, warp_inclusive, 16, 32);
    int phase2_scan_peer_3 = _shfl_up_14;
    if (lane >= 16) {
    ...
    if (M > 1) {
        ...
        for (int sort_expert_a = bid; sort_expert_a < 896; sort_expert_a += num_bids * 2) {
            ...
            if (sort_count_a + sort_count_b > 0) {
                ...
                int _shfl_up_15 = __shfl_up_sync(0xFFFFFFFF, first_inclusive_a, 1, 32);
                int first_peer_a = _shfl_up_15;
                int _shfl_up_16 = __shfl_up_sync(0xFFFFFFFF, first_inclusive_b, 1, 32);
                int first_peer_b = _shfl_up_16;
                if (lane >= 1) {
                    first_inclusive_a += first_peer_a;
                    first_inclusive_b += first_peer_b;
                }
                int _shfl_up_17 = __shfl_up_sync(0xFFFFFFFF, first_inclusive_a, 2, 32);
                int first_peer_a_0 = _shfl_up_17;
                int _shfl_up_18 = __shfl_up_sync(0xFFFFFFFF, first_inclusive_b, 2, 32);
                int first_peer_b_1 = _shfl_up_18;
                if (lane >= 2) {
                    first_inclusive_a += first_peer_a_0;
                    first_inclusive_b += first_peer_b_1;
                }
                int _shfl_up_19 = __shfl_up_sync(0xFFFFFFFF, first_inclusive_a, 4, 32);
                int first_peer_a_2 = _shfl_up_19;
                int _shfl_up_20 = __shfl_up_sync(0xFFFFFFFF, first_inclusive_b, 4, 32);
                int first_peer_b_3 = _shfl_up_20;
                if (lane >= 4) {
                    first_inclusive_a += first_peer_a_2;
                    first_inclusive_b += first_peer_b_3;
                }
                int _shfl_up_21 = __shfl_up_sync(0xFFFFFFFF, first_inclusive_a, 8, 32);
                int first_peer_a_4 = _shfl_up_21;
                int _shfl_up_22 = __shfl_up_sync(0xFFFFFFFF, first_inclusive_b, 8, 32);
                int first_peer_b_5 = _shfl_up_22;
                if (lane >= 8) {
                    first_inclusive_a += first_peer_a_4;
                    first_inclusive_b += first_peer_b_5;
                }
                int _shfl_up_23 = __shfl_up_sync(0xFFFFFFFF, first_inclusive_a, 16, 32);
                int first_peer_a_6 = _shfl_up_23;
                int _shfl_up_24 = __shfl_up_sync(0xFFFFFFFF, first_inclusive_b, 16, 32);
                int first_peer_b_7 = _shfl_up_24;
                if (lane >= 16) {
                ...
                if (warp == 0) {
                    ...
                    int _shfl_up_25 = __shfl_up_sync(0xFFFFFFFF, last_inclusive_a, 1, 32);
                    int last_peer_a = _shfl_up_25;
                    int _shfl_up_26 = __shfl_up_sync(0xFFFFFFFF, last_inclusive_b, 1, 32);
                    int last_peer_b = _shfl_up_26;
                    if (lane >= 1) {
                        last_inclusive_a += last_peer_a;
                        last_inclusive_b += last_peer_b;
                    }
                    int _shfl_up_27 = __shfl_up_sync(0xFFFFFFFF, last_inclusive_a, 2, 32);
                    int last_peer_a_0 = _shfl_up_27;
                    int _shfl_up_28 = __shfl_up_sync(0xFFFFFFFF, last_inclusive_b, 2, 32);
                    int last_peer_b_1 = _shfl_up_28;
                    if (lane >= 2) {
                        last_inclusive_a += last_peer_a_0;
                        last_inclusive_b += last_peer_b_1;
                    }
                    int _shfl_up_29 = __shfl_up_sync(0xFFFFFFFF, last_inclusive_a, 4, 32);
                    int last_peer_a_2 = _shfl_up_29;
                    int _shfl_up_30 = __shfl_up_sync(0xFFFFFFFF, last_inclusive_b, 4, 32);
                    int last_peer_b_3 = _shfl_up_30;
                    if (lane >= 4) {
                        last_inclusive_a += last_peer_a_2;
                        last_inclusive_b += last_peer_b_3;
                    }
                    int _shfl_up_31 = __shfl_up_sync(0xFFFFFFFF, last_inclusive_a, 8, 32);
                    int last_peer_a_4 = _shfl_up_31;
                    int _shfl_up_32 = __shfl_up_sync(0xFFFFFFFF, last_inclusive_b, 8, 32);
                    int last_peer_b_5 = _shfl_up_32;
                    if (lane >= 8) {
                        last_inclusive_a += last_peer_a_4;
                        last_inclusive_b += last_peer_b_5;
                    }
                    int _shfl_up_33 = __shfl_up_sync(0xFFFFFFFF, last_inclusive_a, 16, 32);
                    int last_peer_a_6 = _shfl_up_33;
                    int _shfl_up_34 = __shfl_up_sync(0xFFFFFFFF, last_inclusive_b, 16, 32);
                    int last_peer_b_7 = _shfl_up_34;
                    if (lane >= 16) {
                ...
                if (warp == 0) {
                    ...
                    int _shfl_up_35 = __shfl_up_sync(0xFFFFFFFF, chunk_inclusive_a, 1, 32);
                    int chunk_peer_a = _shfl_up_35;
                    int _shfl_up_36 = __shfl_up_sync(0xFFFFFFFF, chunk_inclusive_b, 1, 32);
                    int chunk_peer_b = _shfl_up_36;
                    if (lane >= 1) {
                        chunk_inclusive_a += chunk_peer_a;
                        chunk_inclusive_b += chunk_peer_b;
                    }
                    int _shfl_up_37 = __shfl_up_sync(0xFFFFFFFF, chunk_inclusive_a, 2, 32);
                    int chunk_peer_a_0 = _shfl_up_37;
                    int _shfl_up_38 = __shfl_up_sync(0xFFFFFFFF, chunk_inclusive_b, 2, 32);
                    int chunk_peer_b_1 = _shfl_up_38;
                    if (lane >= 2) {
                        chunk_inclusive_a += chunk_peer_a_0;
                        chunk_inclusive_b += chunk_peer_b_1;
                    }
                    int _shfl_up_39 = __shfl_up_sync(0xFFFFFFFF, chunk_inclusive_a, 4, 32);
                    int chunk_peer_a_2 = _shfl_up_39;
                    int _shfl_up_40 = __shfl_up_sync(0xFFFFFFFF, chunk_inclusive_b, 4, 32);
                    int chunk_peer_b_3 = _shfl_up_40;
                    if (lane >= 4) {
                        chunk_inclusive_a += chunk_peer_a_2;
                        chunk_inclusive_b += chunk_peer_b_3;
                    }
                    int _shfl_up_41 = __shfl_up_sync(0xFFFFFFFF, chunk_inclusive_a, 8, 32);
                    int chunk_peer_a_4 = _shfl_up_41;
                    int _shfl_up_42 = __shfl_up_sync(0xFFFFFFFF, chunk_inclusive_b, 8, 32);
                    int chunk_peer_b_5 = _shfl_up_42;
                    if (lane >= 8) {
                        chunk_inclusive_a += chunk_peer_a_4;
                        chunk_inclusive_b += chunk_peer_b_5;
                    }
                    int _shfl_up_43 = __shfl_up_sync(0xFFFFFFFF, chunk_inclusive_a, 16, 32);
                    int chunk_peer_a_6 = _shfl_up_43;
                    int _shfl_up_44 = __shfl_up_sync(0xFFFFFFFF, chunk_inclusive_b, 16, 32);
                    int chunk_peer_b_7 = _shfl_up_44;
                    if (lane >= 16) {
