            ...
            for (unsigned int work_idx = blockIdx.x; work_idx < total_items; work_idx += gridDim.x) {
                ...
                }
                {
                    ...
                    for (int h_2 = 0; h_2 < 16; h_2++) {
                        float _shfl_xor_5 = __shfl_xor_sync(0xFFFFFFFF, o_epi[h_2], 1);
                        p_epi[h_2] = _shfl_xor_5;
                    }
                    int odd = lane % 2;
                    int lo = token - odd;
                    #pragma unroll
