...
def _kernels(rows, hidden, intermediate, experts, device):
...
class CudnnFrostBf16MoeRunner(MoERunner):
    ...
    def _check_support(self):
        ...
        if self.config.activation != SwiGLU():
            raise NotImplementedError(
                "cuDNN Frost BF16 MoE supports only default SwiGLU"
            )
    ...
    def _validate_pack(self, act, weights):
        ...
        if tuple(w1.shape) != (e, 2 * i, h) or tuple(w2.shape) != (e, h, i):
    ...
    def accepts(self, act, weights):
        ...
        try:
            ...
            return False
        first, second = _kernels(t * k, h, i, e, self.device)
