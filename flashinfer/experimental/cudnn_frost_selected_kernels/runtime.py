"""Runtime for cuDNN Frost-generated grouped GEMM1 + SwiGLU kernels.
...
class CudnnFrostGroupedGemm1SwiGLUKernel:
    artifact_id: str
    arch: str
    source_path: Path
    source_sha256: str
...
def _read_root(root: Path) -> list[CudnnFrostGroupedGemm1SwiGLUKernel]:
    path = root / _MANIFEST
    if not path.is_file():
        return []
    ...
    result: list[CudnnFrostGroupedGemm1SwiGLUKernel] = []
    seen: set[str] = set()
    for raw in kernels:
        ...
        result.append(
            CudnnFrostGroupedGemm1SwiGLUKernel(
...
def _discover(
    roots: tuple[Path, ...],
) -> tuple[CudnnFrostGroupedGemm1SwiGLUKernel, ...]:
...
def _launch(
    kernel: CudnnFrostGroupedGemm1SwiGLUKernel,
    grouped_tokens: torch.Tensor,
    gate_weights: torch.Tensor,
    up_weights: torch.Tensor,
    first_token_offset: torch.Tensor,
    ...
    operands = (token, gate, up)
    if kernel.swap_ab:
        operands = (gate, up, token)
