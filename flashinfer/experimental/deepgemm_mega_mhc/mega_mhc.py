...
def device_arch(device):
    ...
    if arch is None or arch not in catalogued:
        raise RuntimeError(
            f"Mega mHC has no exported programs for compute capability "
...
def _new_outputs(
    x, residual, post_mix, comb_res_mix, shifted_prev_mix, sf_layout, shared_sf_block_m
):
    import torch
    ...
    if sf_layout != "bf16":
        ...
        if sf_layout == "col":
            ...
            rows = (
                (tokens + shared_sf_block_m - 1)
                // shared_sf_block_m
                * ((shared_sf_block_m + 127) // 128 * 128)
            )
    ...
    def __init__(
        ...
        *,
        ...
        self.options = dict(
            ...
            num_sms=sms,
            shifted=shifted,
            store_fp8=sf_layout != "bf16",
            shared_sf_block_m=shared_sf_block_m if sf_layout == "extra" else 0,
        ...
        try:
            ...
            raise NotImplementedError(
                f"No exported mHC schedule for {self.options}"
            ) from error
        ...
        if sf_layout != "bf16":
            ...
            if sf_layout == "extra":
                ...
                if (
                    ...
                    or out["shared_sf_block_m"] != shared_sf_block_m
                ):
                    raise ValueError(
        ...
        if split_barriers is None:
            split_barriers = torch.zeros(
                barriers_shape, dtype=torch.uint64, device=x.device
            )
        ...
        self.bindings = dict(
            ...
            mix_bases=mix_bases,
            new_prev_mix=out.get("new_prev_mix", dummy_float),
            ...
            y_primary_sf=raw_storage(primary),
            y_shared_sf=raw_storage(shared),
            scratch=scratch,
            ...
            primary_sf_stride_word=primary.stride(1) if primary.ndim == 2 else 0,
