...
#define SMEM_SMEM_QA_STRIDE 65536
#define SMEM_SMEM_QB_OFF 35840
#define SMEM_SMEM_QB_STAGE_BYTES 32768
#define SMEM_SMEM_QB_STRIDE 65536
#define SMEM_SMEM_KV_OFF 134144
...
kernel_cake_kimi_k3_vision_tower_3aed6956b7b08660a264(const __grid_constant__ CUtensorMap Q, __nv_bfloat16* __restrict__ Q_raw, const __grid_constant__ CUtensorMap K, const __grid_constant__ CUtensorMap V, __nv_bfloat16* __restrict__ O, const __grid_constant__ CUtensorMap O_tma, int* __restrict__ seg_begin, int* __restrict__ seg_len, int* __restrict__ unit_table, unsigned long long* __restrict__ probe, unsigned int total_tiles, int num_heads, float softmax_scale_log2)
{
    ...
    __nv_bfloat16* smem_qb = reinterpret_cast<__nv_bfloat16*>(smem_raw + 35840);
    const int smem_qb_addr = smem + 35840;
    __nv_bfloat16* smem_kv = reinterpret_cast<__nv_bfloat16*>(smem_raw + 134144);
    const int smem_kv_addr = smem + 134144;
    ...
    // Mbarrier init (10 pipeline groups, 0 ordered-sequence groups, 26 barriers)
    ...
    if (warp >= 8 && warp <= 11) {
        ...
        { // correction_main
            ...
            for (unsigned int tile_idx_1 = cluster_id; tile_idx_1 < total_tiles; tile_idx_1 += num_clusters) {
                ...
                asm volatile("barrier.sync 1, 256;" ::: "memory");
                asm volatile("barrier.sync 2, 256;" ::: "memory");
                unsigned int slot0 = q_slot_c * 2;
                unsigned int slot1 = q_slot_c * 2 + 1;
                int tile_row0 = m_block_1 * 2 * BLOCK_M;
                int tile0_valid = doc_len_1 - tile_row0;
                ...
                if (tile0_valid >= BLOCK_M) {
                    ...
                    for (int col_2 = 0; col_2 < HEAD_DIM / 16; col_2++) {
                        ...
                        const float2 _scale2_2 = {final_scale, final_scale};
                        #pragma unroll
                        for (int _ls = 0; _ls < 8; _ls++)
                    ...
                    if (warp == 8) {
                        if (elect_sync()) {
                            tma_store_4d((&O_tma), 0, doc_begin_1 + tile_row0, head_1, 0, smem_o_addr + 2 * slot0 * 16384);
                        }
                    }
                    asm volatile("cp.async.bulk.commit_group;");
                } else {
                    float final_sum_0 = scales[warp % 4 * 32 + lane + 2 * BLOCK_M];
                    float final_scale_1;
                ...
                int tile1_valid = tile0_valid - BLOCK_M;
                float final_sum_0_1 = scales[warp % 4 * 32 + lane + BLOCK_M + 2 * BLOCK_M];
                float final_scale_1_1;
                if (final_sum_0_1 != 0.0f && final_sum_0_1 == final_sum_0_1) {
                    float _rcp_2 = approx_rcp(final_sum_0_1);
                    final_scale_1_1 = _rcp_2;
                } else {
                    final_scale_1_1 = 0.0f;
                }
                if (tile1_valid >= BLOCK_M) {
                    ...
                    for (int col_4 = 0; col_4 < HEAD_DIM / 16; col_4++) {
                        ...
                        const float2 _scale2_4 = {final_scale_1_1, final_scale_1_1};
                        #pragma unroll
                        for (int _ls = 0; _ls < 8; _ls++)
                        ...
                        int srow_1 = (unsigned int)(col_4 / 4 * 128) + slot1 * 256 + (unsigned int)(warp % 4 * 32 + lane);
                        asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"((smem_o_addr + (unsigned int)(srow_1 * 128 + col_4 % 4 * 32 ^ (srow_1 * 128 + col_4 % 4 * 32 >> 7 & 7) << 4))), "r"(_tmem_load_6_bf16[0]), "r"(_tmem_load_6_bf16[1]), "r"(_tmem_load_6_bf16[2]), "r"(_tmem_load_6_bf16[3]) : "memory");
                        asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"((smem_o_addr + (unsigned int)(srow_1 * 128 + (col_4 % 4 * 32 + 16) ^ (srow_1 * 128 + (col_4 % 4 * 32 + 16) >> 7 & 7) << 4))), "r"(_tmem_load_6_bf16[4]), "r"(_tmem_load_6_bf16[5]), "r"(_tmem_load_6_bf16[6]), "r"(_tmem_load_6_bf16[7]) : "memory");
                    }
                    asm volatile(
                        "mbarrier.arrive.release.cta.shared::cluster.b64 _, [%0];"
                        :: "r"((p_full_addr + 8) & 0xFEFFFFFF) : "memory");
                    ...
                    if (warp == 8) {
                        if (elect_sync()) {
                            tma_store_4d((&O_tma), 0, doc_begin_1 + tile_row0 + BLOCK_M, head_1, 0, smem_o_addr + 2 * slot1 * 16384);
                        }
                    }
                    asm volatile("cp.async.bulk.commit_group;");
                } else {
                    float final_sum_1 = scales[warp % 4 * 32 + lane + BLOCK_M + 2 * BLOCK_M];
                    float final_scale_2;
                    if (final_sum_1 != 0.0f && final_sum_1 == final_sum_1) {
                        float _rcp_3 = approx_rcp(final_sum_1);
                        final_scale_2 = _rcp_3;
                    } else {
                        final_scale_2 = 0.0f;
                    }
                    int local_row_1 = m_block_1 * 2 * BLOCK_M + BLOCK_M + (warp % 4 * 32 + lane);
                    int out_row_1 = (doc_begin_1 + local_row_1) * num_heads + head_1;
                    #pragma unroll
                    for (int col_5 = 0; col_5 < HEAD_DIM / 16; col_5++) {
                        ...
                        if (local_row_1 < doc_len_1) {
                            {
                                const float2 _prescale2_5 = {final_scale_2, final_scale_2};
                                #if __CUDA_ARCH__ >= 1000
                                #pragma unroll
                                for (int _ps = 0; _ps < 8; _ps++)
                                ...
                                #pragma unroll
                                for (int _ps = 0; _ps < 16; _ps++)
                                    _tmem_load_7[0 + _ps] *= final_scale_2;
                                #endif
                                __nv_bfloat162 _pk[8];
                                ...
                                *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(O + (out_row_1 * HEAD_DIM + col_5 * 16)))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                                *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(O + (out_row_1 * HEAD_DIM + col_5 * 16)))[8]) = *reinterpret_cast<uint4*>(&_pk[4]);
                            }
                        }
                    }
                    asm volatile(
        ...
        { // mma_main
            ...
            if (cta_rank == 0) {
                ...
                for (unsigned int tile_idx_2 = cluster_id; tile_idx_2 < total_tiles; tile_idx_2 += num_clusters) {
                    ...
                    asm volatile("tcgen05.fence::after_thread_sync;");
                    int _mma_a_lo_0 = (((smem_qa_addr) >> 4) & 0x3FFF) + (q_stage) * 4096;
                    ...
                    int _mma_a_lo_1 = (((smem_qb_addr) >> 4) & 0x3FFF) + (q_stage) * 4096;
                    ...
                    :: "r"(tmem_output_0), "r"(_mma_b_lo_3), "r"(tmem_probs_0), "r"(1));
                        ...
                        asm volatile("tcgen05.fence::after_thread_sync;");
                        int _mma_a_lo_4 = (((smem_qa_addr) >> 4) & 0x3FFF) + (q_stage) * 4096;
                    ...
                    :: "r"(tmem_output_1), "r"(_mma_b_lo_6), "r"(tmem_probs_1), "r"(1));
                        ...
                        int _mma_a_lo_7 = (((smem_qb_addr) >> 4) & 0x3FFF) + (q_stage) * 4096;
            ...
            for (unsigned int tile_idx_3 = cluster_id; tile_idx_3 < total_tiles; tile_idx_3 += num_clusters) {
                ...
                if (elect_sync()) {
                    asm volatile(
                        ...
                        :: "r"((q_full_addr + (q_load_stage) * 8) & 0xFEFFFFFF), "r"((uint32_t)(65536)) : "memory");
                    tma_4d_gmem2smem_cta2(smem_qa_addr + q_load_stage * 65536, (&Q), 0, q_row, head_3, 0, ((q_full_addr + (q_load_stage) * 8) & 0xFEFFFFFF));
                    tma_4d_gmem2smem_cta2(smem_qb_addr + q_load_stage * 65536, (&Q), 0, q_row + BLOCK_M, head_3, 0, ((q_full_addr + (q_load_stage) * 8) & 0xFEFFFFFF));
                }
                q_load_stage += 1;
