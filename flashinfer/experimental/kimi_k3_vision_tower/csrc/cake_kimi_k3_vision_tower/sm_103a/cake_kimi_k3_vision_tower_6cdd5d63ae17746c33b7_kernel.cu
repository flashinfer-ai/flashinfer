            ...
            if (elect_sync()) {
                ...
                for (unsigned int item = cluster_id_1; item < num_cluster_tiles; item += num_clusters_1) {
                    ...
                    for (int iter_k = 0; iter_k < 64; iter_k++) {
                        ...
                        tma_3d_gmem2smem_cta2(smem_b_addr + load_stage * 24576, (&B), 0, weight_row, k_step, ((tma_full_addr + (load_stage) * 8) & 0xFEFFFFFF));
                        asm volatile(
                ...
                for (int g_i = 0; g_i < 2; g_i++) {
                    ...
                    for (int q_1 = 0; q_1 < 4; q_1++) {
                        {
                            asm volatile("ld.global.v8.b32 {%0, %1, %2, %3, %4, %5, %6, %7}, [%8];"
                                : "=r"(wn_w[q_1 * 8 + 0]), "=r"(wn_w[q_1 * 8 + 1]), "=r"(wn_w[q_1 * 8 + 2]), "=r"(wn_w[q_1 * 8 + 3]), "=r"(wn_w[q_1 * 8 + 4]), "=r"(wn_w[q_1 * 8 + 5]), "=r"(wn_w[q_1 * 8 + 6]), "=r"(wn_w[q_1 * 8 + 7]) : "l"((const void*)((const char*)(WN + (wn_off + (unsigned long long)(gcol + q_1 * 16)) + 0) + 0)) : "memory");
                        }
                    }
                    float part_sq = 0.0f;
                    #pragma unroll
                    for (int c = 0; c < 4; c++) {
                        ...
                        for (int j_1 = 0; j_1 < 16; j_1++) {
                            ...
                            vals[j_1] = res[c * 16 + j_1] + _cvt_f32_0;
                        }
                        ...
                        for (int q_2 = 0; q_2 < 8; q_2++) {
                            ...
                            asm volatile("mul.rn.bf16x2 %0, %1, %2;" : "=r"(_bf16x2_mul_0) : "r"(vals_bf16[q_2]), "r"(wn_w[c * 8 + q_2]));
