...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_kimi_k3_vision_tower_68703b18d9ed8bd395ab'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_cake_kimi_k3_vision_tower_68703b18d9ed8bd395ab(__nv_bfloat16* __restrict__ y, __nv_bfloat16* __restrict__ weight, float* __restrict__ rowsumsq, int M, float eps);


namespace cake_host_shim_7732858ad7205804 {
...
void Run(TensorView arg_y, TensorView arg_weight, TensorView arg_rowsumsq, int64_t arg_M, double arg_eps, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  CheckCudaTensor(arg_rowsumsq, "rowsumsq");
  CheckDtype(arg_rowsumsq, "rowsumsq", 2, 32, 1);
  CheckContiguous(arg_rowsumsq, "rowsumsq");
  TVM_FFI_CHECK(arg_M >= -2147483648LL && arg_M <= 2147483647LL, ValueError)
      << "scalar 'M' value " << arg_M
      << " is outside i32 range [-2147483648, 2147483647]";
  CheckSameCudaDevice(arg_weight, arg_y, "weight", "y");
  CheckSameCudaDevice(arg_rowsumsq, arg_y, "rowsumsq", "y");
  ...
  float* p_rowsumsq = static_cast<float*>(arg_rowsumsq.data_ptr());
  int32_t v_M = (int32_t)arg_M;
  float v_eps = (float)arg_eps;
  void* kargs[] = {&p_y, &p_weight, &p_rowsumsq, &v_M, &v_eps};
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_kimi_k3_vision_tower_68703b18d9ed8bd395ab), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_kimi_k3_vision_tower_68703b18d9ed8bd395ab failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_7732858ad7205804

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_7732858ad7205804::Run);
