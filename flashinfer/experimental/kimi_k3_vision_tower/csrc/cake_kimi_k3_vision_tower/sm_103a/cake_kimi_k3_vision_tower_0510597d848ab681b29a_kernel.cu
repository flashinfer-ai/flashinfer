        ...
        { // load_main
            ...
            if (elect_sync()) {
                ...
                for (unsigned int item = cluster_id_1; item < num_cluster_tiles; item += num_clusters_1) {
                    ...
                    int weight_row = parity * 1024 + bid_n * 128;
                    ...
                    for (int iter_k = 0; iter_k < 10; iter_k++) {
                        ...
                        if (tile_live < M) {
                            if (parity == 0) {
                                tma_2d_gmem2smem(smem_a_addr + load_stage * 32768, (&A), off_k, pair_row, tma_full_addr + (load_stage) * 8);
                            } else {
                                tma_2d_gmem2smem(smem_a_addr + load_stage * 32768, (&A2), off_k + 584, pair_row, tma_full_addr + (load_stage) * 8);
                            }
