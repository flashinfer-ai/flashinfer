    ...
    if (warp >= 2 && warp <= 5) {
        { // epilogue_main
            ...
            for (unsigned int ct_1 = epi_cluster_id; ct_1 < num_cluster_tiles; ct_1 += epi_num_clusters) {
                ...
                #pragma unroll 1
                for (int grp_1 = grp_lo; grp_1 < grp_lo + 1; grp_1++) {
                    int gcol_1 = grp_1 * 64;
                    ...
                    for (int q = 0; q < 4; q++) {
                        ...
                        {
                            const uint4* _vptr_0 = reinterpret_cast<const uint4*>(R + (row_in + (unsigned long long)(gcol_1 + q * 16)) + 0);
                            uint4 _vld_0[2];
                            #pragma unroll
                    ...
                    for (int q_1 = 0; q_1 < 4; q_1++) {
                        ...
                        {
                            const uint4* _vptr_1 = reinterpret_cast<const uint4*>(WN + (wn_off + (unsigned long long)(gcol_1 + q_1 * 16)) + 0);
                            uint4 _vld_1[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
                    ...
                    for (int c = 0; c < 4; c++) {
                        ...
                        for (int j_3 = 0; j_3 < 16; j_3++) {
                            __nv_bfloat16 _cvt_bf16_0 = __float2bfloat16(_tmem_load_1[c * 16 + j_3]);
                            float _cvt_f32_0 = __bfloat162float(_cvt_bf16_0);
                            vals[j_3] = res[c * 16 + j_3] + _cvt_f32_0;
                        }
                        #pragma unroll
                        for (int j_4 = 0; j_4 < 16; j_4++) {
                            __nv_bfloat16 _cvt_bf16_1 = __float2bfloat16(vals[j_4]);
                            ...
                            {
                                {
                                    ...
                                    asm volatile(
                                        ...
                                        :: "l"((void*)(&((__nv_bfloat16*)(C + (row_out + (unsigned long long)(gcol_1 + c * 16))))[0])), "r"(_pk_u0), "r"(_pk_u1), "r"(_pk_u2), "r"(_pk_u3), "r"(_pk_u4), "r"(_pk_u5), "r"(_pk_u6), "r"(_pk_u7) : "memory");
                                }
                            }
                        }
                        float xwv[16];
                        ...
                        for (int j_5 = 0; j_5 < 16; j_5++) {
                            __nv_bfloat16 _cvt_bf16_2 = __float2bfloat16(vals[j_5]);
                            float _cvt_f32_2 = __bfloat162float(_cvt_bf16_2);
                            xwv[j_5] = _cvt_f32_2 * wn[c * 16 + j_5];
                        }
                        if (store_row < M) {
                            {
                                {
                                    ...
                                    asm volatile(
                                        "st.global.v8.b32 [%0], {%1, %2, %3, %4, %5, %6, %7, %8};"
                                        :: "l"((void*)(&((__nv_bfloat16*)(XW + (row_out + (unsigned long long)(gcol_1 + c * 16))))[0])), "r"(_pk_u0), "r"(_pk_u1), "r"(_pk_u2), "r"(_pk_u3), "r"(_pk_u4), "r"(_pk_u5), "r"(_pk_u6), "r"(_pk_u7) : "memory");
                                }
                            }
                        }
                    }
