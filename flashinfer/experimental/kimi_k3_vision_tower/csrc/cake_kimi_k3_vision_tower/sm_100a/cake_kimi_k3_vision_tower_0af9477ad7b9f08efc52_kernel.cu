        ...
        { // load_main
            ...
            if (elect_sync()) {
                ...
                for (unsigned int ct = cluster_id_1; ct < num_cluster_tiles; ct += num_clusters_1) {
                    ...
                    int block = bid_m / 2;
                    int parity = bid_m % 2;
                    int pair_row = block * 128;
                    int weight_row = parity * 1024 + bid_n * 64;
                    int tile_live = block * 256 + parity;
                    #pragma unroll 1
                    for (int iter_k = 0; iter_k < 10; iter_k++) {
                        mbarrier_wait(mma_done_addr + (load_stage) * 8, _phase_mma_done);
                        int k_step = k_begin + iter_k;
                        int off_k = k_step * 64;
                        if (tile_live < M) {
                            if (parity == 0) {
                                tma_2d_gmem2smem(smem_a_addr + load_stage * 24576, (&A), off_k, pair_row, tma_full_addr + (load_stage) * 8);
                            } else {
                                tma_2d_gmem2smem(smem_a_addr + load_stage * 24576, (&A2), off_k + 584, pair_row, tma_full_addr + (load_stage) * 8);
                            }
                            ...
                            mbarrier_arrive(tma_full_addr + (load_stage) * 8);
                        }
                        load_stage += 1;
    ...
    if (warp == 1) {
        { // mma_main
            ...
            for (unsigned int _ct = mma_cluster_id; _ct < num_cluster_tiles; _ct += mma_num_clusters) {
                ...
                #pragma unroll 1
                for (int iter_k_1 = 0; iter_k_1 < 10; iter_k_1++) {
    ...
    if (warp >= 2 && warp <= 5) {
        { // epilogue_main
            ...
            for (unsigned int ct_1 = epi_cluster_id; ct_1 < num_cluster_tiles; ct_1 += epi_num_clusters) {
                ...
                int global_row = bid_m_1 / 2 * 256 + 2 * local_row + bid_m_1 % 2;
