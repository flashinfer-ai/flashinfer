...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_kimi_k3_vision_tower_8a9d596bcb18eb70d1f0'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
void Run(TensorView arg_Q, TensorView arg_Q_raw, TensorView arg_K, TensorView arg_V, TensorView arg_O, TensorView arg_O_tma, TensorView arg_seg_begin, TensorView arg_seg_len, TensorView arg_unit_table, TensorView arg_probe, int64_t arg_total_tiles, int64_t arg_num_heads, double arg_softmax_scale_log2, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_kimi_k3_vision_tower_8a9d596bcb18eb70d1f0),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_kimi_k3_vision_tower_8a9d596bcb18eb70d1f0 failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_kimi_k3_vision_tower_8a9d596bcb18eb70d1f0), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_kimi_k3_vision_tower_8a9d596bcb18eb70d1f0 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_dcf9d7cade9e02a3

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_dcf9d7cade9e02a3::Run);
