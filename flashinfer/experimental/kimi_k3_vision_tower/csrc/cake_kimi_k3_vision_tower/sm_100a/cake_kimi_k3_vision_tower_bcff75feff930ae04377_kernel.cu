        ...
        { // load_main
            ...
            if (elect_sync()) {
                ...
                for (unsigned int item = cluster_id_1; item < num_cluster_tiles; item += num_clusters_1) {
                    int this_bid = item * 2 + (unsigned int)cta_rank;
    ...
    if (warp >= 2 && warp <= 9) {
        { // epilogue_main
            ...
            for (unsigned int item_2 = epi_cluster_id; item_2 < num_cluster_tiles; item_2 += epi_num_clusters) {
                int this_bid_1 = item_2 * 2 + (unsigned int)cta_rank;
                ...
                for (int g_i = 0; g_i < 2; g_i++) {
                    ...
                    for (int c = 0; c < 4; c++) {
                        ...
                        {
                            asm volatile("ld.global.v8.b32 {%0, %1, %2, %3, %4, %5, %6, %7}, [%8];"
                                : "=r"(wn_c[0 + 0]), "=r"(wn_c[0 + 1]), "=r"(wn_c[0 + 2]), "=r"(wn_c[0 + 3]), "=r"(wn_c[0 + 4]), "=r"(wn_c[0 + 5]), "=r"(wn_c[0 + 6]), "=r"(wn_c[0 + 7]) : "l"((const void*)((const char*)(WN + (wn_off + (unsigned long long)(gcol + c * 16)) + 0) + 0)) : "memory");
                        }
                        #pragma unroll
                        for (int q_1 = 0; q_1 < 8; q_1++) {
                            uint32_t _bf16x2_mul_0;
                            asm volatile("mul.rn.bf16x2 %0, %1, %2;" : "=r"(_bf16x2_mul_0) : "r"(vals_bf16[q_1]), "r"(wn_c[q_1]));
                            xw_w[q_1] = _bf16x2_mul_0;
                        }
                        if (store_row < M) {
                            {
