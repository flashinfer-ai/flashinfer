...
def metadata_size_bytes(queries, capacity, *, fmt, sparse_block_kv, paged, num_sms):
    ...
    splits = (
        queries * ceildiv(capacity, blocks_per_split)
        if paged
        else ceildiv(queries, 2) * ceildiv(2 * capacity, blocks_per_split)
    )
...
def logits_bindings(
    q, sf_q, kv, sf_kv, weights, metadata, output, *, fmt, paged, num_sms
):
    import torch
    ...
    return dict(
        Q=q.view(torch.uint8).reshape(-1, row_bytes),
        SF_Q=sf_q.view(torch.int32).reshape(-1, 32),
        Weights=weights.reshape(-1, 32),
        KV_TMA=kv_tma,
        SF_KV_TMA=sf_tma,
    ...
    for kind, key in record["arg_plan"]:
        if kind == "workspace":
            raise NotImplementedError(
                "Sparse route unexpectedly requires external descriptor storage"
            )
        if stage is None:
...
class SparseMetadataPlan:
    ...
    def __init__(
        ...
        *,
        ...
        self.bindings = metadata_bindings(
            sparse_indices,
            metadata,
            workspace,
            starts=starts,
...
class SparseMqaPlan:
    ...
    def __init__(self, q, sf_q, kv, sf_kv, weights, metadata_plan, *, output=None):
        ...
        if (
            ...
            or tuple(sf_q.shape) != (queries, 32)
        ):
        ...
        if output is None:
            output = torch.empty(
                (queries, capacity * config["sparse_block_kv"]),
                device=q.device,
                dtype=torch.bfloat16,
            )
        if output.dtype != torch.bfloat16 or tuple(output.shape) != (
            queries,
            capacity * config["sparse_block_kv"],
        ):
        ...
        bindings = {
            "metadata": metadata_plan.bindings,
