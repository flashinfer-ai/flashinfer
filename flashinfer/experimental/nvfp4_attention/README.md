...
Preparation packs E2M1 values with E4M3 block scales; the returned callable
writes the caller-owned BF16 output with no CUDA allocation. Prepare again
when input values or bindings change. CUDA Graph capture belongs to the caller.

The prepared object's `main_kwargs` exposes packed tensors and host launch
