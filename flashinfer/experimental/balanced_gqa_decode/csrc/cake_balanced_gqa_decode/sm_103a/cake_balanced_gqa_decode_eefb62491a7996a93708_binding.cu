...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_balanced_gqa_decode_eefb62491a7996a93708'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
void Run(TensorView arg_Q, TensorView arg_K, TensorView arg_V, TensorView arg_O_ptr, TensorView arg_page_table, TensorView arg_seq_lens_kv, TensorView arg_partial_o, TensorView arg_partial_stats, TensorView arg_tile_counters, TensorView arg_queue_counters, int64_t arg_max_pages_per_seq, double arg_softmax_scale_log2, int64_t arg_num_q_heads, int64_t arg_num_kv_heads, int64_t arg_batch_size, int64_t arg_q_len, int64_t arg_max_items, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_balanced_gqa_decode_eefb62491a7996a93708),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_balanced_gqa_decode_eefb62491a7996a93708 failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_balanced_gqa_decode_eefb62491a7996a93708), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_balanced_gqa_decode_eefb62491a7996a93708 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_f28f6359042dcff7

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_f28f6359042dcff7::Run);
