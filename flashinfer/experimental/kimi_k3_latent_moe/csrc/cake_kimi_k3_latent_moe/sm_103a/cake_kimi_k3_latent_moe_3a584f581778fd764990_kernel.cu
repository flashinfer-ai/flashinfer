                ...
                asm volatile("griddepcontrol.wait;" ::: "memory");
                {
                    ...
                    if (nrow < num_tokens) {
                        ...
                        for (int i_2 = 0; i_2 < 56; i_2++) {
                            {
                                ...
                                asm volatile(
                                    ...
                                    "}\n"
                                    : "=&f"(_bf16x2_add_f32_8[0]), "=&f"(_bf16x2_add_f32_8[1]) : "r"(xp[i_2]), "f"(0.0f), "f"(0.0f));
                                nacc[2 * i_2] = _bf16x2_add_f32_8[0];
                                nacc[2 * i_2 + 1] = _bf16x2_add_f32_8[1];
                            }
                        }
                        {
                            #pragma unroll 1
                            for (int p = 1; p < num_partials; p++) {
                                ...
                                for (int i_3 = 0; i_3 < 14; i_3++) {
                                    ...
                                    #pragma unroll
                                    for (int j = 0; j < 8; j++) {
                                        nacc[i_3 * 8 + j] = nacc[i_3 * 8 + j] + _vec_load_4[j];
                                    }
                                }
                            }
                        }
                        ...
                        for (int i_4 = 0; i_4 < 112; i_4++) {
                            __nv_bfloat16 _cvt_bf16_16 = __float2bfloat16(nacc[i_4]);
                            float _cvt_f32_16 = __bfloat162float(_cvt_bf16_16);
                            float sv = _cvt_f32_16;
                            sum_sq += sv * sv;
                        }
                        float _warp_reduce_4 = sum_sq;
                        #pragma unroll
                        for (int offset = 16; offset > 0; offset >>= 1)
                            _warp_reduce_4 += __shfl_xor_sync(0xFFFFFFFF, _warp_reduce_4, offset);
                        float total = _warp_reduce_4;
                        float _rsqrt_4 = rsqrtf(total / 3584.0f + eps);
                        float rstd = _rsqrt_4;
                        #pragma unroll
                        for (int i_5 = 0; i_5 < 14; i_5++) {
                            ...
                            for (int jj = 0; jj < 4; jj++) {
                                ...
                                asm volatile(
                                    ...
                                    "add.rn.f32.bf16 %1, hi, %4;\n\t"
                                    "}\n"
                                    : "=&f"(_bf16x2_add_f32_9[0]), "=&f"(_bf16x2_add_f32_9[1]) : "r"(nwp[i_5 * 4 + jj]), "f"(0.0f), "f"(0.0f));
                                #pragma unroll
                                for (int h = 0; h < 2; h++) {
                                    __nv_bfloat16 _cvt_bf16_17 = __float2bfloat16(nacc[i_5 * 8 + jj * 2 + h]);
                                    float _cvt_f32_17 = __bfloat162float(_cvt_bf16_17);
                                    float s2 = _cvt_f32_17;
                                    __nv_bfloat16 _cvt_bf16_18 = __float2bfloat16(s2 * rstd);
                                    float _cvt_f32_18 = __bfloat162float(_cvt_bf16_18);
                                    float normed = _cvt_f32_18;
                                    __nv_bfloat16 _cvt_bf16_19 = __float2bfloat16(_bf16x2_add_f32_9[h] * normed);
                                    float _cvt_f32_19 = __bfloat162float(_cvt_bf16_19);
                                    nvals[jj * 2 + h] = _cvt_f32_19;
                                }
                            }
                            {
                ...
                for (int m_1 = 0; m_1 < u_count_m; m_1++) {
                    ...
                    int init_flag = ((m_1 == 0) ? 1 : 0);
                    {
                        int init_sub = ((1) ? init_flag : 0);
                        {
                            ...
                            int _mma_b_lo_5 = (((smem_b_addr) >> 4) & 0x3FFF) + (stage_1) * 128;
                            asm volatile(
                    "{\n\t"
                    ".reg .pred p0, p1;\n\t"
