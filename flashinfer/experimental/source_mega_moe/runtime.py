...
class MegaMoEPlan:
        ...
        *,
        ...
        if workspace is None:
            workspace = torch.zeros(
                record["nbytes"], dtype=torch.uint8, device=x.device
            )
        if (
            workspace.dtype != torch.uint8
            or workspace.device != x.device
            or not workspace.is_contiguous()
            or workspace.numel() < record["nbytes"]
            or workspace.data_ptr() % 128
        ):
            raise ValueError(
                "Workspace must be aligned contiguous CUDA uint8 with the catalog extent"
            )
        raw = workspace.reshape(-1)
        views = {
            name: raw.narrow(0, f["offset"], f["nbytes"])
            .view(getattr(torch, f["dtype"]))
            .reshape(f["shape"])
            for name, f in record["fields"].items()
        }
        views["shared_l1_acts"] = views["x"]
        if out is None:
            out = torch.empty((t, h), dtype=torch.bfloat16, device=x.device)
        if (
            out.dtype != torch.bfloat16
            or tuple(out.shape) != (t, h)
        ...
        i = intermediate
        specs = {
            "B1": (torch.uint8, (e * 2 * i, h // pack)),
            "B2": (torch.uint8, (e * h, i // pack)),
            "SFB1": (torch.uint32, (e * h // 128, 2 * i)),
            "SFB2": (torch.uint32, (e * i // 128, h)),
            "SB1": (torch.uint8, (2 * i, h)),
            "SB2": (torch.uint8, (h, i)),
            "SSFB1": (torch.uint32, (h // 128, 2 * i)),
            "SSFB2": (torch.uint32, (i // 128, h)),
        }
        for name, (dtype, shape) in specs.items():
            ...
            if (
                ...
                raise ValueError(
                    f"{name} must be contiguous {dtype}{shape} on the input device"
                )
    ...
    def update_inputs(self, x, x_sf, topk_idx, topk_weights):
        ...
        c = self.config
        t, h = c.num_tokens, c.hidden
        specs = (
            (x, torch.float8_e4m3fn, (t, h)),
            (x_sf, torch.int32, (t, h // 128)),
            (topk_idx, torch.int64, (t, c.top_k)),
            (topk_weights, torch.float32, (t, c.top_k)),
        )
        for tensor, dtype, shape in specs:
            if (
                tensor.dtype != dtype
                or tuple(tensor.shape) != shape
                or tensor.device != self.workspace.device
                or not tensor.is_contiguous()
            ):
                raise ValueError(
                    f"Packed input must be contiguous {dtype}{shape} on the workspace device"
                )
