...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_nvfp4_mla_decode_a218732ec5ff9e037c52'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
void Run(TensorView arg_Q, TensorView arg_QS, TensorView arg_KV, TensorView arg_KVS, TensorView arg_OT, TensorView arg_PO, TensorView arg_O, TensorView arg_LSE, TensorView arg_partial_o, TensorView arg_partial_lse, TensorView arg_work_table, TensorView arg_unit_first, TensorView arg_page_table, TensorView arg_seq_lens, TensorView arg_q_indptr, TensorView arg_sinks, int64_t arg_rows_total, int64_t arg_num_heads, int64_t arg_q_len, int64_t arg_max_pages, int64_t arg_max_splits, double arg_scale_log2, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_nvfp4_mla_decode_a218732ec5ff9e037c52),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_nvfp4_mla_decode_a218732ec5ff9e037c52 failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_nvfp4_mla_decode_a218732ec5ff9e037c52), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_nvfp4_mla_decode_a218732ec5ff9e037c52 failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_1830e3ce282728bc

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_1830e3ce282728bc::Run);
