...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_nvfp4_mla_decode_ff76ef6d8e6453ca5e3c'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_cake_nvfp4_mla_decode_ff76ef6d8e6453ca5e3c(__nv_bfloat16* __restrict__ o, float* __restrict__ lse, __nv_bfloat16* __restrict__ partial_o, float* __restrict__ partial_lse, int* __restrict__ row_splits, int total_q, int num_heads, int max_splits);


namespace cake_host_shim_4d05d36bfcfbe4a9 {
...
void Run(TensorView arg_o, TensorView arg_lse, TensorView arg_partial_o, TensorView arg_partial_lse, TensorView arg_row_splits, int64_t arg_total_q, int64_t arg_num_heads, int64_t arg_max_splits, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  DLDevice dev = arg_o.device();
  ...
  CheckCudaTensor(arg_row_splits, "row_splits");
  CheckDtype(arg_row_splits, "row_splits", 0, 32, 1);
  CheckContiguous(arg_row_splits, "row_splits");
  ...
  TVM_FFI_CHECK(arg_max_splits >= -2147483648LL && arg_max_splits <= 2147483647LL, ValueError)
      ...
      << " is outside i32 range [-2147483648, 2147483647]";
  CheckSameCudaDevice(arg_lse, arg_o, "lse", "o");
  CheckSameCudaDevice(arg_partial_o, arg_o, "partial_o", "o");
  CheckSameCudaDevice(arg_partial_lse, arg_o, "partial_lse", "o");
  CheckSameCudaDevice(arg_row_splits, arg_o, "row_splits", "o");
  CheckCurrentCudaDevice(arg_o, "o");
  ...
  int* p_row_splits = static_cast<int*>(arg_row_splits.data_ptr());
  ...
  void* kargs[] = {&p_o, &p_lse, &p_partial_o, &p_partial_lse, &p_row_splits, &v_total_q, &v_num_heads, &v_max_splits};
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_nvfp4_mla_decode_ff76ef6d8e6453ca5e3c), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_nvfp4_mla_decode_ff76ef6d8e6453ca5e3c failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_4d05d36bfcfbe4a9

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_4d05d36bfcfbe4a9::Run);
