...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_nvfp4_mla_decode_5032fa112bb7a6ce884c'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
inline CUtensorMap EncodeTma_QS(const TensorView& t) {
  ...
  TVM_FFI_CHECK(32u <= global_dim[0] && 128u <= global_dim[1], ValueError)
      << "TMA box (32, 128) exceeds resolved global dims for 'QS'";
  int64_t carrier_stride_0 = s2;
  TVM_FFI_CHECK(carrier_stride_0 >= 0, ValueError)
...
inline CUtensorMap EncodeTma_KV(const TensorView& t) {
  ...
  TVM_FFI_CHECK(64u <= global_dim[0] && 64u <= global_dim[1], ValueError)
      << "TMA box (64, 64) exceeds resolved global dims for 'KV'";
  int64_t carrier_stride_0 = s2;
  TVM_FFI_CHECK(carrier_stride_0 >= 0, ValueError)
...
void Run(TensorView arg_Q, TensorView arg_QS, TensorView arg_KV, TensorView arg_KVS, TensorView arg_O, TensorView arg_LSE, TensorView arg_partial_o, TensorView arg_partial_lse, TensorView arg_work_table, TensorView arg_unit_first, TensorView arg_page_table, TensorView arg_seq_lens, TensorView arg_q_indptr, TensorView arg_sinks, int64_t arg_num_heads, int64_t arg_q_len, int64_t arg_max_pages, int64_t arg_max_splits, double arg_scale_log2, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  void* kargs[] = {&p_Q, &p_QS, &p_KV, &p_KVS, &p_O, &p_LSE, &p_partial_o, &p_partial_lse, &p_work_table, &p_unit_first, &p_page_table, &p_seq_lens, &p_q_indptr, &p_sinks, &v_num_heads, &v_q_len, &v_max_pages, &v_max_splits, &v_scale_log2};
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_nvfp4_mla_decode_5032fa112bb7a6ce884c),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_nvfp4_mla_decode_5032fa112bb7a6ce884c failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_nvfp4_mla_decode_5032fa112bb7a6ce884c), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_nvfp4_mla_decode_5032fa112bb7a6ce884c failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_85fc05b29d2b153e

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_85fc05b29d2b153e::Run);
