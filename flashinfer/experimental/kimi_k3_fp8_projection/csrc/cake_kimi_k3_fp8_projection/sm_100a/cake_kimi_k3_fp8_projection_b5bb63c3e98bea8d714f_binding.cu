...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_kimi_k3_fp8_projection_b5bb63c3e98bea8d714f'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
void Run(TensorView arg_A, TensorView arg_B, TensorView arg_SFA, TensorView arg_SFB, TensorView arg_out, int64_t arg_M, int64_t arg_m_tiles, int64_t arg_n_tiles, int64_t arg_n_valid, int64_t arg_ldo, int64_t arg_store_vec, int64_t arg_num_k_iters, int64_t arg_sf_k_tiles, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_kimi_k3_fp8_projection_b5bb63c3e98bea8d714f),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_kimi_k3_fp8_projection_b5bb63c3e98bea8d714f failed: "
  ...
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_kimi_k3_fp8_projection_b5bb63c3e98bea8d714f), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_kimi_k3_fp8_projection_b5bb63c3e98bea8d714f failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_85e7cc564d1d2c1f

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_85e7cc564d1d2c1f::Run);
