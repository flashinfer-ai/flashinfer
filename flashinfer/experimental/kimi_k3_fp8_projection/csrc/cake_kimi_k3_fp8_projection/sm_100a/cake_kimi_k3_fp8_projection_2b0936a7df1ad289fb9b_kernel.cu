...
kernel_cake_kimi_k3_fp8_projection_2b0936a7df1ad289fb9b(__nv_bfloat16* __restrict__ x, uint8_t* __restrict__ a_q, uint8_t* __restrict__ a_sf, int M, int K, int units_per_row, int sf_k_sets, int sf_rows)
{
    ...
    int unit = (bid * 4 + warp) * 2 + half;
    int row = unit / units_per_row;
    if (row < M) {
        int k_block0 = (unit - row * units_per_row) * 4;
        unsigned long long row_base = (unsigned long long)row * (unsigned long long)K;
        int m_tile = row / sf_rows;
        int row_in_tile = row - m_tile * sf_rows;
        unsigned long long sf_row_base = (unsigned long long)m_tile * (unsigned long long)sf_k_sets * (unsigned long long)SF_TILE_BYTES + (unsigned long long)(row_in_tile & 31) * 16 + (unsigned long long)(row_in_tile >> 5) * 4;
        float staged[32];
        #pragma unroll
        for (int u = 0; u < 4; u++) {
            int k_ld = (k_block0 + u) * 128 + half_lane * 8;
            float _vec_load_0[8];
            {
                const uint4* _vptr_0 = reinterpret_cast<const uint4*>(x + (row_base + (unsigned long long)k_ld) + 0);
                uint4 _vld_0[1];
                #pragma unroll
            ...
            #pragma unroll
            for (int j = 0; j < 8; j++) {
                staged[u * 8 + j] = _vec_load_0[j];
            }
        }
        #pragma unroll
        for (int u_1 = 0; u_1 < 4; u_1++) {
            int k_block = k_block0 + u_1;
            int k = k_block * 128 + half_lane * 8;
            float vals[8];
            float mags[8];
            #pragma unroll
            for (int j_1 = 0; j_1 < 8; j_1++) {
                vals[j_1] = staged[u_1 * 8 + j_1];
                mags[j_1] = staged[u_1 * 8 + j_1];
            }
            ...
            absmax = _max_1;
            float _shfl_xor_2 = __shfl_xor_sync(0xFFFFFFFF, absmax, 4);
            float o4 = _shfl_xor_2;
            float _max_2 = max_noftz(absmax, o4);
            absmax = _max_2;
            float _shfl_xor_3 = __shfl_xor_sync(0xFFFFFFFF, absmax, 8);
            float o8 = _shfl_xor_3;
            float _max_3 = max_noftz(absmax, o8);
            absmax = _max_3;
            float _max_4 = max_noftz(absmax, 0.0001f);
            absmax = _max_4;
            ...
            if (half_lane == 0) {
                unsigned long long sf_addr = sf_row_base + (unsigned long long)k_block * (unsigned long long)SF_TILE_BYTES;
                #pragma unroll
                for (int c = 0; c < 4; c++) {
                    *(reinterpret_cast<unsigned char*>(a_sf + (sf_addr + (unsigned long long)c)) + (0)) = (unsigned char)(scale_byte);
                }
            }
        }
    }
