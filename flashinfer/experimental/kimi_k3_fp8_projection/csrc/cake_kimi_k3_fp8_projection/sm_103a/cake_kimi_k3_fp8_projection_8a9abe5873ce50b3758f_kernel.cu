...
kernel_cake_kimi_k3_fp8_projection_8a9abe5873ce50b3758f(const __grid_constant__ CUtensorMap W, const __grid_constant__ CUtensorMap X, const __grid_constant__ CUtensorMap SFW, const __grid_constant__ CUtensorMap SFX, __nv_bfloat16* __restrict__ out, float* __restrict__ partials, unsigned int* __restrict__ counters, int M, int n_tiles, int n_valid, int ldo, int num_k_iters, int sf_k_tiles, int split, int tok_per_cta, int total_work, int store_vec, __nv_bfloat16* __restrict__ x, int K, const __grid_constant__ CUtensorMap XB)
{
    ...
    uint8_t* smem_sfw0 = reinterpret_cast<uint8_t*>(smem_raw + 66560);
    const int smem_sfw0_addr = smem + 66560;
    uint8_t* smem_sfw1 = reinterpret_cast<uint8_t*>(smem_raw + 67072);
    const int smem_sfw1_addr = smem + 67072;
    uint8_t* smem_sfx_all = reinterpret_cast<uint8_t*>(smem_raw + 67584);
    const int smem_sfx_all_addr = smem + 67584;
    uint8_t* smem_sfx0 = reinterpret_cast<uint8_t*>(smem_raw + 67584);
    const int smem_sfx0_addr = smem + 67584;
    uint8_t* smem_sfx1 = reinterpret_cast<uint8_t*>(smem_raw + 68096);
    const int smem_sfx1_addr = smem + 68096;
    uint8_t* smem_epi = reinterpret_cast<uint8_t*>(smem_raw + 203776);
    const int smem_epi_addr = smem + 203776;
            ...
            if (elect_sync()) {
                ...
                for (int it = 0; it < n_items; it++) {
                    ...
                    for (int i = 0; i < k_count; i++) {
                        ...
                        tma_3d_gmem2smem(smem_w_addr + load_stage * 67584, (&W), 0, 0, w_tile0 + k_group, tma_full_addr + (load_stage) * 8);
                ...
                for (int i_1 = 0; i_1 < k_count_1; i_1++) {
                    ...
                    if (elect_sync()) {
                        ...
                        int _mma_b_lo_1 = (((smem_x_addr + 16384) >> 4) & 0x3FFF) + (mma_stage) * 4224;
                        {
                            ...
                            tcgen05_mma_mxf8_bs((tmem_accum + (acc_stage * 128)), a_desc + 0, b_desc + 0,
                                0x8a00000U, tmem_tmem_sfw + 4, tmem_tmem_sfx + 4, 1);
                            tcgen05_mma_mxf8_bs((tmem_accum + (acc_stage * 128)), a_desc + 2, b_desc + 2,
                ...
                if (split == 1) {
                    ...
                    if (store_vec == 8) {
                        ...
                        for (int j = 0; j < 4; j++) {
                            ...
                            if (tok_g0 + row8 >= 0 && tok_g0 + row8 < 128 && tok8 < M && feat8 < n_valid) {
                                {
                        ...
                        for (int j_1 = 0; j_1 < 8; j_1++) {
                            ...
                            if (tok_g0 + row4 >= 0 && tok_g0 + row4 < 128 && tok4 < M && feat4 < n_valid) {
                                {
                                    uint2 _pk2;
                        ...
                        for (int j_2 = 0; j_2 < 16; j_2++) {
                            ...
                            if (tok_g0 + row2 >= 0 && tok_g0 + row2 < 128 && tok2 < M && feat2 < n_valid) {
                                {
                    ...
                    if (store_vec == 8) {
                        ...
                        for (int j_3 = 0; j_3 < 4; j_3++) {
                            ...
                            if (tok_g0 + 32 + row8_1 >= 0 && tok_g0 + 32 + row8_1 < 128 && tok8_1 < M && feat8_1 < n_valid) {
                                {
                                    __nv_bfloat162 _pk[4];
                        ...
                        for (int j_4 = 0; j_4 < 8; j_4++) {
                            ...
                            if (tok_g0 + 32 + row4_1 >= 0 && tok_g0 + 32 + row4_1 < 128 && tok4_1 < M && feat4_1 < n_valid) {
                                {
                                    uint2 _pk2;
                                    __nv_bfloat162* _pk = reinterpret_cast<__nv_bfloat162*>(&_pk2);
                        ...
                        for (int j_5 = 0; j_5 < 16; j_5++) {
                            ...
                            if (tok_g0 + 32 + row2_1 >= 0 && tok_g0 + 32 + row2_1 < 128 && tok2_1 < M && feat2_1 < n_valid) {
                                {
                    ...
                    for (int t_2 = 0; t_2 < 32; t_2++) {
                        {
                            uint32_t _addr_2 = static_cast<uint32_t>(smem_epi_addr + (unsigned int)(epi_group * 16384 + t_2 * 512 + lane_row * 4));
                            asm volatile("st.shared.f32 [%0], %1;" :: "r"(_addr_2), "f"(_tmem_load_0[64 + t_2]) : "memory");
                        }
                    }
                    asm volatile("barrier.sync %0, 128;" :: "r"(2 + epi_group) : "memory");
                    if (store_vec == 8) {
                        ...
                        for (int j_6 = 0; j_6 < 4; j_6++) {
                            int idx8_2 = j_6 * 128 + epi_tid;
                            int row8_2 = idx8_2 >> 4;
                            int f8_2 = (idx8_2 & 15) * 8;
                            unsigned int words8_2[8];
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                : "=r"(*reinterpret_cast<uint32_t*>(&words8_2[0])), "=r"(*reinterpret_cast<uint32_t*>(&words8_2[(0) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&words8_2[(0) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&words8_2[(0) + 3]))
                                : "r"(epi_base + (unsigned int)(row8_2 * 512 + f8_2 * 4)));
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                : "=r"(*reinterpret_cast<uint32_t*>(&words8_2[4])), "=r"(*reinterpret_cast<uint32_t*>(&words8_2[(4) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&words8_2[(4) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&words8_2[(4) + 3]))
                                : "r"(epi_base + (unsigned int)(row8_2 * 512 + f8_2 * 4 + 16)));
                            float vals8_2[8];
                            #pragma unroll
                            for (int q_6 = 0; q_6 < 8; q_6++) {
                                float v8_2 = 0.0f;
                                v8_2 = reinterpret_cast<float*>(&words8_2[q_6])[0];
                                vals8_2[q_6] = v8_2;
                            }
                            int tok8_2 = tok0 + (tok_g0 + 64) + row8_2;
                            int feat8_2 = n_tile_e * 128 + f8_2;
                            if (tok_g0 + 64 + row8_2 >= 0 && tok_g0 + 64 + row8_2 < 128 && tok8_2 < M && feat8_2 < n_valid) {
                                {
                                    __nv_bfloat162 _pk[4];
                                    _pk[0] = __floats2bfloat162_rn(vals8_2[0 + 0], vals8_2[0 + 1]);
                                    _pk[1] = __floats2bfloat162_rn(vals8_2[0 + 2], vals8_2[0 + 3]);
                                    _pk[2] = __floats2bfloat162_rn(vals8_2[0 + 4], vals8_2[0 + 5]);
                                    _pk[3] = __floats2bfloat162_rn(vals8_2[0 + 6], vals8_2[0 + 7]);
                                    *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(out + ((unsigned long long)tok8_2 * (unsigned long long)ldo + (unsigned long long)feat8_2)))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                                }
                            }
                        }
                    } else if (store_vec == 4) {
                        ...
                        for (int j_7 = 0; j_7 < 8; j_7++) {
                            int idx4_2 = j_7 * 128 + epi_tid;
                            int row4_2 = idx4_2 >> 5;
                            int f4_2 = (idx4_2 & 31) * 4;
                            unsigned int words4_2[4];
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                : "=r"(*reinterpret_cast<uint32_t*>(&words4_2[0])), "=r"(*reinterpret_cast<uint32_t*>(&words4_2[(0) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&words4_2[(0) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&words4_2[(0) + 3]))
                                : "r"(epi_base + (unsigned int)(row4_2 * 512 + f4_2 * 4)));
                            float vals4_2[4];
                            #pragma unroll
                            for (int q_7 = 0; q_7 < 4; q_7++) {
                                float v4_2 = 0.0f;
                                v4_2 = reinterpret_cast<float*>(&words4_2[q_7])[0];
                                vals4_2[q_7] = v4_2;
                            }
                            int tok4_2 = tok0 + (tok_g0 + 64) + row4_2;
                            int feat4_2 = n_tile_e * 128 + f4_2;
                            if (tok_g0 + 64 + row4_2 >= 0 && tok_g0 + 64 + row4_2 < 128 && tok4_2 < M && feat4_2 < n_valid) {
                                {
                                    uint2 _pk2;
                                    __nv_bfloat162* _pk = reinterpret_cast<__nv_bfloat162*>(&_pk2);
                                    _pk[0] = __floats2bfloat162_rn(vals4_2[0 + 0], vals4_2[0 + 1]);
                                    _pk[1] = __floats2bfloat162_rn(vals4_2[0 + 2], vals4_2[0 + 3]);
                                    *reinterpret_cast<uint2*>(&((__nv_bfloat16*)(out + ((unsigned long long)tok4_2 * (unsigned long long)ldo + (unsigned long long)feat4_2)))[0]) = _pk2;
                                }
                            }
                        }
                    } else {
                        ...
                        for (int j_8 = 0; j_8 < 16; j_8++) {
                            int idx2_2 = j_8 * 128 + epi_tid;
                            int row2_2 = idx2_2 >> 6;
                            int f2_2 = (idx2_2 & 63) * 2;
                            unsigned int words2_2[2];
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&words2_2[0])) : "r"(epi_base + (unsigned int)(row2_2 * 512 + f2_2 * 4)));
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&words2_2[1])) : "r"(epi_base + (unsigned int)(row2_2 * 512 + f2_2 * 4 + 4)));
                            float vals2_2[2];
                            #pragma unroll
                            for (int q_8 = 0; q_8 < 2; q_8++) {
                                float v2_2 = 0.0f;
                                v2_2 = reinterpret_cast<float*>(&words2_2[q_8])[0];
                                vals2_2[q_8] = v2_2;
                            }
                            int tok2_2 = tok0 + (tok_g0 + 64) + row2_2;
                            int feat2_2 = n_tile_e * 128 + f2_2;
                            if (tok_g0 + 64 + row2_2 >= 0 && tok_g0 + 64 + row2_2 < 128 && tok2_2 < M && feat2_2 < n_valid) {
                                {
                                    __nv_bfloat162 _pk = __floats2bfloat162_rn(vals2_2[0 + 0], vals2_2[0 + 1]);
                                    *reinterpret_cast<__nv_bfloat162*>(&((__nv_bfloat16*)(out + ((unsigned long long)tok2_2 * (unsigned long long)ldo + (unsigned long long)feat2_2)))[0]) = _pk;
                                }
                            }
                        }
                    }
                    ...
                    for (int t_3 = 0; t_3 < 32; t_3++) {
                        {
                            uint32_t _addr_3 = static_cast<uint32_t>(smem_epi_addr + (unsigned int)(epi_group * 16384 + t_3 * 512 + lane_row * 4));
                            asm volatile("st.shared.f32 [%0], %1;" :: "r"(_addr_3), "f"(_tmem_load_0[96 + t_3]) : "memory");
                        }
                    }
                    asm volatile("barrier.sync %0, 128;" :: "r"(2 + epi_group) : "memory");
                    if (store_vec == 8) {
                        ...
                        for (int j_9 = 0; j_9 < 4; j_9++) {
                            int idx8_3 = j_9 * 128 + epi_tid;
                            int row8_3 = idx8_3 >> 4;
                            int f8_3 = (idx8_3 & 15) * 8;
                            unsigned int words8_3[8];
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                : "=r"(*reinterpret_cast<uint32_t*>(&words8_3[0])), "=r"(*reinterpret_cast<uint32_t*>(&words8_3[(0) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&words8_3[(0) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&words8_3[(0) + 3]))
                                : "r"(epi_base + (unsigned int)(row8_3 * 512 + f8_3 * 4)));
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                : "=r"(*reinterpret_cast<uint32_t*>(&words8_3[4])), "=r"(*reinterpret_cast<uint32_t*>(&words8_3[(4) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&words8_3[(4) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&words8_3[(4) + 3]))
                                : "r"(epi_base + (unsigned int)(row8_3 * 512 + f8_3 * 4 + 16)));
                            float vals8_3[8];
                            #pragma unroll
                            for (int q_9 = 0; q_9 < 8; q_9++) {
                                float v8_3 = 0.0f;
                                v8_3 = reinterpret_cast<float*>(&words8_3[q_9])[0];
                                vals8_3[q_9] = v8_3;
                            }
                            int tok8_3 = tok0 + (tok_g0 + 96) + row8_3;
                            int feat8_3 = n_tile_e * 128 + f8_3;
                            if (tok_g0 + 96 + row8_3 >= 0 && tok_g0 + 96 + row8_3 < 128 && tok8_3 < M && feat8_3 < n_valid) {
                                {
                                    __nv_bfloat162 _pk[4];
                                    _pk[0] = __floats2bfloat162_rn(vals8_3[0 + 0], vals8_3[0 + 1]);
                                    _pk[1] = __floats2bfloat162_rn(vals8_3[0 + 2], vals8_3[0 + 3]);
                                    _pk[2] = __floats2bfloat162_rn(vals8_3[0 + 4], vals8_3[0 + 5]);
                                    _pk[3] = __floats2bfloat162_rn(vals8_3[0 + 6], vals8_3[0 + 7]);
                                    *reinterpret_cast<uint4*>(&((__nv_bfloat16*)(out + ((unsigned long long)tok8_3 * (unsigned long long)ldo + (unsigned long long)feat8_3)))[0]) = *reinterpret_cast<uint4*>(&_pk[0]);
                                }
                            }
                        }
                    } else if (store_vec == 4) {
                        ...
                        for (int j_10 = 0; j_10 < 8; j_10++) {
                            int idx4_3 = j_10 * 128 + epi_tid;
                            int row4_3 = idx4_3 >> 5;
                            int f4_3 = (idx4_3 & 31) * 4;
                            unsigned int words4_3[4];
                            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                                : "=r"(*reinterpret_cast<uint32_t*>(&words4_3[0])), "=r"(*reinterpret_cast<uint32_t*>(&words4_3[(0) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&words4_3[(0) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&words4_3[(0) + 3]))
                                : "r"(epi_base + (unsigned int)(row4_3 * 512 + f4_3 * 4)));
                            float vals4_3[4];
                            #pragma unroll
                            for (int q_10 = 0; q_10 < 4; q_10++) {
                                float v4_3 = 0.0f;
                                v4_3 = reinterpret_cast<float*>(&words4_3[q_10])[0];
                                vals4_3[q_10] = v4_3;
                            }
                            int tok4_3 = tok0 + (tok_g0 + 96) + row4_3;
                            int feat4_3 = n_tile_e * 128 + f4_3;
                            if (tok_g0 + 96 + row4_3 >= 0 && tok_g0 + 96 + row4_3 < 128 && tok4_3 < M && feat4_3 < n_valid) {
                                {
                                    uint2 _pk2;
                                    __nv_bfloat162* _pk = reinterpret_cast<__nv_bfloat162*>(&_pk2);
                                    _pk[0] = __floats2bfloat162_rn(vals4_3[0 + 0], vals4_3[0 + 1]);
                                    _pk[1] = __floats2bfloat162_rn(vals4_3[0 + 2], vals4_3[0 + 3]);
                                    *reinterpret_cast<uint2*>(&((__nv_bfloat16*)(out + ((unsigned long long)tok4_3 * (unsigned long long)ldo + (unsigned long long)feat4_3)))[0]) = _pk2;
                                }
                            }
                        }
                    } else {
                        ...
                        for (int j_11 = 0; j_11 < 16; j_11++) {
                            int idx2_3 = j_11 * 128 + epi_tid;
                            int row2_3 = idx2_3 >> 6;
                            int f2_3 = (idx2_3 & 63) * 2;
                            unsigned int words2_3[2];
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&words2_3[0])) : "r"(epi_base + (unsigned int)(row2_3 * 512 + f2_3 * 4)));
                            asm volatile("ld.shared.b32 %0, [%1];" : "=r"(*reinterpret_cast<uint32_t*>(&words2_3[1])) : "r"(epi_base + (unsigned int)(row2_3 * 512 + f2_3 * 4 + 4)));
                            float vals2_3[2];
                            #pragma unroll
                            for (int q_11 = 0; q_11 < 2; q_11++) {
                                float v2_3 = 0.0f;
                                v2_3 = reinterpret_cast<float*>(&words2_3[q_11])[0];
                                vals2_3[q_11] = v2_3;
                            }
                            int tok2_3 = tok0 + (tok_g0 + 96) + row2_3;
                            int feat2_3 = n_tile_e * 128 + f2_3;
                            if (tok_g0 + 96 + row2_3 >= 0 && tok_g0 + 96 + row2_3 < 128 && tok2_3 < M && feat2_3 < n_valid) {
                                {
                                    __nv_bfloat162 _pk = __floats2bfloat162_rn(vals2_3[0 + 0], vals2_3[0 + 1]);
                                    *reinterpret_cast<__nv_bfloat162*>(&((__nv_bfloat16*)(out + ((unsigned long long)tok2_3 * (unsigned long long)ldo + (unsigned long long)feat2_3)))[0]) = _pk;
                                }
                            }
                        }
                    }
                    ...
                    int _min_0 = ((128) < (t_first + tok_per_cta) ? (128) : (t_first + tok_per_cta));
                    int t_last = _min_0;
                        ...
                        if (t_first < tb + 8 && t_last > tb && tok_g0 <= tb && tb < tok_g0 + 128) {
                            ...
                            for (int s_0 = 0; s_0 < split; s_0 += 8) {
                                ...
                                for (int j_12 = 0; j_12 < 8; j_12++) {
                                    int _min_1 = ((s_0 + j_12) < (split - 1) ? (s_0 + j_12) : (split - 1));
                                    int s_j = _min_1;
                                    unsigned long long rank_off = rbase + (unsigned long long)s_j * 16384;
                                    #pragma unroll
                                    for (int u_1 = 0; u_1 < 8; u_1++) {
                                        parts[u_1 * 8 + j_12] = partials[rank_off + (unsigned long long)((tb + u_1) * 128)];
                                    }
                                }
                                #pragma unroll
                                for (int j_13 = 0; j_13 < 8; j_13++) {
                                    if (s_0 + j_13 < split) {
                                        ...
                                        for (int u_2 = 0; u_2 < 8; u_2++) {
                                            total[u_2] = total[u_2] + parts[u_2 * 8 + j_13];
                                        }
                                    }
                                }
                            }
