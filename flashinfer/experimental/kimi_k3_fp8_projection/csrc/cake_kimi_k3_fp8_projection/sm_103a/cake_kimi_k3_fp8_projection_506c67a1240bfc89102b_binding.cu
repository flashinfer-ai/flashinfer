...
// tvm-ffi direct-source launcher for Cake kernel 'kernel_cake_kimi_k3_fp8_projection_506c67a1240bfc89102b'.
#include <cuda.h>
#include <cuda_bf16.h>
#include <cuda_fp16.h>
...
extern "C" __global__ void kernel_cake_kimi_k3_fp8_projection_506c67a1240bfc89102b(const __grid_constant__ CUtensorMap A, const __grid_constant__ CUtensorMap B, const __grid_constant__ CUtensorMap SFA, const __grid_constant__ CUtensorMap SFB, __nv_bfloat16* __restrict__ out, int M, int m_tiles, int n_tiles, int n_valid, int ldo, int store_vec, int num_k_iters, int sf_k_tiles);


namespace cake_host_shim_e47e58389bc40702 {
...
inline CUtensorMap EncodeTma_B(const TensorView& t) {
  ...
  TVM_FFI_CHECK(128u <= global_dim[0] && 128u <= global_dim[1] && 2u <= global_dim[2], ValueError)
      << "TMA box (128, 128, 2) exceeds resolved global dims for 'B'";
...
inline CUtensorMap EncodeTma_SFA(const TensorView& t) {
  ...
  TVM_FFI_CHECK(128u <= global_dim[0] && 4u <= global_dim[1] && 2u <= global_dim[2], ValueError)
      << "TMA box (128, 4, 2) exceeds resolved global dims for 'SFA'";
  int64_t carrier_stride_0 = d1;
  TVM_FFI_CHECK(carrier_stride_0 >= 0, ValueError)
...
inline CUtensorMap EncodeTma_SFB(const TensorView& t) {
  ...
  TVM_FFI_CHECK(128u <= global_dim[0] && 8u <= global_dim[1] && 2u <= global_dim[2], ValueError)
      << "TMA box (128, 8, 2) exceeds resolved global dims for 'SFB'";
  int64_t carrier_stride_0 = d1;
  TVM_FFI_CHECK(carrier_stride_0 >= 0, ValueError)
...
void Run(TensorView arg_A, TensorView arg_B, TensorView arg_SFA, TensorView arg_SFB, TensorView arg_out, int64_t arg_M, int64_t arg_m_tiles, int64_t arg_n_tiles, int64_t arg_n_valid, int64_t arg_ldo, int64_t arg_store_vec, int64_t arg_num_k_iters, int64_t arg_sf_k_tiles, int64_t grid_x, int64_t grid_y, int64_t grid_z) {
  ...
  void* kargs[] = {&p_A, &p_B, &p_SFA, &p_SFB, &p_out, &v_M, &v_m_tiles, &v_n_tiles, &v_n_valid, &v_ldo, &v_store_vec, &v_num_k_iters, &v_sf_k_tiles};
    ...
    if (it == smem_status_by_device->end()) {
      ...
      if (smem_status == cudaSuccess) {
        smem_status = cudaFuncSetAttribute(
            reinterpret_cast<const void*>(kernel_cake_kimi_k3_fp8_projection_506c67a1240bfc89102b),
  ...
  TVM_FFI_CHECK(smem_status == cudaSuccess, RuntimeError)
      << "cudaFuncSetAttribute for kernel_cake_kimi_k3_fp8_projection_506c67a1240bfc89102b failed: "
  ...
  config.dynamicSmemBytes = 206976u;
  config.stream = stream;
  config.attrs = attrs;
  config.numAttrs = n;
  cudaError_t launch_status = cudaLaunchKernelExC(
      &config, reinterpret_cast<const void*>(kernel_cake_kimi_k3_fp8_projection_506c67a1240bfc89102b), kargs);
  TVM_FFI_CHECK(launch_status == cudaSuccess, RuntimeError)
      << "cudaLaunchKernelExC for kernel_cake_kimi_k3_fp8_projection_506c67a1240bfc89102b failed: "
      << cudaGetErrorString(launch_status);
}

}  // namespace cake_host_shim_e47e58389bc40702

TVM_FFI_DLL_EXPORT_TYPED_FUNC(run, cake_host_shim_e47e58389bc40702::Run);
