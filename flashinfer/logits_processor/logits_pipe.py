...
class LogitsPipe:
    ...
    def __init__(self, ops: List[Op], custom_fusion_rules: Optional[List[FusionRule]] = None,
        ...
        self.ops = list(ops)
        try:
            self.compiled_ops = compile_pipeline(self.ops, custom_fusion_rules, custom_validity_checks)
        except CompileError as e:
            raise ValueError(f"Pipeline compilation failed: {e}") from e
