...
class Op(ABC):    
    ...
    def __init__(self):
        if not self.IN or not self.OUT:
            raise ValueError(f"Operator {self.__class__.__name__} must define IN and OUT type signatures")
        
        if len(self.IN) != len(self.OUT):
            raise ValueError(f"Operator {self.__class__.__name__} must have matching IN/OUT signature lengths")
...
class ParameterizedOp(Op):
    ...
    def _get_param(self, name: str, kwargs: Dict[str, Any], required: bool = True) -> Any:
        if name in kwargs:
            return kwargs.pop(name)
        elif name in self.default_params:
            return self.default_params[name]
        elif required:
            raise ValueError(f"Required parameter '{name}' not provided for {self.__class__.__name__}")
        else:
            return None 