...
def gen_all_modules(
    ...
    has_cake_msa_sm100a = sm_capabilities.get("cake_msa_sm100a", False)
    has_cake_msa_sm100f = sm_capabilities.get("cake_msa_sm100f", False)
    ...
    # CUDA 12.8 predates the SM100-family target, so it retains one exact
    # B200 module per variant. CUDA 12.9+ registers one family module per
    # variant for both CC 10.0 and CC 10.3.
    cake_msa_targets: tuple[tuple[CakeMSATarget, bool], ...] = (
        ("sm100a", has_cake_msa_sm100a),
        ("sm100f", has_cake_msa_sm100f),
    )
    for cake_msa_target, enabled in cake_msa_targets:
        if enabled:
            jit_specs.extend(
                gen_cake_msa_module(variant, cake_msa_target)
                for variant in CAKE_MSA_VARIANTS
            )
...
def detect_sm_capabilities():
    ...
    return {
        ...
        "cake_msa_sm100f": (
            bool(cake_msa_family_arches & compilation_context.TARGET_CUDA_ARCHS)
