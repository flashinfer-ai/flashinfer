...
def gen_attention(
    ...
    # head_dim > 256 FA2 modules are SM90+-only; skip them entirely when no
    # SM90+ architecture is being targeted.
    from .jit.core import current_compilation_context

    has_sm90_or_newer = any(
        major >= 9 for major, _ in current_compilation_context.TARGET_CUDA_ARCHS
    )
    ...
    for (
        ...
        if (head_dim_qk > 256 or head_dim_vo > 256) and not has_sm90_or_newer:
            continue
        yield from gen_fa2(
            dtype_qo=dtype_qo,
            dtype_kv=dtype_kv,
