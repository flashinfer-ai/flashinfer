                ...
                double rope_theta)
{
    ...
    DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(q.scalar_type(), c_type, [&] {
        hipError_t status = BatchQKApplyRotary(
            ...
            static_cast<int32_t *>(offsets.data_ptr()), batch_size,
            num_qo_heads, num_kv_heads, rotary_dim, head_dim, q_stride_n,
            q_stride_h, k_stride_n, k_stride_h, q_rope_stride_n,
            q_rope_stride_h, k_rope_stride_n, k_rope_stride_h, interleave,
            rope_scale, rope_theta, stream);
        TORCH_CHECK(status == hipSuccess,
                    "BatchQKApplyRotary failed with error code " +
                        ...
                        double rope_theta)
{
    ...
    DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(q.scalar_type(), c_type, [&] {
        hipError_t status = BatchQKApplyRotaryPosIds(
            ...
            static_cast<c_type *>(q_rope.data_ptr()),
            static_cast<c_type *>(k_rope.data_ptr()),
            static_cast<int32_t *>(pos_ids.data_ptr()), nnz, num_qo_heads,
            num_kv_heads, rotary_dim, head_dim, q_stride_n, q_stride_h,
            k_stride_n, k_stride_h, q_rope_stride_n, q_rope_stride_h,
            k_rope_stride_n, k_rope_stride_h, interleave, rope_scale,
            rope_theta, stream);
        TORCH_CHECK(status == hipSuccess,
                    "BatchQKApplyRotaryPosIds failed with error code " +
                        std::string(hipGetErrorString(status)));
        return true;
    });
}
                        ...
                        double old_context_length)
{
    ...
    DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(q.scalar_type(), c_type, [&] {
        hipError_t status = BatchQKApplyLlama31Rotary(
            ...
            q_stride_h, k_stride_n, k_stride_h, q_rope_stride_n,
            q_rope_stride_h, k_rope_stride_n, k_rope_stride_h, interleave,
            rope_scale, rope_theta, low_freq_factor, high_freq_factor,
            old_context_length, stream);
        TORCH_CHECK(status == hipSuccess,
                    "BatchQKApplyLlama31Rotary failed with error code " +
                        std::string(hipGetErrorString(status)));
        return true;
    });
}
                                ...
                                double old_context_length)
{
    ...
    DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(q.scalar_type(), c_type, [&] {
        hipError_t status = BatchQKApplyLlama31RotaryPosIds(
            ...
            k_stride_n, k_stride_h, q_rope_stride_n, q_rope_stride_h,
            k_rope_stride_n, k_rope_stride_h, interleave, rope_scale,
            rope_theta, low_freq_factor, high_freq_factor, old_context_length,
            stream);
        TORCH_CHECK(status == hipSuccess,
