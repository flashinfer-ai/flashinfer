 ...
 */
#include <flashinfer/hip/page.hip.h>
                           ...
                           int64_t layout)
{
    ...
    bool success =
        DISPATCH_PYTORCH_DTYPE_TO_CTYPE(kv_scalar_dtype, c_type, [&] {
            paged_kv_t<c_type, int32_t> paged_kv(
                ...
                kv_cache_strides, static_cast<int32_t *>(kv_indices.data_ptr()),
    ...
    int64_t block_size)
{
    ...
    hipError_t status = BlockSparseIndicesToVectorSparseOffset(
        ...
        static_cast<int32_t *>(kv_len_arr.data_ptr()), stride_block, stride_n,
        batch_size, block_size, stream);

    TORCH_CHECK(status == hipSuccess,
                "BlockSparseIndicesToVectorSparseOffset failed with error: ",
                hipGetErrorString(status));
}
    ...
    bool success =
        DISPATCH_PYTORCH_DTYPE_TO_CTYPE(kv_scalar_dtype, c_type, [&] {
            paged_kv_mla_t<c_type, int32_t> paged_mla_kv(
                ...
                static_cast<c_type *>(ckv_cache.data_ptr()), ckv_strides,
                static_cast<c_type *>(kpe_cache.data_ptr()), kpe_strides,
                static_cast<int32_t *>(kv_indices.data_ptr()),
                static_cast<int32_t *>(kv_indptr.data_ptr()),
                static_cast<int32_t *>(kv_last_page_len.data_ptr()));
            hipError_t status = AppendPagedKVMlaCache(
            ...
            TORCH_CHECK(status == hipSuccess,
                        "AppendPagedKVMlaCache failed with error: ",
                        hipGetErrorString(status));
            return true;
        });

    TORCH_CHECK(success, "AppendPagedKVMlaCache failed to dispatch with dtype ",
                kv_scalar_dtype);
}
