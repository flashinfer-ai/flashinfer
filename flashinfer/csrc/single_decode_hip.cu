                                 ...
                                 int64_t window_left ADDITIONAL_FUNC_PARAMS)
{
    ...
    TORCH_CHECK(head_dim_qk == head_dim_vo,
                "CUDA cores template only supports equal head dim for QK and "
                "VO, please use tensor "
    ...
    DISPATCH_context(
        ...
        AttentionVariant, Params, [&] {
            ...
            params.lse = maybe_lse ? static_cast<float *>(maybe_lse->data_ptr())
                                   : nullptr;
            ...
            params.kv_stride_n = (kv_layout == QKVLayout::kNHD)
                                     ? num_kv_heads * head_dim_vo
                                     : head_dim_vo;
            params.kv_stride_h = (kv_layout == QKVLayout::kNHD)
                                     ? head_dim_vo
                                     : kv_len * head_dim_vo;
            ...
            hipError_t status = flashinfer::SingleDecodeWithKVCacheDispatched<
