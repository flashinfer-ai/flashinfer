 ...
 */
#include <flashinfer/hip/attention/cascade.hip.h>
                 ...
                 at::Tensor s_merged)
{
    ...
    bool success =
        DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(v_a.scalar_type(), c_type, [&] {
            hipError_t status =
                MergeState(static_cast<c_type *>(v_a.data_ptr()),
                           ...
                           static_cast<float *>(s_merged.data_ptr()), seq_len,
                          ...
                          std::optional<at::Tensor> mask)
{
    ...
    auto stream = at::hip::getCurrentHIPStream();
    bool success =
        DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(v.scalar_type(), c_type, [&] {
            hipError_t status = MergeStateInPlace(
                ...
                static_cast<float *>(s_other.data_ptr()), seq_len, num_heads,
                head_dim, mask_ptr, stream);
            TORCH_CHECK(status == hipSuccess,
                        "MergeStateInPlace kernel launch failed: ",
    ...
    TORCH_CHECK(
        success,
        "MergeStateInPlace kernel launch failed: unsupported data type");
}
    ...
    bool success =
        DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(v.scalar_type(), c_type, [&] {
            hipError_t status = MergeStates(
                ...
                static_cast<float *>(s.data_ptr()),
                static_cast<c_type *>(v_merged.data_ptr()),
                static_cast<float *>(s_merged.data_ptr()), num_index_sets,
                seq_len, num_heads, head_dim, stream);
            TORCH_CHECK(status == hipSuccess,
                        "MergeStates kernel launch failed: ",
                        hipGetErrorString(status));
            return true;
        });

    TORCH_CHECK(success,
                "MergeStates kernel launch failed: unsupported data type");
}
