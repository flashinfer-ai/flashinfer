...
class AutoTuner:
    ...
    def __init__(
        ...
        # Running counter of choose_one() calls per op while in tuning mode,
        # used to emit "shape #N" progress lines so the log shows forward progress.
        self._tuning_call_count: dict[str, int] = {}
    ...
    def choose_one(
        ...
        **kwargs,
        ...
        with self._lock:
            ...
            # Track every choose_one() call in tuning mode (including full cache hits).
            call_n = self._tuning_call_count.get(custom_op, 0) + 1
            self._tuning_call_count[custom_op] = call_n
            logger.debug(
                f"[AutoTuner]: choose_one {custom_op} (call #{call_n}): "
                f"{len(profiles)} tactics, input_shapes={input_shapes}"
            )
            # _profiling_started tracks whether we emitted the INFO log for this call.
            ...
            for _step, p in enumerate(profiles):
                try:
                    ...
                    if not is_cache_hit:
                        ...
                        if pbar is None:
                            if not _profiling_started:
                                logger.info(
                                    f"[AutoTuner]: Tuning {custom_op} (shape #{call_n}): "
                                    f"{len(profiles)} tactics, input_shapes={input_shapes}"
                                )
