...
class AutoTuner:
    ...
    def _find_nearest_profile_cached(
        ...
        """Bucket-match ``shapes`` against the specs/constraints.

        Keyed only on the dynamic-tensor specs and constraints — never the
        ``tensor_initializers`` closures — so a per-call config carrying fresh
        initializer closures does not grow the cache. The specs themselves must
        still carry stable ``gen``/``map`` callables (see ``make_bucket_mapper``)
        for equivalent configs to collapse to one entry.
        """
