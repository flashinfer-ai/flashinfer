...
def trtllm_ragged_attention_deepseek(
    ...
    skip_all_rows_active_check : bool
        ...
        mirrors. Only the ``trtllm-gen`` backend performs device-side
        detection, so on ``cute-dsl`` the flag only enforces that exclusion.
    ...
    if backend == "cute-dsl":
        ...
        elif not skip_all_rows_active_check:
