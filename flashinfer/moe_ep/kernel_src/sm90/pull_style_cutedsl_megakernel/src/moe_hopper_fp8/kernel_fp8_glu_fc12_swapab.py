...
class Sm90SwapABSwigluFp8Fc12Kernel(_Sm90Fp8Fc12KernelBase):
    ...
    def _kernel_body(
        ...
        ext_fc2_spin_threshold = (
            (
                fc1_intermediate_gateup
                + fc1_completion_tile_m * self.cluster_shape_mn[0]
                - 1
            )
            // (fc1_completion_tile_m * self.cluster_shape_mn[0])
        ) * self.cluster_shape_mn[0]
        if cutlass.const_expr(
            self.phase_mode == "fc12"
            and (
                self.fc1_early_done_publish or self.fc1_store_offload
            )
            and self.epilogue_warpgroup_count == 2
        ):
            # Early publication and offload publish once per WG half.
        ...
        # slot, so its optional non-blocking FC2 peek must fail closed.
        ...
        if cutlass.const_expr(self.enable_token_comm):
            # Keep the role toggle in a compile-time branch.  K2 has no
            # physical dispatch warps or dispatch SMEM, so its lowered IR
            # must not reference either object.
