...
host preprocessing is handled by the MXFP4 backend's ``weights`` module,
which reuses ``flashinfer.fused_moe.prepare`` Humming preprocessing. The runtime
kernel consumes an already interleaved payload and folded exponent offsets.
