...
MXFP4_OPTIMIZATION_VERSION = "fused_local_v1"
...
def resolve_mxfp4_optimizations(
    *,
    ...
    fused = (
        ...
        and split_role == "fused"
        and execution_phase == "fused"
    )
    ...
    # Paired consumer stores depend on the physical layout, not H7168 itself.
    # Keep complete channel clusters so no unvalidated channel tail is paired.
    peer32_layout = (
        fused
        and not pingpong
        and tile == (256, 64, 256)
    ...
    return Mxfp4Optimizations(
        peer32=local_optimizations and peer32_layout,
        offset_bulk=(local_optimizations and fused and not pingpong and tile[2] == 256),
        skip_zero_counts=(skip_zero_counts and fused and not dedup_dispatch),
