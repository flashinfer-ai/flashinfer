...
def resolve_hopper_mxfp4_split_tactic(
    knobs: Optional[Any],
    *,
    world_size: int,
    hidden: int,
    ...
    if isinstance(knobs, dict):
        # Validate the complete data schema before inspecting partition fields.
        # The public config deliberately stays kernel-drop independent, so this
        # resolver is now the first full-validation boundary for dict tactics.
        k1_sms = knobs.get("k1_sm_count")
        k2_sms = knobs.get("k2_sm_count")
        structural_sms = 132
        if all(
            not isinstance(value, bool) and isinstance(value, int) and value > 0
            for value in (k1_sms, k2_sms)
        ):
            structural_sms = int(k1_sms) + int(k2_sms)
        tactic = validate_hopper_mxfp4_tactic(
            knobs,
            execution_mode="split",
            total_sms=structural_sms,
        )
        ...
        requested_sms = tactic["k1_sm_count"] + tactic["k2_sm_count"]
        total_sms = 132
...
_resolve_mxfp4_split_tactic = resolve_hopper_mxfp4_split_tactic
...
class MegaMoEHopperMxfp4SplitSession:
    ...
    def _prepare(self, *, defer_failure_cleanup: bool = False) -> None:
        self._ensure_usable()
    ...
    def prepare_compile_only(self, inputs: MegaMoEHopperMxfp4Inputs) -> None:
        """Compile and allocate without launching Mega/Green workload kernels."""

        self.bind_inputs(inputs)
        if self._prepared is None:
            self._prepare(defer_failure_cleanup=True)
