...
class Sm100MegaMoEKernel(Sm100SwapABSwigluFp4Fc12Kernel):
    ...
    def aot_compile(
        self, out_path: Optional[str] = None, max_active_clusters: Optional[int] = None
    ):
        ...
        fake = dict(
            ...
            activation_sf=fake_tensor(
                scale_dtype, (tokens, activation_sf_cols), (1, 0), {0}, 16
            ),
            ...
            fc2_weight=fake_tensor(
                data_dtype, (experts, intermediate, hidden), (2, 0, 1), {0, 2}, 16
            ),
            fc2_weight_sf=fake_tensor(
                scale_dtype, (experts, fc2_sf_cols), (1, 0), {0}, 16
            ),
            ...
            output_activation=fake_tensor(
                self.fc2_output_dtype, (tokens, hidden), (1, 0), {0}, 16
            ),
            # Opaque byte workspaces: a bare base pointer (no shape), like to_cute_ptr.
            local_workspace=make_ptr(
                cutlass.Uint8, 0, AddressSpace.gmem, assumed_align=16
            ),
            ...
            peer_rank_ptr_mapper_host=SymBufferHost(
                ...
                rank_idx=Int32(
                    0
                ),  # placeholder; rank_idx is a runtime i32 (real PE id supplied at launch)
        ...
        compiled.export_to_c(
            out_path,
            function_name=_aot_symbol_prefix,
            ...
            fc2_output_workspace_u8 = self._make_typed_view(
                ...
                (
                    (
                        pool_token_capacity
                        * self.hidden
                        * int(self.combine_format.act_dtype.width)
                    )
                    // 8,
                ),
                None,
