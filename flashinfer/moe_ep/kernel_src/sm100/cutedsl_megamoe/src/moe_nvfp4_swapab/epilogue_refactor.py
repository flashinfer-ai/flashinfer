...
class _TmemTranspose16x32Core:
    ...
    def load_subtile_raw_acc(
        ...
        """Load the entire 32-lane x 64-col raw acc region of one epi
        ...
        to the next mma right after the first subtile's 4 loads (instead
        ...
        atom used by the per-subtile entry load.  Each output is in the
        ...
        # alignment that the load atom requires.  Using ``cutlass.Int32``
    ...
    def __init__(
        ...
        # drives _src_regs / output / every load/store atom below, so guard the
    ...
    def r1_load(self) -> None:
        """load tmem src region -> ``_src_regs``.  No-op in skip-R1.Load mode."""
...
class TmemTranspose16x32(_TmemTranspose16x32Core):
    ...
    (i.e. straight after the 16-dp x 32-col source load, or as fed in via
...
class SwapABFc1Epilogue(_ImmutableAfterInit):
    ...
    def __call__(
        ...
        if cutlass.const_expr(unroll_tile_cnt > 0):
            ...
            # preload_subtile_first: subtile_idx_first's raw PRE-transpose acc, loaded by
...
class SwapABFc2Epilogue(_ImmutableAfterInit):
        ...
        # for i in cutlass.range_constexpr(remain_subtile_cnt):
            ...
            if subtile_idx * cutlass.Int32(self._EpilogueTokenTileSize) < valid_tokens:
                self.run_subtile(
                    ...
                    fc2_output_router=fc2_output_router,
                    alpha_val=alpha_val,
                    release_after_load=release_after_load,
                    acc_pipeline=acc_pipeline,
                    acc_consumer_state=acc_consumer_state,
                )
                release_after_load = False
    ...
    def run_subtile(
        ...
        alpha_val: Optional[cutlass.Float32],
        release_after_load: Union[cutlass.Boolean, bool],
        acc_pipeline,
        acc_consumer_state,
    ):
...
def fc2_ublk_tmem_acc_load(*, tmem_subtile_tensor: cute.Tensor, epi, **_):
    ...
    # warp's hidden block before issuing load.
...
def fc2_redg_post_f2fp_reorder(
    ...
    **_,
    ...
    core_matrix_reorder_st_atom = cute.make_copy_atom(
        tcgen05.St32x32bOp(tcgen05.Repetition.x16),
        cutlass.Float32,
    )
    core_matrix_reorder_ld_atom = cute.make_copy_atom(
    ...
    for i in cutlass.range_constexpr(cute.size(natural_divided_by_token32_16dp, 1)):
        current_st_src = cute.recast_tensor(natural_divided_by_token32_16dp[None, i], cutlass.Float32)
        cute.copy(core_matrix_reorder_st_atom, wrap_into_copy_standard_layout(current_st_src), wrap_into_copy_standard_layout(tmem_subtile_divided_by_token_group_divided_by_16dp[None, None, None, i]))
        cute.copy(core_matrix_reorder_ld_atom, wrap_into_copy_standard_layout(tmem_subtile_divided_by_token_group_divided_by_16dp[None, None, 0, i]), wrap_into_copy_standard_layout(out_as_i32[(None, 0), i]))
        cute.copy(core_matrix_reorder_ld_atom, wrap_into_copy_standard_layout(tmem_subtile_divided_by_token_group_divided_by_16dp[None, None, 1, i]), wrap_into_copy_standard_layout(out_as_i32[(None, 1), i]))
