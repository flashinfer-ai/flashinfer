"""Lazy-compile mixed NVFP4-weight/BF16-activation MegaMoE API."""
...
MixedKind = Literal["bf16_nvfp4"]
TransformedWeights = Tuple[torch.Tensor, torch.Tensor]
PerExpertEpilogue = torch.Tensor | int | float
...
class MegaMoEBf16Nvfp4Config:
    ...
    def __post_init__(self) -> None:
        ...
        _VALID_IMPLS = Sm100SwapABNvfp4Bf16Fc12Kernel._SupportedImplementationConfigs
        if (
            self.mma_tiler_mnk,
            ...
            raise ValueError("unsupported mixed NVFP4/BF16 implementation tuple.")
    ...
    def weight_dtype(self) -> torch.dtype:
        ...
        if dtype is None:
            raise RuntimeError(
                "torch.float4_e2m1fn_x2 is required for BF16 x NVFP4 MegaMoE."
            )
        return dtype
...
class MegaMoEBf16Nvfp4Frontend:
    ...
    def apply_knobs(self, knobs: dict) -> None:
        ...
        if not is_valid_bf16_nvfp4_for_config(self.config, knobs):
            raise ValueError(
                f"unsupported mixed MegaMoE knobs {knobs}: "
                f"{describe_invalid_knobs(self.config, knobs, is_valid_bf16_nvfp4_for_config)}."
            )
    ...
    def _validate(self, inputs: MegaMoEBf16Nvfp4Inputs, num_tokens: int) -> None:
        ...
        for name, tensor in (
            ...
            if (
                ...
                raise ValueError(
                    f"{name} must be a CUDA E4M3FN tensor with one row per local expert."
                )
        for name, tensor in (
            ...
            if (
                ...
                or tuple(tensor.shape) != (e,)
                or not tensor.is_contiguous()
            ):
                raise ValueError(
                    f"{name} must be a contiguous CUDA float32 tensor with shape ({e},)."
                )
    ...
    def _ensure_compiled(self, inputs: MegaMoEBf16Nvfp4Inputs) -> _CompiledMega:
        ...
        from moe_nvfp4_bf16_glu.epilogue_nvfp4_bf16 import EpilogueTokenTile
...
def get_symm_buffer_for_bf16_nvfp4_mega_moe(
    ...
    *,
    ...
    if not is_valid_bf16_nvfp4_for_config(config, knobs):
        raise ValueError(
            f"unsupported mixed MegaMoE knobs {knobs}: "
            f"{describe_invalid_knobs(config, knobs, is_valid_bf16_nvfp4_for_config)}."
        )
    ...
    from .nvfp4 import _resolve_per_expert_epilogue

    num_local_experts = num_total_experts // world_size
    fc1_alpha_tensor = _resolve_per_expert_epilogue(
        "fc1_alpha", fc1_alpha, num_local_experts
    )
    fc2_alpha_tensor = _resolve_per_expert_epilogue(
        "fc2_alpha", fc2_alpha, num_local_experts
    )
    return MegaMoEBf16Nvfp4SymmBuffer(
        ...
        fc1_alpha_tensor,
        fc2_alpha_tensor,
        MegaMoEBf16Nvfp4Frontend(config),
...
def create_dummy_inputs(
    ...
    *,
    ...
    """Allocate symm buffer, NVFP4 weights, and stage BF16 activations + routing."""
    if num_tokens < 0 or num_tokens > num_max_tokens:
        raise ValueError(
            f"num_tokens must be in [0, {num_max_tokens}], got {num_tokens}."
        )

    # The mixed kernel's transformed weights are the pure-NVFP4 layout (same
    # shapes, dtype and swizzled flat SF), so the weight side is shared.
    from .nvfp4 import _create_dummy_weights
