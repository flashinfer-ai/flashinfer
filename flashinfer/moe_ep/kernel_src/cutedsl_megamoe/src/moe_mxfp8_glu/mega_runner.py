...
Topk weighting: the MXFP8 fused epilogue applies no topk weighting (form A
only), so the kernel writes unweighted fc2 outputs to
``combine_output[t, k, :]`` on the source rank and the host comparison reduces
the topk axis identically on both sides.
...
class MegaMoEMxfp8Tester(MegaMoETester):
    ...
    def generate_inputs(self) -> None:
        ...
        # Form B (REDG): (T, 1, H); kernel reduces topk axis.
    ...
    def validate(self) -> None:
        ...
        * **Form B (REDG)**: kernel writes topk-reduced into ``(T, 1, H)``;
          compare directly against the reference's topk-summed view.
        """
        if self.misc.skip_ref_check:
            return
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    tester = MegaMoEMxfp8Tester(problem, impl, misc, rank=rank, kind=args.kind)
