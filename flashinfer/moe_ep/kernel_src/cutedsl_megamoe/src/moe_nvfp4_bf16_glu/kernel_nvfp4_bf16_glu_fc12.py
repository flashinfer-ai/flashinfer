...
"""Lean fused FC1+FC2 kernel for NVFP4-weight/BF16-activation MoE.
...
tokens are GEMM B), but the tensor core instruction is dense BF16:
    ...
    NVFP4 weight + E4M3/K16 --CUDA-core transform--> BF16 TMEM/SMEM A
...
class Sm100SwapABNvfp4Bf16Fc12Kernel:
    """Lean swap-AB NVFP4-weight/BF16 FC12 kernel for SM100.
    ...
    def __init__(
        ...
        if static_expert_shape is not None:
            ...
            if hidden < self.tma_k_tile or intermediate < self.tma_k_tile:
                raise ValueError(
                    "NVFP4/BF16 reductions must be at least one raw TMA K tile "
                    f"({self.tma_k_tile}); got hidden={hidden}, "
                    f"intermediate/2={intermediate}."
                )
    ...
    def name(self) -> str:
        ...
        return (
            "sm100_swapab_nvfp4_bf16_glu_fc12"
            f"_m{self.mma_tiler[0]}n{self.mma_tiler[1]}"
    ...
    def _smem_misc_budget_bytes(self) -> int:
        """Conservative SMEM reservation outside staged tensor buffers."""
    ...
    def _setup_attributes(self) -> None:
        ...
        staged_smem_bytes = {
            "packed_a": cute.size_in_bytes(
                self.a_dtype, self.a_smem_layout_staged.outer
            ),
            "weight_scale": cute.size_in_bytes(
                self.scale_dtype, self.scale_smem_layout_staged.outer
            ),
            "bf16_b": cute.size_in_bytes(
                cutlass.BFloat16, self.b_smem_layout_staged.outer
            ),
            "transformed_a": self.num_transformed_a_smem_bytes,
        }
        required_smem_bytes = (
            sum(staged_smem_bytes.values())
            + self.epilogue.epi_smem_bytes
            + self._smem_misc_budget_bytes()
        )
        self.smem_budget = {
            **staged_smem_bytes,
            "epilogue": self.epilogue.epi_smem_bytes,
            "misc": self._smem_misc_budget_bytes(),
            "required": required_smem_bytes,
            "capacity": self.smem_capacity // self.occupancy,
        }
        if required_smem_bytes > self.smem_budget["capacity"]:
            raise ValueError(
                "mixed FC12 SMEM budget exceeds device capacity: "
                f"{self.smem_budget}."
            )
    ...
    def _validate_inputs(
        ...
        if cutlass.const_expr(
            fc1_weight.element_type is not cutlass.Float4E2M1FN
        ):
            raise TypeError("fc1_weight must be Float4E2M1FN.")
        if cutlass.const_expr(fc2_weight.element_type is not fc1_weight.element_type):
            raise TypeError("FC1 and FC2 weights must use the same NVFP4 dtype.")
        if cutlass.const_expr(
            fc1_weight_sf.element_type is not cutlass.Float8E4M3FN
            or fc2_weight_sf.element_type is not cutlass.Float8E4M3FN
        ):
            raise TypeError("both weight scale tensors must be Float8E4M3FN.")
        for name, tensor in (
    ...
    def __call__(
        ...
        * ``fc1_weight``: ``[experts, hidden, 2I]`` packed NVFP4, hidden stride one.
        * ``fc1_weight_sf``: atom-swizzled E4M3/K16 storage.
        * ``fc1_output``: ``[physical_tokens, I]`` BF16 workspace.
        * ``fc2_weight``: ``[experts, I, hidden]`` packed NVFP4, I stride one.
        * ``fc2_weight_sf``: atom-swizzled E4M3/K16 storage.
        ...
        * Mega scheduler: omit ``offs``; ``token_comm_args`` must expose
        ...
        for name, alpha in (("fc1_alpha", fc1_alpha), ("fc2_alpha", fc2_alpha)):
            if cutlass.const_expr(alpha is None):
                raise ValueError(f"{name} is required for NVFP4 weights.")
            if cutlass.const_expr(
                cute.rank(alpha) != 1
                or alpha.element_type is not cutlass.Float32
            ):
                raise TypeError(f"{name} must be a rank-1 Float32 tensor.")
            if cutlass.const_expr(isinstance(alpha.shape[0], int)):
                if cutlass.const_expr(alpha.shape[0] != experts):
        ...
        if cutlass.const_expr(
            ...
            raise ValueError("NVFP4 weight must be K-major.")
        if cutlass.const_expr(
            ...
            for scale_half_idx in cutlass.range_constexpr(scale_reuse):
                ...
                for fragment_idx in cutlass.range_constexpr(
                    ...
                    if cutlass.const_expr(
                        ...
                        scale_0 = tSrScale_load[
                            (None, (0, scale_fragment_idx))
                        ].load().to(cutlass.BFloat16)
                        scale_1 = tSrScale_load[
                            (None, (1, scale_fragment_idx))
                        ].load().to(cutlass.BFloat16)
                        scale_2 = tSrScale_load[
                            (None, (2, scale_fragment_idx))
                        ].load().to(cutlass.BFloat16)
                        scale_3 = tSrScale_load[
                            (None, (3, scale_fragment_idx))
                        ].load().to(cutlass.BFloat16)
                        scale_count = cute.size(scale_0.shape)
                        assert value_count == 4 * scale_count
                        scale_01 = mlir_vector.shuffle(
                            scale_0,
                            scale_1,
                            tuple(range(2 * scale_count)),
                        )
                        scale_23 = mlir_vector.shuffle(
                            scale_2,
                            scale_3,
                            tuple(range(2 * scale_count)),
                        )
    ...
    def _issue_tma_a_phase(
        ...
        """Issue one FC phase's packed-weight and scale TMA pipeline."""

        real_a, desc_a = ext.get_gmem_tensor("a", tma_tensor_a, work)
        real_scale, desc_scale = ext.get_gmem_tensor(
            "sfa", tma_tensor_scale, work
        )
        gA = cute.local_tile(
            real_a,
        ...
        gScale = cute.local_tile(
            real_scale,
            (self.mma_tiler[0], self.scale_tile_shape[1]),
            (None, None, None),
        )
        ...
        tSsScale, tSgScale = mixed_input_utils.scale_tma_partition(
            tCsScale,
            tCgScale,
            tma_atom_scale,
            block_in_cluster_coord_vmnk,
        ...
        mma_tile_m = work.tile_m_idx // cute.size(tiled_mma.thr_id.shape)
        tAgA_slice = tAgA[(None, mma_tile_m, None, 0)]
        tSgScale_slice = tSgScale[(None, mma_tile_m, None, 0)]
        rest = cute.filter_zeros(tSgScale_slice[(0, None)].layout)
        tSgScale_filtered = cute.make_tensor(
            tSgScale_slice.iterator,
            cute.make_layout(
                (tSgScale_slice.layout[0].shape, rest.shape),
                stride=(tSgScale_slice.layout[0].stride, rest.stride),
            ),
        )

        a_state.reset_count()
        scale_state.reset_count()
        peek_a = a_load_pipeline.producer_try_acquire(a_state)
        peek_scale = scale_load_pipeline.producer_try_acquire(scale_state)
        if iket_active:
            iket.range_pop()
        scale_reuse = (
            2 if cutlass.const_expr(self.mma_tiler[2] == 64) else 1
        ...
        scale_tile_cnt = k_tile_cnt // scale_reuse
        for _scale_k in cutlass.range(0, scale_tile_cnt, 1, unroll=1):
            scale_load_pipeline.producer_acquire(scale_state, peek_scale)
            cute.copy(
                tma_atom_scale,
                tSgScale_filtered[(None, scale_state.count)],
                tSsScale[(None, scale_state.index)],
                tma_bar_ptr=scale_load_pipeline.producer_get_barrier(
                    scale_state
                ),
            ...
            if scale_state.count < scale_tile_cnt:
                peek_scale = scale_load_pipeline.producer_try_acquire(
                    scale_state
                )
            if cutlass.const_expr(self.tma_k_reuse == 2):
                ...
                cute.copy(
                    ...
                    tma_bar_ptr=a_load_pipeline.producer_get_barrier(a_state),
                    tma_desc_ptr=desc_a,
                    mcast_mask=a_mcast_mask,
                )
                ...
                if a_state.count < scale_tile_cnt:
                    peek_a = a_load_pipeline.producer_try_acquire(a_state)
            for _scale_half in cutlass.range_constexpr(scale_reuse):
                if iket_active:
                    iket.range_push(
                        "mixed_role_coverage",
                        cutlass.Int32(102)
                    ...
                    iket.range_push(
                        "mixed_tma_a_producer_acquire", phase_payload
                    )
                if cutlass.const_expr(self.tma_k_reuse == 1):
                    a_load_pipeline.producer_acquire(a_state, peek_a)
                if iket_active:
                    iket.range_pop()
                    iket.range_push("mixed_tma_a_stage_work", phase_payload)
                if cutlass.const_expr(self.tma_k_reuse == 1):
                    cute.copy(
                        tma_atom_a,
                        tAgA_slice[(None, a_state.count)],
                        tAsA[(None, a_state.index)],
                        tma_bar_ptr=a_load_pipeline.producer_get_barrier(
                            a_state
                        ),
                        tma_desc_ptr=desc_a,
                        mcast_mask=a_mcast_mask,
                ...
                if iket_active:
                    iket.range_pop()
                    iket.range_pop()
        return a_state, scale_state

    @cute.jit
    def _issue_tma_b_phase(
        self,
        work,
        k_tile_cnt,
        tma_atom_b,
        ...
        """Issue one FC phase's BF16 activation/handoff TMA pipeline."""

        real_b, desc_b = ext.get_gmem_tensor("b", tma_tensor_b, work)
        gB = cute.local_tile(
            real_b,
            cute.slice_(self.tma_tiler, (0, None, None)),
            (None, None, None),
        )
        ...
        tBgB_slice = tBgB[(None, work.tile_n_idx, None, 0)]
        b_state.reset_count()
        peek_b = b_load_pipeline.producer_try_acquire(b_state)
        if iket_active:
            iket.range_pop()
        b_tile_cnt = k_tile_cnt // self.tma_k_reuse
        for _k in cutlass.range(0, b_tile_cnt, 1, unroll=1):
            if iket_active:
                iket.range_push(
                    "mixed_role_coverage",
                    cutlass.Int32(202) + phase_payload * cutlass.Int32(10),
                )
                iket.range_push(
                    "mixed_tma_b_producer_acquire", phase_payload
                )
            b_load_pipeline.producer_acquire(b_state, peek_b)
            if iket_active:
                iket.range_pop()
                iket.range_push("mixed_tma_b_stage_work", phase_payload)
            cute.copy(
                tma_atom_b,
                tBgB_slice[(None, b_state.count)],
                tBsB[(None, b_state.index)],
                tma_bar_ptr=b_load_pipeline.producer_get_barrier(b_state),
                tma_desc_ptr=desc_b,
    ...
    def kernel(
        ...
        # Weight/data and its E4M3 plane are issued by one warp.  With the
        # fixed K128 tile there is one staged K16 SF atom per K tile, so the
        ...
        if warp_idx == self.tma_a_warp_id:
            ...
            while work.is_valid_tile:
                ...
                if is_fc1:
                    a_state, scale_state = self._issue_tma_a_phase(
                        work,
                        k_tile_cnt_fc1,
                        tma_atom_w1,
                        tma_tensor_w1,
        ...
        if warp_idx == self.tma_b_warp_id:
            ...
            while work.is_valid_tile:
                ...
                if is_fc1:
                    ...
                    b_state = self._issue_tma_b_phase(
                        work,
                        k_tile_cnt_fc1,
                        tma_atom_activation,
                        tma_tensor_activation,
                    ...
                    if not work.peek_ready:
                        ...
                        spin_wait(
                            counter_ptr,
                            lambda value: value >= ext_fc2_spin_threshold,
        ...
        if (warp_idx >= self.epilogue_warp_id[0]) and (
            warp_idx
            < self.epilogue_warp_id[0] + len(self.epilogue_warp_id)
        ):
...
__all__ = ["Sm100SwapABNvfp4Bf16Fc12Kernel"]
