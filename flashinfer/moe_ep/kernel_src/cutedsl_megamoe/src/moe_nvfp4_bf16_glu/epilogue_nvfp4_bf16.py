...
"""Swap-AB epilogue for NVFP4-weight / BF16-activation fused FC1+FC2.
...
The mixed BF16-activation implementation uses a 16-column gate/up interleave::

    [gate(16), up(16), gate(16), up(16), ...]

That makes each epilogue warp own one complete gate/up pair in a 32-row TMEM
slice.  The quantized-activation NVFP4 path instead uses a historical
32-column interleave.  Adopting that layout here would make a pair span two
epilogue warps and require a different register/warp exchange.
...
class _MixedSwapABFc1Epilogue(_ImmutableAfterInit):
    ...
    def __call__(
        ...
        if cutlass.const_expr(self.optional_epi_args.fc1_alpha is not None):
            alpha = self.optional_epi_args.fc1_alpha[work_tile_info.expert_idx]
        else:
            alpha = None
    ...
    def run_subtile(
        ...
        *,
        ...
        alpha: Optional[cutlass.Float32],
...
class MixedGluBf16Epilogue:
    ...
    def __init__(
        ...
        *,
        ...
        if gate_up_clamp is not None and (
            not math.isfinite(gate_up_clamp) or gate_up_clamp < 0.0
        ):
            raise ValueError(
                "gate_up_clamp must be finite and non-negative when set; "
                f"got {gate_up_clamp!r}."
            )
        self.gate_up_clamp = (
            cutlass.Float32(gate_up_clamp)
        ...
        flag_batches = (1, 1) if epi_flag_batch is None else epi_flag_batch
        if len(flag_batches) != 2:
            raise ValueError(
                "epi_flag_batch must be a (fc1, fc2) pair; got "
                f"{epi_flag_batch!r}."
            )
        validated_batches = []
        for leg, raw_batch in zip(("fc1", "fc2"), flag_batches):
            batch = int(raw_batch)
            if batch < 1 or batch > 32:
                raise ValueError(
                    f"epi_flag_batch[{leg}] must be in [1, 32], got "
                    f"{raw_batch!r}."
                )
            validated_batches.append(batch)
        self.fc1_epi_flag_batch, self.fc2_epi_flag_batch = validated_batches
    ...
    def run(
        ...
        mixed_optional_args = NvFp4OptinalEpiArgs(
            fc1_alpha=optional_epi_args.fc1_alpha,
            fc2_alpha=optional_epi_args.fc2_alpha,
            fc1_norm_const=None,
