...
"""MegaMoE kernel for NVFP4 weights and BF16 token activations.

This module adds token dispatch/combine plumbing to
``Sm100SwapABNvfp4Bf16Fc12Kernel``.  The local GEMMs remain the mixed
NVFP4-weight/BF16-activation swap-AB pipeline:

* weights are K-major packed E2M1 with atom-swizzled E4M3/K16 scales;
...
class Sm100MegaMoENvfp4Bf16Kernel(Sm100SwapABNvfp4Bf16Fc12Kernel):
        ...
        *,
        ...
        if static_expert_shape is None:
            raise NotImplementedError(
                "Sm100MegaMoENvfp4Bf16Kernel requires "
        ...
        if token_back_mode == "standalone_warps":
            raise NotImplementedError(
                ...
                "NVFP4-to-BF16 transform warps at warp ids 12-15; use "
    ...
    def _dispatch_smem_bytes(self) -> int:
        """Conservative size of the dispatch-warps' shared scratch."""

        pull_mbar_bytes = len(self.dispatch_warp_id) * 8
        expert_count_bytes = self.num_total_experts * 4
        pull_buffer_bytes = len(self.dispatch_warp_id) * self.hidden_bytes
        return (
    ...
    def _smem_misc_budget_bytes(self) -> int:
        """Base kernel overhead plus MegaMoE dispatch scratch."""
    ...
    def _sf_flat_elements(
        ...
        scale_columns = reduction_features // 16
    ...
    def _validate_public_inputs(
        ...
        for name, tensor, expected_rank in (
            ...
            ("fc1_alpha", fc1_alpha, 1),
            ("fc2_alpha", fc2_alpha, 1),
        ...
        if cutlass.const_expr(
            ...
            raise TypeError(
                "FC1 and FC2 weights must use the same NVFP4 dtype."
            )
        if cutlass.const_expr(
        ...
        if cutlass.const_expr(
            fc1_alpha.element_type is not cutlass.Float32
            or fc2_alpha.element_type is not cutlass.Float32
        ):
            raise TypeError("both weight global-scale tensors must be Float32.")
        if cutlass.const_expr(
        ...
        expected_static_dims = (
            ...
            ("fc1_alpha.experts", fc1_alpha.shape[0], experts),
            ("fc2_alpha.experts", fc2_alpha.shape[0], experts),
...
__all__ = ["Sm100MegaMoENvfp4Bf16Kernel"]
