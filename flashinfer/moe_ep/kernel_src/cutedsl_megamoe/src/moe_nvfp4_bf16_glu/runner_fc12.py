...
"""Host driver for the lean NVFP4-weight/BF16-activation fused FC12 path.
...
WeightKind = Literal["nvfp4_bf16"]

# Kernel-repository kinds are weight-first.  FlashInfer's activation-first
# spelling for this W4A16 path is ``bf16_nvfp4_bf16``.
_WEIGHT_DTYPE_BY_KIND = {
    "nvfp4_bf16": Nvfp4DataDtype,
}


class Nvfp4Bf16Fc12Tester(SwigluBf16Fc12Tester):
    """Lean FC12 tester with NVFP4 weights and BF16 activations/handoff."""
    ...
    def __init__(
        ...
        *,
        ...
        self.fc1_alpha = torch.linspace(
            0.5, 1.5, problem.experts, dtype=torch.float32, device="cuda"
        )
        self.fc2_alpha = torch.linspace(
            1.5, 0.5, problem.experts, dtype=torch.float32, device="cuda"
        )
    ...
    def _validate_host_tensor_contracts(self) -> None:
        ...
        expected_dtypes = {
            ...
            "fc1_weight_sf": torch.float8_e4m3fn,
            "fc2_weight": self.weight_torch_dtype,
            "fc2_weight_sf": torch.float8_e4m3fn,
            "fc1_alpha": torch.float32,
        ...
        if not self.fc1_alpha.is_contiguous() or not self.fc2_alpha.is_contiguous():
            raise ValueError("fc1_alpha and fc2_alpha must be contiguous.")
    ...
    def _quantize_weight_rows(
        ...
        for _expert in range(experts):
            if self.misc.perf_run:
                ...
                # Small values keep the E4M3 block scales away from saturation
                # while still exercising non-unit scales and BF16 rounding.
                source = (
                    torch.randn(
                        (output_features, k),
                        device="cuda",
    ...
    def _generate_inputs_skeleton(
        ...
        self.fc1_weight = torch.empty(
            (e, i, h), dtype=fp4, device="cuda"
        ).transpose(1, 2)
        self.fc2_weight = torch.empty(
            (e, h, i // 2), dtype=fp4, device="cuda"
        ).transpose(1, 2)
    ...
    def _instantiate_kernel(self, common_kwargs: dict):
        ...
        from moe_nvfp4_bf16_glu.kernel_nvfp4_bf16_glu_fc12 import (
            Sm100SwapABNvfp4Bf16Fc12Kernel,
        )

        kernel = Sm100SwapABNvfp4Bf16Fc12Kernel(
            **common_kwargs,
    ...
    def _extra_runtime_kwargs(self) -> dict:
        ...
        return {
            ...
            "fc1_alpha": _to_cute(self.fc1_alpha),
            "fc2_alpha": _to_cute(self.fc2_alpha),
        }
    ...
    def compute_reference(self) -> None:
        ...
        for expert in range(problem.experts):
            ...
            # The mixed mainloop first expands each NVFP4/E4M3 weight block to
            ...
            fc1_weight_bf16 = nvfp4_weight_from_swizzled_to_bf16(
                self.fc1_weight[expert],
                sf1_flat,
            )
            fc2_weight_bf16 = nvfp4_weight_from_swizzled_to_bf16(
                self.fc2_weight[expert],
                sf2_flat,
            )
            fc1_fp32 = self._ref_mm(
                a=act,
                b=fc1_weight_bf16,
                n=problem.intermediate,
                k=problem.hidden,
            )
            fc1_fp32 *= self.fc1_alpha[expert]
            ...
            if self.misc.ref_compute_graph == "deepgemm":
                swiglu *= topk.unsqueeze(-1)
            fc1_bf16 = swiglu.to(torch.bfloat16)
            fc2_fp32 = self._ref_mm(
                a=fc1_bf16,
                b=fc2_weight_bf16,
                n=problem.hidden,
                k=problem.intermediate // 2,
            )
            fc2_fp32 *= self.fc2_alpha[expert]
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Lean fused FC12: NVFP4 weights + BF16 activations",
    ...
    parser.add_argument(
        ...
        default="nvfp4_bf16",
        help="NVFP4 weight encoding; activations/handoff remain BF16.",
    )
    parser.add_argument(
        "--gate_up_clamp",
        type=float,
