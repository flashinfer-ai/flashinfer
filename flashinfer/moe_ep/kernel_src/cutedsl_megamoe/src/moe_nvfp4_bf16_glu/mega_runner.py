...
"""MegaMoE host runner for NVFP4 weights and BF16 activations.
...
replaced with packed NVFP4 data plus per-K16 E4M3 scale planes.
...
# Kernel-repository kinds are weight-first.  A future FlashInfer integration,
# whose identifiers are activation-first, must register this path as
# ``bf16_nvfp4_bf16`` (W4A16).
_WEIGHT_DTYPE_BY_KIND = {
    "nvfp4_bf16": Nvfp4DataDtype,
}
...
MIXED_RUNTIME_TENSOR_ATTRS = {
    ...
    "fc1_alpha": "my_fc1_alpha",
    "fc2_alpha": "my_fc2_alpha",
...
def _load_runtime_bindings() -> _RuntimeBindings:
    ...
    from moe_nvfp4_bf16_glu.epilogue_nvfp4_bf16 import (
        EpilogueTokenTile,
    )
    from moe_nvfp4_bf16_glu.megamoe_kernel_nvfp4_bf16 import (
        Sm100MegaMoENvfp4Bf16Kernel,
    )
    ...
    return _RuntimeBindings(
        ...
        utils=utils,
        kernel_class=Sm100MegaMoENvfp4Bf16Kernel,
...
def _swizzled_sf_numel(output_size: int, reduction_size: int) -> int:
    """Element count of one atom-swizzled E4M3 scale plane."""

    raw_cols = reduction_size // Nvfp4BlockSize
...
def plan_nvfp4_bf16_mega_weights(
    *,
    world_size: int,
    num_experts_per_rank: int,
    hidden: int,
    intermediate: int,
) -> Nvfp4Bf16MegaWeightPlan:
    ...
    if hidden <= 0 or hidden % 32 != 0:
        raise ValueError(
            "hidden must be a positive multiple of 32, "
            f"got {hidden}."
        )
    ...
    if intermediate_down % Nvfp4BlockSize != 0:
        raise ValueError(
            f"intermediate/2 must be a multiple of {Nvfp4BlockSize}, "
    ...
    return Nvfp4Bf16MegaWeightPlan(
        global_fc1_weight=(r, e, hidden // 2, intermediate),
        global_fc2_weight=(r, e, intermediate_down // 2, hidden),
        global_fc1_sf_swizzled=(r, e, fc1_sf_size),
        global_fc2_sf_swizzled=(r, e, fc2_sf_size),
        local_fc1_weight=(e, hidden // 2, intermediate),
        local_fc2_weight=(e, intermediate_down // 2, hidden),
...
def _quantize_global_weight(
    *,
    ...
    """Create K-major NVFP4 weights and swizzled E4M3 scale planes."""
    ...
    for _rank in range(world_size):
        for _expert in range(num_experts_per_rank):
            ...
            quantized, raw_scale = nvfp4_quantize_per_block_16(source, 1.0)
...
class MegaMoENvfp4Bf16Tester(MegaMoEBf16Tester):
    """BF16 MegaMoE communication with NVFP4-only expert weights."""
    ...
    def __init__(
        ...
        *,
        rank: int,
        kind: WeightKind = "nvfp4_bf16",
        mask_all_routes: bool = False,
    ) -> None:
        ...
        self._weight_plan = plan_nvfp4_bf16_mega_weights(
        ...
        self.mask_all_routes = bool(mask_all_routes)
        self.my_fc1_alpha = None
        self.my_fc2_alpha = None
        self._global_fc1_alpha = None
        self._global_fc2_alpha = None
    ...
    def generate_inputs(self) -> None:
        ...
        if self.mask_all_routes:
            # Keep the fixed-capacity dispatch tensors allocated, but model a
            # forward with zero valid routes.  -1 is the protocol's masked
            # expert id and a zero score prevents accidental contributions.
            self._global_topk_idx.fill_(-1)
            self._global_topk_weights.zero_()
            self.my_topk_idx.fill_(-1)
            self.my_topk_weights.zero_()
        ...
        self._global_fc1_alpha = torch.linspace(
            0.5, 1.5, r * e, dtype=torch.float32, device="cuda"
        ).reshape(r, e)
        self._global_fc2_alpha = torch.linspace(
            1.5, 0.5, r * e, dtype=torch.float32, device="cuda"
        ).reshape(r, e)
    ...
    def _validate_weight_abi(self) -> None:
        ...
        for name in (
            ...
            if tensor.dtype is not torch.float8_e4m3fn:
                raise TypeError(
                    f"{name} must have dtype torch.float8_e4m3fn, "
                    f"got {tensor.dtype}."
                )
            if tensor.stride(-1) != 1:
                raise ValueError(
        ...
        alpha_shapes = {
            ...
            "my_fc1_alpha": (self.problem.num_experts_per_rank,),
            "my_fc2_alpha": (self.problem.num_experts_per_rank,),
        }
        weight_device = self.my_fc1_weight.device
        for name, shape in alpha_shapes.items():
            tensor = getattr(self, name)
            if tensor is None:
                raise RuntimeError(f"{name} was not generated.")
            if tuple(tensor.shape) != shape:
            ...
            if tensor.device != weight_device:
                raise ValueError(
                    f"{name} must be on {weight_device}, got {tensor.device}."
                )
            if not tensor.is_contiguous():
                raise ValueError(f"{name} must be contiguous.")
            if tensor.data_ptr() % torch.float32.itemsize:
    ...
    def compute_reference(self) -> None:
        ...
        from moe_nvfp4_bf16_glu.mega_reference_nvfp4_bf16 import (
            compute_megamoe_reference_nvfp4_bf16,
        )

        ref_result = compute_megamoe_reference_nvfp4_bf16(
    ...
    def _prepare_combine_output_for_launch(self) -> None:
        """Prepare intentionally accumulated or unwritten output cells.
        ...
        Form-A normally overwrites every route slot. Masked routes have no
        producer, however, so clear just those slots. This retains the debug
        poison for every valid slot while ensuring masked cells cannot expose
        stale data from a previous forward.
        """

        if self.combine_output is None:
            raise RuntimeError(
                "launch requires an allocated combine_output."
            )

        if self.impl.in_kernel_fc2_reduce:
            self.combine_output.zero_()
        else:
            if self.my_topk_idx is None or self._global_topk_idx is None:
                raise RuntimeError(
                    "Form-A launch requires generated routing tensors."
                )
            # Every runner rank owns the replicated global routing table, so
            # this collective branch has the same value on every rank even if
            # only a subset of ranks contains masked local slots.
            if not bool(torch.any(self._global_topk_idx < 0).item()):
                return
            self.combine_output.masked_fill_(
                (self.my_topk_idx < 0).unsqueeze(-1),
                0,
            )
        ...
        if (
            ...
            raise RuntimeError(
                "multi-rank output preparation requires initialized "
                "torch.distributed."
            )
    ...
    def _run_debug_relaunch(self, runtime_kwargs) -> None:
        """Exercise a collective second launch on the existing workspaces."""
        ...
        # A peer may begin its second dispatch as soon as it returns from the
        # first launch.  Make the poison collective-safe so no remote Form-A
        # store can race another rank's local fill.
        torch.cuda.synchronize()
        if self.world_size > 1:
            if (
                not torch.distributed.is_available()
                ...
                raise RuntimeError(
                    "multi-rank debug relaunch requires initialized "
                    "torch.distributed."
                )
            torch.distributed.barrier()
            torch.cuda.synchronize()
    ...
    def run_kernel(self) -> None:
        """Compile and launch ``Sm100MegaMoENvfp4Bf16Kernel``."""
        ...
        runtime_kwargs = {
            name: _to_cute(
                tensor,
                assumed_align=4
                if name in {"fc1_alpha", "fc2_alpha"}
                else 16,
            )
...
def _cleanup_distributed_runtime(
    tester: Optional[MegaMoENvfp4Bf16Tester],
    ...
    if tester is not None:
        ...
        for attr in _SYMMETRIC_RUNTIME_TENSOR_ATTRS:
            ...
            if free_tensor is not None and sym_tensor is not None:
                # Record a failed collective free but continue attempting the
                # remaining tensors and the distributed-runtime finalizer.
                attempt(
                    lambda sym_tensor=sym_tensor: free_tensor(sym_tensor)
                )
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = _build_bf16_arg_parser(
        kind_default="nvfp4_bf16",
        kind_choices=tuple(_WEIGHT_DTYPE_BY_KIND),
        kind_help=(
            "NVFP4 encoding for FC1/FC2 weights; activations and combine "
            "stay BF16."
        ),
    )
    parser.description = (
        "MegaMoE NVFP4-weight/BF16-activation host runner"
    )
    parser.set_defaults(
        kind="nvfp4_bf16",
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Set every routing entry to the masked expert id (-1), exercising "
            "a zero-valid-route launch at the normal static tensor capacity."
        ),
    )
    return parser
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    tester: Optional[MegaMoENvfp4Bf16Tester] = None
    ...
    try:
        ...
        tester = MegaMoENvfp4Bf16Tester(
            problem,
            impl,
            misc,
            rank=rank,
            kind=args.kind,
            mask_all_routes=args.mask_all_routes,
        )
...
__all__ = [
    "MegaMoENvfp4Bf16Tester",
    "Nvfp4Bf16MegaWeightPlan",
    "MIXED_RUNTIME_TENSOR_ATTRS",
    "plan_nvfp4_bf16_mega_weights",
]
