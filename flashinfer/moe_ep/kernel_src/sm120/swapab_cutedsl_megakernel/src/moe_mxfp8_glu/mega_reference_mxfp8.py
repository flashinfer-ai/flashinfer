...
def compute_megamoe_reference_mxfp8(
    ...
    gate_up_interleave: int = Mxfp8BlockSize,
    ...
    if gate_up_interleave <= 0 or intermediate % (2 * gate_up_interleave) != 0:
        raise ValueError(
            "intermediate must be divisible by twice gate_up_interleave; "
            f"got intermediate={intermediate}, "
            f"gate_up_interleave={gate_up_interleave}."
        )
    ...
    for global_expert in range(num_total_experts):
        ...
        # SM100 uses block-size interleave=32; SM120 Swap-A/B uses the
        # m16n8 accumulator's natural interleave=8.
        _M, _N = fc1_output_fp32.shape
        _n_pairs = _N // (2 * gate_up_interleave)
        _reshaped = fc1_output_fp32.view(
            _M, _n_pairs, 2, gate_up_interleave
        )
        _gate = _reshaped[:, :, 0, :]
        _up = _reshaped[:, :, 1, :]
