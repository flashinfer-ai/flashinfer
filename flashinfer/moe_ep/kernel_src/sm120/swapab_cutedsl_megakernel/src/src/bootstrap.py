...
def init_dist_and_nvshmem() -> tuple[int, int, int, Device]:
    ...
    # On single-GPU nodes (e.g. DGX Spark GB10) multiple local ranks share
    # one device; fold the rank onto the available ordinals.
    device_ordinal = local_rank % torch.cuda.device_count()
    torch.cuda.set_device(device_ordinal)
    dev = Device(device_ordinal)
    dev.set_current()

    # NCCL cannot form a communicator when several ranks share one physical
    # GPU (e.g. 8 PEs on a DGX Spark GB10) -- every CUDA-tensor collective
    # dies with "Duplicate GPU detected".  MEGA_SINGLE_GPU_GLOO=1 switches
    # the process group to pure gloo and stages CUDA all_gathers through
    # CPU (gloo handles CUDA broadcast/all_reduce/barrier natively but not
    # all_gather).  All heavy traffic goes over NVSHMEM either way.
    single_gpu_gloo = os.environ.get("MEGA_SINGLE_GPU_GLOO") == "1"
    backend = "gloo" if single_gpu_gloo else "cpu:gloo,cuda:nccl"
    ...
    if single_gpu_gloo and not getattr(dist, "_mega_cpu_all_gather_shim", False):
        ...
        dist.all_gather = _cpu_staged_all_gather
        dist._mega_cpu_all_gather_shim = True
    ...
    # Broadcast the UID over gloo (CPU): NCCL cannot build a communicator
    # when multiple ranks share one physical GPU (e.g. 8 ranks on a GB10),
    # and this is the only collective before NVSHMEM takes over.
    gloo_group = dist.new_group(backend="gloo")
    uid_tensor = torch.from_numpy(uid_bytes)
    dist.broadcast(uid_tensor, src=0, group=gloo_group)
