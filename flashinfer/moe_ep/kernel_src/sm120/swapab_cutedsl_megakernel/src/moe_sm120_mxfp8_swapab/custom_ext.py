...
class Sm120SwapABMxfp8Fc12WorkTileInfo(MoEWorkTileInfo):
    ...
    def __init__(
        ...
        # Transient scheduler fields — carried through MLIR serialization but
        # NOT written to SMEM.
        # fc1_counter_index: default tile_n_idx (swap-AB) or cluster_token_block_idx (non-swapAB).
        fc1_counter_index: Int32,
        # valid_tokens_in_cluster: fc1-ready-counter spin threshold for this CTA.
        valid_tokens_in_cluster: Int32,
    ):
        ...
        self.valid_tokens_in_cluster = valid_tokens_in_cluster
    ...
    def __extract_mlir_values__(self) -> List[ir.Value]:
        ...
        values.extend(extract_mlir_values(self.valid_tokens_in_cluster))
        return values

    def __new_from_mlir_values__(
        self, values: List[ir.Value]
    ) -> "Sm120SwapABMxfp8Fc12WorkTileInfo":
        ...
        return type(self)(
            ...
            valid_tokens_in_tile=new_from_mlir_values(
                self.valid_tokens_in_tile, [values[6]]
            ),
            ...
            valid_tokens_in_cluster=new_from_mlir_values(
                self.valid_tokens_in_cluster, [values[9]]
            ),
        )
    ...
    @classmethod
    def from_rmem(cls, rmem: cute.Tensor) -> "Sm120SwapABMxfp8Fc12WorkTileInfo":
        return cls(
            ...
            valid_tokens_in_cluster=rmem[6],  # type: ignore[arg-type]  # alias: correct for swap-AB
        )
...
class Sm120SwapABMxfp8Fc12SchedExtension(MoESchedExtension):
    """Sched extension for the fused fc1+fc2 swap-AB SwiGLU MXFP8 kernel.

    ``WorkTileInfo = Sm120SwapABMxfp8Fc12WorkTileInfo``.  The 8th slot stores
    ...
    WorkTileInfo = Sm120SwapABMxfp8Fc12WorkTileInfo
    ...
    def __new_from_mlir_values__(
        self, values: List[ir.Value]
    ) -> "Sm120SwapABMxfp8Fc12SchedExtension":
        ...
        assert idx == len(values), (
            f"Sm120SwapABMxfp8Fc12SchedExtension serialization mismatch: "
            f"idx={idx} len(values)={len(values)}"
        )
        result = Sm120SwapABMxfp8Fc12SchedExtension.__new__(
            Sm120SwapABMxfp8Fc12SchedExtension
        )
    ...
    def enrich_work_tile_info(
        ...
        return Sm120SwapABMxfp8Fc12WorkTileInfo(
            ...
            valid_tokens_in_cluster=base_work.valid_tokens_in_tile,
        )
    ...
    def get_gmem_tensor(
        ...
        work_tile_info: Sm120SwapABMxfp8Fc12WorkTileInfo,
        ...
        if cutlass.const_expr(tensor_name == "a"):
            ...
            # before MXFP8 quantize, so fc2 mainloop already reads the
