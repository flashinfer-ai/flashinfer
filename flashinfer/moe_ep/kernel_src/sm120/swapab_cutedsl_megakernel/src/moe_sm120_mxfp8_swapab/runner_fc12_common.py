...
This module holds the SM120 MXFP8 single-rank runner machinery
and MXFP8 (``moe_mxfp8_glu/runner_fc12.py``) fused-fc12 runners have in common:
  ...
  * :class:`Fc12TesterBase` -- the host-side input/reference/launch/
    ...
    concrete subclasses implement:
      ...
      - :meth:`_quantize_fc1`              -- post-SwiGLU MXFP8 quantize
...
from moe_sm120_mxfp8_swapab.sm120_mma import (
    ...
    SWAP_AB_INTERLEAVE as Mxfp8Fc1GateUpInterleave,
)
...
class ProblemDesc:
    ...
    # pre-activations before SiLU. ``None`` disables it.
    ...
    def __post_init__(self) -> None:
        ...
        if self.hidden <= 0 or self.hidden % Mxfp8BlockSize != 0:
            raise ValueError(
                f"hidden ({self.hidden}) must be a positive multiple of {Mxfp8BlockSize}."
            )
        if (
            self.intermediate <= 0
            or self.intermediate % INTERMEDIATE_ALIGNMENT != 0
        ):
            raise ValueError(
                f"intermediate ({self.intermediate}) must be a positive multiple of "
                f"{INTERMEDIATE_ALIGNMENT} for MXFP8 block scaling."
            )
        if (self.intermediate // 2) % Mxfp8BlockSize != 0:
            raise ValueError(
                f"intermediate/2 ({self.intermediate // 2}) must be a multiple of "
                f"sf_vec_size ({Mxfp8BlockSize}) for fc2's per-block SF coverage."
            )
        ...
        if self.kind not in ("mxfp8_e4m3", "mxfp8_e5m2"):
            raise ValueError(
                f"kind must be 'mxfp8_e4m3' or 'mxfp8_e5m2'; "
                f"got {self.kind!r}."
            )
        ...
        # the dtype by ``check_tma_leading_dim_align`` (bf16/fp16 = 2B,
        # fp32 = 4B, fp8 = 1B).
        #
        # ``fc1_output`` is the kernel-internal staging buffer (post-fc1
        # MXFP8 + per-block SF, consumed as fc2's B operand).  Its layout
        ...
        data_dtype = kind_data_dtype(self.kind)
...
class ImplDesc:
    ...
    Defaults: mma_tiler ``(M=64, N=128, K=128)``, single-cluster, single-CTA
    ...
    def __post_init__(self) -> None:
        ...
        if n not in (32, 64, 128):
            raise ValueError(f"mma_tiler_n must be 32, 64, or 128, got {n}.")
        if cl != 1:
        ...
        if self.use_2cta_instrs:
            raise ValueError(
                "SM120 warp-level QMMA does not support the SM100 2-CTA path."
            )
...
class MiscDesc:
    ...
    ``ref_compute_graph`` selects which reference compute graph to use:
      ...
      - ``"deepgemm"``: DeepGEMM mega-MoE semantics --
        ``out = down_proj @ (swiglu * topk_score)`` with topk_score
        pre-multiplied into swiglu in fp32 before MXFP8 quantize, so
        per-block SF absorbs the per-token scaling.  This is what the
        multi-rank kernel uses; the single-rank topk=1 path is unweighted.

    The two placements are not bit-exact because topk scaling changes MXFP8
    quantization bins and UE8M0 scale selection.
    ...
    def __str__(self) -> str:
        return (
            ...
            f"iket={'on' if self.enable_iket else 'off'} "
            f"seed={self.seed}"
        )
...
class Fc12TesterBase:
    """Host-side input/reference/launch/validation driver for MXFP8."""
    ...
    def _generate_inputs_skeleton(
        ...
        # fc1_weight: contiguous (expert, N, K), exposed as K-major (expert, K, N).
        self.fc1_weight = torch.empty(
            (experts, intermediate, hidden), dtype=data_dtype, device="cuda"
        ).permute(0, 2, 1)
        # fc2_weight: contiguous (expert, N, K), exposed as K-major (expert, K, N).
    ...
    def _alloc_fc2_output_skeleton(self, data_total_rows: int) -> None:
        ...
        Use the same output shape as the initialized path, but with
        ``torch.empty`` instead of a 0xFF sentinel fill.
        """
        problem = self.problem
    ...
    def _init_global_scales_and_norm(self) -> None:
        """Per-expert tensor-wise global scales and norm const, pinned 1.0.
    ...
    def compute_reference(self) -> None:
        ...
        the kernel's data path (modulo fp32 vs SM120 QMMA rounding inside
        ...
        Per expert ``e`` with ``v_e`` valid tokens (skipping ``v_e == 0``):
          ...
          5. MXFP8-quantize swiglu -> ``(fc1_q, fc1_sf)``
          ...
          9. optionally apply topk after fc2 for transformers semantics
        ...
        from moe_sm120_mxfp8_swapab.mega_reference import (
        ...
        sf_vec_size = kind_sf_vec_size(problem.kind)
        gate_up_interleave = Mxfp8Fc1GateUpInterleave
        ...
        # Per-expert fc1 MXFP8 hand-off snapshots for the fc1-phase validation
        ...
        for expert_idx in range(problem.experts):
            ...
            fc1_topk_weights = None

            act_slice = slice_tensor_logical_dim(
                self.activation, dim=0, start=d_start, end=d_start + v_e
            )

            # Shared core: MXFP8 fc1 GEMM -> SwiGLU(+clamp) -> MXFP8
            # round-trip -> MXFP8 fc2 GEMM.
            ...
            # fc1 MXFP8 hand-off snapshot for phase validation.
    ...
    def _partition_workspace(
        ...
        # MXFP8 workspace layout matching kernel.get_workspace_size_in_bytes.
    ...
    def validate(self) -> None:
        ...
        # Single-rank storage carries a singleton topk axis.
...
def add_common_fc12_arguments(parser: argparse.ArgumentParser) -> None:
    """Register common arguments for the SM120 MXFP8 fc12 runner."""
    # -- Problem --
    parser.add_argument(
    ...
    # -- Impl --
    parser.add_argument(
        "--mma_tiler_mnk", type=str, default="64,128,128",
        help="Comma-separated SM120 swap-AB CTA tile (M, N, K).",
    )
    ...
    parser.add_argument(
        ...
        help="Use random-byte FP8 data instead of sparse {0, +/-1}.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "'deepgemm' (default): topk weight is pre-multiplied into "
            "SwiGLU before MXFP8 quantize when routing metadata is present. "
            "'transformers': apply topk after FC2 in FP32."
        ),
    )
    parser.add_argument(
