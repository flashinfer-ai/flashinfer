...
class Sm120MegaMoEMxfp8SwapABKernel(Sm120SwapABSwigluMxfp8Fc12Kernel):
    ...
    def __init__(
        ...
        *,
        ...
        self.dispatch_warp_id = self.sm120_dispatch_warp_id
        ...
        self.threads_per_cta = 32 * (
            ...
            + 1  # tma_b
            + 1  # scheduler
            + 1  # reserved warp completing the 4-warp setmaxnreg group
        ...
        # Developer tuning overrides.  They are compile-time kernel constants,
        # so experiments do not add a runtime branch to the pull loop.
        dispatch_pull_mode = os.environ.get(
            "MEGA_DISPATCH_PULL_MODE", dispatch_pull_mode
        )
        ...
        dispatch_warps_per_tile = int(
            os.environ.get(
                "MEGA_DISPATCH_WARPS_PER_TILE", str(dispatch_warps_per_tile)
            )
        )
        overlap_env = os.environ.get("MEGA_DISPATCH_COMPUTE_OVERLAP")
        if overlap_env is None:
            if dispatch_compute_overlap is None:
                # At 512 tokens the spin/issue contention costs more than the
                # hidden communication; overlap wins from 1024 tokens onward.
                dispatch_compute_overlap = self.max_tokens_per_rank >= 1024
        else:
            overlap_env = overlap_env.strip().lower()
            if overlap_env not in ("0", "1", "false", "true", "off", "on"):
                raise ValueError(
                    "MEGA_DISPATCH_COMPUTE_OVERLAP must be a boolean value, got "
                    f"{overlap_env!r}."
                )
            dispatch_compute_overlap = overlap_env in ("1", "true", "on")
    ...
    def __call__(
        ...
        token_comm_args = ExtractedTokenCommArgs(
            ...
            local_zero_prefix=grid_sync_counter,
