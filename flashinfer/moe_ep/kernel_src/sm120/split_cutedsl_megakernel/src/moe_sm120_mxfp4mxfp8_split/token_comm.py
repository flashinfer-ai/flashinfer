"""SM120 token communication specialization for NVSHMEM GPU heaps.

The common protocol stays in :mod:`src.token_comm`.  This subclass carries the
SM120 adaptations: peer-store barriers, local receive-count reduction,
...
class Sm120SysmemTokenInPullTokenBackPush(TokenInPullTokenBackPush):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.ibgda_dispatch_chunk_tokens = ibgda_dispatch_chunk_tokens
        if self.ibgda_dispatch_chunk_tokens <= 0:
            raise ValueError("ibgda_dispatch_chunk_tokens must be positive")
        self.ibgda_dispatch_num_chunks = (
            max_tokens_per_rank + self.ibgda_dispatch_chunk_tokens - 1
        ) // self.ibgda_dispatch_chunk_tokens
        self.ibgda_dispatch_ready_words = (
            1 + self.world_size * max_tokens_per_rank
            if comm_backend == "nvshmem_ibgda"
            else 0
        )
        self.ibgda_owner_plan_ready_words = (
        ...
        self.dispatch_rank_cache_debug = dispatch_rank_cache_debug
        ...
        self.k1_ready_queue_base = k1_ready_queue_base
        if k1_ready_queue_m_rotation < 0:
            raise ValueError(
                "k1_ready_queue_m_rotation must be non-negative, got "
                f"{k1_ready_queue_m_rotation}."
            )
        self.k1_ready_queue_m_rotation = (
            k1_ready_queue_m_rotation % k1_ready_queue_m_tiles
            if k1_ready_queue_m_tiles > 0
            else 0
        )
        if self.k1_ready_queue and dispatch_pull_mode != "token_strided":
            raise ValueError(
                "K1 producer-pushed ready queue requires token_strided dispatch."
            )
        if comm_backend == "nvshmem_ibgda" and dispatch_pull_mode != "token_strided":
            raise ValueError(
                "nvshmem_ibgda currently requires token_strided dispatch."
            )
    ...
    def dispatch_barrier(
        ...
        *,
        ...
        if cutlass.const_expr(self.rank_local_combine):
            # Contributor counts and route-map entries are peer-written by
            # every dispatch CTA. Flush them before SM0 publishes admission.
            cute.arch.fence_acq_rel_sys()
    ...
    def _push_k1_ready_queue(
        ...
        *,
        ...
        queue_idx = cute.arch.atomic_add(
            queue_state.iterator + Int32(4),
            Int32(1),
            scope="gpu",
        )
        queue_pos = Int32(self.k1_ready_queue_base) + queue_idx
        desc_base = queue_pos * Int32(7)
        queue_desc[desc_base + Int32(0)] = expert_idx
        queue_desc[desc_base + Int32(1)] = tile_m_idx
        queue_desc[desc_base + Int32(2)] = tile_n_idx
        queue_desc[desc_base + Int32(3)] = cumulative_data
    ...
    def _push_k1_ready_work_tiles(
        ...
        *,
        ...
        if lane_idx == leader_lane:
            queue_begin = cute.arch.atomic_add(
                queue_state.iterator + Int32(4),
                Int32(self.k1_ready_queue_m_tiles),
                scope="gpu",
            )
        queue_begin = cute.arch.shuffle_sync(
            queue_begin,
            offset=leader_lane,
            mask=0xFFFFFFFF,
            mask_and_clamp=31,
        ...
        for work_batch in cutlass.range_constexpr(
            ...
            linear_tile_m_idx = (
                Int32(work_batch * 32) + lane_idx
            )
            if linear_tile_m_idx < Int32(self.k1_ready_queue_m_tiles):
                tile_m_idx = linear_tile_m_idx
                if cutlass.const_expr(self.k1_ready_queue_m_rotation != 0):
                    # Keep queue positions dense while rotating descriptors;
                    # consumers still dequeue every tile exactly once.
                    tile_m_idx = (
                        linear_tile_m_idx
                        + tile_n_idx
                        * Int32(self.k1_ready_queue_m_rotation)
                    ) % Int32(self.k1_ready_queue_m_tiles)
                queue_pos = (
                    Int32(self.k1_ready_queue_base)
                    + queue_begin
                    + linear_tile_m_idx
                )
                desc_base = queue_pos * Int32(7)
        ...
        for work_batch in cutlass.range_constexpr(
            ...
            tile_m_idx = Int32(work_batch * 32) + lane_idx
            if tile_m_idx < Int32(self.k1_ready_queue_m_tiles):
                cute.arch.store(
                    queue_ready.iterator
                    + Int32(self.k1_ready_queue_base)
                    + queue_begin
                    + tile_m_idx,
                    Int32(1),
        ...
        *,
        ...
        while current_expert_idx < Int32(self.num_experts_per_rank):
            ...
            if current_expert_idx < Int32(self.num_experts_per_rank):
                ...
                # Count/pool extents are published before K1 starts, but the
                # source/expert route list is copied independently as its
                # compact sideband chunk arrives.  Do not consume a stale
                # metadata row from the previous replay.
                if cutlass.const_expr(self.comm_backend == "nvshmem_ibgda"):
                    if lane_idx == Int32(0):
                        owner_plan_ready_idx = (
                            Int32(self.ibgda_dispatch_ready_words)
                            + current_rank_in_expert_idx
                            * Int32(self.num_experts_per_rank)
                            + current_expert_idx
                        )
                        dispatch_epoch = sf_staging_buffer[Int32(0)]
                        spin_wait(
                            sf_staging_buffer.iterator
                            + owner_plan_ready_idx,
                            lambda value: value >= dispatch_epoch,
                ...
                if cutlass.const_expr(
                    ...
                    if cutlass.const_expr(self.dispatch_rank_cache_debug):
                        if lane_idx == Int32(0):
                            if cache_old == Int32(0):
                                cute.arch.atomic_add(
                                    dispatch_rank_cache_debug.iterator,
                                    Int32(1),
                                    sem="relaxed",
                                    scope="gpu",
                                )
                                ...
                                cute.arch.atomic_add(
                                    dispatch_rank_cache_debug.iterator + Int32(1),
                                    Int32(1),
                                    sem="relaxed",
                                    scope="gpu",
                                )
                                ...
                                cute.arch.atomic_add(
                                    dispatch_rank_cache_debug.iterator + Int32(2),
                                    Int32(1),
                                    sem="relaxed",
                                    scope="gpu",
                                )
                            if current_rank_in_expert_idx != Int32(self.local_rank):
                                cute.arch.atomic_add(
                                    dispatch_rank_cache_debug.iterator + Int32(3),
                                    Int32(1),
                                    sem="relaxed",
                                    scope="gpu",
                                )
                ...
                if cutlass.const_expr(self.rank_local_combine):
                    # One dispatch warp owns this pool row. The map is reset
                    # before every replay, so absent routes cannot reuse an old
                    # pool index.
                    source_token_key = (
                        current_rank_in_expert_idx
                        * Int32(self.max_tokens_per_rank)
                    ...
                    if lane_idx == Int32(0):
                        sf_staging_buffer[source_token_key, src_topk] = pool_token_idx
                ...
                weight = Float32(0.0)
                if cutlass.const_expr(self.comm_backend == "nvshmem_ibgda"):
                    # Sparse streaming dispatch publishes readiness for the
                    # exact source token after activation, scale factors and
                    # top-k weights have all reached local HBM. Word zero
                    # carries the current replay epoch so stale flags from a
                    # previous invocation cannot release K1 early.
                    dispatch_ready_idx = (
                        Int32(1)
                        + current_rank_in_expert_idx
                        * Int32(self.max_tokens_per_rank)
                    ...
                    if lane_idx == Int32(0):
                        dispatch_epoch = sf_staging_buffer[Int32(0)]
                        spin_wait(
                            sf_staging_buffer.iterator + dispatch_ready_idx,
                            lambda value: value >= dispatch_epoch,
                            fail_sleep_cycles=500,
                        )
                    cute.arch.sync_warp()
                    ...
                    src_token_i32 = cute.make_tensor(
                        cute.recast_ptr(input_token_buffer.iterator, dtype=Int32),
                        cute.make_layout(
                            input_token_buffer.shape[0]
                            * (self.hidden_bytes // 4)
                        ),
                    )
                    dst_token_i32 = cute.make_tensor(
                        cute.recast_ptr(fc1_input_token_buffer.iterator, dtype=Int32),
                        cute.make_layout(
                            fc1_input_token_buffer.shape[0]
                            * (self.hidden_bytes // 4)
                        ),
                    )
                    token_i32_base = (
                        staged_token * Int32(self.hidden_bytes // 4)
                    )
                    pool_i32_base = (
                        pool_token_idx * Int32(self.hidden_bytes // 4)
                    )
                    for j in cutlass.range(
                        ...
                        dst_token_i32[pool_i32_base + j] = src_token_i32[
                            token_i32_base + j
                        ]
                    ...
                    sf_stage_base = Int64(
                        self.ibgda_dispatch_ready_words
                        + self.ibgda_owner_plan_ready_words
                    ) + Int64(pool_token_idx) * Int64(
                        self.sf_uint32_per_token
                    )
                    staged_sf_base = Int64(staged_token) * Int64(
                        self.sf_uint32_per_token
                    )
                    input_sf_i32 = cute.make_tensor(
                        cute.recast_ptr(input_sf_buffer.iterator, dtype=Int32),
                        cute.make_layout(
                            input_sf_buffer.shape[0]
                            * self.sf_uint32_per_token
                        ),
                    )
                    for i in cutlass.range_constexpr(0, sf_passes, 1):
                        ...
                        if j < Int32(self.sf_uint32_per_token):
                            sf_value = input_sf_i32[
                                staged_sf_base + Int64(j)
                            ]
                    ...
                    if cache_old > Int32(0):
                        tma_src_addr = (
                            ...
                            + Int64(cache_master_pool * Int32(self.hidden_bytes))
                        )
                ...
                if cutlass.const_expr(self.k1_ready_queue):
                    release_lane = token_idx & Int32(31)
                    tile_n_idx = (
                        token_idx_in_expert
                        // Int32(self.cluster_tile_tokens)
                    )
                    tile_begin = (
                        tile_n_idx * Int32(self.cluster_tile_tokens)
                    )
                    valid_tokens_in_tile = cutlass.min(
                        Int32(self.cluster_tile_tokens),
                        expert_end_idx - expert_start_idx - tile_begin,
                    )
                    publish_tile = Int32(0)
                    if lane_idx == release_lane:
                    ...
                    publish_tile = cute.arch.shuffle_sync(
                        publish_tile,
                        offset=release_lane,
                        mask=0xFFFFFFFF,
                        mask_and_clamp=31,
                    )
                    if publish_tile != Int32(0):
                        self._push_k1_ready_work_tiles(
                            k1_ready_queue_desc,
                            k1_ready_queue_ready,
                            k1_ready_queue_state,
                            ...
                            cumulative_token_block=expert_task_tile_offset,
                            valid_tokens=valid_tokens_in_tile,
                            lane_idx=lane_idx,
                            leader_lane=release_lane,
                        )
                else:
                    task_tile_addr = (
                        fc1_ready_counter.iterator + task_tile_idx
        ...
        if cutlass.const_expr(not self.k1_ready_queue):
            flag_tracker.fire()
        cute.arch.sync_warp()
    ...
    def token_back_by_push(
        ...
        *,
        ...
        while current_expert_idx < Int32(self.num_experts_per_rank):
            ...
            if current_expert_idx < Int32(self.num_experts_per_rank):
                ...
                if cutlass.const_expr(self.token_back_reduce_topk):
                    token_offset = Int64(src_token) * Int64(fc2_token_bytes)
                else:
                    token_offset = (
                ...
                peer_token_addr = peer_combine_base + token_offset
                smem_ptr_warp = (
                    pull_buffer_ptr + warp_idx * Int32(chunk_bytes)
                )
                mbar_ptr_warp = pull_mbar_ptr + warp_idx
                ...
                for chunk in cutlass.range(num_chunks, unroll=1):
                    ...
                    if cutlass.const_expr(self.token_back_reduce_topk):
                        ...
                        for redg_iter in cutlass.range_constexpr(
                            ...
                            if (
                                byte_off + Int32(redg_bytes_per_lane)
                                <= this_bytes
                            ):
                        ...
                        for copy_iter in cutlass.range_constexpr(
                            ...
                            if (
                                byte_off + Int32(copy_bytes_per_lane)
                                <= this_bytes
                            ):
                                v = ldg_b32_raw(
                                    local_token_addr
                                ...
                                stg_b32_raw(
                                    peer_chunk_addr + Int64(byte_off), v
                                )
                        cute.arch.fence_acq_rel_sys()
                    phase_bit = phase_bit ^ Int32(1)
                    t1 = read_clock64()
        ...
        if _iket_emit:
            _iket.range_push("token_back_final_fence")
        cute.arch.fence_acq_rel_sys()
        if _iket_emit:
            _iket.range_pop()
        ...
        if sm_idx == 0:
            if warp_idx == 0:
                ...
                if cutlass.const_expr(nvlink_barrier_counter is not None):
                    ...
                    # A monotonic generation prevents a stale signal from
                    # satisfying a later replay after phase-slot reuse.
    ...
    def dispatch_warp_body(
        ...
        *,
        ...
        # Record CTA 0's complete four-warp dispatch group. Perfetto keeps
        # each warp on a separate track, so shared labels remain unambiguous.
        iket_active = cta_linear_id == Int32(0)
        if iket_active:
            _iket.range_push("Dispatch_Prep")

        if cutlass.const_expr(self.comm_backend == "nvshmem_ibgda"):
            # Standalone GPU NVSHMEM kernels have already pushed contiguous
            # activation/SF/weight buffers and installed the immutable route
            # control tables before K1 starts.  There are no peer pointers or
            # device-side NVSHMEM calls in the persistent compute kernel.
            pass
        else:
            if cutlass.const_expr(self.rank_local_combine):
                # The rank-local sideband is peer-written.  A fast rank may
                # otherwise enter replay N+1 and publish its new counts while
                # a slower peer is still completing that replay's local
                # graph-ordered reset, which erases the fresh stores.  Join
                # all ranks after their local reset and before the first peer
                # write.  This is an epoch-entry ownership handoff, not a
                # whole-dispatch admission barrier: dispatch_prep and FC1
                # retain their ordinary fine-grained streaming protocol once
                # the new epoch has begun.
                self.nvlink_barrier(
            ...
            if cutlass.const_expr(self.rank_local_combine):
                # Publish one immutable contributor count for every
                # (source rank, source token, destination owner).  The
                # existing dispatch admission protocol orders these remote
                # stores before K2 can consume them.  Publish the source
                # epoch through the same device-side path: K2 may finish a
                # small tile before the terminal dispatch barrier, so reading
                # the source GPU's host-written footer directly is not a
                # valid cross-rank happens-before edge.
                source_tokens = Int32(
                ...
                if (
                    ...
                    epoch = cute.arch.load(
                        combine_ready_flags.iterator
                        + Int32(self.max_tokens_per_rank * self.world_size),
                        Int32,
                        sem="acquire",
                        scope="gpu",
                    )
                ...
                stride = Int32(
                    token_comm_args.sm_count * self.num_dispatch_threads
                )
                while token < Int32(self.max_tokens_per_rank):
                    key = Int32(
                        self.local_rank * self.max_tokens_per_rank
                    ) + token
                    for dst in cutlass.range_constexpr(self.world_size):
                        count = Int32(0)
                        for slot in cutlass.range_constexpr(self.num_topk):
                            expert = Int32(
                                token_comm_args.topk_idx[token, slot]
                            )
                            if (
                                expert >= Int32(dst * self.num_experts_per_rank)
                                and expert
                                < Int32((dst + 1) * self.num_experts_per_rank)
                            ):
                                count += Int32(1)
                        peer_count = (
                            token_comm_args.peer_rank_ptr_mapper.ptr_map_to_rank(
        ...
        if cutlass.const_expr(self.k1_ready_queue):
            if lane_idx == Int32(0):
                old_done = cute.arch.atomic_add(
                    k1_ready_queue_state.iterator + Int32(6),
                    Int32(1),
                    sem="acq_rel",
                    scope="gpu",
                )
                expected_producers = Int32(
                    token_comm_args.sm_count * self.num_dispatch_warps
                )
                if old_done + Int32(1) == expected_producers:
                    for _ in cutlass.range(
                        0, token_comm_args.sm_count, 1, unroll=1
                    ):
                        self._push_k1_ready_queue(
                            k1_ready_queue_desc,
                            k1_ready_queue_ready,
                            k1_ready_queue_state,
                            expert_idx=Int32(-1),
                            tile_m_idx=Int32(0),
                            tile_n_idx=Int32(0),
                            cumulative_data=Int32(0),
                            cumulative_sf=Int32(0),
                            cumulative_token_block=Int32(0),
        ...
        if cutlass.const_expr(
            ...
            and self.comm_backend != "nvshmem_ibgda"
        ):
            if iket_active:
    ...
    def kernel_tail(
        ...
        *,
        ...
        _iket.range_push("kernel_tail_cta_rendezvous")
        ...
        if (warp_idx >= self.dispatch_warp_start) and (
            ...
            local_warp_idx = Int32(warp_idx) - Int32(self.dispatch_warp_start)
            _iket.range_push("kernel_tail_rank_release")
            self.nvlink_barrier(
                token_comm_args.nvlink_barrier_signal,
                token_comm_args.nvlink_barrier_counter,
                token_comm_args.grid_sync_counter,
            ...
            _iket.range_push("kernel_tail_counter_reset")
            ...
            _iket.range_pop()
            _iket.range_push("kernel_tail_final_barrier")
            self.nvlink_barrier(
                token_comm_args.nvlink_barrier_signal,
                token_comm_args.nvlink_barrier_counter,
                token_comm_args.grid_sync_counter,
    ...
    def kernel_tail_after_grid_drain_epoch(
        ...
        *,
        ...
        if (warp_idx >= self.dispatch_warp_start) and (
            ...
            if cutlass.const_expr(self.rank_local_combine):
                rank_combine_records: cutlass.Constexpr[int] = (
                    self.world_size * self.max_tokens_per_rank
                    + self.world_size
                )
                i = thread_linear
                while i < Int32(rank_combine_records):
                    token_comm_args.fc2_done_counter[i] = Int32(0)
                    i = i + stride
    ...
    def kernel_tail_ibgda(
        ...
        *,
        ...
        """Rendezvous K1 threads for the externally staged transport path.

        Cross-rank dispatch/combine completion is enforced by stream-ordered
        NVSHMEM barriers in the host launcher.  Persistent K1 therefore only
        needs its ordinary CTA-local tail rendezvous; replay counters are
        reset by the launcher before the next invocation.
        """
        nb_kernel_tail = pipeline.NamedBarrier(
            barrier_id=self.kernel_tail_named_barrier_id,
            num_threads=self.kernel_tail_threads,
