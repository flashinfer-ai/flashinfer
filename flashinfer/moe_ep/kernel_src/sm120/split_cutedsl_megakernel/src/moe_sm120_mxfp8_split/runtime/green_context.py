...
class GreenContextStreams:
    """Two disjoint SM partitions and their execution streams."""

    k1_stream: torch.cuda.ExternalStream
    k2_stream: torch.cuda.ExternalStream
    k1_sm_count: int
    k2_sm_count: int
    _k1_raw_stream: Any
    _k2_raw_stream: Any
    ...
    def create(
        ...
        *,
        ...
        """Partition one device and create one nonblocking stream per part."""
        ...
        (sm_resource,) = _check_cuda(
            cuda.cudaDeviceGetDevResource(
                device,
                cuda.cudaDevResourceType.cudaDevResourceTypeSm,
            ),
        ...
        groups, num_groups, remainder = _check_cuda(
            cuda.cudaDevSmResourceSplitByCount(
                1,
                sm_resource,
                0,
                k1_sm_count,
        ...
        if int(num_groups) != 1 or len(groups) != 1:
            raise RuntimeError(
                "CUDA did not produce the requested K1 Green Context partition."
            )
        resources = (groups[0], remainder)
        if any(int(resource.sm.smCount) <= 0 for resource in resources):
            raise RuntimeError("CUDA produced an empty Green Context partition.")
        ...
        try:
            for index, resource in enumerate(resources):
                (descriptor,) = _check_cuda(
                    cuda.cudaDevResourceGenerateDesc([resource], 1),
                    f"cudaDevResourceGenerateDesc(K{index + 1})",
                )
                (context,) = _check_cuda(
                    cuda.cudaGreenCtxCreate(descriptor, device, 0),
                    f"cudaGreenCtxCreate(K{index + 1})",
                )
                contexts.append(context)
                (raw_stream,) = _check_cuda(
                    cuda.cudaExecutionCtxStreamCreate(
                        context,
                        cuda.cudaStreamNonBlocking,
                        0,
                    ),
                    f"cudaExecutionCtxStreamCreate(K{index + 1})",
                )
                raw_streams.append(raw_stream)
        except Exception:
            for raw_stream in reversed(raw_streams):
                cuda.cudaStreamDestroy(raw_stream)
            for context in reversed(contexts):
                cuda.cudaExecutionCtxDestroy(context)
            raise

        return cls(
            k1_stream=torch.cuda.ExternalStream(
                int(raw_streams[0]), device=device
            ),
            k2_stream=torch.cuda.ExternalStream(
                int(raw_streams[1]), device=device
            ),
            k1_sm_count=int(resources[0].sm.smCount),
            k2_sm_count=int(resources[1].sm.smCount),
            _k1_raw_stream=raw_streams[0],
            _k2_raw_stream=raw_streams[1],
            _k1_context=contexts[0],
            _k2_context=contexts[1],
        )

    def close(self) -> None:
        """Synchronize and release streams before releasing their contexts."""
        if self._closed:
            return

        from cuda.bindings import runtime as cuda

        self.k1_stream.synchronize()
        self.k2_stream.synchronize()
        _check_cuda(
            cuda.cudaStreamDestroy(self._k1_raw_stream),
            "cudaStreamDestroy(K1)",
        )
        _check_cuda(
            cuda.cudaStreamDestroy(self._k2_raw_stream),
            "cudaStreamDestroy(K2)",
        )
        _check_cuda(
            cuda.cudaExecutionCtxDestroy(self._k1_context),
            "cudaExecutionCtxDestroy(K1)",
        )
        _check_cuda(
            cuda.cudaExecutionCtxDestroy(self._k2_context),
            "cudaExecutionCtxDestroy(K2)",
        )
        self._closed = True
