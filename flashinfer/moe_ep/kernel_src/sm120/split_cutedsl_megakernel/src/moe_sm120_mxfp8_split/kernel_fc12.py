...
"""Fused swap-AB MXFP4-weight/MXFP8-activation fc1+fc2 kernel for SM120."""
...
class Sm120SwapABSwigluMxfp8Fc12Kernel:
    """Fused swap-AB W4A8 fc1+fc2 SwiGLU grouped GEMM for MoE on SM120.
    ...
    def __init__(
        ...
        *,
        ...
        if sf_vec_size != Mxfp8BlockSize:
            raise NotImplementedError(
                f"SM120 MXFP4 x MXFP8 requires sf_vec_size={Mxfp8BlockSize}; "
                f"got {sf_vec_size}."
            )
        if weight_dtype is not cutlass.Float4E2M1FN:
            raise NotImplementedError(
                "SM120 W4A8 requires Float4E2M1FN weights; "
                f"got {weight_dtype}."
            )
        if activation_dtype is not cutlass.Float8E4M3FN:
            raise NotImplementedError(
                "SM120 W4A8 requires Float8E4M3FN activations; "
        ...
        self.num_ab_stages_override = num_ab_stages_override
        self.green_trace_role = green_trace_role

        # Fused fc12 sched-side knobs
        self.group_hint = group_hint
        self.token_padding_block = token_padding_block
        self.fc1_ready_tile_tokens = mma_tiler_mnk[1]
        self.sf_padding_block = sf_padding_block
        self.load_balance_mode = load_balance_mode
        self.jit_config = jit_config or Sm120JitConfig.from_environment()
        # Zero keeps the original grouped order; C>0 emits C token tiles of
        # FC1 followed by the same C tiles of FC2.
        self.streaming_fc12_tiles = self.jit_config.fc12_stream_tiles
        self.streaming_fc12 = self.streaming_fc12_tiles > 0
        # Spread a scheduler wave across token blocks by default. Dispatch
        # publishes token blocks independently, so this avoids concentrating
        # the whole K1 wave on one not-yet-ready release counter.
        self.k1_ready_major = self.jit_config.k1_ready_major
        self.k2_token_stripe = self.jit_config.k2_token_stripe
        self.fc2_ready_bundle_k_tiles = (
            self.jit_config.fc2_ready_bundle_k_tiles
        )
        ...
        if self.jit_config.fc2_packed_store is None:
            self.fc2_packed_store = fc2_output_dtype is cutlass.BFloat16
        else:
            self.fc2_packed_store = self.jit_config.fc2_packed_store
        if self.fc2_packed_store and fc2_output_dtype is not cutlass.BFloat16:
            raise NotImplementedError(
                "fc2_packed_store currently requires BF16 output."
            )
        ...
        self.has_dispatch_warps: bool = False
        # Split-MegaMoE subclasses specialize the shared scheduler without
        # duplicating the validated SM120 QMMA mainloop.
        self.scheduler_phase_mode: Literal["fused", "fc1", "fc2"] = "fused"
        self.k1_ready_queue: bool = False
        self.k1_ready_queue_base: int = 0
        self.scheduler_generation_mode: Literal["fused", "fc1", "fc2"] = "fused"
        self.scheduler_needs_token_layout: bool = False
        self.fc2_ready_wait_enabled: bool = True
        ...
        self.use_warpgroup_reg_realloc = True
    ...
    def _setup_attributes(self) -> None:
        ...
        if self.a_dtype is not self.weight_dtype:
            raise TypeError(
                f"weight tensor dtype must be {self.weight_dtype}, got {self.a_dtype}"
            )
        if self.b_dtype is not self.activation_dtype:
            raise TypeError(
                "activation tensor dtype must be "
                f"{self.activation_dtype}, got {self.b_dtype}"
            )
        self.mixed_mode = self.a_dtype != self.b_dtype
        self.smem_alloc_a_dtype = (
            cutlass.Int8
            if self.mixed_mode and self.a_dtype.width == 4
            else self.a_dtype
        )
        self.smem_alloc_b_dtype = self.b_dtype
        self.tma_internal_a_dtype = (
            cutlass.Int8
            if self.mixed_mode and self.a_dtype.width == 4
            else None
        )
        ...
        # FC1 needs a reusable N16 x downproj32 reduction tile plus the
        # quantized STSM destination. The lean FC2-only specialization owns no
        # FC1 epilogue and therefore reserves none of this storage.
        if self.scheduler_phase_mode == "fc2":
            c_bytes_total = 0
        else:
        ...
        if self.num_ab_stages_override is not None:
            ...
            requested_num_ab_stage = self.jit_config.num_ab_stages_override
        else:
            requested_num_ab_stage = default_num_ab_stage
        if requested_num_ab_stage < 1 or requested_num_ab_stage > max_num_ab_stage:
            raise ValueError(
                "num_ab_stages must be in the SMEM-feasible range "
                f"[1, {max_num_ab_stage}], got {requested_num_ab_stage}."
            )
        self.num_ab_stage = requested_num_ab_stage
        print(
            f"[{self.scheduler_phase_mode} stages] num_ab_stage={self.num_ab_stage} "
            ...
            f"ab_bytes_per_stage={self.ab_bytes_per_stage} "
            f"estimated_smem_per_cta="
            f"{self.fixed_smem_overhead + self.num_ab_stage * self.ab_bytes_per_stage} "
            f"smem_cap={self.smem_capacity} "
    ...
    def _compute_stages(
        ...
        a_smem_layout_stage_one = self._make_sm120_data_smem_layout(
            ...
            dtype=(
                cutlass.Int8
                if a_dtype.width == 4 and a_dtype != b_dtype
                else a_dtype
            ),
            is_a=True,
        )
        ...
        self.ab_bytes_per_stage = ab_bytes_per_stage
        self.fixed_smem_overhead = fixed_overhead
    ...
    def get_workspace_size_in_bytes(
        ...
        experts, intermediate_gateup, _packed_hidden = fc1_weight_tensor.shape
    ...
    def token_comm_hook_fc2_tile_complete(
        ...
        *,
        ...
        """Publish optional split-K3 readiness after an FC2 tile store."""
        pass
    ...
    def token_comm_hook_fc1_cta_complete(
        ...
        *,
        ...
        """Rendezvous all K1 CTA threads before publishing queue completion."""
        pass
    ...
    def _launch_sm120_inline_fc12(
        ...
        if cutlass.const_expr(self.static_expert_shape is not None):
            ...
            fc1_weight = cute.make_tensor(
                fc1_weight.iterator,
                cute.make_layout(
                    (experts_static, intermediate_gateup_static, hidden_static),
            ...
            fc2_weight = cute.make_tensor(
                fc2_weight.iterator,
                cute.make_layout(
                    (experts_static, hidden_static, intermediate_downproj_static),
        ...
        experts, intermediate_gateup, hidden_b = fc1_weight.shape
        ...
        experts2, hidden_b2, intermediate_downproj_b2 = fc2_weight.shape
        ...
        sched_params = MoEFusedFc12SchedulerParams(
            ...
            sf_padding_block=self.sf_padding_block,
            ready_tile_tokens=self.fc1_ready_tile_tokens,
            ...
            phase_mode=self.scheduler_generation_mode,
            fc1_ready_major=self.k1_ready_major,
            fc2_token_stripe=self.k2_token_stripe,
        )
        ...
        class SharedStorage:
            ...
            sA: cute.struct.Align[
                cute.struct.MemRange[
                    self.smem_alloc_a_dtype, cute.cosize(a_smem_layout_staged)
                ],
                128,
            ]
    ...
    def sm120_inline_fc12_kernel(
        ...
        if cutlass.const_expr(
            self.enable_token_comm and self.use_warpgroup_reg_realloc
        ):
    ...
    def __call__(
        ...
        activation: cute.Tensor,           # (token_sum_padded, hidden) MXFP8 E4M3
        fc1_weight: cute.Tensor,           # logical (experts, intermediate_gateup, hidden) MXFP4 E2M1
        ...
        fc2_weight: cute.Tensor,          # logical (experts, hidden, intermediate_downproj) MXFP4 E2M1
        ...
        combine_ready_flags: Optional[cute.Tensor] = None,
        fc2_block_done_counter: Optional[cute.Tensor] = None,
        # ── Sched / runtime ──────────────────────────────────────────────
        ...
        # Optional split-kernel globaltimer trace.
        green_trace: Optional[cute.Tensor] = None,
        # Optional K1-produced K2 bundle queue.
        k2_ready_queue_desc: Optional[cute.Tensor] = None,
        k2_ready_queue_ready: Optional[cute.Tensor] = None,
        k2_ready_queue_state: Optional[cute.Tensor] = None,
    ) -> None:
        ...
        # A_gemm (fc1 weights): (experts, intermediate_gateup, hidden)
        ...
        fc1_weight_gemm = cute.make_tensor(
            ...
            cute.make_layout(
                (intermediate_gateup, hidden_b, experts),
                stride=(fc1_weight.stride[1], fc1_weight.stride[2], fc1_weight.stride[0]),
            ),
        )
        ...
        # A_gemm (fc2 weights): (experts, hidden, intermediate_downproj)
        ...
        fc2_weight_gemm = cute.make_tensor(
            ...
            cute.make_layout(
                (hidden_b2, intermediate_downproj_b2, experts2),
                stride=(fc2_weight.stride[1], fc2_weight.stride[2], fc2_weight.stride[0]),
            ),
        )
        ...
        expert_cnt = experts
        # ``intermediate_gateup`` (= fc1_weight.shape[1]) is what we pass to the
        ...
        # both MXFP4; activation and fc1_output are both MXFP8; scales are
        # all E8M0). ``self.fc1_output_dtype`` selects the fc1 MXFP8 output
    ...
    def fc1fc2_kernel_impl(
        ...
        trace_role = ROLE_K1
        if cutlass.const_expr(self.scheduler_phase_mode == "fc2"):
            trace_role = ROLE_K2
            ...
            if warp_idx == cutlass.Int32(0):
                if cute.arch.lane_idx() == cutlass.Int32(0):
                    cute.arch.store(
                        ...
                        + trace_word(
                            trace_role, cta_linear_id, FIELD_KERNEL_ENTRY
                        ),
                        read_globaltimer(),
                        scope="gpu",
                    )
        ...
        if cutlass.const_expr(self.scheduler_phase_mode != "fc2"):
            fc1_epilogue_storage = smem.allocate(
                Fc1EpilogueStorage, byte_alignment=128
            )
        else:
            fc1_epilogue_storage = None
        ...
        early_internal_init = (
            not (
                (
                    ...
                    and self.k1_ready_queue
                    and k2_ready_queue_state is not None
                )
                or (
                    self.scheduler_phase_mode == "fc2"
                    and k2_ready_queue_state is not None
                )
            )
            and (
                (self.load_balance_mode == "atomic_counter")
                or (not self.scheduler_needs_token_layout)
            )
        )
        ...
        # tile. The FC2-only specialization does not materialize these views.
        if cutlass.const_expr(self.scheduler_phase_mode != "fc2"):
            sFc1Swiglu = fc1_epilogue_storage.sFc1Swiglu.get_tensor(
                cute.make_layout(
                    (16, self.mma_tiler[0] // 2),
                    stride=(self.mma_tiler[0] // 2, 1),
                )
            )
            sFc1OutputStaged = fc1_epilogue_storage.sFc1Output.get_tensor(
                fc1_output_smem_layout_staged.outer,
                swizzle=fc1_output_smem_layout_staged.inner,
            )
            sFc1Output = cute.slice_(sFc1OutputStaged, (None, None, 0))
        else:
            # CuTeDSL traces both sides of the fixed-phase epilogue branch.
            # Re-view existing A SMEM so type checking needs no FC1 allocation;
            # the constant-false FC1 path is removed before SASS emission.
            sFc1Swiglu = cute.make_tensor(
                cute.recast_ptr(sA.iterator, dtype=cutlass.Float32),
                cute.make_layout((16, self.mma_tiler[0] // 2)),
            )
            sFc1OutputStaged = cute.make_tensor(
                sA.iterator, fc1_output_smem_layout_staged.outer
            )
            sFc1Output = cute.slice_(sFc1OutputStaged, (None, None, 0))
        ...
        if warp_idx == self.sm120_aux_warp_id:
            ...
            if cutlass.const_expr(
                ...
                and self.scheduler_phase_mode == "fused"
            ):
                aux_running = cutlass.Boolean(True)
                aux_ready_slot = cutlass.Int32(0)
                while aux_running:
                    iket.range_push("aux_fc2_ready_work_wait")
                    ...
                    if aux_counter_slot < cutlass.Int32(0):
                        ...
                        for aux_bundle_idx in cutlass.range(
                            ...
                            iket.range_push("aux_fc2_ready_counter_wait")
                            ...
                            iket.range_pop()
                            iket.range_push("aux_fc2_ready_release")
                            if aux_ready_slot == cutlass.Int32(0):
                                cute.arch.barrier(
        ...
        if warp_idx == self.sched_warp_id:
            ...
            if cutlass.const_expr(
                ...
                while queue_running:
                    ...
                    if cute.arch.lane_idx() == cutlass.Int32(0):
                        queue_idx = cute.arch.atomic_add(
                            k2_ready_queue_state.iterator + cutlass.Int32(5),
                            cutlass.Int32(1),
                            scope="gpu",
                        )
                    ...
                    if cute.arch.lane_idx() == cutlass.Int32(0):
                        spin_wait(
                            ...
                            + cutlass.Int32(self.k1_ready_queue_base)
                            + queue_idx,
                    ...
                    if cute.arch.lane_idx() == cutlass.Int32(0):
                        desc_base = (
                            cutlass.Int32(self.k1_ready_queue_base) + queue_idx
                        ) * cutlass.Int32(7)
                        expert_idx = k2_ready_queue_desc[
                            desc_base + cutlass.Int32(0)
                        ]
                        tile_m_idx = k2_ready_queue_desc[
                            desc_base + cutlass.Int32(1)
                        ]
                        tile_n_idx = k2_ready_queue_desc[
                            desc_base + cutlass.Int32(2)
                        ]
                        cumulative_data = k2_ready_queue_desc[
                            desc_base + cutlass.Int32(3)
                        ]
                        cumulative_sf = k2_ready_queue_desc[
                            desc_base + cutlass.Int32(4)
                        ]
                        cumulative_token_block = k2_ready_queue_desc[
                            desc_base + cutlass.Int32(5)
                        ]
                        valid_tokens = k2_ready_queue_desc[
                            desc_base + cutlass.Int32(6)
                        ]
                    expert_idx = cute.arch.shuffle_sync(
                        expert_idx, offset=0, mask=0xFFFFFFFF, mask_and_clamp=31
                    )
                    tile_m_idx = cute.arch.shuffle_sync(
                        tile_m_idx, offset=0, mask=0xFFFFFFFF, mask_and_clamp=31
                    )
                    ...
                    if expert_idx < cutlass.Int32(0):
                        ...
                        scheduler.current_work = ext.enrich_work_tile_info(
                            ext.WorkTileInfo(
                                ...
                                cumulative_token_block_count=cumulative_token_block,
                        ...
                        iket.range_push("schedule_fc1_ready_queue_tile")
                        scheduler.publish_work()
                        iket.range_pop()
                ...
                # dispatch/count publication before static scheduler decode.
                ...
                if cutlass.const_expr(
                    self.scheduler_phase_mode == "fc2"
                    and self.scheduler_generation_mode == "fused"
                ):
                    while (
                        scheduler.current_work.is_valid_tile
                        and scheduler.current_work.phase
                        != cutlass.Int32(BlockPhase.Linear2)
                    ):
                        scheduler.gen_next_work()
                while scheduler.current_work.is_valid_tile:
        ...
        if warp_idx == self.tma_a_warp_id:
            ...
            while work_tile_info.is_valid_tile:
                if cutlass.const_expr(self.scheduler_phase_mode == "fused"):
                    is_phase_linear1 = (
                        work_tile_info.phase == cutlass.Int32(BlockPhase.Linear1)
                    )
                else:
                    is_phase_linear1 = self.scheduler_phase_mode == "fc1"
            ...
            while work_tile_info.is_valid_tile:
                ...
                if is_phase_linear1:
                    ...
                    counter_slot = (
                        ...
                        ) // cutlass.Int32(self.fc1_ready_tile_tokens)
                    )
                    if cutlass.const_expr(token_comm_args is None):
                        ...
                        iket.range_push("tma_token_fc2_work_handoff")
                        ...
                        if cutlass.const_expr(token_comm_args is not None):
                            if (
                                ...
                                if cutlass.const_expr(
                                    ...
                                    if cutlass.const_expr(
                                        self.fc2_ready_wait_enabled
                                    ):
                                        bundle_idx = k_tile // cutlass.Int32(
                                            self.fc2_ready_bundle_k_tiles
                                        )
                                        bundle_begin = bundle_idx * cutlass.Int32(
                                            self.fc2_ready_bundle_k_tiles
                                        )
                                        bundle_k_tiles = cutlass.min(
                                            cutlass.Int32(
                                                self.fc2_ready_bundle_k_tiles
                                            ),
                                            k_tile_cnt_fc2 - bundle_begin,
                                        )
                                        if cute.arch.lane_idx() == cutlass.Int32(0):
                                            spin_wait_i32_ge_inline(
                                                fc1_done_counter.iterator
        ...
        if warp_idx < len(self.compute_warp_id):
            ...
            sFc1OutputAtom = cute.composition(
                cute.local_tile(
                    sFc1Output, (16, downproj_per_cta), (0, 0)
                ),
                stsm_remap_layout,
            )
            ...
            rFc1OutputPacked = cute.recast_tensor(
                rFc1Output, cutlass.Int32
            )
            rFc1OutputLocal = cute.make_rmem_tensor((4,), cutlass.Float32)
            rFc1Store = cute.make_rmem_tensor(
                (4,), self.fc1_output_dtype
            )
            rFc1StoreI32 = cute.recast_tensor(rFc1Store, cutlass.Int32)
            ...
            work_tile_info = sched_consumer.consume_work()
            fc1_detail_tiles_seen = cutlass.Int32(0)
            ...
            while work_tile_info.is_valid_tile:
                ...
                # Keep detailed IKET traffic bounded. Every persistent CTA
                # sparsely samples its first FC1/FC2 tile; CTA(0,0,0) also
                # records every K128 step for its first few FC2 tiles so a
                # long FC2 tile can be attributed to A, B, or QMMA work.
                trace_compute_detail = cutlass.Int32(0)
                trace_all_k_detail = cutlass.Int32(0)
                if bidx == cutlass.Int32(0):
                    if is_phase_linear1:
                        if fc1_detail_tiles_seen == cutlass.Int32(0):
                            trace_compute_detail = cutlass.Int32(1)
                    else:
                        if fc2_detail_tiles_seen == cutlass.Int32(0):
                            trace_compute_detail = cutlass.Int32(1)
                        if (
                            bidz == cutlass.Int32(0)
                            and fc2_detail_tiles_seen < cutlass.Int32(4)
                        ):
                            trace_compute_detail = cutlass.Int32(1)
                ...
                for k_tile in cutlass.range(0, k_tile_cnt, 1, unroll=1):
                    ...
                    if trace_k_detail != cutlass.Int32(0):
                        if is_phase_linear1:
                            iket.range_push("sm120_fc1_wait_a")
                        else:
                        ...
                        if compute_warp == cutlass.Int32(0):
                            if lane_idx == cutlass.Int32(0):
                                if k2_trace_a_wait_active:
                                    k2_trace_tma_a_wait_ns += (
                                        read_globaltimer()
                                        - k2_trace_a_wait_start
                                    )
                    if trace_k_detail != cutlass.Int32(0):
                        iket.range_pop()
                        if is_phase_linear1:
                            iket.range_push("sm120_fc1_wait_b")
                        else:
                        ...
                        if compute_warp == cutlass.Int32(0):
                            if lane_idx == cutlass.Int32(0):
                                if k2_trace_b_wait_active:
                                    k2_trace_tma_b_wait_ns += (
                                        read_globaltimer()
                                        - k2_trace_b_wait_start
                                    )
                    if trace_k_detail != cutlass.Int32(0):
                        iket.range_pop()
                    peek_a_full_status = cutlass.Boolean(1)
                ...
                if cutlass.const_expr(green_trace is not None):
                    if compute_warp == cutlass.Int32(0):
                        if lane_idx == cutlass.Int32(0):
                            k2_trace_mainloop_ns += (
                                read_globaltimer()
                                - k2_trace_mainloop_start
                            )
                trace_post_mainloop_start = cutlass.Int64(0)
                if cutlass.const_expr(green_trace is not None):
                    if compute_warp == cutlass.Int32(0):
                        if lane_idx == cutlass.Int32(0):
                            trace_post_mainloop_start = read_globaltimer()
                iket.range_pop()
                if self.scheduler_phase_mode != "fc2" and is_phase_linear1:
                    ...
                    for ng_pair in cutlass.range_constexpr(0, n_groups // 2):
                        ...
                        if tile_token3 >= work_tile_info.valid_tokens_in_tile:
                            val3 = cutlass.Float32(0.0)
                        if trace_compute_detail != cutlass.Int32(0):
                            iket.range_push("sm120_fc1_epi_amax_reduce")
                        amax0 = cute.math.absf(val0)
                        amax1 = cute.math.absf(val1)
                        amax2 = cute.math.absf(val2)
                        amax3 = cute.math.absf(val3)
                        ...
                        if trace_compute_detail != cutlass.Int32(0):
                            iket.range_push("sm120_fc1_epi_quant_pack")
                        ...
                        if trace_compute_detail != cutlass.Int32(0):
                            iket.range_push("sm120_fc1_epi_stsm")
                        sFc1OutputPair = cute.composition(
                            cute.local_tile(
                                sFc1Output,
                                (16, downproj_per_cta),
                    ...
                    for store_iter in cutlass.range_constexpr(
                        ...
                        store_token = linear_chunk // cutlass.Int32(
                            chunks_per_row
                        )
                        store_dp = (
                            linear_chunk
                            % cutlass.Int32(chunks_per_row)
                        ) * cutlass.Int32(4)
                        for j in cutlass.range_constexpr(0, 4):
                            rFc1Store[j] = sFc1Output[
                                store_token,
                        ...
                        if (
                            ...
                            output_ptr = (
                                ...
                                + output_row
                                * real_fc1_output.stride[0]
                                + output_col
                                * real_fc1_output.stride[1]
                            ).align(4)
                            output_i32 = cute.make_tensor(
                                cute.recast_ptr(
                                    output_ptr,
                    ...
                    if cutlass.const_expr(
                        self.fc1_producer_tiles_per_ready_block == 1
                    ):
                        counter_slot = (
                            work_tile_info.cumulative_token_block_count
                            + work_tile_info.tile_n_idx
                    ...
                    if cutlass.const_expr(token_comm_args is not None):
                        if compute_warp == cutlass.Int32(0):
                            ...
                            ready_old = cutlass.Int32(0)
                            if lane_idx == cutlass.Int32(0):
                                ready_old = cute.arch.atomic_add(
                                    fc1_done_counter.iterator
                            ...
                            if cutlass.const_expr(
                                ...
                                ready_old = cute.arch.shuffle_sync(
                                    ready_old,
                                    offset=0,
                                    mask=0xFFFFFFFF,
                                    mask_and_clamp=31,
                                ...
                                if cutlass.const_expr(
                                    ...
                                    expert_token_count = cutlass.Int32(
                                        cutlass.Int64(packed_expert_count)
                                        & cutlass.Int64(0xFFFFFFFF)
                                    )
                                    ready_tile_valid_tokens = cutlass.min(
                                        cutlass.Int32(
                                            self.k2_ready_tile_tokens
                                        ),
                                        expert_token_count
                                ...
                                if final_bundle_complete:
                                    ...
                                    valid_k2_token_tiles = (
                                        ready_tile_valid_tokens
                                        + cutlass.Int32(
                                            self.k2_token_tile_tokens - 1
                                        )
                                    ) // cutlass.Int32(
                                        self.k2_token_tile_tokens
                                    )
                                    queue_entry_count = (
                                        cutlass.Int32(queue_bundle_count)
                                        * valid_k2_token_tiles
                                    )
                                    queue_base = cutlass.Int32(0)
                                    if lane_idx == cutlass.Int32(0):
                                        queue_base = cute.arch.atomic_add(
                                            k2_ready_queue_state.iterator
                                            + cutlass.Int32(1),
                                            queue_entry_count,
                                            scope="gpu",
                                        )
                                    queue_base = cute.arch.shuffle_sync(
                                        queue_base,
                                        offset=0,
                                        mask=0xFFFFFFFF,
                                        mask_and_clamp=31,
                                    ...
                                    for queue_store_iter in cutlass.range_constexpr(
                                        ...
                                        if queue_entry_idx < queue_entry_count:
                                            k2_token_tile = (
                                                queue_entry_idx
                                                // cutlass.Int32(
                                                    queue_bundle_count
                    ...
                    if cutlass.const_expr(
                        combine_ready_flags is not None
                        and fc2_block_done_counter is not None
                    ):
                        # The last hidden tile for this token block publishes
                        # one ready flag per source (token, top-k) row.
                        cute.arch.fence_acq_rel_sys()
                        cute.arch.barrier(
                            barrier_id=self.epilog_sync_bar_id,
                            number_of_threads=32 * len(self.compute_warp_id),
                        )
                        self.token_comm_hook_fc2_tile_complete(
                            token_comm_args,
                            combine_ready_flags,
                            fc2_block_done_counter,
                            work_tile_info,
                ...
                if cutlass.const_expr(green_trace is not None):
                    if compute_warp == cutlass.Int32(0):
                        if lane_idx == cutlass.Int32(0):
                            k2_trace_store_ns += (
                                read_globaltimer() - trace_post_mainloop_start
                            )

                if is_phase_linear1:
                    fc1_detail_tiles_seen += cutlass.Int32(1)
                else:
                    ...
                    if compute_warp == cutlass.Int32(0):
                        if lane_idx == cutlass.Int32(0):
                            cute.arch.store(
                                green_trace.iterator
                                + trace_word(
                                    trace_role, cta_linear_id, FIELD_TILE_COUNT
                                ),
                                k2_trace_tile_count,
                                scope="gpu",
                            )
                            ...
                            cute.arch.store(
                                green_trace.iterator
                                + trace_word(
                                    trace_role, cta_linear_id, FIELD_STORE_NS
                                ),
                                k2_trace_store_ns,
                                scope="gpu",
                            )
        ...
        # ════════════════════════════════════════════════════════════════════
        #
        # ``has_dispatch_warps=False`` means warps 8-11 don't exist at all
        # (threads_per_cta = 256), so the hook call is entirely
        # const_expr-eliminated. When present, the subclass implements the
        # full dispatch chain inside this
        # hook (prep -> cross-rank barrier -> per-token pull -> release
        # to fc1 -> arrive on dispatch-to-sched NamedBarrier).
        if cutlass.const_expr(self.has_dispatch_warps):
        ...
        if cutlass.const_expr(
            ...
            # The final K1 warp is not necessarily warp 0.  Publishing the
            # producer count before every warp has drained its last tile can
            # expose a DONE sentinel while FC1 output is still being written.
            self.token_comm_hook_fc1_cta_complete(
                token_comm_args,
                warp_idx=warp_idx,
                lane_idx=lane_idx,
                tidx=tidx,
            ...
            if warp_idx == cutlass.Int32(0):
                completed_ctas = cutlass.Int32(0)
                if lane_idx == cutlass.Int32(0):
                    completed_ctas = cute.arch.atomic_add(
                        k2_ready_queue_state.iterator + cutlass.Int32(3),
                        cutlass.Int32(1),
                        sem="acq_rel",
                        scope="gpu",
                    )
                completed_ctas = cute.arch.shuffle_sync(
                    completed_ctas,
                    offset=0,
                    mask=0xFFFFFFFF,
                    mask_and_clamp=31,
                ...
                if (
                    completed_ctas + cutlass.Int32(1)
                    == gdimx * gdimy * gdimz
                ):
                    sentinel_base = cutlass.Int32(0)
                    if lane_idx == cutlass.Int32(0):
                        sentinel_base = cute.arch.atomic_add(
                            k2_ready_queue_state.iterator + cutlass.Int32(1),
                            cutlass.Int32(256),
                            scope="gpu",
                        )
                    sentinel_base = cute.arch.shuffle_sync(
                        sentinel_base,
                        offset=0,
                        mask=0xFFFFFFFF,
                        mask_and_clamp=31,
                    )
                    for sentinel_iter in cutlass.range_constexpr(0, 8):
                        sentinel_pos = (
                            sentinel_base
                            + lane_idx
                            + cutlass.Int32(sentinel_iter * 32)
                        )
                        k2_ready_queue_desc[
                            sentinel_pos * cutlass.Int32(7)
                        ] = cutlass.Int32(WorkTileState.DONE)
                    cute.arch.sync_warp()
                    cute.arch.fence_acq_rel_gpu()
                    for sentinel_iter in cutlass.range_constexpr(0, 8):
                        sentinel_pos = (
                        ...
                        cute.arch.store(
                            k2_ready_queue_ready.iterator + sentinel_pos,
                            cutlass.Int32(1),
                            sem="release",
                            scope="gpu",
                        )
            ...
            if warp_idx == cutlass.Int32(0):
                if lane_idx == cutlass.Int32(0):
                    cute.arch.store(
                        green_trace.iterator
                        + trace_word(
                            trace_role, cta_linear_id, FIELD_KERNEL_EXIT
                        ),
                        read_globaltimer(),
                        scope="gpu",
                    )
