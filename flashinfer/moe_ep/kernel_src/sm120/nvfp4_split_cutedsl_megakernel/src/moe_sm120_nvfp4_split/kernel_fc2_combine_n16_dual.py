...
class Sm120Fc2CombineN16DualGroupKernel(Sm120MegaMoENvfp4SwapABKernel):
    ...
    def _owner_direct_row(self, token_comm_args, metadata):
        output_addr = (
            token_comm_args.combine_output.iterator.toint()
            + (
                cutlass.Int64(metadata.src_token)
                * cutlass.Int64(self.num_topk)
                + cutlass.Int64(metadata.src_topk)
            )
            * cutlass.Int64(self.hidden * 2)
        )
        ...
        peer_ptr = token_comm_args.peer_rank_ptr_mapper.ptr_map_to_rank(
            local_ptr, metadata.src_rank
        )
        return cute.make_tensor(peer_ptr, cute.make_layout(self.hidden))

    @cute.jit
    def _owner_direct_row_for_slot(
        self, token_comm_args, metadata, canonical_slot
    ):
        """Return the source-GPU row chosen for one owner group."""
        output_addr = (
            token_comm_args.combine_output.iterator.toint()
            + (
                cutlass.Int64(metadata.src_token)
                * cutlass.Int64(self.num_topk)
                + cutlass.Int64(canonical_slot)
            )
            * cutlass.Int64(self.hidden * 2)
        )
        local_ptr = cute.make_ptr(
    ...
    def fc2_combine_kernel_impl(self, tiled_mma: cute.TiledMma, tiled_mma_sfb: cute.TiledMma, tma_atom_fc1_weight: cute.CopyAtom, tma_tensor_fc1_weight: cute.Tensor, tma_atom_activation: cute.CopyAtom, tma_tensor_activation: cute.Tensor, tma_atom_fc1_weight_sf: cute.CopyAtom, tma_tensor_fc1_weight_sf: cute.Tensor, tma_atom_activation_sf: cute.CopyAtom, tma_tensor_activation_sf: cute.Tensor, tma_atom_fc2_weight: cute.CopyAtom, tma_tensor_fc2_weight: cute.Tensor, tma_atom_fc1_output_as_fc2_input: cute.CopyAtom, tma_tensor_fc1_output_as_fc2_input: cute.Tensor, tma_atom_fc2_weight_sf: cute.CopyAtom, tma_tensor_fc2_weight_sf: cute.Tensor, tma_atom_fc1_output_sf_as_fc2_input: cute.CopyAtom, tma_tensor_fc1_output_sf_as_fc2_input: cute.Tensor, fc1_weight_gemm: cute.Tensor, activation_gemm: cute.Tensor, fc1_output_gemm: cute.Tensor, fc1_weight_sf_gemm: cute.Tensor, activation_sf_gemm: cute.Tensor, fc1_output_sf_gemm: cute.Tensor, fc2_weight_gemm: cute.Tensor, fc2_output: cute.Tensor, fc2_weight_sf_gemm: cute.Tensor, fc1_output_sf_gemm_for_fc2_load: cute.Tensor, topk_scores: cute.Tensor, fc1_done_counter: cute.Tensor, combine_ready_flags: Optional[cute.Tensor], fc2_block_done_counter: Optional[cute.Tensor], fc1_alpha: Optional[cute.Tensor], fc2_alpha: Optional[cute.Tensor], fc1_norm_const: Optional[cute.Tensor], sched_params: MoEFusedFc12SchedulerParams, cluster_layout_vmnk: cute.Layout, cluster_layout_sfb_vmnk: cute.Layout, a_smem_layout_staged: cute.ComposedLayout, b_smem_layout_staged: cute.ComposedLayout, sfa_smem_layout_staged: cute.Layout, sfb_smem_layout_staged: cute.Layout, fc1_output_smem_layout_staged: cute.ComposedLayout, token_comm_args=None, green_trace: Optional[cute.Tensor]=None, k2_ready_queue_desc: Optional[cute.Tensor]=None, k2_ready_queue_ready: Optional[cute.Tensor]=None, k2_ready_queue_state: Optional[cute.Tensor]=None):
        ...
        ext = Sm120SwapABNvfp4Fc12SchedExtension(sf_vec_size=self.sf_vec_size, fc1_done_counter_ptr=fc1_done_counter.iterator, fc2_spin_threshold=ext_fc2_spin_threshold, fc1_ready_counter_ptr=self.token_comm_hook_fc1_ready_counter_ptr(token_comm_args))
        ...
        if warp_idx == self.sched_warp_id:
            ...
            while queue_running:
                ...
                iket.range_push("schedule_fc2_queue_claim")
                ...
                iket.range_pop()
                iket.range_push("schedule_fc2_queue_poll")
                if cute.arch.lane_idx() == cutlass.Int32(0):
                    spin_wait(
                ...
                cute.arch.sync_warp()
                # The descriptor release also orders FC1 data/scale stores.
                # TMA observes a separate async proxy, so both the system
                # acquire and the generic-to-async proxy fence are required.
                ...
                iket.range_push("schedule_fc2_descriptor_decode")
                ...
                if expert_idx < cutlass.Int32(0):
                    ...
                    for bundle_tile in cutlass.range_constexpr(
                        ...
                        if hidden_tile < cutlass.Int32(hidden_tiles):
                            ...
                            iket.range_push("schedule_fc2_publish_work")
        ...
        if warp_idx == self.tma_a_warp_id:
            ...
            while work_tile_info.is_valid_tile:
                ...
                iket.range_push('tma_weight_fc2_descriptor')
                ...
                iket.range_pop()
                a_producer.reset()
                iket.range_push('tma_weight_fc2_stage_probe')
        ...
        if warp_idx < len(self.compute_warp_id):
            ...
            rSFBCompact = cute.make_rmem_tensor((8,), self.sf_dtype)
            ...
            tCrSFA = partition_fragment_sfa_for_sm120_mma(
                compute_tiled_mma, sSFA[None, None, 0], mma_tidx
            )
            ...
            smem_tiled_copy_SFA = cute.make_tiled_copy(
                atom_copy_scale,
                get_layout_sfa_tv(compute_tiled_mma),
                (
            ...
            while work_tile_info.is_valid_tile:
                # The scheduler warp acquired the K1-published descriptor.
                # Transfer that release chain to every compute lane before it
                # later consumes row metadata for direct peer stores.
                cute.arch.fence_acq_rel_sys()
                ...
                # One N128 SFB stage contains eight N16 tiles. Encode both
                # the routed tile slot and this warp's N8 half in the shared
                # address, then keep only the eight K16 scale bytes in a fixed
                # register fragment for the two K64 QMMA issues.
                ...
                for k_tile in cutlass.range(0, k_tile_cnt, 1, unroll=1):
                    ...
                    rSFBCompactI32[0] = lds_b32_raw(
                        sSFB.iterator + sfb_stage_byte_offset
                    )
                    rSFBCompactI32[1] = lds_b32_raw(
                        sSFB.iterator
                        + sfb_stage_byte_offset
                        + cutlass.Int32(512)
                    )
                    ...
                    cute.copy(smem_tiled_copy_SFA, tCsSFA_p_filtered[None, None, 0], tCrSFA_copy_view_filtered[None, None, 0])
                    for k_inner_mma in cutlass.range_constexpr(0, 2):
                        if cutlass.const_expr(k_inner_mma + 1 < 2):
                            ...
                            cute.copy(smem_tiled_copy_SFA, tCsSFA_p_filtered[None, None, k_inner_next], tCrSFA_copy_view_filtered[None, None, k_inner_next])
                ...
                store_expert_idx = sWorkTile[0, compute_warp]
                ...
                fc2_alpha_value = cutlass.Float32(1.0)
                if cutlass.const_expr(fc2_alpha is not None):
                    fc2_alpha_value = ldg_f32_raw(
                        fc2_alpha.iterator.toint()
                        + cutlass.Int64(store_expert_idx)
                        * cutlass.Int64(4)
                    )
                for ng in cutlass.range_constexpr(0, n_groups):
                    acc = accumulators[None, None, ng]
                    scaled_acc0 = acc[0] * fc2_alpha_value
                    scaled_acc1 = acc[1] * fc2_alpha_value
                    scaled_acc2 = acc[2] * fc2_alpha_value
                    scaled_acc3 = acc[3] * fc2_alpha_value
                    token0 = (
                        compute_n_warp * cutlass.Int32(8)
                    ...
                    iket.range_push('sm120_fc2_store_bf16_pack')
                    if cutlass.const_expr(self.fc2_packed_store):
                        partner_lane = lane_idx ^ cutlass.Int32(4)
                        rFc2Store[0] = scaled_acc0.to(self.fc2_output_dtype)
                        rFc2Store[1] = cute.arch.shuffle_sync(scaled_acc0, partner_lane).to(self.fc2_output_dtype)
                        rFc2Store[2] = scaled_acc2.to(self.fc2_output_dtype)
                        rFc2Store[3] = cute.arch.shuffle_sync(scaled_acc2, partner_lane).to(self.fc2_output_dtype)
                    iket.range_pop()
                    if cutlass.const_expr(
                        self.k2_tile_trace_enabled and green_trace is not None
                    ):
                    ...
                    if valid_token0 < store_valid_tokens:
                        ...
                        if cutlass.const_expr(self.fc2_packed_store):
                            if lane_g & cutlass.Int32(1) == cutlass.Int32(0):
                                ...
                                iket.range_pop()
                                iket.range_push('sm120_fc2_store_stg_issue')
                            ...
                            iket.range_pop()
                            iket.range_push('sm120_fc2_store_stg_issue')
                            peer_row0[hidden0] = scaled_acc0.to(self.fc2_output_dtype)
                            peer_row0[hidden1] = scaled_acc2.to(self.fc2_output_dtype)
                            iket.range_pop()
                        else:
                            iket.range_push('sm120_fc2_store_metadata_address')
                            iket.range_pop()
                            iket.range_push('sm120_fc2_store_stg_issue')
                            fc2_output[fc2_store_token0, 0, hidden0] = scaled_acc0.to(self.fc2_output_dtype)
                            fc2_output[fc2_store_token0, 0, hidden1] = scaled_acc2.to(self.fc2_output_dtype)
                            iket.range_pop()
                    if cutlass.const_expr(self.fc2_packed_store):
                        # Keep shuffle_sync warp-uniform.  Reuse the row0 pack
                        # only after its predicated stores have completed.
                        partner_lane = lane_idx ^ cutlass.Int32(4)
                        rFc2Store[0] = scaled_acc1.to(self.fc2_output_dtype)
                        rFc2Store[1] = cute.arch.shuffle_sync(
                            scaled_acc1, partner_lane
                        ).to(self.fc2_output_dtype)
                        rFc2Store[2] = scaled_acc3.to(self.fc2_output_dtype)
                        rFc2Store[3] = cute.arch.shuffle_sync(
                            scaled_acc3, partner_lane
                        ).to(self.fc2_output_dtype)
                    if valid_token1 < store_valid_tokens:
                        ...
                        if cutlass.const_expr(self.fc2_packed_store):
                            ...
                            iket.range_pop()
                            iket.range_push('sm120_fc2_store_stg_issue')
                            peer_row1[hidden0] = scaled_acc1.to(self.fc2_output_dtype)
                            peer_row1[hidden1] = scaled_acc3.to(self.fc2_output_dtype)
                            iket.range_pop()
                        else:
                            iket.range_push('sm120_fc2_store_metadata_address')
                            iket.range_pop()
                            iket.range_push('sm120_fc2_store_stg_issue')
                            fc2_output[fc2_store_token1, 0, hidden0] = scaled_acc1.to(self.fc2_output_dtype)
                            fc2_output[fc2_store_token1, 0, hidden1] = scaled_acc3.to(self.fc2_output_dtype)
                            iket.range_pop()
                    if cutlass.const_expr(
                        self.k2_tile_trace_enabled and green_trace is not None
                    ):
        ...
        # The workers never wait on rank progress or another CTA's admission.
        if cutlass.const_expr(green_trace is not None):
            if warp_idx == cutlass.Int32(0):
                if lane_idx == cutlass.Int32(0):
