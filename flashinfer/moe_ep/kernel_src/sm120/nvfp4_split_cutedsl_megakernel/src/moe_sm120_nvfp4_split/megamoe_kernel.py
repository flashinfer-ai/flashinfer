...
from .kernel_fc12 import Sm120SwapABSwigluNvfp4Fc12Kernel
...
# =============================================================================
# Sm120MegaMoENvfp4SwapABKernel
# =============================================================================


class Sm120MegaMoENvfp4SwapABKernel(Sm120SwapABSwigluNvfp4Fc12Kernel):
    ...
    def __init__(
        ...
        *,
        ...
        if comm_backend != "p2p_direct" and dispatch_warps != 4:
            raise ValueError(
                "reduced dispatch_warps is currently supported only by "
                "p2p_direct"
            )
        if static_expert_shape is None:
            raise NotImplementedError(
                "Sm120MegaMoENvfp4SwapABKernel currently requires "
        ...
        if split_role not in ("k1", "k2"):
            raise ValueError(
                "split_role must be 'k1' or 'k2', got "
                f"{split_role!r}."
            )
        ...
        # The non-epi mode stages FC2 to a local workspace; epi_warps keeps
        # the epilogue STG redirect.
        if token_back_mode not in (
            "epi_warps", "reuse_dispatch_warps"
        ):
            raise ValueError(
                f"token_back_mode must be 'epi_warps' or "
        ...
        super().__init__(
            ...
            activation_dtype=cutlass.Float4E2M1FN,
        ...
        # Packed E2M1 stores two logical activations per byte.
        if self.hidden % 32 != 0:
            raise ValueError(
                f"SM120 NVFP4 requires hidden % 32 == 0; got {self.hidden}."
            )
        self.hidden_bytes = (
            self.hidden * cutlass.Float4E2M1FN.width + 7
        ) // 8
        ...
        if split_role == "k2" and compact_k2:
            ...
            # Keep the physical 4-warp setmaxnreg group intact. Warps beyond
            # dispatch_warps are idle for dispatch, but still participate in
            # the kernel-tail rendezvous as cohabitants.
            num_other_warps = 4 + 1 + 1 + 1 + 1 + (4 - dispatch_warps)
            token_comm_num_dispatch_warps = dispatch_warps
    ...
    def _build_local_region_specs(self) -> List[_RegionSpec]:
        ...
        specs: List[_RegionSpec] = [
            ...
            # as Uint8 bytes; the logical packed-E2M1 view at the same offset is
    ...
    def __call__(
        ...
        activation: cute.Tensor,           # logical (T, hidden) packed NVFP4
        ...
        # L1 token buffer: Uint8 view (dispatch byte arithmetic) + logical NVFP4
        ...
        Sm120SwapABSwigluNvfp4Fc12Kernel.__call__(self,
    ...
    def token_comm_hook_kernel_tail(
        ...
        *,
        ...
        if cutlass.const_expr(
            ...
            # cross-rank writes consumed by K3.
            pass
        else:
