...
def _resolve_split_tactic(
    ...
    *,
    ...
    if (
        ...
        # Backward-compatible explicit split_* representation. Materialize the
        # complete schema, then send it through the same validator/device check
        # as a knobs dict instead of maintaining a weaker parallel path.
        requested_tactic = {
        ...
        requested_tactic = config.knobs if isinstance(config.knobs, dict) else None

    from ......kernel_src.sm90 import pull_style_cutedsl_megakernel as pkg

    return pkg.resolve_hopper_mxfp4_split_tactic(
        requested_tactic,
...
class Sm90PullMxfp4MegaKernelBackend(MegaKernelBackend):
    ...
    def validate_init(
        ...
        if manual_selector:
            ...
            tactic = pkg.resolve_hopper_mxfp4_knobs(
    ...
    def _fused_frontend_config_kwargs(
        self, fleet_params: FleetParams
    ) -> dict[str, Any]:
        """Return the single complete fused frontend request."""
