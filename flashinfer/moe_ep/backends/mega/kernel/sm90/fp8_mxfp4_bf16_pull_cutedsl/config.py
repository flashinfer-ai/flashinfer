...
class Sm90_Fp8_Mxfp4_Bf16_PullCutedsl_MegaMoeConfig:
    ...
    # this correctness contract. Split normalizes these fields to its
    # previously validated role layout in __post_init__.
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.in_kernel_fc2_reduce, bool):
            raise ValueError("in_kernel_fc2_reduce must be a bool")
        ...
        if (
            ...
            and self.fold_producer_warps is True
        ...
        if self.execution_mode == "split":
            if self.dedup_dispatch not in (None, False):
                raise ValueError(
                    "split MXFP4 keeps the validated non-deduplicated dispatch ABI"
                )
            if self.active_dispatch_warps not in (None, 4):
                raise ValueError(
                    "split MXFP4 fixes K1 active_dispatch_warps=4; K2 has no "
                    "dispatch-warp tuning axis"
                )
            if self.fc1_store_offload not in (None, False):
                raise ValueError(
                    "split MXFP4 fixes fc1_store_offload=False for its Green "
                    "handoff protocol"
                )
            if self.fc1_early_done_publish not in (None, False):
                raise ValueError(
                    "split MXFP4 keeps fc1_early_done_publish=False until the "
                    "Green handoff protocol is separately validated"
                )
            if self.fold_producer_warps not in (None, False):
                raise ValueError(
                    "split MXFP4 fixes fold_producer_warps=False for its Green "
                    "handoff protocol"
                )
            if self.load_balance_mode not in (None, "static"):
                raise ValueError(
                    "concurrent split K1/K2 requires load_balance_mode='static'"
                )
            # Normalize omitted selectors to the already validated fixed split
            # protocol. Explicit matching values are accepted; mismatches fail
            # above instead of being silently overwritten.
            self.load_balance_mode = "static"
            self.dedup_dispatch = False
            self.active_dispatch_warps = 4
            self.fc1_store_offload = False
            self.fc1_early_done_publish = False
            self.fold_producer_warps = False
            if self.pingpong:
