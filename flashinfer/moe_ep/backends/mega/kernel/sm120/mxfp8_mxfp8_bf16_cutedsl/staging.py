"""Stage bf16 activations + routing into SM120 MXFP8 mega-MoE symmetric buffers.

Torch-composed staging only: the SM120 tree does not ship the fused
``DataPreprocess`` staging shim the sm100 tree has, so quantization
(``mxfp8_quantize_per_block_32_row``) and the routing repack are separate
stream-ordered ops. The live-token count is memoized on the workspace object
(``_staged_tokens``) for ``compute(output=None)``.
"""
...
def stage_mega_moe_inputs(
    ...
    *,
    ...
    """bf16 ``hidden_states`` → MXFP8 activation + plain K-major E8M0 block scales."""
    # Backend talks only to the swapab_cutedsl_megakernel shim (never src/).
    ...
    if num_tokens == 0:
        ...
        return
    if hidden % Mxfp8BlockSize != 0:
        raise ValueError(f"hidden_size must be a multiple of {Mxfp8BlockSize}.")
    ...
    q, sf = mxfp8_quantize_per_block_32_row(activation_fp32, data_dtype)
...
def validate_sm120_mxfp8_forward_inputs(
    hidden_states: torch.Tensor,
    topk_ids: torch.Tensor,
    topk_weights: torch.Tensor,
    fleet_params,
    *,
    ...
    """SM120 MXFP8 mega-path validation (bf16 staging or pre-staged fp8)."""
