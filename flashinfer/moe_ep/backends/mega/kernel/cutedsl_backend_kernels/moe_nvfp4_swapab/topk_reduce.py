...
# Tile-wise NVFP4 combine: the fc2 epilogue owns exactly 32 contiguous hidden per
# warp (the bf16 reorder's 64-reg layout = 2 tiles of 32), so the per-tensor fp32
# "global" scale is computed per-32-element tile -- each 32-cell is a complete,
# self-contained NVFP4 quantum (fp32 global + two per-16 e4m3 sfc).  A per-128
# global would need a cross-warp amax reduction the epilogue geometry does not have.
# Measured (make_nvfp4_input + nvfp4_reference_sum, no-score sum, bf16 out): g32 and
# g128 are within 0.05 dB (~20.5 dB), so the per-32 granule costs ~nothing on SNR.
NVFP4_GLOBAL_SCALE_BLOCK_SIZE = 32

NVFP4_E2M1_MAX = 6.0
FP8_E4M3FN_MAX = 448.0

# A tile-wise global must tile the per-16 sfc cleanly and must not straddle the
# per-thread receiver tile (NVFP4_HIDDEN_PER_THREAD), so each thread reads exactly
# one global column (topk_reduce_nvfp4_vec_kernel reads global_col once per thread).
assert NVFP4_GLOBAL_SCALE_BLOCK_SIZE % NVFP4_SFC_SCALE_BLOCK_SIZE == 0, (
    "NVFP4 global block must be a whole number of per-16 sfc sub-blocks."
)
assert NVFP4_HIDDEN_PER_THREAD % NVFP4_GLOBAL_SCALE_BLOCK_SIZE == 0, (
    "receiver per-thread tile must not straddle an NVFP4 global-scale boundary."
)
...
def nvfp4_pack6_views(
    plane: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Alias a PACK6 SF plane into the ``(global, sfc)`` 4D views the receiver reads.

    ``plane`` is the combine NVFP4 SF plane: ``uint8``, last dim ``N*8``, holding one
    little-endian 8-byte word per 32-element hidden tile ``g``::

        bytes 8g+0 .. 8g+3 = per-32 global fp32 word
        byte  8g+4         = per-16 sfc sub-block 0 (e4m3)
        byte  8g+5         = per-16 sfc sub-block 1 (e4m3)
        bytes 8g+6, 8g+7   = pad

    Returns ``(global_view, sfc_view)`` as LIVE strided views (never copied)::

        global_view (.., N, 2) fp32 -- [.., g, 0] = the per-32 fp32 word;
                                        [.., g, 1] = bytes 8g+4..8g+7 reinterpreted as
                                        fp32 (sfc|pad), aliased junk the kernel must NOT read.
        sfc_view    (.., N, 2) e4m3 -- [.., g, 0] = sfc0, [.., g, 1] = sfc1.

    This is the single host definition of the PACK6 unpack contract; both the mega
    runner and the topk_reduce tests consume it so the layout cannot drift.  The
    caller must keep ``plane`` alive for as long as the views are used.
    """
    *lead, w = plane.shape
    n_tiles = w // 8
    global_view = plane.view(torch.float32).reshape(*lead, n_tiles, 2)
    sfc_view = plane.view(torch.float8_e4m3fn).reshape(*lead, n_tiles, 8)[..., 4:6]
    return global_view, sfc_view


def make_nvfp4_pack6_input(
    src: torch.Tensor,
    *,
    poison_global_pad: bool = True,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """Quantize FP32 ``src`` to NVFP4 in the production PACK6 8-byte-aligned plane.

    Unlike :func:`make_nvfp4_input` (which returns the legacy 3D scales), this
    assembles the real interleaved SF plane the fc2 combine epilogue emits and
    aliases it with :func:`nvfp4_pack6_views`, exactly as the mega runner does.  The
    FP4 data and the per-32 fp32 / per-16 e4m3 scale *values* are reused from
    :func:`make_nvfp4_input` (its scalar math already matches the device epilogue), so
    this only adds the byte assembly + aliasing.

    Returns ``(q, sfc_view, global_view, plane)``.  ``q`` is the ``(T, K, H//2)``
    fp4x2 tensor; ``sfc_view`` / ``global_view`` are the 4D PACK6 views; ``plane`` is
    the backing ``uint8`` buffer.

    IMPORTANT: the caller MUST keep ``plane`` alive as long as ``sfc_view`` /
    ``global_view`` are used -- they are LIVE strided views into ``plane`` and will
    dangle if it is garbage-collected.  Bind it explicitly, e.g.
    ``q, sfc, glob, _plane = make_nvfp4_pack6_input(src)``.

    PACK6 carries exactly ``N = H // 32`` tiles, so ``H`` must be a multiple of 32.
    """
    H = src.shape[-1]
    if H % NVFP4_GLOBAL_SCALE_BLOCK_SIZE != 0:
        raise ValueError(
            "PACK6 requires the hidden dim to be a multiple of "
            f"{NVFP4_GLOBAL_SCALE_BLOCK_SIZE}; got H={H}."
        )

    q, sfc_fp8_3d, global_fp32_3d = make_nvfp4_input(src)
    T, K, _ = q.shape
    n_tiles = H // NVFP4_GLOBAL_SCALE_BLOCK_SIZE
    assert global_fp32_3d.shape[-1] == n_tiles, (
        f"global cols {global_fp32_3d.shape[-1]} != N={n_tiles}"
    )
    assert sfc_fp8_3d.shape[-1] == 2 * n_tiles, (
        f"sfc cols {sfc_fp8_3d.shape[-1]} != 2*N={2 * n_tiles}"
    )

    plane = torch.zeros((T, K, n_tiles, 8), dtype=torch.uint8, device=q.device)
    # bytes 0..3: little-endian fp32 global word for tile g.
    plane[..., 0:4] = (
        global_fp32_3d.reshape(T, K, n_tiles, 1).contiguous().view(torch.uint8)
    )
    # bytes 4,5: the two per-16 e4m3 sfc sub-blocks of tile g.
    plane[..., 4:6] = (
        sfc_fp8_3d.reshape(T, K, n_tiles, 2).contiguous().view(torch.uint8)
    )
    if poison_global_pad:
        # bytes 6,7 are pad.  Fill with a fixed non-zero pattern so global_view[..,g,1]
        # (= bytes 4..7 reinterpreted as fp32) is a known-bad value that would corrupt
        # the result if the kernel ever read it -- which it must not.
        plane[..., 6:8] = 0xEF
    plane = plane.reshape(T, K, n_tiles * 8).contiguous()

    global_view, sfc_view = nvfp4_pack6_views(plane)
    # Byte-fidelity self-check: the aliased views must round-trip the exact values
    # (catches endianness / byte-offset assembly bugs at construction time).
    assert torch.equal(global_view[..., 0], global_fp32_3d), "global word misassembled"
    assert torch.equal(
        sfc_view.to(torch.float32),
        sfc_fp8_3d.reshape(T, K, n_tiles, 2).to(torch.float32),
    ), "sfc bytes misassembled"
    return q, sfc_view, global_view, plane


def nvfp4_pack6_reference_sum(
    q: torch.Tensor,
    sfc_view: torch.Tensor,
    global_view: torch.Tensor,
    topk_score: Optional[torch.Tensor] = None,
) -> torch.Tensor:
    """K-ordered FP32 reduce of PACK6 NVFP4 input, read from the 4D views.

    Mirrors the kernel's per-element dequant ``x_hat = fp4 * sfc * global`` and
    indexing (``g = h//32``, ``sfc_sub = (h//16) - 2g`` in ``{0, 1}``).  It reads
    ``global_view[.., g, 0]`` ONLY -- never ``[.., g, 1]`` (aliased sfc|pad junk) -- so
    a mismatch means either this reference or the kernel touched the junk word.
    """
    unpacked = unpack_fp4_to_f32(q)
    T, K, H = unpacked.shape
    h = torch.arange(H, device=q.device)
    g_idx = h // NVFP4_GLOBAL_SCALE_BLOCK_SIZE
    sub_idx = h // NVFP4_SFC_SCALE_BLOCK_SIZE - 2 * g_idx
    expanded_sfc = sfc_view.to(torch.float32)[:, :, g_idx, sub_idx]
    expanded_global = global_view.to(torch.float32)[:, :, g_idx, 0]
    acc = torch.zeros((T, H), device=q.device, dtype=torch.float32)
    for k in range(K):
        contrib = unpacked[:, k, :] * expanded_sfc[:, k, :]
        if topk_score is not None:
...
# ---------------------------------------------------------------------------
# Faithful single-rounding-FMA host references (vectorized combine kernels).
#
# The vectorized kernels (topk_reduce_{bf16,mxfp8,nvfp4}_vec_kernel) accumulate
# with `cute.arch.fma_packed_f32x2` -- a hardware FMA that rounds the product
# AND the add as ONE operation (single rounding).  `torch.addcmul` on CUDA FP32
# is NOT fused: it rounds the product, then rounds the add (two roundings).  On
# any tie-break element the two land on adjacent FP32 grid points (~1 FP32 ULP),
# and after the BF16 store that becomes a full 1 BF16 ULP (~2^-7) disagreement
# -- far above 1e-5.  Empirically (GB200 sm_100, T=256,K=8,H=7168) the addcmul
# references mismatch the kernel on ~90/1.8e6 elements per FMA path, while the
# faithful references below match BIT-EXACTLY (0 mismatch) on every path.
#
# The trick: do every per-K step in FP64 but round the result back to FP32
# (`_f32_in_f64`) after each op the kernel rounds.  Because FP32*FP32 and the
# subsequent +acc are both exact in FP64 (for these magnitudes), `_f32_in_f64`
# of the fused expression reproduces the hardware single-rounding FMA exactly.
# The per-dtype dequant is vectorized; only the order-sensitive accumulate stays
# in a Python K-loop (`_ordered_fma_reduce`), shared by all three references.
# These validate the kernel's BF16 output at 1e-5 (in fact atol=0); the dtype
# cast to the caller's output dtype is left to the caller.
# ---------------------------------------------------------------------------


def _f32_in_f64(x: torch.Tensor) -> torch.Tensor:
    """A value living on the FP32 grid, stored in FP64 -- one primitive, two uses:

    * widen a low-precision input (bf16/fp8/fp4/e8m0): exact, since the value is
      already on a coarser-than-FP32 grid;
    * round an FP64 intermediate back onto the FP32 grid (RNE) after each op the
      kernel rounds: one call == one hardware rounding.

    FP32*FP32 and the following ``+acc`` are both exact in FP64 (for combine
    magnitudes), so applying this over a fused ``a*b + c`` reproduces
    ``fma_packed_f32x2``'s single rounding bit-for-bit, and over a bare ``a*b`` it
    reproduces ``mul_packed``.
    """
    return x.float().double()


def _ordered_fma_reduce(
    summand: torch.Tensor, score: Optional[torch.Tensor]
) -> torch.Tensor:
    """K-ordered single-rounding accumulate shared by every faithful_* reducer.

    ``summand`` is ``(T, K, H)`` FP64 holding each topk term right before the fused
    accumulate.  Folds over ``k`` in kernel order, reproducing the vec kernel's
    accumulator op-for-op (``round32`` == :func:`_f32_in_f64`)::

        score is None:  acc = round32(summand[k]            + acc)   # add/fma_packed
        else:           acc = round32(summand[k] * score[k] + acc)   # fma_packed

    The rounding *after each k* is what pins this to the kernel's reduction order;
    a tree/vectorized sum would round differently and break bit-exactness, so the
    Python loop is load-bearing, not laziness.  Returns FP32 (caller re-casts).
    """
    T, K, H = summand.shape
    score = None if score is None else _f32_in_f64(score)
    acc = summand.new_zeros((T, H))
    for k in range(K):
        addend = summand[:, k] if score is None else summand[:, k] * score[:, k, None]
        acc = _f32_in_f64(addend + acc)
    return acc.float()


def faithful_bf16_reference_sum(
    src: torch.Tensor,
    topk_score: Optional[torch.Tensor] = None,
) -> torch.Tensor:
    """BF16 reduce matching the vec kernel's add_packed / fma_packed exactly.

    no-score: acc = round32(acc + val)              (add_packed_f32x2)
    score:    acc = round32(val * score + acc)      (fma_packed_f32x2, 1 rounding)

    Neither path pre-rounds before the fused step, so the widened input is fed
    straight in as the summand.
    """
    return _ordered_fma_reduce(_f32_in_f64(src), topk_score)


def faithful_mxfp8_reference_sum(
    q: torch.Tensor,
    scale: torch.Tensor,
    topk_score: Optional[torch.Tensor] = None,
) -> torch.Tensor:
    """MXFP8 reduce matching topk_reduce_mxfp8_vec_kernel's op sequence exactly.

    no-score: acc = round32(val * scale + acc)              (fma_packed)
    score:    contrib = round32(val * scale)                (mul_packed)
...
def faithful_nvfp4_pack6_reference_sum(
    q: torch.Tensor,
    sfc_view: torch.Tensor,
    global_view: torch.Tensor,
    topk_score: Optional[torch.Tensor] = None,
) -> torch.Tensor:
    """NVFP4 PACK6 reduce matching topk_reduce_nvfp4_vec_kernel's ops exactly.

    contrib = round32(fp4 * sfc)                            (mul_packed)
    no-score: acc = round32(contrib * global + acc)         (fma_packed)
    score:    contrib = round32(contrib * global)           (mul_packed)
              acc = round32(contrib * score + acc)          (fma_packed)

    Reads global_view[.., g, 0] ONLY (never the aliased sfc|pad junk at [.., g, 1]).
    The (n_tiles, sub, per_sub) reshape maps each element h = g*32 + s*16 + i onto
    its per-32 global tile g and per-16 sfc sub-block s by broadcast -- the same
    element->scale mapping as a gather, but as cheap views.
    """
...
def topk_reduce_kernel(
    ...
    if h < Int32(hidden):
        ...
        for k in cutlass.range_constexpr(0, num_topk, 1):
            ...
            if cutlass.const_expr(topk_score is not None):
                score_val = Float32(topk_score[token_idx, Int32(k)])
                # Explicit single-rounding FFMA for `acc = contrib * score + acc`,
                # matching the vectorized kernels' fma_packed_f32x2.  Written as a
                # plain `contrib = contrib * score; acc = acc + contrib`, ptxas is
                # free to either contract into one FMA (1 rounding) or keep FMUL+FADD
                # (2 roundings) -- a BUILD-DEPENDENT choice that made the scalar bf16
                # output diverge by 1 bf16 ULP from a 2-rounding reference on some
                # toolchains (green on stock-torch CI, red in the nv-nightly container).
                # cute.arch exposes no scalar f32 FMA, so use the packed intrinsic with
                # a mirrored 2nd lane and take lane 0 -- bit-identical to the vec path
                # and deterministic across toolchains.
                # Original two-rounding-ambiguous form (kept for traceability):
                #     contrib = contrib * score_val
                #     acc = acc + contrib
...
def topk_reduce_nvfp4_vec_kernel(
    ...
    Scales arrive as the PACK6 8-B/tile INTERLEAVED plane delivered as 4D (T,K,N,2)
    views: for tile g (= global_col) global[..,g,0] is the per-32 fp32 word and
    sfc[..,g,{0,1}] are the two per-16 e4m3 sub-blocks.
    """
    ...
    if base_h < Int32(hidden):
        ...
        for k in cutlass.range_constexpr(0, num_topk, 1):
            # PACK6: global is the 4D (T,K,N,2) view; the per-32 fp32 word lives at
            # [.,.,g,0] (byte 8g of the per-tile [global4|sfc0|sfc1|pad2] b64).
            global_regs[k] = Float32(
                nvfp4_global_scale[token_idx, Int32(k), global_col, Int32(0)]
            )
        if cutlass.const_expr(topk_score is not None):
        ...
        for sfc_block_i in cutlass.range_constexpr(
            ...
            for k in cutlass.range_constexpr(0, num_topk, 1):
                ...
                # PACK6: sfc is the 4D (T,K,N,2) view; the two e4m3 sub-blocks of tile
...
def _validate_tensors(
    ...
    if mxfp8_scale is not None and (
        nvfp4_sfc_scale is not None or nvfp4_global_scale is not None
    ):
    ...
    if mxfp8_scale is None and not nvfp4_mode:
        ...
        if reduced_output.dtype != torch.bfloat16:
            raise TypeError(
                f"MXFP8 reduced_output must be torch.bfloat16, got "
        ...
        if nvfp4_sfc_scale.dtype != torch.float8_e4m3fn:
            raise TypeError(
                f"nvfp4_sfc_scale must be torch.float8_e4m3fn, got "
                f"{nvfp4_sfc_scale.dtype}."
            )
        if nvfp4_global_scale.dtype != torch.float32:
            raise TypeError(
                f"nvfp4_global_scale must be torch.float32, got "
        ...
        # PACK6 INTERLEAVED plane: both scales delivered as 4D (T, K, N, 2),
        # N = H//32 tiles.  global[..,g,0] = the per-32 fp32 word (byte 8g);
        # sfc[..,g,{0,1}] = the two per-16 e4m3 sub-blocks (bytes 8g+4 / 8g+5).
        # 2N == H//16 == sfc_cols.
        if nvfp4_sfc_scale.dim() != 4:
            raise ValueError(
                "nvfp4 scales must be the 4D PACK6 layout (T, K, N, 2); got "
                f"sfc dim={nvfp4_sfc_scale.dim()}."
            )
        n_tiles = global_cols
        expected_sfc_shape = (T, K, n_tiles, 2)
        expected_global_shape = (T, K, n_tiles, 2)
        if nvfp4_sfc_scale.shape != expected_sfc_shape:
            raise ValueError(
                f"packed nvfp4_sfc_scale shape must be {expected_sfc_shape}, "
                f"got {tuple(nvfp4_sfc_scale.shape)}."
            )
        if (
            nvfp4_global_scale.dim() != 4
            or nvfp4_global_scale.shape != expected_global_shape
        ):
            raise ValueError(
                f"packed nvfp4_global_scale shape must be "
                f"{expected_global_shape}, got "
...
def compile_topk_reduce(
    ...
    *,
    ...
    # = per-32 fp32 word; sfc[..,g,{0,1}] = the two per-16 e4m3 sub-blocks).
    if nvfp4_mode:
        assert nvfp4_sfc_scale.dim() == 4, (
            "nvfp4 scales must be 4D packed (T,K,N,2); got "
            f"sfc dim={nvfp4_sfc_scale.dim()}."
        )
    bf16_vectorized = (
        not nvfp4_mode
...
def benchmark_topk_reduce_vs_torch_sum(
    *,
    ...
    if use_mxfp8:
        ...
        # PACK6 8-byte-aligned tile-wise layout (the real combine-output format).
        # _nvfp4_plane MUST stay bound for the whole function: nvfp4_sfc_scale and
        # nvfp4_global_scale are LIVE strided views into it.
        (
            combine_output_ref,
            nvfp4_sfc_scale,
            nvfp4_global_scale,
            _nvfp4_plane,
        ) = make_nvfp4_pack6_input(combine_output_fp32)
        mxfp8_scale = None
    ...
    def reference_result(*, timed_baseline: bool) -> torch.Tensor:
        # timed_baseline=True  -> the torch reduction whose wall-clock we report as
        #   the perf comparison target; must stay a fast/representative torch op.
        # timed_baseline=False -> the ACCURACY reference; uses the faithful
        #   single-rounding-FMA references that reproduce the vec kernels'
        #   fma_packed_f32x2 bit-for-bit, so the 1e-5 gate below passes (atol=0 in
        #   practice) instead of tripping on a 1-bf16-ULP FMA-vs-addcmul tie-break.
        if use_mxfp8:
            if timed_baseline:
                return mxfp8_reference_sum(
                    combine_output_ref, mxfp8_scale, topk_score
                ).to(output_dtype)
            return faithful_mxfp8_reference_sum(
                combine_output_ref, mxfp8_scale, topk_score
            ).to(output_dtype)
        if use_nvfp4:
            if timed_baseline:
                return nvfp4_pack6_reference_sum(
                    combine_output_ref,
                    nvfp4_sfc_scale,
                    nvfp4_global_scale,
                    topk_score,
                ).to(output_dtype)
            return faithful_nvfp4_pack6_reference_sum(
                combine_output_ref,
                nvfp4_sfc_scale,
                nvfp4_global_scale,
                topk_score,
    ...
    # the faithful single-rounding-FMA emulation, which matches the kernel's FP32
    # accumulator bit-for-bit; after the BF16 store the kernel and reference land on
    # the same grid point, so 1e-5 (well below the bf16 ULP) passes deterministically.
    cmp_atol, cmp_rtol = 1e-5, 1e-5
    torch.testing.assert_close(
        topk_output, expected_result, atol=cmp_atol, rtol=cmp_rtol
    )
...
def _parse_bench_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Benchmark CuTeDSL topk_reduce against "
