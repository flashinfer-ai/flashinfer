...
class GluMxfp8Epilogue:
    ...
    def _run_fc2_subtile(
        ...
        *,
        ...
        When ``token_comm_args`` is not None (MegaMoE path), the STG is routed
        to ``combine_output[src_token, src_topk, :]`` on the source rank via
        ``Fc2OutputDest`` instead of the local ``real_fc2_output`` tile.
        """
        ...
        if token_row_in_cta < valid_tokens and hidden_col_start < valid_hidden:
            ...
            for stg_half in cutlass.range_constexpr(EpilogueTileN // 16):
                ...
                if cutlass.const_expr(
                    ...
                    # MegaMoE Form A: route the STG to combine_output[src_token,
                    # src_topk, :] on the source rank via Fc2OutputDest.
                    # For token_back_by_dispatch the epilogue writes to the local
