"""Offline knob tuner for the mixed NVFP4-weight/BF16-activation mega kernel.

Invoked through the :mod:`flashinfer.moe_ep.tune` CLI shim with
``--dtype bf16_nvfp4``. Kernel-specific pieces (dummy
...
def tune_one(args, rank: int, world_size: int, max_tokens: int) -> dict:
    ...
    try:
        y, l1, l2, symm_buffer = create_dummy_bf16_nvfp4_inputs(
            rank,
            world_size,
            args.num_experts,
            max_tokens,
