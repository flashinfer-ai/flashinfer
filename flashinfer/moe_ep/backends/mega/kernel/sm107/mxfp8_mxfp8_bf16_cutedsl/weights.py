"""Weight transform + validation for the SM107 mxfp8 GLU fprop mega kernel.
...
- FC1: logical ``(E, hidden, 2*intermediate)`` with hidden stride-1 and the N
  axis in 32-row gate/up pair stripes; flat atom-swizzled E8M0 SF plane of
...
def preprocess_mega_weights(
    ...
    *,
    ...
    # FC1: interleave the gate‖up halves into 32-row pair stripes, quantize
    # along K (hidden, the trailing dim), then expose the kernel's logical
    # (E, hidden, 2I) view with hidden stride-1.
    w13_interleaved = interleave_gate_up_32(
