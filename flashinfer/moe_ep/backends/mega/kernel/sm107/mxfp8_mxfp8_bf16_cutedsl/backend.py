"""SM107 (Rubin) mxfp8 GLU fprop mega-MoE kernel backend.

Wraps the vendored ``kernel_src/next_cutedsl_megamoe`` drop's fused
dispatch + FC1 + SwiGLU + FC2 + combine kernel
(``Sm107MegaMoEMxfp8GluKernel``) behind the ``MegaKernelBackend`` contract.
The backend talks only to the drop's package ``__init__`` (never ``src/``
directly), keeping ``import flashinfer.moe_ep`` CPU-safe.
"""
...
from ......core.runtime import sm107_mxfp8_glu_runtime_requirements
...
class Sm107Mxfp8GluMegaKernelBackend(MegaKernelBackend):
    ...
    def runtime_requirements(self, bootstrap: BootstrapConfig):
        return sm107_mxfp8_glu_runtime_requirements(bootstrap)
    ...
    def _allocate_workspace(self, fleet_params: FleetParams) -> Any:
        ...
        if k.cluster_shape_mnk is not None:
            kwargs["cluster_shape_mnk"] = k.cluster_shape_mnk
        return get_symm_buffer_for_sm107_mxfp8_glu_mega_moe(
            fp.num_experts,
    ...
    def compute(
        ...
        *,
        ...
        # Backend talks only to the next_cutedsl_megamoe shim (never src/
        # directly).
        from ......kernel_src.next_cutedsl_megamoe import sm107_mxfp8_glu_mega_moe
