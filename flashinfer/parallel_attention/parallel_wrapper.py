...
def ulysses_wrapper(func):
    ...
    def wrapper(self, query, key, value, tensor_layout, attn_mask=None, **kwargs):
        ...
        if ring_size == 1 and truncate_and_pad:
            ...
            # there is no ring, so we can use uneven_cp_config.seq_len to do truncate and pad
        ...
        if ring_size == 1 and varlen_cp_config.cu_seqlens_q_cur_ulysses_group is not None and result.shape[seq_dim] > varlen_cp_config.cu_seqlens_q_cur_ulysses_group[-1]:
            # Zero out padding using torch.narrow
            padding_part = torch.narrow(result, seq_dim, varlen_cp_config.cu_seqlens_q_cur_ulysses_group[-1], result.shape[seq_dim] - varlen_cp_config.cu_seqlens_q_cur_ulysses_group[-1])
            padding_part.zero_()

        result = ulysses_a2a_out(
            result, tensor_layout, ulysses_size=ulysses_size, ulysses_group=ulysses_group
        )
    ...
    def wrapper(self, query, key, value, tensor_layout, attn_mask=None, **kwargs):
        ...
        for i in range(ring_size):
            ...
            if uneven_cp_config.seq_len_cur_ring_group is not None:    
                ...
                if kv_inputs.shape[seq_dim + 1] != uneven_cp_config.seq_len_cur_ring_group[kv_rank]:
                    # Truncate kv_inputs using torch.narrow
                    kv_inputs = torch.narrow(
                        kv_inputs, seq_dim + 1, 0, uneven_cp_config.seq_len_cur_ring_group[kv_rank]
                    )
        ...
        if varlen_cp_config.cu_seqlens_q_cur_ring_group is not None and out.shape[out_seq_dim] > varlen_cp_config.cu_seqlens_q_cur_ring_group[rank][-1]:
