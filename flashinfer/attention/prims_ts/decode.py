...
def _validate_dtype_pair(
    ...
    if not supported:
        raise NotImplementedError(
            "attention-ts decode supports homogeneous FP16/BF16/FP8, "
            "BF16 Q with FP8 K/V, and BF16/FP8 Q with packed NVFP4 K/V; "
            f"got Q {q_dtype}, K/V {kv_dtype}, and output {output_dtype}"
        )
    ...
    *,
    ...
    if logical_head_dim is not None and inferred_head_dim != logical_head_dim:
        raise ValueError(
            ...
            f"head dimension {logical_head_dim}: got {storage_head_dim}"
        )
...
def _resolve_decode_launch_spec(
    ...
    kv_dtype = dtype_map[kv_dtype_key]
...
def _get_compiled_decode(
    ...
    kv_storage_dtype = cutlass.Uint8 if kv_dtype == cutlass.Float4E2M1FN else kv_dtype
    output_dtype = dtype_map[output_dtype_key]
    ...
    kv_storage_head_dim = (
        head_dim // 2 if kv_dtype == cutlass.Float4E2M1FN else head_dim
    )
    k_fake = cute.runtime.make_fake_tensor(
        kv_storage_dtype,
        (physical_pages, num_kv_heads, page_size, kv_storage_head_dim),
        stride=(
            k_outer_stride,
            page_size * kv_storage_head_dim,
            kv_storage_head_dim,
            1,
        ),
        assumed_align=16,
...
def _validate_decode_output_aliasing(
    ...
    *,
    ...
    _validate_out_does_not_overlap_inputs(
        ...
        ("k_sf_cache", runtime.k_sf_cache),
        ("v_sf_cache", runtime.v_sf_cache),
        ("seq_lens", seq_lens),
        ("qo_indptr", qo_indptr),
...
def prims_ts_batch_decode_with_kv_cache(
    ...
    *,
    ...
    ``[pages, Hkv, page_size, D]`` tensors. Packed NVFP4 K/V use ``torch.uint8``
    storage with width ``D / 2`` and require ``kv_scale_factors=(K_SF, V_SF)``
    FP8 tensors with width ``D / 16``. ``K_SF`` uses linear token-major layout;
    ``V_SF`` uses TRTLLM-GEN's 4-token interleaved layout.
    ``block_tables`` is an int32 CUDA tensor with shape ``[B, C]``.
    Entries are contiguous within each row while
    ...
    kv_scale_factors : tuple[torch.Tensor, torch.Tensor], optional
        ...
        have shape ``[pages, Hkv, page_size, D/16]``; K scales are token-major
    ...
    def plan(
        ...
        *,
        ...
        kv_data_type : torch.dtype, optional
            K/V storage dtype used to compile the plan. Defaults to
            ``q_data_type``. Use ``torch.uint8`` for packed NVFP4; ``head_dim``
            remains the logical dimension and each run supplies scale tensors.
...
def batch_decode_with_paged_kv_cache(
    ...
    *,
    ...
    (
        ...
        paged_kv_cache,
        expected_device=q.device,
        logical_head_dim=int(q.shape[-1]),
    )
    _validate_page_size(page_size)
