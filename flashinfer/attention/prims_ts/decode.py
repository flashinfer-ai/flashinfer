...
def _decode_policy_from_config(
    ...
    return (
        ...
        ("use_cga_smem_reduction", bool(config.use_cga_smem_reduction)),
...
def _make_decode_workspace_layout(
    ...
    uses_normalized_separate_workspace = partial_stats_shape == partial_o_shape[:-1]
    if uses_normalized_separate_workspace and output_dtype in (
        torch.bfloat16,
        torch.float8_e4m3fn,
    ):
...
def _read_packed_q_plan_metadata(
    ...
    """Validate cumulative Q offsets at plan time and return max/total Q."""
    ...
    return exact_max_seq_len_q, offsets[-1]
...
def _validate_packed_q_plan_values(
    ...
    *,
    ...
    derived_max_seq_len_q, total_q = _read_packed_q_plan_metadata(qo_indptr)
...
def _resolve_decode_launch_spec(
    ...
    with torch.cuda.device(device_index):
        cfg = make_decode_config(
            headdim=head_dim,
            args={"use_variable_seqlens_q": True} if use_packed_q else None,
            seq_len_q=seq_len_q,
...
def get_prims_ts_batch_decode_workspace_size(
    ...
    *,
    ...
    return _make_decode_workspace_layout(spec.scratch_shapes, out_dtype).total_bytes
...
def prims_ts_batch_decode_with_kv_cache(
    ...
    *,
    ...
    at ``query.shape[0]``, and have every delta at most ``max_seq_len_q``.
    ...
    layout = _make_decode_workspace_layout(spec.scratch_shapes, output_dtype)
...
class BatchDecodePagedTSWrapper:
    ...
    def plan(
        ...
        *,
        ...
        if qo_indptr is not None:
            ...
            derived_max_q, planned_total_q_tokens = _read_packed_q_plan_metadata(
                qo_indptr
            )
    ...
    def run(
        ...
        *,
        ...
        keep the final offset equal to the planned packed tensor extent.
        """
...
def batch_decode_with_paged_kv_cache(
    ...
    *,
    ...
    if qo_indptr is not None:
        ...
        if resolved_seq_len_q is None:
            derived_max_q, total_q = _read_packed_q_plan_metadata(qo_indptr)
