...
class TmemCorrResource(MlaResource):
    ...
    _cga_reduction_alloc: cutlass.Constexpr[Optional[SmemAllocation]] = None
    _cga_reduction_barrier_alloc: cutlass.Constexpr[Optional[SmemAllocation]] = None
    _smem_o: object = None
    _smem_o_i32: object = None
    _sum_scratch: object = None
    _cga_reduction_smem: object = None
    _cga_reduction_barrier: object = None

    def get_smem_requirements(self):
        """Return correction/output SMEM scratch and optional CGA buffers."""
        if self.inst_id != 1:
            return []
        ...
        if self.cfg.cga_reduction_smem_bytes and self.acc_o_tensor is None:
            if self._cga_reduction_alloc is None:
                self._cga_reduction_alloc = SmemAllocation(
                    name=f"{self.name}_cgaReduction",
                    size_bytes=self.cfg.cga_reduction_smem_bytes,
                    alignment=64,
                )
            if self._cga_reduction_barrier_alloc is None:
                self._cga_reduction_barrier_alloc = SmemAllocation(
                    name=f"{self.name}_cgaReductionBarrier",
                    size_bytes=8,
                    alignment=8,
                )
            allocs.append(self._cga_reduction_alloc)
            allocs.append(self._cga_reduction_barrier_alloc)
        return allocs

    @cute.jit
    def _init_store_state_from_context(self, context) -> None:
        """Create correction/output SMEM views and initialize CGA state."""
        ...
        if cutlass.const_expr(self._cga_reduction_alloc is not None):
            self._cga_reduction_smem = smem_array(
                context,
                self._cga_reduction_alloc,
                Int8,
                self.cfg.cga_reduction_smem_bytes,
            )
            self._cga_reduction_barrier = smem_array(
                context,
                self._cga_reduction_barrier_alloc,
                Int64,
                1,
            )
            self._init_cga_reduction_barrier()
    ...
    def initialize_runtime_state_internal(
        ...
        # CGA state is initialized explicitly by the kernel entry before the
        # cluster rendezvous.  This guarantees that every remote transaction
        # barrier is live before any peer can publish a partial.  Non-CGA
        # schedules retain the ordinary task-owned initialization path.
        if cutlass.const_expr(self.cfg.use_cga_reduction != 1):
            self._init_store_state_from_context(context)

    @cute.jit
    def create_cga_function_variables(self, context) -> None:
        """Bind CGA correction SMEM and initialize its transaction barrier."""
    ...
    @cute.jit
    def _init_cga_reduction_barrier(self):
        """Initialize the transaction barrier for this CTA's reduction slice."""
        if cutlass.const_expr(self.cfg.use_cga_reduction == 1):
            cfg = self.cfg
            warp_idx = cute.arch.make_warp_uniform(cute.arch.warp_idx())
            cta_rank = cute.arch.make_warp_uniform(cute.arch.block_idx_in_cluster())
            rows_per_slice = cfg.cga_reduction_rows_per_slice
            num_slices_per_cta = ceil_div(
                cfg.cga_reduction_slices, cfg.num_ctas_per_seq_kv
            )
            ...
            if warp_idx == Int32(cfg.correction_warp_idx):
                with cute.arch.elect_one():
                    prims.mbarrier_init(self._cga_reduction_barrier, 1)
                    prims.mbarrier_arrive_expect_tx(
                        self._cga_reduction_barrier, expected_bytes
                    )

    @cute.jit
    def _cga_rows_per_cta(self):
        """Return how many Q rows this CTA owns during CGA reduction."""

        num_slices_per_cta = ceil_div(
            self.cfg.cga_reduction_slices, self.cfg.num_ctas_per_seq_kv
        )
        return Int32(num_slices_per_cta * self.cfg.cga_reduction_rows_per_slice)

    @cute.jit
    def _cga_o_bytes(self):
        """Return the O bytes contributed by all split CTAs for this owner."""

        rows_per_cta = self._cga_rows_per_cta()
        return Int64(
    ...
    @cute.jit
    def _cga_remote_smem_ptr(self, owner_rank, byte_offset, dtype):
        """Map a local CGA SMEM address to the CTA that owns the row slice."""

        local_ptr = self._cga_local_smem_ptr(byte_offset, dtype)
        return prims.mapa(local_ptr, owner_rank)

    @cute.jit
    def _cga_local_smem_ptr(self, byte_offset, dtype):
        """Return a typed pointer into this CTA's CGA reduction SMEM buffer."""

        return cutlass.inttoptr(
            self._cga_reduction_smem.data_ptr().toint(Int64) + Int64(byte_offset),
            mem_space=3,
            dtype=dtype,
        )

    @cute.jit
    def _store_partial_o_to_cga_smem(
        self,
        local_row_idx,
        cta_idx_kv,
    ...
    def _store_partial_lse_to_cga_smem(self, local_row_idx, cta_idx_kv, lse_val):
        ...
        byte_offset = self._cga_o_bytes() + Int64(
    ...
    def _store_o_slice_to_gmem(
        ...
        for copy_segment_idx in cutlass.range_constexpr(num_copy_segments):
            ...
            if dst_row_idx < Int32(cfg.tile_size_q) and global_head_idx < Int32(
                ...
                if cutlass.const_expr(cfg.use_cga_reduction == 1):
                    self._store_partial_o_to_cga_smem(
                        dst_row_idx,
                        cta_idx_kv,
                    ...
                    byte_offset = Int64(
                        base_elem_offset * Int32(element_bytes)
    ...
    def _store_lse_to_gmem(
        ...
        if should_store_lse:
            ...
            if local_row_idx < Int32(self.cfg.tile_size_q) and head_idx < Int32(
                ...
                if cutlass.const_expr(self.cfg.use_cga_reduction == 1):
                    self._store_partial_lse_to_cga_smem(
    ...
    @cute.jit
    def _cga_lse_byte_offset(self, split_idx, owner_row_idx):
        rows_per_cta = self._cga_rows_per_cta()
        stats_elem_offset = split_idx * rows_per_cta * Int32(2) + owner_row_idx * Int32(
            2
        )
        return self._cga_o_bytes() + Int64(
            stats_elem_offset * Int32(self.cfg.acc_dtype_bytes)
        )

    @cute.jit
    def _cga_o_byte_offset(self, split_idx, owner_row_idx, dim_idx):
        rows_per_cta = self._cga_rows_per_cta()
        elem_offset = (
    ...
    @cute.jit
    def _cga_wait_transaction_barrier(self, warp_grp_thread_idx):
        """Wait for peer CGA stores before the owner reads local DSMEM."""
        ...
        cga_transaction_ready = prims.mbarrier_try_wait_parity(
            self._cga_reduction_barrier, 0, time_limit=0
        )
        if warp_grp_thread_idx == Int32(0):
            while not cga_transaction_ready:
                cga_transaction_ready = prims.mbarrier_try_wait_parity(
                    self._cga_reduction_barrier, 0, time_limit=10_000_000
                )
    ...
    @cute.jit
    def publish_neutral_cga_partial_and_reduce(
        self,
        batch_idx,
        head_base_idx,
        cta_idx_q,
        ...
        CGA rank still contributes the configured partial byte count so owner
        ...
        if cutlass.const_expr(self.cfg.use_cga_reduction == 1):
            ...
            if is_correction_warp:
                ...
                for slice_idx in cutlass.range_constexpr(cfg.cga_reduction_slices):
                    local_row_idx = (
                        Int32(slice_idx * cfg.cga_reduction_rows_per_slice)
                        + row_in_slice
                    )
                ...
                self._reduce_cga_partials_and_store_for_tile(
                    batch_idx,
                    head_base_idx,
                    cta_idx_q,
                    cta_idx_kv,
    ...
    @cute.jit
    def _reduce_cga_partials_and_store(
    ...
    @cute.jit
    def _reduce_cga_partials_and_store_for_tile(
        self,
        batch_idx,
        head_base_idx,
        cta_idx_q,
        ...
        """Reduce one configured-S CGA row-owner slice."""
        if cutlass.const_expr(self.cfg.use_cga_reduction == 1):
            cfg = self.cfg
            rows_per_slice = cfg.cga_reduction_rows_per_slice
            num_slices_per_cta = ceil_div(
            ...
            if valid_rows > Int32(0):
                self._cga_wait_transaction_barrier(warp_grp_thread_idx)
                    ...
                    if local_row_idx < Int32(cfg.tile_size_q):
                        ...
                        if global_head_idx < Int32(cfg.num_heads_q):
                            ...
                            for split_idx in cutlass.range_constexpr(
                                ...
                                lse_ptr = self._cga_local_smem_ptr(
                                    self._cga_lse_byte_offset(
                            ...
                            for split_idx in cutlass.range_constexpr(
                                ...
                                partial_ptr = self._cga_local_smem_ptr(
                                    self._cga_o_byte_offset(
        ...
        for chunk_idx in cutlass.range_constexpr(0, cfg.head_dim_per_stage_v // 2, 8):
            ...
            if global_head_idx < Int32(cfg.num_heads_q) and valid_output_row:
                if cutlass.const_expr(self.acc_o_tensor is not None):
                    ...
                    base_elem_offset = (
                        output_query_row * Int32(cfg.head_dim_v) + store_col_offset
                    )
                    base_ptr = self.o_tensor.iterator.raw_ptr().toint(Int64)
                    element_bytes = cfg.o_dtype_bytes
                dst_ptr = cutlass.inttoptr(
                    base_ptr + Int64(base_elem_offset * Int32(element_bytes)),
                    mem_space=1,
                    dtype=Int32,
                )
                if cutlass.const_expr(
        ...
        *,
        ...
        if cutlass.const_expr(
            ...
            and (
                ...
                or self.cfg.use_cga_reduction == 1
            )
        ):
    ...
    def _normalize_and_store_tail_o(
        ...
        *,
        ...
        if cutlass.const_expr(self.inst_id == 1):
            ...
            self._reduce_cga_partials_and_store(stage_info, task_cache, head_dim_offset)
