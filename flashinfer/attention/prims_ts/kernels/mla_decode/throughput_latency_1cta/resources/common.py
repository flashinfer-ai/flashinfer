...
# =====================================================================
# WorkIdThrottleResource — Dynamic scheduler pacing marker
# =====================================================================


@dataclass(kw_only=True)
class WorkIdThrottleResource(MemoryResource):
    """No-op named throttle resource for dynamic persistent work-id pacing."""

    # Producer side: the load task marks that its work-id slot can be reused.
    # There is no data payload; the resource exists to make the ordering edge
    # visible to TS.
    @producer_work
    @cute.jit
    def publish_work_id(self, stage_info: StageInfo):
        """Signal that the load task has yielded its work-id slot."""
        del stage_info
    ...
    @consumer_work
    @cute.jit
    def consume_work_id(self, stage_info: StageInfo):
        """Wait-side marker before the scheduler reuses a work-id slot."""
        del stage_info
