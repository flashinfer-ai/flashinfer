...
# measurements place the q16/q64 crossover below TRTLLM-gen's 1024-token
...
# when deciding whether the automatic CGA-reduction scratch can fit.
SM100A_SMEM_CAPACITY_BYTES = 232448
ESTIMATED_TS_BARRIER_BYTES = 512
MAX_CGA_SMEM_DATA_BYTES = SM100A_SMEM_CAPACITY_BYTES - ESTIMATED_TS_BARRIER_BYTES
...
def estimated_throughput_latency_smem_data_bytes(cfg: "MlaConfig") -> int:
    ...
    if cfg.cga_reduction_smem_bytes:
        total += align_up(cfg.cga_reduction_smem_bytes, 64)
        total += align_up(8, 8)
    return total
...
class MlaConfig:
    ...
    @property
    def cga_reduction_rows_per_slice(self) -> int:
        # One CGA-reduction SMEM slice is one 16-row x 128B tile.  The row count
    ...
    @property
    def cga_reduction_slices(self) -> int:
        return ceil(self.tile_size_q / self.cga_reduction_rows_per_slice)

    def cga_reduction_smem_bytes_for(self, num_ctas_kv: int) -> int:
        """Return multi-CTA KV CGA reduction SMEM footprint."""
        if num_ctas_kv <= 1:
            return 0
        rows_per_slice = self.cga_reduction_rows_per_slice
        num_slices = self.cga_reduction_slices
    ...
    @property
    def cga_reduction_smem_bytes(self) -> int:
        if self.use_multi_ctas_kv != 1 or self.use_cga_reduction != 1:
            return 0
        max_bytes = 0
        for num_ctas_kv in range(2, self.num_ctas_per_seq_kv + 1):
            max_bytes = max(max_bytes, self.cga_reduction_smem_bytes_for(num_ctas_kv))
        return max_bytes
...
def automatic_split_kv_step_tokens(
    *, qkv_dtype: str, tile_size_q: int, base_work: int
    ...
    exposes more than four CTAs.  Those FP8 factors are TS measurements rather
    than TRTLLM-gen constants. For non-divisible K, the final balanced split
...
def resolve_split_kv_reduction_policy(
    *,
    ...
    """Return whether the selected split-KV profile should use CGA reduction."""

    if reduction_mode not in (None, "auto", "cga", "gmem_separate"):
    ...
    if reduction_mode == "gmem_separate":
        return 0
    if reduction_mode == "cga":
        if profile.use_multi_ctas_kv != 1:
            raise ValueError("explicit CGA reduction requires a split-KV profile")
        if profile.kernel_variant == "keeps_mma_ab":
            raise ValueError(
                "explicit CGA reduction is not supported by keeps-MMA-AB profiles"
            )
        if profile.num_ctas_per_seq_kv > MAX_CGA_CLUSTER_SIZE:
            raise ValueError(
                "explicit CGA reduction exceeds the CUDA cluster-size limit: "
                f"split_kv={profile.num_ctas_per_seq_kv}, "
                f"max_cluster_size={MAX_CGA_CLUSTER_SIZE}"
            )
        return 1

    cga_capable_profile = (
        profile.use_multi_ctas_kv == 1
        and profile.use_cga_reduction == 1
        and profile.kernel_variant != "keeps_mma_ab"
        and profile.num_ctas_per_seq_kv <= MAX_CGA_CLUSTER_SIZE
    )
    if not cga_capable_profile:
        return 0
    ...
    modes = select_split_kv_modes(
        ...
        available_modes=("cga", "gmem_separate"),
    )
    return int_flag(modes[0] == "cga")


def cga_reduction_cluster_count(cfg: MlaConfig) -> int:
    """Return the number of clusters launched by the CGA reduction grid."""
...
def cga_reduction_cluster_shape(cfg: MlaConfig) -> tuple[int, int, int]:
    """Return the CGA reduction cluster shape for the main kernel launch."""

    return (cfg.num_ctas_per_seq_kv, 1, 1)


def resolve_auto_cga_reduction_mode(
    cfg: MlaConfig,
    *,
    reduction_mode: str | None,
    max_active_cga_clusters: int,
) -> str | None:
    """Return the runtime reduction mode after the auto-CGA capacity check."""

    if reduction_mode not in (None, "auto") or cfg.use_cga_reduction != 1:
        return reduction_mode

    if cga_reduction_cluster_count(cfg) > max_active_cga_clusters:
        return "gmem_separate"

    return reduction_mode


def resolve_runtime_cga_reduction_mode(
    cfg: MlaConfig | None,
    *,
    reduction_mode: str | None,
    hardware_info,
    ...
    """Query CGA occupancy and return the final runtime reduction mode."""

    if (
        cfg is None
        or reduction_mode not in (None, "auto")
        or cfg.use_cga_reduction != 1
    ):
        return reduction_mode

    cluster_shape = cga_reduction_cluster_shape(cfg)
    ...
    cluster_count = cga_reduction_cluster_count(cfg)
    if log is not None:
        log(
            "cga_reduction_occupancy",
            f"clusters={cluster_count}, active={active_clusters}",
        )

    resolved_mode = resolve_auto_cga_reduction_mode(
        cfg,
        reduction_mode=reduction_mode,
        max_active_cga_clusters=active_clusters,
    )
    if log is not None:
        if resolved_mode == "gmem_separate":
            ...
            log("cga_reduction", "enabled")
    return resolved_mode


def resolve_auto_cga_reduction_config(
    cfg: MlaConfig,
    *,
    reduction_mode: str | None,
) -> MlaConfig:
    """Disable auto CGA reduction when the static SMEM footprint is too large."""

    if reduction_mode not in (None, "auto") or cfg.use_cga_reduction != 1:
        return cfg

    if estimated_throughput_latency_smem_data_bytes(cfg) > MAX_CGA_SMEM_DATA_BYTES:
        return replace(cfg, use_cga_reduction=0)

    return cfg


def validate_explicit_cga_reduction_config(
    cfg: MlaConfig,
    *,
    reduction_mode: str | None,
) -> None:
    """Reject explicit CGA requests that exceed the SMEM launch budget."""

    if reduction_mode != "cga":
        return
    smem_data_bytes = estimated_throughput_latency_smem_data_bytes(cfg)
    if smem_data_bytes > MAX_CGA_SMEM_DATA_BYTES:
        raise ValueError(
            "explicit CGA reduction exceeds the SM100A shared-memory budget: "
            f"required_data_bytes={smem_data_bytes}, "
            f"max_data_bytes={MAX_CGA_SMEM_DATA_BYTES}"
        )
...
def auto_split_profile(
    *,
    ...
    return MlaProfile(
        ...
        use_cga_reduction=int_flag(split_kv > 1),
        kernel_variant=(
...
def make_throughput_latency_mla_config(
    *,
    ...
    use_cga_reduction = resolve_split_kv_reduction_policy(
    ...
    if use_cga_reduction == 1 and (
        num_heads_q < tile_size_q or num_heads_q % tile_size_q != 0
    ):
        if reduction_mode == "cga":
            raise ValueError(
                "CGA reduction requires every launched Q tile to contain a full "
                "tile_size_q rows: "
    ...
    config_kwargs: dict[str, Any] = dict(
        ...
        use_cga_reduction=use_cga_reduction,
    ...
    validate_explicit_cga_reduction_config(cfg, reduction_mode=reduction_mode)
    cfg = resolve_auto_cga_reduction_config(
        cfg,
        reduction_mode=reduction_mode,
    )
    return cfg
