...
def _make_clc_work_queue_and_throttle(
    ...
    work_id_throttle_pipeline_config = PipelineConfig.create_async_async_pipeline_cfg(
        num_stages=2,
        producer_group=load_group,
        consumer_group=scheduler_group,
        cta_layout_vmnk=cta_layout_vmnk,
    )
    work_id_pipeline_config = PipelineConfig.create_clc_fetch_async_pipeline_cfg(
        num_stages=2,
        num_bytes=16,
    ...
    return (
        ThroughputLatencyMlaClcWorkQueue(
            ...
            cache_seqs=cache_seqs,
            cu_seqlens_q=cu_seqlens_q,
            pipeline_config=work_id_pipeline_config,
            name=queue_name,
        ),
        WorkIdThrottleResource(
            pipeline_config=work_id_throttle_pipeline_config,
            name=throttle_name,
        ),
    )
...
def build_throughput_latency_mla_task_manager(
    ...
    *,
    ...
    if use_clc_dynamic:
        work_queue, work_id_throttle = _make_clc_work_queue_and_throttle(
            cfg,
            tile_sched_params,
            clc_response_ptr,
            cache_seqs,
    ...
    if work_queue is not None:
        ...
        deps = {
            ...
            work_queue: (
                work_queue_deps + [work_id_throttle]
        ...
        if use_page_offsets:
            ...
            deps[smem_kv].append(smem_page_offsets)
        if work_id_throttle is not None:
            deps[work_id_throttle] = [work_queue]
    ...
    # Keep the concrete correction resource reachable after TaskManager wraps
    # captured resources for lowering.  Scanning ``task_manager.resources``
    # with isinstance is not reliable inside a @cute.kernel trace.
    task_manager._mla_cga_corr_resource = corr1
    return task_manager
    ...
    *,
    ...
    if work_queue is not None:
        deps = {
            ...
            work_queue: (
                [work_queue, work_id_throttle] if work_id_throttle is not None else []
            ),
        }
        if use_page_offsets:
            deps[smem_page_offsets] = [work_queue]
    ...
    task_manager._mla_cga_corr_resource = corr
    return task_manager
...
class ThroughputLatencyMlaDecodeTs:
    ...
    def __init__(
        ...
        *,
        ...
        if cfg.use_multi_ctas_kv == 1 and cfg.use_cga_reduction != 1:
    ...
    def initialize_workspace(
        ...
        if cutlass.const_expr(workspace is not None):
            ...
            acc_lse_iter = cute.recast_ptr(
                workspace.iterator
                + cute.cosize(acc_o_layout) * (cutlass.Float16.width // 8),
    ...
    def __call__(
        ...
        use_gmem_reduction = cutlass.const_expr(
            cfg.use_multi_ctas_kv == 1 and cfg.use_cga_reduction != 1
        )
    ...
    def dense_kernel(
        ...
        task_manager = build_throughput_latency_mla_task_manager(
            cfg,
        ...
        cga_corr_resource = None
        if cutlass.const_expr(cfg.use_cga_reduction == 1):
            cga_corr_resource = task_manager._mla_cga_corr_resource
        task_manager.setup_resources_and_tasks()

        if cutlass.const_expr(cfg.use_cga_reduction == 1):
            resource_context = ResourceContext(
                smem_base=task_manager._smem_base,
                tmem_ptr_i32=task_manager._tmem_ptr_i32,
            )
            cga_corr_resource.create_cga_function_variables(resource_context)
        ...
        if cutlass.const_expr(has_runtime_activity_guard):
            if run_task_graph:
                ...
                if cutlass.const_expr(cfg.use_cga_reduction == 1):
                    ...
                    # configured CGA row-owner reduction duties.
                    cga_corr_resource.publish_neutral_cga_partial_and_reduce(
                        batch_idx,
                        head_idx,
                        cta_idx_q,
                        cta_idx_kv,
        ...
        if warp_idx == Int32(cfg.mma_warp_idx):
            tmem_arr_for_dealloc = prims.make_tmem_ptr(
                task_manager._tmem_ptr_i32.load(),
                Float32,
            )
