...
final packed offset remains equal to the planned query/output extent.
...
may cause incorrect results or out-of-bounds access.
...
and is exclusive to one in-flight launch or captured graph. The standalone
hot path does not copy metadata to the host; callers must maintain all page,
length, and packed-offset preconditions for every launch. These live values
are not fully value-checked by the launch.
