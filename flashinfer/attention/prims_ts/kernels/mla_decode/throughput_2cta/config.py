...
def max_skip_corr_threshold(qkv_dtype: str) -> float:
    ...
    A skipped correction lets the scaled P value grow by up to ``2**threshold``.

    * ``"e4m3"``: the enabled P scale is 1.75 and ``1.75 * 2**8 == 448`` is the
      E4M3 maximum, so 8 is both the bound and the recommended value.
    * ``"bf16"``: P is unscaled and shares the FP32 exponent range, so the bound
      is an engineering cap rather than a representability limit.  With
      ``2**20`` KV rows and ``|V| <= 2**10`` the FP32 O accumulator reaches at
      most ``2**(64 + 20 + 10) == 2**94``, 34 bits under the FP32 maximum.
      The reference leaves BF16 unbounded; the benefit saturates far below 64.
    """
...
class MlaDecodeConfig:
    ...
    # Threshold-based skip-correction specialization (``SkipCorrThreshold``
    # in the reference).  The threshold itself remains a runtime scalar; this
    # flag only adds the row-max-freeze and warp-vote instruction paths and is
    # legal for every QKV dtype.  With E4M3 P the enabled protocol also moves
    # the P scale from 448 to 1.75 (``1.75 * 2**8 == 448``); BF16 P stays
    # unscaled.  See ``max_skip_corr_threshold`` for the per-dtype bounds.
    ...
    # INT32 words, the ``RopeQuantUe8m0Sf`` variant, instead of the
    # FP32 ``amax / 448`` value.  Requires ``dsv4_fuses_inv_rope_fp8_quant``.
...
def make_mla_decode_config(
    ...
    # Source DSV4 BMM2 consumes one P[K128] operand with each V[D256] panel.
    pv_k = (
        mma_qk_tiler_mn[1]
        if is_dynamic_token_sparse
