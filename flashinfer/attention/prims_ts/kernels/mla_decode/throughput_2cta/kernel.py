...
from .resources import (
    ...
    prepare_dsv4_deferred_mbarriers,
    initialize_dsv4_deferred_mbarriers,
    routing_row_index,
)
...
def build_mla_decode_task_manager(
    ...
    # DSV4 paces the CLC WorkId path with its own W12-W15 -> W10 throttle
    ...
    use_dsv4_work_throttle = use_clc_dynamic and cfg.is_dynamic_token_sparse
    ...
    # DSV4 runs the S empty-barrier acquire cursor two stages ahead of the
    ...
    # DSV4 has a two-buffer P allocation but no P mbarrier: producer and
    ...
    if cfg.use_fp8_split_mma_schedule:
        ...
        smem_v = SmemVResource(
            ...
            # Raw V Gather4 reuses the K tensor map: with E4M3 it is a [128, 1]
            # box over the [head_dim, token] view, i.e. one H128 transaction,
            # and V issues the second H128 at coord0 + 256.  It is not a
            # PV-side [256, 64] descriptor.
    ...
    softmax_task = create_softmax_task(
        ...
        softmax_group_id=0,
        dsv4_schedule=cfg.is_dynamic_token_sparse,
    )
    if cfg.use_fp8_dual_softmax_schedule:
    ...
    correction_task = create_correction_task(
        ...
        # producer: dense FP8 publishes one O token per N-slice, while the
        # fused DSV4 task hands off the whole O tile.
        per_n_o_pipeline=(
            cfg.use_fp8_split_mma_schedule and not cfg.is_dynamic_token_sparse
        ),
        dsv4_early_corr=cfg.is_dynamic_token_sparse,
        work_queue=work_queue,
        task_class=non_interleaved_task_class,
        domain=task_domain,
    ...
    # ──────────────────────────────────────────────────────────────
    if cfg.use_fp8_split_mma_schedule:
        # DSV4's W9 cp.async -> W12-W15 sparse-index edge is a resource in its
        # own right, so TaskManager validates the six-stage acquire/wait/release
        # protocol instead of treating Gather4 coordinates as an untracked
        # GMEM side input.
...
class MlaDecodeTs:
    ...
    def __call__(
        ...
        if cutlass.const_expr(cfg.dsv4_fuses_inv_rope_fp8_quant):
            ...
            runtime_assert(
                ...
                "DSV4 output scale must be flat/compact",
            )
            ...
            runtime_assert(
                ...
                "DSV4 output scale extent must be divisible by 16*32 (FP32) "
                "or 16*8 (UE8M0)",
            )
            ...
            runtime_assert(
                ...
                "DSV4 output scale M must be pad4(T)",
            )
            runtime_assert(
        ...
        if cutlass.const_expr(cfg.is_dynamic_token_sparse):
            ...
            # K64, S128B layout lets one Gather4 replace four D64 x K32
            # quarter-tile issues.
            sparse_v_cta_tiler = (
        ...
        # Persistent DSV4 uses a tiled CLC launch: W10 owns the one-stage
    ...
    def split_kv_kernel(
        ...
        # Prefetch TMA descriptors from one elected lane of the MMA warp.  DSV4
        ...
        if cutlass.const_expr(use_clc_dynamic):
            # DSV4 uses a one-stage WorkId pipeline, hence a single 16-byte
        ...
        if exit_early:
            ...
            if cutlass.const_expr(use_clc_dynamic):
                work_queue_pipeline_config = (
                    PipelineConfig.create_clc_fetch_async_pipeline_cfg(
                        # DSV4 uses a one-stage WorkId pipeline.
                        num_stages=(1 if cfg.is_dynamic_token_sparse else 2),
                        num_bytes=16,
                        producer_group=pipeline.CooperativeGroup(pipeline.Agent.Thread),
                        consumer_group=pipeline.CooperativeGroup(
                            pipeline.Agent.Thread,
                            # CTA1 omits the leader-only MMA warp(s) and the
                            # leader-only scheduler warp. Exclude them from the
                            # cluster-wide empty-barrier arrival count.  DSV4
                            # fuses QK/PV into W8, so like BF16 it has two such
                            # warps; dense FP8 keeps a separate PV MMA warp.
                            (
                                cfg.threads_per_cta * cfg.num_mma_ctas
                                - (
                                    2
                                    if (
                                        cfg.is_dynamic_token_sparse
                                        or not cfg.use_fp8_split_mma_schedule
                                    )
                                    else 3
                                )
            ...
            # Initialize pipelines (creates mbarriers in SMEM).  DSV4 first
            # binds every active resource to one contiguous mbarrier block so
            # the deferred constructors reuse those pipeline objects without
            # emitting local init CFGs.
            if cutlass.const_expr(cfg.is_dynamic_token_sparse):
                prepare_dsv4_deferred_mbarriers(task_manager)
            task_manager.setup_resources_and_tasks()
            if cutlass.const_expr(cfg.is_dynamic_token_sparse):
                initialize_dsv4_deferred_mbarriers(task_manager)
