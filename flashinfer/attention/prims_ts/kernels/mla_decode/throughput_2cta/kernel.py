...
def build_mla_decode_task_manager(
    ...
    # DSV4 uses source's separate W12--W15 -> W10 throttle edge on the CLC
    # WorkId path. Dense BF16 keeps the existing generic W8-owned throttle.
    ...
    if cfg.is_dynamic_token_sparse:
        ...
        dsv4_page_offset_ring_pipeline_cfg = PipelineConfig(
            ...
            # normal per-lane arrival.  The AsyncLoad lowering uses the
            # equivalent single .noinc completion and is cheaper.
    ...
    # generated kernel.  Dense schedules keep the stock P pipeline.
    ...
    if cfg.use_fp8_split_mma_schedule:
        ...
        smem_v = SmemVResource(
            ...
            # [256, 64] descriptor.  The raw source-matched path must use the
            # K-map descriptor.
    ...
    softmax_task = create_softmax_task(
        ...
        softmax_group_id=0,
        source_schedule=cfg.is_dynamic_token_sparse,
    )
    if cfg.use_fp8_dual_softmax_schedule:
    ...
    correction_task = create_correction_task(
        ...
        source_early_corr=cfg.is_dynamic_token_sparse,
        work_queue=work_queue,
        task_class=non_interleaved_task_class,
        domain=task_domain,
...
class MlaDecodeTs:
    ...
    def __call__(
        ...
        # the separately configured throttle edge.
