...
from ..helpers.deferred_mbarrier import (
    DeferredPipelineMixin,
    initialize_deferred_mbarriers,  # noqa: F401  (re-exported for callers)
    prepare_deferred_mbarriers,  # noqa: F401  (re-exported for callers)
)
...
from ..helpers.ops import (
    ...
    clc_response_predicated,
    convert_f32_vector_to_bf16_satfinite,
    cp_async_cg_16b_l2_128b,
...
class HighThroughputMlaResource(DeferredPipelineMixin, MemoryResource):
...
def routing_row_index(cfg, tile_idx, logical_seq_len_q, cu_seqlens_q=None):
    ...
    Dense MLA metadata is per batch.  dynamic token sparse instead supplies one routing
...
# =====================================================================
# PageIndexRingResource -- W9 page-index cp.async ring
# =====================================================================


@dataclass(kw_only=True)
class PageIndexRingResource(HighThroughputMlaResource):
    """Stage dynamic token sparse sparse token indices in the W9 page-index cp.async ring.
    ...
    def _init_smem_state(self, stage_info: StageInfo) -> None:
        ...
        prologue and passed to their resources; the dynamic token sparse ring follows the
    ...
    def prefetch_pair(self, stage_info: StageInfo) -> None:
        ...
        cp_async_cg_16b_l2_128b(dst_0, src_0)
        cp_async_cg_16b_l2_128b(dst_1, src_1)
    ...
    def page_quad(self, offset: Int32) -> cutlass.Array:
        ...
        stage_base = self.state_src.consumer_work_stage * Int32(
            self.cfg.page_index_ring_entries_per_stage
        )
...
@dataclass(kw_only=True)
class WorkThrottleBarrierResource(DeferredPipelineMixin, MemoryResource):
    ...
    rank 0.  Some tasks (notably dynamic token sparse W12--W15) run the surrounding producer
...
@dataclass(kw_only=True)
class MlaWorkQueue(DeferredPipelineMixin, WorkQueue):
    ...
    sparse_topk_lens: Any = (
        None  # per-packed-Q sparse scan widths (dynamic token sparse)
    )
    ...
    @cute.jit
    def _work_tile_from_clc_response_if_valid(self, result_addr):
        """Decode a CLC response and read dynamic token sparse metadata only when valid."""

        m_idx, n_idx, l_idx, valid_i32 = clc_response_predicated(result_addr)
    ...
    def advance_tile(self, stage_info: StageInfo):
        ...
        if cutlass.const_expr(self.use_clc_dynamic):
            # dynamic token sparse predicates CLC coordinate decode and metadata reads on a
            ...
            if cutlass.const_expr(self.cfg.is_dynamic_token_sparse):
                return self._work_tile_from_clc_response_if_valid(stage_response_ptr)
...
class SmemQResource(HighThroughputMlaResource):
    ...
    def tma_load(self, stage_info: StageInfo) -> None:
        ...
        # In the dynamic token sparse split-load schedule every W12--W15 Gather4 warp enters
...
class SmemKResource(HighThroughputMlaResource):
        ...
        *,
        ...
        for quad_idx in cutlass.range_constexpr(4):
            ...
            page_quad = self.smem_page_offsets.load(
                page_offset_stage * Int32(cfg.page_index_ring_entries_per_stage)
    ...
    def _tma_load_sparse_from_ring(
        ...
        *,
        ...
        indices from a GMEM coordinate tensor, while the sparse kernel's latency-hiding
...
class TmemSResource(HighThroughputMlaResource):
    ...
    def _load_s_impl(
        ...
        *,
        ...
        # the sparse kernel's first sparse tile owns the fixed SWA slot range.  It is
        ...
        if cutlass.const_expr(cfg.is_dynamic_token_sparse):
            ...
            # the dynamic token sparse schedule is tuned around this order.
    ...
    def _finish_softmax_max_impl(
        ...
        *,
        ...
        Keeping this phase separate lets the dynamic token sparse schedule publish
    ...
    def finish_softmax_max(
        ...
        *,
        ...
        """Publishable row-max phase for dynamic token sparse early correction."""
                ...
                if cutlass.const_expr(cfg.dsv4_fuses_inv_rope_fp8_quant):
                    ...
                    for load_idx in cutlass.range_constexpr(num_tmem_loads):
                        for j in cutlass.range_constexpr(4):
                            ...
                            # dynamic token sparse uses the satfinite BF16 conversion; dense
            ...
            if cutlass.const_expr(self.partial_output is not None):
                ...
                for load_idx in cutlass.range_constexpr(num_tmem_loads):
                    for j in cutlass.range_constexpr(4):
                        ...
                        # dynamic token sparse uses the satfinite BF16 conversion; dense MLA
