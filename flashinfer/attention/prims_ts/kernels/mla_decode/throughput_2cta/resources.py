...
from ..helpers.ops import (
    ...
    st_global_u8,
    sub_ftz_f32,
    ue8m0_inv_scale_from_byte,
    ue8m0_scale_byte_from_scale,
)
...
# DSV4 initializes every active mbarrier from one staged W0 region after
# TaskManager assigns the unified-SMEM pointers, which relies on private
# CUTLASS pipeline internals.  Pin that surface at import time so a DSL
# upgrade fails loudly here instead of producing a wrong arrival count and
# a silent hang.
...
def _check_dsv4_private_pipeline_surface() -> None:
    import inspect
    import re

    recast_fields = frozenset(
        re.findall(
            r"new_mbarrier_array\.(\w+)\s*=",
            inspect.getsource(pipeline.MbarrierArray.recast_to_new_op_type),
        )
    )
    missing_helpers = [
    ...
    if recast_fields != _DEFERRED_MBARRIER_FIELDS or missing_helpers:
        raise ImportError(
            ...
            "the DSV4 deferred mbarrier init: MbarrierArray fields "
            f"{sorted(recast_fields ^ _DEFERRED_MBARRIER_FIELDS)} differ and/or "
            f"helpers {missing_helpers} are missing; update "
            "_make_deferred_mbarrier_array / _create_dsv4_deferred_pipeline"
        )


_check_dsv4_private_pipeline_surface()
...
def _make_deferred_mbarrier_array(
    ...
    The field set written here is checked at import time against
    ``MbarrierArray.recast_to_new_op_type``.  DSV4 initializes all active
...
def _create_dsv4_deferred_pipeline(resource, pipeline_config):
    ...
    raise ValueError(
        f"DSV4 deferred mbarrier init does not support pipeline type {pipeline_type}"
    )
...
def prepare_dsv4_deferred_mbarriers(task_manager):
    ...
    storage explicitly makes contiguity a DSV4 contract rather than an
    ...
    task_manager._dsv4_deferred_barrier_resources = barrier_resources
    task_manager._dsv4_deferred_barrier_storage = barrier_storage
    return barrier_storage


@cute.jit
def _initialize_dsv4_deferred_mbarrier_block(
    base_ptr,
...
def initialize_dsv4_deferred_mbarriers(task_manager) -> None:
    """Initialize the explicitly contiguous barrier block from W0 lanes."""

    barrier_resources = task_manager._dsv4_deferred_barrier_resources
    ...
    _initialize_dsv4_deferred_mbarrier_block(base_ptr, arrival_counts)


class Dsv4DeferredPipelineMixin:
    """Route pipeline construction through the DSV4 deferred-init path.
    ...
    def create_pipeline(self, pipeline_config):
        ...
        if cfg is not None and cfg.is_dynamic_token_sparse:
            return _create_dsv4_deferred_pipeline(self, pipeline_config)
        return super().create_pipeline(pipeline_config)


@dataclass(kw_only=True)
class HighThroughputMlaResource(Dsv4DeferredPipelineMixin, MemoryResource):
...
class Dsv4PageOffsetRingResource(HighThroughputMlaResource):
    """Stage DSV4 sparse token indices in the W9 page-index cp.async ring.

    One producer stage is ``int32[256]``: its two halves hold the two
    adjacent K128 sparse-token tiles, copied by W9 as four indices per lane
    with 16-byte ``cp.async``.  The ``AsyncLoad`` pipeline turns ``commit``
    into ``cp.async.mbarrier.arrive`` per W9 lane, so W12--W15 consumers
    wait on data completion rather than instruction issue.
    """
    ...
    def _init_smem_state(self, stage_info: StageInfo) -> None:
        ...
        Q/K/V/P are static ``cutlass.Array`` objects allocated in the kernel
        prologue and passed to their resources; the DSV4 ring follows the
        same model, so ``stage_info.context`` carries no unified ``smem_base``.
        """
        del stage_info
    ...
    def prefetch_pair(self, stage_info: StageInfo) -> None:
        ...
        Each lane copies indices ``(k_tile_base + loop_offset) * 128 + lane * 4``
        and the same address plus 128.  GMEM addresses are clamped to the last
        aligned four-index vector so the odd tail tile never reads past
        ``k_len``.
        """
        cfg = self.cfg
    ...
    def read_k_stage(self, stage_info: StageInfo) -> Int32:
        ...
        K and V consume different, identically populated W9 stages for each
        adjacent K128 pair.  Two labelled consumer operations keep both
        dependency segments recoverable from the captured schedule; the
        physical stage comes from the same consumer pipeline state.
        """
        del stage_info
...
@dataclass(kw_only=True)
class WorkThrottleBarrierResource(Dsv4DeferredPipelineMixin, MemoryResource):
    """Pace CLC schedule token reuse against the leader CTA's active MMA task.

    The barrier has no payload and its producer state advances only on CTA
    rank 0.  Some tasks (notably DSV4 W12--W15) run the surrounding producer
    task on both CTAs and gate only the signal, so the state itself is
    leader-owned as well; otherwise CTA rank 1 advances an uncommitted local
    cursor and deadlocks in ``producer_tail`` at kernel exit.
    """
...
@dataclass(kw_only=True)
class MlaWorkQueue(Dsv4DeferredPipelineMixin, WorkQueue):
    ...
    def advance_tile(self, stage_info: StageInfo):
        ...
        if cutlass.const_expr(self.use_clc_dynamic):
            # DSV4 predicates CLC coordinate decode and metadata reads on a
            # successful cancellation; dense MLA takes the public scheduler
...
class SmemQResource(HighThroughputMlaResource):
    ...
    def tma_load(self, stage_info: StageInfo) -> None:
        ...
        # In the DSV4 split-load schedule every W12--W15 Gather4 warp enters
        # the Q pipeline, but only the first warp (W12) issues the TMA.
        # Without this predicate each warp elects a lane and sends the same
        # TMA four times.
...
class SmemKResource(HighThroughputMlaResource):
    ...
    def tma_load_from_page_ring_pair(
        ...
        *,
        ...
        """Issue K from the pair-ring half selected by K-tile parity.

        Unlike ``k_tile_delta``, half selection changes neither the logical
        K index nor the producer pipeline stage.  Only the pair FSM uses this:
        its ``stage_info.loop_offset`` stays the full K-tile index while W9
        holds one selector stage across the even and odd tiles.
        """
    ...
    def _issue_sparse_ring_bundle(
        ...
        *,
        ...
        """Issue one K Gather4 bundle for a fixed tensor map.

        The descriptor stays fixed across this inlined bundle so ptxas can
        predicate each elected-lane TMA; a descriptor branch inside the
        elected region creates one divergent reconvergence region per
        transaction.
        """
        cfg = self.cfg
    ...
    def _tma_load_sparse_from_ring(
        ...
        *,
        ...
        """Issue the W12--W15 K Gather4 work from the W9 SMEM ring.

        Every load warp owns four page quads and emits all four D128 slices.
        This is a raw Gather4 path on purpose: CuTe's high-level atom takes
        indices from a GMEM coordinate tensor, while DSV4's latency-hiding
        contract needs the four indices to come from W9's cp.async ring.
        """
        ...
        # The K128 tile's sparse-token rows are split across the two CTA ranks
        # (64 each); each page quad lands as four D128 slices of the K stage.
        page_offset_half = (
            Int32(k_tile_delta)
            if cutlass.const_expr(page_offset_half is None)
            else page_offset_half
        )
        # The tile0 branch stays outside the unrolled bundle.  The two paths
        # are deliberately not merged through a descriptor-pointer phi, which
        # does not preserve tensor-map pointer semantics in the current CuTe
        # DSL lowering.
        if k_index == Int32(0):
...
class SmemVResource(HighThroughputMlaResource):
    ...
    sparse_tma_atom_compressed: Any = None
    sparse_tma_tensor_compressed: Any = None
    # DSV4 uses the K tensor map for both raw K and raw V Gather4
    # instructions.  V's own atom only supplies a descriptor address when no
    # raw descriptor pair is bound.
    ...
    def tma_load_from_page_ring_pair(
        ...
        *,
        ...
        """Issue V from the pair-ring half selected by K-tile parity."""
        self._tma_load_sparse_from_ring(
            stage_info,
            page_offset_stage=page_offset_stage,
            page_offset_half=Int32(stage_info.loop_offset) & Int32(1),
    ...
    def _issue_sparse_ring_bundle(
        ...
        *,
        ...
        """Issue one V Gather4 bundle for a fixed tensor map."""
        cfg = self.cfg
    ...
    def _tma_load_sparse_from_ring(
        ...
        *,
        ...
        """Issue the W12--W15 V Gather4 work from the W9 SMEM ring."""
        ...
        # V is multicast across the CTA pair, so its page quad has no CTA-rank
        # term; CTA rank selects interleaved V128 columns instead.  Each
        # W12--W15 warp issues eight page quads x two H256 slices, 4 * 8 * 2 =
        # 64 Gather4 transactions, which is what the 64 KiB completion barrier
        # expects; four quads would cover only 64 KV rows and deadlock.
        page_offset_half = (
            Int32(k_tile_delta)
            if cutlass.const_expr(page_offset_half is None)
            else page_offset_half
    ...
    def v_desc(self, stage_info: StageInfo, *, v_subtile_idx: cutlass.Constexpr[int]):
        ...
        if cutlass.const_expr(cfg.is_dynamic_token_sparse):
            # The PV descriptor is leading=0, stride=1024, S128B.  Raw Gather4
            # already places both H256 pieces at their S128B locations, so a
            # non-zero leading dimension makes tcgen05 consume a different V tile.
    ...
    def v_desc_n_major(
        ...
        *,
        ...
        if cutlass.const_expr(cfg.is_dynamic_token_sparse):
            # Keep the N-major PV path on the same descriptor
            # (leading=0, stride=1024, S128B).
            leading_byte_offset = cutlass.const_expr(0)
            stride_byte_offset = cutlass.const_expr(
                8 * (cfg.mma_pv_tiler[1] // cfg.num_mma_ctas) * cfg.qkv_dtype_bytes
            )
...
class TmemSResource(HighThroughputMlaResource):
    ...
    # E4M3 P: log2(1.75) with skip correction, log2(448) otherwise; BF16 P: 0.
    log2_softmax_p_scale: Any = None
    ...
    @cute.jit
    def preacquire_pair(self) -> None:
        """Acquire S[0] and S[1] once before W8's persistent loop.

        ``producer_state`` is the acquire cursor and ``producer_commit_state``
        the lagging QK commit cursor.  Opening both empty stages here keeps
        acquire exactly two tokens ahead without letting a cloned state escape
        captured control flow.
        """
    ...
    @producer_work(work_attrs=WorkAttr.AUXILIARY)
    @cute.jit
    def acquire_next_stage(self, stage_info: StageInfo) -> None:
        """Wait for ``producer_state + 1`` without moving the owned cursor.

        W8 uses this shifted tail acquire to observe the final softmax P
        store before PV consumes it.
        """
        del stage_info
        assert self.pipeline is not None
    ...
    @cute.jit
    def balance_commits_after_persistent_loop(self) -> None:
        """Emit W8's two final S commits after all persistent tiles."""
        assert self.pipeline is not None
        assert self.pipeline_config is not None
        assert self.pipeline_config.advance_on_acquire
    ...
    def finish_row_sum(
        ...
        *,
        ...
        if cutlass.const_expr(cfg.is_dynamic_token_sparse):
            ...
            # softmax sum; the first view fence stays attached to the packed
            # P store itself.
            prims.fence_proxy(
        ...
        if cutlass.const_expr(cfg.is_dynamic_token_sparse):
            ...
            # This slot carries old_max - new_max so the exp scale is
            # evaluated after the local reduction, not hoisted ahead of MUFU P.
            correction_factor = Float32(1)
            if cutlass.const_expr(cfg.enable_skip_correction):
                # Leave the identity factor in place when the max freeze made
                # old_max == new_max; skipping this MUFU is part of skip
                # correction's performance contract.
    ...
    def _load_s_impl(
        ...
        *,
        ...
        max3 = row_max_tile
        if cutlass.const_expr(cfg.is_dynamic_token_sparse):
            # Reduce sequential groups of four into independent ILP chains.
            # The dense path uses four strided 16-element chains; both give
            # the same max but expose different dependencies to ptxas, and
            # the DSV4 schedule is tuned around this order.
    ...
    def _finish_softmax_max_impl(
        ...
        *,
        ...
        Keeping this phase separate lets the DSV4 schedule publish
        ``(old_max, new_max)`` to Correction before the longer P
        exponentiation/store path.  The dense schedule immediately calls
        :meth:`_materialize_softmax_p_impl`.
        """
        ...
        if cutlass.const_expr(cfg.is_dynamic_token_sparse):
            # The M128 cta_group_2 result is laid out as a 2x2 warp group, so
            # W0 exchanges with W2 and W1 with W3.  Each S pipeline stage owns
            # a disjoint 128-float buffer.  The barrier slot is a runtime Int32
            # on purpose: barrier.cta.sync accepts a register barrier ID.
        ...
        if cutlass.const_expr(cfg.enable_skip_correction):
            # The user threshold is pre-divided by softmax_scale_log2 and
            # freezes a candidate row max whose raw-score increase fits inside
            # that bound.  P can then grow by at most 2^T, so the previous O
            # accumulation needs no rescale for this row.
    ...
    @cute.jit
    def _store_packed_p(self, stage_info: StageInfo, packed_p) -> None:
        """Store the 16 delayed E4M3x4 registers in the swizzled P layout."""
        cfg = self.cfg
        tidx = cute.arch.thread_idx()[0]
        num_compute_threads = cfg.num_compute_warps * cfg.threads_per_warp
        local_tidx = tidx % num_compute_threads
        ...
        # First view fence for the packed P store.  A second cfence plus
        # async-shared proxy fence follows S release, right before the global
        # row-sum reduction.
    ...
    def _materialize_softmax_p_impl(
        ...
        *,
        ...
        correction_factor = Float32(1)
        if cutlass.const_expr(cfg.is_dynamic_token_sparse):
            # exp2 is evaluated only after the four-way local sum.  Carry
            # max_diff through the existing task-local slot to keep that
            # program order.
        ...
        fma_bias = Float32(0)
        if cutlass.const_expr(cfg.is_fp8_qkv()):
            # The runtime scale carries log2(1.75) when skip correction is
            # enabled; the dense default keeps the folded log2(448) constant.
            fma_bias = (
                self.log2_softmax_p_scale
                if cutlass.const_expr(
                    cfg.is_dynamic_token_sparse or cfg.enable_skip_correction
                )
                else fp8_log2_quant_scale()
            )
        if cutlass.const_expr(cfg.is_dynamic_token_sparse):
            # Contract ``-max * scale + bias`` into one FFMA.FTZ before the
            # scalar first P pair.
        ...
        if cutlass.const_expr(cfg.is_dynamic_token_sparse):
            ...
            # Four prefetched FMAs.  The first pair is spelled as scalar FMA
            # to avoid later register-move lowering; the remaining pairs are
            # emitted as contractible affine2 so ptxas can form FFMA2.
            ...
            for i in cutlass.range_constexpr(0, 64, 2):
                # Twelve-element delayed conversion: convert one E4M3x4
                # register before every other even MUFU once the latency
                # window has passed; the final three conversions stay post-loop.
            ...
            self._store_packed_p(stage_info, packed_p)
        else:
            for i in cutlass.range_constexpr(0, 64, 2):
    ...
    def _finish_softmax_impl(
        ...
        *,
        ...
        """Finish row max and immediately materialize P (dense schedule)."""
        (
            qk_acc_regs,
            row_max,
            row_sum,
    ...
    def finish_softmax_max(
        ...
        *,
        ...
        """Publishable row-max phase for DSV4 early correction."""
    ...
    def materialize_softmax_p(
        ...
        *,
        ...
        """Exponentiate P after the early-correction commit."""
        return self._materialize_softmax_p_impl(
            stage_info,
            qk_acc_regs=qk_acc_regs,
            row_max=row_max,
...
class SmemPResource(HighThroughputMlaResource):
    ...
    def _store_p_impl(self, stage_idx, *, qk_acc_regs) -> None:
        ...
        if cutlass.const_expr(
            ...
            # The PV MMA owns one K128 P partition; each W0--W3 thread stores
            # four 16-B vectors into a 128-B S128B-swizzled row.
            row = local_tidx % Int32(64)
            warp_col = local_tidx // Int32(64)
            xor_col = local_tidx % Int32(8)
            for store_i in cutlass.range_constexpr(4):
    ...
    @producer_work(work_attrs=WorkAttr.AUXILIARY)
    @cute.jit
    def mark_p_fused_store(self, stage_info: StageInfo) -> None:
        """Represent TmemS's fused packed-P store in the resource DAG.

        The delayed-conversion schedule keeps the packed P registers local to
        TmemS, so the physical store happens inside ``materialize_softmax_p``.
        This zero-instruction work item keeps the logical tmem_s -> smem_p
        edge without adding a P mbarrier or duplicating the store.
        """
        del stage_info
    ...
    @consumer_work(returns=desc_p_base)
    @cute.jit
    def p_desc_prior_tile(self, stage_info: StageInfo):
        """Return P[(current K tile - 1) mod 2] for W8's loop PV."""
    ...
    @consumer_work(returns=desc_p_base)
    @cute.jit
    def p_desc_final_tile(self, stage_info: StageInfo):
        """Return P[(final K tile) mod 2] for W8's tail PV."""
        return self._p_desc_at_stage(
            cutlass.Int32(stage_info.loop_offset) & cutlass.Int32(1)
        )
...
class TmemCorrResource(HighThroughputMlaResource):
    ...
    def _store_stats_pair(
        ...
        *,
        ...
        """Store one two-float stats payload in TMEM."""
        cfg = self.cfg
        tidx = cute.arch.thread_idx()[0]
        local_tidx = tidx % (cfg.num_compute_warps * cfg.threads_per_warp)
        warp_id = local_tidx >> 5
        # One 32-column tcgen05 row per stage for the stats pair, starting at
        # correction_factor_offset.
    ...
    def store_early_stats(
        ...
        *,
        ...
        """Publish ``(old_max, new_max)`` before P is created."""
        self._store_stats_pair(
            stage_info,
            first=row_max_old,
            second=row_max_new,
        )
    ...
    @consumer_work
    @cute.jit
    def consume_early_head(self, stage_info: StageInfo) -> None:
    ...
    @cute.jit
    def load_early_stats(self, stage_info: StageInfo):
        """Load an early max pair and compute the O rescale factor."""
        cfg = self.cfg
        tidx = cute.arch.thread_idx()[0]
        local_tidx = tidx % (cfg.num_compute_warps * cfg.threads_per_warp)
        warp_id = local_tidx >> 5
    ...
    @consumer_work(returns=(row_sum, row_max, correction_factor, no_correction))
    @cute.jit
    def load_final_stats(self, stage_info: StageInfo):
    ...
    def rescale_o(
        ...
        if not skip_correction:
            ...
            if cutlass.const_expr(cfg.dsv4_fuses_inv_rope_fp8_quant):
                # The fence wrapper is itself one tcgen05.wait::st: complete
                # issued stores once, and do no TMEM completion work when
                # correction was skipped.
                cute.arch.fence_view_async_tmem_store()

        if cutlass.const_expr(not cfg.dsv4_fuses_inv_rope_fp8_quant):
            # Keep the existing completion protocol outside the fused RoPE
            # quant path.
...
class GmemOResource(HighThroughputMlaResource):
    ...
    def epilogue_store(self, stage_info: StageInfo) -> None:
        ...
        if cutlass.const_expr(cfg.dsv4_fuses_inv_rope_fp8_quant):
            # Resolve the inverse-RoPE cache row before the TMEM loads; inside
            # the D384 store branch it puts three dependent global loads plus
            # the first cos/sin miss on the exposed epilogue path.  The row is
            # CTA-uniform (one query token per work tile), so two L1 lines cover
            # every thread; padded rows keep a clamped index and never fault.
            _, rope_query_len = query_batch_bounds(
                self.cu_seqlens_q,
                batch_idx,
                self.logical_seq_len_q,
            )
            rope_position = (
                ...
                - Int32(rope_query_len)
                + Int32(logical_q_idx)
            )
            rope_cos_sin_addr = (
                self.dsv4_inv_rope_cos_sin_cache.iterator.raw_ptr()
                + Int64(rope_position) * Int64(64)
            ).toint(Int64)
            if g_j == Int32(128):
                prefetch_global_l1(rope_cos_sin_addr)
                prefetch_global_l1(rope_cos_sin_addr + Int64(128))
        ...
        for iter_n in cutlass.range_constexpr(cfg.iterations_pv_n):
            ...
            # The RoPE D384 stage loads the upper half [64:128] first and
            # rotates it while only 64 accumulators are live; with all 128
            # loaded, the 144-register budget cannot keep cos/sin loads in
            # flight.
            rope_stage = cutlass.const_expr(
                cfg.dsv4_fuses_inv_rope_fp8_quant and iter_n == 1
            )
            load_halves = ((2, 3), (0, 1)) if rope_stage else ((0, 1, 2, 3),)
            for half_idx in cutlass.range_constexpr(len(load_halves)):
                half = load_halves[half_idx]
                for chunk_idx in cutlass.range_constexpr(len(half)):
                    load_idx = half[chunk_idx]
                ...
                for chunk_idx in cutlass.range_constexpr(len(half)):
                    chunk_base = half[chunk_idx] * TCGEN05_32B_REGS_PER_LOAD
                    for i in cutlass.range_constexpr(
                        chunk_base, chunk_base + TCGEN05_32B_REGS_PER_LOAD, 2
                    ):
                ...
                if cutlass.const_expr(rope_stage and half_idx == 0):
                    # Inverse RoPE covers D[448:512], the upper half of the
                    # D384 quant block (g_j == 128 here); D256 rows pass
                    # through.  The cache row is [cos(32), sin(32)], consumed
                    # in two batches of eight 128-bit loads then 16 rotations.
                    # Padded rows are never stored; the guard only skips loads.
                    for rope_batch_idx in cutlass.range_constexpr(2):
                        value_base = 64 + rope_batch_idx * 32
                        first_0 = qk_acc_regs[value_base + 0]
                        second_0 = qk_acc_regs[value_base + 1]
                        first_1 = qk_acc_regs[value_base + 2]
                        second_1 = qk_acc_regs[value_base + 3]
                        first_2 = qk_acc_regs[value_base + 4]
                        second_2 = qk_acc_regs[value_base + 5]
                        first_3 = qk_acc_regs[value_base + 6]
                        second_3 = qk_acc_regs[value_base + 7]
                        first_4 = qk_acc_regs[value_base + 8]
                        second_4 = qk_acc_regs[value_base + 9]
                        first_5 = qk_acc_regs[value_base + 10]
                        second_5 = qk_acc_regs[value_base + 11]
                        first_6 = qk_acc_regs[value_base + 12]
                        second_6 = qk_acc_regs[value_base + 13]
                        first_7 = qk_acc_regs[value_base + 14]
                        second_7 = qk_acc_regs[value_base + 15]
                        first_8 = qk_acc_regs[value_base + 16]
                        second_8 = qk_acc_regs[value_base + 17]
                        first_9 = qk_acc_regs[value_base + 18]
                        second_9 = qk_acc_regs[value_base + 19]
                        first_10 = qk_acc_regs[value_base + 20]
                        second_10 = qk_acc_regs[value_base + 21]
                        first_11 = qk_acc_regs[value_base + 22]
                        second_11 = qk_acc_regs[value_base + 23]
                        first_12 = qk_acc_regs[value_base + 24]
                        second_12 = qk_acc_regs[value_base + 25]
                        first_13 = qk_acc_regs[value_base + 26]
                        second_13 = qk_acc_regs[value_base + 27]
                        first_14 = qk_acc_regs[value_base + 28]
                        second_14 = qk_acc_regs[value_base + 29]
                        first_15 = qk_acc_regs[value_base + 30]
                        second_15 = qk_acc_regs[value_base + 31]
                        rotated_first_0 = first_0
                        rotated_second_0 = second_0
                        rotated_first_1 = first_1
                        rotated_second_1 = second_1
                        rotated_first_2 = first_2
                        rotated_second_2 = second_2
                        rotated_first_3 = first_3
                        rotated_second_3 = second_3
                        rotated_first_4 = first_4
                        rotated_second_4 = second_4
                        rotated_first_5 = first_5
                        rotated_second_5 = second_5
                        rotated_first_6 = first_6
                        rotated_second_6 = second_6
                        rotated_first_7 = first_7
                        rotated_second_7 = second_7
                        rotated_first_8 = first_8
                        rotated_second_8 = second_8
                        rotated_first_9 = first_9
                        rotated_second_9 = second_9
                        rotated_first_10 = first_10
                        rotated_second_10 = second_10
                        rotated_first_11 = first_11
                        rotated_second_11 = second_11
                        rotated_first_12 = first_12
                        rotated_second_12 = second_12
                        rotated_first_13 = first_13
                        rotated_second_13 = second_13
                        rotated_first_14 = first_14
                        rotated_second_14 = second_14
                        rotated_first_15 = first_15
                        rotated_second_15 = second_15
                        if (
                            row_in_tile < physical_tile_rows
                            and query_is_valid
                            and g_j == Int32(128)
                        ):
                            batch_addr = rope_cos_sin_addr + Int64(rope_batch_idx * 64)
                            cos_0, cos_1, cos_2, cos_3 = ld_global_v4_f32(
                                batch_addr + Int64(0)
                            )
                            cos_4, cos_5, cos_6, cos_7 = ld_global_v4_f32(
                                batch_addr + Int64(16)
                            )
                            cos_8, cos_9, cos_10, cos_11 = ld_global_v4_f32(
                                batch_addr + Int64(32)
                            )
                            cos_12, cos_13, cos_14, cos_15 = ld_global_v4_f32(
                                batch_addr + Int64(48)
                            )
                            sin_0, sin_1, sin_2, sin_3 = ld_global_v4_f32(
                                batch_addr + Int64(128)
                            )
                            sin_4, sin_5, sin_6, sin_7 = ld_global_v4_f32(
                                batch_addr + Int64(144)
                            )
                            sin_8, sin_9, sin_10, sin_11 = ld_global_v4_f32(
                                batch_addr + Int64(160)
                            )
                            sin_12, sin_13, sin_14, sin_15 = ld_global_v4_f32(
                                batch_addr + Int64(176)
                            )
                            sin_prod_0 = mul_packed_f32x2(
                                (second_0, first_0), (sin_0, sin_0), ftz=True
                            )
                            rotated_0 = fma_packed_f32x2(
                                (first_0, second_0),
                                (cos_0, cos_0),
                                (sin_prod_0[0], -sin_prod_0[1]),
                                ftz=True,
                            )
                            rotated_first_0 = rotated_0[0]
                            rotated_second_0 = rotated_0[1]
                            sin_prod_1 = mul_packed_f32x2(
                                (second_1, first_1), (sin_1, sin_1), ftz=True
                            )
                            rotated_1 = fma_packed_f32x2(
                                (first_1, second_1),
                                (cos_1, cos_1),
                                (sin_prod_1[0], -sin_prod_1[1]),
                                ftz=True,
                            )
                            rotated_first_1 = rotated_1[0]
                            rotated_second_1 = rotated_1[1]
                            sin_prod_2 = mul_packed_f32x2(
                                (second_2, first_2), (sin_2, sin_2), ftz=True
                            )
                            rotated_2 = fma_packed_f32x2(
                                (first_2, second_2),
                                (cos_2, cos_2),
                                (sin_prod_2[0], -sin_prod_2[1]),
                                ftz=True,
                            )
                            rotated_first_2 = rotated_2[0]
                            rotated_second_2 = rotated_2[1]
                            sin_prod_3 = mul_packed_f32x2(
                                (second_3, first_3), (sin_3, sin_3), ftz=True
                            )
                            rotated_3 = fma_packed_f32x2(
                                (first_3, second_3),
                                (cos_3, cos_3),
                                (sin_prod_3[0], -sin_prod_3[1]),
                                ftz=True,
                            )
                            rotated_first_3 = rotated_3[0]
                            rotated_second_3 = rotated_3[1]
                            sin_prod_4 = mul_packed_f32x2(
                                (second_4, first_4), (sin_4, sin_4), ftz=True
                            )
                            rotated_4 = fma_packed_f32x2(
                                (first_4, second_4),
                                (cos_4, cos_4),
                                (sin_prod_4[0], -sin_prod_4[1]),
                                ftz=True,
                            )
                            rotated_first_4 = rotated_4[0]
                            rotated_second_4 = rotated_4[1]
                            sin_prod_5 = mul_packed_f32x2(
                                (second_5, first_5), (sin_5, sin_5), ftz=True
                            )
                            rotated_5 = fma_packed_f32x2(
                                (first_5, second_5),
                                (cos_5, cos_5),
                                (sin_prod_5[0], -sin_prod_5[1]),
                                ftz=True,
                            )
                            rotated_first_5 = rotated_5[0]
                            rotated_second_5 = rotated_5[1]
                            sin_prod_6 = mul_packed_f32x2(
                                (second_6, first_6), (sin_6, sin_6), ftz=True
                            )
                            rotated_6 = fma_packed_f32x2(
                                (first_6, second_6),
                                (cos_6, cos_6),
                                (sin_prod_6[0], -sin_prod_6[1]),
                                ftz=True,
                            )
                            rotated_first_6 = rotated_6[0]
                            rotated_second_6 = rotated_6[1]
                            sin_prod_7 = mul_packed_f32x2(
                                (second_7, first_7), (sin_7, sin_7), ftz=True
                            )
                            rotated_7 = fma_packed_f32x2(
                                (first_7, second_7),
                                (cos_7, cos_7),
                                (sin_prod_7[0], -sin_prod_7[1]),
                                ftz=True,
                            )
                            rotated_first_7 = rotated_7[0]
                            rotated_second_7 = rotated_7[1]
                            sin_prod_8 = mul_packed_f32x2(
                                (second_8, first_8), (sin_8, sin_8), ftz=True
                            )
                            rotated_8 = fma_packed_f32x2(
                                (first_8, second_8),
                                (cos_8, cos_8),
                                (sin_prod_8[0], -sin_prod_8[1]),
                                ftz=True,
                            )
                            rotated_first_8 = rotated_8[0]
                            rotated_second_8 = rotated_8[1]
                            sin_prod_9 = mul_packed_f32x2(
                                (second_9, first_9), (sin_9, sin_9), ftz=True
                            )
                            rotated_9 = fma_packed_f32x2(
                                (first_9, second_9),
                                (cos_9, cos_9),
                                (sin_prod_9[0], -sin_prod_9[1]),
                                ftz=True,
                            )
                            rotated_first_9 = rotated_9[0]
                            rotated_second_9 = rotated_9[1]
                            sin_prod_10 = mul_packed_f32x2(
                                (second_10, first_10), (sin_10, sin_10), ftz=True
                            )
                            rotated_10 = fma_packed_f32x2(
                                (first_10, second_10),
                                (cos_10, cos_10),
                                (sin_prod_10[0], -sin_prod_10[1]),
                                ftz=True,
                            )
                            rotated_first_10 = rotated_10[0]
                            rotated_second_10 = rotated_10[1]
                            sin_prod_11 = mul_packed_f32x2(
                                (second_11, first_11), (sin_11, sin_11), ftz=True
                            )
                            rotated_11 = fma_packed_f32x2(
                                (first_11, second_11),
                                (cos_11, cos_11),
                                (sin_prod_11[0], -sin_prod_11[1]),
                                ftz=True,
                            )
                            rotated_first_11 = rotated_11[0]
                            rotated_second_11 = rotated_11[1]
                            sin_prod_12 = mul_packed_f32x2(
                                (second_12, first_12), (sin_12, sin_12), ftz=True
                            )
                            rotated_12 = fma_packed_f32x2(
                                (first_12, second_12),
                                (cos_12, cos_12),
                                (sin_prod_12[0], -sin_prod_12[1]),
                                ftz=True,
                            )
                            rotated_first_12 = rotated_12[0]
                            rotated_second_12 = rotated_12[1]
                            sin_prod_13 = mul_packed_f32x2(
                                (second_13, first_13), (sin_13, sin_13), ftz=True
                            )
                            rotated_13 = fma_packed_f32x2(
                                (first_13, second_13),
                                (cos_13, cos_13),
                                (sin_prod_13[0], -sin_prod_13[1]),
                                ftz=True,
                            )
                            rotated_first_13 = rotated_13[0]
                            rotated_second_13 = rotated_13[1]
                            sin_prod_14 = mul_packed_f32x2(
                                (second_14, first_14), (sin_14, sin_14), ftz=True
                            )
                            rotated_14 = fma_packed_f32x2(
                                (first_14, second_14),
                                (cos_14, cos_14),
                                (sin_prod_14[0], -sin_prod_14[1]),
                                ftz=True,
                            )
                            rotated_first_14 = rotated_14[0]
                            rotated_second_14 = rotated_14[1]
                            sin_prod_15 = mul_packed_f32x2(
                                (second_15, first_15), (sin_15, sin_15), ftz=True
                            )
                            rotated_15 = fma_packed_f32x2(
                                (first_15, second_15),
                                (cos_15, cos_15),
                                (sin_prod_15[0], -sin_prod_15[1]),
                                ftz=True,
                            )
                            rotated_first_15 = rotated_15[0]
                            rotated_second_15 = rotated_15[1]
                        qk_acc_regs[value_base + 0] = rotated_first_0
                        qk_acc_regs[value_base + 1] = rotated_second_0
                        qk_acc_regs[value_base + 2] = rotated_first_1
                        qk_acc_regs[value_base + 3] = rotated_second_1
                        qk_acc_regs[value_base + 4] = rotated_first_2
                        qk_acc_regs[value_base + 5] = rotated_second_2
                        qk_acc_regs[value_base + 6] = rotated_first_3
                        qk_acc_regs[value_base + 7] = rotated_second_3
                        qk_acc_regs[value_base + 8] = rotated_first_4
                        qk_acc_regs[value_base + 9] = rotated_second_4
                        qk_acc_regs[value_base + 10] = rotated_first_5
                        qk_acc_regs[value_base + 11] = rotated_second_5
                        qk_acc_regs[value_base + 12] = rotated_first_6
                        qk_acc_regs[value_base + 13] = rotated_second_6
                        qk_acc_regs[value_base + 14] = rotated_first_7
                        qk_acc_regs[value_base + 15] = rotated_second_7
                        qk_acc_regs[value_base + 16] = rotated_first_8
                        qk_acc_regs[value_base + 17] = rotated_second_8
                        qk_acc_regs[value_base + 18] = rotated_first_9
                        qk_acc_regs[value_base + 19] = rotated_second_9
                        qk_acc_regs[value_base + 20] = rotated_first_10
                        qk_acc_regs[value_base + 21] = rotated_second_10
                        qk_acc_regs[value_base + 22] = rotated_first_11
                        qk_acc_regs[value_base + 23] = rotated_second_11
                        qk_acc_regs[value_base + 24] = rotated_first_12
                        qk_acc_regs[value_base + 25] = rotated_second_12
                        qk_acc_regs[value_base + 26] = rotated_first_13
                        qk_acc_regs[value_base + 27] = rotated_second_13
                        qk_acc_regs[value_base + 28] = rotated_first_14
                        qk_acc_regs[value_base + 29] = rotated_second_14
                        qk_acc_regs[value_base + 30] = rotated_first_15
                        qk_acc_regs[value_base + 31] = rotated_second_15

            # Store O to GMEM
            if row_in_tile < physical_tile_rows and query_is_valid:
                if cutlass.const_expr(cfg.dsv4_fuses_inv_rope_fp8_quant):
                    # Physical output layout: [H/8, T, 8, D].
                    ...
                    # Block amax floor: 1e-12 for FP32 scales, 1e-10 for UE8M0
                    # scales.
                    amax_eps = Float32(
                        1.0e-10 if cfg.dsv4_uses_ue8m0_scale_o else 1.0e-12
                    )
                    max0 = amax_eps
                    max1 = amax_eps
                    ...
                    amax = max3_abs_ftz_f32(max0, max1, amax_eps)
                    scale_buf_m = (total_q + Int64(3)) & Int64(-4)
                    if cutlass.const_expr(cfg.dsv4_uses_ue8m0_scale_o):
                        # Packed UE8M0: byte b of INT32 word
                        # [(g * 8 + h) * scale_buf_m + t] is block b's exponent
                        # (block 0 in the LSB); the bytes come from two threads x
                        # two iter_n, so each is stored alone.  scale is the power
                        # of two >= amax / 448; satfinite E4M3 absorbs overflow.
                        scale_byte = ue8m0_scale_byte_from_scale(
                            amax * Float32(1.0 / 448.0)
                        )
                        inv_scale = ue8m0_inv_scale_from_byte(scale_byte)
                        scale_byte_offset = (
                            (group_idx * Int64(8) + head_idx_in_group) * scale_buf_m
                            + Int64(storage_q_idx)
                        ) * Int64(4) + quant_block_idx
                        st_global_u8(
                            self.dsv4_o_scale.iterator.raw_ptr().toint(Int64)
                            + scale_byte_offset,
                            scale_byte,
                        )
                    else:
                        scale_offset = (
                            ...
                            + (head_idx_in_group * Int64(4) + quant_block_idx)
                            * scale_buf_m
                            + Int64(storage_q_idx)
                        )
                    ...
                    for load_idx in cutlass.range_constexpr(num_tmem_loads):
                        ...
                        for pack_idx in cutlass.range_constexpr(
                            ...
                            # Packed RN multiplies halve the issue slots of
                            # the scale step; per lane they are the same
                            # mul.rn.f32 as the scalar form.
                            scaled_lo = mul_packed_f32x2(
                                (vec_f32[pack_offset], vec_f32[pack_offset + 1]),
                                (inv_scale, inv_scale),
                            )
                            scaled_hi = mul_packed_f32x2(
                                (vec_f32[pack_offset + 2], vec_f32[pack_offset + 3]),
                                (inv_scale, inv_scale),
                            )
                            packed_o[pack_idx] = pack_float4_to_fp8_e4m3(
                                scaled_lo[0], scaled_lo[1], scaled_hi[0], scaled_hi[1]
                            )
                    ...
                    for load_idx in cutlass.range_constexpr(num_tmem_loads):
                        for j in cutlass.range_constexpr(4):
                            ...
                            # DSV4 uses the satfinite BF16 conversion; dense
                            # MLA keeps its round-to-nearest conversion.
                            if cutlass.const_expr(self.cfg.is_dynamic_token_sparse):
                    ...
                    for load_idx in cutlass.range_constexpr(num_tmem_loads):
                        for j in cutlass.range_constexpr(2):
                            ...
                            if cutlass.const_expr(cfg.use_fp8_output == 1):
                                ...
                                if cutlass.const_expr(
                                    self.cfg.is_dynamic_token_sparse
                                    and output_dtype(self.cfg) is cutlass.BFloat16
                                ):
            ...
            if cutlass.const_expr(self.partial_output is not None):
                ...
                for load_idx in cutlass.range_constexpr(num_tmem_loads):
                    for j in cutlass.range_constexpr(4):
                        ...
                        # DSV4 uses the satfinite BF16 conversion; dense MLA
                        # keeps its round-to-nearest conversion.
                        if cutlass.const_expr(self.cfg.is_dynamic_token_sparse):
                            vec_partial = convert_f32_vector_to_bf16_satfinite(
                                vec_f32, BF16_OUTPUT_VECTOR_ELEMENTS
                            )
