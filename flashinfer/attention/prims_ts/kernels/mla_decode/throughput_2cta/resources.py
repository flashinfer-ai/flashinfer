...
class TmemSResource(HighThroughputMlaResource):
    ...
    def _finish_softmax_impl(
        ...
        *,
        ...
        if cutlass.const_expr(cfg.is_fp8_qkv()):
            # Match the 448-scaled E4M3 P convention used by TRTLLM-gen and
...
class TmemCorrResource(HighThroughputMlaResource):
    ...
    def prepare_epilogue_slice_store(self, stage_info: StageInfo):
        ...
        # The two CTAs in the CGA own complementary halves of the 2CTA row.
            ...
            if effective_head_idx < effective_num_heads_q and query_is_valid:
                if cutlass.const_expr(self.partial_output is not None):
                    ...
                    o_base_ptr = (
                        ...
                        + effective_head_idx * (self.split_kv * D)
                        + split_kv_idx * D
                        + seq_q_idx * (self.split_kv * effective_num_heads_q * D)
                        + batch_idx * (effective_num_heads_q * self.split_kv * S_q * D)
                    )
                    ...
                    if cutlass.const_expr(self.cu_seqlens_q is not None):
                        o_base_ptr = (
                            self.output.iterator.raw_ptr() + storage_flat_query_row * D
                        )
                    else:
                        S_q = (
                        ...
                        o_base_ptr = (
                            ...
                            + logical_head_idx * D
                            + logical_q_idx * (logical_num_heads_q * D)
                            + batch_idx * (logical_num_heads_q * D * S_q)
                        )
                    output_base = o_base_ptr + iter_n * tile_d + g_j
                    for load_idx in cutlass.range_constexpr(num_tmem_loads):
            ...
            if effective_lse_head_idx < effective_num_heads_q and lse_query_is_valid:
                if cutlass.const_expr(self.partial_lse is not None):
                    ...
                    lse_base_ptr = (
                        ...
                        + effective_lse_head_idx * self.split_kv
                        + split_kv_idx
                        + seq_q_idx * (effective_num_heads_q * self.split_kv)
                        + batch_idx * (effective_num_heads_q * self.split_kv * S_q)
                    )
                    ...
                    if cutlass.const_expr(self.cu_seqlens_q is not None):
                        ...
                        lse_base_ptr = (
                            ...
                            + logical_lse_head_idx
                            + logical_lse_q_idx * logical_num_heads_q
                            + batch_idx * (logical_num_heads_q * S_q)
                        )
                    lse_base_ptr.store(lse)
