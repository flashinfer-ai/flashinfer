...
class MlaDsv4SourceSHandoffTask(MlaTask):
    """Persistent W8 executor with the reference cross-work-tile S state machine.
...
def create_load_page_offsets_dsv4_task(
    ...
    **task_kwargs,
    ...
    The reference commits two identical ``int32[256]`` stages *per adjacent
...
def create_mma_dsv4_fused_task(
    ...
    **task_kwargs,
    ...
    def mma_dsv4_fused_body(smem_q, smem_k, smem_v, smem_p, tmem_s, tmem_o):
        ...
        # Source K128 BMM2 makes the two D256 MMA calls under one whole-D512
        # O pipeline token, and P readiness rides on the S handoff rather
        # than a P mbarrier.
        with domain_loop(loop_start, loop_end, loop_step):
            staged_qk_mma_k_tile_from_acquired_s(smem_k, tmem_s, iterations_qk)
...
def create_softmax_task(
    ...
    **task_kwargs,
    ...
    ``source_schedule`` selects the generated DSV4 CSA protocol: S is held
    until P is stored and fenced, P readiness rides on the S handoff instead
    of a P mbarrier, softmax publishes (old_max, new_max) early-correction
    tokens plus one terminal (row_sum, row_max) token, and the packed-P store
    is fused into TmemS's software pipeline.
    """
    loop_start, loop_end, loop_step = captured_loop_bounds(task_kwargs, 0)
    use_clc_dynamic = bool(work_queue is not None and work_queue.use_clc_dynamic)
    if source_schedule and softmax_group_id != 0:
        raise ValueError("the source schedule supports the DSV4 softmax group only")
    ...
    def softmax_body(tmem_s, tmem_corr, smem_p, softmax_state):
        ...
        with domain_loop(loop_start, loop_end, loop_step):
            ...
            if source_schedule:
                ...
                # S release is then the P-ready signal; the P buffer follows
                # logical K-tile parity with no P mbarrier.
