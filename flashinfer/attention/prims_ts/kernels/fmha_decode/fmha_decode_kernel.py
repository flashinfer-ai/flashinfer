...
def _build_decode_gen_schedule(
    ...
    if cfg.tile_size_q == 128 and use_distributed_split_kv_stages:
        ...
        # 165-KiB staged footprint of the corresponding TRTLLM-gen kernel.
    ...
    # Note: with cfg.keeps_stats_via_smem the stats-alias justification no
    # longer applies, but a measured B200 A/B (Qwen B32/SQ8, 2026-07-09)
    # showed the shared FIFO still regresses Q128 by ~10 points, so the
    # instruction-local FIFO gate is retained on its own merit.
    ...
    if use_clc_dynamic:
        ...
        wq_pipeline_config = PipelineConfig.create_clc_fetch_async_pipeline_cfg(
            num_stages=_PERSISTENT_WORK_ID_STAGES,
            num_bytes=16,
        ...
        work_id_throttle = WorkIdThrottleResource(
    ...
    scheduler_task = None
    if use_clc_dynamic:
        scheduler_task = create_scheduler_task(work_queue, work_id_throttle, cfg)
    ...
    if work_queue is not None:
        ...
        resource_dependency_graph[work_queue] = (
            [work_queue, work_id_throttle]
            if work_id_throttle is not None
            else ([work_queue] if use_clc_dynamic else [])
        )
    if work_id_throttle is not None:
        resource_dependency_graph[work_id_throttle] = [work_queue]
...
def _run_decode_gen_active(
    ...
    if cutlass.const_expr(use_clc_dynamic_scheduler):
        clc_response_ptr = cute.arch.alloc_smem(
            cutlass.Int128, _PERSISTENT_WORK_ID_STAGES
        )
    ...
    task_manager._tmem_ptr_i32 = tmem_ptr_i32

    task_manager.setup_resources_and_tasks()
    resource_context = ResourceContext(
        smem_base=task_manager._smem_base,
    ...
    for resource in correction_init_resources:
        ...
        # start. In CGA transaction-barrier mode this initializes each owner
...
@cute.jit
def _run_decode_gen_inactive_cga_rank() -> None:
    """Join CGA initialization, then retire an inactive physical split rank."""
...
def _run_decode_gen_runtime_prefix(
    ...
    if cutlass.const_expr(cfg.supports_cga_smem_reduction):
        if split_is_active:
            ...
            _run_decode_gen_inactive_cga_rank()
    else:
        if split_is_active:
            _run_decode_gen_active(
                tma_desc_q,
...
def fmha_decode_launch(
    ...
    if cutlass.const_expr(use_clc_dynamic):
        ...
        tile_sched_params = None
        grid = (grid_x, h_k, b)
    # CGA distributed-SMEM reduction groups the splits_kv split CTAs
    ...
    cluster_shape = [1, 1, 1]
    if cutlass.const_expr(cfg.use_cga_smem_reduction):
