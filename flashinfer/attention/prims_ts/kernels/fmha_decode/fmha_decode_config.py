...
from .fmha_decode_constants import (
    ...
    FP16_VALUES_PER_REG,
    MAX_CGA_CLUSTER_DIM_X,
    MAX_CGA_PARTIAL_SMEM_BYTES,
...
def make_grouped_q_launch_candidate(
    ...
    *,
    ...
    The model follows TRTLLM-gen's measured GQA-generation structure:

    ``(mainloop_factor[TileQ] * seq_len_per_cta_kv +``
    `` reduction_factor[TileQ] * 128 * splits_kv) * waves``.

    Unlike the experimental JSON cost model, this is a small deterministic
    default heuristic.  It deliberately uses the same factors for every dtype;
    the production profile validator still determines which actual-dtype
    recipes are legal.
    """
    if splits_kv <= 0:
...
class FmhaDecodeConfig:
    ...
    # PageOffsetsTask (paged-KV only): warp 14 prefetches logical→physical
    ...
    use_cga_smem_reduction: bool = False
    ...
    def ordered_softmax_early_release(self) -> bool:
        """Whether the P0/P1 baton is released at STTM issue.
        ...
        at STTM issue overlaps that tail with the partner's wakeup while the
    ...
    @property
    def can_use_cga_smem_reduction(self) -> bool:
        """Whether the configured split profile is eligible for CGA reduction."""
        ungrouped_q_layout = (
            not self.groups_tokens_heads_q
            and not self.use_variable_seqlens_q
            and self.max_seq_len_q == 1
    ...
    @property
    def supports_cga_smem_reduction(self) -> bool:
        """Whether CGA reduction is both selected and eligible."""
        return self.use_cga_smem_reduction and self.can_use_cga_smem_reduction
    ...
    @property
    def cga_reduction_rows_per_cta(self) -> int:
    ...
    @property
    def cga_reduction_num_owner_ctas(self) -> int:
    ...
    @property
    def cga_max_runtime_partial_rows(self) -> int:
        """Maximum slice-aligned ``split x owner-row`` records at runtime."""
        if not self.supports_cga_smem_reduction:
            return self.max_splits_kv * self.cga_reduction_rows_per_cta
        rows_per_slice = self.split_reduction_rows_per_slice
        num_slices = (self.tile_size_q + rows_per_slice - 1) // rows_per_slice
        return max(
    ...
    @property
    def cga_transaction_bytes(self) -> int:
        """Return the byte count expected by each CGA owner barrier."""
        row_bytes = (
            self.headdim * PARTIAL_O_ELEMENT_BYTES
            + PARTIAL_STATS_VALUES_PER_ROW * FP32_BYTES
        )
        return self.cga_max_runtime_partial_rows * row_bytes
    ...
    def supports_grouped_keeps(self) -> bool:
        ...
        if (
            ...
            or self.use_cga_smem_reduction
        ):
            return False
...
def cga_smem_reduction_partial_smem_bytes(
    *,
    max_splits_kv: int,
    tile_size_q: int,
    headdim: int,
    ...
    """Bytes staged per owner CTA for every runtime CGA split prefix.
...
def cga_smem_reduction_unsupported_reason(
    *,
    max_splits_kv: int,
    splits_kv: int | None = None,
    tile_size_q: int,
    ...
    correction_num_warps: int = 4,
    cluster_dim_x: int = 1,
    max_partial_smem_bytes: int = MAX_CGA_PARTIAL_SMEM_BYTES,
) -> str:
    """Return why CGA SMEM reduction must be rejected, or "" if supported."""
    if max_splits_kv <= 1:
        return ""
    if max_splits_kv * cluster_dim_x > MAX_CGA_CLUSTER_DIM_X:
        return (
            "splits_kv * clusterDimX exceeds the CGA cluster limit of "
            f"{MAX_CGA_CLUSTER_DIM_X}"
        )
    partial_smem_bytes = cga_smem_reduction_partial_smem_bytes(
    ...
    if partial_smem_bytes > max_partial_smem_bytes:
        return (
            "CGA leader-DSMEM partial staging would use "
...
def select_one_wave_cga_split(
    *,
    ...
    """Return the largest legal CGA split count whose clusters fit one wave.
    ...
    for candidate in range(initial_splits_kv, lower_bound - 1, -1):
        cga_reason = cga_smem_reduction_unsupported_reason(
...
def _apply_split_kv_config(
    ...
    *,
    ...
    elif split_kv_mode == "cga_smem_reduction":
        cfg.use_cga_smem_reduction = True
...
def _config_with_split_kv_mode(
    ...
    return replace(
        cfg,
        use_cga_smem_reduction=split_kv_mode == "cga_smem_reduction",
        use_separate_reduction_kernel=(
...
def _select_auto_split_kv_reduction_mode(
    ...
    *,
    ...
    num_heads_q: int,
    num_heads_kv: int,
    preserve_exact_cga_fanout: bool,
) -> tuple[FmhaDecodeConfig, str]:
    """Select the first supported reduction mode from the shared policy.

    FlashInfer retains its general one-wave CGA search for legacy auto-derived
    ...
    for split_kv_mode in modes:
        ...
        if split_kv_mode == "cga_smem_reduction":
            if not trial.can_use_cga_smem_reduction:
                continue
            q_geometry = make_q_tile_geometry(
            ...
            cga_split = select_one_wave_cga_split(
                initial_splits_kv=trial.splits_kv,
                minimum_splits_kv=(trial.splits_kv if preserve_exact_cga_fanout else 2),
            ...
            if cga_split is None:
                continue
            trial.splits_kv = cga_split
            trial.max_splits_kv = cga_split
        try:
            _validate_profile_support(
                cfg=trial,
                seq_len_q=seq_len_q,
...
def _validate_profile_support(
    *,
    ...
    qualified_fp8_q8_separate_reduction_supported = (
        ...
        and not cfg.use_cga_smem_reduction
    ...
    if cfg.use_variable_seqlens_q:
        ...
        if not use_groups_tokens_heads_q and cfg.use_cga_smem_reduction:
            raise ValueError(
                "packed ungrouped SwapsMmaAb does not support CGA SMEM reduction; "
    ...
    if use_keeps_mma_ab:
        ...
        if cfg.use_cga_smem_reduction:
            raise ValueError(
                "fmha_decode does not support CGA SMEM reduction with keepsMmaAb"
            )
    ...
    if cfg.use_cga_smem_reduction:
        ...
        # SMEM-budget checks. Grouped CGA is an explicit static choice.
        if not (cfg.supports_reduction_dtypes and cfg.can_use_cga_smem_reduction):
            raise ValueError(
                "CGA SMEM reduction requires static SwapsMmaAb headDim in "
        ...
        if cga_reason:
            raise ValueError(f"CGA SMEM reduction rejected: {cga_reason}")
    use_separate_reduction_kernel = cfg.use_separate_reduction_kernel
...
def make_decode_config(
    ...
    *,
    ...
    2. Fill profile-derived defaults: dtype fields, paged-KV metadata, the
       ...
       Production-valid recipes minimize TRTLLM-gen's empirical TileQ
    ...
    5. Choose the reduction mode for an automatic split-KV launch: prefer a
       legal one-wave CGA configuration, otherwise use the measured TileQ/head
       dimension GMEM crossover with support-aware inline/separate fallback.
       A jointly scored recipe keeps its exact fanout; the legacy launch path
    ...
    # Whether the launch mode was left to the auto-tuner (vs explicitly chosen
    # by the caller, e.g. the Suite F split rows). Only auto-derived modes are
    # eligible for the CGA promotion below.
    ...
    reduction_mode_preconfigured = (
        cfg.use_cga_smem_reduction or cfg.use_separate_reduction_kernel
    )
