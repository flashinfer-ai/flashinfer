...
class TmemCorrResource(DecodeGenResourceBase):
    ...
    _cga_partial_o_alloc: Constexpr[SmemAllocation | None] = None
    _cga_partial_stats_alloc: Constexpr[SmemAllocation | None] = None
    _cga_mbarrier_alloc: Constexpr[SmemAllocation | None] = None
    _cga_partial_o_i32: cutlass.Array = None
    _cga_partial_stats: cutlass.Array = None
    _cga_mbarrier: cutlass.Array = None
    ...
    def _init_placeholder_state(self) -> None:
        ...
        cga_o_entries = max(
            self.cfg.cga_max_runtime_partial_rows * self.cfg.headdim * 2 // 4,
            1,
        )
        cga_stats_entries = max(
            self.cfg.cga_max_runtime_partial_rows * 2,
            1,
        )
        ...
        self._cga_partial_o_i32 = _placeholder_smem_array(Int32, cga_o_entries)
        self._cga_partial_stats = _placeholder_smem_array(Float32, cga_stats_entries)
        self._cga_mbarrier = _placeholder_smem_array(cutlass.Int64, 1)
    ...
    def get_smem_requirements(self) -> list[SmemAllocation]:
        """Allocate correction SMEM scratch, staging, and CGA partial buffers."""
        ...
        if (
            ...
            and not self.cfg.supports_cga_smem_reduction
        ...
        if self.cfg.supports_cga_smem_reduction:
            # Owner-CTA distributed-SMEM staging for the CGA reducer: every
            ...
            max_partial_rows = self.cfg.cga_max_runtime_partial_rows
            if self._cga_partial_o_alloc is None:
                self._cga_partial_o_alloc = SmemAllocation(
                    name=f"{self.name}_cgaPartialO",
                    size_bytes=max_partial_rows * self.cfg.headdim * 2,
                    alignment=16,
                )
            if self._cga_partial_stats_alloc is None:
                self._cga_partial_stats_alloc = SmemAllocation(
                    name=f"{self.name}_cgaPartialStats",
                    size_bytes=max_partial_rows * 2 * 4,
                    alignment=16,
                )
            if self._cga_mbarrier_alloc is None:
                # One transaction mbarrier per owner CTA; peers async-store into
                # this CTA's partial buffers and signal it.
                self._cga_mbarrier_alloc = SmemAllocation(
                    name=f"{self.name}_cgaTransactionBarrier",
                    size_bytes=8,
                    alignment=8,
                )
        allocs = []
        ...
        if self._cga_partial_o_alloc is not None:
            allocs.append(self._cga_partial_o_alloc)
        if self._cga_partial_stats_alloc is not None:
            allocs.append(self._cga_partial_stats_alloc)
        if self._cga_mbarrier_alloc is not None:
            allocs.append(self._cga_mbarrier_alloc)
        return allocs
    ...
    @cute.jit
    def _cga_reduction_rows_per_cta(self, splits_kv: Int32) -> Int32:
        """Return the CGA owner row-band height for active splits."""
        return self._split_reduction_rows_per_cta(
            Int32(self.cfg.tile_size_q), splits_kv
        )

    @cute.jit
    def _cga_reduction_cta_for_row(self, row_idx: Int32, splits_kv: Int32) -> Int32:
        """Map an output row to its owning CGA split CTA."""
        return row_idx // self._cga_reduction_rows_per_cta(splits_kv)

    @cute.jit
    def _cga_reduction_local_row(self, row_idx: Int32, splits_kv: Int32) -> Int32:
        """Return the row index inside the owning CGA reducer band."""
        rows_per_cta = self._cga_reduction_rows_per_cta(splits_kv)
        return row_idx - (row_idx // rows_per_cta) * rows_per_cta

    @cute.jit
    def _cga_reduction_row_is_owned(
        self, cta_idx_kv: Int32, row_idx: Int32, splits_kv: Int32
    ) -> bool:
        """Test whether this split CTA owns one CGA reducer row."""
        return self._split_reduction_row_is_owned(
            cta_idx_kv, row_idx, Int32(self.cfg.tile_size_q), splits_kv
        )

    @cute.jit
    def _cga_partial_row_idx(
        self, split_idx: Int32, local_row_idx: Int32, splits_kv: Int32
    ) -> Int32:
        """Linearize a split/local-row pair in CGA partial buffers."""
        return split_idx * self._cga_reduction_rows_per_cta(splits_kv) + local_row_idx

    @cute.jit
    def _cga_partial_o_i32_offset(
        self, row_idx: Int32, col_offset_bytes: Int32
    ) -> Int32:
        """Return the int32 offset for a CGA partial-O fragment."""
        return (row_idx * Int32(self.cfg.headdim * 2) + col_offset_bytes) >> Int32(2)

    @cute.jit
    def _cga_partial_stats_offset(self, row_idx: Int32) -> Int32:
        """Return the float offset for a CGA partial max/sum pair."""
        _ = self
        return row_idx * Int32(2)

    @cute.jit
    def _cga_init_transaction_barrier(
    ...
    @cute.jit
    def _cga_wait_transaction_barrier(
        self,
        mbarrier,
        ...
        """Wait for all peer async CGA partial stores before local reduction."""
        _ = self
        ...
        cga_transaction_ready = prims.mbarrier_try_wait_parity(
            mbarrier, 0, time_limit=0
        )
        if warp_grp_thread_idx == Int32(0):
            while not cga_transaction_ready:
                cga_transaction_ready = prims.mbarrier_try_wait_parity(
    ...
    @cute.jit
    def _cga_complete_inactive_row_bytes(
        self,
        mbarrier,
        cta_idx_kv: Int32,
        splits_kv: Int32,
        ...
        completion_bytes = Int32(cfg.cga_transaction_bytes) - actual_transaction_bytes
        if warp_grp_thread_idx == Int32(0) and completion_bytes > Int32(0):
            prims.mbarrier_complete_tx(mbarrier, completion_bytes)

    @cute.jit
    def _cga_store_async_vec4_i32(self, dst_ptr, vals, mbarrier) -> None:
    ...
    def _store_softmax_normalized_o_vec8(
        ...
        dst_row_base = physical_dst_row_idx * Int32(cfg.headdim * cfg.o_dtype_bytes)
        dst_offset = dst_row_base + reduce_col_idx * Int32(cfg.o_dtype_bytes)
    ...
    def create_function_variables(
        ...
        """Bind epilogue SMEM buffers and initialize CGA reduction barriers."""
        ...
        if cutlass.const_expr(
            context is not None
            and context.smem_base is not None
            and self._cga_partial_o_alloc is not None
        ):
            # CGA distributed-SMEM partial-O staging, addressed as int32 because
            # peers deliver packed 16-byte fp16 fragments via prims.mapa.
            self._cga_partial_o_i32 = cutlass.Array(
                context.smem_base.data_ptr() + self._cga_partial_o_alloc.offset,
                dtype=Int32,
                shape=(self._cga_partial_o_alloc.size_bytes // 4,),
                addrspace=3,
            )
        if cutlass.const_expr(
            context is not None
            and context.smem_base is not None
            and self._cga_partial_stats_alloc is not None
        ):
            # CGA distributed-SMEM stats staging (float2 max/sum per owned row
            # and split).
            self._cga_partial_stats = cutlass.Array(
                context.smem_base.data_ptr() + self._cga_partial_stats_alloc.offset,
                dtype=Float32,
                shape=(self._cga_partial_stats_alloc.size_bytes // 4,),
                addrspace=3,
            )
        if cutlass.const_expr(
            context is not None
            and context.smem_base is not None
            and self._cga_mbarrier_alloc is not None
        ):
            ...
            self._cga_mbarrier = cutlass.Array(
                context.smem_base.data_ptr() + self._cga_mbarrier_alloc.offset,
                dtype=cutlass.Int64,
                shape=(1,),
                addrspace=3,
            )
            self._cga_init_transaction_barrier(
                self._cga_mbarrier.data_ptr(),
                self.cfg.cga_transaction_bytes,
            )
        return {}
    ...
    def init_epilogue_state(self, stage_info: StageInfo) -> None:
        ...
        # so CGA mbarriers are visible before peer async stores. Keep this as
    ...
    def _sync_gmem_split_reducers(
        ...
        if warp_grp_thread_idx == Int32(0):
            ...
            if reducer_cta_idx < Int32(
                cfg.cga_reduction_num_owner_ctas
    ...
    def _reduce_fused_gmem_partials_impl(
        ...
        *,
        ...
        if reducer_cta_idx < Int32(cfg.cga_reduction_num_owner_ctas):
            if cutlass.const_expr(full_prefix):
                rows_per_reducer = Int32(cfg.cga_reduction_rows_per_cta)
    ...
    @cute.jit
    def _reduce_cga_partial_segment(
        self,
        logical_h_k_idx: Int32,
        logical_b_idx: Int32,
        q_row_offset: Int32,
        ...
        *,
        ...
        """Reduce one correction-warpgroup slice from CGA partial SMEM."""
        cfg = self.cfg
        reduction_splits_kv = splits_kv
        if cutlass.const_expr(full_prefix):
            reduction_splits_kv = Int32(cfg.splits_kv)
        ...
        valid_reduce_row = self._cga_reduction_row_is_owned(
        ...
        local_row_idx = self._cga_reduction_local_row(
        ...
        for split_base_i in cutlass.range_constexpr(0, cfg.max_splits_kv, 4):
            ...
            for jj in cutlass.range_constexpr(4):
                ...
                if valid_split_idx and valid_reduce_row:
                    partial_row_idx = self._cga_partial_row_idx(
                        split_idx, local_row_idx, reduction_splits_kv
                    )
                    stats_pair = (
                        self._cga_partial_stats.subview(
                            self._cga_partial_stats_offset(partial_row_idx)
                        )
                        .data_ptr()
                    ...
                    partial_sum[jj] = stats_pair[1]
                    loaded_partial_regs = (
                        self._cga_partial_o_i32.subview(
                            self._cga_partial_o_i32_offset(
                                partial_row_idx,
    ...
    def _reduce_cga_partials_impl(
        ...
        *,
        ...
        """Wait for CGA publication and run static or contracted geometry."""
        cfg = self.cfg
        reduction_splits_kv = splits_kv
        if cutlass.const_expr(full_prefix):
            reduction_splits_kv = Int32(cfg.splits_kv)
        ...
        if cta_idx_kv < Int32(cfg.cga_reduction_num_owner_ctas):
            self._cga_complete_inactive_row_bytes(
                self._cga_mbarrier.data_ptr(),
                cta_idx_kv,
                reduction_splits_kv,
                logical_q_group_idx,
            ...
            self._cga_wait_transaction_barrier(
                self._cga_mbarrier.data_ptr(),
                warp_grp_thread_idx,
                self.store_barrier_id,
            ...
            if cutlass.const_expr(full_prefix):
                rows_per_cta = Int32(cfg.cga_reduction_rows_per_cta)
                for segment_idx in cutlass.range_constexpr(
                    cfg.split_reduction_slices_per_cta
                ):
                    self._reduce_cga_partial_segment(
                        logical_h_k_idx,
                        logical_b_idx,
                        q_row_offset,
                        logical_q_group_idx,
                ...
                rows_per_cta = self._cga_reduction_rows_per_cta(reduction_splits_kv)
    ...
    def _copy_staged_fp16_o_segments(
        ...
        *,
        ...
        for CGA reduction; otherwise it is the final output tensor.
        """
        publication_splits_kv = splits_kv
        if cutlass.const_expr(enable_cga and full_prefix):
            ...
            if cutlass.const_expr(copy_to_partial):
                ...
                if valid_partial_row:
                    ...
                    if cutlass.const_expr(enable_cga):
                        ...
                        self._cga_store_async_vec4_i32(
                            partial_o_dst,
                ...
                if _q_row_is_valid_for_seq(
                    ...
                    dst_row_base = physical_dst_row_idx * Int32(
                        cfg.headdim * cfg.o_dtype_bytes
                    )
                    dst_ptr = cutlass.inttoptr(
                        self.o_ptr.toint()
                        + cutlass.Int64(dst_row_base + dst_col_offset),
                        mem_space=1,
                        dtype=Int32,
                    )
    ...
    def _copy_staged_fp16_o_to_partial(
        ...
        *,
        ...
        """Publish staged partial O to GMEM, or owner CTA SMEM for CGA reduction."""
    ...
    def _store_split_stats_from_scale_arrays(
        ...
        *,
        ...
        Separate-GMEM writes one FP32 log2-LSE value per row. Fused GMEM/CGA
        ...
        if warp_idx == Int32(0) and lane_idx < Int32(4 * num_scale_groups):
            ...
            if _q_row_is_valid_for_seq(
                ...
                if cutlass.const_expr(cfg.use_separate_reduction_kernel):
                ...
                elif cutlass.const_expr(enable_cga):
                    cga_owner = self._cga_reduction_cta_for_row(
                    ...
                    stats_ptr = prims.mapa(
                        self._cga_partial_stats.subview(
                            self._cga_partial_stats_offset(
                                self._cga_partial_row_idx(
                                    cta_idx_kv,
                                    cga_local,
                                    publication_splits_kv,
                    ...
                    self._cga_store_async_vec2_f32(
                        stats_ptr,
                        final_max[stats_idx],
    ...
    @cute.jit
    def _publish_and_reduce_cga_swaps_partials_impl(
        self,
        regs_partial_o: cutlass.Array,
        logical_h_k_idx: Int32,
        logical_b_idx: Int32,
        ...
        *,
        ...
        """Publish and reduce one static or contracted CGA split prefix."""
        cfg = self.cfg
        publication_splits_kv = splits_kv
        if cutlass.const_expr(full_prefix):
            publication_splits_kv = Int32(cfg.splits_kv)
        ...
        self._reduce_cga_partials_impl(
            logical_h_k_idx,
            logical_b_idx,
            q_row_offset,
            logical_q_group_idx,
    ...
    @cute.jit
    def _publish_and_reduce_cga_swaps_partials(
        self,
        regs_partial_o: cutlass.Array,
        logical_h_k_idx: Int32,
        logical_b_idx: Int32,
        ...
        *,
        ...
        """Select full-prefix or contracted CGA publication and reduction."""
        cfg = self.cfg
        if cutlass.const_expr(self.static_full_split_prefix):
            self._publish_and_reduce_cga_swaps_partials_impl(
                regs_partial_o,
                logical_h_k_idx,
                logical_b_idx,
                logical_kv_idx,
    ...
    def _keeps_tail_epilogue(
        ...
        *,
        ...
        dst_row_base = physical_dst_row_idx * Int32(cfg.headdim * cfg.o_dtype_bytes)
        dst_col_offset = col_base * Int32(cfg.o_dtype_bytes)
        if cutlass.const_expr(
            ...
            for store_idx in cutlass.range_constexpr(output_pair_regs // 8):
                ...
                dst_ptr = cutlass.inttoptr(
                    self.o_ptr.toint()
                    + cutlass.Int64(
                        dst_row_base + dst_col_offset + Int32(store_idx * 16)
                    ),
                    mem_space=1,
                    dtype=Int32,
                )
        ...
        for store_idx in cutlass.range_constexpr(output_pair_regs // 4):
            ...
            dst_ptr = cutlass.inttoptr(
                ...
                + cutlass.Int64(
                    dst_row_base
                    + dst_col_offset
                    + Int32(store_idx * (8 if cfg.use_fp8_output else 16))
                ),
                mem_space=1,
                dtype=Int32,
            )
    ...
    def _swaps_wide_tail_epilogue(
        ...
        *,
        ...
        if cutlass.const_expr(cfg.use_split_kv):
            ...
            # plus log2-LSE; fused GMEM/CGA retains unnormalized O plus
            # max/sum state.
            logical_h_k_idx, logical_b_idx = _logical_head_batch(
                stage_info, self.h_k_idx, self.b_idx
            )
            ...
            if cutlass.const_expr(cfg.supports_cga_smem_reduction):
                self._publish_and_reduce_cga_swaps_partials(
                    regs_partial_o,
                    logical_h_k_idx,
                    logical_b_idx,
                    logical_kv_idx,
    ...
    def _swaps_q8_tail_epilogue(
        ...
        *,
        ...
        if cutlass.const_expr(cfg.use_split_kv):
            ...
            # reduction returns after publication; CGA peers reduce through
            ...
            # unnormalized 16-bit O for fused GMEM/CGA reduction.
            regs_partial_o = cutlass.Array(
                Int32,
            ...
            if cutlass.const_expr(cfg.use_separate_reduction_kernel):
                ...
                # also execute the fused in-kernel counter/CGA reduction.
                return
