...
def _copy_transposed_smem8b_to_gmem(
    ...
    for copy_segment_idx in cutlass.range_constexpr(num_copy_segments):
        ...
        if cutlass.const_expr(full_tile_rows):
            ...
            dst_row_base_full = physical_dst_row_idx * Int32(headdim)
            dst_ptr_full = cutlass.inttoptr(
                o_ptr.toint() + cutlass.Int64(dst_row_base_full + dst_col_offset),
                mem_space=1,
                dtype=Int32,
            )
            ...
            if valid_output_row:
                ...
                dst_row_base_guarded = physical_dst_row_idx * Int32(headdim)
                dst_ptr_guarded = cutlass.inttoptr(
                    o_ptr.toint()
                    + cutlass.Int64(dst_row_base_guarded + dst_col_offset),
                    mem_space=1,
                    dtype=Int32,
                )
...
def _transposed_smem128x16b_stsm_offset_bytes(
    ...
    # Store transposed 128x16b rows. Once there are multiple LDTM repeats, STSM
    # groups advance both row and column sub-tiles.
    num_ldtm_reps = max(num_rows // 8, 1)
    if cutlass.const_expr(num_ldtm_reps == 1):
        num_mtx_per_row_per_stsm = 4
        num_mtx_per_col_per_stsm = 1
    else:
        num_mtx_per_row_per_stsm = 2
        num_mtx_per_col_per_stsm = 2
    num_stsm_per_col = max(num_ldtm_reps // num_mtx_per_col_per_stsm, 1)
