...
def _compute_fp8_p_regs_and_local_sums(
    ...
    p0 = cute.math.exp2(scaled_pair_01[0], fastmath=True)
    p1 = cute.math.exp2(scaled_pair_01[1], fastmath=True)
    scaled_pair_45 = ffma2(
        (s4, s5),
        (scale_softmax_log2, scale_softmax_log2),
        neg_scaled_max_pair,
    )
    p2 = cute.math.exp2(scaled_pair_23[0], fastmath=True)
    p3 = cute.math.exp2(scaled_pair_23[1], fastmath=True)
    # Accumulate the local softmax sums while packing P for the BMM2 operand.
    local_sum_pair_01 = fadd2((p0, p1), (p2, p3))
    packed_p_0 = _pack_float4_to_fp8_e4m3(p0, p1, p2, p3)
    scaled_pair_67 = ffma2(
        (s6, s7),
        (scale_softmax_log2, scale_softmax_log2),
        neg_scaled_max_pair,
    )
    p4 = cute.math.exp2(scaled_pair_45[0], fastmath=True)
    p5 = cute.math.exp2(scaled_pair_45[1], fastmath=True)
    p6 = cute.math.exp2(scaled_pair_67[0], fastmath=True)
    p7 = cute.math.exp2(scaled_pair_67[1], fastmath=True)
    local_sum_pair_45 = fadd2((p4, p5), (p6, p7))
    packed_p_1 = _pack_float4_to_fp8_e4m3(p4, p5, p6, p7)
...
def _compute_p_values_and_local_sums_dense(
    ...
    p6 = cute.math.exp2(scaled_pair_67[0], fastmath=True)
    p7 = cute.math.exp2(scaled_pair_67[1], fastmath=True)
    local_sum_pair_02 = fadd2((p0, p1), (p2, p3))
    local_sum_pair_46 = fadd2((p4, p5), (p6, p7))
    local_sum_pair = fadd2(local_sum_pair_02, local_sum_pair_46)
    return p0, p1, p2, p3, p4, p5, p6, p7, local_sum_pair[0], local_sum_pair[1]
