...
class WorkIdThrottleResource(MemoryResource):
    ...
    @producer_work
    @cute.jit
    def publish_work_id(self, stage_info: StageInfo) -> None:
        """Publish that the load task has consumed the current work ID."""
        del stage_info

    @consumer_work
    @cute.jit
    def consume_work_id(self, stage_info: StageInfo) -> None:
        """Wait until the load task no longer needs the current work ID."""
        del stage_info
...
def _work_id_throttle_head(work_id_throttle: MemoryResource | None) -> None:
    """Let the scheduler recycle a work-ID slot once Load owns its tile state."""
...
def _work_id_throttle_tail(work_id_throttle: MemoryResource | None) -> None:
    """Pace scheduler work-ID reuse against the persistent Load task."""
    if work_id_throttle is None:
        return
...
# produced ahead of time by PageOffsetsTask.
# ======================================================================
def create_load_task(
    smem_q: MemoryResource,
    smem_kv: MemoryResource,
    work_queue: WorkQueue | None,
    work_id_throttle: MemoryResource | None,
    cfg: FmhaDecodeConfig,
    *,
    ...
    **kw: TaskKwarg,
    ...
    def load_schedule(
        ...
        work_id_throttle: MemoryResource | None = None,
    ) -> None:
        """Schedule dense shared-KV loads without page-offset resources."""
        _decode_work_tile_schedule(
            cfg,
            work_queue,
            lambda: load_schedule_body(smem_q, smem_kv, None, work_id_throttle),
            lambda: _work_id_throttle_head(work_id_throttle),
        )
    ...
    def load_page_offsets_schedule(
        ...
        _decode_work_tile_schedule(
            ...
            lambda: load_schedule_body(
                smem_q, smem_kv, smem_page_offsets, work_id_throttle
            ),
            lambda: _work_id_throttle_head(work_id_throttle),
        )
    ...
    **kw: TaskKwarg,
    ...
    def load_schedule(
        ...
        _decode_work_tile_schedule(
            cfg,
            work_queue,
            lambda: load_schedule_body(smem_q, smem_k, smem_v, None, work_id_throttle),
            lambda: _work_id_throttle_head(work_id_throttle),
        )
    ...
    def scheduler_schedule(
        ...
        with work_tile_loop(work_queue):
            ...
            with domain_loop(0, 0, 1):
                pass
            _work_id_throttle_tail(work_id_throttle)
