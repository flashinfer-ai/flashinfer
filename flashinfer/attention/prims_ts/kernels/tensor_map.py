...
# Match trtllm-gen's `tg::LargeN` / `tg::XLargeN`
# (src/trtllm/gen/CommonUtils.h).  Kept private — callers never need
# to reference these directly; they are an implementation detail of
...
def create_tensor_map_ragged(
    ...
    *,
    ...
    inserts the trtllm-gen ``UseTmaOobOpt`` synthetic-dim splice
...
def create_tensor_map_ragged_from_tensor(
    ...
    *,
    ...
    trtllm-gen ``UseTmaOobOpt`` pattern exposed as a convenience helper.
...
def transform_ragged_coords(
    ...
    *,
    ...
    (matching ``trtllm-gen/kernels/Gemm/include/Gemm/GmemC.h`` lines
    2722–2754, with an explicit ``ragged_extent == 0`` carve-out so a
