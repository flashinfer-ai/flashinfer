        ...
        *,
        ...
        if family == "2cta":
            ...
            kernel = ThroughputLatencyMlaDecodeTs(
                ...
                logical_seq_len_q=1,
                tile_size_q=tile,
                gather_issue_warps=profile.gather_issue_warps,
                sparse_offset_cache="coalesced",
                mask_type="dense",
                device_scales=True,
            )
            kernel.balanced_sparse_registers = profile.balanced_registers
            kernel.reuse_sparse_kv = profile.reuse_kv
            kernel.sparse_kv_tile_size = profile.kv_tile_size
            kernel.page_pipeline_stages = profile.page_pipeline_stages
            kernel.reuse_sparse_kv_stages = profile.reuse_kv_stages
            kernel.paired_sparse_correction = profile.paired_correction
            kernel.single_sparse_stream = profile.single_kv_stream
            kernel.defer_sparse_max_update = profile.defer_max_update
            kernel.uniform_sparse_offset_cache = profile.uniform_offset_cache
