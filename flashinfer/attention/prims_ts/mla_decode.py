...
def _get_compiled_mla_decode(
    ...
    if isinstance(kernel, MlaDecodeTs):
        # Dense MLA leaves the DSV4 Gather4 descriptor-pair slot unbound.
        compile_args.insert(8, None)
        # Dense MLA has no per-query sparse scan-width input.
        compile_args.insert(11, None)
        # The second (sparse) cache slot is bound to an ignored tensor rather
        # than an optional pointer so the TVM-FFI runtime ABI stays unambiguous.
        ...
        # Skip correction is disabled for the dense compile.
        compile_args.append(cutlass.Float32(0.0))
        # Dense MLA does not use the DSV4 inverse-RoPE/FP8-quant outputs.
        compile_args.extend((None, None))
    else:
        # The 1CTA kernel takes the runtime skip-correction threshold after
        # the two scales.
        compile_args.append(cutlass.Float32(0.0))
...
def _prepare_mla_runtime(
    ...
    *,
    ...
    if validate:
        ...
        effective_skip_corr_threshold = _validate_mla_skip_corr_threshold(
            skip_corr_threshold,
            q_dtype=q_dtype,
...
def _launch_mla_decode(
    ...
    *,
    ...
    if uses_throughput_2cta:
        ...
        launch_args.append(runtime.skip_corr_threshold)
