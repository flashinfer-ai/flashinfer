...
def _derive_max_seq_len_q(
    ...
    *,
    ...
    return max(q_lengths), offsets[-1]
...
def _resolve_mla_decode_launch_spec(
    ...
    from .kernels.mla_decode.throughput_latency_1cta.config import (
        ...
        resolve_runtime_cga_reduction_mode,
        wave_fill_split_kv,
    )
    ...
    with torch.cuda.device(device_index):
        ...
        if use_throughput_latency:
            ...
            reduction_mode = resolve_runtime_cga_reduction_mode(
                decision.config,
                reduction_mode=None,
            ...
            separate_reducer_impl, reducer_cluster_size = _separate_reducer_provenance(
                kernel,
                split_kv=split_kv,
                use_cga_reduction=bool(final_cfg.use_cga_reduction),
            )
            policy = (
...
def _get_compiled_mla_decode(
    ...
    # PyIR reconstructs that stable structure before lowering to TVM FFI.
...
def prims_ts_batch_decode_with_kv_cache_mla(
    ...
    *,
    ...
    The workspace is exclusive to one in-flight launch or captured graph.
    ...
    larger than ``max_seq_len_q``. Warm the semantic key before CUDA graph
    ...
    def plan(
        ...
        *,
        ...
        if packed_query:
            ...
            derived_q_bound, planned_total_q = _derive_max_seq_len_q(
                qo_indptr,
                batch_size=batch_size,
            )
