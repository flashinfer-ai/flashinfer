...
DSV4 sparse MLA has no separate guide yet. All entry points consume
DeepSeek V4 physical routing (128 sliding-window slots followed by
compressed-pool slots per packed query), pick the static 2CTA grid while
`B * max_seq_len_q` fits one resident wave of 2CTA clusters and the CLC
persistent kernel beyond it (the dense MLA resident-wave rule), and accept
`skip_corr_threshold` in `[0, 8]` for their E4M3 P. Dense MLA `plan()` and the
one-shot APIs accept `skip_corr_threshold` as well: the bound is 8 for E4M3 QKV
(`1.75 * 2**8 == 448`) and 64 for BF16 QKV, the default `0.0` keeps the exact
online-softmax rescale, and both MLA kernel families support the knob.
...
`prims_ts_dsv4_sparse_mla_rope_quant_ue8m0` writes the
`RopeQuantUe8m0Sf` layout, one INT32 word per head (`[16, 8, pad4(T)]`) packing
four UE8M0 exponent bytes with `scale = 2 ** (byte - 127)` and
`exp2(ceil(log2(max(amax, 1e-10) / 448)))` rounding; its
`out_scale.permute(2, 0, 1)[:T]` view is the `[T, 16, 8]` scale tensor consumed
by UE8M0 block-scaled GEMMs.
...
`nvidia-cutlass-dsl` wheel, not the system toolkit. The default wheel carries
the CUDA 12 backend; with a CUDA 13 PyTorch install `nvidia-cutlass-dsl[cu13]`
so Blackwell kernels are compiled by ptxas 13.x. On the DSV4 kernels the CUDA
12 backend spills registers under the fixed `setmaxnreg` budgets and runs about
40% slower with bitwise-identical output.
