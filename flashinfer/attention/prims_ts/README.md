...
A mixed geometry costs: the proxy kernel carries both geometries and grows
past its instruction-cache budget, so INT8 SOL proxy runs at 1.34x the time
of equal block sizes with a one-token summary block (B200, CUDA Graph replay
minimum); use it where the summary precision (INT8 summaries with widely
