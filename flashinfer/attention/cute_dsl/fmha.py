...
def _get_checksums(gpu_arch: str) -> dict[str, str]:
    ...
    Fetched through get_artifact() against the manifest pin in
    CheckSumHash.DSL_FMHA_CHECKSUMS, the same way every other manifest consumer
    does it. The hashes parsed out of this file are what verify the .so kernels
    below, so taking the manifest on trust would make that verification
    circular: whoever serves the manifest would also choose the hashes it is
    checked against.
    """
    ...
    from flashinfer.jit.cubin_loader import FLASHINFER_CUBINS_REPOSITORY, get_artifact
    ...
    expected_sha256 = CheckSumHash.DSL_FMHA_CHECKSUMS.get(cpu_arch, {}).get(gpu_arch)
    if expected_sha256 is None:
        raise RuntimeError(
            f"No pinned checksum manifest for DSL FMHA on {cpu_arch}/{gpu_arch}. "
            f"Pinned architectures for {cpu_arch}: "
            f"{sorted(CheckSumHash.DSL_FMHA_CHECKSUMS.get(cpu_arch, {}))}."
        )

    logger.info(f"Loading checksums for {gpu_arch} from {checksums_rel}")
    data = get_artifact(checksums_rel, expected_sha256)
    if not data:
        # get_artifact returns empty bytes both when the manifest cannot be
        # fetched and when what came back does not match the pin. Without this
        # the next parse fails with a bare FileNotFoundError on the local cache
        # path, which reads like a corrupt cache rather than an unreachable or
        # mismatched pin.
        raise RuntimeError(
            f"Failed to obtain a checksum manifest matching the pin for DSL FMHA on "
            f"{cpu_arch}/{gpu_arch} (expected sha256 {expected_sha256}). Check that "
            f"the pin is published in {FLASHINFER_CUBINS_REPOSITORY} and reachable, "
            f"or clear the cache: rm -rf {FLASHINFER_CUBIN_DIR}"
        )
