...
class _Config(Protocol):
    ...
    @property
    def variant(self) -> IntEnum: ...


_ConfigT = TypeVar("_ConfigT", bound=_Config)
_VariantT = TypeVar("_VariantT", bound=IntEnum)


class _TunableWorkspace(Protocol[_ConfigT]):
    @property
    ...
    def launch_config(self, inp: torch.Tensor) -> Optional[_ConfigT]: ...
    ...
    def _launch(
        self, inp: torch.Tensor, out: torch.Tensor, config: _ConfigT
    ) -> None: ...


@dataclass(frozen=True)
class PcieIpcCollectiveTuningSpec(Generic[_ConfigT, _VariantT]):
    cache_stem: str
    version: int
    config_type: Callable[[int, int, _VariantT], _ConfigT]
    variant_type: type[_VariantT]
    grid_variants: Tuple[_VariantT, ...]
    fixed_configs: Tuple[_ConfigT, ...]
    is_launchable: Callable[[int, _ConfigT, int, bool], bool]


def default_cache_path(
    spec: PcieIpcCollectiveTuningSpec[_ConfigT, _VariantT],
    world_size: int,
    dtype: torch.dtype,
...
def tactic_to_config(
    spec: PcieIpcCollectiveTuningSpec[_ConfigT, _VariantT], tactic: Sequence[int]
) -> _ConfigT:
...
class PcieIpcCollectiveTuningState(Generic[_ConfigT, _VariantT]):
    """Exact-shape cache plus correctness-gated, slowest-rank tuning."""

    def __init__(
        self,
        workspace: _TunableWorkspace[_ConfigT],
        spec: PcieIpcCollectiveTuningSpec[_ConfigT, _VariantT],
        *,
        ...
        self._resolved: Dict[Tuple[int, int], _ConfigT] = {}
    ...
    def tuned_launch_config(self, inp: torch.Tensor) -> Optional[_ConfigT]:
    ...
    def tune(
        ...
        *,
        ...
        results: Dict[Tuple[int, int], _ConfigT] = {}
        torch.cuda.synchronize(self.workspace.device)
