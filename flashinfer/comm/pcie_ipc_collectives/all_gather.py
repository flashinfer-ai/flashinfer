...
class PcieIpcAllGatherWorkspace(_PcieIpcWorkspace):
    ...
    def all_gather(
        ...
        *,
        ...
        ----------
        inp : torch.Tensor
            Contiguous rank-local input with shape ``[local_rows, hidden]``.
            Its dtype and device must match the workspace, and its size must
            fit ``max_numel`` and contain complete 16-byte packs.
        out : Optional[torch.Tensor]
            Output with shape ``[world_size * local_rows, hidden]``. If omitted,
            an output tensor is allocated. A supplied tensor must be contiguous,
            16-byte aligned, and must not overlap ``inp``.
        config : Optional[PcieIpcAllGatherLaunchConfig]
        ...
        -------
        torch.Tensor
            Gathered shards in rank order; the supplied ``out`` if provided.
        """
        if config is None:
