...
__device__ __forceinline__ void store_release_sys(uint32_t* ptr, uint32_t value) {
  ...
  asm volatile("st.global.release.sys.u32 [%0], %1;" ::"l"(address), "r"(value)
               : "memory");
}
...
__device__ __forceinline__ uint32_t load_acquire_sys(const uint32_t* ptr) {
  uint32_t value;
  uint64_t address = reinterpret_cast<uint64_t>(ptr);
  asm volatile("ld.global.acquire.sys.u32 %0, [%1];" : "=r"(value) : "l"(address)
               : "memory");
  return value;
}
...
__global__ void publish_signal_kernel(
    ...
    const int64_t* __restrict__ peer_out_ptrs,
    const int64_t* __restrict__ peer_lse_ptrs,
    const int64_t* __restrict__ peer_sig_ptrs, int32_t* __restrict__ epoch_ptr,
    int32_t* __restrict__ dest_done, int rank, int world, int num_tokens,
    int max_tokens, int h_local, int d, int64_t stride_po_tok,
    int64_t stride_po_head, int64_t stride_pl_tok) {
  ...
  float* peer_lse =
      reinterpret_cast<float*>(static_cast<uintptr_t>(peer_lse_ptrs[dest]));
  const bool packed16 = (stride_po_head == d) && (sizeof(T) == 2) &&
                        ((n_items & 7) == 0) && ((stride_po_tok & 7) == 0);

  T* dst = peer_o +
           (static_cast<int64_t>(parity) * world + rank) * max_tokens * n_items +
           static_cast<int64_t>(tok) * n_items;
  if (packed16) {
    const uint4* src = reinterpret_cast<const uint4*>(
        partial_o + tok * stride_po_tok + src_head0 * stride_po_head);
    ...
    for (int i = threadIdx.x; i < n_items; i += blockDim.x) {
      ...
      dst[i] = partial_o[tok * stride_po_tok +
                         (src_head0 + local_head) * stride_po_head + dim];
    }
  }
  float* lse_dst =
      peer_lse +
      (static_cast<int64_t>(parity) * world + rank) * max_tokens * h_local +
  ...
  if (threadIdx.x == 0) {
    ...
    if (finished == num_tokens - 1) {
      uint32_t* sig = reinterpret_cast<uint32_t*>(
          static_cast<uintptr_t>(peer_sig_ptrs[dest]));
...
__global__ void merge_kernel(
    const T* __restrict__ recv_o, const float* __restrict__ recv_lse,
    const int32_t* __restrict__ recv_sig, const int32_t* __restrict__ epoch_ptr,
    T* __restrict__ out, float* __restrict__ lse_out, int world, int num_tokens,
    int max_tokens, int h_local, int d, int64_t stride_out_tok,
    int64_t stride_out_head, int64_t stride_lse_tok, int is_base_e) {
  ...
  if (threadIdx.x == 0) {
    ...
    for (int src = 0; src < world; ++src) {
      const int64_t lse_off =
          ((static_cast<int64_t>(parity) * world + src) * max_tokens + token) *
              h_local +
          head;
    ...
    for (int src = 0; src < world; ++src) {
      const float w =
          is_base_e ? expf(weights[src] - m_math) : exp2f(weights[src] - m_math);
      weights[src] = w;
      sum_w += w;
    }
...
void launch(
    ...
    torch::Tensor peer_out_ptrs, torch::Tensor peer_lse_ptrs,
    torch::Tensor peer_sig_ptrs, torch::Tensor recv_o, torch::Tensor recv_lse,
    torch::Tensor recv_sig, torch::Tensor epoch, torch::Tensor dest_done,
    torch::Tensor out, torch::Tensor lse_out, int64_t world, int64_t rank,
    int64_t max_tokens, int64_t is_base_e) {
  ...
  const dim3 pub_grid(static_cast<unsigned>(num_tokens),
  ...
  auto launch_typed = [&](auto dummy) {
    ...
    publish_signal_kernel<T><<<pub_grid, kPubThreads, 0, stream>>>(
        reinterpret_cast<const T*>(partial_o.data_ptr()),
        partial_lse.data_ptr<float>(), peer_out_ptrs.data_ptr<int64_t>(),
        peer_lse_ptrs.data_ptr<int64_t>(), peer_sig_ptrs.data_ptr<int64_t>(),
        epoch.data_ptr<int32_t>(), dest_done.data_ptr<int32_t>(),
        static_cast<int>(rank), static_cast<int>(world), num_tokens,
        static_cast<int>(max_tokens), h_local, d, partial_o.stride(0),
        partial_o.stride(1), partial_lse.stride(0));
    const dim3 merge_grid(
        static_cast<unsigned>(num_tokens * h_local));
    merge_kernel<T>
        <<<merge_grid, kMergeThreads, sizeof(float) * world, stream>>>(
            reinterpret_cast<const T*>(recv_o.data_ptr()),
            recv_lse.data_ptr<float>(), recv_sig.data_ptr<int32_t>(),
            epoch.data_ptr<int32_t>(), reinterpret_cast<T*>(out.data_ptr()),
            lse_out.data_ptr<float>(), static_cast<int>(world), num_tokens,
            static_cast<int>(max_tokens), h_local, d, out.stride(0),
            out.stride(1), lse_out.stride(0), static_cast<int>(is_base_e));
  };
