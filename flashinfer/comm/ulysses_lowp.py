...
Low-precision Ulysses global-grid payload operations.
SM89/SM120 use Q32/K64; SM90 uses Q16/K128. All shards use one
...
# ALIGNED is a legacy padding policy only; execution always uses BOUNDARY_MERGE.
ALIGNED = "aligned"
...
def get_ulysses_lowp_module():
    ...
    return SimpleNamespace(
        ...
        ulysses_lowp_supports_head_dim=module.ulysses_lowp_supports_head_dim,
    )
...
def _validate_nhd_shape(name: str, tensor: torch.Tensor) -> Tuple[int, int, int, int]:
    ...
    if num_heads <= 0:
        ...
        # the payload spec. Both supported dimensions use the same global grid.
        raise ValueError(f"V2-G requires a positive head count, got H={num_heads}")
    if head_dim not in SUPPORTED_HEAD_DIMS:
        raise ValueError(f"V2-G requires D in {SUPPORTED_HEAD_DIMS}, got D={head_dim}")
...
def _validate_qkv(q, k, v):
    for name, tensor in (("q", q), ("k", k), ("v", v)):
        _validate_nhd_shape(name, tensor)
    if any(
        x.shape != q.shape or x.dtype != q.dtype or x.device != q.device for x in (k, v)
    ):
        raise ValueError("q, k, and v must have identical shape, dtype, and device")


def _validate_tensor_spec(name, tensor, shape, dtype, device):
    ...
    if (
        ...
        raise ValueError(
            f"{name} must have shape {tuple(shape)}, dtype {dtype}, device {device}"
        )
    if not tensor.is_contiguous():
        raise ValueError(f"{name} must be contiguous")


def _validate_stats_context(gathered, ctx, k, q_group, k_group):
    ...
    if ctx.stats_protocol != BOUNDARY_MERGE:
        raise ValueError("ctx must use BOUNDARY_MERGE")
    _world_size(ctx.world_size)
    _rank(ctx.rank, ctx.world_size)
    _require_tensor("k", k)
    _validate_live_length(
        "used_sequence", ctx.used_sequence, ctx.local_sequence * ctx.world_size
    )
    if ctx.stats_numel != ctx.batch_size * ctx.num_heads * ctx.head_dim:
        raise ValueError("invalid context stats_numel")
    if (
        tuple(k.shape)
        != (ctx.batch_size, ctx.local_sequence, ctx.num_heads, ctx.head_dim)
        or k.dtype != ctx.input_dtype
    ):
        raise ValueError("k must match the same shard used by local_stats")
    if ctx.q_group != q_group or ctx.k_group != k_group:
        raise ValueError("ctx belongs to a different quantization layout")
    _validate_tensor_spec(
        "ctx.q_amax",
        ctx.q_amax,
        (ctx.batch_size, ctx.num_heads, slots(ctx.local_sequence, q_group)),
        torch.float32,
        k.device,
    )
    expected_desc = (ctx.batch_size, ctx.num_heads, 2)
    expected_minmax = (*expected_desc, 2, ctx.head_dim)
    if ctx.q_desc_shape != expected_desc or ctx.k_minmax_shape != expected_minmax:
        raise ValueError("invalid context descriptor shapes")
    per_rank = (
        2 * ctx.batch_size * ctx.num_heads * ctx.head_dim
        + math.prod(expected_desc)
        + math.prod(expected_minmax)
    )
    _require_tensor("gathered", gathered)
    if gathered.dtype != torch.float32:
        raise TypeError("gathered must be the fp32 AllGather of local_stats sends")
    if tuple(gathered.shape) not in (
        (ctx.world_size * per_rank,),
        (ctx.world_size, per_rank),
    ):
        raise ValueError(
            f"gathered has shape {tuple(gathered.shape)}; expected a rank-major AllGather of {ctx.world_size} x {per_rank} elements"
        )
    _validate_tensor_spec("gathered", gathered, gathered.shape, torch.float32, k.device)


def _validate_final_stats(stats, q, q_group, k_group):
    if not isinstance(stats, V2GStats):
        raise TypeError("stats must be the V2GStats returned by finalize_stats")
    if stats.stats_protocol != BOUNDARY_MERGE or (stats.q_group, stats.k_group) != (
        q_group,
        k_group,
    ):
        raise ValueError("stats must use this layout's BOUNDARY_MERGE protocol")
    _world_size(stats.world_size)
    _rank(stats.rank, stats.world_size)
    b, length, heads, dim = q.shape
    if heads % stats.world_size:
        raise ValueError("num_heads must be divisible by world_size")
    _validate_live_length(
        "used_sequence", stats.used_sequence, length * stats.world_size
    )
    for name, shape, dtype in (
        ("k_mean_global", (b, heads, dim), q.dtype),
        ("v_scale_global", (b, heads, dim), torch.float32),
        ("q_amax_final", (b, heads, slots(length, q_group)), torch.float32),
        ("k_amax_final", (b, heads, slots(length, k_group)), torch.float32),
    ):
        _validate_tensor_spec(name, getattr(stats, name), shape, dtype, q.device)


def _validate_live_length(name, value, total):
    if value is not None:
        _positive_int(name, value)
        if value > total:
            raise ValueError(f"{name} must lie in (0, local_sequence * world_size]")
...
# Which layout each architecture runs, and hence which module a probe on that
# architecture is allowed to build.  The two translation units are not
# interchangeable: the SM89/SM120 module is compiled ``code=sm_89`` +
# ``code=sm_120a`` and the SM90 module ``code=sm_90a``, SASS only with no
# embedded PTX, so a module built for one has no launchable image on the other.
# A probe must follow the device rather than a fixed default.
_ARCH_LAYOUT_CLASS: Dict[Tuple[int, int], str] = {
    (8, 9): "UlyssesLowpSageLayout",
    (12, 0): "UlyssesLowpSageLayout",
    (9, 0): "UlyssesLowpSageLayoutSM90",
}
...
def _device_capability(
    ...
    """``(major, minor)`` of the selected CUDA device, or None when CUDA is
    unavailable or ``device`` names a non-CUDA device."""
    ...
    if cuda_device.index is None:
        cuda_device = torch.device("cuda", torch.cuda.current_device())
    return tuple(torch.cuda.get_device_capability(cuda_device))
...
def capability(
    ...
    """Describe availability of the ulysses_lowp kernels on the current device.

    Reports the grid the *device* runs rather than one fixed grid: ``(8, 9)``
    (SM89) and ``(12, 0)`` (SM120) run :class:`UlyssesLowpSageLayout`
    (Q_GROUP=32, K_GROUP=64); ``(9, 0)`` (SM90 Hopper) runs
    :class:`UlyssesLowpSageLayoutSM90` (Q_GROUP=16, K_GROUP=128).  Supported
    means that architecture's module loaded and its compiled layout constants
    ...
    ``supported_head_dims`` lists the compiled dimensions (64 and 128).
    ``compiled_head_dim`` retains the default dimension, 128, for compatibility.

    Because the answer follows the device, ``supported`` alone does not mean
    "the module-level functions will run": those stay pinned to the SM89/SM120
    grid.  A caller pinned to one grid must check ``layout_class``.

    Args:
        device: CUDA device to probe; ``None`` uses the current device.

    Returns:
        Dictionary of device capability, compiled grid, supported head dimensions,
        availability and layout class name.
    """

    device_capability = _device_capability(device)
    layout_class = _ARCH_LAYOUT_CLASS.get(device_capability)
    # Probe only the module belonging to the layout this device runs; an
    # architecture with no layout probes nothing at all.
    probe, expected = {
        "UlyssesLowpSageLayout": (
            get_ulysses_lowp_module,
            (Q_GROUP, K_GROUP, HEAD_DIM),
        ),
        "UlyssesLowpSageLayoutSM90": (
            get_ulysses_lowp_sm90_module,
            (Q_GROUP_SM90, K_GROUP_SM90, HEAD_DIM),
        ),
    }.get(layout_class, (None, None))
    ...
    if probe is not None:
        try:
            ...
            supported_head_dims = tuple(
                d for d in SUPPORTED_HEAD_DIMS if mod.ulysses_lowp_supports_head_dim(d)
            )
        except Exception:  # noqa: BLE001 — probe never raises; zeroes signal mismatch
            pass
    supported = bool(
        expected is not None
        and (compiled_q_group, compiled_k_group, compiled_head_dim) == expected
    ...
    return {
        ...
        "supported": supported,
        "layout_class": layout_class if supported else None,
    }


def _reported_grid_is(
    layout: Any, device: Optional[Union[int, str, torch.device]]
) -> bool:
    """True when :func:`capability` reports support for exactly ``layout``'s
    grid.  ``capability()`` follows the device, so a bare ``supported`` check
    would let the SM89/SM120 layout claim a Hopper device (and vice versa);
    every layout must also match the reported compiled constants."""

    cap = capability(device=device)
    return (
        bool(cap["supported"])
        and (
            cap["compiled_q_group"],
            cap["compiled_k_group"],
        )
        == (layout.Q_GROUP, layout.K_GROUP)
        and layout.HEAD_DIM in cap["supported_head_dims"]
    )
...
def payload_spec(
    *,
    ...
    Args:
        ...
        local_sequence: Number of tokens per source shard, including zero padding.
        num_heads: Total heads before the all-to-all; payload packing requires
            divisibility by ``world_size``.
        head_dim: Head dimension, 64 or 128; must agree on every rank.
        world_size: Ulysses group size, one of 2, 4, 6 or 8; 6 is supported structurally
            but has not been validated.

    Returns:
        Dictionary containing byte offsets, per-destination ``chunk_bytes``, total
        ``payload_bytes``, scale counts and sequence sizes.
    """
...
def k_sum_v_amax(
    ...
    *,
    ...
    Both inputs use canonical NHD ``[B, L, H, D]`` storage. The returned
    ``[B, H, D]`` tensors are local statistics; callers must synchronize
    ...
    ``[B, H, ceil(L/256), D]`` fp32 workspaces are allocated here and
    ...
    Args:
        k: BF16/FP16 CUDA K shard of shape ``[B, L, H, D]``, with the same layout and
            alignment requirements as Q.
        v: BF16/FP16 CUDA V shard of shape ``[B, L, H, D]``, with the same layout and
            alignment requirements as Q.
        out: Optional pair of contiguous FP32 ``[B, H, D]`` output tensors on the input
            device.
        enable_pdl: Whether to enable Programmatic Dependent Launch. ``None`` selects
            according to device support.

    Returns:
        Pair ``(k_sum, v_amax)`` of FP32 CUDA tensors of shape ``[B, H, D]``.
    """
...
# ---------------------------------------------------------------------------
# Local grouped amax; crossing groups are finalized by boundary merge.
# ---------------------------------------------------------------------------
...
def q_grouped_amax(
    ...
    *,
    ...
    """Per-touched-global-group ``abs(Q)`` partial amax, ``[B, H, slots(L,32)]``.

    Valid slots are ``[0, touched)``; the surplus allocation stays zero.

    Args:
        q: BF16/FP16 CUDA Q shard of shape ``[B, L, H, D]``; D is 64 or 128. The last
            dimension must be dense and every row 16-byte aligned.
        rank: Sequence-shard rank in ``[0, world_size)``.
        world_size: Ulysses group size, one of 2, 4, 6 or 8; 6 is supported structurally
            but has not been validated.
        enable_pdl: Whether to enable Programmatic Dependent Launch. ``None`` selects
            according to device support.
    ...
    Returns:
        FP32 CUDA partial maxima of shape ``[B, H, slots(L, 32)]``.
    """
...
def k_grouped_amax(
    ...
    *,
    ...
    """Per-touched-global-group ``abs(K - global mean)`` partial amax, ``[B, H, slots(L,64)]``.
    ...
    Tail padding may span many trailing K groups.
    ...
    Args:
        k: BF16/FP16 CUDA K shard of shape ``[B, L, H, D]``, with the same layout and
            alignment requirements as Q.
        k_mean_global: Contiguous ``[B, H, D]`` global K mean, rounded to the input
            dtype.
        rank: Sequence-shard rank in ``[0, world_size)``.
        world_size: Ulysses group size, one of 2, 4, 6 or 8; 6 is supported structurally
            but has not been validated.
        used_sequence: Live global token count. ``None`` uses all ``L * world_size``
            rows. Any remaining input rows must be zero-filled.
        enable_pdl: Whether to enable Programmatic Dependent Launch. ``None`` selects
            according to device support.

    Returns:
        FP32 CUDA partial maxima of shape ``[B, H, slots(L, 64)]``.
    """
...
def _k_boundary_minmax_impl(k, rank, world_size, used_sequence, group):
    """Raw boundary min/max on either architecture's global K grid."""
    ...
    g_first = group_first(rank, local_sequence, group)
    g_last = group_last(rank, local_sequence, group)
    base = rank * local_sequence
    for slot, group_id in enumerate((g_first, g_last)):
        lo = max(group_id * group, base)
        hi = min((group_id + 1) * group, base + local_sequence, used)
        if hi > lo:
...
def _derive_k_boundary_amax_impl(
    grouped_amax,
    gathered_minmax,
    k_mean_global,
    rank,
    ...
    """Merge centered boundary amax using the selected global K grid."""
...
def _repair_k_tail_amax(amax, k, k_mean_global, rank, world_size, used_sequence, group):
    """Exclude centered padding from the one group mixing live and padded rows.

    Every rank touching that group repairs only its live slice, before the
    cross-rank boundary merge. Fully padded groups are not consumed by Sage.
    """
    ...
    if (
        ...
        and int(used_sequence) % group
    ):
        used = int(used_sequence)
        tail_group = (used - 1) // group
        g_first = (rank * local_sequence) // group
        g_last = (rank * local_sequence + local_sequence - 1) // group
        if g_first <= tail_group <= g_last:
            lo = max(tail_group * group, rank * local_sequence)
...
def boundary_descriptors(
    ...
    *,
    ...
    Args:
        grouped_amax: FP32 local maxima ``[B, H, slots(local_sequence, group)]``.
        rank: Sequence-shard rank in ``[0, world_size)``.
        local_sequence: Number of tokens per source shard, including zero padding.
        group: Positive token count per quantization group.
        world_size: Ulysses group size, one of 2, 4, 6 or 8; 6 is supported structurally
            but has not been validated.

    Returns:
        FP32 CUDA boundary maxima of shape ``[B, H, 2]``.
    """
...
def merge_boundary_amax(
    ...
    *,
    ...
    Args:
        grouped_amax: FP32 local maxima ``[B, H, slots(local_sequence, group)]``.
        gathered_descriptors: FP32 ``[world_size, B, H, 2]`` descriptors in group-rank
            order.
        rank: Sequence-shard rank in ``[0, world_size)``.
        local_sequence: Number of tokens per source shard, including zero padding.
        group: Positive token count per quantization group.
    ...
    Returns:
        The input ``grouped_amax`` tensor, with boundary slots updated in place.
    """
...
def k_boundary_minmax(
    ...
    *,
    ...
    boundary group's exact ``abs(K - mean)`` amax locally (see
    ...
    Padding may span multiple groups or whole ranks. Empty boundary slices
    contribute only the amax floor after derivation.
    ...
    Returns:
        FP32 CUDA boundary min/max of shape ``[B, H, 2, 2, D]``.
    """
    ...
    return _k_boundary_minmax_impl(k, rank, world_size, used_sequence, K_GROUP)
...
def derive_k_boundary_amax(
    ...
    *,
    ...
    which is bit-equal to r's ``abs(K - m)`` partial amax over its slice of g: fp32
    ...
    Args:
        grouped_amax: FP32 ``[B, H, slots(local_sequence, 64)]`` local centered-K maxima
            to update in place.
        gathered_minmax: FP32 ``[world_size, B, H, 2, 2, D]`` raw-K boundary min/max in
            group-rank order.
        k_mean_global: Contiguous ``[B, H, D]`` global K mean, rounded to the input
            dtype.
        rank: Sequence-shard rank in ``[0, world_size)``.
    ...
    return _derive_k_boundary_amax_impl(
        grouped_amax,
        gathered_minmax,
        k_mean_global,
        rank,
...
def zero_scale_and_padding(
    ...
    Args:
        send_u8: Contiguous CUDA uint8 destination-major buffer ``[world_size,
            chunk_bytes]`` from the matching payload specification.
        spec: Payload geometry returned by the matching layout's ``payload_spec``.
    """
...
def quant_q_into_payload(
    ...
    *,
    ...
    Args:
        q: BF16/FP16 CUDA Q shard of shape ``[B, L, H, D]``; D is 64 or 128. The last
            dimension must be dense and every row 16-byte aligned.
        q_amax_final: Contiguous FP32 ``[B, H, slots(L, 32)]`` Q maxima after the
            cross-rank boundary merge.
        send_u8: Contiguous CUDA uint8 destination-major buffer ``[world_size,
            chunk_bytes]`` from the matching payload specification.
        rank: Sequence-shard rank in ``[0, world_size)``.
        world_size: Ulysses group size, one of 2, 4, 6 or 8; 6 is supported structurally
...
def quant_kv_into_payload(
    ...
    *,
    ...
    Args:
        ...
        k_mean_global: Contiguous ``[B, H, D]`` global K mean, rounded to the input
            dtype.
        k_amax_final: Contiguous FP32 ``[B, H, slots(L, 64)]`` centered-K maxima after
            the cross-rank boundary merge.
        v_scale_global: Contiguous FP32 ``[B, H, D]`` global per-channel V divisor,
            ``global_amax / 2.25``; zero-amax channels keep scale 0 and quantize to zero.
        send_u8: Contiguous CUDA uint8 destination-major buffer ``[world_size,
            chunk_bytes]`` from the matching payload specification.
        rank: Sequence-shard rank in ``[0, world_size)``.
        world_size: Ulysses group size, one of 2, 4, 6 or 8; 6 is supported structurally
...
def quant_qkv_pack(
    ...
    *,
    ...
    Args:
        ...
        world_size: Ulysses group size, one of 2, 4, 6 or 8; 6 is supported structurally
            but has not been validated.
        out: Optional contiguous uint8 ``[world_size, chunk_bytes]`` output on the input
            device. Scale slots and alignment padding are initialized on every call.
        enable_pdl: Whether to enable Programmatic Dependent Launch. ``None`` selects
            according to device support.

    Returns:
        Contiguous uint8 CUDA payload of shape ``[world_size, chunk_bytes]``.
    """
...
def scale_widths(sequence: int) -> Tuple[int, int]:
    ...
    Args:
        sequence: Positive sequence length presented to the SageAttention consumer.

    Returns:
        Tuple of per-head Q and K scale counts.
    """
...
def unpack_for_sage(
    ...
    *,
    ...
    Returns contiguous logical Q/K ``[B,S,h,D]``, globally packed V
    ``[B,D,h,ceil(S/64)*64]`` with a zero global tail, and global-grid Q/K
    scale tensors at the requested consumer width. Extra Q scale slots
    outside the live prefix contain initialized finite values.
    ...
    ``aligned=True`` asserts L % 128 == 0; None and False accept any
    legal shard. All values use the general receiver. Extra Q scale slots
    outside the live prefix are initialized but need not be zero.

    Args:
        recv_u8: Contiguous CUDA uint8 source-major buffer ``[world_size, chunk_bytes]``
            received by all-to-all.
        batch_size: Positive batch size B.
        local_sequence: Number of tokens per source shard, including zero padding.
        local_heads: Heads owned by this destination after the all-to-all, ``H //
            world_size``.
        head_dim: Head dimension, 64 or 128; must agree on every rank.
        world_size: Ulysses group size, one of 2, 4, 6 or 8; 6 is supported structurally
            but has not been validated.
        aligned: ``True`` additionally requires ``local_sequence % 128 == 0``; ``False``
            and ``None`` accept arbitrary legal shards.
        scale_sequence: Live prefix length determining Q/K scale widths; ``None`` uses
            the full global sequence.
        out: Optional tuple ``(q8, k8, v8, q_scale, k_scale)`` of contiguous tensors on
            the receive device, with shapes and dtypes matching the return value.
        enable_pdl: Whether to enable Programmatic Dependent Launch. ``None`` selects
            according to device support.

    Returns:
        Tuple ``(q8, k8, v8, q_scale, k_scale)``: INT8 Q/K ``[B, S, h, D]``, FP8 E4M3 V
        ``[B, D, h, ceil(S/64)*64]``, and FP32 scales ``[B, h, width]``. V scale remains
        with the caller and must be passed separately to attention.
    """
    ...
    if aligned is not None and not isinstance(aligned, bool):
        raise TypeError("aligned must be None or bool")
    if aligned and local_sequence % 128:
        raise ValueError(
            "ALIGN-128 input assertion: local_sequence must be a whole "
            f"number of 128-token blocks, got {local_sequence}; pass "
            "aligned=False or None for arbitrary shard lengths"
        )
    spec = payload_spec(
        batch_size=batch_size,
        local_sequence=local_sequence,
    ...
    _validate_live_length("scale_sequence", scale_sequence, logical_sequence)
    ...
    for name, tensor, shape, dtype in (
        ...
        ("q_scale", q_scale, (batch_size, local_heads, q_scale_width), torch.float32),
...
# ---------------------------------------------------------------------------
# Boundary statistics pipeline. Padding policy does not select a kernel.
#
...
def stats_protocol_for(local_sequence: int, world_size: int) -> str:
    """Return BOUNDARY_MERGE for every legal shard length.
    ...
    Returns:
        The ``BOUNDARY_MERGE`` protocol name.
    """
...
def required_alignment(world_size: int, stats_protocol, k_group: int = K_GROUP) -> int:
    ...
    Args:
        world_size: Ulysses group size, one of 2, 4, 6 or 8; 6 is supported structurally
            but has not been validated.
        stats_protocol: ``ALIGNED`` or ``BOUNDARY_MERGE`` (legacy integer IDs 3 and 2
            are also accepted).
        k_group: K tokens per quantization group: 64 for SM89/SM120, 128 for SM90.

    Returns:
        Positive alignment multiple for the global sequence.
    """
...
def aligned_length(
    n_tokens: int, world_size: int, stats_protocol: str, k_group: int = K_GROUP
) -> int:
    ...
    ``k_group`` is forwarded to :func:`required_alignment` and must be the
    grid the payload will be packed with, otherwise the two disagree: the
    boundary-merge alignment IS the K group, so rounding to a 64-token
    multiple on the SM90 grid still leaves a global sequence that splits a
    128-token K group.

    Args:
        n_tokens: Positive live global sequence length before padding.
        world_size: Ulysses group size, one of 2, 4, 6 or 8; 6 is supported structurally
            but has not been validated.
        stats_protocol: ``ALIGNED`` or ``BOUNDARY_MERGE`` (legacy integer IDs 3 and 2
            are also accepted).
    ...
    Returns:
        Padded global sequence length.
    """

    n_tokens = _positive_int("n_tokens", n_tokens)
    alignment = required_alignment(world_size, stats_protocol, k_group=k_group)
...
@dataclasses.dataclass(repr=False, eq=False)
class V2GStats:
    """Final global statistics consumed directly by quant_and_pack."""
...
def local_stats(
    ...
    *,
    ...
    """Everything the single boundary-statistics AllGather must carry.

    Returns a contiguous 1-D fp32 tensor (identical numel on every rank) and
    the context for :func:`finalize_stats`. Every shard sends the K
    per-channel sum and V per-channel amax, plus the
    ...
    min=0/max=0, and ``abs(0 - mean)`` would re-create the tail-group scale
    ...
    Returns:
        Tuple of a contiguous one-dimensional FP32 stats send tensor and its
        ``StatsContext``; this function does not issue a collective.
    """
...
def finalize_stats(
    ...
    *,
    ...
    Args:
        gathered: AllGather output in group-rank order, FP32 ``[world_size, N]`` or
            flattened, where N is the local stats send size.
        ctx: Context returned by ``local_stats`` on this rank.
        k: The same local BF16/FP16 K shard used for ``local_stats``.
        enable_pdl: Whether to enable Programmatic Dependent Launch. ``None`` selects
            according to device support.

    Returns:
        ``V2GStats`` with global mean, scales, finalized Q/K maxima and shard geometry.
    """

    if not isinstance(ctx, StatsContext):
        raise TypeError("ctx must be the StatsContext returned by local_stats")
    _validate_stats_context(gathered, ctx, k, Q_GROUP, K_GROUP)
    batch, local_sequence, num_heads, head_dim = _validate_nhd_input("k", k)
...
def quant_and_pack(
    ...
    *,
    ...
    """Quantize and pack using the finalized global-grid Q/K amax and V scale.
    ...
    Args:
        ...
        v: BF16/FP16 CUDA V shard of shape ``[B, L, H, D]``, with the same layout and
            alignment requirements as Q.
        stats: Finalized quantization inputs returned by ``finalize_stats``.
        out: Optional contiguous uint8 ``[world_size, chunk_bytes]`` output on the input
            device.
        enable_pdl: Whether to enable Programmatic Dependent Launch. ``None`` selects
            according to device support.
...
class UlyssesLowpSageLayout:
    ...
    Set ``head_dim=64`` or ``head_dim=128`` (default) when constructing the
    layout; reuse it for inputs with that dimension.
    ...
    def _check_head_dim(self, tensor: torch.Tensor) -> None:
        _require_tensor("tensor", tensor)
        if tensor.ndim != 4 or tensor.shape[-1] != self.HEAD_DIM:
            raise ValueError(
                f"tensor must have shape [B,L,H,{self.HEAD_DIM}] for this layout"
            )
    ...
    def is_supported(
        ...
        The grid check is not redundant with ``supported``: ``capability()``
        reports whichever grid the device runs, and on SM90 that is the SM90
        layout, whose module this class must never be dispatched onto."""
        return _reported_grid_is(self, device)
...
class UlyssesLowpSageLayoutSM90(UlyssesLowpSageLayout):
    ...
    def is_supported(self, device=None) -> bool:
        ...
        if _device_capability(device) != (9, 0):
            # Cheap architecture guard, kept so that probing this layout on an
            # SM89/SM120 box does not build that box's module as a side effect.
            # Correctness comes from the grid check below.
            return False
        return _reported_grid_is(self, device)
    ...
    def aligned_length(
        ...
        """``n_tokens`` rounded up to this layout's :meth:`required_alignment`."""
        return aligned_length(
            n_tokens, world_size, stats_protocol, k_group=self.K_GROUP
        )
    ...
    def local_stats(
        ...
        *,
        ...
        _validate_qkv(q, k, v)
        if _device_capability(q.device) != (9, 0):
            raise ValueError("SM90 layout requires an SM90 device")
        self._check_head_dim(q)
        ...
        global_sequence = local_sequence * world_size
        _validate_live_length("used_sequence", used_sequence, global_sequence)
        # Same admission the module-level local_stats enforces.  Without it
        # used_sequence=0 reaches finalize_stats as the k_mean denominator and
        # silently produces a NaN mean rather than raising here.
        if used_sequence is not None and not 0 < int(used_sequence) <= global_sequence:
            raise ValueError(
                "used_sequence must lie in (0, local_sequence * world_size]"
            )
    ...
    def unpack_for_sage(
        ...
        *,
        ...
        _require_tensor("recv_u8", recv_u8)
        _validate_tensor_spec(
            "recv_u8",
            recv_u8,
            (world_size, spec["chunk_bytes"]),
        ...
        if _device_capability(recv_u8.device) != (9, 0):
            raise ValueError("SM90 layout requires an SM90 device")
        logical_sequence = int(spec["logical_sequence"])
        padded_sequence = int(spec["padded_sequence"])
        if scale_sequence is None:
            scale_sequence = logical_sequence
        scale_sequence = _positive_int("scale_sequence", scale_sequence)
        if scale_sequence > logical_sequence:
            raise ValueError(
                "scale_sequence must not exceed local_sequence * world_size"
            )
    ...
    def _k_boundary_minmax(k, rank, world_size, used_sequence):
        return _k_boundary_minmax_impl(k, rank, world_size, used_sequence, K_GROUP_SM90)

    @staticmethod
    def _k_grouped_amax(
        k, k_mean_global, rank, world_size, used_sequence, enable_pdl, mod
    ):
        batch, local_sequence, num_heads, head_dim = k.shape
        global_sequence = local_sequence * world_size
        _validate_live_length("used_sequence", used_sequence, global_sequence)
...
def verify_duplicate_scale_slots(
    ...
    *,
    ...
    Args:
        ...
        world_size: Ulysses group size, one of 2, 4, 6 or 8; 6 is supported structurally
            but has not been validated.
        q_group: Q tokens per quantization group: 32 for SM89/SM120, 16 for SM90.
        k_group: K tokens per quantization group: 64 for SM89/SM120, 128 for SM90.
        spec: Matching payload geometry. ``None`` derives the SM89/SM120 geometry;
            supply the SM90 layout specification for the SM90 grid.
    """
...
def quant_v_fp8_with_scale(
    ...
    *,
    ...
    Returns:
        A contiguous ``[B, S, H, D]`` uint8 tensor containing raw
    ...
    if head_dim not in SUPPORTED_HEAD_DIMS:
        raise ValueError(
            f"quant_v_fp8_with_scale requires D in {SUPPORTED_HEAD_DIMS}, got D={head_dim}"
        )
