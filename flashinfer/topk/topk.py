...
def top_k(
    ...
    backend : str, optional
        Backend to use for top-k. Options are:

        - ``"cuda"``: FlashInfer CUDA backend (default, stable)
        - ``"cute-dsl"``: CuTe-DSL single-pass multi-CTA radix top-k
          (requires SM100+, experimental, k <= 2048)
    ...
    if backend == "cute-dsl":
        ...
        from .kernels.top_k_cute_dsl import top_k_cute_dsl

        return top_k_cute_dsl(input, k, sorted=sorted)
