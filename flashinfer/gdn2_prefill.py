...
from .trace.templates.gdn import gdn2_prefill_trace
from .utils import (
...
def chunk_gated_delta_rule2(
    ...
    Implements the second-generation gated delta rule linear attention
    mechanism.  GDN-2 differs from GDN (:func:`chunk_gated_delta_rule`) by
    making the forget gate ``g`` and erase gate ``beta`` *channel-wise*
    (per head-dimension channel instead of per-head scalars) and adding a
    channel-wise write gate ``w``:

    .. math::

        S_t &= S_{t-1} \odot \mathrm{diag}(g_t) \\
    ...
    g : torch.Tensor, optional
        Channel-wise forget gate (alpha, raw decay in ``(0, 1]``, NOT log
        space) of shape ``[total_seq_len, num_sab_heads, head_size]`` where
        ``num_sab_heads = max(num_q_heads, num_v_heads)``.  Must be
        float32.  Defaults to all ones when ``None``.
    beta : torch.Tensor, optional
        Channel-wise erase gate of shape
        ``[total_seq_len, num_sab_heads, head_size]`` in ``q.dtype``
        (typical range ``[0, 2]``).  Defaults to all ones when ``None``.
    w : torch.Tensor, optional
        Channel-wise write gate of shape
        ``[total_seq_len, num_sab_heads, head_size]`` in ``q.dtype``
        (typical range ``[0, 1]``).  Defaults to all ones when ``None``.
    ...
    if _arch_major == 10:
        if _cuda_major < 13:
            raise NotImplementedError(
                "Blackwell GDN-2 prefill is only supported on CUDA 13+"
            )
        ...
        if _beta.dtype != q.dtype:
            _beta = _beta.to(q.dtype)
        _w = (
            w
            if w is not None
            else torch.ones(
        ...
        raise NotImplementedError(
            "GDN-2 prefill is only available on SM100 (Blackwell)"
        )
