...
def _prepare_sm90_sparse_metadata(
    ...
    *,
    ...
    """Validate and transpose SM90 sparse metadata into kernel ABI layout."""
...
def bsa_attn_sm90_blk64_fwd(
    ...
    """Forward pass for BSA block-sparse attention using the sm90_blk64 CuTe-DSL kernel (SM90 only).
    ...
    """
    from .sm120_blk64.flash_fwd_sm120 import (  # noqa: PLC0415
        BlockSparseAttnForwardSm120Blk64,
    )

    assert q.dtype in (torch.float16, torch.bfloat16), (
        "bsa_attn_sm90_blk64_fwd only supports fp16/bf16"
    )
    ...
    if arch // 10 != 9:
        raise RuntimeError(
            f"bsa_attn_sm90_blk64_fwd (sm90_blk64) only supports SM90, current device is SM{arch}"
        )
    ...
    assert head_dim == 128, f"sm90_blk64 requires head_dim=128, got {head_dim}"
    assert head_dim_v == 128, f"sm90_blk64 requires head_dim_v=128, got {head_dim_v}"
    ...
    # Transpose from BSHD to the layout expected by the sm90 kernel:
    ...
    if compile_key not in _sm90_compile_cache:
        _sm90_compile_cache[compile_key] = cute.compile(fwd_kernel, *args)

    if not is_fake_mode():
        with torch.cuda.nvtx.range("bsa_attn_sm90_blk64_fwd_kernel"):
            _sm90_compile_cache[compile_key](*args)
