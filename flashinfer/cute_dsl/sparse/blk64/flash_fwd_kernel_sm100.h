            ...
            *reinterpret_cast<volatile int*>(&shared_storage.tmem_ready) = 1;
            ...
            while (work.is_valid) {
                ...
                int raw_bc = CollectiveMainloop::get_tile_raw_block_count(
                        params.mainloop, work.batch, work.head, work.row_tile);
                load_state = mainloop.load(params.mainloop, pipeline_kv, shared_storage,
            ...
            while (work.is_valid) {
                ...
                if (params.mainloop.ptr_block_indices != nullptr) {
                    int tile_idx = (work.batch * params.num_heads + work.head) * params.num_row_tiles + work.row_tile;
                    tile_bi = params.mainloop.ptr_block_indices
                            + tile_idx * params.mainloop.block_indices_stride;
                }
                softmax1_state = mainloop.template softmax</*Stage=*/1, SharedStorage, NamedBarriers>(
                        ...
                        shared_storage, tmem_base, params.mainloop.sm_scale_log2, tile_nkv, softmax1_state,
                        tile_bi, params.mainloop.ptr_block_sizes,
            ...
            while (work.is_valid) {
                ...
                softmax0_state = mainloop.template softmax</*Stage=*/0, SharedStorage, NamedBarriers>(
                        pipeline_s_p_o, pipeline_sm_stats, pipeline_p_lastsplit,
                        shared_storage, tmem_base, params.mainloop.sm_scale_log2, tile_nkv, softmax0_state,
                        tile_bi, params.mainloop.ptr_block_sizes,
                        CollectiveMainloop::get_tile_raw_block_count(
                            params.mainloop, work.batch, work.head, work.row_tile));
                work = tile_sched.consumer_advance();
...
using FusedAttnKernel = FusedAttnFwdSm100<CollectiveMainloopFwd<HasVariableBlockNums, HasBlockSizes>, CollectiveEpilogueFwd>;
