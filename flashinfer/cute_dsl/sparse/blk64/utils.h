...
struct SM100_MMA_F16BF16_WS_TS_NOELECT {
  ...
  fma(uint32_t const& tmem_a, uint64_t const& desc_b,
      uint32_t const& tmem_c, uint32_t const& scale_c, uint64_t const& idesc_e) {
    asm volatile(
        "{\n\t"
        ".reg .pred p;\n\t"
        "setp.ne.b32 p, %4, 0;\n\t"
...
template <class a_type, class b_type, class c_type, int M, int N,
          UMMA::Major a_major, UMMA::Major b_major, UMMA::ScaleIn a_neg, UMMA::ScaleIn b_neg>
struct MMA_Traits<SM100_MMA_F16BF16_WS_TS_NOELECT<a_type, b_type, c_type, M, N, a_major, b_major, a_neg, b_neg>> {
  ...
  template <class TD, class DLayout, class TA, class ALayoutX, class TB, class BLayoutX, class TC, class CLayoutX>
  CUTE_HOST_DEVICE constexpr friend void
  mma_unpack(MMA_Traits const& traits, Tensor<TD, DLayout>& d,
             Tensor<TA, ALayoutX> const& a, Tensor<TB, BLayoutX> const& b, Tensor<TC, CLayoutX> const& c) {
    static_assert(is_tmem<TD>::value && is_tmem<TA>::value && is_rmem<TB>::value && is_tmem<TC>::value);
    SM100_MMA_F16BF16_WS_TS_NOELECT<a_type, b_type, c_type, M, N, a_major, b_major, a_neg, b_neg>::fma(
        raw_pointer_cast(a.data()), b[0], raw_pointer_cast(d.data()),
        uint32_t(traits.accumulate_), UMMA::make_runtime_instr_desc<>(traits.idesc_));
  }
};
...
struct MMA_Traits<SM100_MMA_F16BF16_WS_SS_NOELECT<a_type, b_type, c_type, M, N, a_major, b_major, a_neg, b_neg>> {
  ...
  mma_unpack(MMA_Traits const& traits, Tensor<TD, DLayout>& d,
             Tensor<TA, ALayoutX> const& a, Tensor<TB, BLayoutX> const& b, Tensor<TC, CLayoutX> const& c) {
    static_assert(is_tmem<TD>::value && is_rmem<TA>::value && is_rmem<TB>::value && is_tmem<TC>::value);
    SM100_MMA_F16BF16_WS_SS_NOELECT<a_type, b_type, c_type, M, N, a_major, b_major, a_neg, b_neg>::fma(
        a[0], b[0], raw_pointer_cast(d.data()),
        uint32_t(traits.accumulate_), UMMA::make_runtime_instr_desc<>(traits.idesc_));
  }
};
...
__device__ __forceinline__ void tmem_load(uint32_t tmem_start, float* data_) {
    ...
    static_assert(
        kNumElements == 1 || kNumElements == 2 || kNumElements == 4 || kNumElements == 8 ||
        kNumElements == 16 || kNumElements == 32 || kNumElements == 64 || kNumElements == 128);
...
__device__ __forceinline__ void utcmma_ss(
        TiledMma& tiled_mma, TensorA sA, TensorB sB, TensorFragC tC_frag, bool clear_accum) {
    ...
    constexpr int kAExt3 = []() constexpr {
        if constexpr (kARank > 3) { return decltype(size<3>(sA_frag))::value; } else { return 1; }
    }();
    constexpr int kAExt4 = []() constexpr {
        if constexpr (kARank > 4) { return decltype(size<4>(sA_frag))::value; } else { return 1; }
    }();
    constexpr int kBExt3 = []() constexpr {
        if constexpr (kBRank > 3) { return decltype(size<3>(sB_frag))::value; } else { return 1; }
    }();
        ...
        if constexpr (kARank == 5 && kBRank == 4) {
            if constexpr (kAExt4 == 1 && kAExt3 == kBExt3) {
                ...
                for (int dual_k = 0; dual_k < kAExt3; ++dual_k) {
                    cute::gemm(tiled_mma, sA_frag(_, _, k, dual_k, Int<0>{}), sB_frag(_, _, k, dual_k), tC_frag);
                    tiled_mma.accumulate_ = UMMA::ScaleOut::One;
                }
            } else if constexpr (kAExt3 == 1 && kAExt4 == kBExt3) {
                CUTE_UNROLL
                for (int dual_k = 0; dual_k < kAExt4; ++dual_k) {
                    cute::gemm(tiled_mma, sA_frag(_, _, k, Int<0>{}, dual_k), sB_frag(_, _, k, dual_k), tC_frag);
                    tiled_mma.accumulate_ = UMMA::ScaleOut::One;
                }
            }
...
auto make_umma_k_major_layout() {
    ...
    using base_atom_type = std::conditional_t<
        K == 0 || K == 16,
        UMMA::Layout_K_INTER_Atom<Element>,
        std::conditional_t<K == 32, UMMA::Layout_K_SW32_Atom<Element>,
        std::conditional_t<K == 64, UMMA::Layout_K_SW64_Atom<Element>,
