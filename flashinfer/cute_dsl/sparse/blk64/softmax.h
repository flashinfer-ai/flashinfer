...
static __device__ __forceinline__ void update_row_max(
        Tensor const& s_data, float sm_scale,
        float& row_max, float& acc_scale, float& row_max_safe)
{
...
template<bool IsFirst, typename Tensor>
static __device__ __forceinline__ void update_row_sum(
        Tensor const& s_data, float acc_scale, float& row_sum)
{
    if constexpr (IsFirst) {
