...
def is_cvt_rs_supported(device: torch.device = None) -> bool:
    ...
    Datacenter Blackwell: SM100 (B200, cc 10.0) and SM103 (B300, cc 10.3).
    Rubin: SM107 (cc 10.7). All require architecture-specific 'a' targets.
    ...
    include/flashinfer/mamba/conversion.cuh (SM100_ALL || SM103_ALL || SM107_ALL).
    """
    if device is None:
        device = torch.device("cuda")
    # Match the CUDA guard exactly: only the arches where cvt.rs is supported.
    # SM100a/SM103a were verified via ptxas; SM107a is listed in the PTX ISA.
