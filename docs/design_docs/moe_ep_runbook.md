...
`flashinfer/moe_ep/kernel_src/cutedsl/src/` and are exposed
through `kernel_src/cutedsl/` shim APIs (e.g. `mxfp8_mega_moe`,
...
`kernel_src/cutedsl/` (alongside `nvfp4.py` / `mxfp8.py`) and re-export them
from its `__init__.py` (or point at your own kernel module). Raw kernel sources
live under `kernel_src/cutedsl/src/` — see `kernel_src/cutedsl/SKILL.md` for
how to update that directory when the kernel team ships a new drop. The kernel-specific tuning knobs
