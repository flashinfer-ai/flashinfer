...
Full-group CUDA P2P is required even on the RDMA routes because every exchange's opening and closing epoch barriers read CUDA-mapped signals from all ranks. All routes share one translation unit, so the JIT compile/link needs the libibverbs and libmlx5 headers/libraries and the CUDA driver library (`-lcuda`) even when topology selects all-P2P; runtime verbs requirements apply only to the RDMA routes. Topology probing selects one GID index per rank and native initialization revalidates it (port active, RoCE v2, IPv4-mapped) before creating QPs. A forced backend raises collectively when no route is available.
