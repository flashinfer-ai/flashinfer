...
The two trigger paths are asymmetric, and that is why the declaration lives in the PR body. GitLab's `/bot run TEST_PATH` carries the target inline, so a reviewer can already scope a run by hand. GitHub's triggers — the `run-ci` label and `@flashinfer-bot run` — carry no argument, so a GitHub run has no other channel for learning what to test. Reading the declared block is what lets the same targeted run happen on both. `pr-test.yml` adds one extra H100 lane for PRs carrying the `experimental` label, running only the declared targets; the existing lanes are untouched, so a normal PR is unaffected.

Two properties of that lane are worth stating, because they are easy to erode later:

* **Declared targets must exist.** A path that does not exist collects no tests and passes — a false green, which is the most expensive way for this to fail because it looks like coverage. The validator checks existence in the checkout, so a typo fails the matrix rather than quietly reporting success.
* **The lane decides scope, not trust.** Whether a PR's code — *including its tests* — may run on the runners is settled upstream, by the screening that gates the `run-ci` label; a new test file is part of the diff that decision covers. The workflow therefore does not re-derive trust from the declaration. It does keep the PR body's *handling* defensive, for a reason independent of trust: `run-ci` can be applied by hand or via `@flashinfer-bot run` without any screening having run, so the workflow has to be safe on its own terms. The validator restricts targets to a strict charset (rejecting shell metacharacters, command substitution and smuggled flags outright, rather than inferring safety from a path prefix), the value is passed via `env` and never interpolated into script text, and the shell reads it into an array with globbing disabled instead of relying on word splitting. Each layer holds without the others.
