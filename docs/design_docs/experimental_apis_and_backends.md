...
* **The lane decides scope, not trust.** Whether a PR's code — *including its tests* — may run on the runners is settled upstream, by the screening that gates the `run-ci` label; a new test file is part of the diff that decision covers. The workflow therefore does not re-derive trust from the declaration. It does treat the PR body as untrusted *input*: the value is passed via `env`, written to a file, and never interpolated into script text, since `${{ }}` interpolation of PR-authored content into a `run:` block is a script-injection vector.
