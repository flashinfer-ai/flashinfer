...
| [x] | Wire `ExpertConfig.local_expert_offset` into TRTLLM `pack_inputs(...)` (read from config) and add an EP-offset test (`TestTrtllmEPOffset`) for the pre-routed path. | CR3 |
...
| --- | --- | --- | --- | --- |
| G1 | [x] | IwakuraRein · `api.py:73` | Routing/quant enums here duplicate `tllm_enums.py`; consider removing `tllm_enums.py` and moving its contents into `api.py`. Relates to C42 (keep enums on a shared home, don't fork a parallel surface). | **Done — consolidated onto the shared home (the realization of C42).** The literal suggestion (move into `api.py`) is inverted: `flashinfer/tllm_enums.py` is the *top-level* shared module that `gemm`/`quantization`/`__init__` import, so collapsing it into `fused_moe` would invert layering. Instead we deleted the duplicate API enums (`RoutingMethod`, `Activation`) and the unified API now uses the kernel `IntEnum`s `RoutingMethodType`/`ActivationType` directly — single source of truth, no parallel surface. Since these enums are unmerged we *bent the new code to the established names* rather than aliasing (lowest cost, one spelling). `tllm_enums.py` gained an eval-safe `__repr__` (so `eval(repr(cfg))` still round-trips) and `ActivationType.is_gated`. Sentinels (`RoutingMethodType.Unspecified=9`, `ActivationType.InvalidType=8`) are now exposed on the public enum — accepted; `MoELayer`'s `NotImplementedError` guards gate unsupported members. **`QuantVariant` stays an API-local plain `Enum`**: it has no single kernel counterpart (the quant path is chosen by dtype/scale wiring in the runners), so there is nothing to consolidate. Design rule recorded: *`IntEnum` where the value is a real kernel-ABI int; plain `Enum` for the API-only tag.* |
| G2 | [x] | IwakuraRein · `api.py:140` | Add swizzle-layout and per-token-scale configuration to the config surface (likely `QuantConfig`). | **Done (partial).** Added `swizzled_scale_factors: Optional[bool]` and `per_token_scale: Optional[bool]` to `QuantConfig`, both defaulting to `None` (= backend default). Used `bool` (mirroring core's `swizzled_input_sf`) rather than the `SfLayout` IntEnum because `SfLayout` reprs as `<SfLayout.x: 0>` and would break the tested `eval(repr(cfg))` round-trip. Finer `SfLayout` (128x4/8x4/linear) selection is still deferred (C42): `SfLayout` has no eval-safe `__repr__`, so exposing it would break the round-trip — unlike `RoutingMethodType`/`ActivationType`, which gained one in the G1 consolidation. Fields are not yet read by any runner — config surface only. |
...
| G4 | [x] | IwakuraRein · `api.py:586` | Should `hidden_states` + `hidden_states_scale` be packed into a dataclass? `per_token_scales` is also missing here. | **Done — superseded by the G5 drop.** The original fix added a `per_token_scale` field to `MoETensors`; the sub-dataclass packing was deferred into the G5 pack-rationale discussion. G5 then removed `MoETensors` (+ `Gemm1Tensors`/`Gemm2Tensors`) entirely, so there is no longer an aspirational grouping to restructure. The shipped path's activation grouping is `MoEActivationPack`, which already carries `hidden_states_q` + `hidden_states_scale` together (the packing the comment asked for); per-token scale belongs to the quant wiring, not this pack. |
| G5 | [x] | IwakuraRein · `api.py:674` | Unclear why `MoEActivationPack` / `MoEWeightPack` exist separately — why not add these methods to `MoETensors`? (Needs a doc/justification; see CR7 "two packs, two lifetimes".) | **Done — dropped `MoETensors` (and its `Gemm1Tensors`/`Gemm2Tensors` limbs) from the shipped surface.** The premise that `MoETensors` is the natural home was the problem: it was aspirational (§2–§4 eager API), consumed by *nothing* — no runner/layer, only its own `validate()` unit tests — yet shipped in `__all__`, which is exactly what made the two-surface question arise. Resolved by removing the cluster so the MVP ships one input model: the two packs. **Rationale (now in `api.py` above the packs):** the useful grouping axis is *lifetime/role*, which is backend-invariant — per-call activations (`MoEActivationPack`, rebuilt each forward) vs long-lived weights (`MoEWeightPack`, materialized once, one native view *per backend*). A single per-call bundle can't model a load-time multi-backend weight cache without conflating lifetimes. We also reject grouping by compute-graph stage (`gemm1`/`gemm2`): that mirrors the *unfused* two-GEMM implementation and overfits it — a fused/megakernel backend has no such boundary, so a graph-shaped public API would leak one backend's internals. The §2–§4 eager-API vision is retained as documented future work, not shipped code. |
