...
| [ ] | Promote per-backend NVFP4 preparation into first-class MVP helpers for CuteDSL and TRTLLM, then remove duplicated TRTLLM preparation logic from tests and benchmarks. The activation scale also needs canonicalizing: the Pack carries one `hidden_states_scale`, and while trtllm-gen happily consumes the linear-layout scale (the runner only has to re-`view` `uint8`→`float8_e4m3fn`), a future backend that needs a swizzled activation scale would force per-backend activation prep. | CR2, CR7 |
