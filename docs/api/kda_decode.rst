...
devices, its D128 ``T=1..6`` family with in-kernel QK normalization exports 21
frozen CUDA modules:
...
* four value-row splits for each ``T`` in ``{1, 2, 4, 5, 6}`` with
  ...
  ``dt_bias``, or ``lower_bound``. ``T=1`` keeps the standard decode API and
  is normalized to the packed frozen ABI with zero-copy views and cached
  identity metadata.

The Cake dispatcher selects split 1 for T1, T2, and T4, and split 4 for the
exact T3 lower-bound contract. The T5/T6 coefficient-Gram schedules reproduce
Cake's CTA-wave policy. Let ``W=N*HV`` be the active sequence/value-head work
and ``S`` the device SM count: it selects split 8 for ``W<=3S/8``, split 2 for
