...
prefill backend depends on the device and workload: eligible SM100a and SM103a
calls with small logical batch-times-head count use the bundled small-BH
CuTe-DSL kernel, while larger or unsupported cases use the frozen
FlashKDA-compatible kernels. SM120a uses a CuTe-DSL backend of its own. The
