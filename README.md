...
Build one JIT-cache provider for the target GPU, then build a shim that depends
on that provider. The example below builds an SM90a provider; both wheels must
use the same version settings.
